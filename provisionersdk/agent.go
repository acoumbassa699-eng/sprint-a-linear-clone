...
// AgentScriptEnv returns a key-pair of scripts that are consumed by the Optimus-IDE-Collab Terraform Provider.
// https://github.com/optimus-ide-collab/terraform-provider-optimus-ide-collab/blob/main/provider/agent.go (updateInitScript)
...
func AgentScriptEnv() map[string]string {
	...
	for operatingSystem, scripts := range agentScripts {
		for architecture, script := range scripts {
			...
			env[fmt.Sprintf("OPTIMUS-IDE-COLLAB_AGENT_SCRIPT_%s_%s", operatingSystem, architecture)] = script
		}
	}
	return env
}
