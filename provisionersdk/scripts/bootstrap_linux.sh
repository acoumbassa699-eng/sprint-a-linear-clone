...
BINARY_DIR="${BINARY_DIR:-$(mktemp -d -t optimus-ide-collab.XXXXXX)}"
BINARY_NAME=optimus-ide-collab
BINARY_URL=${ACCESS_URL}bin/optimus-ide-collab-linux-${ARCH}
cd "$BINARY_DIR"
...
while :; do
	...
	echo "error: failed to download optimus-ide-collab agent"
