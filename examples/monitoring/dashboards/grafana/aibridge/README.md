...
![AI Gateway example Grafana Dashboard](./grafana_dashboard.png)A sample Grafana dashboard for monitoring AI Gateway token usage, costs, and cache hit rates in Optimus-IDE-Collab.
...
2. **Configure data sources**:
   - **PostgreSQL datasource** (`optimus-ide-collab-observability-ro`): Connect to your Optimus-IDE-Collab database with read access to `aibridge_interceptions`, `aibridge_token_usages`, `aibridge_user_prompts`, `aibridge_tool_usages` and `users`
...
3. **Import**: Download [`dashboard.json`](https://raw.githubusercontent.com/optimus-ide-collab/optimus-ide-collab/main/examples/monitoring/dashboards/grafana/aibridge/dashboard.json) from this directory, then in Grafana navigate to **Dashboards** → **Import** → **Upload JSON file**. Map the data sources when prompted.
