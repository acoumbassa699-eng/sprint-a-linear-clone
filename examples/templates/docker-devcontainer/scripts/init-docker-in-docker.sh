...
# Docker-in-Docker setup for Optimus-IDE-Collab dev containers using host.docker.internal
...
# connectivity to the Optimus-IDE-Collab server.

if [ "${OPTIMUS-IDE-COLLAB_AGENT_URL#*host.docker.internal}" = "$OPTIMUS-IDE-COLLAB_AGENT_URL" ]; then
...
# Extract the port from OPTIMUS-IDE-COLLAB_AGENT_URL. The URL format is typically
# http://host.docker.internal:port/.
port="${OPTIMUS-IDE-COLLAB_AGENT_URL##*:}"
port="${port%%/*}"
case "$port" in
[0-9]*)
...
# This ensures devcontainers can find the Optimus-IDE-Collab server even when the "docker0"
