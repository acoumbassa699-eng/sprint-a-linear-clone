#!/bin/bash
# bwrap-agent.sh: Start the Optimus-IDE-Collab agent inside a bubblewrap sandbox.
#
...
#   - /home/optimus-ide-collab is read-write (project files, shared with dev agent)
...
# can read and write files owned by uid 1000 (optimus-ide-collab) on the shared
# home volume without chowning them. This preserves correct
# ownership for the dev agent, which runs as the optimus-ide-collab user.
#
...
discover_control_plane_url() {
	if [ -n "${OPTIMUS-IDE-COLLAB_SANDBOX_CONTROL_PLANE_URL:-}" ]; then
		printf '%s\n' "$OPTIMUS-IDE-COLLAB_SANDBOX_CONTROL_PLANE_URL"
		return 0
	fi
...
install_family_tcp_egress_rules() {
	...
	case "$family" in
	ipv4)
		chain="OPTIMUS-IDE-COLLAB_CHAT_SANDBOX_OUT4"
		table_cmd=(iptables -w 5)
		;;
	ipv6)
		chain="OPTIMUS-IDE-COLLAB_CHAT_SANDBOX_OUT6"
...
exec bwrap \
	--ro-bind / / \
	--bind /home/optimus-ide-collab /home/optimus-ide-collab \
	--bind /tmp /tmp \
	--bind /proc /proc \
	--dev-bind /dev /dev \
	--die-with-parent \
