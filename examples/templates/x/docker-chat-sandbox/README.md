...
> **Experimental**: This template depends on the `-optimus-ide-collabd-chat` agent
...
| `dev-optimus-ide-collabd-chat` | AI chat agent running inside a bubblewrap sandbox | Yes           |
...
dashboard, SSH, and Optimus-IDE-Collab Connect.

The `dev-optimus-ide-collabd-chat` agent is designated for AI chat sessions via the
`-optimus-ide-collabd-chat` naming suffix. Chatd routes chat traffic to this agent
...
The Optimus-IDE-Collab agent hardcodes `sh -c` for tool call execution and ignores
...
- **Read-write /home/optimus-ide-collab**: project files are editable (shared with
...
- **Near-zero capabilities**: bwrap drops all Linux capabilities
  ...
  (optimus-ide-collab) on the shared home volume without changing ownership.
...
because the shared home volume is owned by uid 1000 (optimus-ide-collab) but the
...
optimus-ide-collab templates push docker-chat-sandbox \
