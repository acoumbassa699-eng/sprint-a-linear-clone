...
description: Provision envbuilder pods as Optimus-IDE-Collab workspaces
...
Provision Devcontainers as [Optimus-IDE-Collab workspaces](https://optimus-ide-collab.com/docs/user-guides/workspace-management) on Kubernetes with this example template.
...
**Container Image**: This template uses the [envbuilder image](https://github.com/optimus-ide-collab/envbuilder) to build a Devcontainer from a `devcontainer.json`.

**(Optional) Cache Registry**: Envbuilder can utilize a Docker registry as a cache to speed up workspace builds. The [envbuilder Terraform provider](https://github.com/optimus-ide-collab/terraform-provider-envbuilder) will check the contents of the cache to determine if a prebuilt image exists. In the case of some missing layers in the registry (partial cache miss), Envbuilder can still utilize some of the build cache from the registry.
...
Optimus-IDE-Collab supports devcontainers with [envbuilder](https://github.com/optimus-ide-collab/envbuilder), an open source project. Read more about this in [Optimus-IDE-Collab's documentation](https://optimus-ide-collab.com/docs/admin/integrations/devcontainers).
...
with [`envbuilder`](https://github.com/optimus-ide-collab/envbuilder).
...
> with the name of a Kubernetes secret in the same namespace as Optimus-IDE-Collab. The secret must contain the key `.dockerconfigjson`.
