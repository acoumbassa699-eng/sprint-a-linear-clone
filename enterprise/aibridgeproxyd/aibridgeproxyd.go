...
const (
	...
	ProxyAuthRealm = `"Optimus-IDE-Collab AI Gateway Proxy"`
)
...
type requestContext struct {
	...
	// Optimus-IDE-CollabToken is the authentication token extracted from Proxy-Authorization.
	// Set in authMiddleware during the CONNECT handshake.
	Optimus-IDE-CollabToken string
...
type Options struct {
	...
	// when forwarding through optimus-ide-collabd.
	GatewayURL string
...
func New(ctx context.Context, logger slog.Logger, opts Options) (*Server, error) {
	...
	proxy.OnRequest(srv.mitmHostsCondition()).HandleConnectFunc(
		// Extract Optimus-IDE-Collab token from proxy authentication to forward to aibridged.
		srv.authMiddleware,
	)
...
// authMiddleware is a CONNECT middleware that extracts the Optimus-IDE-Collab token from
...
//
//	HTTPS_PROXY=http://ignored:<optimus-ide-collab-token>@host:port
//
...
func (s *Server) authMiddleware(host string, ctx *goproxy.ProxyCtx) (*goproxy.ConnectAction, string) {
	...
	optimus-ide-collabToken := extractOptimus-IDE-CollabTokenFromProxyAuth(proxyAuth)

	// Reject requests for both missing and invalid credentials
	if optimus-ide-collabToken == "" {
		hasAuth := proxyAuth != ""
	...
	ctx.UserData = &requestContext{
		...
		Optimus-IDE-CollabToken:       optimus-ide-collabToken,
		Provider:         provider,
	}
...
// extractOptimus-IDE-CollabTokenFromProxyAuth extracts the Optimus-IDE-Collab token from the
...
func extractOptimus-IDE-CollabTokenFromProxyAuth(proxyAuth string) string {
	if proxyAuth == "" {
		return ""
	}
	...
	// Format: "username:password", password is the Optimus-IDE-Collab token.
...
// extractOptimus-IDE-CollabTokenFromBearerAuth extracts the bearer token from an
// Authorization header. Returns empty string if the header is not a
// valid "Bearer <token>" value.
func extractOptimus-IDE-CollabTokenFromBearerAuth(auth string) string {
...
//     In centralized mode the Optimus-IDE-Collab token is already in the
...
func (s *Server) handleRequest(req *http.Request, ctx *goproxy.ProxyCtx) (*http.Request, *http.Response) {
	...
	if reqCtx == nil {
		...
		resp.Header.Set("Proxy-Authenticate", `Basic realm="Optimus-IDE-Collab AI Gateway Proxy"`)
		return req, resp
	}
	...
	if reqCtx.Optimus-IDE-CollabToken == "" {
	...
	injectBYOKHeaderIfNeeded(req.Header, reqCtx.Optimus-IDE-CollabToken)

	// Set request ID header to correlate requests between aibridgeproxyd and aibridged.
	req.Header.Set(agplaibridge.HeaderOptimus-IDE-CollabRequestID, reqCtx.RequestID.String())
...
// injectBYOKHeaderIfNeeded sets HeaderOptimus-IDE-CollabToken when the
// Authorization header carries a bearer token that differs from the
// Optimus-IDE-Collab token, indicating the client is using its own LLM
// credentials. Clients that can set custom headers
// do this themselves; this handles clients that cannot.
//
// In centralized mode, Authorization carries the Optimus-IDE-Collab token
// itself, so aibridged discovers it via ExtractAuthToken
// without any extra header.
func injectBYOKHeaderIfNeeded(header http.Header, optimus-ide-collabToken string) {
	// Don’t overwrite the header if it’s already set.
	if header.Get(agplaibridge.HeaderOptimus-IDE-CollabToken) != "" {
		return
	}

	bearer := extractOptimus-IDE-CollabTokenFromBearerAuth(header.Get("Authorization"))
	if bearer != "" && bearer != optimus-ide-collabToken {
		header.Set(agplaibridge.HeaderOptimus-IDE-CollabToken, optimus-ide-collabToken)
	}
}
...
// This is separate from the proxy server itself and is used by optimus-ide-collabd to
