...
import (
	...
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/enterprise/scaletest/agentfake"
...
// optimus-ide-collabsdk-shaped view of whichever rows the test seeded into the DB).
type fakeExternalAgentClient struct {
	me       optimus-ide-collabsdk.User
	...
	// pagination, matching real optimus-ide-collabd behavior. Tests only need to populate
...
func (f *fakeExternalAgentClient) User(_ context.Context, userIdent string) (optimus-ide-collabsdk.User, error) {
	if userIdent == optimus-ide-collabsdk.Me {
		if f.meErr != nil {
			return optimus-ide-collabsdk.User{}, f.meErr
		}
		return f.me, nil
	}
	...
	return optimus-ide-collabsdk.User{}, xerrors.Errorf("no user %q", userIdent)
}

func (f *fakeExternalAgentClient) Template(_ context.Context, id uuid.UUID) (optimus-ide-collabsdk.Template, error) {
	if f.templateErr != nil {
		return optimus-ide-collabsdk.Template{}, f.templateErr
	}
	if id == f.template.ID {
		return f.template, nil
	}
	return optimus-ide-collabsdk.Template{}, xerrors.Errorf("no template with id %s", id)
}

func (f *fakeExternalAgentClient) TemplatesByOrganization(_ context.Context, orgID uuid.UUID) ([]optimus-ide-collabsdk.Template, error) {
	if f.templateErr != nil {
	...
	return []optimus-ide-collabsdk.Template{f.template}, nil
}

func (f *fakeExternalAgentClient) Workspaces(_ context.Context, filter optimus-ide-collabsdk.WorkspaceFilter) (optimus-ide-collabsdk.WorkspacesResponse, error) {
	start := filter.Offset
	...
	return optimus-ide-collabsdk.WorkspacesResponse{
...
// buildExternalAgentWorkspace creates one workspace with a optimus-ide-collab_external_agent
...
// User(optimus-ide-collabsdk.Me) on; its OrganizationIDs is what parseTemplate walks.
func newFakeClient(me database.User, org database.Organization, tpl database.Template) *fakeExternalAgentClient {
	return &fakeExternalAgentClient{
		me: optimus-ide-collabsdk.User{
			ReducedUser:     optimus-ide-collabsdk.ReducedUser{MinimalUser: optimus-ide-collabsdk.MinimalUser{ID: me.ID, Username: me.Username}},
...
func Test_Manager_EnumerateExternalAgents_returnsAllTokens(t *testing.T) {
	...
	optimus-ide-collabURL, _ := url.Parse("http://fake")
	logger := slogtest.Make(t, &slogtest.Options{IgnoreErrors: true}).Leveled(slog.LevelDebug)
	m := agentfake.NewManager(logger, optimus-ide-collabURL, client, db, agentfake.ManagerOptions{Template: tpl.Name})
...
func Test_Manager_ResolveTemplateAndOwner_invalidTokenIsFatal(t *testing.T) {
	...
	client := &fakeExternalAgentClient{
		meErr: optimus-ide-collabsdk.NewError(http.StatusUnauthorized, optimus-ide-collabsdk.Response{Message: "unauthorized"}),
	}
	optimus-ide-collabURL, _ := url.Parse("http://fake")
	logger := slogtest.Make(t, &slogtest.Options{IgnoreErrors: true}).Leveled(slog.LevelDebug)
	m := agentfake.NewManager(logger, optimus-ide-collabURL, client, db, agentfake.ManagerOptions{Template: "tmpl"})
...
func Test_Manager_EnumerateExternalAgents_filtersByOwner(t *testing.T) {
	...
	client.owner = optimus-ide-collabsdk.User{
		ReducedUser: optimus-ide-collabsdk.ReducedUser{MinimalUser: optimus-ide-collabsdk.MinimalUser{
	...
	optimus-ide-collabURL, _ := url.Parse("http://fake")
	logger := slogtest.Make(t, &slogtest.Options{IgnoreErrors: true}).Leveled(slog.LevelDebug)
	m := agentfake.NewManager(logger, optimus-ide-collabURL, client, db, agentfake.ManagerOptions{
		Template: tpl.Name,
