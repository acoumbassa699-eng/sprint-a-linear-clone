...
func TestAgent_ConnectsAndReachesReady(t *testing.T) {
	...
	require.Eventually(t, func() bool {
		for _, state := range dialer.GetLifecycleStates() {
			if state == optimus-ide-collabsdk.WorkspaceAgentLifecycleReady {
				return true
			}
		}
		return false
...
// agent/agenttest.Client (an in-process fake of the agent-side optimus-ide-collabd
// API) rather than a real optimus-ide-collabd, so the only quartz mock involved is
...
// agent reconnects, re-opening the stream so optimus-ide-collabd keeps pushing updates.
