...
// ExternalAgentClient is the subset of *optimus-ide-collabsdk.Client the Manager uses to
...
// GetExternalAgentTokensByTemplateID), not via this client. *optimus-ide-collabsdk.Client
// satisfies this interface, so production callers pass their client
// directly; tests substitute a fake without standing up a real optimus-ide-collabd.
type ExternalAgentClient interface {
	User(ctx context.Context, userIdent string) (optimus-ide-collabsdk.User, error)
	Template(ctx context.Context, id uuid.UUID) (optimus-ide-collabsdk.Template, error)
	TemplatesByOrganization(ctx context.Context, orgID uuid.UUID) ([]optimus-ide-collabsdk.Template, error)
	Workspaces(ctx context.Context, filter optimus-ide-collabsdk.WorkspaceFilter) (optimus-ide-collabsdk.WorkspacesResponse, error)
}
...
// TokenInfo is a single workspace-agent auth token retrieved for a optimus-ide-collab external agent, along with the identifying
...
// ManagerOptions configures a Manager. Authentication is supplied via the *optimus-ide-collabsdk.Client passed to NewManager rather
...
// Manager supervises a set of fake Agents in one process. It enumerates the agents it owns from optimus-ide-collabd at Run time
// (via optimus-ide-collab_external_agent tokens on workspaces matching opts.Template), then opens a dRPC stream per agent and keeps
// them connected until ctx is canceled.
type Manager struct {
	optimus-ide-collabURL *url.URL
...
// same Postgres database as the target optimus-ide-collabd; it is used to bulk-fetch
// external-agent tokens for the enumerated workspaces. optimus-ide-collabURL is the URL
// the spawned fake agents will dial.
func NewManager(logger slog.Logger, optimus-ide-collabURL *url.URL, client ExternalAgentClient, db database.Store, opts ManagerOptions) *Manager {
	if opts.Clock == nil {
		opts.Clock = quartz.NewReal()
	}
	return &Manager{
		optimus-ide-collabURL: optimus-ide-collabURL,
		client:   client,
		db:       db,
		logger:   logger,
		opts:     opts,
...
// Run enumerates the Manager's external agents from optimus-ide-collabd, constructs one Agent per token, and runs the "fake agent"
...
func (m *Manager) Run(ctx context.Context) error {
	...
	for i, ti := range tokens {
		agents = append(agents, NewAgent(
			...
			m.optimus-ide-collabURL, ti.Token,
...
func (m *Manager) EnumerateExternalAgents(ctx context.Context) ([]TokenInfo, error) {
	...
	// authn (the optimus-ide-collab-db-url secret), not a optimus-ide-collab session token.
	// nolint:gocritic
...
func (m *Manager) ResolveTemplateAndOwner(ctx context.Context) error {
	me, err := m.client.User(ctx, optimus-ide-collabsdk.Me)
	if err != nil {
...
func parseTemplate(ctx context.Context, client ExternalAgentClient, organizationIDs []uuid.UUID, template string) (tpl optimus-ide-collabsdk.Template, err error) {
...
func (m *Manager) waitForWorkspaceCount(ctx context.Context) error {
	...
	checkWorkspaceCount := func() (bool, error) {
		page, err := m.client.Workspaces(ctx, optimus-ide-collabsdk.WorkspaceFilter{
...
// IsFatalEnumerationError reports whether err from a optimus-ide-collabd API call indicates an unrecoverable misconfiguration that
