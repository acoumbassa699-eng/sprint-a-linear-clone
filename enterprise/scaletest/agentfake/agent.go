...
// Agent is a single fake agent. It owns one workspace-agent auth token and one dRPC connection to optimus-ide-collabd.
type Agent struct {
	optimus-ide-collabURL *url.URL
	token    string
	logger   slog.Logger
	clock    quartz.Clock
	dialer   rpcDialer // nil → built from optimus-ide-collabURL+token in Run
...
// agentsdk.Client built from optimus-ide-collabURL + token. Tests use this to
...
func NewAgent(logger slog.Logger, optimus-ide-collabURL *url.URL, token string, opts ...Option) *Agent {
	a := &Agent{
...
// Run opens a dRPC websocket to optimus-ide-collabd as the "agent" role and keeps it open until ctx is canceled or Close is called.
// On transient failures (e.g., optimus-ide-collabd restart, brief auth churn while the workspace build is finalizing) Run reconnects
...
func (a *Agent) Run(ctx context.Context) error {
	...
	client := a.dialer
	if client == nil {
		client = agentsdk.New(a.optimus-ide-collabURL, agentsdk.WithFixedToken(a.token))
	}
	a.start = a.clock.Now()
	for {
...
func (a *Agent) connectAndServe(ctx context.Context, client rpcDialer) error {
	...
	// "ready" the moment the dRPC stream is open. We send this once per (re)connect because optimus-ide-collabd's per-connection
...
// connectAndServe can reconnect, re-creating optimus-ide-collabd's send goroutine; ctx
...
func (a *Agent) runMetadata(ctx context.Context, rpc proto.DRPCAgentClient29, workspaceID uuid.UUID, descs []*proto.WorkspaceAgentMetadata_Description) error {
	...
	for i, d := range descs {
		...
		// (see optimus-ide-collabd/agentapi/manifest.go and agent/agent.go). Mirror the
	...
	// optimus-ide-collabd truncates the stored value to 2048 chars (see
	// optimus-ide-collabd/agentapi/metadata.go maxValueLen), and the workspace ID
...
// optimus-ide-collabd pairs the two halves onto a single connection_log row.
