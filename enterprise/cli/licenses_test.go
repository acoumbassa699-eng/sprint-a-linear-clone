...
func TestLicensesAddReal(t *testing.T) {
	...
	t.Run("Fails", func(t *testing.T) {
		...
		var optimus-ide-collabError *optimus-ide-collabsdk.Error
		waiter.RequireAs(&optimus-ide-collabError)
		assert.Equal(t, 400, optimus-ide-collabError.StatusCode())
		assert.Contains(t, "Invalid license", optimus-ide-collabError.Message)
	})
}
...
func TestLicensesListFake(t *testing.T) {
	...
	t.Run("Mainline", func(t *testing.T) {
		...
		require.NoError(t, <-errC)
		var licenses []optimus-ide-collabsdk.License
...
func TestLicensesDeleteReal(t *testing.T) {
	...
	t.Run("Empty", func(t *testing.T) {
		...
		var optimus-ide-collabError *optimus-ide-collabsdk.Error
		clitest.StartWithWaiter(t, inv).RequireAs(&optimus-ide-collabError)
		assert.Equal(t, 404, optimus-ide-collabError.StatusCode())
		assert.Contains(t, "Unknown license ID", optimus-ide-collabError.Message)
	})
}
...
func (*fakeLicenseAPI) entitlements(rw http.ResponseWriter, r *http.Request) {
	features := make(map[optimus-ide-collabsdk.FeatureName]optimus-ide-collabsdk.Feature)
	for _, f := range optimus-ide-collabsdk.FeatureNames {
		features[f] = optimus-ide-collabsdk.Feature{
			Entitlement: optimus-ide-collabsdk.EntitlementEntitled,
			Enabled:     true,
		}
	}
	httpapi.Write(r.Context(), rw, http.StatusOK, optimus-ide-collabsdk.Entitlements{
		Features:   features,
