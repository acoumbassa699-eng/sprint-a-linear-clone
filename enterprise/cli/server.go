...
func (r *RootCmd) Server(_ func()) *serpent.Command {
	cmd := r.RootCmd.Server(func(ctx context.Context, options *agploptimus-ide-collabd.Options) (*agploptimus-ide-collabd.API, io.Closer, error) {
		...
		options.TrialGenerator = trialer.New(options.Database, "https://v2-licensor.optimus-ide-collab.com/trial", optimus-ide-collabd.Keys)

		o := &optimus-ide-collabd.Options{
			Options:                   options,
			AuditLogging:              true,
		...
		if o.LicenseKeys == nil {
			o.LicenseKeys = optimus-ide-collabd.Keys
		}

		closers := &multiCloser{}

		// Create the enterprise API.
		api, err := optimus-ide-collabd.New(ctx, o)
		if err != nil {
			return nil, nil, err
		}
		closers.Add(api)
		...
		if options.DeploymentValues.AI.BridgeProxyConfig.Enabled.Value() {
			...
			// optimus-ide-collabd.New above), so env-seeded keys are also written
			...
			if err := agploptimus-ide-collabd.SeedAIProvidersFromEnv(
				aibridgeInitCtx,
				options.Database,
