...
func (r *RootCmd) workspaceProxy() *serpent.Command {
	cmd := &serpent.Command{
		...
		Long: "Workspace proxies provide low-latency experiences for geo-distributed teams. " +
			...
			"Best used if Optimus-IDE-Collab and your workspace are deployed in different regions.",
...
func (r *RootCmd) regenerateProxyToken() *serpent.Command {
	...
	cmd := &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			updated, err := client.PatchWorkspaceProxy(ctx, optimus-ide-collabsdk.PatchWorkspaceProxy{
				ID:              proxy.ID,
				Name:            proxy.Name,
	...
	var (
		...
		formatter   = cliui.NewOutputFormatter(
			...
			cliui.ChangeFormatterData(cliui.TextFormat(), func(data any) (any, error) {
				response, ok := data.(optimus-ide-collabsdk.WorkspaceProxy)
				if !ok {
			...
			cliui.ChangeFormatterData(cliui.TableFormat([]optimus-ide-collabsdk.WorkspaceProxy{}, []string{"name", "url"}),
				func(data any) (any, error) {
					response, ok := data.(optimus-ide-collabsdk.WorkspaceProxy)
					if !ok {
						return nil, xerrors.Errorf("unexpected type %T", data)
					}
					return []optimus-ide-collabsdk.WorkspaceProxy{response}, nil
				}),
		)
	)
	cmd := &serpent.Command{
...
func (r *RootCmd) createProxy() *serpent.Command {
	...
	cmd := &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			resp, err := client.CreateWorkspaceProxy(ctx, optimus-ide-collabsdk.CreateWorkspaceProxyRequest{
				Name:        proxyName,
				DisplayName: displayName,
				Icon:        proxyIcon,
			})
...
func (r *RootCmd) listProxies() *serpent.Command {
	formatter := cliui.NewOutputFormatter(
		cliui.TableFormat([]optimus-ide-collabsdk.WorkspaceProxy{}, []string{"name", "url", "proxy status"}),
		cliui.JSONFormat(),
		cliui.ChangeFormatterData(cliui.TextFormat(), func(data any) (any, error) {
			resp, ok := data.([]optimus-ide-collabsdk.WorkspaceProxy)
			if !ok {
				return nil, xerrors.Errorf("unexpected type %T", data)
			}
...
func (f *updateProxyResponseFormatter) Format(ctx context.Context, data optimus-ide-collabsdk.UpdateWorkspaceProxyResponse) (string, error) {
	if f.onlyToken {
...
func newUpdateProxyResponseFormatter() *updateProxyResponseFormatter {
	...
	up.formatter = cliui.NewOutputFormatter(
		// Text formatter should be human readable.
		cliui.ChangeFormatterData(cliui.TextFormat(), func(data any) (any, error) {
			response, ok := data.(optimus-ide-collabsdk.UpdateWorkspaceProxyResponse)
			if !ok {
				return nil, xerrors.Errorf("unexpected type %T", data)
			}
			...
			return fmt.Sprintf("Workspace Proxy %[1]q updated successfully.\n"+
				...
				cliui.Code("OPTIMUS-IDE-COLLAB_PROXY_SESSION_TOKEN=%[2]s optimus-ide-collab wsproxy server --primary-access-url %[3]s --http-address=0.0.0.0:3001")+
		...
		cliui.JSONFormat(),
		// Table formatter expects a slice, make a slice of one.
		cliui.ChangeFormatterData(cliui.TableFormat([]optimus-ide-collabsdk.UpdateWorkspaceProxyResponse{}, []string{"name", "url", "proxy token"}),
			func(data any) (any, error) {
				response, ok := data.(optimus-ide-collabsdk.UpdateWorkspaceProxyResponse)
				if !ok {
					return nil, xerrors.Errorf("unexpected type %T", data)
				}
				return []optimus-ide-collabsdk.UpdateWorkspaceProxyResponse{response}, nil
			}),
	)
