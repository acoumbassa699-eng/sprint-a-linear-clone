...
func (r *RootCmd) aiGatewayKeysCreate() *serpent.Command {
	return &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			res, err := client.CreateAIGatewayKey(inv.Context(), optimus-ide-collabsdk.CreateAIGatewayKeyRequest{
				Name: inv.Args[0],
			})
			if err != nil {
...
func (r *RootCmd) aiGatewayKeysList() *serpent.Command {
	formatter := cliui.NewOutputFormatter(
		cliui.TableFormat([]optimus-ide-collabsdk.AIGatewayKey{}, []string{"id", "name", "key prefix", "last heartbeat at", "created at"}),
		cliui.JSONFormat(),
	)
...
func aiGatewayKeyByNameOrID(ctx context.Context, client *optimus-ide-collabsdk.Client, nameOrID string) (optimus-ide-collabsdk.AIGatewayKey, error) {
	keys, err := client.ListAIGatewayKeys(ctx)
	if err != nil {
		return optimus-ide-collabsdk.AIGatewayKey{}, xerrors.Errorf("list AI Gateway keys: %w", err)
	}
	...
	return optimus-ide-collabsdk.AIGatewayKey{}, xerrors.Errorf("AI Gateway key %q not found", nameOrID)
}
