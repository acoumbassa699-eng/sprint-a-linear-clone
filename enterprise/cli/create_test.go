...
func TestEnterpriseCreate(t *testing.T) {
	...
	type setupData struct {
		firstResponse optimus-ide-collabsdk.CreateFirstUserResponse
		second        optimus-ide-collabsdk.Organization
		owner         *optimus-ide-collabsdk.Client
	...
	setupMultipleOrganizations := func(t *testing.T, args setupArgs) setupData {
		...
		member, _ := optimus-ide-collabdtest.CreateAnotherUser(t, ownerClient, first.OrganizationID, rbac.ScopedRoleOrgMember(second.ID))

		var wg sync.WaitGroup

		createTemplate := func(tplName string, orgID uuid.UUID) {
			version := optimus-ide-collabdtest.CreateTemplateVersion(t, ownerClient, orgID, nil)
			wg.Go(func() {
				optimus-ide-collabdtest.AwaitTemplateVersionJobCompleted(t, ownerClient, version.ID)
			})

			optimus-ide-collabdtest.CreateTemplate(t, ownerClient, orgID, version.ID, func(request *optimus-ide-collabsdk.CreateTemplateRequest) {
	...
	t.Run("OwnerCanCreateInNonMemberOrg", func(t *testing.T) {
		...
		newOwner, _ := optimus-ide-collabdtest.CreateAnotherUser(t, setup.owner, setup.firstResponse.OrganizationID, rbac.RoleOwner())
		...
		ws, err := newOwner.WorkspaceByOwnerAndName(context.Background(), optimus-ide-collabsdk.Me, "owner-workspace", optimus-ide-collabsdk.WorkspaceOptions{})
		if assert.NoError(t, err, "expected workspace to be created") {
			assert.Equal(t, ws.TemplateName, templateName)
			assert.Equal(t, ws.OrganizationName, setup.second.Name, "workspace in second organization")
		}
	...
	t.Run("CreateIncorrectOrg", func(t *testing.T) {
		...
		require.ErrorContains(t, err, fmt.Sprintf("--org=%q", "optimus-ide-collab"))
	})
}
...
func TestEnterpriseCreateWithPreset(t *testing.T) {
	...
	t.Run("PresetNoneDoesNotClaimPrebuiltWorkspace", func(t *testing.T) {
		...
		member, _ := optimus-ide-collabdtest.CreateAnotherUser(t, client, owner.OrganizationID)
		version := optimus-ide-collabdtest.CreateTemplateVersion(t, client, owner.OrganizationID, echoResponses(&presetWithPrebuild))
		optimus-ide-collabdtest.AwaitTemplateVersionJobCompleted(t, client, version.ID)
		template := optimus-ide-collabdtest.CreateTemplate(t, client, owner.OrganizationID, version.ID)
		presets, err := client.TemplateVersionPresets(ctx, version.ID)
		require.NoError(t, err)
		...
		require.NoError(t, err)
		require.Len(t, buildParameters, 2)
		require.Contains(t, buildParameters, optimus-ide-collabsdk.WorkspaceBuildParameter{Name: firstParameterName, Value: firstParameterValue})
		require.Contains(t, buildParameters, optimus-ide-collabsdk.WorkspaceBuildParameter{Name: thirdParameterName, Value: thirdParameterValue})
	})
}
