...
func (r *RootCmd) scaletestAgentFake() *serpent.Command {
	...
	cmd := &serpent.Command{
		...
		Long: agplcli.FormatExamples(
			agplcli.Example{
				Description: "Connect a fake agent for every external-agent workspace built from the template named " +
					"\"agentfake-runner\".",
				Command: "optimus-ide-collab exp scaletest agentfake --template agentfake-runner",
			},
		) + "\n\n" +
			...
			"encoded into the URL) that points at the same database instance optimus-ide-collabd is using. Intended " +
			"to run inside the same network as optimus-ide-collabd, not from operator machines outside the cluster. " +
			"The workspace listing and external-agent feature are gated server-side. Pair with " +
			"`optimus-ide-collab exp scaletest create-workspaces --no-wait-for-agents` to seed the workspaces this " +
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			if postgresURL == "" {
				return xerrors.New("--postgres-url (OPTIMUS-IDE-COLLAB_PG_CONNECTION_URL) is required")
			}
			...
			sqlDriver := "postgres"
			if optimus-ide-collabsdk.PostgresAuth(postgresAuth) == optimus-ide-collabsdk.PostgresAuthAWSIAMRDS {
				var err error
	...
	cmd.Options = serpent.OptionSet{
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_AGENTFAKE_PROMETHEUS_ADDRESS",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_AGENTFAKE_EXPECTED_AGENTS",
			Default:     "0",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_AGENTFAKE_EXPECTED_AGENTS_TOLERANCE",
			Default:     "0",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_AGENTFAKE_CONNECTION_REPORT_INTERVAL",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_AGENTFAKE_CONNECTION_REPORT_DURATION",
		...
		},
		{
			...
			Description: "URL of the Postgres database that the target optimus-ide-collabd is using. Required; used to bulk-fetch external-agent tokens for the enumerated workspaces in a single query. The same connection string the optimus-ide-collab server pods consume (e.g. the optimus-ide-collab-db-url secret in scaletest deployments).",
		...
		serpent.Option{
			...
			Env:         "OPTIMUS-IDE-COLLAB_PG_CONNECTION_AUTH",
			Default:     "password",
			Value:       serpent.EnumOf(&postgresAuth, optimus-ide-collabsdk.PostgresAuthDrivers...),
		},
	}
