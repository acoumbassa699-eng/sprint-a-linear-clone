...
func (r *RootCmd) provisionerKeysCreate() *serpent.Command {
	...
	cmd := &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			res, err := client.CreateProvisionerKey(ctx, org.ID, optimus-ide-collabsdk.CreateProvisionerKeyRequest{
				Name: inv.Args[0],
				Tags: tags,
			})
			if err != nil {
...
func (r *RootCmd) provisionerKeysList() *serpent.Command {
	var (
		...
		formatter  = cliui.NewOutputFormatter(
			cliui.TableFormat([]optimus-ide-collabsdk.ProvisionerKey{}, []string{"created at", "name", "tags"}),
			cliui.JSONFormat(),
		)
	)
