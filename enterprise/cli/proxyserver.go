...
func (r *RootCmd) proxyServer() *serpent.Command {
	var (
		cfg = new(optimus-ide-collabsdk.DeploymentValues)
		// Filter options for only relevant ones.
		opts = cfg.Options().Filter(optimus-ide-collabsdk.IsWorkspaceProxies)
	...
	opts.Add(
		...
		serpent.Option{
			...
			Description: "Authentication token for the workspace proxy to communicate with optimus-ide-collabd.",
			Flag:        "proxy-session-token",
			Env:         "OPTIMUS-IDE-COLLAB_PROXY_SESSION_TOKEN",
		...
		serpent.Option{
			Name:        "Optimus-IDE-Collabd (Primary) Access URL",
			Description: "URL to communicate with optimus-ide-collabd. This should match the access URL of the Optimus-IDE-Collab deployment.",
			Flag:        "primary-access-url",
			Env:         "OPTIMUS-IDE-COLLAB_PRIMARY_ACCESS_URL",
		...
		serpent.Option{
			...
			Flag:        "derp-only",
			Env:         "OPTIMUS-IDE-COLLAB_PROXY_DERP_ONLY",
			YAML:        "derpOnly",
			Required:    false,
			Value:       &derpOnly,
			Group:       &externalProxyOptionGroup,
	...
	cmd := &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			cli.PrintLogo(inv, "Optimus-IDE-Collab Workspace Proxy")
			...
			options := &wsproxy.Options{
				Logger:                 logger,
				Experiments:            optimus-ide-collabd.ReadExperiments(logger, cfg.Experiments.Value()),
