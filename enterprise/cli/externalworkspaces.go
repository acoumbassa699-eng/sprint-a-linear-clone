...
// externalWorkspaceCreate extends `optimus-ide-collab create` to create an external workspace.
func (r *RootCmd) externalWorkspaceCreate() *serpent.Command {
	opts := agpl.CreateOptions{
		BeforeCreate: func(ctx context.Context, client *optimus-ide-collabsdk.Client, _ optimus-ide-collabsdk.Template, templateVersionID uuid.UUID) error {
		...
		AfterCreate: func(ctx context.Context, inv *serpent.Invocation, client *optimus-ide-collabsdk.Client, workspace optimus-ide-collabsdk.Workspace) error {
			workspace, err := client.WorkspaceByOwnerAndName(ctx, optimus-ide-collabsdk.Me, workspace.Name, optimus-ide-collabsdk.WorkspaceOptions{})
			if err != nil {
	...
	cmd := &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			if out == "" {
				...
				_, _ = fmt.Fprintln(inv.Stderr, "  "+pretty.Sprint(cliui.DefaultStyles.Code, "optimus-ide-collab external-workspaces create <name>"))
				_, _ = fmt.Fprintln(inv.Stderr)
				return nil
			}
...
func fetchExternalAgents(inv *serpent.Invocation, client *optimus-ide-collabsdk.Client, workspace optimus-ide-collabsdk.Workspace, resources []optimus-ide-collabsdk.WorkspaceResource) ([]externalAgent, error) {
	if len(resources) == 0 {
