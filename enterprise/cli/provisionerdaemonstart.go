...
func (r *RootCmd) provisionerDaemonStart() *serpent.Command {
			...
			go func() {
				...
				err := terraform.Serve(ctx, &terraform.ServeOptions{
					ServeOptions: &provisionersdk.ServeOptions{
						...
						Experiments:   optimus-ide-collabd.ReadExperiments(logger, experiments),
					},
					CachePath: cacheDir,
				})
			...
			srv := provisionerd.New(func(ctx context.Context) (provisionerdproto.DRPCProvisionerDaemonClient, error) {
				return client.ServeProvisionerDaemon(ctx, optimus-ide-collabsdk.ServeProvisionerDaemonRequest{
					...
					Provisioners: []optimus-ide-collabsdk.ProvisionerType{
						optimus-ide-collabsdk.ProvisionerTypeTerraform,
					},
					Tags:           tags,
	...
	keyOption := serpent.Option{
		Flag:        "key",
		Env:         "OPTIMUS-IDE-COLLAB_PROVISIONER_DAEMON_KEY",
		Description: "Provisioner key to authenticate with Optimus-IDE-Collab server.",
	...
	cmd.Options = serpent.OptionSet{
		{
			...
			Default:       optimus-ide-collabsdk.DefaultCacheDir(),
			Value:         serpent.StringOf(&cacheDir),
		},
		{
			Flag:          "tag",
			FlagShorthand: "t",
			Env:           "OPTIMUS-IDE-COLLAB_PROVISIONERD_TAGS",
			Description:   "Tags to filter provisioner jobs by.",
			Value:         serpent.StringArrayOf(&rawTags),
		},
		{
			Flag:        "poll-interval",
			Env:         "OPTIMUS-IDE-COLLAB_PROVISIONERD_POLL_INTERVAL",
		...
		{
			Flag:        "poll-jitter",
			Env:         "OPTIMUS-IDE-COLLAB_PROVISIONERD_POLL_JITTER",
			Description: "Deprecated and ignored.",
		...
		},
		{
			...
			Description: "Pre-shared key to authenticate with Optimus-IDE-Collab server.",
		...
		{
			Flag:        "name",
			Env:         "OPTIMUS-IDE-COLLAB_PROVISIONER_DAEMON_NAME",
		...
		{
			Flag:        "verbose",
			Env:         "OPTIMUS-IDE-COLLAB_PROVISIONER_DAEMON_VERBOSE",
		...
		{
			Flag:        "log-human",
			Env:         "OPTIMUS-IDE-COLLAB_PROVISIONER_DAEMON_LOGGING_HUMAN",
		...
		{
			Flag:        "log-json",
			Env:         "OPTIMUS-IDE-COLLAB_PROVISIONER_DAEMON_LOGGING_JSON",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_PROVISIONER_DAEMON_LOGGING_STACKDRIVER",
		...
		{
			Flag:        "log-filter",
			Env:         "OPTIMUS-IDE-COLLAB_PROVISIONER_DAEMON_LOG_FILTER",
