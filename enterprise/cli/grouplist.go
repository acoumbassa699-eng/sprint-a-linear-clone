	...
	cmd := &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			if out == "" {
				...
				_, _ = fmt.Fprintln(inv.Stderr, color.HiMagentaString("  $ optimus-ide-collab groups create <name>\n"))
				return nil
			}
...
type groupTableRow struct {
	// For json output:
	optimus-ide-collabsdk.Group `table:"-"`
...
func groupsToRows(groups ...optimus-ide-collabsdk.Group) []groupTableRow {
