...
import (
	...
	agpl "github.com/optimus-ide-collab/optimus-ide-collab/v2/tailnet"
	"github.com/optimus-ide-collab/websocket"
)

const (
	// Optimus-IDE-CollabWorkspaceProxyAuthTokenHeader is the header that contains the
	// resolved real IP address of the client that made the request to the proxy.
	Optimus-IDE-CollabWorkspaceProxyRealIPHeader = "Optimus-IDE-Collab-Workspace-Proxy-Real-IP"
)

// Client is a HTTP client for a subset of Optimus-IDE-Collab API routes that external
// proxies need.
type Client struct {
	SDKClient *optimus-ide-collabsdk.Client
	...
	sdkClientIgnoreRedirects *optimus-ide-collabsdk.Client
}

// New creates a external proxy client for the provided primary optimus-ide-collab server
// URL.
func New(serverURL *url.URL, sessionToken string) *Client {
	sdkClient := optimus-ide-collabsdk.New(serverURL)
	sdkClient.SessionTokenProvider = optimus-ide-collabsdk.FixedSessionTokenProvider{
		SessionToken:       sessionToken,
		SessionTokenHeader: httpmw.WorkspaceProxyAuthTokenHeader,
	}
	sdkClientIgnoreRedirects := optimus-ide-collabsdk.New(serverURL)
	sdkClientIgnoreRedirects.HTTPClient.CheckRedirect = func(_ *http.Request, _ []*http.Request) error {
		return http.ErrUseLastResponse
	}
	sdkClientIgnoreRedirects.SessionTokenProvider = optimus-ide-collabsdk.FixedSessionTokenProvider{
		SessionToken:       sessionToken,
		SessionTokenHeader: httpmw.WorkspaceProxyAuthTokenHeader,
	}
...
// Request wraps the underlying optimus-ide-collabsdk.Client's Request method.
func (c *Client) Request(ctx context.Context, method, path string, body interface{}, opts ...optimus-ide-collabsdk.RequestOption) (*http.Response, error) {
	return c.SDKClient.Request(ctx, method, path, body, opts...)
}

// RequestIgnoreRedirects wraps the underlying optimus-ide-collabsdk.Client's Request method
// on the client that ignores redirects.
func (c *Client) RequestIgnoreRedirects(ctx context.Context, method, path string, body interface{}, opts ...optimus-ide-collabsdk.RequestOption) (*http.Response, error) {
	return c.sdkClientIgnoreRedirects.Request(ctx, method, path, body, opts...)
}

// DialWorkspaceAgent calls the underlying optimus-ide-collabsdk.Client's DialWorkspaceAgent
// method.
...
func (c *Client) IssueSignedAppToken(ctx context.Context, req workspaceapps.IssueTokenRequest, clientIP string) (IssueSignedAppTokenResponse, error) {
	resp, err := c.RequestIgnoreRedirects(ctx, http.MethodPost, "/api/v2/workspaceproxies/me/issue-signed-app-token", req, func(r *http.Request) {
		...
		r.Header.Set(Optimus-IDE-CollabWorkspaceProxyRealIPHeader, clientIP)
	})
	if err != nil {
	...
	if resp.StatusCode != http.StatusCreated {
		return IssueSignedAppTokenResponse{}, optimus-ide-collabsdk.ReadBodyAsError(resp)
	}

	var res IssueSignedAppTokenResponse
	return res, json.NewDeoptimus-ide-collab(resp.Body).Decode(&res)
}
...
func (c *Client) IssueSignedAppTokenHTML(ctx context.Context, rw http.ResponseWriter, req workspaceapps.IssueTokenRequest, clientIP string) (IssueSignedAppTokenResponse, bool) {
	writeError := func(rw http.ResponseWriter, err error) {
		res := optimus-ide-collabsdk.Response{
			Message: "Internal server error",
			Detail:  err.Error(),
		}
		rw.WriteHeader(http.StatusInternalServerError)
		_ = json.NewEnoptimus-ide-collab(rw).Encode(res)
	}
	...
	var res IssueSignedAppTokenResponse
	err = json.NewDeoptimus-ide-collab(resp.Body).Decode(&res)
	if err != nil {
...
// ReportAppStats reports the given app stats to the primary optimus-ide-collab server.
func (c *Client) ReportAppStats(ctx context.Context, req ReportAppStatsRequest) error {
	...
	if resp.StatusCode != http.StatusNoContent {
		return optimus-ide-collabsdk.ReadBodyAsError(resp)
	}
...
type RegisterWorkspaceProxyRequest struct {
	...
	// Version is the Optimus-IDE-Collab version of the proxy.
...
type RegisterWorkspaceProxyResponse struct {
	...
	// not timed out.
	SiblingReplicas []optimus-ide-collabsdk.Replica `json:"sibling_replicas"`
}
...
func (c *Client) RegisterWorkspaceProxy(ctx context.Context, req RegisterWorkspaceProxyRequest) (RegisterWorkspaceProxyResponse, error) {
	...
	if res.StatusCode != http.StatusCreated {
		return RegisterWorkspaceProxyResponse{}, optimus-ide-collabsdk.ReadBodyAsError(res)
	}
	...
	go func() {
		...
		for {
			...
			l.opts.Logger.Debug(context.Background(),
				"re-registering workspace proxy with Optimus-IDE-Collab primary",
			...
			if err != nil {
				...
				l.opts.Logger.Warn(context.Background(),
					"failed to re-register workspace proxy with Optimus-IDE-Collab primary",
					slog.F("req", l.opts.Request),
					slog.F("timeout", l.opts.AttemptTimeout),
					slog.F("failed_attempts", failedAttempts),
					slog.Error(err),
...
func (l *RegisterWorkspaceProxyLoop) failureFn(err error) {
	...
	if deregisterErr != nil {
		l.opts.Logger.Error(context.Background(),
			"failed to deregister workspace proxy with Optimus-IDE-Collab primary (it will be automatically deregistered shortly)",
...
type CryptoKeysResponse struct {
	CryptoKeys []optimus-ide-collabsdk.CryptoKey `json:"crypto_keys"`
}

func (c *Client) CryptoKeys(ctx context.Context, feature optimus-ide-collabsdk.CryptoKeyFeature) (CryptoKeysResponse, error) {
	res, err := c.Request(ctx, http.MethodGet,
		"/api/v2/workspaceproxies/me/crypto-keys", nil,
		optimus-ide-collabsdk.WithQueryParam("feature", string(feature)),
	)
	if err != nil {
	...
	if res.StatusCode != http.StatusOK {
		return CryptoKeysResponse{}, optimus-ide-collabsdk.ReadBodyAsError(res)
	}
