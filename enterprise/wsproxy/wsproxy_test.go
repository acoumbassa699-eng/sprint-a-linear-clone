...
func TestDERPOnly(t *testing.T) {
	...
	client, closer, api, _ := optimus-ide-collabdenttest.NewWithAPI(t, &optimus-ide-collabdenttest.Options{
		Options: &optimus-ide-collabdtest.Options{
			...
			AppHostname:              "*.primary.test.optimus-ide-collab.com",
...
func TestDERP(t *testing.T) {
	...
	proxyAPI1 := optimus-ide-collabdenttest.NewWorkspaceProxyReplica(t, api, client, &optimus-ide-collabdenttest.ProxyOptions{
		Name: "best-proxy",
	})
	proxyAPI2 := optimus-ide-collabdenttest.NewWorkspaceProxyReplica(t, api, client, &optimus-ide-collabdenttest.ProxyOptions{
		Name: "worst-proxy",
	})

	// Create a running external proxy with DERP disabled.
	proxyAPI3 := optimus-ide-collabdenttest.NewWorkspaceProxyReplica(t, api, client, &optimus-ide-collabdenttest.ProxyOptions{
		Name:         "no-derp-proxy",
		DerpDisabled: true,
	})
...
resourceLoop:
	...
	t.Run("ReturnedInDERPMap", func(t *testing.T) {
		...
		// The primary region:
		require.Equal(t, "Optimus-IDE-Collab Embedded Relay", primaryRegion.RegionName)
		require.Equal(t, "optimus-ide-collab", primaryRegion.RegionCode)
		...
		require.Equal(t, "optimus-ide-collab_best-proxy", proxy1Region.RegionCode)
		...
		require.Equal(t, "optimus-ide-collab_worst-proxy", proxy2Region.RegionCode)
...
func TestDERPEndToEnd(t *testing.T) {
	...
	optimus-ide-collabdenttest.NewWorkspaceProxyReplica(t, api, client, &optimus-ide-collabdenttest.ProxyOptions{
		Name: "best-proxy",
	})
...
func TestDERPMesh(t *testing.T) {
	...
	proxyURL, err := url.Parse("https://proxy.test.optimus-ide-collab.com")
	require.NoError(t, err)
...
func TestWorkspaceProxyDERPMeshProbe(t *testing.T) {
	t.Parallel()
	createProxyRegion := func(ctx context.Context, t *testing.T, client *optimus-ide-collabsdk.Client, name string) optimus-ide-collabsdk.UpdateWorkspaceProxyResponse {
		t.Helper()
	...
	t.Run("ProbeOK", func(t *testing.T) {
		...
		registerBrokenProxy(ctx, t, api.AccessURL, "https://fake-proxy.test.optimus-ide-collab.com", fakeProxyRes.ProxyToken)

		proxyURL, err := url.Parse("https://proxy1.test.optimus-ide-collab.com")
		require.NoError(t, err)
		...
		for _, proxy := range proxies {
			...
			var respJSON optimus-ide-collabsdk.ProxyHealthReport
			err = json.NewDeoptimus-ide-collab(resp.Body).Decode(&respJSON)
			resp.Body.Close()
			require.NoError(t, err)
	...
	t.Run("ProbeFail", func(t *testing.T) {
		...
		proxyURL, err := url.Parse("https://proxy2.test.optimus-ide-collab.com")
		require.NoError(t, err)

		// Create 1 real proxy replica.
		const fakeCount = 5
		replicaPingErr := make(chan string, 4)
		proxy := optimus-ide-collabdenttest.NewWorkspaceProxyReplica(t, api, client, &optimus-ide-collabdenttest.ProxyOptions{
			Name:     "proxy-2",
			ProxyURL: proxyURL,
...
func TestWorkspaceProxyWorkspaceApps(t *testing.T) {
	...
	apptest.Run(t, false, func(t *testing.T, opts *apptest.DeploymentOptions) *apptest.Deployment {
		...
		if opts.PrimaryAppHost == "" {
			opts.PrimaryAppHost = "*.primary.test.optimus-ide-collab.com"
		}
		...
		proxyAPI := optimus-ide-collabdenttest.NewWorkspaceProxyReplica(t, api, client, &optimus-ide-collabdenttest.ProxyOptions{
			Name:            "best-proxy",
...
func createProxyReplicas(ctx context.Context, t *testing.T, opts *createProxyReplicasOptions) []optimus-ide-collabdenttest.WorkspaceProxy {
	t.Helper()

	var (
		proxies = make([]optimus-ide-collabdenttest.WorkspaceProxy, opts.Count)
	...
	for i := range proxies {
		proxies[i] = optimus-ide-collabdenttest.NewWorkspaceProxyReplica(t, opts.API, opts.Client, &optimus-ide-collabdenttest.ProxyOptions{
			Name:     opts.Name,
			Token:    opts.ProxyToken,
			ProxyURL: opts.ProxyURL,
			ReplicaPingCallback: func(siblings []optimus-ide-collabsdk.Replica, err string) {
...
func TestWorkspaceProxyDERPMetrics(t *testing.T) {
	...
	assert.Contains(t, names, "optimus-ide-collab_derp_server_connections",
		"expected optimus-ide-collab_derp_server_connections to be registered")
	assert.Contains(t, names, "optimus-ide-collab_derp_server_bytes_received_total",
		"expected optimus-ide-collab_derp_server_bytes_received_total to be registered")
	assert.Contains(t, names, "optimus-ide-collab_derp_server_packets_dropped_reason_total",
		"expected optimus-ide-collab_derp_server_packets_dropped_reason_total to be registered")
}
