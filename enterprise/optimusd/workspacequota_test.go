...
func verifyQuota(ctx context.Context, t *testing.T, client *optimus-ide-collabsdk.Client, organizationID string, consumed, total int) {
	verifyQuotaUser(ctx, t, client, organizationID, optimus-ide-collabsdk.Me, consumed, total)
}

func verifyQuotaUser(ctx context.Context, t *testing.T, client *optimus-ide-collabsdk.Client, organizationID string, user string, consumed, total int) {
	t.Helper()

	got, err := client.WorkspaceQuota(ctx, organizationID, user)
	require.NoError(t, err)
	require.EqualValues(t, optimus-ide-collabsdk.WorkspaceQuota{
		Budget:          total,
...
func TestWorkspaceQuota(t *testing.T) {
	...
	t.Run("CreateDelete", func(t *testing.T) {
		...
		optimus-ide-collabdtest.NewProvisionerDaemon(t, api.AGPL)
		...
		_, err := client.PatchGroup(ctx, user.OrganizationID, optimus-ide-collabsdk.PatchGroupRequest{
		...
		group1, err := client.CreateGroup(ctx, user.OrganizationID, optimus-ide-collabsdk.CreateGroupRequest{
			Name:           "test-1",
			QuotaAllowance: 1,
		})
		require.NoError(t, err)

		group2, err := client.CreateGroup(ctx, user.OrganizationID, optimus-ide-collabsdk.CreateGroupRequest{
			Name:           "test-2",
			QuotaAllowance: 2,
		})
		require.NoError(t, err)

		_, err = client.PatchGroup(ctx, group1.ID, optimus-ide-collabsdk.PatchGroupRequest{
			AddUsers: []string{user.UserID.String()},
		})
		require.NoError(t, err)

		_, err = client.PatchGroup(ctx, group2.ID, optimus-ide-collabsdk.PatchGroupRequest{
			AddUsers: []string{user.UserID.String()},
		})
		require.NoError(t, err)
		...
		for i := 0; i < 4; i++ {
			wg.Go(func() {
				...
				assert.Equal(t, optimus-ide-collabsdk.WorkspaceStatusRunning, build.Status)
			})
		}
		wg.Wait()
		...
		for _, w := range workspaces.Workspaces {
			if w.LatestBuild.Status != optimus-ide-collabsdk.WorkspaceStatusRunning {
				continue
			}
			build, err := client.CreateWorkspaceBuild(ctx, w.ID, optimus-ide-collabsdk.CreateWorkspaceBuildRequest{
	...
	t.Run("StartStop", func(t *testing.T) {
		...
		build = optimus-ide-collabdtest.CreateWorkspaceBuild(t, client, workspaces[0], database.WorkspaceTransitionStop)
		build = optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, client, build.ID)

		// Quota goes down one
		verifyQuota(ctx, t, client, user.OrganizationID.String(), 3, 4)
		require.Equal(t, optimus-ide-collabsdk.WorkspaceStatusStopped, build.Status)

		build = optimus-ide-collabdtest.CreateWorkspaceBuild(t, client, workspaces[0], database.WorkspaceTransitionStart)
		build = optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, client, build.ID)
	...
	t.Run("AllowanceEveryone", func(t *testing.T) {
		...
		second := optimus-ide-collabdenttest.CreateOrganization(t, owner, optimus-ide-collabdenttest.CreateOrganizationOptions{})
		...
		_, err := owner.PatchGroup(ctx, first.OrganizationID, optimus-ide-collabsdk.PatchGroupRequest{
			QuotaAllowance: ptr.Ref(30),
		})
		require.NoError(t, err)

		_, err = owner.PatchGroup(ctx, second.ID, optimus-ide-collabsdk.PatchGroupRequest{
	...
	t.Run("ManyWorkspaces", func(t *testing.T) {
		...
		owner, db, first := optimus-ide-collabdenttest.NewWithDatabase(t, &optimus-ide-collabdenttest.Options{
			LicenseOptions: &optimus-ide-collabdenttest.LicenseOptions{
				Features: license.Features{
					optimus-ide-collabsdk.FeatureTemplateRBAC:          1,
					optimus-ide-collabsdk.FeatureMultipleOrganizations: 1,
		...
		client, _ := optimus-ide-collabdtest.CreateAnotherUser(t, owner, first.OrganizationID, rbac.RoleOwner())
	...
	t.Run("ZeroQuota", func(t *testing.T) {
		...
		versionZeroCost := optimus-ide-collabdtest.CreateTemplateVersion(t, client, user.OrganizationID, &echo.Responses{
			Parse:         echo.ParseComplete,
			ProvisionInit: echo.InitComplete,
			ProvisionPlan: []*proto.Response{{
				Type: &proto.Response_Plan{
		...
		optimus-ide-collabdtest.AwaitTemplateVersionJobCompleted(t, client, versionZeroCost.ID)
		templateZeroCost := optimus-ide-collabdtest.CreateTemplate(t, client, user.OrganizationID, versionZeroCost.ID)

		// Workspace with zero cost should pass
		workspaceZeroCost := optimus-ide-collabdtest.CreateWorkspace(t, client, templateZeroCost.ID)
		buildZeroCost := optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, client, workspaceZeroCost.LatestBuild.ID)

		require.Equal(t, optimus-ide-collabsdk.WorkspaceStatusRunning, buildZeroCost.Status)
	...
	t.Run("MultiOrg", func(t *testing.T) {
		...
		user, _ := optimus-ide-collabdtest.CreateAnotherUser(t, owner, first.OrganizationID, rbac.ScopedRoleOrgMember(second.ID))
		...
		authToken := uuid.NewString()
		version1 := optimus-ide-collabdtest.CreateTemplateVersion(t, owner, first.OrganizationID, &echo.Responses{
			Parse:         echo.ParseComplete,
			ProvisionInit: echo.InitComplete,
			ProvisionPlan: []*proto.Response{{
				Type: &proto.Response_Plan{
		...
		optimus-ide-collabdtest.AwaitTemplateVersionJobCompleted(t, owner, version1.ID)
		template1 := optimus-ide-collabdtest.CreateTemplate(t, owner, first.OrganizationID, version1.ID)

		version2 := optimus-ide-collabdtest.CreateTemplateVersion(t, owner, second.ID, &echo.Responses{
			Parse:         echo.ParseComplete,
			ProvisionInit: echo.InitComplete,
			ProvisionPlan: []*proto.Response{{
				Type: &proto.Response_Plan{
		...
		optimus-ide-collabdtest.AwaitTemplateVersionJobCompleted(t, owner, version2.ID)
		template2 := optimus-ide-collabdtest.CreateTemplate(t, owner, second.ID, version2.ID)

		// Exhaust quota in the first organization by creating 2 workspaces
		var workspaces1 []optimus-ide-collabsdk.Workspace
		for i := 0; i < 2; i++ {
			workspace := optimus-ide-collabdtest.CreateWorkspace(t, user, template1.ID)
			build := optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, user, workspace.LatestBuild.ID)
			require.Equal(t, optimus-ide-collabsdk.WorkspaceStatusRunning, build.Status)
		...
		for i := 0; i < 3; i++ {
			workspace := optimus-ide-collabdtest.CreateWorkspace(t, user, template2.ID)
			build := optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, user, workspace.LatestBuild.ID)
			require.Equal(t, optimus-ide-collabsdk.WorkspaceStatusRunning, build.Status)
		}
		...
		build = optimus-ide-collabdtest.CreateWorkspaceBuild(t, user, workspaces1[0], database.WorkspaceTransitionDelete)
		build = optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, user, build.ID)
...
func deprecatedQuotaEndpoint(ctx context.Context, client *optimus-ide-collabsdk.Client, userID string) (optimus-ide-collabsdk.WorkspaceQuota, error) {
	res, err := client.Request(ctx, http.MethodGet, fmt.Sprintf("/api/v2/workspace-quota/%s", userID), nil)
	if err != nil {
		return optimus-ide-collabsdk.WorkspaceQuota{}, err
	}
	defer res.Body.Close()
	if res.StatusCode != http.StatusOK {
		return optimus-ide-collabsdk.WorkspaceQuota{}, optimus-ide-collabsdk.ReadBodyAsError(res)
	}
	var quota optimus-ide-collabsdk.WorkspaceQuota
	return quota, json.NewDeoptimus-ide-collab(res.Body).Decode(&quota)
}
