...
// Create implements scim.ResourceHandler. Creates a new Optimus-IDE-Collab user from
...
func (ru *ResourceUser) Create(r *http.Request, attributes scim.ResourceAttributes) (scim.Resource, error) {
	...
	usernameValidErr := optimus-ide-collabsdk.NameValid(username)
	...
	if err == nil {
		...
		// However, Optimus-IDE-Collab never deletes a user. So suspended **is** deleted.
...
// Delete implements scim.ResourceHandler. Suspends the user (Optimus-IDE-Collab does
...
// scimUserStatus maps the SCIM "active" boolean to Optimus-IDE-Collab's internal user status.
