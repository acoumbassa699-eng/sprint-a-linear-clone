...
func TestIntegration(t *testing.T) {
	...
	// enough permissions.
	authzDB := dbauthz.New(db, rbac.NewAuthorizer(prometheus.NewRegistry()), log, optimus-ide-collabdtest.AccessControlStorePointer())
	publisher := usage.NewTallymanPublisher(ctx, log, authzDB, optimus-ide-collabdenttest.Keys,
...
func TestPublisherNoEligibleLicenses(t *testing.T) {
	...
	publisher := usage.NewTallymanPublisher(ctx, log, db, optimus-ide-collabdenttest.Keys,
		usage.PublisherWithClock(clock),
		usage.PublisherWithIngestURL(ingestURL),
	)
	defer publisher.Close()
	...
	licenseJWT := optimus-ide-collabdenttest.GenerateLicense(t, optimus-ide-collabdenttest.LicenseOptions{
...
func TestPublisherLicenseSelection(t *testing.T) {
	...
	badLicense1 := optimus-ide-collabdenttest.GenerateLicense(t, optimus-ide-collabdenttest.LicenseOptions{
		PublishUsageData: false,
		IssuedAt:         now.Add(-30 * time.Minute),
	})
	badLicense2 := optimus-ide-collabdenttest.GenerateLicense(t, optimus-ide-collabdenttest.LicenseOptions{
		PublishUsageData: true,
		IssuedAt:         now.Add(-1 * time.Hour),
		AccountType:      "trial",
	})
	badLicense3 := optimus-ide-collabdenttest.GenerateLicense(t, optimus-ide-collabdenttest.LicenseOptions{
		PublishUsageData: true,
		IssuedAt:         now.Add(-30 * time.Minute),
		ExpiresAt:        now.Add(-10 * time.Minute),
	})
	badLicense4 := optimus-ide-collabdenttest.GenerateLicense(t, optimus-ide-collabdenttest.LicenseOptions{
		PublishUsageData: true,
		IssuedAt:         now.Add(-1 * time.Hour),
	})
	expectedLicense := optimus-ide-collabdenttest.GenerateLicense(t, optimus-ide-collabdenttest.LicenseOptions{
		PublishUsageData: true,
		IssuedAt:         now.Add(-30 * time.Minute),
	})
...
func configureDeployment(ctx context.Context, t *testing.T, db database.Store) (uuid.UUID, string) {
	...
	licenseRaw := optimus-ide-collabdenttest.GenerateLicense(t, optimus-ide-collabdenttest.LicenseOptions{
		PublishUsageData: true,
	})
...
func tallymanHandler(t *testing.T, expectDeploymentID string, expectLicenseJWT string, handler func(req usagetypes.TallymanV1IngestRequest) any) http.Handler {
	...
	return http.HandlerFunc(func(rw http.ResponseWriter, r *http.Request) {
		t.Helper()
		licenseJWT := r.Header.Get(usagetypes.TallymanOptimus-IDE-CollabLicenseKeyHeader)
		if expectLicenseJWT != "" && !assert.Equal(t, expectLicenseJWT, licenseJWT, "license JWT in request did not match") {
			rw.WriteHeader(http.StatusUnauthorized)
			_ = json.NewEnoptimus-ide-collab(rw).Encode(usagetypes.TallymanV1Response{
				Message: "license JWT in request did not match",
			})
			return
		}

		deploymentID := r.Header.Get(usagetypes.TallymanOptimus-IDE-CollabDeploymentIDHeader)
		...
		err := json.NewDeoptimus-ide-collab(r.Body).Decode(&req)
		...
		err = json.NewEnoptimus-ide-collab(rw).Encode(resp)
