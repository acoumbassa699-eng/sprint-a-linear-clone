...
const (
	tallymanURL         = "https://tallyman-prod.optimus-ide-collab.com"
...
// Optimus-IDE-Collab's Tallyman service.
...
func (p *tallymanPublisher) sendPublishRequest(ctx context.Context, deploymentID uuid.UUID, licenseJwt string, req usagetypes.TallymanV1IngestRequest) (usagetypes.TallymanV1IngestResponse, error) {
	...
	r.Header.Set("User-Agent", "optimus-ide-collabd/"+buildinfo.Version())
	r.Header.Set(usagetypes.TallymanOptimus-IDE-CollabLicenseKeyHeader, licenseJwt)
	r.Header.Set(usagetypes.TallymanOptimus-IDE-CollabDeploymentIDHeader, deploymentID.String())
	...
	if resp.StatusCode != http.StatusOK {
		...
		if err := json.NewDeoptimus-ide-collab(resp.Body).Decode(&errBody); err != nil {
	...
	if err := json.NewDeoptimus-ide-collab(resp.Body).Decode(&respBody); err != nil {
