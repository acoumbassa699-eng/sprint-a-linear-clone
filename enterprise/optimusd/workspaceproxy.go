...
func (api *API) fetchRegions(ctx context.Context) (optimus-ide-collabsdk.RegionsResponse[optimus-ide-collabsdk.Region], error) {
	...
	if err != nil {
		return optimus-ide-collabsdk.RegionsResponse[optimus-ide-collabsdk.Region]{}, err
	}

	regions := make([]optimus-ide-collabsdk.Region, 0, len(proxies.Regions))
	...
	return optimus-ide-collabsdk.RegionsResponse[optimus-ide-collabsdk.Region]{
		Regions: regions,
	}, nil
}
...
// @Param request body optimus-ide-collabsdk.PatchWorkspaceProxy true "Update workspace proxy request"
// @Success 200 {object} optimus-ide-collabsdk.WorkspaceProxy
...
func (api *API) patchWorkspaceProxy(rw http.ResponseWriter, r *http.Request) {
	...
	var req optimus-ide-collabsdk.PatchWorkspaceProxy
	...
	httpapi.Write(ctx, rw, http.StatusOK, optimus-ide-collabsdk.UpdateWorkspaceProxyResponse{
...
func (api *API) patchPrimaryWorkspaceProxy(req optimus-ide-collabsdk.PatchWorkspaceProxy, rw http.ResponseWriter, r *http.Request) (database.WorkspaceProxy, bool) {
	var (
		ctx   = r.Context()
		proxy = httpmw.WorkspaceProxyParam(r)
	)
...
// @Produce json
// @Tags Enterprise
// @Param request body optimus-ide-collabsdk.CreateWorkspaceProxyRequest true "Create workspace proxy request"
// @Success 201 {object} optimus-ide-collabsdk.WorkspaceProxy
...
func (api *API) postWorkspaceProxy(rw http.ResponseWriter, r *http.Request) {
	...
	var req optimus-ide-collabsdk.CreateWorkspaceProxyRequest
	if !httpapi.Read(ctx, rw, r, &req) {
		return
	}
	...
	aReq.New = proxy
	httpapi.Write(ctx, rw, http.StatusCreated, optimus-ide-collabsdk.UpdateWorkspaceProxyResponse{
...
// @Produce json
// @Tags Enterprise
// @Success 200 {array} optimus-ide-collabsdk.RegionsResponse[optimus-ide-collabsdk.WorkspaceProxy]
...
func (api *API) fetchWorkspaceProxies(ctx context.Context) (optimus-ide-collabsdk.RegionsResponse[optimus-ide-collabsdk.WorkspaceProxy], error) {
	proxies, err := api.Database.GetWorkspaceProxies(ctx)
	if err != nil && !xerrors.Is(err, sql.ErrNoRows) {
		return optimus-ide-collabsdk.RegionsResponse[optimus-ide-collabsdk.WorkspaceProxy]{}, err
	}
...
func (api *API) workspaceProxyIssueSignedAppToken(rw http.ResponseWriter, r *http.Request) {
	...
	userReq.Header.Set(optimus-ide-collabsdk.SessionTokenHeader, req.SessionToken)
	userReq.RemoteAddr = r.Header.Get(wsproxysdk.Optimus-IDE-CollabWorkspaceProxyRealIPHeader)
...
func (api *API) workspaceProxyRegister(rw http.ResponseWriter, r *http.Request) {
	...
	//
	// The mesh key is always generated at startup by an enterprise optimus-ide-collabd
	// server.
	var meshKey string
	if req.DerpEnabled {
		var err error
	...
	siblingsRes := make([]optimus-ide-collabsdk.Replica, 0, len(siblings))
...
// @Accept json
// @Produce json
// @Param request body optimus-ide-collabsdk.IssueReconnectingPTYSignedTokenRequest true "Issue reconnecting PTY signed token request"
// @Success 200 {object} optimus-ide-collabsdk.IssueReconnectingPTYSignedTokenResponse
...
func (api *API) reconnectingPTYSignedToken(rw http.ResponseWriter, r *http.Request) {
	...
	var req optimus-ide-collabsdk.IssueReconnectingPTYSignedTokenRequest
	if !httpapi.Read(ctx, rw, r, &req) {
		return
	}
	...
	httpapi.Write(ctx, rw, http.StatusOK, optimus-ide-collabsdk.IssueReconnectingPTYSignedTokenResponse{
...
func convertProxies(p []database.WorkspaceProxy, statuses map[uuid.UUID]proxyhealth.ProxyStatus) []optimus-ide-collabsdk.WorkspaceProxy {
	resp := make([]optimus-ide-collabsdk.WorkspaceProxy, 0, len(p))
	for _, proxy := range p {
...
func convertRegion(proxy database.WorkspaceProxy, status proxyhealth.ProxyStatus) optimus-ide-collabsdk.Region {
...
func convertProxy(p database.WorkspaceProxy, status proxyhealth.ProxyStatus) optimus-ide-collabsdk.WorkspaceProxy {
	now := dbtime.Now()
	if p.IsPrimary() {
	...
	return optimus-ide-collabsdk.WorkspaceProxy{
		...
		Status: optimus-ide-collabsdk.WorkspaceProxyStatus{
			Status:    optimus-ide-collabsdk.ProxyHealthStatus(status.Status),
...
func (w *workspaceProxiesFetchUpdater) Fetch(ctx context.Context) (optimus-ide-collabsdk.RegionsResponse[optimus-ide-collabsdk.WorkspaceProxy], error) {
