...
func TestCustomOrganizationRole(t *testing.T) {
	t.Parallel()
	templateAdminCustom := func(orgID uuid.UUID) optimus-ide-collabsdk.Role {
		return optimus-ide-collabsdk.Role{
			...
			OrganizationPermissions: optimus-ide-collabsdk.CreatePermissions(map[optimus-ide-collabsdk.RBACResource][]optimus-ide-collabsdk.RBACAction{
				optimus-ide-collabsdk.ResourceTemplate:  {optimus-ide-collabsdk.ActionCreate, optimus-ide-collabsdk.ActionRead, optimus-ide-collabsdk.ActionUpdate, optimus-ide-collabsdk.ActionViewInsights},
				optimus-ide-collabsdk.ResourceFile:      {optimus-ide-collabsdk.ActionCreate, optimus-ide-collabsdk.ActionRead},
	...
	t.Run("Success", func(t *testing.T) {
		...
		tmplAdmin, _ := optimus-ide-collabdtest.CreateAnotherUser(t, owner, first.OrganizationID, rbac.RoleIdentifier{Name: role.Name, OrganizationID: first.OrganizationID})
		...
		// roleNamesF := func(role optimus-ide-collabsdk.SlimRole) string { return role.Name }
		// require.Contains(t, slice.List(user.Roles, roleNamesF), role.Name)

		// Try to create a template version
		optimus-ide-collabdtest.CreateTemplateVersion(t, tmplAdmin, first.OrganizationID, nil)

		// Verify the role exists in the list
		allRoles, err := tmplAdmin.ListOrganizationRoles(ctx, first.OrganizationID)
		require.NoError(t, err)

		var foundRole optimus-ide-collabsdk.AssignableRoles
		require.True(t, slices.ContainsFunc(allRoles, func(selected optimus-ide-collabsdk.AssignableRoles) bool {
	...
	t.Run("RoleOverrides", func(t *testing.T) {
		...
		require.NoError(t, err)
		file, err := tmplAdmin.Upload(ctx, optimus-ide-collabsdk.ContentTypeTar, bytes.NewReader(data))
		require.NoError(t, err)
		_, err = tmplAdmin.CreateTemplateVersion(ctx, first.OrganizationID, optimus-ide-collabsdk.CreateTemplateVersionRequest{
			FileID:        file.ID,
	...
	t.Run("InvalidName", func(t *testing.T) {
		...
		//nolint:gocritic // owner is required for this
		_, err := owner.CreateOrganizationRole(ctx, optimus-ide-collabsdk.Role{
	...
	t.Run("ExcessPermissions", func(t *testing.T) {
		...
		siteRole.SitePermissions = []optimus-ide-collabsdk.Permission{
			{
		...
		userRole.UserPermissions = []optimus-ide-collabsdk.Permission{
			{
				ResourceType: optimus-ide-collabsdk.ResourceWorkspace,
				Action:       optimus-ide-collabsdk.ActionRead,
			},
	...
	t.Run("MemberPermissions", func(t *testing.T) {
		...
		role.OrganizationMemberPermissions = []optimus-ide-collabsdk.Permission{
			{
				ResourceType: optimus-ide-collabsdk.ResourceWorkspace,
				Action:       optimus-ide-collabsdk.ActionRead,
			},
	...
	t.Run("Delete", func(t *testing.T) {
		...
		orgAdmin, orgAdminUser := optimus-ide-collabdtest.CreateAnotherUser(t, owner, first.OrganizationID, rbac.ScopedRoleOrgAdmin(first.OrganizationID))
		ctx := testutil.Context(t, testutil.WaitMedium)
		...
		_, err = owner.UpdateOrganizationMemberRoles(ctx, first.OrganizationID, orgAdminUser.ID.String(), optimus-ide-collabsdk.UpdateRoles{
		...
		exists := slices.ContainsFunc(existingRoles, func(role optimus-ide-collabsdk.AssignableRoles) bool {
	...
	t.Run("DeleteRoleCascadeMembers", func(t *testing.T) {
		...
		optimus-ide-collabdtest.CreateAnotherUser(t, owner, first.OrganizationID, customRoleIdentifier)
		optimus-ide-collabdtest.CreateAnotherUser(t, owner, first.OrganizationID, rbac.ScopedRoleOrgAdmin(first.OrganizationID), customRoleIdentifier)
		optimus-ide-collabdtest.CreateAnotherUser(t, owner, first.OrganizationID, rbac.ScopedRoleOrgTemplateAdmin(first.OrganizationID), rbac.ScopedRoleOrgAuditor(first.OrganizationID), customRoleIdentifier)
		...
		for _, member := range originalMembers {
			...
			require.True(t, slices.ContainsFunc(member.Roles, func(role optimus-ide-collabsdk.SlimRole) bool {
		...
		for _, member := range members {
			require.False(t, slices.ContainsFunc(member.Roles, func(role optimus-ide-collabsdk.SlimRole) bool {
				return role.Name == customRoleIdentifier.Name
			}), "role should be removed from all users")

			// Verify the rest of the member's roles are unchanged
			original := originalMembers[slices.IndexFunc(originalMembers, func(haystack optimus-ide-collabsdk.OrganizationMemberWithUserData) bool {
				return haystack.UserID == member.UserID
			})]
			originalWithoutCustom := slices.DeleteFunc(original.Roles, func(role optimus-ide-collabsdk.SlimRole) bool {
				return role.Name == customRoleIdentifier.Name
			})
...
func TestListRoles(t *testing.T) {
	...
	testCases := []struct {
		...
		APICall         func(context.Context) ([]optimus-ide-collabsdk.AssignableRoles, error)
		ExpectedRoles   []optimus-ide-collabsdk.AssignableRoles
		AuthorizedError string
	}{
		{
			// Members cannot assign any roles
			Name: "MemberListSite",
			APICall: func(ctx context.Context) ([]optimus-ide-collabsdk.AssignableRoles, error) {
				x, err := member.ListSiteRoles(ctx)
				return x, err
			},
			ExpectedRoles: convertRoles(map[rbac.RoleIdentifier]bool{
				{Name: optimus-ide-collabsdk.RoleOwner}:         false,
				{Name: optimus-ide-collabsdk.RoleAuditor}:       false,
				{Name: optimus-ide-collabsdk.RoleTemplateAdmin}: false,
				{Name: optimus-ide-collabsdk.RoleUserAdmin}:     false,
			}),
		},
		{
			...
			ExpectedRoles: convertRoles(map[rbac.RoleIdentifier]bool{
				{Name: optimus-ide-collabsdk.RoleOrganizationAdmin, OrganizationID: owner.OrganizationID}:                false,
				{Name: optimus-ide-collabsdk.RoleOrganizationAuditor, OrganizationID: owner.OrganizationID}:              false,
				{Name: optimus-ide-collabsdk.RoleOrganizationTemplateAdmin, OrganizationID: owner.OrganizationID}:        false,
				{Name: optimus-ide-collabsdk.RoleOrganizationUserAdmin, OrganizationID: owner.OrganizationID}:            false,
				{Name: optimus-ide-collabsdk.RoleOrganizationWorkspaceCreationBan, OrganizationID: owner.OrganizationID}: false,
				{Name: optimus-ide-collabsdk.RoleOrganizationWorkspaceAccess, OrganizationID: owner.OrganizationID}:      false,
				{Name: optimus-ide-collabsdk.RoleAgentsAccess, OrganizationID: owner.OrganizationID}:                     false,
			}),
		},
		{
		...
		},
		{
			...
			ExpectedRoles: convertRoles(map[rbac.RoleIdentifier]bool{
				{Name: optimus-ide-collabsdk.RoleOrganizationAdmin, OrganizationID: owner.OrganizationID}:                true,
				{Name: optimus-ide-collabsdk.RoleOrganizationAuditor, OrganizationID: owner.OrganizationID}:              true,
				{Name: optimus-ide-collabsdk.RoleOrganizationTemplateAdmin, OrganizationID: owner.OrganizationID}:        true,
				{Name: optimus-ide-collabsdk.RoleOrganizationUserAdmin, OrganizationID: owner.OrganizationID}:            true,
				{Name: optimus-ide-collabsdk.RoleOrganizationWorkspaceCreationBan, OrganizationID: owner.OrganizationID}: true,
				{Name: optimus-ide-collabsdk.RoleOrganizationWorkspaceAccess, OrganizationID: owner.OrganizationID}:      true,
				{Name: optimus-ide-collabsdk.RoleAgentsAccess, OrganizationID: owner.OrganizationID}:                     true,
			}),
		},
		{
		...
		// Admin
		{
			...
			ExpectedRoles: convertRoles(map[rbac.RoleIdentifier]bool{
				{Name: optimus-ide-collabsdk.RoleOwner}:         true,
				{Name: optimus-ide-collabsdk.RoleAuditor}:       true,
				{Name: optimus-ide-collabsdk.RoleTemplateAdmin}: true,
				{Name: optimus-ide-collabsdk.RoleUserAdmin}:     true,
			}),
		},
		{
			Name: "AdminListOrg",
	...
	for _, c := range testCases {
		t.Run(c.Name, func(t *testing.T) {
			...
			if c.AuthorizedError != "" {
				...
				require.NoError(t, err)
				ignorePerms := func(f optimus-ide-collabsdk.AssignableRoles) optimus-ide-collabsdk.AssignableRoles {
					return optimus-ide-collabsdk.AssignableRoles{
						Role: optimus-ide-collabsdk.Role{
							Name:        f.Name,
...
func convertRole(roleName rbac.RoleIdentifier) optimus-ide-collabsdk.Role {
	role, _ := rbac.RoleByName(roleName)
	return db2sdk.RBACRole(role)
}

func convertRoles(assignableRoles map[rbac.RoleIdentifier]bool) []optimus-ide-collabsdk.AssignableRoles {
	converted := make([]optimus-ide-collabsdk.AssignableRoles, 0, len(assignableRoles))
	for roleName, assignable := range assignableRoles {
		role := convertRole(roleName)
		converted = append(converted, optimus-ide-collabsdk.AssignableRoles{
			Role:       role,
