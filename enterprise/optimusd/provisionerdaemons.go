...
func (api *API) provisionerDaemonsEnabledMW(next http.Handler) http.Handler {
	return http.HandlerFunc(func(rw http.ResponseWriter, r *http.Request) {
		if !api.Entitlements.Enabled(optimus-ide-collabsdk.FeatureExternalProvisionerDaemons) {
...
func (p *provisionerDaemonAuth) authorize(r *http.Request, org database.Organization, tags map[string]string) (provisiionerDaemonAuthResponse, error) {
	...
	if provAuth {
		...
		pskKey, err := uuid.Parse(optimus-ide-collabsdk.ProvisionerKeyIDPSK)
		if err != nil {
	...
	userKey, err := uuid.Parse(optimus-ide-collabsdk.ProvisionerKeyIDUserAuth)
	if err != nil {
...
func (api *API) provisionerDaemonServe(rw http.ResponseWriter, r *http.Request) {
	...
	provisionersMap := map[optimus-ide-collabsdk.ProvisionerType]struct{}{}
	for _, provisioner := range r.URL.Query()["provisioner"] {
		switch provisioner {
		case string(optimus-ide-collabsdk.ProvisionerTypeEcho):
			provisionersMap[optimus-ide-collabsdk.ProvisionerTypeEcho] = struct{}{}
		case string(optimus-ide-collabsdk.ProvisionerTypeTerraform):
			provisionersMap[optimus-ide-collabsdk.ProvisionerTypeTerraform] = struct{}{}
		default:
			httpapi.Write(ctx, rw, http.StatusBadRequest, optimus-ide-collabsdk.Response{
	...
	authCtx := ctx
	if r.Header.Get(optimus-ide-collabsdk.ProvisionerDaemonPSK) != "" || r.Header.Get(optimus-ide-collabsdk.ProvisionerDaemonKey) != "" {
		//nolint:gocritic // PSK auth means no actor in request,
		// so use system restricted.
		authCtx = dbauthz.AsSystemRestricted(ctx)
	}

	versionHdrVal := r.Header.Get(optimus-ide-collabsdk.BuildVersionHeader)
