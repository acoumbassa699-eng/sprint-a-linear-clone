...
// @Produce json
// @Tags Enterprise
// @Param request body optimus-ide-collabsdk.CreateGroupRequest true "Create group request"
// @Param organization path string true "Organization ID"
// @Success 201 {object} optimus-ide-collabsdk.Group
...
func (api *API) postGroupByOrganization(rw http.ResponseWriter, r *http.Request) {
	...
	var req optimus-ide-collabsdk.CreateGroupRequest
	...
	if req.Name == database.EveryoneGroup {
		httpapi.Write(ctx, rw, http.StatusBadRequest, optimus-ide-collabsdk.Response{
			...
			Validations: []optimus-ide-collabsdk.ValidationError{{Field: "name", Detail: fmt.Sprintf("%q is a reserved group name", req.Name)}},
		})
		return
	}
	...
	if database.IsUniqueViolation(err) {
		httpapi.Write(ctx, rw, http.StatusConflict, optimus-ide-collabsdk.Response{
			...
			Validations: []optimus-ide-collabsdk.ValidationError{{Field: "name", Detail: "Group names must be unique"}},
		})
		return
	}
	if err != nil {
...
// @Param request body optimus-ide-collabsdk.PatchGroupRequest true "Patch group request"
// @Success 200 {object} optimus-ide-collabsdk.Group
...
func (api *API) patchGroup(rw http.ResponseWriter, r *http.Request) {
	...
	var req optimus-ide-collabsdk.PatchGroupRequest
	if !httpapi.Read(ctx, rw, r, &req) {
		return
	}
...
// @Success 200 {object} optimus-ide-collabsdk.GroupMembersResponse
...
func (api *API) groupMembers(rw http.ResponseWriter, r *http.Request) {
	...
	if len(members) == 0 {
		httpapi.Write(ctx, rw, http.StatusOK, optimus-ide-collabsdk.GroupMembersResponse{
			Users: nil,
			Count: 0,
		})
		return
...
// @Tags Enterprise
// @Param organization path string true "Organization ID" format(uuid)
// @Success 200 {array} optimus-ide-collabsdk.Group
...
func (api *API) groups(rw http.ResponseWriter, r *http.Request) {
	...
	resp := make([]optimus-ide-collabsdk.Group, 0, len(groups))
