...
func newMockUpstream(t *testing.T, name string) *mockUpstream {
	...
	m.server = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		...
		assert.NoError(t, json.NewEnoptimus-ide-collab(w).Encode(map[string]string{"upstream": name}))
	}))
...
// reload -> routing path: every provider mutation made through optimus-ide-collabsdk
...
func TestAIBridgeProviderHotReload(t *testing.T) {
	...
	client, _, api, _ := optimus-ide-collabdenttest.NewWithAPI(t, &optimus-ide-collabdenttest.Options{
	...
	_, err = client.UpdateAIProvider(ctx, "primary", optimus-ide-collabsdk.UpdateAIProviderRequest{
		BaseURL: &newBaseURL,
	})
	require.NoError(t, err)
