...
func dialAIGatewayServe(ctx context.Context, t *testing.T, client *optimus-ide-collabsdk.Client, key string) (aibridged.DRPCClient, error) {
	return dialAIGatewayServeWithVersion(ctx, t, client, key, nil)
}

func dialAIGatewayServeWithVersion(ctx context.Context, t *testing.T, client *optimus-ide-collabsdk.Client, key string, version *string) (aibridged.DRPCClient, error) {
	t.Helper()
...
func TestAIGatewayServeSuccess(t *testing.T) {
	...
	client, firstUser := optimus-ide-collabdenttest.New(t, aibridgeOpts(t))
	ctx := testutil.Context(t, testutil.WaitLong)

	//nolint:gocritic // Owner role is needed for gateway key management.
	created, err := client.CreateAIGatewayKey(ctx, optimus-ide-collabsdk.CreateAIGatewayKeyRequest{Name: "serve-success"})
...
func TestAIGatewayServeKeyAndVersionValidationErr(t *testing.T) {
	...
	//nolint:gocritic // Owner role is needed for gateway key management.
	created, err := client.CreateAIGatewayKey(ctx, optimus-ide-collabsdk.CreateAIGatewayKeyRequest{Name: "serve-quick-failures"})
	require.NoError(t, err)
	validKey := created.Key

	//nolint:gocritic // Owner role is needed for gateway key management.
	revoked, err := client.CreateAIGatewayKey(ctx, optimus-ide-collabsdk.CreateAIGatewayKeyRequest{Name: "serve-revoked"})
	require.NoError(t, err)
...
func TestAIGatewayServeMissingEntitlement(t *testing.T) {
	...
	// *optimus-ide-collabsdk.Error so the standalone gateway's connect loop can detect the
...
func TestAIGatewayServeTrackKeyUsageClosesActiveSession(t *testing.T) {
	...
	t.Run("RevokedEntitlement", func(t *testing.T) {
		...
		require.Eventually(t, func() bool {
			return !session.api.Entitlements.Enabled(optimus-ide-collabsdk.FeatureAIBridge)
...
type activeAIGatewayServeSession struct {
	...
	api     *entoptimus-ide-collabd.API
	created optimus-ide-collabsdk.CreateAIGatewayKeyResponse
	tick    chan time.Time
...
func setupActiveAIGatewayServeSession(ctx context.Context, t *testing.T) activeAIGatewayServeSession {
	...
	client, _, api, _ := optimus-ide-collabdenttest.NewWithAPI(t, opts)

	//nolint:gocritic // Owner role is needed for gateway key management.
	created, err := client.CreateAIGatewayKey(ctx, optimus-ide-collabsdk.CreateAIGatewayKeyRequest{Name: "key-name"})
	require.NoError(t, err)
