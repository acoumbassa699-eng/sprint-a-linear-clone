...
// @Success 200 {object} optimus-ide-collabsdk.WorkspaceSharingSettings
...
func (api *API) workspaceSharingSettings(rw http.ResponseWriter, r *http.Request) {
	...
	owners := optimus-ide-collabsdk.ShareableWorkspaceOwners(org.ShareableWorkspaceOwners)
	if globallyDisabled {
		owners = optimus-ide-collabsdk.ShareableWorkspaceOwnersNone
	}
	httpapi.Write(ctx, rw, http.StatusOK, optimus-ide-collabsdk.WorkspaceSharingSettings{
...
// @Tags Enterprise
// @Param organization path string true "Organization ID" format(uuid)
// @Param request body optimus-ide-collabsdk.UpdateWorkspaceSharingSettingsRequest true "Workspace sharing settings"
// @Success 200 {object} optimus-ide-collabsdk.WorkspaceSharingSettings
...
func (api *API) patchWorkspaceSharingSettings(rw http.ResponseWriter, r *http.Request) {
	...
	var req optimus-ide-collabsdk.UpdateWorkspaceSharingSettingsRequest
	...
	// tf-provider-optimus-ide-collabd v0.0.16)
	allowedOwners := req.ShareableWorkspaceOwners
	if allowedOwners == "" {
		if req.SharingDisabled {
			allowedOwners = optimus-ide-collabsdk.ShareableWorkspaceOwnersNone
		} else {
			allowedOwners = optimus-ide-collabsdk.ShareableWorkspaceOwnersEveryone
		}
	}
	...
	err := api.Database.InTx(func(tx database.Store) error {
		...
		// reconciliation across optimus-ide-collabd instances (e.g. during rolling restarts).
	...
	httpapi.Write(ctx, rw, http.StatusOK, optimus-ide-collabsdk.WorkspaceSharingSettings{
		...
		ShareableWorkspaceOwners: optimus-ide-collabsdk.ShareableWorkspaceOwners(org.ShareableWorkspaceOwners),
	})
}
