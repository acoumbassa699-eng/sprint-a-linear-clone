...
func (api *API) aiGatewayServe(rw http.ResponseWriter, r *http.Request) {
	key := r.Header.Get(optimus-ide-collabsdk.AIGatewayKeyHeader)
	if key == "" {
		httpapi.Write(r.Context(), rw, http.StatusUnauthorized, optimus-ide-collabsdk.Response{
			Message: "AI Gateway key required.",
		})
		return
	}

	// nolint:gocritic // AI Gateway doesn't have Optimus-IDE-Collab identity.System must look up the AI Gateway key to authenticate the request.
	...
	clientOptimus-IDE-CollabVersion := r.Header.Get(optimus-ide-collabsdk.BuildVersionHeader)
	logger := api.Logger.Named("aigateway-serve").With(
		slog.F("remote_addr", r.RemoteAddr),
		slog.F("client_api_version", clientAPIVersion),
		slog.F("client_build_version", clientOptimus-IDE-CollabVersion),
	...
	if err := aibridgedproto.CurrentVersion.Validate(clientAPIVersion); err != nil {
		httpapi.Write(keyCtx, rw, http.StatusBadRequest, optimus-ide-collabsdk.Response{
	...
	connCtx, wsNetConn := optimus-ide-collabsdk.WebsocketNetConn(keyCtx, conn, websocket.MessageBinary)
...
func aiGatewayCheckEntitlementAndTrackKeyUsage(ctx context.Context, ctxCancel context.CancelFunc, api *API, keyID uuid.UUID, logger slog.Logger) {
	...
	for {
		...
		if !api.Entitlements.Enabled(optimus-ide-collabsdk.FeatureAIBridge) {
