...
func createOpenAIModelConfigForTest(
	...
	model, err := client.CreateChatModelConfig(ctx, optimus-ide-collabsdk.CreateChatModelConfigRequest{
...
func TestChatStreamRelay(t *testing.T) {
	...
	// https://github.com/optimus-ide-collab/aibridge/issues/223. Unskip once the
	// reverse-proxy refactor lands and the follow-up tracking ticket is
	// resolved.
	t.Parallel()
	t.Skip("chat stream relay buffers events under AI Gateway routing; see CODAGT-734 and https://github.com/optimus-ide-collab/aibridge/issues/223")
	...
	t.Run("RelayMessagePartsAcrossReplicas", func(t *testing.T) {
		...
		firstClient, _, firstAPI, firstUser := optimus-ide-collabdenttest.NewWithAPI(t, &optimus-ide-collabdenttest.Options{
		...
		expClient := optimus-ide-collabsdk.NewExperimentalClient(firstClient)
		...
		var localClient *optimus-ide-collabsdk.ExperimentalClient
		var relayClient *optimus-ide-collabsdk.ExperimentalClient
		switch runningChat.WorkerID.UUID {
		case firstReplicaID:
			localClient = optimus-ide-collabsdk.NewExperimentalClient(firstClient)
			relayClient = optimus-ide-collabsdk.NewExperimentalClient(secondClient)
		case secondReplicaID:
			localClient = optimus-ide-collabsdk.NewExperimentalClient(secondClient)
			relayClient = optimus-ide-collabsdk.NewExperimentalClient(firstClient)
		default:
			require.FailNowf(
				t,
		...
		require.Equal(t, optimus-ide-collabsdk.ChatMessageRoleAssistant, firstEvent.MessagePart.Role)
		...
		require.Equal(t, optimus-ide-collabsdk.ChatMessageRoleAssistant, secondSnapshotEvent.MessagePart.Role)
	...
	//
	// The bug: optimus-ide-collabsdk.Client.Dial() does not propagate c.HTTPClient to
	...
	t.Run("RelayWithTLSAndCookieAuth", func(t *testing.T) {
		...
		// headers (like Optimus-IDE-Collab-Session-Token) on WebSocket upgrades;
		// they rely on cookies for authentication.
		//
		// We intentionally do NOT call secondClient.SetSessionToken()
		// because that would set the Optimus-IDE-Collab-Session-Token header,
		// which masks the bug.
		...
		// which includes Cookie but the Optimus-IDE-Collab-Session-Token header
	...
	// set custom headers (like Optimus-IDE-Collab-Session-Token) on WebSocket
	...
	// Optimus-IDE-Collab-Session-Token header, masking this code path.
	...
	// HTTPCookies.Middleware normalizes __Host-optimus-ide-collab_session_token
	// to optimus-ide-collab_session_token, the relay forwards the bare cookie.
	// On the worker replica, the same middleware must not strip it.
	//
	// The fix ensures relayHeaders also extracts the token value
	// and sets the Optimus-IDE-Collab-Session-Token header so the worker
	...
	t.Run("RelayCookieOnlyAuthWithHostPrefix", func(t *testing.T) {
		...
		db, pubsub := dbtestutil.NewDB(t)
		hostPrefixValues := optimus-ide-collabdtest.DeploymentValues(t, func(dv *optimus-ide-collabsdk.DeploymentValues) {
		...
		// __Host-optimus-ide-collab_session_token but the middleware
		...
		// optimus-ide-collab_session_token cookie and there's no fallback
		// Optimus-IDE-Collab-Session-Token header, causing a 401.
		secondEvents, secondStream, err := relayClient.StreamChat(ctx, chat.ID, nil)
		require.NoError(t, err)
		defer secondStream.Close()
	...
	t.Run("RelaySnapshotIncludesBufferedParts", func(t *testing.T) {
		...
		for _, text := range bufferedTexts {
			...
			require.Equal(t, optimus-ide-collabsdk.ChatMessageRoleAssistant, event.MessagePart.Role)
		}
...
func waitForStreamTextPart(
	...
	for {
		...
		case event, ok := <-events:
			...
			if event.Type == optimus-ide-collabsdk.ChatStreamEventTypeError {
			...
			if event.Type != optimus-ide-collabsdk.ChatStreamEventTypeMessagePart || event.MessagePart == nil {
				continue
			}
			if event.MessagePart.Part.Type != optimus-ide-collabsdk.ChatMessagePartTypeText {
				continue
			}
...
func replicaIDForClientURL(
	t *testing.T,
	clientURL *url.URL,
	replicas []optimus-ide-collabsdk.Replica,
) uuid.UUID {
	t.Helper()
...
func TestChatModelConfigDefault(t *testing.T) {
	...
	client, _ := optimus-ide-collabdenttest.New(t, nil)
	...
	firstModel, err := expClient.CreateChatModelConfig(
		ctx,
		optimus-ide-collabsdk.CreateChatModelConfigRequest{
			AIProviderID:         &provider.ID,
			Model:                "gpt-5-a",
			DisplayName:          "GPT 5 A",
			IsDefault:            &trueValue,
	...
	updatedFirst, err := expClient.UpdateChatModelConfig(
		ctx,
		firstModel.ID,
		optimus-ide-collabsdk.UpdateChatModelConfigRequest{
			IsDefault: &trueValue,
		},
	)
	require.NoError(t, err)
...
func findChatModelConfigByID(
	t *testing.T,
	modelConfigs []optimus-ide-collabsdk.ChatModelConfig,
	id uuid.UUID,
) optimus-ide-collabsdk.ChatModelConfig {
	t.Helper()
...
// Optimus-IDE-Collab-Session-Token header (for regular API calls) but
...
func (p cookieOnlySessionTokenProvider) AsRequestOption() optimus-ide-collabsdk.RequestOption {
	return func(req *http.Request) {
		req.Header.Set(optimus-ide-collabsdk.SessionTokenHeader, p.token)
	}
}
...
func (p cookieOnlySessionTokenProvider) SetDialOption(opts *websocket.DialOptions) {
	...
	cookieName := optimus-ide-collabsdk.SessionTokenCookie
	if p.hostPrefix {
...
func TestCreateChatNonDefaultOrg(t *testing.T) {
	...
	client, firstUser := optimus-ide-collabdenttest.New(t, &optimus-ide-collabdenttest.Options{
		Options: &optimus-ide-collabdtest.Options{
			DeploymentValues: func() *optimus-ide-collabsdk.DeploymentValues {
				v := optimus-ide-collabdtest.DeploymentValues(t)
				return v
			}(),
		},
		LicenseOptions: &optimus-ide-collabdenttest.LicenseOptions{
...
func TestListChats_OrgAdminOnlySeesOwnChats(t *testing.T) {
	...
	// Create a member with agents-access in both orgs.
	memberClientRaw, _ := optimus-ide-collabdtest.CreateAnotherUser(
		t, client, firstUser.OrganizationID,
		rbac.ScopedRoleAgentsAccess(firstUser.OrganizationID),
		rbac.ScopedRoleAgentsAccess(secondOrg.ID),
	)
	memberExp := optimus-ide-collabsdk.NewExperimentalClient(memberClientRaw)
	// Member creates a chat in the second org.
	memberChat, err := memberExp.CreateChat(ctx, optimus-ide-collabsdk.CreateChatRequest{
		OrganizationID: secondOrg.ID,
		Content: []optimus-ide-collabsdk.ChatInputPart{
			{
				Type: optimus-ide-collabsdk.ChatInputPartTypeText,
	...
	adminClientRaw, _ := optimus-ide-collabdtest.CreateAnotherUser(
		t, client, firstUser.OrganizationID,
		rbac.ScopedRoleOrgAdmin(secondOrg.ID), rbac.ScopedRoleAgentsAccess(secondOrg.ID),
	)
	adminExp := optimus-ide-collabsdk.NewExperimentalClient(adminClientRaw)

	// Admin creates a chat in the second org.
	adminChat, err := adminExp.CreateChat(ctx, optimus-ide-collabsdk.CreateChatRequest{
		OrganizationID: secondOrg.ID,
		Content: []optimus-ide-collabsdk.ChatInputPart{
			{
				Type: optimus-ide-collabsdk.ChatInputPartTypeText,
