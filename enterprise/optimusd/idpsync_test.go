...
func TestGetGroupSyncSettings(t *testing.T) {
	...
	t.Run("OK", func(t *testing.T) {
		...
		owner, db, user := optimus-ide-collabdenttest.NewWithDatabase(t, &optimus-ide-collabdenttest.Options{
		...
		orgAdmin, _ := optimus-ide-collabdtest.CreateAnotherUser(t, owner, user.OrganizationID, rbac.ScopedRoleOrgAdmin(user.OrganizationID))
...
func TestPatchGroupSyncSettings(t *testing.T) {
	...
	t.Run("OK", func(t *testing.T) {
		...
		// Test as org admin
		ctx := testutil.Context(t, testutil.WaitShort)
		settings, err := orgAdmin.PatchGroupIDPSyncSettings(ctx, user.OrganizationID.String(), optimus-ide-collabsdk.GroupSyncSettings{
			Field: "august",
		})
		require.NoError(t, err)
		require.Equal(t, "august", settings.Field)
	...
	t.Run("NotAuthorized", func(t *testing.T) {
		...
		member, _ := optimus-ide-collabdtest.CreateAnotherUser(t, owner, user.OrganizationID)

		ctx := testutil.Context(t, testutil.WaitShort)
		_, err := member.PatchGroupIDPSyncSettings(ctx, user.OrganizationID.String(), optimus-ide-collabsdk.GroupSyncSettings{
			Field: "august",
		})
...
func TestPatchGroupSyncConfig(t *testing.T) {
	...
	t.Run("OK", func(t *testing.T) {
		...
		orgAdmin, _ := optimus-ide-collabdtest.CreateAnotherUser(t, owner, orgID, rbac.ScopedRoleOrgAdmin(user.OrganizationID))

		mapping := map[string][]uuid.UUID{"wibble": {uuid.New()}}

		ctx := testutil.Context(t, testutil.WaitShort)
		_, err := orgAdmin.PatchGroupIDPSyncSettings(ctx, orgID.String(), optimus-ide-collabsdk.GroupSyncSettings{
			Field:             "wibble",
		...
		ctx = testutil.Context(t, testutil.WaitShort)
		settings, err := orgAdmin.PatchGroupIDPSyncConfig(ctx, orgID.String(), optimus-ide-collabsdk.PatchGroupIDPSyncConfigRequest{
			Field:             "wobble",
	...
	t.Run("NotAuthorized", func(t *testing.T) {
		...
		ctx := testutil.Context(t, testutil.WaitShort)
		_, err := member.PatchGroupIDPSyncConfig(ctx, user.OrganizationID.String(), optimus-ide-collabsdk.PatchGroupIDPSyncConfigRequest{})
		var apiError *optimus-ide-collabsdk.Error
		require.ErrorAs(t, err, &apiError)
		require.Equal(t, http.StatusForbidden, apiError.StatusCode())
	})
...
func TestPatchGroupSyncMapping(t *testing.T) {
	...
	t.Run("OK", func(t *testing.T) {
		...
		ctx = testutil.Context(t, testutil.WaitShort)
		settings, err := orgAdmin.PatchGroupIDPSyncMapping(ctx, orgID.String(), optimus-ide-collabsdk.PatchGroupIDPSyncMappingRequest{
			Add: []optimus-ide-collabsdk.IDPSyncMapping[uuid.UUID]{
			...
			Remove: []optimus-ide-collabsdk.IDPSyncMapping[uuid.UUID]{
				{Given: "wobble", Gets: orgs[1]},
			},
		})
	...
	t.Run("NotAuthorized", func(t *testing.T) {
		...
		ctx := testutil.Context(t, testutil.WaitShort)
		_, err := member.PatchGroupIDPSyncMapping(ctx, user.OrganizationID.String(), optimus-ide-collabsdk.PatchGroupIDPSyncMappingRequest{})
		var apiError *optimus-ide-collabsdk.Error
		require.ErrorAs(t, err, &apiError)
		require.Equal(t, http.StatusForbidden, apiError.StatusCode())
	})
...
func TestGetRoleSyncSettings(t *testing.T) {
	...
	t.Run("OK", func(t *testing.T) {
		...
		owner, _, _, user := optimus-ide-collabdenttest.NewWithAPI(t, &optimus-ide-collabdenttest.Options{
			LicenseOptions: &optimus-ide-collabdenttest.LicenseOptions{
				Features: license.Features{
					optimus-ide-collabsdk.FeatureCustomRoles:           1,
					optimus-ide-collabsdk.FeatureMultipleOrganizations: 1,
		...
		ctx := testutil.Context(t, testutil.WaitShort)
		settings, err := orgAdmin.PatchRoleIDPSyncSettings(ctx, user.OrganizationID.String(), optimus-ide-collabsdk.RoleSyncSettings{
			Field: "august",
...
func TestPatchRoleSyncSettings(t *testing.T) {
	...
	t.Run("NotAuthorized", func(t *testing.T) {
		...
		ctx := testutil.Context(t, testutil.WaitShort)
		_, err := member.PatchRoleIDPSyncSettings(ctx, user.OrganizationID.String(), optimus-ide-collabsdk.RoleSyncSettings{
			Field: "august",
		})
		var apiError *optimus-ide-collabsdk.Error
		require.ErrorAs(t, err, &apiError)
...
func TestPatchRoleSyncConfig(t *testing.T) {
	...
	t.Run("OK", func(t *testing.T) {
		...
		ctx := testutil.Context(t, testutil.WaitShort)
		_, err := orgAdmin.PatchRoleIDPSyncSettings(ctx, orgID.String(), optimus-ide-collabsdk.RoleSyncSettings{
			Field:   "wibble",
			Mapping: mapping,
		})
		...
		ctx = testutil.Context(t, testutil.WaitShort)
		settings, err := orgAdmin.PatchRoleIDPSyncConfig(ctx, orgID.String(), optimus-ide-collabsdk.PatchRoleIDPSyncConfigRequest{
			Field: "wobble",
		})
...
func TestPatchRoleSyncMapping(t *testing.T) {
	...
	t.Run("OK", func(t *testing.T) {
		...
		ctx = testutil.Context(t, testutil.WaitShort)
		settings, err := orgAdmin.PatchRoleIDPSyncMapping(ctx, orgID.String(), optimus-ide-collabsdk.PatchRoleIDPSyncMappingRequest{
			Add: []optimus-ide-collabsdk.IDPSyncMapping[string]{
				{Given: "wibble", Gets: "group-00"},
				{Given: "wobble", Gets: "group-01"},
				{Given: "wobble", Gets: "group-02"},
			},
			// Remove takes priority over Add, so "3" should not actually be added to wooble.
			Remove: []optimus-ide-collabsdk.IDPSyncMapping[string]{
				{Given: "wobble", Gets: "group-01"},
			},
		})
...
func TestGetOrganizationSyncSettings(t *testing.T) {
	...
	t.Run("OK", func(t *testing.T) {
		...
		ctx := testutil.Context(t, testutil.WaitShort)
		settings, err := owner.PatchOrganizationIDPSyncSettings(ctx, optimus-ide-collabsdk.OrganizationSyncSettings{
			Field:   "august",
			Mapping: expected,
		})
...
func TestPatchOrganizationSyncConfig(t *testing.T) {
	...
	t.Run("OK", func(t *testing.T) {
		...
		ctx := testutil.Context(t, testutil.WaitShort)
		//nolint:gocritic // Only owners can change Organization IdP sync settings
		_, err := owner.PatchOrganizationIDPSyncSettings(ctx, optimus-ide-collabsdk.OrganizationSyncSettings{
			Field:         "wibble",
			AssignDefault: true,
			Mapping:       mapping,
		})
		...
		ctx = testutil.Context(t, testutil.WaitShort)
		settings, err := owner.PatchOrganizationIDPSyncConfig(ctx, optimus-ide-collabsdk.PatchOrganizationIDPSyncConfigRequest{
			Field: "wobble",
		})
	...
	t.Run("NotAuthorized", func(t *testing.T) {
		...
		ctx := testutil.Context(t, testutil.WaitShort)
		_, err := member.PatchOrganizationIDPSyncConfig(ctx, optimus-ide-collabsdk.PatchOrganizationIDPSyncConfigRequest{})
		var apiError *optimus-ide-collabsdk.Error
		require.ErrorAs(t, err, &apiError)
		require.Equal(t, http.StatusForbidden, apiError.StatusCode())
	})
...
func TestPatchOrganizationSyncMapping(t *testing.T) {
	...
	t.Run("OK", func(t *testing.T) {
		...
		ctx := testutil.Context(t, testutil.WaitShort)
		//nolint:gocritic // Only owners can change Organization IdP sync settings
		settings, err := owner.PatchOrganizationIDPSyncMapping(ctx, optimus-ide-collabsdk.PatchOrganizationIDPSyncMappingRequest{
			Add: []optimus-ide-collabsdk.IDPSyncMapping[uuid.UUID]{
				{Given: "wibble", Gets: orgs[0]},
	...
	t.Run("NotAuthorized", func(t *testing.T) {
		...
		ctx := testutil.Context(t, testutil.WaitShort)
		_, err := member.PatchOrganizationIDPSyncMapping(ctx, optimus-ide-collabsdk.PatchOrganizationIDPSyncMappingRequest{})
		var apiError *optimus-ide-collabsdk.Error
		require.ErrorAs(t, err, &apiError)
		require.Equal(t, http.StatusForbidden, apiError.StatusCode())
	})
