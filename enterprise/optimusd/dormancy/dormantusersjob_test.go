...
import (
	...
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/enterprise/optimus-ide-collabd/dormancy"
...
func TestCheckInactiveUsers(t *testing.T) {
	...
	inactiveUser1 := setupUser(ctx, t, db, "dormant-user-1@optimus-ide-collab.com", database.UserStatusActive, dormancyThreshold.Add(-time.Minute))
	inactiveUser2 := setupUser(ctx, t, db, "dormant-user-2@optimus-ide-collab.com", database.UserStatusActive, dormancyThreshold.Add(-time.Hour))
	inactiveUser3 := setupUser(ctx, t, db, "dormant-user-3@optimus-ide-collab.com", database.UserStatusActive, dormancyThreshold.Add(-6*time.Hour))

	// Create active users (last seen AFTER dormancy threshold)
	activeUser1 := setupUser(ctx, t, db, "active-user-1@optimus-ide-collab.com", database.UserStatusActive, baseTime.Add(-time.Minute))
	activeUser2 := setupUser(ctx, t, db, "active-user-2@optimus-ide-collab.com", database.UserStatusActive, baseTime.Add(-time.Hour))
	activeUser3 := setupUser(ctx, t, db, "active-user-3@optimus-ide-collab.com", database.UserStatusActive, baseTime.Add(-6*time.Hour))

	suspendedUser1 := setupUser(ctx, t, db, "suspended-user-1@optimus-ide-collab.com", database.UserStatusSuspended, dormancyThreshold.Add(-time.Minute))
	suspendedUser2 := setupUser(ctx, t, db, "suspended-user-2@optimus-ide-collab.com", database.UserStatusSuspended, dormancyThreshold.Add(-time.Hour))
	suspendedUser3 := setupUser(ctx, t, db, "suspended-user-3@optimus-ide-collab.com", database.UserStatusSuspended, dormancyThreshold.Add(-6*time.Hour))
