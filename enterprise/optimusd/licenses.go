...
// key20220812 is the Optimus-IDE-Collab license public key with id 2022-08-12 used to validate licenses signed
...
//
//  1. Upgrades --- if the license format changes from one version of Optimus-IDE-Collab to the next, during a
//     rolling update you will have different Optimus-IDE-Collab servers that need different licenses to function.
...
// @Produce json
// @Tags Enterprise
// @Param request body optimus-ide-collabsdk.AddLicenseRequest true "Add license request"
// @Success 201 {object} optimus-ide-collabsdk.License
...
func (api *API) postLicense(rw http.ResponseWriter, r *http.Request) {
	...
	var addLicense optimus-ide-collabsdk.AddLicenseRequest
...
// @Produce json
// @Tags Enterprise
// @Success 201 {object} optimus-ide-collabsdk.Response
...
// @Produce json
// @Tags Enterprise
// @Success 200 {array} optimus-ide-collabsdk.License
...
func (api *API) licenses(rw http.ResponseWriter, r *http.Request) {
	...
	if xerrors.Is(err, sql.ErrNoRows) {
		httpapi.Write(ctx, rw, http.StatusOK, []optimus-ide-collabsdk.License{})
		return
	}
	if err != nil {
		httpapi.Write(ctx, rw, http.StatusInternalServerError, optimus-ide-collabsdk.Response{
	...
	licenses, err = optimus-ide-collabd.AuthorizeFilter(api.AGPL.HTTPAuth, r, policy.ActionRead, licenses)
	if err != nil {
		httpapi.Write(ctx, rw, http.StatusInternalServerError, optimus-ide-collabsdk.Response{
			Message: "Internal error fetching licenses.",
			Detail:  err.Error(),
...
func convertLicense(dl database.License, c jwt.MapClaims) optimus-ide-collabsdk.License {
	return optimus-ide-collabsdk.License{
		ID:         dl.ID,
		UUID:       dl.UUID,
...
func convertLicenses(licenses []database.License) ([]optimus-ide-collabsdk.License, error) {
	out := make([]optimus-ide-collabsdk.License, 0, len(licenses))
...
// if they are expired, or signed by a key this version of Optimus-IDE-Collab no longer considers valid.
//
...
func decodeClaims(l database.License) (jwt.MapClaims, error) {
	...
	d := json.NewDeoptimus-ide-collab(bytes.NewBuffer(cb))
	d.UseNumber()
	err = d.Decode(&c)
	return c, err
}
