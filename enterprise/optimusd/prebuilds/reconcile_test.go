...
func TestNoReconciliationActionsIfNoPresets(t *testing.T) {
	...
	cfg := optimus-ide-collabsdk.PrebuildsConfig{
...
// I'm explicitly _not_ checking for EventJobPosted (optimus-ide-collabd/database/provisionerjobs/provisionerjobs.go) since that
...
func TestTrackResourceReplacement(t *testing.T) {
	...
	fakeEnqueuer := newFakeEnqueuer()
	registry := prometheus.NewRegistry()
	cache := files.New(registry, &optimus-ide-collabdtest.FakeAuthorizer{})
	reconciler := prebuilds.NewStoreReconciler(
		db, ps, cache, optimus-ide-collabsdk.PrebuildsConfig{}, logger,
		clock,
		registry,
		fakeEnqueuer,
		newNoopUsageCheckerPtr(),
	...
	templateAdmin := dbgen.User(t, db, database.User{
		RBACRoles: []string{optimus-ide-collabsdk.RoleTemplateAdmin},
	})
		...
		for _, tt := range []struct {
			...
			// Should not cancel pending user workspace build jobs
			{
				...
				setupBuild: func(
					...
					_, member := optimus-ide-collabdtest.CreateAnotherUser(t, client, orgID, rbac.RoleMember())
					return dbfake.WorkspaceBuild(t, db, database.WorkspaceTable{
						OwnerID:        member.ID,
						OrganizationID: orgID,
						TemplateID:     uuid.Nil,
...
//
//	go test -bench='^BenchmarkReconcileAll_NoOps$' -run=^$ -benchtime=5x -count=2 ./enterprise/optimus-ide-collabd/prebuilds/
func BenchmarkReconcileAll_NoOps(b *testing.B) {
	...
	for _, bc := range benchCases {
		b.Run(bc.name, func(b *testing.B) {
			...
			// Default value for OPTIMUS-IDE-COLLAB_PG_CONN_MAX_OPEN is 10.
			maxOpenConns := 10
...
//
//	go test -bench='^BenchmarkReconcileAll_ConnectionContention$' -run=^$ -benchtime=5x -count=2 ./enterprise/optimus-ide-collabd/prebuilds/
...
//
//	go test -bench='^BenchmarkReconcileAll_Mix$' -run=^$ -benchtime=5x -count=2 ./enterprise/optimus-ide-collabd/prebuilds/
