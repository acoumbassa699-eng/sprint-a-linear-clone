...
func (c *StoreReconciler) Run(ctx context.Context) {
	...
	// will periodically pick up any queued jobs (see poll(time.Duration) in optimus-ide-collabd/provisionerdserver/acquirer.go).
	wg.Add(1)
	go func() {
		defer wg.Done()
		for {
...
func (c *StoreReconciler) ReconcileAll(ctx context.Context) (stats prebuilds.ReconcileStats, err error) {
	...
	err = c.WithReconciliationLock(ctx, logger, func(ctx context.Context, db database.Store) error {
		...
		var settings optimus-ide-collabsdk.PrebuildsSettings
		...
		// Limit concurrency to avoid exhausting the optimus-ide-collabd database connection pool.
...
func (c *StoreReconciler) WithReconciliationLock(
	...
	// This tx holds a global lock, which prevents any other optimus-ide-collabd replica from starting a reconciliation and
