		...
		for _, useDefaultOrg := range []bool{true, false} {
			t.Run(name, func(t *testing.T) {
				...
				reconciler := prebuilds.NewStoreReconciler(
					spy, pubsub, cache, optimus-ide-collabsdk.PrebuildsConfig{}, logger,
					quartz.NewMock(t),
				...
				version := optimus-ide-collabdtest.CreateTemplateVersion(t, client, orgID, templateWithAgentAndPresetsWithPrebuilds(desiredInstances))
				...
				userClient, user := optimus-ide-collabdtest.CreateAnotherUser(t, client, orgID, rbac.RoleMember())
				...
				userWorkspace, err := userClient.CreateUserWorkspace(ctx, user.Username, optimus-ide-collabsdk.CreateWorkspaceRequest{
				...
				case tc.claimingErr != nil && (isNoPrebuiltWorkspaces || isUnsupported):
					require.NoError(t, err)
					optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, userClient, userWorkspace.LatestBuild.ID)
				...
				require.NoError(t, err)
				build := optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, userClient, stopBuild.ID)
				require.Equal(t, build.Job.Status, optimus-ide-collabsdk.ProvisionerJobSucceeded)

				startBuild, err := userClient.CreateWorkspaceBuild(ctx, workspace.ID, optimus-ide-collabsdk.CreateWorkspaceBuildRequest{
					TemplateVersionID:   version.ID,
					Transition:          optimus-ide-collabsdk.WorkspaceTransitionStart,
					RichParameterValues: wp,
				})
				require.NoError(t, err)
				build = optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, userClient, startBuild.ID)
				require.Equal(t, build.Job.Status, optimus-ide-collabsdk.ProvisionerJobSucceeded)
