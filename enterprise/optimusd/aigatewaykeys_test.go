...
import (
	...
	aibridgekeys "github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabd/aibridge/keys"
...
func TestAIGatewayKeys(t *testing.T) {
	...
	t.Run("CRUD", func(t *testing.T) {
		...
		ownerClient, _ := optimus-ide-collabdenttest.New(t, aibridgeOpts(t))
		...
		created, err := ownerClient.CreateAIGatewayKey(ctx, optimus-ide-collabsdk.CreateAIGatewayKeyRequest{Name: name})
		require.NoError(t, err)
	...
	t.Run("ListResponseDoesNotLeakSecrets", func(t *testing.T) {
		...
		//nolint:gocritic // Managing AI Gateway keys is owner-only.
		created, err := ownerClient.CreateAIGatewayKey(ctx, optimus-ide-collabsdk.CreateAIGatewayKeyRequest{
	...
	t.Run("CreateValidation", func(t *testing.T) {
		...
		//nolint:gocritic // Managing AI Gateway keys is owner-only.
		_, err := ownerClient.CreateAIGatewayKey(ctx, optimus-ide-collabsdk.CreateAIGatewayKeyRequest{Name: ""})
		...
		_, err = ownerClient.CreateAIGatewayKey(ctx, optimus-ide-collabsdk.CreateAIGatewayKeyRequest{Name: longName})
		require.ErrorAs(t, err, &sdkErr)
		require.Equal(t, http.StatusBadRequest, sdkErr.StatusCode())
		require.ErrorContains(t, err, "Invalid key name")

		// Uppercase name -> 400 (DB check constraint rejects non-lowercase).
		_, err = ownerClient.CreateAIGatewayKey(ctx, optimus-ide-collabsdk.CreateAIGatewayKeyRequest{Name: "UPPER-CASE"})
		require.ErrorAs(t, err, &sdkErr)
		require.Equal(t, http.StatusBadRequest, sdkErr.StatusCode())
		require.ErrorContains(t, err, "Invalid key name")
		...
		_, err = ownerClient.CreateAIGatewayKey(ctx, optimus-ide-collabsdk.CreateAIGatewayKeyRequest{Name: name})
		require.NoError(t, err)
		_, err = ownerClient.CreateAIGatewayKey(ctx, optimus-ide-collabsdk.CreateAIGatewayKeyRequest{Name: name})
		require.ErrorAs(t, err, &sdkErr)
		require.Equal(t, http.StatusBadRequest, sdkErr.StatusCode())
	...
	t.Run("ReturnsForbiddenForNonOwners", func(t *testing.T) {
		...
		ownerClient, owner := optimus-ide-collabdenttest.New(t, aibridgeOpts(t))
		ctx := testutil.Context(t, testutil.WaitLong)
		member, _ := optimus-ide-collabdtest.CreateAnotherUser(t, ownerClient, owner.OrganizationID)

		_, err := member.CreateAIGatewayKey(ctx, optimus-ide-collabsdk.CreateAIGatewayKeyRequest{
...
func TestAIGatewayKeyAudit(t *testing.T) {
	...
	opts.LicenseOptions.Features[optimus-ide-collabsdk.FeatureAuditLog] = 1

	ownerClient, _ := optimus-ide-collabdenttest.New(t, opts)
	ctx, cancel := context.WithTimeout(context.Background(), testutil.WaitMedium)
	defer cancel()

	name := uniqueName(t, "audit")
	//nolint:gocritic // Managing AI Gateway optimus-ide-collabd keys is owner-only.
	created, err := ownerClient.CreateAIGatewayKey(ctx, optimus-ide-collabsdk.CreateAIGatewayKeyRequest{Name: name})
	require.NoError(t, err)
	//nolint:gocritic // Managing AI Gateway optimus-ide-collabd keys is owner-only.
	require.NoError(t, ownerClient.DeleteAIGatewayKey(ctx, created.ID))
...
func TestAIGatewayKeysDatabaseErrors(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			body:       optimus-ide-collabsdk.CreateAIGatewayKeyRequest{Name: "db-err-create"},
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			require.NoError(t, json.NewDeoptimus-ide-collab(resp.Body).Decode(&sdkResp))
