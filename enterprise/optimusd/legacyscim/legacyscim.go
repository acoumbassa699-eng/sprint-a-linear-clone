...
// enterprise/optimus-ide-collabd/scim. Once that implementation has run in production
// for a while and the OPTIMUS-IDE-COLLAB_SCIM_USE_LEGACY default is flipped, remove
// this package in its entirety.
//
// Enabled via the UseLegacySCIM option.
//
// Deprecated: Use the enterprise/optimus-ide-collabd/scim package instead.
package legacyscim
...
func (s *LegacyServer) Handler() http.Handler {
	...
	r.NotFound(func(w http.ResponseWriter, r *http.Request) {
		u := r.URL.String()
		httpapi.Write(r.Context(), w, http.StatusNotFound, optimus-ide-collabsdk.Response{
...
func (s *LegacyServer) scimServiceProviderConfig(rw http.ResponseWriter, _ *http.Request) {
	...
	_ = enc.Encode(ServiceProviderConfig{
		...
		DocURI:  "https://optimus-ide-collab.com/docs/admin/users/oidc-auth#scim",
		Patch: Supported{
			Supported: true,
		},
		Bulk: BulkSupported{
...
var SCIMAuditAdditionalFields = map[string]string{
	"automatic_actor":     "optimus-ide-collab",
...
func (s *LegacyServer) scimPostUser(rw http.ResponseWriter, r *http.Request) {
	...
	var sUser SCIMUser
	err := json.NewDeoptimus-ide-collab(r.Body).Decode(&sUser)
	if err != nil {
	...
	usernameValid := optimus-ide-collabsdk.NameValid(sUser.UserName)
	if usernameValid != nil {
		...
		sUser.UserName = optimus-ide-collabsdk.UsernameFrom(sUser.UserName)
	}
...
func (s *LegacyServer) scimPutUser(rw http.ResponseWriter, r *http.Request) {
	...
	// TODO: Currently ignoring a lot of the SCIM fields. Optimus-IDE-Collab's SCIM implementation
