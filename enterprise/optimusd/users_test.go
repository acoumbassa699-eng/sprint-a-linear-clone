...
const TimeFormatHHMM = optimus-ide-collabd.TimeFormatHHMM
...
func TestUserQuietHours(t *testing.T) {
	...
	t.Run("DefaultToUTC", func(t *testing.T) {
		...
		client, user := optimus-ide-collabdtest.CreateAnotherUser(t, adminClient, adminUser.OrganizationID)
	...
	t.Run("OK", func(t *testing.T) {
		...
		ctx := testutil.Context(t, testutil.WaitLong)
		sched1, err := client.UserQuietHoursSchedule(ctx, optimus-ide-collabsdk.Me)
		require.NoError(t, err)
		...
		sched2, err := client.UpdateUserQuietHoursSchedule(ctx, user.ID.String(), optimus-ide-collabsdk.UpdateUserQuietHoursScheduleRequest{
		...
		_, err = client.UpdateUserQuietHoursSchedule(ctx, user.ID.String(), optimus-ide-collabsdk.UpdateUserQuietHoursScheduleRequest{
			Schedule: "garbage",
		})
		require.Error(t, err)
...
func TestAssignCustomOrgRoles(t *testing.T) {
	...
	client, _ := optimus-ide-collabdtest.CreateAnotherUser(t, ownerClient, owner.OrganizationID, rbac.ScopedRoleOrgAdmin(owner.OrganizationID))
	tv := optimus-ide-collabdtest.CreateTemplateVersion(t, client, owner.OrganizationID, nil)
	optimus-ide-collabdtest.AwaitTemplateVersionJobCompleted(t, client, tv.ID)

	ctx := testutil.Context(t, testutil.WaitShort)
	// Create a custom role as an organization admin that allows making templates.
	auditorRole, err := client.CreateOrganizationRole(ctx, optimus-ide-collabsdk.Role{
		...
		OrganizationPermissions: optimus-ide-collabsdk.CreatePermissions(map[optimus-ide-collabsdk.RBACResource][]optimus-ide-collabsdk.RBACAction{
			optimus-ide-collabsdk.ResourceTemplate: optimus-ide-collabsdk.RBACResourceActions[optimus-ide-collabsdk.ResourceTemplate], // All template perms
		}),
		UserPermissions: nil,
	})
	require.NoError(t, err)

	createTemplateReq := optimus-ide-collabsdk.CreateTemplateRequest{
		Name:        "name",
	...
	_, err = client.UpdateOrganizationMemberRoles(ctx, owner.OrganizationID, member.ID.String(), optimus-ide-collabsdk.UpdateRoles{
...
func TestGrantSiteRoles(t *testing.T) {
	...
	requireStatusCode := func(t *testing.T, err error, statusCode int) {
		t.Helper()
		var e *optimus-ide-collabsdk.Error
		require.ErrorAs(t, err, &e, "error is optimus-ide-collabsdk error")
		require.Equal(t, statusCode, e.StatusCode(), "correct status code")
	}

	dv := optimus-ide-collabdtest.DeploymentValues(t)
	admin, first := optimus-ide-collabdenttest.New(t, &optimus-ide-collabdenttest.Options{
		Options: &optimus-ide-collabdtest.Options{
			DeploymentValues: dv,
		},
		LicenseOptions: &optimus-ide-collabdenttest.LicenseOptions{
	...
	member, _ := optimus-ide-collabdtest.CreateAnotherUser(t, admin, first.OrganizationID)
	orgAdmin, _ := optimus-ide-collabdtest.CreateAnotherUser(t, admin, first.OrganizationID, rbac.ScopedRoleOrgAdmin(first.OrganizationID))
	randOrg := optimus-ide-collabdenttest.CreateOrganization(t, admin, optimus-ide-collabdenttest.CreateOrganizationOptions{})

	_, randOrgUser := optimus-ide-collabdtest.CreateAnotherUser(t, admin, randOrg.ID, rbac.ScopedRoleOrgAdmin(randOrg.ID))
	userAdmin, _ := optimus-ide-collabdtest.CreateAnotherUser(t, admin, first.OrganizationID, rbac.RoleUserAdmin())
	...
	testCases := []struct {
	...
	}{
		{
			Name:         "OrgRoleInSite",
			Client:       admin,
			AssignToUser: optimus-ide-collabsdk.Me,
		...
		},
		{
			...
			Roles:        []string{optimus-ide-collabsdk.RoleOwner},
			Error:        true,
		...
		},
		{
			...
			Client:       userAdmin,
			AssignToUser: newUser,
			Roles:        []string{optimus-ide-collabsdk.RoleMember},
			ExpectedRoles: []string{
				optimus-ide-collabsdk.RoleMember,
			},
			Error: false,
		},
	}
	...
	for _, c := range testCases {
		t.Run(c.Name, func(t *testing.T) {
			...
			if c.AssignToUser == newUser {
				...
				_, newUser := optimus-ide-collabdtest.CreateAnotherUser(t, admin, orgID)
				c.AssignToUser = newUser.ID.String()
			}

			var newRoles []optimus-ide-collabsdk.SlimRole
			if c.OrgID != uuid.Nil {
				// Org assign
				var mem optimus-ide-collabsdk.OrganizationMember
				mem, err = c.Client.UpdateOrganizationMemberRoles(ctx, c.OrgID, c.AssignToUser, optimus-ide-collabsdk.UpdateRoles{
					Roles: c.Roles,
				})
				newRoles = mem.Roles
			} else {
				...
				user, err = c.Client.UpdateUserRoles(ctx, c.AssignToUser, optimus-ide-collabsdk.UpdateRoles{
					Roles: c.Roles,
				})
				newRoles = user.Roles
			}
...
func TestEnterprisePostUser(t *testing.T) {
	...
	t.Run("OrganizationNoAccess", func(t *testing.T) {
		...
		notInOrg, _ := optimus-ide-collabdtest.CreateAnotherUser(t, client, first.OrganizationID)
		other, _ := optimus-ide-collabdtest.CreateAnotherUser(t, client, first.OrganizationID, rbac.RoleOwner(), rbac.RoleMember())

		ctx, cancel := context.WithTimeout(context.Background(), testutil.WaitLong)
		defer cancel()

		org := optimus-ide-collabdenttest.CreateOrganization(t, other, optimus-ide-collabdenttest.CreateOrganizationOptions{}, func(request *optimus-ide-collabsdk.CreateOrganizationRequest) {
			request.Name = "another"
		})

		_, err := notInOrg.CreateUserWithOrgs(ctx, optimus-ide-collabsdk.CreateUserRequestWithOrgs{
	...
	t.Run("OrganizationNoAccess", func(t *testing.T) {
		...
		org := optimus-ide-collabdenttest.CreateOrganization(t, other, optimus-ide-collabdenttest.CreateOrganizationOptions{})
	...
	t.Run("CreateWithoutOrg", func(t *testing.T) {
		...
		optimus-ide-collabdenttest.CreateOrganization(t, client, optimus-ide-collabdenttest.CreateOrganizationOptions{})

		// nolint:gocritic // intentional using the owner.
		// Manually making a user with the request instead of the optimus-ide-collabdtest util
	...
	t.Run("MultipleOrganizations", func(t *testing.T) {
		...
		second := optimus-ide-collabdenttest.CreateOrganization(t, client, optimus-ide-collabdenttest.CreateOrganizationOptions{})
		third := optimus-ide-collabdenttest.CreateOrganization(t, client, optimus-ide-collabdenttest.CreateOrganizationOptions{})
	...
	t.Run("ServiceAccount/OK", func(t *testing.T) {
		...
		require.NoError(t, err)
		require.Equal(t, optimus-ide-collabsdk.LoginTypeNone, user.LoginType)
	...
	t.Run("ServiceAccount/DefaultLoginType", func(t *testing.T) {
		...
		require.NoError(t, err)
		require.Equal(t, optimus-ide-collabsdk.LoginTypeNone, found.LoginType)
	...
	t.Run("ServiceAccount/MultipleWithoutEmail", func(t *testing.T) {
		...
		user2, err := client.CreateUserWithOrgs(ctx, optimus-ide-collabsdk.CreateUserRequestWithOrgs{
			OrganizationIDs: []uuid.UUID{first.OrganizationID},
