...
func TestNotifications(t *testing.T) {
	...
	// Regression test for https://github.com/optimus-ide-collab/optimus-ide-collab/issues/20913
...
func TestTemplateUpdatePrebuilds(t *testing.T) {
	...
	autostopWeekdays, err := optimus-ide-collabsdk.WeekdaysToBitmap(optimus-ide-collabsdk.AllDaysOfWeek)
	require.NoError(t, err)

	// Autostart configuration set to everyday at midnight
	autostartSchedule, err := cron.Weekly("CRON_TZ=UTC 0 0 * * *")
	require.NoError(t, err)
	autostartWeekdays, err := optimus-ide-collabsdk.WeekdaysToBitmap(optimus-ide-collabsdk.AllDaysOfWeek)
	require.NoError(t, err)
