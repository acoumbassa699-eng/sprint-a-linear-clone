...
import (
	...
	agpl "github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabd/schedule"
...
func (s *EnterpriseTemplateScheduleStore) updateWorkspaceBuild(ctx context.Context, db database.Store, build database.WorkspaceBuild) error {
	ctx, span := tracing.StartSpan(ctx,
		trace.WithAttributes(attribute.String("optimus-ide-collab.workspace_id", build.WorkspaceID.String())),
		trace.WithAttributes(attribute.String("optimus-ide-collab.workspace_build_id", build.ID.String())),
	)
	defer span.End()
	...
	if optimus-ide-collabsdk.ProvisionerJobStatus(job.JobStatus) != optimus-ide-collabsdk.ProvisionerJobSucceeded {
