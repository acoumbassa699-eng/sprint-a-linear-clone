...
import (
	...
	entprebuilds "github.com/optimus-ide-collab/optimus-ide-collab/v2/enterprise/optimus-ide-collabd/prebuilds"
...
// New constructs a optimus-ide-collabsdk client connected to an in-memory Enterprise API instance.
func New(t *testing.T, options *Options) (*optimus-ide-collabsdk.Client, optimus-ide-collabsdk.CreateFirstUserResponse) {
	client, _, _, user := NewWithAPI(t, options)
	return client, user
}

func NewWithDatabase(t *testing.T, options *Options) (*optimus-ide-collabsdk.Client, database.Store, optimus-ide-collabsdk.CreateFirstUserResponse) {
	client, _, api, user := NewWithAPI(t, options)
	return client, api.Database, user
}

func NewWithAPI(t *testing.T, options *Options) (
	*optimus-ide-collabsdk.Client, io.Closer, *optimus-ide-collabd.API, optimus-ide-collabsdk.CreateFirstUserResponse,
) {
	t.Helper()
	...
	if options.Options == nil {
		options.Options = &optimus-ide-collabdtest.Options{}
	}
	require.False(t, options.DontAddFirstUser && !options.DontAddLicense, "DontAddFirstUser requires DontAddLicense")
	setHandler, cancelFunc, serverURL, oop := optimus-ide-collabdtest.NewOptions(t, options.Options)
	optimus-ide-collabAPI, err := optimus-ide-collabd.New(context.Background(), &optimus-ide-collabd.Options{
		RBAC:                       true,
	...
	setHandler(optimus-ide-collabAPI.AGPL.RootHandler)
	var provisionerCloser io.Closer = nopcloser{}
	if options.IncludeProvisionerDaemon {
		provisionerCloser = optimus-ide-collabdtest.NewProvisionerDaemon(t, optimus-ide-collabAPI.AGPL)
	}
	...
	var user optimus-ide-collabsdk.CreateFirstUserResponse
	if !options.DontAddFirstUser {
		...
		if !options.DontAddLicense {
			...
			if options.LicenseOptions != nil {
				...
				if lo.AllFeatures || (lo.Features != nil && lo.Features[optimus-ide-collabsdk.FeatureHighAvailability] != 0) {
					// we check for the in-memory test types so that the real types don't have to exported
					_, ok := optimus-ide-collabAPI.Pubsub.(*pubsub.MemoryPubsub)
	...
	return client, provisionerCloser, optimus-ide-collabAPI, user
}
...
type LicenseOptions struct {
	...
	Trial            bool
	FeatureSet       optimus-ide-collabsdk.FeatureSet
	AllFeatures      bool
	...
	Addons   []optimus-ide-collabsdk.Addon
...
func (opts *LicenseOptions) UserLimit(limit int64) *LicenseOptions {
	return opts.Feature(optimus-ide-collabsdk.FeatureUserLimit, limit)
}

func (opts *LicenseOptions) AIGovernanceAddon(limit int64) *LicenseOptions {
	opts.Addons = append(opts.Addons, optimus-ide-collabsdk.AddonAIGovernance)
	return opts.Feature(optimus-ide-collabsdk.FeatureAIGovernanceUserLimit, limit)
}

func (opts *LicenseOptions) ManagedAgentLimit(limit int64) *LicenseOptions {
	return opts.Feature(optimus-ide-collabsdk.FeatureManagedAgentLimit, limit)
}

func (opts *LicenseOptions) Feature(name optimus-ide-collabsdk.FeatureName, value int64) *LicenseOptions {
...
func AddFullLicense(t *testing.T, client *optimus-ide-collabsdk.Client) optimus-ide-collabsdk.License {
	return AddLicense(t, client, LicenseOptions{AllFeatures: true})
}

// AddLicense generates a new license with the options provided and inserts it.
func AddLicense(t *testing.T, client *optimus-ide-collabsdk.Client, options LicenseOptions) optimus-ide-collabsdk.License {
	l, err := client.AddLicense(context.Background(), optimus-ide-collabsdk.AddLicenseRequest{
...
type CreateOrganizationOptions struct {
	...
	// This requires enterprise and the feature 'optimus-ide-collabsdk.FeatureExternalProvisionerDaemons'
	IncludeProvisionerDaemon bool
}

func CreateOrganization(t *testing.T, client *optimus-ide-collabsdk.Client, opts CreateOrganizationOptions, mutators ...func(*optimus-ide-collabsdk.CreateOrganizationRequest)) optimus-ide-collabsdk.Organization {
	ctx := testutil.Context(t, testutil.WaitMedium)
	req := optimus-ide-collabsdk.CreateOrganizationRequest{
...
func NewExternalProvisionerDaemon(t testing.TB, client *optimus-ide-collabsdk.Client, org uuid.UUID, tags map[string]string) io.Closer {
	t.Helper()
	return newExternalProvisionerDaemon(t, client, org, tags, optimus-ide-collabsdk.ProvisionerTypeEcho)
}
...
// behaviors!
func NewExternalProvisionerDaemonTerraform(t testing.TB, client *optimus-ide-collabsdk.Client, org uuid.UUID, tags map[string]string) io.Closer {
	t.Helper()
	return newExternalProvisionerDaemon(t, client, org, tags, optimus-ide-collabsdk.ProvisionerTypeTerraform)
}

// nolint // This function is a helper for tests and should not be linted.
func newExternalProvisionerDaemon(t testing.TB, client *optimus-ide-collabsdk.Client, org uuid.UUID, tags map[string]string, provisionerType optimus-ide-collabsdk.ProvisionerType) io.Closer {
	t.Helper()

	entitlements, err := client.Entitlements(context.Background())
	if err != nil {
		t.Errorf("external provisioners requires a license with entitlements. The client failed to fetch the entitlements, is this an enterprise instance of optimus-ide-collabd?")
		t.FailNow()
		return nil
	}

	feature := entitlements.Features[optimus-ide-collabsdk.FeatureExternalProvisionerDaemons]
	if !feature.Enabled || feature.Entitlement != optimus-ide-collabsdk.EntitlementEntitled {
	...
	daemon := provisionerd.New(func(ctx context.Context) (provisionerdproto.DRPCProvisionerDaemonClient, error) {
		return client.ServeProvisionerDaemon(ctx, optimus-ide-collabsdk.ServeProvisionerDaemonRequest{
			Name:         testutil.GetRandomName(t),
			Organization: org,
			Provisioners: []optimus-ide-collabsdk.ProvisionerType{provisionerType},
			Tags:         tags,
		})
	...
	closer := optimus-ide-collabdtest.NewProvisionerDaemonCloser(daemon)
	t.Cleanup(func() {
		_ = closer.Close()
	})
...
func MustRunReconciliationLoopForPreset(
	...
	preset optimus-ide-collabsdk.Preset,
