...
func TestStreamPartsDialerUsesConfiguredDialer(t *testing.T) {
	...
	chatID := uuid.New()
	workerID := uuid.New()
	headers := http.Header{optimus-ide-collabsdk.SessionTokenHeader: {"token-value"}}
	...
	require.Equal(t, "token-value", gotInput.RequestHeader.Get(optimus-ide-collabsdk.SessionTokenHeader))
}
...
func TestStreamPartsDialerDialsPartsEndpoint(t *testing.T) {
	...
	session, err := dialer(context.Background(), osschatd.StreamPartsDialInput{
		...
		RequestHeader: http.Header{
			optimus-ide-collabsdk.SessionTokenHeader: {"session-token"},
		},
	})
	require.NoError(t, err)
	...
	headers := <-received
	require.Equal(t, "session-token", headers.Get(optimus-ide-collabsdk.SessionTokenHeader))
