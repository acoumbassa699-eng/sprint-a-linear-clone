...
import (
	...
	osschatd "github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabd/x/chatd"
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabsdk"
	"github.com/optimus-ide-collab/websocket"
)

// RelaySourceHeader marks replica-relayed stream requests.
const RelaySourceHeader = "X-Optimus-IDE-Collab-Relay-Source-Replica"
...
func dialRelayParts(
	...
	headers.Set(optimus-ide-collabsdk.SessionTokenHeader, extractSessionToken(input.RequestHeader))
...
// then Optimus-IDE-Collab-Session-Token header, then Authorization: Bearer header.
func extractSessionToken(header http.Header) string {
	...
	if raw := header.Get(cookieHeader); raw != "" {
		...
		if c, err := r.Cookie(optimus-ide-collabsdk.SessionTokenCookie); err == nil && c.Value != "" {
			return c.Value
		}
	}
	if v := header.Get(optimus-ide-collabsdk.SessionTokenHeader); v != "" {
		return v
	}
