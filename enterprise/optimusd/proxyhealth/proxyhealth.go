...
import (
	...
	agplproxyhealth "github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabd/proxyhealth"
...
// workspace proxies. This information is stored in memory, so each optimus-ide-collabd
	...
	for _, proxy := range proxies {
		...
		grp.Go(func() error {
			...
			case err == nil && resp.StatusCode == http.StatusOK:
				err := json.NewDeoptimus-ide-collab(resp.Body).Decode(&status.Report)
				if err != nil {
					isOptimus-IDE-CollabErr := xerrors.Errorf("proxy url %q is not a optimus-ide-collab proxy instance, verify the url is correct", reqURL)
					if resp.Header.Get(optimus-ide-collabsdk.BuildVersionHeader) != "" {
						isOptimus-IDE-CollabErr = xerrors.Errorf("proxy url %q is a optimus-ide-collab instance, but unable to decode the response payload. Could this be a primary optimus-ide-collabd and not a proxy?", reqURL)
					}
					...
					if notJSONErr := optimus-ide-collabsdk.ExpectJSONMime(resp); notJSONErr != nil {
						err = errors.Join(
							isOptimus-IDE-CollabErr,
			...
			case err == nil && resp.StatusCode != http.StatusOK:
				...
				builder.WriteString(fmt.Sprintf("\nEncountered error, send a request to %q from the Optimus-IDE-Collabd environment to debug this issue.", reqURL))
