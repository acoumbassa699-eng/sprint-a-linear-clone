...
func TestCountWorkspaceCapableUsers(t *testing.T) {
	...
	t.Run("EntitlementsAddonGate", func(t *testing.T) {
		...
		experimentOn := optimus-ide-collabsdk.Experiments{optimus-ide-collabsdk.ExperimentWorkspaceCapableLicensing}

		// No license: legacy count, even with the experiment on.
		entitlements, err := license.Entitlements(ctx, testutil.Logger(t), db, 1, 1, optimus-ide-collabdenttest.Keys, enablements, authorizer, experimentOn)
		require.NoError(t, err)
		require.Equal(t, int64(2), *entitlements.Features[optimus-ide-collabsdk.FeatureUserLimit].Actual)
		...
		_, err = db.InsertLicense(ctx, database.InsertLicenseParams{
			JWT: optimus-ide-collabdenttest.GenerateLicense(t, optimus-ide-collabdenttest.LicenseOptions{
				Features: license.Features{optimus-ide-collabsdk.FeatureUserLimit: 100},
			}),
		...
		require.Equal(t, int64(1), *entitlements.Features[optimus-ide-collabsdk.FeatureUserLimit].Actual)

		// Addon present but experiment off: legacy count.
		entitlements, err = license.Entitlements(ctx, testutil.Logger(t), db, 1, 1, optimus-ide-collabdenttest.Keys, enablements, authorizer, nil)
		require.NoError(t, err)
		require.Equal(t, int64(2), *entitlements.Features[optimus-ide-collabsdk.FeatureUserLimit].Actual)

		// Addon present, experiment on, but no authorizer: fall back to the
		// legacy count instead of failing.
		entitlements, err = license.Entitlements(ctx, testutil.Logger(t), db, 1, 1, optimus-ide-collabdenttest.Keys, enablements, nil, experimentOn)
		require.NoError(t, err)
		require.Equal(t, int64(2), *entitlements.Features[optimus-ide-collabsdk.FeatureUserLimit].Actual)
	})
	...
	t.Run("LicensesEntitlementsCountFn", func(t *testing.T) {
		...
		dbLicense := func(opts optimus-ide-collabdenttest.LicenseOptions) database.License {
			return database.License{
				UUID: uuid.New(),
				JWT:  optimus-ide-collabdenttest.GenerateLicense(t, opts),
				Exp:  now.Add(time.Hour * 24 * 60),
			}
		}
		addonLicense := func() database.License {
			return dbLicense(*(&optimus-ide-collabdenttest.LicenseOptions{
				Features: license.Features{optimus-ide-collabsdk.FeatureUserLimit: 100},
			}).Valid(now).AIGovernanceAddon(10))
		}

		t.Run("NoAddonFnNotCalled", func(t *testing.T) {
			licenses := []database.License{dbLicense(*(&optimus-ide-collabdenttest.LicenseOptions{
				Features: license.Features{optimus-ide-collabsdk.FeatureUserLimit: 100},
			}).Valid(now))}
			entitlements, err := license.LicensesEntitlements(ctx, now, licenses, enablements, optimus-ide-collabdenttest.Keys, license.FeatureArguments{
				ActiveUserCount:  7,
			...
			require.NoError(t, err)
			require.Equal(t, int64(7), *entitlements.Features[optimus-ide-collabsdk.FeatureUserLimit].Actual)
		})
		...
		t.Run("AddonMissingDependenciesIgnored", func(t *testing.T) {
			...
			opts := (&optimus-ide-collabdenttest.LicenseOptions{
				Features: license.Features{optimus-ide-collabsdk.FeatureUserLimit: 100},
			}).Valid(now)
			opts.Addons = append(opts.Addons, optimus-ide-collabsdk.AddonAIGovernance)
			entitlements, err := license.LicensesEntitlements(ctx, now, []database.License{dbLicense(*opts)}, enablements, optimus-ide-collabdenttest.Keys, license.FeatureArguments{
				ActiveUserCount:  7,
				UserCountingMode: license.UserCountingModeWorkspaceCapable,
				WorkspaceCapableUserCountFn: func(context.Context) (int64, error) {
		...
		t.Run("ActiveModeIgnoresFn", func(t *testing.T) {
			...
			entitlements, err := license.LicensesEntitlements(ctx, now, []database.License{addonLicense()}, enablements, optimus-ide-collabdenttest.Keys, license.FeatureArguments{
				ActiveUserCount: 7,
				WorkspaceCapableUserCountFn: func(context.Context) (int64, error) {
			...
			require.NoError(t, err)
			require.Equal(t, int64(7), *entitlements.Features[optimus-ide-collabsdk.FeatureUserLimit].Actual)
			require.Equal(t, int64(100), *entitlements.Features[optimus-ide-collabsdk.FeatureUserLimit].Limit)
		})
		...
		t.Run("AddonUsesFn", func(t *testing.T) {
			...
			require.NoError(t, err)
			require.Empty(t, entitlements.Errors)
			require.Equal(t, int64(3), *entitlements.Features[optimus-ide-collabsdk.FeatureUserLimit].Actual)
			// Permission-based counting applies to workspace seats only:
			// AI Governance seats keep their own count and limit.
			aiSeats := entitlements.Features[optimus-ide-collabsdk.FeatureAIGovernanceUserLimit]
		...
		t.Run("BestPairSelection", func(t *testing.T) {
			...
			licenses := []database.License{
				dbLicense(*(&optimus-ide-collabdenttest.LicenseOptions{
					Features: license.Features{optimus-ide-collabsdk.FeatureUserLimit: 200},
				}).Valid(now)),
				addonLicense(), // user_limit 100, AI Governance addon.
			}
			run := func(t *testing.T, activeUsers, capableUsers int64) optimus-ide-collabsdk.Entitlements {
				entitlements, err := license.LicensesEntitlements(ctx, now, licenses, enablements, optimus-ide-collabdenttest.Keys, license.FeatureArguments{
			...
			t.Run("LegacyPairCompliant", func(t *testing.T) {
				...
				require.Equal(t, int64(180), *entitlements.Features[optimus-ide-collabsdk.FeatureUserLimit].Actual)
				require.Equal(t, int64(200), *entitlements.Features[optimus-ide-collabsdk.FeatureUserLimit].Limit)
				for _, warning := range entitlements.Warnings {
					require.NotContains(t, warning, "users but")
				}
			})
			...
			t.Run("AddonPairCompliant", func(t *testing.T) {
				...
				require.Equal(t, int64(90), *entitlements.Features[optimus-ide-collabsdk.FeatureUserLimit].Actual)
				require.Equal(t, int64(100), *entitlements.Features[optimus-ide-collabsdk.FeatureUserLimit].Limit)
				for _, warning := range entitlements.Warnings {
					require.NotContains(t, warning, "users but")
				}
			...
			t.Run("NeitherPairCompliant", func(t *testing.T) {
				...
				require.Equal(t, int64(250), *entitlements.Features[optimus-ide-collabsdk.FeatureUserLimit].Actual)
				require.Equal(t, int64(200), *entitlements.Features[optimus-ide-collabsdk.FeatureUserLimit].Limit)
			...
			t.Run("GraceAddonCompliantBeatsEntitledOver", func(t *testing.T) {
				...
				require.Equal(t, int64(90), *entitlements.Features[optimus-ide-collabsdk.FeatureUserLimit].Actual)
				require.Equal(t, int64(100), *entitlements.Features[optimus-ide-collabsdk.FeatureUserLimit].Limit)
				require.Equal(t, optimus-ide-collabsdk.EntitlementGracePeriod, entitlements.Features[optimus-ide-collabsdk.FeatureUserLimit].Entitlement)
				require.Contains(t, entitlements.Warnings,
			...
			t.Run("EqualLimitsPreferAddon", func(t *testing.T) {
				...
				require.NoError(t, err)
				require.Equal(t, int64(30), *entitlements.Features[optimus-ide-collabsdk.FeatureUserLimit].Actual)
				require.Equal(t, int64(100), *entitlements.Features[optimus-ide-collabsdk.FeatureUserLimit].Limit)
			})
			...
			t.Run("TwoAddonCandidates", func(t *testing.T) {
				...
				licenses := []database.License{
					...
					dbLicense(*(&optimus-ide-collabdenttest.LicenseOptions{
						Features: license.Features{optimus-ide-collabsdk.FeatureUserLimit: 300},
					}).Valid(now).AIGovernanceAddon(10)),
				}
				entitlements, err := license.LicensesEntitlements(ctx, now, licenses, enablements, optimus-ide-collabdenttest.Keys, license.FeatureArguments{
					ActiveUserCount:  500,
				...
				require.NoError(t, err)
				require.Equal(t, int64(150), *entitlements.Features[optimus-ide-collabsdk.FeatureUserLimit].Actual)
				require.Equal(t, int64(300), *entitlements.Features[optimus-ide-collabsdk.FeatureUserLimit].Limit)
				require.Equal(t, optimus-ide-collabsdk.EntitlementEntitled, entitlements.Features[optimus-ide-collabsdk.FeatureUserLimit].Entitlement)
				for _, warning := range entitlements.Warnings {
					require.NotContains(t, warning, "fully expires")
					require.NotContains(t, warning, "users but")
				}
		...
		t.Run("ModeWithoutFnIsDevError", func(t *testing.T) {
			_, err := license.LicensesEntitlements(ctx, now, []database.License{addonLicense()}, enablements, optimus-ide-collabdenttest.Keys, license.FeatureArguments{
				ActiveUserCount:  7,
				UserCountingMode: license.UserCountingModeWorkspaceCapable,
			})
