...
func TestCollectLicenseMetrics(t *testing.T) {
	...
	sut.Entitlements.Modify(func(entitlements *optimus-ide-collabsdk.Entitlements) {
		entitlements.Features[optimus-ide-collabsdk.FeatureUserLimit] = optimus-ide-collabsdk.Feature{
			Enabled: true,
...
func TestCollectLicenseMetrics_WarningsAndErrors(t *testing.T) {
	...
	t.Run("NoWarningsOrErrors", func(t *testing.T) {
		...
		warnings, ok := findMetric(metrics, "optimus-ide-collabd_license_warnings")
		require.True(t, ok)
		require.Zero(t, warnings)

		errors, ok := findMetric(metrics, "optimus-ide-collabd_license_errors")
		require.True(t, ok)
