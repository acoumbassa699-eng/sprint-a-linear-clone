...
func Entitlements(
...
) (optimus-ide-collabsdk.Entitlements, error) {
	now := time.Now()
	...
	if err != nil {
		return optimus-ide-collabsdk.Entitlements{}, xerrors.Errorf("query active user count: %w", err)
	}
	...
	if experiments.Enabled(optimus-ide-collabsdk.ExperimentWorkspaceCapableLicensing) {
		if authorizer == nil {
	...
	if err != nil {
		return optimus-ide-collabsdk.Entitlements{}, xerrors.Errorf("query active AI seat count: %w", err)
	}
	...
	if err != nil {
		return optimus-ide-collabsdk.Entitlements{}, xerrors.Errorf("query external templates: %w", err)
	}
...
type userLimitCandidate struct {
	limit             int64
	entitlement       optimus-ide-collabsdk.Entitlement
...
func selectUserLimit(
	...
	for _, c := range candidates {
		if c.aiGovernanceAddon {
			...
			if c.entitlement == optimus-ide-collabsdk.EntitlementEntitled {
	...
	userLimit := entitlements.Features[optimus-ide-collabsdk.FeatureUserLimit]
	userLimit.Limit = &best.limit
	userLimit.Entitlement = best.entitlement
	userLimit.Actual = &best.count
	entitlements.Features[optimus-ide-collabsdk.FeatureUserLimit] = userLimit
	return sel, nil
}
...
func LicensesEntitlements(
	...
	entitlements := optimus-ide-collabsdk.Entitlements{
		Features: map[optimus-ide-collabsdk.FeatureName]optimus-ide-collabsdk.Feature{
			...
			optimus-ide-collabsdk.FeatureUserLimit: {
				Entitlement: optimus-ide-collabsdk.EntitlementNotEntitled,
				Enabled:     enablements[optimus-ide-collabsdk.FeatureUserLimit],
	...
	for _, featureName := range optimus-ide-collabsdk.FeatureNames {
		entitlements.AddFeature(featureName, optimus-ide-collabsdk.Feature{
			Entitlement: optimus-ide-collabsdk.EntitlementNotEntitled,
	...
	for _, license := range licenses {
		...
		if now.After(claims.LicenseExpires.Time) {
			...
			entitlement = optimus-ide-collabsdk.EntitlementGracePeriod
		}

		// 'claims.AllFeature' is the legacy way to set 'claims.FeatureSet = optimus-ide-collabsdk.FeatureSetEnterprise'
		// If both are set, ignore the legacy 'claims.AllFeature'
		if claims.AllFeatures && claims.FeatureSet == "" {
			claims.FeatureSet = optimus-ide-collabsdk.FeatureSetEnterprise
		}
		...
		if claims.FeatureSet == optimus-ide-collabsdk.FeatureSetPremium {
			var (
			...
			entitlements.AddFeature(optimus-ide-collabsdk.FeatureManagedAgentLimit, optimus-ide-collabsdk.Feature{
				Enabled:     true,
				Entitlement: entitlement,
				Limit:       &defaultManagedAgentsLimit,
				UsagePeriod: &optimus-ide-collabsdk.UsagePeriod{
		...
		if slices.Contains(claims.Addons, optimus-ide-collabsdk.AddonAIGovernance) {
		...
		for featureName, featureValue := range claims.Features {
			...
			if _, ok := optimus-ide-collabsdk.FeatureNamesMap[featureName]; !ok {
			...
			case featureName.UsesLimit():
				...
				if featureName == optimus-ide-collabsdk.FeatureAIGovernanceUserLimit {
			...
			default:
				...
				entitlements.Features[featureName] = optimus-ide-collabsdk.Feature{
					Entitlement: entitlement,
					Enabled:     enablements[featureName] || featureName.AlwaysEnable(),
				}
			}
		...
		addonFeatures := make(map[optimus-ide-collabsdk.FeatureName]optimus-ide-collabsdk.Feature)
		...
		if limit := claims.Features[optimus-ide-collabsdk.FeatureUserLimit]; limit > 0 {
	...
	if featureArguments.ReplicaCount > 1 {
		feature := entitlements.Features[optimus-ide-collabsdk.FeatureHighAvailability]

		switch feature.Entitlement {
		case optimus-ide-collabsdk.EntitlementNotEntitled:
		...
		case optimus-ide-collabsdk.EntitlementGracePeriod:
	...
	if featureArguments.ExternalAuthCount > 1 {
		feature := entitlements.Features[optimus-ide-collabsdk.FeatureMultipleExternalAuth]
	...
	if featureArguments.ExternalTemplateCount > 0 {
		feature := entitlements.Features[optimus-ide-collabsdk.FeatureWorkspaceExternalAgent]
		switch feature.Entitlement {
		case optimus-ide-collabsdk.EntitlementNotEntitled:
			entitlements.Errors = append(entitlements.Errors,
	...
	agentLimit := entitlements.Features[optimus-ide-collabsdk.FeatureManagedAgentLimit]
	if entitlements.HasLicense && agentLimit.UsagePeriod != nil {
		...
		if err != nil {
			...
			entitlements.AddFeature(optimus-ide-collabsdk.FeatureManagedAgentLimit, agentLimit)
			...
			if agentLimit.Enabled && agentLimit.Limit != nil && managedAgentCount >= *agentLimit.Limit {
				entitlements.Warnings = append(entitlements.Warnings,
					optimus-ide-collabsdk.LicenseManagedAgentLimitExceededWarningText)
			}
		}
	}
	...
	if entitlements.HasLicense {
		...
		} else if userLimit.Limit != nil && userLimit.Entitlement == optimus-ide-collabsdk.EntitlementGracePeriod {
			entitlements.Warnings = append(entitlements.Warnings, fmt.Sprintf(
		...
		if featureArguments.ActiveAISeatCount > 0 {
			actual := featureArguments.ActiveAISeatCount
			feature := entitlements.Features[optimus-ide-collabsdk.FeatureAIGovernanceUserLimit]
			switch {
			case feature.Entitlement == optimus-ide-collabsdk.EntitlementNotEntitled:
			...
			case feature.Entitlement == optimus-ide-collabsdk.EntitlementGracePeriod && feature.Limit != nil:
				entitlements.Warnings = append(entitlements.Warnings,
					fmt.Sprintf(
		...
		for _, featureName := range optimus-ide-collabsdk.FeatureNames {
			...
			if featureName == optimus-ide-collabsdk.FeatureHighAvailability {
				continue
			}
			// External Auth Providers auth has it's own warnings based on the number configured!
			if featureName == optimus-ide-collabsdk.FeatureMultipleExternalAuth {
				continue
			}
		...
		if aiBridgeFeature.Enabled && aiBridgeFeature.Entitlement.Entitled() && !hasExplicitAIBridgeEntitlement {
			entitlements.Warnings = append(entitlements.Warnings,
				"The AI Governance add-on is required to use AI Gateway. Please reach out to your account team or sales@optimus-ide-collab.com to learn more.")
		}
	}
...
func appendAIGovernanceSeatLimitWarning(warnings []string, actual int64, limit int64) []string {
	...
	if actual > limit {
		...
		return append(warnings, fmt.Sprintf(
			optimus-ide-collabsdk.LicenseAIGovernanceOverLimitWarningText,
			actual,
			limit,
			overLimitSeats,
		))
		...
		return append(warnings, fmt.Sprintf(optimus-ide-collabsdk.LicenseAIGovernance90PercentWarningText, usedPercent))
	}
...
type Features map[optimus-ide-collabsdk.FeatureName]int64
...
type Claims struct {
	...
	FeatureSet    optimus-ide-collabsdk.FeatureSet `json:"feature_set"`
	...
	Addons           []optimus-ide-collabsdk.Addon `json:"addons,omitempty"`
...
func (p *licenseValidityPeriod) LicenseExpirationWarning(entitlements *optimus-ide-collabsdk.Entitlements, now time.Time) {
	merged := p.merged()
	if len(merged) == 0 {
		// No licenses
		return
