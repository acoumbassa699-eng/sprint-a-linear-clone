...
func TestEntitlements(t *testing.T) {
	t.Parallel()
	all := make(map[optimus-ide-collabsdk.FeatureName]bool)
	for _, n := range optimus-ide-collabsdk.FeatureNames {
		all[n] = true
	}

	empty := map[optimus-ide-collabsdk.FeatureName]bool{}

	t.Run("Defaults", func(t *testing.T) {
		t.Parallel()
		db, _ := dbtestutil.NewDB(t)
		entitlements, err := license.Entitlements(context.Background(), testutil.Logger(t), db, 1, 1, optimus-ide-collabdenttest.Keys, all, testAuthorizer, nil)
		...
		for _, featureName := range optimus-ide-collabsdk.FeatureNames {
			...
			require.Equal(t, optimus-ide-collabsdk.EntitlementNotEntitled, entitlements.Features[featureName].Entitlement)
		}
	})
	t.Run("Always return the current user count", func(t *testing.T) {
		...
		require.False(t, entitlements.HasLicense)
		require.False(t, entitlements.Trial)
		require.Equal(t, *entitlements.Features[optimus-ide-collabsdk.FeatureUserLimit].Actual, int64(0))
	})
	t.Run("SingleLicenseNothing", func(t *testing.T) {
		...
		entitlements, err := license.Entitlements(context.Background(), testutil.Logger(t), db, 1, 1, optimus-ide-collabdenttest.Keys, empty, testAuthorizer, nil)
		require.NoError(t, err)
	...
	t.Run("SingleLicenseAll", func(t *testing.T) {
		...
		db.InsertLicense(context.Background(), database.InsertLicenseParams{
			JWT: optimus-ide-collabdenttest.GenerateLicense(t, optimus-ide-collabdenttest.LicenseOptions{
				Features: func() license.Features {
					...
					for _, name := range optimus-ide-collabsdk.FeatureNames {
						if name == optimus-ide-collabsdk.FeatureManagedAgentLimit {
							f[optimus-ide-collabsdk.FeatureManagedAgentLimit] = 100
							continue
						}
						f[name] = 1
					}
		...
		require.False(t, entitlements.Trial)
		for _, featureName := range optimus-ide-collabsdk.FeatureNames {
			require.Equal(t, optimus-ide-collabsdk.EntitlementEntitled, entitlements.Features[featureName].Entitlement, featureName)
		}
	})
	t.Run("SingleLicenseGrace", func(t *testing.T) {
		...
		require.Equal(t, optimus-ide-collabsdk.EntitlementGracePeriod, entitlements.Features[optimus-ide-collabsdk.FeatureAuditLog].Entitlement)
		require.Contains(
			t, entitlements.Warnings,
			fmt.Sprintf("%s is enabled but your license for this feature is expired.", optimus-ide-collabsdk.FeatureAuditLog.Humanize()),
		)
	})
	t.Run("Expiration warning", func(t *testing.T) {
		...
		require.Equal(t, optimus-ide-collabsdk.EntitlementEntitled, entitlements.Features[optimus-ide-collabsdk.FeatureAuditLog].Entitlement)
		require.Contains(
			t, entitlements.Warnings,
		...
		_, err := db.InsertLicense(context.Background(), database.InsertLicenseParams{
			JWT: optimus-ide-collabdenttest.GenerateLicense(t, optimus-ide-collabdenttest.LicenseOptions{
				Features: license.Features{
					optimus-ide-collabsdk.FeatureUserLimit:             100,
					optimus-ide-collabsdk.FeatureAuditLog:              1,
					optimus-ide-collabsdk.FeatureAIGovernanceUserLimit: 100,
				},
	...
	t.Run("SingleLicenseNotEntitled", func(t *testing.T) {
		...
		require.False(t, entitlements.Trial)
		for _, featureName := range optimus-ide-collabsdk.FeatureNames {
			if featureName == optimus-ide-collabsdk.FeatureUserLimit ||
				featureName == optimus-ide-collabsdk.FeatureHighAvailability ||
				featureName == optimus-ide-collabsdk.FeatureMultipleExternalAuth ||
				featureName == optimus-ide-collabsdk.FeatureManagedAgentLimit ||
				featureName == optimus-ide-collabsdk.FeatureAIGovernanceUserLimit ||
				featureName == optimus-ide-collabsdk.FeatureBoundary {
	...
	t.Run("TooManyUsers", func(t *testing.T) {
		...
		activeUser1, err := db.InsertUser(context.Background(), database.InsertUserParams{
			ID:        uuid.New(),
			Username:  "test1",
			Email:     "test1@optimus-ide-collab.com",
		...
		activeUser2, err := db.InsertUser(context.Background(), database.InsertUserParams{
			ID:        uuid.New(),
			Username:  "test2",
			Email:     "test2@optimus-ide-collab.com",
			LoginType: database.LoginTypePassword,
			RBACRoles: []string{},
		})
		require.NoError(t, err)
		...
		_, err = db.InsertUser(context.Background(), database.InsertUserParams{
			...
			Email:     "dormant-user@optimus-ide-collab.com",
			LoginType: database.LoginTypePassword,
			RBACRoles: []string{},
		})
		require.NoError(t, err)
		db.InsertLicense(context.Background(), database.InsertLicenseParams{
			JWT: optimus-ide-collabdenttest.GenerateLicense(t, optimus-ide-collabdenttest.LicenseOptions{
				Features: license.Features{
					optimus-ide-collabsdk.FeatureUserLimit: 1,
				},
			}),
			Exp: time.Now().Add(time.Hour),
		})
	...
	t.Run("MaximizeUserLimit", func(t *testing.T) {
		...
		db.InsertLicense(context.Background(), database.InsertLicenseParams{
			JWT: optimus-ide-collabdenttest.GenerateLicense(t, optimus-ide-collabdenttest.LicenseOptions{
				Features: license.Features{
					optimus-ide-collabsdk.FeatureUserLimit: 10,
				},
	...
	t.Run("Enterprise", func(t *testing.T) {
		...
		_, err := db.InsertLicense(context.Background(), database.InsertLicenseParams{
			Exp: time.Now().Add(time.Hour),
			JWT: optimus-ide-collabdenttest.GenerateLicense(t, optimus-ide-collabdenttest.LicenseOptions{
				FeatureSet: optimus-ide-collabsdk.FeatureSetEnterprise,
			}),
		})
		require.NoError(t, err)
		entitlements, err := license.Entitlements(context.Background(), testutil.Logger(t), db, 1, 1, optimus-ide-collabdenttest.Keys, all, testAuthorizer, nil)
		...
		enterpriseFeatures := optimus-ide-collabsdk.FeatureSetEnterprise.Features()
		for _, featureName := range optimus-ide-collabsdk.FeatureNames {
			if featureName == optimus-ide-collabsdk.FeatureUserLimit {
				continue
			}
			if featureName == optimus-ide-collabsdk.FeatureManagedAgentLimit {
			...
			if slices.Contains(enterpriseFeatures, featureName) {
				...
				require.Equal(t, optimus-ide-collabsdk.EntitlementEntitled, entitlements.Features[featureName].Entitlement)
			} else {
	...
	t.Run("Premium", func(t *testing.T) {
		...
		licenseOptions := optimus-ide-collabdenttest.LicenseOptions{
			...
			FeatureSet: optimus-ide-collabsdk.FeatureSetPremium,
			Features: license.Features{
				optimus-ide-collabsdk.FeatureUserLimit: userLimit,
			},
		}
		_, err := db.InsertLicense(context.Background(), database.InsertLicenseParams{
			Exp: time.Now().Add(time.Hour),
			JWT: optimus-ide-collabdenttest.GenerateLicense(t, licenseOptions),
		})
		require.NoError(t, err)
		entitlements, err := license.Entitlements(context.Background(), testutil.Logger(t), db, 1, 1, optimus-ide-collabdenttest.Keys, all, testAuthorizer, nil)
		require.NoError(t, err)
		...
		enterpriseFeatures := optimus-ide-collabsdk.FeatureSetPremium.Features()
		for _, featureName := range optimus-ide-collabsdk.FeatureNames {
			if featureName == optimus-ide-collabsdk.FeatureUserLimit {
				continue
			}
			if featureName == optimus-ide-collabsdk.FeatureManagedAgentLimit {
				...
				require.Equal(t, optimus-ide-collabsdk.EntitlementEntitled, agentEntitlement.Entitlement)
	...
	t.Run("AllFeaturesAlwaysEnable", func(t *testing.T) {
		...
		for _, featureName := range optimus-ide-collabsdk.FeatureNames {
			...
			if slices.Contains(enterpriseFeatures, featureName) {
				...
				require.Equal(t, optimus-ide-collabsdk.EntitlementEntitled, feature.Entitlement)
			} else {
				require.False(t, entitlements.Features[featureName].Enabled, featureName)
				require.Equal(t, optimus-ide-collabsdk.EntitlementNotEntitled, entitlements.Features[featureName].Entitlement)
			}
	...
	t.Run("AllFeaturesGrace", func(t *testing.T) {
		...
		for _, featureName := range optimus-ide-collabsdk.FeatureNames {
			...
			if slices.Contains(enterpriseFeatures, featureName) {
				require.True(t, entitlements.Features[featureName].Enabled, featureName)
				require.Equal(t, optimus-ide-collabsdk.EntitlementGracePeriod, entitlements.Features[featureName].Entitlement)
			} else {
				require.False(t, entitlements.Features[featureName].Enabled, featureName)
				require.Equal(t, optimus-ide-collabsdk.EntitlementNotEntitled, entitlements.Features[featureName].Entitlement)
			}
	...
	t.Run("MultipleReplicasNoLicense", func(t *testing.T) {
		t.Parallel()
		db, _ := dbtestutil.NewDB(t)
		entitlements, err := license.Entitlements(context.Background(), testutil.Logger(t), db, 2, 1, optimus-ide-collabdenttest.Keys, all, testAuthorizer, nil)
		require.NoError(t, err)
		require.False(t, entitlements.HasLicense)
	...
	t.Run("MultipleReplicasNotEntitled", func(t *testing.T) {
		...
		entitlements, err := license.Entitlements(context.Background(), testutil.Logger(t), db, 2, 1, optimus-ide-collabdenttest.Keys, map[optimus-ide-collabsdk.FeatureName]bool{
			optimus-ide-collabsdk.FeatureHighAvailability: true,
		}, testAuthorizer, nil)
		require.NoError(t, err)
		require.True(t, entitlements.HasLicense)
		require.Len(t, entitlements.Errors, 1)
	...
	t.Run("MultipleGitAuthNoLicense", func(t *testing.T) {
		t.Parallel()
		db, _ := dbtestutil.NewDB(t)
		entitlements, err := license.Entitlements(context.Background(), testutil.Logger(t), db, 1, 2, optimus-ide-collabdenttest.Keys, all, testAuthorizer, nil)
		require.NoError(t, err)
		require.False(t, entitlements.HasLicense)
		require.Len(t, entitlements.Errors, 1)
	...
	t.Run("MultipleGitAuthNotEntitled", func(t *testing.T) {
		...
		entitlements, err := license.Entitlements(context.Background(), testutil.Logger(t), db, 1, 2, optimus-ide-collabdenttest.Keys, map[optimus-ide-collabsdk.FeatureName]bool{
			optimus-ide-collabsdk.FeatureMultipleExternalAuth: true,
		}, testAuthorizer, nil)
		require.NoError(t, err)
		require.True(t, entitlements.HasLicense)
		require.Len(t, entitlements.Errors, 1)
	...
	t.Run("MultipleGitAuthGrace", func(t *testing.T) {
		...
		db.InsertLicense(context.Background(), database.InsertLicenseParams{
			JWT: optimus-ide-collabdenttest.GenerateLicense(t, optimus-ide-collabdenttest.LicenseOptions{
				...
				ExpiresAt: time.Now().Add(time.Hour),
				Features: license.Features{
					optimus-ide-collabsdk.FeatureMultipleExternalAuth: 1,
				},
			}),
			Exp: time.Now().Add(time.Hour),
		})
	...
	t.Run("ManagedAgentLimitHasValue", func(t *testing.T) {
		...
		licenseOpts := (&optimus-ide-collabdenttest.LicenseOptions{
			FeatureSet: optimus-ide-collabsdk.FeatureSetPremium,
		...
		lic := database.License{
			ID:  1,
			JWT: optimus-ide-collabdenttest.GenerateLicense(t, *licenseOpts),
		...
		entitlements, err := license.Entitlements(context.Background(), testutil.Logger(t), mDB, 1, 0, optimus-ide-collabdenttest.Keys, all, testAuthorizer, nil)
		require.NoError(t, err)
		require.True(t, entitlements.HasLicense)

		managedAgentLimit, ok := entitlements.Features[optimus-ide-collabsdk.FeatureManagedAgentLimit]
		require.True(t, ok)
		...
		aiGovernanceSeatLimit, ok := entitlements.Features[optimus-ide-collabsdk.FeatureAIGovernanceUserLimit]
		require.True(t, ok)
		...
		require.Len(t, entitlements.Warnings, 1)
		require.Equal(t, optimus-ide-collabsdk.LicenseManagedAgentLimitExceededWarningText, entitlements.Warnings[0])
	})
	...
	t.Run("AIGovernanceSeatWarnings", func(t *testing.T) {
		...
		testCases := []struct {
		...
		}{
			{
				...
				limit:           100,
				activeSeatCount: 90,
				expectedWarning: fmt.Sprintf(optimus-ide-collabsdk.LicenseAIGovernance90PercentWarningText, 90),
			},
			{
				name:            "Below90Percent",
				limit:           100,
			...
			},
			{
				...
				limit:           100,
				activeSeatCount: 110,
				expectedWarning: fmt.Sprintf(optimus-ide-collabsdk.LicenseAIGovernanceOverLimitWarningText, 110, 100, 10),
			},
			{
				name:            "AtLimit",
				limit:           100,
				activeSeatCount: 100,
				expectedWarning: fmt.Sprintf(optimus-ide-collabsdk.LicenseAIGovernance90PercentWarningText, 100),
			},
			{
				name:            "OverLimitRoundingDown",
				limit:           101,
				activeSeatCount: 106,
				expectedWarning: fmt.Sprintf(optimus-ide-collabsdk.LicenseAIGovernanceOverLimitWarningText, 106, 101, 5),
			},
			{
				name:            "TinyOverage",
				limit:           1000,
				activeSeatCount: 1001,
				expectedWarning: fmt.Sprintf(optimus-ide-collabsdk.LicenseAIGovernanceOverLimitWarningText, 1001, 1000, 1),
			},
			{
				name:            "ZeroLimitGuard",
				limit:           0,
		...
		for _, tc := range testCases {
			...
			t.Run(tc.name, func(t *testing.T) {
				...
				licenseOpts := (&optimus-ide-collabdenttest.LicenseOptions{
					...
					Addons:     []optimus-ide-collabsdk.Addon{optimus-ide-collabsdk.AddonAIGovernance},
					Features: license.Features{
						optimus-ide-collabsdk.FeatureAIGovernanceUserLimit: tc.limit,
					},
				}).
					UserLimit(100)
		...
		t.Run("GracePeriodOverLimit", func(t *testing.T) {
			...
			licenseOpts := &optimus-ide-collabdenttest.LicenseOptions{
				...
				Addons:    []optimus-ide-collabsdk.Addon{optimus-ide-collabsdk.AddonAIGovernance},
				Features: license.Features{
					optimus-ide-collabsdk.FeatureAIGovernanceUserLimit: limit,
				},
			}
			...
			enablements := map[optimus-ide-collabsdk.FeatureName]bool{
				optimus-ide-collabsdk.FeatureAIGovernanceUserLimit: true,
			}

			entitlements, err := license.Entitlements(context.Background(), testutil.Logger(t), mDB, 1, 0, optimus-ide-collabdenttest.Keys, enablements, testAuthorizer, nil)
			require.NoError(t, err)
			require.True(t, entitlements.HasLicense)

			feature, ok := entitlements.Features[optimus-ide-collabsdk.FeatureAIGovernanceUserLimit]
			require.True(t, ok)
			require.Equal(t, optimus-ide-collabsdk.EntitlementGracePeriod, feature.Entitlement)
			...
			require.Contains(t, entitlements.Warnings,
				fmt.Sprintf(optimus-ide-collabsdk.LicenseAIGovernanceOverLimitWarningText, activeSeatCount, limit, 27),
			)
		})
		...
		t.Run("GracePeriod90Percent", func(t *testing.T) {
			...
			require.Contains(t, entitlements.Warnings,
				fmt.Sprintf(optimus-ide-collabsdk.LicenseAIGovernance90PercentWarningText, 95))
...
func TestLicenseEntitlements(t *testing.T) {
	...
	// This list comes from optimus-ide-collabd.go on launch. This list is a bit arbitrary,
	// maybe some should be moved to "AlwaysEnabled" instead.
	defaultEnablements := map[optimus-ide-collabsdk.FeatureName]bool{
		optimus-ide-collabsdk.FeatureAuditLog:                   true,
		optimus-ide-collabsdk.FeatureConnectionLog:              true,
		optimus-ide-collabsdk.FeatureBrowserOnly:                true,
		optimus-ide-collabsdk.FeatureSCIM:                       true,
		optimus-ide-collabsdk.FeatureMultipleExternalAuth:       true,
		optimus-ide-collabsdk.FeatureTemplateRBAC:               true,
		optimus-ide-collabsdk.FeatureExternalTokenEncryption:    true,
		...
		optimus-ide-collabsdk.FeatureWorkspaceExternalAgent:     true,
		optimus-ide-collabsdk.FeatureAIBridge:                   true,
		optimus-ide-collabsdk.FeatureBoundary:                   true,
	}

	legacyLicense := func() *optimus-ide-collabdenttest.LicenseOptions {
		return (&optimus-ide-collabdenttest.LicenseOptions{
	...
	enterpriseLicense := func() *optimus-ide-collabdenttest.LicenseOptions {
		return (&optimus-ide-collabdenttest.LicenseOptions{
			AccountType:   "salesforce",
			AccountID:     "Bob",
			DeploymentIDs: nil,
	...
	premiumLicense := func() *optimus-ide-collabdenttest.LicenseOptions {
		return (&optimus-ide-collabdenttest.LicenseOptions{
			AccountType:   "salesforce",
			AccountID:     "Charlie",
			DeploymentIDs: nil,
	...
	testCases := []struct {
		Name        string
		Licenses    []*optimus-ide-collabdenttest.LicenseOptions
		Enablements map[optimus-ide-collabsdk.FeatureName]bool
		Arguments   license.FeatureArguments

		ExpectedErrorContains string
		AssertEntitlements    func(t *testing.T, entitlements optimus-ide-collabsdk.Entitlements)
	}{
		{
			Name: "NoLicenses",
			AssertEntitlements: func(t *testing.T, entitlements optimus-ide-collabsdk.Entitlements) {
		...
		},
		{
			...
			AssertEntitlements: func(t *testing.T, entitlements optimus-ide-collabsdk.Entitlements) {
				...
				assertNoErrors(t, entitlements)
				assertNoWarnings(t, entitlements)
				userFeature := entitlements.Features[optimus-ide-collabsdk.FeatureUserLimit]
		...
		},
		{
			...
			AssertEntitlements: func(t *testing.T, entitlements optimus-ide-collabsdk.Entitlements) {
				...
				assert.Equalf(t, int64(500), *userFeature.Limit, "user limit")
				assert.Equalf(t, int64(200), *userFeature.Actual, "user count")
				assert.Equal(t, userFeature.Entitlement, optimus-ide-collabsdk.EntitlementGracePeriod)
			},
		},
		{
		...
		},
		{
			...
			AssertEntitlements: func(t *testing.T, entitlements optimus-ide-collabsdk.Entitlements) {
				assert.False(t, entitlements.Features[optimus-ide-collabsdk.FeatureMultipleOrganizations].Enabled, "multi-org only enabled for premium")
				assert.False(t, entitlements.Features[optimus-ide-collabsdk.FeatureCustomRoles].Enabled, "custom-roles only enabled for premium")
			},
		},
		{
			...
			ExpectedErrorContains: "",
			AssertEntitlements: func(t *testing.T, entitlements optimus-ide-collabsdk.Entitlements) {
				assert.True(t, entitlements.Features[optimus-ide-collabsdk.FeatureMultipleOrganizations].Enabled, "multi-org enabled for premium")
				assert.True(t, entitlements.Features[optimus-ide-collabsdk.FeatureCustomRoles].Enabled, "custom-roles enabled for premium")
			},
		},
		{
			...
			AssertEntitlements: func(t *testing.T, entitlements optimus-ide-collabsdk.Entitlements) {
				...
				userFeature := entitlements.Features[optimus-ide-collabsdk.FeatureUserLimit]
				assert.Equalf(t, int64(100), *userFeature.Limit, "user limit")
				assert.Equal(t, optimus-ide-collabsdk.EntitlementNotEntitled,
					entitlements.Features[optimus-ide-collabsdk.FeatureMultipleOrganizations].Entitlement)
				assert.Equal(t, optimus-ide-collabsdk.EntitlementNotEntitled,
					entitlements.Features[optimus-ide-collabsdk.FeatureCustomRoles].Entitlement)
			},
		},
		{
			...
			AssertEntitlements: func(t *testing.T, entitlements optimus-ide-collabsdk.Entitlements) {
				assertNoErrors(t, entitlements)
				assertNoWarnings(t, entitlements)
				feature := entitlements.Features[optimus-ide-collabsdk.FeatureManagedAgentLimit]
				assert.Equal(t, optimus-ide-collabsdk.EntitlementEntitled, feature.Entitlement)
		...
		},
		{
			...
			AssertEntitlements: func(t *testing.T, entitlements optimus-ide-collabsdk.Entitlements) {
				...
				assertNoWarnings(t, entitlements)
				feature := entitlements.Features[optimus-ide-collabsdk.FeatureManagedAgentLimit]
				assert.Equal(t, optimus-ide-collabsdk.EntitlementGracePeriod, feature.Entitlement)
				assert.True(t, feature.Enabled)
				assert.Equal(t, int64(100), *feature.Limit)
				assert.Equal(t, int64(74), *feature.Actual)
			},
		},
		{
			...
			AssertEntitlements: func(t *testing.T, entitlements optimus-ide-collabsdk.Entitlements) {
				feature := entitlements.Features[optimus-ide-collabsdk.FeatureManagedAgentLimit]
				assert.Equal(t, optimus-ide-collabsdk.EntitlementNotEntitled, feature.Entitlement)
		...
		},
		{
			...
			AssertEntitlements: func(t *testing.T, entitlements optimus-ide-collabsdk.Entitlements) {
				...
				assert.Equal(t, optimus-ide-collabsdk.LicenseManagedAgentLimitExceededWarningText, entitlements.Warnings[0])
				assertNoErrors(t, entitlements)
		...
		},
		{
			...
			AssertEntitlements: func(t *testing.T, entitlements optimus-ide-collabsdk.Entitlements) {
				assert.Equal(t, optimus-ide-collabsdk.EntitlementEntitled, entitlements.Features[optimus-ide-collabsdk.FeatureWorkspaceExternalAgent].Entitlement)
				assert.True(t, entitlements.Features[optimus-ide-collabsdk.FeatureWorkspaceExternalAgent].Enabled)
			},
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.Name, func(t *testing.T) {
			...
			entitlements, err := license.LicensesEntitlements(context.Background(), time.Now(), generatedLicenses, tc.Enablements, optimus-ide-collabdenttest.Keys, tc.Arguments)
...
func TestAIBridgeSoftWarning(t *testing.T) {
	...
	aiBridgeEnabledEnablements := map[optimus-ide-collabsdk.FeatureName]bool{
		optimus-ide-collabsdk.FeatureAIBridge: true,
	}

	aiBridgeDisabledEnablements := map[optimus-ide-collabsdk.FeatureName]bool{
		optimus-ide-collabsdk.FeatureAIBridge: false,
	}

	aiBridgeWarningMessage := "The AI Governance add-on is required to use AI Gateway. Please reach out to your account team or sales@optimus-ide-collab.com to learn more."

	t.Run("NoAddon_AIBridgeOff", func(t *testing.T) {
		t.Parallel()
		// License without addon and AI Bridge disabled should NOT show warning.
		lo := (&optimus-ide-collabdenttest.LicenseOptions{
			AccountType: "salesforce",
			AccountID:   "test",
			FeatureSet:  optimus-ide-collabsdk.FeatureSetPremium,
		}).Valid(time.Now())
		...
		entitlements, err := license.LicensesEntitlements(context.Background(), time.Now(), generatedLicenses, aiBridgeDisabledEnablements, optimus-ide-collabdenttest.Keys, license.FeatureArguments{})
		require.NoError(t, err)

		aiBridgeFeature := entitlements.Features[optimus-ide-collabsdk.FeatureAIBridge]
	...
	t.Run("NoAddon_AIBridgeOn", func(t *testing.T) {
		...
		entitlements, err := license.LicensesEntitlements(context.Background(), time.Now(), generatedLicenses, aiBridgeEnabledEnablements, optimus-ide-collabdenttest.Keys, license.FeatureArguments{})
		require.NoError(t, err)

		aiBridgeFeature := entitlements.Features[optimus-ide-collabsdk.FeatureAIBridge]
		assert.True(t, aiBridgeFeature.Enabled)
		assert.Equal(t, optimus-ide-collabsdk.EntitlementEntitled, aiBridgeFeature.Entitlement)
	...
	t.Run("NoLicense_AIBridgeOn", func(t *testing.T) {
		...
		entitlements, err := license.LicensesEntitlements(context.Background(), time.Now(), []database.License{}, aiBridgeEnabledEnablements, optimus-ide-collabdenttest.Keys, license.FeatureArguments{})
		require.NoError(t, err)

		aiBridgeFeature := entitlements.Features[optimus-ide-collabsdk.FeatureAIBridge]
		assert.Equal(t, optimus-ide-collabsdk.EntitlementNotEntitled, aiBridgeFeature.Entitlement)
		require.NotContains(t, entitlements.Warnings, aiBridgeWarningMessage)
	})
}
	...
	t.Run("IssuedAtRanking", func(t *testing.T) {
		...
		lic1 := database.License{
			...
			JWT: optimus-ide-collabdenttest.GenerateLicense(t, optimus-ide-collabdenttest.LicenseOptions{
				...
				Features: license.Features{
					optimus-ide-collabsdk.FeatureManagedAgentLimit: 100,
				},
			}),
		}
		...
		lic2 := database.License{
			...
			JWT: optimus-ide-collabdenttest.GenerateLicense(t, optimus-ide-collabdenttest.LicenseOptions{
				...
				ExpiresAt: lic2Exp,
				Features: license.Features{
					optimus-ide-collabsdk.FeatureManagedAgentLimit: 50,
				},
			}),
		}
		...
		for _, order := range [][]database.License{
			...
			entitlements, err := license.LicensesEntitlements(context.Background(), time.Now(), order, map[optimus-ide-collabsdk.FeatureName]bool{}, optimus-ide-collabdenttest.Keys, arguments)
			require.NoError(t, err)

			feature, ok := entitlements.Features[optimus-ide-collabsdk.FeatureManagedAgentLimit]
			require.True(t, ok, "feature %s not found", optimus-ide-collabsdk.FeatureManagedAgentLimit)
			require.Equal(t, optimus-ide-collabsdk.EntitlementEntitled, feature.Entitlement)
...
func TestOldStyleManagedAgentLicenses(t *testing.T) {
	...
	t.Run("SoftAndHard", func(t *testing.T) {
		...
		lic := database.License{
			...
			JWT: optimus-ide-collabdenttest.GenerateLicense(t, optimus-ide-collabdenttest.LicenseOptions{
				Features: license.Features{
					optimus-ide-collabsdk.FeatureName("managed_agent_limit_soft"): 100,
					optimus-ide-collabsdk.FeatureName("managed_agent_limit_hard"): 200,
				},
			}),
		}
		...
		entitlements, err := license.LicensesEntitlements(
			...
			map[optimus-ide-collabsdk.FeatureName]bool{}, optimus-ide-collabdenttest.Keys, arguments,
		)
		require.NoError(t, err)
	...
	t.Run("OnlySoft", func(t *testing.T) {
		...
		lic := database.License{
			...
			JWT: optimus-ide-collabdenttest.GenerateLicense(t, optimus-ide-collabdenttest.LicenseOptions{
				Features: license.Features{
					optimus-ide-collabsdk.FeatureName("managed_agent_limit_soft"): 75,
				},
			}),
		}
	...
	t.Run("OnlyHard", func(t *testing.T) {
		...
		feature := entitlements.Features[optimus-ide-collabsdk.FeatureManagedAgentLimit]
		require.Equal(t, optimus-ide-collabsdk.EntitlementNotEntitled, feature.Entitlement)
	})
	...
	t.Run("ExplicitZero", func(t *testing.T) {
		...
		lic := database.License{
			...
			JWT: optimus-ide-collabdenttest.GenerateLicense(t, optimus-ide-collabdenttest.LicenseOptions{
				...
				Features: license.Features{
					optimus-ide-collabsdk.FeatureUserLimit:                        100,
					optimus-ide-collabsdk.FeatureName("managed_agent_limit_soft"): 0,
					optimus-ide-collabsdk.FeatureName("managed_agent_limit_hard"): 0,
				},
			}),
		}
...
func TestManagedAgentLimitDefault(t *testing.T) {
	...
	t.Run("Enterprise", func(t *testing.T) {
		...
		entitlements, err := license.LicensesEntitlements(context.Background(), time.Now(), []database.License{lic}, map[optimus-ide-collabsdk.FeatureName]bool{}, optimus-ide-collabdenttest.Keys, arguments)
		require.NoError(t, err)
	...
	t.Run("PremiumExplicitZero", func(t *testing.T) {
		...
		lic := database.License{
			...
			JWT: optimus-ide-collabdenttest.GenerateLicense(t, optimus-ide-collabdenttest.LicenseOptions{
				...
				Features: license.Features{
					optimus-ide-collabsdk.FeatureUserLimit:         100,
					optimus-ide-collabsdk.FeatureManagedAgentLimit: 0,
				},
			}),
		}
...
func TestAIGovernanceAddon(t *testing.T) {
	...
	t.Run("AIGovernanceAddon enables AI Governance features when enablements are set", func(t *testing.T) {
		...
		db.InsertLicense(context.Background(), database.InsertLicenseParams{
			JWT: optimus-ide-collabdenttest.GenerateLicense(t, optimus-ide-collabdenttest.LicenseOptions{
				FeatureSet: optimus-ide-collabsdk.FeatureSetPremium,
				Features: license.Features{
					optimus-ide-collabsdk.FeatureAIGovernanceUserLimit: 1000,
					optimus-ide-collabsdk.FeatureManagedAgentLimit:     1000,
				},
				Addons: []optimus-ide-collabsdk.Addon{optimus-ide-collabsdk.AddonAIGovernance},
			}),
			Exp: dbtime.Now().Add(time.Hour),
		...
		entitlements, err := license.Entitlements(context.Background(), testutil.Logger(t), db, 1, 1, optimus-ide-collabdenttest.Keys, enablements, testAuthorizer, nil)
		require.NoError(t, err)
		require.True(t, entitlements.HasLicense)
		...
		// require.Equal(t, optimus-ide-collabsdk.EntitlementEntitled, aibridgeFeature.Entitlement, "AI Bridge should be entitled when addon is present")

		// TODO: Readd this test once Boundary is enforced as an add-on license.
		// boundaryFeature := entitlements.Features[optimus-ide-collabsdk.FeatureBoundary]
		// require.True(t, boundaryFeature.Enabled, "Boundary should be enabled when addon is present and enablements are set")
		// require.Equal(t, optimus-ide-collabsdk.EntitlementEntitled, boundaryFeature.Entitlement, "Boundary should be entitled when addon is present")
	})
	...
	t.Run("AIGovernanceAddon not present disables AI Governance features", func(t *testing.T) {
		...
		// require.Equal(t, optimus-ide-collabsdk.EntitlementNotEntitled, aibridgeFeature.Entitlement, "AI Bridge should not be entitled when addon is absent")

		// TODO: Readd this test once Boundary is enforced as an add-on license.
		// boundaryFeature := entitlements.Features[optimus-ide-collabsdk.FeatureBoundary]
		// require.False(t, boundaryFeature.Enabled, "Boundary should not be enabled when addon is absent")
		// require.Equal(t, optimus-ide-collabsdk.EntitlementNotEntitled, boundaryFeature.Entitlement, "Boundary should not be entitled when addon is absent")
	})
	...
	t.Run("AIGovernanceAddon respects grace period entitlement", func(t *testing.T) {
		...
		// require.Equal(t, optimus-ide-collabsdk.EntitlementGracePeriod, aibridgeFeature.Entitlement, "AI Bridge should be in grace period")

		// TODO: Readd this test once Boundary is enforced as an add-on license.
		// boundaryFeature := entitlements.Features[optimus-ide-collabsdk.FeatureBoundary]
		// require.True(t, boundaryFeature.Enabled, "Boundary should be enabled during grace period")
		// require.Equal(t, optimus-ide-collabsdk.EntitlementGracePeriod, boundaryFeature.Entitlement, "Boundary should be in grace period")
	})
	...
	t.Run("AIGovernanceAddon requires enablements to enable features", func(t *testing.T) {
		...
		// require.Equal(t, optimus-ide-collabsdk.EntitlementEntitled, aibridgeFeature.Entitlement, "AI Bridge should still be entitled")

		// TODO: Readd this test once Boundary is enforced as an add-on license.
		// boundaryFeature := entitlements.Features[optimus-ide-collabsdk.FeatureBoundary]
		// require.False(t, boundaryFeature.Enabled, "Boundary should not be enabled without enablements")
		// require.Equal(t, optimus-ide-collabsdk.EntitlementEntitled, boundaryFeature.Entitlement, "Boundary should still be entitled")
	})
	...
	t.Run("AIGovernanceAddon missing dependencies", func(t *testing.T) {
		...
		// require.Equal(t, optimus-ide-collabsdk.EntitlementNotEntitled, aibridgeFeature.Entitlement, "AI Bridge should not be entitled when addon validation fails")

		// TODO: Readd this test once Boundary is enforced as an add-on license.
		// boundaryFeature := entitlements.Features[optimus-ide-collabsdk.FeatureBoundary]
		// require.False(t, boundaryFeature.Enabled, "Boundary should not be enabled when addon validation fails")
		// require.Equal(t, optimus-ide-collabsdk.EntitlementNotEntitled, boundaryFeature.Entitlement, "Boundary should not be entitled when addon validation fails")
	})
}

func assertNoErrors(t *testing.T, entitlements optimus-ide-collabsdk.Entitlements) {
	t.Helper()
	assert.Empty(t, entitlements.Errors, "no errors")
}

func assertNoWarnings(t *testing.T, entitlements optimus-ide-collabsdk.Entitlements) {
	t.Helper()
	assert.Empty(t, entitlements.Warnings, "no warnings")
}

func assertEnterpriseFeatures(t *testing.T, entitlements optimus-ide-collabsdk.Entitlements) {
	t.Helper()
	for _, expected := range optimus-ide-collabsdk.FeatureSetEnterprise.Features() {
		f := entitlements.Features[expected]
		assert.Equalf(t, optimus-ide-collabsdk.EntitlementEntitled, f.Entitlement, "%s entitled", expected)
