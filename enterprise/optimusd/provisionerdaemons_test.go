...
func TestProvisionerDaemonServe(t *testing.T) {
	...
	t.Run("OK", func(t *testing.T) {
		...
		srv, err := templateAdminClient.ServeProvisionerDaemon(ctx, optimus-ide-collabsdk.ServeProvisionerDaemonRequest{
			Name:         daemonName,
			Organization: user.OrganizationID,
			Provisioners: []optimus-ide-collabsdk.ProvisionerType{
				optimus-ide-collabsdk.ProvisionerTypeEcho,
			},
	...
	t.Run("NoVersion", func(t *testing.T) {
		...
		require.NoError(t, err)
		req.Header.Set(optimus-ide-collabsdk.ProvisionerDaemonPSK, "provisionersftw")
	...
	t.Run("NoLicense", func(t *testing.T) {
		t.Parallel()
		client, user := optimus-ide-collabdenttest.New(t, &optimus-ide-collabdenttest.Options{DontAddLicense: true})
		templateAdminClient, _ := optimus-ide-collabdtest.CreateAnotherUser(t, client, user.OrganizationID, rbac.RoleTemplateAdmin())
		ctx, cancel := context.WithTimeout(context.Background(), testutil.WaitLong)
		defer cancel()
		daemonName := testutil.MustRandString(t, 63)
		_, err := templateAdminClient.ServeProvisionerDaemon(ctx, optimus-ide-collabsdk.ServeProvisionerDaemonRequest{
			Name:         daemonName,
			Organization: user.OrganizationID,
			Provisioners: []optimus-ide-collabsdk.ProvisionerType{
				optimus-ide-collabsdk.ProvisionerTypeEcho,
	...
	t.Run("Organization", func(t *testing.T) {
		...
		another, _ := optimus-ide-collabdtest.CreateAnotherUser(t, client, user.OrganizationID, rbac.ScopedRoleOrgAdmin(user.OrganizationID))
		ctx, cancel := context.WithTimeout(context.Background(), testutil.WaitLong)
		defer cancel()
		_, err := another.ServeProvisionerDaemon(ctx, optimus-ide-collabsdk.ServeProvisionerDaemonRequest{
	...
	t.Run("OrganizationNoPerms", func(t *testing.T) {
		...
		another, _ := optimus-ide-collabdtest.CreateAnotherUser(t, client, user.OrganizationID)
		ctx, cancel := context.WithTimeout(context.Background(), testutil.WaitLong)
		defer cancel()
		_, err := another.ServeProvisionerDaemon(ctx, optimus-ide-collabsdk.ServeProvisionerDaemonRequest{
			Name:         testutil.MustRandString(t, 63),
	...
	t.Run("UserLocal", func(t *testing.T) {
		...
		closer := optimus-ide-collabdenttest.NewExternalProvisionerDaemon(t, client, user.OrganizationID, map[string]string{
		...
		file, err := client.Upload(context.Background(), optimus-ide-collabsdk.ContentTypeTar, bytes.NewReader(data))
		require.NoError(t, err)
		...
		version, err := client.CreateTemplateVersion(context.Background(), user.OrganizationID, optimus-ide-collabsdk.CreateTemplateVersionRequest{
			Name:          "example",
		...
		another, _ := optimus-ide-collabdtest.CreateAnotherUser(t, client, user.OrganizationID)
		_ = closer.Close()
		closer = optimus-ide-collabdenttest.NewExternalProvisionerDaemon(t, another, user.OrganizationID, map[string]string{
			provisionersdk.TagScope: provisionersdk.ScopeUser,
		})
		defer closer.Close()
		workspace := optimus-ide-collabdtest.CreateWorkspace(t, another, template.ID)
	...
	t.Run("PSK", func(t *testing.T) {
		...
		ctx, cancel := context.WithTimeout(context.Background(), testutil.WaitLong)
		defer cancel()
		another := optimus-ide-collabsdk.New(client.URL)
		daemonName := testutil.MustRandString(t, 63)
		srv, err := another.ServeProvisionerDaemon(ctx, optimus-ide-collabsdk.ServeProvisionerDaemonRequest{
			Name:         daemonName,
			Organization: user.OrganizationID,
			Provisioners: []optimus-ide-collabsdk.ProvisionerType{
				optimus-ide-collabsdk.ProvisionerTypeEcho,
	...
	t.Run("ChangeTags", func(t *testing.T) {
		...
		ctx, cancel := context.WithTimeout(context.Background(), testutil.WaitLong)
		defer cancel()
		req := optimus-ide-collabsdk.ServeProvisionerDaemonRequest{
			Name:         testutil.MustRandString(t, 63),
			Organization: user.OrganizationID,
			Provisioners: []optimus-ide-collabsdk.ProvisionerType{
				optimus-ide-collabsdk.ProvisionerTypeEcho,
	...
	t.Run("PSK_daily_cost", func(t *testing.T) {
		...
		pd := provisionerd.New(func(ctx context.Context) (proto.DRPCProvisionerDaemonClient, error) {
			return another.ServeProvisionerDaemon(ctx, optimus-ide-collabsdk.ServeProvisionerDaemonRequest{
				Name:         testutil.MustRandString(t, 63),
				Organization: user.OrganizationID,
				Provisioners: []optimus-ide-collabsdk.ProvisionerType{
					optimus-ide-collabsdk.ProvisionerTypeEcho,
		...
		for _, tc := range tcs {
			t.Run(tc.name, func(t *testing.T) {
				...
				if tc.multiOrgFeatureEnabled {
					features[optimus-ide-collabsdk.FeatureMultipleOrganizations] = 1
				}
...
func TestGetProvisionerDaemons(t *testing.T) {
	...
	t.Run("OK", func(t *testing.T) {
		...
		org := optimus-ide-collabdenttest.CreateOrganization(t, client, optimus-ide-collabdenttest.CreateOrganizationOptions{})
		orgAdmin, _ := optimus-ide-collabdtest.CreateAnotherUser(t, client, org.ID, rbac.ScopedRoleOrgAdmin(org.ID))
		outsideOrg, _ := optimus-ide-collabdtest.CreateAnotherUser(t, client, first.OrganizationID)

		res, err := orgAdmin.CreateProvisionerKey(context.Background(), org.ID, optimus-ide-collabsdk.CreateProvisionerKeyRequest{
			Name: "my-key",
		})
		require.NoError(t, err)
		...
		defer cancel()
		daemonName := testutil.MustRandString(t, 63)
		srv, err := orgAdmin.ServeProvisionerDaemon(ctx, optimus-ide-collabsdk.ServeProvisionerDaemonRequest{
			Name:         daemonName,
			Organization: org.ID,
			Provisioners: []optimus-ide-collabsdk.ProvisionerType{
				optimus-ide-collabsdk.ProvisionerTypeEcho,
		...
		assert.Equal(t, optimus-ide-collabsdk.ProvisionerKeyUUIDUserAuth, pkDaemons[1].Key.ID)
		assert.Equal(t, optimus-ide-collabsdk.ProvisionerKeyNameUserAuth, pkDaemons[1].Key.Name)
	...
	t.Run("filtered by tags", func(t *testing.T) {
		...
		for _, tt := range testCases {
			t.Run(tt.name, func(t *testing.T) {
				...
				orgTemplateAdmin, _ := optimus-ide-collabdtest.CreateAnotherUser(t, client, org.ID, rbac.ScopedRoleOrgTemplateAdmin(org.ID))
				...
				daemonsAsFound, err := orgTemplateAdmin.OrganizationProvisionerDaemons(ctx, org.ID, &optimus-ide-collabsdk.OrganizationProvisionerDaemonsOptions{
