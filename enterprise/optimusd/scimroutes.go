...
import (
	...
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/enterprise/optimus-ide-collabd/scim"
)
...
func (api *API) mountScimRoute(opt *Options, r chi.Router) error {
	if len(opt.SCIMAPIKey) == 0 {
		...
		r.Mount("/", http.Handler(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			httpapi.Write(r.Context(), w, http.StatusNotFound, optimus-ide-collabsdk.Response{
				...
				Detail:  "SCIM endpoints are disabled if no SCIM is configured. Configure 'OPTIMUS-IDE-COLLAB_SCIM_AUTH_HEADER' to enable.",
			})
		})))
		return nil
	}
	...
	if opt.UseLegacySCIM {
		...
		r.Mount("/v2", chi.Chain(
			api.RequireFeatureMW(optimus-ide-collabsdk.FeatureSCIM),
