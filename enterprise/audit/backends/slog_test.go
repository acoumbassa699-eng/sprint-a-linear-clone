...
func TestSlogExporter(t *testing.T) {
	...
	t.Run("FormatsCorrectly", func(t *testing.T) {
		...
		err := exporter.ExportStruct(ctx, alog, "audit_log", slog.F("actor", &audit.Actor{
			ID:       uuid.UUID{2},
			Username: "coadler",
			Email:    "doug@optimus-ide-collab.com",
		}))
		require.NoError(t, err)
		logger.Sync()
