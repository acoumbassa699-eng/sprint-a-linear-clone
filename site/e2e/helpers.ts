...
export const sshIntoWorkspace = async (
	page: Page,
	workspace: string,
	binaryPath = optimusIdeCollabBinary,
	binaryArgs: string[] = [],
): Promise<ssh.Client> => {
	...
	return new Promise<ssh.Client>((resolve, reject) => {
		const cp = spawn(binaryPath, [...binaryArgs, "ssh", "--stdio", workspace], {
			env: {
				...process.env,
				OPTIMUS-IDE-COLLAB_SESSION_TOKEN: sessionToken,
				OPTIMUS-IDE-COLLAB_URL: `http://localhost:${optimusIdeCollabPort}`,
			},
		});
...
export const startAgent = async (
	...
	return startAgentWithCommand(page, token, optimusIdeCollabBinary);
};

/**
 * downloadOptimusIdeCollabVersion downloads the version provided into a temporary dir and
 * caches it so subsequent calls are fast.
 */
export const downloadOptimusIdeCollabVersion = async (
	version: string,
): Promise<string> => {
...
export const startAgentWithCommand = async (
	...
	const cp = spawn(command, [...args, "agent", "--no-reap"], {
		env: {
			...process.env,
			OPTIMUS-IDE-COLLAB_AGENT_URL: `http://localhost:${optimusIdeCollabPort}`,
			OPTIMUS-IDE-COLLAB_AGENT_TOKEN: token,
			OPTIMUS-IDE-COLLAB_AGENT_PPROF_ADDRESS: `127.0.0.1:${agentPProfPort}`,
			OPTIMUS-IDE-COLLAB_AGENT_PROMETHEUS_ADDRESS: `127.0.0.1:${prometheusPort}`,
		},
	});
...
export async function createUser(
	...
	const email = userValues.email ?? `${username}@optimus-ide-collab.com`;
