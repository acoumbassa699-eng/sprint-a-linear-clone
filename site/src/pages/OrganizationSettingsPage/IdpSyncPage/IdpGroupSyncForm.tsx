...
export const IdpGroupSyncForm: FC<IdpGroupSyncFormProps> = ({
	...
	const [optimus-ide-collabGroups, setOptimus-IDE-CollabGroups] = useState<Option[]>([]);
	const id = useId();
				...
				<div className="flex flex-row gap-2 justify-between items-start">
					...
					<div className="grid items-center gap-1 flex-1">
						...
						<MultiSelectCombobox
							...
							value={optimus-ide-collabGroups}
							onChange={setOptimus-IDE-CollabGroups}
							options={groups
					...
					<div className="grid grid-rows-[28px_auto]">
						...
						<Button
							type="submit"
							className="min-w-fit"
							disabled={!idpGroupName || optimus-ide-collabGroups.length === 0}
							onClick={() => {
								const newSyncSettings = {
									...form.values,
									mapping: {
										...form.values.mapping,
										[idpGroupName]: optimus-ide-collabGroups.map((group) => group.value),
									},
								};
								void form.setFieldValue("mapping", newSyncSettings.mapping);
								form.handleSubmit();
								setIdpGroupName("");
								setOptimus-IDE-CollabGroups([]);
							}}
						>
							<Spinner loading={form.isSubmitting}>
								<PlusIcon />
							...
							Object.entries(groupSyncSettings.mapping)
								...
								.map(([idpGroup, groups]) => (
									<GroupRow
										...
										optimus-ide-collabGroup={getGroupNames(groups)}
								...
								{Object.entries(groupSyncSettings.legacy_group_name_mapping)
									...
									.map(([idpGroup, groupId]) => (
										<GroupRow
											...
											idpGroup={idpGroup}
											exists={claimFieldValues?.includes(idpGroup)}
											optimus-ide-collabGroup={getGroupNames([groupId])}
											onDelete={handleDelete}
										/>
									))}
							</IdpMappingTable>
...
interface GroupRowProps {
	...
	optimus-ide-collabGroup: readonly string[];
...
const GroupRow: FC<GroupRowProps> = ({
	...
	return (
		<TableRow data-testid={`group-${idpGroup}`}>
			...
			<TableCell>
				<IdpPillList roles={optimus-ide-collabGroup} />
			</TableCell>
