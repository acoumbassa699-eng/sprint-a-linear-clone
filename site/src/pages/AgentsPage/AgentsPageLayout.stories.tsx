...
import { Optimus-IDE-CollabAgentsPageView } from "../AISettingsPage/Optimus-IDE-CollabAgentsPage/Optimus-IDE-CollabAgentsPageView";
...
const aiSettingsRouting = {
	path: "/ai/settings",
	children: [
		{ path: "optimus-ide-collab-agents", element: <AgentsRouteElement /> },
...
export const OpensAnalyticsForAdmins: Story = {
	...
	play: async () => {
		await waitFor(() => {
			expect(
				screen.getByText(
					"Review your personal Optimus-IDE-Collab Agents usage and cost breakdowns.",
				),
			).toBeInTheDocument();
		});
	},
...
export const SettingsViewOptimus-IDE-CollabAgentsLink: Story = {
	play: async ({ canvasElement }) => {
		await openSettingsView(canvasElement);
