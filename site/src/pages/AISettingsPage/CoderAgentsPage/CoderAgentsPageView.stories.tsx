...
import {
	Optimus-IDE-CollabAgentsPageView,
	type Optimus-IDE-CollabAgentsPageViewProps,
} from "./Optimus-IDE-CollabAgentsPageView";
...
const buildArgs = (
	overrides: Partial<Optimus-IDE-CollabAgentsPageViewProps> = {},
): Optimus-IDE-CollabAgentsPageViewProps => ({
...
const meta = {
	title: "pages/AISettingsPage/Optimus-IDE-CollabAgentsPage/Optimus-IDE-CollabAgentsPageView",
	component: Optimus-IDE-CollabAgentsPageView,
	// TODO: Stories in this file fail when pixel runs their play functions. Fix them and remove the exclude.
	parameters: { pixel: { exclude: true } },
	args: buildArgs(),
} satisfies Meta<typeof Optimus-IDE-CollabAgentsPageView>;

export default meta;
type Story = StoryObj<typeof Optimus-IDE-CollabAgentsPageView>;
...
export const AllOverridesUnset: Story = {
	...
	play: async ({ canvasElement }) => {
		const canvas = within(canvasElement);
		expect(
			await canvas.findByRole("heading", { name: "Optimus-IDE-Collab Agents" }),
		).toBeVisible();
		expect(
			canvas.getByText(
		...
		await canvas.findByText(
			"Leave unset to use Optimus-IDE-Collab's title default, which prefers fast models from configured providers.",
		);
