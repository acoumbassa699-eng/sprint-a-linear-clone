...
interface Optimus-IDE-Collabnaut {
	x: number;
	padIdx: number;
	dir: 1 | -1;
	speed: number;
...
// How many Optimus-IDE-Collabnauts appear per round.
const OPTIMUS-IDE-COLLABNAUTS_PER_ROUND = 10;
...
function generateTerrain(w: number, h: number): Terrain {
	...
	// Nominate the widest pad as the Optimus-IDE-Collab space station.
	let widest = pads[0];
	for (const p of pads) {
...
// ---------------------------------------------------------------------------
// Optimus-IDE-Collabnauts - little astronauts stranded on the landing pads
// ---------------------------------------------------------------------------
...
function createOptimus-IDE-Collabnauts(
	pads: LandingPad[],
	entries: { name: string; role: string }[],
): Optimus-IDE-Collabnaut[] {
	const nauts: Optimus-IDE-Collabnaut[] = [];
...
function updateOptimus-IDE-Collabnauts(nauts: Optimus-IDE-Collabnaut[], pads: LandingPad[], dt: number) {
...
/**
 * Draw a single Optimus-IDE-Collabnaut at their position on the pad surface.
 * The figure is ~10 px tall: round helmet, stick body, animated
 * legs and arms. Drawn in gray to match the station aesthetic.
 */
function drawOptimus-IDE-Collabnaut(
	ctx: CanvasRenderingContext2D,
	naut: Optimus-IDE-Collabnaut,
	groundY: number,
) {
	const color = "#888";
...
function drawTerrain(ctx: CanvasRenderingContext2D, terrain: Terrain) {
	...
	for (const pad of pads) {
		...
		if (pad.isStation) {
			...
			drawVectorText(ctx, "OPTIMUS-IDE-COLLAB BASE", midX, pad.y + tick + 10, 7);
		} else {
...
// ---------------------------------------------------------------------------
// Disembark animation - Optimus-IDE-Collabnauts walking into the base
// ---------------------------------------------------------------------------
...
function drawSidebar(
	...
	ctx.textAlign = "left";
	ctx.fillText("OPTIMUS-IDE-COLLABNAUTS ROSTER", x + padX, headerH - 6);
...
export const LunarLander: FC = () => {
	...
	useEffect(() => {
		...
		let optimus-ide-collabnauts: Optimus-IDE-Collabnaut[] = [];
		...
		function startNewRound() {
			...
			const pick = unsaved.slice(0, OPTIMUS-IDE-COLLABNAUTS_PER_ROUND);
			terrain = generateTerrain(logicalW - SIDEBAR_W, logicalH - DASHBOARD_H);
			optimus-ide-collabnauts = createOptimus-IDE-Collabnauts(terrain.pads, pick);
			disembarks = [];
			resetLander();
		...
		function resetLander() {
			...
			explosionParts = [];
			fuel = 100;
			for (const n of optimus-ide-collabnauts) {
		...
		function onClick(e: MouseEvent) {
			...
			if (dialogVisible) {
				...
				if (
					...
					// Yes - travel to another base with this Optimus-IDE-Collabnaut.
					...
					if (entry) {
						...
						const pick = [entry, ...unsaved.slice(0, OPTIMUS-IDE-COLLABNAUTS_PER_ROUND - 1)];
			...
			const onScreen = optimus-ide-collabnauts.find(
		...
		function loop(ts: number) {
			...
			if (introTimer > 0) {
				...
				drawVectorText(cx, "OPTIMUS-IDE-COLLABNAUTS", dashW / 2, playH * 0.43, 44);
			...
			if (victory) {
				...
				cx.textAlign = "center";
				cx.fillText("All Optimus-IDE-Collabnauts have been saved!", dashW / 2, playH * 0.6);
			...
			// -- update optimus-ide-collabnauts --
			if (terrain) updateOptimus-IDE-Collabnauts(optimus-ide-collabnauts, terrain.pads, dt);
					...
					if (
						...
						if (pad) {
							...
							if (pad.isStation) {
								...
								const aboardCount = optimus-ide-collabnauts.filter((c) => c.aboard).length;
							...
							if (optimus-ide-collabnauts.every((c) => c.saved)) {
			...
			if (terrain) {
				...
				for (const naut of optimus-ide-collabnauts) {
					if (!naut.aboard && !naut.saved) {
						drawOptimus-IDE-Collabnaut(cx, naut, terrain.pads[naut.padIdx].y);
					}
				}
			}
			...
			for (const a of disembarks) {
				const tmpNaut: Optimus-IDE-Collabnaut = {
					x: a.x,
					padIdx: 0,
				...
				drawOptimus-IDE-Collabnaut(cx, tmpNaut, a.groundY);
			}
			...
			const currentNautNames = new Set(
				optimus-ide-collabnauts.filter((c) => !c.aboard && !c.saved).map((c) => c.name),
			);
			drawSidebar(
				cx,
				logicalW,
			...
			const cargo = optimus-ide-collabnauts.filter((c) => c.aboard);
			const savedPct = (savedNames.size / ROSTER.length) * 100;
