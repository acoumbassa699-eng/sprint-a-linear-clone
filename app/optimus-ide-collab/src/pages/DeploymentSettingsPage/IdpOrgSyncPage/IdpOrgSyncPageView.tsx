...
export const IdpOrgSyncPageView: FC<IdpSyncPageViewProps> = ({
	...
	const [optimus-ide-collabOrgs, setOptimus-IDE-CollabOrgs] = useState<Option[]>([]);
						...
						<div className="flex flex-row pt-8 gap-2 justify-between items-start">
							...
							<div className="grid items-center gap-1 flex-1">
								...
								<MultiSelectCombobox
									...
									value={optimus-ide-collabOrgs}
									onChange={setOptimus-IDE-CollabOrgs}
							...
							<div className="grid grid-rows-[28px_auto]">
								...
								<Button
									type="submit"
									className="min-w-fit"
									disabled={!idpOrgName || optimus-ide-collabOrgs.length === 0}
									onClick={async () => {
										const newSyncSettings = {
											...form.values,
											mapping: {
												...form.values.mapping,
												[idpOrgName]: optimus-ide-collabOrgs.map((org) => org.value),
											},
										};
										void form.setFieldValue("mapping", newSyncSettings.mapping);
										form.handleSubmit();
										setIdpOrgName("");
										setOptimus-IDE-CollabOrgs([]);
									}}
								>
									<Spinner loading={form.isSubmitting}>
										<PlusIcon />
								...
								Object.entries(form.values.mapping)
									...
									.map(([idpOrg, organizations]) => (
										<OrganizationRow
											key={idpOrg}
											idpOrg={idpOrg}
											optimus-ide-collabOrgs={getOrgNames(organizations)}
...
interface OrganizationRowProps {
	...
	optimus-ide-collabOrgs: readonly string[];
...
const OrganizationRow: FC<OrganizationRowProps> = ({
	...
	return (
		<TableRow data-testid={`idp-org-${idpOrg}`}>
			...
			<TableCell>
				<OrganizationPills organizations={optimus-ide-collabOrgs} />
			</TableCell>
			<TableCell>
				<Button
					variant="outline"
