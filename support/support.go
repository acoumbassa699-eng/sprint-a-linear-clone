...
type Deployment struct {
	BuildInfo      *optimus-ide-collabsdk.BuildInfoResponse  `json:"build"`
	Config         *optimus-ide-collabsdk.DeploymentConfig   `json:"config"`
	Experiments    optimus-ide-collabsdk.Experiments         `json:"experiments"`
	HealthReport   *healthsdk.HealthcheckReport `json:"health_report"`
	Licenses       []optimus-ide-collabsdk.License           `json:"licenses"`
	Stats          *optimus-ide-collabsdk.DeploymentStats    `json:"stats"`
	Entitlements   *optimus-ide-collabsdk.Entitlements       `json:"entitlements"`
	HealthSettings *healthsdk.HealthSettings    `json:"health_settings"`
	Workspaces     *optimus-ide-collabsdk.WorkspacesResponse `json:"workspaces"`
...
type Workspace struct {
	Workspace          optimus-ide-collabsdk.Workspace                 `json:"workspace"`
	Parameters         []optimus-ide-collabsdk.WorkspaceBuildParameter `json:"parameters"`
	Template           optimus-ide-collabsdk.Template                  `json:"template"`
	TemplateVersion    optimus-ide-collabsdk.TemplateVersion           `json:"template_version"`
	TemplateFileBase64 string                             `json:"template_file_base64"`
	BuildLogs          []optimus-ide-collabsdk.ProvisionerJobLog       `json:"build_logs"`
}

type Agent struct {
	Agent                 *optimus-ide-collabsdk.WorkspaceAgent                       `json:"agent"`
	ConnectionInfo        *workspacesdk.AgentConnectionInfo              `json:"connection_info"`
	ListeningPorts        *optimus-ide-collabsdk.WorkspaceAgentListeningPortsResponse `json:"listening_ports"`
	Logs                  []byte                                         `json:"logs"`
	...
	Prometheus            []byte                                         `json:"prometheus"`
	StartupLogs           []optimus-ide-collabsdk.WorkspaceAgentLog                   `json:"startup_logs"`
}
...
func DeploymentInfo(ctx context.Context, client *optimus-ide-collabsdk.Client, log slog.Logger, workspacesCap int) Deployment {
	...
	eg.Go(func() error {
		...
		if err != nil {
			if cerr, ok := optimus-ide-collabsdk.AsError(err); ok && (cerr.StatusCode() == http.StatusForbidden || cerr.StatusCode() == http.StatusUnauthorized) {
	...
	eg.Go(func() error {
		...
		if licenses == nil {
			licenses = make([]optimus-ide-collabsdk.License, 0)
		}
		d.Licenses = licenses
		return nil
	})
	...
	eg.Go(func() error {
		...
		if err != nil {
			...
			if cerr, ok := optimus-ide-collabsdk.AsError(err); ok && (cerr.StatusCode() == http.StatusForbidden || cerr.StatusCode() == http.StatusUnauthorized || cerr.StatusCode() == http.StatusBadRequest) {
	...
	eg.Go(func() error {
		...
		if err != nil {
			...
			if cerr, ok := optimus-ide-collabsdk.AsError(err); ok && (cerr.StatusCode() == http.StatusNotFound || cerr.StatusCode() == http.StatusForbidden) {
	...
	eg.Go(func() error {
		var (
			offset int
			limit  = 200
			all    []optimus-ide-collabsdk.Workspace
			count  int
		)
		capTotal := workspacesCap
		for {
			resp, err := client.Workspaces(ctx, optimus-ide-collabsdk.WorkspaceFilter{Offset: offset, Limit: limit})
			if err != nil {
	...
	if d.Config != nil && d.Config.Values != nil {
		...
		if prometheusCfg.Enable.Value() {
			...
			if err != nil {
				log.Warn(ctx, "fetch optimus-ide-collabd prometheus metrics", slog.Error(err))
			} else {
...
func fetchPrometheusMetrics(ctx context.Context, client *optimus-ide-collabsdk.Client, log slog.Logger) ([]byte, error) {
	if client == nil {
	...
	if resp.StatusCode != http.StatusOK {
		log.Debug(ctx, "optimus-ide-collabd prometheus metrics fetch non-200",
...
func NetworkInfo(ctx context.Context, client *optimus-ide-collabsdk.Client, log slog.Logger) Network {
	var (
		n  Network
		eg errgroup.Group
	)
...
func WorkspaceInfo(ctx context.Context, client *optimus-ide-collabsdk.Client, log slog.Logger, workspaceID uuid.UUID) Workspace {
	var (
		w  Workspace
		eg errgroup.Group
	)
	...
	eg.Go(func() error {
		...
		raw, ctype, err := client.DownloadWithFormat(ctx, tv.Job.FileID, optimus-ide-collabsdk.FormatZip)
		if err != nil {
			return err
		}
		if ctype != optimus-ide-collabsdk.ContentTypeZip {
			return xerrors.Errorf("expected content-type %s, got %s", optimus-ide-collabsdk.ContentTypeZip, ctype)
		}
...
func AgentInfo(ctx context.Context, client *optimus-ide-collabsdk.Client, log slog.Logger, agentID uuid.UUID, workspaceFilePatterns []string) Agent {
	var (
		a  Agent
		eg errgroup.Group
	)
...
func connectedAgentInfo(ctx context.Context, client *optimus-ide-collabsdk.Client, log slog.Logger, agentID uuid.UUID, workspaceFilePatterns []string, eg *errgroup.Group, a *Agent) (closer func()) {
	...
	eg.Go(func() error {
		...
		if err := json.NewDeoptimus-ide-collab(bytes.NewReader(manifestRes)).Decode(&a.Manifest); err != nil {
	...
	eg.Go(func() error {
		...
		if err != nil {
			return xerrors.Errorf("fetch optimus-ide-collab agent logs: %w", err)
		}
		a.Logs = logBytes
		return nil
	})
...
func PprofInfo(ctx context.Context, client *optimus-ide-collabsdk.Client, log slog.Logger) *PprofCollection {
	if client == nil {
		return nil
	}
...
func PprofInfoFromArchive(ctx context.Context, client *optimus-ide-collabsdk.Client, log slog.Logger, duration time.Duration) (*PprofCollection, error) {
	if client == nil {
		return nil, xerrors.New("client is nil")
	}

	body, err := client.DebugCollectProfile(ctx, optimus-ide-collabsdk.DebugProfileOptions{
		Duration: duration,
...
func CanGenerateFull(ctx context.Context, client *optimus-ide-collabsdk.Client) (bool, error) {
	if client == nil {
		return false, xerrors.Errorf("developer error: missing client!")
	}

	authChecks := map[string]optimus-ide-collabsdk.AuthorizationCheck{
		"Read DeploymentValues": {
			Object: optimus-ide-collabsdk.AuthorizationObject{
				ResourceType: optimus-ide-collabsdk.ResourceDeploymentConfig,
			},
			Action: optimus-ide-collabsdk.ActionRead,
		},
	}

	authResp, err := client.AuthCheck(ctx, optimus-ide-collabsdk.AuthorizationRequest{Checks: authChecks})
	if err != nil {
...
func Run(ctx context.Context, d *Deps) (*Bundle, error) {
	...
	me, err := d.Client.User(ctx, optimus-ide-collabsdk.Me)
	if err != nil {
		return nil, err
	}
	...
	eg.Go(func() error {
		...
		raw, ctype, err := d.Client.DownloadWithFormat(ctx, tv.Job.FileID, optimus-ide-collabsdk.FormatZip)
		if err != nil || ctype != optimus-ide-collabsdk.ContentTypeZip {
	...
	// Pprof endpoints require Optimus-IDE-Collab server version 2.28.0 or newer.
	if d.CollectPprof {
...
// minPprofVersion is the minimum Optimus-IDE-Collab server version that supports
