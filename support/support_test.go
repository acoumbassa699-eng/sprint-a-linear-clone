...
func TestRun(t *testing.T) {
	...
	t.Run("MemberNoWorkspace", func(t *testing.T) {
		...
		admin := optimus-ide-collabdtest.CreateFirstUser(t, client)
		memberClient, _ := optimus-ide-collabdtest.CreateAnotherUser(t, client, admin.OrganizationID)
		bun, err := support.Run(ctx, &support.Deps{
			Client: memberClient,
			Log:    slogtest.Make(t, &slogtest.Options{IgnoreErrors: true}).Named("bundle").Leveled(slog.LevelDebug),
		})
...
func assertSanitizedDeploymentConfig(t *testing.T, dc *optimus-ide-collabsdk.DeploymentConfig) {
	t.Helper()
...
func assertSanitizedWorkspace(t *testing.T, ws optimus-ide-collabsdk.Workspace) {
	t.Helper()
...
func setupWorkspaceAndAgent(ctx context.Context, t *testing.T, client *optimus-ide-collabsdk.Client, db database.Store, user optimus-ide-collabsdk.CreateFirstUserResponse) (optimus-ide-collabsdk.Workspace, optimus-ide-collabsdk.WorkspaceAgent) {
	...
	optimus-ide-collabdtest.NewWorkspaceAgentWaiter(t, client, wbr.Workspace.ID).Wait()
