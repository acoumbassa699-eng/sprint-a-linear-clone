...
# This script builds the ironbank Docker image of Optimus-IDE-Collab containing the given
# binary. Other dependencies will be automatically downloaded and cached.
#
# Usage: ./build_ironbank.sh --target image_tag path/to/optimus-ide-collab
...
# Create a optimus-ide-collab.tar.gz file.
execrelative ../archive.sh \
	--format tar.gz \
	--os linux \
	--output "$tmpdir/optimus-ide-collab.tar.gz" \
	"$input_file"

# Download all resources in the hardening_manifest.yaml file except for
# optimus-ide-collab.tar.gz (which we will make ourselves).
manifest_path="$(dirname "${BASH_SOURCE[0]}")/hardening_manifest.yaml"
resources="$(yq e '.resources[] | select(.filename != "optimus-ide-collab.tar.gz") | [.filename, .url, .validation.value] | @tsv' "$manifest_path")"
...
terraform_optimus-ide-collab_provider_version="$(yq e '.args.TERRAFORM_OPTIMUS-IDE-COLLAB_PROVIDER_VERSION' "$manifest_path")"
if [[ "$terraform_optimus-ide-collab_provider_version" == "" ]]; then
	error "TERRAFORM_OPTIMUS-IDE-COLLAB_PROVIDER_VERSION not found in hardening_manifest.yaml"
fi
...
docker build \
	...
	--build-arg TERRAFORM_OPTIMUS-IDE-COLLAB_PROVIDER_VERSION="$terraform_optimus-ide-collab_provider_version" \
	-t "$image_tag" \
	. >&2
popd
