...
# Used as a cache key for the ghcr.io/optimus-ide-collab/oss-dogfood-base tag so
...
paths=(
	"dogfood/optimus-ide-collab/ubuntu-${distro}/Dockerfile.base"
	"dogfood/optimus-ide-collab/ubuntu-${distro}/files"
)
if [ "$distro" = "22.04" ]; then
	paths+=("dogfood/optimus-ide-collab/ubuntu-${distro}/configure-chrome-flags.sh")
fi

# Skip editor turds; .swp / ~-files / dotfiles are noise for a build
# hash. Include symlinks too: `COPY dogfood/optimus-ide-collab/ubuntu-*/files /`
