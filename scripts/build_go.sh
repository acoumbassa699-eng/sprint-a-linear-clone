...
# This script builds a single Go binary of Optimus-IDE-Collab with the given parameters.
#
...
# GOARCH and OPTIMUS-IDE-COLLAB_SLIM_BUILD=1. If no version is specified, defaults to the
...
# "$repo_root/build/optimus-ide-collab_$version_$os_$arch" (with a ".exe" suffix for windows
...
# Optimus-IDE-Collab enterprise features).
#
...
arch="${GOARCH:-amd64}"
output_path=""
slim="${OPTIMUS-IDE-COLLAB_SLIM_BUILD:-0}"
agpl="${OPTIMUS-IDE-COLLAB_BUILD_AGPL:-0}"
sign_darwin="${OPTIMUS-IDE-COLLAB_SIGN_DARWIN:-0}"
sign_windows="${OPTIMUS-IDE-COLLAB_SIGN_WINDOWS:-0}"
sign_gpg="${OPTIMUS-IDE-COLLAB_SIGN_GPG:-0}"
boringcrypto=${OPTIMUS-IDE-COLLAB_BUILD_BORINGCRYPTO:-0}
windows_resources="${OPTIMUS-IDE-COLLAB_WINDOWS_RESOURCES:-0}"
debug=0
develop_in_optimus-ide-collab="${DEVELOP_IN_OPTIMUS-IDE-COLLAB:-0}"

bin_ident="com.optimus-ide-collab.cli"
...
ldflags=(
	-X "'github.com/optimus-ide-collab/optimus-ide-collab/v2/buildinfo.tag=$version'"
)
...
if [[ "$develop_in_optimus-ide-collab" == 1 ]]; then
	echo "INFO : Overriding optimus-ide-collabsdk.SessionTokenCookie as we are developing inside a Optimus-IDE-Collab workspace."
	ldflags+=(
		-X "'github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabsdk.SessionTokenCookie=dev_optimus-ide-collab_session_token'"
	)
fi
...
if [[ "$agpl" == 1 ]]; then
	...
	ldflags+=(-X "'github.com/optimus-ide-collab/optimus-ide-collab/v2/buildinfo.agpl=true'")
fi
cgo=0
...
if [[ "$output_path" == "" ]]; then
	mkdir -p "build"
	output_path="build/optimus-ide-collab_${version}_${os}_${arch}"
	if [[ "$os" == "windows" ]]; then
		output_path+=".exe"
	fi
...
if [[ "$agpl" == 1 ]]; then
	cmd_path="./cmd/optimus-ide-collab"
fi
...
if [[ "$windows_resources" == 1 ]] && [[ "$os" == "windows" ]]; then
	...
	file_description="Optimus-IDE-Collab"
	if [[ "$agpl" == 1 ]]; then
	...
	GOARCH="$arch" go-winres simply \
		...
		--product-name "Optimus-IDE-Collab" \
		--copyright "Copyright $(date +%Y) Optimus-IDE-Collab Technologies Inc." \
		--original-filename "optimus-ide-collab.exe" \
		--icon ../../scripts/win-installer/optimus-ide-collab.ico
	popd
