...
func main() {
	...
	expectedSchemaAfter, err := gitShow("optimus-ide-collabd/database/dump.sql", migrateToVersion)
	if err != nil {
		panic(err)
	}
...
func makeMigrateFS(version string) (fs.FS, error) {
	...
	out, err := exec.Command("git", "archive", "--format=zip", version, "optimus-ide-collabd/database/migrations").CombinedOutput()
	if err != nil {
	...
	subbed, err := fs.Sub(zr, "optimus-ide-collabd/database/migrations")
	if err != nil {
