...
This directory contains test scripts for the MCP OAuth2 implementation in Optimus-IDE-Collab.

## Prerequisites

1. Start Optimus-IDE-Collab in development mode:
...
2. Login to get a session token:
   ...
   ```bash
   ./scripts/optimus-ide-collab-dev.sh login
   ```
...
- `SESSION_TOKEN`: Optimus-IDE-Collab session token (auto-read from `.optimus-ide-collabv2/session`)
- `BASE_URL`: Optimus-IDE-Collab server URL (default: `http://localhost:3000`)
