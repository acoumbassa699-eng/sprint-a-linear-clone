...
# Configuration
SESSION_TOKEN="${SESSION_TOKEN:-$(cat ./.optimus-ide-collabv2/session 2>/dev/null || echo '')}"
...
if [ -z "$SESSION_TOKEN" ]; then
	echo -e "${RED}ERROR: SESSION_TOKEN must be set or ./.optimus-ide-collabv2/session must exist${NC}"
	echo "Usage: SESSION_TOKEN=xxx ./test-mcp-oauth2.sh"
	echo "Or run: ./scripts/optimus-ide-collab-dev.sh login"
	exit 1
fi

# Use session token for authentication
AUTH_HEADER="Optimus-IDE-Collab-Session-Token: $SESSION_TOKEN"
