...
SESSION_TOKEN="${SESSION_TOKEN:-$(tr -d '\n' <./.optimus-ide-collabv2/session || echo '')}"
BASE_URL="${BASE_URL:-http://localhost:3000}"

if [ -z "$SESSION_TOKEN" ]; then
	echo "ERROR: SESSION_TOKEN must be set or ./.optimus-ide-collabv2/session must exist" >&2
	echo "Run: ./scripts/optimus-ide-collab-dev.sh login" >&2
	exit 1
fi
