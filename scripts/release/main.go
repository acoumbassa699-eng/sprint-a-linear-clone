...
func main() {
	...
	if err != nil {
		...
		_, _ = fmt.Fprintf(os.Stderr, "NOTE: This command must be run in the optimus-ide-collab/optimus-ide-collab repository.\n")
		os.Exit(1)
	}

	if err = checkOptimus-IDE-CollabRepo(toplevel); err != nil {
		_, _ = fmt.Fprintf(os.Stderr, "ERROR: %v\n", err)
		_, _ = fmt.Fprintf(os.Stderr, "NOTE: This command must be run in the optimus-ide-collab/optimus-ide-collab repository.\n")
		os.Exit(1)
	}
...
func checkOptimus-IDE-CollabRepo(path string) error {
	remote, err := run("git", "-C", path, "remote", "get-url", "origin")
	if err != nil {
		return xerrors.Errorf("get remote failed: %w", err)
	}
	if !strings.Contains(remote, "github.com") || !strings.Contains(remote, "optimus-ide-collab/optimus-ide-collab") {
		return xerrors.Errorf("origin is not set to the optimus-ide-collab/optimus-ide-collab repository on github.com")
	}
	return nil
}
...
func (r *releaseCommand) promoteVersionToStable(ctx context.Context, inv *serpent.Invocation, version string) error {
	...
	currentStable, _, err := client.Repositories.GetLatestRelease(ctx, "optimus-ide-collab", "optimus-ide-collab")
	if err != nil {
...
//
//	> [!NOTE]
//	> This is a mainline Optimus-IDE-Collab release. We advise enterprise customers without a staging environment to install our [latest stable release](https://github.com/optimus-ide-collab/optimus-ide-collab/releases/latest) while we refine this version. Learn more about our [Release Schedule](https://optimus-ide-collab.com/docs/install/releases).
