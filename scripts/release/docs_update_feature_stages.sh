...
parse_all_experiments() {
	...
	experiments_output=$(go doc -all ./optimus-ide-collabsdk "${experiments_var}" 2>/dev/null || true)
	...
	if [[ -z "${experiments_output}" ]]; then
		...
		if [[ -z "${experiments_output}" ]]; then
			log "Warning: Neither ExperimentsSafe nor ExperimentsAll found in ./optimus-ide-collabsdk"
			return
		fi
	fi
...
parse_experiments() {
	go doc -all ./optimus-ide-collabsdk Experiment |
		sed \
...
# Collect experiments from the current optimus-ide-collabsdk package.
