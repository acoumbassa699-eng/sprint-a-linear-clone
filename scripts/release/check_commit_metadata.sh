...
# useful for dry-runs.
ignore_missing_metadata=${OPTIMUS-IDE-COLLAB_IGNORE_MISSING_COMMIT_METADATA:-0}
...
main() {
	...
	declare -A humanized_areas=(
		...
		["optimus-ide-collabd/database"]="Database"
		["enterprise/audit"]="Auditing"
		["enterprise/cli"]="CLI"
		["enterprise/optimus-ide-collabd"]="Server"
		...
		[agent]="Agent"
		[cli]="CLI"
		[optimus-ide-collabd]="Server"
		[optimus-ide-collabsdk]="SDK"
		[docs]="Documentation"
	...
	for commit in "${renamed_cherry_pick_commits_pending[@]}"; do
		...
		if [[ ! -v renamed_cherry_pick_commits[${commit}] ]]; then
			if [[ ${OPTIMUS-IDE-COLLAB_IGNORE_MISSING_COMMIT_METADATA:-0} == 1 ]]; then
				log "WARNING: Missing original commit for cherry-picked commit ${commit}, but continuing due to OPTIMUS-IDE-COLLAB_IGNORE_MISSING_COMMIT_METADATA being set."
				continue
			else
