...
workspace=${1:-}
optimus-ide-collab_repo=${2:-.}
port=${3:-3111}

if [[ -z "${workspace}" ]]; then
	echo "Usage: $0 <workspace> [workspace optimus-ide-collab/optimus-ide-collab dir] [e2e port]"
	exit 1
fi
...
main() {
	...
	playwright_version=$(
		ssh "optimus-ide-collab.${workspace}" \
			"cat '${optimus-ide-collab_repo}'/site/pnpm-lock.yaml | grep \"^  '@playwright/test@\"" |
			cut -d '@' -f 3 |
			tr -d ":'" |
			sort -V |
			tail -n 1
	...
	dest=~/.cache/optimus-ide-collab-remote-playwright
	...
	# shellcheck disable=SC2029 # This is intended to expand client-side.
	ssh -t "${port_args[@]}" optimus-ide-collab."${workspace}" "export OPTIMUS-IDE-COLLAB_E2E_PORT='${port}'; export OPTIMUS-IDE-COLLAB_E2E_WS_ENDPOINT='${ws_endpoint}'; [[ -d '${optimus-ide-collab_repo}/site' ]] && cd '${optimus-ide-collab_repo}/site'; exec \"\$(grep \"\${USER}\": /etc/passwd | cut -d: -f7)\" -i -l"
}
