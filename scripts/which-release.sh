...
REMOTE=$(git remote -v | grep optimus-ide-collab/optimus-ide-collab | awk '{print $1}' | head -n1)
if [[ -z "${REMOTE}" ]]; then
	error "Could not find remote for optimus-ide-collab/optimus-ide-collab"
fi
