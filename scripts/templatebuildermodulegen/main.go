...
type ModuleConfig struct {
	...
	// Namespace is the registry namespace (e.g. "optimus-ide-collab" or "optimus-ide-collab-labs").
	// When empty, defaults to "optimus-ide-collab".
...
var moduleConfigs = map[string]ModuleConfig{
	...
	"codex":              {Category: "AI Agent", CompatibleOS: []string{"linux"}, ConflictsWith: []string{}, Namespace: "optimus-ide-collab-labs"},
...
func main() {
	...
	baseURL := flag.String("registry-url", registryBaseURL, "Base URL of the Optimus-IDE-Collab registry API")
	flag.Parse()
	...
	args := flag.Args()
	if len(args) == 0 {
		log.Fatal("specify one or more registry module IDs to generate (e.g. optimus-ide-collab/antigravity optimus-ide-collab-labs/codex)")
	}
...
func resolveModuleArgs(args []string) ([]moduleRef, error) {
	...
	for _, arg := range args {
		...
		if len(parts) != 2 || parts[0] == "" || parts[1] == "" {
			return nil, xerrors.Errorf("invalid module ID %q; expected namespace/slug (e.g. optimus-ide-collab/antigravity)", arg)
		}
