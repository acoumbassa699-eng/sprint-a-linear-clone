...
const registryBaseURL = "https://registry.optimus-ide-collab.com"
...
var skipVarNames = map[string]bool{
	"order":                 true,
	"optimus-ide-collab_app_order":       true,
	"optimus-ide-collab_parameter_order": true,
	"group":                 true,
	"slug":                  true,
	"display_name":          true,
	"log_path":              true,
...
// The id should be "namespace/slug" (e.g. "optimus-ide-collab/code-server"); it will be
...
func fetchLatestVersion(ctx context.Context, baseURL, namespace, slug string) (string, error) {
	reqURL := fmt.Sprintf("%s/terraform_protocol/%s/%s/optimus-ide-collab/versions", baseURL, namespace, slug)
...
// "/icon/code.svg" in the Optimus-IDE-Collab dashboard.
