...
import (
	...
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabsdk/x/agenthooks"
)
...
// consumerState demonstrates consumer-owned hook state. Optimus-IDE-Collab persists no
...
func run() error {
	...
	if cfg.secret == "" {
		return xerrors.New("secret is required through --secret or OPTIMUS-IDE-COLLAB_AGENTHOOKS_SECRET")
	}
	...
	if cfg.audience == "" {
		return xerrors.New("audience is required through --audience or OPTIMUS-IDE-COLLAB_AGENTHOOKS_AUDIENCE")
	}
	...
	enoptimus-ide-collab := json.NewEnoptimus-ide-collab(os.Stdout)
	logEvent := func(event eventLog) error {
		logMu.Lock()
		defer logMu.Unlock()
		if err := enoptimus-ide-collab.Encode(event); err != nil {
...
func parseFlags() (config, error) {
	logOnly, err := envBool("OPTIMUS-IDE-COLLAB_AGENTHOOKS_LOG_ONLY", true)
	if err != nil {
		return config{}, err
	}
	var cfg config
	cfg.logOnly = logOnly
	flag.StringVar(&cfg.listen, "listen", envOrDefault("OPTIMUS-IDE-COLLAB_AGENTHOOKS_LISTEN", "127.0.0.1:8081"), "Listen address (OPTIMUS-IDE-COLLAB_AGENTHOOKS_LISTEN)")
	flag.StringVar(&cfg.audience, "audience", os.Getenv("OPTIMUS-IDE-COLLAB_AGENTHOOKS_AUDIENCE"), "Expected aud claim, which is the deployment's OPTIMUS-IDE-COLLAB_CHAT_HOOK_URL, required (OPTIMUS-IDE-COLLAB_AGENTHOOKS_AUDIENCE)")
	flag.StringVar(&cfg.secret, "secret", os.Getenv("OPTIMUS-IDE-COLLAB_AGENTHOOKS_SECRET"), "Shared HS256 secret, required (OPTIMUS-IDE-COLLAB_AGENTHOOKS_SECRET)")
	flag.StringVar(&cfg.issuer, "issuer", os.Getenv("OPTIMUS-IDE-COLLAB_AGENTHOOKS_ISSUER"), "Expected iss claim, normally the Optimus-IDE-Collab deployment ID (OPTIMUS-IDE-COLLAB_AGENTHOOKS_ISSUER)")
	flag.StringVar(&cfg.tlsCert, "tls-cert", os.Getenv("OPTIMUS-IDE-COLLAB_AGENTHOOKS_TLS_CERT"), "TLS certificate path (OPTIMUS-IDE-COLLAB_AGENTHOOKS_TLS_CERT)")
	flag.StringVar(&cfg.tlsKey, "tls-key", os.Getenv("OPTIMUS-IDE-COLLAB_AGENTHOOKS_TLS_KEY"), "TLS private key path (OPTIMUS-IDE-COLLAB_AGENTHOOKS_TLS_KEY)")
	flag.BoolVar(&cfg.logOnly, "log-only", cfg.logOnly, "Return an empty response for every event, on by default (OPTIMUS-IDE-COLLAB_AGENTHOOKS_LOG_ONLY)")
	flag.StringVar(&cfg.denyToolPattern, "deny-tool-pattern", os.Getenv("OPTIMUS-IDE-COLLAB_AGENTHOOKS_DENY_TOOL_PATTERN"), "Example regexp for denied tool names, requires -log-only=false (OPTIMUS-IDE-COLLAB_AGENTHOOKS_DENY_TOOL_PATTERN)")
	flag.StringVar(&cfg.redactPrompt, "redact-prompt-pattern", os.Getenv("OPTIMUS-IDE-COLLAB_AGENTHOOKS_REDACT_PROMPT_PATTERN"), "Example regexp to redact in prompts, requires -log-only=false to override the prompt (OPTIMUS-IDE-COLLAB_AGENTHOOKS_REDACT_PROMPT_PATTERN)")
	flag.Parse()
	return cfg, nil
}
