...
gh_auth() {
	if [[ -z ${GITHUB_TOKEN:-} ]]; then
		...
		elif [[ ${OPTIMUS-IDE-COLLAB:-} == true ]]; then
			if ! output=$(optimus-ide-collab external-auth access-token github 2>&1); then
				# TODO(mafredri): We could allow checking `gh auth token` here.
				log "${output}"
				error "Could not authenticate with GitHub using Optimus-IDE-Collab external auth."
			else
...
# Usage: maybedryrun 0 docker push ghcr.io/optimus-ide-collab/optimus-ide-collab:latest
maybedryrun() {
	if [[ "$1" == 1 ]]; then
		shift
		log "DRYRUN: $*"
...
if [[ "${OPTIMUS-IDE-COLLAB_LIBSH_NO_CHECK_DEPENDENCIES:-}" != *t* ]]; then
	libsh_bad_dependencies=0

	if ((BASH_VERSINFO[0] < 4)); then
		libsh_bad_dependencies=1
		log "ERROR: You need at least bash 4.0 to run the scripts in the Optimus-IDE-Collab repo."
		if isdarwin; then
			log "On darwin:"
	...
	if [[ "$(getopt --version)" == *--* ]]; then
		libsh_bad_dependencies=1
		log "ERROR: You need GNU getopt to run the scripts in the Optimus-IDE-Collab repo."
		if isdarwin; then
			log "On darwin:"
	...
	if [[ ${make_version//.*/} -lt 4 ]]; then
		libsh_bad_dependencies=1
		log "ERROR: You need at least make 4.0 to run the scripts in the Optimus-IDE-Collab repo."
		if isdarwin; then
			log "On darwin:"
	...
	if command -v yq4 >/dev/null; then
		export OPTIMUS-IDE-COLLAB_LIBSH_YQ=yq4
	elif command -v yq >/dev/null; then
		if [[ $(yq --version) == *" v4."* ]]; then
			export OPTIMUS-IDE-COLLAB_LIBSH_YQ=yq
		fi
	fi
	...
	export OPTIMUS-IDE-COLLAB_LIBSH_NO_CHECK_DEPENDENCIES=true
fi

# Alias yq to the version we want by shadowing with a function.
if [[ -n ${OPTIMUS-IDE-COLLAB_LIBSH_YQ:-} ]]; then
	yq() {
		command $OPTIMUS-IDE-COLLAB_LIBSH_YQ "$@"
	}
fi
