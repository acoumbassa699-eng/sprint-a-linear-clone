...
import (
	...
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/scripts/atomicwrite"
)
...
func run() error {
	...
	databasePath := filepath.Join(localPath, "..", "..", "..", "optimus-ide-collabd", "database")
...
// readQuerierFunctions reads the functions from optimus-ide-collabd/database/querier.go
...
func parseDBFile(filename string) (*dst.File, error) {
	...
	querierPath := filepath.Join(localPath, "..", "..", "..", "optimus-ide-collabd", "database", filename)
	...
	for _, method := range allMethods {
		...
		for _, t := range []*dst.FieldList{funcType.Params, funcType.Results, funcType.TypeParams} {
			...
			for _, f := range t.List {
				...
				ident.Path = "github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabd/database"
			}
		}
