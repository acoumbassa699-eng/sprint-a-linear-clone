...
# This script packages a Windows installer for Optimus-IDE-Collab containing the given
# binary.
#
...
ln "$input_file" "$temp_dir/bin/optimus-ide-collab.exe"
...
cp "$(realpath scripts/win-installer/optimus-ide-collab.ico)" "$temp_dir/optimus-ide-collab.ico"
...
if [[ "$agpl" == 0 ]]; then
	cat <<-EOF >"$license_path"
		This distribution of Optimus-IDE-Collab includes some enterprise-licensed code which is not
	...
	cat <<-EOF >"$license_path"
		This distribution of Optimus-IDE-Collab is free software and is licensed under the AGPL
		license:
...
makensis \
	-V4 \
	-DOPTIMUS-IDE-COLLAB_VERSION="$version" \
	-DOPTIMUS-IDE-COLLAB_NSIS_VERSION="$nsis_version" \
	-DOPTIMUS-IDE-COLLAB_YEAR="$(date +%Y)" \
	installer.nsi
popd
