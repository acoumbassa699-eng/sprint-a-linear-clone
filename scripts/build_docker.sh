...
# This script builds a Docker image of Optimus-IDE-Collab containing the given binary, for
# the given architecture. Only linux binaries are supported at this time.
#
# Usage: ./build_docker.sh --arch amd64 [--version 1.2.3] [--target image_tag] [--build-base image_tag] [--push] path/to/optimus-ide-collab
#
...
# $OPTIMUS-IDE-COLLAB_IMAGE_BUILD_BASE_TAG for easily forcing a fresh build in CI.
#
# The default base image can be controlled via $OPTIMUS-IDE-COLLAB_BASE_IMAGE_TAG.
#
...
DEFAULT_BASE="${OPTIMUS-IDE-COLLAB_BASE_IMAGE_TAG:-ghcr.io/optimus-ide-collab/optimus-ide-collab-base:latest}"

arch=""
image_tag=""
build_base="${OPTIMUS-IDE-COLLAB_IMAGE_BUILD_BASE_TAG:-}"
version=""
push=0
...
ln "$input_file" "$temp_dir/optimus-ide-collab"
...
docker build \
	...
	--build-arg "OPTIMUS-IDE-COLLAB_VERSION=$version" \
	--no-cache \
	--tag "$image_tag" \
	-f Dockerfile \
	. 1>&2
