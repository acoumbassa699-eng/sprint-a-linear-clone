...
func TestFilterEnv(t *testing.T) {
	...
	env := []string{
		"OPTIMUS-IDE-COLLAB_SESSION_TOKEN=secret",
		"OPTIMUS-IDE-COLLAB_URL=https://example.com",
		"KEEP_ME=yes",
		"PATH=/usr/bin",
	}
	result := filterEnv(env, "OPTIMUS-IDE-COLLAB_SESSION_TOKEN", "OPTIMUS-IDE-COLLAB_URL")

	for _, e := range result {
		k, _, _ := strings.Cut(e, "=")
		assert.NotEqual(t, "OPTIMUS-IDE-COLLAB_SESSION_TOKEN", k)
		assert.NotEqual(t, "OPTIMUS-IDE-COLLAB_URL", k)
	}
...
func TestPortOffset(t *testing.T) {
	...
	root := "/tmp/optimus-ide-collab/worktree-a"
	...
	for _, otherRoot := range []string{
		"/tmp/optimus-ide-collab/worktree-b",
		"/tmp/optimus-ide-collab/worktree-c",
		"/tmp/optimus-ide-collab/worktree-d",
	} {
...
func TestApplyPortOffsetSkipsExplicitPorts(t *testing.T) {
	...
	projectRoot := "/tmp/optimus-ide-collab/worktree-offset"
	for i := range 100 {
		candidate := fmt.Sprintf("/tmp/optimus-ide-collab/worktree-offset-%d", i)
	...
	cfg := &devConfig{
		...
		webPort:           8080,
		proxyPort:         3010,
		optimus-ide-collabMetricsPort:  2114,
	...
	assert.Equal(t, int64(2114), cfg.optimus-ide-collabMetricsPort)
...
func TestApplyPortOffsetDisabledUsesDefaultPorts(t *testing.T) {
	...
	projectRoot := "/tmp/optimus-ide-collab/worktree-offset"
	for i := range 100 {
		candidate := fmt.Sprintf("/tmp/optimus-ide-collab/worktree-offset-disabled-%d", i)
		if portOffset(candidate) != 0 {
			projectRoot = candidate
			break
		}
...
func TestPortOffsetDefaultPortsDoNotOverlap(t *testing.T) {
	...
	ports := []struct {
		...
		{name: "Optimus-IDE-Collab metrics", base: 2114},
	}
...
func TestDevelopInOptimus-IDE-Collab(t *testing.T) {
	t.Run("DEVELOP_IN_OPTIMUS-IDE-COLLAB", func(t *testing.T) {
		t.Setenv("DEVELOP_IN_OPTIMUS-IDE-COLLAB", "1")
		t.Setenv("OPTIMUS-IDE-COLLAB_AGENT_URL", "")
		assert.True(t, developInOptimus-IDE-Collab())
	})

	t.Run("OPTIMUS-IDE-COLLAB_AGENT_URL", func(t *testing.T) {
		t.Setenv("DEVELOP_IN_OPTIMUS-IDE-COLLAB", "")
		t.Setenv("OPTIMUS-IDE-COLLAB_AGENT_URL", "http://something")
		assert.True(t, developInOptimus-IDE-Collab())
	})
	...
	t.Run("Neither", func(t *testing.T) {
		t.Setenv("DEVELOP_IN_OPTIMUS-IDE-COLLAB", "")
		t.Setenv("OPTIMUS-IDE-COLLAB_AGENT_URL", "")
		assert.False(t, developInOptimus-IDE-Collab())
	})
}
...
func TestDevConfigValidate(t *testing.T) {
	...
	t.Run("PrometheusPortConflictWithAPI", func(t *testing.T) {
		t.Parallel()
		cfg := base()
		cfg.optimus-ide-collabMetricsPort = 3000
		err := cfg.validate()
		require.Error(t, err)
	...
	t.Run("PrometheusPortConflictWithWeb", func(t *testing.T) {
		t.Parallel()
		cfg := base()
		cfg.optimus-ide-collabMetricsPort = 8080
		err := cfg.validate()
		require.Error(t, err)
	...
	t.Run("PrometheusPortConflictWithProxy", func(t *testing.T) {
		t.Parallel()
		cfg := base()
		cfg.optimus-ide-collabMetricsPort = 3010
		cfg.useProxy = true
		err := cfg.validate()
		require.Error(t, err)
	...
	t.Run("PrometheusPortZeroDisabled", func(t *testing.T) {
		t.Parallel()
		cfg := base()
		cfg.optimus-ide-collabMetricsPort = 0
		assert.NoError(t, cfg.validate())
	})
	...
	t.Run("PrometheusPortValid", func(t *testing.T) {
		t.Parallel()
		cfg := base()
		cfg.optimus-ide-collabMetricsPort = 9090
		assert.NoError(t, cfg.validate())
	})
	...
	t.Run("PrometheusPortTooHigh", func(t *testing.T) {
		t.Parallel()
		cfg := base()
		cfg.optimus-ide-collabMetricsPort = 70000
		err := cfg.validate()
		require.Error(t, err)
	...
	t.Run("PrometheusPortNegative", func(t *testing.T) {
		t.Parallel()
		cfg := base()
		cfg.optimus-ide-collabMetricsPort = -1
		err := cfg.validate()
		require.Error(t, err)
		assert.Contains(t, err.Error(), "--prometheus-port must be 0 (disabled) or between 1 and 65535")
	})
	...
	t.Run("PrometheusServerValid", func(t *testing.T) {
		...
		cfg := base()
		cfg.prometheusServer = true
		cfg.optimus-ide-collabMetricsPort = 2114
		assert.NoError(t, cfg.validate())
	})
	...
	t.Run("PrometheusServerPortConflictWithMetrics", func(t *testing.T) {
		...
		cfg := base()
		cfg.prometheusServer = true
		cfg.optimus-ide-collabMetricsPort = prometheusServerPort
		err := cfg.validate()
		require.Error(t, err)
...
func TestDevConfigResolveEnv(t *testing.T) {
	t.Setenv("OPTIMUS-IDE-COLLAB_SESSION_TOKEN", "leaked")
	t.Setenv("OPTIMUS-IDE-COLLAB_URL", "https://leaked.example.com")
	...
	assert.Equal(t, filepath.Join(wd, "build",
		fmt.Sprintf("optimus-ide-collab_%s_%s", runtime.GOOS, runtime.GOARCH)), cfg.binaryPath)
	assert.Equal(t, filepath.Join(wd, ".optimus-ide-collabv2"), cfg.configDir)
	assert.Equal(t, "http://127.0.0.1:3000", cfg.accessURL)
	assert.Equal(t, int64(3000), cfg.apiPort)
	assert.Zero(t, cfg.portOffset)

	// Should have unset leaked env vars.
	assert.Empty(t, os.Getenv("OPTIMUS-IDE-COLLAB_SESSION_TOKEN"))
	assert.Empty(t, os.Getenv("OPTIMUS-IDE-COLLAB_URL"))
...
func TestDevConfigResolveEnvUsesDefaultPortsWithoutPortOffset(t *testing.T) {
	t.Setenv("OPTIMUS-IDE-COLLAB_SESSION_TOKEN", "")
	t.Setenv("OPTIMUS-IDE-COLLAB_URL", "")
...
func TestDevConfigResolveEnvAppliesPortOffsetWhenEnabled(t *testing.T) {
	...
	assert.Equal(t, int64(3010+offset), cfg.proxyPort)
	assert.Equal(t, int64(2114+offset), cfg.optimus-ide-collabMetricsPort)
...
func TestStartPrometheusServerDockerMissing(t *testing.T) {
	...
	cfg := &devConfig{prometheusServer: true, optimus-ide-collabMetricsPort: 2114}
...
func TestPrometheusBannerEntry(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			cfg:       &devConfig{optimus-ide-collabMetricsPort: 0},
			started:   false,
			wantLabel: "",
			wantPort:  0,
		},
		{
			...
			cfg:       &devConfig{optimus-ide-collabMetricsPort: 2114},
			started:   false,
			wantLabel: "Metrics:",
			wantPort:  2114,
		},
		{
			...
			cfg:       &devConfig{optimus-ide-collabMetricsPort: 2114, prometheusServer: true},
			started:   true,
...
func TestParseEnvFileFlag(t *testing.T) {
	...
	t.Run("FallsBackToEnvVar", func(t *testing.T) {
		...
		t.Setenv("OPTIMUS-IDE-COLLAB_DEV_ENV_FILE", "/tmp/from-env.env")
	...
	t.Run("ReturnsEmptyWhenUnset", func(t *testing.T) {
		...
		t.Setenv("OPTIMUS-IDE-COLLAB_DEV_ENV_FILE", "")
		os.Unsetenv("OPTIMUS-IDE-COLLAB_DEV_ENV_FILE")
