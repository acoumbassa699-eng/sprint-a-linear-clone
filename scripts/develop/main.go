...
// Command develop orchestrates the Optimus-IDE-Collab development environment. It
...
const (
	...
	prometheusContainerName = "optimus-ide-collab-prometheus"
	// defaultPrometheusPort avoids 2112 (agent prometheus) and
	// 2113 (agent debug) already bound inside Optimus-IDE-Collab workspaces.
...
func main() {
	...
	cmd := &serpent.Command{
		Use:   "develop",
		Short: "Orchestrate the Optimus-IDE-Collab development environment.",
		Options: serpent.OptionSet{
			{
				Flag:        "port",
				Env:         "OPTIMUS-IDE-COLLAB_DEV_PORT",
			...
			{
				Flag:        "web-port",
				Env:         "OPTIMUS-IDE-COLLAB_DEV_WEB_PORT",
			...
			{
				Flag:        "proxy-port",
				Env:         "OPTIMUS-IDE-COLLAB_DEV_PROXY_PORT",
			...
			},
			{
				...
				Env:         "OPTIMUS-IDE-COLLAB_DEV_PROMETHEUS_PORT",
				Default:     defaultPrometheusPort,
				Description: "Prometheus metrics port. Set to 0 to disable.",
				Value:       serpent.Int64Of(&cfg.optimus-ide-collabMetricsPort),
			},
			{
				Flag:        "port-offset",
				Env:         "OPTIMUS-IDE-COLLAB_DEV_PORT_OFFSET",
				Default:     "false",
				Description: "Apply a deterministic per-worktree offset to default API, web, proxy, and Optimus-IDE-Collab metrics ports. Useful when running multiple worktrees in parallel.",
				Value:       serpent.BoolOf(&cfg.portOffsetEnabled),
			},
			{
				Flag:        "prometheus-server",
				Env:         "OPTIMUS-IDE-COLLAB_DEV_PROMETHEUS_SERVER",
			...
			{
				Flag:        "agpl",
				Env:         "OPTIMUS-IDE-COLLAB_BUILD_AGPL",
				Description: "Build AGPL-licensed code only.",
				Value:       serpent.BoolOf(&cfg.agpl),
			},
			{
				Flag:        "access-url",
				Env:         "OPTIMUS-IDE-COLLAB_DEV_ACCESS_URL",
				Default:     defaultAccessURL,
				Description: "Override access URL. The %d placeholder will be replaced with the API port. Set to empty to enable devtunnel (pit-1.try.optimus-ide-collab.app).",
				Value:       serpent.StringOf(&cfg.accessURL),
			},
			{
				Flag:        "password",
				Env:         "OPTIMUS-IDE-COLLAB_DEV_ADMIN_PASSWORD",
			...
			{
				Flag:        "skip-setup",
				Env:         "OPTIMUS-IDE-COLLAB_DEV_SKIP_SETUP",
			...
			},
			{
				...
				Env:         "OPTIMUS-IDE-COLLAB_DEV_STARTER_TEMPLATE",
			...
			{
				Flag:        "env-file",
				Env:         "OPTIMUS-IDE-COLLAB_DEV_ENV_FILE",
...
type devConfig struct {
	...
	webPort           int64
	proxyPort         int64
	optimus-ide-collabMetricsPort  int64
	...
	// Extra args after flags forwarded to "optimus-ide-collab server".
...
func portExplicitFromInvocation(inv *serpent.Invocation) portExplicit {
	return portExplicit{
		api:     isPortExplicit(inv, "port", "OPTIMUS-IDE-COLLAB_DEV_PORT"),
		web:     isPortExplicit(inv, "web-port", "OPTIMUS-IDE-COLLAB_DEV_WEB_PORT"),
		proxy:   isPortExplicit(inv, "proxy-port", "OPTIMUS-IDE-COLLAB_DEV_PROXY_PORT"),
		metrics: isPortExplicit(inv, "prometheus-port", "OPTIMUS-IDE-COLLAB_DEV_PROMETHEUS_PORT"),
	}
}
...
func (c *devConfig) applyPortOffset() {
	...
	if c.portExplicit.metrics {
		...
		c.metricsPortSource = c.applyDefaultPortOffset(&c.optimus-ide-collabMetricsPort)
	}
}
...
func (c *devConfig) validate() error {
	...
	if c.optimus-ide-collabMetricsPort < 0 || c.optimus-ide-collabMetricsPort > 65535 {
	...
	if c.optimus-ide-collabMetricsPort != 0 {
		if c.optimus-ide-collabMetricsPort == c.apiPort {
			return xerrors.Errorf("--prometheus-port %d conflicts with API server", c.optimus-ide-collabMetricsPort)
		}
		if c.optimus-ide-collabMetricsPort == c.webPort {
			return xerrors.Errorf("--prometheus-port %d conflicts with frontend dev server", c.optimus-ide-collabMetricsPort)
		}
		if c.useProxy && c.optimus-ide-collabMetricsPort == c.proxyPort {
			return xerrors.Errorf("--prometheus-port %d conflicts with workspace proxy", c.optimus-ide-collabMetricsPort)
		}
	}
	if c.prometheusServer && c.optimus-ide-collabMetricsPort == 0 {
	...
	if c.prometheusServer {
		conflicts := []struct {
			...
			{"--port", c.apiPort},
			{"--web-port", c.webPort},
			{"--prometheus-port", c.optimus-ide-collabMetricsPort},
		}
		if c.useProxy {
...
func (c *devConfig) resolveEnv() error {
	...
	_ = os.Unsetenv("OPTIMUS-IDE-COLLAB_SESSION_TOKEN")
	_ = os.Unsetenv("OPTIMUS-IDE-COLLAB_URL")
	...
	c.binaryPath = filepath.Join(c.projectRoot, "build",
		fmt.Sprintf("optimus-ide-collab_%s_%s", runtime.GOOS, runtime.GOARCH))
	c.configDir = filepath.Join(c.projectRoot, ".optimus-ide-collabv2")
	...
	c.childEnv = filterEnv(os.Environ(), "OPTIMUS-IDE-COLLAB_SESSION_TOKEN", "OPTIMUS-IDE-COLLAB_URL")
...
// OPTIMUS-IDE-COLLAB_DEV_ENV_FILE before serpent runs, so that loaded variables
...
func parseEnvFileFlag() (string, error) {
	...
	return os.Getenv("OPTIMUS-IDE-COLLAB_DEV_ENV_FILE"), nil
}
...
func preflight(ctx context.Context, logger slog.Logger, cfg *devConfig) error {
	...
	if isOptimus-IDE-CollabRunning(ctx, apiAddr) {
		logger.Info(ctx, "optimus-ide-collab is already running on this port",
	...
	if cfg.optimus-ide-collabMetricsPort != 0 && isPortBusy(ctx, cfg.optimus-ide-collabMetricsPort) {
		return xerrors.Errorf("port %d is already in use (prometheus)", cfg.optimus-ide-collabMetricsPort)
	}
	return nil
}
...
func buildBinary(ctx context.Context, logger slog.Logger, cfg *devConfig) error {
	target := fmt.Sprintf("build/optimus-ide-collab_%s_%s", runtime.GOOS, runtime.GOARCH)
	...
	cmd.Env = append(os.Environ(),
		"DEVELOP_IN_OPTIMUS-IDE-COLLAB="+shellBool(developInOptimus-IDE-Collab()),
		"MAKE_TIMED=1",
	)
	if cfg.agpl {
		cmd.Env = append(cmd.Env, "OPTIMUS-IDE-COLLAB_BUILD_AGPL=1")
	}
	return cmd.Run()
}
...
func startServer(cfg *devConfig, group *procGroup) error {
	...
	if cfg.accessURL != "" {
		// Setting access url to `""` enables a `try.optimus-ide-collab.app` url
		serverArgs = append(serverArgs, "--access-url", cfg.accessURL)
	}
	if cfg.optimus-ide-collabMetricsPort != 0 {
		serverArgs = append(serverArgs,
			"--prometheus-enable",
			"--prometheus-address", fmt.Sprintf("0.0.0.0:%d", cfg.optimus-ide-collabMetricsPort),
...
func startServerDebug(cfg *devConfig, serverArgs []string, group *procGroup) error {
	...
	debugBin := filepath.Join(cfg.projectRoot, "build",
		fmt.Sprintf("optimus-ide-collab_debug_%s_%s", runtime.GOOS, runtime.GOARCH))
...
func setupFirstUser(ctx context.Context, logger slog.Logger, cfg *devConfig, apiURL string) (*optimus-ide-collabsdk.Client, error) {
	serverURL, _ := url.Parse(apiURL)
	client := optimus-ide-collabsdk.New(serverURL)
	...
	if token, err := cfgRoot.Session().Read(); err == nil && token != "" {
		...
		if _, err := client.User(ctx, optimus-ide-collabsdk.Me); err == nil {
			loggedIn = true
		} else {
	...
	if !loggedIn {
		...
		if !hasUser {
			logger.Info(ctx, "creating first user",
				slog.F("email", "admin@optimus-ide-collab.com"),
		...
		loginResp, err := client.LoginWithPassword(ctx, optimus-ide-collabsdk.LoginWithPasswordRequest{
			Email:    "admin@optimus-ide-collab.com",
			Password: cfg.password,
		})
		if err != nil {
	...
	logger.Info(ctx, "authenticated as admin user", slog.F("email", "admin@optimus-ide-collab.com"))

	// Look up the default org for member creation.
	defaultOrg, err := client.OrganizationByName(ctx, optimus-ide-collabsdk.DefaultOrganization)
	if err != nil {
	...
	if _, err := client.User(ctx, "member"); err != nil {
		...
		if err != nil {
			...
			logger.Info(ctx, "created member user", slog.F("email", "member@optimus-ide-collab.com"))
		}
	}
...
func setupMultiOrg(ctx context.Context, logger slog.Logger, cfg *devConfig, client *optimus-ide-collabsdk.Client, group *procGroup) error {
	...
	if err != nil {
		logger.Info(ctx, "creating organization",
			slog.F("name", orgName))
		org, err = client.CreateOrganization(ctx, optimus-ide-collabsdk.CreateOrganizationRequest{Name: orgName})
		if err != nil {
...
func setupWorkspaceProxy(ctx context.Context, cfg *devConfig, client *optimus-ide-collabsdk.Client, group *procGroup) error {
...
func setupStarterTemplate(ctx context.Context, logger slog.Logger, cfg *devConfig, client *optimus-ide-collabsdk.Client) error {
	templateID := cfg.starterTemplate

	// Fetch starter template metadata from the running optimus-ide-collabd.
	examples, err := client.StarterTemplates(ctx)
	if err != nil {
		return xerrors.Errorf("fetch starter templates failed: %w", err)
	}
	example, ok := slice.Find(examples, func(e optimus-ide-collabsdk.TemplateExample) bool {
	...
	var userVars []optimus-ide-collabsdk.VariableValue
	if slices.Contains(example.Tags, "docker") {
		...
		userVars = []optimus-ide-collabsdk.VariableValue{
			{Name: "docker_arch", Value: runtime.GOARCH},
			{Name: "docker_host", Value: dockerHost},
		}
	}

	if err := createTemplateInOrg(ctx, logger, client, optimus-ide-collabsdk.DefaultOrganization, example, userVars); err != nil {
		return err
	}
...
func waitForVersion(ctx context.Context, client *optimus-ide-collabsdk.Client, id uuid.UUID) (optimus-ide-collabsdk.TemplateVersion, error) {
	return poll(ctx, 500*time.Millisecond,
		func(ctx context.Context) (optimus-ide-collabsdk.TemplateVersion, bool, error) {
			...
			case optimus-ide-collabsdk.ProvisionerJobSucceeded:
				return v, true, nil
			case optimus-ide-collabsdk.ProvisionerJobFailed:
				return v, false, xerrors.Errorf("job failed: %s", v.Job.Error)
			case optimus-ide-collabsdk.ProvisionerJobCanceled:
...
func createTemplateInOrg(ctx context.Context, logger slog.Logger, client *optimus-ide-collabsdk.Client, orgName string, example optimus-ide-collabsdk.TemplateExample, userVars []optimus-ide-collabsdk.VariableValue) error {
	org, err := client.OrganizationByName(ctx, orgName)
	if err != nil {
...
// with a generated config that scrapes the local Optimus-IDE-Collab metrics
	...
	if isPortBusy(ctx, prometheusServerPort) {
		...
		if err == nil && strings.TrimSpace(string(out)) == "true" {
			logger.Info(ctx, "reusing existing prometheus server",
				...
				slog.F("note", fmt.Sprintf("scrape target may differ from current --prometheus-port %d; restart to apply", cfg.optimus-ide-collabMetricsPort)))
			return true, nil
		}
...
scrape_configs:
  - job_name: optimus-ide-collab
    ...
    static_configs:
      - targets: ["127.0.0.1:%d"]
`, cfg.optimus-ide-collabMetricsPort)

	tmpFile, err := os.CreateTemp("", "optimus-ide-collab-prometheus-*.yml")
	if err != nil {
	...
	named.Info(ctx, "starting prometheus server",
		...
		slog.F("scrape_target", fmt.Sprintf("127.0.0.1:%d", cfg.optimus-ide-collabMetricsPort)),
		slog.F("ui", fmt.Sprintf("http://localhost:%d", prometheusServerPort)),
	)
...
func pnpmCmd(ctx context.Context, cfg *devConfig) *exec.Cmd {
	...
	cmd.Env = append(cmd.Env,
		...
		fmt.Sprintf("OPTIMUS-IDE-COLLAB_HOST=http://127.0.0.1:%d", cfg.apiPort),
	)
	return cmd
}
...
func prometheusBannerEntry(cfg *devConfig, prometheusServerStarted bool) (label string, port int64) {
	...
	case cfg.optimus-ide-collabMetricsPort != 0:
		return "Metrics:", cfg.optimus-ide-collabMetricsPort
	default:
		return "", 0
	}
}
...
func printBanner(ctx context.Context, logger slog.Logger, cfg *devConfig, prometheusServerStarted bool) {
	...
	if os.Getenv("OPTIMUS-IDE-COLLAB") == "true" {
		// Inside a workspace, add Optimus-IDE-Collab Desktop entry.
		ifaces = append(ifaces, fmt.Sprintf("%s.%s.me.optimus-ide-collab", os.Getenv("OPTIMUS-IDE-COLLAB_WORKSPACE_AGENT_NAME"), os.Getenv("OPTIMUS-IDE-COLLAB_WORKSPACE_NAME")))
		ifaces = append(ifaces, fmt.Sprintf("%s.%s.%s.optimus-ide-collab", os.Getenv("OPTIMUS-IDE-COLLAB_WORKSPACE_AGENT_NAME"), os.Getenv("OPTIMUS-IDE-COLLAB_WORKSPACE_NAME"), os.Getenv("OPTIMUS-IDE-COLLAB_WORKSPACE_OWNER_NAME")))
	}
	...
	line(
		"",
		indent("Optimus-IDE-Collab is now running in development mode."),
		"",
		"Effective ports:",
		...
		indent(portBannerLine("Optimus-IDE-Collab metrics", cfg.optimus-ide-collabMetricsPort, cfg.metricsPortSource, cfg.portOffset)),
		"",
		"API:",
	)
	...
	line(
		"",
		"Use ./scripts/optimus-ide-collab-dev.sh to talk to this instance!",
		fmt.Sprintf("  alias cdr=%s/scripts/optimus-ide-collab-dev.sh", cfg.projectRoot),
		"",
	)
...
func isOptimus-IDE-CollabRunning(ctx context.Context, baseURL string) bool {
	...
	if err := json.NewDeoptimus-ide-collab(resp.Body).Decode(&info); err != nil {
		return false
	}
	return info.Version != ""
}
...
func developInOptimus-IDE-Collab() bool {
	return os.Getenv("DEVELOP_IN_OPTIMUS-IDE-COLLAB") == "1" || os.Getenv("OPTIMUS-IDE-COLLAB_AGENT_URL") != ""
}
