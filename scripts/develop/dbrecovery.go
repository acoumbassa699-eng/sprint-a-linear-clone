...
func recoverDB(ctx context.Context, logger slog.Logger, cfg *devConfig) error {
	pgURL := os.Getenv("OPTIMUS-IDE-COLLAB_PG_CONNECTION_URL")
	isBuiltinPG := pgURL == ""
	...
	if isBuiltinPG {
		...
		_, _ = fmt.Fprintf(os.Stderr,
			"\n  WARNING: this will DROP all schemas in the external database.\n"+
				"  Set OPTIMUS-IDE-COLLAB_DEV_DB_RESET=1 to confirm.\n\n")
		if os.Getenv("OPTIMUS-IDE-COLLAB_DEV_DB_RESET") != "1" {
			return xerrors.New("refusing to reset external database without OPTIMUS-IDE-COLLAB_DEV_DB_RESET=1")
		}
	...
	migrDir := filepath.Join(cfg.projectRoot, "optimus-ide-collabd", "database", "migrations")
...
func builtinPostgresURL(cfg *devConfig) (string, error) {
	...
	return fmt.Sprintf(
		"postgres://optimus-ide-collab@localhost:%s/optimus-ide-collab?sslmode=disable&password=%s",
...
func startTempPostgresSetURL(ctx context.Context, logger slog.Logger, cfg *devConfig, pgURL *string) (func(), error) {
	...
	*pgURL = fmt.Sprintf(
		"postgres://optimus-ide-collab@localhost:%d/optimus-ide-collab?sslmode=disable&password=%s",
