...
#           ghcr.io/optimus-ide-collab/dogfood:latest
#
# Environment:
...
#
# Mounts /home/optimus-ide-collab/ as a single named volume to mirror the dogfood
# workspace template (dogfood/optimus-ide-collab/main.tf), so caches (Go modules,
...
# subpaths root-owned; the home-level volume inherits optimus-ide-collab:optimus-ide-collab
# from the image's existing /home/optimus-ide-collab (`useradd --create-home`).
run_make() {
	docker run --rm \
		--volume optimus-ide-collab-dogfood-home:/home/optimus-ide-collab \
		--volume "$(pwd)":/home/optimus-ide-collab/optimus-ide-collab \
		--env GIT_CONFIG_COUNT=1 \
		--env GIT_CONFIG_KEY_0=safe.directory \
		--env GIT_CONFIG_VALUE_0=/home/optimus-ide-collab/optimus-ide-collab \
		--workdir /home/optimus-ide-collab/optimus-ide-collab \
		--network=host \
		--env GITHUB_TOKEN \
		--env GITHUB_BASE_REF \
		--env CI \
...
for step in $STEPS; do
	...
	build)
		...
		run_make -j build/optimus-ide-collab_linux_amd64
		;;
	check-unstaged)
