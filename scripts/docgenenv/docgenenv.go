...
// Prepare clears OPTIMUS-IDE-COLLAB_* variables and pins the cache, config, and temp
// directories so generated docs don't embed the generating host's home
// directory.
func Prepare() {
	for _, env := range os.Environ() {
		if !strings.HasPrefix(env, "OPTIMUS-IDE-COLLAB_") {
			continue
		}
	...
	mustSetenv("CLIDOCGEN_CONFIG_DIRECTORY", "~/.config/optimus-ide-collabv2")
