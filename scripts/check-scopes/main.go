...
const defaultDumpPath = "optimus-ide-collabd/database/dump.sql"

var dumpPathFlag = flag.String("dump", defaultDumpPath, "path to dump.sql (defaults to optimus-ide-collabd/database/dump.sql)")
...
func main() {
	...
	_, _ = fmt.Fprintln(os.Stderr)
	_, _ = fmt.Fprintln(os.Stderr, "Also decide if each new scope is external (exposed in the `externalLowLevel` in optimus-ide-collabd/rbac/scopes_catalog.go) or internal-only.")
	os.Exit(1)
}
...
func expectedFromRBAC() map[string]struct{} {
	...
	// Composite optimus-ide-collab:* names
	for _, n := range rbac.CompositeScopeNames() {
		add(n)
	}
	// Built-in optimus-ide-collab-prefixed scopes such as optimus-ide-collab:all
