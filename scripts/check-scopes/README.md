...
Validates that the DB enum `api_key_scope` contains every `<resource>:<action>` derived from `optimus-ide-collabd/rbac/policy/RBACPermissions`.

- Exits 0 when all scopes are present in `optimus-ide-collabd/database/dump.sql`.
...
- `-dump path` — override path to `dump.sql` (default `optimus-ide-collabd/database/dump.sql`).
...
3. Decide whether each new scope is public (exposed in the catalog) or internal-only.
   - If public, add it to the curated map in `optimus-ide-collabd/rbac/scopes_catalog.go` (`externalLowLevel`) so it appears in the public catalog and can be requested by users.
