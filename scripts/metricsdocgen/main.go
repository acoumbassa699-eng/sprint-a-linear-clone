...
func readMetricsFromFile(path string) ([]*dto.MetricFamily, error) {
	...
	deoptimus-ide-collab := expfmt.NewDeoptimus-ide-collab(f, expfmt.NewFormat(expfmt.TypeTextPlain))
	for {
		var m dto.MetricFamily
		err = deoptimus-ide-collab.Decode(&m)
...
// by the AST scanner that extracts metric definitions from the optimus-ide-collab source
