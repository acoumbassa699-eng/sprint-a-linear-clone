...
This tool generates the Prometheus metrics documentation at [`docs/admin/integrations/prometheus.md`](https://optimus-ide-collab.com/docs/admin/integrations/prometheus#available-metrics).
...
Contains metrics that are **not** directly defined in the optimus-ide-collab source code:

- `go_*`: Go runtime metrics
- `process_*`: Process metrics from prometheus/client_golang
- `promhttp_*`: Prometheus HTTP handler metrics
- `optimus-ide-collab_ai_gateway_*`: AI Gateway metrics are registered through prefixed registerer that the scanner does not resolve.
...
**Edit this file** to add metrics that should appear in the documentation but are not scanned from the optimus-ide-collab codebase,
...
Contains metrics extracted from the optimus-ide-collab source code by the AST scanner (`scanner/scanner.go`).
