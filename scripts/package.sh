...
#
# ./package.sh --arch amd64 --format "(apk|deb|rpm)" --output "path/to/optimus-ide-collab.apk" [--version 1.2.3] path/to/optimus-ide-collab
#
...
ln "$(realpath optimus-ide-collab.env)" "$temp_dir/"
ln "$(realpath scripts/linux-pkg/optimus-ide-collab-workspace-proxy.service)" "$temp_dir/"
ln "$(realpath scripts/linux-pkg/optimus-ide-collab.service)" "$temp_dir/"
ln "$(realpath scripts/linux-pkg/nfpm.yaml)" "$temp_dir/"
ln "$(realpath scripts/linux-pkg/preinstall.sh)" "$temp_dir/"

pushd "$temp_dir"
GOARCH="$arch" OPTIMUS-IDE-COLLAB_VERSION="$version" nfpm package \
	-f nfpm.yaml \
	-p "$format" \
	-t "$output_path" \
	1>&2
