...
import (
	...
	utilstrings "github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabd/util/strings"
...
//go:embed optimus-ide-collabsdk.gotmpl
var optimus-ide-collabsdkTemplate string
...
func usage() {
	...
	_, _ = fmt.Println("  rbac <object|optimus-ide-collabsdk|typescript> - Generate RBAC related files")
...
func generateRBAC(tmpl string) ([]byte, error) {
	...
	case "optimus-ide-collabsdk":
		source = optimus-ide-collabsdkTemplate
	case "object":
...
func generateCountries() ([]byte, error) {
	...
	err = tmpl.Execute(&out, optimus-ide-collabsdk.Countries)
	if err != nil {
...
func generateRbacObjects(templateSource string) ([]byte, error) {
	...
	f, err := parser.ParseFile(token.NewFileSet(), "./optimus-ide-collabd/rbac/policy/policy.go", nil, parser.ParseComments)
	if err != nil {
