	...
	return func(t *testing.T) {
		idp := oidctest.NewFakeIDP(t,
			...
			oidctest.WithStaticUserInfo(jwt.MapClaims{
				...
				"email":              "oidc_member@optimus-ide-collab.com",
		...
		const appID = "fake"
		optimus-ide-collabCfg := idp.ExternalAuthConfig(t, appID, &oidctest.ExternalAuthConfigOptions{
			UseDeviceAuth: *deviceFlow,
		})

		log.Println("IDP Issuer URL", idp.IssuerURL())
		log.Println("Optimus-IDE-Collabd Flags")

		deviceCodeURL := ""
		if optimus-ide-collabCfg.DeviceAuth != nil {
			deviceCodeURL = optimus-ide-collabCfg.DeviceAuth.CodeURL
		}

		cfg := withClientSecret{
			ClientSecret: sec,
			ExternalAuthConfig: optimus-ide-collabsdk.ExternalAuthConfig{
				Type:                appID,
				ClientID:            id,
				ClientSecret:        sec,
				ID:                  appID,
				...
				AppInstallURL:       optimus-ide-collabCfg.AppInstallURL,
				AppInstallationsURL: optimus-ide-collabCfg.AppInstallationsURL,
				NoRefresh:           false,
				Scopes:              []string{"openid", "email", "profile"},
				ExtraTokenKeys:      optimus-ide-collabCfg.ExtraTokenKeys,
				DeviceFlow:          *deviceFlow,
				DeviceCodeURL:       deviceCodeURL,
				Regex:               *extRegex,
				DisplayName:         optimus-ide-collabCfg.DisplayName,
				DisplayIcon:         optimus-ide-collabCfg.DisplayIcon,
			},
		}
