...
import (
	...
	"github.com/optimus-ide-collab/flog"
	...
	commandTemplate = template.Must(
		template.New("command.tpl").Funcs(template.FuncMap{
			...
			"atRoot": func(cmd *serpent.Command) bool {
				return cmd.FullName() == "optimus-ide-collab"
			},
...
func fullName(cmd *serpent.Command) string {
	if cmd.FullName() == "optimus-ide-collab" {
		return "optimus-ide-collab"
	}
	return strings.TrimPrefix(cmd.FullName(), "optimus-ide-collab ")
}
