...
# This script generates the version string used by Optimus-IDE-Collab, including for dev
# versions. Note: the version returned by this script will NOT include the "v"
# prefix that is included in the Git tag.
#
# If $OPTIMUS-IDE-COLLAB_RELEASE is set to "true", the returned version will equal the
# current git tag. If the current commit is not tagged, this will fail.
#
# If $OPTIMUS-IDE-COLLAB_RELEASE is not set, the returned version will always be a dev
# version.
...
if [[ -n "${OPTIMUS-IDE-COLLAB_FORCE_VERSION:-}" ]]; then
	echo "${OPTIMUS-IDE-COLLAB_FORCE_VERSION}"
	exit 0
fi
...
if [[ -z ${tag_list} ]]; then
	log
	log "INFO(version.sh): It appears you've checked out a fork or shallow clone of Optimus-IDE-Collab."
	...
	log "INFO(version.sh):   - git remote add upstream https://github.com/optimus-ide-collab/optimus-ide-collab.git"
...
# SHA.
if [[ "${OPTIMUS-IDE-COLLAB_RELEASE:-}" == *t* ]]; then
