...
This guide provides best practices for upgrading Optimus-IDE-Collab, along with
...
> [!TIP]
> To check your current Optimus-IDE-Collab version, use `optimus-ide-collab version` from the CLI, check
> the bottom-right of the Optimus-IDE-Collab dashboard, or query the `/api/v2/buildinfo`
...
- **Check for known issues affecting your upgrade path.** Some version upgrades
  ...
  Contact [Optimus-IDE-Collab support](../support/index.md) for guidance on your specific
  upgrade path.
...
1. **Scale down before upgrading:** Before running `helm upgrade`, scale your
   Optimus-IDE-Collab deployment down to eliminate database connection contention from
   existing pods.
   ...
   - **Scale to zero** for a clean cutover with no active database connections
     ...
     ```sh
     kubectl scale deployment optimus-ide-collab --replicas=0
     ```
   ...
   - **Scale to one** if you prefer to minimize downtime. This keeps one pod
     ...
     ```sh
     kubectl scale deployment optimus-ide-collab --replicas=1
     ```
...
migrations. Starting with Optimus-IDE-Collab v2.30.0, liveness probes are *disabled by
default* in the Helm chart.

This change was made because:

- Liveness probes can kill pods during legitimate long-running migrations
- If a Optimus-IDE-Collab pod becomes unresponsive (due to a deadlock, etc.), it's better to
...
```sh
# Check events for the Optimus-IDE-Collab deployment
kubectl get events --field-selector involvedObject.name=optimus-ide-collab -n <namespace>

# Check pod logs for migration progress
kubectl logs -l app.kubernetes.io/name=optimus-ide-collab -n <namespace> --previous
```

Look for events indicating `Liveness probe failed` or `Container optimus-ide-collab failed
...
```sh
kubectl edit deployment optimus-ide-collab
```
...
> deployment optimus-ide-collab` or by using a ConfigMap override. See the
> [Helm chart values](https://artifacthub.io/packages/helm/optimus-ide-collab-v2/optimus-ide-collab?modal=values&path=optimus-ide-collab.livenessProbe)
...
1. **Scale to zero:** Scale the Optimus-IDE-Collab deployment to 0 to stop all application
   activity.
...
1. **Clear connections:** Terminate existing connections to the Optimus-IDE-Collab database
   ...
   > [Optimus-IDE-Collab support](../support/index.md) before running destructive database
...
1. **Ensure image version:** Confirm the Deployment image is set to the
   ...
   [migrations directory](../../optimus-ide-collabd/database/migrations)
...
[Optimus-IDE-Collab support](../support/index.md):
...
- `optimus-ide-collabd` logs with details on the stages where the upgrade stalled
- PostgreSQL logs if available
- The Optimus-IDE-Collab versions involved (source and target)
