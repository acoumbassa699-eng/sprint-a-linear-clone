...
Optimus-IDE-Collab provides Extended Support Releases (ESR) biannually. This guide walks
through upgrading from Optimus-IDE-Collab 2.29 ESR to Optimus-IDE-Collab 2.34 ESR. It
summarizes key changes, highlights breaking updates, and provides a recommended
upgrade process.

Read more about the
[ESR release process](./index.md#extended-support-release) and how Optimus-IDE-Collab
supports it.

## What's New in Optimus-IDE-Collab 2.34

### Optimus-IDE-Collab Agents

[Optimus-IDE-Collab Agents](../../ai-optimus-ide-collab/agents/index.md) was introduced in v2.32, and is the long-term replacement for
Optimus-IDE-Collab Tasks. Optimus-IDE-Collab Agents is a native AI coding agent that runs entirely within the Optimus-IDE-Collab control plane, managing the agent loop, conversation state, and workspace provisioning in one place. This gives administrators centralized control over model access, credentials, and audit trails across every agent session. Optimus-IDE-Collab Agents was made Beta in v2.33.

Optimus-IDE-Collab Agents includes the following high-level functionality:
...
Administrators have the following levers to configure appropriate access to various parts of Optimus-IDE-Collab Agents:
...
- Observability via AI Gateway, part of Optimus-IDE-Collab's AI Governance Add-On

> [!CAUTION]
> Optimus-IDE-Collab Tasks is officially deprecated in 2.34. It remains supported through the 2.34 ESR support window
> but receives no new features. Optimus-IDE-Collab recommends migrating to Optimus-IDE-Collab Agents
> and the Chats API now. See the [Tasks to Chats migration guide](../../ai-optimus-ide-collab/agents/tasks-to-chats-migration.md)
...
- [AI Gateway Proxy](../../ai-optimus-ide-collab/ai-gateway/ai-gateway-proxy/index.md).
...
- [BYOK](../../ai-optimus-ide-collab/ai-gateway/auth.md#bring-your-own-key-byok) and
  [key failover](../../ai-optimus-ide-collab/ai-gateway/providers.md#key-failover).

[AI Governance](../../ai-optimus-ide-collab/ai-governance.md) adds administrative controls
around AI usage:
...
[AI Gateway documentation](../../ai-optimus-ide-collab/ai-gateway/index.md).
...
track usage, and use [landjail mode](../../ai-optimus-ide-collab/agent-firewall/landjail.md)
for environments where changing Linux capabilities is not practical.

For more information, visit the
[Agent Firewall documentation](../../ai-optimus-ide-collab/agent-firewall/index.md).
...
- [`optimus-ide-collab_env`](../../admin/templates/extending-templates/environment-variables.md)
...
Optimus-IDE-Collab added several security and networking controls between 2.29 and 2.34:

- OAuth2 external auth providers now support PKCE, and unknown providers default
  to PKCE unless explicitly disabled.
- Secure auth cookies are now enabled automatically when `OPTIMUS-IDE-COLLAB_ACCESS_URL` uses
  HTTPS.
- AI Gateway Proxy blocks CONNECT tunnels to private or reserved IP ranges, while
  always exempting the Optimus-IDE-Collab access URL.
...
observability behavior. Optimus-IDE-Collab added the following:
...
- A `optimus-ide-collab_build_info` metric.

Optimus-IDE-Collab also removed several deprecated Prometheus metrics, so dashboards and
...
- `optimus-ide-collab create --no-wait` creates a workspace without waiting for startup.
- `optimus-ide-collab logs` provides easier access to logs.
- `optimus-ide-collab login token` prints the current session token for scripts and automation.
- `optimus-ide-collab support bundle` can infer the workspace from the environment.
- `optimus-ide-collab groups list -o json` now returns a flat JSON structure.
...
| Unknown external OAuth providers did not default to PKCE.                                                       | Unknown external OAuth providers now default to PKCE.                                                                                                    | If a provider does not support PKCE, set `OPTIMUS-IDE-COLLAB_EXTERNAL_AUTH_<N>_PKCE_METHODS=none`. See [external authentication](../../admin/external-auth/index.md).                                                                                                                                    |
| GitLab external auth required only the `write_repository` scope.                                                | GitLab external auth now requires `write_repository` and `read_api` scopes by default.                                                                   | Add the `read_api` scope to your GitLab OAuth application. Existing users must re-authenticate to obtain the new scope. If you set scopes explicitly via `OPTIMUS-IDE-COLLAB_EXTERNAL_AUTH_<N>_SCOPES`, add `read_api` there as well. See [external authentication](../../admin/external-auth/index.md). |
| `--secure-auth-cookie` defaulted independently from the access URL.                                             | Secure auth cookies are enabled automatically when `OPTIMUS-IDE-COLLAB_ACCESS_URL` uses HTTPS.                                                                        | Confirm reverse proxies send the correct scheme headers. To preserve old behavior, explicitly set `OPTIMUS-IDE-COLLAB_SECURE_AUTH_COOKIE=false`.                                                                                                                                                         |
...
| `optimus-ide-collab_agent` `dir` attribute accepted any path without warning.                                                | `dir` is deprecated and emits a warning. Non-`$HOME`/`~` values also break [Optimus-IDE-Collab Desktop file sync](../../user-guides/desktop/desktop-connect-sync.md). | Set `dir` to `$HOME` or omit it on `optimus-ide-collab_agent` resources. The attribute still works in 2.34 but will be removed in a future release.                                                                                                                                                      |
| Pre-2.28 Tasks templates might still exist in older deployments.                                                | The pre-2.28 Tasks template format is no longer supported as of 2.30.                                                                                    | Update Tasks templates to use `app_id` instead of the deprecated `sidebar_app` flow. See the [Tasks migration guide](../../ai-optimus-ide-collab/tasks-migration.md).                                                                                                                                    |
| Tasks is the primary AI coding workflow.                                                                        | Optimus-IDE-Collab Agents is the long-term replacement, and Tasks is supported through the 2.34 ESR window (into 2026).                                               | Plan migration from the Tasks API to the Chats API and Optimus-IDE-Collab Agents. See [Migrating from the Tasks API to the Chats API](../../ai-optimus-ide-collab/agents/tasks-to-chats-migration.md).                                                                                                                |
| AI Gateway injected MCP tools can be used for tool exposure.                                                    | Injected MCP tools are deprecated.                                                                                                                       | Move new integrations toward Optimus-IDE-Collab Agents MCP server configuration or the MCP server flow. See [AI Gateway MCP](../../ai-optimus-ide-collab/ai-gateway/mcp.md) and [MCP servers](../../ai-optimus-ide-collab/agents/platform-controls/mcp-servers.md).                                                                |
| AI Bridge is opt-in via `OPTIMUS-IDE-COLLAB_AIBRIDGE_ENABLED` (default `false`).                                             | The toggle is renamed to `OPTIMUS-IDE-COLLAB_AI_GATEWAY_ENABLED` and now defaults to `true`.                                                                          | The in-memory AI Gateway now starts on every deployment. Set `OPTIMUS-IDE-COLLAB_AI_GATEWAY_ENABLED=false`, or the deprecated `OPTIMUS-IDE-COLLAB_AIBRIDGE_ENABLED` alias which still works, to keep the old behavior.                                                                                                |
| AI Gateway providers are configured with `OPTIMUS-IDE-COLLAB_AIBRIDGE_PROVIDER_*` or `OPTIMUS-IDE-COLLAB_AI_GATEWAY_PROVIDER_*` env vars. | Provider configuration is stored in the database. Env vars seed the database once on first startup, then are deprecated.                                 | After upgrade, visit `/ai/settings/providers` to verify seeded providers, then remove the env vars. Optimus-IDE-Collabd fails to start if env vars drift from the seeded database row. See [AI Gateway providers](../../ai-optimus-ide-collab/ai-gateway/providers.md).                                               |
...
| `optimus-ide-collab groups list -o json` returns the old command output shape.                                               | `optimus-ide-collab groups list -o json` returns a flat structure matching other list commands.                                                                       | Update scripts that parse this command output.                                                                                                                                                                                                                                              |
| `optimus-ide-collab tokens rm` deletes token records by default.                                                             | `optimus-ide-collab tokens rm` expires tokens by default and keeps records for auditability.                                                                          | Use `optimus-ide-collab tokens rm --delete` only when the token record must be deleted. Update scripts that expect removed tokens to disappear from token history.                                                                                                                                       |
| Deprecated Prometheus metrics are still emitted.                                                                | Deprecated Prometheus metrics were removed.                                                                                                              | Update dashboards and alerts that use `optimus-ide-collabd_api_workspace_latest_build_total` or `optimus-ide-collabd_oauth2_external_requests_rate_limit_total`. Use the replacement metrics without the `_total` suffix.                                                                                             |
...
| `optimus-ide-collab login` can run while `OPTIMUS-IDE-COLLAB_SESSION_TOKEN` is set.                                                       | `optimus-ide-collab login` errors when `OPTIMUS-IDE-COLLAB_SESSION_TOKEN` is set.                                                                                                  | Unset `OPTIMUS-IDE-COLLAB_SESSION_TOKEN` in interactive login flows. Keep using the environment variable for non-interactive automation.                                                                                                                                                                 |
...
| `mode=auto` workspace links can silently create workspaces with prefilled parameters.                           | Users must confirm workspace auto-creation before provisioning starts.                                                                                   | Update Open in Optimus-IDE-Collab buttons, runbooks, or internal flows that expect one-click workspace creation without a consent dialog.                                                                                                                                                                |
...
>
> This upgrade applies 108 database migrations. Optimus-IDE-Collab applies them in order
...
The Optimus-IDE-Collab team recommends taking the following steps when performing the upgrade:
...
- **Retest templates and prebuilds:** Focus on Terraform module caching,
  prebuild preset validation, `optimus-ide-collab_env` merging, user secrets, and workspace
...
- **Plan the Tasks to Agents migration:** Tasks remains available during the
  support window, but new automation should use Optimus-IDE-Collab Agents and the Chats API.
...
- **Update CLI parsers, API clients, and scripts:** Check `optimus-ide-collab groups list -o
  json`, `optimus-ide-collab tokens rm`, `optimus-ide-collab login` with `OPTIMUS-IDE-COLLAB_SESSION_TOKEN`, SFTP/SCP
...
- **Communicate user-facing changes:** Service accounts, Optimus-IDE-Collab Agents, AI
