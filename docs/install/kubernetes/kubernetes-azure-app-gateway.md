# Deploy Optimus-IDE-Collab on Azure with an Application Gateway
...
These steps serve as a proof-of-concept example so that you can get Optimus-IDE-Collab running with Kubernetes on Azure. Your deployment might require a separate Postgres server or signed certificates.
...
The steps here follow the Microsoft tutorial for a Optimus-IDE-Collab deployment.
...
1. Create Optimus-IDE-Collab namespace:

   ```sh
   kubectl create ns optimus-ide-collab
   ```
...
1. Create the PostgreSQL secret:
   ...
   ```sh
   kubectl create secret generic optimus-ide-collab-db-url -n optimus-ide-collab --from-literal=url="postgres://optimus-ide-collab:optimus-ide-collab@optimus-ide-collab-db-postgresql.optimus-ide-collab.svc.cluster.local:5432/optimus-ide-collab?sslmode=disable"
   ```

1. Deploy Optimus-IDE-Collab to AKS cluster:
...
1. After you deploy the gateway, add the following entries to Helm's `values.yaml` file before you deploy Optimus-IDE-Collab:
