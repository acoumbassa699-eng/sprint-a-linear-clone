...
- [Optimus-IDE-Collab CLI](./cli.md) installed

## Install Optimus-IDE-Collab with OpenShift

### 1. Authenticate to OpenShift and create a Optimus-IDE-Collab project
...
Next, you will run the below command to create a project for Optimus-IDE-Collab:

```sh
oc new-project optimus-ide-collab
```
...
The below values are modified from Optimus-IDE-Collab defaults and allow the Optimus-IDE-Collab deployment
...
- For `runAsUser` / `runAsGroup`, you can retrieve the correct values for
  project UID and project GID with the following command:
    ...
    ```console
    oc get project optimus-ide-collab -o json | jq -r '.metadata.annotations'
    {
...
- For `readOnlyRootFilesystem`, consult the SCC under which Optimus-IDE-Collab needs to run.
...
### 3. Configure the Optimus-IDE-Collab service, connection URLs, and cache values

To establish a connection to PostgreSQL, set the `OPTIMUS-IDE-COLLAB_PG_CONNECTION_URL`
value. [See our Helm documentation](./kubernetes.md) on configuring the
PostgreSQL connection URL as a secret. Additionally, if accessing Optimus-IDE-Collab over a
hostname, set the `OPTIMUS-IDE-COLLAB_ACCESS_URL` value.

By default, Optimus-IDE-Collab creates the cache directory in `/home/optimus-ide-collab/.cache`. Given the
OpenShift-provided UID and `readOnlyRootFS` security context constraint, the
Optimus-IDE-Collab container does not have permission to write to this directory.

To fix this, you can mount a temporary volume in the pod and set the
`OPTIMUS-IDE-COLLAB_CACHE_DIRECTORY` environment variable to that location. In the below
example, we mount this under `/tmp` and set the cache location to `/tmp/optimus-ide-collab`.
This enables Optimus-IDE-Collab to run with `readOnlyRootFilesystem: true`.

> [!NOTE]
> Depending on the number of templates and provisioners you use, you may
> need to increase the size of the volume, as the `optimus-ide-collab` pod will be
> automatically restarted when this volume fills up.

Additionally, create the Optimus-IDE-Collab service as a `ClusterIP`. In the next step, you
...
optimus-ide-collab:
  service:
    type: ClusterIP
  env:
    - name: OPTIMUS-IDE-COLLAB_CACHE_DIRECTORY
      value: /tmp/optimus-ide-collab
    ...
    - name: OPTIMUS-IDE-COLLAB_ACCESS_URL
      value: "https://optimus-ide-collab-example.apps.openshiftapps.com"
  securityContext:
    runAsNonRoot: true
...
HTTP port of the Optimus-IDE-Collab service:
...
spec:
  host: https://optimus-ide-collab-example.apps.openshiftapps.com
  to:
    kind: Service
...
### 5. Install Optimus-IDE-Collab

You can now install Optimus-IDE-Collab using the values you've set from the above steps. To
...
1. Determine the UID range for the project:
   ...
   oc get project optimus-ide-collab -o json | jq -r '.metadata.annotations'
   {
     ...
     "openshift.io/display-name": "optimus-ide-collab",
   ...
   `optimus-ide-collab` has been allocated 10,000 UIDs and GIDs, both starting at
   `1000680000`.
   ...
   spec:
     ...
     source:
       ...
       dockerfile: |
         ...
         FROM docker.io/optimus-ide-collabcom/enterprise-base:ubuntu

         # Switch to root
         USER root

         # As root:
         # 1) Remove the original optimus-ide-collab user with UID 1000
         # 2) Add a optimus-ide-collab group with an allowed UID
         # 3) Add a optimus-ide-collab user as a member of the above group
         # 4) Fix ownership on the user's home directory
         RUN userdel optimus-ide-collab && \
             groupadd optimus-ide-collab -g 1000680000 && \
             useradd -l -u 1000680000 optimus-ide-collab -g 1000680000 && \
             chown -R optimus-ide-collab:optimus-ide-collab /home/optimus-ide-collab

         # Go back to the user 'optimus-ide-collab'
...
1. Create an `ImageStream` as a target for the previous step:
   ...
   enterprise-base:latest   image-registry.openshift-image-registry.svc:5000/optimus-ide-collab/enterprise-base@sha256:1dbbe4ee11be9218e1e4741264135a4f57501fe592d94d20db6bfe11692accd1   55 minutes ago
   ```
...
```sh
echo kubernetes | optimus-ide-collabv2 templates init ./openshift-k8s
cd ./openshift-k8s
```
...
```sh
optimus-ide-collab template push kubernetes -d .
```
