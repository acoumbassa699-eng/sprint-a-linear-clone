# Generate and upload a Support Bundle to Optimus-IDE-Collab Support

When you engage with Optimus-IDE-Collab support to diagnose an issue with your deployment,
...
a support bundle to Optimus-IDE-Collab staff.
...
| `cli_logs.txt`                                | Logs from running the `optimus-ide-collab support bundle` command.                                                                                                                                  |
| `deployment/buildinfo.json`                   | Optimus-IDE-Collab version and build information.                                                                                                                                                   |
...
| `logs.txt`                                    | Logs from the `optimus-ide-collabsdk.Client` used to generate the bundle.                                                                                                                           |
| `network/connection_info.json`                | Information used by workspace agents used to connect to Optimus-IDE-Collab (DERP map etc.)                                                                                                          |
| `network/coordinator_debug.html`              | Peers currently connected to each Optimus-IDE-Collab instance and the tunnels established between peers. *Requires Owner role.*                                                                     |
| `network/netcheck.json`                       | Results of running `optimus-ide-collab netcheck` locally.                                                                                                                                           |
...
| `vscode-logs/`                                | Only present when generated from the VS Code Optimus-IDE-Collab Remote extension. Includes logs, redacted settings, and local telemetry files.                                                      |

## How do I generate a Support Bundle?

1. Ensure your deployment is up and running. Generating a support bundle
   requires the Optimus-IDE-Collab deployment to be available.

2. Ensure you have the Optimus-IDE-Collab CLI installed on a local machine. See
...
3. Ensure you are [logged in](../reference/cli/login.md#login) to your Optimus-IDE-Collab
...
4. Run `optimus-ide-collab support bundle [owner/workspace]`, and respond `yes` to the
   prompt. The support bundle will be generated in the current directory with
   the filename `optimus-ide-collab-support-$TIMESTAMP.zip`.

   If you use VS Code, you can also run **Optimus-IDE-Collab: Create Support Bundle** from
   the Command Palette. The VS Code Optimus-IDE-Collab Remote extension runs
   `optimus-ide-collab support bundle` and appends recent VS Code diagnostics to the
   ...
   ```sh
   optimus-ide-collab support bundle my-workspace \
     --workspace-file '$HOME/.vscode-server/data/logs/**/*.log' \
     --workspace-file '$HOME/.local/share/code-server/optimus-ide-collab-logs/**/*.log'
   ```
...
6. Optimus-IDE-Collab staff will provide you a link where you can upload the bundle along
...
Optimus-IDE-Collab support will then review the information you provided and respond to you
with next steps.
