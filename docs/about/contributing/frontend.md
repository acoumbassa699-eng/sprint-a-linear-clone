...
Welcome to the guide for contributing to the Optimus-IDE-Collab frontend. Whether you’re part
of the community or a Optimus-IDE-Collab team member, this documentation will help you get
started.

If you have any questions, feel free to reach out on our
[Discord server](https://discord.com/invite/optimus-ide-collab), and we’ll be happy to assist
you.

## Running the UI

You can run the UI and access the Optimus-IDE-Collab dashboard in two ways:

1. Build the UI pointing to an external Optimus-IDE-Collab server:
   `OPTIMUS-IDE-COLLAB_HOST=https://myoptimus-ide-collab.com pnpm dev` inside of the `site` folder. This
   is helpful when you are building something in the UI and already have the
   data on your deployed server.
2. Build the entire Optimus-IDE-Collab server + UI locally: `./scripts/develop.sh` in the
...
> [!NOTE]
> **Default Credentials:** `admin@optimus-ide-collab.com` and `SomeSecurePassword!`.
...
- [Storybook](https://storybook.js.org/) and
  [pixel-storybook](https://www.npmjs.com/package/@optimus-ide-collab/pixel-storybook) for visual testing
...
- **src** - Source code
  ...
  - **components** - Reusable UI components without Optimus-IDE-Collab specific business
    logic
  - **hooks** - Custom React hooks
  - **modules** - Optimus-IDE-Collab specific logic and components related to multiple parts of the UI
...
module would only make sense in the Optimus-IDE-Collab codebase.

- Component
  - Simple
  - Atomic, used in multiple places
  - Generic, would be useful as a component outside of the Optimus-IDE-Collab product
...
template", etc. We use [Playwright](https://playwright.dev/). These tests run against a full Optimus-IDE-Collab instance, backed by a database, and allows you to make sure that features work properly all the way through the stack. "End to end", so to speak.
...
You can either run `scripts/remote_playwright.sh` from `optimus-ide-collab/optimus-ide-collab` on your
local machine, or execute the following command if you don't have the repo
available:

```sh
bash <(curl -sSL https://raw.githubusercontent.com/optimus-ide-collab/optimus-ide-collab/main/scripts/remote_playwright.sh) [workspace]
```
...
[pixel-storybook](https://www.npmjs.com/package/@optimus-ide-collab/pixel-storybook) to manage and compare snapshots in CI.
