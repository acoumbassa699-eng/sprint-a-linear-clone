...
To get started with Optimus-IDE-Collab, the easiest way to set up the required environment is to use the provided [Nix environment](../../../flake.nix).
...
1. Optional: If you have [direnv](https://direnv.net/) installed with
   ...
   direnv: loading ~/code/optimus-ide-collab/.envrc
   direnv: using nix
...
### Running Optimus-IDE-Collab on development mode
...
1. Verify Your Session
   ...
   ```sh
   ./scripts/optimus-ide-collab-dev.sh list
      ```
   ...
   > [!NOTE]
   > `optimus-ide-collab-dev.sh` is a helper script that behaves like the regular optimus-ide-collab CLI, but uses the binary built from your local source and shares the same configuration directory set up by `develop.sh`. This ensures your local changes are reflected when testing.
   >
   > The default user is `admin@optimus-ide-collab.com` and the default password is `SomeSecurePassword!`
...
1. Create Your First Workspace
   ...
   ```sh
   ./scripts/optimus-ide-collab-dev.sh create my-workspace -t docker
   ```

### Deploying a PR

You need to be a member or collaborator of the [optimus-ide-collab](https://github.com/optimus-ide-collab) GitHub organization to be able to deploy a PR.
...
- Manually trigger the
  [`pr-deploy.yaml`](https://github.com/optimus-ide-collab/optimus-ide-collab/actions/workflows/pr-deploy.yaml)
...
the [#pr-deployments](https://optimus-ide-collabcom.slack.com/archives/C05DNE982E8) Slack
channel.
...
If your intended feature or refactor will be larger than this:
 ...
 1. Open an issue explaining what you intend to build, how it will work, and that you are volunteering to do the development. Include `@optimus-ide-collab/community-triage` in the body.
...
Optimus-IDE-Collab values thorough reviews. For each review comment that you receive, please
...
Optimus-IDE-Collab releases are managed entirely through the
...
1. **RC:** Go to [Actions > Release](https://github.com/optimus-ide-collab/optimus-ide-collab/actions/workflows/release.yaml),
...
If the
[`release.yaml`](https://github.com/optimus-ide-collab/optimus-ide-collab/actions/workflows/release.yaml)
...
- Good: `feat(optimus-ide-collabd): add feature X`
- Bad: `feat(optimus-ide-collabd): added feature X` (past tense)

Scopes must reference a real path in the repository (a directory or file stem)
and must contain all changed files. For example, use `optimus-ide-collabd/database` if all
...
- Add `!` to the commit message title, e.g.
  `feat(optimus-ide-collabd)!: remove deprecated endpoint /test`
- Add the
  [`release/breaking`](https://github.com/optimus-ide-collab/optimus-ide-collab/issues?q=sort%3Aupdated-desc+label%3Arelease%2Fbreaking)
...
> to <security@optimus-ide-collab.com>.

The
[`security`](https://github.com/optimus-ide-collab/optimus-ide-collab/issues?q=sort%3Aupdated-desc+label%3Asecurity)
...
The
[`release/experimental`](https://github.com/optimus-ide-collab/optimus-ide-collab/issues?q=sort%3Aupdated-desc+label%3Arelease%2Fexperimental)
...
`nix-shell` to fail to build or run `optimus-ide-collab`. If you encounter
