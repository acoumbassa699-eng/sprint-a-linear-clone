...
A Quickstart workspace keeps your home directory, `/home/optimus-ide-collab`, on a persistent volume.
Everything outside `/home/optimus-ide-collab` comes from the workspace image, and Optimus-IDE-Collab rebuilds it from that image every time the workspace starts.

A tool survives a restart only when both of these are true:

- The tool installs into `/home/optimus-ide-collab`.
- Your shell finds the tool through a file in `/home/optimus-ide-collab`, such as `.bashrc`.
...
Open your workspace in the Optimus-IDE-Collab dashboard and select **Restart**.
...
```sh
optimus-ide-collab restart <your-workspace>
optimus-ide-collab ssh <your-workspace>
```
...
mise installed `bat` under `/home/optimus-ide-collab`, which persists, so `bat` survived.
Homebrew installed `ripgrep` to `/home/linuxbrew`, outside `/home/optimus-ide-collab`, so the rebuild discarded Homebrew and every formula you installed with it.
The `brew shellenv` line stayed in your `.bashrc` because it lives in `/home/optimus-ide-collab`, which is why your shell still tries to load the missing `brew` and prints the error above.
...
resource "docker_volume" "homebrew_volume" {
  name = "optimus-ide-collab-${data.optimus-ide-collab_workspace.me.id}-homebrew"
  lifecycle {
    ignore_changes = all
  }
}
...
Then mount it in the `docker_container "workspace"` resource, alongside the block that mounts `/home/optimus-ide-collab`:
...
Make the edits in `~/optimus-ide-collab-quickstart/main.tf`, then publish and update by name:
...
Homebrew now persists because `/home/linuxbrew` lives on its own volume, the same way mise's tools persist because they live in `/home/optimus-ide-collab`.
...
It installs prebuilt binaries into `/home/optimus-ide-collab`, which already persists, so the first start stays quick and later starts reuse the tools.

In `main.tf`, add a script next to the existing `install_languages` resource:

```tf
resource "optimus-ide-collab_script" "install_tools" {
  ...
  script       = <<-EOT
    ...
    # Install mise on first start. It lives in /home/optimus-ide-collab, so later starts reuse it.
...
Add the resource in `~/optimus-ide-collab-quickstart/main.tf`, then publish and update by name:
...
- mise installs into `~/.local/share/mise`, inside your home directory, and activates from `~/.bashrc`.
  Both are in `/home/optimus-ide-collab`, so its tools persist with no template change.
- Homebrew installs to `/home/linuxbrew`, outside `/home/optimus-ide-collab`, so its tools are discarded on every restart until you mount that path on a persistent volume.
...
Refer to [Resource persistence](../../admin/templates/extending-templates/resource-persistence.md) for how Optimus-IDE-Collab decides what survives a restart.
...
- [Resource persistence](../../admin/templates/extending-templates/resource-persistence.md) in the Optimus-IDE-Collab documentation
- [Dotfiles](../../user-guides/workspace-dotfiles.md) in the Optimus-IDE-Collab documentation
