...
A parameter is a question Optimus-IDE-Collab asks when someone creates a workspace.
Each parameter comes from a `optimus-ide-collab_parameter` [data source](https://developer.hashicorp.com/terraform/language/data-sources) in the template.
...
In `main.tf`, find the `data "optimus-ide-collab_parameter" "languages"` block.
...
> Optimus-IDE-Collab reads the parameter's choices from these blocks, so a misplaced `option` doesn't appear in the form.

The `icon` value points at an icon bundled with your Optimus-IDE-Collab deployment.
To browse the full set and copy a path, open `https://<your-optimus-ide-collab-url>/icons`.
...
Make the edit in `~/optimus-ide-collab-quickstart/main.tf`, then publish a new version:
...
3. Apply the change. Optimus-IDE-Collab updates the workspace to the new template version and restarts it with Ruby selected.

### CLI

Update the workspace and re-select its parameters.
Replace `<your-workspace>` with your workspace's name (run `optimus-ide-collab list` to see it):
...
> If you replace the `apt-get` line with `brew install ruby`, the build fails: the `optimus-ide-collabcom/enterprise-base:ubuntu` image doesn't include Homebrew, so the workspace logs `brew: command not found` and Ruby never installs.
...
Make the edit in `~/optimus-ide-collab-quickstart/install-languages.sh.tftpl`, then publish and update:
...
```tf
data "optimus-ide-collab_workspace_preset" "ruby_dev" {
...
Expanding the preset parameters after selecting **Ruby Development** shows the parameter values Optimus-IDE-Collab will use to create the workspace:
...
- The `optimus-ide-collab_parameter` `option` block added Ruby to the workspace creation form.
...
<details>
<summary>How does Optimus-IDE-Collab install the language you pick?</summary>

The selection travels through the template in four steps:

1. The `option` block in `data "optimus-ide-collab_parameter" "languages"` (`main.tf`) adds `ruby` to the values the form accepts.
...
3. `optimus-ide-collab_script.install_languages` renders the startup script with that list and runs it on the agent (also in `main.tf`):
