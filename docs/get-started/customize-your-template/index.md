...
In [Launch your first workspace](../index.md), you installed the `optimus-ide-collab` CLI, started the Optimus-IDE-Collab server, and built a workspace from the Quickstart template.
...
Every Optimus-IDE-Collab template declares the `optimus-ide-collab` provider inside a `required_providers` block:
...
- A [provider](https://developer.hashicorp.com/terraform/language/providers) is a plugin that connects Terraform to a platform or service.
  The `optimus-ide-collab` provider connects Terraform to Optimus-IDE-Collab.
...
- A [module](https://developer.hashicorp.com/terraform/language/modules) is a reusable bundle of resources, data sources, providers, and other Terraform blocks you pull in by reference.
  ...
  You can pull modules from the [Optimus-IDE-Collab Registry](https://registry.optimus-ide-collab.com).
...
Open it for editing now, either in the Optimus-IDE-Collab web editor or in your local editor with the `optimus-ide-collab` CLI.

<div class="tabs">

### UI

1. Log in to Optimus-IDE-Collab and select **Templates**.
2. Find the **Optimus-IDE-Collab Quickstart** template you created and open it.
3. Select the three-dots menu next to **Create Workspace**, then select **Edit files**.

The template web editor opens:

![Optimus-IDE-Collab template web editor](../../images/screenshots/template-web-editor.png)_Optimus-IDE-Collab template web editor_
...
`optimus-ide-collab login` opens a browser page that generates a session token.
Copy the token, return to the terminal, and paste it.
Then pull the template:

```sh
optimus-ide-collab templates pull quickstart ~/optimus-ide-collab-quickstart
```

Open the `~/optimus-ide-collab-quickstart` folder in your preferred editor.
...
The guides use the CLI command `optimus-ide-collab templates push`, which reads from the `~/optimus-ide-collab-quickstart` folder you pulled.
...
- [Extending templates](../../admin/templates/extending-templates/index.md) in the Optimus-IDE-Collab documentation
