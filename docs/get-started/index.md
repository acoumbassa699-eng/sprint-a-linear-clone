...
Follow this guide to get your first Optimus-IDE-Collab development environment
...
- ✅ Install Optimus-IDE-Collab server.
...
## A 30-second metaphor for Optimus-IDE-Collab

Before diving in, the following table breaks down the core concepts that power Optimus-IDE-Collab,
...
**Putting it Together:** Optimus-IDE-Collab separates who _defines_ environments from who _uses_ them. Admins create and manage Templates, the recipes, while developers use those Templates to launch Workspaces, the meals.
...
> [!TIP]
> If you use a coding agent like Claude Code, the [optimus-ide-collab/skills](https://github.com/optimus-ide-collab/skills) `setup` skill can train the coding agent on the following steps (install a container runtime, install Optimus-IDE-Collab, create your first template, and launch a workspace).

## Step 1: Install a container runtime

Optimus-IDE-Collab needs a Docker-compatible container runtime running on the host, such as
...
[Step 2](#step-2-install-and-start-optimus-ide-collab). Otherwise, follow the steps below to
...
1. Point `DOCKER_HOST` at the Colima socket so Optimus-IDE-Collab can reach the daemon:

   ```sh
   export DOCKER_HOST="unix://${HOME}/.colima/default/docker.sock"
   ```

   Colima exposes its Docker socket at `~/.colima/default/docker.sock`, not `/var/run/docker.sock`, so Optimus-IDE-Collab needs `DOCKER_HOST` to find it.
   Set it in the same terminal where you'll run `optimus-ide-collab server` in the next step.
...
## Step 2: Install and start Optimus-IDE-Collab

Install the `optimus-ide-collab` CLI to get started:

<div class="tabs">

### Linux/macOS

1. Install Optimus-IDE-Collab:
   ...
   - For standalone binaries, system packages, or other alternate installation
     ...
     [latest release on GitHub](https://github.com/optimus-ide-collab/optimus-ide-collab/releases/latest).

1. Start Optimus-IDE-Collab:
...
Optimus-IDE-Collab will attempt to open the setup page in your browser. If it doesn't open
...
If your Optimus-IDE-Collab server is on a network or cloud device, or you are having trouble
viewing the page, locate the web UI URL in Optimus-IDE-Collab logs in your terminal. It looks
like `https://<CUSTOM-STRING>.<TUNNEL>.try.optimus-ide-collab.app`. It's one of the first
lines of output, so you might have to scroll up to find it.

This section includes some of the quicker ways to install Optimus-IDE-Collab. For more detailed options, visit the [Install](../install/index.md) page.
...
1. Create your admin account:
   ...
   account. Optimus-IDE-Collab automatically grants admin permissions to the first user that signs in.

   ![Welcome to Optimus-IDE-Collab - Create admin user](../images/screenshots/welcome-create-admin-user.png)

## Step 4: Create your first template and workspace

> [!TIP]
> If you use an AI coding assistant, the [optimus-ide-collab-templates](https://github.com/optimus-ide-collab/registry/blob/main/.agents/skills/optimus-ide-collab-templates/SKILL.md) agent skill can guide you through creating and customizing templates with best practices built-in.
...
1. Select **Create Template**. Optimus-IDE-Collab composes and validates the Terraform
...
any modules you chose, and generated a valid Terraform configuration. Optimus-IDE-Collab
...
A Optimus-IDE-Collab template is a [Terraform](https://developer.hashicorp.com/terraform/intro) configuration, and Optimus-IDE-Collab is built on top of Terraform.
When you create a workspace from this template, a Optimus-IDE-Collab [provisioner](../admin/infrastructure/architecture.md#provisionerd) runs a Terraform job from the template's configuration to build your environment.
For the Docker base template, that job starts a Docker container with the Optimus-IDE-Collab agent pre-configured, along with any modules you selected.

To learn how Optimus-IDE-Collab uses Terraform to provision and run workspaces, refer to the [architecture overview](../admin/infrastructure/architecture.md).
...
After a short wait (10-15 seconds on most modern computers), Optimus-IDE-Collab will start your new workspace:
...
This guide uses **VS Code Desktop**, which opens the workspace in the VS Code installed on your local machine, using the Optimus-IDE-Collab extension.
...
1. Select **Clone Repository** and enter the repository URL.
   ...
   For example, to clone the Optimus-IDE-Collab repo, enter
   `https://github.com/optimus-ide-collab/optimus-ide-collab.git`.
...
4. You are now using VS Code in your Optimus-IDE-Collab environment!

## What's next?

You now have:

- A Optimus-IDE-Collab server running locally.
...
- [Try Optimus-IDE-Collab Agents](../ai-optimus-ide-collab/agents/getting-started.md), the chat
  interface and API for delegating development work to coding agents in your
  Optimus-IDE-Collab deployment.

- [Read about managing Workspaces for your team](../user-guides/workspace-management.md)

- [Read about implementing monitoring tools for your Optimus-IDE-Collab Deployment](../admin/monitoring/index.md)
...
This usually means a container runtime is either not installed or not running on the machine where Optimus-IDE-Collab is running.
A runtime must be running before you create a workspace from a Docker-based template.

If the runtime is running but Optimus-IDE-Collab still cannot connect, the daemon may expose its socket at a path other than `/var/run/docker.sock`.
This is common with Colima on macOS and with rootless Docker on Linux.
In that case, point Optimus-IDE-Collab at the socket with the `DOCKER_HOST` environment variable, then restart the Optimus-IDE-Collab server.
...
1. Assign your user to the `docker` group so Optimus-IDE-Collab can access the daemon
   without root:
...
1. If `docker ps` works but Optimus-IDE-Collab still cannot connect, point `DOCKER_HOST` at the Colima socket, then restart the Optimus-IDE-Collab server:
...
### Can't start Optimus-IDE-Collab server: Address already in use

```txt
Encountered an error running "optimus-ide-collab server", see "optimus-ide-collab server --help" for more information
...
1. Stop the process:
   ...
   ```sh
   sudo systemctl stop optimus-ide-collab
   ```
