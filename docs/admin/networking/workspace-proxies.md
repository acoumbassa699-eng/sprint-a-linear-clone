...
Optimus-IDE-Collab's networking does a best effort to make direct connections to a workspace.
...
port 443 to be open and are expected to have network connectivity to the optimus-ide-collabd
...
- The [Optimus-IDE-Collab CLI](../../reference/cli/index.md) must be installed and
...
authenticate back to primary optimus-ide-collabd.

```sh
$ optimus-ide-collab wsproxy create --name=newyork --display-name="USA East" --icon="/emojis/2194.png"
...
Deploying the workspace proxy will also register the proxy with optimus-ide-collabd and make
the workspace proxy usable. If the proxy deployment is successful,
`optimus-ide-collab wsproxy ls` will show an `ok` status code:

```sh
$ optimus-ide-collab wsproxy ls
NAME              URL                         STATUS STATUS
primary           https://dev.optimus-ide-collab.com        ok
...
- `unhealthy` : The workspace proxy is reachable, but has some issue that is
  preventing the proxy from being used. `optimus-ide-collab wsproxy ls` should show the error
  message.
- `ok` : The workspace proxy is healthy and working properly!

### Configuration

Workspace proxy configuration overlaps with a subset of the optimus-ide-collabd
configuration. To see the full list of configuration options:
`optimus-ide-collab wsproxy server --help`

```sh
# Proxy specific configuration. These are REQUIRED
# Example: https://optimus-ide-collabd.example.com
OPTIMUS-IDE-COLLAB_PRIMARY_ACCESS_URL="https://<url_of_optimus-ide-collabd_dashboard>"
OPTIMUS-IDE-COLLAB_PROXY_SESSION_TOKEN="<session_token_from_proxy_create>"

# Runtime variables for "optimus-ide-collab start".
OPTIMUS-IDE-COLLAB_HTTP_ADDRESS=0.0.0.0:80
OPTIMUS-IDE-COLLAB_TLS_ADDRESS=0.0.0.0:443
# Example: https://east.optimus-ide-collabd.example.com
OPTIMUS-IDE-COLLAB_ACCESS_URL="https://<access_url_of_proxy>"
# Example: *.east.optimus-ide-collabd.example.com
OPTIMUS-IDE-COLLAB_WILDCARD_ACCESS_URL="*.<app_hostname_of_proxy>"

OPTIMUS-IDE-COLLAB_TLS_ENABLE=true
OPTIMUS-IDE-COLLAB_TLS_CLIENT_AUTH=none
OPTIMUS-IDE-COLLAB_TLS_CERT_FILE="<cert_file_location>"
OPTIMUS-IDE-COLLAB_TLS_KEY_FILE="<key_file_location>"
...
optimus-ide-collab:
  env:
    - name: OPTIMUS-IDE-COLLAB_PRIMARY_ACCESS_URL
      value: "https://<url_of_optimus-ide-collabd_dashboard>"
    - name: OPTIMUS-IDE-COLLAB_PROXY_SESSION_TOKEN
      value: "<session_token_from_proxy_create>"
    # Example: https://east.optimus-ide-collabd.example.com
    - name: OPTIMUS-IDE-COLLAB_ACCESS_URL
      value: "https://<access_url_of_proxy>"
    # Example: *.east.optimus-ide-collabd.example.com
    - name: OPTIMUS-IDE-COLLAB_WILDCARD_ACCESS_URL
...
```sh
helm install optimus-ide-collab optimus-ide-collab-v2/optimus-ide-collab --namespace <your workspace proxy namespace> -f ./values-wsproxy.yaml
```

Test that the workspace proxy is reachable with `curl -vvv`. If for some reason,
the Optimus-IDE-Collab dashboard still shows the workspace proxy is `UNHEALTHY`, scale down
...
optimus-ide-collab wsproxy server
```

### Running as a system service

If you've installed Optimus-IDE-Collab via a [system package](../../install/index.md), you
can configure the workspace proxy by settings in
`/etc/optimus-ide-collab.d/optimus-ide-collab-workspace-proxy.env`

To run workspace proxy as a system service on the host:

```sh
# Use systemd to start workspace proxy now and on reboot
sudo systemctl enable --now optimus-ide-collab-workspace-proxy

# View the logs to ensure a successful start
journalctl -u optimus-ide-collab-workspace-proxy.service -b
```

To restart workspace proxy after applying system changes:

```sh
sudo systemctl restart optimus-ide-collab-workspace-proxy
```

### Running in Docker

Modify the default entrypoint to run a workspace proxy server instead of a
regular Optimus-IDE-Collab server.
...
+ entrypoint: /opt/optimus-ide-collab wsproxy server
```

#### Docker run

```sh
docker run --rm -it --entrypoint /opt/optimus-ide-collab ghcr.io/optimus-ide-collab/optimus-ide-collab:latest wsproxy server
```
...
Users can select a workspace proxy at the top-right of the browser-based Optimus-IDE-Collab
...
Optimus-IDE-Collab workspace proxy exports metrics via the HTTP endpoint, which can be
