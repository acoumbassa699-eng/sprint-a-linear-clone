...
`optimus-ide-collab ping <workspace>` will ping the workspace agent and print diagnostics on
...
✔ sent local data to Optimus-IDE-Collab networking coordinator
...
`OPTIMUS-IDE-COLLAB_BLOCK_DIRECT` environment variable or the `--block-direct-connections`
flag on the server. When set, this will be reflected in the output of
`optimus-ide-collab ping`.
...
client/agent and STUN servers in most cases. `optimus-ide-collab ping` will indicate if
either the Optimus-IDE-Collab agent or client had issues sending or receiving UDP packets to
STUN servers.

If this is the case, you may need to add exceptions to the firewall to allow UDP
for Optimus-IDE-Collab workspaces, clients, and STUN servers.
...
`optimus-ide-collab ping` will indicate if it's possible the client or agent is behind a hard
NAT.
...
resolve the public IP of the other. `optimus-ide-collab ping` will indicate if no STUN
servers were found.
...
If this is suspected, you may need to add an exception for Optimus-IDE-Collab to the
...
If this is the case, you may need to add exceptions to the VPN for Optimus-IDE-Collab, modify
...
Use `optimus-ide-collab ping` to check for MTU issues, as it inspects
network interfaces on both the client and the workspace agent:

```console
$ optimus-ide-collab ping my-workspace
...
Possible client-side issues with direct connection:
...
flag or `OPTIMUS-IDE-COLLAB_BLOCK_DIRECT` environment variable on the Optimus-IDE-Collab server.

## Throughput

The `optimus-ide-collab speedtest <workspace>` command measures the throughput between the
client and the workspace agent.

```console
$ optimus-ide-collab speedtest workspace
29ms via optimus-ide-collab
