...
context of a Optimus-IDE-Collab workspace agent and client, this is generally not the case,
...
This problem is often referred to as NAT traversal, and Optimus-IDE-Collab uses a standard
...
order for the Optimus-IDE-Collab client and agent to establish a direct connection with each
...
- **Coordination:** The client and agent then exchange this information through
  the Optimus-IDE-Collab server. They will then construct packets that should be able to
...
They then exchange this information through Optimus-IDE-Collab server, and can then
...
2. Ensure firewalls on client devices (e.g. Windows Defender Firewall) have an
   inbound policy allowing all UDP ports either to the `optimus-ide-collab` or `optimus-ide-collab.exe`
