...
There are four ways to forward ports in Optimus-IDE-Collab:

| Method                                 | Details                                                                                                                                                        |
|:---------------------------------------|:---------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [Optimus-IDE-Collab Desktop](#optimus-ide-collab-desktop)        | Automatic port forwarding via VPN tunnel. All workspace ports are available at `workspace.optimus-ide-collab:PORT` with no manual setup. Supports peer-to-peer connections. |
| [CLI](#the-optimus-ide-collab-port-forward-command) | Forwards specific TCP or UDP ports from the workspace to local ports. Supports peer-to-peer connections.                                                       |
...
Optimus-IDE-Collab Desktop and `optimus-ide-collab port-forward` are generally more performant than:

1. The Dashboard which proxies traffic through the Optimus-IDE-Collab control plane versus
   peer-to-peer which is possible with the Optimus-IDE-Collab CLI and Optimus-IDE-Collab Desktop
1. `sshd` which does double encryption of traffic with both Wireguard and SSH

## Optimus-IDE-Collab Desktop

[Optimus-IDE-Collab Desktop](../../user-guides/desktop/index.md) provides automatic port forwarding to every service running in your workspace.
Once Optimus-IDE-Collab Connect is enabled, any port your application listens on is instantly accessible at `<workspace-name>.optimus-ide-collab:PORT` from your local machine, with no additional commands or configuration.

This is the simplest option for most users. See the [Optimus-IDE-Collab Desktop documentation](../../user-guides/desktop/index.md) for installation and setup.
...
```console
optimus-ide-collab port-forward myworkspace.agent --tcp 3000,9990-9999
```
...
### From an optimus-ide-collab_app resource
...
optimus-ide-collab users or publicly. Using the open ports interface, developers can assign a
sharing levels that match our `optimus-ide-collab_app`’s share option in
[Optimus-IDE-Collab terraform provider](https://registry.terraform.io/providers/optimus-ide-collab/optimus-ide-collab/latest/docs/resources/app#share-1).
...
This can also be used to change the sharing level of `optimus-ide-collab_app`s by entering
their port number in the sharable ports UI. The `share` attribute on `optimus-ide-collab_app`
...
When forwarding via the dashboard, Optimus-IDE-Collab automatically sets headers that allow
...
Optimus-IDE-Collab does not intercept these cases. See below for the required headers.
...
```txt
Access to fetch at
'<https://optimus-ide-collab.example.com/api/v2/applications/auth-redirect>' from origin
'<https://8000--dev--user--apps.optimus-ide-collab.example.com>' has been blocked by CORS
...
Below is a list of the cross-origin headers Optimus-IDE-Collab sets with example values:

```txt
access-control-allow-credentials: true
access-control-allow-methods: PUT
access-control-allow-headers: X-Custom-Header
access-control-allow-origin: https://8000--dev--user--apps.optimus-ide-collab.example.com
vary: Origin
