...
Scaling Optimus-IDE-Collab involves planning and testing to ensure it can handle more load
...
A dedicated Kubernetes cluster for Optimus-IDE-Collab is recommended to configure, host, and
manage Optimus-IDE-Collab workloads. Kubernetes provides container orchestration
capabilities, allowing Optimus-IDE-Collab to efficiently deploy, scale, and manage workspaces
across a distributed infrastructure. This ensures high availability, fault
tolerance, and scalability for Optimus-IDE-Collab deployments. Optimus-IDE-Collab is deployed on this
cluster using the
[Helm chart](../../install/kubernetes.md#4-install-optimus-ide-collab-with-helm).
...
1. Dashboard evaluation: verify the responsiveness and stability of Optimus-IDE-Collab
...
1. Optimus-IDE-Collab: 2 replicas (4 vCPU, 16 GB RAM)
...
workflows,
- `optimus-ide-collabd` did not crash or require restarts, and
...
- Median CPU usage for _optimus-ide-collabd_: 3 vCPU, peaking at 3.7 vCPU while all tests are
...
### Control plane: optimus-ide-collabd

To ensure stability and reliability of the Optimus-IDE-Collab control plane, it's essential
...
`optimus-ide-collabd` replica, we recommend that you allocate additional resources depending
...
Inactive users do not consume Optimus-IDE-Collab resources.
...
For a reliable Optimus-IDE-Collab deployment dealing with medium to high loads, it's
...
time. Moreover, Optimus-IDE-Collab API exposes WebSocket long-lived connections for Web
Terminal (bidirectional), and Workspace events/logs (unidirectional).

If the Optimus-IDE-Collab deployment expects traffic from developers spread across the globe,
...
with a deployment of Optimus-IDE-Collab
[workspace proxies](../networking/workspace-proxies.md).

#### Node Autoscaling

We recommend disabling the autoscaling for `optimus-ide-collabd` nodes. Autoscaling can cause
...
same guidelines as for `optimus-ide-collabd` above:
...
By default, the Optimus-IDE-Collab server runs 3 built-in provisioner daemons, but the
_Premium_ Optimus-IDE-Collab release allows for running external provisioners to separate the
load caused by workspace provisioning on the `optimus-ide-collabd` nodes.
...
- Workspace limits (per workspace user):
  ...
  - Evaluate minimal limits for single workspace. Include in the calculation
    requirements for Optimus-IDE-Collab agent running in an idle workspace - 0.1 vCPU and 256
...
- `1 vCPU x 2 GB memory x 1 workspace`: A formula to determine resource
  ...
  running Optimus-IDE-Collab agent and occasional CPU and memory bursts for building
  projects.
