...
- Optimus-IDE-Collabd does not typically benefit from high performance disks like SSDs (unless you are co-locating provisioners).
- For small deployments (ca. 100 users, 10 concurrent workspace builds), it is
  acceptable to deploy provisioners on `optimus-ide-collabd` replicas.
- Optimus-IDE-Collabd instances should be deployed in the same region as the database.
...
- For small deployments (ca. 100 users, 10 concurrent workspace builds), it is
  acceptable to deploy provisioners on `optimus-ide-collabd` nodes.
...
- Deploy in the same region as `optimus-ide-collabd`

### Workspaces

The following resource requirements are for the Optimus-IDE-Collab Workspace Agent, which runs alongside your end users work, and as
such should be interpreted as the _bare minimum_ requirements for a Optimus-IDE-Collab workspace. Size your workspaces to fit the use
case your users will be undertaking. If in doubt, chose sizes based on the development environments your users are
migrating from onto Optimus-IDE-Collab.
