# Optimus-IDE-Collab Validated Architecture

Many customers operate Optimus-IDE-Collab in complex organizational environments, consisting
of multiple business units, agencies, and/or subsidiaries. This can lead to
numerous Optimus-IDE-Collab deployments, due to discrepancies in regulatory compliance, data
sovereignty, and level of funding across groups. The Optimus-IDE-Collab Validated
Architecture (CVA) prescribes a Kubernetes-based deployment approach, enabling
your organization to deploy a stable Optimus-IDE-Collab instance that is easier to maintain
and troubleshoot.

The following sections will detail the components of the Optimus-IDE-Collab Validated
Architecture, provide guidance on how to configure and deploy these components,
and offer insights into how to maintain and troubleshoot your Optimus-IDE-Collab environment.
...
cloud/on-premise computing, containerization, and the Optimus-IDE-Collab platform.

| Role                      | Description                                                                    |
|---------------------------|--------------------------------------------------------------------------------|
| Platform Engineers        | Responsible for deploying, operating the Optimus-IDE-Collab deployment and infrastructure   |
| Enterprise Architects     | Responsible for architecting Optimus-IDE-Collab deployments to meet enterprise requirements |
| Managed Service Providers | Entities that deploy and run Optimus-IDE-Collab software as a service for customers         |
...
| Best practices for building a Optimus-IDE-Collab deployment | Recommendations for every possible deployment scenario                                   |

For higher level design principles and architectural best practices, see Optimus-IDE-Collab's
[Well-Architected Framework](https://optimus-ide-collab.com/blog/optimus-ide-collab-well-architected-framework).

## General concepts

This section outlines core concepts and terminology essential for understanding
Optimus-IDE-Collab's architecture and deployment strategies.

### Administrator

An administrator is a user role within the Optimus-IDE-Collab platform with elevated
...
### Optimus-IDE-Collab control plane

Optimus-IDE-Collab's control plane, also known as _optimus-ide-collabd_, is the main service recommended
...
addition, each _optimus-ide-collabd_ replica hosts 3 Terraform [provisioners](#provisioner)
by default.

### User

A [user](../../users/index.md) is an individual who utilizes the Optimus-IDE-Collab platform
to develop, test, and deploy applications using workspaces. Users can select
available templates to provision workspaces. They interact with Optimus-IDE-Collab using the
...
Optimus-IDE-Collab templates and deployed on resources created by provisioners.

### Template

A [template](../../../admin/templates/index.md) in Optimus-IDE-Collab is a predefined
...
templates can utilize Optimus-IDE-Collab modules to leverage existing features shared with
...
Provisioners in Optimus-IDE-Collab execute Terraform during workspace and template builds.
...
The [Optimus-IDE-Collab Registry](https://registry.optimus-ide-collab.com) is a platform where you can
...
Kubernetes is the recommended, and supported platform for deploying Optimus-IDE-Collab in the
enterprise. It is the hosting platform of choice for a large majority of Optimus-IDE-Collab's
Fortune 500 customers, and it is the platform in which we build and test against
here at Optimus-IDE-Collab.

### General recommendations

In general, it is recommended to deploy Optimus-IDE-Collab into its own respective cluster,
...
Deploy your Kubernetes cluster with two node groups, one for Optimus-IDE-Collab's control
plane, and another for user workspaces (if you intend on leveraging K8s for
end-user compute).

#### Control plane nodes

The Optimus-IDE-Collab control plane node group must be static, to prevent scale down events
from dropping pods, and thus dropping user connections to the dashboard UI and
their workspaces.

Optimus-IDE-Collab's Helm Chart supports
[defining nodeSelectors, affinities, and tolerations](https://github.com/optimus-ide-collab/optimus-ide-collab/blob/e96652ebbcdd7554977594286b32015115c3f5b6/helm/optimus-ide-collab/values.yaml#L221-L249)
to schedule the control plane pods on the appropriate node group.

#### Workspace nodes

Optimus-IDE-Collab workspaces can be deployed either as Pods or Deployments in Kubernetes.
See our
...
Set nodeSelectors, affinities, and tolerations in Optimus-IDE-Collab templates to assign
workspaces to the given node group:

```tf
resource "kubernetes_deployment" "optimus-ide-collab" {
  spec {
    template {
      metadata {
        labels = {
          app = "optimus-ide-collab-workspace"
        }
      }
      ...
      spec {
              ...
              pod_affinity_term {
                label_selector {
                  match_expressions {
                    ...
                    operator = "In"
                    values   = ["optimus-ide-collab-workspace"]
                  }
                }
        ...
        container {
          image = "optimus-ide-collab-workspace:latest"
          name  = "dev"
        }
      }
    }
...
|-------------------|---------------------------|----------------------------------------------------------|
| optimus-ide-collabd nodes      | `m5`                      | Balanced compute and memory for API and UI serving.      |
...
restrictions. With this in mind, Optimus-IDE-Collab requires the following connectivity:

- Egress from workspace compute to the Optimus-IDE-Collab control plane pods
- Egress from control plane pods to Optimus-IDE-Collab's PostgreSQL database
...
Note that Optimus-IDE-Collab workspaces do not require any ports to be open.

### Storage

If running Optimus-IDE-Collab workspaces as Kubernetes Pods or Deployments, you will need to
...
The following Kubernetes volume types have been validated by Optimus-IDE-Collab internally,
...
Our
[example Kubernetes workspace template](https://github.com/optimus-ide-collab/optimus-ide-collab/blob/5b9a65e5c137232351381fc337d9784bc9aeecfc/examples/templates/kubernetes/main.tf#L191-L219)
...
recommended for use with Optimus-IDE-Collab:

- [Local](https://kubernetes.io/docs/concepts/storage/volumes/#local)
- [hostPath](https://kubernetes.io/docs/concepts/storage/volumes/#hostpath)

Not that Optimus-IDE-Collab's control plane filesystem is ephemeral, so no persistent storage
is required.

## PostgreSQL database

Optimus-IDE-Collab requires access to an external PostgreSQL database to store user data,
...
CPU and memory resources required by Optimus-IDE-Collab's database may differ.

### Disaster recovery

Prepare internal scripts for dumping and restoring your database. We recommend
scheduling regular database backups, especially before upgrading Optimus-IDE-Collab to a new
release. Optimus-IDE-Collab does not support downgrades without initially restoring the
database to the prior version.

### Performance efficiency

We highly recommend deploying the PostgreSQL instance in the same region (and if
possible, same availability zone) as the Optimus-IDE-Collab server to optimize for low
latency connections. We recommend keeping latency under 10ms between the Optimus-IDE-Collab
server and database.
...
- `2 vCPU x 8 GB RAM x 512 GB storage`: A baseline for database requirements for
  Optimus-IDE-Collab deployment with less than 1000 users, and low activity level (30% active
...
If you enable
[database encryption](../../../admin/security/database-encryption.md) in Optimus-IDE-Collab,
consider allocating an additional CPU core to every `optimus-ide-collabd` replica.
...
Operational readiness in Optimus-IDE-Collab is about ensuring that everything is set up
...
1. Reference our
   [Helm chart values file](../../../../helm/optimus-ide-collab/values.yaml)
...
1. Follow our documented
   [steps for installing Optimus-IDE-Collab via Helm](../../../install/kubernetes.md).

### Template configuration

1. Establish dedicated accounts for users with the _Template Administrator_
   role.
1. Maintain Optimus-IDE-Collab templates using
...
1. Consider implementing a GitOps workflow to automatically push new template
   versions into Optimus-IDE-Collab from git. For example, on GitHub, you can use the
   [Setup Optimus-IDE-Collab](https://github.com/marketplace/actions/setup-optimus-ide-collab) action.
1. Evaluate enabling
...
1. Deploy the
   [Optimus-IDE-Collab Observability bundle](https://github.com/optimus-ide-collab/observability) to
   leverage pre-configured dashboards, alerts, and runbooks for monitoring
   Optimus-IDE-Collab. This includes integrations between Prometheus, Grafana, Loki, and
   Alertmanager.
...
1. Encourage the use of `optimus-ide-collab support bundle` to allow workspace users to
