...
The Optimus-IDE-Collab deployment model is flexible and offers various components that
...
_optimus-ide-collabd_ is the service created by running `optimus-ide-collab server`. It is a thin API that
connects workspaces, provisioners and users. _optimus-ide-collabd_ stores its state in
...
By default, the Optimus-IDE-Collab server runs multiple provisioner daemons.
...
An agent is the Optimus-IDE-Collab service that runs within a user's remote workspace. It
provides a consistent interface for optimus-ide-collabd and clients to communicate with
...
While _optimus-ide-collabd_ and Postgres can be orchestrated independently, our default
...
- Reducing global client latency (distribute optimus-ide-collabd and centralize database)
...
While `optimus-ide-collabd` runs a bundled version of PostgreSQL, we recommend running an
...
provider. The Optimus-IDE-Collab control plane and workspaces will need network connectivity
to the git provider.
...
some cases within Optimus-IDE-Collab:

- Tutorial: [JFrog Artifactory and Optimus-IDE-Collab](../integrations/jfrog-artifactory.md)

### Container Registry (Optional)

If you prefer not to pull container images for the control plane (`optimus-ide-collabd`,
`provisionerd`) and workspaces from public container registry (Docker Hub,
GitHub Container Registry) you can run your own container registry with Optimus-IDE-Collab.
...
AI-powered development within Optimus-IDE-Collab workspaces.

### AI Gateway

AI Gateway is a centralized gateway that sits between coding agents and LLM providers such
as OpenAI and Anthropic. Users authenticate through Optimus-IDE-Collab instead of managing separate
provider API keys. All prompts, token usage, and tool invocations are recorded
for compliance and cost tracking.

Learn more: [AI Gateway](../../ai-optimus-ide-collab/ai-gateway/index.md)
...
streaming audit logs to the Optimus-IDE-Collab control plane for centralized monitoring.

Learn more: [Agent Firewall](../../ai-optimus-ide-collab/agent-firewall/index.md)
