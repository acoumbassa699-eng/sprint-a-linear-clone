...
We scale-test Optimus-IDE-Collab with a built-in utility that can
be used in your environment for insights into how Optimus-IDE-Collab scales with your
infrastructure. For scale-testing Kubernetes clusters we recommend that you install
and use the dedicated Optimus-IDE-Collab template,
[scaletest-runner](../../../scaletest/templates/scaletest-runner).

Learn more about [Optimus-IDE-Collab’s architecture](./architecture.md) and our
[scale-testing methodology](./scale-testing.md).

For more information about scaling, see our [Optimus-IDE-Collab scaling best practices](../../tutorials/best-practices/scale-optimus-ide-collab.md).
...
| Environment      | Optimus-IDE-Collab CPU | Optimus-IDE-Collab RAM | Optimus-IDE-Collab Replicas | Database          | Users | Concurrent builds | Concurrent connections (Terminal/SSH) | Optimus-IDE-Collab Version | Last tested  |
|------------------|-----------|-----------|----------------|-------------------|-------|-------------------|---------------------------------------|---------------|--------------|
...
Since Optimus-IDE-Collab's performance is highly dependent on the templates and workflows you
...
The following command will provision a number of Optimus-IDE-Collab workspaces using the
specified template and extra parameters:

```sh
optimus-ide-collab exp scaletest create-workspaces \
        --retry 5 \
...
```sh
optimus-ide-collab exp scaletest create-workspaces --help
```

### Traffic Generation

Given an existing set of workspaces created previously with `create-workspaces`,
the following command will generate traffic similar to that of Optimus-IDE-Collab's Web
Terminal against those workspaces.

```sh
# Produce load at about 1000MB/s (25MB/40ms).
optimus-ide-collab exp scaletest workspace-traffic \
...
```sh
optimus-ide-collab exp scaletest cleanup \
...
The template deploys a main workspace with scripts used to orchestrate Optimus-IDE-Collab,
...
- Workspace size selection: minimal/small/medium/large (_default_: minimal,
  which contains just enough resources for a Optimus-IDE-Collab agent to run without
...
it against the staging cluster (or production at your own risk). Optimus-IDE-Collab provides
different
...
The greedy agent variant is a template modification that makes the Optimus-IDE-Collab agent
transmit large metadata (size: 4K) while reporting stats. The transmission of
large chunks puts extra overhead on optimus-ide-collabd instances and agents when handling
...
Optimus-IDE-Collab offers a comprehensive overview
...
- Utilization of resources within the Optimus-IDE-Collab control plane (CPU, memory, pods)
- Database performance metrics (CPU, memory, I/O, connections, queries)
- Optimus-IDE-Collabd API performance (requests, latency, error rate)
- Resource consumption within Optimus-IDE-Collab workspaces (CPU, memory, network usage)
...
environment variable `OPTIMUS-IDE-COLLAB_PROMETHEUS_COLLECT_DB_METRICS` to `true`.
...
of Optimus-IDE-Collab, necessitating action from operators.

## Autoscaling

We generally do not recommend using an autoscaler that modifies the number of
optimus-ide-collabd replicas. In particular, scale down events can cause interruptions for a
large number of users.

Optimus-IDE-Collabd is different from a simple request-response HTTP service in that it
...
workspaces. Loss of a optimus-ide-collabd replica will drop these long-lived connections and
interrupt users. For example, if you have 4 optimus-ide-collabd replicas behind a load
...
The severity of the interruption varies by application. Optimus-IDE-Collab's web terminal,
for example, will reconnect to the same session and continue. So, this should
not be interpreted as saying optimus-ide-collabd replicas should never be taken down for any
reason.

We recommend you plan to run enough optimus-ide-collabd replicas to comfortably meet your
weekly high-water-mark load, and monitor optimus-ide-collabd peak CPU & memory utilization
...
Kubernetes _nodes_ according to load. Optimus-IDE-Collab can coexist with such cluster
autoscalers, but we recommend you take steps to prevent the autoscaler from
evicting optimus-ide-collabd pods, as an eviction will cause the same interruptions as
...
an annotation on the optimus-ide-collabd deployment.

## Troubleshooting

If a load test fails or if you are experiencing performance issues during
day-to-day use, you can leverage Optimus-IDE-Collab's
