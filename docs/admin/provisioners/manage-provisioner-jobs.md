...
Optimus-IDE-Collab admins can view and manage provisioner jobs.
...
- **CLI**: `optimus-ide-collab provisioner jobs list`
- **API**: `/api/v2/provisioner/jobs`

## Manage provisioner jobs from the dashboard

View more information about and manage your provisioner jobs from the Optimus-IDE-Collab dashboard.
...
1. Filter jobs by `pending` status in the dashboard, or use the CLI:
   ...
   ```sh
   optimus-ide-collab provisioner jobs list -s pending
   ```
...
1. Cancel the job through the dashboard, or use the CLI:
   ...
   ```sh
   optimus-ide-collab provisioner jobs cancel <job-id>
   ```
