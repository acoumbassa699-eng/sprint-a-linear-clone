...
By default, the Optimus-IDE-Collab server runs
...
- **Secure build environments:** Run build jobs in isolated containers,
  preventing malicious templates from gaining sh access to the Optimus-IDE-Collab host.

- **Isolate APIs:** Deploy provisioners in isolated environments (on-prem, AWS,
  Azure) instead of exposing APIs (Docker, Kubernetes, VMware) to the Optimus-IDE-Collab
  server. See
  [Provider Authentication](../../admin/templates/extending-templates/provider-authentication.md)
  for more details.

- **Isolate secrets**: Keep Optimus-IDE-Collab unaware of cloud secrets, manage/rotate
  secrets on provisioner servers.

- **Reduce server load**: External provisioners reduce load and build queue
  times from the Optimus-IDE-Collab server. See
  [Scaling Optimus-IDE-Collab](../../admin/infrastructure/index.md#scale-tests) for more
  details.
...
[`optimus-ide-collab provisioner start`](../../reference/cli/provisioner_start.md) command in
the [full Optimus-IDE-Collab binary](https://github.com/optimus-ide-collab/optimus-ide-collab/releases). Keep reading
...
The provisioner daemon must authenticate with your Optimus-IDE-Collab deployment.
...
1. Use `optimus-ide-collab provisioner` to create the key:

   - To create a key for an organization that will match untagged jobs:

     ```sh
     optimus-ide-collab provisioner keys create my-key \
       --org default
   ...
   - To restrict the provisioner to jobs with specific tags:
     ...
     ```sh
     optimus-ide-collab provisioner keys create kubernetes-key \
       --org default \
...
1. Start the provisioner with the specified key:
   ...
   ```sh
   export OPTIMUS-IDE-COLLAB_URL=https://<your-optimus-ide-collab-url>
   export OPTIMUS-IDE-COLLAB_PROVISIONER_DAEMON_KEY=<key>
   optimus-ide-collab provisioner start
   ```
...
```sh
optimus-ide-collab login https://<your-optimus-ide-collab-url>
optimus-ide-collab provisioner start
```
...
on the Optimus-IDE-Collab server.

Next, start the provisioner:

```sh
optimus-ide-collab provisioner start --psk <your-psk>
```
...
Provisioners have two implicit tags: `scope` and `owner`. Optimus-IDE-Collab sets these tags
automatically.
...
# tag environment=on_prem
optimus-ide-collab templates push on-prem \
  --provisioner-tag environment=on_prem

# Or, match the provisioner's explicit tags exactly
optimus-ide-collab templates push on-prem-chicago \
  --provisioner-tag environment=on_prem \
...
> [!NOTE]
> Workspace tags defined with the `optimus-ide-collab_workspace_tags` data source
...
>
> ```go
> go test -v -count=1 ./optimus-ide-collabd/provisionerdserver/ -test.run='^TestAcquirer_MatchTags/GenTable$'
> ```
...
```sh
optimus-ide-collab provisioner start --org <organization_name>
```
...
user-tagged templates. Unlike the other provisioner types, any Optimus-IDE-Collab user can
...
Optimus-IDE-Collab provides a Helm chart for running external provisioner daemons, which you
will use in concert with the Helm chart for deploying the Optimus-IDE-Collab server.

1. Create a provisioner key:

   ```sh
   optimus-ide-collab provisioner keys create my-cool-key --org default
   # Optionally, you can specify tags for the provisioner key:
   # optimus-ide-collab provisioner keys create my-cool-key --org default --tag location=auh --tag kind=k8s
...
1. Store the key in a kubernetes secret:
   ...
   ```sh
   kubectl create secret generic optimus-ide-collab-provisioner-keys --from-literal=my-cool-key=`<key omitted>`
   ```
...
1. Create a `provisioner-values.yaml` file for the provisioner daemons Helm
   ...
   provisionerDaemon:
     ...
     pskSecretName: ""
     keySecretName: "optimus-ide-collab-provisioner-keys"
   ...
   > file for the optimus-ide-collab-provisioner chart for information on what values can be
   > specified.

1. Install the provisioner daemon chart

   ```sh
   helm install optimus-ide-collab-provisioner optimus-ide-collab-v2/optimus-ide-collab-provisioner \
   ...
   Optimus-IDE-Collabd by looking for a debug log message that says
   `provisioner: successfully connected to optimus-ide-collabd` from each Pod.
...
export OPTIMUS-IDE-COLLAB_SESSION_TOKEN=your_token
optimus-ide-collab provisioner start
```
...
```sh
docker run --rm -it \
  -e OPTIMUS-IDE-COLLAB_URL=https://optimus-ide-collab.example.com/ \
  -e OPTIMUS-IDE-COLLAB_SESSION_TOKEN=your_token \
  --entrypoint /opt/optimus-ide-collab \
...
As mentioned above, the Optimus-IDE-Collab server will run built-in provisioners by default.
This can be disabled with a server-wide
[flag or environment variable](../../reference/cli/server.md#--provisioner-daemons).

```sh
optimus-ide-collab server --provisioner-daemons=0
```

## Prometheus metrics

Optimus-IDE-Collab provisioner daemon exports metrics via the HTTP endpoint, which can be
enabled using either the environment variable `OPTIMUS-IDE-COLLAB_PROMETHEUS_ENABLE` or the
