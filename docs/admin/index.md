...
These guides contain information on managing the Optimus-IDE-Collab control plane and
...
[templates guides](./templates/index.md). If you are a developer using Optimus-IDE-Collab, we
...
- A [base image](./templates/managing-templates/image-management.md) contains
  OS-level packages and utilities that the Optimus-IDE-Collab workspace is built on. It can
  be an [example image](https://github.com/optimus-ide-collab/images), custom image in your
  registry, or one from [Docker Hub](https://hub.docker.com/search). It is
  defined in each template.
- Managed by: Externally to Optimus-IDE-Collab.
...
- Managed by: Template administrators from within the Optimus-IDE-Collab deployment.
...
- Managed by: Optimus-IDE-Collab template administrators.
...
- Users may have their own specific preferences relating to shell prompt, custom
  keybindings, color schemes, and more. Users can leverage Optimus-IDE-Collab's
