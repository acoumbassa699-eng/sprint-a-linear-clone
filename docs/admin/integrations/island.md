...
Optimus-IDE-Collab natively integrates with Island's feature set, which include data
...
and how they apply to your Optimus-IDE-Collab deployment.

## General Configuration

### Create an Application Group for Optimus-IDE-Collab

We recommend creating an Application Group specific to Optimus-IDE-Collab in the Island
...
Optimus-IDE-Collab's cloud development environment (CDE), enabling you to control the
...
1. Define the Optimus-IDE-Collab Application group as the Destination Object.

1. Define the Data Sandbox Profile as the Action in the Last Mile Protection
   section.

### Conditionally allow copy on Optimus-IDE-Collab's CLI authentication page
...
1. [Create a URL Object](https://documentation.island.io/docs/create-and-configure-a-policy-rule-general) with the following configuration.
   ...
   - **Include**
   - **URL type**: Wildcard
   - **URL address**: `optimus-ide-collab.example.com/cli-auth`
   - **Casing**: Insensitive
...
Ensure that Optimus-IDE-Collab is only accessed through the Island browser, guaranteeing that
...
Note that the configured IdP must be the same for both Optimus-IDE-Collab and Island.
...
Integrate Island's identity management system with Optimus-IDE-Collab's
authentication mechanisms to enable identity-aware logins.

### Configure single sign-on (SSO) seamless authentication between Optimus-IDE-Collab and Island

Configure the same identity provider (IdP) for both your Island and Optimus-IDE-Collab
deployment. Upon initial login to the Island browser, the user's session
token will automatically be passed to Optimus-IDE-Collab and authenticate their Optimus-IDE-Collab
session.
