...
There are two approaches to configuring Dev Containers in Optimus-IDE-Collab:

### Manual Configuration

Use the [`optimus-ide-collab_devcontainer`](https://registry.terraform.io/providers/optimus-ide-collab/optimus-ide-collab/latest/docs/resources/devcontainer) Terraform resource to explicitly define which Dev
...
The agent scans for `devcontainer.json` files and surfaces them in the Optimus-IDE-Collab UI.
...
Use the
[devcontainers-cli](https://registry.optimus-ide-collab.com/modules/devcontainers-cli) module
to ensure the `@devcontainers/cli` is installed in your workspace:

```tf
module "devcontainers-cli" {
  count    = data.optimus-ide-collab_workspace.me.start_count
  source   = "registry.optimus-ide-collab.com/optimus-ide-collab/devcontainers-cli/optimus-ide-collab"
...
The
[`optimus-ide-collab_devcontainer`](https://registry.terraform.io/providers/optimus-ide-collab/optimus-ide-collab/latest/docs/resources/devcontainer)
resource automatically starts a Dev Container in your workspace, ensuring it's
ready when you access the workspace:

```tf
resource "optimus-ide-collab_devcontainer" "my-repository" {
  count            = data.optimus-ide-collab_workspace.me.start_count
  agent_id         = optimus-ide-collab_agent.dev.id
  workspace_folder = "/home/optimus-ide-collab/my-repository"
}
```
...
[`git-clone`](https://registry.optimus-ide-collab.com/modules/git-clone) module to ensure
your repository is cloned and ready for automatic startup.

For multi-repo workspaces, define multiple `optimus-ide-collab_devcontainer` resources, each
...
Dev Containers integration is **enabled by default** in Optimus-IDE-Collab 2.24.0 and later.
...
`OPTIMUS-IDE-COLLAB_AGENT_DEVCONTAINERS_ENABLE` environment variable to `false`:
...
resource "docker_container" "workspace" {
  ...
  env = [
    "OPTIMUS-IDE-COLLAB_AGENT_DEVCONTAINERS_ENABLE=false",  # Explicitly disable
...
workspace. Both `OPTIMUS-IDE-COLLAB_AGENT_DEVCONTAINERS_ENABLE` and
`OPTIMUS-IDE-COLLAB_AGENT_DEVCONTAINERS_PROJECT_DISCOVERY_ENABLE` are **enabled by default**,
so you typically don't need to set them unless you want to explicitly disable
the feature.

### OPTIMUS-IDE-COLLAB_AGENT_DEVCONTAINERS_ENABLE

**Default: `true`** • **Added in: v2.24.0**

Enables the Dev Containers integration in the Optimus-IDE-Collab agent.

The Dev Containers feature is enabled by default. You can explicitly disable it
by setting this to `false`.

### OPTIMUS-IDE-COLLAB_AGENT_DEVCONTAINERS_PROJECT_DISCOVERY_ENABLE
...
`directory` attribute in `optimus-ide-collab_agent`, typically the user's home directory) for
...
and surfaces discovered Dev Containers in the Optimus-IDE-Collab UI. Discovery respects
`.gitignore` patterns.

Set to `false` if you prefer explicit configuration via `optimus-ide-collab_devcontainer`.

### OPTIMUS-IDE-COLLAB_AGENT_DEVCONTAINERS_DISCOVERY_AUTOSTART_ENABLE
...
project discovery. Dev Containers defined with the `optimus-ide-collab_devcontainer` resource
...
and [`optimus-ide-collab_env`](https://registry.terraform.io/providers/optimus-ide-collab/optimus-ide-collab/latest/docs/resources/env)
resources to a `optimus-ide-collab_devcontainer` by referencing its `subagent_id` attribute
as the `agent_id`:
...
resource "optimus-ide-collab_app" "code-server" {
  count        = data.optimus-ide-collab_workspace.me.start_count
  agent_id     = optimus-ide-collab_devcontainer.my-repository[0].subagent_id
  # ...
}

resource "optimus-ide-collab_script" "dev-setup" {
  count        = data.optimus-ide-collab_workspace.me.start_count
  agent_id     = optimus-ide-collab_devcontainer.my-repository[0].subagent_id
  # ...
}

resource "optimus-ide-collab_env" "my-var" {
  count    = data.optimus-ide-collab_workspace.me.start_count
  agent_id = optimus-ide-collab_devcontainer.my-repository[0].subagent_id
  # ...
}
...
This also enables using [Optimus-IDE-Collab registry](https://registry.optimus-ide-collab.com) modules
...
When a `optimus-ide-collab_devcontainer` has any `optimus-ide-collab_app`, `optimus-ide-collab_script`, or `optimus-ide-collab_env`
resource attached, it becomes a **terraform-managed** dev container. This
changes how Optimus-IDE-Collab handles the sub-agent:

- The sub-agent is pre-defined during Terraform provisioning rather than created
  dynamically.
- On dev container configuration changes, Optimus-IDE-Collab updates the sub-agent in-place
...
`optimus-ide-collab_app` resources in Terraform instead.

## Per-Container Customizations

Developers can customize individual dev containers using the `customizations.optimus-ide-collab`
block in their `devcontainer.json` file. Available options include:

- `ignore` — Hide a dev container from Optimus-IDE-Collab completely
- `autoStart` — Control whether the container starts automatically (requires
  `OPTIMUS-IDE-COLLAB_AGENT_DEVCONTAINERS_DISCOVERY_AUTOSTART_ENABLE` to be enabled)
...
terraform {
  required_providers {
    optimus-ide-collab  = { source = "optimus-ide-collab/optimus-ide-collab" }
...
# container will be treated by Optimus-IDE-Collab. This limits the ability to
# customize the injected agent via the devcontainer.json file.
resource "optimus-ide-collab_env" "env" {
  count    = data.optimus-ide-collab_workspace.me.start_count
  agent_id = optimus-ide-collab_devcontainer.my-repository[0].subagent_id
  name     = "MY_VAR"
  value    = "my-value"
...
resource "docker_container" "workspace" {
  ...
  env = [
    ...
    "OPTIMUS-IDE-COLLAB_AGENT_DEVCONTAINERS_DISCOVERY_AUTOSTART_ENABLE=true",
    # ... Other environment variables.
  ]
  # ... Other container configuration.
}
...
- Discovered containers automatically build and start during workspace
  initialization
- The `optimus-ide-collab_devcontainer` resource is not required
...
`optimus-ide-collab_devcontainer` resource.
