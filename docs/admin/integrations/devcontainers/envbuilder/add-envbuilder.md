...
A Optimus-IDE-Collab administrator adds an Envbuilder-compatible template to Optimus-IDE-Collab. This
...
You can create template files through the Optimus-IDE-Collab dashboard, CLI, or you can
choose a template from the
[Optimus-IDE-Collab registry](https://registry.optimus-ide-collab.com/templates):

<div class="tabs">

## Dashboard

1. In the Optimus-IDE-Collab dashboard, select **Templates** > **New Template**.
...
1. Use the `template init` command to initialize your choice of image:
   ...
   ```sh
   optimus-ide-collab template init --id kubernetes-devcontainer
   ```

   A list of available templates is shown in the
   [templates_init](../../../../reference/cli/templates.md) reference.

1. `cd` into the directory and push the template to your Optimus-IDE-Collab deployment:

   ```sh
   cd kubernetes-devcontainer && optimus-ide-collab templates push
   ```

   You can also edit the files or make changes to the files before you push them
   to Optimus-IDE-Collab.

## Registry

1. Go to the [Optimus-IDE-Collab registry](https://registry.optimus-ide-collab.com/templates) and select a
   dev container-compatible template.

1. Copy the files to your local device, then edit them to fit your needs.

1. Upload them to Optimus-IDE-Collab through the CLI or dashboard:

   - CLI:

   ```sh
   optimus-ide-collab templates push <template-name> -d <path to folder containing main.tf>
   ```
...
To set variables such as the namespace, go to the template in your Optimus-IDE-Collab
...
When using the
[Envbuilder Terraform provider](https://registry.terraform.io/providers/optimus-ide-collab/envbuilder/latest/docs),
