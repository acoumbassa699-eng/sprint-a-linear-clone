...
> [Premium license](https://optimus-ide-collab.com/pricing#compare-plans). For more details,
> [contact your account team](https://optimus-ide-collab.com/contact).

Organizations can be used to segment and isolate resources inside a Optimus-IDE-Collab
deployment for different user groups or projects.

## Example

Here is an example of how one could use organizations to run a Optimus-IDE-Collab deployment
...
All Optimus-IDE-Collab deployments start with one organization called `optimus-ide-collab`. All new users
...
- Optimus-IDE-Collab v2.16+ deployment with Premium license and Organizations enabled
  ([contact your account team](https://optimus-ide-collab.com/contact)) for more details.
...
1. Using Optimus-IDE-Collab CLI, run the following command to create a key that will be used
   to authenticate the provisioner:

   ```sh
   optimus-ide-collab provisioner keys create data-cluster-key --org data-platform
...
1. Start the provisioner with the key on your desired platform.
   ...
   In this example, start the provisioner using the Optimus-IDE-Collab CLI on a host with
   ...
   optimus-ide-collab provisionerd start --org <org-name>
   ```
