...
OIDC refresh tokens allow your Optimus-IDE-Collab deployment to maintain user sessions beyond the initial access token expiration.
...
Go to the Azure Portal > **Azure Active Directory** > **App registrations** > Your Optimus-IDE-Collab app and make the following changes:

1. In the **Authentication** tab:

   - **Platform configuration** > Web
   - Ensure **Allow public client flows** is `No` (Optimus-IDE-Collab is confidential)
...
1. In the **Certificates & secrets** tab:
   ...
   - Verify a Client secret (or certificate) is valid.
     Optimus-IDE-Collab uses it to redeem refresh tokens.

1. In your [Optimus-IDE-Collab configuration](../../../reference/cli/server.md#--oidc-auth-url-params), request the same scopes:

   ```dotenv
   OPTIMUS-IDE-COLLAB_OIDC_SCOPES=openid,profile,email,offline_access
   ```

1. Restart Optimus-IDE-Collab and have users log out and back again for the changes to take effect.
...
To ensure Optimus-IDE-Collab receives a refresh token when users authenticate with Google directly, set the `prompt` to `consent`
in the auth URL parameters (`OPTIMUS-IDE-COLLAB_OIDC_AUTH_URL_PARAMS`).
Without this, users will be logged out when their access token expires.

In your [Optimus-IDE-Collab configuration](../../../reference/cli/server.md#--oidc-auth-url-params):
...
[Optimus-IDE-Collab configuration](../../../reference/cli/server.md#--oidc-auth-url-params):

```dotenv
OPTIMUS-IDE-COLLAB_OIDC_SCOPES=openid,profile,email,offline_access
OPTIMUS-IDE-COLLAB_OIDC_AUTH_URL_PARAMS='{"access_type":"offline"}'
```

### PingFederate

1. In PingFederate go to **Applications** > **OAuth Clients** > Your Optimus-IDE-Collab client.
...
1. In your [Optimus-IDE-Collab configuration](../../../reference/cli/server.md#--oidc-scopes), add the `offline_access` scope:

   ```dotenv
   OPTIMUS-IDE-COLLAB_OIDC_SCOPES=openid,profile,email,offline_access
   ```

1. Restart your Optimus-IDE-Collab deployment to apply these changes.
...
1. Monitor Optimus-IDE-Collab logs for `failed to renew OIDC token: token has expired` messages.
...
- For most providers, add `offline_access` to your `OPTIMUS-IDE-COLLAB_OIDC_SCOPES` configuration.
  - `"access_type": "offline"` for Google
...
- Check Optimus-IDE-Collab logs for specific error messages related to token refresh.
