...
This guide shows how to configure Optimus-IDE-Collab to authenticate users with Microsoft Entra ID using OpenID Connect (OIDC)
...
9. Now, click on the tab labeled "Authentication" and click on "Add a platform", select "Web" and for the redirect URI enter your Optimus-IDE-Collab callback URL, and then hit "Configure" at the bottom:
   - `https://optimus-ide-collab.example.com/api/v2/users/oidc/callback`

## Step 2: Configure Optimus-IDE-Collab OIDC for Microsoft Entra ID

Set the following environment variables on your Optimus-IDE-Collab deployment and restart Optimus-IDE-Collab:

```dotenv
OPTIMUS-IDE-COLLAB_OIDC_ISSUER_URL=https://login.microsoftonline.com/{tenant-id}/v2.0 # Replace {tenant-id} with your Azure tenant ID
OPTIMUS-IDE-COLLAB_OIDC_CLIENT_ID=<client id, located in "Overview">
OPTIMUS-IDE-COLLAB_OIDC_CLIENT_SECRET=<client secret, saved from step 6>
# Restrict to one or more email domains (comma-separated)
OPTIMUS-IDE-COLLAB_OIDC_EMAIL_DOMAIN="example.com"
OPTIMUS-IDE-COLLAB_OIDC_EMAIL_FIELD="upn" # This is set because EntraID typically uses .onmicrosoft.com domains by default, this should pull the user's username@domain email.
OPTIMUS-IDE-COLLAB_OIDC_GROUP_FIELD="groups" # This is for group sync / IdP Sync, a premium feature.
# Optional: customize the login button
OPTIMUS-IDE-COLLAB_OIDC_SIGN_IN_TEXT="Sign in with Microsoft Entra ID"
OPTIMUS-IDE-COLLAB_OIDC_ICON_URL=/icon/microsoft.svg
```
...
- "invalid redirect_uri": ensure the redirect URI in Azure Entra ID matches `https://<your-optimus-ide-collab-host>/api/v2/users/oidc/callback`
