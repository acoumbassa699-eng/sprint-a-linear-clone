...
> [!NOTE]
> Creating service accounts requires a [Premium license](https://optimus-ide-collab.com/pricing).

Service accounts are headless user accounts that cannot use the web UI to log in
to Optimus-IDE-Collab. This is useful for creating accounts for automated systems, such as
CI/CD pipelines or for users who only consume Optimus-IDE-Collab via another client/API. Service accounts do not have passwords or associated email addresses.
...
optimus-ide-collab users create \
  --username="optimus-ide-collab-bot" \
  --service-account
```
