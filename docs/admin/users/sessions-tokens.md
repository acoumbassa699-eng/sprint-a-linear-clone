...
The [Optimus-IDE-Collab CLI](../../install/cli.md) and
[Backstage Plugin](https://github.com/optimus-ide-collab/backstage-plugins) use short-lived
token to authenticate. To generate a short-lived session token on behalf of your
account, visit the following URL: `https://optimus-ide-collab.example.com/cli-auth`
...
```sh
optimus-ide-collab login token
```

This is useful for passing your session token to other tools:

```sh
export OPTIMUS-IDE-COLLAB_SESSION_TOKEN=$(optimus-ide-collab login token)
```
...
configure
[`OPTIMUS-IDE-COLLAB_SESSION_DURATION`](../../reference/cli/server.md#--session-duration) to
change the duration and
[`OPTIMUS-IDE-COLLAB_DISABLE_SESSION_EXPIRY_REFRESH`](../../reference/cli/server.md#--disable-session-expiry-refresh)
...
to `https://optimus-ide-collab.example.com/settings/account`
...
```sh
optimus-ide-collab tokens create --name=my-token --lifetime=720h
```

See the help docs for
[`optimus-ide-collab tokens create`](../../reference/cli/tokens_create.md) for more info.
...
As of Optimus-IDE-Collab v2.17+, you can use the CLI or API to create long-lived tokens on
behalf of other users. Use the API for earlier versions of Optimus-IDE-Collab.
...
```sh
optimus-ide-collab tokens create --name my-token --user <username>
```

See the full CLI reference for
[`optimus-ide-collab tokens create`](../../reference/cli/tokens_create.md)
...
You can use the
[`OPTIMUS-IDE-COLLAB_MAX_TOKEN_LIFETIME`](https://optimus-ide-collab.com/docs/reference/cli/server#--max-token-lifetime)
server flag to set the maximum duration for long-lived tokens in your
deployment.

### Remove or expire a token

You can remove a token using the CLI or the API. By default, `optimus-ide-collab tokens remove`
expires the token, (soft-delete):

```console
optimus-ide-collab tokens remove <name|id>
```
...
```console
optimus-ide-collab tokens list --include-expired
```

To hard-delete a token, use the `--delete` flag:

```console
optimus-ide-collab tokens remove --delete <name|id>
```
...
optimus-ide-collab tokens create --name "readonly-token" --scope "workspace:read"

# Create a token with multiple scopes
optimus-ide-collab tokens create --name "limited-token" --scope "workspace:read" --scope "template:read"
```
...
optimus-ide-collab tokens create --name "workspace-token" \
...
optimus-ide-collab tokens create --name "limited-token" \
