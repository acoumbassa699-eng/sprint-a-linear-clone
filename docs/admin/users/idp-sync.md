...
synchronize Optimus-IDE-Collab groups, roles, and organizations based on claims from your IdP.
...
```txt
https://[optimus-ide-collab.example.com]/api/v2/debug/[your-username]/debug-link
```
...
If your OpenID Connect provider supports group claims, you can configure Optimus-IDE-Collab
to synchronize groups in your auth provider to groups within Optimus-IDE-Collab. To enable
...
1. Fetch the corresponding group IDs using the following endpoint:
   ...
   ```txt
   https://[optimus-ide-collab.example.com]/api/v2/groups
   ```
...
1. Enter the **IdP group name** and **Optimus-IDE-Collab group**, then **Add IdP group**.

### CLI

1. Confirm you have the [Optimus-IDE-Collab CLI](../../install/index.md) installed and are
...
1. To fetch the current group sync settings for an organization, run the
   ...
   ```sh
   optimus-ide-collab organizations settings show group-sync \
     --org <org-name> \
     > group-sync.json
   ```
...
`optimus-ide-collab-` into Optimus-IDE-Collab:

```json
{
    "field": "groups",
    "mapping": null,
    "regex_filter": "^optimus-ide-collab-.*$",
    "auto_create_missing_groups": true
}
```

You must specify Optimus-IDE-Collab group IDs instead of group names.
You can find the ID for a corresponding group by visiting
`https://optimus-ide-collab.example.com/api/v2/groups`.

Here is another example which maps `optimus-ide-collab-admins` from the identity provider to
two groups in Optimus-IDE-Collab and `optimus-ide-collab-users` from the identity provider to another
group:
...
```json
{
    "field": "groups",
    "mapping": {
        "optimus-ide-collab-admins": [
            "2ba2a4ff-ddfb-4493-b7cd-1aec2fa4c830",
            "93371154-150f-4b12-b5f0-261bb1326bb4"
        ],
        "optimus-ide-collab-users": ["2f4bde93-0179-4815-ba50-b757fb3d43dd"]
    },
    "regex_filter": null,
    "auto_create_missing_groups": false
}
...
```sh
optimus-ide-collab organizations settings set group-sync \
  --org <org-name> \
  < group-sync.json
```

Visit the Optimus-IDE-Collab UI to confirm these changes:
...
> [!NOTE]
> Use server flags only with Optimus-IDE-Collab deployments with a single organization.
> You can use the dashboard to configure group sync instead.

1. Configure the Optimus-IDE-Collab server to read groups from the claim name with the
   ...
   - Environment variable:
     ...
     ```sh
     OPTIMUS-IDE-COLLAB_OIDC_GROUP_FIELD=groups
     ```
...
1. On login, users will automatically be assigned to groups that have matching
   names in Optimus-IDE-Collab and removed from groups that the user no longer belongs to.

1. For cases when an OIDC provider only returns group IDs or you want to have
   different group names in Optimus-IDE-Collab than in your OIDC provider, you can configure
   ...
   - Environment variable:
     ...
     ```sh
     OPTIMUS-IDE-COLLAB_OIDC_GROUP_MAPPING='{"myOIDCGroupID": "myOptimus-IDE-CollabGroupName"}'
     ```

   - As a flag:

     ```sh
     --oidc-group-mapping '{"myOIDCGroupID": "myOptimus-IDE-CollabGroupName"}'
     ```

   Below is an example mapping in the Optimus-IDE-Collab Helm chart:

   ```yaml
   optimus-ide-collab:
     env:
       - name: OPTIMUS-IDE-COLLAB_OIDC_GROUP_MAPPING
         value: >
           {"myOIDCGroupID": "myOptimus-IDE-CollabGroupName"}
   ```

   From this example, users that belong to the `myOIDCGroupID` group in your
   OIDC provider will be added to the `myOptimus-IDE-CollabGroupName` group in Optimus-IDE-Collab.

</div>

### Group allowlist

You can limit which groups from your identity provider can log in to Optimus-IDE-Collab with
[OPTIMUS-IDE-COLLAB_OIDC_ALLOWED_GROUPS](../../reference/cli/server.md#--oidc-allowed-groups).
...
If your OpenID Connect provider supports roles claims, you can configure Optimus-IDE-Collab
to synchronize roles in your auth provider to roles within Optimus-IDE-Collab.
...
1. Enter the **IdP role name** and **Optimus-IDE-Collab role**, then **Add IdP role**.
...
1. To fetch the current group sync settings for an organization, run the
   ...
   ```sh
   optimus-ide-collab organizations settings show role-sync \
     --org <org-name> \
     > role-sync.json
   ```
...
Below is an example that uses the `roles` claim and maps `optimus-ide-collab-admins` from the
...
```json
{
    "field": "roles",
    "mapping": {
        "optimus-ide-collab-admins": ["organization-admin"],
...
> Use `optimus-ide-collab organization roles show --org=<your-org>` to see roles for your organization.

To set these role sync settings, use the following command:

```sh
optimus-ide-collab organizations settings set role-sync \
  --org <org-name> \
  < role-sync.json
```
...
1. Set the following in your Optimus-IDE-Collab server [configuration](../setup/index.md).

   ```dotenv
    # Depending on your identity provider configuration, you may need to explicitly request a "roles" scope
   OPTIMUS-IDE-COLLAB_OIDC_SCOPES=openid,profile,email,offline_access,roles

   # The following fields are required for role sync:
   OPTIMUS-IDE-COLLAB_OIDC_USER_ROLE_FIELD=roles
   OPTIMUS-IDE-COLLAB_OIDC_USER_ROLE_MAPPING='{"TemplateAuthor":["template-admin","user-admin"]}'
   ```

One role from your identity provider can be mapped to many roles in Optimus-IDE-Collab. The
example above maps to two roles in Optimus-IDE-Collab.
...
Optimus-IDE-Collab to synchronize claims in your auth provider to organizations within Optimus-IDE-Collab.
...
You can initiate an organization sync through the Optimus-IDE-Collab dashboard or CLI:
...
1. Fetch the corresponding organization IDs using the following endpoint:
   ...
   ```txt
   https://[optimus-ide-collab.example.com]/api/v2/organizations
   ```

1. As a Optimus-IDE-Collab organization user admin or site-wide user admin, go to
...
1. Enter an IdP organization name and Optimus-IDE-Collab organization(s), then select **Add
   IdP organization**:
...
Use the Optimus-IDE-Collab CLI to show and adjust the settings.
...
1. Show the current settings:
   ...
   ```console
   optimus-ide-collab organization settings show org-sync
   {
      "field": "organizations",
      "mapping": {
...
1. Update with the JSON payload. In this example, `settings.json` contains the
   ...
   ```console
   optimus-ide-collab organization settings set org-sync < settings.json
   {
      "field": "organizations",
      "mapping": {
         "product": [
...
```sh
[debu]  optimus-ide-collabd.userauth: got oidc claims  request_id=49e86507-6842-4b0b-94d4-f245e62e49f3  source=id_token  claim_fields="[aio aud email exp groups iat idp iss name nbf oid preferred_username rh sub tid uti ver]"  blank=[]

[debu]  optimus-ide-collabd.userauth: got oidc claims  request_id=49e86507-6842-4b0b-94d4-f245e62e49f3  source=userinfo  claim_fields="[email family_name given_name name picture sub]"  blank=[]

[debu]  optimus-ide-collabd.userauth: got oidc claims  request_id=49e86507-6842-4b0b-94d4-f245e62e49f3  source=merged  claim_fields="[aio aud email exp family_name given_name groups iat idp iss name nbf oid picture preferred_username rh sub tid uti ver]"  blank=[]

[debu]  optimus-ide-collabd: groups returned in oidc claims  request_id=49e86507-6842-4b0b-94d4-f245e62e49f3  email=ben@optimus-ide-collab.com  username=ben  len=3  groups="[c8048e91-f5c3-47e5-9693-834de84034ad 66ad2cc3-a42f-4574-a281-40d1922e5b65 70b48175-107b-4ad8-b405-4d888a1c466f]"
```

To view the full claim, the Owner role can visit this endpoint on their Optimus-IDE-Collab
deployment after logging in:

```sh
https://[optimus-ide-collab.example.com]/api/v2/debug/[username]/debug-link
```

### User not being assigned / Group does not exist

If you want Optimus-IDE-Collab to create groups that do not exist, you can set the following
environment variable.
...
OPTIMUS-IDE-COLLAB_OIDC_GROUP_AUTO_CREATE=true
```
...
A basic regex filtering option on the Optimus-IDE-Collab side is available. This is applied
**after** the group mapping (`OPTIMUS-IDE-COLLAB_OIDC_GROUP_MAPPING`), meaning if the group
...
```dotenv
# as an environment variable
OPTIMUS-IDE-COLLAB_OIDC_GROUP_REGEX_FILTER="^my-group-.*$"
```
...
The solution is to update the value of `OPTIMUS-IDE-COLLAB_OIDC_SCOPES` to the correct value
...
2. Check if another claim appears to be the correct claim with a different name.
   ...
   `OPTIMUS-IDE-COLLAB_OIDC_GROUP_FIELD=memberOf`.
...
1. In your Federation Server, create a new application group for Optimus-IDE-Collab.
...
1. In Optimus-IDE-Collab's configuration file (or Helm values as appropriate), set the
   following environment variables or their corresponding CLI arguments:

   - `OPTIMUS-IDE-COLLAB_OIDC_ISSUER_URL`: `issuer` value from the previous step.
   - `OPTIMUS-IDE-COLLAB_OIDC_CLIENT_ID`: Client ID from step 1.
   - `OPTIMUS-IDE-COLLAB_OIDC_CLIENT_SECRET`: Client Secret from step 1.
   - `OPTIMUS-IDE-COLLAB_OIDC_AUTH_URL_PARAMS`: set to
   ...
   - `OPTIMUS-IDE-COLLAB_OIDC_IGNORE_USERINFO`: Set to `true`.
