...
[startup script](https://registry.terraform.io/providers/optimus-ide-collab/optimus-ide-collab/latest/docs/resources/agent#startup_script-1)
...
```console
$ optimus-ide-collab ssh myworkspace
...
- Ensure the resource can `curl` your Optimus-IDE-Collab
...
- Manually connect to the resource and check the agent logs (e.g.,
  ...
  - The Optimus-IDE-Collab agent logs are typically stored in `/tmp/optimus-ide-collab-agent.log`
  - The Optimus-IDE-Collab agent startup script logs are typically stored in
    `/tmp/optimus-ide-collab-startup-script.log`
  - The Optimus-IDE-Collab agent shutdown script logs are typically stored in
    `/tmp/optimus-ide-collab-shutdown-script.log`
- This can also happen if the websockets are not being forwarded correctly when
  running Optimus-IDE-Collab behind a reverse proxy.
...
[startup script behavior](https://registry.terraform.io/providers/optimus-ide-collab/optimus-ide-collab/latest/docs/resources/agent#startup_script_behavior-1)
...
`optimus-ide-collab ssh` or `optimus-ide-collab config-ssh`). In such an event, you can always access the
...
[`startup_script`](https://registry.terraform.io/providers/optimus-ide-collab/optimus-ide-collab/latest/docs/resources/agent#startup_script-1)
section. For more ways to override the startup script behavior, see the
[`startup_script_behavior`](https://registry.terraform.io/providers/optimus-ide-collab/optimus-ide-collab/latest/docs/resources/agent#startup_script_behavior-1)
section.
...
[startup script](https://registry.terraform.io/providers/optimus-ide-collab/optimus-ide-collab/latest/docs/resources/agent#startup_script-1)
is to open the workspace in the Optimus-IDE-Collab dashboard and click "Show startup log" (if
...
`/tmp/optimus-ide-collab-startup-script.log`). If the logs don't indicate what's going on or
...
If a Docker-based template fails to provision with an error like `Cannot connect to the Docker daemon at unix:///var/run/docker.sock`, the Optimus-IDE-Collab host cannot reach the Docker socket.
