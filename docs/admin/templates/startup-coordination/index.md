...
Optimus-IDE-Collab's workspace startup coordination feature lets you declare
...
Breaking this out into multiple independent `optimus-ide-collab_script` resources improves startup times by allowing the scripts to run in parallel.
...
To start using workspace startup coordination, add calls to `optimus-ide-collab exp sync (start|complete)` in your startup scripts where required:

  ```sh
  trap 'optimus-ide-collab exp sync complete my-script' EXIT
  optimus-ide-collab exp sync want my-script my-other-script
  optimus-ide-collab exp sync start my-script
