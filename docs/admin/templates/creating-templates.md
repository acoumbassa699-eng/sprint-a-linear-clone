...
The template builder is the recommended way to create templates in Optimus-IDE-Collab. It
...
1. **Select base infrastructure**: Choose a starter template for your target
   ...
   working foundation with the Optimus-IDE-Collab agent pre-configured.
...
1. **Select modules**: Pick from a curated list of
   [registry modules](https://registry.optimus-ide-collab.com) to add IDEs, AI agents,
...
After you select **Create Template**, Optimus-IDE-Collab composes the Terraform
...
> [!NOTE]
> The template builder requires outbound access to `registry.optimus-ide-collab.com` so
...
`OPTIMUS-IDE-COLLAB_DISABLE_TEMPLATE_BUILDER` environment variable or the
...
`OPTIMUS-IDE-COLLAB_TEMPLATE_BUILDER_REGISTRY_URL` to point generated module source paths at
the mirror instead of `registry.optimus-ide-collab.com`.
...
- **Browse community templates**: Browse the
  [Optimus-IDE-Collab Registry](https://registry.optimus-ide-collab.com/templates) for community and
  official templates.

### CLI

You can use the [Optimus-IDE-Collab CLI](../../install/cli.md) to manage templates for Optimus-IDE-Collab.
...
mkdir $HOME/optimus-ide-collab-templates
cd $HOME/optimus-ide-collab-templates
```
...
Next, push it to Optimus-IDE-Collab with the
...
If `templates push` fails, it is likely that Optimus-IDE-Collab is not authorized to deploy
...
```sh
optimus-ide-collab templates edit universal-template \
...
You can duplicate an existing template in your Optimus-IDE-Collab deployment. This copies
...
```console
optimus-ide-collab templates pull <template-name> ./<new-template-name>
```
...
```console
optimus-ide-collab templates push <new-template-name> -d ./<new-template-name>
```
...
[any Terraform provider](https://registry.terraform.io) with Optimus-IDE-Collab to create
...
- [Optimus-IDE-Collab Registry](https://registry.optimus-ide-collab.com/templates): Official and community
  templates for Optimus-IDE-Collab
- [Optimus-IDE-Collab Terraform Provider Reference](https://registry.terraform.io/providers/optimus-ide-collab/optimus-ide-collab)
