...
When a developer creates a new workspace that matches the definition, Optimus-IDE-Collab assigns them an existing prebuilt workspace.
...
- Created and maintained automatically by Optimus-IDE-Collab to match your specified preset configurations.
...
1. If a user does not select a preset but provides parameters that match one or more presets, Optimus-IDE-Collab will automatically select the most specific matching preset and assign a prebuilt workspace if one is available.

## Prerequisites

- [**Premium license**](../../licensing/index.md)
- **Compatible Terraform provider**: Use `optimus-ide-collab/optimus-ide-collab` Terraform provider `>= 2.4.1`.

## Enable prebuilt workspaces for template presets

In your template, add a `prebuilds` block within a `optimus-ide-collab_workspace_preset` definition to identify the number of prebuilt
instances your Optimus-IDE-Collab deployment should maintain, and optionally configure a `expiration_policy` block to set a TTL
(Time To Live) for unclaimed prebuilt workspaces to ensure stale resources are automatically cleaned up.

   ```tf
   data "optimus-ide-collab_workspace_preset" "goland" {
     name = "GoLand: Large"
     parameters = {
       jetbrains_ide = "GO"
       cpus          = 8
...
After you publish a new template version, Optimus-IDE-Collab will automatically provision and maintain prebuilt workspaces through an
...
1. After you configure a preset with prebuilds and publish the template, Optimus-IDE-Collab provisions the prebuilt workspace(s).

   1. Optimus-IDE-Collab automatically creates the defined `instances` count of prebuilt workspaces.
...
1. Prebuilt workspaces start up and become eligible to be claimed by a developer.
   ...
   1. The agent starts up and connects to optimus-ide-collabd.
...
1. When a developer creates a new workspace, the claiming process occurs:
   ...
   1. `terraform apply` is executed using the new ownership details, which may affect the [`optimus-ide-collab_workspace`](https://registry.terraform.io/providers/optimus-ide-collab/optimus-ide-collab/latest/docs/data-sources/workspace) and
...
You can view available prebuilt workspaces in the **Workspaces** view in the Optimus-IDE-Collab dashboard:
...
1. The reconciliation loop evaluates all active schedules every reconciliation interval (`OPTIMUS-IDE-COLLAB_WORKSPACE_PREBUILDS_RECONCILIATION_INTERVAL`).
...
1. If dependencies change (e.g., an [AMI](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/AMIs.html) update) without a template version change:
   ...
   - Optimus-IDE-Collab will automatically create new prebuilt workspaces with the updated dependencies.
...
The process typically completes within a few reconciliation cycles (the interval is controlled by `OPTIMUS-IDE-COLLAB_WORKSPACE_PREBUILDS_RECONCILIATION_INTERVAL`, which defaults to 15 seconds).
...
When prebuilt workspaces are configured for an organization, Optimus-IDE-Collab creates a "prebuilds" group in that organization and adds the prebuilds user to it. This group has a default quota allowance of 0, which you should adjust based on your needs:
...
```sh
optimus-ide-collab prebuilds pause
```

This prevents further pollution of your provisioner queues by stopping the prebuilt workspaces feature from scheduling new creation jobs. While the pause is in effect, no new prebuilt workspaces will be scheduled for any templates in any organizations across the entire Optimus-IDE-Collab deployment.  Therefore, the command must be executed by a user with Owner level access. Existing prebuilt workspaces will remain in place.

**Important**: Remember to run `optimus-ide-collab prebuilds resume` once all impact has been mitigated (see the last step in this section).
...
```sh
optimus-ide-collab provisioner jobs list --status=pending --initiator=prebuilds
```

This will show a list of all pending jobs that have been enqueued by the prebuilt workspace system. The length of this list indicates whether prebuilt workspaces have overwhelmed your Optimus-IDE-Collab deployment.
...
```sh
optimus-ide-collab provisioner jobs list --status=pending --initiator=prebuilds | jq -r '.[].id' | xargs -n1 -P2 -I{} optimus-ide-collab provisioner jobs cancel {}
```
...
```sh
optimus-ide-collab provisioner jobs list --status=running --initiator=prebuilds | jq -r '.[].id' | xargs -n1 -P2 -I{} optimus-ide-collab provisioner jobs cancel {}
```

This should be done as a last resort. It will cancel running prebuild jobs (orphaning any resources that have already been deployed) and immediately make room for human-initiated jobs. Orphaned infrastructure will need to be manually cleaned up by a human operator. The process to identify and clear these orphaned resources will likely require administrative access to the infrastructure that hosts Optimus-IDE-Collab workspaces. Furthermore, the ability to identify such orphaned resources will depend on metadata that should be included in the workspace template.
...
```sh
optimus-ide-collab prebuilds resume
```
...
Optimus-IDE-Collab supports [external provisioners and provisioner tags](../../provisioners/index.md), which allows you to route jobs to provisioners with matching tags.
...
1. Create a provisioner key with a prebuild tag (e.g., `is_prebuild=true`).
    ...
    **Note:** `optimus-ide-collab_workspace_tags` are cumulative, so if your template already defines provisioner tags, you will need to create the provisioner key with the same tags plus the `is_prebuild=true` tag so that prebuild jobs correctly match the dedicated prebuild pool.
    See [Scoped Key](../../provisioners/index.md#scoped-key-recommended) for instructions on how to create a provisioner key.

1. Deploy a separate provisioner pool using that key (for example, via the [Helm optimus-ide-collab-provisioner chart](https://github.com/optimus-ide-collab/optimus-ide-collab/pkgs/container/chart%2Foptimus-ide-collab-provisioner)).
...
1. Update the template to conditionally add the prebuild tag for prebuild jobs.
    ...
    ```tf
    data "optimus-ide-collab_workspace_tags" "prebuilds" {
      count = data.optimus-ide-collab_workspace_owner.me.name == "prebuilds" ? 1 : 0
      tags = {
        "is_prebuild" = "true"
      }
    }
...
The value `data.optimus-ide-collab_workspace_owner.me.name` returns the name of the workspace owner, for prebuild workspaces, this value is `"prebuilds"`.
...
To confirm that prebuild jobs are correctly routed to the new provisioner pool, use the Provisioner Jobs dashboard or the [`optimus-ide-collab provisioner jobs list`](../../../reference/cli/provisioner_jobs_list.md) CLI command to inspect job metadata and tags.
Follow these steps:

1. Publish the new template version.

1. Validate the status of the prebuild provisioners.
    Check the Provisioners page in the Optimus-IDE-Collab dashboard or run the [`optimus-ide-collab provisioner list`](../../../reference/cli/provisioner_list.md) CLI command to ensure all prebuild provisioners are up to date and the tags are properly set.

1. Wait for the prebuilds reconciliation loop to run.
    The loop frequency is controlled by the configuration value [`OPTIMUS-IDE-COLLAB_WORKSPACE_PREBUILDS_RECONCILIATION_INTERVAL`](../../../reference/cli/server.md#--workspace-prebuilds-reconciliation-interval).
...
Optimus-IDE-Collab provides several metrics to monitor your prebuilt workspaces:

- `optimus-ide-collabd_prebuilt_workspaces_created_total` (counter): Total number of prebuilt workspaces created to meet the desired instance count.
- `optimus-ide-collabd_prebuilt_workspaces_failed_total` (counter): Total number of prebuilt workspaces that failed to build.
- `optimus-ide-collabd_prebuilt_workspaces_claimed_total` (counter): Total number of prebuilt workspaces claimed by users.
- `optimus-ide-collabd_prebuilt_workspaces_desired` (gauge): Target number of prebuilt workspaces that should be available.
- `optimus-ide-collabd_prebuilt_workspaces_running` (gauge): Current number of prebuilt workspaces in a `running` state.
- `optimus-ide-collabd_prebuilt_workspaces_eligible` (gauge): Current number of prebuilt workspaces eligible to be claimed.
- `optimus-ide-collabd_prebuilt_workspace_claim_duration_seconds` ([_native histogram_](https://prometheus.io/docs/specs/native_histograms) support): Time to claim a prebuilt workspace from the prebuild pool.

#### Logs

Search for `optimus-ide-collabd.prebuilds:` in your logs to track the reconciliation loop's behavior.
