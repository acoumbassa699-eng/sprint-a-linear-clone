...
There are a few ways to run Docker within container-based Optimus-IDE-Collab workspaces.
...
inside Optimus-IDE-Collab workspaces. See [Systemd in Docker](#systemd-in-docker).

### Use Sysbox in Docker-based templates

After [installing Sysbox](https://github.com/nestybox/sysbox#installation) on
the Optimus-IDE-Collab host, modify your template to use the sysbox-runc runtime:

```tf
resource "docker_container" "workspace" {
  # ...
  name    = "optimus-ide-collab-${data.optimus-ide-collab_workspace.me.owner}-${lower(data.optimus-ide-collab_workspace.me.name)}"
...
variable "workspaces_namespace" {
  default = "optimus-ide-collab-namespace"
}
...
resource "kubernetes_pod" "dev" {
  ...
  metadata {
    name      = "optimus-ide-collab-${data.optimus-ide-collab_workspace.me.owner}-${data.optimus-ide-collab_workspace.me.name}"
...
[Envbox](https://github.com/optimus-ide-collab/envbox) is an image developed and maintained
by Optimus-IDE-Collab that bundles the sysbox runtime. It works by starting an outer
...
or visit the [repo](https://github.com/optimus-ide-collab/envbox).

### Authenticating with a Private Registry

Authenticating with a private container registry can be done by referencing the
credentials via the `OPTIMUS-IDE-COLLAB_IMAGE_PULL_SECRET` environment variable. It is
...
```tf
env {
  name = "OPTIMUS-IDE-COLLAB_IMAGE_PULL_SECRET"
  value_from {
    secret_key_ref {
      name = "<name>"
...
4. Use this
   [kubernetes-with-podman](https://github.com/optimus-ide-collab/community-templates/tree/main/kubernetes-podman)
   example template, or make your own.

   ```sh
   echo "kubernetes-with-podman" | optimus-ide-collab templates init
   cd ./kubernetes-with-podman
   optimus-ide-collab templates create
   ```
...
resource "docker_network" "private_network" {
  name = "network-${data.optimus-ide-collab_workspace.me.id}"
}

resource "docker_container" "dind" {
  image      = "docker:dind"
  privileged = true
  name       = "dind-${data.optimus-ide-collab_workspace.me.id}"
...
resource "kubernetes_pod" "dev" {
  ...
  spec {
    ...
    container {
      ...
      image = "optimus-ide-collabcom/enterprise-base:ubuntu"
      command = ["sh", "-c", <<EOF
    # Start the Optimus-IDE-Collab agent as the "optimus-ide-collab" user
    # once systemd has started up
    sudo -u optimus-ide-collab --preserve-env=OPTIMUS-IDE-COLLAB_AGENT_TOKEN /bin/bash -- <<-'    EOT' &
    while [[ ! $(systemctl is-system-running) =~ ^(running|degraded) ]]
    do
      echo "Waiting for system to start... $(systemctl is-system-running)"
      sleep 2
    done
    ${optimus-ide-collab_agent.main.init_script}
    EOT
