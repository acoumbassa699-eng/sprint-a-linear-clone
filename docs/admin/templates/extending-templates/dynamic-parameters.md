...
Optimus-IDE-Collab v2.24.0 introduces Dynamic Parameters to extend Optimus-IDE-Collab [parameters](./parameters.md) with conditional form controls,
...
- Only show select parameters to target Optimus-IDE-Collab roles or groups.

You can try the Dynamic Parameter syntax and any of the code examples below in the
[Parameters Playground](https://playground.optimus-ide-collab.app/parameters).
...
existing optimus-ide-collab templates.
When you opt-in to the new experience, no functional changes will be applied to your production parameters.

Some reasons Optimus-IDE-Collab template admins should try Dynamic Parameters:
...
They allow you to set resource guardrails by referencing Optimus-IDE-Collab identity in the `optimus-ide-collab_workspace_owner` data source.

## How to use Dynamic Parameters

Dynamic Parameters is the standard workspace creation experience.
In Optimus-IDE-Collab v2.25.0 and later it is enabled automatically, and the classic parameter flow is deprecated.

To use the features described on this page in an existing template:

1. Update your template to use version >=2.4.0 of the Optimus-IDE-Collab provider with the following Terraform block.
...
- **Reference User Properties**
  ...
  - Read user data at build time from [`optimus-ide-collab_workspace_owner`](https://registry.terraform.io/providers/optimus-ide-collab/optimus-ide-collab/latest/docs/data-sources/workspace_owner)
...
> [Optimus-IDE-Collab GitHub repository](https://github.com/optimus-ide-collab/optimus-ide-collab).
...
[`form_type`](https://registry.terraform.io/providers/optimus-ide-collab/optimus-ide-collab/latest/docs/data-sources/parameter#form_type-1) attribute.
...
The `optimus-ide-collab_parameter` resource supports an additional `styling` attribute for special cosmetic changes that can be used
...
```tf
data "optimus-ide-collab_parameter" "styled_parameter" {
  ...
  styling = jsonencode({
    disabled = true
  })
...
[`form_type`](https://registry.terraform.io/providers/optimus-ide-collab/optimus-ide-collab/latest/docs/data-sources/parameter#form_type-1)
...
[Try dropdown lists on the Parameter Playground](https://playground.optimus-ide-collab.app/parameters/kgNBpjnz7x)
...
data "optimus-ide-collab_parameter" "ides_dropdown" {
  name = "ides_dropdown"
...
[Try textarea parameters on the Parameter Playground](https://playground.optimus-ide-collab.app/parameters/RCAHA1Oi1_)

```tf

data "optimus-ide-collab_parameter" "text_area" {
  name = "text_area"
...
[Try multi-select parameters on the Parameter Playground](https://playground.optimus-ide-collab.app/parameters/XogX54JV_f)
...
data "optimus-ide-collab_parameter" "ide_selector" {
  name = "ide_selector"
...
[Try radio parameters on the Parameter Playground](https://playground.optimus-ide-collab.app/parameters/3OMDp5ANZI).

```tf
data "optimus-ide-collab_parameter" "environment" {
  name         = "environment"
...
[Try checkbox parameters on the Parameters Playground](https://playground.optimus-ide-collab.app/parameters/ycWuQJk2Py).

```tf
data "optimus-ide-collab_parameter" "enable_gpu" {
  name         = "enable_gpu"
...
[Try slider parameters on the Parameters Playground](https://playground.optimus-ide-collab.app/parameters/RsBNcWVvfm).

```tf
data "optimus-ide-collab_parameter" "cpu_cores" {
  name         = "cpu_cores"
...
[Try private parameters on the Parameters Playground](https://playground.optimus-ide-collab.app/parameters/wmiP7FM3Za).
...
```tf
data "optimus-ide-collab_parameter" "private_api_key" {
...
[Try conditional parameters on the Parameter Playground](https://playground.optimus-ide-collab.app/parameters/xmG5MKEGNM).

```tf
data "optimus-ide-collab_parameter" "show_cpu_cores" {
  name         = "show_cpu_cores"
...
data "optimus-ide-collab_parameter" "cpu_cores" {
  ...
  count = data.optimus-ide-collab_parameter.show_cpu_cores.value ? 1 : 0
...
[Try dynamic defaults in the Parameter Playground](https://playground.optimus-ide-collab.app/parameters/DEi-Bi6DVe).
...
data "optimus-ide-collab_parameter" "git_repo" {
  ...
  option {
    ...
    name = "optimus-ide-collab/optimus-ide-collab"
    value = "optimus-ide-collab/optimus-ide-collab"
  }

  option {
    # A python-heavy repository
    name = "optimus-ide-collab/mlkit"
    value = "optimus-ide-collab/mlkit"
  }
}

data "optimus-ide-collab_parameter" "ide_selector" {
  # Conditionally expose this parameter
  count = try(data.optimus-ide-collab_parameter.git_repo.value, "") != "" ? 1 : 0
  ...
  form_type = "multi-select"
  type      = "list(string)"
  default   = try(data.optimus-ide-collab_parameter.git_repo.value, "") == "optimus-ide-collab/mlkit" ? local.mlkit_ides : local.core_ides
...
[Try dynamic validation in the Parameter Playground](https://playground.optimus-ide-collab.app/parameters/sdbzXxagJ4).
...
data "optimus-ide-collab_parameter" "cpu_cores" {
  ...
  default      = try(data.optimus-ide-collab_parameter.git_repo.value, "") == "optimus-ide-collab/mlkit" ? 12 : 6

  validation {
    min = 1

    # Dynamically set max validation
    max = try(data.optimus-ide-collab_parameter.git_repo.value, "") == "optimus-ide-collab/mlkit" ? 16 : 8
  }
}
```
...
[Try daisy-chaining parameters in the Parameter Playground](https://playground.optimus-ide-collab.app/parameters/jLUUhoDLIa).
...
locals {
  ...
  is_ml_repo = data.optimus-ide-collab_parameter.git_repo == "optimus-ide-collab/mlkit"

  selected = jsondecode(data.optimus-ide-collab_parameter.ide_selector[0].value)

  # selected = try(jsondecode(data.optimus-ide-collab_parameter.ide_selector[0].value), [])
}
...
[`optimus-ide-collab_workspace_owner`](https://registry.terraform.io/providers/optimus-ide-collab/optimus-ide-collab/latest/docs/data-sources/workspace_owner) data source.
...
[`optimus-ide-collab_workspace_owner`](https://registry.terraform.io/providers/optimus-ide-collab/optimus-ide-collab/latest/docs/data-sources/workspace_owner)
data source.

[Try out admin-only options in the Playground](https://playground.optimus-ide-collab.app/parameters/5Gn9W3hYs7).

```tf

locals {
  roles = [for r in data.optimus-ide-collab_workspace_owner.me.rbac_roles: r.name]
  is_admin = contains(data.optimus-ide-collab_workspace_owner.me.groups, "admin")
  has_admin_role = contains(local.roles, "owner")
}

data "optimus-ide-collab_workspace_owner" "me" {}

data "optimus-ide-collab_parameter" "advanced_settings" {
...
[Try user-aware regions in the parameter playground](https://playground.optimus-ide-collab.app/parameters/tBD-mbZRGm)
...
locals {
  ...
  is_eu_dev = contains(data.optimus-ide-collab_workspace_owner.me.groups, local.eu_group_name)
  region_desc_tag = local.is_eu_dev ? "european" : "american"
}

data "optimus-ide-collab_parameter" "region" {
  name         = "region"
...
[Try groups as options in the Parameter Playground](https://playground.optimus-ide-collab.app/parameters/lKbU53nYjl).

```tf
locals {
  groups = data.optimus-ide-collab_workspace_owner.me.groups
}

data "optimus-ide-collab_workspace_owner" "me" {}

data "optimus-ide-collab_parameter" "your_groups" {
  type         = "string"
  name         = "your_groups"
  display_name = "Your Optimus-IDE-Collab Groups"
...
Dynamic Parameters is now in general availability. We're tracking a list of known issues [here in Github](https://github.com/optimus-ide-collab/optimus-ide-collab/issues?q=sort%3Aupdated-desc%20is%3Aissue%20is%3Aopen%20label%3Aparameters) as we continue to polish and improve the workflow.
If you have any issues during upgrade, please file an issue in our
[GitHub repository](https://github.com/optimus-ide-collab/optimus-ide-collab/issues/new?labels=parameters) with the `parameters` label and include a
[Playground link](https://playground.optimus-ide-collab.app/parameters) where applicable.
We appreciate the feedback and look forward to what the community creates with this system!

You can also [search or track the list of known issues](https://github.com/optimus-ide-collab/optimus-ide-collab/issues?q=is%3Aissue%20state%3Aopen%20label%3Aparameters).

You can share anything you build with Dynamic Parameters in our [Discord](https://optimus-ide-collab.com/chat).

### Enabled Dynamic Parameters, but my template looks the same

Ensure that the following version requirements are met:

- `optimus-ide-collab/optimus-ide-collab`: >= [v2.25.0](https://github.com/optimus-ide-collab/optimus-ide-collab/releases/tag/v2.25.0)
- `optimus-ide-collab/terraform-provider-optimus-ide-collab`: >= [v2.5.3](https://github.com/optimus-ide-collab/terraform-provider-optimus-ide-collab/releases/tag/v2.5.3)
...
[file an issue](https://github.com/optimus-ide-collab/optimus-ide-collab/issues/new?labels=parameters) with the `parameters` label.

### Template variables not showing up

Dynamic Parameters are GA as of [v2.25.0](https://github.com/optimus-ide-collab/optimus-ide-collab/releases/tag/v2.25.0). Template variables are fully supported in Dynamic Parameters.
...
Dynamic Parameters require Terraform modules to be archived and stored in the database. Optimus-IDE-Collab limits module archives to **20MB total** to prevent database bloat. If your template uses modules that exceed this limit, some modules will be unavailable for parameter declarations.
...
```txt
[API] 2026-01-29 22:00:22.691 [warn]  provisionerd-nixos-0.executor: some (or all) terraform modules were not archived, template will have reduced function  skipped_modules=large:git::https://github.com/optimus-ide-collab/large-module.git
```
