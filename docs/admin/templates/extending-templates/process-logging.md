...
Please note that these logs are not recorded or captured by the Optimus-IDE-Collab
organization in any way, shape, or form.

## How this works

Optimus-IDE-Collab uses [eBPF](https://ebpf.io/) (which we chose for its minimal performance
...
[exectrace](https://github.com/optimus-ide-collab/exectrace) GitHub repo. The enterprise
...
[exectrace repo](https://github.com/optimus-ide-collab/exectrace/tree/main/enterprise/templates).
   ...
   locals {
     ...
     exectrace_init_script = <<EOT
       ...
       if ! command -v curl >/dev/null 2>&1; then
         echo "curl is required to download the Optimus-IDE-Collab binary"
...
1. Add the following container to your workspace pod spec.
   ...
   resource "kubernetes_pod" "main" {
     ...
     spec {
       ...
       container {
         name              = "exectrace"
         image             = "ghcr.io/optimus-ide-collab/exectrace:latest"
         image_pull_policy = "Always"
         command = [
           "/opt/exectrace",
           "--init-address", "127.0.0.1:56123",
           "--label", "workspace_id=${data.optimus-ide-collab_workspace.me.id}",
           "--label", "workspace_name=${data.optimus-ide-collab_workspace.me.name}",
           "--label", "user_id=${data.optimus-ide-collab_workspace_owner.me.id}",
           "--label", "username=${data.optimus-ide-collab_workspace_owner.me.name}",
           "--label", "user_email=${data.optimus-ide-collab_workspace_owner.me.email}",
         ]
         security_context {
...
1. Add the following environment variable to your workspace pod:
   ...
   resource "kubernetes_pod" "main" {
     ...
     spec {
       ...
       env {
         name = "OPTIMUS-IDE-COLLAB_AGENT_SUBSYSTEM"
         value = "exectrace"
       }
       ...
     }
...
```json
{
    ...
    "fields": {
        "labels": {
            "user_email": "jessie@optimus-ide-collab.com",
...
- `exectrace` will log processes from nested Docker containers (including deeply
  nested containers) correctly, but Optimus-IDE-Collab does not distinguish between processes
