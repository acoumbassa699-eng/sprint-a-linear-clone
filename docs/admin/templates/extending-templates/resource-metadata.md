...
You can use `optimus-ide-collab_metadata` to show Terraform resource attributes like these:
...
![ui](../../../images/admin/templates/optimus-ide-collab-metadata-ui.png)

> [!NOTE]
> Optimus-IDE-Collab automatically generates the <code>type</code> metadata.
...
resource "optimus-ide-collab_metadata" "pvc" {
  ...
  item {
    key = "dir"
    value = "/home/optimus-ide-collab"
  }
}

resource "optimus-ide-collab_metadata" "deployment" {
  count = data.optimus-ide-collab_workspace.me.start_count
  resource_id = kubernetes_deployment.optimus-ide-collab[0].id
  item {
    key = "name"
    value = kubernetes_deployment.optimus-ide-collab[0].metadata[0].name
  }
}
```
...
```tf
resource "optimus-ide-collab_metadata" "hide_serviceaccount" {
  count = data.optimus-ide-collab_workspace.me.start_count
...
```tf
resource "optimus-ide-collab_metadata" "resource_with_icon" {
  count = data.optimus-ide-collab_workspace.me.start_count
  resource_id = kubernetes_service_account.user_data.id
