...
Some of the examples use the [`optimus-ide-collab stat`](../../../reference/cli/stat.md)
...
resource "optimus-ide-collab_agent" "main" {
  ...
  metadata {
    ...
    key  = "cpu"
    # Uses the optimus-ide-collab stat command to get container CPU usage.
  ...
  metadata {
    ...
    key  = "mem"
    # Uses the optimus-ide-collab stat command to get container memory usage in GiB.
    script = "optimus-ide-collab stat mem --prefix Gi"
    interval = 1
    timeout = 1
  }
...
Agent metadata can generate a significant write load and overwhelm your Optimus-IDE-Collab
