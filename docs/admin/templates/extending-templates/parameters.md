...
workspaces with
[_parameters_](https://registry.terraform.io/providers/optimus-ide-collab/optimus-ide-collab/latest/docs/data-sources/parameter).
...
```tf
data "optimus-ide-collab_parameter" "docker_host" {
  name        = "Region"
...
```tf
provider "docker" {
  host = data.optimus-ide-collab_parameter.docker_host.value
}
```

## Types

A Optimus-IDE-Collab parameter can have one of these types:
...
```tf
data "optimus-ide-collab_parameter" "security_groups" {
...
> parameter, you will need to pass the following argument to `optimus-ide-collab create`:
>
> ```shell
...
When creating Optimus-IDE-Collab templates, authors have the flexibility to modify parameter
...
```tf
data "optimus-ide-collab_parameter" "account_name" {
  name        = "Account name"
...
If a parameter contains the `default` property, Optimus-IDE-Collab will use this value if the
user does not specify any:

```tf
data "optimus-ide-collab_parameter" "base_image" {
  name        = "Base image"
...
```tf
data "optimus-ide-collab_parameter" "dotfiles_url" {
  name        = "dotfiles URL"
...
behaviors in a Optimus-IDE-Collab workspace, such as reverting to a previous image, restoring
...
```tf
data "optimus-ide-collab_parameter" "force_rebuild" {
  name         = "force_rebuild"
  type         = "bool"
...
Optimus-IDE-Collab supports parameters with multiple validation modes: min, max,
...
```tf
data "optimus-ide-collab_parameter" "instances" {
  name        = "Instances"
  type        = "number"
...
> [!NOTE]
> As of
> [`terraform-provider-optimus-ide-collab` v0.19.0](https://registry.terraform.io/providers/optimus-ide-collab/optimus-ide-collab/0.19.0/docs),
...
Use the
[`optimus-ide-collab_workspace_preset`](https://registry.terraform.io/providers/optimus-ide-collab/optimus-ide-collab/latest/docs/data-sources/workspace_preset)
...
[Terraform provider documentation](https://registry.terraform.io/providers/optimus-ide-collab/optimus-ide-collab/latest/docs/data-sources/workspace_preset#schema).

<details><summary>Expand for an example</summary>

```tf
data "optimus-ide-collab_workspace_preset" "goland-gpu" {
...
data "optimus-ide-collab_workspace_preset" "pittsburgh" {
  name        = "Pittsburgh"
...
data "optimus-ide-collab_parameter" "machine_type" {
  name          = "machine_type"
...
data "optimus-ide-collab_parameter" "attach_gpu" {
  name          = "attach_gpu"
...
data "optimus-ide-collab_parameter" "gcp_region" {
  name          = "gcp_region"
...
data "optimus-ide-collab_parameter" "jetbrains_ide" {
  name          = "jetbrains_ide"
...
When the template doesn't specify default values, Optimus-IDE-Collab may still autofill
parameters in one of two ways:

- Optimus-IDE-Collab will look for URL query parameters with form `param.<name>=<value>`.

  This feature enables platform teams to create pre-filled template creation links.

- Optimus-IDE-Collab can populate recently used parameter key-value pairs for the user.
  ...
  ```sh
  optimus-ide-collab server --experiments=auto-fill-parameters
  ```

  Or set the [environment variable](../../setup/index.md), `OPTIMUS-IDE-COLLAB_EXPERIMENTS=auto-fill-parameters`

## Dynamic Parameters

Optimus-IDE-Collab v2.24.0 introduces [Dynamic Parameters](./dynamic-parameters.md) to extend the existing parameter system with
