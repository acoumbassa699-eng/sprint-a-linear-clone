...
In Optimus-IDE-Collab, Terraform templates offer extensive flexibility through template-wide
...
Optimus-IDE-Collab endeavors to fully support them. Native support includes `string`,
...
2. _Define `default` property_: set a default property for the template
   variable. If the administrator doesn't input a value via CLI, Optimus-IDE-Collab
...
It's crucial to note that Optimus-IDE-Collab templates operate based on resolved values
...
1. Create a `terraform.tfvars` file in in the template directory:
   ...
   ```tf
   optimus-ide-collab_image = newimage:tag
   ```

1. Push the new template revision using Optimus-IDE-Collab CLI:

   ```sh
   optimus-ide-collab templates push my-template -y # no need to use --var
   ```

This file serves as a mechanism to override the template settings for variables.
It can be stored in the repository for easy access and reference. Optimus-IDE-Collab CLI
automatically detects it and loads variable values.

## Input options

When working with Terraform configurations in Optimus-IDE-Collab, you have several options
for providing values to variables using the Optimus-IDE-Collab CLI:
...
Optimus-IDE-Collab automatically loads variable definition files following a specific order,
