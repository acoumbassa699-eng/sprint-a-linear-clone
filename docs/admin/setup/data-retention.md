...
Optimus-IDE-Collab supports configurable retention policies that automatically purge old
...
|----------------------|------------------------------------|----------------------------------------|----------------|-----------------------------------------|
| Audit Logs           | `--audit-logs-retention`           | `OPTIMUS-IDE-COLLAB_AUDIT_LOGS_RETENTION`           | `0` (disabled) | How long to retain Audit Log entries    |
| Connection Logs      | `--connection-logs-retention`      | `OPTIMUS-IDE-COLLAB_CONNECTION_LOGS_RETENTION`      | `0` (disabled) | How long to retain Connection Logs      |
| API Keys             | `--api-keys-retention`             | `OPTIMUS-IDE-COLLAB_API_KEYS_RETENTION`             | `7d`           | How long to retain expired API keys     |
| Workspace Agent Logs | `--workspace-agent-logs-retention` | `OPTIMUS-IDE-COLLAB_WORKSPACE_AGENT_LOGS_RETENTION` | `7d`           | How long to retain workspace agent logs |
| AI Gateway           | `--ai-gateway-retention`           | `OPTIMUS-IDE-COLLAB_AI_GATEWAY_RETENTION`           | `60d`          | How long to retain AI Gateway records   |

> [!NOTE]
> AI Gateway retention is configured separately from other retention settings.
> See [AI Gateway Setup](../../ai-optimus-ide-collab/ai-gateway/setup.md#data-retention) for
...
```sh
export OPTIMUS-IDE-COLLAB_AUDIT_LOGS_RETENTION=365d
export OPTIMUS-IDE-COLLAB_CONNECTION_LOGS_RETENTION=90d
export OPTIMUS-IDE-COLLAB_API_KEYS_RETENTION=7d
export OPTIMUS-IDE-COLLAB_WORKSPACE_AGENT_LOGS_RETENTION=7d
export OPTIMUS-IDE-COLLAB_AI_GATEWAY_RETENTION=60d
```
...
Optimus-IDE-Collab runs a background process that periodically deletes old records. The
purge process:
...
Keeping expired keys for a short period allows Optimus-IDE-Collab to return a more helpful
...
[AI Gateway Data Retention](../../ai-optimus-ide-collab/ai-gateway/setup.md#data-retention)
documentation.
...
configure shorter retention periods in Optimus-IDE-Collab since logs are preserved
externally. See
...
- [Connection Logs](../monitoring/connection-logs.md): Learn about Connection
  Logs and monitoring.
- [AI Gateway](../../ai-optimus-ide-collab/ai-gateway/index.md): Learn about AI Gateway for
  centralized LLM and MCP proxy management.
- [AI Gateway Setup](../../ai-optimus-ide-collab/ai-gateway/setup.md#data-retention): Configure
  AI Gateway data retention.
- [AI Gateway Monitoring](../../ai-optimus-ide-collab/ai-gateway/monitoring.md): Monitor AI
