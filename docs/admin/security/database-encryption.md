...
By default, Optimus-IDE-Collab stores external user tokens and other sensitive values in
plaintext in the database. Database Encryption allows Optimus-IDE-Collab administrators to
encrypt these values at-rest, preventing attackers with database access from
reading or misusing them.

## How it works

Optimus-IDE-Collab allows administrators to specify
[external token encryption keys](../../reference/cli/server.md#--external-token-encryption-keys).
If configured, Optimus-IDE-Collab will use these keys to encrypt external user tokens before
storing them in the database. The encryption algorithm used is AES-256-GCM with
a 32-byte key length.

Optimus-IDE-Collab will use the first key provided for both encryption and decryption. If
additional keys are provided, Optimus-IDE-Collab will use it for decryption only. This allows
...
used to encrypt the data. This allows Optimus-IDE-Collab to rotate encryption keys without
...
- Ensure you have a valid backup of your database. **Do not skip this step.** If
  ...
  [`optimus-ide-collab server postgres-builtin-url`](../../reference/cli/server_postgres-builtin-url.md)
...
```sh
kubectl create secret generic optimus-ide-collab-external-token-encryption-keys --from-literal=keys=<key>
```

- In your Optimus-IDE-Collab configuration set `OPTIMUS-IDE-COLLAB_EXTERNAL_TOKEN_ENCRYPTION_KEYS` to a
...
optimus-ide-collab:
  env:
    [...]
    - name: OPTIMUS-IDE-COLLAB_EXTERNAL_TOKEN_ENCRYPTION_KEYS
      valueFrom:
        secretKeyRef:
          name: optimus-ide-collab-external-token-encryption-keys
          key: keys
```

- Restart the Optimus-IDE-Collab server. The server will now encrypt all new data with the
  provided key.
...
metadata:
  name: optimus-ide-collab-external-token-encryption-keys
  namespace: optimus-ide-collab-namespace
data:
  keys: <new-key>,<old-key1>,<old-key2>,...
```

- After updating the configuration, restart the Optimus-IDE-Collab server. The server will
...
- To re-encrypt all encrypted database fields with the new key, run
  [`optimus-ide-collab server dbcrypt rotate`](../../reference/cli/server_dbcrypt_rotate.md).
...
- Once the above command completes successfully, remove the old encryption key
  from Optimus-IDE-Collab's configuration and restart Optimus-IDE-Collab once more. You can now safely
...
- Stop all active optimus-ide-collabd instances. This will prevent new encrypted data from
  being written, which may cause the next step to fail.

- Run
  [`optimus-ide-collab server dbcrypt decrypt`](../../reference/cli/server_dbcrypt_decrypt.md).
  ...
  > `--keys` is `OPTIMUS-IDE-COLLAB_EXTERNAL_TOKEN_ENCRYPTION_DECRYPT_KEYS` and not
  > `OPTIMUS-IDE-COLLAB_EXTERNAL_TOKEN_ENCRYPTION_KEYS`. This is explicitly named differently
  > to help prevent accidentally decrypting data.

- Remove all
  [external token encryption keys](../../reference/cli/server.md#--external-token-encryption-keys)
  from Optimus-IDE-Collab's configuration.

- Start optimus-ide-collabd. You can now safely delete the encryption keys from your secret
  store.
...
- Run
  [`optimus-ide-collab server dbcrypt delete`](../../reference/cli/server_dbcrypt_delete.md).
...
- If Optimus-IDE-Collab detects that the data stored in the database was not encrypted with
...
- If Optimus-IDE-Collab detects that the data stored in the database was encrypted with a key
...
- Decryption may fail if newly encrypted data is written while decryption is in
  progress. If this happens, ensure that all active optimus-ide-collab instances are stopped,
  and retry.
