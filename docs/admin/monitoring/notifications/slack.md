...
messages, and integrations with external tools. With Optimus-IDE-Collab's integration, you
...
in your Optimus-IDE-Collab environment.

Administrators can configure Optimus-IDE-Collab to send notifications via an incoming webhook
...
consistent between Slack and their Optimus-IDE-Collab login.
...
- Optimus-IDE-Collab platform >=v2.16.0

## Create Slack Application

To integrate Slack with Optimus-IDE-Collab, follow these steps to create a Slack application:
...
The Slack bot for Optimus-IDE-Collab runs as a _Bolt application_, which is a framework
...
3. Create and edit the `app.js` file. Below is an example of the basic
   ...
   (async () => {
       ...
       console.log("⚡️ Optimus-IDE-Collab Slack bot is running!");
   })();
   ```
...
## Enable Webhook Integration in Optimus-IDE-Collab

To enable webhook integration in Optimus-IDE-Collab, define the POST webhook endpoint
matching the deployed Slack bot:

```sh
export OPTIMUS-IDE-COLLAB_NOTIFICATIONS_WEBHOOK_ENDPOINT=http://localhost:6000/v1/webhook`
```

Finally, go to the **Notification Settings** in Optimus-IDE-Collab and switch the notifier to
**Webhook**.
