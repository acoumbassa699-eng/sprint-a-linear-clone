...
Notifications are sent by Optimus-IDE-Collab in response to specific internal events, such as
a workspace being deleted or a user being created.

Available events may differ between versions.
For a list of all events, visit your Optimus-IDE-Collab deployment's
`https://optimus-ide-collab.example.com/deployment/notifications`.
...
Optimus-IDE-Collab supports the following list of events:
...
Notifications can be delivered through the Optimus-IDE-Collab dashboard Inbox and by SMTP or webhook.
OOM/OOD notifications can be delivered to users in VS Code.

You can configure:

- SMTP or webhooks globally with
[`OPTIMUS-IDE-COLLAB_NOTIFICATIONS_METHOD`](../../../reference/cli/server.md#--notifications-method)
(default: `smtp`).
- Optimus-IDE-Collab dashboard Inbox with
[`OPTIMUS-IDE-COLLAB_NOTIFICATIONS_INBOX_ENABLED`](../../../reference/cli/server.md#--notifications-inbox-enabled)
(default: `true`).
...
You can modify the notification delivery behavior in your Optimus-IDE-Collab deployment's
`https://optimus-ide-collab.example.com/settings/notifications`, or with the following server flags:

| Required | CLI                                 | Env                                     | Type       | Description                                                                                                           | Default |
|:--------:|-------------------------------------|-----------------------------------------|------------|-----------------------------------------------------------------------------------------------------------------------|---------|
|    ✔️    | `--notifications-dispatch-timeout`  | `OPTIMUS-IDE-COLLAB_NOTIFICATIONS_DISPATCH_TIMEOUT`  | `duration` | How long to wait while a notification is being sent before giving up.                                                 | 1m      |
|    ✔️    | `--notifications-method`            | `OPTIMUS-IDE-COLLAB_NOTIFICATIONS_METHOD`            | `string`   | Which delivery method to use (available options: 'smtp', 'webhook'). See [Delivery Methods](#delivery-methods) below. | smtp    |
|    -️    | `--notifications-max-send-attempts` | `OPTIMUS-IDE-COLLAB_NOTIFICATIONS_MAX_SEND_ATTEMPTS` | `int`      | The upper limit of attempts to send a notification.                                                                   | 5       |
|    -️    | `--notifications-inbox-enabled`     | `OPTIMUS-IDE-COLLAB_NOTIFICATIONS_INBOX_ENABLED`     | `bool`     | Enable or disable inbox notifications in the Optimus-IDE-Collab dashboard.                                                         | true    |
...
Use the `smtp` method to deliver notifications by email to your users. Optimus-IDE-Collab
does not ship with an SMTP server, so you will need to configure Optimus-IDE-Collab to use an
existing one.
...
| Required | CLI                 | Env                     | Type     | Description                                                       | Default   |
|:--------:|---------------------|-------------------------|----------|-------------------------------------------------------------------|-----------|
|    ✔️    | `--email-from`      | `OPTIMUS-IDE-COLLAB_EMAIL_FROM`      | `string` | The sender's address to use (e.g. `"Optimus-IDE-Collab <optimus-ide-collab@example.com>"`). |           |
|    ✔️    | `--email-smarthost` | `OPTIMUS-IDE-COLLAB_EMAIL_SMARTHOST` | `string` | The SMTP relay to send messages (format: `hostname:port`)         |           |
|    ✔️    | `--email-hello`     | `OPTIMUS-IDE-COLLAB_EMAIL_HELLO`     | `string` | The hostname identifying the SMTP server.                         | localhost |

**Authentication Settings:**

| Required | CLI                          | Env                              | Type     | Description                                                               |
|:--------:|------------------------------|----------------------------------|----------|---------------------------------------------------------------------------|
|    -     | `--email-auth-username`      | `OPTIMUS-IDE-COLLAB_EMAIL_AUTH_USERNAME`      | `string` | Username to use with PLAIN/LOGIN authentication.                          |
|    -     | `--email-auth-password`      | `OPTIMUS-IDE-COLLAB_EMAIL_AUTH_PASSWORD`      | `string` | Password to use with PLAIN/LOGIN authentication.                          |
|    -     | `--email-auth-password-file` | `OPTIMUS-IDE-COLLAB_EMAIL_AUTH_PASSWORD_FILE` | `string` | File from which to load password for use with PLAIN/LOGIN authentication. |
|    -     | `--email-auth-identity`      | `OPTIMUS-IDE-COLLAB_EMAIL_AUTH_IDENTITY`      | `string` | Identity to use with PLAIN authentication.                                |

**TLS Settings:**

| Required | CLI                         | Env                           | Type     | Description                                                                                                                                                        | Default |
|:--------:|-----------------------------|-------------------------------|----------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------|
|    -     | `--email-force-tls`         | `OPTIMUS-IDE-COLLAB_EMAIL_FORCE_TLS`       | `bool`   | Force a TLS connection to the configured SMTP smarthost. If port 465 is used, TLS will be forced. See <https://datatracker.ietf.org/doc/html/rfc8314#section-3.3>. | false   |
|    -     | `--email-tls-starttls`      | `OPTIMUS-IDE-COLLAB_EMAIL_TLS_STARTTLS`    | `bool`   | Enable STARTTLS to upgrade insecure SMTP connections using TLS. Ignored if `OPTIMUS-IDE-COLLAB_EMAIL_FORCE_TLS` is set.                                                         | false   |
|    -     | `--email-tls-skip-verify`   | `OPTIMUS-IDE-COLLAB_EMAIL_TLS_SKIPVERIFY`  | `bool`   | Skip verification of the target server's certificate (**insecure**).                                                                                               | false   |
|    -     | `--email-tls-server-name`   | `OPTIMUS-IDE-COLLAB_EMAIL_TLS_SERVERNAME`  | `string` | Server name to verify against the target certificate.                                                                                                              |         |
|    -     | `--email-tls-cert-file`     | `OPTIMUS-IDE-COLLAB_EMAIL_TLS_CERTFILE`    | `string` | Certificate file to use.                                                                                                                                           |         |
|    -     | `--email-tls-cert-key-file` | `OPTIMUS-IDE-COLLAB_EMAIL_TLS_CERTKEYFILE` | `string` | Certificate key file to use.                                                                                                                                       |         |

**NOTE:** you _MUST_ use `OPTIMUS-IDE-COLLAB_EMAIL_FORCE_TLS` if your smarthost supports TLS
...
1. Set the following configuration options:
   ...
   ```txt
   OPTIMUS-IDE-COLLAB_EMAIL_SMARTHOST=smtp.gmail.com:465
   OPTIMUS-IDE-COLLAB_EMAIL_AUTH_USERNAME=<user>@<domain>
   OPTIMUS-IDE-COLLAB_EMAIL_AUTH_PASSWORD="<app password created above (no spaces)>"
   ```

   **Note:** The `OPTIMUS-IDE-COLLAB_EMAIL_AUTH_PASSWORD` must be entered without spaces.
...
1. Set the following configuration options:
   ...
   ```txt
   OPTIMUS-IDE-COLLAB_EMAIL_SMARTHOST=smtp-mail.outlook.com:587
   OPTIMUS-IDE-COLLAB_EMAIL_TLS_STARTTLS=true
   OPTIMUS-IDE-COLLAB_EMAIL_AUTH_USERNAME=<user>@<domain>
   OPTIMUS-IDE-COLLAB_EMAIL_AUTH_PASSWORD="<account password>"
   ```
...
| Required | CLI                                | Env                                    | Type  | Description                             |
|:--------:|------------------------------------|----------------------------------------|-------|-----------------------------------------|
|    ✔️    | `--notifications-webhook-endpoint` | `OPTIMUS-IDE-COLLAB_NOTIFICATIONS_WEBHOOK_ENDPOINT` | `url` | The endpoint to which to send webhooks. |

Here is an example payload for Optimus-IDE-Collab's webhook notification:
...
```json
{
    ...
    "payload": {
        ...
        "user_email": "danny@optimus-ide-collab.com",
        "user_name": "danny",
        "user_username": "danny",
        "actions": [
            {
                "label": "View workspaces",
                "url": "https://et23ntkhpueak.pit-1.try.optimus-ide-collab.app/workspaces"
            },
            {
                "label": "View templates",
                "url": "https://et23ntkhpueak.pit-1.try.optimus-ide-collab.app/templates"
            }
        ],
        "labels": {
            "initiator": "danny",
...
- `user_id`: Optimus-IDE-Collab internal user identifier of the target user (UUID)
...
`https://$OPTIMUS-IDE-COLLAB_ACCESS_URL/deployment/notifications?tab=events`.

## Custom notifications

Custom notifications let you send an ad‑hoc notification to yourself using the Optimus-IDE-Collab CLI.
...
To send a custom notification, execute [`optimus-ide-collab notifications custom <title> <message>`](../../../reference/cli/notifications_custom.md).
...
<!-- TODO(ssncferreira): Update when sending custom notifications to multiple users/roles is supported.
	 ...
	 See: https://github.com/optimus-ide-collab/optimus-ide-collab/issues/19768
-->
...
```sh
optimus-ide-collab templates push -y && optimus-ide-collab notifications custom "Template push complete" "Template version uploaded."
```
...
if make test 2>&1 | tee test_output.log; then
  optimus-ide-collab notifications custom "Tests Succeeded" $'Test results:\n  • ✅ success'
else
  failures=$(grep -Po '\d+(?=\s+failures)' test_output.log | tail -n1 || echo 0)
  optimus-ide-collab notifications custom "Tests Failed" $'Test results:\n  • ❌ failed ('"$failures"' tests failed)'
  exit 1
fi
```
...
[`optimus-ide-collab notifications pause`](../../../reference/cli/notifications_pause.md).

To resume sending notifications, execute
[`optimus-ide-collab notifications resume`](../../../reference/cli/notifications_resume.md).
...
1. Review the logs. Search for the term `notifications` for diagnostic information.
   ...
   - If you do not see any relevant logs, set
    `OPTIMUS-IDE-COLLAB_LOG_FILTER=".*notifications.*"` to filter for notification-related logs.
1. If you are on version 2.15.x, notifications must be enabled using the
    ...
    Notifications are enabled by default in Optimus-IDE-Collab v2.16.0 and later.

## Internals

The notification system is built to operate concurrently in a single- or
multi-replica Optimus-IDE-Collab deployment, and has a built-in retry mechanism. It uses the
...
_A notifier here refers to a Optimus-IDE-Collab replica which is responsible for dispatching
the notification. All running replicas act as notifiers to process pending
messages._

- a message begins in `pending` state
- transitions to `leased` when a Optimus-IDE-Collab replica acquires new messages from the
  database
  - new messages are checked for every `OPTIMUS-IDE-COLLAB_NOTIFICATIONS_FETCH_INTERVAL`
    (default: 15s)
...
- if a message encounters a retryable error (e.g. temporary server outage), it
  ...
  - this message will be retried up to `OPTIMUS-IDE-COLLAB_NOTIFICATIONS_MAX_SEND_ATTEMPTS`
    (default: 5)
  - this message will transition back to `pending` state after
    `OPTIMUS-IDE-COLLAB_NOTIFICATIONS_RETRY_INTERVAL` (default: 5m) and be retried
  - after `OPTIMUS-IDE-COLLAB_NOTIFICATIONS_MAX_SEND_ATTEMPTS` is exceeded, it transitions to
    `permanent_failure`
