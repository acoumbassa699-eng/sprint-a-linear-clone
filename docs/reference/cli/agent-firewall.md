...
```console
optimus-ide-collab agent-firewall [flags] [args...]
```
...
|-------------|----------------------------------------------------------|
| Type        | <code>string</code>                                      |
| Environment | <code>$OPTIMUS-IDE-COLLAB_AGENT_BOUNDARY_LOG_PROXY_SOCKET_PATH</code> |
...
Enable session correlation header injection. When no inject targets are configured, the target is auto-derived from OPTIMUS-IDE-COLLAB_AGENT_URL (set automatically inside Optimus-IDE-Collab workspaces). Disable for deployments without Optimus-IDE-Collab AI Gateway in front.
...
Inject target for session correlation headers. Repeat the flag once per target; each value describes exactly one target. Format: "domain=<host> [path=<glob>]". Example: --session-id-inject-target "domain=prod.optimus-ide-collab.com path=/api/v2/aibridge/*".
