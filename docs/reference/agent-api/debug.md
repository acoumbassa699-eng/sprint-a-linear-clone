...
```sh
curl $OPTIMUS-IDE-COLLAB_AGENT_DEBUG_ADDRESS/debug/logs
```

`GET /debug/logs`

Get the first 10MiB of data from `$OPTIMUS-IDE-COLLAB_AGENT_LOG_DIR/optimus-ide-collab-agent.log`.
...
```sh
curl $OPTIMUS-IDE-COLLAB_AGENT_DEBUG_ADDRESS/debug/magicsock
```
...
```sh
curl $OPTIMUS-IDE-COLLAB_AGENT_DEBUG_ADDRESS/debug/magicsock/debug-logging/true
```
...
```sh
curl $OPTIMUS-IDE-COLLAB_AGENT_DEBUG_ADDRESS/debug/manifest
```

`GET /debug/manifest`

Get the manifest the agent fetched from `optimus-ide-collabd` upon startup.
