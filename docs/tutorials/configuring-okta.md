...
Sign On (SSO) on Optimus-IDE-Collab.

To configure custom claims in Okta to support syncing roles and groups with
Optimus-IDE-Collab, you must first have setup an Okta application with
[OIDC working with Optimus-IDE-Collab](../admin/users/oidc-auth/index.md).
From here, we will add additional claims for Optimus-IDE-Collab to use for syncing groups and
roles.
...
If the Optimus-IDE-Collab roles & Optimus-IDE-Collab groups can be inferred from
...
Configure Optimus-IDE-Collab to use these claims for group sync.
...
OPTIMUS-IDE-COLLAB_OIDC_SCOPES=openid,profile,email,offline_access,groups
# This name needs to match the "Claim name" in the configuration above.
OPTIMUS-IDE-COLLAB_OIDC_GROUP_FIELD=groups
```

> [!NOTE]
> The `offline_access` scope is required in Optimus-IDE-Collab v2.23.0+ to prevent hourly session timeouts.
...
OPTIMUS-IDE-COLLAB_OIDC_SCOPES=openid,profile,email,offline_access,groups
# This name needs to match the "Claim name" in the configuration above.
OPTIMUS-IDE-COLLAB_OIDC_USER_ROLE_FIELD=groups
# Example configuration to map a group to some roles
OPTIMUS-IDE-COLLAB_OIDC_USER_ROLE_MAPPING='{"admin-group":["template-admin","user-admin"]}'
```
...
Optimus-IDE-Collab can query an Okta profile for the application from the `/userinfo` OIDC endpoint.
To pass attributes to Optimus-IDE-Collab, create the attribute in your application,
...
Configure using these new attributes in Optimus-IDE-Collab:

```sh
# This must be set to false. Optimus-IDE-Collab uses this endpoint to grab the attributes.
OPTIMUS-IDE-COLLAB_OIDC_IGNORE_USERINFO=false
...
OPTIMUS-IDE-COLLAB_OIDC_USER_ROLE_FIELD=approles
# See our docs for mapping okta roles to optimus-ide-collab roles.
OPTIMUS-IDE-COLLAB_OIDC_USER_ROLE_MAPPING='{"admin-group":["template-admin","user-admin"]}'

# If you added an attribute for groups, set that here.
# OPTIMUS-IDE-COLLAB_OIDC_GROUP_FIELD=...
```
...
Configure Optimus-IDE-Collab to use this:
...
OPTIMUS-IDE-COLLAB_OIDC_ISSUER_URL=https://dev-12222860.okta.com/oauth2/default
# Add the new scope you just configured and offline_access for refresh tokens
OPTIMUS-IDE-COLLAB_OIDC_SCOPES=openid,profile,email,roles,offline_access
...
**Cause**: Missing `offline_access` scope in `OPTIMUS-IDE-COLLAB_OIDC_SCOPES`
**Solution**:

1. Add `offline_access` to your `OPTIMUS-IDE-COLLAB_OIDC_SCOPES` configuration
1. Restart your Optimus-IDE-Collab deployment
...
1. Check that `offline_access` is included in your `OPTIMUS-IDE-COLLAB_OIDC_SCOPES`
1. Verify users can stay logged in beyond Okta's access token lifetime (typically one hour)
1. Monitor Optimus-IDE-Collab logs for any OIDC refresh errors during token renewal
