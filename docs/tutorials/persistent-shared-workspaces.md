...
> [Premium license](https://optimus-ide-collab.com/pricing#compare-plans) because service
...
- A running Optimus-IDE-Collab deployment (v2.32+) with workspace sharing enabled. Sharing
  is on by default for OSS; Premium deployments may require
  [admin configuration](../user-guides/shared-workspaces.md#policies).
- The [Optimus-IDE-Collab CLI](../install/index.md) installed and authenticated.
...
- A [wildcard access URL](../admin/networking/wildcard-access-url.md) configured
  (e.g. `*.optimus-ide-collab.example.com`) so that shared users can access workspace apps
  without a 404.
...
```sh
export OPTIMUS-IDE-COLLAB_SESSION_TOKEN="<token-from-step-2>"

optimus-ide-collab create oncall-sre/oncall-workspace \
...
Use `optimus-ide-collab sharing share` to grant access to users who need the workspace:

```sh
optimus-ide-collab sharing share oncall-sre/oncall-workspace --user alice
```
...
```sh
optimus-ide-collab sharing share oncall-sre/oncall-workspace --user alice:admin
```

To share with multiple users at once:

```sh
optimus-ide-collab sharing share oncall-sre/oncall-workspace --user alice:admin,bob
```

To share with an entire Optimus-IDE-Collab group:

```sh
optimus-ide-collab sharing share oncall-sre/oncall-workspace --group sre-oncall
```
...
optimus-ide-collab sharing remove oncall-sre/oncall-workspace --user alice

# Add incoming user
optimus-ide-collab sharing share oncall-sre/oncall-workspace --user carol
```
...
```sh
optimus-ide-collab sharing status oncall-sre/oncall-workspace
```
...
if [ -n "$OUTGOING" ]; then
  ...
  optimus-ide-collab sharing remove "$WORKSPACE" --user "$OUTGOING"
fi

echo "Granting access to $INCOMING..."
optimus-ide-collab sharing share "$WORKSPACE" --user "$INCOMING"

echo "Restarting workspace to apply changes..."
optimus-ide-collab restart "$WORKSPACE" --yes
...
1. Configure [Group Sync](../admin/users/idp-sync.md#group-sync) to
   synchronize the group from your IdP to Optimus-IDE-Collab.
...
1. When your IdP rotates group membership, Optimus-IDE-Collab group membership updates on
...
optimus-ide-collab list --search shared_with_user:alice

# List workspaces shared with a specific group
optimus-ide-collab list --search shared_with_group:sre-oncall
```
...
`optimus-ide-collab restart <workspace>` after removing a user or group.
...
Optimus-IDE-Collab server logs with
`OPTIMUS-IDE-COLLAB_LOG_FILTER=".*userauth.*|.*groups returned.*"` for details. See
