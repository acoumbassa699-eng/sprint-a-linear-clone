# Configure Optimus-IDE-Collab to connect to PostgreSQL using SSL
...
supply Optimus-IDE-Collab with the appropriate certificates, and have it connect over SSL,
...
```sh
kubectl create secret tls postgres-certs -n optimus-ide-collab --key="postgres.key" --cert="postgres.crt"
```
...
```sh
kubectl create secret tls postgres-certs -n optimus-ide-collab --key="postgres-root.key" --cert="postgres-root.crt"
```
...
```sh
postgres://<user>:<password>@databasehost:<port>/<db-name>?sslmode=verify-full&sslrootcert="/home/optimus-ide-collab/.postgresql/postgres-root.crt"
```
