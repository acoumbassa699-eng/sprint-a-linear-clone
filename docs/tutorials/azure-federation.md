# Federating Optimus-IDE-Collab's control plane to Azure
...
This guide will walkthrough how to authenticate a Optimus-IDE-Collab Provisioner to Microsoft
Azure, using a Service Principal with a client certificate. You can use this
guide for authenticating Optimus-IDE-Collab to Azure, regardless of where Optimus-IDE-Collab is run,
...
We'll need to create the certificate Optimus-IDE-Collab will use for authentication. Run the
...
Next, generate a `.pfx` file to be used by Optimus-IDE-Collab's Provisioner to authenticate
...
- **Name**: this is a friendly identifier and can be anything (e.g. "Optimus-IDE-Collab")
...
Optimus-IDE-Collab users. Navigate to the Subscriptions blade in the Azure Portal, select the
...
resources you need for your Optimus-IDE-Collab workspaces. `Contributor` will provide
...
## Step 5: Configure Optimus-IDE-Collab to use the Client Certificate

Now that the client certificate is uploaded to Azure, we need to mount the
certificate files into the Optimus-IDE-Collab deployment. If running Optimus-IDE-Collab on Kubernetes, you
...
```console
kubectl create secret generic -n optimus-ide-collab azure-client-cert-secret --from-file=client.pfx=/path/to/your/client.pfx
```
...
Next, set the following values in Optimus-IDE-Collab's Helm chart:
...
optimus-ide-collab:
  env:
    ...
    - name: ARM_CLIENT_CERTIFICATE_PATH
      value: /home/optimus-ide-collab/az/
  ...
  volumeMounts:
    - name: "azure-client-cert"
      mountPath: "/home/optimus-ide-collab/az/"
      readOnly: true
```

Upgrade the Optimus-IDE-Collab deployment using the following `helm` command:

```console
helm upgrade optimus-ide-collab optimus-ide-collab-v2/optimus-ide-collab -n optimus-ide-collab -f values.yaml
```
