...
authenticate the Optimus-IDE-Collab control plane to AWS and create an EC2 workspace. The
below steps assume your Optimus-IDE-Collab control plane is running in Google Cloud and has
the relevant service account assigned.

For steps on assigning a service account to a resource like Optimus-IDE-Collab, visit the
...
In this example, Optimus-IDE-Collab will need permissions to create the EC2 instance. Add the
...
## 5. Set identity token in Optimus-IDE-Collab control plane

You will need to set the token created in the previous step on a location in the
Optimus-IDE-Collab control plane. Follow the below steps for your specific deployment type:

### VM control plane

- Write the token to a file on the host, preferably inside the `/home/optimus-ide-collab`
  directory:

```console
/home/optimus-ide-collab/.aws/gcp-identity-token
```
...
```console
kubectl create secret generic gcp-identity-token -n optimus-ide-collab --from-literal=token=<enter-token-here>
```

Make sure the secret is created inside the same namespace where Optimus-IDE-Collab is
running.

- Mount the token file into the Optimus-IDE-Collab pod using the values below:
...
optimus-ide-collab:
  ...
  volumeMounts:
    - name: "gcp-identity-mount"
      mountPath: "/home/optimus-ide-collab/.aws/gcp-identity-token"
      readOnly: true
```

## 6. Configure the AWS Terraform provider

Navigate to your EC2 workspace template in Optimus-IDE-Collab, and configure the AWS provider
...
provider "aws" {
  assume_role_with_web_identity {
    ...
    session_name = "optimus-ide-collab-session"
    # define location of token file on control plane here
    web_identity_token_file = "/home/optimus-ide-collab/.aws/gcp-identity-token"
  }
}
```
...
aws sts assume-role-with-web-identity \
  ...
  --role-session-name optimus-ide-collab-session \
