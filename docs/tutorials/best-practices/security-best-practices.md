...
Optimus-IDE-Collab Server is the main control core of a Optimus-IDE-Collab deployment.

If the Optimus-IDE-Collab Server is compromised in a security incident, it can affect every
other part of your deployment. Even a successful read-only attack against the
Optimus-IDE-Collab Server could result in a complete compromise of the Optimus-IDE-Collab deployment if
...
of truth, so that when users change roles or leave, their permissions in Optimus-IDE-Collab
are automatically up to date.

### Encryption in transit

Place Optimus-IDE-Collab behind a TLS-capable reverse-proxy/load balancer and enable
...
Enable [TLS](../../reference/cli/server.md#--tls-address) on Optimus-IDE-Collab Server and
encrypt traffic from the reverse-proxy/load balancer to Optimus-IDE-Collab Server, so that
even if an attacker gains access to your network, they will not be able to snoop
on Optimus-IDE-Collab Server traffic.

### Encryption at rest

Optimus-IDE-Collab Server persists no state locally. No action is required.

### Server logs and audit logs

Capture the logging output of all Optimus-IDE-Collab Server instances and persist them.
...
If a security incident with Optimus-IDE-Collab does occur, audit logs are invaluable in
...
Path-based apps share the same origin as the Optimus-IDE-Collab API, which can be convenient for trialing Optimus-IDE-Collab,
but can expose the deployment to cross-site-scripting (XSS) attacks in production.
A malicious workspace could reuse Optimus-IDE-Collab cookies to call the API or interact with other workspaces owned by the same user.

1. [Enable sub-domain apps with a wildcard DNS record](../../admin/setup/index.md#wildcard-access-url) (like `*.optimus-ide-collab.example.com`)

1. Disable path-based apps:

   ```sh
   optimus-ide-collabd server --disable-path-apps
   # or
   export OPTIMUS-IDE-COLLAB_DISABLE_PATH_APPS=true
   ```

By default, Optimus-IDE-Collab mitigates the impact of having path-based apps enabled, but we still recommend disabling it to prevent
malicious workspaces accessing other workspaces owned by the same user or performing requests against the Optimus-IDE-Collab API.

If you do keep path-based apps enabled:

- Path-based apps cannot be shared with other users unless you start the Optimus-IDE-Collab server with `--dangerous-allow-path-app-sharing`.
...
PostgreSQL is the persistent datastore underlying the entire Optimus-IDE-Collab deployment.
If the database is compromised, it may leave every other part of your deployment
vulnerable.

Optimus-IDE-Collab session tokens and API keys are salted and hashed, so a read-only
compromise of the database is unlikely to allow an attacker to log into Optimus-IDE-Collab.
...
takeover of an owner account in Optimus-IDE-Collab which could lead to a complete compromise
of the Optimus-IDE-Collab deployment.
...
1. Use environment variables to pass the PostgreSQL URL to Optimus-IDE-Collab.
...
[postgres URL](../../reference/cli/server.md#--postgres-url) on Optimus-IDE-Collab Server.
This configures Optimus-IDE-Collab Server to only establish TLS connections to PostgreSQL and
...
purpose of Optimus-IDE-Collab is to deploy workspaces in the cluster/cloud that can run
developer code.
...
When Optimus-IDE-Collab workspaces are deployed into multiple clusters/clouds, or workspaces
are in a different cluster/cloud than the Optimus-IDE-Collab Server, use external provisioner
daemons.
...
- Means that you don’t have to open any ingress ports on the clusters/clouds
  ...
  - The external provisioner daemons dial out to Optimus-IDE-Collab Server.
...
1. Use a [scoped key](../../admin/provisioners/index.md#scoped-key-recommended) to
   authenticate the provisioner daemons with Optimus-IDE-Collab. These keys can only be used
   to authenticate provisioner daemons (not other APIs on the Optimus-IDE-Collab Server).
...
Enable TLS on Optimus-IDE-Collab Server and ensure you use an `https://` URL to access the
Optimus-IDE-Collab Server.
...
Optimus-IDE-Collab takes care to ensure the user credentials processed by workspace proxies
are scoped to application access and do not grant full access to the Optimus-IDE-Collab API
...
1. Securely store the workspace proxy token generated by
   [`optimus-ide-collab wsproxy create`](../../admin/networking/workspace-proxies.md#step-1-create-the-proxy).
...
Optimus-IDE-Collab templates are executed on provisioner daemons and can include arbitrary
code via the
[local-exec provisioner](https://developer.hashicorp.com/terraform/language/resources/provisioners/local-exec).

Furthermore, Optimus-IDE-Collab templates are designed to provision compute resources in one
or more clusters/clouds, and template authors are generally in full control over
code and scripts executed by the Optimus-IDE-Collab agent in those compute resources.
...
- Use Optimus-IDE-Collab parameters to accept secrets from end users at build time.

Optimus-IDE-Collab does not attempt to obscure the contents of template files from users
...
- Ensure that operators who write, review or modify Optimus-IDE-Collab templates are working
  on laptops/workstations with full disk encryption, or do their work inside a
  Optimus-IDE-Collab workspace with full disk encryption.
...
1. Store Optimus-IDE-Collab templates in a single Git repository, or a single repository per
   Optimus-IDE-Collab organization, and use the
   [Optimus-IDE-Collabd Terraform provider](https://registry.terraform.io/providers/optimus-ide-collab/optimus-ide-collabd/latest/docs/resources/template)
   to push changes from the main branch to Optimus-IDE-Collab using a CI/CD tool.

   This gives you an easily browsable, auditable history of template changes and
   who made them. Optimus-IDE-Collab audit logs establish who and when changes happen, but
   git repositories are particularly handy for analyzing exactly what changes to
   templates are made.

1. Use a Optimus-IDE-Collab user account exclusively for the purpose of pushing template
...
rogue” and acting unilaterally to maliciously modify Optimus-IDE-Collab templates.

## Workspaces

The central purpose of Optimus-IDE-Collab is to give end users access to managed compute in
clusters/clouds designated by Optimus-IDE-Collab’s operators (like platform or developer
...
clusters/clouds that host Optimus-IDE-Collab workspaces.

It is important to limit Optimus-IDE-Collab users to trusted insiders and/or take steps to
constrain malicious activity that could be undertaken from a Optimus-IDE-Collab workspace.
...
datastores and configure the network to limit access from Optimus-IDE-Collab workspaces.
...
Optimus-IDE-Collab manages inbound network access to your workspaces via a set of Wireguard
...
Optimus-IDE-Collab Server and Workspace Proxies include a DERP service by default. Tailcale
...
DERP to be sensitive, stick to the Optimus-IDE-Collab-provided DERP services which run on
...
mappings. It is an essential component of Optimus-IDE-Collab’s networking to enable encrypted
tunnels to be established without a relay for best performance.

Optimus-IDE-Collab does not ship with a STUN service because it needs to be run directly
connected to the network, not behind a reverse proxy or load balancer as Optimus-IDE-Collab
usually is.
...
device of Optimus-IDE-Collab workspaces and the end user's device if STUN is configured.
...
[configure Optimus-IDE-Collab to use it](../../reference/cli/server.md#--derp-server-stun-addresses).
...
Optimus-IDE-Collab workspace apps are a way to allow users to access web applications running
in the workspace via the Optimus-IDE-Collab Server or Workspace Proxy.

1. [Disable workspace apps on sub-paths](../../reference/cli/server.md#--disable-path-apps)
   of the main Optimus-IDE-Collab domain name.
...
1. [Use a separate, wildcard domain name](../../admin/setup/index.md#wildcard-access-url)
   ...
   browsers, serving web apps on the main Optimus-IDE-Collab domain would allow those apps to
   send API requests to the Optimus-IDE-Collab Server, authenticated as the logged-in user
   without their explicit consent.

#### Port sharing

Optimus-IDE-Collab supports the option to allow users to designate specific network ports on
...
Deploy Optimus-IDE-Collab workspaces using full disk encryption for all volumes.
