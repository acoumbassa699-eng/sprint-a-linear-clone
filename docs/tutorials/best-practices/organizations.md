...
Optimus-IDE-Collab [Organizations](../../admin/users/organizations.md) allow administrators
...
## How Optimus-IDE-Collab organizations work
...
Organizations increase the maintenance overhead of a Optimus-IDE-Collab deployment, so we
...
deployed with Optimus-IDE-Collab and has 1000 users in production. Today, MegaCo has a single
...
|--------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------|
| Mergers and acquisitions                                                             | Raptix, a 180-person startup recently acquired by MegaCo, has an independent cloud account, platform team, and Terraform modules and pipelines for deploying their code. They want to use Optimus-IDE-Collab.                                                                                                                                                                                  | ✅ Organizations                                                                                         |
| Independent cloud-native teams that manage their namespaces, images, and/or clusters | MegaCo has six teams responsible for their own dev, staging, and production Kubernetes clusters and frequently deploy & test their work with `kubectl` and `helm`.<br /><br />They wish to hook up Optimus-IDE-Collab to their cluster so they can write and manage IDE templates for connecting to the cluster with their IDE                                                                 | ✅ Organizations                                                                                         |
...
| Off-shore contractors                                                                | MegaCo employs off-shore contractors but has not onboarded them onto Optimus-IDE-Collab due to privacy concerns, data sovereignty rules, and latency considerations.<br /><br />They considered a minimal, localized second deployment of Optimus-IDE-Collab, but decided against it due to maintenance overhead.                                                                                           | ✅ Organizations + Workspace Proxies                                                                     |
...
| ML Platform Team & ML Developers                                                     | MegaCo's data platform team maintains a homegrown "MLBox" product for data environments with a GPU, Jupyter, etc.<br /><br />This team is interested in migrating to Optimus-IDE-Collab for improved cost-saving and auditing of environments, but they need to hook up their own cluster and cloud accounts. They also want their templates only to be accessible to a specific set of users. | ✅ Organizations                                                                                         |
...
or [through the Optimus-IDE-Collab CLI](../../reference/cli/templates_edit.md#--deprecated).
...
cluster/infrastructure and have an isolated key to authenticate back with Optimus-IDE-Collab.
...
- [optimus-ide-collab/optimus-ide-collab#15192](https://github.com/optimus-ide-collab/optimus-ide-collab/issues/15192)

## Identity Provider (SSO) Sync

While the Optimus-IDE-Collab UI or API can be used to assign specific users to organizations,
...
allocated in Optimus-IDE-Collab. Learn more about [IDP sync](../../admin/users/idp-sync.md).
...
[create a GitHub issue](https://github.com/optimus-ide-collab/internal/issues/new?title=request%28orgs%29%3A+request+title+here&labels=["customer-feedback"]&body=please+enter+your+request+here)
...
We suggest using the optimus-ide-collabd Terraform provider to manage organizations at scale.
Documentation and examples for the Organization and Group Sync resources is available
at https://registry.terraform.io/providers/optimus-ide-collab/optimus-ide-collabd/latest/docs. Feature requests
for additional functionality can be created at https://github.com/optimus-ide-collab/terraform-provider-optimus-ide-collabd/issues.
