# Scale Optimus-IDE-Collab

This best practice guide helps you prepare a Optimus-IDE-Collab deployment that you can
...
Observability is one of the most important aspects to a scalable Optimus-IDE-Collab deployment.
...
[Monitor your Optimus-IDE-Collab deployment](../../admin/monitoring/index.md) with log output
...
- Log output
  - Capture log output from from Optimus-IDE-Collab Server instances and external provisioner daemons
...
- Metrics
  ...
  Optimus-IDE-Collab Server, external provisioner daemon, workspace proxy, and PostgreSQL instances.
  - Capture Optimus-IDE-Collab Server and External Provisioner daemons metrics
  [via Prometheus](#how-to-capture-optimus-ide-collab-server-metrics-with-prometheus).
...
- Tail latency of Optimus-IDE-Collab Server API requests
  - High tail latency can indicate Optimus-IDE-Collab Server or the PostgreSQL database is underprovisioned
  for the load.
  - Use the `optimus-ide-collabd_api_request_latencies_seconds` metric.

- Tail latency of database queries
  - High tail latency can indicate the PostgreSQL database is low in resources.
  - Use the `optimus-ide-collabd_db_query_latencies_seconds` metric.

### How to capture Optimus-IDE-Collab server metrics with Prometheus

Edit your Helm `values.yaml` to capture metrics from Optimus-IDE-Collab Server and external provisioner daemons with
[Prometheus](../../admin/integrations/prometheus.md):

1. Enable Prometheus metrics:

   ```yaml
   OPTIMUS-IDE-COLLAB_PROMETHEUS_ENABLE=true
   ```

1. Enable database metrics:

   ```yaml
   OPTIMUS-IDE-COLLAB_PROMETHEUS_COLLECT_DB_METRICS=true
   ```
...
1. For a high scale deployment, configure agent stats to avoid large cardinality or disable them:
   ...
   - Configure agent stats:
     ...
     ```yaml
     OPTIMUS-IDE-COLLAB_PROMETHEUS_AGGREGATE_AGENT_STATS_BY=agent_name
     ```

   - Disable agent stats:

     ```yaml
     OPTIMUS-IDE-COLLAB_PROMETHEUS_COLLECT_AGENT_STATS=false
     ```
...
If increased availability of the Optimus-IDE-Collab API is a concern, deploy at least three
instances of Optimus-IDE-Collab Server. Spread the instances across nodes with anti-affinity rules in
Kubernetes or in different availability zones of the same geographic region.

Do not deploy in different geographic regions.

Optimus-IDE-Collab Servers need to be able to communicate with one another directly with low
latency, under 10ms. Note that this is for the availability of the Optimus-IDE-Collab API.
Workspaces are not fault tolerant unless they are explicitly built that way at
the template level.

Deploy Optimus-IDE-Collab Server instances as geographically close to PostgreSQL as possible.
Low-latency communication (under 10ms) with Postgres is essential for Optimus-IDE-Collab
Server's performance.

### Scaling

Optimus-IDE-Collab Server can be scaled both vertically for bigger instances and horizontally
for more instances.

Aim to keep the number of Optimus-IDE-Collab Server instances relatively small, preferably
under ten instances, and opt for vertical scale over horizontal scale after
meeting availability requirements.

Optimus-IDE-Collab's
...
We don't recommend that you autoscale the Optimus-IDE-Collab Servers. Instead, scale the
deployment for peak weekly usage.

Although Optimus-IDE-Collab Server persists no internal state, it operates as a proxy for end
...
1. As an HTTP proxy when they access workspace applications in their browser via
   the Optimus-IDE-Collab Dashboard.

1. As a DERP proxy when establishing tunneled connections with CLI tools like
   `optimus-ide-collab ssh`, `optimus-ide-collab port-forward`, and others, and with desktop IDEs.

Stopping a Optimus-IDE-Collab Server instance will (momentarily) disconnect any users
...
[provisioner daemon deployments external to Optimus-IDE-Collab Server](../../admin/provisioners/index.md)
and disable provisioner daemons within your Optimus-IDE-Collab Server.
This allows you to scale them independently of the Optimus-IDE-Collab Server:

```yaml
OPTIMUS-IDE-COLLAB_PROVISIONER_DAEMONS=0
```
...
- Provisioner daemons need to be able to connect to Optimus-IDE-Collab Server, but this does not need
...
time. Therefore, the maximum number of simultaneous builds your Optimus-IDE-Collab deployment
...
each has a unique `OPTIMUS-IDE-COLLAB_CACHE_DIRECTORY` value.
...
PostgreSQL is the primary persistence layer for all of Optimus-IDE-Collab's deployment data.
We also use `LISTEN` and `NOTIFY` to coordinate between different instances of
Optimus-IDE-Collab Server.

### Locality

Optimus-IDE-Collab Server instances must have low-latency connections (under 10ms) to
...
performance. Optimus-IDE-Collab's
[validated architectures](../../admin/infrastructure/validated-architectures/index.md)
give specific sizing recommendations for various user scales.

### Connection pool tuning

Optimus-IDE-Collab Server maintains a pool of connections to PostgreSQL. You can tune the
...
> set appropriately to accommodate all Optimus-IDE-Collab Server replicas multiplied by the
...
- `--postgres-conn-max-open` (env: `OPTIMUS-IDE-COLLAB_PG_CONN_MAX_OPEN`): Maximum number of open
  connections. Default: 10.
- `--postgres-conn-max-idle` (env: `OPTIMUS-IDE-COLLAB_PG_CONN_MAX_IDLE`): Maximum number of idle
...
- **Capacity**: `go_sql_max_open_connections - go_sql_in_use_connections` shows
  how many connections are available for new requests. If this is 0, Optimus-IDE-Collab
  ...
  each Optimus-IDE-Collab replica spent waiting on the connection pool (i.e. no free connections);
...
Workspace proxies proxy HTTP traffic from end users to workspaces for Optimus-IDE-Collab apps
...
Workspaces represent the vast majority of resources in most Optimus-IDE-Collab deployments.
...
It is important to be aware of these limits and monitor Optimus-IDE-Collab workspace resource
utilization against the limits, so that a new influx of users don't encounter
failed builds. Monitoring these is outside the scope of Optimus-IDE-Collab, but we recommend
...
- If you do overprovision workspaces onto nodes, keep them in a separate node
  pool and schedule Optimus-IDE-Collab control plane (Optimus-IDE-Collab Server, PostgreSQL, workspace
  proxies) components on a different node pool to avoid resource spikes
  affecting them.

Optimus-IDE-Collab customers have had success with both:
...
to their workspaces. This drastically reduces the load on Optimus-IDE-Collab Server and
