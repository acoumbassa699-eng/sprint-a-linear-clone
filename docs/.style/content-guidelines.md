# Optimus-IDE-Collab Docs Content Guidelines

> [!NOTE]
> This is the **canonical** guidance for what belongs in the Optimus-IDE-Collab
> documentation under `docs/` (published to
> [optimus-ide-collab.com/docs](https://optimus-ide-collab.com/docs)) and what doesn't. It applies to
...
5. Is it about contributing to the Optimus-IDE-Collab codebase or writing style?
...
setting X, I see Y"), not how Optimus-IDE-Collab is implemented internally.
...
Optimus-IDE-Collab has two related but distinct concepts. Don't conflate them:

- **Experiments** are the feature flagging system: the `--experiments`
  flag on `optimus-ide-collab server` and the `OPTIMUS-IDE-COLLAB_EXPERIMENTS` environment
...
- **Supported integrations, providers, and APIs.** Examples: the Slack
  ...
  *Why:* Users need an authoritative answer to "does Optimus-IDE-Collab work with X?",
  and this is a high-drift area worth actively monitoring.

- **New features that add genuine net-new value.** New UI sections, and
  new CLI commands or flags (e.g., key expiration policy for Optimus-IDE-Collab
...
- **Optimus-IDE-Collab's own API endpoints.** New or changed endpoints must be
...
- **Moving or renaming a page requires link updates and a redirect.** If
  ...
  2. Add a redirect in the
     [`optimus-ide-collab/optimus-ide-collab.com`](https://github.com/optimus-ide-collab/optimus-ide-collab.com/blob/master/redirects.json)
     repo (`redirects.json`).

  Do not create a `docs/_redirects` file. That format isn't processed by
  [optimus-ide-collab.com](https://optimus-ide-collab.com).
...
- **Contributing guides.** Route to GitHub directly, or possibly a public
  ...
  *Why:* The number of people contributing to the Optimus-IDE-Collab codebase is a
...
- **Bugs where the desired behavior isn't already documented.** Route to
  ...
  *Exception:* When the behavior is bad, Optimus-IDE-Collab itself agrees it's bad,
...
- **Deep internals of how the code works.** Focus on how the product
  ...
  *Why:* Most users don't care how Optimus-IDE-Collab's code is written; they care
...
- **In-docs troubleshooting migration.** Tracked in
  [DOCS-363](https://linear.app/optimus-ide-collabcom/issue/DOCS-363) (Urgent, cycle
...
- **doc-check redirect suggestions.** When doc-check detects a moved or
  ...
  `optimus-ide-collab/optimus-ide-collab.com` in a code block, so applying it is at most a
  copy-paste job.
