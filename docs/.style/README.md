...
Contributor-facing style and content guidance for the Optimus-IDE-Collab documentation.
Nothing under this directory is published to
[optimus-ide-collab.com/docs](https://optimus-ide-collab.com/docs).
...
| `styles/Optimus-IDE-Collab/`         | Custom Vale rules specific to Optimus-IDE-Collab (product voice, terms)          |
...
`styles/Optimus-IDE-Collab/` directory holds the custom Vale rules that enforce parts
...
guide's own prose under `style-guide/`, which is exempt); optimus-ide-collab.com's docs
site does not. Refer to "What does not run against this directory" below for
that exemption.

## How exclusion from optimus-ide-collab.com works

[optimus-ide-collab.com/docs](https://optimus-ide-collab.com/docs) routes and search are
manifest-driven:

- Route discovery lives in
  [`optimus-ide-collab/optimus-ide-collab.com:src/utils/docs/docs.ts`](https://github.com/optimus-ide-collab/optimus-ide-collab.com/blob/master/src/utils/docs/docs.ts)
...
- The Algolia surgical indexer at
  [`optimus-ide-collab/optimus-ide-collab.com:src/utils/algoliaDocs/surgical.ts`](https://github.com/optimus-ide-collab/optimus-ide-collab.com/blob/master/src/utils/algoliaDocs/surgical.ts)
...
- Vale (`make lint/prose`) lints everything here except the style guide's own
  ...
  annotation demo (whose `Optimus-IDE-Collab.Demo*` rules are meant to fire), and the Optimus-IDE-Collab
...
- Vale's `Optimus-IDE-Collab` rules on the style guide's own prose under
  ...
  the landing page, `content-guidelines.md`, the annotation demo, and the Optimus-IDE-Collab
...
2. **Enable commit**: add the rule to `.vale.ini` at its chosen severity, write a corresponding section under the matching subpage of `docs/.style/style-guide/`, and add the custom rule YAML under `docs/.style/styles/Optimus-IDE-Collab/` if applicable.
...
Optimus-IDE-Collab-authored rule with the precision the situation needs instead of
accepting an imprecise third-party rule at any severity.

This applies equally to Optimus-IDE-Collab-authored rules (under
`docs/.style/styles/Optimus-IDE-Collab/`) and third-party rules from Google, alex,
and write-good.
