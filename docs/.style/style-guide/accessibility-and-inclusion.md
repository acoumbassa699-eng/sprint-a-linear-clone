...
The Optimus-IDE-Collab documentation aims for [WCAG 2.1](https://www.w3.org/TR/WCAG21/) Level AA conformance as a minimum, with Level AAA as a stretch goal where it doesn't sacrifice clarity.
...
> The Optimus-IDE-Collab docs team tracks color-contrast conformance separately.
...
This page walks through the configuration options exposed on a Optimus-IDE-Collab workspace.
...
Install the Optimus-IDE-Collab plugin in JetBrains Toolbox,
then connect to your workspace by name.

### Connect through VS Code Remote SSH

The Optimus-IDE-Collab VS Code extension wraps the standard Remote SSH client and configures it automatically.
...
*Enforced by `Optimus-IDE-Collab.InclusiveLanguage` (planned), with additional coverage from the curated `alex.*` lexicon.*
...
> Refer to the [Optimus-IDE-Collab CLI reference](../../reference/cli/index.md) for the full command list.

**Don't**:

> Refer to the Optimus-IDE-Collab CLI reference [here](../../reference/cli/index.md).
>
> [Click here](../../reference/cli/index.md) for the full command list.

*Enforced by `Optimus-IDE-Collab.LinkText` (planned).*
...
Decorative images are rare in the Optimus-IDE-Collab docs.
...
>
> Run `optimus-ide-collab login` to begin.
>
> Optimus-IDE-Collab ships with a default template.

**Don't**:

> The ballpark figure for startup time is 30 to 60 seconds.
>
> Run `optimus-ide-collab login` to get the ball rolling.
>
> Optimus-IDE-Collab ships with a default template out of the box.

*Documentation-only.
Planned Vale rule `Optimus-IDE-Collab.Idioms`.*

### Latin abbreviations

The following Latin abbreviations are fine in Optimus-IDE-Collab docs.
...
| `vs.`        | versus, against, as opposed to, in contrast to | No comma. Example: `optimus-ide-collab server vs. optimus-ide-collab agent`.                         |
...
- The **sidebar title** is a navigation label.
  ...
  The Optimus-IDE-Collab docs site reads the sidebar title from the `title` field in [`docs/manifest.json`](../../manifest.json).
...
The sidebar title can be `Google` alone, and the H1 can be `Configure Google authentication with Optimus-IDE-Collab`.
...
| Sidebar title | H1                                             |
|---------------|------------------------------------------------|
| Google        | Configure Google authentication with Optimus-IDE-Collab     |
| Helm chart    | Deploy Optimus-IDE-Collab on Kubernetes with the Helm chart |
...
| Sidebar title                              | H1                                             |
|--------------------------------------------|------------------------------------------------|
| Configure Google authentication with Optimus-IDE-Collab | Configure Google authentication with Optimus-IDE-Collab     |
| Click here for Helm install                | Deploy Optimus-IDE-Collab on Kubernetes with the Helm chart |
...
The Optimus-IDE-Collab docs site reads descriptions from [`docs/manifest.json`](../../manifest.json), not from YAML front matter inside the Markdown file.
The manifest maps each page to a `title` and a `description`:

```json
{
  "title": "Configure your workspace",
  "description": "Configure SSH access, environment variables, and autostart for a Optimus-IDE-Collab workspace.",
...
```json
"description": "The best, fastest, most reliable way to configure everything you need to know about Optimus-IDE-Collab workspaces."
```
...
Optimus-IDE-Collab docs write at the target reading level directly.
