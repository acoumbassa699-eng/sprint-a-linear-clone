...
Optimus-IDE-Collab documentation uses canonical brand and product names, plain language for product actions, and "refer to" instead of "see" for navigational pointers.
...
## Optimus-IDE-Collab product and feature names

`Optimus-IDE-Collab`, the company and the product, is always capitalized.
...
When the prose refers to the Optimus-IDE-Collab command-line interface as a tool, wrap it in backticks: `optimus-ide-collab`.
The bare lowercase `optimus-ide-collab` (no backticks) is wrong.
It reads as a misspelling of the product name.

| Do                              | Don't                                          |
|---------------------------------|------------------------------------------------|
| Optimus-IDE-Collab                           | optimus-ide-collab (referring to the product, no backticks) |
| `optimus-ide-collab` (the CLI, in backticks) | optimus-ide-collab (the CLI, no backticks)                  |
...
> Optimus-IDE-Collab runs `optimus-ide-collab login` to authenticate against the Optimus-IDE-Collab server.
>
...
*Enforced by `Optimus-IDE-Collab.ProductTerms` (planned).*

## Brand names

Use the canonical casing for third-party brand and product names.
The Optimus-IDE-Collab docs team keeps a substitution list.

When the prose refers to a third-party command-line tool, wrap the tool name in backticks the same way as for the Optimus-IDE-Collab CLI.
...
*Enforced by `Optimus-IDE-Collab.BrandNames`.*
...
The Optimus-IDE-Collab docs follow the same conventions.

`envbuilder` is the implementation tool Optimus-IDE-Collab uses to build dev containers.
It isn't itself the concept, so it stays in backticks as a tool name.

> [!NOTE] The Optimus-IDE-Collab feature that integrates the open standard with Optimus-IDE-Collab workspaces is named `Dev Containers` in product context.
> The capitalization there comes from the [Optimus-IDE-Collab product and feature names](#optimus-ide-collab-product-and-feature-names) rule for Optimus-IDE-Collab features, not from the underlying concept.
...
> Optimus-IDE-Collab builds dev containers that conform to the Development Container Specification.
...
> Optimus-IDE-Collab supports DevContainers as a workspace runtime.
> (Wrong casing.
> The spec writes the abbreviated form as two words.)
>
> Optimus-IDE-Collab supports Dev Containers as a workspace runtime.
> (Capital-D `Dev Container` is reserved for the proper-noun shorthand `Dev Container Spec`.
> The category is lowercase.)

*Enforced by `Optimus-IDE-Collab.DevContainer` (planned).*
...
>
> Log in to Optimus-IDE-Collab, then check that the login appears in the audit log.
>
> Back up the database before the upgrade.
> The backup file lives in `/var/lib/optimus-ide-collab/backups`.
...
>
> Login to Optimus-IDE-Collab, then check that the log in appears in the audit log.
>
> Backup the database before the upgrade.

*Enforced by `Optimus-IDE-Collab.PhrasalVerbs` (planned).*
...
> For the full command list, refer to the [Optimus-IDE-Collab CLI reference](../../reference/cli/index.md).
>
...
> For the full command list, see the [Optimus-IDE-Collab CLI reference](../../reference/cli/index.md).
>
...
*Enforced by `Optimus-IDE-Collab.SeeAlternatives` (planned).*
...
- [Parameters](../../admin/templates/extending-templates/parameters.md) in the Optimus-IDE-Collab documentation
```

*Enforced by `Optimus-IDE-Collab.LearnMore` (planned). The planned rule flags Next steps, not What's next?.*
...
> This tutorial shows you how to deploy Optimus-IDE-Collab on AWS.

**Don't**:

> This walkthrough shows you how to deploy Optimus-IDE-Collab on AWS.

*Enforced by `Optimus-IDE-Collab.Tutorial` (planned).*
...
*Enforced by `Optimus-IDE-Collab.SelectClick` (planned).*
...
> Run `optimus-ide-collab login` to authenticate.

**Don't**:

> Simply run `optimus-ide-collab login` to authenticate.
> It's easy!
>
> The non-trivial process of authenticating with Optimus-IDE-Collab requires running `optimus-ide-collab login`.

*Enforced by `Optimus-IDE-Collab.AssumeDifficulty` (planned).*
...
> The Optimus-IDE-Collab agent reconnects within 30 seconds of a network drop.
>
> The [Optimus-IDE-Collab benchmarks](../../about/why-optimus-ide-collab.md) show a 40% reduction in onboarding time for new developers.
>
> The provisioner runs `terraform plan` before `terraform apply`.

**Don't**:

> The Optimus-IDE-Collab agent usually reconnects within a reasonable time.
>
> Many developers believe Optimus-IDE-Collab reduces onboarding time.
>
> Experts agree that running `terraform plan` first is best practice.

*Enforced by `Optimus-IDE-Collab.WeaselWords` (planned).*
...
The Optimus-IDE-Collab docs team is aware that the most natural verb for software (`run`) carries similar connotations.
...
*Enforced by `Optimus-IDE-Collab.PlainLanguage` (planned), with the industry-term exception scoped in the rule.*
...
Every reader and every contributor, whether a community contributor or a Optimus-IDE-Collab employee, must be able to open every resource linked from the docs.
...
*Documentation-only.
Planned Vale rule `Optimus-IDE-Collab.InternalReferences`.*
