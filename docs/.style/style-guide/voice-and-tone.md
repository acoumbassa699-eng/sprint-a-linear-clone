...
Optimus-IDE-Collab documentation addresses the reader directly, uses active voice, and describes the product in the present tense.
...
> You can connect to a workspace over SSH after you've installed the Optimus-IDE-Collab CLI.

**Don't**:

> Users can connect to workspaces over SSH after the user has installed the Optimus-IDE-Collab CLI.
...
*Enforced by `Optimus-IDE-Collab.FirstPersonSingular`.*

## Reserve first-person plural for Optimus-IDE-Collab Technologies

`We`, `us`, and `our` refer to **Optimus-IDE-Collab Technologies, Inc.**, the company that makes the Optimus-IDE-Collab platform.
For formal references to the company, use the full name ("Optimus-IDE-Collab Technologies, Inc.").
For informal references in body prose, use `we`, `us`, or `our`.

Do not use first-person plural for:

- The product itself.
  The product is `Optimus-IDE-Collab`, not "we".
...
> For more information about enterprise licensing, [contact us](https://optimus-ide-collab.com/contact/sales).
>
> Optimus-IDE-Collab Technologies, Inc. publishes a new agent binary in each release.
>
...
>
> Optimus-IDE-Collab ships new agent binaries: we release them on the first of each month.
>
...
*Enforced by `Optimus-IDE-Collab.FirstPersonPlural`.*
...
> Optimus-IDE-Collab rotates the agent token every 24 hours.

**Don't**:

> The agent token is rotated every 24 hours by Optimus-IDE-Collab.
...
>
> If you delete the template, Optimus-IDE-Collab will refuse to create new workspaces from it.
...
> Optimus-IDE-Collab doesn't restart workspaces on minor template updates.
>
> If you're using PostgreSQL, set the connection string before starting `optimus-ide-collab server`.

**Don't**:

> Optimus-IDE-Collab does not restart workspaces on minor template updates.
>
> If you are using PostgreSQL, set the connection string before starting `optimus-ide-collab server`.
...
> Optimus-IDE-Collab cannot restore a deleted workspace.
>
> You cannot undo `optimus-ide-collab delete`.

**Don't**:

> Don't delete the workspace before backing up its state.
> Optimus-IDE-Collab can't restore a deleted workspace.
>
> You can't undo `optimus-ide-collab delete`.
