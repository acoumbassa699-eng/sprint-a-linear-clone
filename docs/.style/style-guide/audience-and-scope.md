...
Every page in the Optimus-IDE-Collab documentation targets one audience working toward one outcome.
...
The single canonical Optimus-IDE-Collab example is **install Optimus-IDE-Collab**.
An end user wants to connect their local editor to a Optimus-IDE-Collab workspace and start coding.
A platform engineer wants to deploy the Optimus-IDE-Collab control plane to their company's Kubernetes cluster.
Both groups search for "install Optimus-IDE-Collab.
...
```md
# Connect Visual Studio Code to your Optimus-IDE-Collab workspace

*Audience: a developer with an existing Optimus-IDE-Collab workspace.*
...
```md
# Connect to your Optimus-IDE-Collab workspace
...
"Configure SSO" isn't.
"Deploy Optimus-IDE-Collab on AWS" is one outcome.
"Deploy Optimus-IDE-Collab" isn't.
...
This page is the entry point for configuring authentication in Optimus-IDE-Collab.
...
Do not use the [persona names](#personas-the-optimus-ide-collab-docs-serve) inside the page body either.
Persona names are vocabulary for writers planning the page, not for readers reading it.
Name the audience by the role the reader recognizes from their own work (`developer`, `template author`, `Optimus-IDE-Collab deployment administrator`, `organization owner`).
...
This guide is for a developer with an existing Optimus-IDE-Collab workspace.
...
Optimus-IDE-Collab runs on Kubernetes.
This page covers many topics related to running Optimus-IDE-Collab on Kubernetes.
```
...
This guide is for a Optimus-IDE-Collab deployment administrator
who has access to both the Optimus-IDE-Collab control plane and the Okta tenant.

> [!IMPORTANT]
> You must be a Optimus-IDE-Collab deployment administrator to complete this guide.
> If you aren't a deployment administrator,
> ask your administrator to complete the steps for you.
```

The prerequisite callout uses the role the reader recognizes (`Optimus-IDE-Collab deployment administrator`), not the writer-facing persona name (`Perry the Platform Engineer`).
...
When the page assumes a reader who runs their own Optimus-IDE-Collab deployment, that reader is their own administrator.
...
## Personas the Optimus-IDE-Collab docs serve

When deciding which audience a page targets, match the reader to one of the canonical personas the Optimus-IDE-Collab docs serve.
Each persona summary captures who the reader is, what they need from the docs, and the Optimus-IDE-Collab surface they typically work with.
...
Inside a page, name the audience by the role the reader recognizes from their own work (`developer`, `template author`, `Optimus-IDE-Collab deployment administrator`).
...
They own the templates, governance, and integrations that turn the Optimus-IDE-Collab control plane Ada deploys into the default workflow developers actually use.
...
*Optimus-IDE-Collab surface:* template authoring (Terraform, modules, prebuilds), RBAC, organizations and groups, policy and governance, integrations (CI/CD, observability, secrets, Git), audit logs.

#### Dave the Developer

Dave is a software engineer at a company that has adopted Optimus-IDE-Collab.
...
*Optimus-IDE-Collab surface:* workspaces, `optimus-ide-collab` CLI, IDE integrations (VS Code, Cursor, JetBrains, Windsurf, Zed, Vim, Emacs), web terminal, dotfiles, SSH and port forwarding.

#### Elliot the End User

Elliot uses a Optimus-IDE-Collab workspace day-to-day but isn't a software engineer.
...
*Optimus-IDE-Collab surface:* workspaces, web terminal, IDE and notebook integrations (VS Code, Jupyter, RStudio), dotfiles, port forwarding, SSH, file uploads and downloads.

> [!NOTE] Elliot is a stopgap umbrella persona for non-developer end users.
> The Optimus-IDE-Collab docs team plans to revisit the persona model with product and design once the broader audience is mapped out.

#### Ada the Infrastructure Admin

Ada runs the underlying infrastructure that Optimus-IDE-Collab deploys onto: Kubernetes clusters, cloud accounts, networking, storage, identity, and security policy.
...
*Optimus-IDE-Collab surface:* control plane install (Helm, Docker, VM, airgapped), database, networking and DERP, IAM and SSO/OIDC/SAML, telemetry and audit logs, backup and disaster recovery.

#### Steven the Sponsor

Steven is the CTO.
They approve the Optimus-IDE-Collab purchase and stay close enough to the architecture to ask sharp questions, but they no longer write code.
They need overview pages that explain what Optimus-IDE-Collab is, how it fits the existing stack, what it costs, and what its security and compliance posture looks like.

*Optimus-IDE-Collab surface:* architecture overviews, why-Optimus-IDE-Collab framing, pricing and licensing, security and compliance summaries, release notes, success-metric dashboards.
...
*Optimus-IDE-Collab surface:* GPU-enabled workspaces and templates, devcontainers, persistent storage, large-resource workspace configurations.
...
*Optimus-IDE-Collab surface:* workspaces for test environments, CI integrations, reproducible build patterns, workspace lifecycle (start, stop, rebuild).
...
They need docs that explain Optimus-IDE-Collab Tasks and the agent-driven flows in plain language, with no assumed dev-environment knowledge and no manual setup steps.
They avoid anything that requires opening a terminal or editing a config file.

*Optimus-IDE-Collab surface:* Optimus-IDE-Collab Tasks, AI Gateway, prompt-driven workflows, web-based interfaces.
...
*Optimus-IDE-Collab surface:* usage reports, audit logs, license management, billing and seat counts, telemetry exports.
...
*Optimus-IDE-Collab surface:* SSO and OIDC/SAML, RBAC, secrets management, audit logs, security architecture pages, compliance and trust-center content, allowlists and network policies.

#### Tara the Team Leader

Tara is an engineering manager or senior tech lead assigned the Group Admin role in Optimus-IDE-Collab RBAC.
...
*Optimus-IDE-Collab surface:* groups, group memberships, group-owned secrets, group-scoped templates, group audit logs.
...
- [Optimus-IDE-Collab documentation content guidelines](../content-guidelines.md)
