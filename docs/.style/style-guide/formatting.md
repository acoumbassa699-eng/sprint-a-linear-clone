...
Optimus-IDE-Collab documentation uses bold for UI elements, italics for emphasis, and code font for identifiers.
...
> The Optimus-IDE-Collab agent connects to the workspace, opens a Tailscale tunnel, and forwards SSH and IDE traffic over the tunnel.
...
> Run `optimus-ide-collab login --token <token>` to authenticate.
> Set `OPTIMUS-IDE-COLLAB_URL` in your environment first.
>
...
- `ps1` for Windows command-line blocks.
  PowerShell is the default Windows shell in the Optimus-IDE-Collab docs.
...
- `tf` for Terraform and HCL.
  ...
  Shiki ships `terraform` and `hcl` as two distinct grammars; `tf` is an alias of the more specific `terraform` grammar (not `hcl`), and matches what nearly every Optimus-IDE-Collab docs code block actually is.
- `yaml` for YAML.
...
The auto-generated Optimus-IDE-Collab CLI reference under `docs/reference/cli/` labels its command-usage blocks `console`.
...
```console
$ optimus-ide-collab templates list NAME        LAST UPDATED quickstart  2 minutes ago
```
````
...
````markdown
```
optimus-ide-collab login --token <token>
```
...
```sh
brew install optimus-ide-collab/optimus-ide-collab/optimus-ide-collab
```
...
```md
1. Run `optimus-ide-collab login` to authenticate.
...
```md
1. The user runs `optimus-ide-collab login` to authenticate
...
Prefer relative paths within the docs (`../reference/cli/index.md`) over absolute URLs (`https://optimus-ide-collab.com/docs/reference/cli`), so the link survives a future move of the docs site.

Links to non-docs locations in the Optimus-IDE-Collab codebase (source files, CI workflows, tests) also use relative paths.
