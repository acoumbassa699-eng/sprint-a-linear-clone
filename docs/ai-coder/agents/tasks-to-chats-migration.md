...
> [!NOTE]
> The Chats API is experimental in current Optimus-IDE-Collab releases. Endpoints live under `/api/experimental/chats` and may change without notice until the feature graduates to GA.

## When to migrate

Optimus-IDE-Collab Tasks is being deprecated. Support continues on the ESR release and
through Optimus-IDE-Collab v2.36. See the deprecation notice on the [Optimus-IDE-Collab Tasks](../tasks.md) page for the full timeline.

If you currently run workflows on the Tasks API, you should plan to
migrate to the Chats API and [Optimus-IDE-Collab Agents](./index.md). Optimus-IDE-Collab Agents
runs the agent loop in the Optimus-IDE-Collab control plane rather than inside the
...
| Template requirements  | Requires `optimus-ide-collab_ai_task` resource, `optimus-ide-collab_task` data source, and an agent module | Any template with a clear description works                |
| Chat state             | Stored in the workspace (AgentAPI state file)                                    | Persisted in the Optimus-IDE-Collab database                            |
...
variables (e.g. `ANTHROPIC_API_KEY`). With Optimus-IDE-Collab Agents, credentials are
...
You no longer pass API keys in template variables or workspace environment. See https://optimus-ide-collab.com/docs/ai-optimus-ide-collab/agents/getting-started for more information.
...
curl -X POST https://optimus-ide-collab.example.com/api/v2/tasks/me \
...
curl -X POST https://optimus-ide-collab.example.com/api/experimental/chats \
  -H "Optimus-IDE-Collab-Session-Token: $OPTIMUS-IDE-COLLAB_SESSION_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
...
curl -X POST https://optimus-ide-collab.example.com/api/v2/tasks/me/my-task/send \
  -H "Optimus-IDE-Collab-Session-Token: $OPTIMUS-IDE-COLLAB_SESSION_TOKEN" \
  -H "Content-Type: application/json" \
...
curl -X POST \
  https://optimus-ide-collab.example.com/api/experimental/chats/$CHAT_ID/messages \
  -H "Optimus-IDE-Collab-Session-Token: $OPTIMUS-IDE-COLLAB_SESSION_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "content": [
...
```sh
# Tasks API: get logs
curl https://optimus-ide-collab.example.com/api/v2/tasks/me/my-task/logs \
  -H "Optimus-IDE-Collab-Session-Token: $OPTIMUS-IDE-COLLAB_SESSION_TOKEN"
```
...
```txt
GET wss://optimus-ide-collab.example.com/api/experimental/chats/{chat}/stream
```
...
defined in `optimus-ide-collabsdk.ChatStatus`:
...
```diff
- curl -X DELETE https://optimus-ide-collab.example.com/api/v2/tasks/me/my-task \
-   -H "Optimus-IDE-Collab-Session-Token: $OPTIMUS-IDE-COLLAB_SESSION_TOKEN"

+ curl -X PATCH https://optimus-ide-collab.example.com/api/experimental/chats/$CHAT_ID \
+   -H "Optimus-IDE-Collab-Session-Token: $OPTIMUS-IDE-COLLAB_SESSION_TOKEN" \
+   -H "Content-Type: application/json" \
+   -d '{"archived": true}'
```
...
# Tasks API
curl -X POST \
  https://optimus-ide-collab.example.com/api/v2/tasks/me/my-task/pause \
  -H "Optimus-IDE-Collab-Session-Token: $OPTIMUS-IDE-COLLAB_SESSION_TOKEN"

curl -X POST \
  https://optimus-ide-collab.example.com/api/v2/tasks/me/my-task/resume \
  -H "Optimus-IDE-Collab-Session-Token: $OPTIMUS-IDE-COLLAB_SESSION_TOKEN"
```
...
# Chats API: interrupt
curl -X POST \
  https://optimus-ide-collab.example.com/api/experimental/chats/$CHAT_ID/interrupt \
  -H "Optimus-IDE-Collab-Session-Token: $OPTIMUS-IDE-COLLAB_SESSION_TOKEN"
...
If you use the
[Create Task Action](https://github.com/optimus-ide-collab/create-task-action) GitHub
Action, replace it with the dedicated
[`optimus-ide-collab/create-agent-chat-action`](https://github.com/optimus-ide-collab/create-agent-chat-action).
...
[releases](https://github.com/optimus-ide-collab/create-agent-chat-action/releases) for
updates.
...
-         optimus-ide-collab-task-prompt: >-
...
+     - name: Optimus-IDE-Collab Create Agent Chat
+       uses: optimus-ide-collab/create-agent-chat-action@v0
+       with:
+         optimus-ide-collab-url: ${{ secrets.OPTIMUS-IDE-COLLAB_URL }}
+         optimus-ide-collab-token: ${{ secrets.OPTIMUS-IDE-COLLAB_TOKEN }}
+         chat-prompt: >-
...
- No `optimus-ide-collab-template-name` or `optimus-ide-collab-task-name-prefix`. The agent
  auto-provisions a workspace; pass `workspace-id` if you want to pin to
  an existing workspace instead.
- The prompt input is renamed from `optimus-ide-collab-task-prompt` to `chat-prompt`.
- LLM credentials are no longer passed through the template. They are
  configured in the Optimus-IDE-Collab control plane.
- Identify the user with `github-user-id` (the action resolves it to a
  Optimus-IDE-Collab user via the GitHub OAuth link) or with `optimus-ide-collab-username`
  directly.

See the
[action README](https://github.com/optimus-ide-collab/create-agent-chat-action#inputs)
...
<!-- NEEDS REVIEW: This section contains initial recommendations based on
     the current Optimus-IDE-Collab Agents architecture. Platform teams should validate
     these against their own deployment patterns before adopting them. -->

> [!NOTE]
> This section contains recommendations that may evolve as Optimus-IDE-Collab Agents
> matures. Review these against your deployment requirements.

With Optimus-IDE-Collab Tasks, every task-capable template requires specific Terraform
resources (`optimus-ide-collab_ai_task`, `optimus-ide-collab_task`, agent modules, and LLM API
keys). With Optimus-IDE-Collab Agents, templates no longer need any of these. The
...
- **Network boundaries.** Agent workspaces inherit whatever network access
  ...
  agent templates to only reach the Optimus-IDE-Collab control plane and your git
...
If you are converting an existing task template for use with Optimus-IDE-Collab Agents,
...
+   # The Optimus-IDE-Collab Agents control plane handles the agent loop.
  }
```
...
WORKSPACE_ID=$(curl -s -X POST \
  https://optimus-ide-collab.example.com/api/v2/users/me/workspaces \
  -H "Optimus-IDE-Collab-Session-Token: $OPTIMUS-IDE-COLLAB_SESSION_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
...
curl -s -X POST https://optimus-ide-collab.example.com/api/experimental/chats \
  -H "Optimus-IDE-Collab-Session-Token: $OPTIMUS-IDE-COLLAB_SESSION_TOKEN" \
  -H "Content-Type: application/json" \
  -d "{
    \"organization_id\": \"<your-org-id>\",
...
`template_version_id` plus `optimus-ide-collab-template-name`: you decide which
...
```sh
curl -s https://optimus-ide-collab.example.com/api/experimental/chats/models \
  -H "Optimus-IDE-Collab-Session-Token: $OPTIMUS-IDE-COLLAB_SESSION_TOKEN" | jq '.[].display_name'
```
...
```sh
websocat -H "Optimus-IDE-Collab-Session-Token: $OPTIMUS-IDE-COLLAB_SESSION_TOKEN" \
  "wss://optimus-ide-collab.example.com/api/experimental/chats/$CHAT_ID/stream"
```
...
standard Optimus-IDE-Collab UI, then attach it to a new chat from the chat composer:

1. In the Optimus-IDE-Collab dashboard, create a workspace from the agent template
   you migrated.
...
```sh
curl -s "https://optimus-ide-collab.example.com/api/experimental/chats/$CHAT_ID" \
  -H "Optimus-IDE-Collab-Session-Token: $OPTIMUS-IDE-COLLAB_SESSION_TOKEN" | jq '{workspace_id, status}'
```
...
```sh
curl -s "https://optimus-ide-collab.example.com/api/experimental/chats/$CHAT_ID" \
  -H "Optimus-IDE-Collab-Session-Token: $OPTIMUS-IDE-COLLAB_SESSION_TOKEN" | jq '.status'
```
...
# Archive
curl -s -X PATCH \
  "https://optimus-ide-collab.example.com/api/experimental/chats/$CHAT_ID" \
  -H "Optimus-IDE-Collab-Session-Token: $OPTIMUS-IDE-COLLAB_SESSION_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"archived": true}'

# Confirm it no longer appears in the default list
curl -s "https://optimus-ide-collab.example.com/api/experimental/chats" \
  -H "Optimus-IDE-Collab-Session-Token: $OPTIMUS-IDE-COLLAB_SESSION_TOKEN" \
...
The Tasks CLI (`optimus-ide-collab task`) remains separate from the Optimus-IDE-Collab Agents Chats API.
Optimus-IDE-Collab no longer ships an interactive Optimus-IDE-Collab Agents TUI. Use the web UI for
interactive chat and direct API calls for automation.

| Tasks CLI           | Chats equivalent                       |
|---------------------|----------------------------------------|
| `optimus-ide-collab task create` | Web UI or `POST /chats`                |
| `optimus-ide-collab task list`   | Web UI or `GET /chats`                 |
| `optimus-ide-collab task logs`   | `GET /chats/{chat}/stream` (WebSocket) |
| `optimus-ide-collab task pause`  | `POST /chats/{chat}/interrupt`         |
| `optimus-ide-collab task resume` | Send a follow-up message to the chat   |
