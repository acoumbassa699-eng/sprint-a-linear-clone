...
This guide walks platform teams and administrators through setting up Optimus-IDE-Collab
Agents, preparing your deployment, and running your first Optimus-IDE-Collab Agent.

> [!NOTE]
> Optimus-IDE-Collab Agents is in Beta. APIs, behavior, and configuration may change
> between releases without notice; pin a release before broad rollout.
> Use **Optimus-IDE-Collab version 2.33.1 or greater**.

## Prerequisites

Before you begin, confirm the following:

- **Optimus-IDE-Collab deployment** running the latest release.
...
- **Admin access** to the Optimus-IDE-Collab deployment for configuring providers.
- **Optimus-IDE-Collab Agents User role** assigned to each user who needs to interact with Optimus-IDE-Collab Agents.
  ...
  **Members**. See [Grant Optimus-IDE-Collab Agents User](#step-2-grant-optimus-ide-collab-agents-user)
  below.
...
> **Owner** role (Optimus-IDE-Collab administrator). Non-admin users cannot access the
> admin Settings panel or modify deployment-level Agents configuration.

To configure Optimus-IDE-Collab Agents:
...
## Step 2: Grant Optimus-IDE-Collab Agents User

The **Optimus-IDE-Collab Agents User** role controls which users can interact with Optimus-IDE-Collab
...
1. Open **Admin settings** > **Organizations** in the Optimus-IDE-Collab dashboard, then
...
1. Toggle on **Optimus-IDE-Collab Agents User** and save.
...
ORG="my-org"
USER="alice"
ROLES=$(optimus-ide-collab organizations members list -O "$ORG" -o json \
  | jq -r --arg user "$USER" \
      '.[] | select(.username == $user) | [.roles[].name, "agents-access"]
      | unique | join(" ")')
# shellcheck disable=SC2086
optimus-ide-collab organizations members edit-roles "$USER" -O "$ORG" $ROLES
```
...
```sh
ORG="my-org"
optimus-ide-collab organizations members list -O "$ORG" -o json \
  | jq -c '.[] | {user_id, roles: [.roles[].name]}' \
  | while read -r row; do
      user_id=$(echo "$row" | jq -r '.user_id')
      roles=$(echo "$row" | jq -r '(.roles + ["agents-access"]) | unique | join(" ")')
      # shellcheck disable=SC2086
      optimus-ide-collab organizations members edit-roles "$user_id" -O "$ORG" $roles
    done
```

You can also set the organization with the `OPTIMUS-IDE-COLLAB_ORGANIZATION` environment
variable instead of `-O`.

## Step 3: Start your first Optimus-IDE-Collab Agent

1. Go to the **Agents** page in the Optimus-IDE-Collab dashboard.
...
Optimus-IDE-Collab Agents is under active development. APIs, behavior, and
...
Optimus-IDE-Collab Agents use browser push notifications to alert you when a task
...
Administrators can set a system prompt that applies to all Optimus-IDE-Collab Agents across the
...
Configure the system prompt from **AI Settings** > **Optimus-IDE-Collab Agents** > **Instructions**
...
The [Chats API](../../reference/api/chats.md) enables programmatic access to Optimus-IDE-Collab Agents.
This is useful for building automations such as:

- Triggering Optimus-IDE-Collab Agents from CI/CD pipelines when builds fail.
- Creating Optimus-IDE-Collab Agents from GitHub webhooks on new issues or PRs.
- Building internal tools or dashboards on top of the API.
- Scripting batch operations across repositories.

**Quick example — create a Optimus-IDE-Collab Agent via the API:**
...
Create an `AGENTS.md` file in the home directory (`~/.optimus-ide-collab/AGENTS.md`) or
...
for every conversation with a Optimus-IDE-Collab Agent that uses that workspace.
...
Optimus-IDE-Collab Agents is a collaborative evaluation between your team and Optimus-IDE-Collab.
...
channel** with the Optimus-IDE-Collab team.
