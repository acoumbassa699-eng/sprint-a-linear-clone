...
Optimus-IDE-Collab's AI agent interacts with workspaces over the same
...
When a developer opens a web terminal in the Optimus-IDE-Collab dashboard, connects via
VS Code Remote, or runs `optimus-ide-collab ssh`, the traffic follows this path:
...
This is unchanged from how workspaces already operate in Optimus-IDE-Collab. Enabling
Optimus-IDE-Collab Agents does not change your workspace network requirements.
...
`.mcp.json` files. The agent has no direct access to the Optimus-IDE-Collab API beyond
...
Optimus-IDE-Collab Agents run workspace commands through a POSIX-compatible shell (`sh`).
...
| Git authentication  | Control plane | Uses existing Optimus-IDE-Collab external auth (GitHub, GitLab, Bitbucket).            |
...
already configured in your Optimus-IDE-Collab deployment.
...
through the Optimus-IDE-Collab dashboard or CLI, the agent cannot access it either. There
...
The control plane overhead for Optimus-IDE-Collab Agents is minimal. The heavy computation
happens elsewhere:
