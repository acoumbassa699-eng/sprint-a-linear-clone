...
Not every chat with Optimus-IDE-Collab Agents requires a workspace. A workspace is only provisioned when the
...
and useful to Optimus-IDE-Collab Agents.
...
metadata shown on the templates page in the Optimus-IDE-Collab dashboard, sorted by number
...
`optimus-ide-collab templates push`, so updating it is part of your normal template
...
- **The Optimus-IDE-Collab control plane** — required for the workspace daemon to function.
...
access a resource through the Optimus-IDE-Collab dashboard, the agent cannot access it
either.
...
- **Configure external auth at the template level.** Use Optimus-IDE-Collab's
...
existing git authentication configured in your Optimus-IDE-Collab deployment. There is no
shared bot account.
...
resource "optimus-ide-collab_agent" "main" {
  os   = "linux"
  arch = "amd64"
  dir  = "/home/optimus-ide-collab/payments-service"
}
```
...
preset, Optimus-IDE-Collab assigns an already-running prebuilt workspace instead of
provisioning from scratch. The agent can begin working immediately.

## Checklist

Use this as a quick reference when creating or updating templates for Optimus-IDE-Collab
Agents:
