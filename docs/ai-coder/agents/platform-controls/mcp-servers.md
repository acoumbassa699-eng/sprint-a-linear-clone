...
This is an admin-only feature accessible at **AI Settings** > **Optimus-IDE-Collab Agents** > **MCP servers**
(`/ai/settings/mcp-servers`).

## Add an MCP server

1. Navigate to **AI Settings** > **Optimus-IDE-Collab Agents** > **MCP servers**.
1. Click **Add**.
...
| `forward_optimus-ide-collab_headers` | No       | When enabled, forwards Optimus-IDE-Collab identity headers on every outgoing MCP request. See [Optimus-IDE-Collab identity headers](#optimus-ide-collab-identity-headers). |
...
deleted from Optimus-IDE-Collab, and the disconnect response reports whether provider
...
Optimus-IDE-Collab via OIDC. Users who logged in with password or GitHub will see
...
## Optimus-IDE-Collab identity headers

MCP servers configured with `forward_optimus-ide-collab_headers = true` receive the
following identity headers on every outgoing request, alongside the
auth header for the configured `auth_type`:

| Header                 | Description                                                                                                  |
|------------------------|--------------------------------------------------------------------------------------------------------------|
| `X-Optimus-IDE-Collab-Owner-Id`     | Optimus-IDE-Collab user who owns the chat that issued the tool call.                                                      |
| `X-Optimus-IDE-Collab-Chat-Id`      | Top-level (parent) chat ID. For root chats this is the chat's own ID; for subchats it is the parent chat ID. |
| `X-Optimus-IDE-Collab-Subchat-Id`   | Subchat ID. Only present when the request originates from a child chat.                                      |
| `X-Optimus-IDE-Collab-Workspace-Id` | Workspace associated with the chat, if any.                                                                  |

Optimus-IDE-Collab sends the same identity headers to LLM providers, so a first-party
