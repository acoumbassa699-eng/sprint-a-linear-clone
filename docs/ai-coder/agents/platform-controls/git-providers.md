...
Optimus-IDE-Collab Agents leverages your existing
...
OPTIMUS-IDE-COLLAB_EXTERNAL_AUTH_0_API_BASE_URL="https://github.example.com/api/v3"
OPTIMUS-IDE-COLLAB_EXTERNAL_AUTH_0_REGEX=github\.example\.com
```

Without `API_BASE_URL`, Optimus-IDE-Collab defaults to `https://api.github.com`. Clone
and push still work (they use `AUTH_URL` and `TOKEN_URL` directly), but
the diff viewer silently fails because Optimus-IDE-Collab builds its URL-matching
...
For `gitlab.com`, no additional `API_BASE_URL` is needed. Optimus-IDE-Collab
...
Optimus-IDE-Collab Agents, configure:
...
OPTIMUS-IDE-COLLAB_EXTERNAL_AUTH_0_CLIENT_ID=xxxxxx
OPTIMUS-IDE-COLLAB_EXTERNAL_AUTH_0_CLIENT_SECRET=xxxxxxx
OPTIMUS-IDE-COLLAB_EXTERNAL_AUTH_0_SCOPES="write_repository read_api"
```
...
Optimus-IDE-Collab derives `API_BASE_URL` automatically from `AUTH_URL`:
...
Add `API_BASE_URL` to your GHE external auth config and restart Optimus-IDE-Collab.
