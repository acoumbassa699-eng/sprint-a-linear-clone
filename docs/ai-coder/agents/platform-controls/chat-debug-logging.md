...
1. **Deployment override.** Setting `OPTIMUS-IDE-COLLAB_CHAT_DEBUG_LOGGING_ENABLED=true`
...
curl -fsS \
  ...
  "$OPTIMUS-IDE-COLLAB_URL/api/experimental/chats/$CHAT_ID/debug/runs/$RUN_ID" \
  | jq . > "optimus-ide-collab-agents-debug-run-$RUN_ID.json"
```

Fetch every run returned by the list endpoint and save a chat-level export.
Using the same `OPTIMUS-IDE-COLLAB_URL`, `OPTIMUS-IDE-COLLAB_SESSION_TOKEN`, and `CHAT_ID` variables
from above:

```sh
RUN_IDS=$(curl -fsS \
  -H "Optimus-IDE-Collab-Session-Token: $OPTIMUS-IDE-COLLAB_SESSION_TOKEN" \
  "$OPTIMUS-IDE-COLLAB_URL/api/experimental/chats/$CHAT_ID/debug/runs" \
  | jq -r '.[].id') || {
...
jq -s \
  ...
  '{
    ...
    runs: .
  }' "$RUN_EXPORTS" > "optimus-ide-collab-agents-debug-chat-$CHAT_ID.json"
```
