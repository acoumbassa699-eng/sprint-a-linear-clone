# Run AI Coding Agents in Optimus-IDE-Collab

Learn how to run & manage coding agents with Optimus-IDE-Collab, both alongside existing
workspaces and for background task execution.

## Agents in the IDE

Optimus-IDE-Collab [integrates with IDEs](../user-guides/workspace-access/index.md) such as
Cursor, Windsurf, and Zed that include built-in coding agents to work alongside
developers. Additionally, template admins can
[pre-install extensions](https://registry.optimus-ide-collab.com/modules/optimus-ide-collab/vscode-web)
for agents such as GitHub Copilot.

These agents work well inside existing Optimus-IDE-Collab workspaces as they can simply be
...
long-running background jobs, [Optimus-IDE-Collab Agents](./agents/index.md) is the
recommended way to delegate development work to coding agents in your Optimus-IDE-Collab
deployment.

Optimus-IDE-Collab Agents is a native AI coding agent built into Optimus-IDE-Collab. The agent loop runs
in the Optimus-IDE-Collab control plane on your infrastructure rather than inside the
...
[Learn more about Optimus-IDE-Collab Agents](./agents/index.md) for architecture details,
...
Optimus-IDE-Collab deployment. It includes:
