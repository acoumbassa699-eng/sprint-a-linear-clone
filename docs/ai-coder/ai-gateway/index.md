...
AI Gateway supports clients running inside or outside Optimus-IDE-Collab workspaces.
...
1. **Centralized authn/z management**: no more issuing & managing API tokens for OpenAI/Anthropic usage.
   Users use their Optimus-IDE-Collab session or API tokens to authenticate with `optimus-ide-collabd` (Optimus-IDE-Collab control plane), and
   `optimus-ide-collabd` securely communicates with the upstream APIs on their behalf.
...
- [Set up AI Gateway](./setup.md) on your Optimus-IDE-Collab deployment
