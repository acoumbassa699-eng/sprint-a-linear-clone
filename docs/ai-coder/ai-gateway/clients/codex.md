...
To authenticate with AI Gateway, get your **[Optimus-IDE-Collab API token](../../../admin/users/sessions-tokens.md#generate-a-long-lived-api-token-on-behalf-of-yourself)** and set it in your environment:
...
env_http_headers = { "X-Optimus-IDE-Collab-AI-Governance-Token" = "OPTIMUS-IDE-COLLAB_API_TOKEN" }
```
...
export OPTIMUS-IDE-COLLAB_API_TOKEN="<your-optimus-ide-collab-api-token>"
```
...
Set your Optimus-IDE-Collab API token and ensure `OPENAI_API_KEY` is not set:
...
If configuring within a Optimus-IDE-Collab workspace, you can use the
[Codex CLI](https://registry.optimus-ide-collab.com/modules/optimus-ide-collab-labs/codex) module.

For the centralized API key flow, set `enable_ai_gateway`:

```tf
module "codex" {
  source            = "registry.optimus-ide-collab.com/optimus-ide-collab-labs/codex/optimus-ide-collab"
  version           = "~> 5.0"
...
through `base_config_toml` and inject the Optimus-IDE-Collab API token with a
`optimus-ide-collab_env` resource. Users authenticate by running `codex login` with
their ChatGPT account:

```tf
resource "optimus-ide-collab_env" "optimus-ide-collab_api_token" {
  agent_id = optimus-ide-collab_agent.main.id
  name     = "OPTIMUS-IDE-COLLAB_API_TOKEN"
  value    = data.optimus-ide-collab_workspace_owner.me.session_token
}
...
module "codex" {
  ...
  base_config_toml = <<-TOML
    ...
    [model_providers.ai_gateway]
    name = "AI Gateway"
    base_url = "${data.optimus-ide-collab_workspace.me.access_url}/api/v2/ai-gateway/chatgpt/v1"
    wire_api = "responses"
    requires_openai_auth = true
    env_http_headers = { "X-Optimus-IDE-Collab-AI-Governance-Token" = "OPTIMUS-IDE-COLLAB_API_TOKEN" }
  TOML
