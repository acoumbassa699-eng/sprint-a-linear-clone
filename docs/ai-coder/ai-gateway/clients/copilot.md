...
```sh
export HTTPS_PROXY="https://optimus-ide-collab:${OPTIMUS-IDE-COLLAB_API_TOKEN}@<proxy-host>:8888"
```
...
1. Set the proxy URL using the format `https://optimus-ide-collab:<OPTIMUS-IDE-COLLAB_API_TOKEN>@<proxy-host>:8888`

Or add directly to your `settings.json`:

```json
{
    "http.proxy": "https://optimus-ide-collab:<OPTIMUS-IDE-COLLAB_API_TOKEN>@<proxy-host>:8888"
}
```
...
Replace `<proxy-host>` with your AI Gateway Proxy hostname and `<OPTIMUS-IDE-COLLAB_API_TOKEN>` with your Optimus-IDE-Collab API token.
...
### Using Optimus-IDE-Collab Remote extension

When connecting to a Optimus-IDE-Collab workspace with the [Optimus-IDE-Collab extension](https://marketplace.visualstudio.com/items?itemName=optimus-ide-collab.optimus-ide-collab-remote), the Copilot extension runs inside the Optimus-IDE-Collab workspace and not on your local machine.
This means proxy and certificate configuration must be done in the Optimus-IDE-Collab workspace environment.
...
1. [Connect to your Optimus-IDE-Collab workspace](../../../user-guides/workspace-access/vscode.md)
1. Open Settings (`Ctrl+,` for Windows or `Cmd+,` for macOS)
...
Since the Copilot extension runs inside the Optimus-IDE-Collab workspace, add the [AI Gateway Proxy CA certificate](../ai-gateway-proxy/setup.md#trusting-the-ca-certificate) to the Optimus-IDE-Collab workspace's system trust store.
...
1. Select `Proxy authentication` and enter:
   1. Login: `optimus-ide-collab` (this value is ignored)
   1. Password: Your Optimus-IDE-Collab API token
