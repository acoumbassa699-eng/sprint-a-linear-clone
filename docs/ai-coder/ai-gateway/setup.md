...
AI Gateway runs inside the Optimus-IDE-Collab control plane (`optimus-ide-collabd`), requiring no separate compute to deploy or scale. Once enabled, `optimus-ide-collabd` runs the `aibridged` in-memory and brokers traffic to your configured AI providers on behalf of authenticated users.
...
```sh
export OPTIMUS-IDE-COLLAB_AI_GATEWAY_ENABLED=true
optimus-ide-collab server
# or
optimus-ide-collab server --ai-gateway-enabled=true
```
...
without restarting `optimus-ide-collabd`.
...
disable it without restarting `optimus-ide-collabd`.
...
```sh
optimus-ide-collab server --ai-gateway-dump-dir=/var/lib/optimus-ide-collab/ai-gateway-dumps
```
...
```yaml
ai_gateway:
  api_dump_dir: /var/lib/optimus-ide-collab/ai-gateway-dumps
```
...
with `/var/lib/optimus-ide-collab/ai-gateway-dumps` configured writes to
`/var/lib/optimus-ide-collab/ai-gateway-dumps/anthropic-corp`.
...
Configure retention using `--ai-gateway-retention` or `OPTIMUS-IDE-COLLAB_AI_GATEWAY_RETENTION`:

```sh
optimus-ide-collab server --ai-gateway-retention=90d
```
...
Enable with `--ai-gateway-structured-logging` or `OPTIMUS-IDE-COLLAB_AI_GATEWAY_STRUCTURED_LOGGING`:

```sh
optimus-ide-collab server --ai-gateway-structured-logging=true
```
...
These logs are written to the same output stream as all other `optimus-ide-collabd` logs,
