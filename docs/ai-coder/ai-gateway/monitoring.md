...
AI Gateway records the last `user` prompt, token usage, model reasoning, and every tool invocation for each intercepted request. Each capture is tied to a single "interception" that maps back to the authenticated Optimus-IDE-Collab identity, making it easy to attribute spend and behaviour.
...
|--------------------------------------------------------------------------|---------|--------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------|
| `optimus-ide-collab_ai_gateway_provider_info`                                         | gauge   | `provider_name`, `provider_type`, `status` | One series per configured provider. Value is always `1`; the `status` label (`enabled`, `disabled`, `error`) carries the alertable signal. |
| `optimus-ide-collab_ai_gateway_providers_last_reload_timestamp_seconds`               | gauge   |                                            | Unix timestamp of the last reload attempt, success or failure.                                                                             |
| `optimus-ide-collab_ai_gateway_providers_last_reload_success_timestamp_seconds`       | gauge   |                                            | Unix timestamp of the last reload that successfully refreshed the pool.                                                                    |
| `optimus-ide-collab_ai_gateway_proxy_provider_info`                                   | gauge   | `provider_name`, `provider_type`, `status` | Same shape as `optimus-ide-collab_ai_gateway_provider_info` but reported by the external proxy.                                                         |
| `optimus-ide-collab_ai_gateway_proxy_providers_last_reload_timestamp_seconds`         | gauge   |                                            | Last reload attempt timestamp in the external proxy.                                                                                       |
| `optimus-ide-collab_ai_gateway_proxy_providers_last_reload_success_timestamp_seconds` | gauge   |                                            | Last successful reload timestamp in the external proxy.                                                                                    |
| `optimus-ide-collab_ai_gateway_proxy_connect_sessions_total`                          | counter | `type` (`mitm`, `tunneled`)                | CONNECT sessions established by the proxy.                                                                                                 |
| `optimus-ide-collab_ai_gateway_proxy_mitm_requests_total`                             | counter | `provider`                                 | MITM requests handled.                                                                                                                     |
| `optimus-ide-collab_ai_gateway_proxy_inflight_mitm_requests`                          | gauge   | `provider`                                 | In-flight MITM requests.                                                                                                                   |
| `optimus-ide-collab_ai_gateway_proxy_mitm_responses_total`                            | counter | `code`, `provider`                         | MITM responses by HTTP status code.                                                                                                        |

> [!IMPORTANT]
> The AI Gateway metric prefixes were renamed: `optimus-ide-collab_aibridged_*` became
> `optimus-ide-collab_ai_gateway_*` and `optimus-ide-collab_aibridgeproxyd_*` became
> `optimus-ide-collab_ai_gateway_proxy_*`. This rename covers every AI Gateway metric,
...
> The legacy `optimus-ide-collab_aibridged_*` and `optimus-ide-collab_aibridgeproxyd_*` names are still
...
>     regex: 'optimus-ide-collab_ai_gateway_proxy_(.*)'
>     target_label: __name__
>     replacement: 'optimus-ide-collab_aibridgeproxyd_${1}'
>   - source_labels: [__name__]
>     regex: 'optimus-ide-collab_ai_gateway_(.*)'
>     target_label: __name__
>     replacement: 'optimus-ide-collab_aibridged_${1}'
> ```
...
```promql
sum by (provider_name, status) (optimus-ide-collab_ai_gateway_provider_info{status!="enabled"}) > 0
```
...
```promql
(optimus-ide-collab_ai_gateway_providers_last_reload_timestamp_seconds
  - optimus-ide-collab_ai_gateway_providers_last_reload_success_timestamp_seconds) > 300
```

Repeat the same query against `optimus-ide-collab_ai_gateway_proxy_*` if you run the
external proxy.
...
You can retrieve AI Gateway sessions via the Optimus-IDE-Collab API, with filtering and pagination support.

```sh
curl -X GET "https://optimus-ide-collab.example.com/api/v2/ai-gateway/sessions" \
...
AI Gateway tracing is enabled when tracing is enabled for the Optimus-IDE-Collab server.
To enable tracing set `OPTIMUS-IDE-COLLAB_TRACE_ENABLE` environment variable or
[--trace](https://optimus-ide-collab.com/docs/reference/cli/server#--trace) CLI flag:

```sh
export OPTIMUS-IDE-COLLAB_TRACE_ENABLE=true
```

```sh
optimus-ide-collab server --trace
```
...
by setting `OPTIMUS-IDE-COLLAB_TRACE_LOGS` environment variable or using
[--trace-logs](https://optimus-ide-collab.com/docs/reference/cli/server#--trace-logs) flag:

```sh
export OPTIMUS-IDE-COLLAB_TRACE_ENABLE=true
export OPTIMUS-IDE-COLLAB_TRACE_LOGS=true
```

```sh
optimus-ide-collab server --trace --trace-logs
```
