...
> `OPTIMUS-IDE-COLLAB_AI_GATEWAY_PROVIDER_<N>_*`, `OPTIMUS-IDE-COLLAB_AI_GATEWAY_OPENAI_*`,
> `OPTIMUS-IDE-COLLAB_AI_GATEWAY_ANTHROPIC_*`, and their `--aibridge/ai-gateway-*`
...
> completed. Visit `https://<your-optimus-ide-collab-host>/ai/settings/providers` to see
...
- **AWS SDK default credential chain (recommended).** When no explicit
  ...
  Attaching an IAM Role to the compute running Optimus-IDE-Collab follows
...
3. **Create an access key** for the IAM user:
   ...
   - Select **Next**.
   - Add a description like "Optimus-IDE-Collab AI Gateway token".
...
restarting `optimus-ide-collabd`. AI Gateway records the timestamp of each reload
...
- `optimus-ide-collab_ai_gateway_providers_last_reload_success_timestamp_seconds`

If you run the [external proxy](./ai-gateway-proxy/index.md), it exposes
the same pair under the `optimus-ide-collab_ai_gateway_proxy_` prefix.
