...
AI Gateway authenticates clients with the same Optimus-IDE-Collab API token
that a user already uses against the rest of the Optimus-IDE-Collab API.
No separate AI Gateway login or credential is required.

Authenticating with a Optimus-IDE-Collab token avoids distributing provider-specific API keys
...
> [!NOTE]
> Only Optimus-IDE-Collab-issued tokens can authenticate users to AI Gateway.
...
## Create a Optimus-IDE-Collab API token

You can generate a token from the Optimus-IDE-Collab dashboard or the CLI.
...
From the CLI, print your current session token with [`optimus-ide-collab login token`](../../reference/cli/login_token.md):
...
```sh
optimus-ide-collab tokens create --lifetime 30d -n my-ai-token
```
...
If you're logged in with the Optimus-IDE-Collab CLI, you can retrieve your current session token
by using [`optimus-ide-collab login token`](../../reference/cli/login_token.md):

```sh
export ANTHROPIC_API_KEY=$(optimus-ide-collab login token)
...
from the Optimus-IDE-Collab dashboard.
...
The Optimus-IDE-Collab token is supplied in the proxy URL:

```sh
export HTTPS_PROXY="https://optimus-ide-collab:$(optimus-ide-collab login token)@<proxy-host>:8888"
```
...
- A **Optimus-IDE-Collab API token** to authenticate with AI Gateway.
...
Optimus-IDE-Collab Agents requests routed through AI Gateway are in-process control plane
requests, not external client requests that send their own AI Gateway bearer
token. Optimus-IDE-Collab Agents use this same global BYOK setting. When BYOK is enabled,
...
Administrators can disable it using `--ai-gateway-allow-byok=false` or `OPTIMUS-IDE-COLLAB_AI_GATEWAY_ALLOW_BYOK=false`:

```sh
optimus-ide-collab server --ai-gateway-allow-byok=false
```
...
1. Create a new token with `optimus-ide-collab tokens create`.
2. Update the client's configuration to use the new token.
3. Delete the old token from the dashboard or with `optimus-ide-collab tokens rm <name>`.
