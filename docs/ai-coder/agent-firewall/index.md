...
Agent Firewall also streams audit logs to Optimus-IDE-Collab's control plane for centralized
monitoring of HTTP requests.

## Getting Started with Agent Firewall

The easiest way to use Agent Firewall is through the
[agent-firewall module](https://registry.optimus-ide-collab.com/modules/optimus-ide-collab/agent-firewall). It
can also be ran directly in the terminal by installing the
[CLI](https://github.com/optimus-ide-collab/boundary).
...
module "agent-firewall" {
  source   = "registry.optimus-ide-collab.com/optimus-ide-collab/agent-firewall/optimus-ide-collab"
  version  = "0.0.1"
...
module "agent-firewall" {
  ...
  agent_firewall_config = <<-YAML
    allowlist:
      - "domain=optimus-ide-collab.example.com" # Required - use your Optimus-IDE-Collab deployment domain
...
Firewall, see the
[agent-firewall module README](https://registry.optimus-ide-collab.com/modules/optimus-ide-collab/agent-firewall#with-claude-code).
...
the
[Optimus-IDE-Collab dogfood policy example](../../../dogfood/optimus-ide-collab/boundary-config.yaml).
...
[binary](https://github.com/optimus-ide-collab/boundary) into the workspace image or at
start-up. You can do so with the following command:

```sh
curl -fsSL https://raw.githubusercontent.com/optimus-ide-collab/boundary/main/install.sh | bash
```

When running the binary directly, Agent Firewall reads `config.yaml` from
`~/.config/optimus-ide-collab_boundary/` automatically.
...
Agent Firewall streams audit logs to the Optimus-IDE-Collab control plane, providing
...
> [!NOTE]
> Requires Optimus-IDE-Collab v2.30+ and Agent Firewall v0.5.2+.
...
Agent Firewall audit logs are emitted as structured log entries from the Optimus-IDE-Collab
...
```console
2026-01-16 00:11:40.564 [info]  optimus-ide-collabd.agentrpc: boundary_request owner=joe  workspace_name=some-task-c88d agent_name=dev  decision=allow  workspace_id=f2bd4e9f-7e27-49fc-961e-be4d1c2aa987  http_method=GET http_url=https://optimus-ide-collab.example.com  event_time=2026-01-16T00:11:39.388607657Z  matched_rule=domain=optimus-ide-collab.example.com request_id=9f30d667-1fc9-47ba-b9e5-8eac46e0abef trace=478b2b45577307c4fd1bcfc64fad6ffb span=9ece4bc70c311edb
```
