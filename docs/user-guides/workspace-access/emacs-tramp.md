...
`optimus-ide-collab.<WORKSPACE NAME>`.

In Emacs type `C-x d` and then input: `/-:optimus-ide-collab.<WORKSPACE NAME>:` and hit
enter. This will open up Dired on the workspace's home directory.

### Using SSH

By default Emacs TRAMP is setup to use SCP to access files on the Optimus-IDE-Collab
...
Then when you access the workspace instance via `/-:optimus-ide-collab.<WORKSPACE NAME>`
...
1. In your workspace Terraform template be sure to add the following:
   ...
   resource "optimus-ide-collab_agent" "main" {
     # ...
     env = {
       name = "OPTIMUS-IDE-COLLAB_WORKSPACE_NAME"
...
2. Next in the shell profile file on the workspace (ex., `~/.bashrc` for Bash
   ...
   ansi_term_announce_host() {
       printf '\033AnSiTh %s\n' "optimus-ide-collab.$OPTIMUS-IDE-COLLAB_WORKSPACE_NAME"
   }
...
`optimus-ide-collab.<WORKSPACE NAME>`. Be sure to replace the values between angle brackets:
...
(connection-local-set-profiles '(:machine "optimus-ide-collab.<WORKSPACE NAME>") 'remote-path-lsp-servers)
```
...
`optimus-ide-collab.<WORKSPACE NAME>`. Be sure to replace `<PATH TO ADD>` with the directory
