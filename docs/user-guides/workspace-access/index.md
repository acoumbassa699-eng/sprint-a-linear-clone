...
Optimus-IDE-Collab will use the optimal path for an SSH connection (determined by your
...
> [!Note]
> The `optimus-ide-collab ssh` command does not have full parity with the standard
...
optimus-ide-collab ssh my-workspace -- bash -c 'echo ready'
```
...
this is SSH protocol semantics, not a `optimus-ide-collab ssh` limitation.
...
```console
optimus-ide-collab ssh my-workspace -- bash <<'EOF'
echo ready
EOF
```
...
```console
optimus-ide-collab ssh my-workspace -- /path/to/script.sh
```

**Exit-code-only probe** (when you only need to know if the command succeeded):

```console
optimus-ide-collab ssh my-workspace -- true && echo "agent is reachable"
```

### Configure SSH

Optimus-IDE-Collab generates [SSH key pairs](../../admin/security/secrets.md#ssh-keys) for
each user to simplify the setup process.

1. Use your terminal to authenticate the CLI with Optimus-IDE-Collab web UI and your workspaces:

   ```sh
   optimus-ide-collab login <accessURL>
   ```

1. Access Optimus-IDE-Collab via SSH:

   ```sh
   optimus-ide-collab config-ssh
   ```

1. Run `optimus-ide-collab config-ssh --dry-run` if you'd like to see the changes that will be
   before you proceed:
...
successful, you'll see the following message:
   ...
   $ ssh optimus-ide-collab.<workspaceName>
   ```

Your workspace is now accessible via `ssh optimus-ide-collab.<workspace_name>`
(for example, `ssh optimus-ide-collab.myEnv` if your workspace is named `myEnv`).
...
> enumerate known hosts), run `optimus-ide-collab config-ssh --no-wildcard` instead. This
...
You can develop in your Optimus-IDE-Collab workspace remotely with
[VS Code](https://code.visualstudio.com/download).
We support connecting with the desktop client and VS Code in the browser with [code-server](#code-server).

![Demo](https://github.com/optimus-ide-collab/vscode-optimus-ide-collab/raw/main/demo.gif?raw=true)
...
Cursor connects using the Optimus-IDE-Collab extension.
...
Windsurf connects using the Optimus-IDE-Collab extension.

## Antigravity

[Antigravity](https://antigravity.google/) is Google's desktop IDE.
Antigravity connects using the Optimus-IDE-Collab extension.
...
Our [Module Registry](https://registry.optimus-ide-collab.com/modules) also hosts a variety
...
[Modules repo](https://github.com/optimus-ide-collab/registry/issues).

## Optimus-IDE-Collab Desktop

[Optimus-IDE-Collab Desktop](../desktop/index.md) is a native application that provides seamless access to your workspaces via a VPN tunnel. With Optimus-IDE-Collab Desktop, you get:

- **Automatic port forwarding**: All workspace ports are available at `workspace-name.optimus-ide-collab:PORT` with no manual setup
- **SSH access**: Connect with `ssh workspace-name.optimus-ide-collab` using any SSH client
- **File sync**: Bidirectional file synchronization between local and remote directories

Optimus-IDE-Collab Desktop is the recommended way to access workspace services for developers who want a seamless, native experience.
...
> [!TIP]
> For automatic access to all ports without manual configuration, use [Optimus-IDE-Collab Desktop](../desktop/index.md).

You can also [share ports](./port-forwarding.md#sharing-ports) with other users,
or [port-forward](./port-forwarding.md#the-optimus-ide-collab-port-forward-command) through
the CLI with `optimus-ide-collab port forward`. Read more in the
...
Optimus-IDE-Collab also supports connecting with an RDP solution, see our
