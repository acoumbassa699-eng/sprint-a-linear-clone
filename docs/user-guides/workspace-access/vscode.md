...
automatically installs the [Optimus-IDE-Collab Remote](https://github.com/optimus-ide-collab/vscode-optimus-ide-collab)
extension, authenticates with Optimus-IDE-Collab, and connects to the workspace.
...
```txt
ext install optimus-ide-collab.optimus-ide-collab-remote
```

Alternatively, manually install the VSIX from the
[latest release](https://github.com/optimus-ide-collab/vscode-optimus-ide-collab/releases/latest).

## Local telemetry

The Optimus-IDE-Collab Remote extension records local telemetry to help diagnose extension
and workspace connection issues. Telemetry is stored on your machine. It is not
sent to Optimus-IDE-Collab unless you export it or include it in a support bundle and share
that file.

Local telemetry is controlled by the VS Code setting `optimus-ide-collab.telemetry.level`:
...
[extension event reference](https://github.com/optimus-ide-collab/vscode-optimus-ide-collab/blob/main/src/instrumentation/EVENTS.md).
...
You can tune local retention with the advanced `optimus-ide-collab.telemetry.local` setting.
...
- **Optimus-IDE-Collab: Export Telemetry** exports only local telemetry. Choose a date range
  and JSON or OTLP JSON zip format, then review the file before sharing it.
- **Optimus-IDE-Collab: Create Support Bundle** runs `optimus-ide-collab support bundle` and adds a
  ...
  added when the bundle is created from the VS Code Optimus-IDE-Collab Remote extension;
  bundles created with the CLI alone do not include it.
- **Optimus-IDE-Collab: View Logs** opens the extension output logs in VS Code.
...
extensions can be from Optimus-IDE-Collab's public marketplace, Eclipse Open VSX's public
...
1. Add the `vsix` extension files to the same folder as your Dockerfile.
   ...
   ~/images/base
    ➜  ls -l
    -rw-r--r-- 1 optimus-ide-collab optimus-ide-collab       0 Aug 1 19:23 Dockerfile
    -rw-r--r-- 1 optimus-ide-collab optimus-ide-collab 8925314 Aug 1 19:40 GitHub.copilot.vsix
   ```
...
```console
SERVICE_URL=https://extensions.optimus-ide-collab.com/api ITEM_URL=https://extensions.optimus-ide-collab.com/item /path/to/code-server --install-extension GitHub.copilot
```
...
For your local VS Code to pickup extension files in your Optimus-IDE-Collab workspace,
