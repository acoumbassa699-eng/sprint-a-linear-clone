...
access to your workspace's shell environment directly from the Optimus-IDE-Collab dashboard.
...
1. Navigate to your workspace in the Optimus-IDE-Collab dashboard
...
```txt
https://optimus-ide-collab.example.com/@username/workspace-name/terminal
```

To access a specific agent in a multi-agent workspace:

```txt
https://optimus-ide-collab.example.com/@username/workspace-name.agent-name/terminal
```
...
3. **Optimus-IDE-Collab Server**: Routes traffic between browser and workspace
4. **Workspace Agent**: Manages the pseudo-terminal (PTY) session
5. **Shell Process**: Your actual bash/zsh/fish shell

The connection flow is: Browser ↔ WebSocket ↔ Optimus-IDE-Collab Server ↔ Workspace Agent ↔ Shell Process
...
```yaml
# In your Optimus-IDE-Collab deployment configuration
...
```sh
OPTIMUS-IDE-COLLAB_WEB_TERMINAL_RENDERER=canvas
```
...
> **Note:** The renderer setting is deployment-wide and requires a Optimus-IDE-Collab server
...
- **Localhost URLs** (e.g., `http://localhost:3000`) are automatically
  port-forwarded through Optimus-IDE-Collab's [port forwarding](./port-forwarding.md) system
...
```txt
https://optimus-ide-collab.example.com/@user/workspace/terminal?command=htop
```
...
```txt
https://optimus-ide-collab.example.com/@user/workspace/terminal?container=sidecar
```

You can also specify the container user:

```txt
https://optimus-ide-collab.example.com/@user/workspace/terminal?container=app&container_user=node
```
...
```txt
https://optimus-ide-collab.example.com/@user/workspace/terminal?debug
```
