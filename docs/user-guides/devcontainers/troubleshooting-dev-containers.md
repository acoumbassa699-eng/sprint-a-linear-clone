...
1. Check the agent logs for error messages:
   ...
   - `/tmp/optimus-ide-collab-script-[script_id].log`
...
Use `optimus-ide-collab show` to list all agents in your workspace, including dev container
sub-agents:

```console
optimus-ide-collab show <workspace>
```
...
If `optimus-ide-collab ssh <workspace>.<agent>` fails:

1. Verify the agent name using `optimus-ide-collab show <workspace>`.
...
1. Check the workspace agent logs for container-related errors:
   ...
   ```console
   grep -i container /tmp/optimus-ide-collab-agent.log
   ```

## VS Code connection issues

VS Code connects to dev containers through the Optimus-IDE-Collab extension. The extension
...
1. Ensure you have the latest Optimus-IDE-Collab VS Code extension.
1. Verify the dev container is running in the Optimus-IDE-Collab dashboard.
...
1. Collect logs from `/tmp/optimus-ide-collab-agent.log` (both workspace and container).
1. Note the exact error messages.
1. Check [Optimus-IDE-Collab GitHub issues](https://github.com/optimus-ide-collab/optimus-ide-collab/issues) for
   similar problems.
1. Contact your Optimus-IDE-Collab administrator for template-specific issues.
