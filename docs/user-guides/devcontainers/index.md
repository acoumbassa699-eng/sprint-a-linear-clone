...
as code using a `devcontainer.json` file. Optimus-IDE-Collab's Dev Containers integration
...
- Optimus-IDE-Collab version 2.24.0 or later
...
Optimus-IDE-Collab automatically discovers dev container configurations in your repositories
and displays them in your workspace dashboard. From there, you can start a dev
container with a single click.

![Discovered dev containers with Start buttons](../../images/user-guides/devcontainers/devcontainer-discovery.png)_Optimus-IDE-Collab detects dev container configurations and displays them with a Start button_

If your template administrator has configured automatic startup (via the
`optimus-ide-collab_devcontainer` Terraform resource or autostart settings), your dev
...
- **Web terminal** in the Optimus-IDE-Collab dashboard
- **SSH** using `optimus-ide-collab ssh <workspace>.<agent>`
...
containers within your Optimus-IDE-Collab workspace.

When a workspace with Dev Containers integration starts:

1. If the template defines `optimus-ide-collab_app`, `optimus-ide-collab_script`, or `optimus-ide-collab_env` resources
...
1. Detected dev containers appear in the Optimus-IDE-Collab dashboard.
1. If auto-start is configured (via `optimus-ide-collab_devcontainer` or autostart settings),
   the integration builds and starts the dev container automatically.
1. Optimus-IDE-Collab creates a sub-agent (or updates the pre-created one) for the running
...
path. For example, a dev container with workspace folder `/home/optimus-ide-collab/my-app`
...
- The `forwardPorts` property in `devcontainer.json` with `host:port` syntax
  ...
  supported. For single-container dev containers, use `optimus-ide-collab port-forward` to
