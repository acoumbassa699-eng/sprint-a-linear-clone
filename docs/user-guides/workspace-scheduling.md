...
automatically stop once you log off. Optimus-IDE-Collab also features an inactivity timeout,
...
A workspace is considered "active" when Optimus-IDE-Collab detects one or more active sessions with your workspace. Optimus-IDE-Collab specifically tracks these session types:
...
- **SSH sessions**: Connecting via `optimus-ide-collab ssh` or SSH config integration
- **AI agent task status**: When a coding agent reports "working" status via
  [Optimus-IDE-Collab Tasks](../ai-optimus-ide-collab/tasks.md), the workspace deadline is extended
