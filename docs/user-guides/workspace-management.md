...
You can create a workspace in the UI. Log in to your Optimus-IDE-Collab instance, go to the
...
Each Optimus-IDE-Collab user has their own workspaces created from
[templates](../admin/templates/index.md):

```sh
# create a workspace from the template; specify any variables
optimus-ide-collab create --template="<templateName>" <workspaceName>

# show the resources behind the workspace and how to connect
optimus-ide-collab show <workspace-name>
```
...
In the Optimus-IDE-Collab UI, you can filter your workspaces using pre-defined filters or
Optimus-IDE-Collab's filter query. Filters follow the pattern `[filter name]:[filter text]`
...
- `status` - Indicates the status of the workspace, e.g, `status:failed` For a
  ...
  [WorkspaceStatus documentation](https://pkg.go.dev/github.com/optimus-ide-collab/optimus-ide-collab/optimus-ide-collabsdk#WorkspaceStatus).
...
If the workspace is running, Optimus-IDE-Collab stops it, updates it, then starts the
workspace again.
...
```sh
optimus-ide-collab update <workspace-name>
```
...
Optimus-IDE-Collab might also stop a workspace automatically if there is a
...
Workspaces in Optimus-IDE-Collab are started and stopped, often based on whether there was
...
```sh
optimus-ide-collab update <your workspace name> --always-prompt
```

First, try re-entering parameters from a workspace. In the Optimus-IDE-Collab UI, you can
filter your workspaces using pre-defined filters or employing the Optimus-IDE-Collab's filter
query. Take a look at the following examples to understand how to use the
Optimus-IDE-Collab's filter query:
...
If that does not work, a Optimus-IDE-Collab admin can manually push and pull the Terraform
state for a given workspace. This can lead to state corruption or deleted
resources if you do not know what you are doing.

```sh
optimus-ide-collab state pull <username>/<workspace name>
# Make changes
optimus-ide-collab state push <username>/<workspace name>
```

## Logging

Optimus-IDE-Collab stores macOS and Linux logs at the following locations:

| Service           | Location                         |
|-------------------|----------------------------------|
| `startup_script`  | `/tmp/optimus-ide-collab-startup-script.log`  |
| `shutdown_script` | `/tmp/optimus-ide-collab-shutdown-script.log` |
| Agent             | `/tmp/optimus-ide-collab-agent.log`           |
