...
Multiple users can securely connect to a single Optimus-IDE-Collab workspace for programming and debugging.

<!-- Insert screenshot of UI here -->

## Features

Workspace sharing is available to all Optimus-IDE-Collab users by default, but platform admins with a Premium subscription can choose to disable sharing within their organizations or for their entire deployment.
...
- AI workflows where an agent prepares a workspace and a developer takes over to review or finalize the work (ex. with [Optimus-IDE-Collab Tasks](https://optimus-ide-collab.com/docs/ai-optimus-ide-collab/tasks).)

## Getting Started

Workspaces can be shared through either the Optimus-IDE-Collab CLI or UI.

Before you begin, ensure that you have a version of Optimus-IDE-Collab with workspace sharing enabled and that your account has permission to share workspaces. This is true by default if you are an OSS user, but deployments with Premium licenses may be restricted by admins.

### CLI

To share a workspace:

- `optimus-ide-collab sharing share <workspace> --user alice`
  - Shares the workspace with a single user, `alice`, with `use` permissions
- `optimus-ide-collab sharing share <workspace> --user alice:admin,bob`
  - Shares the workspace with two users - `alice` with `admin` permissions, and `bob` with `use` permissions
- `optimus-ide-collab sharing share <workspace> --group contractor`
  - Shares the workspace with `contractor`, which is a group of users

To remove sharing from a workspace:

- `optimus-ide-collab sharing remove <workspace> --user alice`
  - Workspace is no longer shared with the user `alice`.
- `optimus-ide-collab sharing remove <workspace> --group contractor`
...
- `optimus-ide-collab sharing status <workspace>`

To list shared workspaces:

- `optimus-ide-collab list --search shared:true`
- `optimus-ide-collab list --search shared_with_user:me`
- `optimus-ide-collab list --search shared_with_user:<user>`
- `optimus-ide-collab list --search shared_with_group:<group>`
...
routing** (e.g., `optimus-ide-collab.example.com/@user/workspace/apps/code-server/`).
Path-based apps share the same origin as the Optimus-IDE-Collab dashboard, so Optimus-IDE-Collab blocks
...
1. Set a
   ...
   on your deployment
   (e.g., `OPTIMUS-IDE-COLLAB_WILDCARD_ACCESS_URL=*.optimus-ide-collab.example.com`).
2. Set `subdomain = true` on the workspace app. For example, if you use the
   [code-server module](https://registry.optimus-ide-collab.com/modules/optimus-ide-collab/code-server):
...
Subdomain-based apps run in an isolated browser security context, so Optimus-IDE-Collab
...
The **Disabled** policy can also be applied to the entire deployment by [setting the `OPTIMUS-IDE-COLLAB_DISABLE_WORKSPACE_SHARING` environment variable, or by using the corresponding command argument or config value](https://optimus-ide-collab.com/docs/reference/cli/server#--disable-workspace-sharing).
