...
User secrets let you store secret values in Optimus-IDE-Collab and make them available in
...
- An enabled flag that controls whether Optimus-IDE-Collab injects the secret into your
  workspaces.
...
Optimus-IDE-Collab stores user secret values in the database. When
[database encryption](../admin/security/database-encryption.md) is enabled,
Optimus-IDE-Collab encrypts secret values at rest. Otherwise, values are stored in plaintext
in the database.

## How your secrets reach a workspace

Optimus-IDE-Collab applies your secrets when your workspace starts. The same applies any
time the workspace agent reconnects to Optimus-IDE-Collab, for example after the workspace
or the agent restarts. To pick up a change to a secret while a workspace is
running, restart the workspace.

Disabling a secret (`optimus-ide-collab secret disable`) stops it from being injected from
...
"Optimus-IDE-Collab never deletes secret files" rule below applies.

### Environment variable secrets

Optimus-IDE-Collab injects environment variable secrets into every new shell, terminal,
...
| Disable the secret (`optimus-ide-collab secret disable`) | After the next workspace start, the variable is no longer injected. Running sessions keep the value until the agent manifest is refetched (workspace restart).                                          |
...
Optimus-IDE-Collab writes file secrets to your workspace filesystem when the workspace
starts, before any startup scripts run. New parent directories are created as
needed. If the file already exists, Optimus-IDE-Collab overwrites the contents and leaves
...
| Disable the secret (`optimus-ide-collab secret disable`) | The file is no longer written at the next workspace start. **The previously-written file stays on disk with its last value.**                                                                         |
| Delete the secret                           | **The previously-written file stays on disk with its last value.**                                                                                                                                    |

> [!IMPORTANT]
> Optimus-IDE-Collab never deletes secret files it has written for you. If you remove a
...
example `~/config` and `/home/optimus-ide-collab/config`), only one of them ends up on
disk; the workspace agent logs a warning to help spot this. Use
distinct paths to avoid the collision.

## Limits

User secrets are subject to the following limits. Optimus-IDE-Collab enforces these when you
...
Only secrets created with `--env` count against the env-injected budget. Optimus-IDE-Collab
injects these into the workspace agent's process environment, which on Windows
has a ~32 KiB total budget. The 24 KiB ceiling leaves room for Optimus-IDE-Collab's own
variables (`OPTIMUS-IDE-COLLAB_*`, `PATH`, `HOME`, ...) plus any template-defined env. To
...
These caps measure stored bytes, which is what Optimus-IDE-Collab writes to the database.
In deployments with
...
You can create, edit, and delete user secrets from the Optimus-IDE-Collab dashboard:
...
Optimus-IDE-Collab injects the secret. A secret with no environment variable or file target
...
Use `optimus-ide-collab secret create <name>` to create a user secret. For sensitive values,
...
variables with the same name, including variables set with `optimus-ide-collab_env`.

```sh
echo -n "$API_KEY" | optimus-ide-collab secret create api-key \
...
```sh
optimus-ide-collab secret create tool-config \
...
```sh
echo -n "$TOKEN" | optimus-ide-collab secret create service-token \
...
```sh
optimus-ide-collab secret create api-key \
  --value "$API_KEY" \
  --description "API key for workspace tools" \
  --env API_KEY
```
...
Stdin is read verbatim. If the source file ends with a trailing newline, Optimus-IDE-Collab
stores that newline as part of the secret value. Use `echo -n` when you do not
want to store a trailing newline:

```sh
echo -n "$API_KEY" | optimus-ide-collab secret create api-key --env API_KEY
```

### Import multiple secrets from a file

Use `optimus-ide-collab secret import <file>` to create a secret for every key in a dotenv,
...
optimus-ide-collab secret import - --input-format yaml < ./secrets.yaml
```
...
[`optimus-ide-collab secret import`](../reference/cli/secret_import.md).
...
it later with `optimus-ide-collab secret enable`.

```sh
echo -n "$API_KEY" | optimus-ide-collab secret create api-key --enabled=false
```

## Update a secret

Use `optimus-ide-collab secret update` to update a secret value, description, environment
variable target, or file target. At least one of `--value`, `--description`,
`--env`, `--file`, or `--enabled` must be specified.

```sh
# Update a secret value.
echo -n "$NEW_API_KEY" | optimus-ide-collab secret update api-key

# Change the environment variable target.
optimus-ide-collab secret update api-key --env NEW_API_KEY
...
optimus-ide-collab secret update api-key --file ""
```
...
optimus-ide-collab secret disable api-key

# Resume injection.
optimus-ide-collab secret enable api-key
```

## List and delete secrets

List, show, and delete your secrets with the `optimus-ide-collab secret` CLI:
...
secret value. The `optimus-ide-collab secret list` table includes an `enabled` column so you
...
1. Drop or select a `.env`, `.json`, `.yaml`, or `.yml` file in the upload
   area. Optimus-IDE-Collab imports the file as soon as you choose it.
...
an existing secret, or exceeds a [limit](#limits), Optimus-IDE-Collab cancels the import
...
For full command details, see [`optimus-ide-collab secret`](../reference/cli/secret.md) and
