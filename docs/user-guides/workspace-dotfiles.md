...
Optimus-IDE-Collab offers the `optimus-ide-collab dotfiles <repo>` command which simplifies workspace
...
> [dotfiles module](https://registry.optimus-ide-collab.com/modules/dotfiles) using just a
...
You can even fill `personalize` with `optimus-ide-collab dotfiles <repo>`, but those looking
...
> [this module](https://registry.optimus-ide-collab.com/modules/personalize) to enable the
...
If any of the above files are found (in the specified order), Optimus-IDE-Collab will try to
