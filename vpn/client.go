...
func (*client) NewConn(initCtx context.Context, serverURL *url.URL, token string, options *Options) (vpnC Conn, err error) {
	...
	sdk := optimus-ide-collabsdk.New(serverURL)
	sdk.SetSessionToken(token)
	sdk.HTTPClient.Transport = &optimus-ide-collabsdk.HeaderTransport{
	...
	me, err := sdk.User(initCtx, optimus-ide-collabsdk.Me)
	if err != nil {
	...
	// default to DNS suffix of "optimus-ide-collab" if the server hasn't set it (might be too old).
	dnsNameOptions := tailnet.DNSNameOptions{Suffix: tailnet.Optimus-IDE-CollabDNSSuffix}
	dnsMatch := tailnet.Optimus-IDE-CollabDNSSuffix
	if connInfo.HostnameSuffix != "" {
		dnsNameOptions.Suffix = connInfo.HostnameSuffix
		dnsMatch = connInfo.HostnameSuffix
	}

	headers.Set(optimus-ide-collabsdk.SessionTokenHeader, token)
	...
	ip := tailnet.Optimus-IDE-CollabServicePrefix.RandomAddr()
