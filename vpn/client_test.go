...
import (
	...
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/vpn"
...
func TestClient_WorkspaceUpdates(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			hostnames: []string{"wrk.optimus-ide-collab.", "agnt.wrk.me.optimus-ide-collab.", "agnt.wrk.rootbeer.optimus-ide-collab."},
		},
		{
			name: "suffix",
			agentConnectionInfo: workspacesdk.AgentConnectionInfo{
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				...
				case "/api/v2/users/me":
					values := r.Header.Values(optimus-ide-collabsdk.SessionTokenHeader)
					assert.Len(t, values, 1, "expected exactly one session token header value")
					httpapi.Write(ctx, w, http.StatusOK, optimus-ide-collabsdk.User{
						ReducedUser: optimus-ide-collabsdk.ReducedUser{
				...
				case "/api/v2/tailnet":
					...
					wsCtx, nc := optimus-ide-collabsdk.WebsocketNetConn(ctx, sws, websocket.MessageBinary)
			...
			for _, name := range tc.hostnames {
				expectedHosts[dnsname.FQDN(name)] = []netip.Addr{tailnet.Optimus-IDE-CollabServicePrefix.AddrFromUUID(agentID)}
			}
