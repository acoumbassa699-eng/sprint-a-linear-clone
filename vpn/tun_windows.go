...
func GetNetworkingStack(t *Tunnel, _ *StartRequest, logger slog.Logger) (NetworkStack, error) {
	...
	if err != nil {
		return NetworkStack{}, xerrors.Errorf("could not load %q, it should be in the same directory as the executable (in Optimus-IDE-Collab Desktop, this should have been installed automatically): %w", wintunDLL, err)
	}
