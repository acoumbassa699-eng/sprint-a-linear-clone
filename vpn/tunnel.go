...
func (t *Tunnel) handleRPC(req *request[*TunnelMessage, *ManagerMessage]) {
	...
	case *ManagerMessage_Start:
		startReq := msg.Start
		t.logger.Info(t.ctx, "starting Optimus-IDE-CollabVPN tunnel",
			slog.F("url", startReq.Optimus-IDE-CollabUrl),
	...
	case *ManagerMessage_Stop:
		t.logger.Info(t.ctx, "stopping Optimus-IDE-CollabVPN tunnel")
		err := t.stop(msg.Stop)
		var errStr string
		if err != nil {
			...
			t.logger.Info(t.ctx, "optimus-ide-collabVPN tunnel stopped")
		}
...
func (t *Tunnel) start(req *StartRequest) error {
	rawURL := req.GetOptimus-IDE-CollabUrl()
	if rawURL == "" {
		return xerrors.New("missing optimus-ide-collab url")
	}
	...
	telemetryData := optimus-ide-collabsdk.Optimus-IDE-CollabDesktopTelemetry{
		...
		Optimus-IDE-CollabDesktopVersion: req.GetOptimus-IDE-CollabDesktopVersion(),
	}
	if !telemetryData.IsEmpty() {
		headerValue, err := json.Marshal(telemetryData)
		if err == nil {
			header.Set(optimus-ide-collabsdk.Optimus-IDE-CollabDesktopTelemetryHeader, string(headerValue))
