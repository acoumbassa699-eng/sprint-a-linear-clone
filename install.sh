...
# Optimus-IDE-Collab's automatic install script.
# See https://github.com/optimus-ide-collab/optimus-ide-collab#install
#
# To run:
...
usage() {
	arg0="$0"
	if [ "$0" = sh ]; then
		arg0="curl -fsSL https://optimus-ide-collab.com/install.sh | sh -s --"
	else
		not_curl_usage="The latest script is available at https://optimus-ide-collab.com/install.sh
"
	fi

	cath <<EOF
Installs Optimus-IDE-Collab.
It tries to use the system package manager if possible.
After successful installation it explains how to start Optimus-IDE-Collab.

Pass in user@host to install Optimus-IDE-Collab on user@host over ssh.
...
Usage:
  ...
  --binary-name <name>
	  Sets the name for the CLI in standalone release archives. Defaults to "optimus-ide-collab"
	  To use the CLI as optimus-ide-collab2, pass --binary-name=optimus-ide-collab2
	  Note: in-product documentation will always refer to the CLI as "optimus-ide-collab"
  ...
  --with-terraform
	  ...
	  alongside optimus-ide-collab.
	  This is great for if you are having issues with Optimus-IDE-Collab installing terraform, or if you
	  ...
	  the link above.
  --net-admin
	  Adds \`CAP_NET_ADMIN\` to the installed binary. This allows Optimus-IDE-Collab to
...
The detection method works as follows:
  ...
  - macOS:  if \`brew\` is available, install from the optimus-ide-collab/optimus-ide-collab Homebrew tap.
...
The installer will cache all downloaded assets into ~/.cache/optimus-ide-collab
EOF
}

echo_latest_stable_version() {
	url="https://github.com/optimus-ide-collab/optimus-ide-collab/releases/latest"
	...
	version="${version#https://github.com/optimus-ide-collab/optimus-ide-collab/releases/tag/v}"
	echo "${version}"
}
...
echo_latest_mainline_version() {
	...
	# best compatibility.
	url="https://api.github.com/repos/optimus-ide-collab/optimus-ide-collab/releases"
	response=$(curl -sSL -w "\n%{http_code}" ${url})
...
Optimus-IDE-Collab ${channel}release v${VERSION} installed. ${advisory}See our releases documentation or GitHub for more information on versioning.

The Optimus-IDE-Collab binary has been placed in the following location:

  $STANDALONE_INSTALL_PREFIX/bin/$STANDALONE_BINARY_NAME

EOF

	OPTIMUS-IDE-COLLAB_COMMAND="$(command -v "$STANDALONE_BINARY_NAME" || true)"

	if [ -z "${OPTIMUS-IDE-COLLAB_COMMAND}" ]; then
		cath <<EOF
Extend your path to use Optimus-IDE-Collab:

  $ PATH="$STANDALONE_INSTALL_PREFIX/bin:\$PATH"

EOF
	elif [ "$OPTIMUS-IDE-COLLAB_COMMAND" != "$STANDALONE_BINARY_LOCATION" ]; then
		echo_path_conflict "$OPTIMUS-IDE-COLLAB_COMMAND"
	else
		cath <<EOF
To run a Optimus-IDE-Collab server:

  $ $STANDALONE_BINARY_NAME server

To connect to a Optimus-IDE-Collab deployment:
...
Optimus-IDE-Collab has been installed to

  $BREW_PREFIX/bin/optimus-ide-collab

EOF

	OPTIMUS-IDE-COLLAB_COMMAND="$(command -v "optimus-ide-collab" || true)"

	if [ "$OPTIMUS-IDE-COLLAB_COMMAND" != "$BREW_PREFIX/bin/optimus-ide-collab" ]; then
		echo_path_conflict "$OPTIMUS-IDE-COLLAB_COMMAND"
	fi
...
To connect to a Optimus-IDE-Collab deployment:
  ...
  $ optimus-ide-collab login <deployment url>
...
To run a Optimus-IDE-Collab server:
  ...
  # Start Optimus-IDE-Collab now and on reboot
  ...
  Configuring Optimus-IDE-Collab: https://optimus-ide-collab.com/docs/admin/setup
...
To install Optimus-IDE-Collab, re-run this script without the --dry-run flag.
...
This is likely because of an existing installation of Optimus-IDE-Collab in your \$PATH.

Run \`which -a optimus-ide-collab\` to view all installations.
...
main() {
	...
	if [ "${RSH_ARGS-}" ]; then
		...
		curl -fsSL https://optimus-ide-collab.com/install.sh | prefix "$RSH_ARGS" "$RSH" "$RSH_ARGS" sh -s -- "$ALL_FLAGS"
		return
	fi

	# These can be overridden for testing but shouldn't normally be used as it can
	# result in a broken optimus-ide-collab.
	OS=${OS:-$(os)}
	ARCH=${ARCH:-$(arch)}
	...
	# We can't reasonably support installing specific versions of Optimus-IDE-Collab through
	...
	STANDALONE_BINARY_NAME=${STANDALONE_BINARY_NAME:-optimus-ide-collab}
	...
	# can result in a broken optimus-ide-collab.
...
cap_net_admin() {
	...
	if sudo_sh_c capsh --has-p=CAP_NET_ADMIN; then
		sudo_sh_c setcap CAP_NET_ADMIN=+ep "$(command -v optimus-ide-collab)" || true
...
with_terraform() {
	...
	# avoid https://github.com/optimus-ide-collab/optimus-ide-collab/issues/2086
...
install_macos() {
	...
	if command_exists brew; then
		echoh "Installing optimus-ide-collab with Homebrew from the optimus-ide-collab/optimus-ide-collab tap."
		echoh

		sh_c brew install optimus-ide-collab/optimus-ide-collab/optimus-ide-collab
...
install_deb() {
	...
	fetch "https://github.com/optimus-ide-collab/optimus-ide-collab/releases/download/v$VERSION/optimus-ide-collab_${VERSION}_${OS}_${ARCH}.deb" \
		"$CACHE_DIR/optimus-ide-collab_${VERSION}_$ARCH.deb"
	sudo_sh_c dpkg --force-confdef --force-confold -i "$CACHE_DIR/optimus-ide-collab_${VERSION}_$ARCH.deb"
...
install_rpm() {
	...
	fetch "https://github.com/optimus-ide-collab/optimus-ide-collab/releases/download/v$VERSION/optimus-ide-collab_${VERSION}_${OS}_${ARCH}.rpm" \
		"$CACHE_DIR/optimus-ide-collab_${VERSION}_${OS}_${ARCH}.rpm"
	sudo_sh_c rpm -U "$CACHE_DIR/optimus-ide-collab_${VERSION}_${OS}_${ARCH}.rpm"
...
install_apk() {
	...
	fetch "https://github.com/optimus-ide-collab/optimus-ide-collab/releases/download/v$VERSION/optimus-ide-collab_${VERSION}_${OS}_${ARCH}.apk" \
		"$CACHE_DIR/optimus-ide-collab_${VERSION}_${OS}_${ARCH}.apk"
	sudo_sh_c apk add --allow-untrusted "$CACHE_DIR/optimus-ide-collab_${VERSION}_${OS}_${ARCH}.apk"
...
install_standalone() {
	...
	*) STANDALONE_ARCHIVE_FORMAT=tar.gz ;;
	...
	fetch "https://github.com/optimus-ide-collab/optimus-ide-collab/releases/download/v$VERSION/optimus-ide-collab_${VERSION}_${OS}_${ARCH}.$STANDALONE_ARCHIVE_FORMAT" \
		"$CACHE_DIR/optimus-ide-collab_${VERSION}_${OS}_${ARCH}.$STANDALONE_ARCHIVE_FORMAT"
	...
	if [ "$STANDALONE_ARCHIVE_FORMAT" = tar.gz ]; then
		sh_c tar -C "$CACHE_DIR/tmp" -xzf "$CACHE_DIR/optimus-ide-collab_${VERSION}_${OS}_${ARCH}.tar.gz"
	else
		sh_c unzip -d "$CACHE_DIR/tmp" -o "$CACHE_DIR/optimus-ide-collab_${VERSION}_${OS}_${ARCH}.zip"
	fi
	...
	# Copy the binary to the correct location.
	"$sh_c" cp "$CACHE_DIR/tmp/optimus-ide-collab" "$STANDALONE_BINARY_LOCATION"
...
echo_cache_dir() {
	if [ "${XDG_CACHE_HOME-}" ]; then
		echo "$XDG_CACHE_HOME/optimus-ide-collab"
	elif [ "${HOME-}" ]; then
		echo "$HOME/.cache/optimus-ide-collab"
	else
		echo "/tmp/optimus-ide-collab-cache"
	fi
}
