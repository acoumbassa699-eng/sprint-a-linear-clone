# This is the Optimus-IDE-Collab Makefile. The build directory for most tasks is `build/`.
#
...
# - build/optimus-ide-collab(-slim)?_${os}_${arch}(.exe)?: build a single fat binary
# - build/optimus-ide-collab_${os}_${arch}.(zip|tar.gz): build a release archive
# - build/optimus-ide-collab_linux_${arch}.(apk|deb|rpm): build a release Linux package
# - build/optimus-ide-collab_${version}_linux_${arch}.tag: build a release Linux Docker image
# - build/optimus-ide-collab_helm.tgz: build a release Helm chart
...
.PRECIOUS: \
	optimusd/database/dump.sql \
	optimusd/database/querier.go \
	optimusd/database/unique_constraint.go \
	optimusd/database/dbmetrics/querymetrics.go \
	optimusd/database/dbauthz/dbauthz.go \
	optimusd/database/dbmock/dbmock.go \
	optimusd/database/pubsub/psmock/psmock.go \
	agent/agentcontainers/acmock/acmock.go \
	optimusd/httpmw/loggermw/loggermock/loggermock.go \
	optimus-sdk/workspacesdk/agentconnmock/agentconnmock.go \
	tailnet/tailnettest/coordinatormock.go \
	tailnet/tailnettest/coordinateemock.go \
	tailnet/tailnettest/workspaceupdatesprovidermock.go \
	tailnet/tailnettest/subscriptionmock.go \
	optimusd/aibridged/aibridgedmock/clientmock.go \
	optimusd/aibridged/aibridgedmock/poolmock.go \
	...
	vpn/vpn.pb.go \
	optimusd/aibridged/proto/aibridged.pb.go \
	...
	optimusd/apidoc/swagger.json \
	optimusd/rbac/object_gen.go \
	optimusd/rbac/scopes_constants_gen.go \
	optimus-sdk/rbacresources_gen.go \
	optimus-sdk/apikey_scopes_gen.go
...
RBAC_GO_FILES := \
	$(wildcard optimusd/rbac/*.go) \
	$(wildcard optimusd/rbac/policy/*.go)

DBDUMP_INPUTS := \
	$(wildcard optimusd/database/migrations/*.go) \
	$(wildcard optimusd/database/migrations/*.sql)
...
TYPEGEN_RBAC_GO_FILES := \
	$(filter-out optimusd/rbac/%_gen.go,$(wildcard optimusd/rbac/*.go)) \
	$(wildcard optimusd/rbac/policy/*.go)
...
TYPEGEN_INPUTS := \
	...
	$(TYPEGEN_RBAC_GO_FILES) \
	$(wildcard optimusd/util/strings/*.go) \
	optimus-sdk/countries.go
...
_gen/bin/apitypings: $(wildcard scripts/apitypings/*.go) $(wildcard optimus-sdk/*.go) | _gen
	@mkdir -p _gen/bin
...
_gen/bin/dbdump: $(wildcard optimusd/database/gen/dump/*.go) $(DBDUMP_INPUTS) | _gen
	@mkdir -p _gen/bin
	go build -o $@ ./optimusd/database/gen/dump
...
_gen/bin/modeloptionsgen: $(wildcard scripts/modeloptionsgen/*.go) $(wildcard optimus-sdk/*.go) | _gen
	@mkdir -p _gen/bin
...
POSTGRES_VERSION ?= 17
POSTGRES_IMAGE   ?= us-docker.pkg.dev/optimus-ide-collab-v2-images-public/public/postgres:$(POSTGRES_VERSION)
...
ifeq ($(OPTIMUS-IDE-COLLAB_RELEASE),true)
ZSTDFLAGS := -22 --ultra
else
ZSTDFLAGS := -6 -T0
endif
...
FIND_EXCLUSIONS= \
	-not \( \( -path '*/.git/*' -o -path './build/*' -o -path './vendor/*' -o -path './.optimus-ide-collabv2/*' -o -path '*/node_modules/*' -o -path '*/out/*' -o -path './optimusd/apidoc/*' -o -path '*/.next/*' -o -path '*/.terraform/*' -o -path './_gen/*' \) -prune \)
...
MIGRATION_FILES := $(shell find ./optimusd/database/migrations/ -maxdepth 1 $(FIND_EXCLUSIONS) -type f -name '*.sql')
FIXTURE_FILES := $(shell find ./optimusd/database/migrations/testdata/fixtures/ $(FIND_EXCLUSIONS) -type f -name '*.sql')
...
OPTIMUS-IDE-COLLAB_SLIM_BINARIES      := $(addprefix build/optimus-ide-collab-slim_$(VERSION)_,$(OS_ARCHES))
OPTIMUS-IDE-COLLAB_FAT_BINARIES       := $(addprefix build/optimus-ide-collab_$(VERSION)_,$(OS_ARCHES))
OPTIMUS-IDE-COLLAB_ALL_BINARIES       := $(OPTIMUS-IDE-COLLAB_SLIM_BINARIES) $(OPTIMUS-IDE-COLLAB_FAT_BINARIES)
OPTIMUS-IDE-COLLAB_TAR_GZ_ARCHIVES    := $(foreach os_arch, $(ARCHIVE_TAR_GZ), build/optimus-ide-collab_$(VERSION)_$(os_arch).tar.gz)
OPTIMUS-IDE-COLLAB_ZIP_ARCHIVES       := $(foreach os_arch, $(ARCHIVE_ZIP), build/optimus-ide-collab_$(VERSION)_$(os_arch).zip)
OPTIMUS-IDE-COLLAB_ALL_ARCHIVES       := $(OPTIMUS-IDE-COLLAB_TAR_GZ_ARCHIVES) $(OPTIMUS-IDE-COLLAB_ZIP_ARCHIVES)
OPTIMUS-IDE-COLLAB_ALL_PACKAGES       := $(foreach os_arch, $(PACKAGE_OS_ARCHES), $(addprefix build/optimus-ide-collab_$(VERSION)_$(os_arch).,$(PACKAGE_FORMATS)))
OPTIMUS-IDE-COLLAB_ARCH_IMAGES        := $(foreach arch, $(DOCKER_ARCHES), build/optimus-ide-collab_$(VERSION)_linux_$(arch).tag)
OPTIMUS-IDE-COLLAB_ARCH_IMAGES_PUSHED := $(addprefix push/, $(OPTIMUS-IDE-COLLAB_ARCH_IMAGES))
OPTIMUS-IDE-COLLAB_MAIN_IMAGE         := build/optimus-ide-collab_$(VERSION)_linux.tag

OPTIMUS-IDE-COLLAB_SLIM_NOVERSION_BINARIES     := $(addprefix build/optimus-ide-collab-slim_,$(OS_ARCHES))
OPTIMUS-IDE-COLLAB_FAT_NOVERSION_BINARIES      := $(addprefix build/optimus-ide-collab_,$(OS_ARCHES))
OPTIMUS-IDE-COLLAB_ALL_NOVERSION_IMAGES        := $(foreach arch, $(DOCKER_ARCHES), build/optimus-ide-collab_linux_$(arch).tag) build/optimus-ide-collab_linux.tag
OPTIMUS-IDE-COLLAB_ALL_NOVERSION_IMAGES_PUSHED := $(addprefix push/, $(OPTIMUS-IDE-COLLAB_ALL_NOVERSION_IMAGES))
...
OPTIMUS-IDE-COLLAB_ARCH_IMAGE_PREREQUISITES :=
else
OPTIMUS-IDE-COLLAB_ARCH_IMAGE_PREREQUISITES := \
	build/optimus-ide-collab_$(VERSION)_%.apk \
	build/optimus-ide-collab_$(VERSION)_%.deb \
	build/optimus-ide-collab_$(VERSION)_%.rpm \
	build/optimus-ide-collab_$(VERSION)_%.tar.gz
endif
...
build-slim: $(OPTIMUS-IDE-COLLAB_SLIM_BINARIES)
.PHONY: build-slim

build-fat build-full build: $(OPTIMUS-IDE-COLLAB_FAT_BINARIES)
.PHONY: build-fat build-full build

release: $(OPTIMUS-IDE-COLLAB_FAT_BINARIES) $(OPTIMUS-IDE-COLLAB_ALL_ARCHIVES) $(OPTIMUS-IDE-COLLAB_ALL_PACKAGES) $(OPTIMUS-IDE-COLLAB_ARCH_IMAGES) build/optimus-ide-collab_helm_$(VERSION).tgz
.PHONY: release

build/optimus-ide-collab-slim_$(VERSION)_checksums.sha1: site/out/bin/optimus-ide-collab.sha1
	cp "$<" "$@"

site/out/bin/optimus-ide-collab.sha1: $(OPTIMUS-IDE-COLLAB_SLIM_BINARIES)
	pushd ./site/out/bin
		openssl dgst -r -sha1 optimus-ide-collab-* | tee optimus-ide-collab.sha1
	popd

build/optimus-ide-collab-slim_$(VERSION).tar: build/optimus-ide-collab-slim_$(VERSION)_checksums.sha1 $(OPTIMUS-IDE-COLLAB_SLIM_BINARIES)
	pushd ./site/out/bin
		tar cf "../../../build/$(@F)" optimus-ide-collab-*
	popd

	# delete the uncompressed binaries from the embedded dir
	rm -f site/out/bin/optimus-ide-collab-*

site/out/bin/optimus-ide-collab.tar.zst: build/optimus-ide-collab-slim_$(VERSION).tar.zst
	cp "$<" "$@"

build/optimus-ide-collab-slim_$(VERSION).tar.zst: build/optimus-ide-collab-slim_$(VERSION).tar
	zstd $(ZSTDFLAGS) \
		--force \
		--long \
		--no-progress \
		-o "build/optimus-ide-collab-slim_$(VERSION).tar.zst" \
		"build/optimus-ide-collab-slim_$(VERSION).tar"

# Redirect from version-less targets to the versioned ones. There is a similar
# target for slim binaries below.
#
# Called like this:
#   make build/optimus-ide-collab_linux_amd64
#   make build/optimus-ide-collab_windows_amd64.exe
$(OPTIMUS-IDE-COLLAB_FAT_NOVERSION_BINARIES): build/optimus-ide-collab_%: build/optimus-ide-collab_$(VERSION)_%
	rm -f "$@"
	ln "$<" "$@"
...
#
# Called like this:
#   make build/optimus-ide-collab-slim_linux_amd64
#   make build/optimus-ide-collab-slim_windows_amd64.exe
$(OPTIMUS-IDE-COLLAB_SLIM_NOVERSION_BINARIES): build/optimus-ide-collab-slim_%: build/optimus-ide-collab-slim_$(VERSION)_%
	rm -f "$@"
	ln "$<" "$@"

# "fat" binaries always depend on the site and the compressed slim binaries.
$(OPTIMUS-IDE-COLLAB_FAT_BINARIES): \
	site/out/index.html \
	site/out/bin/optimus-ide-collab.sha1 \
	site/out/bin/optimus-ide-collab.tar.zst
...
define get-mode-os-arch-ext =
	mode="$$([[ "$@" = build/optimus-ide-collab-slim* ]] && echo "slim" || echo "fat")"
	os="$$(echo $@ | cut -d_ -f3)"
...
# this format:
#     build/optimus-ide-collab(-slim)?_${version}_${os}_${arch}(.exe)?
#
# You should probably use the non-version targets above instead if you're
# calling this manually.
$(OPTIMUS-IDE-COLLAB_ALL_BINARIES): go.mod go.sum \
	$(GO_SRC_FILES) \
	...
	if [[ "$$mode" == "slim" ]]; then
		...
		cp "$@" "./site/out/bin/optimus-ide-collab-$$os-$$arch$$dot_ext"

		if [[ "$${OPTIMUS-IDE-COLLAB_SIGN_GPG:-0}" == "1" ]]; then
			cp "$@.asc" "./site/out/bin/optimus-ide-collab-$$os-$$arch$$dot_ext.asc"
		fi
	fi

# This task builds all archives. It parses the target name to get the metadata
# for the build, so it must be specified in this format:
#     build/optimus-ide-collab_${version}_${os}_${arch}.${format}
#
...
$(OPTIMUS-IDE-COLLAB_ALL_ARCHIVES): $(OPTIMUS-IDE-COLLAB_FAT_BINARIES)
	$(get-mode-os-arch-ext)
	bin_ext=""
	if [[ "$$os" == "windows" ]]; then
		bin_ext=".exe"
	...
	./scripts/archive.sh \
		...
		--os "$$os" \
		--output "$@" \
		"build/optimus-ide-collab_$(VERSION)_$${os}_$${arch}$${bin_ext}"

# This task builds all packages. It parses the target name to get the metadata
# for the build, so it must be specified in this format:
#     build/optimus-ide-collab_${version}_linux_${arch}.${format}
#
...
OPTIMUS-IDE-COLLAB_PACKAGE_DEPS := $(foreach os_arch, $(PACKAGE_OS_ARCHES), build/optimus-ide-collab_$(VERSION)_$(os_arch) build/optimus-ide-collab_$(VERSION)_$(os_arch).tar.gz)
$(OPTIMUS-IDE-COLLAB_ALL_PACKAGES): $(OPTIMUS-IDE-COLLAB_PACKAGE_DEPS)
	$(get-mode-os-arch-ext)
	...
	./scripts/package.sh \
		...
		--version "$(VERSION)" \
		--output "$@" \
		"build/optimus-ide-collab_$(VERSION)_$${os}_$${arch}"

# This task builds a Windows amd64 installer. Depends on makensis.
build/optimus-ide-collab_$(VERSION)_windows_amd64_installer.exe: build/optimus-ide-collab_$(VERSION)_windows_amd64.exe
...
#
# Called like this:
#   make build/optimus-ide-collab_linux_amd64.tag
$(OPTIMUS-IDE-COLLAB_ALL_NOVERSION_IMAGES): build/optimus-ide-collab_%: build/optimus-ide-collab_$(VERSION)_%
.PHONY: $(OPTIMUS-IDE-COLLAB_ALL_NOVERSION_IMAGES)

# Redirect from version-less push Docker image targets to the versioned ones.
#
# Called like this:
#   make push/build/optimus-ide-collab_linux_amd64.tag
$(OPTIMUS-IDE-COLLAB_ALL_NOVERSION_IMAGES_PUSHED): push/build/optimus-ide-collab_%: push/build/optimus-ide-collab_$(VERSION)_%
.PHONY: $(OPTIMUS-IDE-COLLAB_ALL_NOVERSION_IMAGES_PUSHED)
...
#     build/optimus-ide-collab_${version}_${os}_${arch}.tag
#
...
$(OPTIMUS-IDE-COLLAB_ARCH_IMAGES): build/optimus-ide-collab_$(VERSION)_%.tag: build/optimus-ide-collab_$(VERSION)_% $(OPTIMUS-IDE-COLLAB_ARCH_IMAGE_PREREQUISITES)
	$(get-mode-os-arch-ext)
...
# built AND pushed.
$(OPTIMUS-IDE-COLLAB_MAIN_IMAGE): $(OPTIMUS-IDE-COLLAB_ARCH_IMAGES_PUSHED)
...
# Push a Docker image.
$(OPTIMUS-IDE-COLLAB_ARCH_IMAGES_PUSHED): push/%: %
	image_tag="$$(cat "$<")"
	docker push "$$image_tag"
.PHONY: $(OPTIMUS-IDE-COLLAB_ARCH_IMAGES_PUSHED)

# Push the multi-arch Docker manifest.
push/$(OPTIMUS-IDE-COLLAB_MAIN_IMAGE): $(OPTIMUS-IDE-COLLAB_MAIN_IMAGE)
	image_tag="$$(cat "$<")"
	docker manifest push "$$image_tag"
.PHONY: push/$(OPTIMUS-IDE-COLLAB_MAIN_IMAGE)

# Helm charts that are available
charts = optimus-ide-collab provisioner ai-gateway
...
build/optimus-ide-collab_docs_$(VERSION).tgz: offlinedocs/out/index.html
	tar -czf "$@" -C offlinedocs/out .

install: build/optimus-ide-collab_$(VERSION)_$(GOOS)_$(GOARCH)$(GOOS_BIN_EXT)
	install_dir="$$(go env GOPATH)/bin"
	output_file="$${install_dir}/optimus-ide-collab$(GOOS_BIN_EXT)"
...
lint/check-scopes: optimusd/database/dump.sql | _gen/bin/check-scopes
...
pre-commit:
	start=$$(date +%s)
	logdir=$$(mktemp -d "$${TMPDIR:-/tmp}/optimus-ide-collab-pre-commit.XXXXXX")
	...
	$(MAKE) --no-print-directory -j$(PARALLEL_JOBS) MAKE_TIMED=1 MAKE_LOGDIR=$$logdir \
		lint \
		lint/typos \
		build/optimus-ide-collab-slim_$(GOOS)_$(GOARCH)$(GOOS_BIN_EXT)
	$(check-unstaged)
	$(check-untracked)
	rm -rf $$logdir
...
pre-commit-light:
	start=$$(date +%s)
	logdir=$$(mktemp -d "$${TMPDIR:-/tmp}/optimus-ide-collab-pre-commit-light.XXXXXX")
...
pre-push:
	start=$$(date +%s)
	logdir=$$(mktemp -d "$${TMPDIR:-/tmp}/optimus-ide-collab-pre-push.XXXXXX")
...
optimusd/aibridge/prices/data/prices.json: _gen/bin/aibridgepricesgen _gen/models-dev.json | _gen
	@mkdir -p $(dir $@)
	$(call atomic_write,_gen/bin/aibridgepricesgen -upstream _gen/models-dev.json)
.PHONY: optimusd/aibridge/prices/data/prices.json
...
gen/aibridge-prices: \
	optimusd/aibridge/prices/data/prices.json \
...
gen/golden-files: \
	...
	optimusd/.gen-golden \
	optimusd/notifications/.gen-golden \
	enterprise/cli/testdata/.gen-golden \
	enterprise/tailnet/testdata/.gen-golden \
	helm/optimus-ide-collab/tests/testdata/.gen-golden \
...
# applied.
optimusd/database/dump.sql: optimusd/database/gen/dump/main.go $(DBDUMP_INPUTS) | _gen/bin/dbdump
	_gen/bin/dbdump
	touch "$@"

# Generates Go code for querying the database.
# optimusd/database/queries.sql.go
# optimusd/database/models.go
#
...
optimusd/database/dbmetrics/querymetrics.go \
optimusd/database/dbauthz/dbauthz.go &: \
	optimusd/database/sqlc.yaml \
	optimusd/database/dump.sql \
	$(wildcard optimusd/database/queries/*.sql)
	SKIP_DUMP_SQL=1 ./optimusd/database/generate.sh
	touch optimusd/database/querier.go optimusd/database/unique_constraint.go optimusd/database/dbmetrics/querymetrics.go optimusd/database/dbauthz/dbauthz.go

optimusd/database/dbmock/dbmock.go: optimusd/database/db.go optimusd/database/querier.go
	go generate ./optimusd/database/dbmock/
	touch "$@"

optimusd/database/pubsub/psmock/psmock.go: optimusd/database/pubsub/pubsub.go
	go generate ./optimusd/database/pubsub/psmock
	touch "$@"
...
optimusd/httpmw/loggermw/loggermock/loggermock.go: optimusd/httpmw/loggermw/logger.go
	go generate ./optimusd/httpmw/loggermw/loggermock/
	touch "$@"

optimus-sdk/workspacesdk/agentconnmock/agentconnmock.go: optimus-sdk/workspacesdk/agentconn.go
	go generate ./optimus-sdk/workspacesdk/agentconnmock/
	./scripts/format_go_file.sh "$@"
	touch "$@"

$(AIBRIDGED_MOCKS): optimusd/aibridged/client.go optimusd/aibridged/pool.go
	go generate ./optimusd/aibridged/aibridgedmock/
	touch "$@"
...
optimusd/aibridged/proto/aibridged.pb.go: optimusd/aibridged/proto/aibridged.proto
	./scripts/atomic_protoc.sh \
		--go_out=. \
		--go_opt=paths=source_relative \
		--go-drpc_out=. \
		--go-drpc_opt=paths=source_relative \
		./optimusd/aibridged/proto/aibridged.proto

site/src/api/typesGenerated.ts: site/node_modules/.installed $(wildcard scripts/apitypings/*) \
		$(shell find ./optimus-sdk $(FIND_EXCLUSIONS) -type f -name '*.go') \
		$(wildcard optimusd/healthcheck/health/*.go) \
		$(wildcard optimus-sdk/healthsdk/*.go) | _gen _gen/bin/apitypings
...
optimusd/rbac/object_gen.go: scripts/typegen/rbacobject.gotmpl scripts/typegen/main.go optimusd/rbac/object.go optimusd/rbac/policy/policy.go | _gen _gen/bin/typegen
	$(call atomic_write,_gen/bin/typegen rbac object)
	touch "$@"

# NOTE: depends on object_gen.go because the generator build
# compiles optimusd/rbac which includes it.
optimusd/rbac/scopes_constants_gen.go: scripts/typegen/scopenames.gotmpl scripts/typegen/main.go optimusd/rbac/policy/policy.go \
	optimusd/rbac/object_gen.go | _gen _gen/bin/typegen
...
# the generator build compiles optimusd/rbac which includes both.
optimus-sdk/rbacresources_gen.go: scripts/typegen/optimus-sdk.gotmpl scripts/typegen/main.go optimusd/rbac/object.go optimusd/rbac/policy/policy.go \
	optimusd/rbac/object_gen.go optimusd/rbac/scopes_constants_gen.go | _gen _gen/bin/typegen
	# Write to a temp file to avoid truncating the target, which
	# would break the optimus-sdk package and any parallel build targets.
	$(call atomic_write,_gen/bin/typegen rbac optimus-sdk)
	touch "$@"

# NOTE: depends on object_gen.go and scopes_constants_gen.go because
# the generator build compiles optimusd/rbac which includes both.
optimus-sdk/apikey_scopes_gen.go: scripts/apikeyscopesgen/main.go optimusd/rbac/scopes_catalog.go optimusd/rbac/scopes.go \
	optimusd/rbac/object_gen.go optimusd/rbac/scopes_constants_gen.go | _gen _gen/bin/apikeyscopesgen
...
# NOTE: depends on object_gen.go and scopes_constants_gen.go because
# the generator build compiles optimusd/rbac which includes both.
site/src/api/rbacresourcesGenerated.ts: site/node_modules/.installed scripts/typegen/optimus-sdk.gotmpl scripts/typegen/main.go optimusd/rbac/object.go optimusd/rbac/policy/policy.go \
	optimusd/rbac/object_gen.go optimusd/rbac/scopes_constants_gen.go | _gen _gen/bin/typegen
	$(call atomic_write,_gen/bin/typegen rbac typescript,./scripts/biome_format.sh)

site/src/api/countriesGenerated.ts: site/node_modules/.installed scripts/typegen/countries.tstmpl scripts/typegen/main.go optimus-sdk/countries.go | _gen _gen/bin/typegen
	$(call atomic_write,_gen/bin/typegen countries,./scripts/biome_format.sh)

site/src/api/chatModelOptionsGenerated.json: scripts/modeloptionsgen/main.go optimus-sdk/chats.go | _gen _gen/bin/modeloptionsgen
...
docs/admin/security/audit-logs.md: node_modules/.installed optimusd/database/querier.go scripts/auditdocgen/main.go enterprise/audit/table.go optimusd/rbac/object_gen.go | _gen _gen/bin/auditdocgen
	tmpdir=$$(mktemp -d -p _gen) && tmpfile=$$(realpath "$$tmpdir")/$(notdir $@) && cp "$@" "$$tmpfile" && \
...
optimusd/apidoc/.gen: \
	...
	$(wildcard optimusd/*.go) \
	$(wildcard enterprise/optimusd/*.go) \
	$(wildcard optimus-sdk/*.go) \
	$(wildcard enterprise/wsproxy/wsproxysdk/*.go) \
	$(wildcard optimusd/workspaceconnwatcher/*.go) \
	$(DB_GEN_FILES) \
	optimusd/rbac/object_gen.go \
	.swaggo \
	...
	tmpdir=$$(mktemp -d -p _gen) && swagtmp=$$(mktemp -d -p _gen) && \
		...
		mv "$$swagtmp/docs.go" optimusd/apidoc/docs.go && \
		mv "$$swagtmp/swagger.json" optimusd/apidoc/swagger.json && \
		rm -rf "$$tmpdir" "$$swagtmp"
	touch "$@"

docs/manifest.json: site/node_modules/.installed optimusd/apidoc/.gen docs/reference/cli/index.md | _gen
	tmpdir=$$(mktemp -d -p _gen) && tmpfile=$$(realpath "$$tmpdir")/$(notdir $@) && \
		cp _gen/manifest-staging.json "$$tmpfile" && \
		./scripts/biome_format.sh "$$tmpfile" && \
		mv "$$tmpfile" "$@" && rm -rf "$$tmpdir"

optimusd/apidoc/swagger.json: site/node_modules/.installed optimusd/apidoc/.gen
	touch "$@"
...
clean/golden-files:
	...
	find \
		cli/testdata \
		optimusd/notifications/testdata \
		optimusd/testdata \
		enterprise/cli/testdata \
		enterprise/tailnet/testdata \
		helm/optimus-ide-collab/tests/testdata \
...
helm/optimus-ide-collab/tests/testdata/.gen-golden: $(wildcard helm/optimus-ide-collab/tests/testdata/*.yaml) $(wildcard helm/optimus-ide-collab/tests/testdata/*.golden) $(GO_SRC_FILES) $(wildcard helm/optimus-ide-collab/tests/*_test.go)
	if command -v helm >/dev/null 2>&1; then
		TZ=UTC go test ./helm/optimus-ide-collab/tests -run=TestUpdateGoldenFiles -update
	else
		echo "WARNING: helm not found; skipping helm/optimus-ide-collab golden generation" >&2
	fi
	touch "$@"
...
optimusd/.gen-golden: $(wildcard optimusd/testdata/*/*.golden) $(GO_SRC_FILES) $(wildcard optimusd/*_test.go)
	TZ=UTC go test ./optimusd -run="Test.*Golden$$" -update
	touch "$@"

optimusd/notifications/.gen-golden: $(wildcard optimusd/notifications/testdata/*/*.golden) $(GO_SRC_FILES) $(wildcard optimusd/notifications/*_test.go)
	TZ=UTC go test ./optimusd/notifications -run="Test.*Golden$$" -update
	touch "$@"
...
sqlc-push: sqlc-cloud-is-setup test-postgres-docker
	...
	sqlc push -f optimusd/database/sqlc.yaml && echo "Passed sqlc push"
.PHONY: sqlc-push

sqlc-verify: sqlc-cloud-is-setup test-postgres-docker
	echo "--- sqlc verify"
	SQLC_DATABASE_URL="postgresql://postgres:postgres@localhost:5432/$$(go run scripts/migrate-ci/main.go)" \
	sqlc verify -f optimusd/database/sqlc.yaml && echo "Passed sqlc verify"
.PHONY: sqlc-verify

sqlc-vet: test-postgres-docker
	echo "--- sqlc vet"
	SQLC_DATABASE_URL="postgresql://postgres:postgres@localhost:5432/$$(go run scripts/migrate-ci/main.go)" \
	sqlc vet -f optimusd/database/sqlc.yaml && echo "Passed sqlc vet"
.PHONY: sqlc-vet
...
test-tailnet-integration:
	env \
		OPTIMUS-IDE-COLLAB_TAILNET_TESTS=true \
		OPTIMUS-IDE-COLLAB_MAGICSOCK_DEBUG_LOGGING=true \
...
site/e2e/bin/optimus-ide-collab: go.mod go.sum $(GO_SRC_FILES) site/out/index.html
	go build -o $@ \
		-tags embed,ts_omit_aws,ts_omit_bird,ts_omit_tap,ts_omit_kube \
		./enterprise/cmd/optimus-ide-collab

test-e2e: site/e2e/bin/optimus-ide-collab site/node_modules/.installed site/out/index.html
	cd site/
	pnpm playwright:install
ifdef CI
...
count-test-databases:
	PGPASSWORD=postgres psql -h localhost -U postgres -d optimus-ide-collab_testing -P pager=off -c 'SELECT test_package, count(*) as count from test_databases GROUP BY test_package ORDER BY count DESC'
