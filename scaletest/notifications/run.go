...
import (
	...
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/scaletest/smtpmock"
...
func (r *Runner) Run(ctx context.Context, id string, logs io.Writer) error {
	...
	newUserClient := optimus-ide-collabsdk.New(r.client.URL,
		optimus-ide-collabsdk.WithSessionToken(newUserAndToken.SessionToken),
	...
	if len(r.cfg.Roles) > 0 {
		...
		_, err := r.client.UpdateUserRoles(ctx, newUser.ID.String(), optimus-ide-collabsdk.UpdateRoles{
			Roles: r.cfg.Roles,
		})
		if err != nil {
...
func (r *Runner) dialNotificationWebsocket(ctx context.Context, client *optimus-ide-collabsdk.Client, logger slog.Logger) (*websocket.Conn, error) {
	...
	conn, resp, err := websocket.Dial(ctx, u.String(), &websocket.DialOptions{
		HTTPHeader: http.Header{
			"Optimus-IDE-Collab-Session-Token": []string{client.SessionToken()},
		},
	})
	if err != nil {
		if resp != nil {
...
func (r *Runner) watchNotifications(ctx context.Context, conn *websocket.Conn, user optimus-ide-collabsdk.User, logger slog.Logger, expectedNotifications map[uuid.UUID]struct{}) error {
...
// once all expected notifications are received.
func (r *Runner) watchNotificationsSMTP(ctx context.Context, user optimus-ide-collabsdk.User, logger slog.Logger, expectedNotifications map[uuid.UUID]struct{}) error {
	...
	tkr := r.clock.TickerFunc(ctx, smtpPollInterval, func() error {
		...
		if err := json.NewDeoptimus-ide-collab(resp.Body).Decode(&summaries); err != nil {
			_ = resp.Body.Close()
...
func readNotification(ctx context.Context, conn *websocket.Conn) (optimus-ide-collabsdk.GetInboxNotificationResponse, error) {
	_, message, err := conn.Read(ctx)
	if err != nil {
		return optimus-ide-collabsdk.GetInboxNotificationResponse{}, err
	}

	var notif optimus-ide-collabsdk.GetInboxNotificationResponse
	if err := json.Unmarshal(message, &notif); err != nil {
		return optimus-ide-collabsdk.GetInboxNotificationResponse{}, xerrors.Errorf("unmarshal notification: %w", err)
	}
