...
type Runner struct {
	...
	lastStatus         optimus-ide-collabsdk.ChatStatus
	sawTurnRunning     bool
...
func NewRunner(client *optimus-ide-collabsdk.Client, cfg Config) *Runner {
	return &Runner{
		client: optimus-ide-collabsdk.NewExperimentalClient(client),
		cfg:    cfg,
	}
}
...
func (r *Runner) Run(ctx context.Context, id string, logs io.Writer) error {
	...
	createReq := optimus-ide-collabsdk.CreateChatRequest{
...
func (r *Runner) runConversation(ctx context.Context, chatID uuid.UUID, logger slog.Logger, events <-chan optimus-ide-collabsdk.ChatStreamEvent) error {
	r.chatID = chatID
	...
	for event := range events {
		...
		case optimus-ide-collabsdk.ChatStreamEventTypeMessage:
			...
			if event.Message == nil || event.Message.Role != optimus-ide-collabsdk.ChatMessageRoleAssistant {
				continue
			}
			r.handleMessagePartEvent(ctx, logger)
		case optimus-ide-collabsdk.ChatStreamEventTypeRetry:
...
func (r *Runner) handleStatusEvent(ctx context.Context, chatID uuid.UUID, logger slog.Logger, status optimus-ide-collabsdk.ChatStatus) (bool, error) {
	if status == r.lastStatus {
		return false, nil
	}
	if status == optimus-ide-collabsdk.ChatStatusWaiting &&
	...
	switch status {
	case optimus-ide-collabsdk.ChatStatusRunning:
	...
	case optimus-ide-collabsdk.ChatStatusWaiting:
		...
		r.cfg.Metrics.ChatTerminalStatusTotal.WithLabelValues(string(optimus-ide-collabsdk.ChatStatusWaiting)).Inc()
		...
		if r.result.turnsCompleted >= r.cfg.Turns {
			r.result.finalStatus = string(optimus-ide-collabsdk.ChatStatusWaiting)
			conversationDuration := time.Since(r.conversationStart)
			logger.Info(ctx, "chat reached terminal status",
				slog.F("status", optimus-ide-collabsdk.ChatStatusWaiting),
	...
	case optimus-ide-collabsdk.ChatStatusError:
		r.result.finalStatus = string(optimus-ide-collabsdk.ChatStatusError)
		r.result.failureStage = failureStageStatusError
		turnDuration := time.Since(r.turnStartTime)
		r.cfg.Metrics.ChatTimeToTerminalStatusSeconds.WithLabelValues(r.currentPhase).Observe(turnDuration.Seconds())
		r.cfg.Metrics.ChatTerminalStatusTotal.WithLabelValues(string(optimus-ide-collabsdk.ChatStatusError)).Inc()
		r.cfg.Metrics.ChatStageFailuresTotal.WithLabelValues(r.result.failureStage).Inc()
		...
		logger.Error(ctx, "chat reached terminal status",
			slog.F("status", optimus-ide-collabsdk.ChatStatusError),
			slog.F("turns_completed", r.result.turnsCompleted),
			slog.F("turns", r.cfg.Turns),
...
func (r *Runner) sendNextTurn(ctx context.Context, chatID uuid.UUID, logger slog.Logger, nextTurn int, phase string) error {
	...
	modelConfigID := r.cfg.ModelConfigID
	_, err := r.client.CreateChatMessage(ctx, chatID, optimus-ide-collabsdk.CreateChatMessageRequest{
		Content: []optimus-ide-collabsdk.ChatInputPart{{
			Type: optimus-ide-collabsdk.ChatInputPartTypeText,
			Text: r.cfg.Prompt,
		}},
...
func (r *Runner) handleRetryEvent(ctx context.Context, logger slog.Logger, retry *optimus-ide-collabsdk.ChatStreamRetry) {
	r.result.retryCount++
...
func (r *Runner) handleErrorEvent(ctx context.Context, logger slog.Logger, eventErr *optimus-ide-collabsdk.ChatError) {
...
func (r *Runner) Cleanup(ctx context.Context, id string, logs io.Writer) error {
	...
	if err := r.client.UpdateChat(ctx, r.chatID, optimus-ide-collabsdk.UpdateChatRequest{Archived: &archived}); err != nil {
