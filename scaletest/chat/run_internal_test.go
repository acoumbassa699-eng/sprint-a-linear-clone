...
func TestRunnerRunConversation(t *testing.T) {
	...
	t.Run("OneTurnHappyPath", func(t *testing.T) {
		...
		events := make(chan optimus-ide-collabsdk.ChatStreamEvent, 3)
		events <- statusEvent(chatID, optimus-ide-collabsdk.ChatStatusRunning)
		events <- messagePartEvent(chatID)
		events <- statusEvent(chatID, optimus-ide-collabsdk.ChatStatusWaiting)
		close(events)

		err := runTestConversation(t, runner, chatID, events, noopMarkTurnStartReady)
		require.NoError(t, err)
		result := runner.result
		require.Equal(t, string(optimus-ide-collabsdk.ChatStatusWaiting), result.finalStatus)
	...
	t.Run("DuplicateWaitingDoesNotAdvanceTurn", func(t *testing.T) {
		...
		events := make(chan optimus-ide-collabsdk.ChatStreamEvent, 7)
		events <- statusEvent(chatID, optimus-ide-collabsdk.ChatStatusRunning)
		events <- messagePartEvent(chatID)
		events <- statusEvent(chatID, optimus-ide-collabsdk.ChatStatusWaiting)
		events <- statusEvent(chatID, optimus-ide-collabsdk.ChatStatusWaiting)
	...
	t.Run("FirstTurnGatesFollowUpStorm", func(t *testing.T) {
		...
		events := make(chan optimus-ide-collabsdk.ChatStreamEvent, 4)
		events <- statusEvent(chatID, optimus-ide-collabsdk.ChatStatusRunning)
		events <- messagePartEvent(chatID)
		events <- statusEvent(chatID, optimus-ide-collabsdk.ChatStatusWaiting)
	...
	t.Run("FirstOutputFromAssistantMessageEvent", func(t *testing.T) {
		...
		runner := newTestRunner(t, newRunConfig(t))
		events := make(chan optimus-ide-collabsdk.ChatStreamEvent, 3)
		events <- messageEvent(chatID, optimus-ide-collabsdk.ChatMessageRoleUser)
		events <- messageEvent(chatID, optimus-ide-collabsdk.ChatMessageRoleAssistant)
		events <- statusEvent(chatID, optimus-ide-collabsdk.ChatStatusWaiting)
		close(events)
...
func runTestConversation(t *testing.T, runner *Runner, chatID uuid.UUID, events <-chan optimus-ide-collabsdk.ChatStreamEvent, markTurnStartReady func()) error {
	t.Helper()
...
type fakeChatClient struct {
	createChatFunc        func(context.Context, optimus-ide-collabsdk.CreateChatRequest) (optimus-ide-collabsdk.Chat, error)
	streamChatFunc        func(context.Context, uuid.UUID, *optimus-ide-collabsdk.StreamChatOptions) (<-chan optimus-ide-collabsdk.ChatStreamEvent, io.Closer, error)
	createChatMessageFunc func(context.Context, uuid.UUID, optimus-ide-collabsdk.CreateChatMessageRequest) (optimus-ide-collabsdk.CreateChatMessageResponse, error)
	updateChatFunc        func(context.Context, uuid.UUID, optimus-ide-collabsdk.UpdateChatRequest) error
}
...
func (f *fakeChatClient) CreateChat(ctx context.Context, req optimus-ide-collabsdk.CreateChatRequest) (optimus-ide-collabsdk.Chat, error) {
	if f.createChatFunc == nil {
		return optimus-ide-collabsdk.Chat{}, xerrors.New("unexpected CreateChat call")
	}
	return f.createChatFunc(ctx, req)
}

func (f *fakeChatClient) StreamChat(ctx context.Context, chatID uuid.UUID, opts *optimus-ide-collabsdk.StreamChatOptions) (<-chan optimus-ide-collabsdk.ChatStreamEvent, io.Closer, error) {
...
func (f *fakeChatClient) CreateChatMessage(ctx context.Context, chatID uuid.UUID, req optimus-ide-collabsdk.CreateChatMessageRequest) (optimus-ide-collabsdk.CreateChatMessageResponse, error) {
	if f.createChatMessageFunc == nil {
		return optimus-ide-collabsdk.CreateChatMessageResponse{}, xerrors.New("unexpected CreateChatMessage call")
	}
	return f.createChatMessageFunc(ctx, chatID, req)
}

func (f *fakeChatClient) UpdateChat(ctx context.Context, chatID uuid.UUID, req optimus-ide-collabsdk.UpdateChatRequest) error {
...
func newTestRunnerWithChatArchive(t *testing.T, chatID uuid.UUID, updateErr error) (*Runner, func() bool) {
	...
	client.updateChatFunc = func(ctx context.Context, gotChatID uuid.UUID, req optimus-ide-collabsdk.UpdateChatRequest) error {
		if gotChatID != chatID {
...
func newTestRunnerWithChatMessage(t *testing.T, cfg Config, chatID uuid.UUID, onMessage func()) *Runner {
	...
	client := newFakeChatClient(t)
	client.createChatMessageFunc = func(ctx context.Context, gotChatID uuid.UUID, req optimus-ide-collabsdk.CreateChatMessageRequest) (optimus-ide-collabsdk.CreateChatMessageResponse, error) {
		if gotChatID != chatID {
			return optimus-ide-collabsdk.CreateChatMessageResponse{}, xerrors.Errorf("unexpected chat message ID: %s", gotChatID)
		}
		if err := validatePromptParts(req.Content, cfg.Prompt); err != nil {
			return optimus-ide-collabsdk.CreateChatMessageResponse{}, err
		}
		if req.ModelConfigID == nil || *req.ModelConfigID != cfg.ModelConfigID {
			return optimus-ide-collabsdk.CreateChatMessageResponse{}, xerrors.Errorf("unexpected chat message model config ID: %v", req.ModelConfigID)
		}

		if onMessage != nil {
			onMessage()
		}
		return optimus-ide-collabsdk.CreateChatMessageResponse{Queued: true}, nil
	}
	return &Runner{client: client, cfg: cfg}
}

func validatePromptParts(parts []optimus-ide-collabsdk.ChatInputPart, prompt string) error {
	if len(parts) != 1 || parts[0].Type != optimus-ide-collabsdk.ChatInputPartTypeText || parts[0].Text != prompt {
		return xerrors.Errorf("unexpected chat message content: %#v", parts)
	}
	return nil
}

func statusEvent(chatID uuid.UUID, status optimus-ide-collabsdk.ChatStatus) optimus-ide-collabsdk.ChatStreamEvent {
	return optimus-ide-collabsdk.ChatStreamEvent{
		Type:   optimus-ide-collabsdk.ChatStreamEventTypeStatus,
		ChatID: chatID,
		Status: &optimus-ide-collabsdk.ChatStreamStatus{Status: status},
	}
}

func messagePartEvent(chatID uuid.UUID) optimus-ide-collabsdk.ChatStreamEvent {
	return optimus-ide-collabsdk.ChatStreamEvent{
...
func messageEvent(chatID uuid.UUID, role optimus-ide-collabsdk.ChatMessageRole) optimus-ide-collabsdk.ChatStreamEvent {
	return optimus-ide-collabsdk.ChatStreamEvent{
		Type:    optimus-ide-collabsdk.ChatStreamEventTypeMessage,
		ChatID:  chatID,
		Message: &optimus-ide-collabsdk.ChatMessage{Role: role},
	}
}
