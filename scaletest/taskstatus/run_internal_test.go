...
type fakeClient struct {
	...
	workspaceUpdatesCh            chan optimus-ide-collabsdk.Workspace
	workspaceByOwnerAndNameStatus chan optimus-ide-collabsdk.ProvisionerJobStatus
...
func newFakeClient(t *testing.T) *fakeClient {
	return &fakeClient{
		t:                             t,
		workspaceUpdatesCh:            make(chan optimus-ide-collabsdk.Workspace),
		workspaceByOwnerAndNameStatus: make(chan optimus-ide-collabsdk.ProvisionerJobStatus),
...
func (m *fakeClient) watchWorkspace(ctx context.Context, workspaceID uuid.UUID) (<-chan optimus-ide-collabsdk.Workspace, error) {
...
func workspaceWithJobStatus(status optimus-ide-collabsdk.ProvisionerJobStatus) optimus-ide-collabsdk.Workspace {
...
func (m *fakeClient) CreateUserWorkspace(ctx context.Context, userID string, req optimus-ide-collabsdk.CreateWorkspaceRequest) (optimus-ide-collabsdk.Workspace, error) {
	m.logger.Debug(ctx, "called fake CreateUserWorkspace", slog.F("user_id", userID), slog.F("req", req))
	return workspaceWithJobStatus(optimus-ide-collabsdk.ProvisionerJobPending), nil
}

func (m *fakeClient) WorkspaceByOwnerAndName(ctx context.Context, owner string, name string, params optimus-ide-collabsdk.WorkspaceOptions) (optimus-ide-collabsdk.Workspace, error) {
...
func (m *fakeClient) WorkspaceExternalAgentCredentials(ctx context.Context, workspaceID uuid.UUID, agentName string) (optimus-ide-collabsdk.ExternalAgentCredentials, error) {
	...
	return optimus-ide-collabsdk.ExternalAgentCredentials{
...
func TestRunner_Run(t *testing.T) {
	...
	testutil.RequireSend(ctx, t, fClient.workspaceByOwnerAndNameStatus, optimus-ide-collabsdk.ProvisionerJobSucceeded)
	w.MustWait(ctx)
	...
	for _, mf := range metricFamilies {
		switch mf.GetName() {
		case "optimus-ide-collabd_scaletest_task_status_to_workspace_update_latency_seconds":
			latencyMetricFound = true
			require.Len(t, mf.GetMetric(), 1)
			hist := mf.GetMetric()[0].GetHistogram()
			assert.Equal(t, uint64(4), hist.GetSampleCount())
		case "optimus-ide-collabd_scaletest_missing_status_updates_total":
...
func TestRunner_RunMissedUpdate(t *testing.T) {
	...
	w := mClock.Advance(30 * time.Second)
	testutil.RequireSend(testCtx, t, fClient.workspaceByOwnerAndNameStatus, optimus-ide-collabsdk.ProvisionerJobSucceeded)
	w.MustWait(testCtx)
...
func TestRunner_Run_WithErrors(t *testing.T) {
	...
	for _, mf := range metricFamilies {
		...
		case "optimus-ide-collabd_scaletest_report_task_status_errors_total":
...
func TestRunner_Run_BuildFailed(t *testing.T) {
	...
	buildTickerTrap.MustWait(testCtx).MustRelease(testCtx)
	w := mClock.Advance(30 * time.Second)
	testutil.RequireSend(testCtx, t, fClient.workspaceByOwnerAndNameStatus, optimus-ide-collabsdk.ProvisionerJobFailed)
	w.MustWait(testCtx)
