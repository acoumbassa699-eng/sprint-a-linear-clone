...
// NewRunner creates a new Runner with the provided optimus-ide-collabsdk.Client and configuration.
func NewRunner(optimus-ide-collabClient *optimus-ide-collabsdk.Client, cfg Config) *Runner {
	return &Runner{
		client:      newClient(optimus-ide-collabClient),
		updater:     newAppStatusUpdater(optimus-ide-collabClient),
		cfg:         cfg,
...
func (r *Runner) Run(ctx context.Context, name string, logs io.Writer) error {
	...
	result, err := r.createExternalWorkspace(ctx, optimus-ide-collabsdk.CreateWorkspaceRequest{
...
func (r *Runner) reportTaskStatus(ctx context.Context) error {
	...
	getRandPeriod := func() time.Duration {
		...
		// artificially large instantaneous stress on Optimus-IDE-Collab and the database.
...
func (r *Runner) createExternalWorkspace(ctx context.Context, req optimus-ide-collabsdk.CreateWorkspaceRequest) (createExternalWorkspaceResult, error) {
	// Create the workspace
	workspace, err := r.client.CreateUserWorkspace(ctx, optimus-ide-collabsdk.Me, req)
	if err != nil {
	...
	var finalWorkspace optimus-ide-collabsdk.Workspace
	...
	waiter := r.clock.TickerFunc(ctx, 30*time.Second, func() error {
		...
		workspace, err := r.client.WorkspaceByOwnerAndName(ctx, optimus-ide-collabsdk.Me, workspace.Name, optimus-ide-collabsdk.WorkspaceOptions{})
		if err != nil {
		...
		case optimus-ide-collabsdk.ProvisionerJobPending, optimus-ide-collabsdk.ProvisionerJobRunning, optimus-ide-collabsdk.ProvisionerJobCanceling:
	...
	for _, resource := range finalWorkspace.LatestBuild.Resources {
		if resource.Type != "optimus-ide-collab_external_agent" || len(resource.Agents) == 0 {
			continue
		}
