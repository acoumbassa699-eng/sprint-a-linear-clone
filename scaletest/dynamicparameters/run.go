...
func (r *Runner) Run(ctx context.Context, _ string, logs io.Writer) (retErr error) {
	startTime := time.Now()
	stream, err := r.client.TemplateVersionDynamicParameters(ctx, optimus-ide-collabsdk.Me, r.cfg.TemplateVersion)
	if err != nil {
	...
	case resp, ok := <-respCh:
		...
		if !slices.ContainsFunc(resp.Parameters, func(p optimus-ide-collabsdk.PreviewParameter) bool {
			return p.Name == "zero"
		}) {
...
func checkNoDiagnostics(resp optimus-ide-collabsdk.DynamicParametersResponse) error {
