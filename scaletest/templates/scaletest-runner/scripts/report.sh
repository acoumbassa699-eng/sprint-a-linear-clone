...
# https://github.com/optimus-ide-collab/optimus-ide-collab/issues/9877
OPTIMUS-IDE-COLLAB_URL="${OPTIMUS-IDE-COLLAB_URL%/}"
buildinfo="$(curl -sSL "${OPTIMUS-IDE-COLLAB_URL}/api/v2/buildinfo")"
server_version="$(jq -r '.version' <<<"${buildinfo}")"
server_version_commit="$(jq -r '.external_url' <<<"${buildinfo}")"

# Since `optimus-ide-collab show` doesn't support JSON output, we list the workspaces instead.
# Use `command` here to bypass dry run.
workspace_json="$(
	command optimus-ide-collab list --all --output json |
		jq --arg workspace "${OPTIMUS-IDE-COLLAB_WORKSPACE}" --arg user "${OPTIMUS-IDE-COLLAB_USER}" 'map(select(.name == $workspace) | select(.owner_name == $user)) | .[0]'
)"
...
started)
	...
	header="New scaletest started at \`${created_at}\` by \`${initiator_name}\` on ${OPTIMUS-IDE-COLLAB_URL} (<${server_version_commit}|\`${server_version}\`>)."
	;;
completed)
	completed_at=$(date -Iseconds)
	header="Scaletest completed at \`${completed_at}\` (started by \`${initiator_name}\`) on ${OPTIMUS-IDE-COLLAB_URL} (<${server_version_commit}|\`${server_version}\`>)."
	;;
failed)
	failed_at=$(date -Iseconds)
	header="Scaletest failed at \`${failed_at}\` (started by \`${initiator_name}\`) on ${OPTIMUS-IDE-COLLAB_URL} (<${server_version_commit}|\`${server_version}\`>)."
	;;
*)
	echo "Unknown status: ${status}" >&2
	exit 1
...
text_arr=(
	...
	"${bullet} Workspace (runner): ${OPTIMUS-IDE-COLLAB_URL}/@${owner_name}/${workspace_name}"
