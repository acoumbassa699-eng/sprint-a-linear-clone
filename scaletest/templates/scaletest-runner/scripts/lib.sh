...
# scripts/lib.sh from optimus-ide-collab/optimus-ide-collab below.
if [[ ${SCRIPTS_LIB_IS_SOURCED:-0} == 1 ]]; then
	return 0
fi

# Source scripts/lib.sh from optimus-ide-collab/optimus-ide-collab for common functions.
# shellcheck source=scripts/lib.sh
. "${HOME}/optimus-ide-collab/scripts/lib.sh"
...
SCALETEST_OPTIMUS-IDE-COLLAB_BINARY="/tmp/optimus-ide-collab-full-${SCALETEST_RUN_ID}"

mkdir -p "${SCALETEST_STATE_DIR}" "${SCALETEST_RESULTS_DIR}" "${SCALETEST_LOGS_DIR}" "${SCALETEST_PPROF_DIR}"

optimus-ide-collab() {
	if [[ ! -x "${SCALETEST_OPTIMUS-IDE-COLLAB_BINARY}" ]]; then
		log "Fetching full optimus-ide-collab binary..."
		fetch_optimus-ide-collab_full
	fi
	maybedryrun "${DRY_RUN}" "${SCALETEST_OPTIMUS-IDE-COLLAB_BINARY}" "${@}"
}
...
get_appearance() {
	session_token=$OPTIMUS-IDE-COLLAB_USER_TOKEN
	if [[ -f "${OPTIMUS-IDE-COLLAB_CONFIG_DIR}/session" ]]; then
		session_token="$(<"${OPTIMUS-IDE-COLLAB_CONFIG_DIR}/session")"
	fi
	curl -sSL \
		-H "Optimus-IDE-Collab-Session-Token: ${session_token}" \
		"${OPTIMUS-IDE-COLLAB_URL}/api/v2/appearance"
}
set_appearance() {
...
# fetch_optimus-ide-collab_full fetches the full (non-slim) optimus-ide-collab binary from one of the optimus-ide-collab pods
# running in the same namespace as the current pod.
fetch_optimus-ide-collab_full() {
	if [[ -x "${SCALETEST_OPTIMUS-IDE-COLLAB_BINARY}" ]]; then
		log "Full Optimus-IDE-Collab binary already exists at ${SCALETEST_OPTIMUS-IDE-COLLAB_BINARY}"
		return 0
	fi
	ns=$(namespace)
	if [[ -z "${ns}" ]]; then
	...
	pods=$(optimus-ide-collab_pods)
	if [[ -z ${pods} ]]; then
		log "Could not find optimus-ide-collab pods!"
		return 1
	fi
	pod=$(cut -d ' ' -f 1 <<<"${pods}")
	if [[ -z ${pod} ]]; then
	...
	log "Fetching full Optimus-IDE-Collab binary from ${pod}"
	...
	maybedryrun "${DRY_RUN}" kubectl \
		...
		--retries 10 \
		"${pod}:/opt/optimus-ide-collab" "${SCALETEST_OPTIMUS-IDE-COLLAB_BINARY}"
	maybedryrun "${DRY_RUN}" chmod +x "${SCALETEST_OPTIMUS-IDE-COLLAB_BINARY}"
	log "Full Optimus-IDE-Collab binary downloaded to ${SCALETEST_OPTIMUS-IDE-COLLAB_BINARY}"
}

# set_pod_status_annotation annotates the currently running pod with the key
# com.optimus-ide-collab.scaletest.status. It will overwrite the previous status.
set_pod_status_annotation() {
	if [[ $# -ne 1 ]]; then
		log "BUG: Must specify an annotation value"
		return 1
	else
		maybedryrun "${DRY_RUN}" kubectl --namespace "$(namespace)" annotate pod "$(hostname)" "com.optimus-ide-collab.scaletest.status=$1" --overwrite
	fi
}
