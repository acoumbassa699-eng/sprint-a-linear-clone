...
func TestServer_SendAndReceiveEmail(t *testing.T) {
	...
	err = json.NewDeoptimus-ide-collab(resp.Body).Decode(&summaries)
	require.NoError(t, err)
...
func TestServer_FilterByEmail(t *testing.T) {
	...
	err = sendTestEmail(srv.SMTPAddress(), "admin@optimus-ide-collab.com", "Email for admin", "Body 1")
	require.NoError(t, err)

	err = sendTestEmail(srv.SMTPAddress(), "test-user@optimus-ide-collab.com", "Email for test-user", "Body 2")
	require.NoError(t, err)
	...
	url := fmt.Sprintf("%s/messages?email=admin@optimus-ide-collab.com", srv.APIAddress())
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, url, nil)
	require.NoError(t, err)
...
func TestServer_NotificationTemplateID(t *testing.T) {
	...
	err = sendTestEmail(srv.SMTPAddress(), "test-user@optimus-ide-collab.com", "Notification", body)
	require.NoError(t, err)
...
func TestServer_Purge(t *testing.T) {
	...
	err = sendTestEmail(srv.SMTPAddress(), "test-user@optimus-ide-collab.com", "Test", "Body")
	require.NoError(t, err)
...
func sendTestEmail(smtpAddr, to, subject, body string) error {
	from := "noreply@optimus-ide-collab.com"
