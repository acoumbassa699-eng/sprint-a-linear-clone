...
func Test_Runner(t *testing.T) {
	...
	testParameterValues := []optimus-ide-collabsdk.WorkspaceBuildParameter{
		{
			Name:  "foo",
			Value: "baz",
		},
	...
	t.Run("OK", func(t *testing.T) {
		...
		const (
			...
			email    = "scaletest@test.optimus-ide-collab.com"
		)
	...
	t.Run("CleanupPendingBuild", func(t *testing.T) {
		...
		version = optimus-ide-collabdtest.AwaitTemplateVersionJobCompleted(t, client, version.ID)
		template := optimus-ide-collabdtest.CreateTemplate(t, client, user.OrganizationID, version.ID, func(request *optimus-ide-collabsdk.CreateTemplateRequest) {
		...
		jobCh := make(chan optimus-ide-collabsdk.ProvisionerJob, 1)
		testutil.Eventually(checkJobStartedCtx, t, func(ctx context.Context) bool {
			workspaces, err := client.Workspaces(checkJobStartedCtx, optimus-ide-collabsdk.WorkspaceFilter{})
			if err != nil {
				return false
			}
			...
			if ws.LatestBuild.Transition != optimus-ide-collabsdk.WorkspaceTransitionStart {
				t.Errorf("expected build transition %s, got %s", optimus-ide-collabsdk.WorkspaceTransitionStart, ws.LatestBuild.Transition)
				return false
			}

			if ws.LatestBuild.Job.Status != optimus-ide-collabsdk.ProvisionerJobRunning {
				return false
			}
		...
		testutil.Eventually(cleanupCtx, t, func(ctx context.Context) bool {
			...
			if pj.Status != optimus-ide-collabsdk.ProvisionerJobFailed &&
				pj.Status != optimus-ide-collabsdk.ProvisionerJobCanceling &&
				pj.Status != optimus-ide-collabsdk.ProvisionerJobCanceled {
				return false
			}
...
func goEventuallyStartFakeAgent(ctx context.Context, t *testing.T, client *optimus-ide-collabsdk.Client, agentToken string) chan io.Closer {
	t.Helper()
