...
type WorkspaceDispatcher struct {
	...
	Channels map[string]chan optimus-ide-collabsdk.WorkspaceBuildUpdate
}
...
func NewWorkspaceDispatcher(workspaceNames []string) *WorkspaceDispatcher {
	channels := make(map[string]chan optimus-ide-collabsdk.WorkspaceBuildUpdate, len(workspaceNames))
	for _, name := range workspaceNames {
		channels[name] = make(chan optimus-ide-collabsdk.WorkspaceBuildUpdate, 16)
	}
...
func (d *WorkspaceDispatcher) Start(ctx context.Context, source <-chan optimus-ide-collabsdk.WorkspaceBuildUpdate) {
	go func() {
