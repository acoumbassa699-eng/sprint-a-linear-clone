...
func TestWorkspaceDispatcher(t *testing.T) {
	...
	source := make(chan optimus-ide-collabsdk.WorkspaceBuildUpdate, 10)

	// Start the dispatcher.
	dispatcher.Start(ctx, source)

	// Send updates for each workspace.
	updates := []optimus-ide-collabsdk.WorkspaceBuildUpdate{
		{
...
func TestWorkspaceDispatcher_UnknownWorkspace(t *testing.T) {
	...
	source <- optimus-ide-collabsdk.WorkspaceBuildUpdate{
