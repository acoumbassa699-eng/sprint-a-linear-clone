...
func (r *Runner) RunReturningResult(ctx context.Context, id string, logs io.Writer) (RunResult, error) {
	...
	err = waitForBuild(createWorkspaceCtx, logger, buildUpdates, optimus-ide-collabsdk.WorkspaceTransitionStart)
	if err != nil {
	...
	_, err = newUserClient.CreateWorkspaceBuild(ctx, workspace.ID, optimus-ide-collabsdk.CreateWorkspaceBuildRequest{
	...
	err = waitForBuild(stopBuildCtx, logger, buildUpdates, optimus-ide-collabsdk.WorkspaceTransitionStop)
	if err != nil {
	...
	err = newUserClient.UpdateWorkspaceAutostart(ctx, workspace.ID, optimus-ide-collabsdk.UpdateWorkspaceAutostartRequest{
		Schedule: &schedule,
	})
	if err != nil {
	...
	err = waitForBuild(autostartBuildCtx, logger, buildUpdates, optimus-ide-collabsdk.WorkspaceTransitionStart)
	if err != nil {
...
func waitForBuild(ctx context.Context, logger slog.Logger, updates <-chan optimus-ide-collabsdk.WorkspaceBuildUpdate, transition optimus-ide-collabsdk.WorkspaceTransition) error {
	for {
		select {
		case <-ctx.Done():
			return ctx.Err()
		case update, ok := <-updates:
			...
			switch optimus-ide-collabsdk.ProvisionerJobStatus(update.JobStatus) {
