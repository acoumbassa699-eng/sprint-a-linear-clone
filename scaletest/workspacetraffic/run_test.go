...
func TestRun(t *testing.T) {
	...
	t.Run("RPTY", func(t *testing.T) {
		t.Parallel()
		// We need to stand up an in-memory optimus-ide-collabd and run a fake workspace.
		var (
		...
		resources := optimus-ide-collabdtest.AwaitWorkspaceAgents(t, client, ws.ID)
	...
	t.Run("App", func(t *testing.T) {
		...
		// the roundtrip to optimus-ide-collabd/agent and simply tests the http request conn
		// directly.
		...
		client := optimus-ide-collabsdk.New(&url.URL{})
		runner := workspacetraffic.NewRunner(client, workspacetraffic.Config{
			BytesPerTick: int64(bytesPerTick),
			TickInterval: tickInterval,
			Duration:     testutil.WaitLong,
