...
func NewRunner(client *optimus-ide-collabsdk.Client, cfg Config) *Runner {
	return &Runner{
		client: client,
		cfg:    cfg,
	}
...
func (r *Runner) Run(ctx context.Context, id string, logs io.Writer) error {
	...
	templateReq := optimus-ide-collabsdk.CreateTemplateRequest{
		Name:        templateName,
		Description: "`optimus-ide-collab exp scaletest prebuilds` template",
	...
	if err != nil {
		...
		if xerrors.As(err, &sdkErr) && sdkErr.StatusCode() == http.StatusConflict {
			existing, listErr := r.client.Templates(ctx, optimus-ide-collabsdk.TemplateFilter{
...
func (r *Runner) measureCreation(ctx context.Context, logger slog.Logger) error {
	...
	tkr := r.cfg.Clock.TickerFunc(workspacesCtx, workspacePollInterval, func() error {
		workspaces, err := r.client.Workspaces(workspacesCtx, optimus-ide-collabsdk.WorkspaceFilter{
		...
		for _, ws := range workspaces.Workspaces {
			...
			case optimus-ide-collabsdk.ProvisionerJobRunning:
				runningCount++
			case optimus-ide-collabsdk.ProvisionerJobFailed, optimus-ide-collabsdk.ProvisionerJobCanceled:
				failedCount++
...
func (r *Runner) measureDeletion(ctx context.Context, logger slog.Logger) error {
	...
	initialWorkspaces, err := r.client.Workspaces(deletionCtx, optimus-ide-collabsdk.WorkspaceFilter{
		Template: r.template.Name,
	})
	if err != nil {
	...
	tkr := r.cfg.Clock.TickerFunc(deletionCtx, workspacePollInterval, func() error {
		workspaces, err := r.client.Workspaces(deletionCtx, optimus-ide-collabsdk.WorkspaceFilter{
			Template: r.template.Name,
		})
		if err != nil {
			return xerrors.Errorf("list workspaces: %w", err)
		...
		for _, ws := range workspaces.Workspaces {
			if ws.LatestBuild.Transition != optimus-ide-collabsdk.WorkspaceTransitionDelete {
			...
			switch ws.LatestBuild.Job.Status {
			case optimus-ide-collabsdk.ProvisionerJobRunning, optimus-ide-collabsdk.ProvisionerJobPending:
				runningCount++
			...
			case optimus-ide-collabsdk.ProvisionerJobFailed, optimus-ide-collabsdk.ProvisionerJobCanceled:
				...
				_, retryErr := r.client.CreateWorkspaceBuild(deletionCtx, ws.ID, optimus-ide-collabsdk.CreateWorkspaceBuildRequest{
...
func (r *Runner) createTemplateVersion(ctx context.Context, templateID uuid.UUID, numPresets, numPresetPrebuilds int) (optimus-ide-collabsdk.TemplateVersion, error) {
	tarData, err := TemplateTarData(numPresets, numPresetPrebuilds)
	if err != nil {
		return optimus-ide-collabsdk.TemplateVersion{}, xerrors.Errorf("create prebuilds template tar: %w", err)
	}
	uploadResp, err := r.client.Upload(ctx, optimus-ide-collabsdk.ContentTypeTar, bytes.NewReader(tarData))
	if err != nil {
		return optimus-ide-collabsdk.TemplateVersion{}, xerrors.Errorf("upload prebuilds template tar: %w", err)
	}
	...
	if err != nil {
		return optimus-ide-collabsdk.TemplateVersion{}, xerrors.Errorf("create template version: %w", err)
	}
	if version.MatchedProvisioners != nil && version.MatchedProvisioners.Count == 0 {
		return optimus-ide-collabsdk.TemplateVersion{}, xerrors.Errorf("no provisioners matched for template version")
	}
	...
	err = tkr.Wait()
	if !xerrors.Is(err, errTickerDone) {
		return optimus-ide-collabsdk.TemplateVersion{}, xerrors.Errorf("wait for template version provisioning: %w", err)
	}
	return version, nil
}
...
func (r *Runner) pushEmptyTemplateVersion(ctx context.Context) error {
	...
	if err = r.client.UpdateActiveTemplateVersion(ctx, r.template.ID, optimus-ide-collabsdk.UpdateActiveTemplateVersion{
		ID: emptyVersion.ID,
	}); err != nil {
...
func (r *Runner) Cleanup(ctx context.Context, _ string, logs io.Writer) error {
	...
	for attempt := range maxDeletionAttempts {
		...
		var failed []optimus-ide-collabsdk.Workspace
...
func allWorkspacesForTemplate(ctx context.Context, client *optimus-ide-collabsdk.Client, templateName string) ([]optimus-ide-collabsdk.Workspace, error) {
	const pageSize = 100
