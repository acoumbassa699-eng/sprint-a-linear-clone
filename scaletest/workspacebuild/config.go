...
type Config struct {
	...
	// Request is the request to send to the Optimus-IDE-Collab API to create the workspace.
	// request.template_id must be set. A name will be generated if not
	// specified.
	Request optimus-ide-collabsdk.CreateWorkspaceRequest `json:"request"`
...
func (c Config) Validate() error {
	...
	if c.UserID != optimus-ide-collabsdk.Me {
		_, err := uuid.Parse(c.UserID)
		if err != nil {
			return xerrors.Errorf("user_id must be %q or a valid UUID: %w", optimus-ide-collabsdk.Me, err)
		}
	}
