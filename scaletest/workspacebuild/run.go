	...
	if r.cfg.NoWaitForBuild {
		...
		if err != nil {
			for i := 0; i < r.cfg.Retry; i++ {
				...
				workspace.LatestBuild, err = r.client.CreateWorkspaceBuild(ctx, workspace.ID, optimus-ide-collabsdk.CreateWorkspaceBuildRequest{
...
func NewCleanupRunner(client *optimus-ide-collabsdk.Client, workspaceID uuid.UUID) *CleanupRunner {
...
func (r *CleanupRunner) Run(ctx context.Context, _ string, logs io.Writer) error {
	...
	if err == nil && build.Job.Status.Active() {
		...
		if err = r.client.CancelWorkspaceBuild(ctx, build.ID, optimus-ide-collabsdk.CancelWorkspaceBuildParams{}); err != nil {
	...
	build, err = r.client.CreateWorkspaceBuild(ctx, r.workspaceID, optimus-ide-collabsdk.CreateWorkspaceBuildRequest{
...
func waitForBuild(ctx context.Context, w io.Writer, client *optimus-ide-collabsdk.Client, buildID uuid.UUID) error {
	ctx, span := tracing.StartSpan(ctx)
	defer span.End()
	...
	for {
		...
		if build.Job.Status != optimus-ide-collabsdk.ProvisionerJobPending {
			break
		}
...
func waitForAgents(ctx context.Context, w io.Writer, client *optimus-ide-collabsdk.Client, workspaceID uuid.UUID) error {
	ctx, span := tracing.StartSpan(ctx)
	defer span.End()
