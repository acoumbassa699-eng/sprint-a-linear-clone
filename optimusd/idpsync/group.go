	...
	if err != nil {
		...
		if s.DeploymentSyncSettings.Legacy.GroupField != "" {
			...
			if defaultOrganization.ID == orgID {
				settings = ptr.Ref(GroupSyncSettings(optimus-ide-collabsdk.GroupSyncSettings{
...
type GroupSyncSettings optimus-ide-collabsdk.GroupSyncSettings
...
// the external IDP group name matches the Optimus-IDE-Collab one.
