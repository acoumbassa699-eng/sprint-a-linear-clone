...
func TestGroupSyncTable(t *testing.T) {
	...
	testCases := []orgSetupDefinition{
		{
			Name: "SwitchGroups",
			GroupSettings: &optimus-ide-collabsdk.GroupSyncSettings{
				Field: "groups",
...
func TestApplyGroupDifference(t *testing.T) {
	...
	for _, tc := range testCase {
		t.Run(tc.Name, func(t *testing.T) {
			...
			s := idpsync.NewAGPLSync(slogtest.Make(t, &slogtest.Options{}), mgr, idpsync.FromDeploymentValues(optimus-ide-collabdtest.DeploymentValues(t)))
