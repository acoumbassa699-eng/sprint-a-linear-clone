...
// claims to the internal representation of a user in Optimus-IDE-Collab.
...
type DeploymentSyncSettings struct {
	...
	// to site roles within Optimus-IDE-Collab.
	// map[oidcRoleName][]optimus-ide-collabRoleName
...
func FromDeploymentValues(dv *optimus-ide-collabsdk.DeploymentValues) DeploymentSyncSettings {
	if dv == nil {
...
func (e HTTPError) Write(rw http.ResponseWriter, r *http.Request) {
	...
	httpapi.Write(r.Context(), rw, e.Code, optimus-ide-collabsdk.Response{
		Message: e.Msg,
		Detail:  e.Detail,
	})
}
