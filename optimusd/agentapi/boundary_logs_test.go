...
// to simulate a session created by a prior request or a different optimus-ide-collabd replica.
...
func TestReportBoundaryLogs(t *testing.T) {
	...
	t.Run("SessionAlreadyExistsDifferentInstance", func(t *testing.T) {
		...
		// optimus-ide-collabd replica). A second instance backed by the same database receives
		// logs for the same session ID.
		f := newBoundaryFixture(t)
		api1 := f.api(t)
		api2 := f.api(t) // independent struct, simulates a different optimus-ide-collabd replica
		sessionID := uuid.New()
		now := dbtime.Now()
