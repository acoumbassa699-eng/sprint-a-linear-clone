...
func TestGetAnnouncementBanners(t *testing.T) {
	...
	t.Run("OK", func(t *testing.T) {
		...
		cfg := []optimus-ide-collabsdk.BannerConfig{{
			Enabled:         true,
			Message:         "The beep-bop will be boop-beeped on Saturday at 12AM PST.",
			BackgroundColor: "#00FF00",
		}}

		var ff appearance.Fetcher = fakeFetcher{cfg: optimus-ide-collabsdk.AppearanceConfig{AnnouncementBanners: cfg}}
...
func (f fakeFetcher) Fetch(context.Context) (optimus-ide-collabsdk.AppearanceConfig, error) {
	return f.cfg, f.err
}
