...
type Fetcher interface {
	Fetch(ctx context.Context, feature optimus-ide-collabsdk.CryptoKeyFeature) ([]optimus-ide-collabsdk.CryptoKey, error)
}
...
func (d *DBFetcher) Fetch(ctx context.Context, feature optimus-ide-collabsdk.CryptoKeyFeature) ([]optimus-ide-collabsdk.CryptoKey, error) {
...
type cache struct {
	...
	fetcher Fetcher
	logger  slog.Logger
	feature optimus-ide-collabsdk.CryptoKeyFeature

	mu        sync.Mutex
	keys      map[int32]optimus-ide-collabsdk.CryptoKey
	lastFetch time.Time
...
func NewSigningCache(ctx context.Context, logger slog.Logger, fetcher Fetcher,
	feature optimus-ide-collabsdk.CryptoKeyFeature, opts ...func(*cache),
...
func newCache(ctx context.Context, logger slog.Logger, fetcher Fetcher, feature optimus-ide-collabsdk.CryptoKeyFeature, opts ...func(*cache)) *cache {
	cache := &cache{
...
func isEncryptionKeyFeature(feature optimus-ide-collabsdk.CryptoKeyFeature) bool {
	return feature == optimus-ide-collabsdk.CryptoKeyFeatureWorkspaceAppsAPIKey
}

func isSigningKeyFeature(feature optimus-ide-collabsdk.CryptoKeyFeature) bool {
	switch feature {
	case optimus-ide-collabsdk.CryptoKeyFeatureTailnetResume, optimus-ide-collabsdk.CryptoKeyFeatureOIDCConvert, optimus-ide-collabsdk.CryptoKeyFeatureWorkspaceAppsToken, optimus-ide-collabsdk.CryptoKeyFeatureNATSCA:
		return true
	default:
		return false
	}
...
// deoptimus-ide-collab injected at construction) so new key types can be added without
// editing this function.
func idSecret(k optimus-ide-collabsdk.CryptoKey) (string, interface{}, error) {
	id := strconv.FormatInt(int64(k.Sequence), 10)

	if k.Feature == optimus-ide-collabsdk.CryptoKeyFeatureNATSCA {
...
func (c *cache) cryptoKey(ctx context.Context, sequence int32) (string, interface{}, error) {
	...
	var key optimus-ide-collabsdk.CryptoKey
	var ok bool
...
func (c *cache) key(sequence int32) (optimus-ide-collabsdk.CryptoKey, bool) {
...
func checkKey(key optimus-ide-collabsdk.CryptoKey, sequence int32, now time.Time) (string, interface{}, error) {
	if sequence == latestSequence {
		if !key.CanSign(now) {
...
func (c *cache) cryptoKeys(ctx context.Context) (map[int32]optimus-ide-collabsdk.CryptoKey, error) {
...
func toKeyMap(keys []optimus-ide-collabsdk.CryptoKey, now time.Time) map[int32]optimus-ide-collabsdk.CryptoKey {
	m := make(map[int32]optimus-ide-collabsdk.CryptoKey)
	var latest optimus-ide-collabsdk.CryptoKey
...
func toSDKKeys(keys []database.CryptoKey) []optimus-ide-collabsdk.CryptoKey {
	into := make([]optimus-ide-collabsdk.CryptoKey, 0, len(keys))
	for _, key := range keys {
...
func toSDK(key database.CryptoKey) optimus-ide-collabsdk.CryptoKey {
