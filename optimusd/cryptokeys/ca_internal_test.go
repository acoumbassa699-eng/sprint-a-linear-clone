...
func TestNATSCASigningCache(t *testing.T) {
	...
	t.Run("ActiveAndVerifyingByID", func(t *testing.T) {
		...
		cache, err := NewSigningCache(ctx, testutil.Logger(t), &DBFetcher{DB: db}, optimus-ide-collabsdk.CryptoKeyFeatureNATSCA)
		require.NoError(t, err)
		defer cache.Close()
	...
	t.Run("RefreshesOnRotation", func(t *testing.T) {
		...
		cache, err := NewSigningCache(ctx, testutil.Logger(t), &DBFetcher{DB: db}, optimus-ide-collabsdk.CryptoKeyFeatureNATSCA, WithCacheClock(clock))
		require.NoError(t, err)
		defer cache.Close()
