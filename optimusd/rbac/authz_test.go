...
func BenchmarkCacher(b *testing.B) {
	...
	authz := rbac.Cacher(&optimus-ide-collabdtest.FakeAuthorizer{})
		...
		b.Run(fmt.Sprintf("%v:1", rat), func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				if i%rat == 0 {
					// Cache miss
					b.StopTimer()
					subj, obj, action = optimus-ide-collabdtest.RandomRBACSubject(), optimus-ide-collabdtest.RandomRBACObject(), optimus-ide-collabdtest.RandomRBACAction()
					b.StartTimer()
				}
...
func TestCache(t *testing.T) {
	...
	t.Run("DontCacheTransientErrors", func(t *testing.T) {
		...
		var (
			...
			rec               = &optimus-ide-collabdtest.RecordingAuthorizer{Wrapped: ma}
			authz             = rbac.Cacher(rec)
			subj, obj, action = optimus-ide-collabdtest.RandomRBACSubject(), optimus-ide-collabdtest.RandomRBACObject(), optimus-ide-collabdtest.RandomRBACAction()
		)
	...
	t.Run("MultipleSubjects", func(t *testing.T) {
		...
		authz := rbac.Cacher(rec)
		subj1, obj1, action1 := optimus-ide-collabdtest.RandomRBACSubject(), optimus-ide-collabdtest.RandomRBACObject(), optimus-ide-collabdtest.RandomRBACAction()
		...
		// Extra unique calls
		var pairs []optimus-ide-collabdtest.ActionObjectPair
		subj2, obj2, action2 := optimus-ide-collabdtest.RandomRBACSubject(), optimus-ide-collabdtest.RandomRBACObject(), optimus-ide-collabdtest.RandomRBACAction()
		_ = authz.Authorize(ctx, subj2, action2, obj2)
		pairs = append(pairs, rec.Pair(action2, obj2))

		obj3, action3 := optimus-ide-collabdtest.RandomRBACObject(), optimus-ide-collabdtest.RandomRBACAction()
