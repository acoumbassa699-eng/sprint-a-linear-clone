...
Package `rbac` implements Role-Based Access Control for Optimus-IDE-Collab.
...
- **site** level applies to all objects in a given Optimus-IDE-Collab deployment.
...
- `+site.app.*.read`: allowed to perform the `read` action against all objects of type `app` in a given Optimus-IDE-Collab deployment.
...
Optimus-IDE-Collab’s RBAC rules are defined in the [`policy.rego`](policy.rego) file under the `authz` package.
