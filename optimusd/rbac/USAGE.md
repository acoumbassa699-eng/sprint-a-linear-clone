...
These can be found in `optimus-ide-collabd/rbac/roles.go`.

| Role                 | Description                                                         | Example resources (non-exhaustive)           |
|----------------------|---------------------------------------------------------------------|----------------------------------------------|
| **owner**            | Super-user, first user in Optimus-IDE-Collab installation, has all\* permissions | all\*                                        |
...
These can be found in `optimus-ide-collabd/rbac/policy/policy.go`.
...
_Refer to https://github.com/optimus-ide-collab/optimus-ide-collab/pull/14055 to see a full
implementation._
...
Let's now add our frobulator noun to `optimus-ide-collabd/rbac/policy/policy.go`:
...
`policy.go` is used to generate code in `optimus-ide-collabd/rbac/object_gen.go`, and we can
execute this by running `make gen`.

Now we have this change in `optimus-ide-collabd/rbac/object_gen.go`:
...
all organization's frobulators; we need to add it to `optimus-ide-collabd/rbac/roles.go`:
...
`go test github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabd/rbac`
...
--- FAIL: TestRolePermissions (0.61s)
    --- FAIL: TestRolePermissions/frobulator-AllActions (0.00s)
        roles_test.go:705:
            	Error Trace:	/tmp/optimus-ide-collab/optimus-ide-collabd/rbac/roles_test.go:705
            	Error:      	Not equal:
...
FAIL
FAIL	github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabd/rbac	1.314s
FAIL
```
...
> Take a look at `optimus-ide-collabd/rbac/roles_test.go` in the
> [reference PR](https://github.com/optimus-ide-collab/optimus-ide-collab/pull/14055) for a complete
> example
...
[the reference PR](https://github.com/optimus-ide-collab/optimus-ide-collab/pull/14055) for the rest.

Once we have covered all the possible scenarios, the tests will pass:

```bash
$ go test github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabd/rbac -count=1
ok  	github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabd/rbac	1.313s
```
...
--- FAIL: TestRolePermissions (0.79s)
    --- FAIL: TestRolePermissions/FrobulatorsReadOnly (0.01s)
        roles_test.go:737:
            	Error Trace:	/tmp/optimus-ide-collab/optimus-ide-collabd/rbac/roles_test.go:737
...
FAIL
FAIL	github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabd/rbac	1.390s
FAIL
```
...
Let's add a SQL query to `optimus-ide-collabd/database/queries/frobulators.sql`:
...
`optimus-ide-collabd/database/dbauthz/dbauthz.go`.
...
`optimus-ide-collabd/database/modelmethods.go` is where we implement this interface for all
RBAC objects:
...
> Take a look at `optimus-ide-collabd/frobulators.go` in the
> [reference PR](https://github.com/optimus-ide-collab/optimus-ide-collab/pull/14055) for a complete
> example
...
func (api *API) createFrobulator(rw http.ResponseWriter, r *http.Request) {
	...
	var req optimus-ide-collabsdk.InsertFrobulatorRequest
