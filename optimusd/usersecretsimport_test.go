...
func TestImportUserSecrets(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		secrets, err := client.ImportUserSecrets(ctx, optimus-ide-collabsdk.Me, optimus-ide-collabsdk.ImportUserSecretsRequest{
			Format:  optimus-ide-collabsdk.SecretsFileFormatEnv,
			Content: "ALPHA=a\nBETA=b\nPATH=c\n",
		})
		...
		listed, err := client.UserSecrets(ctx, optimus-ide-collabsdk.Me)
		require.NoError(t, err)
	...
	t.Run("ValuesNotInResponse", func(t *testing.T) {
		...
		res, err := client.Request(ctx, http.MethodPost,
			fmt.Sprintf("/api/v2/users/%s/secrets/batch", optimus-ide-collabsdk.Me),
...
func TestImportUserSecretsForbiddenForAnotherUser(t *testing.T) {
	...
	_, err := memberClient.ImportUserSecrets(ctx, owner.UserID.String(), optimus-ide-collabsdk.ImportUserSecretsRequest{
		Format:  optimus-ide-collabsdk.SecretsFileFormatEnv,
...
func TestImportUserSecretsBodyTooLarge(t *testing.T) {
	...
	_, err := client.ImportUserSecrets(ctx, optimus-ide-collabsdk.Me, optimus-ide-collabsdk.ImportUserSecretsRequest{
		Format:  optimus-ide-collabsdk.SecretsFileFormatEnv,
		Content: strings.Repeat("a", 8*optimus-ide-collabsdk.MaxSecretsFileBytes),
	})
	var sdkErr *optimus-ide-collabsdk.Error
	require.ErrorAs(t, err, &sdkErr)
...
func TestImportUserSecretsValidationRollback(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "OversizedValue", badLine: "BIG=" + strings.Repeat("a", optimus-ide-collabsdk.MaxUserSecretValueBytes+1)},
...
func TestImportUserSecretsLimits(t *testing.T) {
	...
	t.Run("CountLimit", func(t *testing.T) {
		...
		before, err := client.UserSecrets(ctx, optimus-ide-collabsdk.Me)
		require.NoError(t, err)
		require.Len(t, before, optimus-ide-collabsdk.MaxUserSecretsPerUserCount-1)
		...
		after, err := client.UserSecrets(ctx, optimus-ide-collabsdk.Me)
		require.NoError(t, err)
	...
	t.Run("EnvBytesLimit", func(t *testing.T) {
		...
		content := fmt.Sprintf("ENV_A=%s\nENV_B=%s\n",
			strings.Repeat("a", optimus-ide-collabsdk.MaxUserSecretValueBytes-16),
...
func TestImportUserSecretsParseErrors(t *testing.T) {
	...
	_, err := client.ImportUserSecrets(ctx, optimus-ide-collabsdk.Me, optimus-ide-collabsdk.ImportUserSecretsRequest{
		Format:  optimus-ide-collabsdk.SecretsFileFormatJSON,
		Content: "{not json",
	})
	var sdkErr *optimus-ide-collabsdk.Error
	require.ErrorAs(t, err, &sdkErr)
