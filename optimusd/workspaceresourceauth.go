...
func (api *API) handleAuthInstanceID(rw http.ResponseWriter, r *http.Request, instanceID string, agentName string) {
	...
	if agentName != "" {
		...
		// configure OPTIMUS-IDE-COLLAB_AGENT_NAME. The caller has already proven
		...
		httpapi.Write(ctx, rw, http.StatusConflict, optimus-ide-collabsdk.Response{
			Message: fmt.Sprintf(
				"Multiple agents found with instance ID %q. Set OPTIMUS-IDE-COLLAB_AGENT_NAME to one of: %s",
				instanceID,
				strings.Join(names, ", "),
			),
		})
