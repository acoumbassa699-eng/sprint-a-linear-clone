	...
	tests := []struct {
		...
		},
		{
			...
			params: url.Values{
				...
				"optimus-ide-collab_signed_app_token":            []string{"jwt-token"},
				"optimus-ide-collab_application_connect_api_key": []string{"encrypted-key"},
...
func TestRequestLogger_AuthContext(t *testing.T) {
	...
	require.Equal(t, entries[0].Fields[2].Value, "test@optimus-ide-collab.com")
}
