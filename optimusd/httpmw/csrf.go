...
func CSRF(cookieCfg optimus-ide-collabsdk.HTTPCookieConfig) func(next http.Handler) http.Handler {
	return func(next http.Handler) http.Handler {
		mw := nosurf.New(next)
		mw.SetBaseCookie(*cookieCfg.Apply(&http.Cookie{Path: "/", HttpOnly: true}))
		mw.SetFailureHandler(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			sessCookie, err := r.Cookie(optimus-ide-collabsdk.SessionTokenCookie)
			if err == nil &&
				r.Header.Get(optimus-ide-collabsdk.SessionTokenHeader) != "" &&
				r.Header.Get(optimus-ide-collabsdk.SessionTokenHeader) != sessCookie.Value {
				...
				http.Error(w,
					fmt.Sprintf("CSRF error encountered. Authentication via %q cookie and %q header detected, but the values do not match. "+
						...
						optimus-ide-collabsdk.SessionTokenCookie, optimus-ide-collabsdk.SessionTokenHeader),
		...
		mw.ExemptFunc(func(r *http.Request) bool {
			...
			// cross-site authorization code theft (optimus-ide-collab/security#121).
			...
			if token := r.Header.Get(optimus-ide-collabsdk.SessionTokenHeader); token == sessCookie.Value {
			...
			if token := r.URL.Query().Get(optimus-ide-collabsdk.SessionTokenCookie); token == sessCookie.Value {
				// If the auth is set in a url param and matches the cookie, it
				// is the same as just using the url param.
				return true
			}

			if r.Header.Get(optimus-ide-collabsdk.ProvisionerDaemonPSK) != "" {
			...
			if r.Header.Get(optimus-ide-collabsdk.ProvisionerDaemonKey) != "" {
				// If present, the provisioner daemon also is providing an api key
				// that will make them exempt from CSRF. But this is still useful
				// for enumerating the external auths.
				return true
