...
func TestOrganizationParam(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		_ = dbgen.OrganizationMember(t, db, database.OrganizationMember{
			...
			UserID:         user.ID,
			Roles:          []string{optimus-ide-collabsdk.RoleOrganizationMember},
		})
		_, err := db.UpdateUserRoles(ctx, database.UpdateUserRolesParams{
			ID:           user.ID,
			GrantedRoles: []string{optimus-ide-collabsdk.RoleTemplateAdmin},
		})
		require.NoError(t, err)
		...
		// Try by 'default'
		chi.RouteContext(r.Context()).URLParams.Add("organization", optimus-ide-collabsdk.DefaultOrganization)
		chi.RouteContext(r.Context()).URLParams.Add("user", user.ID.String())
		rtr.ServeHTTP(rw, r)
		res = rw.Result()
		defer res.Body.Close()
