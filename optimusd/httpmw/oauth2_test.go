...
func TestOAuth2(t *testing.T) {
	...
	t.Run("NotSetup", func(t *testing.T) {
		...
		httpmw.ExtractOAuth2(nil, nil, optimus-ide-collabsdk.HTTPCookieConfig{}, nil, nil, nil, "")(nil).ServeHTTP(res, req)
	...
	t.Run("RedirectWithoutCode", func(t *testing.T) {
		...
		httpmw.ExtractOAuth2(tp, nil, optimus-ide-collabsdk.HTTPCookieConfig{}, nil, nil, nil, "")(nil).ServeHTTP(res, req)
	...
	t.Run("ExchangeCodeAndState", func(t *testing.T) {
		...
		res := httptest.NewRecorder()
		tp := newTestOAuth2Provider(t, oauth2.AccessTypeOffline)
		httpmw.ExtractOAuth2(tp, nil, optimus-ide-collabsdk.HTTPCookieConfig{}, nil, nil, nil, "")(http.HandlerFunc(func(_ http.ResponseWriter, r *http.Request) {
	...
	t.Run("CustomAuthCodeOptions", func(t *testing.T) {
		...
		httpmw.ExtractOAuth2(tp, nil, optimus-ide-collabsdk.HTTPCookieConfig{}, authOpts, nil, nil, "")(nil).ServeHTTP(res, req)
		location := res.Header().Get("Location")
	...
	t.Run("PresetConvertState", func(t *testing.T) {
		...
		res := httptest.NewRecorder()
		tp := newTestOAuth2Provider(t, oauth2.AccessTypeOffline)
		httpmw.ExtractOAuth2(tp, nil, optimus-ide-collabsdk.HTTPCookieConfig{
			Secure:   true,
			SameSite: "none",
		...
		for _, cookie := range res.Result().Cookies() {
			if cookie.Name == optimus-ide-collabsdk.OAuth2StateCookie {
...
func TestOAuth2DynamicRedirect(t *testing.T) {
	...
	const primaryHost = "optimus-ide-collab.test.netflix.net"
	...
	t.Run("InitOnAllowedHostSetsCookieAndOverridesRedirect", func(t *testing.T) {
		...
		httpmw.ExtractOAuth2(tp, nil, optimus-ide-collabsdk.HTTPCookieConfig{}, nil, nil,
		...
		for _, c := range res.Result().Cookies() {
			if c.Name == optimus-ide-collabsdk.OAuth2RedirectURICookie {
				redirectCookie = c
				break
			}
		}
		require.NotNil(t, redirectCookie, "expected %s cookie", optimus-ide-collabsdk.OAuth2RedirectURICookie)
	...
	t.Run("InitOnDisallowedHostReturnsBadRequest", func(t *testing.T) {
		...
		require.Equal(t, http.StatusBadRequest, res.Result().StatusCode)
		for _, c := range res.Result().Cookies() {
			require.NotEqual(t, optimus-ide-collabsdk.OAuth2RedirectURICookie, c.Name, "must not set redirect_uri cookie when host is rejected")
		}
	})
	...
	t.Run("ExchangeReusesRedirectURIFromCookie", func(t *testing.T) {
		...
		req.Host = altHost
		req.AddCookie(&http.Cookie{Name: optimus-ide-collabsdk.OAuth2StateCookie, Value: "something"})
		req.AddCookie(&http.Cookie{Name: optimus-ide-collabsdk.OAuth2RedirectCookie, Value: "/dashboard"})
		req.AddCookie(&http.Cookie{Name: optimus-ide-collabsdk.OAuth2RedirectURICookie, Value: wantAltURI})
		res := httptest.NewRecorder()
	...
	t.Run("CallbackOnDisallowedHostReturnsBadRequest", func(t *testing.T) {
		...
		req.Host = "evil.example.com"
		req.AddCookie(&http.Cookie{Name: optimus-ide-collabsdk.OAuth2StateCookie, Value: "something"})
		req.AddCookie(&http.Cookie{Name: optimus-ide-collabsdk.OAuth2RedirectURICookie, Value: wantPrimaryURI})
		res := httptest.NewRecorder()
	...
	t.Run("AllowlistDisabledLeavesBehaviorUnchanged", func(t *testing.T) {
		...
		require.Equal(t, http.StatusTemporaryRedirect, res.Result().StatusCode)
		for _, c := range res.Result().Cookies() {
			require.NotEqual(t, optimus-ide-collabsdk.OAuth2RedirectURICookie, c.Name)
		}
	})
}
