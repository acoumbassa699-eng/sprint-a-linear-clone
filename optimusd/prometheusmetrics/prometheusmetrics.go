...
func Agents(ctx context.Context, logger slog.Logger, registerer prometheus.Registerer, db database.Store, coordinator *atomic.Pointer[tailnet.Coordinator], derpMapFn func() *tailcfg.DERPMap, agentInactiveDisconnectTimeout, duration time.Duration) (func(), error) {
	...
	ctx, cancelFunc := context.WithCancel(ctx)
	// nolint:gocritic // Prometheus must collect metrics for all Optimus-IDE-Collab users.
...
func Experiments(registerer prometheus.Registerer, active optimus-ide-collabsdk.Experiments) error {
	...
	for _, exp := range optimus-ide-collabsdk.ExperimentsSafe {
		var val float64
...
func BuildInfo(registerer prometheus.Registerer, version, revision string) error {
	gauge := prometheus.NewGaugeVec(prometheus.GaugeOpts{
		Namespace: "optimus-ide-collabd",
		Name:      "build_info",
		Help:      "Describes the current build/version of the Optimus-IDE-Collab server. Value is always 1.",
