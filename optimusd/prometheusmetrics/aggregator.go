...
const (
	...
	// help messages for the same metric in a "gather". If our optimus-ide-collab agents are
	// on different versions, this is a possible scenario.
	metricHelpForAgent = "Metrics are forwarded from workspace agents connected to this instance of optimus-ide-collabd."
)
...
var MetricLabelValueEnoptimus-ide-collab = strings.NewReplacer("\\", "\\\\", "|", "\\|", ",", "\\,", "=", "\\=")
...
func hashKey(req *updateRequest, m *agentproto.Stats_Metric) metricKey {
	...
	for _, label := range m.GetLabels() {
		...
		labelPairs = append(labelPairs, fmt.Sprintf("%s=%s", label.Name, MetricLabelValueEnoptimus-ide-collab.Replace(label.Value)))
	}
	labelPairs.Sort()
	return metricKey{
