...
var (
	templatesActiveUsersDesc     = prometheus.NewDesc("optimus-ide-collabd_insights_templates_active_users", "The number of active users of the template.", []string{"template_name", "organization_name"}, nil)
	applicationsUsageSecondsDesc = prometheus.NewDesc("optimus-ide-collabd_insights_applications_usage_seconds", "The application usage per template.", []string{"template_name", "application_name", "slug", "organization_name"}, nil)
	parametersDesc               = prometheus.NewDesc("optimus-ide-collabd_insights_parameters", "The parameter usage per template.", []string{"template_name", "parameter_name", "parameter_type", "parameter_value", "organization_name"}, nil)
)
...
func (mc *MetricsCollector) Collect(metricsCh chan<- prometheus.Metric) {
	...
	for _, templateRow := range data.templates {
		...
		metricsCh <- prometheus.MustNewConstMetric(applicationsUsageSecondsDesc, prometheus.GaugeValue,
			...
			optimus-ide-collabsdk.TemplateBuiltinAppDisplayNameVSCode,
			"", orgName)

		metricsCh <- prometheus.MustNewConstMetric(applicationsUsageSecondsDesc, prometheus.GaugeValue,
			float64(templateRow.UsageJetbrainsSeconds),
			data.templateNames[templateRow.TemplateID],
			optimus-ide-collabsdk.TemplateBuiltinAppDisplayNameJetBrains,
			"", orgName)

		metricsCh <- prometheus.MustNewConstMetric(applicationsUsageSecondsDesc, prometheus.GaugeValue,
			float64(templateRow.UsageReconnectingPtySeconds),
			data.templateNames[templateRow.TemplateID],
			optimus-ide-collabsdk.TemplateBuiltinAppDisplayNameWebTerminal,
			"", orgName)

		metricsCh <- prometheus.MustNewConstMetric(applicationsUsageSecondsDesc, prometheus.GaugeValue,
			float64(templateRow.UsageSshSeconds),
			data.templateNames[templateRow.TemplateID],
			optimus-ide-collabsdk.TemplateBuiltinAppDisplayNameSSH,
			"", orgName)
	}
