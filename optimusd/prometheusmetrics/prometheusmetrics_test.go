...
func TestWorkspaceLatestBuildTotals(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		Total    int
		Status   map[optimus-ide-collabsdk.ProvisionerJobStatus]int
	}{{
		Name: "None",
		Database: func() database.Store {
			db, _ := dbtestutil.NewDB(t)
		...
		Total: 7,
		Status: map[optimus-ide-collabsdk.ProvisionerJobStatus]int{
			optimus-ide-collabsdk.ProvisionerJobCanceled:  1,
			optimus-ide-collabsdk.ProvisionerJobFailed:    2,
			optimus-ide-collabsdk.ProvisionerJobSucceeded: 3,
			optimus-ide-collabsdk.ProvisionerJobRunning:   1,
		},
	}, {
		...
		Status: map[optimus-ide-collabsdk.ProvisionerJobStatus]int{
			optimus-ide-collabsdk.ProvisionerJobCanceled:  1,
			optimus-ide-collabsdk.ProvisionerJobFailed:    1,
			optimus-ide-collabsdk.ProvisionerJobSucceeded: 1,
			optimus-ide-collabsdk.ProvisionerJobRunning:   1,
		},
	}} {
		t.Run(tc.Name, func(t *testing.T) {
			...
			require.Eventually(t, func() bool {
				...
				for _, m := range metrics {
					if m.GetName() != "optimus-ide-collabd_api_workspace_latest_build" {
						continue
					}

					for _, metric := range m.Metric {
						count, ok := tc.Status[optimus-ide-collabsdk.ProvisionerJobStatus(metric.Label[0].GetValue())]
...
func TestWorkspaceLatestBuildStatuses(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		ExpectedStatuses   map[optimus-ide-collabsdk.ProvisionerJobStatus]int
	}{{
		Name: "None",
		Database: func() database.Store {
			db, _ := dbtestutil.NewDB(t)
		...
		t.Run(tc.Name, func(t *testing.T) {
			...
			require.Eventually(t, func() bool {
				...
				stMap := map[optimus-ide-collabsdk.ProvisionerJobStatus]int{}
				for _, m := range metrics {
					if m.GetName() != "optimus-ide-collabd_workspace_latest_build_status" {
						continue
					}
					...
					for _, metric := range m.Metric {
						for _, l := range metric.Label {
							...
							if l.GetName() == "status" {
								status := optimus-ide-collabsdk.ProvisionerJobStatus(l.GetValue())
	...
	for _, tc := range []struct {
		...
		t.Run(tc.Name, func(t *testing.T) {
			...
			require.Eventually(t, func() bool {
				...
				sum := 0
				for _, m := range metrics {
					if m.GetName() != "optimus-ide-collabd_workspace_creation_total" {
						continue
					}
					for _, metric := range m.Metric {
...
func TestAgents(t *testing.T) {
	...
	require.Eventually(t, func() bool {
		...
		for _, metric := range metrics {
			...
			case "optimus-ide-collabd_agents_up":
				...
				agentsUp = true
			case "optimus-ide-collabd_agents_connections":
			...
			case "optimus-ide-collabd_agents_apps":
				assert.Equal(t, "testagent", metric.Metric[0].Label[0].GetValue())        // Agent name
				...
				assert.Equal(t, 1, int(metric.Metric[0].Gauge.GetValue()))                // Metric value
				agentsApps = true
			case "optimus-ide-collabd_prometheusmetrics_agents_execution_seconds":
...
func TestAgentStats(t *testing.T) {
	...
	assert.Eventually(t, func() bool {
		...
		for _, metric := range metrics {
			switch metric.GetName() {
			case "optimus-ide-collabd_prometheusmetrics_agentstats_execution_seconds":
				executionSeconds = true
			case "optimus-ide-collabd_agentstats_connection_count",
				"optimus-ide-collabd_agentstats_connection_median_latency_seconds",
				"optimus-ide-collabd_agentstats_rx_bytes",
				"optimus-ide-collabd_agentstats_tx_bytes",
				"optimus-ide-collabd_agentstats_session_count_jetbrains",
				"optimus-ide-collabd_agentstats_session_count_reconnecting_pty",
				"optimus-ide-collabd_agentstats_session_count_ssh",
				"optimus-ide-collabd_agentstats_session_count_vscode":
...
func TestExperimentsMetric(t *testing.T) {
	...
	if len(optimus-ide-collabsdk.ExperimentsSafe) == 0 {
	...
	tests := []struct {
		...
		expected    map[optimus-ide-collabsdk.Experiment]float64
	}{
		{
			name: "Enabled experiment is exported in metrics",
			experiments: optimus-ide-collabsdk.Experiments{
				optimus-ide-collabsdk.ExperimentsSafe[0],
			},
			expected: map[optimus-ide-collabsdk.Experiment]float64{
				optimus-ide-collabsdk.ExperimentsSafe[0]: 1,
			},
		},
		{
			...
			experiments: optimus-ide-collabsdk.Experiments{},
			expected: map[optimus-ide-collabsdk.Experiment]float64{
				optimus-ide-collabsdk.ExperimentsSafe[0]: 0,
			},
		},
		{
			name:        "Unknown experiment is not exported in metrics",
			experiments: optimus-ide-collabsdk.Experiments{optimus-ide-collabsdk.Experiment("bob")},
			expected:    map[optimus-ide-collabsdk.Experiment]float64{},
		},
	}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			seen := make(map[optimus-ide-collabsdk.Experiment]float64)

			for _, metric := range out[0].GetMetric() {
				require.Equal(t, "optimus-ide-collabd_experiments", out[0].GetName())

				labels := metric.GetLabel()
				require.Lenf(t, labels, 1, "unexpected number of labels")

				experiment := optimus-ide-collabsdk.Experiment(labels[0].GetValue())
			...
			// We don't want to define the state of all experiments because optimus-ide-collabsdk.ExperimentAll will change at some
...
func TestBuildInfo(t *testing.T) {
	...
	require.NoError(t, err)
	require.Len(t, out, 1)
	require.Equal(t, "optimus-ide-collabd_build_info", out[0].GetName())
...
func prepareWorkspaceAndAgent(ctx context.Context, t *testing.T, client *optimus-ide-collabsdk.Client, user optimus-ide-collabsdk.CreateFirstUserResponse, workspaceNum int) agentproto.DRPCAgentClient {
