...
func TestInstrument(t *testing.T) {
	...
	const metricname = "optimus-ide-collabd_oauth2_external_requests_total"
	count := func(source string) int {
		labels["source"] = source
		return promhelp.CounterValue(t, reg, "optimus-ide-collabd_oauth2_external_requests_total", labels)
	}
	...
	for _, c := range cases {
		t.Run(c.Name, func(t *testing.T) {
			...
			if !c.ExpectNoMetrics {
				pass = pass && assert.Equal(t, promhelp.GaugeValue(t, reg, "optimus-ide-collabd_oauth2_external_requests_rate_limit", labels), c.Limit, "limit")
				pass = pass && assert.Equal(t, promhelp.GaugeValue(t, reg, "optimus-ide-collabd_oauth2_external_requests_rate_limit_remaining", labels), c.Remaining, "remaining")
				pass = pass && assert.Equal(t, promhelp.GaugeValue(t, reg, "optimus-ide-collabd_oauth2_external_requests_rate_limit_used", labels), c.Used, "used")
				if !c.at.IsZero() {
					until := c.Reset.Sub(c.at)
					// Float accuracy is not great, so we allow a delta of 2
					pass = pass && assert.InDelta(t, promhelp.GaugeValue(t, reg, "optimus-ide-collabd_oauth2_external_requests_rate_limit_reset_in_seconds", labels), int(until.Seconds()), 2, "reset in")
				}
			} else {
				pass = pass && assert.Nil(t, promhelp.MetricValue(t, reg, "optimus-ide-collabd_oauth2_external_requests_rate_limit", labels), "not exists")
			}
