...
// @Success 200 {object} optimus-ide-collabsdk.Template
...
// @Produce json
// @Tags Templates
// @Param request body optimus-ide-collabsdk.CreateTemplateRequest true "Request body"
...
func (api *API) postTemplateByOrganization(rw http.ResponseWriter, r *http.Request) {
	var (
		...
		createTemplate                     optimus-ide-collabsdk.CreateTemplateRequest
	...
	if createTemplate.AutostartRequirement != nil {
		...
		autostartRequirementDaysOfWeek = optimus-ide-collabsdk.BitmapToWeekdays(0b01111111)
	}
	...
	var (
		validErrs                            []optimus-ide-collabsdk.ValidationError
	...
	if defaultTTL < 0 {
		validErrs = append(validErrs, optimus-ide-collabsdk.ValidationError{Field: "default_ttl_ms", Detail: "Must be a positive integer."})
	}
	if activityBump < 0 {
		validErrs = append(validErrs, optimus-ide-collabsdk.ValidationError{Field: "activity_bump_ms", Detail: "Must be a positive integer."})
	}
	if timeTilAutostopNotify < 0 {
		validErrs = append(validErrs, optimus-ide-collabsdk.ValidationError{Field: "time_til_autostop_notify_ms", Detail: "Must be a positive integer."})
	} else if timeTilAutostopNotify != 0 && timeTilAutostopNotify < time.Minute {
		validErrs = append(validErrs, optimus-ide-collabsdk.ValidationError{Field: "time_til_autostop_notify_ms", Detail: "Must be 0 (disabled) or at least one minute."})
	}

	if len(autostopRequirementDaysOfWeek) > 0 {
		autostopRequirementDaysOfWeekParsed, err = optimus-ide-collabsdk.WeekdaysToBitmap(autostopRequirementDaysOfWeek)
		if err != nil {
			validErrs = append(validErrs, optimus-ide-collabsdk.ValidationError{Field: "autostop_requirement.days_of_week", Detail: err.Error()})
		}
	}
	if len(autostartRequirementDaysOfWeek) > 0 {
		autostartRequirementDaysOfWeekParsed, err = optimus-ide-collabsdk.WeekdaysToBitmap(autostartRequirementDaysOfWeek)
		if err != nil {
			validErrs = append(validErrs, optimus-ide-collabsdk.ValidationError{Field: "autostart_requirement.days_of_week", Detail: err.Error()})
		}
	}
	if createTemplate.MaxPortShareLevel != nil {
		err = portSharer.ValidateTemplateMaxLevel(*createTemplate.MaxPortShareLevel)
		if err != nil {
			validErrs = append(validErrs, optimus-ide-collabsdk.ValidationError{Field: "max_port_share_level", Detail: err.Error()})
		} else {
	...
	if autostopRequirementWeeks < 0 {
		validErrs = append(validErrs, optimus-ide-collabsdk.ValidationError{Field: "autostop_requirement.weeks", Detail: "Must be a positive integer."})
	}
	if autostopRequirementWeeks > schedule.MaxTemplateAutostopRequirementWeeks {
		validErrs = append(validErrs, optimus-ide-collabsdk.ValidationError{Field: "autostop_requirement.weeks", Detail: fmt.Sprintf("Must be less than %d.", schedule.MaxTemplateAutostopRequirementWeeks)})
	}
	if failureTTL < 0 {
		validErrs = append(validErrs, optimus-ide-collabsdk.ValidationError{Field: "failure_ttl_ms", Detail: "Must be a positive integer."})
	}
	if dormantTTL < 0 {
		validErrs = append(validErrs, optimus-ide-collabsdk.ValidationError{Field: "time_til_dormant_autodeletion_ms", Detail: "Must be a positive integer."})
	}
	...
	if !createTemplate.DisableEveryoneGroupAccess {
		...
		defaultsGroups[organization.ID.String()] = db2sdk.TemplateRoleActions(optimus-ide-collabsdk.TemplateRoleUse)
	}
...
// @Success 200 {array} optimus-ide-collabsdk.Template
...
// @Tags Templates
// @Param template path string true "Template ID" format(uuid)
// @Param request body optimus-ide-collabsdk.UpdateTemplateMeta true "Patch template settings request"
// @Success 200 {object} optimus-ide-collabsdk.Template
...
func (api *API) patchTemplateMeta(rw http.ResponseWriter, r *http.Request) {
	...
	var req optimus-ide-collabsdk.UpdateTemplateMeta
	...
	if resolved.failureTTLMillis < 0 || (resolved.failureTTLMillis > 0 && resolved.failureTTLMillis < minTTL) {
		validErrs = append(validErrs, optimus-ide-collabsdk.ValidationError{Field: "failure_ttl_ms", Detail: "Value must be at least one minute."})
	}
	if resolved.timeTilDormantMillis < 0 || (resolved.timeTilDormantMillis > 0 && resolved.timeTilDormantMillis < minTTL) {
		validErrs = append(validErrs, optimus-ide-collabsdk.ValidationError{Field: "time_til_dormant_ms", Detail: "Value must be at least one minute."})
	}
	if resolved.timeTilDormantAutoDeleteMillis < 0 || (resolved.timeTilDormantAutoDeleteMillis > 0 && resolved.timeTilDormantAutoDeleteMillis < minTTL) {
		validErrs = append(validErrs, optimus-ide-collabsdk.ValidationError{Field: "time_til_dormant_autodelete_ms", Detail: "Value must be at least one minute."})
	}
	...
	if req.MaxPortShareLevel != nil && *req.MaxPortShareLevel != portSharer.ConvertMaxLevel(template.MaxPortSharingLevel) {
		...
		if err != nil {
			validErrs = append(validErrs, optimus-ide-collabsdk.ValidationError{Field: "max_port_sharing_level", Detail: err.Error()})
		} else {
...
// @Tags Templates
// @Param organization path string true "Organization ID" format(uuid)
// @Success 200 {array} optimus-ide-collabsdk.TemplateExample
...
func (api *API) convertTemplates(templates []database.Template) []optimus-ide-collabsdk.Template {
	apiTemplates := make([]optimus-ide-collabsdk.Template, 0, len(templates))
...
func (api *API) convertTemplate(
	...
	return optimus-ide-collabsdk.Template{
		...
		Provisioner:                    optimus-ide-collabsdk.ProvisionerType(template.Provisioner),
		...
		AutostopRequirement: optimus-ide-collabsdk.TemplateAutostopRequirement{
			DaysOfWeek: optimus-ide-collabsdk.BitmapToWeekdays(uint8(template.AutostopRequirementDaysOfWeek)), // #nosec G115 - Safe conversion as AutostopRequirementDaysOfWeek is a 7-bit bitmap
			Weeks:      autostopRequirementWeeks,
		},
		AutostartRequirement: optimus-ide-collabsdk.TemplateAutostartRequirement{
			DaysOfWeek: optimus-ide-collabsdk.BitmapToWeekdays(template.AutostartAllowedDays()),
		},
		...
		CORSBehavior:            optimus-ide-collabsdk.CORSBehavior(template.CorsBehavior),
...
func findTemplateAdmins(ctx context.Context, store database.Store) ([]database.GetUsersRow, error) {
	templateAdmins, err := store.GetUsers(ctx, database.GetUsersParams{
		RbacRole: []string{optimus-ide-collabsdk.RoleTemplateAdmin, optimus-ide-collabsdk.RoleOwner},
	})
	if err != nil {
