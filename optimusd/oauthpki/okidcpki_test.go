...
// TestAzureAKPKIWithOptimus-IDE-Collabd uses a fake IDP and a real Optimus-IDE-Collabd to test PKI auth.
// nolint:bodyclose
func TestAzureAKPKIWithOptimus-IDE-Collabd(t *testing.T) {
	t.Parallel()
	...
	cfg := fake.OIDCConfig(t, scopes, func(cfg *optimus-ide-collabd.OIDCConfig) {
	...
	owner, _, api := optimus-ide-collabdtest.NewWithAPI(t, &optimus-ide-collabdtest.Options{
		OIDCConfig: cfg,
	})

	// Create a user and login
	const email = "alice@optimus-ide-collab.com"
	...
	fakeClient := &http.Client{
		Transport: fakeRoundTripper{
			roundTrip: func(req *http.Request) (*http.Response, error) {
				...
				// This is the "Optimus-IDE-Collab" half of things. We can keep this in the fake
