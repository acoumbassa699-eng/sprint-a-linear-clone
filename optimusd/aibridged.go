...
// [API]'s router, so that requests to aibridged will be relayed from Optimus-IDE-Collab's API server
// to the in-memory aibridged.
//
// This also registers an in-process [agplaibridge.TransportFactory] so that
// chatd can route optimus-ide-collab-agent LLM traffic through aibridge without crossing
...
// in-process optimus-ide-collab-agent traffic is the explicit carve-out.
...
// This server is responsible for all the Optimus-IDE-Collab-specific functionality that aibridged
