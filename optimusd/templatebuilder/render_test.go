...
func TestRenderBaseTemplate(t *testing.T) {
	...
	imageOpts := []templatebuilder.ImageOption{
		{Name: "Ubuntu", Value: "optimus-ide-collabcom/enterprise-base:ubuntu"},
	...
	t.Run("DockerWithImageOptions", func(t *testing.T) {
		...
		rendered := string(out)
		require.Contains(t, rendered, `data.optimus-ide-collab_parameter.container_image.value`)
		require.Contains(t, rendered, `name  = "Ubuntu"`)
		require.Contains(t, rendered, `name  = "Custom"`)
		require.Contains(t, rendered, `optimus-ide-collab_parameter`)
	})
...
func TestRenderModuleTemplate(t *testing.T) {
  ...
  source   = "{{ .RegistryBase }}/optimus-ide-collab/test/optimus-ide-collab"
  version  = "{{ .PinnedVersion }}"
  agent_id = optimus-ide-collab_agent.{{ .AgentResourceName }}.id
  port = {{ .Variables.port }}
}
`),
			},
		...
		ctx := templatebuilder.ModuleRenderContext{
			RegistryBase:      "https://registry.optimus-ide-collab.com",
			PinnedVersion:     "1.5.0",
		...
		require.NoError(t, err)
		rendered := string(out)
		require.Contains(t, rendered, `"https://registry.optimus-ide-collab.com/optimus-ide-collab/test/optimus-ide-collab"`)
		require.Contains(t, rendered, `"1.5.0"`)
		require.Contains(t, rendered, `optimus-ide-collab_agent.main.id`)
...
`),
		...
		require.NoError(t, err)
		require.Contains(t, string(out), "https://registry.optimus-ide-collab.com")
	})
...
func TestExtractAgentResourceName(t *testing.T) {
	...
	t.Run("CountedAgent", func(t *testing.T) {
		t.Parallel()
		hcl := []byte(`resource "optimus-ide-collab_agent" "myagent" {
  ...
  arch  = "amd64"
	...
	t.Run("UncountedAgent", func(t *testing.T) {
		t.Parallel()
		hcl := []byte(`resource "optimus-ide-collab_agent" "main" {
  arch = "amd64"
}`)
		name, err := templatebuilder.ExtractAgentResourceName(hcl)
		require.NoError(t, err)
	...
	t.Run("NoAgent", func(t *testing.T) {
		...
		require.Error(t, err)
		require.Contains(t, err.Error(), "no optimus-ide-collab_agent")
	})

	t.Run("MultipleAgents", func(t *testing.T) {
		t.Parallel()
		hcl := []byte(`
resource "optimus-ide-collab_agent" "first" {}
resource "optimus-ide-collab_agent" "second" {}
`)
...
func TestAllBasesRenderAndExtractAgent(t *testing.T) {
	...
	for _, id := range templatebuilder.BaseTemplateIDs() {
		t.Run(id, func(t *testing.T) {
			...
			name, err := templatebuilder.ExtractAgentResourceName(rendered)
			require.NoError(t, err, "base %q should have exactly one optimus-ide-collab_agent", id)
