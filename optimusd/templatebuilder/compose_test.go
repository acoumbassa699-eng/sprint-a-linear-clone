...
func TestCompose(t *testing.T) {
	...
	t.Run("BaseOnly", func(t *testing.T) {
		...
		result, err := templatebuilder.Compose(templatebuilder.ComposeRequest{
			...
			RegistryURL:    "https://registry.optimus-ide-collab.com",
		})
		require.NoError(t, err)
		require.NotEmpty(t, result.MainTF)
		require.Contains(t, string(result.MainTF), `resource "optimus-ide-collab_agent" "main"`)
	...
	t.Run("BaseWithModuleAndVariableOverride", func(t *testing.T) {
		...
		require.Contains(t, modules, `optimus-ide-collab_agent.main.id`)
		require.Contains(t, modules, `registry.optimus-ide-collab.com`)
	...
	t.Run("AWSLinuxAgentName", func(t *testing.T) {
		...
		require.NoError(t, err)
		require.Contains(t, string(result.ModulesTF), `optimus-ide-collab_agent.dev[0].id`)
	})
	...
	t.Run("GCPLinuxBaseWithProjectID", func(t *testing.T) {
		...
		require.Contains(t, mainTF, `resource "optimus-ide-collab_agent" "main"`)
	...
	t.Run("MultipleModulesWithRequiredVariable", func(t *testing.T) {
		...
		require.Contains(t, modules, `"https://github.com/optimus-ide-collab/optimus-ide-collab"`)
	})
	...
	t.Run("DockerDefaultContainerImage", func(t *testing.T) {
		...
		require.NoError(t, err)
		require.Contains(t, string(result.MainTF), `"optimus-ide-collabcom/example-base:ubuntu"`)
	})
	...
	t.Run("DockerCustomContainerImage", func(t *testing.T) {
		...
		require.NotContains(t, mainTF, `optimus-ide-collabcom/example-base:ubuntu`)
	})
