...
func TestToSDK(t *testing.T) {
		...
		Variables: []templatebuilder.ModuleVariable{
			{
				Name:        "agent_id",
				Type:        "string",
				Description: "The Optimus-IDE-Collab agent ID.",
				Required:    true,
				Sensitive:   false,
				Computed:    true,
			},
	...
	t.Run("AllVariableFields", func(t *testing.T) {
		...
		require.Equal(t, optimus-ide-collabsdk.TemplateBuilderVariableTypeNumber, port.Type)
		...
		require.Equal(t, optimus-ide-collabsdk.TemplateBuilderVariableTypeString, secret.Type)
