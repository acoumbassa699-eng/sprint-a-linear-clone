...
func TestParseBasesFromFS(t *testing.T) {
	...
	t.Run("ValidManifest", func(t *testing.T) {
		...
		fsys := fstest.MapFS{
			"bases/docker/base.json": &fstest.MapFile{
				Data: []byte(`{
					...
					"os": "linux",
					"default_context": {
						"container_image": "optimus-ide-collabcom/enterprise-base:ubuntu"
					}
				}`),
			},
		...
		require.Equal(t, "optimus-ide-collabcom/enterprise-base:ubuntu", b.Manifest.DefaultContext.ContainerImage)
	...
	t.Run("AcceptsWindowsOS", func(t *testing.T) {
		...
		fsys := fstest.MapFS{
			...
			"bases/winbox/main.tf.tmpl": &fstest.MapFile{
				Data: []byte(`resource "optimus-ide-collab_agent" "main" {}`),
			},
