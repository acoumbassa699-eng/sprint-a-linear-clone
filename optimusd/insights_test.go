...
func TestDeploymentInsights(t *testing.T) {
	...
	daus, err := client.DeploymentDAUs(ctx, optimus-ide-collabsdk.TimezoneOffsetHour(clientTz))
	require.NoError(t, err)

	_ = agenttest.New(t, client.URL, authToken)
	resources := optimus-ide-collabdtest.NewWorkspaceAgentWaiter(t, client, workspace.ID).Wait()
...
func TestUserActivityInsights_SanityCheck(t *testing.T) {
	...
	user := optimus-ide-collabdtest.CreateFirstUser(t, client)
	_, _ = optimus-ide-collabdtest.CreateAnotherUser(t, client, user.OrganizationID)
	authToken := uuid.NewString()
	version := optimus-ide-collabdtest.CreateTemplateVersion(t, client, user.OrganizationID, &echo.Responses{
		Parse:          echo.ParseComplete,
		ProvisionPlan:  echo.PlanComplete,
	...
	var userActivities optimus-ide-collabsdk.UserActivityInsightsResponse
	require.Eventuallyf(t, func() bool {
		...
		userActivities, err = client.UserActivityInsights(ctx, optimus-ide-collabsdk.UserActivityInsightsRequest{
			StartTime:   today,
...
func TestUserLatencyInsights(t *testing.T) {
	...
	var userLatencies optimus-ide-collabsdk.UserLatencyInsightsResponse
	require.Eventuallyf(t, func() bool {
		// Keep connection active.
		_, err := w.Write([]byte("hello world\n"))
		if !assert.NoError(t, err) {
		...
		userLatencies, err = client.UserLatencyInsights(ctx, optimus-ide-collabsdk.UserLatencyInsightsRequest{
			StartTime:   today,
			EndTime:     time.Now().UTC().Truncate(time.Hour).Add(time.Hour), // Round up to include the current hour.
			TemplateIDs: []uuid.UUID{template.ID},
		})
...
func TestUserLatencyInsights_BadRequest(t *testing.T) {
	...
	_, err := client.UserLatencyInsights(ctx, optimus-ide-collabsdk.UserLatencyInsightsRequest{
		StartTime: today,
...
func TestUserActivityInsights_BadRequest(t *testing.T) {
	...
	_, err = client.UserActivityInsights(ctx, optimus-ide-collabsdk.UserActivityInsightsRequest{
		StartTime: today,
		EndTime:   today.AddDate(0, 0, -1),
	})
	assert.Error(t, err, "want error for end time before start time")
...
func TestTemplateInsights_Golden(t *testing.T) {
	...
	type testUser struct {
		...
		sdk    optimus-ide-collabsdk.User
	}
	...
	prepare := func(t *testing.T, templates []*testTemplate, users []*testUser, testData map[*testWorkspace]testDataGen, disableStorage bool) (*optimus-ide-collabsdk.Client, chan dbrollup.Event) {
		logger := testutil.Logger(t)
		db, ps := dbtestutil.NewDB(t)
		...
		for _, user := range users {
			user.client, user.sdk = optimus-ide-collabdtest.CreateAnotherUserMutators(t, client, firstUser.OrganizationID, nil, func(r *optimus-ide-collabsdk.CreateUserRequestWithOrgs) {
		...
		for _, template := range templates {
			...
			for _, user := range users {
				for _, workspace := range user.workspaces {
					...
					var buildParameters []optimus-ide-collabsdk.WorkspaceBuildParameter
					for _, buildParameter := range workspace.buildParameters {
						buildParameters = append(buildParameters, optimus-ide-collabsdk.WorkspaceBuildParameter{
					...
					createWorkspaces = append(createWorkspaces, func(templateID uuid.UUID) {
						...
						createdWorkspace := optimus-ide-collabdtest.CreateWorkspace(t, user.client, templateID, func(cwr *optimus-ide-collabsdk.CreateWorkspaceRequest) {
							cwr.RichParameterValues = buildParameters
						})
						workspace.id = createdWorkspace.ID
						waitWorkspaces = append(waitWorkspaces, func() {
							optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, user.client, createdWorkspace.LatestBuild.ID)
			...
			// 	createdTemplate := optimus-ide-collabdtest.CreateTemplate(t, client, firstUser.OrganizationID, version.ID)
			createdTemplate := dbgen.Template(t, db, database.Template{
				...
				GroupACL: database.TemplateACL{
					firstUser.OrganizationID.String(): db2sdk.TemplateRoleActions(optimus-ide-collabsdk.TemplateRoleUse),
				},
			})
		...
		// NOTE(mafredri): Ideally we would pass batcher as a optimus-ide-collabd option and
	...
	type testRequest struct {
		name        string
		makeRequest func([]*testTemplate) optimus-ide-collabsdk.TemplateInsightsRequest
		ignoreTimes bool
	}
	tests := []struct {
		name           string
			...
			requests: []testRequest{
				{
					...
					makeRequest: func(templates []*testTemplate) optimus-ide-collabsdk.TemplateInsightsRequest {
						return optimus-ide-collabsdk.TemplateInsightsRequest{
							StartTime: frozenWeekAgo,
							EndTime:   frozenWeekAgo.AddDate(0, 0, 7),
							Interval:  optimus-ide-collabsdk.InsightsReportIntervalDay,
						}
					},
				},
				{
					...
					makeRequest: func(templates []*testTemplate) optimus-ide-collabsdk.TemplateInsightsRequest {
						return optimus-ide-collabsdk.TemplateInsightsRequest{
							...
							Interval:  optimus-ide-collabsdk.InsightsReportIntervalWeek,
						}
					},
				},
				{
				...
				},
				{
					...
					makeRequest: func(templates []*testTemplate) optimus-ide-collabsdk.TemplateInsightsRequest {
						return optimus-ide-collabsdk.TemplateInsightsRequest{
							...
							EndTime:     frozenWeekAgo.AddDate(0, 0, 7),
							Interval:    optimus-ide-collabsdk.InsightsReportIntervalWeek,
							Sections:    []optimus-ide-collabsdk.TemplateInsightsSection{optimus-ide-collabsdk.TemplateInsightsSectionReport},
						}
					},
				},
				{
					...
					makeRequest: func(templates []*testTemplate) optimus-ide-collabsdk.TemplateInsightsRequest {
						return optimus-ide-collabsdk.TemplateInsightsRequest{
							...
							EndTime:     frozenWeekAgo.AddDate(0, 0, 7),
							Interval:    optimus-ide-collabsdk.InsightsReportIntervalWeek,
							Sections:    []optimus-ide-collabsdk.TemplateInsightsSection{optimus-ide-collabsdk.TemplateInsightsSectionIntervalReports},
						}
					},
				},
			},
			...
			requests: []testRequest{
				{
					...
					ignoreTimes: true,
					makeRequest: func(_ []*testTemplate) optimus-ide-collabsdk.TemplateInsightsRequest {
						now := time.Now().UTC()
						return optimus-ide-collabsdk.TemplateInsightsRequest{
			...
			for _, req := range tt.requests {
				t.Run(req.name, func(t *testing.T) {
					...
					if *updateGoldenFiles {
						...
						defer f.Close()
						enc := json.NewEnoptimus-ide-collab(f)
						enc.SetIndent("", "  ")
						enc.Encode(report)
						return
					}
					...
					defer f.Close()
					var want optimus-ide-collabsdk.TemplateInsightsResponse
					err = json.NewDeoptimus-ide-collab(f).Decode(&want)
...
func TestUserActivityInsights_Golden(t *testing.T) {
	...
	prepare := func(t *testing.T, templates []*testTemplate, users []*testUser, testData map[*testWorkspace]testDataGen, disableStorage bool) (*optimus-ide-collabsdk.Client, chan dbrollup.Event) {
		...
		for _, user := range users {
			...
			require.NoError(t, err)
			userClient := optimus-ide-collabsdk.New(client.URL, optimus-ide-collabsdk.WithSessionToken(token.Key))

			optimus-ide-collabUser, err := userClient.User(context.Background(), user.id.String())
			require.NoError(t, err)

			user.client = userClient
			user.sdk = optimus-ide-collabUser
		...
		for _, template := range templates {
			...
			for _, user := range users {
				for _, workspace := range user.workspaces {
					...
					createWorkspaces = append(createWorkspaces, func(templateID uuid.UUID) {
						// Create workspace using the users client.
						createdWorkspace := optimus-ide-collabdtest.CreateWorkspace(t, user.client, templateID)
						workspace.id = createdWorkspace.ID
						waitWorkspaces = append(waitWorkspaces, func() {
							optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, user.client, createdWorkspace.LatestBuild.ID)
							ctx := testutil.Context(t, testutil.WaitShort)
	...
	type testRequest struct {
		name        string
		makeRequest func([]*testTemplate) optimus-ide-collabsdk.UserActivityInsightsRequest
		ignoreTimes bool
	}
	tests := []struct {
		name           string
	...
	}{
		{
			...
			requests: []testRequest{
				{
					name: "week deployment wide",
					makeRequest: func(templates []*testTemplate) optimus-ide-collabsdk.UserActivityInsightsRequest {
						return optimus-ide-collabsdk.UserActivityInsightsRequest{
							StartTime: frozenWeekAgo,
							EndTime:   frozenWeekAgo.AddDate(0, 0, 7),
						}
					},
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, req := range tt.requests {
				t.Run(req.name, func(t *testing.T) {
					...
					require.NoError(t, err, "open golden file, run \"make gen/golden-files\" and commit the changes")
					defer f.Close()
					var want optimus-ide-collabsdk.UserActivityInsightsResponse
					err = json.NewDeoptimus-ide-collab(f).Decode(&want)
					require.NoError(t, err, "want no error decoding golden file")
...
func TestTemplateInsights_BadRequest(t *testing.T) {
	...
	_, err := client.TemplateInsights(ctx, optimus-ide-collabsdk.TemplateInsightsRequest{
		StartTime: today,
		EndTime:   today.AddDate(0, 0, -1),
	})
	assert.Error(t, err, "want error for end time before start time")
	...
	_, err = client.TemplateInsights(ctx, optimus-ide-collabsdk.TemplateInsightsRequest{
		...
		EndTime:   today,
		Interval:  optimus-ide-collabsdk.InsightsReportIntervalWeek,
		Sections:  []optimus-ide-collabsdk.TemplateInsightsSection{"invalid"},
	})
...
func TestTemplateInsights_RBAC(t *testing.T) {
	...
	tests := []test{
		{optimus-ide-collabsdk.InsightsReportIntervalDay, true},
		{optimus-ide-collabsdk.InsightsReportIntervalDay, false},
		{"", true},
		{"", false},
	}
	...
	for _, tt := range tests {
		t.Run(fmt.Sprintf("with interval=%q", tt.interval), func(t *testing.T) {
			...
			t.Run("AsTemplateAdmin", func(t *testing.T) {
				...
				_, err := templateAdmin.TemplateInsights(ctx, optimus-ide-collabsdk.TemplateInsightsRequest{
					StartTime:   today.AddDate(0, 0, -1),
					EndTime:     today,
					Interval:    tt.interval,
					TemplateIDs: templateIDs,
			...
			t.Run("AsRegularUser", func(t *testing.T) {
				...
				regular, _ := optimus-ide-collabdtest.CreateAnotherUser(t, client, owner.OrganizationID)
...
func TestGenericInsights_RBAC(t *testing.T) {
	...
	type fetchInsightsFunc func(ctx context.Context, client *optimus-ide-collabsdk.Client, startTime, endTime time.Time, templateIDs ...uuid.UUID) error
	...
	endpoints := map[string]fetchInsightsFunc{
		"UserLatency": func(ctx context.Context, client *optimus-ide-collabsdk.Client, startTime, endTime time.Time, templateIDs ...uuid.UUID) error {
			_, err := client.UserLatencyInsights(ctx, optimus-ide-collabsdk.UserLatencyInsightsRequest{
				StartTime:   startTime,
				EndTime:     endTime,
				TemplateIDs: templateIDs,
		...
		"UserActivity": func(ctx context.Context, client *optimus-ide-collabsdk.Client, startTime, endTime time.Time, templateIDs ...uuid.UUID) error {
			_, err := client.UserActivityInsights(ctx, optimus-ide-collabsdk.UserActivityInsightsRequest{
				StartTime:   startTime,
				EndTime:     endTime,
				TemplateIDs: templateIDs,
...
func TestGenericInsights_Disabled(t *testing.T) {
	...
	client := optimus-ide-collabdtest.New(t, &optimus-ide-collabdtest.Options{
		...
		DeploymentValues: optimus-ide-collabdtest.DeploymentValues(t, func(dv *optimus-ide-collabsdk.DeploymentValues) {
			dv.StatsCollection = optimus-ide-collabsdk.StatsCollectionConfig{
				UsageStats: optimus-ide-collabsdk.UsageStatsConfig{
					Enable: false,
				},
			}
		}),
	...
	tests := []struct {
		...
		},
		{
			...
			fn: func(ctx context.Context) error {
				_, err := client.GetUserStatusCounts(ctx, optimus-ide-collabsdk.GetUserStatusCountsRequest{
...
func TestGetUserStatusCounts(t *testing.T) {
	...
	type testCase struct {
		name          string
		request       optimus-ide-collabsdk.GetUserStatusCountsRequest
		checkError    func(t *testing.T, err error)
		checkResponse func(t *testing.T, resp optimus-ide-collabsdk.GetUserStatusCountsResponse)
	}

	happyResponseCheck := func(t *testing.T, resp optimus-ide-collabsdk.GetUserStatusCountsResponse) {
		require.Len(t, resp.StatusCounts, 1)
		require.NotNil(t, resp.StatusCounts[optimus-ide-collabsdk.UserStatusActive])
		require.Len(t, resp.StatusCounts[optimus-ide-collabsdk.UserStatusActive], 61)
		...
		counts := resp.StatusCounts[optimus-ide-collabsdk.UserStatusActive]
