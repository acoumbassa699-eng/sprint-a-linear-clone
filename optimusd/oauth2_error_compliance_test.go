	...
	tests := []struct {
		...
		},
		{
			...
			req: optimus-ide-collabsdk.OAuth2ClientRegistrationRequest{
				...
				ClientName:   fmt.Sprintf("test-client-%d", time.Now().UnixNano()),
				GrantTypes:   []optimus-ide-collabsdk.OAuth2ProviderGrantType{"unsupported_grant_type"},
			},
			expectedError: "invalid_client_metadata",
			expectedCode:  http.StatusBadRequest,
		},
		{
			...
			req: optimus-ide-collabsdk.OAuth2ClientRegistrationRequest{
				RedirectURIs:  []string{"https://example.com/callback"},
				ClientName:    fmt.Sprintf("test-client-%d", time.Now().UnixNano()),
				ResponseTypes: []optimus-ide-collabsdk.OAuth2ProviderResponseType{"unsupported_response_type"},
			},
			expectedError: "invalid_client_metadata",
			expectedCode:  http.StatusBadRequest,
		},
