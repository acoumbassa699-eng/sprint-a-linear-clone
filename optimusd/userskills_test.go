...
func TestPatchUserSkill(t *testing.T) {
	...
	ownerRawClient := optimus-ide-collabdtest.New(t, nil)
	firstUser := optimus-ide-collabdtest.CreateFirstUser(t, ownerRawClient)
	memberRawClient, member := optimus-ide-collabdtest.CreateAnotherUser(t, ownerRawClient, firstUser.OrganizationID)
	memberClient := optimus-ide-collabsdk.NewExperimentalClient(memberRawClient)
	auditorRawClient, _ := optimus-ide-collabdtest.CreateAnotherUser(t, ownerRawClient, firstUser.OrganizationID, rbac.RoleAuditor())
	auditorClient := optimus-ide-collabsdk.NewExperimentalClient(auditorRawClient)
	ctx := testutil.Context(t, testutil.WaitMedium)

	_, err := memberClient.CreateUserSkill(ctx, optimus-ide-collabsdk.Me, optimus-ide-collabsdk.CreateUserSkillRequest{
		Content: userSkillMarkdown("forbidden-skill", "Test skill", "Original body."),
	})
	require.NoError(t, err)

	_, err = auditorClient.UpdateUserSkill(ctx, member.ID.String(), "forbidden-skill", optimus-ide-collabsdk.UpdateUserSkillRequest{
...
func TestUserSkillsCRUD(t *testing.T) {
	...
	ownerClient, _ := optimus-ide-collabdtest.CreateAnotherUser(t, adminClient, firstUser.OrganizationID)
	owner := optimus-ide-collabsdk.NewExperimentalClient(ownerClient)
	ctx := testutil.Context(t, testutil.WaitMedium)

	emptyList, err := owner.UserSkills(ctx, optimus-ide-collabsdk.Me)
	require.NoError(t, err)
	...
	require.NoError(t, json.NewDeoptimus-ide-collab(emptyRes.Body).Decode(&rawEmptyList))
	...
	created, err := owner.CreateUserSkill(ctx, optimus-ide-collabsdk.Me, optimus-ide-collabsdk.CreateUserSkillRequest{Content: content})
	require.NoError(t, err)
	...
	list, err := owner.UserSkills(ctx, optimus-ide-collabsdk.Me)
	require.NoError(t, err)
	...
	require.NoError(t, json.NewDeoptimus-ide-collab(res.Body).Decode(&rawList))
	require.Len(t, rawList, 1)
	assert.NotContains(t, rawList[0], "content")

	got, err := owner.UserSkillByName(ctx, optimus-ide-collabsdk.Me, "crud-skill")
	require.NoError(t, err)
	assert.Equal(t, created.ID, got.ID)
	assert.Equal(t, content, got.Content)

	updatedContent := userSkillMarkdown("crud-skill", "Updated description", "Updated body.")
	updated, err := owner.UpdateUserSkill(ctx, optimus-ide-collabsdk.Me, "crud-skill", optimus-ide-collabsdk.UpdateUserSkillRequest{Content: updatedContent})
	require.NoError(t, err)
	assert.Equal(t, created.ID, updated.ID)
	assert.Equal(t, "Updated description", updated.Description)
	assert.Equal(t, updatedContent, updated.Content)

	require.NoError(t, owner.DeleteUserSkill(ctx, optimus-ide-collabsdk.Me, "crud-skill"))
	_, err = owner.UserSkillByName(ctx, optimus-ide-collabsdk.Me, "crud-skill")
...
func TestUserSkillValidationAndConflicts(t *testing.T) {
	...
	firstUser := optimus-ide-collabdtest.CreateFirstUser(t, adminClient)
	ownerClient, _ := optimus-ide-collabdtest.CreateAnotherUser(t, adminClient, firstUser.OrganizationID)
	otherClient, _ := optimus-ide-collabdtest.CreateAnotherUser(t, adminClient, firstUser.OrganizationID)
	owner := optimus-ide-collabsdk.NewExperimentalClient(ownerClient)
	other := optimus-ide-collabsdk.NewExperimentalClient(otherClient)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := owner.CreateUserSkill(subCtx, optimus-ide-collabsdk.Me, optimus-ide-collabsdk.CreateUserSkillRequest{Content: tt.content})
	...
	t.Run("PatchEmptyBody", func(t *testing.T) {
		...
		_, err := owner.CreateUserSkill(subCtx, optimus-ide-collabsdk.Me, optimus-ide-collabsdk.CreateUserSkillRequest{Content: patchValidationContent})
		require.NoError(t, err)
		_, err = owner.UpdateUserSkill(subCtx, optimus-ide-collabsdk.Me, "patch-validation", optimus-ide-collabsdk.UpdateUserSkillRequest{
	...
	t.Run("DuplicateNameConflict", func(t *testing.T) {
		...
		_, err := owner.CreateUserSkill(subCtx, optimus-ide-collabsdk.Me, optimus-ide-collabsdk.CreateUserSkillRequest{Content: sharedContent})
		require.NoError(t, err)
		_, err = owner.CreateUserSkill(subCtx, optimus-ide-collabsdk.Me, optimus-ide-collabsdk.CreateUserSkillRequest{Content: sharedContent})
	...
	t.Run("CrossUserSameNameAllowed", func(t *testing.T) {
		...
		subCtx := testutil.Context(t, testutil.WaitMedium)
		sharedContent := userSkillMarkdown("shared-skill", "Shared", "Shared body.")
		_, err := other.CreateUserSkill(subCtx, optimus-ide-collabsdk.Me, optimus-ide-collabsdk.CreateUserSkillRequest{Content: sharedContent})
		require.NoError(t, err)
	})
}
...
func TestUserSkillLimit(t *testing.T) {
	...
	for i := range skills.MaxPersonalSkillsPerUser {
		...
		_, err := owner.CreateUserSkill(ctx, optimus-ide-collabsdk.Me, optimus-ide-collabsdk.CreateUserSkillRequest{
...
func TestUserSkillRequestAllowsEscapedMaxSizeContent(t *testing.T) {
	...
	raw, err := json.Marshal(optimus-ide-collabsdk.CreateUserSkillRequest{Content: content})
	require.NoError(t, err)
	require.Greater(t, len(raw), skills.MaxPersonalSkillSizeBytes+1024)

	created, err := owner.CreateUserSkill(ctx, optimus-ide-collabsdk.Me, optimus-ide-collabsdk.CreateUserSkillRequest{
		Content: content,
	})
	require.NoError(t, err)
...
func TestUserSkillMissingAndUpdateMismatch(t *testing.T) {
	...
	_, err := owner.UserSkillByName(ctx, optimus-ide-collabsdk.Me, "missing-skill")
	requireSDKErrorStatus(t, err, http.StatusNotFound)

	_, err = owner.UpdateUserSkill(ctx, optimus-ide-collabsdk.Me, "missing-skill", optimus-ide-collabsdk.UpdateUserSkillRequest{
		Content: userSkillMarkdown("missing-skill", "Missing", "Body."),
	})
	requireSDKErrorStatus(t, err, http.StatusNotFound)

	err = owner.DeleteUserSkill(ctx, optimus-ide-collabsdk.Me, "missing-skill")
	requireSDKErrorStatus(t, err, http.StatusNotFound)
	...
	require.NoError(t, err)
	_, err = owner.UpdateUserSkill(ctx, optimus-ide-collabsdk.Me, "old-name", optimus-ide-collabsdk.UpdateUserSkillRequest{
...
func TestUserSkillAuthorization(t *testing.T) {
	...
	adminClient := optimus-ide-collabdtest.New(t, nil)
	firstUser := optimus-ide-collabdtest.CreateFirstUser(t, adminClient)
	ownerClient, ownerUser := optimus-ide-collabdtest.CreateAnotherUser(t, adminClient, firstUser.OrganizationID)
	otherClient, _ := optimus-ide-collabdtest.CreateAnotherUser(t, adminClient, firstUser.OrganizationID)
	userAdminClient, _ := optimus-ide-collabdtest.CreateAnotherUser(t, adminClient, firstUser.OrganizationID, rbac.RoleUserAdmin())
	admin := optimus-ide-collabsdk.NewExperimentalClient(adminClient)
	owner := optimus-ide-collabsdk.NewExperimentalClient(ownerClient)
	other := optimus-ide-collabsdk.NewExperimentalClient(otherClient)
	userAdmin := optimus-ide-collabsdk.NewExperimentalClient(userAdminClient)
	ctx := testutil.Context(t, testutil.WaitMedium)
	...
	requireSDKErrorStatus(t, err, http.StatusNotFound)
	_, err = other.CreateUserSkill(ctx, targetUser, optimus-ide-collabsdk.CreateUserSkillRequest{
		Content: userSkillMarkdown("denied-create", "Denied", "Body."),
	})
	requireSDKErrorStatus(t, err, http.StatusNotFound)
	_, err = other.UpdateUserSkill(ctx, targetUser, "auth-skill", optimus-ide-collabsdk.UpdateUserSkillRequest{
	...
	requireSDKErrorStatus(t, err, http.StatusNotFound)
	_, err = userAdmin.CreateUserSkill(ctx, targetUser, optimus-ide-collabsdk.CreateUserSkillRequest{
		Content: userSkillMarkdown("denied-admin-create", "Denied", "Body."),
	})
	requireSDKErrorStatus(t, err, http.StatusForbidden)
	_, err = userAdmin.UpdateUserSkill(ctx, targetUser, "auth-skill", optimus-ide-collabsdk.UpdateUserSkillRequest{
		Content: userSkillMarkdown("auth-skill", "Denied", "Body."),
	})
	requireSDKErrorStatus(t, err, http.StatusForbidden)
	...
	_, err = admin.CreateUserSkill(ctx, targetUser, optimus-ide-collabsdk.CreateUserSkillRequest{
	...
	_, err = admin.UpdateUserSkill(ctx, targetUser, "auth-skill", optimus-ide-collabsdk.UpdateUserSkillRequest{
...
func TestUserSkillSoftDeleteCleanup(t *testing.T) {
	...
	adminClient, _, api := optimus-ide-collabdtest.NewWithAPI(t, nil)
	firstUser := optimus-ide-collabdtest.CreateFirstUser(t, adminClient)
	ownerClient, ownerUser := optimus-ide-collabdtest.CreateAnotherUser(t, adminClient, firstUser.OrganizationID)
	owner := optimus-ide-collabsdk.NewExperimentalClient(ownerClient)
	ctx := testutil.Context(t, testutil.WaitMedium)
...
func TestUserSkillDatabaseConstraints(t *testing.T) {
	...
	adminClient, _, api := optimus-ide-collabdtest.NewWithAPI(t, nil)
	firstUser := optimus-ide-collabdtest.CreateFirstUser(t, adminClient)
	_, ownerUser := optimus-ide-collabdtest.CreateAnotherUser(t, adminClient, firstUser.OrganizationID)
	tests := []struct {
		name       string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ctx := testutil.Context(t, testutil.WaitMedium)
			authzCtx := dbauthz.As(ctx, optimus-ide-collabdtest.AuthzUserSubject(ownerUser))
...
func TestUserSkillAudit(t *testing.T) {
	...
	firstUser := optimus-ide-collabdtest.CreateFirstUser(t, adminClient)
	memberClient, _ := optimus-ide-collabdtest.CreateAnotherUser(t, adminClient, firstUser.OrganizationID)
	...
	t.Run("UpdateEmitsLog", func(t *testing.T) {
		...
		require.NoError(t, err)
		_, err = member.UpdateUserSkill(ctx, optimus-ide-collabsdk.Me, name, optimus-ide-collabsdk.UpdateUserSkillRequest{
	...
	t.Run("DeleteEmitsLog", func(t *testing.T) {
		...
		require.NoError(t, err)
		require.NoError(t, member.DeleteUserSkill(ctx, optimus-ide-collabsdk.Me, name))
	...
	t.Run("ReadsDoNotEmitLogs", func(t *testing.T) {
		...
		_, err := member.CreateUserSkill(ctx, optimus-ide-collabsdk.Me, optimus-ide-collabsdk.CreateUserSkillRequest{
			Content: userSkillMarkdown(name, "Read", "Body."),
		})
		require.NoError(t, err)
		auditor.ResetLogs()

		_, err = member.UserSkills(ctx, optimus-ide-collabsdk.Me)
		require.NoError(t, err)
		_, err = member.UserSkillByName(ctx, optimus-ide-collabsdk.Me, name)
		require.NoError(t, err)
	...
	t.Run("MissingSkillFailuresDoNotEmitLogs", func(t *testing.T) {
		...
		_, err := member.UpdateUserSkill(ctx, optimus-ide-collabsdk.Me, "missing-audit-skill", optimus-ide-collabsdk.UpdateUserSkillRequest{
			Content: userSkillMarkdown("missing-audit-skill", "Missing", "Body."),
		})
		requireSDKErrorStatus(t, err, http.StatusNotFound)
		err = member.DeleteUserSkill(ctx, optimus-ide-collabsdk.Me, "missing-audit-skill")
		requireSDKErrorStatus(t, err, http.StatusNotFound)
		assert.Empty(t, auditor.AuditLogs())
	})
}
...
func requireSDKErrorStatus(t *testing.T, err error, status int, msgAndArgs ...any) *optimus-ide-collabsdk.Error {
	t.Helper()
	require.Error(t, err, msgAndArgs...)
	sdkErr := optimus-ide-collabdtest.SDKError(t, err)
