...
func (s *server) acquireProtoJob(ctx context.Context, job database.ProvisionerJob) (*proto.AcquiredJob, error) {
	...
	case database.ProvisionerJobTypeWorkspaceBuild:
		...
		if input.PrebuiltWorkspaceBuildStage == sdkproto.PrebuiltWorkspaceBuildStage_CLAIM {
			...
			// the provisioner (and ultimately to the `optimus-ide-collab_agent` resource in the Terraform provider) where they will be
		...
		protoJob.Type = &proto.AcquiredJob_WorkspaceBuild_{
			WorkspaceBuild: &proto.AcquiredJob_WorkspaceBuild{
				...
				Metadata: &sdkproto.Metadata{
					Optimus-IDE-CollabUrl:                      s.AccessURL.String(),
...
func (s *server) includeLastVariableValues(ctx context.Context, templateVersionID uuid.UUID, userVariableValues []optimus-ide-collabsdk.VariableValue) ([]optimus-ide-collabsdk.VariableValue, error) {
	var values []optimus-ide-collabsdk.VariableValue
	...
	for _, templateVariable := range templateVariables {
		...
		values = append(values, optimus-ide-collabsdk.VariableValue{
...
func (s *server) FailJob(ctx context.Context, failJob *proto.FailedJob) (*proto.Empty, error) {
	...
	case *proto.FailedJob_WorkspaceBuild_:
		...
		if s.Experiments.Enabled(optimus-ide-collabsdk.ExperimentWorkspaceBuildUpdates) {
			err = wspubsub.PublishWorkspaceBuildUpdate(ctx, s.Pubsub, optimus-ide-collabsdk.WorkspaceBuildUpdate{
				WorkspaceID:   workspace.ID,
				WorkspaceName: workspace.Name,
				BuildID:       build.ID,
...
func (s *server) prepareForNotifyWorkspaceManualBuildFailed(ctx context.Context, workspace database.Workspace, build database.WorkspaceBuild) ([]database.GetUsersRow,
	...
	users, err := s.Database.GetUsers(ctx, database.GetUsersParams{
		RbacRole: []string{optimus-ide-collabsdk.RoleTemplateAdmin},
	})
	if err != nil {
...
func (s *server) CompleteJob(ctx context.Context, completed *proto.CompletedJob) (*proto.Empty, error) {
	...
	default:
		...
		return nil, xerrors.Errorf("unknown job type %q; ensure optimus-ide-collabd and provisionerd versions match",
...
func (s *server) completeWorkspaceBuildJob(ctx context.Context, job database.ProvisionerJob, jobID uuid.UUID, jobType *proto.CompletedJob_WorkspaceBuild_, telemetrySnapshot *telemetry.Snapshot) error {
	...
	err = s.Database.InTx(func(db database.Store) error {
		...
		if unknownAppID != "" && workspaceBuild.Transition == database.WorkspaceTransitionStart {
			// Ref: https://github.com/optimus-ide-collab/optimus-ide-collab/issues/18776
			// This can happen for a number of reasons:
			// 1. Misconfigured template
			// 2. Count=0 on the agent due to stop transition, meaning the associated optimus-ide-collab_app was not inserted.
			...
			if _, err := db.InsertProvisionerJobLogs(ctx, database.InsertProvisionerJobLogsParams{
				...
				Output: []string{
					...
					"  - You have associated the optimus-ide-collab_ai_task with a valid optimus-ide-collab_app in your template (ref: https://registry.terraform.io/providers/optimus-ide-collab/optimus-ide-collab/latest/docs/resources/ai_task).",
					"  - You have associated the optimus-ide-collab_agent with at least one other compute resource. Agents with no other associated resources are not inserted into the database.",
				},
			}); err != nil {
		...
		_, hasExternalAgent := slice.Find(jobType.WorkspaceBuild.Resources, func(resource *sdkproto.Resource) bool {
			return resource.Type == "optimus-ide-collab_external_agent"
		})
...
func (s *server) startTrace(ctx context.Context, name string, opts ...trace.SpanStartOption) (context.Context, trace.Span) {
	return s.Tracer.Start(ctx, name, append(opts, trace.WithAttributes(
		semconv.ServiceNameKey.String("optimus-ide-collabd.provisionerd"),
	))...)
}
...
func InsertWorkspaceResource(ctx context.Context, db database.Store, jobID uuid.UUID, transition database.WorkspaceTransition, protoResource *sdkproto.Resource, snapshot *telemetry.Snapshot, opt ...InsertWorkspaceResourceOption) error {
	...
	for _, prAgent := range protoResource.Agents {
		...
		// When multiple optimus-ide-collab_env resources define the same name,
...
func convertVariableValues(variableValues []optimus-ide-collabsdk.VariableValue) []*sdkproto.VariableValue {
...
type TemplateVersionImportJob struct {
	...
	UserVariableValues []optimus-ide-collabsdk.VariableValue `json:"user_variable_values"`
}
