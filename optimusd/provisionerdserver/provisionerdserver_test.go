...
func TestTokenIsRefreshedEarly(t *testing.T) {
	...
	t.Run("WithOptimus-IDE-Collabd", func(t *testing.T) {
		t.Parallel()
		tokenRefreshCount := 0
...
func TestTokenIsRefreshedEarlyWithoutOptimus-IDE-Collabd(t *testing.T) {
	t.Parallel()
	tokenRefreshCount := 0
	fake := oidctest.NewFakeIDP(t,
		oidctest.WithServing(),
		...
		for _, prebuiltWorkspaceBuildStage := range []sdkproto.PrebuiltWorkspaceBuildStage{
			...
			t.Run(tc.name+"_WorkspaceBuildJob_Stage"+prebuiltWorkspaceBuildStage.String(), func(t *testing.T) {
				...
				dv := &optimus-ide-collabsdk.DeploymentValues{
					Sessions: optimus-ide-collabsdk.SessionLifetime{
				...
				wantedMetadata := &sdkproto.Metadata{
					Optimus-IDE-CollabUrl:                      (&url.URL{}).String(),
...
func TestCompleteJob(t *testing.T) {
	...
	t.Run("AITasks", func(t *testing.T) {
		...
		// it is set to "true" if the template has any optimus-ide-collab_ai_task resources defined.
		t.Run("TemplateImport", func(t *testing.T) {
			...
			for _, tc := range []testcase{
				...
				t.Run(tc.name, func(t *testing.T) {
					...
					tvID := uuid.New()
					templateAdminUser := dbgen.User(t, db, database.User{RBACRoles: []string{optimus-ide-collabsdk.RoleTemplateAdmin}})
					template := dbgen.Template(t, db, database.Template{
						Name:           "template",
		...
		// it is set to "true" if the related template has any optimus-ide-collab_ai_task resources defined, and its sidebar app ID
		...
		t.Run("WorkspaceBuild", func(t *testing.T) {
			...
			for _, tc := range []testcase{
				...
				{
					name:       "has_ai_task is false even if there are optimus-ide-collab_ai_task resources, but no task_id",
					transition: database.WorkspaceTransitionStart,
					input: &proto.CompletedJob_WorkspaceBuild{
				...
				// Checks regression for https://github.com/optimus-ide-collab/optimus-ide-collab/issues/18776
				{
					...
					// the correct app ID you won't be able to communicate with it via Optimus-IDE-Collab.
					expectHasAiTask:  true,
					expectUsageEvent: true,
				},
				{
				...
				t.Run(tc.name, func(t *testing.T) {
					...
					importJobID := uuid.New()
					tvID := uuid.New()
					templateUser := dbgen.User(t, db, database.User{RBACRoles: []string{optimus-ide-collabsdk.RoleTemplateAdmin}})
					template := dbgen.Template(t, db, database.Template{
						Name:           "template",
	...
	t.Run("Devcontainers", func(t *testing.T) {
		...
		tests := []struct {
			...
			},
			{
				...
				// AuthInstanceID.
				// Context: https://github.com/optimus-ide-collab/optimus-ide-collab/pull/22196
...
func TestNotifications(t *testing.T) {
	...
	t.Run("Manual build failed, template admins notified", func(t *testing.T) {
		...
		templateAdmin := dbgen.User(t, db, database.User{RBACRoles: []string{optimus-ide-collabsdk.RoleTemplateAdmin}})
		_ /* other template admin, should not receive notification */ = dbgen.User(t, db, database.User{RBACRoles: []string{optimus-ide-collabsdk.RoleTemplateAdmin}})
...
func assertWorkspaceAppRebindWarning(t *testing.T, logSink *recordingSlogSink, want workspaceAppRebindWarning) {
	...
	for _, entry := range logSink.Entries() {
		...
		require.Contains(t, entry.File, "optimus-ide-collabd/provisionerdserver/provisionerdserver.go")
...
func setup(t *testing.T, ignoreLogErrors bool, ov *overrides) (proto.DRPCProvisionerDaemonServer, database.Store, pubsub.Pubsub, database.ProvisionerDaemon) {
	...
	serverDB := dbauthz.New(db, authorizer, logger, optimus-ide-collabdtest.AccessControlStorePointer())
	srv, err := provisionerdserver.NewServer(
		...
		optimus-ide-collabd.ReadExperiments(logger, deploymentValues.Experiments),
	)
	require.NoError(t, err)
...
func newFakeUsageInserter() (*optimus-ide-collabdtest.UsageInserter, *atomic.Pointer[usage.Inserter]) {
	poitr := &atomic.Pointer[usage.Inserter]{}
	fake := optimus-ide-collabdtest.NewUsageInserter()
...
// in-memory pipe and returns a connected client, mirroring how optimus-ide-collabd serves
