...
func TestWorkspaceAgent(t *testing.T) {
	...
	t.Run("Connect", func(t *testing.T) {
		...
		tmpDir := t.TempDir()
		anotherClient, anotherUser := optimus-ide-collabdtest.CreateAnotherUser(t, client, user.OrganizationID)
	...
	t.Run("Timeout", func(t *testing.T) {
		...
		testutil.Eventually(ctx, t, func(ctx context.Context) (done bool) {
			...
			return workspace.LatestBuild.Resources[0].Agents[0].Status == optimus-ide-collabsdk.WorkspaceAgentTimeout
	...
	t.Run("DisplayApps", func(t *testing.T) {
		...
		require.NoError(t, err)
		expectedApps := []optimus-ide-collabsdk.DisplayApp{
			optimus-ide-collabsdk.DisplayAppPortForward,
			optimus-ide-collabsdk.DisplayAppSSH,
			optimus-ide-collabsdk.DisplayAppVSCodeDesktop,
			optimus-ide-collabsdk.DisplayAppVSCodeInsiders,
			optimus-ide-collabsdk.DisplayAppWebTerminal,
		}
...
func TestWorkspaceAgentLogs(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		var logChunk []optimus-ide-collabsdk.WorkspaceAgentLog
		select {
		case <-ctx.Done():
		case logChunk = <-logs:
		}
	...
	t.Run("Close logs on outdated build", func(t *testing.T) {
		...
		_ = optimus-ide-collabdtest.CreateWorkspaceBuild(t, client, workspace, database.WorkspaceTransitionStart)
	...
	t.Run("PublishesOnOverflow", func(t *testing.T) {
		...
		for {
			var update optimus-ide-collabsdk.Workspace
			select {
			case <-ctx.Done():
				t.FailNow()
			case update = <-updates:
...
func TestWorkspaceAgentAppStatus(t *testing.T) {
	...
	client, db := optimus-ide-collabdtest.NewWithDatabase(t, nil)
	user := optimus-ide-collabdtest.CreateFirstUser(t, client)
	client, user2 := optimus-ide-collabdtest.CreateAnotherUser(t, client, user.OrganizationID)
...
func TestWorkspaceAgentAppStatus_ActivityBump(t *testing.T) {
	...
	tests := []struct {
		name       string
		prevState  *optimus-ide-collabsdk.WorkspaceAppStatusState // nil means no previous state
		newState   optimus-ide-collabsdk.WorkspaceAppStatusState
		shouldBump bool
	}{
		{
			name:       "FirstStatusBumps",
			prevState:  nil,
			newState:   optimus-ide-collabsdk.WorkspaceAppStatusStateWorking,
			shouldBump: true,
		},
		{
			name:       "WorkingToIdleBumps",
			prevState:  ptr.Ref(optimus-ide-collabsdk.WorkspaceAppStatusStateWorking),
			newState:   optimus-ide-collabsdk.WorkspaceAppStatusStateIdle,
			shouldBump: true,
		},
		{
			name:       "WorkingToCompleteBumps",
			prevState:  ptr.Ref(optimus-ide-collabsdk.WorkspaceAppStatusStateWorking),
			newState:   optimus-ide-collabsdk.WorkspaceAppStatusStateComplete,
			shouldBump: true,
		},
		{
			name:       "CompleteToIdleNoBump",
			prevState:  ptr.Ref(optimus-ide-collabsdk.WorkspaceAppStatusStateComplete),
			newState:   optimus-ide-collabsdk.WorkspaceAppStatusStateIdle,
			shouldBump: false,
		},
		{
		...
		},
		{
			...
			prevState:  ptr.Ref(optimus-ide-collabsdk.WorkspaceAppStatusStateFailure),
			newState:   optimus-ide-collabsdk.WorkspaceAppStatusStateIdle,
			shouldBump: false,
		},
		{
			...
			prevState:  ptr.Ref(optimus-ide-collabsdk.WorkspaceAppStatusStateFailure),
			newState:   optimus-ide-collabsdk.WorkspaceAppStatusStateFailure,
			shouldBump: false,
		},
		{
		...
		},
		{
			...
			prevState:  ptr.Ref(optimus-ide-collabsdk.WorkspaceAppStatusStateIdle),
			newState:   optimus-ide-collabsdk.WorkspaceAppStatusStateIdle,
			shouldBump: false,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := client.UpdateTemplateMeta(ctx, r.Template.ID, optimus-ide-collabsdk.UpdateTemplateMeta{
...
func TestWorkspaceAgentConnectRPC(t *testing.T) {
	...
	t.Run("Connect", func(t *testing.T) {
		...
		for _, tc := range []struct {
			...
			t.Run(tc.name, func(t *testing.T) {
				...
				resources := optimus-ide-collabdtest.NewWorkspaceAgentWaiter(t, client, r.Workspace.ID).AgentNames([]string{}).Wait()
	...
	t.Run("FailNonLatestBuild", func(t *testing.T) {
		...
		version = optimus-ide-collabdtest.UpdateTemplateVersion(t, client, user.OrganizationID, &echo.Responses{
			Parse:         echo.ParseComplete,
			ProvisionPlan: echo.PlanComplete,
...
func TestWorkspaceAgentTailnet(t *testing.T) {
	...
	_ = agenttest.New(t, client.URL, r.AgentToken)
	resources := optimus-ide-collabdtest.AwaitWorkspaceAgents(t, client, r.Workspace.ID)
...
func TestWorkspaceAgentClientCoordinate_BadVersion(t *testing.T) {
	...
	resp, err := client.Request(ctx, http.MethodGet,
		...
		nil,
		optimus-ide-collabsdk.WithQueryParam("version", "99.99"))
	require.NoError(t, err)
...
func TestWorkspaceAgentClientCoordinate_ResumeToken(t *testing.T) {
	...
	t.Run("OK", func(t *testing.T) {
		...
		client, closer, api := optimus-ide-collabdtest.NewWithAPI(t, &optimus-ide-collabdtest.Options{
...
// returned.
func connectToCoordinatorAndFetchResumeToken(ctx context.Context, logger slog.Logger, sdkClient *optimus-ide-collabsdk.Client, agentID uuid.UUID, resumeToken string) (string, error) {
	...
	wsConn, resp, err := websocket.Dial(ctx, u.String(), &websocket.DialOptions{
		HTTPHeader: http.Header{
			"Optimus-IDE-Collab-Session-Token": []string{sdkClient.SessionToken()},
		},
	})
	if err != nil {
...
func TestWorkspaceAgentTailnetDirectDisabled(t *testing.T) {
	...
	err = json.NewDeoptimus-ide-collab(res.Body).Decode(&connInfo)
	require.NoError(t, err)
...
func (g *fakeListeningPortsGetter) GetListeningPorts() ([]optimus-ide-collabsdk.WorkspaceAgentListeningPort, error) {
	g.Lock()
	defer g.Unlock()
	return slices.Clone(g.ports), nil
}

func (g *fakeListeningPortsGetter) setPorts(ports ...optimus-ide-collabsdk.WorkspaceAgentListeningPort) {
	g.Lock()
	defer g.Unlock()
...
func TestWorkspaceAgentListeningPorts(t *testing.T) {
	...
	testPort := optimus-ide-collabsdk.WorkspaceAgentListeningPort{
		Network:     "tcp",
		ProcessName: "test-app",
		Port:        44762,
	}
	filteredPort := optimus-ide-collabsdk.WorkspaceAgentListeningPort{
		Network:     "tcp",
		ProcessName: "postgres",
		Port:        5432,
	}

	setup := func(t *testing.T, apps []*proto.App, dv *optimus-ide-collabsdk.DeploymentValues) (*optimus-ide-collabsdk.Client, uuid.UUID, *fakeListeningPortsGetter) {
		t.Helper()
		...
		resources := optimus-ide-collabdtest.NewWorkspaceAgentWaiter(t, client, r.Workspace.ID).Wait()
	...
	for _, tc := range []struct {
		name  string
		setDV func(t *testing.T, dv *optimus-ide-collabsdk.DeploymentValues)
	}{
		{
			name:  "Mainline",
			setDV: func(*testing.T, *optimus-ide-collabsdk.DeploymentValues) {},
		},
		{
			name: "BlockDirect",
			setDV: func(t *testing.T, dv *optimus-ide-collabsdk.DeploymentValues) {
		...
		t.Run("OK_"+tc.name, func(t *testing.T) {
			...
			require.NoError(t, err)
			require.Equal(t, []optimus-ide-collabsdk.WorkspaceAgentListeningPort{testPort}, res.Ports)
...
func TestWorkspaceAgentContainers(t *testing.T) {
	...
	// OPTIMUS-IDE-COLLAB_TEST_USE_DOCKER=1 go test -run TestWorkspaceAgentContainers/Docker ./optimus-ide-collabd
	t.Run("Docker", func(t *testing.T) {
		t.Parallel()
		if ctud, ok := os.LookupEnv("OPTIMUS-IDE-COLLAB_TEST_USE_DOCKER"); !ok || ctud != "1" {
			t.Skip("Set OPTIMUS-IDE-COLLAB_TEST_USE_DOCKER=1 to run this test")
		}

		pool, err := dockertest.NewPool("")
		require.NoError(t, err, "Could not connect to docker")
		testLabels := map[string]string{
			"com.optimus-ide-collab.test":  uuid.New().String(),
			"com.optimus-ide-collab.empty": "",
		}
		...
		ct2, err := pool.RunWithOptions(&dockertest.RunOptions{
			...
			Labels: map[string]string{
				"com.optimus-ide-collab.test":  "ignoreme",
				"com.optimus-ide-collab.empty": "",
			},
		}, func(config *docker.HostConfig) {
			config.AutoRemove = true
	...
	t.Run("Mock", func(t *testing.T) {
		...
		testResponse := optimus-ide-collabsdk.WorkspaceAgentListContainersResponse{
		...
		for _, tc := range []struct {
			name      string
			setupMock func(*acmock.MockContainerCLI) (optimus-ide-collabsdk.WorkspaceAgentListContainersResponse, error)
		}{
			{
				name: "test response",
				setupMock: func(mcl *acmock.MockContainerCLI) (optimus-ide-collabsdk.WorkspaceAgentListContainersResponse, error) {
			...
			},
			{
				name: "error response",
				setupMock: func(mcl *acmock.MockContainerCLI) (optimus-ide-collabsdk.WorkspaceAgentListContainersResponse, error) {
					mcl.EXPECT().List(gomock.Any()).Return(optimus-ide-collabsdk.WorkspaceAgentListContainersResponse{}, assert.AnError).AnyTimes()
					return optimus-ide-collabsdk.WorkspaceAgentListContainersResponse{}, assert.AnError
				},
			},
		} {
			t.Run(tc.name, func(t *testing.T) {
...
func TestWatchWorkspaceAgentDevcontainers(t *testing.T) {
	...
	t.Run("OK", func(t *testing.T) {
		...
		var (
			...
			client, db = optimus-ide-collabdtest.NewWithDatabase(t, &optimus-ide-collabdtest.Options{Logger: &logger})
			user       = optimus-ide-collabdtest.CreateFirstUser(t, client)
			r          = dbfake.WorkspaceBuild(t, db, database.WorkspaceTable{
				OrganizationID: user.OrganizationID,
				OwnerID:        user.UserID,
			...
			fakeContainer1 = optimus-ide-collabsdk.WorkspaceAgentContainer{
				ID:           "container1",
				CreatedAt:    dbtime.Now(),
			...
			fakeContainer2 = optimus-ide-collabsdk.WorkspaceAgentContainer{
				ID:           "container1",
				CreatedAt:    dbtime.Now(),
		...
		mCCLI.EXPECT().List(gomock.Any()).Return(optimus-ide-collabsdk.WorkspaceAgentListContainersResponse{Containers: stages[0].containers}, nil)
		...
		for i, stage := range stages[1:] {
			mCCLI.EXPECT().List(gomock.Any()).Return(optimus-ide-collabsdk.WorkspaceAgentListContainersResponse{Containers: stage.containers}, nil)
...
func TestWorkspaceAgentRecreateDevcontainer(t *testing.T) {
	...
	t.Run("Mock", func(t *testing.T) {
		...
		var (
			...
			devContainer = optimus-ide-collabsdk.WorkspaceAgentContainer{
				ID:           uuid.NewString(),
				CreatedAt:    dbtime.Now(),
				FriendlyName: testutil.GetRandomName(t),
				Image:        "busybox:latest",
			...
			devcontainer = optimus-ide-collabsdk.WorkspaceAgentDevcontainer{
				ID:              devcontainerID,
		...
		for _, tc := range []struct {
		...
		}{
			{
				...
				devcontainers:  []optimus-ide-collabsdk.WorkspaceAgentDevcontainer{devcontainer},
				containers:     []optimus-ide-collabsdk.WorkspaceAgentContainer{devContainer},
				expectRecreate: true,
			},
			{
			...
			t.Run(tc.name, func(t *testing.T) {
				...
				if tc.expectErrorCode > 0 {
					...
					require.True(t, ok, "expected error to be a optimus-ide-collab error")
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			var (
				...
				_, workspaceOwner      = optimus-ide-collabdtest.CreateAnotherUser(t, client, admin.OrganizationID)
				templateAdminClient, _ = optimus-ide-collabdtest.CreateAnotherUser(t, client, admin.OrganizationID, tc.role(admin.OrganizationID))
	...
	setupDevcontainerMocks := func(t *testing.T) (
		...
		*acmock.MockDevcontainerCLI,
		...
		devcontainerAPIOptions := []agentcontainers.Option{
			...
			agentcontainers.WithDevcontainerCLI(mDCCLI),
			agentcontainers.WithWatcher(watcher.NewNoop()),
			agentcontainers.WithDevcontainers([]optimus-ide-collabsdk.WorkspaceAgentDevcontainer{devcontainer}, nil),
		}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.useAnotherUser {
				testClient, _ = optimus-ide-collabdtest.CreateAnotherUser(t, client, user.OrganizationID)
			}
...
func TestWorkspaceAgentAppHealth(t *testing.T) {
	...
	manifest := requireGetManifest(ctx, t, aAPI)
	require.EqualValues(t, optimus-ide-collabsdk.WorkspaceAppHealthDisabled, manifest.Apps[0].Health)
	require.EqualValues(t, optimus-ide-collabsdk.WorkspaceAppHealthInitializing, manifest.Apps[1].Health)
	// empty
	...
	require.NoError(t, err)
	manifest = requireGetManifest(ctx, t, aAPI)
	require.EqualValues(t, optimus-ide-collabsdk.WorkspaceAppHealthHealthy, manifest.Apps[1].Health)
	// update to unhealthy
	_, err = aAPI.BatchUpdateAppHealths(ctx, &agentproto.BatchUpdateAppHealthRequest{
		Updates: []*agentproto.BatchUpdateAppHealthRequest_HealthUpdate{
			{
	...
	require.NoError(t, err)
	manifest = requireGetManifest(ctx, t, aAPI)
	require.EqualValues(t, optimus-ide-collabsdk.WorkspaceAppHealthUnhealthy, manifest.Apps[1].Health)
}
	...
	t.Run("Set", func(t *testing.T) {
		...
		for _, res := range workspace.LatestBuild.Resources {
			for _, a := range res.Agents {
				require.Equal(t, optimus-ide-collabsdk.WorkspaceAgentLifecycleCreated, a.LifecycleState)
			}
		}
		...
		tests := []struct {
			state   optimus-ide-collabsdk.WorkspaceAgentLifecycle
			wantErr bool
		}{
			{optimus-ide-collabsdk.WorkspaceAgentLifecycleCreated, false},
			{optimus-ide-collabsdk.WorkspaceAgentLifecycleStarting, false},
			{optimus-ide-collabsdk.WorkspaceAgentLifecycleStartTimeout, false},
			{optimus-ide-collabsdk.WorkspaceAgentLifecycleStartError, false},
			{optimus-ide-collabsdk.WorkspaceAgentLifecycleReady, false},
			{optimus-ide-collabsdk.WorkspaceAgentLifecycleShuttingDown, false},
			{optimus-ide-collabsdk.WorkspaceAgentLifecycleShutdownTimeout, false},
			{optimus-ide-collabsdk.WorkspaceAgentLifecycleShutdownError, false},
			{optimus-ide-collabsdk.WorkspaceAgentLifecycleOff, false},
			{optimus-ide-collabsdk.WorkspaceAgentLifecycle("nonexistent_state"), true},
			{optimus-ide-collabsdk.WorkspaceAgentLifecycle(""), true},
		}
...
func TestWorkspaceAgent_Metadata(t *testing.T) {
	...
	post := func(ctx context.Context, key string, mr optimus-ide-collabsdk.WorkspaceAgentMetadataResult) {
	...
	var update []optimus-ide-collabsdk.WorkspaceAgentMetadata

	wantMetadata1 := optimus-ide-collabsdk.WorkspaceAgentMetadataResult{
	...
	recvUpdate := func() []optimus-ide-collabsdk.WorkspaceAgentMetadata {
		select {
		case <-ctx.Done():
	...
	check := func(want optimus-ide-collabsdk.WorkspaceAgentMetadataResult, got optimus-ide-collabsdk.WorkspaceAgentMetadata, retry bool) {
...
func TestWorkspaceAgent_Metadata_CatchMemoryLeak(t *testing.T) {
	...
	logger := slogtest.Make(t, &slogtest.Options{IgnoreErrors: true}).Named("optimus-ide-collabd").Leveled(slog.LevelDebug)
	client := optimus-ide-collabdtest.New(t, &optimus-ide-collabdtest.Options{
		Database:                 db,
		Pubsub:                   psub,
		IncludeProvisionerDaemon: true,
	...
	post := func(ctx context.Context, key, value string) error {
		_, err := aAPI.BatchUpdateMetadata(ctx, &agentproto.BatchUpdateMetadataRequest{
			Metadata: []*agentproto.Metadata{
				{
					Key: key,
					Result: agentsdk.ProtoFromMetadataResult(optimus-ide-collabsdk.WorkspaceAgentMetadataResult{
						CollectedAt: time.Now(),
						Value:       value,
					}),
				},
...
func TestWorkspaceAgent_Startup(t *testing.T) {
	...
	t.Run("OK", func(t *testing.T) {
		...
		var (
			...
			expectedDir        = "/home/optimus-ide-collab"
			expectedSubsystems = []optimus-ide-collabsdk.AgentSubsystem{
				optimus-ide-collabsdk.AgentSubsystemEnvbox,
				optimus-ide-collabsdk.AgentSubsystemExectrace,
			}
		)
...
// TestWorkspaceAgent_UpdatedDERP runs a real optimus-ide-collabd server, with a real agent
		...
		ownerClient, db := optimus-ide-collabdtest.NewWithDatabase(t, &optimus-ide-collabdtest.Options{
			...
			ExternalAuthConfigs: []*externalauth.Config{
				fake.ExternalAuthConfig(t, providerID, nil, func(cfg *externalauth.Config) {
					cfg.Type = optimus-ide-collabsdk.EnhancedExternalAuthProviderGitLab.String()
				}),
			},
		})
		first := optimus-ide-collabdtest.CreateFirstUser(t, ownerClient)
		tmpDir := t.TempDir()
		client, user := optimus-ide-collabdtest.CreateAnotherUser(t, ownerClient, first.OrganizationID)
...
func TestOwnedWorkspacesCoordinate(t *testing.T) {
	...
	ctx := testutil.Context(t, testutil.WaitLong)
	logger := testutil.Logger(t)
	firstClient, _, api := optimus-ide-collabdtest.NewWithAPI(t, &optimus-ide-collabdtest.Options{
		Coordinator: tailnet.NewCoordinator(logger),
	})
	firstUser := optimus-ide-collabdtest.CreateFirstUser(t, firstClient)
	member, memberUser := optimus-ide-collabdtest.CreateAnotherUser(t, firstClient, firstUser.OrganizationID, rbac.RoleTemplateAdmin())
	...
	wsConn, resp, err := websocket.Dial(ctx, u.String(), &websocket.DialOptions{
		HTTPHeader: http.Header{
			"Optimus-IDE-Collab-Session-Token": []string{member.SessionToken()},
		},
	})
	if err != nil {
...
func TestUserTailnetTelemetry(t *testing.T) {
	...
	telemetryData := &optimus-ide-collabsdk.Optimus-IDE-CollabDesktopTelemetry{
		DeviceOS:            "Windows",
		DeviceID:            "device001",
		Optimus-IDE-CollabDesktopVersion: "0.22.1",
	}
	...
	testCases := []struct {
		...
		// only used for DeviceID, DeviceOS, Optimus-IDE-CollabDesktopVersion
		...
		},
		{
			name: "full header",
			headers: map[string]string{
				optimus-ide-collabsdk.Optimus-IDE-CollabDesktopTelemetryHeader: string(fullHeader),
			},
			expected: telemetry.UserTailnetConnection{
				DeviceOS:            ptr.Ref("Windows"),
				DeviceID:            ptr.Ref("device001"),
				Optimus-IDE-CollabDesktopVersion: ptr.Ref("0.22.1"),
			},
		},
		{
			name: "empty header",
			headers: map[string]string{
				optimus-ide-collabsdk.Optimus-IDE-CollabDesktopTelemetryHeader: "",
			},
			expected: telemetry.UserTailnetConnection{},
		},
		{
			name: "invalid header",
			headers: map[string]string{
				optimus-ide-collabsdk.Optimus-IDE-CollabDesktopTelemetryHeader: "{\"device_os",
			},
			expected: telemetry.UserTailnetConnection{},
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			firstClient := optimus-ide-collabdtest.New(t, &optimus-ide-collabdtest.Options{
				Logger:            &logger,
			...
			requireEqualOrBothNil(t, telemetryConnection.Optimus-IDE-CollabDesktopVersion, tc.expected.Optimus-IDE-CollabDesktopVersion)
...
func buildWorkspaceWithAgent(
	...
	_ = agenttest.New(t, client.URL, r.AgentToken)
	optimus-ide-collabdtest.NewWorkspaceAgentWaiter(t, client, r.Workspace.ID).Wait()
	return r.Workspace
}
...
func TestAgentConnectionInfo(t *testing.T) {
	...
	client, db := optimus-ide-collabdtest.NewWithDatabase(t, &optimus-ide-collabdtest.Options{DeploymentValues: dv})
	user := optimus-ide-collabdtest.CreateFirstUser(t, client)
	r := dbfake.WorkspaceBuild(t, db, database.WorkspaceTable{
		OrganizationID: user.OrganizationID,
		OwnerID:        user.UserID,
...
func TestWorkspaceAgentsExternalAuthExpiresAt(t *testing.T) {
	...
	// newSetup creates a optimus-ide-collabdtest server with a minimal external-auth
	...
	newSetup := func(t *testing.T) (agentToken string, agentClient *agentsdk.Client, db database.Store, ownerID uuid.UUID) {
		...
		first := optimus-ide-collabdtest.CreateFirstUser(t, ownerClient)
		_, user := optimus-ide-collabdtest.CreateAnotherUser(t, ownerClient, first.OrganizationID)
