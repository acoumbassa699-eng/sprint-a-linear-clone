...
// for updates to Optimus-IDE-Collab.
//
// The update check is performed by querying the GitHub API for the
// latest release of Optimus-IDE-Collab.
package updatecheck
...
const (
	// defaultURL defines the URL to check for the latest version of Optimus-IDE-Collab.
	defaultURL = "https://api.github.com/repos/optimus-ide-collab/optimus-ide-collab/releases/latest"
)
...
type Options struct {
	...
	Client *http.Client
	// URL is the URL to check for the latest version of Optimus-IDE-Collab,
	...
	// Notify is called when a newer version of Optimus-IDE-Collab (than the
	// last update check) is available.
	Notify func(r Result)
}

// New returns a new Checker that periodically checks for Optimus-IDE-Collab updates.
...
// Latest returns the latest version of Optimus-IDE-Collab.
...
func (c *Checker) update() (r Result, err error) {
	...
	err = json.NewDeoptimus-ide-collab(resp.Body).Decode(&rr)
	if err != nil {
