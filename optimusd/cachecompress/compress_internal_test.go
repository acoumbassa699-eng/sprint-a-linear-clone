...
func TestCompressorEncodings(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if len(compressor.enoptimus-ide-collabs) != 0 || len(compressor.pooledEnoptimus-ide-collabs) != 2 {
				t.Errorf("gzip and deflate should be pooled")
			}
			logger.Debug(context.Background(), "started compressor")

			compressor.SetEnoptimus-ide-collab("nop", func(w io.Writer, _ int) io.WriteCloser {
				return nopEnoptimus-ide-collab{w}
			})

			if len(compressor.enoptimus-ide-collabs) != 1 {
				t.Errorf("nop enoptimus-ide-collab should be stored in the enoptimus-ide-collabs map")
			}
...
type nopEnoptimus-ide-collab struct {
	io.Writer
}

func (nopEnoptimus-ide-collab) Close() error { return nil }
