...
type Compressor struct {
	logger slog.Logger
	// The mapping of enoptimus-ide-collab names to enoptimus-ide-collab functions.
	enoptimus-ide-collabs map[string]Enoptimus-ide-collabFunc
	// The mapping of pooled enoptimus-ide-collabs to pools.
	pooledEnoptimus-ide-collabs map[string]*sync.Pool
	// The list of enoptimus-ide-collabs in order of decreasing precedence.
...
func NewCompressor(logger slog.Logger, level int, cacheDir string, orig http.FileSystem) *Compressor {
	c := &Compressor{
		...
		level:          level,
		enoptimus-ide-collabs:       make(map[string]Enoptimus-ide-collabFunc),
		pooledEnoptimus-ide-collabs: make(map[string]*sync.Pool),
		cacheDir:       cacheDir,
		orig:           orig,
		cache:          make(map[cacheKey]ref),
	}

	// Set the default enoptimus-ide-collabs.  The precedence order uses the reverse
	// ordering that the enoptimus-ide-collabs were added. This means adding new enoptimus-ide-collabs
	...
	c.SetEnoptimus-ide-collab("deflate", enoptimus-ide-collabDeflate)

	// TODO: Exception for old MSIE browsers that can't handle non-HTML?
	// https://zoompf.com/blog/2012/02/lose-the-wait-http-compression
	c.SetEnoptimus-ide-collab("gzip", enoptimus-ide-collabGzip)
...
// SetEnoptimus-ide-collab can be used to set the implementation of a compression algorithm.
//
...
//	compressor.SetEnoptimus-ide-collab("br", func(w io.Writer, level int) io.Writer {
//		params := brotli_enc.NewBrotliParams()
//		params.SetQuality(level)
//		return brotli_enc.NewBrotliWriter(params, w)
//	})
func (c *Compressor) SetEnoptimus-ide-collab(encoding string, fn Enoptimus-ide-collabFunc) {
	encoding = strings.ToLower(encoding)
	if encoding == "" {
		panic("the encoding can not be empty")
	}
	if fn == nil {
		panic("attempted to set a nil enoptimus-ide-collab function")
	}

	// If we are adding a new enoptimus-ide-collab that is already registered, we have to
	// clear that one out first.
	delete(c.pooledEnoptimus-ide-collabs, encoding)
	delete(c.enoptimus-ide-collabs, encoding)

	// If the enoptimus-ide-collab supports Resetting (IoReseterWriter), then it can be pooled.
	enoptimus-ide-collab := fn(io.Discard, c.level)
	if _, ok := enoptimus-ide-collab.(ioResetterWriter); ok {
		pool := &sync.Pool{
			New: func() interface{} {
				return fn(io.Discard, c.level)
			},
		}
		c.pooledEnoptimus-ide-collabs[encoding] = pool
	}
	// If the enoptimus-ide-collab is not in the pooledEnoptimus-ide-collabs, add it to the normal enoptimus-ide-collabs.
	if _, ok := c.pooledEnoptimus-ide-collabs[encoding]; !ok {
		c.enoptimus-ide-collabs[encoding] = fn
	}
...
func (c *Compressor) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	encoding := c.selectEnoptimus-ide-collab(r.Header)
...
func (c *Compressor) compress(ctx context.Context, encoding string, cref ref, r *http.Request) {
	cachePath := cref.key.filePath(c.cacheDir)
	var err error
	// we want to handle closing either cref.done or cref.err in a defer at the bottom of the stack so that the enoptimus-ide-collab
	...
	defer cacheFile.Close()
	enoptimus-ide-collab, cleanup := c.getEnoptimus-ide-collab(encoding, cacheFile)
	if enoptimus-ide-collab == nil {
		// can only hit this if there is a programming error
		c.logger.Critical(ctx, "got nil enoptimus-ide-collab", slog.F("encoding", encoding))
		err = xerrors.New("nil enoptimus-ide-collab")
		return
	}
	defer cleanup()
	defer enoptimus-ide-collab.Close() // ensures we flush, needs to be called before cleanup(), so we defer after it.

	cw := &compressResponseWriter{
		w:       enoptimus-ide-collab,
...
// selectEnoptimus-ide-collab returns the name of the enoptimus-ide-collab
func (c *Compressor) selectEnoptimus-ide-collab(h http.Header) string {
	header := h.Get("Accept-Encoding")

	// Parse the names of all accepted algorithms from the header.
	accepted := strings.Split(strings.ToLower(header), ",")

	// Find supported enoptimus-ide-collab by accepted list by precedence
	...
	// No enoptimus-ide-collab found to match the accepted encoding
	return ""
}

// getEnoptimus-ide-collab returns a writer that encodes and writes to the provided writer, and a cleanup func.
func (c *Compressor) getEnoptimus-ide-collab(name string, w io.Writer) (io.WriteCloser, func()) {
	if pool, ok := c.pooledEnoptimus-ide-collabs[name]; ok {
		enoptimus-ide-collab, typeOK := pool.Get().(ioResetterWriter)
		if !typeOK {
			return nil, nil
		}
		cleanup := func() {
			pool.Put(enoptimus-ide-collab)
		}
		enoptimus-ide-collab.Reset(w)
		return enoptimus-ide-collab, cleanup
	}
	if fn, ok := c.enoptimus-ide-collabs[name]; ok {
...
// An Enoptimus-ide-collabFunc is a function that wraps the provided io.Writer with a
// streaming compression algorithm and returns it.
//
// In case of failure, the function should return nil.
type Enoptimus-ide-collabFunc func(w io.Writer, level int) io.WriteCloser
...
func enoptimus-ide-collabGzip(w io.Writer, level int) io.WriteCloser {
...
func enoptimus-ide-collabDeflate(w io.Writer, level int) io.WriteCloser {
