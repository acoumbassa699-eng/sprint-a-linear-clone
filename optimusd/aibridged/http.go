...
var (
	...
	ErrConnect               = xerrors.New("could not connect to optimus-ide-collabd")
...
func (s *Server) ServeHTTP(rw http.ResponseWriter, r *http.Request) {
	...
	// aibridgeproxyd.
	if proxyReqID := r.Header.Get(agplaibridge.HeaderOptimus-IDE-CollabRequestID); proxyReqID != "" {
	...
	r.Header.Del(agplaibridge.HeaderOptimus-IDE-CollabRequestID)
	...
	// Strip every header that may carry the Optimus-IDE-Collab token so it is never
	...
	// headers, and we still want to scrub any Optimus-IDE-Collab-specific credentials
	// that may have leaked through. After stripping, the aibridge library
	// can treat the request as a normal LLM API call with no
	// Optimus-IDE-Collab-specific information.
	if byok {
		// In BYOK mode the Optimus-IDE-Collab token is in X-Optimus-IDE-Collab-AI-Governance-Token;
		// Authorization and X-Api-Key carry the user's own LLM
		// credentials and must be preserved.
		r.Header.Del(agplaibridge.HeaderOptimus-IDE-CollabToken)
	} else {
		// In centralized mode the Optimus-IDE-Collab token may be in Authorization
		// (the documented path) or X-Api-Key (legacy clients that set
		// ANTHROPIC_API_KEY to their Optimus-IDE-Collab token). Both are stripped.
	...
	if err != nil {
		logger.Warn(ctx, "failed to connect to optimus-ide-collabd", slog.Error(err))
