...
import (
	...
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/aibridge/aibridgetest"
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/aibridge/intercept"
	...
	mock "github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabd/aibridged/aibridgedmock"
...
func sdkError(status int, message string) error {
	return optimus-ide-collabsdk.ReadBodyAsError(&http.Response{
		StatusCode: status,
...
func TestServeHTTP_FailureModes(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			reqHeaders: map[string]string{
				optimus-ide-collabsdk.SessionTokenHeader: "key", // Optimus-IDE-Collab-Session-Token is not supported; requests originate with AI clients, not optimus-ide-collab CLI.
			},
		...
		// Optimus-IDE-Collabd connection-related failures.
		{
...
// LLM credentials must still be forwarded upstream while the Optimus-IDE-Collab governance
// token is stripped.
	...
	tests := []struct {
	...
	}{
		{
			...
			expectAbsent: []string{
				"Authorization",
				"X-Api-Key",
				agplaibridge.HeaderOptimus-IDE-CollabToken,
			},
		},
		{
			...
			reqHeaders: map[string]string{
				...
				// ExtractAuthToken prefers HeaderOptimus-IDE-CollabToken.
				agplaibridge.HeaderOptimus-IDE-CollabToken: "optimus-ide-collab-token-byok",
			...
			applyMocks: func(t *testing.T, client *mock.MockDRPCClient, pool *mock.MockPooler, mockH *mockHandler) {
				...
				pool.EXPECT().Acquire(gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any()).DoAndReturn(
					func(_ context.Context, req aibridged.Request, _ aibridged.ClientFunc, _ aibridged.MCPProxyBuilder) (http.Handler, error) {
						assert.Equal(t, "optimus-ide-collab-token-byok", req.SessionKey,
							"BYOK delegated request must still surface the extracted Optimus-IDE-Collab token as SessionKey")
						return mockH, nil
					})
			},
			expectStatus:  http.StatusOK,
...
// effect. The delegated key ID identifies the user (no Optimus-IDE-Collab token over the
// wire) while the user's own LLM credentials in Authorization must flow
// through to the downstream handler. The Optimus-IDE-Collab governance token, if set by
...
func TestServeHTTP_DelegatedAPIKey_BYOK_Integration(t *testing.T) {
	...
	require.NoError(t, err)
	// HeaderOptimus-IDE-CollabToken marks the request as BYOK. Its value is irrelevant on
	// the delegated path (identity comes from context) and it must be
	// stripped before forwarding upstream.
	req.Header.Set(agplaibridge.HeaderOptimus-IDE-CollabToken, "ignored-on-delegated-path")
	...
	require.Empty(t, mockH.headersReceived.Get(agplaibridge.HeaderOptimus-IDE-CollabToken),
		"Optimus-IDE-Collab governance token must be stripped before forwarding upstream")
}
...
func TestServeHTTP_StripOptimus-IDE-CollabToken(t *testing.T) {
	t.Parallel()
	...
	cases := []struct {
	...
	}{
		{
			...
			// but does not include HeaderOptimus-IDE-CollabToken.
		...
		{
			// BYOK with access token: Optimus-IDE-Collab token in BYOK header,
		...
		{
			// BYOK with personal API key: Optimus-IDE-Collab token in BYOK header,
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// HeaderOptimus-IDE-CollabToken should always be stripped
			require.Empty(t, mockH.headersReceived.Get(agplaibridge.HeaderOptimus-IDE-CollabToken),
				"header %q should be stripped", agplaibridge.HeaderOptimus-IDE-CollabToken)
		})
	}
}
...
func TestExtractAuthToken(t *testing.T) {
	...
	cases := []struct {
		...
		// BYOK: X-Optimus-IDE-Collab-AI-Governance-Token carries the Optimus-IDE-Collab
		// token and has the highest priority.
		{
			name:    "byok/empty",
			headers: map[string]string{agplaibridge.HeaderOptimus-IDE-CollabToken: ""},
		},
		{
			name:        "byok/ok",
			headers:     map[string]string{agplaibridge.HeaderOptimus-IDE-CollabToken: "optimus-ide-collab-token"},
			expectedKey: "optimus-ide-collab-token",
		},
		{
...
// by optimus-ide-collab/aibridge's handling logic in a provider-specific manner.
// We must validate that logic that pertains to optimus-ide-collab/optimus-ide-collab is exercised.
// aibridge will only handle certain routes; we don't need to test these exhaustively
// (that's optimus-ide-collab/aibridge's responsibility), but we do need to validate that it handles
// requests correctly.
func TestRouting(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// the incoming request, and anything beyond that is the responsibility of optimus-ide-collab/aibridge to test.
...
// TestServeHTTP_StripInternalHeaders verifies that internal X-Optimus-IDE-Collab-*
...
func TestServeHTTP_StripInternalHeaders(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name:   "X-Optimus-IDE-Collab-AI-Governance-Token",
			header: agplaibridge.HeaderOptimus-IDE-CollabToken,
			value:  "optimus-ide-collab-token",
		},
		{
			name:   "X-Optimus-IDE-Collab-AI-Governance-Request-Id",
			header: agplaibridge.HeaderOptimus-IDE-CollabRequestID,
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			req.Header.Set("Authorization", "Bearer optimus-ide-collab-token")
			...
			// Assert no X-Optimus-IDE-Collab-* headers were forwarded upstream.
			for name := range mockH.headersReceived {
				require.NotContains(t, name, "X-Optimus-IDE-Collab-",
