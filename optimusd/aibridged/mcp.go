...
const (
	InternalMCPServerID = "optimus-ide-collab"
)

// Deprecated: Injected MCP in AI Bridge is deprecated and will be removed in a future release.
type MCPProxyBuilder interface {
	// Build creates a [mcp.ServerProxier] for the given request initiator.
	// At minimum, the Optimus-IDE-Collab MCP server will be proxied.
	// The SessionKey from [Request] is used to authenticate against the Optimus-IDE-Collab MCP server.
	//
...
func (m *MCPProxyFactory) retrieveMCPServerConfigs(ctx context.Context, req Request) (map[string]mcp.ServerProxier, error) {
	...
	proxiers := make(map[string]mcp.ServerProxier, len(mcpSrvCfgs.GetExternalAuthMcpConfigs())+1) // Extra one for Optimus-IDE-Collab MCP server.

	if mcpSrvCfgs.GetOptimus-IDE-CollabMcpConfig() != nil {
		// Delegated callers (e.g., chatd) do not hold the user's API key
		// secret and so cannot authenticate against the Optimus-IDE-Collab MCP server.
		// Skip the proxy in that case rather than attempting a connection
		// with an empty bearer token, which will fail upstream.
		if req.SessionKey == "" {
			m.logger.Debug(ctx, "skipping Optimus-IDE-Collab MCP server proxy: no session key (delegated request)", slog.F("mcp_server_id", mcpSrvCfgs.GetOptimus-IDE-CollabMcpConfig().GetId()))
		} else {
			// Setup the Optimus-IDE-Collab MCP server proxy.
			optimus-ide-collabMCPProxy, err := m.newStreamableHTTPServerProxy(mcpSrvCfgs.GetOptimus-IDE-CollabMcpConfig(), req.SessionKey) // The session key is used to auth against our internal MCP server.
			if err != nil {
				m.logger.Warn(ctx, "failed to create MCP server proxy", slog.F("mcp_server_id", mcpSrvCfgs.GetOptimus-IDE-CollabMcpConfig().GetId()), slog.Error(err))
			} else {
				proxiers[InternalMCPServerID] = optimus-ide-collabMCPProxy
			}
		}
	}
