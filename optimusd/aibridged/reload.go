...
// WatchProviderReload opens a optimus-ide-collabd WatchAIProviders stream via clientFn and
...
// the daemon to connect to optimus-ide-collabd) unblocks when ctx is canceled, leaving no
// goroutine behind.
...
func watchProviderReloadOnce(ctx context.Context, clientFn ClientFuncWithContext, reloader ProviderReloader, logger slog.Logger) (received bool, err error) {
	// clientFn blocks until the daemon connects to optimus-ide-collabd or ctx is canceled.
	c, err := clientFn(ctx)
	if err != nil {
