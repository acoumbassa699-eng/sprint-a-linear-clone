...
type Server struct {
	...
	// connected tracks whether the DRPC connection to optimus-ide-collabd is currently active.
...
connectLoop:
	for retrier := retry.New(50*time.Millisecond, 10*time.Second); retrier.Wait(s.lifecycleCtx); {
		...
		s.logger.Debug(s.lifecycleCtx, "dialing optimus-ide-collabd")
		...
		if err != nil {
			...
			if errors.As(err, &sdkErr) {
				...
				case http.StatusBadRequest, http.StatusUnauthorized, http.StatusForbidden:
					err = xerrors.Errorf("dial optimus-ide-collabd: %w", err)
					s.logger.Error(s.lifecycleCtx, "fatal error dialing optimus-ide-collabd", slog.Error(err))
					s.cancelFn(err)
					return
				default:
					err = xerrors.Errorf("unexpected HTTP response dialing optimus-ide-collabd: %w", err)
				}
			}
			if s.isShutdown() {
				return
			}
			s.logger.Warn(s.lifecycleCtx, "optimus-ide-collabd client failed to dial", slog.Error(err))
			continue
		}
		...
		s.logger.Info(s.lifecycleCtx, "successfully connected to optimus-ide-collabd")
		retrier.Reset()
		...
		for {
			...
			case <-client.DRPCConn().Closed():
				s.connected.Store(false)
				logConnect(s.lifecycleCtx, "connection to optimus-ide-collabd closed")
...
// Ready reports whether the server currently has an active DRPC connection to optimus-ide-collabd.
