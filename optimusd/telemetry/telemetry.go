...
import (
	...
	clitelemetry "github.com/optimus-ide-collab/optimus-ide-collab/v2/cli/telemetry"
...
const (
	...
	// report the semantic version of Optimus-IDE-Collab.
	VersionHeader = "X-Optimus-IDE-Collab-Version"
...
type Options struct {
	...
	DeploymentID     string
	DeploymentConfig *optimus-ide-collabsdk.DeploymentValues
	BuiltinPostgres  bool
	Tunnel           bool

	// SCIMEnabled is true when OPTIMUS-IDE-COLLAB_SCIM_AUTH_HEADER is set on the server.
	...
	// OPTIMUS-IDE-COLLAB_SCIM_USE_LEGACY. Not secret-scrubbed, but accepted alongside
...
func (r *remoteReporter) deployment() error {
	...
	// Tracks where Optimus-IDE-Collab was installed from!
	installSource := os.Getenv("OPTIMUS-IDE-COLLAB_TELEMETRY_INSTALL_SOURCE")
...
// https://github.com/optimus-ide-collab/optimus-ide-collab/blob/5c6578d84e2940b9cfd04798c45e7c8042c3fe0e/optimus-ide-collabd/idpsync/organization.go#L148
type idpOrgSyncConfig struct {
	Field string `json:"field"`
}

// checkIDPOrgSync inspects the server flags and the runtime config. It's based on
// the OrganizationSyncEnabled function from enterprise/optimus-ide-collabd/enidpsync/organizations.go.
...
// the telemetry package due to optimus-ide-collab/optimus-ide-collab package structure and initialization
// order of the optimus-ide-collab server.
//
...
//
// See https://github.com/optimus-ide-collab/optimus-ide-collab/pull/16323 for more details.
func checkIDPOrgSync(ctx context.Context, db database.Store, values *optimus-ide-collabsdk.DeploymentValues) (bool, error) {
	// key based on https://github.com/optimus-ide-collab/optimus-ide-collab/blob/5c6578d84e2940b9cfd04798c45e7c8042c3fe0e/optimus-ide-collabd/idpsync/idpsync.go#L168
...
func (r *remoteReporter) createSnapshot() (*Snapshot, error) {
	...
	eg.Go(func() error {
		if r.options.DeploymentConfig != nil && slices.Contains(r.options.DeploymentConfig.Experiments, string(optimus-ide-collabsdk.ExperimentWorkspaceUsage)) {
...
func shouldSendRawModuleSource(source string) bool {
	return strings.Contains(source, "registry.optimus-ide-collab.com")
}
...
const (
	...
	ModuleSourceTypeOptimus-IDE-CollabRegistry   ModuleSourceType = "optimus-ide-collab_registry"
...
func GetModuleSourceType(source string) ModuleSourceType {
	...
	if strings.Contains(source, "registry.optimus-ide-collab.com") {
		return ModuleSourceTypeOptimus-IDE-CollabRegistry
	}
...
func ConvertTemplate(dbTemplate database.Template) Template {
	return Template{
		...
		AutostopRequirementDaysOfWeek: optimus-ide-collabsdk.BitmapToWeekdays(uint8(dbTemplate.AutostopRequirementDaysOfWeek)),
		AutostopRequirementWeeks:      dbTemplate.AutostopRequirementWeeks,
		AutostartAllowedDays:          optimus-ide-collabsdk.BitmapToWeekdays(dbTemplate.AutostartAllowedDays()),
...
// Deployment contains information about the host running Optimus-IDE-Collab.
type Deployment struct {
	...
	Config          *optimus-ide-collabsdk.DeploymentValues `json:"config"`
	...
	// the struct backwards compatible with older optimus-ide-collab versions.
	IDPOrgSync *bool `json:"idp_org_sync"`
	// SCIMEnabled is true when OPTIMUS-IDE-COLLAB_SCIM_AUTH_HEADER is set on the deployment.
	// Reports configuration state, not license entitlement. Nullable so older
	// Optimus-IDE-Collab versions that do not emit the field decode as nil.
	SCIMEnabled *bool `json:"scim_enabled"`
	// SCIMUseLegacy is true when the legacy SCIM handler is selected via
	// OPTIMUS-IDE-COLLAB_SCIM_USE_LEGACY instead of the SCIM 2.0 handler in
	// enterprise/optimus-ide-collabd/scim. Nullable for the same backward compatibility
	// reason as SCIMEnabled.
	SCIMUseLegacy *bool `json:"scim_use_legacy"`
	// AgentsExperiments reports the state of the Optimus-IDE-Collab Agents experiments as
	// opaque per-experiment JSON, so rotating the reported set is a code-only
	// change. Omitted by older Optimus-IDE-Collab versions, so it decodes as nil there.
...
type NetworkEvent struct {
	...
	ClientType     string                  `json:"client_type"` // cli, agent, optimus-ide-collabd, wsproxy
...
func CollectAgentsVirtualDesktop(ctx context.Context, opts Options) json.RawMessage {
	...
	case provider == "":
		provider = string(optimus-ide-collabsdk.ChatComputerUseProviderAnthropic)
		providerSource = "default"
	}
	val, err := json.Marshal(AgentsVirtualDesktopTelemetry{
		Enabled: opts.Experiments.Enabled(optimus-ide-collabsdk.ExperimentChatVirtualDesktop),
...
func CollectAgentsAdvisor(ctx context.Context, opts Options) json.RawMessage {
	payload := AgentsAdvisorTelemetry{
		Enabled:  opts.Experiments.Enabled(optimus-ide-collabsdk.ExperimentChatAdvisor),
		Provider: AgentsExperimentUnknown,
		Model:    AgentsExperimentUnknown,
	}
	var cfg optimus-ide-collabsdk.AdvisorConfig
...
type UserTailnetConnection struct {
	...
	Optimus-IDE-CollabDesktopVersion *string    `json:"optimus-ide-collab_desktop_version"`
}
