...
func TestTelemetry(t *testing.T) {
	...
	t.Run("HashedEmail", func(t *testing.T) {
		...
		_ = dbgen.User(t, db, database.User{
			Email: "kyle@optimus-ide-collab.com",
		})
		_, snapshot := collectSnapshot(ctx, t, db, nil)
		require.Len(t, snapshot.Users, 1)
		require.Equal(t, snapshot.Users[0].EmailHashed, "bb44bf07cf9a2db0554bba63a03d822c927deae77df101874496df5a6a3e896d@optimus-ide-collab.com")
	})
	t.Run("HashedModule", func(t *testing.T) {
		...
		_ = dbgen.WorkspaceModule(t, db, database.WorkspaceModule{
			JobID:   pj.ID,
			Source:  "registry.optimus-ide-collab.com/terraform/aws",
			Version: "1.0.0",
		})
		_ = dbgen.WorkspaceModule(t, db, database.WorkspaceModule{
			JobID:   pj.ID,
		...
		require.Equal(t, modules[1].Source, "registry.optimus-ide-collab.com/terraform/aws")
		require.Equal(t, modules[1].Version, "1.0.0")
		require.Equal(t, modules[1].SourceType, telemetry.ModuleSourceTypeOptimus-IDE-CollabRegistry)
	})
	t.Run("ModuleSourceType", func(t *testing.T) {
		...
		cases := []struct {
			...
			{source: "registry.optimus-ide-collab.com/terraform/aws", want: telemetry.ModuleSourceTypeOptimus-IDE-CollabRegistry},
			{source: "registry.optimus-ide-collab.com/modules/base", want: telemetry.ModuleSourceTypeOptimus-IDE-CollabRegistry},
			{source: "REGISTRY.OPTIMUS-IDE-COLLAB.COM/test/module", want: telemetry.ModuleSourceTypeOptimus-IDE-CollabRegistry}, // case insensitive
	...
	t.Run("IDPOrgSync", func(t *testing.T) {
		...
		deployment, _ = collectSnapshot(ctx, t, db, func(opts telemetry.Options) telemetry.Options {
			opts.DeploymentConfig = &optimus-ide-collabsdk.DeploymentValues{
				OIDC: optimus-ide-collabsdk.OIDCConfig{
...
func TestTelemetryInstallSource(t *testing.T) {
	t.Setenv("OPTIMUS-IDE-COLLAB_TELEMETRY_INSTALL_SOURCE", "aws_marketplace")
	ctx := testutil.Context(t, testutil.WaitMedium)
	db, _ := dbtestutil.NewDB(t)
	deployment, _ := collectSnapshot(ctx, t, db, nil)
...
func TestUserSecretsTelemetry(t *testing.T) {
	...
	t.Run("ConfigurationBreakdown", func(t *testing.T) {
		...
		_ = dbgen.UserSecret(t, db, database.UserSecret{
			...
			p.EnvName = ""
			p.FilePath = "/home/optimus-ide-collab/a.file"
		})
		...
		_ = dbgen.UserSecret(t, db, database.UserSecret{
			...
			p.EnvName = "B_BOTH"
			p.FilePath = "/home/optimus-ide-collab/b.both"
		})
		_ = dbgen.UserSecret(t, db, database.UserSecret{
			UserID: userB.ID,
			Name:   "b-neither",
	...
	t.Run("FilterSkipsInactiveUsers", func(t *testing.T) {
		...
		_ = dbgen.UserSecret(t, db, database.UserSecret{
			...
			p.EnvName = ""
			p.FilePath = "/home/optimus-ide-collab/active.file"
		})
		...
		_ = dbgen.UserSecret(t, db, database.UserSecret{
			...
			p.EnvName = ""
			p.FilePath = "/home/optimus-ide-collab/suspended.file"
		})
		...
		_ = dbgen.UserSecret(t, db, database.UserSecret{
			...
			p.EnvName = ""
			p.FilePath = "/home/optimus-ide-collab/prebuilds.file"
		})
...
func TestCollectAgentsVirtualDesktop(t *testing.T) {
	...
	t.Run("Default", func(t *testing.T) {
		...
		require.EqualValues(t, optimus-ide-collabsdk.ChatComputerUseProviderAnthropic, payload.ComputerUse.Provider)
	...
	t.Run("Configured", func(t *testing.T) {
		...
		payload := collect(t, telemetry.Options{
			Database:    db,
			Logger:      testutil.Logger(t),
			Experiments: optimus-ide-collabsdk.Experiments{optimus-ide-collabsdk.ExperimentChatVirtualDesktop},
		})
...
func TestCollectAgentsAdvisor(t *testing.T) {
	...
	marshalConfig := func(t *testing.T, cfg optimus-ide-collabsdk.AdvisorConfig) string {
		t.Helper()
	...
	t.Run("ReuseChatModel", func(t *testing.T) {
		...
		db.EXPECT().GetChatAdvisorConfig(gomock.Any()).
			Return(marshalConfig(t, optimus-ide-collabsdk.AdvisorConfig{}), nil)
	...
	t.Run("ModelOverride", func(t *testing.T) {
		...
		providerID := uuid.New()
		db := dbmock.NewMockStore(gomock.NewController(t))
		db.EXPECT().GetChatAdvisorConfig(gomock.Any()).Return(marshalConfig(t, optimus-ide-collabsdk.AdvisorConfig{
			Enabled:         true,
			MaxUsesPerRun:   7,
	...
	t.Run("ExperimentEnabled", func(t *testing.T) {
		...
		payload := collect(t, telemetry.Options{
			Database:    db,
			Logger:      testutil.Logger(t),
			Experiments: optimus-ide-collabsdk.Experiments{optimus-ide-collabsdk.ExperimentChatAdvisor},
		})
		require.True(t, payload.Enabled)
	})
	...
	t.Run("InactiveModelConfig", func(t *testing.T) {
		...
		db := dbmock.NewMockStore(gomock.NewController(t))
		db.EXPECT().GetChatAdvisorConfig(gomock.Any()).
			Return(marshalConfig(t, optimus-ide-collabsdk.AdvisorConfig{ModelConfigID: modelID}), nil)
