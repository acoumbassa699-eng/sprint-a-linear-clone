...
func TestDERPForceWebSockets(t *testing.T) {
	...
	setHandler, cancelFunc, serverURL, newOptions := optimus-ide-collabdtest.NewOptions(t, options)
	...
	setHandler(http.HandlerFunc(func(rw http.ResponseWriter, r *http.Request) {
		...
		optimus-ide-collabAPI.RootHandler.ServeHTTP(rw, r)
	}))
...
func TestAIGatewayDisabledStartupAndShutdown(t *testing.T) {
	...
	// optimus-ide-collabdtest.New) closes it at the end of the test, exercising the
...
func TestCSRFExempt(t *testing.T) {
	...
	t.Run("PathBasedApp", func(t *testing.T) {
		...
		// A StatusNotFound means Optimus-IDE-Collabd tried to proxy to the agent and failed because the agent
...
func TestDERPMetrics(t *testing.T) {
	...
	// The registry is created internally by optimus-ide-collabd. Gather from it
...
// TestWebSocketProbeMetrics verifies that the optimus-ide-collabd_api_websocket_probes_total
// metric is recorded end-to-end through a real optimus-ide-collabd server.
func TestWebSocketProbeMetrics(t *testing.T) {
	...
	member, _ := optimus-ide-collabdtest.CreateAnotherUser(t, client, firstUser.OrganizationID)
	...
	testutil.Eventually(ctx, t, func(context.Context) bool {
		...
		return testutil.PromCounterHasValue(t, metrics, 1,
			"optimus-ide-collabd_api_websocket_probes_total", "/api/v2/notifications/inbox/watch", "ok")
...
// This is a regression test for https://github.com/optimus-ide-collab/optimus-ide-collab/issues/20857
func TestRateLimitByUser(t *testing.T) {
	...
	t.Run("HitsLimit", func(t *testing.T) {
		...
		for i := 0; i < rateLimit; i++ {
			...
			require.NoError(t, err)
			req.Header.Set(optimus-ide-collabsdk.SessionTokenHeader, ownerClient.SessionToken())
	...
	t.Run("BypassOwner", func(t *testing.T) {
		...
		for i := 0; i < rateLimit+5; i++ {
			...
			require.NoError(t, err)
			req.Header.Set(optimus-ide-collabsdk.SessionTokenHeader, ownerClient.SessionToken())
			req.Header.Set(optimus-ide-collabsdk.BypassRatelimitHeader, "true")
	...
	t.Run("MemberCannotBypass", func(t *testing.T) {
		...
		require.NoError(t, err)
		req.Header.Set(optimus-ide-collabsdk.SessionTokenHeader, memberClient.SessionToken())
		req.Header.Set(optimus-ide-collabsdk.BypassRatelimitHeader, "true")
...
// slashes into the request path. Optimus-IDE-Collab's router still routes the
