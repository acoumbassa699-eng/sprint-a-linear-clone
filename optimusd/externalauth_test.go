		...
		client := optimus-ide-collabdtest.New(t, &optimus-ide-collabdtest.Options{
			ExternalAuthConfigs: []*externalauth.Config{
				fake.ExternalAuthConfig(t, providerID, nil, func(cfg *externalauth.Config) {
					cfg.Type = optimus-ide-collabsdk.EnhancedExternalAuthProviderGitHub.String()
	...
	t.Run("AuthenticatedNoUser", func(t *testing.T) {
		...
		client := optimus-ide-collabdtest.New(t, &optimus-ide-collabdtest.Options{
			ExternalAuthConfigs: []*externalauth.Config{
				...
				fake.ExternalAuthConfig(t, providerID, nil, func(cfg *externalauth.Config) {
					cfg.Type = optimus-ide-collabsdk.EnhancedExternalAuthProviderAzureDevops.String()
				}),
			},
		})
...
func TestExternalAuthManagement(t *testing.T) {
	...
	t.Run("ListProviders", func(t *testing.T) {
		...
		owner := optimus-ide-collabdtest.New(t, &optimus-ide-collabdtest.Options{
			ExternalAuthConfigs: []*externalauth.Config{
				...
				slack.ExternalAuthConfig(t, slackID, nil, func(cfg *externalauth.Config) {
					cfg.Type = optimus-ide-collabsdk.EnhancedExternalAuthProviderSlack.String()
					cfg.RevokeURL = ""
				}),
				azure.ExternalAuthConfig(t, azureID, nil, func(cfg *externalauth.Config) {
					cfg.Type = optimus-ide-collabsdk.EnhancedExternalAuthProviderAzureDevopsEntra.String()
				}),
			},
		})
		ownerUser := optimus-ide-collabdtest.CreateFirstUser(t, owner)
		// Just a regular user
		client, _ := optimus-ide-collabdtest.CreateAnotherUser(t, owner, ownerUser.OrganizationID)
		...
		require.True(t, slices.ContainsFunc(list.Links, func(l optimus-ide-collabsdk.ExternalAuthLink) bool { return l.ProviderID == githubID }))
		require.True(t, slices.ContainsFunc(list.Links, func(l optimus-ide-collabsdk.ExternalAuthLink) bool { return l.ProviderID == slackID }))
		require.True(t, slices.ContainsFunc(list.Links, func(l optimus-ide-collabsdk.ExternalAuthLink) bool { return l.ProviderID == azureID }))
	...
	t.Run("RefreshAllProviders", func(t *testing.T) {
		...
		ownerUser := optimus-ide-collabdtest.CreateFirstUser(t, owner)
		// Just a regular user
		client, user := optimus-ide-collabdtest.CreateAnotherUser(t, owner, ownerUser.OrganizationID)
		ctx := testutil.Context(t, testutil.WaitLong)
		...
		links, err := db.GetExternalAuthLinksByUserID(
			dbauthz.As(ctx, optimus-ide-collabdtest.AuthzUserSubject(user)), user.ID)
		require.NoError(t, err)
		require.Len(t, links, 2)

		// Expire the links
		for _, l := range links {
			_, err := db.UpdateExternalAuthLink(dbauthz.As(ctx, optimus-ide-collabdtest.AuthzUserSubject(user)), database.UpdateExternalAuthLinkParams{
...
func TestExternalAuthDevice(t *testing.T) {
	...
	t.Run("FetchCode", func(t *testing.T) {
		...
		srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			httpapi.Write(r.Context(), w, http.StatusOK, optimus-ide-collabsdk.ExternalAuthDevice{
				UserCode: "hey",
			})
		}))
		defer srv.Close()
	...
	t.Run("ExchangeCode", func(t *testing.T) {
		...
		optimus-ide-collabdtest.CreateFirstUser(t, client)
		err := client.ExternalAuthDeviceExchange(context.Background(), "test", optimus-ide-collabsdk.ExternalAuthDeviceExchange{
			DeviceCode: "hey",
		})
		var sdkErr *optimus-ide-collabsdk.Error
		require.ErrorAs(t, err, &sdkErr)
...
func TestExternalAuthCallback(t *testing.T) {
	...
	t.Run("CustomRedirect", func(t *testing.T) {
		...
		_ = optimus-ide-collabdtest.CreateFirstUser(t, client)
		resp := optimus-ide-collabdtest.RequestExternalAuthCallback(t, "github", client, func(req *http.Request) {
	...
	t.Run("ExpiredNoRefresh", func(t *testing.T) {
		...
		// to return an expired token. Optimus-IDE-Collab consumes this
		// and stores it.
		resp := optimus-ide-collabdtest.RequestExternalAuthCallback(t, "github", client)
		require.Equal(t, http.StatusTemporaryRedirect, resp.StatusCode)
