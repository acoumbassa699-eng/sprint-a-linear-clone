...
type authorizeParams struct {
	...
	responseType        optimus-ide-collabsdk.OAuth2ProviderResponseType
	scope               []string
	state               string
...
func extractAuthorizeParams(r *http.Request, callbackURL *url.URL) (authorizeParams, []optimus-ide-collabsdk.ValidationError, error) {
	...
	params := authorizeParams{
		...
		responseType:        httpapi.ParseCustom(p, vals, "", "response_type", httpapi.ParseEnum[optimus-ide-collabsdk.OAuth2ProviderResponseType]),
	...
	if params.responseType == optimus-ide-collabsdk.OAuth2ProviderResponseTypeCode && params.codeChallenge == "" {
...
func ShowAuthorizePage(accessURL *url.URL) http.HandlerFunc {
	return func(rw http.ResponseWriter, r *http.Request) {
		...
		if params.responseType != optimus-ide-collabsdk.OAuth2ProviderResponseTypeCode {
			site.RenderStaticErrorPage(rw, r, site.ErrorPageData{
				Status:      http.StatusBadRequest,
				HideStatus:  false,
		...
		if err := optimus-ide-collabsdk.ValidateRedirectURIScheme(cancel); err != nil {
			site.RenderStaticErrorPage(rw, r, site.ErrorPageData{
				Status:      http.StatusBadRequest,
				HideStatus:  false,
		...
		site.RenderOAuthAllowPage(rw, r, site.RenderOAuthAllowData{
			...
			// optimus-ide-collabsdk.ValidateRedirectURIScheme above.
...
func ProcessAuthorize(db database.Store) http.HandlerFunc {
	return func(rw http.ResponseWriter, r *http.Request) {
		...
		callbackURL, err := url.Parse(app.CallbackURL)
		if err != nil {
			httpapi.WriteOAuth2Error(r.Context(), rw, http.StatusInternalServerError, optimus-ide-collabsdk.OAuth2ErrorCodeServerError, "Failed to validate query parameters")
			return
		}

		params, _, err := extractAuthorizeParams(r, callbackURL)
		if err != nil {
			httpapi.WriteOAuth2Error(ctx, rw, http.StatusBadRequest, optimus-ide-collabsdk.OAuth2ErrorCodeInvalidRequest, err.Error())
			return
		}
		...
		if params.responseType != optimus-ide-collabsdk.OAuth2ProviderResponseTypeCode {
			httpapi.WriteOAuth2Error(ctx, rw, http.StatusBadRequest,
				optimus-ide-collabsdk.OAuth2ErrorCodeUnsupportedResponseType,
		...
		if params.codeChallengeMethod == "" {
			params.codeChallengeMethod = string(optimus-ide-collabsdk.OAuth2PKCECodeChallengeMethodS256)
		}
		if err := optimus-ide-collabsdk.ValidatePKCECodeChallengeMethod(params.codeChallengeMethod); err != nil {
			httpapi.WriteOAuth2Error(ctx, rw, http.StatusBadRequest, optimus-ide-collabsdk.OAuth2ErrorCodeInvalidRequest, err.Error())
			return
		}
		...
		if err != nil {
			httpapi.WriteOAuth2Error(r.Context(), rw, http.StatusInternalServerError, optimus-ide-collabsdk.OAuth2ErrorCodeServerError, "Failed to generate OAuth2 app authorization code")
			return
		}
		...
		if err != nil {
			httpapi.WriteOAuth2Error(ctx, rw, http.StatusInternalServerError, optimus-ide-collabsdk.OAuth2ErrorCodeServerError, "Failed to generate OAuth2 authorization code")
			return
		}
