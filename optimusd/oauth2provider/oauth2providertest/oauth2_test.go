...
func TestOAuth2TokenExchangeClientSecretBasicInvalidSecret(t *testing.T) {
	...
	require.Equal(t, `Basic realm="optimus-ide-collab"`, resp.Header.Get("WWW-Authenticate"), "missing WWW-Authenticate header")
...
func TestOAuth2PKCEPlainMethodRejected(t *testing.T) {
	...
	authParams := oauth2providertest.AuthorizeParams{
		ClientID:            app.ID.String(),
		ResponseType:        string(optimus-ide-collabsdk.OAuth2ProviderResponseTypeCode),
		RedirectURI:         oauth2providertest.TestRedirectURI,
		State:               state,
		CodeChallenge:       codeChallenge,
		CodeChallengeMethod: string(optimus-ide-collabsdk.OAuth2PKCECodeChallengeMethodPlain),
	}
