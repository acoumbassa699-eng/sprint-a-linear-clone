...
func TestExtractTokenParams_Scopes(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			scopeParam:     "optimus-ide-collab:workspace.create optimus-ide-collab:workspace.operate",
			expectedScopes: []string{"optimus-ide-collab:workspace.create", "optimus-ide-collab:workspace.operate"},
		...
		{
			name:           "SingleScope",
			scopeParam:     "optimus-ide-collab:workspace.create",
			expectedScopes: []string{"optimus-ide-collab:workspace.create"},
		...
		{
			name:           "ColonInScope",
			scopeParam:     "optimus-ide-collab:workspace:read optimus-ide-collab:workspace:write",
			expectedScopes: []string{"optimus-ide-collab:workspace:read", "optimus-ide-collab:workspace:write"},
...
func TestExtractTokenParams_ScopesURLEncoded(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			rawQuery:       "grant_type=authorization_code&client_id=test&client_secret=secret&code=code&scope=optimus-ide-collab%3Aworkspace.create+optimus-ide-collab%3Aworkspace.operate",
			expectedScopes: []string{"optimus-ide-collab:workspace.create", "optimus-ide-collab:workspace.operate"},
		},
	}
...
func TestExtractTokenParams_ScopesEdgeCases(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			setupForm: func() url.Values {
				longScope := "optimus-ide-collab:workspace:project:resource:action:create:read:write:delete:admin"
				form := url.Values{}
				form.Set("grant_type", "authorization_code")
				form.Set("client_id", "test-client")
				form.Set("client_secret", "test-secret")
			...
			expectedScopes: []string{"optimus-ide-collab:workspace:project:resource:action:create:read:write:delete:admin"},
...
func TestExtractAuthorizeParams_Scopes(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:           "Optimus-IDE-CollabScopes",
			scopeParam:     "optimus-ide-collab:workspace.create optimus-ide-collab:workspace.read optimus-ide-collab:workspace.delete",
			expectedScopes: []string{"optimus-ide-collab:workspace.create", "optimus-ide-collab:workspace.read", "optimus-ide-collab:workspace.delete"},
		},
	}
...
func TestExtractAuthorizeParams_TokenResponseTypeDoesNotRequirePKCE(t *testing.T) {
	...
	query := url.Values{}
	query.Set("response_type", string(optimus-ide-collabsdk.OAuth2ProviderResponseTypeToken))
	query.Set("client_id", "test-client")
	query.Set("redirect_uri", "http://localhost:3000/callback")
	...
	require.NoError(t, err)
	require.Empty(t, validationErrs)
	require.Equal(t, optimus-ide-collabsdk.OAuth2ProviderResponseTypeToken, params.responseType)
}
...
func TestRefreshTokenGrant_Scopes(t *testing.T) {
	...
	require.NoError(t, err)
	require.Empty(t, validationErrs)
	require.Equal(t, optimus-ide-collabsdk.OAuth2ProviderGrantTypeRefreshToken, tokenReq.GrantType)
