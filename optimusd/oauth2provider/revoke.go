...
func extractRevocationRequest(r *http.Request) (optimus-ide-collabsdk.OAuth2TokenRevocationRequest, error) {
	if err := r.ParseForm(); err != nil {
		return optimus-ide-collabsdk.OAuth2TokenRevocationRequest{}, xerrors.Errorf("invalid form data: %w", err)
	}

	req := optimus-ide-collabsdk.OAuth2TokenRevocationRequest{
		Token:         r.Form.Get("token"),
		TokenTypeHint: optimus-ide-collabsdk.OAuth2RevocationTokenTypeHint(r.Form.Get("token_type_hint")),
	...
	if req.Token == "" {
		return optimus-ide-collabsdk.OAuth2TokenRevocationRequest{}, xerrors.New("missing token parameter")
	}
...
func RevokeToken(db database.Store, logger slog.Logger) http.HandlerFunc {
	return func(rw http.ResponseWriter, r *http.Request) {
		...
		if r.Method != http.MethodPost {
			httpapi.WriteOAuth2Error(ctx, rw, http.StatusMethodNotAllowed, optimus-ide-collabsdk.OAuth2ErrorCodeInvalidRequest, "Method not allowed")
			return
		}
		...
		// Determine if this is a refresh token (starts with "optimus-ide-collab_") or API key
		// APIKeys do not have the SecretIdentifier prefix.
		const optimus-ide-collabPrefix = SecretIdentifier + "_"
		isRefreshToken := strings.HasPrefix(req.Token, optimus-ide-collabPrefix)
		...
		if err != nil {
			...
			if errors.Is(err, ErrInvalidTokenFormat) {
				...
				logger.Debug(ctx, "token revocation failed: invalid token format",
					slog.F("client_id", app.ID.String()),
					slog.F("app_name", app.Name))
				httpapi.WriteOAuth2Error(ctx, rw, http.StatusBadRequest, optimus-ide-collabsdk.OAuth2ErrorCodeInvalidRequest, "Invalid token format")
				return
			}
			logger.Error(ctx, "token revocation failed with internal server error",
				...
				slog.F("client_id", app.ID.String()),
				slog.F("app_name", app.Name))
			httpapi.WriteOAuth2Error(ctx, rw, http.StatusInternalServerError, optimus-ide-collabsdk.OAuth2ErrorCodeServerError, "Internal server error")
			return
		}
