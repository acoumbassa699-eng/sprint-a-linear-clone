...
func TestValidatePKCECodeChallengeMethod(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:        "S256IsValid",
			method:      string(optimus-ide-collabsdk.OAuth2PKCECodeChallengeMethodS256),
			expectError: false,
		},
		{
			name:          "PlainIsRejected",
			method:        string(optimus-ide-collabsdk.OAuth2PKCECodeChallengeMethodPlain),
			expectError:   true,
			errorContains: "plain",
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			err := optimus-ide-collabsdk.ValidatePKCECodeChallengeMethod(tt.method)
			if tt.expectError {
				require.Error(t, err)
				if tt.errorContains != "" {
