...
func extractTokenRequest(r *http.Request, callbackURL *url.URL) (optimus-ide-collabsdk.OAuth2TokenRequest, []optimus-ide-collabsdk.ValidationError, error) {
	p := httpapi.NewQueryParamParser()
	err := r.ParseForm()
	if err != nil {
		return optimus-ide-collabsdk.OAuth2TokenRequest{}, nil, xerrors.Errorf("parse form: %w", err)
	}

	vals := r.Form
	p.RequiredNotEmpty("grant_type")
	grantType := httpapi.ParseCustom(p, vals, "", "grant_type", httpapi.ParseEnum[optimus-ide-collabsdk.OAuth2ProviderGrantType])

	// Grant-type specific validation - must be called before parsing values.
	switch grantType {
	case optimus-ide-collabsdk.OAuth2ProviderGrantTypeRefreshToken:
		p.RequiredNotEmpty("refresh_token")
	case optimus-ide-collabsdk.OAuth2ProviderGrantTypeAuthorizationCode:
		p.RequiredNotEmpty("code")
	}

	req := optimus-ide-collabsdk.OAuth2TokenRequest{
		GrantType:    grantType,
	...
	if user, pass, ok := r.BasicAuth(); ok && user != "" {
		if req.ClientID != "" && req.ClientID != user {
			return optimus-ide-collabsdk.OAuth2TokenRequest{}, nil, errConflictingClientAuth
		}
	...
	if req.GrantType == optimus-ide-collabsdk.OAuth2ProviderGrantTypeAuthorizationCode {
		if req.ClientID == "" {
			p.Errors = append(p.Errors, optimus-ide-collabsdk.ValidationError{
				Field:  "client_id",
	...
	if len(p.Errors) > 0 {
		return optimus-ide-collabsdk.OAuth2TokenRequest{}, p.Errors, xerrors.Errorf("invalid query params: %w", p.Errors)
	}
	return req, nil, nil
}
...
func Tokens(db database.Store, lifetimes optimus-ide-collabsdk.SessionLifetime) http.HandlerFunc {
	return func(rw http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			if errors.Is(err, errConflictingClientAuth) {
				httpapi.WriteOAuth2Error(ctx, rw, http.StatusBadRequest, optimus-ide-collabsdk.OAuth2ErrorCodeInvalidRequest, "Conflicting client credentials between Authorization header and request body")
				return
			}

			// Check for specific validation errors in priority order
			if slices.ContainsFunc(validationErrs, func(validationError optimus-ide-collabsdk.ValidationError) bool {
				return validationError.Field == "grant_type"
			}) {
				httpapi.WriteOAuth2Error(ctx, rw, http.StatusBadRequest, optimus-ide-collabsdk.OAuth2ErrorCodeUnsupportedGrantType, "The grant type is missing or unsupported")
				return
			}
			...
			for _, field := range []string{"code", "client_id", "client_secret"} {
				if slices.ContainsFunc(validationErrs, func(validationError optimus-ide-collabsdk.ValidationError) bool {
					...
					httpapi.WriteOAuth2Error(ctx, rw, http.StatusBadRequest, optimus-ide-collabsdk.OAuth2ErrorCodeInvalidRequest, fmt.Sprintf("Missing required parameter: %s", field))
					return
				}
			}
			// Generic invalid request for other validation errors
			httpapi.WriteOAuth2Error(ctx, rw, http.StatusBadRequest, optimus-ide-collabsdk.OAuth2ErrorCodeInvalidRequest, "The request is missing required parameters or is otherwise malformed")
			return
		}

		var token optimus-ide-collabsdk.OAuth2TokenResponse
		...
		default:
			...
			httpapi.WriteOAuth2Error(ctx, rw, http.StatusBadRequest, optimus-ide-collabsdk.OAuth2ErrorCodeUnsupportedGrantType, fmt.Sprintf("The grant type %q is not supported", req.GrantType))
			return
		}

		if errors.Is(err, errBadSecret) {
			httpapi.WriteOAuth2Error(ctx, rw, http.StatusUnauthorized, optimus-ide-collabsdk.OAuth2ErrorCodeInvalidClient, "The client credentials are invalid")
			return
		}
		if errors.Is(err, errBadCode) {
			httpapi.WriteOAuth2Error(ctx, rw, http.StatusBadRequest, optimus-ide-collabsdk.OAuth2ErrorCodeInvalidGrant, "The authorization code is invalid or expired")
			return
		}
		if errors.Is(err, errInvalidPKCE) {
			httpapi.WriteOAuth2Error(ctx, rw, http.StatusBadRequest, optimus-ide-collabsdk.OAuth2ErrorCodeInvalidGrant, "The PKCE code verifier is invalid")
			return
		}
		if errors.Is(err, errInvalidResource) {
			httpapi.WriteOAuth2Error(ctx, rw, http.StatusBadRequest, optimus-ide-collabsdk.OAuth2ErrorCodeInvalidTarget, "The resource parameter is invalid")
			return
		}
		if errors.Is(err, errBadToken) {
			httpapi.WriteOAuth2Error(ctx, rw, http.StatusBadRequest, optimus-ide-collabsdk.OAuth2ErrorCodeInvalidGrant, "The refresh token is invalid or expired")
			return
		}
		if err != nil {
			httpapi.Write(ctx, rw, http.StatusInternalServerError, optimus-ide-collabsdk.Response{
...
func authorizationCodeGrant(ctx context.Context, db database.Store, app database.OAuth2ProviderApp, lifetimes optimus-ide-collabsdk.SessionLifetime, req optimus-ide-collabsdk.OAuth2TokenRequest) (optimus-ide-collabsdk.OAuth2TokenResponse, error) {
	// Validate the client secret.
	secret, err := ParseFormattedSecret(req.ClientSecret)
	if err != nil {
		return optimus-ide-collabsdk.OAuth2TokenResponse{}, errBadSecret
	}
	...
	if err != nil {
		return optimus-ide-collabsdk.OAuth2TokenResponse{}, err
	}
	...
	if err != nil {
		return optimus-ide-collabsdk.OAuth2TokenResponse{}, errBadCode
	}
	...
	// challenge.
	if req.CodeVerifier == "" {
		return optimus-ide-collabsdk.OAuth2TokenResponse{}, errInvalidPKCE
	}
	...
	if dbCode.ResourceUri.Valid && dbCode.ResourceUri.String != "" {
		...
		if req.Resource == "" {
			return optimus-ide-collabsdk.OAuth2TokenResponse{}, errInvalidResource
		}
	...
	if err != nil {
		return optimus-ide-collabsdk.OAuth2TokenResponse{}, xerrors.Errorf("fetch user actor: %w", err)
	}
	...
	return optimus-ide-collabsdk.OAuth2TokenResponse{
		AccessToken:  sessionToken,
		TokenType:    optimus-ide-collabsdk.OAuth2TokenTypeBearer,
...
func refreshTokenGrant(ctx context.Context, db database.Store, app database.OAuth2ProviderApp, lifetimes optimus-ide-collabsdk.SessionLifetime, req optimus-ide-collabsdk.OAuth2TokenRequest) (optimus-ide-collabsdk.OAuth2TokenResponse, error) {
	// Validate the token.
	token, err := ParseFormattedSecret(req.RefreshToken)
	if err != nil {
		return optimus-ide-collabsdk.OAuth2TokenResponse{}, errBadToken
	}
