...
func TestOAuth2ProviderAppValidation(t *testing.T) {
	...
	t.Run("ValidationErrors", func(t *testing.T) {
		...
		tests := []struct {
			name string
			req  optimus-ide-collabsdk.PostOAuth2ProviderAppRequest
		}{
			{
				name: "NameMissing",
				req: optimus-ide-collabsdk.PostOAuth2ProviderAppRequest{
			...
			},
			{
				...
				req: optimus-ide-collabsdk.PostOAuth2ProviderAppRequest{
					Name:        "foo",
					CallbackURL: "optimus-ide-collab.com",
				},
			},
			{
				name: "URLNoColon",
				req: optimus-ide-collabsdk.PostOAuth2ProviderAppRequest{
					Name:        "foo",
					CallbackURL: "http//optimus-ide-collab",
				},
			},
			{
				name: "URLJustBar",
	...
	t.Run("DuplicateNames", func(t *testing.T) {
		...
		// Create first app
		//nolint:gocritic // OAuth2 app management requires owner permission.
		app1, err := client.PostOAuth2ProviderApp(ctx, optimus-ide-collabsdk.PostOAuth2ProviderAppRequest{
			Name:        appName,
		...
		//nolint:gocritic // OAuth2 app management requires owner permission.
		app2, err := client.PostOAuth2ProviderApp(ctx, optimus-ide-collabsdk.PostOAuth2ProviderAppRequest{
			Name:        appName,
		...
		//nolint:gocritic // OAuth2 app management requires owner permission.
		app3, err := client.PostOAuth2ProviderApp(ctx, optimus-ide-collabsdk.PostOAuth2ProviderAppRequest{
			Name:        appName,
...
func TestOAuth2ClientRegistrationValidation(t *testing.T) {
	...
	t.Run("InvalidURIs", func(t *testing.T) {
		...
		for _, tc := range testCases {
			t.Run(tc.name, func(t *testing.T) {
				...
				subClient := optimus-ide-collabdtest.New(t, nil)
				_ = optimus-ide-collabdtest.CreateFirstUser(t, subClient)
	...
	t.Run("InvalidGrantTypes", func(t *testing.T) {
		...
		req := optimus-ide-collabsdk.OAuth2ClientRegistrationRequest{
			...
			GrantTypes:   []optimus-ide-collabsdk.OAuth2ProviderGrantType{"unsupported_grant"},
		}
	...
	t.Run("InvalidResponseTypes", func(t *testing.T) {
		...
		req := optimus-ide-collabsdk.OAuth2ClientRegistrationRequest{
			...
			ResponseTypes: []optimus-ide-collabsdk.OAuth2ProviderResponseType{"token"}, // Not supported
		}
...
func TestOAuth2ProviderAppOperations(t *testing.T) {
	...
	t.Run("DeleteNonExisting", func(t *testing.T) {
		...
		another, _ := optimus-ide-collabdtest.CreateAnotherUser(t, client, owner.OrganizationID)
	...
	t.Run("BasicOperations", func(t *testing.T) {
		...
		// No apps yet.
		apps, err := another.OAuth2ProviderApps(ctx, optimus-ide-collabsdk.OAuth2ProviderAppFilter{})
		require.NoError(t, err)
		require.Len(t, apps, 0)

		// Should be able to add apps.
		expectedApps := generateApps(ctx, t, client, "get-apps")
		expectedOrder := []optimus-ide-collabsdk.OAuth2ProviderApp{
		...
		req := optimus-ide-collabsdk.PutOAuth2ProviderAppRequest{
			Name:        expectedApps.Default.Name,
			CallbackURL: "http://optimus-ide-collab.com",
			Icon:        "test",
		}
		//nolint:gocritic // OAuth2 app management requires owner permission.
		...
		newApps, err := another.OAuth2ProviderApps(ctx, optimus-ide-collabsdk.OAuth2ProviderAppFilter{})
		require.NoError(t, err)
	...
	t.Run("ByUser", func(t *testing.T) {
		...
		client := optimus-ide-collabdtest.New(t, nil)
		owner := optimus-ide-collabdtest.CreateFirstUser(t, client)
		another, user := optimus-ide-collabdtest.CreateAnotherUser(t, client, owner.OrganizationID)
		ctx := testutil.Context(t, testutil.WaitLong)
