...
func TestProvisionerJobs(t *testing.T) {
	...
	t.Run("ProvisionerJobs", func(t *testing.T) {
		...
		t.Run("Single", func(t *testing.T) {
			...
			t.Run("Workspace", func(t *testing.T) {
				...
				t.Run("OK", func(t *testing.T) {
					...
					assert.Equal(t, job2.Metadata, optimus-ide-collabsdk.ProvisionerJobMetadata{
						...
						WorkspaceBuildTransition: optimus-ide-collabsdk.WorkspaceTransitionStart,
					})
				})
			})
		...
		t.Run("IDs", func(t *testing.T) {
			t.Parallel()
			ctx := testutil.Context(t, testutil.WaitMedium)
			jobs, err := templateAdminClient.OrganizationProvisionerJobs(ctx, owner.OrganizationID, &optimus-ide-collabsdk.OrganizationProvisionerJobsOptions{
		...
		t.Run("Status", func(t *testing.T) {
			...
			ctx := testutil.Context(t, testutil.WaitMedium)
			jobs, err := templateAdminClient.OrganizationProvisionerJobs(ctx, owner.OrganizationID, &optimus-ide-collabsdk.OrganizationProvisionerJobsOptions{
				Status: []optimus-ide-collabsdk.ProvisionerJobStatus{optimus-ide-collabsdk.ProvisionerJobRunning},
			})
			require.NoError(t, err)
		...
		t.Run("InitiatorWithOtherFilters", func(t *testing.T) {
			...
			jobs, err := templateAdminClient.OrganizationProvisionerJobs(ctx, owner.OrganizationID, &optimus-ide-collabsdk.OrganizationProvisionerJobsOptions{
				...
				Status:    []optimus-ide-collabsdk.ProvisionerJobStatus{optimus-ide-collabsdk.ProvisionerJobSucceeded},
			})
			require.NoError(t, err)
			...
			for _, job := range jobs {
				...
				require.Equal(t, optimus-ide-collabsdk.ProvisionerJobSucceeded, job.Status)
			}
		})
		...
		t.Run("MemberDeniedWithInitiator", func(t *testing.T) {
			...
			jobs, err := memberClient.OrganizationProvisionerJobs(ctx, owner.OrganizationID, &optimus-ide-collabsdk.OrganizationProvisionerJobsOptions{
				Initiator: member.ID.String(),
			})
			require.Error(t, err)
			require.Len(t, jobs, 0)
	...
	t.Run("AssignedProvisionerJob", func(t *testing.T) {
		...
		provisionerDaemon := optimus-ide-collabdtest.NewTaggedProvisionerDaemon(t, optimus-ide-collabdAPI, provisionerDaemonName, map[string]string{"owner": "", "scope": "organization"})
		owner := optimus-ide-collabdtest.CreateFirstUser(t, client)
		templateAdminClient, _ := optimus-ide-collabdtest.CreateAnotherUser(t, client, owner.OrganizationID, rbac.ScopedRoleOrgTemplateAdmin(owner.OrganizationID))
		...
		t.Run("Get_IncludesWorkerIDAndName", func(t *testing.T) {
			...
			for _, job := range jobs {
				if job.Type == optimus-ide-collabsdk.ProvisionerJobTypeWorkspaceBuild {
