...
func convertRows(rows []database.GetWorkspacesAndAgentsByOwnerIDRow) workspacesByID {
	...
	for _, row := range rows {
		...
		out[row.ID] = ownedWorkspace{
			...
			Status:        tailnet.WorkspaceStatusToProto(optimus-ide-collabsdk.ConvertWorkspaceStatus(optimus-ide-collabsdk.ProvisionerJobStatus(row.JobStatus), optimus-ide-collabsdk.WorkspaceTransition(row.Transition))),
			Agents:        agents,
		}
	}
	return out
