...
// @Success 200 {object} optimus-ide-collabsdk.WorkspaceBuild
...
// @Success 200 {array} optimus-ide-collabsdk.WorkspaceBuild
...
// @Tags Builds
// @Param workspace path string true "Workspace ID" format(uuid)
// @Param request body optimus-ide-collabsdk.CreateWorkspaceBuildRequest true "Create workspace build request"
// @Success 200 {object} optimus-ide-collabsdk.WorkspaceBuild
...
func (api *API) postWorkspaceBuilds(rw http.ResponseWriter, r *http.Request) {
	...
	workspace := httpmw.WorkspaceParam(r)
	var createBuild optimus-ide-collabsdk.CreateWorkspaceBuildRequest
	...
	if workspace.Deleted && createBuild.Transition != optimus-ide-collabsdk.WorkspaceTransitionDelete {
...
func (api *API) postWorkspaceBuildsInternal(
	...
	createBuild optimus-ide-collabsdk.CreateWorkspaceBuildRequest,
	...
	if createBuild.OnSuccess != nil {
		...
		if childParameterValues == nil {
			childParameterValues = []optimus-ide-collabsdk.WorkspaceBuildParameter{}
		}

		var err error
		childParameterValuesJSON, err = json.Marshal(childParameterValues)
		if err != nil {
			return optimus-ide-collabsdk.WorkspaceBuild{}, httperror.NewResponseError(
	...
	err := api.Database.InTx(func(tx database.Store) error {
		...
		if createBuild.Orphan {
			if createBuild.Transition != optimus-ide-collabsdk.WorkspaceTransitionDelete {
	...
	if provisionerJob != nil {
		...
		if createBuild.Orphan && createBuild.Transition == optimus-ide-collabsdk.WorkspaceTransitionDelete && provisionerJob.CompletedAt.Valid {
...
func validateCreateWorkspaceBuildOnSuccess(createBuild optimus-ide-collabsdk.CreateWorkspaceBuildRequest) error {
	onSuccess := createBuild.OnSuccess
	if onSuccess == nil {
		return nil
	}

	if createBuild.Transition != optimus-ide-collabsdk.WorkspaceTransitionStop {
		return httperror.NewResponseError(http.StatusBadRequest, optimus-ide-collabsdk.Response{
			Message: "OnSuccess is only permitted when stopping a workspace.",
		})
	}
	if onSuccess.Transition != optimus-ide-collabsdk.WorkspaceTransitionStart {
		return httperror.NewResponseError(http.StatusBadRequest, optimus-ide-collabsdk.Response{
...
func (api *API) patchCancelWorkspaceBuild(rw http.ResponseWriter, r *http.Request) {
	...
	if api.Experiments.Enabled(optimus-ide-collabsdk.ExperimentWorkspaceBuildUpdates) {
		err = wspubsub.PublishWorkspaceBuildUpdate(ctx, api.Pubsub, optimus-ide-collabsdk.WorkspaceBuildUpdate{
			WorkspaceID:   workspace.ID,
			WorkspaceName: workspace.Name,
...
// @Tags Builds
// @Param workspacebuild path string true "Workspace build ID"
// @Success 200 {array} optimus-ide-collabsdk.WorkspaceBuildParameter
...
// @Success 200 {array} optimus-ide-collabsdk.ProvisionerJobLog
...
// @Param request body optimus-ide-collabsdk.UpdateWorkspaceBuildStateRequest true "Request body"
// @Success 204
...
func (api *API) workspaceBuildUpdateState(rw http.ResponseWriter, r *http.Request) {
	...
	var req optimus-ide-collabsdk.UpdateWorkspaceBuildStateRequest
...
func (api *API) convertWorkspaceBuilds(
	...
	apiBuilds := []optimus-ide-collabsdk.WorkspaceBuild{}
	for _, build := range workspaceBuilds {
...
func (api *API) convertWorkspaceBuild(
	...
	apiResources := make([]optimus-ide-collabsdk.WorkspaceResource, 0)
	...
	for _, resource := range resources {
		...
		apiAgents := make([]optimus-ide-collabsdk.WorkspaceAgent, 0)
		...
		for _, agent := range agents {
			...
			if err != nil {
				return optimus-ide-collabsdk.WorkspaceBuild{}, xerrors.Errorf("converting workspace agent: %w", err)
			}
	...
	transition := optimus-ide-collabsdk.WorkspaceTransition(build.Transition)
	return optimus-ide-collabsdk.WorkspaceBuild{
		ID:                      build.ID,
		...
		Job:                     apiJob,
		Deadline:                optimus-ide-collabsdk.NewNullTime(build.Deadline, !build.Deadline.IsZero()),
		MaxDeadline:             optimus-ide-collabsdk.NewNullTime(build.MaxDeadline, !build.MaxDeadline.IsZero()),
		Reason:                  optimus-ide-collabsdk.BuildReason(build.Reason),
		Resources:               apiResources,
		Status:                  optimus-ide-collabsdk.ConvertWorkspaceStatus(apiJob.Status, transition),
...
func convertWorkspaceResource(resource database.WorkspaceResource, agents []optimus-ide-collabsdk.WorkspaceAgent, metadata []database.WorkspaceResourceMetadatum) optimus-ide-collabsdk.WorkspaceResource {
	var convertedMetadata []optimus-ide-collabsdk.WorkspaceResourceMetadata
	for _, field := range metadata {
		convertedMetadata = append(convertedMetadata, optimus-ide-collabsdk.WorkspaceResourceMetadata{
			Key:       field.Key,
	...
	return optimus-ide-collabsdk.WorkspaceResource{
		ID:         resource.ID,
		CreatedAt:  resource.CreatedAt,
		JobID:      resource.JobID,
		Transition: optimus-ide-collabsdk.WorkspaceTransition(resource.Transition),
		Type:       resource.Type,
		Name:       resource.Name,
		Hide:       resource.Hide,
		Icon:       resource.Icon,
...
func (api *API) buildTimings(ctx context.Context, build database.WorkspaceBuild) (optimus-ide-collabsdk.WorkspaceBuildTimings, error) {
	provisionerTimings, err := api.Database.GetProvisionerJobTimingsByJobID(ctx, build.JobID)
	if err != nil && !errors.Is(err, sql.ErrNoRows) {
		return optimus-ide-collabsdk.WorkspaceBuildTimings{}, xerrors.Errorf("fetching provisioner job timings: %w", err)
	}
	...
	if err != nil && !errors.Is(err, sql.ErrNoRows) {
		return optimus-ide-collabsdk.WorkspaceBuildTimings{}, xerrors.Errorf("fetching workspace agent script timings: %w", err)
	}

	resources, err := api.Database.GetWorkspaceResourcesByJobID(ctx, build.JobID)
	if err != nil && !errors.Is(err, sql.ErrNoRows) {
		return optimus-ide-collabsdk.WorkspaceBuildTimings{}, xerrors.Errorf("fetching workspace resources: %w", err)
	}
	...
	agents, err := api.Database.GetWorkspaceAgentsByResourceIDs(dbauthz.AsSystemRestricted(ctx), resourceIDs)
	if err != nil && !errors.Is(err, sql.ErrNoRows) {
		return optimus-ide-collabsdk.WorkspaceBuildTimings{}, xerrors.Errorf("fetching workspace agents: %w", err)
	}

	res := optimus-ide-collabsdk.WorkspaceBuildTimings{
		ProvisionerTimings:     make([]optimus-ide-collabsdk.ProvisionerTiming, 0, len(provisionerTimings)),
		AgentScriptTimings:     make([]optimus-ide-collabsdk.AgentScriptTiming, 0, len(agentScriptTimings)),
		AgentConnectionTimings: make([]optimus-ide-collabsdk.AgentConnectionTiming, 0, len(agents)),
	}
	...
	for _, t := range provisionerTimings {
		...
		res.ProvisionerTimings = append(res.ProvisionerTimings, optimus-ide-collabsdk.ProvisionerTiming{
			JobID:     t.JobID,
			Stage:     optimus-ide-collabsdk.TimingStage(t.Stage),
			Source:    t.Source,
			Action:    t.Action,
			Resource:  t.Resource,
	...
	for _, t := range agentScriptTimings {
		...
		res.AgentScriptTimings = append(res.AgentScriptTimings, optimus-ide-collabsdk.AgentScriptTiming{
			StartedAt:          t.StartedAt,
			EndedAt:            t.EndedAt,
			ExitCode:           t.ExitCode,
			Stage:              optimus-ide-collabsdk.TimingStage(t.Stage),
	...
	for _, agent := range agents {
		...
		res.AgentConnectionTimings = append(res.AgentConnectionTimings, optimus-ide-collabsdk.AgentConnectionTiming{
			WorkspaceAgentID:   agent.ID.String(),
			WorkspaceAgentName: agent.Name,
			StartedAt:          agent.CreatedAt,
			Stage:              optimus-ide-collabsdk.TimingStageConnect,
