...
import (
	...
	"cdr.dev/slog/v3"
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabd/util/xnet"
...
func New(
	...
	return &Dispatcher{
		...
		userAgent:    "optimus-ide-collabd-agenthooks/" + optimus-ide-collabVersion,
...
func (d *Dispatcher) prepareAndPost(ctx context.Context, event Event, dispatchID uuid.UUID) dispatchOutcome {
	...
	token, err := agenthooks.SignClaims(d.secret, agenthooks.Claims{
		...
		Subject:    "optimus-ide-collab:chat:" + event.ChatID.String(),
		Audience:   d.hookURL,
		IssuedAt:   now.Unix(),
...
func decodeResponse(trimmed []byte, response *agenthooks.Response) error {
	if err := rejectDuplicateKeys(json.NewDeoptimus-ide-collab(bytes.NewReader(trimmed)), 0, keyMatchingFolded); err != nil {
		return err
	}
	deoptimus-ide-collab := json.NewDeoptimus-ide-collab(bytes.NewReader(trimmed))
	deoptimus-ide-collab.DisallowUnknownFields()
	if err := deoptimus-ide-collab.Decode(response); err != nil {
		return err
	}
	if err := deoptimus-ide-collab.Decode(&struct{}{}); err != io.EOF {
...
func rejectDuplicateKeys(deoptimus-ide-collab *json.Deoptimus-ide-collab, depth int, matching keyMatching) error {
	if depth > maxResponseJSONDepth {
		return xerrors.New("response JSON exceeds supported nesting depth")
	}
	token, err := deoptimus-ide-collab.Token()
	if err != nil {
		return err
	}
	...
	case '{':
		...
		for deoptimus-ide-collab.More() {
			keyToken, err := deoptimus-ide-collab.Token()
			if err != nil {
				return err
			}
			...
			nested := matching
			// The deoptimus-ide-collab binds this field case-insensitively, so the
			// boundary must be detected the same way.
			if matching == keyMatchingFolded && canonical == foldedInputOverride {
				nested = keyMatchingExact
			}
			if err := rejectDuplicateKeys(deoptimus-ide-collab, depth+1, nested); err != nil {
				return err
			}
		}
		_, err = deoptimus-ide-collab.Token()
		return err
	case '[':
		for deoptimus-ide-collab.More() {
			if err := rejectDuplicateKeys(deoptimus-ide-collab, depth+1, matching); err != nil {
				return err
			}
		}
		_, err = deoptimus-ide-collab.Token()
...
func validateUserPromptSubmitOverride(input json.RawMessage) error {
	...
	if err := rejectDuplicateKeys(json.NewDeoptimus-ide-collab(bytes.NewReader(input)), 0, keyMatchingFolded); err != nil {
		return xerrors.Errorf("user_prompt_submit input_override: %w", err)
	}
	var override struct {
		Prompt *string `json:"prompt"`
	}
	deoptimus-ide-collab := json.NewDeoptimus-ide-collab(bytes.NewReader(input))
	deoptimus-ide-collab.DisallowUnknownFields()
	if err := deoptimus-ide-collab.Decode(&override); err != nil {
