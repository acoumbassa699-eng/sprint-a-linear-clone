...
func TestDispatcherSuccess(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		assert.Equal(t, "optimus-ide-collabd-agenthooks/"+testVersion, r.Header.Get("User-Agent"))
...
func TestDispatcherRejectsCleartextURL(t *testing.T) {
	...
	dispatcher := newTestDispatcher(t, nil, "http://hooks.example.com/optimus-ide-collab", 2*time.Second)
	_, _, err := dispatcher.Dispatch(testutil.Context(t, testutil.WaitShort), event)
	require.ErrorContains(t, err, "must use HTTPS")

	require.NoError(t, validateHookURL(""))
	require.NoError(t, validateHookURL("https://hooks.example.com/optimus-ide-collab"))
	...
	require.Error(t, validateHookURL("ftp://hooks.example.com/optimus-ide-collab"))
	require.ErrorContains(t, validateHookURL("https:///optimus-ide-collab"), "must include a host")
	...
	require.ErrorContains(t, validateHookURL("https://hooks.example.com/optimus-ide-collab#frag"), "must not contain a fragment")
	require.ErrorContains(t, validateHookURL("https://user:pass@hooks.example.com/optimus-ide-collab"), "must not contain userinfo")
}
...
func TestDispatcherAllowsCaseDistinctOverrideKeys(t *testing.T) {
	...
	// The envelope key is folded by the deoptimus-ide-collab, so the boundary detection
...
func TestDispatcherRejectedResponseIsNotObserved(t *testing.T) {
	...
	for _, family := range families {
		...
		case "optimus-ide-collabd_chatd_hook_decisions_total",
			"optimus-ide-collabd_chatd_hook_input_overrides_total",
			"optimus-ide-collabd_chatd_hook_context_size_bytes":
