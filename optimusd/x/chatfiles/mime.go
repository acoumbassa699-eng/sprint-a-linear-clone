...
var (
	...
	// allowedPromptInputMediaTypes is derived from optimus-ide-collabsdk.AllChatAttachmentMediaTypes
	...
	// optimus-ide-collabsdk const block instead.
	allowedPromptInputMediaTypes = func() map[string]struct{} {
		m := make(map[string]struct{}, len(optimus-ide-collabsdk.AllChatAttachmentMediaTypes))
		for _, t := range optimus-ide-collabsdk.AllChatAttachmentMediaTypes {
			m[string(t)] = struct{}{}
		}
		return m
	}()
...
func HasSVGRootElement(data []byte) bool {
	...
	deoptimus-ide-collab := xml.NewDeoptimus-ide-collab(bytes.NewReader(data))
	for {
