...
import (
	...
	openaicomputeruse "github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabd/x/chatd/chatopenai/computeruse"
...
func TestUpdateChatSummary(t *testing.T) {
	...
	t.Run("TrimsAndPublishes", func(t *testing.T) {
		...
		require.Equal(t, optimus-ide-collabsdk.ChatWatchEventKindChatSummaryChange, events[0].Kind)
...
func TestComputerUseProviderAndModelFromConfig(t *testing.T) {
	...
	tests := []struct {
		name         string
		rawProvider  string
		wantProvider optimus-ide-collabsdk.ChatComputerUseProvider
		wantErr      string
	}{
		{
			name:         "DefaultAnthropic",
			rawProvider:  "",
			wantProvider: optimus-ide-collabsdk.ChatComputerUseProviderAnthropic,
		},
		{
			name:         "OpenAI",
			rawProvider:  " openai ",
			wantProvider: optimus-ide-collabsdk.ChatComputerUseProviderOpenAI,
		},
		{
			name:        "Unknown",
			rawProvider: "bogus",
...
func TestResolveUserProviderAPIKeysAndProviderForProviderTypeProviderMatch(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, "test-key", keys.APIKey(string(optimus-ide-collabsdk.ChatComputerUseProviderOpenAI)))
...
func TestAppendComputerUseProviderTool(t *testing.T) {
	...
	providerTools, err := appendComputerUseProviderTool(
		...
		computerUseProviderToolOptions{
			provider:      optimus-ide-collabsdk.ChatComputerUseProviderOpenAI,
			isComputerUse: true,
			logger:        slogtest.Make(t, &slogtest.Options{IgnoreErrors: true}),
		},
	)
...
func TestAppendComputerUseProviderTool_AnthropicHasNoResultMetadata(t *testing.T) {
	...
	providerTools, err := appendComputerUseProviderTool(
		nil,
		computerUseProviderToolOptions{
			provider:      optimus-ide-collabsdk.ChatComputerUseProviderAnthropic,
			isComputerUse: true,
			logger:        slogtest.Make(t, &slogtest.Options{IgnoreErrors: true}),
		},
	)
...
func TestRegenerateChatTitle_PersistsAndBroadcasts(t *testing.T) {
	...
	messageEvents := make(chan struct {
		payload optimus-ide-collabsdk.ChatWatchEvent
		err     error
	}, 1)
	cancelSub, err := pubsub.SubscribeWithErr(
		optimus-ide-collabdpubsub.ChatWatchEventChannel(ownerID),
		optimus-ide-collabdpubsub.HandleChatWatchEvent(func(_ context.Context, payload optimus-ide-collabsdk.ChatWatchEvent, err error) {
	...
	db.EXPECT().GetChatMessagesByChatIDAscPaginated(
		...
		mustChatMessage(
			...
			database.ChatMessageVisibilityBoth,
			optimus-ide-collabsdk.ChatMessageText("checking the diff now"),
		),
	}, nil)
	...
	case event := <-messageEvents:
		...
		require.Equal(t, optimus-ide-collabsdk.ChatWatchEventKindTitleChange, event.payload.Kind)
...
func TestRegenerateChatTitle_SkipsPersistWhenTitleChangedConcurrently(t *testing.T) {
	...
	titleEvents := make(chan optimus-ide-collabsdk.ChatWatchEvent, 1)
	cancelSub, err := pubsub.SubscribeWithErr(
		optimus-ide-collabdpubsub.ChatWatchEventChannel(ownerID),
		optimus-ide-collabdpubsub.HandleChatWatchEvent(func(_ context.Context, payload optimus-ide-collabsdk.ChatWatchEvent, err error) {
			require.NoError(t, err)
...
func TestSubscribeSurfacesTransientLookupFailureAsInitialError(t *testing.T) {
	...
	require.Equal(t, optimus-ide-collabsdk.ChatStreamEventTypeError, snapshot[0].Type)
...
func TestResolveUserCompactionThreshold(t *testing.T) {
	...
	userID := uuid.New()
	modelConfigID := uuid.New()
	expectedKey := optimus-ide-collabsdk.CompactionThresholdKey(modelConfigID)
...
func TestResolveFallbackModelConfigID(t *testing.T) {
	...
	// An explicit model whose provider was disabled after the optimus-ide-collabd
