...
func personalModelOverrideContextForSubagent(
	overrideContext optimus-ide-collabsdk.ChatModelOverrideContext,
) (optimus-ide-collabsdk.ChatPersonalModelOverrideContext, error) {
	switch overrideContext {
	case optimus-ide-collabsdk.ChatModelOverrideContextGeneral:
		return optimus-ide-collabsdk.ChatPersonalModelOverrideContextGeneral, nil
	case optimus-ide-collabsdk.ChatModelOverrideContextExplore:
		return optimus-ide-collabsdk.ChatPersonalModelOverrideContextExplore, nil
	default:
		return "", xerrors.Errorf(
...
func (p *Server) resolvePersonalSubagentModelConfigID(
	...
	parsed := ParseChatPersonalModelOverride(
		raw,
		optimus-ide-collabsdk.ChatPersonalModelOverrideModeDeploymentDefault,
	)
	if parsed.Malformed {
		p.logger.Debug(ctx,
...
func withResolvedReasoningEffort(
	...
	callConfig.ReasoningEffort = &optimus-ide-collabsdk.ChatModelReasoningEffortConfig{
	...
	if raw := strings.TrimSpace(args.ReasoningEffort); raw != "" {
		...
		if !chatprovider.IsValidReasoningEffort(effort) {
			return nil, nil, xerrors.Errorf(
				...
				strings.Join(optimus-ide-collabsdk.ChatModelReasoningEffortValues(), ", "),
			)
		}
...
func (p *Server) createChildSubagentChatWithOptions(
	...
	userContent, err := chatprompt.MarshalParts([]optimus-ide-collabsdk.ChatMessagePart{optimus-ide-collabsdk.ChatMessageText(prompt)})
	if err != nil {
	...
	if deploymentPrompt != "" {
		deploymentContent, err := chatprompt.MarshalParts([]optimus-ide-collabsdk.ChatMessagePart{
	...
	if childSystemPrompt != "" {
		childSystemPromptContent, err := chatprompt.MarshalParts([]optimus-ide-collabsdk.ChatMessagePart{
			optimus-ide-collabsdk.ChatMessageText(childSystemPrompt),
		})
		if err != nil {
	...
	p.publishChatPubsubEvent(child, optimus-ide-collabsdk.ChatWatchEventKindCreated, nil)
	return child, nil
}
...
func (p *Server) sendSubagentMessage(
	...
	sendResult, err := p.SendMessage(ctx, SendMessageOptions{
		...
		Content:      []optimus-ide-collabsdk.ChatMessagePart{optimus-ide-collabsdk.ChatMessageText(message)},
...
func (p *Server) awaitSubagentCompletion(
	...
	cancel, subErr := p.pubsub.SubscribeWithErr(
		optimus-ide-collabdpubsub.ChatStateUpdateChannel(targetChatID),
...
// raw stored bytes.
func subagentLastError(raw pqtype.NullRawMessage) (*optimus-ide-collabsdk.ChatError, string) {
	if !raw.Valid {
		return nil, ""
	}
