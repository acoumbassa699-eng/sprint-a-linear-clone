...
func ComputeUsagePeriodBounds(now time.Time, period optimus-ide-collabsdk.ChatUsageLimitPeriod) (start, end time.Time) {
	utcNow := now.UTC()

	switch period {
	case optimus-ide-collabsdk.ChatUsageLimitPeriodDay:
		start = time.Date(utcNow.Year(), utcNow.Month(), utcNow.Day(), 0, 0, 0, 0, time.UTC)
		end = start.AddDate(0, 0, 1)
	case optimus-ide-collabsdk.ChatUsageLimitPeriodWeek:
	...
	case optimus-ide-collabsdk.ChatUsageLimitPeriodMonth:
...
func ResolveUsageLimitStatus(ctx context.Context, db database.Store, userID uuid.UUID, organizationID uuid.NullUUID, now time.Time) (*optimus-ide-collabsdk.ChatUsageLimitStatus, error) {
	...
	return &optimus-ide-collabsdk.ChatUsageLimitStatus{
		IsLimited:        true,
		Period:           period,
...
func mapDBPeriodToSDK(dbPeriod string) (optimus-ide-collabsdk.ChatUsageLimitPeriod, bool) {
	switch dbPeriod {
	case string(optimus-ide-collabsdk.ChatUsageLimitPeriodDay):
		return optimus-ide-collabsdk.ChatUsageLimitPeriodDay, true
	case string(optimus-ide-collabsdk.ChatUsageLimitPeriodWeek):
		return optimus-ide-collabsdk.ChatUsageLimitPeriodWeek, true
	case string(optimus-ide-collabsdk.ChatUsageLimitPeriodMonth):
		return optimus-ide-collabsdk.ChatUsageLimitPeriodMonth, true
	default:
		return "", false
	}
}
