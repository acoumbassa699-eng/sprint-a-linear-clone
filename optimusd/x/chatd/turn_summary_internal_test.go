...
func TestUpdateLastTurnSummaryRejectsStaleWrites(t *testing.T) {
	...
	assistantContent, err := chatprompt.MarshalParts([]optimus-ide-collabsdk.ChatMessagePart{
		optimus-ide-collabsdk.ChatMessageText("assistant response"),
	})
	require.NoError(t, err)
...
func (d *recordingWebpushDispatcher) Dispatch(
	_ context.Context,
	_ uuid.UUID,
	_ optimus-ide-collabsdk.WebpushMessage,
) error {
	d.dispatchCount.Add(1)
	return nil
}

func (*recordingWebpushDispatcher) Test(_ context.Context, _ optimus-ide-collabsdk.WebpushSubscription) error {
	return nil
}
