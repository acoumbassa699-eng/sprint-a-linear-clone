...
type Part struct {
	...
	MessagePart optimus-ide-collabsdk.ChatMessagePart
}
...
func (b *Buffer) AddPart(key Key, role optimus-ide-collabsdk.ChatMessageRole, part optimus-ide-collabsdk.ChatMessagePart) error {
	b.mu.Lock()
	defer b.mu.Unlock()
	if b.closed {
		return ErrMessagePartBufferClosed
