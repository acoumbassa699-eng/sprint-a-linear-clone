...
func TestBuffer_AddPartAndGetParts(t *testing.T) {
	...
	key := testEpisodeKey()
	require.NoError(t, buffer.CreateEpisode(key))
	require.NoError(t, buffer.AddPart(key, optimus-ide-collabsdk.ChatMessageRoleAssistant, optimus-ide-collabsdk.ChatMessageText("hello")))

	parts, err := buffer.GetParts(key)
	require.NoError(t, err)
	require.Len(t, parts, 1)
	require.Equal(t, int64(1), parts[0].Seq)
	require.Equal(t, optimus-ide-collabsdk.ChatMessageRoleAssistant, parts[0].Role)
	require.Equal(t, optimus-ide-collabsdk.ChatMessageText("hello"), parts[0].MessagePart)
}
...
func TestBuffer_AddPartMissingEpisodeReturnsNotFound(t *testing.T) {
	...
	buffer := messagepartbuffer.New(messagepartbuffer.Options{})
	defer buffer.Close()
	err := buffer.AddPart(testEpisodeKey(), optimus-ide-collabsdk.ChatMessageRoleAssistant, optimus-ide-collabsdk.ChatMessageText("hello"))
...
func TestBuffer_AddPartFullEpisodeReturnsFull(t *testing.T) {
	...
	key := testEpisodeKey()
	require.NoError(t, buffer.CreateEpisode(key))
	err := buffer.AddPart(key, optimus-ide-collabsdk.ChatMessageRoleAssistant, optimus-ide-collabsdk.ChatMessageText("hello"))
...
func TestBuffer_CloseEpisodeMissingCreatesClosedEpisode(t *testing.T) {
	...
	require.NoError(t, err)
	require.Empty(t, parts)
	err = buffer.AddPart(key, optimus-ide-collabsdk.ChatMessageRoleAssistant, optimus-ide-collabsdk.ChatMessageText("tail"))
...
func TestBuffer_SubscribeExistingReplaysThenStreamsLiveParts(t *testing.T) {
	...
	key := testEpisodeKey()
	require.NoError(t, buffer.CreateEpisode(key))
	require.NoError(t, buffer.AddPart(key, optimus-ide-collabsdk.ChatMessageRoleAssistant, optimus-ide-collabsdk.ChatMessageText("before")))
	...
	require.NoError(t, buffer.AddPart(key, optimus-ide-collabsdk.ChatMessageRoleAssistant, optimus-ide-collabsdk.ChatMessageText("after")))
...
func TestBuffer_SubscribeBeforeCreateReturnsAndWaitsWithoutNotFound(t *testing.T) {
	...
	require.NoError(t, buffer.CreateEpisode(key))
	require.NoError(t, buffer.AddPart(key, optimus-ide-collabsdk.ChatMessageRoleAssistant, optimus-ide-collabsdk.ChatMessageText("live")))
...
func TestBuffer_AddPartAssignsContiguousSeq(t *testing.T) {
	...
	require.NoError(t, buffer.CreateEpisode(key))
	for i := range 3 {
		require.NoError(t, buffer.AddPart(key, optimus-ide-collabsdk.ChatMessageRoleAssistant, optimus-ide-collabsdk.ChatMessageText(string(rune('a'+i)))))
	}
	parts, err := buffer.GetParts(key)
	require.NoError(t, err)
...
func TestBuffer_EpisodeByteLimitUsesJSONAccounting(t *testing.T) {
	...
	limit := serializedPartBytes(t, messagepartbuffer.Part{Seq: 1, Role: optimus-ide-collabsdk.ChatMessageRoleAssistant, MessagePart: part})
	buffer := messagepartbuffer.New(messagepartbuffer.Options{MaxEpisodeBytes: limit})
	defer buffer.Close()
	key := testEpisodeKey()
	require.NoError(t, buffer.CreateEpisode(key))
	require.NoError(t, buffer.AddPart(key, optimus-ide-collabsdk.ChatMessageRoleAssistant, part))
	err := buffer.AddPart(key, optimus-ide-collabsdk.ChatMessageRoleAssistant, optimus-ide-collabsdk.ChatMessageText("too much"))
	require.ErrorIs(t, err, messagepartbuffer.ErrEpisodeFull)
}
...
func TestBuffer_GCClosedEpisodeAfterGraceAndNoSubscribers(t *testing.T) {
	...
	key := testEpisodeKey()
	require.NoError(t, buffer.CreateEpisode(key))
	require.NoError(t, buffer.AddPart(key, optimus-ide-collabsdk.ChatMessageRoleAssistant, optimus-ide-collabsdk.ChatMessageText("held")))
	ctx := testutil.Context(t, testutil.WaitLong)
	ch, cancel, err := buffer.SubscribeToEpisode(ctx, key)
	require.NoError(t, err)
	require.NoError(t, buffer.CloseEpisode(key))
...
func TestBuffer_GCRetainedSubscribedEpisodeDoesNotBlockOtherExpiredEpisodes(t *testing.T) {
	...
	require.NoError(t, buffer.AddPart(retainedKey, optimus-ide-collabsdk.ChatMessageRoleAssistant, optimus-ide-collabsdk.ChatMessageText("held")))
	require.NoError(t, buffer.CreateEpisode(collectedKey))
	require.NoError(t, buffer.AddPart(collectedKey, optimus-ide-collabsdk.ChatMessageRoleAssistant, optimus-ide-collabsdk.ChatMessageText("collect me")))
	ctx := testutil.Context(t, testutil.WaitLong)
...
func TestBuffer_SlowSubscriberClosed(t *testing.T) {
	...
	require.NoError(t, buffer.AddPart(key, optimus-ide-collabsdk.ChatMessageRoleAssistant, optimus-ide-collabsdk.ChatMessageText("blocked")))
	call := trap.MustWait(ctx)
	call.MustRelease(ctx)
	clock.Advance(time.Second).MustWait(ctx)
...
func serializedPartBytes(t *testing.T, part messagepartbuffer.Part) int64 {
	...
	data, err := json.Marshal(struct {
		Seq  int64                    `json:"seq"`
		Role optimus-ide-collabsdk.ChatMessageRole `json:"role"`
		Part optimus-ide-collabsdk.ChatMessagePart `json:"part"`
	}{
		Seq:  part.Seq,
		Role: part.Role,
