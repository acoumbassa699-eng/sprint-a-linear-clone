...
func TestAdvisorToolPublishesAdviceDeltasWithToolCallID(t *testing.T) {
	...
	resp := runAdvisorToolWithPublisher(t, tool, chatadvisor.AdvisorArgs{Question: "What's safest?"},
		func(role optimus-ide-collabsdk.ChatMessageRole, part optimus-ide-collabsdk.ChatMessagePart) {
			require.Equal(t, optimus-ide-collabsdk.ChatMessageRoleTool, role)
			published = append(published, part)
		})
	require.False(t, resp.IsError)
	require.Len(t, published, 2)
	for _, part := range published {
		require.Equal(t, optimus-ide-collabsdk.ChatMessagePartTypeToolResult, part.Type)
...
func runAdvisorTool(
	...
	t.Helper()
	return runAdvisorToolWithPublisher(t, tool, args, func(optimus-ide-collabsdk.ChatMessageRole, optimus-ide-collabsdk.ChatMessagePart) {})
}
