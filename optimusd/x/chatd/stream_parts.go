...
func (e streamPartsEndpoint) serve(ctx context.Context, transport streamPartsServerTransport) error {
	...
	for {
		...
		case part, ok := <-parts:
			...
			lastSeq = part.Seq
			event := optimus-ide-collabsdk.ChatStreamEvent{
			...
			if err := transport.WriteEvents(ctx, []optimus-ide-collabsdk.ChatStreamEvent{event}); err != nil {
				if ctx.Err() != nil || streamPartsExpectedTransportClose(err) {
					return nil
				}
				return err
...
func StreamPartFromEvent(event optimus-ide-collabsdk.ChatStreamEvent) (StreamPart, bool) {
