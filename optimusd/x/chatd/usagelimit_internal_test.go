...
func TestComputeUsagePeriodBounds(t *testing.T) {
	...
	tests := []struct {
		name      string
		now       time.Time
		period    optimus-ide-collabsdk.ChatUsageLimitPeriod
		wantStart time.Time
		wantEnd   time.Time
	}{
		{
			...
			period:    optimus-ide-collabsdk.ChatUsageLimitPeriodDay,
		...
		},
		{
			...
			period:    optimus-ide-collabsdk.ChatUsageLimitPeriodWeek,
		...
		},
		{
			...
			now:       time.Date(2025, time.June, 15, 0, 0, 0, 0, time.UTC),
			period:    optimus-ide-collabsdk.ChatUsageLimitPeriodMonth,
