...
func stripForeignProviderExecutedToolRows(
	...
	for _, row := range rows {
		...
		kept := make([]optimus-ide-collabsdk.ChatMessagePart, 0, len(parts))
		var removedCalls, removedResults int
		for _, part := range parts {
			switch {
			case part.Type == optimus-ide-collabsdk.ChatMessagePartTypeToolCall && part.ProviderExecuted:
				removedCalls++
			case part.Type == optimus-ide-collabsdk.ChatMessagePartTypeToolResult && part.ProviderExecuted:
				removedResults++
			default:
