...
func (m *runnerManager) releaseOwnershipHints(keys []runnerKey) {
	...
	for _, key := range keys {
		payload, err := json.Marshal(optimus-ide-collabdpubsub.ChatStateOwnershipMessage{
			ChatID:          key.ChatID,
		...
		if err := m.opts.Pubsub.Publish(optimus-ide-collabdpubsub.ChatStateOwnershipChannel, payload); err != nil {
