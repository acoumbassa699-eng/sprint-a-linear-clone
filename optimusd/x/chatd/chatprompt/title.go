...
func TitleText(parts []optimus-ide-collabsdk.ChatMessagePart, pasteText map[uuid.UUID]string) string {
...
func SyntheticPasteFileIDs(parts []optimus-ide-collabsdk.ChatMessagePart) []uuid.UUID {
	var ids []uuid.UUID
	for _, part := range parts {
		if part.Type != optimus-ide-collabsdk.ChatMessagePartTypeFile || !part.FileID.Valid {
			continue
