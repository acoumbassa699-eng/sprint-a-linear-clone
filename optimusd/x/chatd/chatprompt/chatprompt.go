...
import (
	...
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabd/util/shellparse"
...
func ConvertMessagesWithFiles(
	...
	for i, msg := range messages {
		...
		parsed[i] = parsedMessage{role: optimus-ide-collabsdk.ChatMessageRole(msg.Role), parts: parts}

		// Collect file IDs from user messages for resolution.
		if resolver != nil && msg.Role == database.ChatMessageRoleUser {
			for _, part := range parts {
				if part.Type == optimus-ide-collabsdk.ChatMessagePartTypeFile && part.FileID.Valid {
	...
	for _, pm := range parsed {
		...
		switch pm.role {
		case optimus-ide-collabsdk.ChatMessageRoleSystem:
		...
		case optimus-ide-collabsdk.ChatMessageRoleUser:
		...
		case optimus-ide-collabsdk.ChatMessageRoleAssistant:
		...
		case optimus-ide-collabsdk.ChatMessageRoleTool:
...
const (
	...
	// ContentVersionV1 stores content as []optimus-ide-collabsdk.ChatMessagePart
	// JSON for all roles.
...
func ParseContent(msg database.ChatMessage) ([]optimus-ide-collabsdk.ChatMessagePart, error) {
	if !msg.Content.Valid || len(msg.Content.RawMessage) == 0 {
		return nil, nil
	}

	role := optimus-ide-collabsdk.ChatMessageRole(msg.Role)
...
func parseLegacyContent(role optimus-ide-collabsdk.ChatMessageRole, raw pqtype.NullRawMessage) ([]optimus-ide-collabsdk.ChatMessagePart, error) {
	switch role {
	case optimus-ide-collabsdk.ChatMessageRoleSystem:
...
func parseContentV1(role optimus-ide-collabsdk.ChatMessageRole, raw pqtype.NullRawMessage) ([]optimus-ide-collabsdk.ChatMessagePart, error) {
...
// single text part.
func parseSystemRole(raw pqtype.NullRawMessage) ([]optimus-ide-collabsdk.ChatMessagePart, error) {
	var text string
	...
	return []optimus-ide-collabsdk.ChatMessagePart{optimus-ide-collabsdk.ChatMessageText(text)}, nil
}
...
// invariant tied to Go's json deoptimus-ide-collab behavior for []byte.
func parseAssistantRole(raw pqtype.NullRawMessage) ([]optimus-ide-collabsdk.ChatMessagePart, error) {
	if isFantasyEnvelopeFormat(raw.RawMessage) {
		return parseLegacyFantasyBlocks(string(optimus-ide-collabsdk.ChatMessageRoleAssistant), raw)
	}
...
func parseToolRole(raw pqtype.NullRawMessage) ([]optimus-ide-collabsdk.ChatMessagePart, error) {
	// Try SDK parts.
	var parts []optimus-ide-collabsdk.ChatMessagePart
	...
	parts = make([]optimus-ide-collabsdk.ChatMessagePart, 0, len(rows))
	for _, row := range rows {
		part := optimus-ide-collabsdk.ChatMessageToolResult(row.ToolCallID, row.ToolName, row.Result, row.IsError, row.IsMedia)
...
func parseUserRole(raw pqtype.NullRawMessage) ([]optimus-ide-collabsdk.ChatMessagePart, error) {
...
func parseLegacyUserBlocks(raw pqtype.NullRawMessage) ([]optimus-ide-collabsdk.ChatMessagePart, error) {
	var rawBlocks []json.RawMessage
	if err := json.Unmarshal(raw.RawMessage, &rawBlocks); err != nil {
		return nil, xerrors.Errorf("parse user content: %w", err)
	}

	parts := make([]optimus-ide-collabsdk.ChatMessagePart, 0, len(rawBlocks))
...
func parseLegacyFantasyBlocks(role string, raw pqtype.NullRawMessage) ([]optimus-ide-collabsdk.ChatMessagePart, error) {
	var rawBlocks []json.RawMessage
	if err := json.Unmarshal(raw.RawMessage, &rawBlocks); err != nil {
		return nil, xerrors.Errorf("parse %s content: %w", role, err)
	}
...
func hasNonEmptyType(parts []optimus-ide-collabsdk.ChatMessagePart) bool {
	for _, p := range parts {
		if p.Type != "" {
			return true
		}
...
func hasToolResultType(parts []optimus-ide-collabsdk.ChatMessagePart) bool {
	for _, p := range parts {
		if p.Type == optimus-ide-collabsdk.ChatMessagePartTypeToolResult {
			return true
		}
	}
	return false
...
func PartFromContent(block fantasy.Content) optimus-ide-collabsdk.ChatMessagePart {
...
func sdkPartFromContent(
	...
	case fantasy.SourceContent:
		return optimus-ide-collabsdk.ChatMessagePart{
			Type:             optimus-ide-collabsdk.ChatMessagePartTypeSource,
			SourceID:         value.ID,
			URL:              value.URL,
			Title:            value.Title,
			ProviderMetadata: marshalProviderMetadata(value.ProviderMetadata),
...
// database.
func ToolResultToPart(toolCallID, toolName string, result json.RawMessage, isError bool, isMedia bool) optimus-ide-collabsdk.ChatMessagePart {
	return optimus-ide-collabsdk.ChatMessageToolResult(toolCallID, toolName, result, isError, isMedia)
}
...
// may carry structured JSON. Keep in sync with optimus-ide-collabd/x/chatd/subagent.go.
...
func MarshalParts(parts []optimus-ide-collabsdk.ChatMessagePart) (pqtype.NullRawMessage, error) {
	if len(parts) == 0 {
		return pqtype.NullRawMessage{}, nil
	}
...
// Any new text type added to optimus-ide-collabsdk.AllChatAttachmentMediaTypes must
...
// representation.
func fileReferencePartToText(part optimus-ide-collabsdk.ChatMessagePart) string {
...
func toolResultPartToMessagePart(logger slog.Logger, part optimus-ide-collabsdk.ChatMessagePart) fantasy.ToolResultPart {
...
func partsToMessageParts(
	...
	for _, part := range parts {
		...
		case optimus-ide-collabsdk.ChatMessagePartTypeReasoning:
			opts := providerMetadataToOptions(logger, part.ProviderMetadata)
		...
		case optimus-ide-collabsdk.ChatMessagePartTypeFile:
			data := part.Data
		...
		case optimus-ide-collabsdk.ChatMessagePartTypeContextFile:
		...
		case optimus-ide-collabsdk.ChatMessagePartTypeSource:
...
func encodeNulInJSON(raw json.RawMessage) json.RawMessage {
	...
	dec := json.NewDeoptimus-ide-collab(bytes.NewReader(raw))
	dec.UseNumber()
	var v any
...
func encodeNulInParts(parts []optimus-ide-collabsdk.ChatMessagePart) []optimus-ide-collabsdk.ChatMessagePart {
	encoded := make([]optimus-ide-collabsdk.ChatMessagePart, len(parts))
	copy(encoded, parts)
	for i := range encoded {
		p := &encoded[i]
...
func decodeNulInParts(parts []optimus-ide-collabsdk.ChatMessagePart) {
	for i := range parts {
		p := &parts[i]
