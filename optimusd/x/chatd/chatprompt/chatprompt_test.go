...
func testMsg(role optimus-ide-collabsdk.ChatMessageRole, raw pqtype.NullRawMessage) database.ChatMessage {
...
func TestConvertMessagesWithFilesPreservesEmptyRedactedReasoning(t *testing.T) {
	...
	content, err := chatprompt.MarshalParts([]optimus-ide-collabsdk.ChatMessagePart{
		...
		optimus-ide-collabsdk.ChatMessageText("done"),
	})
	require.NoError(t, err)
...
func TestConvertMessagesWithFilesRoundTripsAnthropicInterleavedWebSearch(t *testing.T) {
			...
			ProviderMetadata: fantasy.ProviderMetadata{
				fantasyanthropic.Name: &fantasyanthropic.WebSearchResultMetadata{
					Results: []fantasyanthropic.WebSearchResultItem{
						{
							URL:              "https://optimus-ide-collab.com",
	...
	storedParts := make([]optimus-ide-collabsdk.ChatMessagePart, 0, len(content))
	...
	require.Equal(t, optimus-ide-collabsdk.ChatMessagePartTypeReasoning, parsedParts[0].Type)
	require.Equal(t, optimus-ide-collabsdk.ChatMessagePartTypeToolCall, parsedParts[1].Type)
	require.Equal(t, optimus-ide-collabsdk.ChatMessagePartTypeToolResult, parsedParts[2].Type)
	require.Equal(t, optimus-ide-collabsdk.ChatMessagePartTypeReasoning, parsedParts[3].Type)
	require.Equal(t, optimus-ide-collabsdk.ChatMessagePartTypeText, parsedParts[4].Type)
	...
	require.JSONEq(t, `{"query":"optimus-ide-collab"}`, call.Input)
...
func testMsgV1(role optimus-ide-collabsdk.ChatMessageRole, raw pqtype.NullRawMessage) database.ChatMessage {
	return database.ChatMessage{
		Role:           database.ChatMessageRole(role),
		Content:        raw,
		ContentVersion: chatprompt.CurrentContentVersion,
...
func TestInjectMissingToolResults_SkipsProviderExecutedAndInjectsLocal(t *testing.T) {
	...
	providerCall := optimus-ide-collabsdk.ChatMessageToolCall(
		"srvtoolu_web_search",
		"web_search",
		json.RawMessage(`{"query":"optimus-ide-collab"}`),
	)
	providerCall.ProviderExecuted = true
	localCall := optimus-ide-collabsdk.ChatMessageToolCall(
		"toolu_read",
		"read_file",
...
func TestInjectMissingToolResults_AdjacentAssistantsInjectLocalResults(t *testing.T) {
	...
	assistantAContent, err := chatprompt.MarshalParts([]optimus-ide-collabsdk.ChatMessagePart{
		optimus-ide-collabsdk.ChatMessageText("assistant a"),
		optimus-ide-collabsdk.ChatMessageToolCall(
			"toolu_a",
			"read_file",
	...
	require.NoError(t, err)
	assistantBContent, err := chatprompt.MarshalParts([]optimus-ide-collabsdk.ChatMessagePart{
		optimus-ide-collabsdk.ChatMessageText("assistant b"),
		optimus-ide-collabsdk.ChatMessageToolCall(
			"toolu_b",
			"read_file",
	...
	userContent, err := chatprompt.MarshalParts([]optimus-ide-collabsdk.ChatMessagePart{
		optimus-ide-collabsdk.ChatMessageText("next user message"),
	})
	require.NoError(t, err)
...
func TestSDKPartsNeverProduceFantasyEnvelopeShape(t *testing.T) {
	...
	parts := []optimus-ide-collabsdk.ChatMessagePart{
		...
		{Type: optimus-ide-collabsdk.ChatMessagePartTypeFile, FileID: uuid.NullUUID{UUID: uuid.New(), Valid: true}, MediaType: "image/png"},
		{Type: optimus-ide-collabsdk.ChatMessagePartTypeFile, MediaType: "image/png", Data: []byte("fake-image-data")},
		{Type: optimus-ide-collabsdk.ChatMessagePartTypeFileReference, FileName: "main.go", StartLine: 1, EndLine: 10, Content: "func main() {}"},
		{Type: optimus-ide-collabsdk.ChatMessagePartTypeReasoning, Text: "thinking..."},
		{Type: optimus-ide-collabsdk.ChatMessagePartTypeToolCall, ToolCallID: "abc", ToolName: "read_file", Args: json.RawMessage(`{"path":"main.go"}`)},
		{Type: optimus-ide-collabsdk.ChatMessagePartTypeToolResult, ToolCallID: "abc", ToolName: "read_file", Result: json.RawMessage(`{"output":"code"}`)},
		{Type: optimus-ide-collabsdk.ChatMessagePartTypeSource, SourceID: "s1", URL: "https://example.com", Title: "Example"},
	}
...
func TestParseContent_BackwardCompat(t *testing.T) {
	...
	newAssistantWithMeta, err := chatprompt.MarshalParts([]optimus-ide-collabsdk.ChatMessagePart{{
		Type:             optimus-ide-collabsdk.ChatMessagePartTypeText,
		Text:             "here is my answer",
		ProviderMetadata: sdkMetadata,
	}})
	require.NoError(t, err)

	newAssistantToolCall, err := chatprompt.MarshalParts([]optimus-ide-collabsdk.ChatMessagePart{{
	...
	newToolResult, err := chatprompt.MarshalParts([]optimus-ide-collabsdk.ChatMessagePart{{
	...
	tests := []struct {
		...
		check func(t *testing.T, parts []optimus-ide-collabsdk.ChatMessagePart)
	}{
		{
			name: "system/plain_string",
			role: optimus-ide-collabsdk.ChatMessageRoleSystem,
			raw:  nullRaw(mustJSON(t, "You are helpful.")),
			check: func(t *testing.T, parts []optimus-ide-collabsdk.ChatMessagePart) {
				require.Len(t, parts, 1)
				assert.Equal(t, optimus-ide-collabsdk.ChatMessagePartTypeText, parts[0].Type)
				assert.Equal(t, "You are helpful.", parts[0].Text)
			},
		},
		{
			name: "user/fantasy_text",
			role: optimus-ide-collabsdk.ChatMessageRoleUser,
		...
		},
		{
			...
			check: func(t *testing.T, parts []optimus-ide-collabsdk.ChatMessagePart) {
				require.Len(t, parts, 1)
				assert.Equal(t, optimus-ide-collabsdk.ChatMessagePartTypeFile, parts[0].Type)
		...
		},
		{
			...
			check: func(t *testing.T, parts []optimus-ide-collabsdk.ChatMessagePart) {
				require.Len(t, parts, 1)
				assert.Equal(t, optimus-ide-collabsdk.ChatMessagePartTypeReasoning, parts[0].Type)
		...
		},
		{
			...
			check: func(t *testing.T, parts []optimus-ide-collabsdk.ChatMessagePart) {
				require.Len(t, parts, 1)
				assert.Equal(t, optimus-ide-collabsdk.ChatMessagePartTypeSource, parts[0].Type)
		...
		},
		{
			...
			check: func(t *testing.T, parts []optimus-ide-collabsdk.ChatMessagePart) {
				require.Len(t, parts, 1)
				assert.Equal(t, optimus-ide-collabsdk.ChatMessagePartTypeToolCall, parts[0].Type)
		...
		},
		{
			...
			role: optimus-ide-collabsdk.ChatMessageRoleTool,
			...
			check: func(t *testing.T, parts []optimus-ide-collabsdk.ChatMessagePart) {
				require.Len(t, parts, 1)
				assert.Equal(t, optimus-ide-collabsdk.ChatMessagePartTypeToolResult, parts[0].Type)
				assert.Equal(t, "call_123", parts[0].ToolCallID)
				assert.Equal(t, "read_file", parts[0].ToolName)
		...
		},
		{
			name: "user/sdk_text",
			role: optimus-ide-collabsdk.ChatMessageRoleUser,
			raw: nullRaw(mustJSON(t, []optimus-ide-collabsdk.ChatMessagePart{
				{Type: optimus-ide-collabsdk.ChatMessagePartTypeText, Text: "hello sdk"},
			})),
			check: func(t *testing.T, parts []optimus-ide-collabsdk.ChatMessagePart) {
				require.Len(t, parts, 1)
				assert.Equal(t, optimus-ide-collabsdk.ChatMessagePartTypeText, parts[0].Type)
		...
		},
		{
			...
			check: func(t *testing.T, parts []optimus-ide-collabsdk.ChatMessagePart) {
				require.Len(t, parts, 1)
				assert.Equal(t, optimus-ide-collabsdk.ChatMessagePartTypeFileReference, parts[0].Type)
		...
		},
		{
			...
			role: optimus-ide-collabsdk.ChatMessageRoleUser,
			raw: nullRaw(mustJSON(t, []optimus-ide-collabsdk.ChatMessagePart{
				{Type: optimus-ide-collabsdk.ChatMessagePartTypeFile, FileID: uuid.NullUUID{UUID: fileID, Valid: true}, MediaType: "image/png"},
			})),
			check: func(t *testing.T, parts []optimus-ide-collabsdk.ChatMessagePart) {
				require.Len(t, parts, 1)
				assert.Equal(t, optimus-ide-collabsdk.ChatMessagePartTypeFile, parts[0].Type)
...
func TestParseContent_V1(t *testing.T) {
	...
	t.Run("system", func(t *testing.T) {
		t.Parallel()
		raw, err := chatprompt.MarshalParts([]optimus-ide-collabsdk.ChatMessagePart{
			optimus-ide-collabsdk.ChatMessageText("You are helpful."),
		})
		require.NoError(t, err)

		parts, err := chatprompt.ParseContent(testMsgV1(optimus-ide-collabsdk.ChatMessageRoleSystem, raw))
		require.NoError(t, err)
		require.Len(t, parts, 1)
		assert.Equal(t, optimus-ide-collabsdk.ChatMessagePartTypeText, parts[0].Type)
		assert.Equal(t, "You are helpful.", parts[0].Text)
	...
	t.Run("system_bare_string_errors", func(t *testing.T) {
		...
		_, err := chatprompt.ParseContent(testMsgV1(
			optimus-ide-collabsdk.ChatMessageRoleSystem,
	...
	t.Run("unknown_version_errors", func(t *testing.T) {
		t.Parallel()
		msg := testMsgV1(optimus-ide-collabsdk.ChatMessageRoleUser, nullRaw(json.RawMessage(`[{"type":"text","text":"hi"}]`)))
...
func TestProviderMetadataRoundTrip(t *testing.T) {
	...
	parts, err := chatprompt.ParseContent(testMsg(optimus-ide-collabsdk.ChatMessageRoleAssistant, legacyContent))
	require.NoError(t, err)
	require.Len(t, parts, 1)
...
func TestFileReferencePreservation(t *testing.T) {
	...
	raw, err := chatprompt.MarshalParts([]optimus-ide-collabsdk.ChatMessagePart{{
		Type:      optimus-ide-collabsdk.ChatMessagePartTypeFileReference,
		FileName:  "main.go",
		StartLine: 10,
		EndLine:   20,
		Content:   "func main() {}",
	...
	parts, err := chatprompt.ParseContent(testMsg(optimus-ide-collabsdk.ChatMessageRoleUser, raw))
	require.NoError(t, err)
	require.Len(t, parts, 1)
	assert.Equal(t, optimus-ide-collabsdk.ChatMessagePartTypeFileReference, parts[0].Type)
	assert.Equal(t, "main.go", parts[0].FileName)
...
func TestAssistantWriteRoundTrip(t *testing.T) {
	...
	require.Equal(t, optimus-ide-collabsdk.ChatMessagePartTypeText, sdkPart.Type)
	require.NotNil(t, sdkPart.ProviderMetadata)

	raw, err := chatprompt.MarshalParts([]optimus-ide-collabsdk.ChatMessagePart{sdkPart})
	require.NoError(t, err)

	// Read back via ParseContent (takes the new SDK path, not
	// the legacy fallback, because the stored format is flat).
	parts, err := chatprompt.ParseContent(testMsg(optimus-ide-collabsdk.ChatMessageRoleAssistant, raw))
	require.NoError(t, err)
	require.Len(t, parts, 1)
...
func TestMixedFormatConversation(t *testing.T) {
	...
	newUserRaw, err := chatprompt.MarshalParts([]optimus-ide-collabsdk.ChatMessagePart{
		{Type: optimus-ide-collabsdk.ChatMessagePartTypeText, Text: "Check this diff."},
		{Type: optimus-ide-collabsdk.ChatMessagePartTypeFileReference, FileName: "main.go", StartLine: 5, EndLine: 15, Content: "func main() {}"},
	})
	require.NoError(t, err)
	...
	newAssistantRaw, err := chatprompt.MarshalParts([]optimus-ide-collabsdk.ChatMessagePart{
		{Type: optimus-ide-collabsdk.ChatMessagePartTypeText, Text: "Here is my analysis.", ProviderMetadata: newAssistantMeta},
	})
	require.NoError(t, err)
...
func TestQueuedMessageRoundTrip(t *testing.T) {
	...
	parts := []optimus-ide-collabsdk.ChatMessagePart{
		{Type: optimus-ide-collabsdk.ChatMessagePartTypeText, Text: "Review this change."},
		{Type: optimus-ide-collabsdk.ChatMessagePartTypeFileReference, FileName: "api.go", StartLine: 42, EndLine: 58, Content: "func handleRequest() {}"},
	}
	...
	assert.Equal(t, optimus-ide-collabsdk.ChatMessagePartTypeText, queuedMsg.Content[0].Type)
	assert.Equal(t, "Review this change.", queuedMsg.Content[0].Text)
	assert.Equal(t, optimus-ide-collabsdk.ChatMessagePartTypeFileReference, queuedMsg.Content[1].Type)
	...
	promoted, err := chatprompt.ParseContent(testMsg(optimus-ide-collabsdk.ChatMessageRoleUser, pqtype.NullRawMessage{
		RawMessage: raw.RawMessage,
		Valid:      true,
	}))
	require.NoError(t, err)
	require.Len(t, promoted, 2)
	assert.Equal(t, optimus-ide-collabsdk.ChatMessagePartTypeText, promoted[0].Type)
	assert.Equal(t, "Review this change.", promoted[0].Text)
	assert.Equal(t, optimus-ide-collabsdk.ChatMessagePartTypeFileReference, promoted[1].Type)
...
func TestParseContent_ErrorPaths(t *testing.T) {
	...
	t.Run("null_content_returns_nil", func(t *testing.T) {
		t.Parallel()
		parts, err := chatprompt.ParseContent(testMsg(optimus-ide-collabsdk.ChatMessageRoleUser, pqtype.NullRawMessage{}))
		require.NoError(t, err)
		assert.Nil(t, parts)
	})
	...
	t.Run("empty_content_returns_nil", func(t *testing.T) {
		t.Parallel()
		parts, err := chatprompt.ParseContent(testMsg(optimus-ide-collabsdk.ChatMessageRoleAssistant, pqtype.NullRawMessage{
			RawMessage: []byte{},
			Valid:      true,
		}))
		require.NoError(t, err)
	...
	t.Run("unknown_role", func(t *testing.T) {
		t.Parallel()
		_, err := chatprompt.ParseContent(testMsg(optimus-ide-collabsdk.ChatMessageRole("banana"), nullRaw(json.RawMessage(`"hello"`))))
		require.Error(t, err)
	...
	t.Run("system/malformed_json", func(t *testing.T) {
		t.Parallel()
		_, err := chatprompt.ParseContent(testMsg(optimus-ide-collabsdk.ChatMessageRoleSystem, nullRaw(json.RawMessage(`not json`))))
		require.Error(t, err)
	...
	t.Run("user/malformed_json", func(t *testing.T) {
		t.Parallel()
		_, err := chatprompt.ParseContent(testMsg(optimus-ide-collabsdk.ChatMessageRoleUser, nullRaw(json.RawMessage(`{not json`))))
		require.Error(t, err)
	})

	t.Run("assistant/malformed_json", func(t *testing.T) {
		t.Parallel()
		_, err := chatprompt.ParseContent(testMsg(optimus-ide-collabsdk.ChatMessageRoleAssistant, nullRaw(json.RawMessage(`{not json`))))
		require.Error(t, err)
	})

	t.Run("tool/malformed_json", func(t *testing.T) {
		t.Parallel()
		_, err := chatprompt.ParseContent(testMsg(optimus-ide-collabsdk.ChatMessageRoleTool, nullRaw(json.RawMessage(`{not json`))))
		require.Error(t, err)
	})
}
...
func TestNulEscapeRoundTrip(t *testing.T) {
	...
	textTests := []struct {
		...
		// U+E000 is the sentinel character. The enoptimus-ide-collab must
		...
		// encoded NUL in the encoded form, so the enoptimus-ide-collab must
	...
	for _, tc := range textTests {
		t.Run(tc.name, func(t *testing.T) {
			...
			parts := []optimus-ide-collabsdk.ChatMessagePart{
				optimus-ide-collabsdk.ChatMessageText(tc.input),
			}
			...
			msg := testMsgV1(optimus-ide-collabsdk.ChatMessageRoleAssistant, encoded)
	...
	t.Run("ToolResultWithNul", func(t *testing.T) {
		...
		parts := []optimus-ide-collabsdk.ChatMessagePart{
			optimus-ide-collabsdk.ChatMessageToolResult("call-1", "my_tool", resultJSON, false, false),
		}
		...
		msg := testMsgV1(optimus-ide-collabsdk.ChatMessageRoleTool, encoded)
		decoded, err := chatprompt.ParseContent(msg)
		require.NoError(t, err)
		require.Len(t, decoded, 1)
	...
	t.Run("MultiPartMixed", func(t *testing.T) {
		...
		parts := []optimus-ide-collabsdk.ChatMessagePart{
			optimus-ide-collabsdk.ChatMessageText("clean text"),
			optimus-ide-collabsdk.ChatMessageText("has\x00nul"),
		}
...
func TestConvertMessagesWithFiles_FiltersEmptyTextAndReasoningParts(t *testing.T) {
	...
	makeMsg := func(t *testing.T, role database.ChatMessageRole, parts []optimus-ide-collabsdk.ChatMessagePart) database.ChatMessage {
		t.Helper()
		encoded, err := chatprompt.MarshalParts(parts)
		require.NoError(t, err)
		return database.ChatMessage{
	...
	t.Run("UserRole", func(t *testing.T) {
		...
		parts := []optimus-ide-collabsdk.ChatMessagePart{
			optimus-ide-collabsdk.ChatMessageText(""),                     // empty, filtered
			optimus-ide-collabsdk.ChatMessageText("   \t\n "),             // whitespace, filtered
			optimus-ide-collabsdk.ChatMessageReasoning(""),                // empty, filtered
			optimus-ide-collabsdk.ChatMessageReasoning("  \n"),            // whitespace, filtered
			optimus-ide-collabsdk.ChatMessageText("hello"),                // kept
			optimus-ide-collabsdk.ChatMessageText("  hello  "),            // kept with original whitespace
			optimus-ide-collabsdk.ChatMessageReasoning("thinking deeply"), // kept
			optimus-ide-collabsdk.ChatMessageToolCall("call-1", "my_tool", json.RawMessage(`{"x":1}`)),
			optimus-ide-collabsdk.ChatMessageToolResult("call-1", "my_tool", json.RawMessage(`{"ok":true}`), false, false),
		}
	...
	t.Run("AssistantRole", func(t *testing.T) {
		...
		parts := []optimus-ide-collabsdk.ChatMessagePart{
			optimus-ide-collabsdk.ChatMessageText(""),          // empty, filtered
			optimus-ide-collabsdk.ChatMessageText(" "),         // whitespace, filtered
			optimus-ide-collabsdk.ChatMessageReasoning(""),     // empty, filtered
			optimus-ide-collabsdk.ChatMessageText("  reply  "), // kept with whitespace
			optimus-ide-collabsdk.ChatMessageToolCall("tc-1", "read_file", json.RawMessage(`{"path":"x"}`)),
		}
...
func TestConvertMessagesWithFiles_AssistantAttachmentIsNotReplayed(t *testing.T) {
	...
	userContent, err := chatprompt.MarshalParts([]optimus-ide-collabsdk.ChatMessagePart{
		optimus-ide-collabsdk.ChatMessageFile(userFileID, "image/png", "user.png"),
	})
	require.NoError(t, err)

	assistantContent, err := chatprompt.MarshalParts([]optimus-ide-collabsdk.ChatMessagePart{
		optimus-ide-collabsdk.ChatMessageText("I attached logs above."),
		optimus-ide-collabsdk.ChatMessageFile(assistantFileID, "text/plain", "agent.log"),
	})
	require.NoError(t, err)
...
func TestMediaToolResultRoundTrip(t *testing.T) {
	...
	insertPair := func(
		...
		resultParts []optimus-ide-collabsdk.ChatMessagePart,
	) database.Chat {
		t.Helper()
		...
		callPart := optimus-ide-collabsdk.ChatMessageToolCall(callID, toolName, json.RawMessage(`{}`))
		assistantEncoded, encErr := chatprompt.MarshalParts([]optimus-ide-collabsdk.ChatMessagePart{callPart})
	...
	t.Run("MediaResultRoundTripsAsMedia", func(t *testing.T) {
		...
		// Computer use is a provider-defined tool, but Optimus-IDE-Collab executes it
		...
		chat := insertPair(t, callID, toolName, []optimus-ide-collabsdk.ChatMessagePart{sdkPart})
	...
	t.Run("TextResultStaysText", func(t *testing.T) {
		...
		chat := insertPair(t, callID, toolName, []optimus-ide-collabsdk.ChatMessagePart{
			optimus-ide-collabsdk.ChatMessageToolResult(callID, toolName, textResult, false, false),
		})
	...
	t.Run("MissingMimeTypeStaysText", func(t *testing.T) {
		...
		chat := insertPair(t, callID, toolName, []optimus-ide-collabsdk.ChatMessagePart{
			optimus-ide-collabsdk.ChatMessageToolResult(callID, toolName, noMimeJSON, false, false),
		})
	...
	t.Run("MissingDataStaysText", func(t *testing.T) {
		...
		chat := insertPair(t, callID, toolName, []optimus-ide-collabsdk.ChatMessagePart{
			optimus-ide-collabsdk.ChatMessageToolResult(callID, toolName, noDataJSON, false, false),
		})
	...
	t.Run("NonMediaResultTypeStaysText", func(t *testing.T) {
		...
		chat := insertPair(t, callID, toolName, []optimus-ide-collabsdk.ChatMessagePart{
			optimus-ide-collabsdk.ChatMessageToolResult(callID, toolName, textJSON, false, false),
		})
	...
	t.Run("IsMediaTrueButMissingMimeType", func(t *testing.T) {
		...
		chat := insertPair(t, callID, toolName, []optimus-ide-collabsdk.ChatMessagePart{
			optimus-ide-collabsdk.ChatMessageToolResult(callID, toolName, noMimeJSON, false, true),
		})
	...
	t.Run("IsMediaTrueButMissingData", func(t *testing.T) {
		...
		chat := insertPair(t, callID, toolName, []optimus-ide-collabsdk.ChatMessagePart{
			optimus-ide-collabsdk.ChatMessageToolResult(callID, toolName, noDataJSON, false, true),
		})
	...
	t.Run("IsMediaTrueButGarbageJSON", func(t *testing.T) {
		...
		chat := insertPair(t, callID, toolName, []optimus-ide-collabsdk.ChatMessagePart{
			optimus-ide-collabsdk.ChatMessageToolResult(callID, toolName, garbageJSON, false, true),
		})
...
func TestPartFromContent_ExecuteToolParsedCommands(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			require.Equal(t, optimus-ide-collabsdk.ChatMessagePartTypeToolCall, part.Type)
