...
func TestAttachFile(t *testing.T) {
	...
	t.Run("ValidTextFileStoresAttachment", func(t *testing.T) {
		...
		mockConn.EXPECT().
			ReadFile(gomock.Any(), "/home/optimus-ide-collab/build.log", int64(0), int64(10<<20+1)).
		...
		tool := newAttachFileTool(t, mockConn, func(_ context.Context, name string, detectName string, data []byte) (chattool.AttachmentMetadata, error) {
			storedName = name
			require.Equal(t, "/home/optimus-ide-collab/build.log", detectName)
		...
		resp, err := tool.Run(context.Background(), fantasy.ToolCall{
			ID: "call-1", Name: "attach_file", Input: `{"path":"/home/optimus-ide-collab/build.log"}`,
		})
		require.NoError(t, err)
		...
		assert.Equal(t, "/home/optimus-ide-collab/build.log", decoded.Path)
	...
	t.Run("WindowsAbsolutePathUsesBaseName", func(t *testing.T) {
		...
		mockConn := agentconnmock.NewMockAgentConn(ctrl)
		content := "build succeeded\n"
		path := `C:\Users\optimus-ide-collab\build.log`
		mockConn.EXPECT().
	...
	t.Run("CustomNameOverridePreservesJSONSubtype", func(t *testing.T) {
		...
		content := `{"ok":true}`
		mockConn.EXPECT().
			ReadFile(gomock.Any(), "/home/optimus-ide-collab/report.json", int64(0), int64(10<<20+1)).
			Return(io.NopCloser(strings.NewReader(content)), "text/plain", nil)
		...
		tool := newAttachFileTool(t, mockConn, func(_ context.Context, name string, detectName string, data []byte) (chattool.AttachmentMetadata, error) {
			storedName = name
			require.Equal(t, "/home/optimus-ide-collab/report.json", detectName)
		...
		resp, err := tool.Run(context.Background(), fantasy.ToolCall{
			ID: "call-json", Name: "attach_file", Input: `{"path":"/home/optimus-ide-collab/report.json","name":"payload.txt"}`,
		})
		require.NoError(t, err)
		assert.False(t, resp.IsError)
	...
	t.Run("EmptyFileRejected", func(t *testing.T) {
		...
		mockConn := agentconnmock.NewMockAgentConn(ctrl)
		mockConn.EXPECT().
			ReadFile(gomock.Any(), "/home/optimus-ide-collab/empty.txt", int64(0), int64(10<<20+1)).
		...
		resp, err := tool.Run(context.Background(), fantasy.ToolCall{
			ID: "call-empty", Name: "attach_file", Input: `{"path":"/home/optimus-ide-collab/empty.txt"}`,
		})
		require.NoError(t, err)
		assert.True(t, resp.IsError)
	...
	t.Run("StoreFileErrorSurfaces", func(t *testing.T) {
		...
		resp, err := tool.Run(context.Background(), fantasy.ToolCall{
			ID: "call-cap", Name: "attach_file", Input: `{"path":"/home/optimus-ide-collab/build.log"}`,
		})
		require.NoError(t, err)
		assert.True(t, resp.IsError)
