...
func TestCreateWorkspace_PrefersChatSuffixAgent(t *testing.T) {
	...
	db.EXPECT().
		...
		Return([]database.WorkspaceAgent{
			...
			{ID: chatAgentID, Name: "dev-optimus-ide-collabd-chat", DisplayOrder: 1},
		}, nil)
	db.EXPECT().
	...
	createFn := func(_ context.Context, _ uuid.UUID, req optimus-ide-collabsdk.CreateWorkspaceRequest) (optimus-ide-collabsdk.Workspace, error) {
...
func TestCreateWorkspace_ReturnsSelectionErrorImmediately(t *testing.T) {
	...
	db.EXPECT().
		GetWorkspaceAgentsInLatestBuildByWorkspaceID(gomock.Any(), workspaceID).
		Return([]database.WorkspaceAgent{
			{ID: uuid.New(), Name: "alpha-optimus-ide-collabd-chat", DisplayOrder: 0},
			{ID: uuid.New(), Name: "beta-optimus-ide-collabd-chat", DisplayOrder: 1},
		}, nil)
...
func TestCreateWorkspace_PostCreationQuotaFailure(t *testing.T) {
	...
	db.EXPECT().
		...
		Return(database.ProvisionerJob{
			...
			ErrorCode: sql.NullString{
				String: string(optimus-ide-collabsdk.InsufficientQuota),
				Valid:  true,
			},
		}, nil)
	...
	var result map[string]any
	require.NoError(t, json.Unmarshal([]byte(resp.Content), &result))
	require.Equal(t, string(optimus-ide-collabsdk.InsufficientQuota), result["error_code"])
...
func TestCreateWorkspace_ExistingBuildQuotaFailure(t *testing.T) {
	...
	tool := CreateWorkspace(db, orgID, chatID, CreateWorkspaceOptions{
		OwnerID: ownerID,
		CreateFn: func(context.Context, uuid.UUID, optimus-ide-collabsdk.CreateWorkspaceRequest) (optimus-ide-collabsdk.Workspace, error) {
			t.Fatal("CreateFn should not be called when an existing build is in progress")
			return optimus-ide-collabsdk.Workspace{}, nil
		},
		WorkspaceMu: &sync.Mutex{},
		Logger:      slogtest.Make(t, &slogtest.Options{IgnoreErrors: true}),
	})
...
func TestCreateWorkspace_ResponderErrorPreservesStructuredFields(t *testing.T) {
	...
	tool := CreateWorkspace(db, orgID, chatID, CreateWorkspaceOptions{
		OwnerID: ownerID,
		CreateFn: func(context.Context, uuid.UUID, optimus-ide-collabsdk.CreateWorkspaceRequest) (optimus-ide-collabsdk.Workspace, error) {
			return optimus-ide-collabsdk.Workspace{}, httperror.NewResponseError(400, optimus-ide-collabsdk.Response{
	...
	var result struct {
		...
		Validations []optimus-ide-collabsdk.ValidationError `json:"validations"`
	}
	require.NoError(t, json.Unmarshal([]byte(resp.Content), &result))
...
func TestCreateWorkspaceWithNameRetry(t *testing.T) {
	...
	t.Run("NameConflictRetriesWithGeneratedName", func(t *testing.T) {
		...
		workspace, err := createWorkspaceWithNameRetry(
			context.Background(),
			uuid.New(),
			optimus-ide-collabsdk.CreateWorkspaceRequest{Name: "fun-dashboard"},
			func(_ context.Context, _ uuid.UUID, req optimus-ide-collabsdk.CreateWorkspaceRequest) (optimus-ide-collabsdk.Workspace, error) {
				names = append(names, req.Name)
				if len(names) == 1 {
					return optimus-ide-collabsdk.Workspace{}, workspaceNameConflictError(req.Name)
				}

				require.Regexp(t, `^fun-dashboard-[0-9a-f]{4}$`, req.Name)
				return optimus-ide-collabsdk.Workspace{Name: req.Name}, nil
			},
		)
	...
	t.Run("OtherConflictDoesNotRetry", func(t *testing.T) {
		...
		calls := 0
		wantErr := httperror.NewResponseError(http.StatusConflict, optimus-ide-collabsdk.Response{
		...
		_, err := createWorkspaceWithNameRetry(
			...
			uuid.New(),
			optimus-ide-collabsdk.CreateWorkspaceRequest{Name: "fun-dashboard"},
			func(context.Context, uuid.UUID, optimus-ide-collabsdk.CreateWorkspaceRequest) (optimus-ide-collabsdk.Workspace, error) {
				calls++
				return optimus-ide-collabsdk.Workspace{}, wantErr
			},
		)
...
func TestCreateWorkspace_GlobalTTL(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			var capturedReq optimus-ide-collabsdk.CreateWorkspaceRequest
			createFn := func(_ context.Context, _ uuid.UUID, req optimus-ide-collabsdk.CreateWorkspaceRequest) (optimus-ide-collabsdk.Workspace, error) {
				capturedReq = req
				return optimus-ide-collabsdk.Workspace{
					ID:        workspaceID,
...
func TestCreateWorkspace_InvalidPresetID(t *testing.T) {
	...
	tool := CreateWorkspace(s.DB, s.OrgID, s.ChatID, CreateWorkspaceOptions{
		OwnerID: s.OwnerID,
		CreateFn: func(_ context.Context, _ uuid.UUID, _ optimus-ide-collabsdk.CreateWorkspaceRequest) (optimus-ide-collabsdk.Workspace, error) {
