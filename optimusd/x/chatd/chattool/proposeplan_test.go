...
func TestProposePlan(t *testing.T) {
	...
	t.Run("NonMdPathReturnsError", func(t *testing.T) {
		...
		resp, err := tool.Run(context.Background(), fantasy.ToolCall{
			ID:    "call-1",
			Name:  "propose_plan",
			Input: `{"path":"/home/optimus-ide-collab/plan.txt"}`,
		})
		require.NoError(t, err)
		assert.True(t, resp.IsError)
	...
	t.Run("OversizedFileRejected", func(t *testing.T) {
		...
		mockConn.EXPECT().
			ReadFile(gomock.Any(), "/home/optimus-ide-collab/PLAN.md", int64(0), int64(32*1024+1)).
		...
		resp, err := tool.Run(context.Background(), fantasy.ToolCall{
			ID:    "call-1",
			Name:  "propose_plan",
			Input: `{"path":"/home/optimus-ide-collab/PLAN.md"}`,
		})
		require.NoError(t, err)
		assert.True(t, resp.IsError)
	...
	t.Run("ValidPlanReadsFile", func(t *testing.T) {
		...
		mockConn.EXPECT().
			ReadFile(gomock.Any(), "/home/optimus-ide-collab/docs/PLAN.md", int64(0), int64(32*1024+1)).
		...
		tool := newProposePlanToolWithPlanPath(
			...
			func(context.Context) (string, string, error) {
				planPathCalled = true
				return "/home/optimus-ide-collab/.optimus-ide-collab/plans/PLAN-xxx.md", "/home/optimus-ide-collab", nil
			},
		)
		resp, err := tool.Run(context.Background(), fantasy.ToolCall{
			ID:    "call-1",
			Name:  "propose_plan",
			Input: `{"path":"/home/optimus-ide-collab/docs/PLAN.md"}`,
		})
		require.NoError(t, err)
		assert.False(t, resp.IsError)
		...
		assert.Equal(t, "/home/optimus-ide-collab/docs/PLAN.md", result.Path)
	...
	t.Run("NestedPlanPathUnderHomeIsAllowed", func(t *testing.T) {
		...
		mockConn.EXPECT().
			ReadFile(gomock.Any(), "/home/optimus-ide-collab/myproject/plan.md", int64(0), int64(32*1024+1)).
		...
		resp, err := tool.Run(context.Background(), fantasy.ToolCall{
			ID:    "call-1",
			Name:  "propose_plan",
			Input: `{"path":"/home/optimus-ide-collab/myproject/plan.md"}`,
		})
		require.NoError(t, err)
		assert.False(t, resp.IsError)
		assert.True(t, planPathCalled)
		...
		result := decodeProposePlanResponse(t, resp)
		assert.True(t, result.OK)
		assert.Equal(t, "/home/optimus-ide-collab/myproject/plan.md", result.Path)
	...
	t.Run("RejectsSharedPlanPathWithResolvedPath", func(t *testing.T) {
		...
		assert.Equal(
			t,
			sharedPlanPathResolvedMessage(chattool.LegacySharedPlanPath, "/home/optimus-ide-collab/.optimus-ide-collab/plans/PLAN-chat.md"),
			resp.Content,
		)
	})
	...
	t.Run("PlanTurnDefaultsEmptyPathToResolvedPath", func(t *testing.T) {
		...
		ctrl := gomock.NewController(t)
		mockConn := agentconnmock.NewMockAgentConn(ctrl)
		chatPlanPath := "/home/optimus-ide-collab/.optimus-ide-collab/plans/PLAN-chat.md"
	...
	t.Run("PlanTurnRejectsWrongPath", func(t *testing.T) {
		...
		resp, err := tool.Run(context.Background(), fantasy.ToolCall{
			ID:    "call-1",
			Name:  "propose_plan",
			Input: `{"path":"/home/optimus-ide-collab/README.md"}`,
		})
		require.NoError(t, err)
		assert.True(t, resp.IsError)
