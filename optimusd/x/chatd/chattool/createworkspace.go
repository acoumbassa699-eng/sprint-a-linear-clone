	...
	return fantasy.NewAgentTool(
		...
		func(ctx context.Context, args createWorkspaceArgs, _ fantasy.ToolCall) (fantasy.ToolResponse, error) {
			...
			if err != nil {
				...
				d, parseErr := optimus-ide-collabsdk.ParseChatWorkspaceTTL(raw)
				if parseErr != nil {
			...
			} else if err := optimus-ide-collabsdk.NameValid(name); err != nil {
			...
			for k, v := range args.Parameters {
				createReq.RichParameterValues = append(
					...
					optimus-ide-collabsdk.WorkspaceBuildParameter{Name: k, Value: v},
				)
			}
...
func waitForBuild(
	...
	for {
		...
		case database.ProvisionerJobStatusFailed:
			...
			var code optimus-ide-collabsdk.JobErrorCode
			if job.ErrorCode.Valid {
				code = optimus-ide-collabsdk.JobErrorCode(job.ErrorCode.String)
			}
...
func generatedWorkspaceName(seed string) string {
	base := optimus-ide-collabsdk.UsernameFrom(strings.TrimSpace(strings.ToLower(seed)))
	...
	if err := optimus-ide-collabsdk.NameValid(name); err == nil {
		return name
	}
