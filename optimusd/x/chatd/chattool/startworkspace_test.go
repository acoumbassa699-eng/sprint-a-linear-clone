...
func TestStartWorkspace(t *testing.T) {
	...
	t.Run("NoWorkspace", func(t *testing.T) {
		...
		tool := chattool.StartWorkspace(db, chat.ID, chattool.StartWorkspaceOptions{
			StartFn: func(_ context.Context, _ uuid.UUID, _ uuid.UUID, _ optimus-ide-collabsdk.CreateWorkspaceBuildRequest) (optimus-ide-collabsdk.WorkspaceBuild, error) {
				t.Fatal("StartFn should not be called")
				return optimus-ide-collabsdk.WorkspaceBuild{}, nil
			},
	...
	t.Run("AlreadyRunningPrefersChatSuffixAgent", func(t *testing.T) {
		...
		wsResp := dbfake.WorkspaceBuild(t, db, database.WorkspaceTable{
			...
			return append(agents, &sdkproto.Agent{
				Id:   uuid.NewString(),
				Name: "dev-optimus-ide-collabd-chat",
		...
		for _, agent := range wsResp.Agents {
			if agent.Name == "dev-optimus-ide-collabd-chat" {
	...
	t.Run("AlreadyRunningPreservesAgentSelectionError", func(t *testing.T) {
		...
		wsResp := dbfake.WorkspaceBuild(t, db, database.WorkspaceTable{
			...
			agents[0].Name = "alpha-optimus-ide-collabd-chat"
			return append(agents, &sdkproto.Agent{
				Id:   uuid.NewString(),
				Name: "beta-optimus-ide-collabd-chat",
				Auth: &sdkproto.Agent_Token{Token: uuid.NewString()},
				Env:  map[string]string{},
			})
		}).Seed(database.WorkspaceBuild{
	...
	t.Run("StoppedWorkspace", func(t *testing.T) {
		...
		startFn := func(_ context.Context, _ uuid.UUID, wsID uuid.UUID, req optimus-ide-collabsdk.CreateWorkspaceBuildRequest) (optimus-ide-collabsdk.WorkspaceBuild, error) {
			startCalled = true
			require.Equal(t, optimus-ide-collabsdk.WorkspaceTransitionStart, req.Transition)
			...
			return optimus-ide-collabsdk.WorkspaceBuild{ID: buildResp.Build.ID}, nil
		}
	...
	t.Run("PassesParameters", func(t *testing.T) {
		...
		expectedParams := []optimus-ide-collabsdk.WorkspaceBuildParameter{
	...
	t.Run("ManualUpdateRequired", func(t *testing.T) {
		...
		tool := chattool.StartWorkspace(db, chat.ID, chattool.StartWorkspaceOptions{
			OwnerID: user.ID,
			StartFn: func(_ context.Context, _ uuid.UUID, _ uuid.UUID, _ optimus-ide-collabsdk.CreateWorkspaceBuildRequest) (optimus-ide-collabsdk.WorkspaceBuild, error) {
				return optimus-ide-collabsdk.WorkspaceBuild{}, httperror.NewResponseError(400, optimus-ide-collabsdk.Response{
	...
	t.Run("ResponderErrorWithoutValidationsOmitsTemplateID", func(t *testing.T) {
		...
		tool := chattool.StartWorkspace(db, chat.ID, chattool.StartWorkspaceOptions{
			OwnerID: user.ID,
			StartFn: func(_ context.Context, _ uuid.UUID, _ uuid.UUID, _ optimus-ide-collabsdk.CreateWorkspaceBuildRequest) (optimus-ide-collabsdk.WorkspaceBuild, error) {
				return optimus-ide-collabsdk.WorkspaceBuild{}, httperror.NewResponseError(502, optimus-ide-collabsdk.Response{
