...
func (r *runner) bootstrap() bool {
	channel := optimus-ide-collabdpubsub.ChatStateUpdateChannel(r.rec.key.ChatID)
	unsubscribe, err := r.opts.Pubsub.SubscribeWithErr(channel, optimus-ide-collabdpubsub.HandleChatStateUpdate(
		func(ctx context.Context, payload optimus-ide-collabdpubsub.ChatStateUpdateMessage, err error) {
			if err != nil {
...
func stateUpdateFromPubsub(chatID uuid.UUID, payload optimus-ide-collabdpubsub.ChatStateUpdateMessage) runnerStateUpdate {
