...
func (w *chatWorker) archiveOnce(ctx context.Context, start time.Time) {
	autoArchiveDays, err := w.opts.Store.GetChatAutoArchiveDays(ctx, optimus-ide-collabsdk.DefaultChatAutoArchiveDays)
	if err != nil {
		if ctx.Err() == nil {
...
func (w *chatWorker) archiveCandidate(
	...
	w.server.publishChatPubsubEvents(familyChats, optimus-ide-collabsdk.ChatWatchEventKindDeleted)
