...
func TestAIBridgeProviderFormatMapping(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "OpenAI", providerType: database.AIProviderTypeOpenai, wantProvider: "openai", wantBaseURL: "http://optimus-ide-collab-aibridge/v1"},
		{name: "Anthropic", providerType: database.AIProviderTypeAnthropic, wantProvider: "anthropic", wantBaseURL: "http://optimus-ide-collab-aibridge"},
		{name: "Bedrock", providerType: database.AIProviderTypeBedrock, wantProvider: "anthropic", wantBaseURL: "http://optimus-ide-collab-aibridge"},
		{name: "Google", providerType: database.AIProviderTypeGoogle, wantProvider: "openai-compat", wantBaseURL: "http://optimus-ide-collab-aibridge/v1"},
	}
...
func TestAIGatewayModelForwardsProviderAuth(t *testing.T) {
	...
	newServer := func(t *testing.T, provider database.AIProvider, auth aiGatewayProviderAuth, seen chan seenRequest) (*Server, aiGatewayModelRoute) {
		factory := &aibridgeTestFactory{rt: roundTripFunc(func(req *http.Request) (*http.Response, error) {
			...
			seen <- seenRequest{
				...
				optimus-ide-collabToken:    req.Header.Get(aibridge.HeaderOptimus-IDE-CollabToken),
				apiKeyID:      apiKeyID,
				path:          req.URL.Path,
			}
	...
	t.Run("OpenAI", func(t *testing.T) {
		...
		require.Equal(t, aibridgeDelegatedBYOKMarker, got.optimus-ide-collabToken)
	...
	t.Run("NoUserKeyLeavesPlaceholderForAIBridged", func(t *testing.T) {
		...
		require.Empty(t, got.xAPIKey)
		require.Empty(t, got.optimus-ide-collabToken)
		require.Equal(t, apiKeyID, got.apiKeyID)
	})
}
...
func TestAIBridgeRoutingFailClosed(t *testing.T) {
	...
	t.Run("OpenRouterMisconfiguredAsOpenAI", func(t *testing.T) {
		...
		classified := chaterror.Classify(err)
		require.Equal(t, optimus-ide-collabsdk.ChatErrorKindConfig, classified.Kind)
		require.False(t, classified.Retryable)
	})
...
func TestAIBridgeComputerUseModelUsesRoute(t *testing.T) {
	...
	require.EqualValues(t, optimus-ide-collabsdk.ChatComputerUseProviderOpenAI, resolvedProvider)
