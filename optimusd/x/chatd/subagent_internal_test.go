...
func withInternalTestServerExperiments(experiments optimus-ide-collabsdk.Experiments) internalTestServerOpt {
	return func(cfg *internalTestServerConfig) {
...
func experimentsOrDefault(experiments optimus-ide-collabsdk.Experiments) optimus-ide-collabsdk.Experiments {
	if experiments == nil {
		return optimus-ide-collabsdk.ExperimentsKnown
	}
	return experiments
}
...
func TestCreateChildSubagentChatCopiesPlanMode(t *testing.T) {
	...
	parent, err := server.CreateChat(ctx, CreateOptions{
		...
		InitialUserContent: []optimus-ide-collabsdk.ChatMessagePart{
			optimus-ide-collabsdk.ChatMessageText("plan this change"),
		},
	})
	require.NoError(t, err)
...
func TestSpawnAgent_GeneralHonorsPersonalModelOverrides(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			enablePersonalOverride: true,
			personalRaw: func(database.ChatModelConfig) string {
				return string(optimus-ide-collabsdk.ChatPersonalModelOverrideModeDeploymentDefault)
			},
			wantModelID: func(_, deploymentModel, _ database.ChatModelConfig) uuid.UUID {
				return deploymentModel.ID
			},
		},
		{
			...
			enablePersonalOverride: true,
			personalRaw: func(database.ChatModelConfig) string {
				return string(optimus-ide-collabsdk.ChatPersonalModelOverrideModeChatDefault)
			},
		...
		},
		{
			...
			personalRaw: func(personalModel database.ChatModelConfig) string {
				return string(optimus-ide-collabsdk.ChatPersonalModelOverrideModeModel) + ":" +
...
func TestSpawnAgent_GeneralOverrideLogsAndFallsBackWhenCredentialsUnavailable(t *testing.T) {
	...
	parent, err := server.CreateChat(ctx, CreateOptions{
		...
		ModelConfigID:  model.ID,
		InitialUserContent: []optimus-ide-collabsdk.ChatMessagePart{
			optimus-ide-collabsdk.ChatMessageText("delegate work"),
		},
	})
	require.NoError(t, err)
	parentChat, err := db.GetChatByID(ctx, parent.ID)
...
func TestWithResolvedReasoningEffort(t *testing.T) {
	...
	baseOptions, err := json.Marshal(optimus-ide-collabsdk.ChatModelCallConfig{
		MaxOutputTokens: ptr.Ref(int64(123)),
		ReasoningEffort: &optimus-ide-collabsdk.ChatModelReasoningEffortConfig{
			Default: ptr.Ref(optimus-ide-collabsdk.ChatModelReasoningEffortLow),
			Max:     ptr.Ref(optimus-ide-collabsdk.ChatModelReasoningEffortHigh),
		},
	})
	require.NoError(t, err)
	...
	t.Run("InvalidJSONReturnsOriginalConfig", func(t *testing.T) {
		...
		require.Equal(t, modelConfig, withResolvedReasoningEffort(modelConfig, ptr.Ref(optimus-ide-collabsdk.ChatModelReasoningEffortHigh)))
	})
	...
	t.Run("ClampsRequestedEffortAndPreservesOtherOptions", func(t *testing.T) {
		...
		got := withResolvedReasoningEffort(modelConfig, ptr.Ref(optimus-ide-collabsdk.ChatModelReasoningEffortXHigh))

		var callConfig optimus-ide-collabsdk.ChatModelCallConfig
		require.NoError(t, json.Unmarshal(got.Options, &callConfig))
		require.Equal(t, ptr.Ref(int64(123)), callConfig.MaxOutputTokens)
		require.Equal(t, ptr.Ref(optimus-ide-collabsdk.ChatModelReasoningEffortHigh), callConfig.ReasoningEffort.Default)
		require.Equal(t, ptr.Ref(optimus-ide-collabsdk.ChatModelReasoningEffortHigh), callConfig.ReasoningEffort.Max)
	})
}
...
func TestSpawnAgent_ExplicitModelBeatsConfiguredOverrides(t *testing.T) {
	...
	upsertInternalUserChatPersonalModelOverride(
		...
		user.ID,
		optimus-ide-collabsdk.ChatPersonalModelOverrideContextGeneral,
		string(optimus-ide-collabsdk.ChatPersonalModelOverrideModeModel)+":"+personalModel.ID.String()+":low",
	)
	parentChat := createInternalParentChat(
...
func TestCreateChat_ExploreRootStartsWithoutMCPSnapshot(t *testing.T) {
	...
	root, err := server.CreateChat(ctx, CreateOptions{
		...
		InitialUserContent: []optimus-ide-collabsdk.ChatMessagePart{optimus-ide-collabsdk.ChatMessageText("inspect the codebase")},
	})
	require.NoError(t, err)
...
func TestSpawnAgent_PlanModeDescriptionOmitsComputerUse(t *testing.T) {
	...
	parent, err := server.CreateChat(ctx, CreateOptions{
		...
		InitialUserContent: []optimus-ide-collabsdk.ChatMessagePart{optimus-ide-collabsdk.ChatMessageText("plan this change")},
	})
	require.NoError(t, err)
	parentChat, err := db.GetChatByID(ctx, parent.ID)
	require.NoError(t, err)
...
func TestSpawnAgent_ComputerUseAvailabilityUsesConfiguredProvider(t *testing.T) {
	...
	require.NoError(t, db.UpsertChatComputerUseProvider(
		ctx,
		string(optimus-ide-collabsdk.ChatComputerUseProviderOpenAI),
	))
	server := newInternalTestServer(t, db, ps, chatprovider.ProviderAPIKeys{})
...
func TestSpawnAgent_ComputerUseRejectsDesktopDisabled(t *testing.T) {
	...
	experiments := slices.DeleteFunc(
		...
		func(e optimus-ide-collabsdk.Experiment) bool { return e == optimus-ide-collabsdk.ExperimentChatVirtualDesktop },
	)
	server := newInternalTestServer(t, db, ps, chatprovider.ProviderAPIKeys{
		Anthropic: "test-anthropic-key",
...
func TestSubagentLifecycleToolErrorsIncludePersistedSubagentType(t *testing.T) {
	...
	unrelated, err := server.CreateChat(ctx, CreateOptions{
		...
		ModelConfigID:      model.ID,
		InitialUserContent: []optimus-ide-collabsdk.ChatMessagePart{optimus-ide-collabsdk.ChatMessageText("other")},
	})
	require.NoError(t, err)
...
func TestIsSubagentDescendant(t *testing.T) {
	...
	root, err := server.CreateChat(ctx, CreateOptions{
		...
		Title:              "root",
		ModelConfigID:      model.ID,
		InitialUserContent: []optimus-ide-collabsdk.ChatMessagePart{optimus-ide-collabsdk.ChatMessageText("root")},
	})
	require.NoError(t, err)
	...
	child, err := server.CreateChat(ctx, CreateOptions{
		...
		Title:              "child",
		ModelConfigID:      model.ID,
		InitialUserContent: []optimus-ide-collabsdk.ChatMessagePart{optimus-ide-collabsdk.ChatMessageText("child")},
	})
	require.NoError(t, err)
	...
	grandchild, err := server.CreateChat(ctx, CreateOptions{
		...
		Title:              "grandchild",
		ModelConfigID:      model.ID,
		InitialUserContent: []optimus-ide-collabsdk.ChatMessagePart{optimus-ide-collabsdk.ChatMessageText("grandchild")},
	})
	require.NoError(t, err)
	...
	unrelated, err := server.CreateChat(ctx, CreateOptions{
		...
		ModelConfigID:      model.ID,
		InitialUserContent: []optimus-ide-collabsdk.ChatMessagePart{optimus-ide-collabsdk.ChatMessageText("unrelated")},
	})
	require.NoError(t, err)
	...
	unrelatedChild, err := server.CreateChat(ctx, CreateOptions{
		...
		ModelConfigID:      model.ID,
		InitialUserContent: []optimus-ide-collabsdk.ChatMessagePart{optimus-ide-collabsdk.ChatMessageText("unrelated-child")},
	})
	require.NoError(t, err)
...
func createParentChildChats(
	...
	child, err = server.CreateChat(ctx, CreateOptions{
		...
		Title:              "child-" + t.Name(),
		ModelConfigID:      model.ID,
		InitialUserContent: []optimus-ide-collabsdk.ChatMessagePart{optimus-ide-collabsdk.ChatMessageText("do work")},
	})
	require.NoError(t, err)
...
func setChatStatus(
	...
	if lastError != "" {
		encodedLastError, err := json.Marshal(optimus-ide-collabsdk.ChatError{
			Message: lastError,
...
func setChatStatusWithError(
	...
	chatID uuid.UUID,
	status database.ChatStatus,
	chatErr optimus-ide-collabsdk.ChatError,
) {
	t.Helper()
...
func insertAssistantMessage(
	...
	parts := []optimus-ide-collabsdk.ChatMessagePart{optimus-ide-collabsdk.ChatMessageText(text)}
...
func TestAwaitSubagentCompletion(t *testing.T) {
	...
	t.Run("CompletesViaPubsub", func(t *testing.T) {
		...
		cancelProbe, err := ps.SubscribeWithErr(
			optimus-ide-collabdpubsub.ChatStateUpdateChannel(child.ID),
...
func TestWaitAgentErrorStatusReturnsStructuredPayload(t *testing.T) {
	...
	require.Equal(t, subagentTypeGeneral, result["type"])
	require.Equal(t, string(optimus-ide-collabsdk.ChatErrorKindGeneric), result["last_error_kind"])
...
func TestWaitAgentErrorStatusSurfacesLastErrorDetail(t *testing.T) {
	...
	WaitUntilIdleForTest(server)
	setChatStatusWithError(ctx, t, db, child.ID, database.ChatStatusError, optimus-ide-collabsdk.ChatError{
...
func TestWaitAgentTimeoutGapCompletesWithError(t *testing.T) {
	...
	require.Equal(t, child.ID.String(), m["chat_id"])
	require.Equal(t, subagentTypeGeneral, m["type"])
	require.Equal(t, string(optimus-ide-collabsdk.ChatErrorKindGeneric), m["last_error_kind"])
...
func TestListAgents(t *testing.T) {
	...
	newChild := func(t *testing.T, ctx context.Context, parent database.Chat, title string, mode database.NullChatMode) database.Chat {
		...
		child, err := server.CreateChat(ctx, CreateOptions{
			...
			ChatMode:       mode,
			InitialUserContent: []optimus-ide-collabsdk.ChatMessagePart{
				optimus-ide-collabsdk.ChatMessageText("do work"),
			},
		})
		require.NoError(t, err)
		return child
