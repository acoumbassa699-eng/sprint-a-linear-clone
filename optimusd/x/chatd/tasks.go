...
func (s *taskStarter) StartInterrupt(ctx context.Context, input chatWorkerTaskStartInput) error {
	...
	if err != nil {
		if current, ok := s.committedStateAfterUpdateError(ctx, committed); ok {
			return s.publishWatchAndRoute(ctx, current, optimus-ide-collabsdk.ChatWatchEventKindStatusChange)
		}
...
func (s *taskStarter) cancelRequiresAction(
	...
	return s.publishWatchAndRoute(ctx, committed, optimus-ide-collabsdk.ChatWatchEventKindStatusChange)
}
...
func (s *taskStarter) publishWatchAndRoute(
	ctx context.Context,
	chat database.Chat,
	kind optimus-ide-collabsdk.ChatWatchEventKind,
) error {
...
func publishChatWatchEvent(pubsub chatWorkerPubsub, chat database.Chat, kind optimus-ide-collabsdk.ChatWatchEventKind) error {
	...
	if err := pubsub.Publish(optimus-ide-collabdpubsub.ChatWatchEventChannel(chat.OwnerID), payload); err != nil {
...
func committedPendingLocalToolCancellationMessages(
	...
	for _, call := range localCalls {
		...
		part := optimus-ide-collabsdk.ChatMessageToolResult(call.ToolCallID, call.ToolName, payload, true, false)
