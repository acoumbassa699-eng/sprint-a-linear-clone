...
func GenerateAssistant(ctx context.Context, opts GenerateAssistantOptions) (AssistantOutcome, error) {
	...
	publishMessagePart := func(role optimus-ide-collabsdk.ChatMessageRole, part optimus-ide-collabsdk.ChatMessagePart) {
...
func contentFilterError(provider string, metadata fantasy.ProviderMetadata) error {
	classified := chaterror.ClassifiedError{
		Kind:      optimus-ide-collabsdk.ChatErrorKindContentFilter,
		Provider:  provider,
		Retryable: false,
	}
...
func ExecuteLocalTools(ctx context.Context, opts ExecuteLocalToolsOptions) (ToolExecutionOutcome, error) {
	...
	if exclusiveViolation {
		...
		for _, tr := range policyResults {
			...
			publishMessagePart(optimus-ide-collabsdk.ChatMessageRoleTool, ssePart)
...
func processStepStream(
	...
	for part := range stream {
		...
		case fantasy.StreamPartTypeTextDelta:
			...
			publishMessagePart(optimus-ide-collabsdk.ChatMessageRoleAssistant, optimus-ide-collabsdk.ChatMessageText(part.Delta))
		...
		case fantasy.StreamPartTypeReasoningDelta:
			reasoningPart := optimus-ide-collabsdk.ChatMessageReasoning(part.Delta)
			...
			publishMessagePart(optimus-ide-collabsdk.ChatMessageRoleAssistant, reasoningPart)
		...
		case fantasy.StreamPartTypeToolInputDelta:
			...
			publishMessagePart(optimus-ide-collabsdk.ChatMessageRoleAssistant, optimus-ide-collabsdk.ChatMessagePart{
		...
		case fantasy.StreamPartTypeToolCall:
			...
			ssePart.CreatedAt = &now
			publishMessagePart(
				optimus-ide-collabsdk.ChatMessageRoleAssistant,
				ssePart,
			)
		...
		case fantasy.StreamPartTypeToolResult:
			...
			if part.ProviderExecuted {
				...
				ssePart.CreatedAt = &now
				publishMessagePart(
					optimus-ide-collabsdk.ChatMessageRoleTool,
					ssePart,
				)
			}
...
func publishToolAttachments(
	...
	for _, attachment := range attachments {
		filePart := optimus-ide-collabsdk.ChatMessageFile(
			attachment.FileID,
			attachment.MediaType,
			attachment.Name,
		)
		filePart.CreatedAt = &createdAt
		publishMessagePart(optimus-ide-collabsdk.ChatMessageRoleAssistant, filePart)
	}
}
