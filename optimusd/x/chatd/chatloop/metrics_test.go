...
func TestNewMetrics_RegistersAllMetrics(t *testing.T) {
	...
	m.StreamRetriesTotal.WithLabelValues("anthropic", "claude-sonnet-4-5", string(optimus-ide-collabsdk.ChatErrorKindTimeout))
	...
	expected := map[string]dto.MetricType{
		"optimus-ide-collabd_chatd_chats":                       dto.MetricType_GAUGE,
		"optimus-ide-collabd_chatd_message_count":               dto.MetricType_HISTOGRAM,
		"optimus-ide-collabd_chatd_prompt_size_bytes":           dto.MetricType_HISTOGRAM,
		"optimus-ide-collabd_chatd_tool_result_size_bytes":      dto.MetricType_HISTOGRAM,
		"optimus-ide-collabd_chatd_ttft_seconds":                dto.MetricType_HISTOGRAM,
		"optimus-ide-collabd_chatd_compaction_total":            dto.MetricType_COUNTER,
		"optimus-ide-collabd_chatd_steps_total":                 dto.MetricType_COUNTER,
		"optimus-ide-collabd_chatd_stream_retries_total":        dto.MetricType_COUNTER,
		"optimus-ide-collabd_chatd_stream_buffer_dropped_total": dto.MetricType_COUNTER,
		"optimus-ide-collabd_chatd_tool_errors_total":           dto.MetricType_COUNTER,
	}
...
func TestNopMetrics_DoesNotPanic(t *testing.T) {
	...
	m.StreamRetriesTotal.WithLabelValues("anthropic", "claude-sonnet-4-5", string(optimus-ide-collabsdk.ChatErrorKindTimeout)).Inc()
	...
	nilMetrics.RecordStreamRetry("anthropic", "claude-sonnet-4-5", chaterror.ClassifiedError{Kind: optimus-ide-collabsdk.ChatErrorKindTimeout})
...
func TestRecordCompaction(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantCount == 0 {
				for _, f := range families {
					assert.NotEqual(t, "optimus-ide-collabd_chatd_compaction_total", f.GetName(),
						"compaction_total should not be recorded")
				}
				return
			}

			requireCounter(t, reg, "optimus-ide-collabd_chatd_compaction_total", float64(tt.wantCount), map[string]string{
...
func TestRecordStreamRetry(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "overloaded", kind: optimus-ide-collabsdk.ChatErrorKindOverloaded},
		{name: "rate_limit", kind: optimus-ide-collabsdk.ChatErrorKindRateLimit},
		{name: "timeout", kind: optimus-ide-collabsdk.ChatErrorKindTimeout},
		{name: "stream_silence_timeout", kind: optimus-ide-collabsdk.ChatErrorKindStreamSilenceTimeout},
		{name: "auth", kind: optimus-ide-collabsdk.ChatErrorKindAuth},
		{name: "config", kind: optimus-ide-collabsdk.ChatErrorKindConfig},
		{name: "missing_key", kind: optimus-ide-collabsdk.ChatErrorKindMissingKey},
		{name: "generic", kind: optimus-ide-collabsdk.ChatErrorKindGeneric},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			requireCounter(t, reg, "optimus-ide-collabd_chatd_stream_retries_total", 1, map[string]string{
				"provider": "test-provider",
				"model":    "test-model",
				"kind":     string(tt.kind),
			})
...
func TestRecordStreamBufferDropped(t *testing.T) {
	...
	t.Run("increments monotonically", func(t *testing.T) {
		...
		var found bool
		for _, f := range families {
			if f.GetName() != "optimus-ide-collabd_chatd_stream_buffer_dropped_total" {
				continue
			}
			found = true
...
func TestRecordToolError(t *testing.T) {
	...
	t.Run("increments with correct labels", func(t *testing.T) {
		...
		requireCounter(t, reg, "optimus-ide-collabd_chatd_tool_errors_total", 1, map[string]string{
			"provider":  "test-provider",
			"model":     "test-model",
			"tool_name": "read_file",
		})
...
func TestGenerateAssistant_StreamRetryRecordsMetric(t *testing.T) {
	...
	requireCounter(t, reg, "optimus-ide-collabd_chatd_stream_retries_total", 1, map[string]string{
		"provider": "test-provider",
		"model":    "test-model",
		"kind":     string(optimus-ide-collabsdk.ChatErrorKindRateLimit),
	})
}
...
func TestGenerateAssistant_StreamRetry_ContextCanceledTransportResetIncrements(t *testing.T) {
	...
	requireCounter(t, reg, "optimus-ide-collabd_chatd_stream_retries_total", 1, map[string]string{
		"provider": "test-provider",
		"model":    "test-model",
		"kind":     string(optimus-ide-collabsdk.ChatErrorKindTimeout),
	})
}
