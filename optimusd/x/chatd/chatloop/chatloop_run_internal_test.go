...
func TestGenerateAssistant_ErrorProviderOverridesTransportLabel(t *testing.T) {
	...
	model := &chattest.FakeModel{
		...
		ModelName:    "qwen3-optimus-ide-collab-next",
		StreamFn: func(_ context.Context, _ fantasy.Call) (fantasy.StreamResponse, error) {
			return nil, xerrors.New("upstream returned status 400")
		},
	}
...
func TestGenerateAssistant_StreamSilenceTimeoutRetryClassification(t *testing.T) {
	...
	t.Run("timeout before first part", func(t *testing.T) {
		...
		require.Error(t, err)
		classified := chaterror.Classify(err)
		require.Equal(t, optimus-ide-collabsdk.ChatErrorKindStreamSilenceTimeout, classified.Kind)
		require.Equal(t, "openai", classified.Provider)
		require.True(t, classified.Retryable)
		require.Equal(t, int32(1), calls.Load())
	})
...
func TestSanitizeAnthropicProviderToolContent(t *testing.T) {
	...
	metadataCall := providerCall("ws-meta", "web_search", `{"query":"optimus-ide-collab"}`)
	...
	pointerCall := providerCall("ws-pointer", "web_search", `{"query":"optimus-ide-collab"}`)
	...
	testCases := []struct {
		...
		},
		{
			...
			content: []fantasy.Content{
				providerResult("ws-1", "web_search"),
				providerCall("ws-1", "web_search", `{"query":"optimus-ide-collab"}`),
			},
			wantRemovedCalls:   1,
			wantRemovedResults: 1,
			wantTexts:          []string{"ok"},
		...
		},
		{
			...
			provider: fantasyanthropic.Name,
			content: []fantasy.Content{
				providerCall("", "web_search", `{"query":"optimus-ide-collab"}`),
		...
		},
		{
			...
			provider: fantasyanthropic.Name,
			content: []fantasy.Content{
				providerCall("ws-empty", "", `{"query":"optimus-ide-collab"}`),
		...
		},
		{
			...
			provider: fantasyanthropic.Name,
			content: []fantasy.Content{
				providerCall("ws-mismatch", "web_search", `{"query":"optimus-ide-collab"}`),
		...
		},
		{
			...
			provider: fantasyanthropic.Name,
			content: []fantasy.Content{
				providerCall("dup-1", "web_search", `{"query":"optimus-ide-collab"}`),
		...
		},
		{
			...
			provider: fantasyanthropic.Name,
			content: []fantasy.Content{
				providerCall("mix-1", "web_search", `{"query":"optimus-ide-collab"}`),
