...
func GenerateCompaction(ctx context.Context, opts GenerateCompactionOptions) (CompactionResult, error) {
	...
	if config.PublishMessagePart != nil && config.ToolCallID != "" {
		config.PublishMessagePart(
			...
			optimus-ide-collabsdk.ChatMessageToolCall(config.ToolCallID, config.ToolName, nil),
		)
	}
	...
	if config.PublishMessagePart != nil && config.ToolCallID != "" {
		...
		config.PublishMessagePart(
			...
			optimus-ide-collabsdk.ChatMessageToolResult(config.ToolCallID, config.ToolName, resultJSON, false, false),
		)
	}
	return result, nil
}
...
func publishCompactionError(config CompactionOptions, msg string) {
	...
	config.PublishMessagePart(
		optimus-ide-collabsdk.ChatMessageRoleTool,
		optimus-ide-collabsdk.ChatMessageToolResult(config.ToolCallID, config.ToolName, errJSON, true, false),
	)
}
