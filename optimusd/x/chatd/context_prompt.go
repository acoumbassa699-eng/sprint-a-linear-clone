...
// the agent context push (optimus-ide-collabd/agentapi/context.go). DiscardUnknown keeps
...
func mcpToolsFromServerBody(server string, body json.RawMessage) []optimus-ide-collabsdk.ChatContextTool {
	...
	out := make([]optimus-ide-collabsdk.ChatContextTool, 0, len(tools))
	for _, t := range tools {
		name := strings.TrimPrefix(t.GetName(), prefix)
		if name == "" {
			continue
		}
		out = append(out, optimus-ide-collabsdk.ChatContextTool{
			Name:        name,
...
func contextResourcesToPrompt(
	...
	var contextFileParts []optimus-ide-collabsdk.ChatMessagePart
	for _, r := range resources {
		...
		case database.WorkspaceAgentContextBodyKindInstructionFile:
			...
			contextFileParts = append(contextFileParts, optimus-ide-collabsdk.ChatMessagePart{
		...
		case database.WorkspaceAgentContextBodyKindSkill:
			...
			// OPTIMUS-IDE-COLLAB_AGENT_EXP_SKILL_META_FILE is not preserved on this
...
func (server *Server) ContextResources(
	ctx context.Context,
	chat database.Chat,
) ([]optimus-ide-collabsdk.ChatContextResource, error) {
...
// the body deoptimus-ide-collabs, but where the prompt builder keeps only OK instruction
...
func pinnedContextResources(resources []database.ChatContextResource) []optimus-ide-collabsdk.ChatContextResource {
	var out []optimus-ide-collabsdk.ChatContextResource
	for _, r := range resources {
		...
		if r.Status != database.WorkspaceAgentContextResourceStatusOk {
			...
			out = append(out, optimus-ide-collabsdk.ChatContextResource{
				Source:    r.Source,
				Kind:      kind,
				SizeBytes: r.SizeBytes,
				Status:    optimus-ide-collabsdk.ChatContextResourceStatus(r.Status),
				Error:     r.Error,
			})
			continue
		}
...
// contextResourceKind maps a database body kind to the optimus-ide-collabsdk kind reported
...
// resource list.
func contextResourceKind(kind database.WorkspaceAgentContextBodyKind) (optimus-ide-collabsdk.ChatContextResourceKind, bool) {
	switch kind {
	case database.WorkspaceAgentContextBodyKindInstructionFile:
		return optimus-ide-collabsdk.ChatContextResourceKindInstructionFile, true
	case database.WorkspaceAgentContextBodyKindSkill:
		return optimus-ide-collabsdk.ChatContextResourceKindSkill, true
	case database.WorkspaceAgentContextBodyKindMcpConfig:
		return optimus-ide-collabsdk.ChatContextResourceKindMCPConfig, true
	case database.WorkspaceAgentContextBodyKindMcpServer:
		return optimus-ide-collabsdk.ChatContextResourceKindMCPServer, true
	default:
		return "", false
	}
}
