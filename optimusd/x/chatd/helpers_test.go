...
func (p *recordingPubsub) ownershipMessages(t *testing.T) []optimus-ide-collabdpubsub.ChatStateOwnershipMessage {
	t.Helper()
	p.mu.Lock()
	defer p.mu.Unlock()
	messages := make([]optimus-ide-collabdpubsub.ChatStateOwnershipMessage, 0)
	for _, event := range p.events {
		if event.channel != optimus-ide-collabdpubsub.ChatStateOwnershipChannel {
			continue
		}
		var msg optimus-ide-collabdpubsub.ChatStateOwnershipMessage
...
func (p *recordingPubsub) watchEvents(t *testing.T) []optimus-ide-collabsdk.ChatWatchEvent {
	t.Helper()
	p.mu.Lock()
	defer p.mu.Unlock()
	events := make([]optimus-ide-collabsdk.ChatWatchEvent, 0)
	for _, event := range p.events {
		var msg optimus-ide-collabsdk.ChatWatchEvent
		if err := json.Unmarshal(event.payload, &msg); err != nil {
			continue
		}
		if event.channel != optimus-ide-collabdpubsub.ChatWatchEventChannel(msg.Chat.OwnerID) {
			continue
		}
...
func (p *recordingPubsub) stateUpdateMessages(t *testing.T, chatID uuid.UUID) []optimus-ide-collabdpubsub.ChatStateUpdateMessage {
	t.Helper()
	p.mu.Lock()
	defer p.mu.Unlock()
	messages := make([]optimus-ide-collabdpubsub.ChatStateUpdateMessage, 0)
	for _, event := range p.events {
		if event.channel != optimus-ide-collabdpubsub.ChatStateUpdateChannel(chatID) {
			continue
		}
		var msg optimus-ide-collabdpubsub.ChatStateUpdateMessage
		require.NoError(t, json.Unmarshal(event.payload, &msg))
		messages = append(messages, msg)
	}
	return messages
