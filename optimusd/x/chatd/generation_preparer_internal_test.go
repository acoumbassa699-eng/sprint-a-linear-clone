...
func mustMarshalText(t *testing.T, parts ...string) pqtype.NullRawMessage {
	t.Helper()
	messageParts := make([]optimus-ide-collabsdk.ChatMessagePart, 0, len(parts))
	for _, p := range parts {
		messageParts = append(messageParts, optimus-ide-collabsdk.ChatMessageText(p))
	}
...
func TestPrepareGenerationClampsRequestedReasoningEffortToMax(t *testing.T) {
	...
	modelConfigRaw, err := json.Marshal(optimus-ide-collabsdk.ChatModelCallConfig{
		ReasoningEffort: &optimus-ide-collabsdk.ChatModelReasoningEffortConfig{
			...
			Max:     ptr.Ref(optimus-ide-collabsdk.ChatModelReasoningEffortMedium),
		},
	})
	require.NoError(t, err)
