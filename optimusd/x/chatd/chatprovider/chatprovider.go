...
import (
	...
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabd/x/chatd/chatutil"
...
var providerDisplayNameByName = map[string]string{
	...
	string(optimus-ide-collabsdk.AIProviderTypeCopilot): "GitHub Copilot",
}
...
func AgentsSupportsProvider(provider string) bool {
	providerType := optimus-ide-collabsdk.AIProviderType(strings.ToLower(strings.TrimSpace(provider)))
	if optimus-ide-collabsdk.IsAgentsUnsupportedProviderType(providerType) {
		return false
	}
...
func UnsupportedProviders(configured []ConfiguredProvider) []optimus-ide-collabsdk.ChatUnsupportedProvider {
	seen := make(map[string]struct{}, len(configured))
	unsupported := make([]optimus-ide-collabsdk.ChatUnsupportedProvider, 0)
	for _, provider := range configured {
		...
		seen[key] = struct{}{}
		unsupported = append(unsupported, optimus-ide-collabsdk.ChatUnsupportedProvider{
			Provider:    key,
			DisplayName: ProviderDisplayName(provider.Provider),
		})
	}
	slices.SortFunc(unsupported, func(a, b optimus-ide-collabsdk.ChatUnsupportedProvider) int {
...
// ambient credentials from the Optimus-IDE-Collab server instead of an explicit
// API key.
...
func InlineImageCapBytes(provider string) (int, bool) {
	...
	case fantasyanthropic.Name, fantasybedrock.Name:
		return optimus-ide-collabsdk.AnthropicInlineImageCapBytes, true
	default:
		return 0, false
	}
}
...
func AcceptsFilePartMediaType(provider, modelID, mediaType string) bool {
	...
	// allowlist (optimus-ide-collabsdk.AllChatAttachmentMediaTypes).
...
type ProviderAvailability struct {
	Available         bool
	UnavailableReason optimus-ide-collabsdk.ChatModelProviderUnavailableReason
}
	...
	for _, provider := range providers {
		...
		case centralKey != "":
			if !provider.AllowUserAPIKey || provider.AllowCentralAPIKeyFallback {
				...
				resolved.UnavailableReason = optimus-ide-collabsdk.ChatModelProviderUnavailableReasonUserAPIKeyRequired
			}
		case normalizedProvider == fantasybedrock.Name && provider.CentralAPIKeyEnabled:
			// Bedrock can use ambient AWS credentials from the Optimus-IDE-Collab server
		...
		case provider.AllowUserAPIKey:
			resolved.UnavailableReason = optimus-ide-collabsdk.ChatModelProviderUnavailableReasonUserAPIKeyRequired
		default:
			resolved.UnavailableReason = optimus-ide-collabsdk.ChatModelProviderUnavailableMissingAPIKey
		}
...
func (*ModelCatalog) ListConfiguredModels(
...
) (optimus-ide-collabsdk.ChatModelsResponse, bool) {
	if len(configuredModels) == 0 {
		return optimus-ide-collabsdk.ChatModelsResponse{}, false
	}

	modelsByProvider := make(map[string][]optimus-ide-collabsdk.ChatModel)
	...
	response := optimus-ide-collabsdk.ChatModelsResponse{
		Providers: make([]optimus-ide-collabsdk.ChatModelProvider, 0, len(providers)),
	}
	for _, provider := range providers {
		...
		result := optimus-ide-collabsdk.ChatModelProvider{
			Provider: provider,
			Models:   models,
		}
		if avail, ok := availabilityByProvider[provider]; ok {
			...
			result.UnavailableReason = optimus-ide-collabsdk.ChatModelProviderUnavailableMissingAPIKey
		}
...
func (*ModelCatalog) ListConfiguredProviderAvailability(
	...
	response := optimus-ide-collabsdk.ChatModelsResponse{
		Providers: make([]optimus-ide-collabsdk.ChatModelProvider, 0, len(supportedProviderNames)),
	}
...
func newChatModel(provider, modelID, displayName string) optimus-ide-collabsdk.ChatModel {
	name := strings.TrimSpace(displayName)
	if name == "" {
		name = modelID
	}

	return optimus-ide-collabsdk.ChatModel{
...
func sortChatModels(models []optimus-ide-collabsdk.ChatModel) {
	slices.SortFunc(models, func(a, b optimus-ide-collabsdk.ChatModel) int {
...
// Optimus-IDE-Collab entities.
const (
	// HeaderOptimus-IDE-CollabOwnerID identifies the Optimus-IDE-Collab user who owns the chat.
	HeaderOptimus-IDE-CollabOwnerID = "X-Optimus-IDE-Collab-Owner-Id"
	// HeaderOptimus-IDE-CollabChatID identifies the top-level (parent) chat.
	// For root chats this is the chat's own ID; for subchats it
	// is the parent chat's ID.
	HeaderOptimus-IDE-CollabChatID = "X-Optimus-IDE-Collab-Chat-Id"
	// HeaderOptimus-IDE-CollabSubchatID identifies the current subchat. Only
	// present when the request originates from a child chat.
	HeaderOptimus-IDE-CollabSubchatID = "X-Optimus-IDE-Collab-Subchat-Id"
	// HeaderOptimus-IDE-CollabWorkspaceID identifies the workspace associated
	// with the chat, if any.
	HeaderOptimus-IDE-CollabWorkspaceID = "X-Optimus-IDE-Collab-Workspace-Id"
)

// Optimus-IDE-CollabHeaders builds the set of Optimus-IDE-Collab identity headers to attach
// to outgoing LLM API requests for the given chat.
func Optimus-IDE-CollabHeaders(chat database.Chat) map[string]string {
	chatID := chat.ID
	if chat.ParentChatID.Valid {
		chatID = chat.ParentChatID.UUID
	}
	h := map[string]string{
		HeaderOptimus-IDE-CollabOwnerID: chat.OwnerID.String(),
		HeaderOptimus-IDE-CollabChatID:  chatID.String(),
	}
	if chat.ParentChatID.Valid {
		h[HeaderOptimus-IDE-CollabSubchatID] = chat.ID.String()
	}
	if chat.WorkspaceID.Valid {
		h[HeaderOptimus-IDE-CollabWorkspaceID] = chat.WorkspaceID.UUID.String()
	}
	return h
}
...
// Bedrock calls.
func BetaHeadersFromCallConfig(providerName string, config *optimus-ide-collabsdk.ChatModelCallConfig) map[string]string {
...
func ProviderOptionsFromChatModelConfig(
	...
	options *optimus-ide-collabsdk.ChatModelProviderOptions,
...
func anthropicProviderOptionsFromChatConfig(
	options *optimus-ide-collabsdk.ChatModelAnthropicProviderOptions,
...
func googleProviderOptionsFromChatConfig(
	options *optimus-ide-collabsdk.ChatModelGoogleProviderOptions,
...
func openAICompatProviderOptionsFromChatConfig(
	options *optimus-ide-collabsdk.ChatModelOpenAICompatProviderOptions,
...
func openRouterProviderOptionsFromChatConfig(
	options *optimus-ide-collabsdk.ChatModelOpenRouterProviderOptions,
...
func vercelProviderOptionsFromChatConfig(
	options *optimus-ide-collabsdk.ChatModelVercelProviderOptions,
