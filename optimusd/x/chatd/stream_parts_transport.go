...
type streamPartsServerTransport interface {
	...
	WriteEvents(context.Context, []optimus-ide-collabsdk.ChatStreamEvent) error
	Close() error
}

type streamPartsClientTransport interface {
	WriteControl(context.Context, streamPartsControl) error
	ReadEvents(context.Context) ([]optimus-ide-collabsdk.ChatStreamEvent, error)
	Close() error
}
...
func (t streamPartsWebSocketServerTransport) WriteEvents(ctx context.Context, events []optimus-ide-collabsdk.ChatStreamEvent) error {
...
func (t streamPartsWebSocketClientTransport) ReadEvents(ctx context.Context) ([]optimus-ide-collabsdk.ChatStreamEvent, error) {
	var batch []optimus-ide-collabsdk.ChatStreamEvent
...
type streamPartsChannelPipe struct {
	...
	eventsCh  chan []optimus-ide-collabsdk.ChatStreamEvent
	done      chan struct{}
	closeOnce sync.Once
}
...
func newStreamPartsChannelTransportPair() (streamPartsServerTransport, streamPartsClientTransport) {
	pipe := &streamPartsChannelPipe{
		...
		eventsCh:  make(chan []optimus-ide-collabsdk.ChatStreamEvent, 128),
...
func (t streamPartsChannelServerTransport) WriteEvents(ctx context.Context, events []optimus-ide-collabsdk.ChatStreamEvent) error {
	select {
	case <-ctx.Done():
		return ctx.Err()
	case <-t.pipe.done:
...
func (t streamPartsChannelClientTransport) ReadEvents(ctx context.Context) ([]optimus-ide-collabsdk.ChatStreamEvent, error) {
	select {
	case <-ctx.Done():
		return nil, ctx.Err()
	case <-t.pipe.done:
