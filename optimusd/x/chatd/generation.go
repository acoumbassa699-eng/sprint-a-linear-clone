...
type generationOutcome struct {
	...
	WatchEventKind    optimus-ide-collabsdk.ChatWatchEventKind
	LastError         string
...
func unresolvedToolCallsFromHistory(
	...
	for _, part := range assistantParts {
		if part.Type != optimus-ide-collabsdk.ChatMessagePartTypeToolCall || part.ProviderExecuted || handled[part.ToolCallID] {
			continue
		}
...
func (*taskStarter) recordGenerationRetry(
	...
	var payload *optimus-ide-collabsdk.ChatStreamRetry
...
func (s *taskStarter) beginGenerationAttempt(
	...
	return generationAttempt{
		number: attempt,
		publish: func(role optimus-ide-collabsdk.ChatMessageRole, part optimus-ide-collabsdk.ChatMessagePart) {
...
func (s *taskStarter) commitGenerationStep(
	...
	err := machine.Update(ctx, func(tx *chatstate.Tx, store database.Store) error {
		...
		for _, msg := range commitResult.InsertedMessages {
			insertedMessages = append(insertedMessages, runnerActionMessage{ID: msg.ID, Role: optimus-ide-collabsdk.ChatMessageRole(msg.Role)})
		}
		committed, err = store.GetChatByID(ctx, input.ChatID)
		if err != nil {
			return xerrors.Errorf("load committed chat: %w", err)
...
func (s *taskStarter) enterRequiresAction(
	...
	if err := s.publishWatchAndRoute(ctx, committed, optimus-ide-collabsdk.ChatWatchEventKindActionRequired); err != nil {
		return xerrors.Errorf("publish watch and route: %w", err)
	}
	return s.afterGenerationOutcome(ctx, generationOutcome{
		Chat:           committed,
		Kind:           runnerActionKindEnterRequiresAction,
		WatchEventKind: optimus-ide-collabsdk.ChatWatchEventKindActionRequired,
	})
}
...
func (s *taskStarter) finishGenerationTurn(
	...
	defer cancel()
	if err := s.publishWatchWithRetry(watchCtx, committed, optimus-ide-collabsdk.ChatWatchEventKindStatusChange); err != nil {
		return xerrors.Errorf("publish watch and route: %w", err)
	}
	if err := s.afterGenerationOutcome(ctx, generationOutcome{
		Chat:              committed,
		Kind:              runnerActionKindFinishTurn,
		WatchEventKind:    optimus-ide-collabsdk.ChatWatchEventKindStatusChange,
...
func (s *taskStarter) finishGenerationError(
	...
	if err := s.publishWatchAndRoute(ctx, committed, optimus-ide-collabsdk.ChatWatchEventKindStatusChange); err != nil {
		return xerrors.Errorf("publish watch and route: %w", err)
	}
	return s.afterGenerationOutcome(ctx, generationOutcome{
		Chat:           committed,
...
func generationLastError(err error) (pqtype.NullRawMessage, string) {
	...
	if payload == nil {
		payload = &optimus-ide-collabsdk.ChatError{Message: err.Error()}
	}
