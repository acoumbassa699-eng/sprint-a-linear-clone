...
type cachedAdvisorConfig struct {
	config    optimus-ide-collabsdk.AdvisorConfig
	expiresAt time.Time
}
...
type chatConfigCache struct {
	...
	advisorConfigFetches    singleflight.Group[string, optimus-ide-collabsdk.AdvisorConfig]
}
...
func (c *chatConfigCache) AdvisorConfig(ctx context.Context) (optimus-ide-collabsdk.AdvisorConfig, error) {
	...
	config, err := singleflightDoChan(
		...
		func() (optimus-ide-collabsdk.AdvisorConfig, error) {
			...
			if err != nil {
				return optimus-ide-collabsdk.AdvisorConfig{}, err
			}
...
func (c *chatConfigCache) cachedAdvisorConfig() (optimus-ide-collabsdk.AdvisorConfig, bool) {
	c.mu.RLock()
	entry := c.advisorConfig
	c.mu.RUnlock()
	if entry == nil {
		return optimus-ide-collabsdk.AdvisorConfig{}, false
	}
	if c.clock.Now().Before(entry.expiresAt) {
...
func (c *chatConfigCache) storeAdvisorConfig(generation uint64, config optimus-ide-collabsdk.AdvisorConfig) {
	c.mu.Lock()
	defer c.mu.Unlock()
