...
func Classify(err error) ClassifiedError {
	...
	for _, rule := range rules {
		...
		detail := structured.detail
		if rule.kind != optimus-ide-collabsdk.ChatErrorKindAuth {
...
func normalizeClassification(classified ClassifiedError) ClassifiedError {
	...
	classified.Kind = optimus-ide-collabsdk.ChatErrorKind(strings.TrimSpace(string(classified.Kind)))
	...
	if classified.Kind == "" && classified.Message == "" {
		...
		classified.Kind = optimus-ide-collabsdk.ChatErrorKindGeneric
	}
	if classified.Kind == "" {
		classified.Kind = optimus-ide-collabsdk.ChatErrorKindGeneric
	}
