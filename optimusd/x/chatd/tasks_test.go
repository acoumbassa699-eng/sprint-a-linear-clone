...
func TestInterruptTask_FinishInterruptionOnly(t *testing.T) {
	...
	require.NoError(t, buffer.AddPart(key, optimus-ide-collabsdk.ChatMessageRoleAssistant, optimus-ide-collabsdk.ChatMessageText("partial answer")))
	...
	f.requireWatchEvent(t, chat.ID, optimus-ide-collabsdk.ChatWatchEventKindStatusChange)
	...
	require.NoError(t, err)
	require.Equal(t, []optimus-ide-collabsdk.ChatMessagePart{optimus-ide-collabsdk.ChatMessageText("partial answer")}, parts)
...
func TestInterruptTask_BufferedPartsBecomePartialMessages(t *testing.T) {
	...
	require.NoError(t, buffer.AddPart(key, optimus-ide-collabsdk.ChatMessageRoleAssistant, optimus-ide-collabsdk.ChatMessagePart{
...
func TestGenerationTask_RecordRetryState(t *testing.T) {
	...
	var retryPayload optimus-ide-collabsdk.ChatStreamRetry
	...
	require.Equal(t, optimus-ide-collabsdk.ChatErrorKindRateLimit, retryPayload.Kind)
...
func (f *taskTestFixture) createRequiresActionChat(t *testing.T) database.Chat {
	...
	dynamicTools, err := json.Marshal([]optimus-ide-collabsdk.DynamicTool{{
		Name:        toolName,
...
func (f *taskTestFixture) publishChatUpdate(t *testing.T, chat database.Chat) {
	t.Helper()
	msg := optimus-ide-collabdpubsub.ChatStateUpdateMessage{
	...
	require.NoError(t, err)
	require.NoError(t, f.pubsub.Publish(optimus-ide-collabdpubsub.ChatStateUpdateChannel(chat.ID), payload))
}

func (f *taskTestFixture) requireWatchEvent(t *testing.T, chatID uuid.UUID, kind optimus-ide-collabsdk.ChatWatchEventKind) {
	t.Helper()
...
func taskUserTextMessage(t *testing.T, text string, createdBy uuid.UUID, modelConfigID uuid.UUID, apiKeyID string) chatstate.Message {
	t.Helper()
	raw, err := chatprompt.MarshalParts([]optimus-ide-collabsdk.ChatMessagePart{optimus-ide-collabsdk.ChatMessageText(text)})
	require.NoError(t, err)
...
func (p *taskRecordingPubsub) watchEvents(t *testing.T) []optimus-ide-collabsdk.ChatWatchEvent {
	t.Helper()
	events := p.events()
	out := make([]optimus-ide-collabsdk.ChatWatchEvent, 0)
	for _, event := range events {
		var payload optimus-ide-collabsdk.ChatWatchEvent
		if err := json.Unmarshal(event.payload, &payload); err != nil {
			continue
		}
		if event.channel != optimus-ide-collabdpubsub.ChatWatchEventChannel(payload.Chat.OwnerID) {
			continue
		}
