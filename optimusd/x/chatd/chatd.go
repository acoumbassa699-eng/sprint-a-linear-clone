...
import (
	...
	skillspkg "github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabd/x/skills"
...
func (p *Server) loadAdvisorConfig(ctx context.Context, logger slog.Logger) optimus-ide-collabsdk.AdvisorConfig {
	cfg, err := p.configCache.AdvisorConfig(ctx)
	if err != nil {
		logger.Warn(ctx, "failed to load advisor config", slog.Error(err))
		return optimus-ide-collabsdk.AdvisorConfig{}
	}
	return cfg
}
...
func (p *Server) resolveAdvisorModelOverride(
	ctx context.Context,
	chat database.Chat,
	advisorCfg optimus-ide-collabsdk.AdvisorConfig,
	fallbackModel fantasy.LanguageModel,
	fallbackCallConfig optimus-ide-collabsdk.ChatModelCallConfig,
	modelOpts modelBuildOptions,
	logger slog.Logger,
) (fantasy.LanguageModel, optimus-ide-collabsdk.ChatModelCallConfig, error) {
	...
	overrideCallConfig := optimus-ide-collabsdk.ChatModelCallConfig{}
	...
	if err != nil {
		if overrideConfig.AIProviderID.Valid {
			return nil, optimus-ide-collabsdk.ChatModelCallConfig{}, xerrors.Errorf("resolve advisor override route: %w", err)
		}
		logger.Warn(
			ctx,
	...
	if err != nil {
		if overrideConfig.AIProviderID.Valid {
			return nil, optimus-ide-collabsdk.ChatModelCallConfig{}, xerrors.Errorf("create advisor override model: %w", err)
		}
		logger.Warn(
			ctx,
	...
	if advisorCfg.ReasoningEffort != nil {
		...
		if resolvedEffort != nil {
			overrideCallConfig.ReasoningEffort = &optimus-ide-collabsdk.ChatModelReasoningEffortConfig{
	...
	for attempt := 0; attempt < 2; attempt++ {
		...
		if c.conn == nil {
			...
			agentConn.SetExtraHeaders(http.Header{
				workspacesdk.Optimus-IDE-CollabChatIDHeader:          {chatSnapshot.ID.String()},
				workspacesdk.Optimus-IDE-CollabAncestorChatIDsHeader: {string(ancestorJSON)},
			})
...
func (p *Server) CreateChat(ctx context.Context, opts CreateOptions) (database.Chat, error) {
	...
	workspaceAwarenessContent, err := chatprompt.MarshalParts([]optimus-ide-collabsdk.ChatMessagePart{
		optimus-ide-collabsdk.ChatMessageText(workspaceAwareness),
	})
	if err != nil {
	...
	if deploymentPrompt != "" {
		deploymentContent, marshalErr := chatprompt.MarshalParts([]optimus-ide-collabsdk.ChatMessagePart{
			optimus-ide-collabsdk.ChatMessageText(deploymentPrompt),
		})
		if marshalErr != nil {
	...
	if userPrompt != "" {
		userPromptContent, marshalErr := chatprompt.MarshalParts([]optimus-ide-collabsdk.ChatMessagePart{
			optimus-ide-collabsdk.ChatMessageText(userPrompt),
		})
		if marshalErr != nil {
	...
	p.publishChatPubsubEvent(chat, optimus-ide-collabsdk.ChatWatchEventKindCreated, nil)
...
func (p *Server) SendMessage(
	...
	p.publishChatPubsubEvent(result.Chat, optimus-ide-collabsdk.ChatWatchEventKindStatusChange, nil)
	return result, nil
}
...
// the optimus-ide-collabd preflight can race an admin disabling the model or provider.
...
func (p *Server) ArchiveChat(ctx context.Context, chat database.Chat) error {
	...
	return p.setChatFamilyArchived(ctx, chat, true, optimus-ide-collabsdk.ChatWatchEventKindDeleted)
}
...
func (p *Server) UnarchiveChat(ctx context.Context, chat database.Chat) error {
	...
	return p.setChatFamilyArchived(ctx, chat, false, optimus-ide-collabsdk.ChatWatchEventKindCreated)
}
...
func (p *Server) setChatFamilyArchived(
	...
	chat database.Chat,
	archived bool,
	watchKind optimus-ide-collabsdk.ChatWatchEventKind,
) error {
	if chat.ID == uuid.Nil {
		return xerrors.New("chat_id is required")
	}
...
func (p *Server) PromoteQueued(
	...
	if refreshedOK {
		p.publishChatPubsubEvent(refreshChat, optimus-ide-collabsdk.ChatWatchEventKindStatusChange, nil)
	}
	return result, nil
}
...
func (p *Server) InterruptChat(
	...
	p.publishChatPubsubEvent(refreshed, optimus-ide-collabsdk.ChatWatchEventKindStatusChange, nil)
...
func (p *Server) regenerateChatTitleWithStore(
	...
	p.publishChatPubsubEvent(updatedChat, optimus-ide-collabsdk.ChatWatchEventKindTitleChange, nil)
	return updatedChat, nil
}
...
func New(ps pubsub.Pubsub, cfg Config) *Server {
	...
	cancelConfigSub, err := p.pubsub.SubscribeWithErr(
		optimus-ide-collabdpubsub.ChatConfigEventChannel,
		optimus-ide-collabdpubsub.HandleChatConfigEvent(func(ctx context.Context, ev optimus-ide-collabdpubsub.ChatConfigEvent, err error) {
			if err != nil {
				p.logger.Warn(ctx, "chat config event error", slog.Error(err))
				return
			}
			switch ev.Kind {
			case optimus-ide-collabdpubsub.ChatConfigEventModelConfig:
				p.configCache.InvalidateModelConfig(ev.EntityID)
			case optimus-ide-collabdpubsub.ChatConfigEventUserPrompt:
				p.configCache.InvalidateUserPrompt(ev.EntityID)
			case optimus-ide-collabdpubsub.ChatConfigEventAdvisorConfig:
	...
	cancelProviderSub, err := p.pubsub.SubscribeWithErr(
		optimus-ide-collabdpubsub.AIProvidersChangedChannel,
...
func subscribeWithInitialError(chatID uuid.UUID, message string) (
	...
	<-chan optimus-ide-collabsdk.ChatStreamEvent,
	func(),
	bool,
) {
	events := make(chan optimus-ide-collabsdk.ChatStreamEvent)
	close(events)
	return []optimus-ide-collabsdk.ChatStreamEvent{{
		Type:   optimus-ide-collabsdk.ChatStreamEventTypeError,
		ChatID: chatID,
		Error:  &optimus-ide-collabsdk.ChatError{Message: message},
	}}, events, func() {}, true
}

// publishChatPubsubEvents broadcasts a lifecycle event for each affected chat.
func (p *Server) publishChatPubsubEvents(chats []database.Chat, kind optimus-ide-collabsdk.ChatWatchEventKind) {
...
func chatWatchEventSDKChat(chat database.Chat, diffStatus *optimus-ide-collabsdk.ChatDiffStatus) optimus-ide-collabsdk.Chat {
...
func (p *Server) publishChatPubsubEvent(chat database.Chat, kind optimus-ide-collabsdk.ChatWatchEventKind, diffStatus *optimus-ide-collabsdk.ChatDiffStatus) {
	if p.pubsub == nil {
		return
	}
	event := optimus-ide-collabsdk.ChatWatchEvent{
		Kind: kind,
	...
	if err := p.pubsub.Publish(optimus-ide-collabdpubsub.ChatWatchEventChannel(chat.OwnerID), payload); err != nil {
...
func (p *Server) PublishDiffStatusChange(ctx context.Context, chatID uuid.UUID) error {
	...
	p.publishChatPubsubEvent(chat, optimus-ide-collabsdk.ChatWatchEventKindDiffStatusChange, &sdkStatus)
	return nil
}
...
func (p *Server) fetchPersonalSkillMetadata(
	...
	// See package optimus-ide-collabd/x/skills (doc.go) for why metadata fetch failures
...
func (p *Server) appendRootChatTools(
	...
	onChatUpdated := func(updatedChat database.Chat) {
		...
		p.publishChatPubsubEvent(updatedChat, optimus-ide-collabsdk.ChatWatchEventKindStatusChange, nil)
	}
...
func appendDynamicTools(
	...
	var dynamicToolDefs []optimus-ide-collabsdk.DynamicTool
	if raw.Valid {
	...
	var filteredDefs []optimus-ide-collabsdk.DynamicTool
...
func buildProviderTools(options *optimus-ide-collabsdk.ChatModelProviderOptions) []chatloop.ProviderTool {
...
func parseDynamicToolNames(raw pqtype.NullRawMessage) (map[string]bool, error) {
	...
	var tools []optimus-ide-collabsdk.DynamicTool
...
func (p *Server) updateLastTurnSummary(
	...
	p.publishChatPubsubEvent(updatedChat, optimus-ide-collabsdk.ChatWatchEventKindSummaryChange, nil)
}
...
func (p *Server) updateChatSummary(
	...
	p.publishChatPubsubEvent(updatedChat, optimus-ide-collabsdk.ChatWatchEventKindChatSummaryChange, nil)
}
...
func (p *Server) dispatchPush(
	...
	pushMsg := optimus-ide-collabsdk.WebpushMessage{
		Title: chat.Title,
		Body:  body,
		Icon:  "/favicon.ico",
