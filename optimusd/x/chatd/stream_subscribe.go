...
func (p *Server) subscribeStreamLoop(
...
) ([]optimus-ide-collabsdk.ChatStreamEvent, <-chan optimus-ide-collabsdk.ChatStreamEvent, func(), bool) {
	...
	events := make(chan optimus-ide-collabsdk.ChatStreamEvent, 128)
	logger := p.logger.With(slog.F("chat_id", chatID))

	updateCh := make(chan streamSyncHint, 32)
	pubsubCancel, err := p.pubsub.SubscribeWithErr(
		optimus-ide-collabdpubsub.ChatStateUpdateChannel(chatID),
		optimus-ide-collabdpubsub.HandleChatStateUpdate(func(_ context.Context, payload optimus-ide-collabdpubsub.ChatStateUpdateMessage, err error) {
			if err != nil {
...
func (p *Server) syncStreamWithRetry(
	...
	loop *streamLoop,
	hint streamSyncHint,
) ([]optimus-ide-collabsdk.ChatStreamEvent, streamRelayTarget, bool, error) {
	var (
		syncEvents []optimus-ide-collabsdk.ChatStreamEvent
...
func sendStreamEvent(ctx context.Context, ch chan<- optimus-ide-collabsdk.ChatStreamEvent, event optimus-ide-collabsdk.ChatStreamEvent) bool {
	select {
	case ch <- event:
		return true
	case <-ctx.Done():
