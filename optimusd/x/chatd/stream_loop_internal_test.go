...
func TestStreamLoopMessageSyncAfterIDAndEdits(t *testing.T) {
	...
	requireEventTypes(t, events,
		optimus-ide-collabsdk.ChatStreamEventTypeMessage,
		optimus-ide-collabsdk.ChatStreamEventTypeStatus,
		optimus-ide-collabsdk.ChatStreamEventTypePreviewReset,
	)
...
func TestStreamLoopHistoryReset(t *testing.T) {
	...
	requireEventTypes(t, events,
		optimus-ide-collabsdk.ChatStreamEventTypeHistoryReset,
		optimus-ide-collabsdk.ChatStreamEventTypeMessage,
		optimus-ide-collabsdk.ChatStreamEventTypePreviewReset,
	)
...
func TestStreamLoopQueueStatusRetryErrorActionRequiredAndPreviewReset(t *testing.T) {
	...
	chatID := uuid.New()
	retry := optimus-ide-collabsdk.ChatStreamRetry{Attempt: 2, DelayMs: 100, Error: "retrying", RetryingAt: time.Now()}
	retryRaw, err := json.Marshal(retry)
	require.NoError(t, err)
	chatError := optimus-ide-collabsdk.ChatError{Message: "provider failed", Kind: optimus-ide-collabsdk.ChatErrorKindConfig}
	...
	requireEventTypes(t, events,
		optimus-ide-collabsdk.ChatStreamEventTypeQueueUpdate,
		optimus-ide-collabsdk.ChatStreamEventTypeStatus,
		optimus-ide-collabsdk.ChatStreamEventTypeError,
		optimus-ide-collabsdk.ChatStreamEventTypeRetry,
		optimus-ide-collabsdk.ChatStreamEventTypePreviewReset,
	)
	...
	actionEvents := actionLoop.applyDBSnapshot(streamDBSnapshot{
		...
		actionRequired: &optimus-ide-collabsdk.ChatStreamActionRequired{ToolCalls: []optimus-ide-collabsdk.ChatStreamToolCall{{ToolCallID: "call-1", ToolName: "browser"}}},
	})
	requireEventTypes(t, actionEvents,
		optimus-ide-collabsdk.ChatStreamEventTypeStatus,
		optimus-ide-collabsdk.ChatStreamEventTypeActionRequired,
		optimus-ide-collabsdk.ChatStreamEventTypePreviewReset,
	)
...
func TestStreamLoopActionRequiredFromHistory(t *testing.T) {
	...
	chatID := uuid.New()
	toolDefs, err := json.Marshal([]optimus-ide-collabsdk.DynamicTool{{Name: "browser"}})
	require.NoError(t, err)
	assistant := streamMessageParts(t, chatID, 1, 1, database.ChatMessageRoleAssistant, []optimus-ide-collabsdk.ChatMessagePart{{
...
func TestStreamLoopPartValidation(t *testing.T) {
	...
	event, accepted, err := loop.part(StreamPart{HistoryVersion: 7, GenerationAttempt: 3, Seq: 1, Role: optimus-ide-collabsdk.ChatMessageRoleAssistant, Part: optimus-ide-collabsdk.ChatMessageText("a")})
	require.NoError(t, err)
	require.True(t, accepted)
	require.Equal(t, optimus-ide-collabsdk.ChatStreamEventTypeMessagePart, event.Type)
	...
	_, accepted, err = loop.part(StreamPart{HistoryVersion: 6, GenerationAttempt: 3, Seq: 2, Part: optimus-ide-collabsdk.ChatMessageText("old history")})
	require.NoError(t, err)
	require.False(t, accepted)
	_, accepted, err = loop.part(StreamPart{HistoryVersion: 7, GenerationAttempt: 2, Seq: 2, Part: optimus-ide-collabsdk.ChatMessageText("old attempt")})
	require.NoError(t, err)
	require.False(t, accepted)
	_, accepted, err = loop.part(StreamPart{HistoryVersion: 7, GenerationAttempt: 3, Seq: 1, Part: optimus-ide-collabsdk.ChatMessageText("dup")})
	require.NoError(t, err)
	require.False(t, accepted)
	_, accepted, err = loop.part(StreamPart{HistoryVersion: 7, GenerationAttempt: 3, Seq: 3, Part: optimus-ide-collabsdk.ChatMessageText("gap")})
	require.Error(t, err)
	require.False(t, accepted)
}
...
func TestStreamLoopInitialSyncRecoversWithoutHint(t *testing.T) {
	...
	requireEventTypes(t, events, optimus-ide-collabsdk.ChatStreamEventTypeStatus)
	require.Equal(t, optimus-ide-collabsdk.ChatStatusWaiting, events[0].Status.Status)
}

func requireEventTypes(t *testing.T, events []optimus-ide-collabsdk.ChatStreamEvent, types ...optimus-ide-collabsdk.ChatStreamEventType) {
	t.Helper()
...
func streamMessage(t *testing.T, chatID uuid.UUID, id int64, revision int64, role database.ChatMessageRole, text string, deleted bool) database.ChatMessage {
	t.Helper()
	return streamMessageParts(t, chatID, id, revision, role, []optimus-ide-collabsdk.ChatMessagePart{optimus-ide-collabsdk.ChatMessageText(text)}, deleted)
}

func streamMessageParts(t *testing.T, chatID uuid.UUID, id int64, revision int64, role database.ChatMessageRole, parts []optimus-ide-collabsdk.ChatMessagePart, deleted bool) database.ChatMessage {
	t.Helper()
