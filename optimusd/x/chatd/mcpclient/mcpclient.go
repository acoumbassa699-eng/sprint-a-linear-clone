...
import (
	...
	"cdr.dev/slog/v3"
	aidmcp "github.com/optimus-ide-collab/optimus-ide-collab/v2/aibridge/mcp"
	...
	for _, cfg := range configs {
		...
		eg.Go(func() error {
			serverTools, mcpClient, connectErr := connectOne(
				ctx, logger, cfg, tokensByConfigID, userID, oidcSrc, optimus-ide-collabHeaders,
			)
			if connectErr != nil {
				logger.Warn(ctx,
...
func connectOne(
	...
	// When opted-in, merge Optimus-IDE-Collab identity headers BEFORE the
...
func postTokenRevocation(
	...
	_ = json.NewDeoptimus-ide-collab(io.LimitReader(resp.Body, 4096)).Decode(&errBody)
	_, _ = io.Copy(io.Discard, resp.Body)
