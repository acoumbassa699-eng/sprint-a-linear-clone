...
func buildAssistantPartsForPersist(
	...
	for _, block := range assistantBlocks {
		...
		if part.Type == optimus-ide-collabsdk.ChatMessagePartTypeToolCall && part.ToolCallID != "" && step.ToolCallCreatedAt != nil {
			if ts, ok := step.ToolCallCreatedAt[part.ToolCallID]; ok {
				part.CreatedAt = &ts
			}
		}
		if part.Type == optimus-ide-collabsdk.ChatMessagePartTypeToolResult && part.ToolCallID != "" && step.ToolResultCreatedAt != nil {
	...
	for _, tr := range toolResults {
		...
		for _, attachment := range attachments {
			parts = append(parts, optimus-ide-collabsdk.ChatMessageFile(
				attachment.FileID,
