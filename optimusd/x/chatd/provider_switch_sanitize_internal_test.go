...
func TestStripForeignProviderExecutedToolRows(t *testing.T) {
	...
	peCall := func(id string) optimus-ide-collabsdk.ChatMessagePart {
		p := optimus-ide-collabsdk.ChatMessageToolCall(id, "web_search", json.RawMessage(`{"query":"x"}`))
		p.ProviderExecuted = true
		return p
	}
	peResult := func(id string) optimus-ide-collabsdk.ChatMessagePart {
		p := optimus-ide-collabsdk.ChatMessageToolResult(id, "web_search", json.RawMessage(`{"ok":true}`), false, false)
		p.ProviderExecuted = true
		return p
	}
	localCall := func(id string) optimus-ide-collabsdk.ChatMessagePart {
		return optimus-ide-collabsdk.ChatMessageToolCall(id, "read_file", json.RawMessage(`{}`))
	}
	text := optimus-ide-collabsdk.ChatMessageText

	assistantRow := func(t *testing.T, cfg uuid.UUID, parts ...optimus-ide-collabsdk.ChatMessagePart) database.ChatMessage {
		t.Helper()
	...
	userRow := func(t *testing.T, s string) database.ChatMessage {
		t.Helper()
		content, err := chatprompt.MarshalParts([]optimus-ide-collabsdk.ChatMessagePart{text(s)})
		require.NoError(t, err)
		return database.ChatMessage{
	...
	partsOf := func(t *testing.T, row database.ChatMessage) []optimus-ide-collabsdk.ChatMessagePart {
		t.Helper()
	...
	t.Run("anthropic to bedrock drops provider blocks", func(t *testing.T) {
		...
		require.Len(t, got, 2)
		require.Equal(t, []optimus-ide-collabsdk.ChatMessagePart{text("done")}, partsOf(t, got[1]))
	...
	t.Run("multi-provider keeps native strips foreign", func(t *testing.T) {
		...
		got, stats := stripForeignProviderExecutedToolRows(rows, anthropic, resolver)
		require.Len(t, got, 2)
		require.Equal(t, []optimus-ide-collabsdk.ChatMessagePart{text("openai")}, partsOf(t, got[0]))
	...
	t.Run("unknown origin fails closed", func(t *testing.T) {
		...
		got, stats := stripForeignProviderExecutedToolRows(rows, bedrock, resolver)
		require.Len(t, got, 1)
		require.Equal(t, []optimus-ide-collabsdk.ChatMessagePart{text("done")}, partsOf(t, got[0]))
