...
func synthesizePendingToolCancellations(
	...
	for _, part := range assistantParts {
		...
		resultPart := optimus-ide-collabsdk.ChatMessagePart{
		...
		raw, err := chatprompt.MarshalParts([]optimus-ide-collabsdk.ChatMessagePart{resultPart})
		if err != nil {
...
func outstandingToolCallIDs(ctx context.Context, store database.Store, chat database.Chat, accept func(toolName string) bool) (map[string]string, error) {
	...
	for _, p := range parts {
		if p.Type != optimus-ide-collabsdk.ChatMessagePartTypeToolCall {
			continue
		}
