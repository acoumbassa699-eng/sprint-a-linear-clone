...
func TestCreateChatPublishesAfterCommit(t *testing.T) {
	...
	channel := optimus-ide-collabdpubsub.ChatStateUpdateChannel(res.Chat.ID)
...
func TestUpdatePublishesAfterCommit(t *testing.T) {
	...
	createIdx := publishedOn(f, optimus-ide-collabdpubsub.ChatStateUpdateChannel(created.Chat.ID))
	...
	updIdx := publishedOn(f, optimus-ide-collabdpubsub.ChatStateUpdateChannel(created.Chat.ID))
...
func TestUpdatePublishesOneFinalChatUpdateForTransitionBundle(t *testing.T) {
	...
	baseUpdates := len(publishedOn(f, optimus-ide-collabdpubsub.ChatStateUpdateChannel(created.Chat.ID)))
...
func TestChatUpdateMessagePayloadShape(t *testing.T) {
	...
	created := createTestChat(t, f)
	m := chatstate.NewChatMachine(f.DB, f.Pub, created.Chat.ID)
	channel := optimus-ide-collabdpubsub.ChatStateUpdateChannel(created.Chat.ID)
	...
	var typed optimus-ide-collabdpubsub.ChatStateUpdateMessage
...
func TestChatOwnershipMessagePayloadShape(t *testing.T) {
	...
	idx := publishedOn(f, optimus-ide-collabdpubsub.ChatStateOwnershipChannel)
	require.NotEmpty(t, idx, "CreateChat publishes at least one chat:ownership hint")

	payload := f.Pub.payloads[idx[len(idx)-1]]
	var typed optimus-ide-collabdpubsub.ChatStateOwnershipMessage
