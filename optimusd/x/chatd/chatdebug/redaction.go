...
func RedactJSONSecrets(data []byte) []byte {
	...
	deoptimus-ide-collab := json.NewDeoptimus-ide-collab(bytes.NewReader(data))
	deoptimus-ide-collab.UseNumber()

	var value any
	if err := deoptimus-ide-collab.Decode(&value); err != nil {
	...
	if err := consumeJSONEOF(deoptimus-ide-collab); err != nil {
...
func consumeJSONEOF(deoptimus-ide-collab *json.Deoptimus-ide-collab) error {
	var extra any
	err := deoptimus-ide-collab.Decode(&extra)
