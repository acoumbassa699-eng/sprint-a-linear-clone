...
import (
	...
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabd/x/chatd/messagepartbuffer"
...
func TestBuildCommitStepMessages_AssistantTextAndReasoning(t *testing.T) {
	...
	require.Equal(t, optimus-ide-collabsdk.ChatMessagePartTypeReasoning, parts[0].Type)
	...
	require.Equal(t, optimus-ide-collabsdk.ChatMessagePartTypeText, parts[1].Type)
...
func TestBuildCommitStepMessages_LocalToolResultsBecomeToolMessages(t *testing.T) {
	...
	require.Equal(t, optimus-ide-collabsdk.ChatMessagePartTypeToolCall, assistantParts[0].Type)
	...
	require.Equal(t, optimus-ide-collabsdk.ChatMessagePartTypeToolResult, toolParts[0].Type)
...
func TestBuildCommitStepMessages_ProviderExecutedResultsStayAssistantContent(t *testing.T) {
	...
	require.Len(t, parts, 2)
	require.Equal(t, optimus-ide-collabsdk.ChatMessagePartTypeToolCall, parts[0].Type)
	require.True(t, parts[0].ProviderExecuted)
	require.Equal(t, optimus-ide-collabsdk.ChatMessagePartTypeToolResult, parts[1].Type)
...
func TestBuildCompactionMessages_CompressedSummaryToolCallAndResult(t *testing.T) {
	...
	require.Equal(t, optimus-ide-collabsdk.ChatMessagePartTypeToolCall, callPart.Type)
	...
	require.Equal(t, optimus-ide-collabsdk.ChatMessagePartTypeToolResult, resultPart.Type)
...
func TestCurrentTurnStepCount_ExcludesCompressedCompactionMessages(t *testing.T) {
	...
	messages := []database.ChatMessage{
		dbMessage(t, 1, database.ChatMessageRoleUser, false, optimus-ide-collabsdk.ChatMessageText("start")),
		dbMessage(t, 2, database.ChatMessageRoleAssistant, false, optimus-ide-collabsdk.ChatMessageText("first")),
		dbMessage(t, 3, database.ChatMessageRoleUser, true, optimus-ide-collabsdk.ChatMessageText("compressed summary")),
		dbMessage(t, 4, database.ChatMessageRoleAssistant, true, optimus-ide-collabsdk.ChatMessageToolCall("summary", "chat_summarized", nil)),
		dbMessage(t, 5, database.ChatMessageRoleTool, true, optimus-ide-collabsdk.ChatMessageToolResult("summary", "chat_summarized", json.RawMessage(`{}`), false, false)),
		dbMessage(t, 6, database.ChatMessageRoleAssistant, false, optimus-ide-collabsdk.ChatMessageText("second")),
	}
...
func TestCurrentTurnStepCount_CountsAssistantMessagesAfterLatestUser(t *testing.T) {
	...
	messages := []database.ChatMessage{
		dbMessage(t, 1, database.ChatMessageRoleUser, false, optimus-ide-collabsdk.ChatMessageText("old")),
		dbMessage(t, 2, database.ChatMessageRoleAssistant, false, optimus-ide-collabsdk.ChatMessageText("old answer")),
		dbMessage(t, 3, database.ChatMessageRoleUser, false, optimus-ide-collabsdk.ChatMessageText("new")),
		dbMessage(t, 4, database.ChatMessageRoleAssistant, false, optimus-ide-collabsdk.ChatMessageText("one")),
		dbMessage(t, 5, database.ChatMessageRoleTool, false, optimus-ide-collabsdk.ChatMessageToolResult("call", "tool", json.RawMessage(`{}`), false, false)),
		dbMessage(t, 6, database.ChatMessageRoleAssistant, false, optimus-ide-collabsdk.ChatMessageText("two")),
	}
	got := currentTurnStepCount(messages)
	require.Equal(t, 2, got)
}
...
func TestDecisionCompactsAgainAfterPostCompactionTurn(t *testing.T) {
	...
	messages := []database.ChatMessage{
		dbMessage(t, 1, database.ChatMessageRoleUser, false, optimus-ide-collabsdk.ChatMessageText("initial request")),
		dbMessage(t, 2, database.ChatMessageRoleUser, true, optimus-ide-collabsdk.ChatMessageText("compacted summary")),
		dbMessage(t, 3, database.ChatMessageRoleAssistant, true, optimus-ide-collabsdk.ChatMessageToolCall("summary-1", "chat_summarized", nil)),
		dbMessage(t, 4, database.ChatMessageRoleTool, true, optimus-ide-collabsdk.ChatMessageToolResult("summary-1", "chat_summarized", json.RawMessage(`{}`), false, false)),
		dbMessage(t, 5, database.ChatMessageRoleAssistant, false, optimus-ide-collabsdk.ChatMessageText("continued after compaction")),
		dbMessage(t, 6, database.ChatMessageRoleUser, false, optimus-ide-collabsdk.ChatMessageText("next request")),
	}
...
func TestDecisionForcedCompaction(t *testing.T) {
	...
	t.Run("beats history complete", func(t *testing.T) {
		...
		messages := []database.ChatMessage{
			dbMessage(t, 1, database.ChatMessageRoleUser, false, optimus-ide-collabsdk.ChatMessageText("question")),
			dbMessage(t, 2, database.ChatMessageRoleAssistant, false, optimus-ide-collabsdk.ChatMessageText("answer")),
		}
		decision, err := decideGenerationAction(generationDecisionInput{
			chat:     requestedChat,
			messages: messages,
	...
	t.Run("loses to unresolved tool calls", func(t *testing.T) {
		...
		messages := []database.ChatMessage{
			dbMessage(t, 1, database.ChatMessageRoleUser, false, optimus-ide-collabsdk.ChatMessageText("question")),
			dbMessage(t, 2, database.ChatMessageRoleAssistant, false, optimus-ide-collabsdk.ChatMessageToolCall("read-1", "read_file", json.RawMessage(`{}`))),
		}
		decision, err := decideGenerationAction(generationDecisionInput{
			chat:     requestedChat,
			messages: messages,
	...
	t.Run("skipped when nothing compactable", func(t *testing.T) {
		...
		messages := []database.ChatMessage{
			dbMessage(t, 1, database.ChatMessageRoleUser, true, optimus-ide-collabsdk.ChatMessageText("summary")),
			dbMessage(t, 2, database.ChatMessageRoleAssistant, true, optimus-ide-collabsdk.ChatMessageToolCall("summary-1", "chat_summarized", nil)),
			dbMessage(t, 3, database.ChatMessageRoleTool, true, optimus-ide-collabsdk.ChatMessageToolResult("summary-1", "chat_summarized", json.RawMessage(`{}`), false, false)),
			dbMessage(t, 4, database.ChatMessageRoleUser, false, optimus-ide-collabsdk.ChatMessageText("follow-up question")),
			dbMessage(t, 5, database.ChatMessageRoleAssistant, false, optimus-ide-collabsdk.ChatMessageText("follow-up answer")),
		}
...
func TestCompactionStatusFromHistory(t *testing.T) {
	...
	t.Run("needed after under limit post compaction assistant", func(t *testing.T) {
		...
		messages := []database.ChatMessage{
			...
			dbMessage(t, 2, database.ChatMessageRoleAssistant, true, optimus-ide-collabsdk.ChatMessageToolCall("summary-1", "chat_summarized", nil)),
			dbMessage(t, 3, database.ChatMessageRoleTool, true, optimus-ide-collabsdk.ChatMessageToolResult("summary-1", "chat_summarized", json.RawMessage(`{}`), false, false)),
			withUsage(dbMessage(t, 4, database.ChatMessageRoleAssistant, false, optimus-ide-collabsdk.ChatMessageText("continued")), 20, 100),
			dbMessage(t, 5, database.ChatMessageRoleUser, false, optimus-ide-collabsdk.ChatMessageText("next")),
		}
	...
	t.Run("still over limit from first post compaction assistant usage", func(t *testing.T) {
		...
		messages := []database.ChatMessage{
			...
			dbMessage(t, 2, database.ChatMessageRoleAssistant, true, optimus-ide-collabsdk.ChatMessageToolCall("summary-1", "chat_summarized", nil)),
			dbMessage(t, 3, database.ChatMessageRoleTool, true, optimus-ide-collabsdk.ChatMessageToolResult("summary-1", "chat_summarized", json.RawMessage(`{}`), false, false)),
			withUsage(dbMessage(t, 4, database.ChatMessageRoleAssistant, false, optimus-ide-collabsdk.ChatMessageToolCall("read-1", "read_file", json.RawMessage(`{}`))), 80, 100),
		}
	...
	t.Run("still over limit includes prompt cache tokens", func(t *testing.T) {
		...
		messages := []database.ChatMessage{
			...
			dbMessage(t, 2, database.ChatMessageRoleAssistant, true, optimus-ide-collabsdk.ChatMessageToolCall("summary-1", "chat_summarized", nil)),
			dbMessage(t, 3, database.ChatMessageRoleTool, true, optimus-ide-collabsdk.ChatMessageToolResult("summary-1", "chat_summarized", json.RawMessage(`{}`), false, false)),
			withUsageTokens(dbMessage(t, 4, database.ChatMessageRoleAssistant, false, optimus-ide-collabsdk.ChatMessageToolCall("read-1", "read_file", json.RawMessage(`{}`))), fantasy.Usage{CacheReadTokens: 80}, 100),
		}
	...
	t.Run("still over limit uses configured context limit", func(t *testing.T) {
		...
		messages := []database.ChatMessage{
			...
			dbMessage(t, 2, database.ChatMessageRoleAssistant, true, optimus-ide-collabsdk.ChatMessageToolCall("summary-1", "chat_summarized", nil)),
			dbMessage(t, 3, database.ChatMessageRoleTool, true, optimus-ide-collabsdk.ChatMessageToolResult("summary-1", "chat_summarized", json.RawMessage(`{}`), false, false)),
			withUsage(dbMessage(t, 4, database.ChatMessageRoleAssistant, false, optimus-ide-collabsdk.ChatMessageToolCall("read-1", "read_file", json.RawMessage(`{}`))), 80, 200),
		}
	...
	t.Run("still over limit includes exact threshold boundary", func(t *testing.T) {
		...
		messages := []database.ChatMessage{
			...
			dbMessage(t, 2, database.ChatMessageRoleAssistant, true, optimus-ide-collabsdk.ChatMessageToolCall("summary-1", "chat_summarized", nil)),
			dbMessage(t, 3, database.ChatMessageRoleTool, true, optimus-ide-collabsdk.ChatMessageToolResult("summary-1", "chat_summarized", json.RawMessage(`{}`), false, false)),
			withUsage(dbMessage(t, 4, database.ChatMessageRoleAssistant, false, optimus-ide-collabsdk.ChatMessageToolCall("read-1", "read_file", json.RawMessage(`{}`))), 70, 100),
		}
...
func TestDecisionDetectsStopAfterToolFromCommittedHistory(t *testing.T) {
	...
	messages := []database.ChatMessage{
		dbMessage(t, 1, database.ChatMessageRoleUser, false, optimus-ide-collabsdk.ChatMessageText("plan")),
		dbMessage(t, 2, database.ChatMessageRoleAssistant, false, optimus-ide-collabsdk.ChatMessageToolCall("plan-1", "propose_plan", json.RawMessage(`{}`))),
		dbMessage(t, 3, database.ChatMessageRoleTool, false, optimus-ide-collabsdk.ChatMessageToolResult("plan-1", "propose_plan", json.RawMessage(`{"ok":true}`), false, false)),
	}
	got, err := historyHasStopAfterToolResult(messages, map[string]struct{}{"propose_plan": {}})
	require.NoError(t, err)
	require.True(t, got)

	messages[2] = dbMessage(t, 3, database.ChatMessageRoleTool, false, optimus-ide-collabsdk.ChatMessageToolResult("plan-1", "propose_plan", json.RawMessage(`{"error":"no"}`), true, false))
	got, err = historyHasStopAfterToolResult(messages, map[string]struct{}{"propose_plan": {}})
	require.NoError(t, err)
	require.False(t, got)
}
...
func TestDecisionDetectsCurrentHistoryCompletion(t *testing.T) {
	...
	complete, err := currentHistoryComplete([]database.ChatMessage{
		dbMessage(t, 1, database.ChatMessageRoleUser, false, optimus-ide-collabsdk.ChatMessageText("hello")),
		dbMessage(t, 2, database.ChatMessageRoleAssistant, false, optimus-ide-collabsdk.ChatMessageText("done")),
	})
	require.NoError(t, err)
	...
	complete, err = currentHistoryComplete([]database.ChatMessage{
		dbMessage(t, 1, database.ChatMessageRoleUser, false, optimus-ide-collabsdk.ChatMessageText("hello")),
		dbMessage(t, 2, database.ChatMessageRoleAssistant, false, optimus-ide-collabsdk.ChatMessageToolCall("call-1", "execute", json.RawMessage(`{}`))),
	})
	require.NoError(t, err)
	...
	complete, err = currentHistoryComplete([]database.ChatMessage{
		dbMessage(t, 1, database.ChatMessageRoleUser, false, optimus-ide-collabsdk.ChatMessageText("hello")),
		dbMessage(t, 2, database.ChatMessageRoleAssistant, false, optimus-ide-collabsdk.ChatMessageToolCall("call-1", "execute", json.RawMessage(`{}`))),
		dbMessage(t, 3, database.ChatMessageRoleTool, false, optimus-ide-collabsdk.ChatMessageToolResult("call-1", "execute", json.RawMessage(`{"ok":true}`), false, false)),
	})
	require.NoError(t, err)
	require.False(t, complete)
}
...
func TestBufferedPartsToPartialMessages_NormalizesToolCallDeltasBeforeFinal(t *testing.T) {
	...
	parts := []messagepartbuffer.Part{
		{Seq: 1, Role: optimus-ide-collabsdk.ChatMessageRoleAssistant, MessagePart: optimus-ide-collabsdk.ChatMessageText("partial ")},
		{Seq: 2, Role: optimus-ide-collabsdk.ChatMessageRoleAssistant, MessagePart: optimus-ide-collabsdk.ChatMessagePart{Type: optimus-ide-collabsdk.ChatMessagePartTypeToolCall, ToolCallID: "call-1", ToolName: "execute", ArgsDelta: `{"cmd":`}},
		{Seq: 3, Role: optimus-ide-collabsdk.ChatMessageRoleAssistant, MessagePart: optimus-ide-collabsdk.ChatMessagePart{Type: optimus-ide-collabsdk.ChatMessagePartTypeToolCall, ToolCallID: "call-1", ToolName: "execute", ArgsDelta: `"ignored"}`}},
		{Seq: 4, Role: optimus-ide-collabsdk.ChatMessageRoleAssistant, MessagePart: optimus-ide-collabsdk.ChatMessageToolCall("call-1", "execute", json.RawMessage(`{"cmd":"pwd"}`))},
	}
	...
	require.Equal(t, optimus-ide-collabsdk.ChatMessagePartTypeText, assistantParts[0].Type)
	call := assistantParts[1]
	require.Equal(t, optimus-ide-collabsdk.ChatMessagePartTypeToolCall, call.Type)
...
func TestBufferedPartsToPartialMessages_MergesToolCallDeltasWithoutFinal(t *testing.T) {
	...
	parts := []messagepartbuffer.Part{
		{Seq: 1, Role: optimus-ide-collabsdk.ChatMessageRoleAssistant, MessagePart: optimus-ide-collabsdk.ChatMessagePart{Type: optimus-ide-collabsdk.ChatMessagePartTypeToolCall, ToolCallID: "call-1", ToolName: "execute", ArgsDelta: `{"cmd":`}},
		{Seq: 2, Role: optimus-ide-collabsdk.ChatMessageRoleAssistant, MessagePart: optimus-ide-collabsdk.ChatMessagePart{Type: optimus-ide-collabsdk.ChatMessagePartTypeToolCall, ToolCallID: "call-1", ToolName: "execute", ArgsDelta: `"pwd"}`}},
	}
	got, err := bufferedPartsToPartialMessages(bufferedPartsToPartialMessagesInput{
		parts:          parts,
		modelConfigID:  uuid.New(),
...
func TestBufferedPartsToPartialMessages_DeltaOnlyToolResultDoesNotAnswer(t *testing.T) {
	...
	parts := []messagepartbuffer.Part{
		{Seq: 1, Role: optimus-ide-collabsdk.ChatMessageRoleAssistant, MessagePart: optimus-ide-collabsdk.ChatMessageToolCall("call-1", "advisor", json.RawMessage(`{}`))},
		{Seq: 2, Role: optimus-ide-collabsdk.ChatMessageRoleTool, MessagePart: optimus-ide-collabsdk.ChatMessagePart{Type: optimus-ide-collabsdk.ChatMessagePartTypeToolResult, ToolCallID: "call-1", ToolName: "advisor", ResultDelta: `{"type":"advice"}`}},
	}
	got, err := bufferedPartsToPartialMessages(bufferedPartsToPartialMessagesInput{
		parts:          parts,
		modelConfigID:  uuid.New(),
...
func TestBufferedPartsToPartialMessages_LogsMalformedSkippedParts(t *testing.T) {
	...
	logger := slogtest.Make(t, &slogtest.Options{IgnoreErrors: true}).AppendSinks(logSink)
	parts := []messagepartbuffer.Part{
		{Seq: 1, Role: optimus-ide-collabsdk.ChatMessageRoleSystem, MessagePart: optimus-ide-collabsdk.ChatMessageText("bad role")},
		{Seq: 2, Role: optimus-ide-collabsdk.ChatMessageRoleAssistant, MessagePart: optimus-ide-collabsdk.ChatMessagePart{}},
		{Seq: 3, Role: optimus-ide-collabsdk.ChatMessageRoleTool, MessagePart: optimus-ide-collabsdk.ChatMessagePart{Type: optimus-ide-collabsdk.ChatMessagePartTypeToolResult, ToolName: "execute", Result: json.RawMessage(`{"ok":true}`)}},
		{Seq: 4, Role: optimus-ide-collabsdk.ChatMessageRoleAssistant, MessagePart: optimus-ide-collabsdk.ChatMessagePart{Type: optimus-ide-collabsdk.ChatMessagePartTypeToolCall, ToolCallID: "bad-args", ToolName: "execute", ArgsDelta: `{"cmd":`}},
	}
	got, err := bufferedPartsToPartialMessages(bufferedPartsToPartialMessagesInput{
		parts:          parts,
		modelConfigID:  uuid.New(),
...
func TestBufferedPartsToPartialMessages_SynthesizesMissingToolResults(t *testing.T) {
	...
	reasoningPart := optimus-ide-collabsdk.ChatMessageReasoning("partial thought")
	reasoningPart.CreatedAt = &reasoningStartedAt
	parts := []messagepartbuffer.Part{
		{Seq: 1, Role: optimus-ide-collabsdk.ChatMessageRoleAssistant, MessagePart: optimus-ide-collabsdk.ChatMessageText("partial ")},
		{Seq: 2, Role: optimus-ide-collabsdk.ChatMessageRoleAssistant, MessagePart: reasoningPart},
		{Seq: 3, Role: optimus-ide-collabsdk.ChatMessageRoleAssistant, MessagePart: optimus-ide-collabsdk.ChatMessageToolCall("call-1", "execute", json.RawMessage(`{}`))},
		{Seq: 4, Role: optimus-ide-collabsdk.ChatMessageRoleAssistant, MessagePart: optimus-ide-collabsdk.ChatMessageToolCall("call-2", "read_file", json.RawMessage(`{}`))},
		{Seq: 5, Role: optimus-ide-collabsdk.ChatMessageRoleTool, MessagePart: withCreatedAt(optimus-ide-collabsdk.ChatMessageToolResult("call-2", "read_file", json.RawMessage(`{"ok":true}`), false, false), createdAt)},
	}
	got, err := bufferedPartsToPartialMessages(bufferedPartsToPartialMessagesInput{
		parts:          parts,
		modelConfigID:  modelConfigID,
	...
	require.Equal(t, optimus-ide-collabsdk.ChatMessagePartTypeReasoning, assistantParts[1].Type)
	...
	require.Equal(t, optimus-ide-collabsdk.ChatMessagePartTypeToolCall, assistantParts[2].Type)
	require.Equal(t, optimus-ide-collabsdk.ChatMessagePartTypeToolCall, assistantParts[3].Type)
...
func parseMessageParts(t *testing.T, role database.ChatMessageRole, raw pqtype.NullRawMessage) []optimus-ide-collabsdk.ChatMessagePart {
	t.Helper()
...
func dbMessage(t *testing.T, id int64, role database.ChatMessageRole, compressed bool, parts ...optimus-ide-collabsdk.ChatMessagePart) database.ChatMessage {
	t.Helper()
...
func withCreatedAt(part optimus-ide-collabsdk.ChatMessagePart, createdAt time.Time) optimus-ide-collabsdk.ChatMessagePart {
