...
func TestOpenAIResponsesNoStaleWebSearchReplay(t *testing.T) {
	...
	openAIURL := chattest.NewOpenAI(t, func(req *chattest.OpenAIRequest) chattest.OpenAIResponse {
		...
		case 1:
			...
			resp.WebSearch = &chattest.OpenAIWebSearchCall{
				ID:    webSearchID,
				Query: "optimus-ide-collab changelog",
			}
			return resp
		default:
			resp := chattest.OpenAIStreamingResponse(
	...
	chat, err := server.CreateChat(ctx, chatd.CreateOptions{
		...
		InitialUserContent: []optimus-ide-collabsdk.ChatMessagePart{
			optimus-ide-collabsdk.ChatMessageText("search for the latest Optimus-IDE-Collab docs"),
		},
	})
	...
	_, err = server.SendMessage(ctx, chatd.SendMessageOptions{
		...
		Content: []optimus-ide-collabsdk.ChatMessagePart{
			optimus-ide-collabsdk.ChatMessageText("summarize the result without searching again"),
		},
	})
	require.NoError(t, err)
	waitForChatProcessed(ctx, t, db, chat.ID, server)
