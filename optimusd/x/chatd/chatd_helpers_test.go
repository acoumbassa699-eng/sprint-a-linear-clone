...
func insertOrphanProviderToolCall(ctx context.Context, t *testing.T, db database.Store, chatID uuid.UUID, modelID uuid.UUID) {
	...
	parts := []optimus-ide-collabsdk.ChatMessagePart{
		...
		optimus-ide-collabsdk.ChatMessageText("partial"),
	}
...
func createChatThroughServer(
	...
	chat, err := server.CreateChat(ctx, chatd.CreateOptions{
		...
		OwnerID:            userID,
		Title:              "test chat",
		InitialUserContent: []optimus-ide-collabsdk.ChatMessagePart{optimus-ide-collabsdk.ChatMessageText(text)},
...
func requireTextPart(t *testing.T, msg database.ChatMessage, text string) {
	...
	for _, part := range parts {
		if part.Type == optimus-ide-collabsdk.ChatMessagePartTypeText && part.Text == text {
			return
		}
	}
