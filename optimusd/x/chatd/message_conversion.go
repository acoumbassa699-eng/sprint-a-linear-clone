...
func buildCommitStepMessages(input buildCommitStepMessagesInput) (stepMessagesForCommit, error) {
	...
	for _, toolResult := range toolResults {
		...
		content, err := chatprompt.MarshalParts([]optimus-ide-collabsdk.ChatMessagePart{part})
		if err != nil {
...
func buildAssistantParts(
	...
	parts := make([]optimus-ide-collabsdk.ChatMessagePart, 0, len(assistantBlocks)+len(toolResults))
	reasoningIdx := 0
	...
	for _, tr := range toolResults {
		...
		for _, attachment := range attachments {
			parts = append(parts, optimus-ide-collabsdk.ChatMessageFile(attachment.FileID, attachment.MediaType, attachment.Name))
		}
	}
	return parts
}

func applyToolMetadata(part *optimus-ide-collabsdk.ChatMessagePart, toolNameToConfigID map[string]uuid.UUID) {
...
func textFromParts(parts []optimus-ide-collabsdk.ChatMessagePart) string {
...
func buildCompactionMessages(input buildCompactionMessagesInput) (compactionMessagesForCommit, error) {
	...
	systemContent, err := chatprompt.MarshalParts([]optimus-ide-collabsdk.ChatMessagePart{optimus-ide-collabsdk.ChatMessageText(input.compaction.SystemSummary)})
	if err != nil {
	...
	assistantContent, err := chatprompt.MarshalParts([]optimus-ide-collabsdk.ChatMessagePart{
		optimus-ide-collabsdk.ChatMessageToolCall(input.toolCallID, toolName, args),
	})
	if err != nil {
	...
	toolContent, err := chatprompt.MarshalParts([]optimus-ide-collabsdk.ChatMessagePart{
		optimus-ide-collabsdk.ChatMessageToolResult(input.toolCallID, toolName, summaryResult, false, false),
	})
	if err != nil {
...
func isCompactionBoundaryMessage(msg database.ChatMessage) bool {
	...
	for _, part := range parts {
		if part.ToolName == "chat_summarized" &&
			(part.Type == optimus-ide-collabsdk.ChatMessagePartTypeToolCall || part.Type == optimus-ide-collabsdk.ChatMessagePartTypeToolResult) {
			return true
		}
	}
	return false
...
func historyHasStopAfterToolResult(messages []database.ChatMessage, stopAfterTools map[string]struct{}) (bool, error) {
	...
	for _, msg := range messages[start:] {
		...
		for _, part := range parts {
			if part.Type != optimus-ide-collabsdk.ChatMessagePartTypeToolResult || part.IsError {
				continue
			}
...
func currentHistoryComplete(messages []database.ChatMessage) (bool, error) {
	...
	for _, part := range parts {
		if part.Type == optimus-ide-collabsdk.ChatMessagePartTypeToolCall && !part.ProviderExecuted {
			return false, nil
		}
	}
	return true, nil
...
func handledToolCallIDs(messages []database.ChatMessage) (map[string]bool, error) {
	...
	for _, msg := range messages {
		...
		for _, part := range parts {
			if part.Type == optimus-ide-collabsdk.ChatMessagePartTypeToolResult && part.ToolCallID != "" {
...
type partialMessageConversionState struct {
	...
	messages        []chatstate.Message
	assistantParts  []optimus-ide-collabsdk.ChatMessagePart
...
func (s *partialMessageConversionState) consumeAssistantPart(buffered messagepartbuffer.Part) {
	...
	call.part.Type = optimus-ide-collabsdk.ChatMessagePartTypeToolCall
	...
	if len(durable.Args) > 0 && !json.Valid(durable.Args) {
		call.valid = false
		s.assistantParts[call.index] = optimus-ide-collabsdk.ChatMessagePart{}
...
func (s *partialMessageConversionState) toolCall(id string) *partialToolCall {
	...
	s.assistantParts = append(s.assistantParts, optimus-ide-collabsdk.ChatMessagePart{})
	return call
}
...
func (s *partialMessageConversionState) flushAssistant() error {
	...
	durable := make([]optimus-ide-collabsdk.ChatMessagePart, 0, len(s.assistantParts))
...
func (s *partialMessageConversionState) flushAccumulatedToolResults() error {
	for _, id := range s.toolResultOrder {
		...
		s.logSkippedPart(messagepartbuffer.Part{Role: optimus-ide-collabsdk.ChatMessageRoleTool, MessagePart: result.part}, "streaming tool result delta is not durable")
	}
	return nil
}

func (s *partialMessageConversionState) appendToolResult(part optimus-ide-collabsdk.ChatMessagePart) error {
	content, err := chatprompt.MarshalParts([]optimus-ide-collabsdk.ChatMessagePart{part})
	if err != nil {
...
func (s *partialMessageConversionState) appendSyntheticInterruptionResults() error {
	for _, id := range s.toolCallOrder {
		...
		part := optimus-ide-collabsdk.ChatMessageToolResult(call.part.ToolCallID, call.part.ToolName, result, true, false)
