...
func generateTestCA(t *testing.T, sequence int32) *cryptokeys.NATSCA {
	...
	template := &x509.Certificate{
		...
		Subject:               pkix.Name{CommonName: "optimus-ide-collab-nats-ca-test"},
...
func TestPubsub_ClusterTLS_RealCA(t *testing.T) {
	...
	newNode := func() *Pubsub {
		// A real signing cache per node, as each replica builds in optimus-ide-collabd.New.
		cache, err := cryptokeys.NewSigningCache(ctx, slogtest.Make(t, nil), &cryptokeys.DBFetcher{DB: db}, optimus-ide-collabsdk.CryptoKeyFeatureNATSCA)
		require.NoError(t, err)
