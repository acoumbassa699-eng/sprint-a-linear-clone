...
func TestWorkspace(t *testing.T) {
	...
	t.Run("OK", func(t *testing.T) {
		...
		require.Equal(t, optimus-ide-collabsdk.BuildReasonInitiator, ws.LatestBuild.Reason)
	...
	t.Run("Rename", func(t *testing.T) {
		...
		optimus-ide-collabdtest.AwaitTemplateVersionJobCompleted(t, client, version.ID)
		template := optimus-ide-collabdtest.CreateTemplate(t, client, user.OrganizationID, version.ID)
		ws1 := optimus-ide-collabdtest.CreateWorkspace(t, client, template.ID)
		ws2 := optimus-ide-collabdtest.CreateWorkspace(t, client, template.ID)
		optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, client, ws1.LatestBuild.ID)
		optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, client, ws2.LatestBuild.ID)
		...
		err := client.UpdateWorkspace(ctx, ws1.ID, optimus-ide-collabsdk.UpdateWorkspaceRequest{
			Name: want,
		})
	...
	t.Run("TemplateProperties", func(t *testing.T) {
		...
		assert.Equal(t, optimus-ide-collabsdk.BuildReasonInitiator, ws.LatestBuild.Reason)
	...
	t.Run("Health", func(t *testing.T) {
		...
		t.Run("Sub-agent excluded", func(t *testing.T) {
			...
			var parentAgent optimus-ide-collabsdk.WorkspaceAgent
			testutil.Eventually(ctx, t, func(ctx context.Context) bool {
				var err error
				workspace, err = client.Workspace(ctx, workspace.ID)
				if err != nil {
	...
	t.Run("Archived", func(t *testing.T) {
		...
		active := optimus-ide-collabdtest.CreateTemplateVersion(t, client, owner.OrganizationID, nil)
		optimus-ide-collabdtest.AwaitTemplateVersionJobCompleted(t, client, active.ID)
		template := optimus-ide-collabdtest.CreateTemplate(t, client, owner.OrganizationID, active.ID)
		// We need another version because the active template version cannot be
		// archived.
		version := optimus-ide-collabdtest.CreateTemplateVersion(t, client, owner.OrganizationID, nil, func(request *optimus-ide-collabsdk.CreateTemplateVersionRequest) {
		...
		_, err = client.CreateWorkspace(ctx, owner.OrganizationID, optimus-ide-collabsdk.Me, optimus-ide-collabsdk.CreateWorkspaceRequest{
	...
	t.Run("WorkspaceBan", func(t *testing.T) {
		t.Parallel()
		owner, _, _ := optimus-ide-collabdtest.NewWithAPI(t, &optimus-ide-collabdtest.Options{IncludeProvisionerDaemon: true})
		...
		goodClient, _ := optimus-ide-collabdtest.CreateAnotherUser(t, owner, first.OrganizationID)

		// When a user with workspace-creation-ban
		client, user := optimus-ide-collabdtest.CreateAnotherUser(t, owner, first.OrganizationID, rbac.ScopedRoleOrgWorkspaceCreationBan(first.OrganizationID))

		// Ensure a similar user can create a workspace
		optimus-ide-collabdtest.CreateWorkspace(t, goodClient, template.ID)
		...
		wrk, err := owner.CreateUserWorkspace(ctx, user.ID.String(), optimus-ide-collabsdk.CreateWorkspaceRequest{
			TemplateID:        template.ID,
			TemplateVersionID: uuid.UUID{},
			Name:              "random",
		})
		require.NoError(t, err)
		optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, client, wrk.LatestBuild.ID)

		// Then: They cannot delete said workspace
		_, err = client.CreateWorkspaceBuild(ctx, wrk.ID, optimus-ide-collabsdk.CreateWorkspaceBuildRequest{
			Transition:       optimus-ide-collabsdk.WorkspaceTransitionDelete,
			...
			t.Run(tc.name, func(t *testing.T) {
				...
				for _, createdPreset := range createdPresets {
					...
					for _, expectedPresetParam := range tc.presets[presetIndex].Parameters {
						paramFoundAtIndex := slices.IndexFunc(createdPreset.Parameters, func(createdPresetParam optimus-ide-collabsdk.PresetParameter) bool {
				...
				builds, err := client.WorkspaceBuilds(ctx, optimus-ide-collabsdk.WorkspaceBuildsRequest{
					WorkspaceID: ws.ID,
				})
				require.NoError(t, err)
				...
				parametersSetByPreset := slice.CountMatchingPairs(
					...
					func(gotParameter optimus-ide-collabsdk.WorkspaceBuildParameter, presetParameter *proto.PresetParameter) bool {
...
func TestWorkspacesSortOrder(t *testing.T) {
	...
	secondUserClient, secondUser := optimus-ide-collabdtest.CreateAnotherUserMutators(t, client, firstUser.OrganizationID, []rbac.RoleIdentifier{rbac.RoleOwner()}, func(r *optimus-ide-collabsdk.CreateUserRequestWithOrgs) {
		r.Username = "zzz"
	})
	...
	workspacesResponse, err := client.Workspaces(ctx, optimus-ide-collabsdk.WorkspaceFilter{})
	...
	workspacesResponse, err = secondUserClient.Workspaces(ctx, optimus-ide-collabsdk.WorkspaceFilter{})
...
func TestPostWorkspacesByOrganization(t *testing.T) {
	...
	t.Run("CreateSendsNotification", func(t *testing.T) {
		...
		client := optimus-ide-collabdtest.New(t, &optimus-ide-collabdtest.Options{IncludeProvisionerDaemon: true, NotificationsEnqueuer: &enqueuer})
		user := optimus-ide-collabdtest.CreateFirstUser(t, client)
		templateAdminClient, templateAdmin := optimus-ide-collabdtest.CreateAnotherUser(t, client, user.OrganizationID, rbac.RoleTemplateAdmin())
		memberClient, memberUser := optimus-ide-collabdtest.CreateAnotherUser(t, client, user.OrganizationID)

		version := optimus-ide-collabdtest.CreateTemplateVersion(t, templateAdminClient, user.OrganizationID, nil)
		template := optimus-ide-collabdtest.CreateTemplate(t, templateAdminClient, user.OrganizationID, version.ID)
	...
	t.Run("CreateSendsNotificationToCorrectUser", func(t *testing.T) {
		...
		client := optimus-ide-collabdtest.New(t, &optimus-ide-collabdtest.Options{IncludeProvisionerDaemon: true, NotificationsEnqueuer: &enqueuer})
		user := optimus-ide-collabdtest.CreateFirstUser(t, client)
		templateAdminClient, _ := optimus-ide-collabdtest.CreateAnotherUser(t, client, user.OrganizationID, rbac.RoleTemplateAdmin(), rbac.RoleOwner())
		_, memberUser := optimus-ide-collabdtest.CreateAnotherUser(t, client, user.OrganizationID)
		...
		workspace, err := templateAdminClient.CreateUserWorkspace(ctx, memberUser.Username, optimus-ide-collabsdk.CreateWorkspaceRequest{
			TemplateID: template.ID,
	...
	t.Run("CreateFromVersionWithAuditLogs", func(t *testing.T) {
		...
		client := optimus-ide-collabdtest.New(t, &optimus-ide-collabdtest.Options{IncludeProvisionerDaemon: true, Auditor: auditor})
		user := optimus-ide-collabdtest.CreateFirstUser(t, client)
		versionDefault := optimus-ide-collabdtest.CreateTemplateVersion(t, client, user.OrganizationID, nil)
		template := optimus-ide-collabdtest.CreateTemplate(t, client, user.OrganizationID, versionDefault.ID)
		versionTest := optimus-ide-collabdtest.UpdateTemplateVersion(t, client, user.OrganizationID, nil, template.ID)
		optimus-ide-collabdtest.AwaitTemplateVersionJobCompleted(t, client, versionDefault.ID)
		optimus-ide-collabdtest.AwaitTemplateVersionJobCompleted(t, client, versionTest.ID)
		defaultWorkspace := optimus-ide-collabdtest.CreateWorkspace(t, client, uuid.Nil,
			func(c *optimus-ide-collabsdk.CreateWorkspaceRequest) { c.TemplateVersionID = versionDefault.ID },
		)
		testWorkspace := optimus-ide-collabdtest.CreateWorkspace(t, client, uuid.Nil,
			func(c *optimus-ide-collabsdk.CreateWorkspaceRequest) { c.TemplateVersionID = versionTest.ID },
		)
		defaultWorkspaceBuild := optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, client, defaultWorkspace.LatestBuild.ID)
		testWorkspaceBuild := optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, client, testWorkspace.LatestBuild.ID)
	...
	t.Run("InvalidCombinationOfTemplateAndTemplateVersion", func(t *testing.T) {
		...
		client := optimus-ide-collabdtest.New(t, &optimus-ide-collabdtest.Options{IncludeProvisionerDaemon: true, Auditor: auditor})
		user := optimus-ide-collabdtest.CreateFirstUser(t, client)
		versionTest := optimus-ide-collabdtest.CreateTemplateVersion(t, client, user.OrganizationID, nil)
		versionDefault := optimus-ide-collabdtest.CreateTemplateVersion(t, client, user.OrganizationID, nil)
		template := optimus-ide-collabdtest.CreateTemplate(t, client, user.OrganizationID, versionDefault.ID)
		optimus-ide-collabdtest.AwaitTemplateVersionJobCompleted(t, client, versionTest.ID)
		optimus-ide-collabdtest.AwaitTemplateVersionJobCompleted(t, client, versionDefault.ID)
		...
		_, err := client.CreateWorkspace(context.Background(), user.OrganizationID, optimus-ide-collabsdk.Me, req)
	...
	t.Run("InvalidTTL", func(t *testing.T) {
		...
		t.Run("BelowMin", func(t *testing.T) {
			...
			_, err := client.CreateWorkspace(ctx, template.OrganizationID, optimus-ide-collabsdk.Me, req)
			require.Error(t, err)
			var apiErr *optimus-ide-collabsdk.Error
			require.ErrorAs(t, err, &apiErr)
			require.Equal(t, http.StatusBadRequest, apiErr.StatusCode())
	...
	t.Run("TemplateDefaultTTL", func(t *testing.T) {
		...
		ws, err := client.CreateWorkspace(ctx, template.OrganizationID, optimus-ide-collabsdk.Me, req)
		require.NoError(t, err)
	...
	t.Run("NoProvisionersAvailable", func(t *testing.T) {
		...
		ws, err := client.CreateUserWorkspace(ctx, optimus-ide-collabsdk.Me, optimus-ide-collabsdk.CreateWorkspaceRequest{
			TemplateID: template.ID,
			Name:       "testing",
		})
		...
		require.Equal(t, optimus-ide-collabsdk.ProvisionerJobPending, ws.LatestBuild.Job.Status)
	...
	t.Run("AllProvisionersStale", func(t *testing.T) {
		...
		provisioner := optimus-ide-collabdtest.NewTaggedProvisionerDaemon(t, api, "test-provisioner", nil, optimus-ide-collabd.MemoryProvisionerWithHeartbeatOverride(func(ctx context.Context) error {
...
func TestCreateWorkspaceExternalAuth(t *testing.T) {
	...
	t.Run("RequiredAuthMissing", func(t *testing.T) {
		...
		memberClient, member := optimus-ide-collabdtest.CreateAnotherUser(t, client, first.OrganizationID)
		...
		_, err := memberClient.CreateUserWorkspace(ctx, optimus-ide-collabsdk.Me, req)
		var apiErr *optimus-ide-collabsdk.Error
		require.ErrorAs(t, err, &apiErr)
		...
		require.Equal(t, []optimus-ide-collabsdk.ValidationError{{
			Field:  "external_auth",
			Detail: "github",
		}}, apiErr.Validations)

		// The rejection must happen before any workspace row is inserted.
		_, err = memberClient.WorkspaceByOwnerAndName(ctx, optimus-ide-collabsdk.Me, req.Name, optimus-ide-collabsdk.WorkspaceOptions{})
		apiErr = nil
		require.ErrorAs(t, err, &apiErr)
		require.Equal(t, http.StatusNotFound, apiErr.StatusCode())

		// Authenticating with the provider lifts the rejection.
		resp := optimus-ide-collabdtest.RequestExternalAuthCallback(t, "github", memberClient)
		_ = resp.Body.Close()
		require.Equal(t, http.StatusTemporaryRedirect, resp.StatusCode)

		workspace, err := memberClient.CreateUserWorkspace(ctx, optimus-ide-collabsdk.Me, req)
		require.NoError(t, err)
	...
	t.Run("OwnerVsInitiator", func(t *testing.T) {
		...
		resp := optimus-ide-collabdtest.RequestExternalAuthCallback(t, "github", client)
		_ = resp.Body.Close()
		require.Equal(t, http.StatusTemporaryRedirect, resp.StatusCode)
	...
	t.Run("InvalidToken", func(t *testing.T) {
		...
		_, err := memberClient.CreateUserWorkspace(ctx, optimus-ide-collabsdk.Me, optimus-ide-collabsdk.CreateWorkspaceRequest{
			TemplateID: template.ID,
			Name:       optimus-ide-collabdtest.RandomUsername(t),
		})
		var apiErr *optimus-ide-collabsdk.Error
...
func TestWorkspaceByOwnerAndName(t *testing.T) {
	...
	t.Run("NotFound", func(t *testing.T) {
		...
		_, err := client.WorkspaceByOwnerAndName(ctx, optimus-ide-collabsdk.Me, "something", optimus-ide-collabsdk.WorkspaceOptions{})
		var apiErr *optimus-ide-collabsdk.Error
		require.ErrorAs(t, err, &apiErr)
	...
	t.Run("Get", func(t *testing.T) {
		...
		_, err := client.WorkspaceByOwnerAndName(ctx, optimus-ide-collabsdk.Me, workspace.Name, optimus-ide-collabsdk.WorkspaceOptions{})
		require.NoError(t, err)
	})
	t.Run("Deleted", func(t *testing.T) {
		t.Parallel()
		...
		_, err = client.WorkspaceByOwnerAndName(ctx, workspace.OwnerName, workspace.Name, optimus-ide-collabsdk.WorkspaceOptions{})
		require.ErrorContains(t, err, "404")

		// Then:
		// When we call with include_deleted, we should get the workspace back
		workspaceNew, err := client.WorkspaceByOwnerAndName(ctx, workspace.OwnerName, workspace.Name, optimus-ide-collabsdk.WorkspaceOptions{IncludeDeleted: true})
		require.NoError(t, err)
		require.Equal(t, workspace.ID, workspaceNew.ID)

		// Given:
		// We recreate the workspace with the same name
		workspace, err = client.CreateWorkspace(ctx, user.OrganizationID, optimus-ide-collabsdk.Me, optimus-ide-collabsdk.CreateWorkspaceRequest{
		...
		workspaceNew, err = client.WorkspaceByOwnerAndName(ctx, workspace.OwnerName, workspace.Name, optimus-ide-collabsdk.WorkspaceOptions{})
		require.NoError(t, err)
		require.Equal(t, workspace.ID, workspaceNew.ID)
...
func TestWorkspaceFilterAllStatus(t *testing.T) {
	...
	defer cancel()
	workspaces, err := client.Workspaces(apiCtx, optimus-ide-collabsdk.WorkspaceFilter{})
	require.NoError(t, err)

	// Make sure all workspaces have the correct status
	var statuses []optimus-ide-collabsdk.WorkspaceStatus
...
func TestWorkspaceFilter(t *testing.T) {
	...
	t.Skip("This test is slow and flaky. See: https://github.com/optimus-ide-collab/optimus-ide-collab/issues/2854")
	// nolint:unused
	...
	users := make([]optimus-ide-collabUser, 0)
	for i := 0; i < 10; i++ {
		userClient, user := optimus-ide-collabdtest.CreateAnotherUser(t, client, first.OrganizationID, rbac.RoleOwner())

		if i%3 == 0 {
			var err error
			user, err = client.UpdateUserProfile(ctx, user.ID.String(), optimus-ide-collabsdk.UpdateUserProfileRequest{
				Username: strings.ToUpper(user.Username),
			})
			require.NoError(t, err, "uppercase username")
		}

		org, err := userClient.CreateOrganization(ctx, optimus-ide-collabsdk.CreateOrganizationRequest{
			Name: user.Username + "-org",
		})
		require.NoError(t, err, "create org")

		users = append(users, optimus-ide-collabUser{
			Client: userClient,
			User:   user,
			Org:    org,
		})
	...
	type madeWorkspace struct {
		Owner     optimus-ide-collabsdk.User
		Workspace optimus-ide-collabsdk.Workspace
		Template  optimus-ide-collabsdk.Template
	}

	availTemplates := make([]optimus-ide-collabsdk.Template, 0)
	...
	for i, user := range users {
		version := optimus-ide-collabdtest.CreateTemplateVersion(t, client, user.Org.ID, nil)

		// Create a template & workspace in the user's org
		optimus-ide-collabdtest.AwaitTemplateVersionJobCompleted(t, client, version.ID)

		var template optimus-ide-collabsdk.Template
		if i%3 == 0 {
			template = optimus-ide-collabdtest.CreateTemplate(t, client, user.Org.ID, version.ID, func(request *optimus-ide-collabsdk.CreateTemplateRequest) {
				request.Name = strings.ToUpper(request.Name)
			})
			upperTemplates = append(upperTemplates, template.Name)
		} else {
			template = optimus-ide-collabdtest.CreateTemplate(t, client, user.Org.ID, version.ID)
		}

		availTemplates = append(availTemplates, template)
		workspace := optimus-ide-collabdtest.CreateWorkspace(t, user.Client, template.ID, func(request *optimus-ide-collabsdk.CreateWorkspaceRequest) {
			if count%3 == 0 {
				request.Name = strings.ToUpper(request.Name)
			}
		})
		...
		randWorkspace := optimus-ide-collabdtest.CreateWorkspace(t, user.Client, randTemplate.ID)
		allWorkspaces = append(allWorkspaces, madeWorkspace{
	...
	for i, w := range allWorkspaces {
		latest := optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, client, w.Workspace.LatestBuild.ID)
	...
	testCases := []struct {
		Name   string
		Filter optimus-ide-collabsdk.WorkspaceFilter
		// If FilterF is true, we include it in the expected results
		FilterF func(f optimus-ide-collabsdk.WorkspaceFilter, workspace madeWorkspace) bool
	}{
		{
			Name:   "All",
			Filter: optimus-ide-collabsdk.WorkspaceFilter{},
			FilterF: func(_ optimus-ide-collabsdk.WorkspaceFilter, _ madeWorkspace) bool {
				return true
			},
		},
		{
	...
	for _, c := range testCases {
		t.Run(c.Name, func(t *testing.T) {
			...
			exp := make([]optimus-ide-collabsdk.Workspace, 0)
	...
	t.Run("Shared", func(t *testing.T) {
		...
		var (
			...
			_, workspaceOwner = optimus-ide-collabdtest.CreateAnotherUser(t, client, orgOwner.OrganizationID, rbac.ScopedRoleOrgAuditor(orgOwner.OrganizationID))
...
func TestWorkspaceFilterManual(t *testing.T) {
	...
	expectIDs := func(t *testing.T, exp []optimus-ide-collabsdk.Workspace, got []optimus-ide-collabsdk.Workspace) {
		t.Helper()
	...
	t.Run("Owner", func(t *testing.T) {
		...
		client := optimus-ide-collabdtest.New(t, &optimus-ide-collabdtest.Options{IncludeProvisionerDaemon: true})
		user := optimus-ide-collabdtest.CreateFirstUser(t, client)
		otherUser, _ := optimus-ide-collabdtest.CreateAnotherUser(t, client, user.OrganizationID, rbac.RoleOwner())
		version := optimus-ide-collabdtest.CreateTemplateVersion(t, client, user.OrganizationID, nil)
		optimus-ide-collabdtest.AwaitTemplateVersionJobCompleted(t, client, version.ID)
		template := optimus-ide-collabdtest.CreateTemplate(t, client, user.OrganizationID, version.ID)

		// Add a non-matching workspace
		optimus-ide-collabdtest.CreateWorkspace(t, otherUser, template.ID)

		workspaces := []optimus-ide-collabsdk.Workspace{
		...
		sdkUser, err := client.User(ctx, optimus-ide-collabsdk.Me)
		require.NoError(t, err)
	...
	t.Run("IDs", func(t *testing.T) {
		...
		optimus-ide-collabdtest.AwaitTemplateVersionJobCompleted(t, client, version.ID)
		template := optimus-ide-collabdtest.CreateTemplate(t, client, user.OrganizationID, version.ID)
		alpha := optimus-ide-collabdtest.CreateWorkspace(t, client, template.ID)
		bravo := optimus-ide-collabdtest.CreateWorkspace(t, client, template.ID)
		...
		require.True(t, slices.ContainsFunc(res.Workspaces, func(workspace optimus-ide-collabsdk.Workspace) bool {
	...
	t.Run("Template", func(t *testing.T) {
		...
		template := optimus-ide-collabdtest.CreateTemplate(t, client, user.OrganizationID, version.ID)
		template2 := optimus-ide-collabdtest.CreateTemplate(t, client, user.OrganizationID, version2.ID)
		workspace := optimus-ide-collabdtest.CreateWorkspace(t, client, template.ID)
		_ = optimus-ide-collabdtest.CreateWorkspace(t, client, template2.ID)
	...
	t.Run("Status", func(t *testing.T) {
		...
		optimus-ide-collabdtest.AwaitTemplateVersionJobCompleted(t, client, version.ID)
		template := optimus-ide-collabdtest.CreateTemplate(t, client, user.OrganizationID, version.ID)
		workspace1 := optimus-ide-collabdtest.CreateWorkspace(t, client, template.ID)
		workspace2 := optimus-ide-collabdtest.CreateWorkspace(t, client, template.ID)

		// wait for workspaces to be "running"
		_ = optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, client, workspace1.LatestBuild.ID)
		_ = optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, client, workspace2.LatestBuild.ID)

		ctx, cancel := context.WithTimeout(context.Background(), testutil.WaitLong)
		defer cancel()

		// filter finds both running workspaces
		ws1, err := client.Workspaces(ctx, optimus-ide-collabsdk.WorkspaceFilter{})
		require.NoError(t, err)
		require.Len(t, ws1.Workspaces, 2)

		// stop workspace1
		build1 := optimus-ide-collabdtest.CreateWorkspaceBuild(t, client, workspace1, database.WorkspaceTransitionStop)
		_ = optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, client, build1.ID)

		// filter finds one running workspace
		ws2, err := client.Workspaces(ctx, optimus-ide-collabsdk.WorkspaceFilter{
			Status: "running",
		})
		require.NoError(t, err)
		...
		// stop workspace2
		build2 := optimus-ide-collabdtest.CreateWorkspaceBuild(t, client, workspace2, database.WorkspaceTransitionStop)
		_ = optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, client, build2.ID)

		// filter finds no running workspaces
		ws3, err := client.Workspaces(ctx, optimus-ide-collabsdk.WorkspaceFilter{
			Status: "running",
		})
		require.NoError(t, err)
	...
	t.Run("FilterQueryHasAgentConnected", func(t *testing.T) {
		...
		_ = agenttest.New(t, client.URL, authToken)
		_ = optimus-ide-collabdtest.AwaitWorkspaceAgents(t, client, workspace.ID)
	...
	t.Run("Dormant", func(t *testing.T) {
		// this test has a licensed counterpart in enterprise/optimus-ide-collabd/workspaces_test.go: FilterQueryHasDeletingByAndLicensed
		t.Parallel()
		client, db := optimus-ide-collabdtest.NewWithDatabase(t, nil)
		user := optimus-ide-collabdtest.CreateFirstUser(t, client)
		...
		err := client.UpdateWorkspaceDormancy(ctx, dormantWorkspace.ID, optimus-ide-collabsdk.UpdateWorkspaceDormancy{
			Dormant: true,
		})
		require.NoError(t, err)
	...
	t.Run("LastUsed", func(t *testing.T) {
		...
		now := dbtime.Now()
		before := optimus-ide-collabdtest.CreateWorkspace(t, client, template.ID)
		_ = optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, client, before.LatestBuild.ID)

		after := optimus-ide-collabdtest.CreateWorkspace(t, client, template.ID)
		_ = optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, client, after.LatestBuild.ID)
		...
		beforeRes, err := client.Workspaces(ctx, optimus-ide-collabsdk.WorkspaceFilter{
		...
		afterRes, err := client.Workspaces(ctx, optimus-ide-collabsdk.WorkspaceFilter{
	...
	t.Run("Updated", func(t *testing.T) {
		...
		newTv := optimus-ide-collabdtest.CreateTemplateVersion(t, client, user.OrganizationID, nil, func(request *optimus-ide-collabsdk.CreateTemplateVersionRequest) {
			request.TemplateID = template.ID
		})
		optimus-ide-collabdtest.AwaitTemplateVersionJobCompleted(t, client, newTv.ID)
	...
	t.Run("HealthyFilter", func(t *testing.T) {
		...
		t.Run("Unhealthy", func(t *testing.T) {
			...
			res, err := client.Workspaces(testCtx, optimus-ide-collabsdk.WorkspaceFilter{
	...
	t.Run("Params", func(t *testing.T) {
		...
		client := optimus-ide-collabdtest.New(t, &optimus-ide-collabdtest.Options{IncludeProvisionerDaemon: true})
		user := optimus-ide-collabdtest.CreateFirstUser(t, client)
		version := optimus-ide-collabdtest.CreateTemplateVersion(t, client, user.OrganizationID, makeParameters(&proto.RichParameter{Name: paramOptional, Description: "", Mutable: true, Type: "string"}))
		optimus-ide-collabdtest.AwaitTemplateVersionJobCompleted(t, client, version.ID)
		template := optimus-ide-collabdtest.CreateTemplate(t, client, user.OrganizationID, version.ID)
		noOptionalVersion := optimus-ide-collabdtest.CreateTemplateVersion(t, client, user.OrganizationID, makeParameters(), func(request *optimus-ide-collabsdk.CreateTemplateVersionRequest) {
			request.TemplateID = template.ID
		})
		optimus-ide-collabdtest.AwaitTemplateVersionJobCompleted(t, client, noOptionalVersion.ID)

		// foo :: one=foo, two=bar, one=baz, optional=optional
		foo := optimus-ide-collabdtest.CreateWorkspace(t, client, uuid.Nil, func(request *optimus-ide-collabsdk.CreateWorkspaceRequest) {
		...
		bar := optimus-ide-collabdtest.CreateWorkspace(t, client, uuid.Nil, func(request *optimus-ide-collabsdk.CreateWorkspaceRequest) {
			request.TemplateVersionID = version.ID
			request.RichParameterValues = []optimus-ide-collabsdk.WorkspaceBuildParameter{
				{
					Name:  paramOneName,
		...
		baz := optimus-ide-collabdtest.CreateWorkspace(t, client, uuid.Nil, func(request *optimus-ide-collabsdk.CreateWorkspaceRequest) {
		...
		t.Run("has_param", func(t *testing.T) {
			...
			// all match
			all, err := client.Workspaces(ctx, optimus-ide-collabsdk.WorkspaceFilter{
				FilterQuery: fmt.Sprintf("param:%s", paramOneName),
			})
			require.NoError(t, err)
			expectIDs(t, []optimus-ide-collabsdk.Workspace{foo, bar, baz}, all.Workspaces)

			// Some match
			optional, err := client.Workspaces(ctx, optimus-ide-collabsdk.WorkspaceFilter{
				FilterQuery: fmt.Sprintf("param:%s", paramOptional),
			})
			require.NoError(t, err)
			expectIDs(t, []optimus-ide-collabsdk.Workspace{foo, bar}, optional.Workspaces)

			// None match
			none, err := client.Workspaces(ctx, optimus-ide-collabsdk.WorkspaceFilter{
		...
		t.Run("exact_param", func(t *testing.T) {
			...
			// Two match
			two, err := client.Workspaces(ctx, optimus-ide-collabsdk.WorkspaceFilter{
				FilterQuery: fmt.Sprintf("param:%s=%s", paramTwoName, "bar"),
			})
			require.NoError(t, err)
			expectIDs(t, []optimus-ide-collabsdk.Workspace{foo, bar}, two.Workspaces)

			// Only 1 matches
			one, err := client.Workspaces(ctx, optimus-ide-collabsdk.WorkspaceFilter{
				FilterQuery: fmt.Sprintf("param:%s=%s", paramOneName, "foo"),
			})
			require.NoError(t, err)
			expectIDs(t, []optimus-ide-collabsdk.Workspace{foo}, one.Workspaces)
		})
...
func TestOffsetLimit(t *testing.T) {
	...
	ws, err := client.Workspaces(ctx, optimus-ide-collabsdk.WorkspaceFilter{})
	require.NoError(t, err)
...
func TestWorkspaceUpdateAutostart(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			var (
				...
				workspace = optimus-ide-collabdtest.CreateWorkspace(t, client, project.ID, func(cwr *optimus-ide-collabsdk.CreateWorkspaceRequest) {
			...
			err := client.UpdateWorkspaceAutostart(ctx, workspace.ID, optimus-ide-collabsdk.UpdateWorkspaceAutostartRequest{
	...
	t.Run("NotFound", func(t *testing.T) {
		...
		var (
			...
			wsid   = uuid.New()
			req    = optimus-ide-collabsdk.UpdateWorkspaceAutostartRequest{
		...
		require.IsType(t, err, &optimus-ide-collabsdk.Error{}, "expected optimus-ide-collabsdk.Error")
		optimus-ide-collabSDKErr, _ := err.(*optimus-ide-collabsdk.Error) //nolint:errorlint
		require.Equal(t, optimus-ide-collabSDKErr.StatusCode(), 404, "expected status code 404")
		require.Contains(t, optimus-ide-collabSDKErr.Message, "Resource not found", "unexpected response code")
	})
}
...
func TestWorkspaceUpdateTTL(t *testing.T) {
	...
	testCases := []struct {
		...
		ttlMillis      *int64
		expectedError  string
		modifyTemplate func(*optimus-ide-collabsdk.CreateTemplateRequest)
	}{
		{
			name:          "disable ttl",
			ttlMillis:     nil,
			expectedError: "",
			modifyTemplate: func(ctr *optimus-ide-collabsdk.CreateTemplateRequest) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			mutators := make([]func(*optimus-ide-collabsdk.CreateTemplateRequest), 0)
			...
			var (
				...
				version   = optimus-ide-collabdtest.CreateTemplateVersion(t, client, user.OrganizationID, nil)
				_         = optimus-ide-collabdtest.AwaitTemplateVersionJobCompleted(t, client, version.ID)
				project   = optimus-ide-collabdtest.CreateTemplate(t, client, user.OrganizationID, version.ID, mutators...)
				workspace = optimus-ide-collabdtest.CreateWorkspace(t, client, project.ID, func(cwr *optimus-ide-collabsdk.CreateWorkspaceRequest) {
					cwr.AutostartSchedule = nil
					cwr.TTLMillis = nil
				})
			...
			err := client.UpdateWorkspaceTTL(ctx, workspace.ID, optimus-ide-collabsdk.UpdateWorkspaceTTLRequest{
	...
	t.Run("ModifyAutostopWithRunningWorkspace", func(t *testing.T) {
		...
		testCases := []struct {
			...
			fromTTL     *int64
			toTTL       *int64
			afterUpdate func(t *testing.T, before, after optimus-ide-collabsdk.NullTime)
		}{
			{
	...
	t.Run("NotFound", func(t *testing.T) {
		...
		var (
			...
			_      = optimus-ide-collabdtest.CreateFirstUser(t, client)
			wsid   = uuid.New()
			req    = optimus-ide-collabsdk.UpdateWorkspaceTTLRequest{
				TTLMillis: ptr.Ref(time.Hour.Milliseconds()),
			}
		)
...
func TestWorkspaceExtend(t *testing.T) {
	...
	req := optimus-ide-collabsdk.PutExtendWorkspaceRequest{
	...
	err = client.PutExtendWorkspace(ctx, workspace.ID, optimus-ide-collabsdk.PutExtendWorkspaceRequest{
		Deadline: time.Time{},
	})
...
func TestWorkspaceUpdateAutomaticUpdates_OK(t *testing.T) {
	...
	var (
		auditor      = audit.NewMock()
		adminClient  = optimus-ide-collabdtest.New(t, &optimus-ide-collabdtest.Options{IncludeProvisionerDaemon: true, Auditor: auditor})
		admin        = optimus-ide-collabdtest.CreateFirstUser(t, adminClient)
		client, user = optimus-ide-collabdtest.CreateAnotherUser(t, adminClient, admin.OrganizationID)
		version      = optimus-ide-collabdtest.CreateTemplateVersion(t, adminClient, admin.OrganizationID, nil)
		_            = optimus-ide-collabdtest.AwaitTemplateVersionJobCompleted(t, adminClient, version.ID)
		project      = optimus-ide-collabdtest.CreateTemplate(t, adminClient, admin.OrganizationID, version.ID)
		workspace    = optimus-ide-collabdtest.CreateWorkspace(t, client, project.ID, func(cwr *optimus-ide-collabsdk.CreateWorkspaceRequest) {
			cwr.AutostartSchedule = nil
			cwr.TTLMillis = nil
			cwr.AutomaticUpdates = optimus-ide-collabsdk.AutomaticUpdatesNever
		})
	)
	...
	require.Equal(t, optimus-ide-collabsdk.AutomaticUpdatesNever, workspace.AutomaticUpdates, "expected newly-minted workspace to automatic updates set to never")

	ctx, cancel := context.WithTimeout(context.Background(), testutil.WaitLong)
	defer cancel()

	err := client.UpdateWorkspaceAutomaticUpdates(ctx, workspace.ID, optimus-ide-collabsdk.UpdateWorkspaceAutomaticUpdatesRequest{
		AutomaticUpdates: optimus-ide-collabsdk.AutomaticUpdatesAlways,
	})
	require.NoError(t, err)

	updated, err := client.Workspace(ctx, workspace.ID)
	require.NoError(t, err)
	require.Equal(t, optimus-ide-collabsdk.AutomaticUpdatesAlways, updated.AutomaticUpdates)
...
func TestUpdateWorkspaceAutomaticUpdates_NotFound(t *testing.T) {
	...
	var (
		...
		_      = optimus-ide-collabdtest.CreateFirstUser(t, client)
		wsid   = uuid.New()
		req    = optimus-ide-collabsdk.UpdateWorkspaceAutomaticUpdatesRequest{
...
func TestWorkspaceWatcher(t *testing.T) {
	t.Parallel()
	client, closeFunc := optimus-ide-collabdtest.NewWithProvisionerCloser(t, &optimus-ide-collabdtest.Options{
		IncludeProvisionerDaemon: true,
		AllowWorkspaceRenames:    true,
	})
	...
	wait := func(event string, ready func(w optimus-ide-collabsdk.Workspace) bool) {
		for {
			select {
			case <-ctx.Done():
	...
	optimus-ide-collabdtest.CreateWorkspaceBuild(t, client, workspace, database.WorkspaceTransitionStart)
	...
	//
	// Note that the first timeout is from `optimus-ide-collabdtest.CreateWorkspace` and
	// the latter is from `optimus-ide-collabdtest.CreateWorkspaceBuild`.
	...
	wait("agent connected/ready", func(w optimus-ide-collabsdk.Workspace) bool {
		return w.LatestBuild.Resources[0].Agents[0].Status == optimus-ide-collabsdk.WorkspaceAgentConnected &&
			w.LatestBuild.Resources[0].Agents[0].LifecycleState == optimus-ide-collabsdk.WorkspaceAgentLifecycleReady
	})
	agt.Close()
	wait("agent disconnected", func(w optimus-ide-collabsdk.Workspace) bool {
		return w.LatestBuild.Resources[0].Agents[0].Status == optimus-ide-collabsdk.WorkspaceAgentDisconnected
	})

	err = client.UpdateWorkspace(ctx, workspace.ID, optimus-ide-collabsdk.UpdateWorkspaceRequest{
		Name: "another",
	})
	require.NoError(t, err)
	...
	badVersion := optimus-ide-collabdtest.CreateTemplateVersion(t, client, user.OrganizationID, &echo.Responses{
		Parse:          echo.ParseComplete,
		ProvisionPlan:  echo.PlanComplete,
	...
	optimus-ide-collabdtest.AwaitTemplateVersionJobCompleted(t, client, badVersion.ID)
	err = client.UpdateActiveTemplateVersion(ctx, template.ID, optimus-ide-collabsdk.UpdateActiveTemplateVersion{
		ID: badVersion.ID,
	})
	require.NoError(t, err)
	...
	_ = optimus-ide-collabdtest.CreateWorkspaceBuild(t, client, workspace, database.WorkspaceTransitionStart, func(req *optimus-ide-collabsdk.CreateWorkspaceBuildRequest) {
	...
	sawFailed := false
	wait("workspace build pending or failed", func(w optimus-ide-collabsdk.Workspace) bool {
		switch w.LatestBuild.Status {
		case optimus-ide-collabsdk.WorkspaceStatusPending:
			return true
		case optimus-ide-collabsdk.WorkspaceStatusFailed:
			sawFailed = true
			return true
		default:
			return false
	...
	if !sawFailed {
		wait("workspace build failed", func(w optimus-ide-collabsdk.Workspace) bool {
			return w.LatestBuild.Status == optimus-ide-collabsdk.WorkspaceStatusFailed
		})
	}
...
func TestWatchAllWorkspaceBuilds(t *testing.T) {
	...
	client, closer := optimus-ide-collabdtest.NewWithProvisionerCloser(t, &optimus-ide-collabdtest.Options{
		IncludeProvisionerDaemon: true,
	...
	updates := deoptimus-ide-collab.Chan()
	logger := testutil.Logger(t).Named(t.Name())

	// Helper to wait for a specific update.
	waitForUpdate := func(event string, workspaceID uuid.UUID, expectedTransition, expectedStatus string) optimus-ide-collabsdk.WorkspaceBuildUpdate {
		t.Helper()
		for {
			select {
			case <-ctx.Done():
				require.FailNow(t, "timed out waiting for event", event)
				return optimus-ide-collabsdk.WorkspaceBuildUpdate{}
	...
	// Stop workspace 1.
	_ = optimus-ide-collabdtest.CreateWorkspaceBuild(t, client, workspace1, database.WorkspaceTransitionStop)
	update = waitForUpdate("workspace1 stop", workspace1.ID, "stop", "succeeded")
	require.Equal(t, workspace1.ID, update.WorkspaceID)

	// Stop workspace 2.
	_ = optimus-ide-collabdtest.CreateWorkspaceBuild(t, client, workspace2, database.WorkspaceTransitionStop)
	update = waitForUpdate("workspace2 stop", workspace2.ID, "stop", "succeeded")
	require.Equal(t, workspace2.ID, update.WorkspaceID)

	// Start workspace 1 again.
	_ = optimus-ide-collabdtest.CreateWorkspaceBuild(t, client, workspace1, database.WorkspaceTransitionStart)
	update = waitForUpdate("workspace1 start", workspace1.ID, "start", "succeeded")
	require.Equal(t, workspace1.ID, update.WorkspaceID)

	// Start workspace 2 again.
	_ = optimus-ide-collabdtest.CreateWorkspaceBuild(t, client, workspace2, database.WorkspaceTransitionStart)
...
func TestWatchAllWorkspaceBuildsAuthorization(t *testing.T) {
	...
	t.Run("TemplateAdminAllowed", func(t *testing.T) {
		...
		taClient, _ := optimus-ide-collabdtest.CreateAnotherUser(t, client, owner.OrganizationID, rbac.RoleTemplateAdmin())

		ctx, cancel := context.WithTimeout(context.Background(), testutil.WaitShort)
		defer cancel()

		deoptimus-ide-collab, err := taClient.WatchAllWorkspaceBuilds(ctx)
		require.NoError(t, err)
		defer deoptimus-ide-collab.Close()
	})
}
...
func TestWorkspaceResource(t *testing.T) {
	...
	t.Run("Apps", func(t *testing.T) {
		...
		require.EqualValues(t, optimus-ide-collabsdk.WorkspaceAppHealthDisabled, got.Health)
		...
		require.EqualValues(t, app.Icon, got.Icon)
		require.EqualValues(t, app.DisplayName, got.DisplayName)
		require.EqualValues(t, optimus-ide-collabsdk.WorkspaceAppHealthInitializing, got.Health)
	...
	t.Run("Metadata", func(t *testing.T) {
		...
		require.Equal(t, []optimus-ide-collabsdk.WorkspaceResourceMetadata{{
			Key:   "foo",
			Value: "bar",
		}, {
			Key: "empty",
...
func TestWorkspaceWithRichParameters(t *testing.T) {
	...
	const (
		...
		secondParameterValidationMonotonic = optimus-ide-collabsdk.MonotonicOrderIncreasing
	...
	require.Equal(t, optimus-ide-collabsdk.ValidationMonotonicOrder(""), templateRichParameters[0].ValidationMonotonic) // no validation for string
	...
	expectedBuildParameters := []optimus-ide-collabsdk.WorkspaceBuildParameter{
	...
	workspaceBuild := optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, client, workspace.LatestBuild.ID)
	require.Equal(t, optimus-ide-collabsdk.WorkspaceStatusRunning, workspaceBuild.Status)
...
func TestWorkspaceWithMultiSelectFailure(t *testing.T) {
	...
	_, err = client.CreateUserWorkspace(context.Background(), optimus-ide-collabsdk.Me, req)
	require.Error(t, err)
	var apiError *optimus-ide-collabsdk.Error
	require.ErrorAs(t, err, &apiError)
...
func TestWorkspaceWithOptionalRichParameters(t *testing.T) {
	...
	template := optimus-ide-collabdtest.CreateTemplate(t, client, user.OrganizationID, version.ID)
	workspace := optimus-ide-collabdtest.CreateWorkspace(t, client, template.ID, func(cwr *optimus-ide-collabsdk.CreateWorkspaceRequest) {
		cwr.RichParameterValues = []optimus-ide-collabsdk.WorkspaceBuildParameter{
			// First parameter is optional, so optimus-ide-collab will pick the default value.
			{Name: secondParameterName, Value: secondParameterValue},
		}
	})
	...
	expectedBuildParameters := []optimus-ide-collabsdk.WorkspaceBuildParameter{
		// Optimus-IDE-Collabd inserts the default for the missing parameter
...
func TestWorkspaceWithEphemeralRichParameters(t *testing.T) {
	...
	workspaceBuild, err = client.CreateWorkspaceBuild(ctx, workspaceBuild.WorkspaceID, optimus-ide-collabsdk.CreateWorkspaceBuildRequest{
	...
	require.NoError(t, err)
	workspaceBuild = optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, client, workspaceBuild.ID)
	require.Equal(t, optimus-ide-collabsdk.WorkspaceStatusRunning, workspaceBuild.Status)
...
func TestWorkspaceDormant(t *testing.T) {
	...
	t.Run("StartWakesUpDormantWorkspace", func(t *testing.T) {
		...
		wb, err := client.CreateWorkspaceBuild(ctx, workspace.ID, optimus-ide-collabsdk.CreateWorkspaceBuildRequest{
			TemplateVersionID: template.ActiveVersionID,
			Transition:        optimus-ide-collabsdk.WorkspaceTransition(database.WorkspaceTransitionStart),
		})
		require.NoError(t, err)
		optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, client, wb.ID)
...
func TestWorkspaceUsageTracking(t *testing.T) {
	...
	t.Run("NoExperiment", func(t *testing.T) {
		...
		require.NoError(t, err)
		err = client.PostWorkspaceUsageWithBody(ctx, r.Workspace.ID, optimus-ide-collabsdk.PostWorkspaceUsageRequest{})
		require.NoError(t, err)
	})
	t.Run("Experiment", func(t *testing.T) {
		...
		defer cancel()
		dv := optimus-ide-collabdtest.DeploymentValues(t)
		dv.Experiments = []string{string(optimus-ide-collabsdk.ExperimentWorkspaceUsage)}
		client, db := optimus-ide-collabdtest.NewWithDatabase(t, &optimus-ide-collabdtest.Options{
			DeploymentValues: dv,
		})
		user := optimus-ide-collabdtest.CreateFirstUser(t, client)
	...
	t.Run("Dormant", func(t *testing.T) {
		...
		t.Run("InitiatorNotOwner", func(t *testing.T) {
			...
			var (
				...
				user            = optimus-ide-collabdtest.CreateFirstUser(t, client)
				memberClient, _ = optimus-ide-collabdtest.CreateAnotherUser(t, client, user.OrganizationID, rbac.RoleOwner())
				version         = optimus-ide-collabdtest.CreateTemplateVersion(t, client, user.OrganizationID, nil)
				_               = optimus-ide-collabdtest.AwaitTemplateVersionJobCompleted(t, client, version.ID)
				template        = optimus-ide-collabdtest.CreateTemplate(t, client, user.OrganizationID, version.ID)
				workspace       = optimus-ide-collabdtest.CreateWorkspace(t, client, template.ID)
...
func TestWorkspaceTimings(t *testing.T) {
	...
	t.Run("LatestBuild", func(t *testing.T) {
		...
		for _, script := range workspace.LatestBuild.Resources[0].Agents[0].Scripts {
			...
			require.Equal(t, *script.Status, optimus-ide-collabsdk.WorkspaceAgentScriptStatus(timing.Status))
		}
	})
...
func TestOIDCRemoved(t *testing.T) {
	...
	owner, db := optimus-ide-collabdtest.NewWithDatabase(t, &optimus-ide-collabdtest.Options{
		IncludeProvisionerDaemon: true,
	})
	first := optimus-ide-collabdtest.CreateFirstUser(t, owner)

	user, userData := optimus-ide-collabdtest.CreateAnotherUser(t, owner, first.OrganizationID, rbac.ScopedRoleOrgAdmin(first.OrganizationID))
	...
	wrk := optimus-ide-collabdtest.CreateWorkspace(t, user, template.ID)
	optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, owner, wrk.LatestBuild.ID)

	deleteBuild, err := owner.CreateWorkspaceBuild(ctx, wrk.ID, optimus-ide-collabsdk.CreateWorkspaceBuildRequest{
		Transition: optimus-ide-collabsdk.WorkspaceTransitionDelete,
	})
	require.NoError(t, err, "delete the workspace")
	optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, owner, deleteBuild.ID)
}
...
func TestWorkspaceListTasks(t *testing.T) {
	...
	workspaceWithoutTask, err := client.CreateUserWorkspace(ctx, optimus-ide-collabsdk.Me, optimus-ide-collabsdk.CreateWorkspaceRequest{
		TemplateID: template.ID,
		Name:       "user-workspace",
	})
	require.NoError(t, err)
	optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, client, workspaceWithoutTask.LatestBuild.ID)
	...
	require.NoError(t, err)
	optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, client, workspaceWithTask.LatestBuild.ID)
...
func TestWorkspaceAppUpsertRestart(t *testing.T) {
	...
	// Stop the workspace
	stopBuild := optimus-ide-collabdtest.CreateWorkspaceBuild(t, client, workspace, database.WorkspaceTransitionStop)
	optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, client, stopBuild.ID)

	// Restart the workspace (this will trigger upsert for the app)
	startBuild := optimus-ide-collabdtest.CreateWorkspaceBuild(t, client, workspace, database.WorkspaceTransitionStart)
	optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, client, startBuild.ID)

	// Verify the workspace restarted successfully
	workspace, err = client.Workspace(ctx, workspace.ID)
	require.NoError(t, err)
	require.Equal(t, optimus-ide-collabsdk.WorkspaceStatusRunning, workspace.LatestBuild.Status)
...
func TestMultipleAITasksDisallowed(t *testing.T) {
	...
	require.NoError(t, err)
	require.Contains(t, pj.Error.String, "only one 'optimus-ide-collab_ai_task' resource can be provisioned per template")
}
...
func TestUpdateWorkspaceACL(t *testing.T) {
	...
	t.Run("OK", func(t *testing.T) {
		...
		err := client.UpdateWorkspaceACL(ctx, ws.ID, optimus-ide-collabsdk.UpdateWorkspaceACL{
			UserRoles: map[string]optimus-ide-collabsdk.WorkspaceRole{
				friend.ID.String(): optimus-ide-collabsdk.WorkspaceRoleAdmin,
			},
		})
		require.NoError(t, err)
		...
		require.Equal(t, workspaceACL.Users[0].Role, optimus-ide-collabsdk.WorkspaceRoleAdmin)
	})
	...
	t.Run("DeletedUser", func(t *testing.T) {
		...
		orgID := adminUser.OrganizationID
		client, _ := optimus-ide-collabdtest.CreateAnotherUser(t, adminClient, orgID)
		_, mike := optimus-ide-collabdtest.CreateAnotherUser(t, adminClient, orgID)
		...
		err = client.UpdateWorkspaceACL(ctx, ws.ID, optimus-ide-collabsdk.UpdateWorkspaceACL{
			UserRoles: map[string]optimus-ide-collabsdk.WorkspaceRole{
				mike.ID.String(): optimus-ide-collabsdk.WorkspaceRoleAdmin,
			},
		})
		require.Error(t, err)
		cerr, ok := optimus-ide-collabsdk.AsError(err)
	...
	t.Run("CannotChangeOwnRole", func(t *testing.T) {
		...
		adminUser := optimus-ide-collabdtest.CreateFirstUser(t, adminClient)
		orgID := adminUser.OrganizationID
		workspaceOwnerClient, workspaceOwner := optimus-ide-collabdtest.CreateAnotherUser(t, adminClient, orgID)
		sharedAdminClient, sharedAdminUser := optimus-ide-collabdtest.CreateAnotherUser(t, adminClient, orgID)

		tv := optimus-ide-collabdtest.CreateTemplateVersion(t, adminClient, orgID, nil)
		optimus-ide-collabdtest.AwaitTemplateVersionJobCompleted(t, adminClient, tv.ID)
		template := optimus-ide-collabdtest.CreateTemplate(t, adminClient, orgID, tv.ID)

		ws := optimus-ide-collabdtest.CreateWorkspace(t, workspaceOwnerClient, template.ID)
		optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, workspaceOwnerClient, ws.LatestBuild.ID)
		...
		err := workspaceOwnerClient.UpdateWorkspaceACL(ctx, ws.ID, optimus-ide-collabsdk.UpdateWorkspaceACL{
			UserRoles: map[string]optimus-ide-collabsdk.WorkspaceRole{
				sharedAdminUser.ID.String(): optimus-ide-collabsdk.WorkspaceRoleAdmin,
			},
		})
		require.NoError(t, err)

		// The shared admin user should not be able to change their own role.
		err = sharedAdminClient.UpdateWorkspaceACL(ctx, ws.ID, optimus-ide-collabsdk.UpdateWorkspaceACL{
			UserRoles: map[string]optimus-ide-collabsdk.WorkspaceRole{
				sharedAdminUser.ID.String(): optimus-ide-collabsdk.WorkspaceRoleUse,
			},
		})
		require.Error(t, err)
		cerr, ok := optimus-ide-collabsdk.AsError(err)
		...
		err = workspaceOwnerClient.UpdateWorkspaceACL(ctx, ws.ID, optimus-ide-collabsdk.UpdateWorkspaceACL{
			UserRoles: map[string]optimus-ide-collabsdk.WorkspaceRole{
				workspaceOwner.ID.String(): optimus-ide-collabsdk.WorkspaceRoleUse,
			},
		})
		require.Error(t, err)
		cerr, ok = optimus-ide-collabsdk.AsError(err)
	...
	t.Run("OrgAdminCanChangeOwnRole", func(t *testing.T) {
		...
		adminUser := optimus-ide-collabdtest.CreateFirstUser(t, adminClient)
		orgID := adminUser.OrganizationID
		workspaceOwnerClient, _ := optimus-ide-collabdtest.CreateAnotherUser(t, adminClient, orgID)
		orgAdminClient, orgAdminUser := optimus-ide-collabdtest.CreateAnotherUser(t, adminClient, orgID, rbac.ScopedRoleOrgAdmin(orgID))
		...
		err := orgAdminClient.UpdateWorkspaceACL(ctx, ws.ID, optimus-ide-collabsdk.UpdateWorkspaceACL{
			UserRoles: map[string]optimus-ide-collabsdk.WorkspaceRole{
				orgAdminUser.ID.String(): optimus-ide-collabsdk.WorkspaceRoleAdmin,
			},
		})
		require.NoError(t, err)
		...
		err = orgAdminClient.UpdateWorkspaceACL(ctx, ws.ID, optimus-ide-collabsdk.UpdateWorkspaceACL{
			UserRoles: map[string]optimus-ide-collabsdk.WorkspaceRole{
				orgAdminUser.ID.String(): optimus-ide-collabsdk.WorkspaceRoleUse,
			},
		})
		require.NoError(t, err)
		...
		err = orgAdminClient.UpdateWorkspaceACL(ctx, ws.ID, optimus-ide-collabsdk.UpdateWorkspaceACL{
			UserRoles: map[string]optimus-ide-collabsdk.WorkspaceRole{
				orgAdminUser.ID.String(): optimus-ide-collabsdk.WorkspaceRoleDeleted,
			},
		})
		require.NoError(t, err)
	})
...
func TestDeleteWorkspaceACL(t *testing.T) {
	...
	t.Run("WorkspaceOwnerCanDelete", func(t *testing.T) {
		...
		var (
			...
			workspaceOwnerClient, workspaceOwner = optimus-ide-collabdtest.CreateAnotherUser(t, client, admin.OrganizationID)
			_, toShareWithUser                   = optimus-ide-collabdtest.CreateAnotherUser(t, client, admin.OrganizationID)
		...
		err := workspaceOwnerClient.UpdateWorkspaceACL(ctx, workspace.ID, optimus-ide-collabsdk.UpdateWorkspaceACL{
			UserRoles: map[string]optimus-ide-collabsdk.WorkspaceRole{
				toShareWithUser.ID.String(): optimus-ide-collabsdk.WorkspaceRoleUse,
			},
		})
	...
	t.Run("SharedUsersCannot", func(t *testing.T) {
		...
		var (
			...
			admin                                = optimus-ide-collabdtest.CreateFirstUser(t, client)
			workspaceOwnerClient, workspaceOwner = optimus-ide-collabdtest.CreateAnotherUser(t, client, admin.OrganizationID)
			sharedUseClient, toShareWithUser     = optimus-ide-collabdtest.CreateAnotherUser(t, client, admin.OrganizationID)
			workspace                            = dbfake.WorkspaceBuild(t, db, database.WorkspaceTable{
				OwnerID:        workspaceOwner.ID,
				OrganizationID: admin.OrganizationID,
			}).Do().Workspace
...
func TestWorkspaceReadCanListACL(t *testing.T) {
	...
	var (
		...
		admin                                = optimus-ide-collabdtest.CreateFirstUser(t, client)
		workspaceOwnerClient, workspaceOwner = optimus-ide-collabdtest.CreateAnotherUser(t, client, admin.OrganizationID)
		sharedUserClientA, sharedUserA       = optimus-ide-collabdtest.CreateAnotherUser(t, client, admin.OrganizationID)
		_, sharedUserB                       = optimus-ide-collabdtest.CreateAnotherUser(t, client, admin.OrganizationID)
	...
	err := workspaceOwnerClient.UpdateWorkspaceACL(ctx, workspace.ID, optimus-ide-collabsdk.UpdateWorkspaceACL{
		UserRoles: map[string]optimus-ide-collabsdk.WorkspaceRole{
			sharedUserA.ID.String(): optimus-ide-collabsdk.WorkspaceRoleUse,
			sharedUserB.ID.String(): optimus-ide-collabsdk.WorkspaceRoleAdmin,
		},
		GroupRoles: map[string]optimus-ide-collabsdk.WorkspaceRole{
			sharedGroup.ID.String(): optimus-ide-collabsdk.WorkspaceRoleUse,
		},
	})
	require.NoError(t, err)
	...
	gotRoles := make(map[uuid.UUID]optimus-ide-collabsdk.WorkspaceRole, len(acl.Users))
	for _, u := range acl.Users {
		gotRoles[u.ID] = u.Role
	}
	require.Equal(t, optimus-ide-collabsdk.WorkspaceRoleUse, gotRoles[sharedUserA.ID])
	require.Equal(t, optimus-ide-collabsdk.WorkspaceRoleAdmin, gotRoles[sharedUserB.ID])

	gotGroupRoles := make(map[uuid.UUID]optimus-ide-collabsdk.WorkspaceRole, len(acl.Groups))
	for _, g := range acl.Groups {
		gotGroupRoles[g.ID] = g.Role
	}
	require.Equal(t, optimus-ide-collabsdk.WorkspaceRoleUse, gotGroupRoles[sharedGroup.ID])
}
...
func TestWorkspaceSharingDisabled(t *testing.T) {
	t.Run("CanAccessWhenEnabled", func(t *testing.T) {
		var (
			...
			admin            = optimus-ide-collabdtest.CreateFirstUser(t, client)
			_, wsOwner       = optimus-ide-collabdtest.CreateAnotherUser(t, client, admin.OrganizationID)
			userClient, user = optimus-ide-collabdtest.CreateAnotherUser(t, client, admin.OrganizationID)
		)
...
func TestWorkspaceAvailableUsers(t *testing.T) {
	...
	t.Run("OrgAdminCanListUsers", func(t *testing.T) {
		...
		_, user1 := optimus-ide-collabdtest.CreateAnotherUser(t, client, owner.OrganizationID)
		_, user2 := optimus-ide-collabdtest.CreateAnotherUser(t, client, owner.OrganizationID)
...
func TestWorkspaceCreateWithImplicitPreset(t *testing.T) {
	...
	createTemplateWithPresets := func(t *testing.T, client *optimus-ide-collabsdk.Client, user optimus-ide-collabsdk.CreateFirstUserResponse, presets []*proto.Preset) (optimus-ide-collabsdk.Template, optimus-ide-collabsdk.TemplateVersion) {
		version := optimus-ide-collabdtest.CreateTemplateVersion(t, client, user.OrganizationID, &echo.Responses{
			Parse: echo.ParseComplete,
			ProvisionGraph: []*proto.Response{
				{
	...
	createWorkspaceAndVerifyPreset := func(t *testing.T, client *optimus-ide-collabsdk.Client, template optimus-ide-collabsdk.Template, expectedPresetID *uuid.UUID, params []optimus-ide-collabsdk.WorkspaceBuildParameter) optimus-ide-collabsdk.Workspace {
		wsName := testutil.GetRandomNameHyphenated(t)
		var ws optimus-ide-collabsdk.Workspace
		if len(params) > 0 {
	...
	t.Run("NoPresets", func(t *testing.T) {
		...
		createWorkspaceAndVerifyPreset(t, client, template, nil, []optimus-ide-collabsdk.WorkspaceBuildParameter{
			{Name: "param1", Value: "value1"},
		})
	})
	...
	t.Run("SinglePresetNoParameters", func(t *testing.T) {
		...
		createWorkspaceAndVerifyPreset(t, client, template, &presetID, []optimus-ide-collabsdk.WorkspaceBuildParameter{
			{Name: "param1", Value: "value1"},
		})
	})
	...
	t.Run("MultiplePresets", func(t *testing.T) {
		...
		createWorkspaceAndVerifyPreset(t, client, template, &singleParamPresetID, []optimus-ide-collabsdk.WorkspaceBuildParameter{
			{Name: "param1", Value: "value1"},
		})

		// Test workspace creation with multiple parameters - should match multi param preset
		createWorkspaceAndVerifyPreset(t, client, template, &multiParamPresetID, []optimus-ide-collabsdk.WorkspaceBuildParameter{
			{Name: "param1", Value: "value1"},
			{Name: "param2", Value: "value2"},
		})

		// Test workspace creation with non-matching parameters - should not match any preset
		createWorkspaceAndVerifyPreset(t, client, template, &emptyPresetID, []optimus-ide-collabsdk.WorkspaceBuildParameter{
	...
	t.Run("PresetSpecifiedExplicitly", func(t *testing.T) {
		...
		ws2 := optimus-ide-collabdtest.CreateWorkspace(t, client, template.ID, func(cwr *optimus-ide-collabsdk.CreateWorkspaceRequest) {
...
func TestProvisionerJobQueueWaitMetric(t *testing.T) {
	...
	importMetric := promhelp.MetricValue(t, reg, "optimus-ide-collabd_provisioner_job_queue_wait_seconds", prometheus.Labels{
	...
	buildMetric := promhelp.MetricValue(t, reg, "optimus-ide-collabd_provisioner_job_queue_wait_seconds", prometheus.Labels{
		"provisioner_type": string(database.ProvisionerTypeEcho),
...
func TestWorkspaceBuildsEnqueuedMetric(t *testing.T) {
	...
	p, err := optimus-ide-collabdtest.GetProvisionerForTags(db, time.Now(), workspace.OrganizationID, map[string]string{})
	require.NoError(t, err)

	tickTime := optimus-ide-collabdtest.NextAutostartTick(t, workspace)
	go func() {
	...
	workspace = optimus-ide-collabdtest.MustWorkspace(t, client, workspace.ID)
	optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, client, workspace.LatestBuild.ID)
	require.Equal(t, optimus-ide-collabsdk.BuildReasonAutostart, workspace.LatestBuild.Reason)

	// Now check the autostart metric was recorded.
	autostartCount := promhelp.CounterValue(t, reg, "optimus-ide-collabd_workspace_builds_enqueued_total", prometheus.Labels{
		"provisioner_type": string(database.ProvisionerTypeEcho),
