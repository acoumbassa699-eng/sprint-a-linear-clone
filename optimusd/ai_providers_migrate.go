...
func SeedAIProvidersFromEnv(
	...
	err = db.InTx(func(tx database.Store) error {
		...
		for _, dp := range desired {
			settings, err := encodeAIProviderSettings(optimus-ide-collabsdk.AIProviderSettings{Bedrock: dp.Bedrock})
			if err != nil {
			...
			case found:
				...
				return xerrors.Errorf("AI provider %q already exists in the database and differs from the current environment configuration; update the provider through the API or remove the OPTIMUS-IDE-COLLAB_AIBRIDGE_* (legacy) / OPTIMUS-IDE-COLLAB_AI_GATEWAY_* env vars to stop seeding it", dp.Name)
			}
...
func computeKeysHash(bearerKeys []string, bedrock *optimus-ide-collabsdk.AIProviderBedrockSettings) string {
...
// (legacy single-provider env vars and indexed OPTIMUS-IDE-COLLAB_AIBRIDGE_PROVIDER_<N>_*
...
func providersFromEnv(ctx context.Context, cfg optimus-ide-collabsdk.AIBridgeConfig, logger slog.Logger) ([]desiredAIProvider, error) {
	...
	hasLegacyBedrock := optimus-ide-collabsdk.IsBedrockConfigured(cfg.LegacyBedrock.BaseURL.String(), bedrock)
	if hasAnthropicKey || hasLegacyBedrock {
		...
		if hasLegacyBedrock {
			...
			// Bedrock-only deployments use OPTIMUS-IDE-COLLAB_AIBRIDGE_BEDROCK_BASE_URL
	...
	for _, p := range cfg.Providers {
		...
		if !optimus-ide-collabsdk.AIProviderNameRegex.MatchString(name) {
			return nil, xerrors.Errorf("invalid AI provider name %q: must match %s", name, optimus-ide-collabsdk.AIProviderNameRegex)
		}
		...
		if dp.Type == database.AIProviderTypeAnthropic || dp.Type == database.AIProviderTypeBedrock {
			...
			isBedrock = optimus-ide-collabsdk.IsBedrockConfigured(p.BedrockBaseURL, bedrock)
			if isBedrock {
				dp.Bedrock = &bedrock
