...
import (
	...
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabd/mcp"
...
func (api *API) mcpHTTPHandler() http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			...
			httpapi.Write(r.Context(), w, http.StatusInternalServerError, optimus-ide-collabsdk.Response{
				Message: "MCP server initialization failed",
			})
			return
		}
		// Extract the original session token from the request
		authenticatedClient := optimus-ide-collabsdk.New(api.AccessURL,
			optimus-ide-collabsdk.WithSessionToken(httpmw.APITokenFromRequest(r)))
		...
		default:
			httpapi.Write(r.Context(), w, http.StatusBadRequest, optimus-ide-collabsdk.Response{
