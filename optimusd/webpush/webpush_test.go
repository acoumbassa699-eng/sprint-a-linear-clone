...
func TestPush(t *testing.T) {
	...
	t.Run("NoSubscriptions", func(t *testing.T) {
		...
		userID := uuid.New()
		notification := optimus-ide-collabsdk.WebpushMessage{
			Title: "Test Title",
			Body:  "Test Body",
		}
...
func randomWebpushMessage(t testing.TB) optimus-ide-collabsdk.WebpushMessage {
	t.Helper()
	return optimus-ide-collabsdk.WebpushMessage{
		Title: testutil.GetRandomName(t),
		Body:  testutil.GetRandomName(t),

		Actions: []optimus-ide-collabsdk.WebpushMessageAction{
...
func assertWebpushPayload(t testing.TB, r *http.Request) {
	...
	// encrypted.
	assert.Error(t, json.NewDeoptimus-ide-collab(r.Body).Decode(io.Discard))
}
...
func TestNoopWebpusher(t *testing.T) {
	...
	dispatchErr := noop.Dispatch(context.Background(), uuid.New(), optimus-ide-collabsdk.WebpushMessage{})
	require.Error(t, dispatchErr)
	require.Contains(t, dispatchErr.Error(), "push disabled")

	testErr := noop.Test(context.Background(), optimus-ide-collabsdk.WebpushSubscription{})
...
func TestSSRFPrevention(t *testing.T) {
	...
	err = manager.Test(ctx, optimus-ide-collabsdk.WebpushSubscription{
		Endpoint:  server.URL,
	...
	err = manager.Dispatch(ctx, user.ID, optimus-ide-collabsdk.WebpushMessage{
		Title: "SSRF test",
