...
// optimus-ide-collabd; keep the two in sync.
...
func (api *API) listMCPServerConfigs(rw http.ResponseWriter, r *http.Request) {
	...
	resp := make([]optimus-ide-collabsdk.MCPServerConfig, 0, len(configs))
	for _, config := range configs {
		var sdkConfig optimus-ide-collabsdk.MCPServerConfig
		if isAdmin {
...
func (api *API) createMCPServerConfig(rw http.ResponseWriter, r *http.Request) {
	...
	var req optimus-ide-collabsdk.CreateMCPServerConfigRequest
	...
	case "oauth2":
		...
		if req.OAuth2ClientID == "" && req.OAuth2AuthURL == "" && req.OAuth2TokenURL == "" {
			...
			inserted, err := api.Database.InsertMCPServerConfig(ctx, database.InsertMCPServerConfigParams{
				...
				ForwardOptimus-IDE-CollabHeaders:     req.ForwardOptimus-IDE-CollabHeaders,
			...
			updated, err := api.Database.UpdateMCPServerConfig(ctx, database.UpdateMCPServerConfigParams{
				...
				ForwardOptimus-IDE-CollabHeaders:     inserted.ForwardOptimus-IDE-CollabHeaders,
				UpdatedBy:               apiKey.UserID,
			})
			if err != nil {
				httpapi.Write(ctx, rw, http.StatusInternalServerError, optimus-ide-collabsdk.Response{
...
func (api *API) updateMCPServerConfig(rw http.ResponseWriter, r *http.Request) {
	...
	var req optimus-ide-collabsdk.UpdateMCPServerConfigRequest
	if !httpapi.Read(ctx, rw, r, &req) {
		return
	}
	...
	err := api.Database.InTx(func(tx database.Store) error {
		...
		forwardOptimus-IDE-CollabHeaders := existing.ForwardOptimus-IDE-CollabHeaders
		if req.ForwardOptimus-IDE-CollabHeaders != nil {
			forwardOptimus-IDE-CollabHeaders = *req.ForwardOptimus-IDE-CollabHeaders
		}
...
func (api *API) mcpServerOAuth2Callback(rw http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		httpapi.Write(ctx, rw, http.StatusBadGateway, optimus-ide-collabsdk.Response{
...
func (api *API) mcpServerOAuth2Disconnect(rw http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			...
			httpapi.Write(ctx, rw, http.StatusOK, optimus-ide-collabsdk.MCPServerOAuth2DisconnectResponse{})
			return
		}
		httpapi.Write(ctx, rw, http.StatusInternalServerError, optimus-ide-collabsdk.Response{
	...
	resp := optimus-ide-collabsdk.MCPServerOAuth2DisconnectResponse{}
	if config.AuthType == "oauth2" {
...
func convertMCPServerConfig(config database.MCPServerConfig) optimus-ide-collabsdk.MCPServerConfig {
	return optimus-ide-collabsdk.MCPServerConfig{
		ID:          config.ID,
		...
		ForwardOptimus-IDE-CollabHeaders: config.ForwardOptimus-IDE-CollabHeaders,
...
// non-admin callers.
func convertMCPServerConfigRedacted(config database.MCPServerConfig) optimus-ide-collabsdk.MCPServerConfig {
...
func discoverAndRegisterMCPOAuth2(ctx context.Context, httpClient *http.Client, mcpServerURL, callbackURL string) (*mcpOAuth2Discovery, error) {
	...
	clientID, clientSecret, err := registerOAuth2Client(
		ctx, httpClient, asMeta.RegistrationEndpoint, callbackURL, "Optimus-IDE-Collab",
	)
	if err != nil {
		return nil, xerrors.Errorf(
