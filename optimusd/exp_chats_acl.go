...
// @Success 200 {object} optimus-ide-collabsdk.ChatACL
...
func (api *API) getChatACL(rw http.ResponseWriter, r *http.Request) {
	...
	if chat.IsSubChat() {
		resp := optimus-ide-collabsdk.Response{Message: "Chat ACLs can only be set on root chats."}
	...
	httpapi.Write(ctx, rw, http.StatusOK, optimus-ide-collabsdk.ChatACL{
		Users:  users,
		Groups: groups,
	})
}
...
// @Accept json
// @Param chat path string true "Chat ID" format(uuid)
// @Param request body optimus-ide-collabsdk.UpdateChatACL true "Update chat ACL request"
// @Success 204
...
func (api *API) patchChatACL(rw http.ResponseWriter, r *http.Request) {
	...
	var req optimus-ide-collabsdk.UpdateChatACL
	...
	err := api.Database.InTx(func(tx database.Store) error {
		...
		for id, role := range req.UserRoles {
			if role == optimus-ide-collabsdk.ChatRoleDeleted {
...
func (api *API) chatACLUsers(ctx context.Context, rw http.ResponseWriter, chat database.Chat, entries database.ChatACL) ([]optimus-ide-collabsdk.ChatUser, bool) {
	...
	users := make([]optimus-ide-collabsdk.ChatUser, 0, len(dbUsers))
	for _, user := range dbUsers {
		entry := entries[user.ID.String()]
		users = append(users, optimus-ide-collabsdk.ChatUser{
...
func (api *API) chatACLGroups(ctx context.Context, rw http.ResponseWriter, chat database.Chat, entries database.ChatACL) ([]optimus-ide-collabsdk.ChatGroup, bool) {
	...
	groups := make([]optimus-ide-collabsdk.ChatGroup, 0, len(dbGroups))
	for _, group := range dbGroups {
		...
		groups = append(groups, optimus-ide-collabsdk.ChatGroup{
...
type ChatACLUpdateValidator optimus-ide-collabsdk.UpdateChatACL

var _ acl.UpdateValidator[optimus-ide-collabsdk.ChatRole] = ChatACLUpdateValidator{}

func (c ChatACLUpdateValidator) Users() (map[string]optimus-ide-collabsdk.ChatRole, string) {
	return c.UserRoles, "user_roles"
}

func (c ChatACLUpdateValidator) Groups() (map[string]optimus-ide-collabsdk.ChatRole, string) {
	return c.GroupRoles, "group_roles"
}

func (ChatACLUpdateValidator) ValidateRole(role optimus-ide-collabsdk.ChatRole) error {
	if role == optimus-ide-collabsdk.ChatRoleDeleted || role == optimus-ide-collabsdk.ChatRoleRead {
		return nil
	}
	return xerrors.Errorf("role %q is not a valid chat role", role)
}

func convertToChatRole(actions []policy.Action) optimus-ide-collabsdk.ChatRole {
	if slice.SameElements(actions, db2sdk.ChatRoleActions(optimus-ide-collabsdk.ChatRoleRead)) {
		return optimus-ide-collabsdk.ChatRoleRead
	}

	return optimus-ide-collabsdk.ChatRoleDeleted
}
