...
import (
	...
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/archive/archivetest"
...
func TestPostFiles(t *testing.T) {
	...
	t.Run("Insert", func(t *testing.T) {
		...
		_, err := client.Upload(ctx, optimus-ide-collabsdk.ContentTypeTar, bytes.NewReader(make([]byte, 1024)))
		require.NoError(t, err)
	})
	...
	t.Run("InsertAlreadyExists", func(t *testing.T) {
		...
		data := make([]byte, 1024)
		_, err := client.Upload(ctx, optimus-ide-collabsdk.ContentTypeTar, bytes.NewReader(data))
		require.NoError(t, err)
		_, err = client.Upload(ctx, optimus-ide-collabsdk.ContentTypeTar, bytes.NewReader(data))
		require.NoError(t, err)
	})
	t.Run("InvalidZipMetadata", func(t *testing.T) {
		...
		_, err := client.Upload(ctx, optimus-ide-collabsdk.ContentTypeZip, bytes.NewReader(zipBytes))
	...
	t.Run("OversizedZipExpansion", func(t *testing.T) {
		...
		// file size limit.
		size := int64(optimus-ide-collabd.HTTPFileMaxBytes - 1024)
...
func TestDownload(t *testing.T) {
	...
	t.Run("InsertTar_DownloadTar", func(t *testing.T) {
		...
		// when
		resp, err := client.Upload(ctx, optimus-ide-collabsdk.ContentTypeTar, bytes.NewReader(tarball))
		require.NoError(t, err)
		...
		require.Equal(t, optimus-ide-collabsdk.ContentTypeTar, contentType)
	...
	t.Run("InsertZip_DownloadTar", func(t *testing.T) {
		...
		// when
		resp, err := client.Upload(ctx, optimus-ide-collabsdk.ContentTypeZip, bytes.NewReader(zipContent))
		require.NoError(t, err)
		data, contentType, err := client.Download(ctx, resp.ID)
		require.NoError(t, err)
	...
	t.Run("InsertTar_DownloadZip", func(t *testing.T) {
		...
		resp, err := client.Upload(ctx, optimus-ide-collabsdk.ContentTypeTar, bytes.NewReader(tarball))
		require.NoError(t, err)
		data, contentType, err := client.DownloadWithFormat(ctx, resp.ID, optimus-ide-collabsdk.FormatZip)
		require.NoError(t, err)

		// then
		require.Equal(t, optimus-ide-collabsdk.ContentTypeZip, contentType)
