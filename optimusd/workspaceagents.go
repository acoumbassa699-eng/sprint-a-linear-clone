...
import (
	...
	maputil "github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabd/util/maps"
...
// @Success 200 {object} optimus-ide-collabsdk.WorkspaceAgent
...
func (api *API) patchWorkspaceAgentLogs(rw http.ResponseWriter, r *http.Request) {
	...
	for _, logEntry := range req.Logs {
		...
		if logEntry.Level == "" {
			...
			logEntry.Level = optimus-ide-collabsdk.LogLevelInfo
		}
...
func (api *API) patchWorkspaceAgentAppStatus(rw http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		sdkErr := new(optimus-ide-collabsdk.Error)
...
// @Success 200 {array} optimus-ide-collabsdk.WorkspaceAgentLog
...
func (api *API) workspaceAgentLogs(rw http.ResponseWriter, r *http.Request) {
	...
	// needed with e.g. optimus-ide-collab/websocket and Safari (confirmed in 16.5).
	//
	// See:
	...
	enoptimus-ide-collab := wsjson.NewEnoptimus-ide-collab[[]optimus-ide-collabsdk.WorkspaceAgentLog](conn, websocket.MessageText)
	defer enoptimus-ide-collab.Close(websocket.StatusNormalClosure)

	err = enoptimus-ide-collab.Encode(convertWorkspaceAgentLogs(logs))
	if err != nil {
		return
	}
...
// @Tags Agents
// @Param workspaceagent path string true "Workspace agent ID" format(uuid)
// @Success 200 {object} optimus-ide-collabsdk.WorkspaceAgentListeningPortsResponse
...
func (api *API) workspaceAgentListeningPorts(rw http.ResponseWriter, r *http.Request) {
	...
	if apiAgent.Status != optimus-ide-collabsdk.WorkspaceAgentConnected {
		httpapi.Write(ctx, rw, http.StatusBadRequest, optimus-ide-collabsdk.Response{
			Message: fmt.Sprintf("Agent state is %q, it must be in the %q state.", apiAgent.Status, optimus-ide-collabsdk.WorkspaceAgentConnected),
		})
		return
	}
	...
	filteredPorts := make([]optimus-ide-collabsdk.WorkspaceAgentListeningPort, 0, len(portsResponse.Ports))
...
// @Tags Agents
// @Param workspaceagent path string true "Workspace agent ID" format(uuid)
// @Success 200 {object} optimus-ide-collabsdk.WorkspaceAgentListContainersResponse
...
func (api *API) watchWorkspaceAgentContainers(rw http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		case containers, ok := <-containersCh:
			...
			if err := enoptimus-ide-collab.Encode(containers); err != nil {
...
func (api *API) workspaceAgentListContainers(rw http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if errors.Is(err, context.Canceled) {
			httpapi.Write(ctx, rw, http.StatusRequestTimeout, optimus-ide-collabsdk.Response{
				Message: "Failed to fetch containers from agent.",
				Detail:  "Request timed out.",
			})
			return
		}
		// If the agent returns a optimus-ide-collabsdk.Error, we can return that directly.
	...
	cts.Containers = slices.DeleteFunc(cts.Containers, func(ct optimus-ide-collabsdk.WorkspaceAgentContainer) bool {
...
// @Param workspaceagent path string true "Workspace agent ID" format(uuid)
// @Param devcontainer path string true "Devcontainer ID"
// @Success 202 {object} optimus-ide-collabsdk.Response
...
func (api *API) workspaceAgentRecreateDevcontainer(rw http.ResponseWriter, r *http.Request) {
	...
	agentConn, release, err := api.agentProvider.AgentConn(dialCtx, waws.WorkspaceAgent.ID)
	if err != nil {
		httpapi.Write(dialCtx, rw, http.StatusInternalServerError, optimus-ide-collabsdk.Response{
			Message: "Internal error dialing workspace agent.",
			Detail:  err.Error(),
		})
		return
...
func (api *API) derpMapUpdates(rw http.ResponseWriter, r *http.Request) {
	...
	enoptimus-ide-collab := wsjson.NewEnoptimus-ide-collab[*tailcfg.DERPMap](ws, websocket.MessageBinary)
	defer enoptimus-ide-collab.Close(websocket.StatusGoingAway)
	...
	for {
		...
		if lastDERPMap == nil || !tailnet.CompareDERPMaps(lastDERPMap, derpMap) {
			err := enoptimus-ide-collab.Encode(derpMap)
			if err != nil {
				return
			}
			lastDERPMap = derpMap
...
func (api *API) workspaceAgentClientCoordinate(rw http.ResponseWriter, r *http.Request) {
	...
	// Namely, disabling the route using `OPTIMUS-IDE-COLLAB_BROWSER_ONLY`.
	...
	ctx, wsNetConn := optimus-ide-collabsdk.WebsocketNetConn(ctx, conn, websocket.MessageBinary)
	defer wsNetConn.Close()
...
// cannot identify the user (see optimus-ide-collabd/agentapi/connectionlog.go), and
...
func (api *API) handleResumeToken(ctx context.Context, rw http.ResponseWriter, r *http.Request) (peerID uuid.UUID, err error) {
	...
	if resumeToken != "" {
		...
		case xerrors.Is(err, jwtutils.ErrMissingKeyID):
			...
			err = nil
		case err != nil:
			httpapi.Write(ctx, rw, http.StatusUnauthorized, optimus-ide-collabsdk.Response{
...
// @Success 200 {object} optimus-ide-collabsdk.WorkspaceAgentLogSource
...
func (api *API) workspaceAgentPostLogSource(rw http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if database.IsUniqueViolation(err, "workspace_agent_log_sources_pkey") {
			httpapi.Write(ctx, rw, http.StatusCreated, optimus-ide-collabsdk.WorkspaceAgentLogSource{
				WorkspaceAgentID: workspaceAgent.ID,
				CreatedAt:        dbtime.Now(),
				ID:               req.ID,
...
func convertProvisionedApps(dbApps []database.WorkspaceApp) []optimus-ide-collabsdk.WorkspaceApp {
	return db2sdk.Apps(dbApps, []database.WorkspaceAppStatus{}, database.WorkspaceAgent{}, "", database.WorkspaceTable{})
}

func convertLogSources(dbLogSources []database.WorkspaceAgentLogSource) []optimus-ide-collabsdk.WorkspaceAgentLogSource {
	logSources := make([]optimus-ide-collabsdk.WorkspaceAgentLogSource, 0)
	for _, dbLogSource := range dbLogSources {
		logSources = append(logSources, optimus-ide-collabsdk.WorkspaceAgentLogSource{
			ID:               dbLogSource.ID,
...
func convertScripts(dbScripts []database.GetWorkspaceAgentScriptsByAgentIDsRow) []optimus-ide-collabsdk.WorkspaceAgentScript {
	scripts := make([]optimus-ide-collabsdk.WorkspaceAgentScript, 0)
...
func convertWorkspaceAgentMetadata(db []database.WorkspaceAgentMetadatum) []optimus-ide-collabsdk.WorkspaceAgentMetadata {
	...
	result := make([]optimus-ide-collabsdk.WorkspaceAgentMetadata, len(db))
	for i, datum := range db {
		result[i] = optimus-ide-collabsdk.WorkspaceAgentMetadata{
			...
			Description: optimus-ide-collabsdk.WorkspaceAgentMetadataDescription{
...
func (api *API) workspaceAgentsExternalAuth(rw http.ResponseWriter, r *http.Request) {
	...
	if externalAuthConfig == nil {
		...
		httpapi.Write(ctx, rw, http.StatusNotFound, optimus-ide-collabsdk.Response{
			Message: fmt.Sprintf("No matching external auth provider found in Optimus-IDE-Collab for the url %q.", match),
			Detail:  detail,
		})
		return
	}
	...
	// MarkStale will trigger a refresh by optimus-ide-collabd/gitsync. This allows us to
	...
	if err != nil {
		handleRetrying(http.StatusInternalServerError, optimus-ide-collabsdk.Response{
...
func (api *API) workspaceAgentsExternalAuthListen(ctx context.Context, rw http.ResponseWriter, previous *database.ExternalAuthLink, externalAuthConfig *externalauth.Config, workspace database.Workspace, gitRef chatGitRef) {
	...
	for {
		...
		// MarkStale will trigger a refresh by optimus-ide-collabd/gitsync.
		//nolint:gocritic // Chat processor context required for cross-user chat lookup
		api.gitSyncWorker.MarkStale(dbauthz.AsChatd(ctx), gitsync.MarkStaleParams{
			WorkspaceID: workspace.ID,
			Branch:      gitRef.Branch,
...
func (api *API) tailnetRPCConn(rw http.ResponseWriter, r *http.Request) {
	...
	connectionTelemetryEvent := telemetry.UserTailnetConnection{
		...
		DeviceID:            nil,
		DeviceOS:            nil,
		Optimus-IDE-CollabDesktopVersion: nil,
	}

	fillOptimus-IDE-CollabDesktopTelemetry(r, &connectionTelemetryEvent, api.Logger)
...
// fillOptimus-IDE-CollabDesktopTelemetry fills out the provided event based on a Optimus-IDE-Collab Desktop telemetry header on the request, if
// present.
func fillOptimus-IDE-CollabDesktopTelemetry(r *http.Request, event *telemetry.UserTailnetConnection, logger slog.Logger) {
	// Parse desktop telemetry from header if it exists
	desktopTelemetryHeader := r.Header.Get(optimus-ide-collabsdk.Optimus-IDE-CollabDesktopTelemetryHeader)
	if desktopTelemetryHeader != "" {
		var telemetryData optimus-ide-collabsdk.Optimus-IDE-CollabDesktopTelemetry
		if err := telemetryData.FromHeader(desktopTelemetryHeader); err == nil {
			...
			if telemetryData.Optimus-IDE-CollabDesktopVersion != "" {
				event.Optimus-IDE-CollabDesktopVersion = &telemetryData.Optimus-IDE-CollabDesktopVersion
			}
			logger.Debug(r.Context(), "received desktop telemetry",
				...
				slog.F("desktop_version", telemetryData.Optimus-IDE-CollabDesktopVersion))
		} else {
...
func createExternalAuthResponse(typ, token string, extra pqtype.NullRawMessage, expiry time.Time) (agentsdk.ExternalAuthResponse, error) {
	...
	switch typ {
	case string(optimus-ide-collabsdk.EnhancedExternalAuthProviderGitLab):
		// https://stackoverflow.com/questions/25409700/using-gitlab-token-to-clone-without-authentication
		resp = agentsdk.ExternalAuthResponse{
			Username: "oauth2",
			Password: token,
		}
	case string(optimus-ide-collabsdk.EnhancedExternalAuthProviderBitBucketCloud), string(optimus-ide-collabsdk.EnhancedExternalAuthProviderBitBucketServer):
...
func convertWorkspaceAgentLogs(logs []database.WorkspaceAgentLog) []optimus-ide-collabsdk.WorkspaceAgentLog {
	sdk := make([]optimus-ide-collabsdk.WorkspaceAgentLog, 0, len(logs))
...
// drift markers. It backs the in-workspace `optimus-ide-collab exp chat context refresh`
