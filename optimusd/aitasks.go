...
import (
	...
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabd/taskname"
...
// @Param request body optimus-ide-collabsdk.CreateTaskRequest true "Create task request"
// @Success 201 {object} optimus-ide-collabsdk.Task
...
func (api *API) tasksCreate(rw http.ResponseWriter, r *http.Request) {
	...
	var req optimus-ide-collabsdk.CreateTaskRequest
	...
	if !templateVersion.HasAITask.Valid || !templateVersion.HasAITask.Bool {
		httpapi.Write(ctx, rw, http.StatusBadRequest, optimus-ide-collabsdk.Response{
			Message: `Template does not have a valid "optimus-ide-collab_ai_task" resource.`,
		})
		return
	}
	...
	taskName := req.Name
	if taskName != "" {
		if err := optimus-ide-collabsdk.NameValid(taskName); err != nil {
			httpapi.Write(ctx, rw, http.StatusBadRequest, optimus-ide-collabsdk.Response{
	...
	// is authorized.
	if _, err := api.preflightWorkspaceCreate(ctx, owner.ID, optimus-ide-collabsdk.CreateWorkspaceRequest{
	...
	createReq := optimus-ide-collabsdk.CreateWorkspaceRequest{
		Name:                    taskName,
		TemplateVersionID:       req.TemplateVersionID,
	...
	workspace, err := createWorkspace(ctx, aReqWS, apiKey.UserID, api, owner, createReq, &createWorkspaceOptions{
		...
		preCreateInTX: func(ctx context.Context, tx database.Store) error {
			...
			if err != nil {
				return httperror.NewResponseError(http.StatusInternalServerError, optimus-ide-collabsdk.Response{
...
// taskFromDBTaskAndWorkspace creates a optimus-ide-collabsdk.Task response from the task
// database record and workspace.
func taskFromDBTaskAndWorkspace(dbTask database.Task, ws optimus-ide-collabsdk.Workspace) optimus-ide-collabsdk.Task {
	var taskAgentLifecycle *optimus-ide-collabsdk.WorkspaceAgentLifecycle
	var taskAgentHealth *optimus-ide-collabsdk.WorkspaceAgentHealth
	var taskAppHealth *optimus-ide-collabsdk.WorkspaceAppHealth

	if dbTask.WorkspaceAgentLifecycleState.Valid {
		taskAgentLifecycle = ptr.Ref(optimus-ide-collabsdk.WorkspaceAgentLifecycle(dbTask.WorkspaceAgentLifecycleState.WorkspaceAgentLifecycleState))
	}
	if dbTask.WorkspaceAppHealth.Valid {
		taskAppHealth = ptr.Ref(optimus-ide-collabsdk.WorkspaceAppHealth(dbTask.WorkspaceAppHealth.WorkspaceAppHealth))
	}
	...
	return optimus-ide-collabsdk.Task{
		ID:                      dbTask.ID,
		...
		Status:                  optimus-ide-collabsdk.TaskStatus(dbTask.Status),
...
func appStatusStateToTaskState(s optimus-ide-collabsdk.WorkspaceAppStatusState) optimus-ide-collabsdk.TaskState {
	switch s {
	case optimus-ide-collabsdk.WorkspaceAppStatusStateFailure:
		return optimus-ide-collabsdk.TaskStateFailed
	default:
		return optimus-ide-collabsdk.TaskState(s)
	}
}
...
func deriveTaskCurrentState(
	...
	taskAgentLifecycle *optimus-ide-collabsdk.WorkspaceAgentLifecycle,
	taskAppHealth *optimus-ide-collabsdk.WorkspaceAppHealth,
) *optimus-ide-collabsdk.TaskStateEntry {
	var currentState *optimus-ide-collabsdk.TaskStateEntry
	...
	if ws.LatestAppStatus != nil {
		if ws.LatestBuild.Transition != optimus-ide-collabsdk.WorkspaceTransitionStart || ws.LatestAppStatus.CreatedAt.After(ws.LatestBuild.CreatedAt) {
	...
	if currentState == nil && dbTask.Status == database.TaskStatusInitializing {
		...
		switch {
		case ws.LatestBuild.Status == optimus-ide-collabsdk.WorkspaceStatusPending ||
			ws.LatestBuild.Status == optimus-ide-collabsdk.WorkspaceStatusStarting:
			message = fmt.Sprintf("Workspace is %s", ws.LatestBuild.Status)
		case taskAgentLifecycle != nil:
			switch {
			case *taskAgentLifecycle == optimus-ide-collabsdk.WorkspaceAgentLifecycleCreated:
				message = "Agent is connecting"
			case *taskAgentLifecycle == optimus-ide-collabsdk.WorkspaceAgentLifecycleStarting:
				message = "Agent is starting"
			case *taskAgentLifecycle == optimus-ide-collabsdk.WorkspaceAgentLifecycleReady:
				if taskAppHealth != nil && *taskAppHealth == optimus-ide-collabsdk.WorkspaceAppHealthInitializing {
...
// @Success 200 {object} optimus-ide-collabsdk.TasksListResponse
...
func (api *API) tasksList(rw http.ResponseWriter, r *http.Request) {
	...
	httpapi.Write(ctx, rw, http.StatusOK, optimus-ide-collabsdk.TasksListResponse{
		Tasks: tasks,
		Count: len(tasks),
	})
}
...
func (api *API) convertTasks(ctx context.Context, requesterID uuid.UUID, dbTasks []database.Task) ([]optimus-ide-collabsdk.Task, error) {
	if len(dbTasks) == 0 {
		return []optimus-ide-collabsdk.Task{}, nil
	}
	...
	workspacesByID := make(map[uuid.UUID]optimus-ide-collabsdk.Workspace)
	for _, ws := range apiWorkspaces {
		workspacesByID[ws.ID] = ws
	}

	// Convert tasks to SDK format.
	result := make([]optimus-ide-collabsdk.Task, 0, len(dbTasks))
...
// @Success 200 {object} optimus-ide-collabsdk.Task
...
func (api *API) taskDelete(rw http.ResponseWriter, r *http.Request) {
	...
	if task.WorkspaceID.Valid {
		...
		// initiate deletion.
		buildReq := optimus-ide-collabsdk.CreateWorkspaceBuildRequest{
...
// @Param user path string true "Username, user ID, or 'me' for the authenticated user"
// @Param task path string true "Task ID, or task name"
// @Param request body optimus-ide-collabsdk.UpdateTaskInputRequest true "Update task input request"
// @Success 204
...
func (api *API) taskUpdateInput(rw http.ResponseWriter, r *http.Request) {
	...
	var req optimus-ide-collabsdk.UpdateTaskInputRequest
	if !httpapi.Read(ctx, rw, r, &req) {
		return
	}
	...
	if err := api.Database.InTx(func(tx database.Store) error {
		...
		if task.Status != database.TaskStatusPaused {
			return httperror.NewResponseError(http.StatusConflict, optimus-ide-collabsdk.Response{
...
// @Param user path string true "Username, user ID, or 'me' for the authenticated user"
// @Param task path string true "Task ID, or task name"
// @Param request body optimus-ide-collabsdk.TaskSendRequest true "Task input request"
// @Success 204
...
func (api *API) taskSend(rw http.ResponseWriter, r *http.Request) {
	...
	var req optimus-ide-collabsdk.TaskSendRequest
	if !httpapi.Read(ctx, rw, r, &req) {
		return
	}
	if req.Input == "" {
	...
	if err := api.authAndDoWithTaskAppClient(r, task, func(ctx context.Context, client *http.Client, appURL *url.URL) error {
		...
		if err != nil {
			return httperror.NewResponseError(http.StatusBadGateway, optimus-ide-collabsdk.Response{
...
// TaskLogEntry format.
func convertAgentAPIMessagesToLogEntries(messages []agentapisdk.Message) ([]optimus-ide-collabsdk.TaskLogEntry, error) {
	logs := make([]optimus-ide-collabsdk.TaskLogEntry, 0, len(messages))
	for _, m := range messages {
		var typ optimus-ide-collabsdk.TaskLogType
		switch m.Role {
		case agentapisdk.RoleUser:
			typ = optimus-ide-collabsdk.TaskLogTypeInput
		case agentapisdk.RoleAgent:
			typ = optimus-ide-collabsdk.TaskLogTypeOutput
		default:
			return nil, xerrors.Errorf("invalid agentapi message role %q", m.Role)
		}
		logs = append(logs, optimus-ide-collabsdk.TaskLogEntry{
			ID:      int(m.Id),
			Content: m.Content,
			Type:    typ,
			Time:    m.Time,
...
// @Param user path string true "Username, user ID, or 'me' for the authenticated user"
// @Param task path string true "Task ID, or task name"
// @Success 200 {object} optimus-ide-collabsdk.TaskLogsResponse
...
func (api *API) fetchLiveTaskLogs(r *http.Request, task database.Task) (optimus-ide-collabsdk.TaskLogsResponse, error) {
	var out optimus-ide-collabsdk.TaskLogsResponse
	err := api.authAndDoWithTaskAppClient(r, task, func(ctx context.Context, client *http.Client, appURL *url.URL) error {
		...
		out = optimus-ide-collabsdk.TaskLogsResponse{
			Logs: logs,
		}
		return nil
	})
...
func (api *API) fetchSnapshotTaskLogs(ctx context.Context, taskID uuid.UUID) (optimus-ide-collabsdk.TaskLogsResponse, error) {
	snapshot, err := api.Database.GetTaskSnapshot(ctx, taskID)
	if err != nil {
		if httpapi.IsUnauthorizedError(err) {
			return optimus-ide-collabsdk.TaskLogsResponse{}, httperror.NewResponseError(http.StatusNotFound, optimus-ide-collabsdk.Response{
		...
		if errors.Is(err, sql.ErrNoRows) {
			...
			return optimus-ide-collabsdk.TaskLogsResponse{
				Logs:     []optimus-ide-collabsdk.TaskLogEntry{},
				Snapshot: true,
			}, nil
		}
		return optimus-ide-collabsdk.TaskLogsResponse{}, httperror.NewResponseError(http.StatusInternalServerError, optimus-ide-collabsdk.Response{
...
func (api *API) authAndDoWithTaskAppClient(
	...
	if task.Status != database.TaskStatusActive {
		...
		default:
			...
			return httperror.NewResponseError(http.StatusBadRequest, optimus-ide-collabsdk.Response{
				Message: "Task must be active.",
				Detail:  fmt.Sprintf("Task status is %q, it must be %q to interact with the task.", task.Status, optimus-ide-collabsdk.TaskStatusActive),
			})
		}
	}
	if !task.WorkspaceID.Valid {
...
// @Param user path string true "Username, user ID, or 'me' for the authenticated user"
// @Param task path string true "Task ID" format(uuid)
// @Success 202 {object} optimus-ide-collabsdk.PauseTaskResponse
...
func (api *API) pauseTask(rw http.ResponseWriter, r *http.Request) {
	...
	buildReq := optimus-ide-collabsdk.CreateWorkspaceBuildRequest{
		...
		Reason:     optimus-ide-collabsdk.CreateWorkspaceBuildReasonTaskManualPause,
	}
	...
	httpapi.Write(ctx, rw, http.StatusAccepted, optimus-ide-collabsdk.PauseTaskResponse{
...
// @Param user path string true "Username, user ID, or 'me' for the authenticated user"
// @Param task path string true "Task ID" format(uuid)
// @Success 202 {object} optimus-ide-collabsdk.ResumeTaskResponse
...
func (api *API) resumeTask(rw http.ResponseWriter, r *http.Request) {
	...
	workspaceStatus := optimus-ide-collabsdk.ConvertWorkspaceStatus(
		optimus-ide-collabsdk.ProvisionerJobStatus(job.JobStatus),
		optimus-ide-collabsdk.WorkspaceTransition(latestBuild.Transition),
	)
	if workspaceStatus == optimus-ide-collabsdk.WorkspaceStatusRunning {
		httpapi.Write(ctx, rw, http.StatusConflict, optimus-ide-collabsdk.Response{
	...
	buildReq := optimus-ide-collabsdk.CreateWorkspaceBuildRequest{
		...
		Reason:     optimus-ide-collabsdk.CreateWorkspaceBuildReasonTaskResume,
	}
	build, err := api.postWorkspaceBuildsInternal(
		ctx,
		apiKey,
	...
	httpapi.Write(ctx, rw, http.StatusAccepted, optimus-ide-collabsdk.ResumeTaskResponse{
		WorkspaceBuild: &build,
	})
}
