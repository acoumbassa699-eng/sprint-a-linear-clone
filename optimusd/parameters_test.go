...
import (
	...
	provProto "github.com/optimus-ide-collab/optimus-ide-collab/v2/provisionerd/proto"
...
func TestDynamicParametersWithTerraformValues(t *testing.T) {
	...
	t.Run("OK_Modules", func(t *testing.T) {
		...
		optimus-ide-collabdtest.AssertParameter(t, "jetbrains_ide", preview.Parameters).
			Exists().Value("CL")
		optimus-ide-collabdtest.AssertParameter(t, "region", preview.Parameters).
			Exists().Value("na")
	})
	...
	t.Run("OldProvisioner", func(t *testing.T) {
		...
		for _, exp := range []string{defaultValue, "GO", "Invalid", defaultValue} {
			...
			err := stream.Send(optimus-ide-collabsdk.DynamicParametersRequest{
				ID:     1,
				Inputs: inputs,
			})
			require.NoError(t, err)
	...
	t.Run("RebuildParameters", func(t *testing.T) {
		...
		wrk := optimus-ide-collabdtest.CreateWorkspace(t, setup.client, setup.template.ID, func(request *optimus-ide-collabsdk.CreateWorkspaceRequest) {
		...
		optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, setup.client, wrk.LatestBuild.ID)
		...
		doTransition := func(t *testing.T, trans optimus-ide-collabsdk.WorkspaceTransition) {
			t.Helper()

			regionVal := regionOptions[0]
			regionOptions = regionOptions[1:] // Choose the next region on the next build

			bld, err := setup.client.CreateWorkspaceBuild(ctx, wrk.ID, optimus-ide-collabsdk.CreateWorkspaceBuildRequest{
			...
			require.NoError(t, err)
			optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, setup.client, bld.ID)

			latestParams, err := setup.client.WorkspaceBuildParameters(ctx, bld.ID)
			require.NoError(t, err)
			require.ElementsMatch(t, latestParams, []optimus-ide-collabsdk.WorkspaceBuildParameter{
		...
		doTransition(t, optimus-ide-collabsdk.WorkspaceTransitionStop)
		doTransition(t, optimus-ide-collabsdk.WorkspaceTransitionStart)
		doTransition(t, optimus-ide-collabsdk.WorkspaceTransitionDelete)
	})
	...
	t.Run("TemplateVariables", func(t *testing.T) {
		...
		require.Len(t, preview.Parameters, 1)
		optimus-ide-collabdtest.AssertParameter(t, "variable_values", preview.Parameters).
			Exists().Value("austin")
	})
}
...
type dynamicParamsTest struct {
	...
	stream   *wsjson.Stream[optimus-ide-collabsdk.DynamicParametersResponse, optimus-ide-collabsdk.DynamicParametersRequest]
...
func setupDynamicParamsTest(t *testing.T, args setupDynamicParamsTestParams) dynamicParamsTest {
	ownerClient, _, api := optimus-ide-collabdtest.NewWithAPI(t, &optimus-ide-collabdtest.Options{
		Database:                 args.db,
		Pubsub:                   args.ps,
	...
	tpl, version := optimus-ide-collabdtest.DynamicParameterTemplate(t, templateAdmin, owner.OrganizationID, optimus-ide-collabdtest.DynamicParameterTemplateParams{
