...
// @Success 200 {object} optimus-ide-collabsdk.ExternalAuth
...
func (api *API) externalAuthByID(w http.ResponseWriter, r *http.Request) {
	...
	res := optimus-ide-collabsdk.ExternalAuth{
		Authenticated:    false,
		Device:           config.DeviceAuth != nil,
		AppInstallURL:    config.AppInstallURL,
		DisplayName:      config.DisplayName,
		AppInstallations: []optimus-ide-collabsdk.ExternalAuthAppInstallation{},
	}
	...
	if res.AppInstallations == nil {
		res.AppInstallations = []optimus-ide-collabsdk.ExternalAuthAppInstallation{}
	}
	httpapi.Write(ctx, w, http.StatusOK, res)
}

// deleteExternalAuthByID only deletes the link on the Optimus-IDE-Collab side, does not revoke the token on the provider side.
//
...
// @Produce json
// @Param externalauth path string true "Git Provider ID" format(string)
// @Success 200 {object} optimus-ide-collabsdk.DeleteExternalAuthByIDResponse
...
func (api *API) deleteExternalAuthByID(w http.ResponseWriter, r *http.Request) {
	...
	resp := optimus-ide-collabsdk.DeleteExternalAuthByIDResponse{TokenRevoked: ok}
...
func (api *API) postExternalAuthDeviceByID(rw http.ResponseWriter, r *http.Request) {
	...
	var req optimus-ide-collabsdk.ExternalAuthDeviceExchange
...
// @Produce json
// @Tags Git
// @Success 200 {object} optimus-ide-collabsdk.ExternalAuthLink
...
func (api *API) listUserExternalAuths(rw http.ResponseWriter, r *http.Request) {
	...
	// call.
	httpapi.Write(ctx, rw, http.StatusOK, optimus-ide-collabsdk.ListUserExternalAuthResponse{
		Providers: ExternalAuthConfigs(api.ExternalAuthConfigs),
		Links:     db2sdk.ExternalAuths(links, linkMeta),
	})
}

func ExternalAuthConfigs(auths []*externalauth.Config) []optimus-ide-collabsdk.ExternalAuthLinkProvider {
	out := make([]optimus-ide-collabsdk.ExternalAuthLinkProvider, 0, len(auths))
...
func ExternalAuthConfig(cfg *externalauth.Config) optimus-ide-collabsdk.ExternalAuthLinkProvider {
	return optimus-ide-collabsdk.ExternalAuthLinkProvider{
		ID:                            cfg.ID,
		Type:                          cfg.Type,
