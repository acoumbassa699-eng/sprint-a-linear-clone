...
// separately in optimus-ide-collabd/exp_chats_test.go against a real optimus-ide-collabdtest
// server.
...
func TestWatchChatGit(t *testing.T) {
	...
	t.Run("DisconnectedAgentRejected", func(t *testing.T) {
		...
		mDB.EXPECT().GetWorkspaceAgentsInLatestBuildByWorkspaceID(gomock.Any(), workspaceID).
			Return([]database.WorkspaceAgent{
				{
					ID:             subAgentID,
					ParentID:       uuid.NullUUID{UUID: agentID, Valid: true},
					Name:           "dev-container",
	...
	t.Run("BidirectionalProxyWorks", func(t *testing.T) {
		...
		// through the optimus-ide-collabd proxy.
		...
		mAgentConn.EXPECT().WatchGit(gomock.Any(), gomock.Any(), chatID).
			DoAndReturn(func(ctx context.Context, _ slog.Logger, _ uuid.UUID) (*wsjson.Stream[optimus-ide-collabsdk.WorkspaceAgentGitServerMessage, optimus-ide-collabsdk.WorkspaceAgentGitClientMessage], error) {
				...
				// From optimus-ide-collabd's perspective: reads server messages
		...
		// Given: We create the HTTP server.
		optimus-ide-collabdSrv := httptest.NewServer(r)
		defer optimus-ide-collabdSrv.Close()

		// And: Dial the WebSocket as a client.
		wsURL := strings.Replace(optimus-ide-collabdSrv.URL, "http://", "ws://", 1)
		...
		err = agentStream.Send(optimus-ide-collabsdk.WorkspaceAgentGitServerMessage{
			Type:    optimus-ide-collabsdk.WorkspaceAgentGitServerMessageTypeChanges,
			Message: "test-changes",
		})
		require.NoError(t, err)

		serverMsg := testutil.RequireReceive(ctx, t, clientCh)
		require.Equal(t, optimus-ide-collabsdk.WorkspaceAgentGitServerMessageTypeChanges, serverMsg.Type)
		...
		err = clientStream.Send(optimus-ide-collabsdk.WorkspaceAgentGitClientMessage{
			Type: optimus-ide-collabsdk.WorkspaceAgentGitClientMessageTypeRefresh,
		})
		require.NoError(t, err)

		clientMsg := testutil.RequireReceive(ctx, t, agentCh)
		require.Equal(t, optimus-ide-collabsdk.WorkspaceAgentGitClientMessageTypeRefresh, clientMsg.Type)
	...
	t.Run("ClientDisconnectTearsDown", func(t *testing.T) {
		...
		clientDeoptimus-ide-collab := wsjson.NewDeoptimus-ide-collab[optimus-ide-collabsdk.WorkspaceAgentGitServerMessage](clientConn, websocket.MessageText, logger)
		decodeCh := clientDeoptimus-ide-collab.Chan()
...
func TestWatchAgentContainers(t *testing.T) {
	...
	t.Run("Optimus-IDE-CollabdWebSocketCanHandleClientClosing", func(t *testing.T) {
		t.Parallel()
		...
		// response to this issue: https://github.com/optimus-ide-collab/optimus-ide-collab/issues/19449
		...
		var (
			...
			containersCh = make(chan optimus-ide-collabsdk.WorkspaceAgentListContainersResponse)
		...
		mAgentConn.EXPECT().WatchContainers(gomock.Any(), gomock.Any()).
			DoAndReturn(func(_ context.Context, _ slog.Logger) (<-chan optimus-ide-collabsdk.WorkspaceAgentListContainersResponse, io.Closer, error) {
		...
		// And: Create a streaming deoptimus-ide-collab
		deoptimus-ide-collab := wsjson.NewDeoptimus-ide-collab[optimus-ide-collabsdk.WorkspaceAgentListContainersResponse](conn, websocket.MessageText, logger)
		defer deoptimus-ide-collab.Close()
		decodeCh := deoptimus-ide-collab.Chan()

		// And: We can successfully send through the channel.
		testutil.RequireSend(ctx, t, containersCh, optimus-ide-collabsdk.WorkspaceAgentListContainersResponse{
	...
	t.Run("Optimus-IDE-CollabdWebSocketCanHandleAgentClosing", func(t *testing.T) {
		t.Parallel()

		// This test ensures that the agent containers `/watch` websocket can gracefully
		// handle the underlying websocket unexpectedly closing. This test was created in
		// response to this issue: https://github.com/optimus-ide-collab/optimus-ide-collab/issues/19372
...
func TestCreateExternalAuthResponse(t *testing.T) {
	...
	providerTests := []struct {
		...
		{
			name:         "GitHub",
			typ:          optimus-ide-collabsdk.EnhancedExternalAuthProviderGitHub.String(),
			token:        "ghtoken",
		...
		{
			name:         "GitLab",
			typ:          optimus-ide-collabsdk.EnhancedExternalAuthProviderGitLab.String(),
			token:        "gltoken",
			wantUsername: "oauth2",
		...
		{
			name:         "BitbucketCloud",
			typ:          optimus-ide-collabsdk.EnhancedExternalAuthProviderBitBucketCloud.String(),
			token:        "bbtoken",
		...
		},
		{
			...
			typ:          optimus-ide-collabsdk.EnhancedExternalAuthProviderBitBucketServer.String(),
			token:        "bbtoken",
			wantUsername: "x-token-auth",
			wantPassword: "bbtoken",
		},
