...
func TestPagination(t *testing.T) {
	...
	testCases := []struct {
		...
		ExpectedParams optimus-ide-collabsdk.Pagination
	}{
		{
			Name:          "BadAfterID",
			AfterID:       "bogus",
	...
	for _, c := range testCases {
		t.Run(c.Name, func(t *testing.T) {
			...
			params, ok := optimus-ide-collabd.ParsePagination(rw, r)
			if c.ExpectedError == "" {
				...
				err := json.NewDeoptimus-ide-collab(rw.Body).Decode(&apiError)
