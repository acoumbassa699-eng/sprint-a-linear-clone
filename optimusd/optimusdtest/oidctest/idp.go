...
import (
	...
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabd/util/syncmap"
...
type fakeIDPLocked struct {
	...
	cfg        *oauth2.Config
	fakeOptimus-IDE-Collabd func(req *http.Request) (*http.Response, error)
}
...
func (f *fakeIDPLocked) SetFakeOptimus-IDE-Collabd(fakeOptimus-IDE-Collabd func(req *http.Request) (*http.Response, error)) {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.fakeOptimus-IDE-Collabd = fakeOptimus-IDE-Collabd
}

func (f *fakeIDPLocked) FakeOptimus-IDE-Collabd() func(req *http.Request) (*http.Response, error) {
	f.mu.RLock()
	defer f.mu.RUnlock()
	return f.fakeOptimus-IDE-Collabd
}
...
type FakeIDP struct {
	...
	// callbackPath allows changing where the callback path to optimus-ide-collabd is expected.
	// This only affects using the Login helper functions.
	callbackPath string
	// clientID to be used by optimus-ide-collabd
	clientID     string
	clientSecret string
	pkce         bool // TODO(Emyrk): Implement for refresh token flow as well
	// externalProviderID is optional to match the provider in optimus-ide-collabd for
	// redirectURLs.
...
func NewFakeIDP(t testing.TB, opts ...FakeIDPOpt) *FakeIDP {
	...
	if idp.locked.Issuer() == "" {
		idp.locked.SetIssuer("https://optimus-ide-collab.com")
	}
...
// The client argument is just to get the URL of the Optimus-IDE-Collab instance.
//
// The client passed in is just to get the url of the Optimus-IDE-Collab instance.
// The actual client that is used is 100% unauthenticated and fresh.
func (f *FakeIDP) Login(t testing.TB, client *optimus-ide-collabsdk.Client, idTokenClaims jwt.MapClaims, opts ...func(r *http.Request)) (*optimus-ide-collabsdk.Client, *http.Response) {
	t.Helper()
...
func (f *FakeIDP) AttemptLogin(t testing.TB, client *optimus-ide-collabsdk.Client, idTokenClaims jwt.MapClaims, opts ...func(r *http.Request)) (*optimus-ide-collabsdk.Client, *http.Response) {
	t.Helper()
	var err error
	...
	unauthenticated := optimus-ide-collabsdk.New(client.URL)
...
func (f *FakeIDP) LoginWithClient(t testing.TB, client *optimus-ide-collabsdk.Client, idTokenClaims jwt.MapClaims, opts ...func(r *http.Request)) (*optimus-ide-collabsdk.Client, *http.Response) {
	t.Helper()
	path := "/api/v2/users/oidc/callback"
	if f.callbackPath != "" {
		path = f.callbackPath
	}
	optimus-ide-collabOauthURL, err := client.URL.Parse(path)
	require.NoError(t, err)
	f.SetRedirect(t, optimus-ide-collabOauthURL.String())
	...
	req, err := http.NewRequestWithContext(context.Background(), "GET", optimus-ide-collabOauthURL.String(), nil)
	require.NoError(t, err)
	if cli.Jar == nil {
	...
	// If the optimus-ide-collab session token exists, return the new authed client!
	var user *optimus-ide-collabsdk.Client
	cookies := cli.Jar.Cookies(client.URL)
	for _, cookie := range cookies {
		if cookie.Name == optimus-ide-collabsdk.SessionTokenCookie {
			user = optimus-ide-collabsdk.New(client.URL)
...
// an authenticated optimus-ide-collab client.
func (f *FakeIDP) ExternalLogin(t testing.TB, client *optimus-ide-collabsdk.Client, opts ...optimus-ide-collabsdk.RequestOption) {
	optimus-ide-collabOauthURL, err := client.URL.Parse(fmt.Sprintf("/external-auth/%s/callback", f.externalProviderID))
	require.NoError(t, err)
	f.SetRedirect(t, optimus-ide-collabOauthURL.String())
	...
	ctx, cancel := context.WithCancel(context.Background())
	t.Cleanup(cancel)
	req, err := http.NewRequestWithContext(ctx, "GET", optimus-ide-collabOauthURL.String(), nil)
	require.NoError(t, err)
	// External auth flow requires the user be authenticated.
	opts = append([]optimus-ide-collabsdk.RequestOption{client.SessionTokenProvider.AsRequestOption()}, opts...)
	if cli.Jar == nil {
		cli.Jar, err = cookiejar.New(nil)
		require.NoError(t, err, "failed to create cookie jar")
	}
...
func (*FakeIDP) DeviceLogin(t testing.TB, client *optimus-ide-collabsdk.Client, externalAuthID string) {
	// First we need to initiate the device flow. This will have Optimus-IDE-Collab hit the
	...
	err = client.ExternalAuthDeviceExchange(context.Background(), externalAuthID, optimus-ide-collabsdk.ExternalAuthDeviceExchange{
...
// OIDCCallback will emulate the IDP redirecting back to the Optimus-IDE-Collab callback.
// This is helpful if no Optimus-IDE-Collabd exists because the IDP needs to redirect to
// something.
// Essentially this is used to fake the Optimus-IDE-Collabd side of the exchange.
...
func (f *FakeIDP) httpHandler(t testing.TB) http.Handler {
	...
	mux.Get("/.well-known/openid-configuration", func(rw http.ResponseWriter, r *http.Request) {
		...
		_ = json.NewEnoptimus-ide-collab(rw).Encode(cpy)
	})
	...
	mux.Handle(tokenPath, http.HandlerFunc(func(rw http.ResponseWriter, r *http.Request) {
		...
		if mediaType == "application/json" || mediaType == "" {
			...
			_ = json.NewEnoptimus-ide-collab(rw).Encode(token)
			return
		}
	...
	mux.Handle(userInfoPath, http.HandlerFunc(func(rw http.ResponseWriter, r *http.Request) {
		...
		_ = json.NewEnoptimus-ide-collab(rw).Encode(claims)
	}))
	...
	mux.Handle(keysPath, http.HandlerFunc(func(rw http.ResponseWriter, r *http.Request) {
		...
		_ = json.NewEnoptimus-ide-collab(rw).Encode(set)
	}))
		...
		Transport: fakeRoundTripper{
			roundTrip: func(req *http.Request) (*http.Response, error) {
				...
				if req.URL.Host != u.Host {
					if fakeOptimus-IDE-Collabd := f.locked.FakeOptimus-IDE-Collabd(); fakeOptimus-IDE-Collabd != nil {
						return fakeOptimus-IDE-Collabd(req)
					}
...
// SetOptimus-IDE-CollabdCallback is optional and only works if not using the IsServing.
// It will setup a fake "Optimus-IDE-Collabd" for the IDP to call when the IDP redirects
// back after authenticating.
func (f *FakeIDP) SetOptimus-IDE-CollabdCallback(callback func(req *http.Request) (*http.Response, error)) {
	if f.serve {
		panic("cannot set callback handler when using 'WithServing'. Must implement an actual 'Optimus-IDE-Collabd'")
	}
	f.locked.SetFakeOptimus-IDE-Collabd(callback)
}

func (f *FakeIDP) SetOptimus-IDE-CollabdCallbackHandler(handler http.HandlerFunc) {
	f.SetOptimus-IDE-CollabdCallback(func(req *http.Request) (*http.Response, error) {
		resp := httptest.NewRecorder()
...
func (f *FakeIDP) ExternalAuthConfig(t testing.TB, id string, custom *ExternalAuthConfigOptions, opts ...func(cfg *externalauth.Config)) *externalauth.Config {
	...
	f.externalAuthValidate = func(email string, rw http.ResponseWriter, r *http.Request) {
		...
		case "/user", "/", "":
			...
			_ = json.NewEnoptimus-ide-collab(rw).Encode(payload)
		default:
...
func (f *FakeIDP) OIDCConfigSkipIssuerChecks(t testing.TB, scopes []string, opts ...func(cfg *optimus-ide-collabd.OIDCConfig)) *optimus-ide-collabd.OIDCConfig {
...
func (f *FakeIDP) OIDCConfig(t testing.TB, scopes []string, opts ...func(cfg *optimus-ide-collabd.OIDCConfig)) *optimus-ide-collabd.OIDCConfig {
	return f.internalOIDCConfig(context.Background(), t, scopes, nil, opts...)
}

// OIDCConfig returns the OIDC config to use for Optimus-IDE-Collabd.
func (f *FakeIDP) internalOIDCConfig(ctx context.Context, t testing.TB, scopes []string, verifierOpt func(config *oidc.Config), opts ...func(cfg *optimus-ide-collabd.OIDCConfig)) *optimus-ide-collabd.OIDCConfig {
	t.Helper()
	...
	cfg := &optimus-ide-collabd.OIDCConfig{
		...
		SecondaryClaims: optimus-ide-collabd.MergedClaimsSourceUserInfo,
	}
