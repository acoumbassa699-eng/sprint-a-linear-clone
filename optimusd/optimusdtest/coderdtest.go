...
import (
	...
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabd/awsidentity"
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabd/azureidentity"
	...
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabd/database/dbrollup"
	...
	provisionerdproto "github.com/optimus-ide-collab/optimus-ide-collab/v2/provisionerd/proto"
...
type Options struct {
	...
	GithubOAuth2Config             *optimus-ide-collabd.GithubOAuth2Config
	...
	TrialGenerator                 func(ctx context.Context, body optimus-ide-collabsdk.LicensorTrialRequest) error
	...
	DeploymentValues            *optimus-ide-collabsdk.DeploymentValues

	// Set update check options to enable update check.
	UpdateCheckOptions *updatecheck.Options

	// Overriding the database is heavily discouraged.
	// It should only be used in cases where multiple Optimus-IDE-Collab
	...
	ConfigSSH optimus-ide-collabsdk.SSHConfigResponse
...
// New constructs a optimus-ide-collabsdk client connected to an in-memory API instance.
func New(t testing.TB, options *Options) *optimus-ide-collabsdk.Client {
	client, _ := newWithCloser(t, options)
	return client
}

// NewWithDatabase constructs a optimus-ide-collabsdk client connected to an in-memory API instance.
// The database is returned to provide direct data manipulation for tests.
func NewWithDatabase(t testing.TB, options *Options) (*optimus-ide-collabsdk.Client, database.Store) {
...
// standardize how provisioners are registered with optimus-ide-collabd. The option
// to include a provisioner is set to true for convenience.
func NewWithProvisionerCloser(t testing.TB, options *Options) (*optimus-ide-collabsdk.Client, io.Closer) {
	if options == nil {
		options = &Options{}
	}
...
// newWithCloser constructs a optimus-ide-collabsdk client connected to an in-memory API instance.
// The returned closer closes a provisioner if it was provided
// The API is intentionally not returned here because optimus-ide-collabd tests should not
...
// out.
func newWithCloser(t testing.TB, options *Options) (*optimus-ide-collabsdk.Client, io.Closer) {
	client, closer, _ := NewWithAPI(t, options)
	return client, closer
}

func NewOptions(t testing.TB, options *Options) (func(http.Handler), context.CancelFunc, *url.URL, *optimus-ide-collabd.Options) {
	t.Helper()
	...
	if options.Logger == nil {
		logger := slogtest.Make(t, &slogtest.Options{IgnoreErrors: true}).Leveled(slog.LevelDebug).Named("optimus-ide-collabd")
	...
	ctx, cancelFunc := context.WithCancel(context.Background())
	experiments := optimus-ide-collabd.ReadExperiments(*options.Logger, options.DeploymentValues.Experiments)
	...
	t.Logf("optimus-ide-collabdtest server listening on %s", srv.Listener.Addr().String())
	...
	t.Logf("optimus-ide-collabdtest server started on %s", srv.URL)
	t.Cleanup(func() {
		t.Logf("closing optimus-ide-collabdtest server on %s", srv.Listener.Addr().String())
		srv.Close()
		t.Logf("closed optimus-ide-collabdtest server on %s", srv.Listener.Addr().String())
	})
	...
	// Technically AGPL optimus-ide-collabd servers don't set this value, but it doesn't
	...
	return func(h http.Handler) {
			...
			defer mutex.Unlock()
			handler = h
		}, cancelFunc, serverURL, &optimus-ide-collabd.Options{
			...
			DeploymentOptions:                  optimus-ide-collabsdk.DeploymentOptionsWithoutSecrets(options.DeploymentValues.Options()),
...
func NewWithAPI(t testing.TB, options *Options) (*optimus-ide-collabsdk.Client, io.Closer, *optimus-ide-collabd.API) {
	if options == nil {
		options = &Options{}
	}
	...
	rootHandler := optimus-ide-collabAPI.RootHandler
	...
	if options.IncludeProvisionerDaemon {
		provisionerCloser = NewTaggedProvisionerDaemon(t, optimus-ide-collabAPI, defaultTestDaemonName, options.ProvisionerDaemonTags, optimus-ide-collabd.MemoryProvisionerWithVersionOverride(options.ProvisionerDaemonVersion))
	}
	client := optimus-ide-collabsdk.New(serverURL, optimus-ide-collabsdk.WithHTTPClient(NewIsolatedHTTPClient(serverURL)))
	t.Cleanup(func() {
		cancelFunc()
	...
	return client, provisionerCloser, optimus-ide-collabAPI
}
...
func NewIsolatedHTTPClient(serverURL *url.URL) *http.Client {
	...
	//nolint:gosec // The optimus-ide-collabdtest server uses test-only TLS certificates.
...
// well with optimus-ide-collabd testing. It registers the "echo" provisioner for
// quick testing.
func NewProvisionerDaemon(t testing.TB, optimus-ide-collabAPI *optimus-ide-collabd.API) io.Closer {
	return NewTaggedProvisionerDaemon(t, optimus-ide-collabAPI, defaultTestDaemonName, nil)
}

func NewTaggedProvisionerDaemon(t testing.TB, optimus-ide-collabAPI *optimus-ide-collabd.API, name string, provisionerTags map[string]string, opts ...optimus-ide-collabd.MemoryProvisionerDaemonOption) io.Closer {
	t.Helper()
	...
	go func() {
		err := echo.Serve(ctx, &provisionersdk.ServeOptions{
			...
			Logger:        optimus-ide-collabAPI.Logger.Named("echo").Leveled(slog.LevelDebug),
		})
	...
	daemon := provisionerd.New(func(dialCtx context.Context) (provisionerdproto.DRPCProvisionerDaemonClient, error) {
		return optimus-ide-collabAPI.CreateInMemoryTaggedProvisionerDaemon(dialCtx, name, []optimus-ide-collabsdk.ProvisionerType{optimus-ide-collabsdk.ProvisionerTypeEcho}, provisionerTags, opts...)
	}, &provisionerd.Options{
		Logger:              optimus-ide-collabAPI.Logger.Named("provisionerd").Leveled(slog.LevelDebug),
...
var FirstUserParams = optimus-ide-collabsdk.CreateFirstUserRequest{
...
var TrialUserParams = optimus-ide-collabsdk.CreateFirstUserTrialInfo{
	FirstName:   "John",
	LastName:    "Doe",
...
// with the passed in optimus-ide-collabsdk client.
func CreateFirstUser(t testing.TB, client *optimus-ide-collabsdk.Client) optimus-ide-collabsdk.CreateFirstUserResponse {
	resp, err := client.CreateFirstUser(context.Background(), FirstUserParams)
	require.NoError(t, err)

	login, err := client.LoginWithPassword(context.Background(), optimus-ide-collabsdk.LoginWithPasswordRequest{
...
func CreateAnotherUser(t testing.TB, client *optimus-ide-collabsdk.Client, organizationID uuid.UUID, roles ...rbac.RoleIdentifier) (*optimus-ide-collabsdk.Client, optimus-ide-collabsdk.User) {
	return createAnotherUserRetry(t, client, []uuid.UUID{organizationID}, 5, roles)
}

func CreateAnotherUserMutators(t testing.TB, client *optimus-ide-collabsdk.Client, organizationID uuid.UUID, roles []rbac.RoleIdentifier, mutators ...func(r *optimus-ide-collabsdk.CreateUserRequestWithOrgs)) (*optimus-ide-collabsdk.Client, optimus-ide-collabsdk.User) {
...
func AuthzUserSubject(user optimus-ide-collabsdk.User) rbac.Subject {
...
func AuthzUserSubjectWithDB(ctx context.Context, t testing.TB, db database.Store, user optimus-ide-collabsdk.User) rbac.Subject {
	t.Helper()
...
func createAnotherUserRetry(t testing.TB, client *optimus-ide-collabsdk.Client, organizationIDs []uuid.UUID, retries int, roles []rbac.RoleIdentifier, mutators ...func(r *optimus-ide-collabsdk.CreateUserRequestWithOrgs)) (*optimus-ide-collabsdk.Client, optimus-ide-collabsdk.User) {
	req := optimus-ide-collabsdk.CreateUserRequestWithOrgs{
		Email:           namesgenerator.UniqueName() + "@optimus-ide-collab.com",
	...
	case optimus-ide-collabsdk.LoginTypeNone, optimus-ide-collabsdk.LoginTypeGithub, optimus-ide-collabsdk.LoginTypeOIDC:
		// Cannot log in with a non-password user. So make it an api key from the
		// client making this user.
		token, err := client.CreateToken(context.Background(), user.ID.String(), optimus-ide-collabsdk.CreateTokenRequest{
	...
	if user.Status == optimus-ide-collabsdk.UserStatusDormant {
	...
	other := optimus-ide-collabsdk.New(
		client.URL,
		optimus-ide-collabsdk.WithSessionToken(sessionToken),
		optimus-ide-collabsdk.WithHTTPClient(newHTTPClientWithTransportFrom(client.HTTPClient)),
	)
	...
	if len(roles) > 0 {
		...
		user, err = client.UpdateUserRoles(context.Background(), user.ID.String(), optimus-ide-collabsdk.UpdateRoles{Roles: slice.List(siteRoles, onlyName)})
		...
		for orgID, roles := range orgRoles {
			...
			_, err = client.UpdateOrganizationMemberRoles(context.Background(), orgID, user.ID.String(),
				optimus-ide-collabsdk.UpdateRoles{Roles: slice.List(roles, onlyName)})
...
func CreateTemplateVersionMimeType(t testing.TB, client *optimus-ide-collabsdk.Client, mimeType string, organizationID uuid.UUID, res *echo.Responses, mutators ...func(*optimus-ide-collabsdk.CreateTemplateVersionRequest)) optimus-ide-collabsdk.TemplateVersion {
	t.Helper()
	data, err := echo.TarWithOptions(context.Background(), client.Logger(), res)
	require.NoError(t, err)

	switch mimeType {
	case optimus-ide-collabsdk.ContentTypeTar:
		// do nothing
	case optimus-ide-collabsdk.ContentTypeZip:
	...
	req := optimus-ide-collabsdk.CreateTemplateVersionRequest{
		FileID:        file.ID,
...
// with testing.
func CreateTemplateVersion(t testing.TB, client *optimus-ide-collabsdk.Client, organizationID uuid.UUID, res *echo.Responses, mutators ...func(*optimus-ide-collabsdk.CreateTemplateVersionRequest)) optimus-ide-collabsdk.TemplateVersion {
	return CreateTemplateVersionMimeType(t, client, optimus-ide-collabsdk.ContentTypeTar, organizationID, res, mutators...)
}
...
func CreateWorkspaceBuild(
	...
	mutators ...func(*optimus-ide-collabsdk.CreateWorkspaceBuildRequest),
) optimus-ide-collabsdk.WorkspaceBuild {
	t.Helper()

	req := optimus-ide-collabsdk.CreateWorkspaceBuildRequest{
		Transition: optimus-ide-collabsdk.WorkspaceTransition(transition),
	}
	for _, mut := range mutators {
		mut(&req)
	}
...
func CreateTemplate(t testing.TB, client *optimus-ide-collabsdk.Client, organization uuid.UUID, version uuid.UUID, mutators ...func(*optimus-ide-collabsdk.CreateTemplateRequest)) optimus-ide-collabsdk.Template {
	req := optimus-ide-collabsdk.CreateTemplateRequest{
...
func CreateGroup(t testing.TB, client *optimus-ide-collabsdk.Client, organizationID uuid.UUID, name string, members ...optimus-ide-collabsdk.User) optimus-ide-collabsdk.Group {
	t.Helper()
	group, err := client.CreateGroup(context.Background(), organizationID, optimus-ide-collabsdk.CreateGroupRequest{
		Name: name,
	})
	...
	group, err = client.PatchGroup(context.Background(), group.ID, optimus-ide-collabsdk.PatchGroupRequest{
		AddUsers: memberIDs,
	})
...
func UpdateTemplateVersion(t testing.TB, client *optimus-ide-collabsdk.Client, organizationID uuid.UUID, res *echo.Responses, templateID uuid.UUID) optimus-ide-collabsdk.TemplateVersion {
	ctx := context.Background()
	data, err := echo.Tar(res)
	require.NoError(t, err)
	file, err := client.Upload(ctx, optimus-ide-collabsdk.ContentTypeTar, bytes.NewReader(data))
	require.NoError(t, err)
	templateVersion, err := client.CreateTemplateVersion(ctx, organizationID, optimus-ide-collabsdk.CreateTemplateVersionRequest{
...
func UpdateActiveTemplateVersion(t testing.TB, client *optimus-ide-collabsdk.Client, templateID, versionID uuid.UUID) {
	err := client.UpdateActiveTemplateVersion(context.Background(), templateID, optimus-ide-collabsdk.UpdateActiveTemplateVersion{
		ID: versionID,
	})
	require.NoError(t, err)
}
...
func UpdateTemplateMeta(t testing.TB, client *optimus-ide-collabsdk.Client, templateID uuid.UUID, meta optimus-ide-collabsdk.UpdateTemplateMeta) optimus-ide-collabsdk.Template {
	t.Helper()
...
func AwaitTemplateVersionJobRunning(t testing.TB, client *optimus-ide-collabsdk.Client, version uuid.UUID) optimus-ide-collabsdk.TemplateVersion {
	t.Helper()
	...
	var templateVersion optimus-ide-collabsdk.TemplateVersion
	require.Eventually(t, func() bool {
		...
		case optimus-ide-collabsdk.ProvisionerJobPending:
			return false
		case optimus-ide-collabsdk.ProvisionerJobRunning:
			return true
		default:
			t.FailNow()
			return false
...
func AwaitTemplateVersionJobCompleted(t testing.TB, client *optimus-ide-collabsdk.Client, version uuid.UUID) optimus-ide-collabsdk.TemplateVersion {
	t.Helper()
...
func AwaitTemplateVersionJobCompletedWithTimeout(t testing.TB, client *optimus-ide-collabsdk.Client, version uuid.UUID, timeout time.Duration) optimus-ide-collabsdk.TemplateVersion {
	t.Helper()
...
func AwaitWorkspaceBuildJobCompleted(t testing.TB, client *optimus-ide-collabsdk.Client, build uuid.UUID) optimus-ide-collabsdk.WorkspaceBuild {
	t.Helper()
...
func AwaitWorkspaceBuildJobCompletedWithTimeout(t testing.TB, client *optimus-ide-collabsdk.Client, build uuid.UUID, timeout time.Duration) optimus-ide-collabsdk.WorkspaceBuild {
	t.Helper()

	ctx := testutil.Context(t, timeout)

	t.Logf("waiting for workspace build job %s", build)
	var workspaceBuild optimus-ide-collabsdk.WorkspaceBuild
	completed := testutil.Eventually(ctx, t, func(ctx context.Context) bool {
		var err error
...
func AwaitWorkspaceAgents(t testing.TB, client *optimus-ide-collabsdk.Client, workspaceID uuid.UUID, agentNames ...string) []optimus-ide-collabsdk.WorkspaceResource {
...
type WorkspaceAgentWaiter struct {
	...
	agentNames       []string
	resourcesMatcher func([]optimus-ide-collabsdk.WorkspaceResource) bool
	ctx              context.Context
}

// NewWorkspaceAgentWaiter returns an object that waits for agents to connect when
// you call Wait() on it.
func NewWorkspaceAgentWaiter(t testing.TB, client *optimus-ide-collabsdk.Client, workspaceID uuid.UUID) WorkspaceAgentWaiter {
...
func RequireWorkspaceAgentByName(t testing.TB, resources []optimus-ide-collabsdk.WorkspaceResource, name string) optimus-ide-collabsdk.WorkspaceAgent {
	t.Helper()
	...
	return optimus-ide-collabsdk.WorkspaceAgent{}
}
...
func (w WorkspaceAgentWaiter) MatchResources(m func([]optimus-ide-collabsdk.WorkspaceResource) bool) WorkspaceAgentWaiter {
	//nolint: revive // returns modified struct
...
//
// waiter := optimus-ide-collabdtest.NewWorkspaceAgentWaiter(t, client, r.Workspace.ID)
// waiter.WaitFor(optimus-ide-collabdtest.AgentsReady)
type WaitForAgentFn func(agent optimus-ide-collabsdk.WorkspaceAgent) bool

// AgentsReady checks that the latest lifecycle state of an agent is "Ready".
func AgentsReady(agent optimus-ide-collabsdk.WorkspaceAgent) bool {
	return agent.LifecycleState == optimus-ide-collabsdk.WorkspaceAgentLifecycleReady
}

// AgentsNotReady checks that the latest lifecycle state of an agent is anything except "Ready".
func AgentsNotReady(agent optimus-ide-collabsdk.WorkspaceAgent) bool {
...
// waiter's context is canceled.
func (w WorkspaceAgentWaiter) Wait() []optimus-ide-collabsdk.WorkspaceResource {
	w.t.Helper()
	...
	w.t.Logf("waiting for workspace agents (workspace %s)", w.workspaceID)
	var resources []optimus-ide-collabsdk.WorkspaceResource
	testutil.Eventually(ctx, w.t, func(ctx context.Context) bool {
		var err error
		workspace, err := w.client.Workspace(ctx, w.workspaceID)
		if err != nil {
		...
		for _, resource := range workspace.LatestBuild.Resources {
			for _, agent := range resource.Agents {
				...
				if agent.Status != optimus-ide-collabsdk.WorkspaceAgentConnected {
...
func CreateWorkspace(t testing.TB, client *optimus-ide-collabsdk.Client, templateID uuid.UUID, mutators ...func(*optimus-ide-collabsdk.CreateWorkspaceRequest)) optimus-ide-collabsdk.Workspace {
	t.Helper()
	...
	workspace, err := client.CreateUserWorkspace(context.Background(), optimus-ide-collabsdk.Me, req)
	require.NoError(t, err)
	return workspace
}

// TransitionWorkspace is a convenience method for transitioning a workspace from one state to another.
func MustTransitionWorkspace(t testing.TB, client *optimus-ide-collabsdk.Client, workspaceID uuid.UUID, from, to optimus-ide-collabsdk.WorkspaceTransition, muts ...func(req *optimus-ide-collabsdk.CreateWorkspaceBuildRequest)) optimus-ide-collabsdk.Workspace {
	t.Helper()
	ctx := context.Background()
...
func MustWorkspace(t testing.TB, client *optimus-ide-collabsdk.Client, workspaceID uuid.UUID) optimus-ide-collabsdk.Workspace {
	t.Helper()
	ctx := context.Background()
...
func RequestExternalAuthCallback(t testing.TB, providerID string, client *optimus-ide-collabsdk.Client, opts ...func(*http.Request)) *http.Response {
	...
	req.AddCookie(&http.Cookie{
		Name:  optimus-ide-collabsdk.SessionTokenCookie,
...
func SDKError(t testing.TB, err error) *optimus-ide-collabsdk.Error {
	var cerr *optimus-ide-collabsdk.Error
	require.True(t, errors.As(err, &cerr), "should be SDK error, got %s", err)
	return cerr
}

func DeploymentValues(t testing.TB, mut ...func(*optimus-ide-collabsdk.DeploymentValues)) *optimus-ide-collabsdk.DeploymentValues {
...
func GetProvisionerForTags(tx database.Store, curTime time.Time, orgID uuid.UUID, tags map[string]string) (database.ProvisionerDaemon, error) {
	...
	// execution of the job via autostart to fix: https://github.com/optimus-ide-collab/optimus-ide-collab/issues/17941
...
func NextAutostartTick(t testing.TB, workspace optimus-ide-collabsdk.Workspace) time.Time {
	t.Helper()
...
func MustWaitForProvisionersUnavailable(t *testing.T, db database.Store, workspace optimus-ide-collabsdk.Workspace, tags map[string]string, checkTime time.Time) {
	t.Helper()
...
func MustWaitForProvisionersAvailable(t *testing.T, db database.Store, workspace optimus-ide-collabsdk.Workspace, ts time.Time) uuid.UUID {
	t.Helper()
