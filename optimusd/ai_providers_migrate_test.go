...
func TestSeedAIProvidersFromEnv(t *testing.T) {
	...
	t.Run("EmptyConfigNoOp", func(t *testing.T) {
		...
		err := optimus-ide-collabd.SeedAIProvidersFromEnv(ctx, db, optimus-ide-collabsdk.AIBridgeConfig{}, testLogger(t))
	...
	t.Run("LegacyOpenAI", func(t *testing.T) {
		...
		cfg := optimus-ide-collabsdk.AIBridgeConfig{
			LegacyOpenAI: optimus-ide-collabsdk.AIBridgeOpenAIConfig{
				BaseURL: serpent.String("https://api.openai.com/v1"),
				Key:     serpent.String("sk-legacy"),
			},
		}
		var firstSeedLogs bytes.Buffer
		err := optimus-ide-collabd.SeedAIProvidersFromEnv(ctx, db, cfg, capturedLogger(&firstSeedLogs))
		require.NoError(t, err)
		...
		err = optimus-ide-collabd.SeedAIProvidersFromEnv(ctx, db, cfg, capturedLogger(&rerunLogs))
		require.NoError(t, err)
	...
	t.Run("DriftFailsStartup", func(t *testing.T) {
		...
		require.NoError(t, optimus-ide-collabd.SeedAIProvidersFromEnv(ctx, db, cfg, testLogger(t)))
		...
		err := optimus-ide-collabd.SeedAIProvidersFromEnv(ctx, db, cfg, testLogger(t))
		require.Error(t, err)
