...
func TestMetrics(t *testing.T) {
	...
	expected := map[string]func(metric *dto.Metric, series string) bool{
		"optimus-ide-collabd_notifications_dispatch_attempts_total": func(metric *dto.Metric, series string) bool {
			...
			for result, val := range results {
				...
				if debug {
					t.Logf("optimus-ide-collabd_notifications_dispatch_attempts_total{result=%q} == %v: %v", result, val, metric.Counter.GetValue())
				}
		...
		"optimus-ide-collabd_notifications_retry_count": func(metric *dto.Metric, series string) bool {
			...
			if debug {
				t.Logf("optimus-ide-collabd_notifications_retry_count == %v: %v", maxAttempts-1, metric.Counter.GetValue())
			}

			// 1 original attempts + 2 retries = maxAttempts
			return metric.Counter.GetValue() == maxAttempts-1
		},
		"optimus-ide-collabd_notifications_queued_seconds": func(metric *dto.Metric, series string) bool {
			...
			if debug {
				t.Logf("optimus-ide-collabd_notifications_queued_seconds > 0: %v", metric.Histogram.GetSampleSum())
			}
		...
		"optimus-ide-collabd_notifications_dispatcher_send_seconds": func(metric *dto.Metric, series string) bool {
			assert.Truef(t, hasMatchingFingerprint(metric, methodFP) || hasMatchingFingerprint(metric, methodFPWithInbox), "found unexpected series %q", series)

			if debug {
				t.Logf("optimus-ide-collabd_notifications_dispatcher_send_seconds > 0: %v", metric.Histogram.GetSampleSum())
			}
		...
		"optimus-ide-collabd_notifications_inflight_dispatches": func(metric *dto.Metric, series string) bool {
		...
		"optimus-ide-collabd_notifications_pending_updates": func(metric *dto.Metric, series string) bool {
			// This is a gauge, so it can be difficult to get the timing right to catch it.
			// See TestPendingUpdatesMetric for a more precise test.
			return true
		},
		"optimus-ide-collabd_notifications_synced_updates_total": func(metric *dto.Metric, _ string) bool {
			if debug {
				t.Logf("optimus-ide-collabd_notifications_synced_updates_total = %v: %v", maxAttempts+1, metric.Counter.GetValue())
			}
