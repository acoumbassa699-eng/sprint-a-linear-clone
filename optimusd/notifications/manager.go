...
//
// The notifier dequeues messages from the store _OPTIMUS-IDE-COLLAB_NOTIFICATIONS_LEASE_COUNT_ at a time and concurrently "dispatches"
...
// of size OPTIMUS-IDE-COLLAB_NOTIFICATIONS_STORE_SYNC_INTERVAL in the Manager, and updates are sent to the store about which messages
// succeeded or failed every OPTIMUS-IDE-COLLAB_NOTIFICATIONS_STORE_SYNC_INTERVAL seconds.
...
type Manager struct {
	cfg optimus-ide-collabsdk.NotificationsConfig
...
// access URL etc.
func NewManager(cfg optimus-ide-collabsdk.NotificationsConfig, store Store, ps pubsub.Pubsub, helpers template.FuncMap, metrics *Metrics, log slog.Logger, opts ...ManagerOption) (*Manager, error) {
...
func defaultHandlers(cfg optimus-ide-collabsdk.NotificationsConfig, log slog.Logger, store Store, ps pubsub.Pubsub) map[database.NotificationMethod]Handler {
