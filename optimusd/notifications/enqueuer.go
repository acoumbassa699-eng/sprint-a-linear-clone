...
func NewStoreEnqueuer(cfg optimus-ide-collabsdk.NotificationsConfig, store Store, helpers template.FuncMap, log slog.Logger, clock quartz.Clock) (*StoreEnqueuer, error) {
...
func (s *StoreEnqueuer) EnqueueWithData(ctx context.Context, userID, templateID uuid.UUID, labels map[string]string, data map[string]any, createdBy string, targets ...uuid.UUID) ([]uuid.UUID, error) {
	...
	for _, method := range methods {
		...
		// We do not want password reset notifications to end up in Optimus-IDE-Collab Inbox.
