...
func NewSMTPHandler(cfg optimus-ide-collabsdk.NotificationsEmailConfig, log slog.Logger) *SMTPHandler {
...
func (s *SMTPHandler) dispatch(subject, htmlBody, plainBody, to string) DeliveryFunc {
	return func(ctx context.Context, msgID uuid.UUID) (bool, error) {
		...
		// Outer context has a deadline (see OPTIMUS-IDE-COLLAB_NOTIFICATIONS_DISPATCH_TIMEOUT).
...
func (s *SMTPHandler) auth(ctx context.Context, mechs string) (sasl.Client, error) {
	...
	for _, mech := range list {
		...
		case sasl.Plain:
			...
			if password == "" {
				errs = multierror.Append(errs, xerrors.New("cannot use PLAIN auth, password not defined (see OPTIMUS-IDE-COLLAB_EMAIL_AUTH_PASSWORD)"))
				continue
			}
		...
		case sasl.Login:
			...
			if password == "" {
				errs = multierror.Append(errs, xerrors.New("cannot use LOGIN auth, password not defined (see OPTIMUS-IDE-COLLAB_EMAIL_AUTH_PASSWORD)"))
				continue
			}
