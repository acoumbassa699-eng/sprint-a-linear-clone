...
func TestAuditLogDescription(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name: "chat_archived",
			alog: chatAuditLogRow(t, optimus-ide-collabsdk.AuditDiff{
		...
		},
		{
			...
			alog: func() database.GetAuditLogsOffsetRow {
				row := chatAuditLogRow(t, optimus-ide-collabsdk.AuditDiff{
					"archived": {Old: false, New: true},
				})
...
func chatAuditLogRow(t *testing.T, diff optimus-ide-collabsdk.AuditDiff) database.GetAuditLogsOffsetRow {
	t.Helper()
