...
func TestTemplateVersion(t *testing.T) {
	...
	t.Run("Get", func(t *testing.T) {
		...
		authz := optimus-ide-collabdtest.AssertRBAC(t, api, client).Reset()

		version := optimus-ide-collabdtest.CreateTemplateVersion(t, client, user.OrganizationID, nil, func(req *optimus-ide-collabsdk.CreateTemplateVersionRequest) {
			req.Name = "bananas"
		...
		if assert.Equal(t, tv.Job.Status, optimus-ide-collabsdk.ProvisionerJobPending) {
	...
	t.Run("Message limit exceeded", func(t *testing.T) {
		t.Parallel()
		client, _, _ := optimus-ide-collabdtest.NewWithAPI(t, nil)
		user := optimus-ide-collabdtest.CreateFirstUser(t, client)

		ctx, cancel := context.WithTimeout(context.Background(), testutil.WaitLong)
		defer cancel()

		file, err := client.Upload(ctx, optimus-ide-collabsdk.ContentTypeTar, bytes.NewReader([]byte{}))
		require.NoError(t, err)
		_, err = client.CreateTemplateVersion(ctx, user.OrganizationID, optimus-ide-collabsdk.CreateTemplateVersionRequest{
			Name:          "bananas",
...
func TestPostTemplateVersionsByOrganization(t *testing.T) {
	...
	t.Run("WithParameters", func(t *testing.T) {
		...
		client, db := optimus-ide-collabdtest.NewWithDatabase(t, &optimus-ide-collabdtest.Options{IncludeProvisionerDaemon: true, Auditor: auditor})
		user := optimus-ide-collabdtest.CreateFirstUser(t, client)
		...
		require.NoError(t, err)
		version, err := client.CreateTemplateVersion(ctx, user.OrganizationID, optimus-ide-collabsdk.CreateTemplateVersionRequest{
			Name:          "bananas",
			StorageMethod: optimus-ide-collabsdk.ProvisionerStorageMethodFile,
			FileID:        file.ID,
			Provisioner:   optimus-ide-collabsdk.ProvisionerTypeEcho,
		...
		if assert.Equal(t, version.Job.Status, optimus-ide-collabsdk.ProvisionerJobPending) {
		...
		require.NoError(t, err)
		tvDB, err := db.GetTemplateVersionByID(dbauthz.As(ctx, optimus-ide-collabdtest.AuthzUserSubject(admin)), version.ID)
		require.NoError(t, err)
	...
	t.Run("Example", func(t *testing.T) {
		...
		tv, err := client.CreateTemplateVersion(ctx, user.OrganizationID, optimus-ide-collabsdk.CreateTemplateVersionRequest{
			Name:          "my-example",
			StorageMethod: optimus-ide-collabsdk.ProvisionerStorageMethodFile,
			ExampleID:     ls[0].ID,
			Provisioner:   optimus-ide-collabsdk.ProvisionerTypeEcho,
		...
		admin, err := client.User(ctx, user.UserID.String())
		require.NoError(t, err)
		tvDB, err := db.GetTemplateVersionByID(dbauthz.As(ctx, optimus-ide-collabdtest.AuthzUserSubject(admin)), tv.ID)
		require.NoError(t, err)
	...
	t.Run("WorkspaceTags", func(t *testing.T) {
		t.Parallel()
		// This test ensures that when creating a template version from an archive continaining a optimus-ide-collab_workspace_tags
		...
		templateAdmin, templateAdminUser := optimus-ide-collabdtest.CreateAnotherUser(t, client, owner.OrganizationID, rbac.RoleTemplateAdmin())
		...
		for _, tt := range []struct {
			...
			variables   []optimus-ide-collabsdk.VariableValue
			expectError string
		}{
			{
				name:     "empty",
			...
			},
			{
				...
				files: map[string]string{
					`main.tf`: `
						...
						data "optimus-ide-collab_parameter" "b" {
							name = "b"
							type = string
							default = "2"
						}
					...
					`main.tf`: `
						...
						data "optimus-ide-collab_workspace_tags" "tags" {
							tags = {
								"foo": "bar",
								"a": var.a,
								"b": data.optimus-ide-collab_parameter.b.value,
							}
						}`,
				},
			...
			// FIXME(cian): we should skip evaluating tags for which values have already been provided.
			{
				...
				variables: []optimus-ide-collabsdk.VariableValue{{Name: "a", Value: "b"}},
			},
			{
			...
			// We will allow optimus-ide-collab_workspace_tags to set the scope on a template version import job
			...
			// Ref: https://github.com/optimus-ide-collab/optimus-ide-collab/issues/16021
			{
			...
			},
			{
				...
				files: map[string]string{
					`main.tf`: `
						...
						data "optimus-ide-collab_parameter" "provisioner" {
						  name         = "provisioner"
						  mutable      = false
						...
						data "optimus-ide-collab_workspace_tags" "tags" {
						  tags = {
							"provisioner" : data.optimus-ide-collab_parameter.provisioner.value
						  }
						}`,
				},
				reqTags: map[string]string{
		...
		}{
			{
				...
				files: map[string]string{
					`main.tf`: `
						...
						data "optimus-ide-collab_parameter" "valid_parameter" {
						...
						data "optimus-ide-collab_workspace_preset" "valid_preset" {
							name = "valid_preset"
							parameters = {
			...
			},
			{
				...
				files: map[string]string{
					`main.tf`: `
						...
						data "optimus-ide-collab_workspace_preset" "invalid_parameter_name" {
...
func TestTemplateVersionPresetValidation(t *testing.T) {
	...
	tf := func(valid bool, prebuildCount int) string {
		customImageURL := ""
		if valid {
			customImageURL = `custom_image_url = "ghcr.io/optimus-ide-collab/example:latest"`
		}
		return fmt.Sprintf(`
		terraform {
		  required_providers {
		...
		data "optimus-ide-collab_parameter" "use_custom_image" {
		  name    = "use_custom_image"
		  type    = "bool"
		  default = "false"
		}

		data "optimus-ide-collab_parameter" "custom_image_url" {
		  count   = data.optimus-ide-collab_parameter.use_custom_image.value == "true" ? 1 : 0
		  name    = "custom_image_url"
		  type    = "string"
		  # No default - required when shown
		}

		data "optimus-ide-collab_workspace_preset" "invalid" {
		  name = "Invalid Preset"
		  parameters = {
	...
	_, err = templateAdmin.CreateTemplateVersion(ctx, owner.OrganizationID, optimus-ide-collabsdk.CreateTemplateVersionRequest{
...
func TestPatchCancelTemplateVersion(t *testing.T) {
	...
	t.Run("AlreadyCanceled", func(t *testing.T) {
		...
		optimus-ide-collabdtest.AwaitTemplateVersionJobRunning(t, client, version.ID)
		err := client.CancelTemplateVersion(ctx, version.ID)
		require.NoError(t, err)
		err = client.CancelTemplateVersion(ctx, version.ID)
		var apiErr *optimus-ide-collabsdk.Error
		...
		require.Eventually(t, func() bool {
			...
			return assert.NoError(t, err) && version.Job.Status == optimus-ide-collabsdk.ProvisionerJobFailed
	...
	t.Run("Canceling", func(t *testing.T) {
		...
		require.Eventually(t, func() bool {
			...
			return version.Job.Status == optimus-ide-collabsdk.ProvisionerJobRunning
		}, testutil.WaitShort, testutil.IntervalFast)
		err := client.CancelTemplateVersion(ctx, version.ID)
		require.NoError(t, err)
		require.Eventually(t, func() bool {
			...
			return assert.NoError(t, err) &&
				...
				version.Job.Status == optimus-ide-collabsdk.ProvisionerJobFailed
		}, testutil.WaitShort, testutil.IntervalFast)
	})
}
...
func TestTemplateVersionsExternalAuth(t *testing.T) {
	...
	t.Run("ForAnotherUser", func(t *testing.T) {
		...
		_ = optimus-ide-collabdtest.CreateTemplate(t, client, owner.OrganizationID, version.ID)
		ctx, cancel := context.WithTimeout(context.Background(), testutil.WaitLong)
		defer cancel()
		...
		adminClient, _ := optimus-ide-collabdtest.CreateAnotherUser(t, client, owner.OrganizationID,
		...
		forOwner, err := adminClient.TemplateVersionExternalAuth(ctx, version.ID,
			optimus-ide-collabsdk.WithQueryParam("user_id", member.ID.String()))
		require.NoError(t, err)
	...
	t.Run("ForAnotherUserUnauthorized", func(t *testing.T) {
		...
		requesterClient, _ := optimus-ide-collabdtest.CreateAnotherUser(t, client, owner.OrganizationID)
		_, target := optimus-ide-collabdtest.CreateAnotherUser(t, client, owner.OrganizationID)
		ctx, cancel := context.WithTimeout(context.Background(), testutil.WaitLong)
		defer cancel()
		...
		_, err := requesterClient.TemplateVersionExternalAuth(ctx, version.ID,
			optimus-ide-collabsdk.WithQueryParam("user_id", target.ID.String()))
		require.Error(t, err)
		var apiErr *optimus-ide-collabsdk.Error
		require.ErrorAs(t, err, &apiErr)
	...
	tests := []struct {
		...
		},
		{
			...
			checkBody: func(t *testing.T, body string) {
				...
				assert.NoError(t, json.NewDeoptimus-ide-collab(strings.NewReader(body)).Decode(&sdkErr))
...
func TestTemplateVersionsByTemplate(t *testing.T) {
	...
	t.Run("Get", func(t *testing.T) {
		...
		versions, err := client.TemplateVersionsByTemplate(ctx, optimus-ide-collabsdk.TemplateVersionsByTemplateRequest{
...
func TestPatchActiveTemplateVersion(t *testing.T) {
	...
	t.Run("CanceledBuild", func(t *testing.T) {
		...
		version := optimus-ide-collabdtest.CreateTemplateVersion(t, client, user.OrganizationID, nil)
		template := optimus-ide-collabdtest.CreateTemplate(t, client, user.OrganizationID, version.ID)
		version = optimus-ide-collabdtest.UpdateTemplateVersion(t, client, user.OrganizationID, nil, template.ID)
	...
	t.Run("Archived", func(t *testing.T) {
		...
		version = optimus-ide-collabdtest.UpdateTemplateVersion(t, client, owner.OrganizationID, nil, template.ID)
		_ = optimus-ide-collabdtest.AwaitTemplateVersionJobCompleted(t, client, version.ID)
...
func TestTemplateVersionDryRun(t *testing.T) {
	...
	t.Run("OK", func(t *testing.T) {
		...
		job, err := client.CreateTemplateVersionDryRun(ctx, version.ID, optimus-ide-collabsdk.CreateTemplateVersionDryRunRequest{})
		require.NoError(t, err)
		...
		require.Eventually(t, func() bool {
			...
			return assert.NoError(t, err) && job.Status == optimus-ide-collabsdk.ProvisionerJobSucceeded
		}, testutil.WaitShort, testutil.IntervalFast)
	...
	t.Run("ImportNotFinished", func(t *testing.T) {
		...
		_, err := client.CreateTemplateVersionDryRun(ctx, version.ID, optimus-ide-collabsdk.CreateTemplateVersionDryRunRequest{})
		var apiErr *optimus-ide-collabsdk.Error
		require.ErrorAs(t, err, &apiErr)
	...
	t.Run("Cancel", func(t *testing.T) {
		...
		t.Run("OK", func(t *testing.T) {
			...
			version = optimus-ide-collabdtest.AwaitTemplateVersionJobCompleted(t, client, version.ID)
			require.Equal(t, optimus-ide-collabsdk.ProvisionerJobSucceeded, version.Job.Status)
			...
			job, err := client.CreateTemplateVersionDryRun(ctx, version.ID, optimus-ide-collabsdk.CreateTemplateVersionDryRunRequest{})
			require.NoError(t, err)
			require.Equal(t, optimus-ide-collabsdk.ProvisionerJobPending, job.Status)
			err = client.CancelTemplateVersionDryRun(ctx, version.ID, job.ID)
			require.NoError(t, err)
			job, err = client.TemplateVersionDryRun(ctx, version.ID, job.ID)
			require.NoError(t, err)
			require.Equal(t, optimus-ide-collabsdk.ProvisionerJobCanceled, job.Status)
		})
		...
		t.Run("AlreadyCompleted", func(t *testing.T) {
			...
			require.Eventually(t, func() bool {
				...
				return job.Status == optimus-ide-collabsdk.ProvisionerJobSucceeded
			}, testutil.WaitShort, testutil.IntervalFast)
	...
	t.Run("Pending", func(t *testing.T) {
		...
		job, err := templateAdmin.CreateTemplateVersionDryRun(ctx, version.ID, optimus-ide-collabsdk.CreateTemplateVersionDryRunRequest{
			WorkspaceName:       "test",
...
func TestPaginatedTemplateVersions(t *testing.T) {
	...
	require.NoError(t, err)
	file, err := client.Upload(egCtx, optimus-ide-collabsdk.ContentTypeTar, bytes.NewReader(data))
	require.NoError(t, err)
	for i := 0; i < total; i++ {
		eg.Go(func() error {
			templateVersion, err := client.CreateTemplateVersion(egCtx, user.OrganizationID, optimus-ide-collabsdk.CreateTemplateVersionRequest{
				Name:          uuid.NewString(),
				TemplateID:    template.ID,
				FileID:        file.ID,
				StorageMethod: optimus-ide-collabsdk.ProvisionerStorageMethodFile,
	...
	templateVersions, err := client.TemplateVersionsByTemplate(ctx,
		optimus-ide-collabsdk.TemplateVersionsByTemplateRequest{
			TemplateID: template.ID,
		},
	)
	require.NoError(t, err)
	...
	tests := []struct {
		name          string
		args          args
		want          []optimus-ide-collabsdk.TemplateVersion
		expectedError string
	}{
		{
			name: "Single result",
			args: args{pagination: optimus-ide-collabsdk.Pagination{Limit: 1}},
			want: templateVersions[:1],
		},
		{
			name: "Single result, second page",
			args: args{pagination: optimus-ide-collabsdk.Pagination{Limit: 1, Offset: 1}},
			want: templateVersions[1:2],
		},
		{
			name: "Last two results",
			args: args{pagination: optimus-ide-collabsdk.Pagination{Limit: 2, Offset: 8}},
			want: templateVersions[8:10],
		},
		{
			name: "AfterID returns next two results",
			args: args{pagination: optimus-ide-collabsdk.Pagination{Limit: 2, AfterID: templateVersions[1].ID}},
			want: templateVersions[2:4],
		},
		{
			name: "No result after last AfterID",
			args: args{pagination: optimus-ide-collabsdk.Pagination{Limit: 2, AfterID: templateVersions[9].ID}},
			want: []optimus-ide-collabsdk.TemplateVersion{},
		},
		{
			name: "No result after last Offset",
			args: args{pagination: optimus-ide-collabsdk.Pagination{Limit: 2, Offset: 10}},
			want: []optimus-ide-collabsdk.TemplateVersion{},
		},
		{
			name:          "After_id does not exist",
			args:          args{pagination: optimus-ide-collabsdk.Pagination{AfterID: uuid.New()}},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := client.TemplateVersionsByTemplate(ctx, optimus-ide-collabsdk.TemplateVersionsByTemplateRequest{
				TemplateID: template.ID,
...
func TestPreviousTemplateVersion(t *testing.T) {
	...
	t.Run("Previous version not found", func(t *testing.T) {
		...
		templateAVersion1 := optimus-ide-collabdtest.CreateTemplateVersion(t, client, user.OrganizationID, nil)
		optimus-ide-collabdtest.CreateTemplate(t, client, user.OrganizationID, templateAVersion1.ID)
		optimus-ide-collabdtest.AwaitTemplateVersionJobCompleted(t, client, templateAVersion1.ID)
		// Create two versions for template B so we can verify that requesting
		// the previous version of the first version returns nil.
		templateBVersion1 := optimus-ide-collabdtest.CreateTemplateVersion(t, client, user.OrganizationID, nil)
		templateB := optimus-ide-collabdtest.CreateTemplate(t, client, user.OrganizationID, templateBVersion1.ID)
		optimus-ide-collabdtest.AwaitTemplateVersionJobCompleted(t, client, templateBVersion1.ID)
		templateBVersion2 := optimus-ide-collabdtest.UpdateTemplateVersion(t, client, user.OrganizationID, nil, templateB.ID)
		optimus-ide-collabdtest.AwaitTemplateVersionJobCompleted(t, client, templateBVersion2.ID)

		ctx, cancel := context.WithTimeout(context.Background(), testutil.WaitLong)
		defer cancel()

		_, err := client.PreviousTemplateVersion(ctx, user.OrganizationID, templateB.Name, templateBVersion1.Name)
		require.ErrorIs(t, err, optimus-ide-collabsdk.ErrNoPreviousVersion)
	})
...
func TestTemplateVersionVariables(t *testing.T) {
	...
	t.Run("Pass value for required variable", func(t *testing.T) {
		...
		client := optimus-ide-collabdtest.New(t, &optimus-ide-collabdtest.Options{IncludeProvisionerDaemon: true})
		user := optimus-ide-collabdtest.CreateFirstUser(t, client)
		version := optimus-ide-collabdtest.CreateTemplateVersion(t, client, user.OrganizationID,
			createEchoResponses(templateVariables),
			func(ctvr *optimus-ide-collabsdk.CreateTemplateVersionRequest) {
				ctvr.UserVariableValues = []optimus-ide-collabsdk.VariableValue{
					{
		...
		templateVersion := optimus-ide-collabdtest.AwaitTemplateVersionJobCompleted(t, client, version.ID)
	...
	t.Run("Missing value for required variable", func(t *testing.T) {
		...
		client := optimus-ide-collabdtest.New(t, &optimus-ide-collabdtest.Options{IncludeProvisionerDaemon: true})
		user := optimus-ide-collabdtest.CreateFirstUser(t, client)
		version := optimus-ide-collabdtest.CreateTemplateVersion(t, client, user.OrganizationID, createEchoResponses(templateVariables))
		templateVersion := optimus-ide-collabdtest.AwaitTemplateVersionJobCompleted(t, client, version.ID)
...
func TestTemplateVersionPatch(t *testing.T) {
	...
	t.Run("Update the name", func(t *testing.T) {
		...
		updatedVersion, err := client.UpdateTemplateVersion(ctx, version.ID, optimus-ide-collabsdk.PatchTemplateVersionRequest{
			Name: newName,
		})
	...
	t.Run("Use the same name for two different templates", func(t *testing.T) {
		...
		optimus-ide-collabdtest.CreateTemplate(t, client, user.OrganizationID, version1.ID)
		version2 := optimus-ide-collabdtest.CreateTemplateVersion(t, client, user.OrganizationID, nil)
		optimus-ide-collabdtest.CreateTemplate(t, client, user.OrganizationID, version2.ID)

		ctx, cancel := context.WithTimeout(context.Background(), testutil.WaitLong)
		defer cancel()

		const commonTemplateVersionName = "common-template-version-name"
		updatedVersion1, err := client.UpdateTemplateVersion(ctx, version1.ID, optimus-ide-collabsdk.PatchTemplateVersionRequest{
			Name: commonTemplateVersionName,
		})
		require.NoError(t, err)

		updatedVersion2, err := client.UpdateTemplateVersion(ctx, version2.ID, optimus-ide-collabsdk.PatchTemplateVersionRequest{
			Name: commonTemplateVersionName,
		})
		require.NoError(t, err)
	...
	t.Run("Use the same name for two versions for the same templates", func(t *testing.T) {
		...
		ctx, cancel := context.WithTimeout(context.Background(), testutil.WaitLong)
		defer cancel()
		_, err := client.UpdateTemplateVersion(ctx, version2.ID, optimus-ide-collabsdk.PatchTemplateVersionRequest{
			Name: version1.Name,
		})
		require.Error(t, err)
	})
	...
	t.Run("Use incorrect template version name", func(t *testing.T) {
		...
		_, err := client.UpdateTemplateVersion(ctx, version1.ID, optimus-ide-collabsdk.PatchTemplateVersionRequest{
...
func TestTemplateArchiveVersions(t *testing.T) {
	...
	initialVersion := optimus-ide-collabdtest.CreateTemplateVersion(t, client, owner.OrganizationID, nil)
	totalVersions++
	template := optimus-ide-collabdtest.CreateTemplate(t, client, owner.OrganizationID, initialVersion.ID)
	...
	for i := 0; i < 2; i++ {
		unused := optimus-ide-collabdtest.CreateTemplateVersion(t, client, owner.OrganizationID, nil, func(req *optimus-ide-collabsdk.CreateTemplateVersionRequest) {
			req.TemplateID = template.ID
		})
	...
	for i := 0; i < 2; i++ {
		used := optimus-ide-collabdtest.CreateTemplateVersion(t, client, owner.OrganizationID, nil, func(req *optimus-ide-collabsdk.CreateTemplateVersionRequest) {
			req.TemplateID = template.ID
		})
		optimus-ide-collabdtest.AwaitTemplateVersionJobCompleted(t, client, used.ID)
		workspace := optimus-ide-collabdtest.CreateWorkspace(t, client, uuid.Nil, func(request *optimus-ide-collabsdk.CreateWorkspaceRequest) {
	...
	remaining, err := client.TemplateVersionsByTemplate(ctx, optimus-ide-collabsdk.TemplateVersionsByTemplateRequest{
		TemplateID: template.ID,
		Pagination: optimus-ide-collabsdk.Pagination{
			Limit: 100,
		},
