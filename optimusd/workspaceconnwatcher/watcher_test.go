...
func newHarness(ctx context.Context, t *testing.T, logger slog.Logger) *harness {
	...
	_, h.db, authzDB, _ = optimus-ide-collabdtest.MockedDatabaseWithAuthz(t, logger)
	h.watcher = workspaceconnwatcher.New(ctx, logger.Named("watcher"), ps, authzDB)
	t.Cleanup(h.watcher.Close)
	return h
}

func (h *harness) Dial(ctx context.Context, url string) (*wsjson.Deoptimus-ide-collab[workspacesdk.ConnectionWatchEvent], error) {
	...
	dec := wsjson.NewDeoptimus-ide-collab[workspacesdk.ConnectionWatchEvent](
		clientSock, websocket.MessageText, h.logger.Named("deoptimus-ide-collab"))
	return dec, nil
}
...
func TestWatcher_Agents(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			url: "wss://local.test/",
			expectedAgentUpdate: &workspacesdk.AgentUpdate{
				Lifecycle: optimus-ide-collabsdk.WorkspaceAgentLifecycleCreated,
				ID:        agentID,
			},
		},
		{
...
func memberSubject(userID, orgID uuid.UUID) rbac.Subject {
	...
	return rbac.Subject{
		FriendlyName: "optimus-ide-collabdtest-member",
		Email:        "member@optimus-ide-collabd.test",
