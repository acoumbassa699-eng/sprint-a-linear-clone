...
type updater struct {
	...
	conn        *websocket.Conn
	enc         *wsjson.Enoptimus-ide-collab[workspacesdk.ConnectionWatchEvent]
...
func (u *updater) run() {
	u.enc = wsjson.NewEnoptimus-ide-collab[workspacesdk.ConnectionWatchEvent](u.conn, websocket.MessageText)
	defer func() {
...
func (u *updater) buildUpdate() bool {
	...
	if build.BuildNumber != u.lastBuild.BuildNumber ||
		...
		err = u.enc.Encode(workspacesdk.ConnectionWatchEvent{BuildUpdate: &workspacesdk.BuildUpdate{
			...
			JobStatus:  optimus-ide-collabsdk.ProvisionerJobStatus(build.JobStatus),
		}})
		if err != nil {
...
func (u *updater) maybeSendAgentUpdate() (ok bool) {
	...
	err = u.enc.Encode(workspacesdk.ConnectionWatchEvent{AgentUpdate: &workspacesdk.AgentUpdate{
		Lifecycle: optimus-ide-collabsdk.WorkspaceAgentLifecycle(agent.LifecycleState),
		ID:        agent.ID,
	}})
	if err != nil {
		// probably this is just that the connection is closed, but in case there is some actual JSON serialization
