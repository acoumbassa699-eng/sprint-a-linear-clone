...
func TestGetChatCostQueriesRequestedChat(t *testing.T) {
	...
	var cost optimus-ide-collabsdk.ChatCost
	require.NoError(t, json.NewDeoptimus-ide-collab(resp.Body).Decode(&cost))
...
func TestEnrichMissingChatAgentIDs(t *testing.T) {
	...
	t.Run("batch selection and shared workspace", func(t *testing.T) {
		...
		chats := []optimus-ide-collabsdk.Chat{{WorkspaceID: &workspaceID, Children: []optimus-ide-collabsdk.Chat{{WorkspaceID: &workspaceID}}}, {WorkspaceID: &otherWorkspaceID}}
	...
	t.Run("query error", func(t *testing.T) {
		...
		chats := []optimus-ide-collabsdk.Chat{{WorkspaceID: &workspaceID}, {WorkspaceID: &otherWorkspaceID}}
		api.enrichChatWithWorkspaceAgentIDs(testutil.Context(t, testutil.WaitShort), chats)
	...
	t.Run("selection error and skips bound or unbound", func(t *testing.T) {
		...
		bound := otherAgentID
		chats := []optimus-ide-collabsdk.Chat{{}, {WorkspaceID: &workspaceID}, {WorkspaceID: &workspaceID, AgentID: &bound}}
		api.enrichChatWithWorkspaceAgentIDs(testutil.Context(t, testutil.WaitShort), chats)
		require.Nil(t, chats[1].AgentID)
...
func TestValidateChatModelProviderOptions_AnthropicThinkingDisplay(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			display := tt.display
			err := validateChatModelProviderOptions(&optimus-ide-collabsdk.ChatModelProviderOptions{
