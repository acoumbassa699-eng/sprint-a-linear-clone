...
func TestRefreshTokenWithScopes(t *testing.T) {
	...
	newConfig := func(t *testing.T, scopes []string) *externalauth.Config {
		...
		configs, err := externalauth.ConvertConfig(instrument, []optimus-ide-collabsdk.ExternalAuthConfig{{
			ID:           "test",
			Type:         optimus-ide-collabsdk.EnhancedExternalAuthProviderAzureDevopsEntra.String(),
			ClientID:     "id",
			ClientSecret: "secret",
			...
			Scopes:       scopes,
		}}, &url.URL{Scheme: "https", Host: "optimus-ide-collab.example.com"})
		require.NoError(t, err)
		return configs[0]
	}
...
func TestRevokeToken(t *testing.T) {
	...
	t.Run("RevokeTokenGitHub_OK", func(t *testing.T) {
		...
		config.Type = optimus-ide-collabsdk.EnhancedExternalAuthProviderGitHub.String()
...
func TestExchangeWithClientSecret(t *testing.T) {
	...
	configs, err := externalauth.ConvertConfig(instrument, []optimus-ide-collabsdk.ExternalAuthConfig{{
		...
		Type:         optimus-ide-collabsdk.EnhancedExternalAuthProviderJFrog.String(),
		ClientID:     "id",
		ClientSecret: "secret",
	}}, &url.URL{})
	require.NoError(t, err)
...
func TestTokenRevocationResponseOk(t *testing.T) {
	...
	ghType := optimus-ide-collabsdk.EnhancedExternalAuthProviderGitHub.String()
	rfcType := optimus-ide-collabsdk.EnhancedExternalAuthProviderAzureDevops.String()
	tests := []struct {
		name string
...
func TestConvertYAML(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		Name: "InvalidID",
		Input: []optimus-ide-collabsdk.ExternalAuthConfig{{
			Type: string(optimus-ide-collabsdk.EnhancedExternalAuthProviderGitHub),
			ID:   "$hi$",
		}},
	...
	t.Run("CustomScopesAndEndpoint", func(t *testing.T) {
		t.Parallel()
		config, err := externalauth.ConvertConfig(instrument, []optimus-ide-collabsdk.ExternalAuthConfig{{
			Type:         string(optimus-ide-collabsdk.EnhancedExternalAuthProviderGitLab),
			ClientID:     "id",
			ClientSecret: "secret",
...
func TestConstantQueryParams(t *testing.T) {
	...
	fake, config, _ := setupOauth2Test(t, testConfig{
		...
		Optimus-IDE-CollabOIDCConfigOpts: []func(cfg *optimus-ide-collabd.OIDCConfig){
			func(cfg *optimus-ide-collabd.OIDCConfig) {
	...
	fake.SetOptimus-IDE-CollabdCallbackHandler(func(writer http.ResponseWriter, request *http.Request) {
	...
	// We are not using a real server, so it fakes https://optimus-ide-collab.com
...
func setupOauth2Test(t *testing.T, settings testConfig) (*oidctest.FakeIDP, *externalauth.Config, database.ExternalAuthLink) {
	...
	config := &externalauth.Config{
		InstrumentedOAuth2Config: f.New("test-oauth2",
			fake.OIDCConfig(t, nil, settings.Optimus-IDE-CollabOIDCConfigOpts...)),
		ID:                            providerID,
		ClientID:                      cid,
		ClientSecret:                  cs,
...
func TestApplyDefaultsToConfig_CaseInsensitive(t *testing.T) {
	...
	instrument := promoauth.NewFactory(prometheus.NewRegistry())
	accessURL, err := url.Parse("https://optimus-ide-collab.example.com")
	require.NoError(t, err)
