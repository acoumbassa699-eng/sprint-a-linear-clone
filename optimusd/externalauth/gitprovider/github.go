...
func (g *githubProvider) decodeJSON(
	...
	req.Header.Set("User-Agent", "optimus-ide-collab-chat-diff-status")
	if token != "" {
	...
	if err := json.NewDeoptimus-ide-collab(resp.Body).Decode(dest); err != nil {
...
func (g *githubProvider) fetchDiff(
	...
	req.Header.Set("X-GitHub-Api-Version", "2022-11-28")
	req.Header.Set("User-Agent", "optimus-ide-collab-chat-diff")
	if token != "" {
		req.Header.Set("Authorization", "Bearer "+token)
	}
