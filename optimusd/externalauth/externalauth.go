...
type Config struct {
	...
	// NoRefresh stops Optimus-IDE-Collab from using the refresh token
	...
	// injected into Optimus-IDE-Collab AI Bridge upstream requests.
...
func (c *Config) Git(client *http.Client) (gitprovider.Provider, error) {
	...
	if !optimus-ide-collabsdk.EnhancedExternalAuthProvider(norm).Git() {
...
validate:
	...
	if !valid {
		...
		if c.Type == string(optimus-ide-collabsdk.EnhancedExternalAuthProviderGitHub) && r.Wait(retryCtx) {
			goto validate
		}
...
// or 429).
func (c *Config) ValidateToken(ctx context.Context, link *oauth2.Token) (bool, *optimus-ide-collabsdk.ExternalAuthUser, error) {
	if link == nil {
	...
	var user *optimus-ide-collabsdk.ExternalAuthUser
	if c.Type == string(optimus-ide-collabsdk.EnhancedExternalAuthProviderGitHub) {
		var ghUser github.User
		err = json.NewDeoptimus-ide-collab(res.Body).Decode(&ghUser)
		if err == nil {
			user = &optimus-ide-collabsdk.ExternalAuthUser{
				ID:         ghUser.GetID(),
...
func (c *Config) AppInstallations(ctx context.Context, token string) ([]optimus-ide-collabsdk.ExternalAuthAppInstallation, bool, error) {
	...
	installs := []optimus-ide-collabsdk.ExternalAuthAppInstallation{}
	if c.Type == string(optimus-ide-collabsdk.EnhancedExternalAuthProviderGitHub) {
		var ghInstalls struct {
			Installations []*github.Installation `json:"installations"`
		}
		err = json.NewDeoptimus-ide-collab(res.Body).Decode(&ghInstalls)
		if err != nil {
			return nil, false, err
		}
		for _, installation := range ghInstalls.Installations {
			...
			installs = append(installs, optimus-ide-collabsdk.ExternalAuthAppInstallation{
				ID:           int(installation.GetID()),
				ConfigureURL: installation.GetHTMLURL(),
				Account: optimus-ide-collabsdk.ExternalAuthUser{
					ID:         account.GetID(),
...
func (c *Config) TokenRevocationRequest(ctx context.Context, link database.ExternalAuthLink) (*http.Request, error) {
	if c.Type == optimus-ide-collabsdk.EnhancedExternalAuthProviderGitHub.String() {
...
func (c *DeviceAuth) AuthorizeDevice(ctx context.Context) (*optimus-ide-collabsdk.ExternalAuthDevice, error) {
	if c.CodeURL == "" {
	...
	err = json.NewDeoptimus-ide-collab(resp.Body).Decode(&r)
	if err != nil {
...
func (c *DeviceAuth) ExchangeDeviceCode(ctx context.Context, deviceCode string) (*oauth2.Token, error) {
	...
	if resp.StatusCode != http.StatusOK {
		return nil, optimus-ide-collabsdk.ReadBodyAsError(resp)
	}
	var body ExchangeDeviceCodeResponse
	err = json.NewDeoptimus-ide-collab(resp.Body).Decode(&body)
	if err != nil {
		return nil, err
	}
	if body.Error != "" {
...
// to the parsed and ready-to-consume in optimus-ide-collabd provider type.
func ConvertConfig(instrument *promoauth.Factory, entries []optimus-ide-collabsdk.ExternalAuthConfig, accessURL *url.URL) ([]*Config, error) {
	...
	for _, entry := range entries {
		...
		valid := optimus-ide-collabsdk.NameValid(entry.ID)
		if valid != nil {
		...
		if entry.Type == string(optimus-ide-collabsdk.EnhancedExternalAuthProviderAzureDevops) {
			oauthConfig = &jwtConfig{oc}
		}
		if entry.Type == string(optimus-ide-collabsdk.EnhancedExternalAuthProviderAzureDevopsEntra) {
			oauthConfig = &entraV1Oauth{oc}
		}
		if entry.Type == string(optimus-ide-collabsdk.EnhancedExternalAuthProviderJFrog) {
			oauthConfig = &exchangeWithClientSecret{oc}
		}

		instrumented := instrument.New(entry.ID, oauthConfig)
		if strings.EqualFold(entry.Type, string(optimus-ide-collabsdk.EnhancedExternalAuthProviderGitHub)) {
...
func applyDefaultsToConfig(config *optimus-ide-collabsdk.ExternalAuthConfig) {
	configType := optimus-ide-collabsdk.EnhancedExternalAuthProvider(strings.ToLower(config.Type))
	if configType == "bitbucket" {
		// For backwards compatibility, we need to support the "bitbucket" string.
		configType = optimus-ide-collabsdk.EnhancedExternalAuthProviderBitBucketCloud
		defer func() {
			...
			config.Type = string(optimus-ide-collabsdk.EnhancedExternalAuthProviderBitBucketCloud)
		}()
	}
	...
	// Dynamic defaults
	switch configType {
	case optimus-ide-collabsdk.EnhancedExternalAuthProviderGitHub:
		copyDefaultSettings(config, gitHubDefaults(config))
		return
	case optimus-ide-collabsdk.EnhancedExternalAuthProviderGitLab:
		copyDefaultSettings(config, gitlabDefaults(config))
		return
	case optimus-ide-collabsdk.EnhancedExternalAuthProviderBitBucketServer:
		copyDefaultSettings(config, bitbucketServerDefaults(config))
		return
	case optimus-ide-collabsdk.EnhancedExternalAuthProviderJFrog:
		copyDefaultSettings(config, jfrogArtifactoryDefaults(config))
		return
	case optimus-ide-collabsdk.EnhancedExternalAuthProviderGitea:
		copyDefaultSettings(config, giteaDefaults(config))
		return
	case optimus-ide-collabsdk.EnhancedExternalAuthProviderAzureDevopsEntra:
	...
	default:
		...
		copyDefaultSettings(config, optimus-ide-collabsdk.ExternalAuthConfig{})
		return
	}
}

func copyDefaultSettings(config *optimus-ide-collabsdk.ExternalAuthConfig, defaults optimus-ide-collabsdk.ExternalAuthConfig) {
	if config.AuthURL == "" {
	...
	if config.APIBaseURL == "" {
		...
		switch optimus-ide-collabsdk.EnhancedExternalAuthProvider(normType) {
		case optimus-ide-collabsdk.EnhancedExternalAuthProviderGitHub:
...
func gitHubDefaults(config *optimus-ide-collabsdk.ExternalAuthConfig) optimus-ide-collabsdk.ExternalAuthConfig {
	defaults := optimus-ide-collabsdk.ExternalAuthConfig{
...
func bitbucketServerDefaults(config *optimus-ide-collabsdk.ExternalAuthConfig) optimus-ide-collabsdk.ExternalAuthConfig {
	defaults := optimus-ide-collabsdk.ExternalAuthConfig{
	...
	// Bitbucket servers will have some base url, e.g. https://bitbucket.optimus-ide-collab.com.
...
func gitlabDefaults(config *optimus-ide-collabsdk.ExternalAuthConfig) optimus-ide-collabsdk.ExternalAuthConfig {
	cloud := optimus-ide-collabsdk.ExternalAuthConfig{
...
func jfrogArtifactoryDefaults(config *optimus-ide-collabsdk.ExternalAuthConfig) optimus-ide-collabsdk.ExternalAuthConfig {
	defaults := optimus-ide-collabsdk.ExternalAuthConfig{
	...
	// Artifactory servers will have some base url, e.g. https://jfrog.optimus-ide-collab.com.
...
func giteaDefaults(config *optimus-ide-collabsdk.ExternalAuthConfig) optimus-ide-collabsdk.ExternalAuthConfig {
	defaults := optimus-ide-collabsdk.ExternalAuthConfig{
		DisplayName:                   "Gitea",
	...
	// Gitea's servers will have some base url, e.g: https://gitea.optimus-ide-collab.com.
...
func azureDevopsEntraDefaults(config *optimus-ide-collabsdk.ExternalAuthConfig) optimus-ide-collabsdk.ExternalAuthConfig {
	defaults := optimus-ide-collabsdk.ExternalAuthConfig{
...
var staticDefaults = map[optimus-ide-collabsdk.EnhancedExternalAuthProvider]optimus-ide-collabsdk.ExternalAuthConfig{
	optimus-ide-collabsdk.EnhancedExternalAuthProviderAzureDevops: {
	...
	optimus-ide-collabsdk.EnhancedExternalAuthProviderBitBucketCloud: {
	...
	optimus-ide-collabsdk.EnhancedExternalAuthProviderSlack: {
