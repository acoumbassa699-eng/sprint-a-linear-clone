...
func childBuildRequestFromOrchestration(orchestration database.WorkspaceBuildOrchestration) (optimus-ide-collabsdk.CreateWorkspaceBuildRequest, error) {
	var childParameterValues []optimus-ide-collabsdk.WorkspaceBuildParameter
	if len(orchestration.ChildRichParameterValues) > 0 {
		...
		if err != nil {
			return optimus-ide-collabsdk.CreateWorkspaceBuildRequest{}, xerrors.Errorf("unmarshal child rich parameter values: %w", err)
		}
	}
	...
	request := optimus-ide-collabsdk.CreateWorkspaceBuildRequest{
		Transition:          optimus-ide-collabsdk.WorkspaceTransition(orchestration.ChildTransition),
		RichParameterValues: childParameterValues,
		LogLevel:            optimus-ide-collabsdk.ProvisionerLogLevel(orchestration.ChildLogLevel),
	}
	...
	if orchestration.ChildReason.Valid {
		request.Reason = optimus-ide-collabsdk.CreateWorkspaceBuildReason(orchestration.ChildReason.BuildReason)
	}
