...
func TestWebpushSubscribeUnsubscribe(t *testing.T) {
	...
	_, anotherMember := optimus-ide-collabdtest.CreateAnotherUser(t, client, owner.OrganizationID)
	...
	err = memberClient.PostWebpushSubscription(ctx, "me", optimus-ide-collabsdk.WebpushSubscription{
		Endpoint:  endpoint,
	...
	err = memberClient.DeleteWebpushSubscription(ctx, "me", optimus-ide-collabsdk.DeleteWebpushSubscription{
		Endpoint: endpoint,
	})
	...
	err = memberClient.PostWebpushSubscription(ctx, anotherMember.ID.String(), optimus-ide-collabsdk.WebpushSubscription{
		Endpoint:  endpoint,
		AuthKey:   validEndpointAuthKey,
		P256DHKey: validEndpointP256dhKey,
	})
	...
	err = memberClient.DeleteWebpushSubscription(ctx, anotherMember.ID.String(), optimus-ide-collabsdk.DeleteWebpushSubscription{
		Endpoint: endpoint,
	})
...
// with new p256dh / auth keys, and Optimus-IDE-Collab must replace the stored keys so
...
func (d *testWebpushDispatcher) Dispatch(_ context.Context, _ uuid.UUID, _ optimus-ide-collabsdk.WebpushMessage) error {
	d.dispatchCalls.Add(1)
	return nil
}

func (d *testWebpushDispatcher) Test(_ context.Context, _ optimus-ide-collabsdk.WebpushSubscription) error {
	d.testCalls.Add(1)
	return nil
}
