...
// @Success 200 {object} optimus-ide-collabsdk.OrganizationMember
...
func (api *API) deleteOrganizationMember(rw http.ResponseWriter, r *http.Request) {
	...
	if member.UserID == apiKey.UserID {
		httpapi.Write(ctx, rw, http.StatusBadRequest, optimus-ide-collabsdk.Response{Message: "cannot remove self from an organization"})
		return
	}
...
// @Param organization path string true "Organization ID"
// @Param user path string true "User ID, name, or me"
// @Success 200 {object} optimus-ide-collabsdk.OrganizationMemberWithUserData
// @Produce json
...
// @Success 200 {object} []optimus-ide-collabsdk.PaginatedMembersResponse
...
func (api *API) paginatedMembers(rw http.ResponseWriter, r *http.Request) {
	...
	if len(paginatedMemberRows) == 0 {
		httpapi.Write(ctx, rw, http.StatusOK, optimus-ide-collabsdk.PaginatedMembersResponse{
			Members: []optimus-ide-collabsdk.OrganizationMemberWithUserData{},
			Count:   0,
		})
		return
	}
	...
	resp := optimus-ide-collabsdk.PaginatedMembersResponse{
		Members: members,
...
// We
func convertOrganizationMembers(ctx context.Context, db database.Store, mems []database.OrganizationMember) ([]optimus-ide-collabsdk.OrganizationMember, error) {
	converted := make([]optimus-ide-collabsdk.OrganizationMember, 0, len(mems))
	roleLookup := make([]database.NameOrganizationPair, 0)

	for _, m := range mems {
		converted = append(converted, optimus-ide-collabsdk.OrganizationMember{
			UserID:         m.UserID,
			OrganizationID: m.OrganizationID,
			CreatedAt:      m.CreatedAt,
			UpdatedAt:      m.UpdatedAt,
			Roles: slice.List(m.Roles, func(r string) optimus-ide-collabsdk.SlimRole {
...
func convertOrganizationMembersWithUserData(ctx context.Context, db database.Store, rows []database.OrganizationMembersRow, aiSeatSet map[uuid.UUID]struct{}) ([]optimus-ide-collabsdk.OrganizationMemberWithUserData, error) {
	...
	converted := make([]optimus-ide-collabsdk.OrganizationMemberWithUserData, 0)
	for i := range convertedMembers {
		_, hasAISeat := aiSeatSet[rows[i].OrganizationMember.UserID]
		converted = append(converted, optimus-ide-collabsdk.OrganizationMemberWithUserData{
			...
			Status:             optimus-ide-collabsdk.UserStatus(rows[i].Status),
			IsServiceAccount:   rows[i].IsServiceAccount,
			LoginType:          optimus-ide-collabsdk.LoginType(rows[i].LoginType),
