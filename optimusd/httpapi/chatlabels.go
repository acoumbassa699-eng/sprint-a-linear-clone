...
func ValidateChatLabels(labels map[string]string) []optimus-ide-collabsdk.ValidationError {
	var errs []optimus-ide-collabsdk.ValidationError

	if len(labels) > maxLabelsPerChat {
		errs = append(errs, optimus-ide-collabsdk.ValidationError{
			Field:  "labels",
