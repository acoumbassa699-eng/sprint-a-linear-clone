...
type QueryParamParser struct {
	...
	Errors []optimus-ide-collabsdk.ValidationError
...
func (p *QueryParamParser) JSONStringMap(vals url.Values, def map[string]string, queryParam string) map[string]string {
	v, err := parseQueryParam(p, vals, func(v string) (map[string]string, error) {
		...
		if err := json.NewDeoptimus-ide-collab(strings.NewReader(v)).Decode(&m); err != nil {
			return nil, err
		}
		return m, nil
	}, def, queryParam)
...
func (p *QueryParamParser) ProvisionerDaemonStatuses(vals url.Values, def []optimus-ide-collabsdk.ProvisionerDaemonStatus, queryParam string) []optimus-ide-collabsdk.ProvisionerDaemonStatus {
	return ParseCustomList(p, vals, def, queryParam, func(v string) (optimus-ide-collabsdk.ProvisionerDaemonStatus, error) {
		return optimus-ide-collabsdk.ProvisionerDaemonStatus(v), nil
	})
}
