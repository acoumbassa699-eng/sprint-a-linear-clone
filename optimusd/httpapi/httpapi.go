...
import (
	...
	"cdr.dev/slog/v3"
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabd/httpapi/httpapiconstraints"
...
func init() {
	...
	nameValidator := func(fl validator.FieldLevel) bool {
		...
		valid := optimus-ide-collabsdk.NameValid(str)
		return valid == nil
	}
	...
	displayNameValidator := func(fl validator.FieldLevel) bool {
		...
		valid := optimus-ide-collabsdk.DisplayNameValid(str)
		return valid == nil
	}
	...
	templateVersionNameValidator := func(fl validator.FieldLevel) bool {
		...
		valid := optimus-ide-collabsdk.TemplateVersionNameValid(str)
		return valid == nil
	}
	...
	userRealNameValidator := func(fl validator.FieldLevel) bool {
		...
		valid := optimus-ide-collabsdk.UserRealNameValid(str)
		return valid == nil
	}
	...
	groupNameValidator := func(fl validator.FieldLevel) bool {
		...
		valid := optimus-ide-collabsdk.GroupNameValid(str)
		return valid == nil
	}
...
var ResourceNotFoundResponse = optimus-ide-collabsdk.Response{Message: "Resource not found or you do not have access to this resource"}
...
var ResourceForbiddenResponse = optimus-ide-collabsdk.Response{
	Message: "Forbidden.",
...
func InternalServerError(rw http.ResponseWriter, err error) {
	...
	Write(context.Background(), rw, http.StatusInternalServerError, optimus-ide-collabsdk.Response{
...
func RouteNotFound(rw http.ResponseWriter) {
	Write(context.Background(), rw, http.StatusNotFound, optimus-ide-collabsdk.Response{
...
func Write(ctx context.Context, rw http.ResponseWriter, status int, response interface{}) {
	...
	enc := json.NewEnoptimus-ide-collab(rw)
...
func Read(ctx context.Context, rw http.ResponseWriter, r *http.Request, value interface{}) bool {
	...
	err := json.NewDeoptimus-ide-collab(r.Body).Decode(value)
	if err != nil {
		if _, ok := errors.AsType[*http.MaxBytesError](err); ok {
			Write(ctx, rw, http.StatusRequestEntityTooLarge, optimus-ide-collabsdk.Response{
				Message: "Request body too large.",
				Detail:  err.Error(),
			})
			return false
		}
		Write(ctx, rw, http.StatusBadRequest, optimus-ide-collabsdk.Response{
	...
	if errors.As(err, &validationErrors) {
		apiErrors := make([]optimus-ide-collabsdk.ValidationError, 0, len(validationErrors))
		for _, validationError := range validationErrors {
			apiErrors = append(apiErrors, optimus-ide-collabsdk.ValidationError{
	...
	if err != nil {
		Write(ctx, rw, http.StatusInternalServerError, optimus-ide-collabsdk.Response{
...
func WebsocketCloseSprintf(format string, vars ...any) string {
	...
	// Cap msg length at 123 bytes. optimus-ide-collab/websocket only allows close messages
	// of this length.
...
type EventSender func(rw http.ResponseWriter, r *http.Request) (
	sendEvent func(sse optimus-ide-collabsdk.ServerSentEvent) error,
	done <-chan struct{},
	err error,
)
...
func ServerSentEventSender(rw http.ResponseWriter, r *http.Request) (
	func(sse optimus-ide-collabsdk.ServerSentEvent) error,
	<-chan struct{},
	error,
) {
	h := rw.Header()
	...
	go func() {
		...
		for {
			...
			case <-ticker.C:
				event = sseEvent{
					payload: []byte(fmt.Sprintf("event: %s\n\n", optimus-ide-collabsdk.ServerSentEventTypePing)),
				}
			}
	...
	sendEvent := func(newEvent optimus-ide-collabsdk.ServerSentEvent) error {
		buf := &bytes.Buffer{}
...
func OneWayWebSocketEventSender(log slog.Logger, watcher *WSWatcher) func(rw http.ResponseWriter, r *http.Request) (
	func(event optimus-ide-collabsdk.ServerSentEvent) error,
	<-chan struct{},
	error,
) {
	return func(rw http.ResponseWriter, r *http.Request) (
		...
		eventC := make(chan optimus-ide-collabsdk.ServerSentEvent, 64)
		...
		sendEvent := func(event optimus-ide-collabsdk.ServerSentEvent) error {
...
func WriteOAuth2Error(ctx context.Context, rw http.ResponseWriter, status int, errorCode optimus-ide-collabsdk.OAuth2ErrorCode, description string) {
	// RFC 6749 §5.2: invalid_client SHOULD use 401 and MUST include a
	// WWW-Authenticate response header.
	if status == http.StatusUnauthorized && errorCode == optimus-ide-collabsdk.OAuth2ErrorCodeInvalidClient {
		rw.Header().Set("WWW-Authenticate", `Basic realm="optimus-ide-collab"`)
	}

	Write(ctx, rw, status, optimus-ide-collabsdk.OAuth2Error{
		Error:            errorCode,
