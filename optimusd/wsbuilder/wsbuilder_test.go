...
func TestBuilder_NoOptions(t *testing.T) {
	...
	fc := files.New(prometheus.NewRegistry(), &optimus-ide-collabdtest.FakeAuthorizer{})
...
func TestWorkspaceBuildWithTags(t *testing.T) {
	...
	workspaceTags := []database.TemplateVersionWorkspaceTag{
		{
			Key:   "fruits_tag",
			Value: "data.optimus-ide-collab_parameter.number_of_apples.value + data.optimus-ide-collab_parameter.number_of_oranges.value",
		},
		{
			Key:   "cluster_tag",
		...
		{
			Key:   "project_tag",
			Value: `"${data.optimus-ide-collab_parameter.project.value}+12345"`,
		},
		{
			Key:   "team_tag",
			Value: `data.optimus-ide-collab_parameter.team.value`,
		},
		{
			Key:   "yes_or_no",
			Value: `data.optimus-ide-collab_parameter.is_debug_build.value`,
		},
		{
			Key:   "actually_no",
			Value: `!data.optimus-ide-collab_parameter.is_debug_build.value`,
		...
		{
			Key:   "is_debug_build",
			Value: `data.optimus-ide-collab_parameter.is_debug_build.value == "true" ? "in-debug-mode" : "no-debug"`,
		},
		{
			Key:   "variable_tag",
			Value: `var.tag`,
	...
	buildParameters := []optimus-ide-collabsdk.WorkspaceBuildParameter{
...
func TestWorkspaceBuildWithRichParameters(t *testing.T) {
	...
	t.Run("UpdateParameterValues", func(t *testing.T) {
		...
		nextBuildParameters := []optimus-ide-collabsdk.WorkspaceBuildParameter{
			{Name: firstParameterName, Value: firstParameterValue},
...
func TestWsbuildError(t *testing.T) {
	...
	require.True(t, ok, "should be a Optimus-IDE-Collab SDK error")
