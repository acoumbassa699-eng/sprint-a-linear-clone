...
import (
	...
	"cdr.dev/slog/v3"
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabd/aibridge/budget"
...
type Server struct {
	...
	optimus-ide-collabMCPConfig    *proto.MCPServerConfig // may be nil if not available
	...
	budgetPolicy optimus-ide-collabsdk.AIBudgetPolicy
	...
	budgetPeriod  optimus-ide-collabsdk.AIBudgetPeriod
	clock         quartz.Clock
...
func NewServer(lifecycleCtx context.Context, opts Options) (*Server, error) {
	...
	srv := &Server{
		...
		budgetPolicy:        optimus-ide-collabsdk.NewAIBudgetPolicyFromString(opts.GatewayCfg.BudgetPolicy),
		budgetPeriod:        optimus-ide-collabsdk.NewAIBudgetPeriodFromString(opts.GatewayCfg.BudgetPeriod),
		clock:               opts.Clock,
		notifEnqueuer:       enqueuer,
		metrics:             opts.Metrics,
	}

	if opts.GatewayCfg.InjectOptimus-IDE-CollabMCPTools {
		opts.Logger.Warn(lifecycleCtx, "inject MCP tools option is deprecated and will be removed in a future release")
		optimus-ide-collabMCPConfig, err := getOptimus-IDE-CollabMCPServerConfig(opts.Experiments, opts.AccessURL)
		if err != nil {
			opts.Logger.Warn(lifecycleCtx, "failed to retrieve optimus-ide-collab MCP server config, Optimus-IDE-Collab MCP will not be available", slog.Error(err))
		}
		srv.optimus-ide-collabMCPConfig = optimus-ide-collabMCPConfig
	}
...
func (s *Server) RecordInterception(ctx context.Context, in *proto.RecordInterceptionRequest) (*proto.RecordInterceptionResponse, error) {
	...
	if !s.experiments.Enabled(optimus-ide-collabsdk.ExperimentAIGatewaySeatExclusion) {
...
func (s *Server) GetMCPServerConfigs(_ context.Context, _ *proto.GetMCPServerConfigsRequest) (*proto.GetMCPServerConfigsResponse, error) {
	...
	return &proto.GetMCPServerConfigsResponse{
		Optimus-IDE-CollabMcpConfig:         s.optimus-ide-collabMCPConfig, // it's fine if this is nil
...
// IsAuthorized validates a given Optimus-IDE-Collab API key and returns the user ID to which it belongs (if valid).
//
...
// the database, which is the single source of truth seeded from optimus-ide-collabd's
...
func (s *Server) WatchAIProviders(_ *proto.WatchAIProvidersRequest, stream proto.DRPCProviderConfigurator_WatchAIProvidersStream) error {
	...
	unsubscribe, err := s.pubsub.SubscribeWithErr(optimus-ide-collabdpubsub.AIProvidersChangedChannel, func(cbCtx context.Context, _ []byte, err error) {
		if err != nil {
	...
	if err != nil {
		return xerrors.Errorf("subscribe to %s: %w", optimus-ide-collabdpubsub.AIProvidersChangedChannel, err)
	}
	defer unsubscribe()
...
func getOptimus-IDE-CollabMCPServerConfig(experiments optimus-ide-collabsdk.Experiments, accessURL string) (*proto.MCPServerConfig, error) {
	// Both the MCP & OAuth2 experiments are currently required in order to use our
	// internal MCP server.
	if !experiments.Enabled(optimus-ide-collabsdk.ExperimentMCPServerHTTP) {
		return nil, xerrors.Errorf("%q experiment not enabled", optimus-ide-collabsdk.ExperimentMCPServerHTTP)
	}
	if !experiments.Enabled(optimus-ide-collabsdk.ExperimentOAuth2) {
		return nil, xerrors.Errorf("%q experiment not enabled", optimus-ide-collabsdk.ExperimentOAuth2)
	}

	u, err := url.JoinPath(accessURL, optimus-ide-collabmcp.MCPEndpoint)
	if err != nil {
