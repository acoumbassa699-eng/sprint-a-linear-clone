...
func TestUpdateBlockedUsers(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			var err error
			for range tt.rounds {
				err = m.updateBlockedUsers(t.Context(), clk, db, optimus-ide-collabsdk.AIBudgetPeriodMonth)
			}
...
func TestStartBlockedUsersCollector(t *testing.T) {
	...
	t.Run("NilReceiverNoop", func(t *testing.T) {
		...
		closeFn := m.StartBlockedUsersCollector(t.Context(), testutil.Logger(t), quartz.NewMock(t), nil, optimus-ide-collabsdk.AIBudgetPeriodMonth, time.Minute)
	...
	t.Run("TicksAndStops", func(t *testing.T) {
		...
		closeFn := m.StartBlockedUsersCollector(ctx, testutil.Logger(t), clk, db, optimus-ide-collabsdk.AIBudgetPeriodMonth, time.Minute)
		defer closeFn()
