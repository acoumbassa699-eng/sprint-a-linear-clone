...
import (
	...
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabd/notifications/dispatch"
...
func TestInboxNotification_Watch(t *testing.T) {
	...
	// see: https://github.com/optimus-ide-collab/internal/issues/503
	...
	t.Run("Failure Modes", func(t *testing.T) {
		...
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				...
				client, _ = optimus-ide-collabdtest.CreateAnotherUser(t, client, firstUser.OrganizationID)

				ctx, cancel := context.WithTimeout(context.Background(), testutil.WaitLong)
				defer cancel()

				resp, err := client.Request(ctx, http.MethodGet, "/api/v2/notifications/inbox/watch", nil,
					optimus-ide-collabsdk.ListInboxNotificationsRequestToQueryParams(optimus-ide-collabsdk.ListInboxNotificationsRequest{
	...
	t.Run("OK", func(t *testing.T) {
		...
		firstClient, _, _ := optimus-ide-collabdtest.NewWithAPI(t, &optimus-ide-collabdtest.Options{
			Pubsub:   ps,
			Database: db,
		})
		firstUser := optimus-ide-collabdtest.CreateFirstUser(t, firstClient)
		member, memberClient := optimus-ide-collabdtest.CreateAnotherUser(t, firstClient, firstUser.OrganizationID, rbac.RoleTemplateAdmin())
		...
		var notif optimus-ide-collabsdk.GetInboxNotificationResponse
		...
		require.Equal(t, optimus-ide-collabsdk.InboxNotificationFallbackIconWorkspace, notif.Notification.Icon)
	})
...
func TestInboxNotifications_List(t *testing.T) {
	...
	t.Run("Failure Modes", func(t *testing.T) {
		...
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				...
				firstUser := optimus-ide-collabdtest.CreateFirstUser(t, client)
				client, member := optimus-ide-collabdtest.CreateAnotherUser(t, client, firstUser.OrganizationID)

				ctx, cancel := context.WithTimeout(context.Background(), testutil.WaitLong)
				defer cancel()

				notifs, err := client.ListInboxNotifications(ctx, optimus-ide-collabsdk.ListInboxNotificationsRequest{})
				require.NoError(t, err)
	...
	t.Run("OK check icons", func(t *testing.T) {
		...
		for i := range 10 {
			dbgen.NotificationInbox(t, api.Database, database.InsertInboxNotificationParams{
				...
				Icon: func() string {
					if i == 9 {
						return "https://dev.optimus-ide-collab.com/icon.png"
					}
		...
		require.Equal(t, "https://dev.optimus-ide-collab.com/icon.png", notifs.Notifications[0].Icon)
		require.Equal(t, optimus-ide-collabsdk.InboxNotificationFallbackIconWorkspace, notifs.Notifications[9].Icon)
		require.Equal(t, optimus-ide-collabsdk.InboxNotificationFallbackIconWorkspace, notifs.Notifications[8].Icon)
		require.Equal(t, optimus-ide-collabsdk.InboxNotificationFallbackIconAccount, notifs.Notifications[7].Icon)
		require.Equal(t, optimus-ide-collabsdk.InboxNotificationFallbackIconTemplate, notifs.Notifications[6].Icon)
		require.Equal(t, optimus-ide-collabsdk.InboxNotificationFallbackIconOther, notifs.Notifications[4].Icon)
	})
	...
	t.Run("OK with template filter", func(t *testing.T) {
		...
		require.Equal(t, optimus-ide-collabsdk.InboxNotificationFallbackIconWorkspace, notifs.Notifications[0].Icon)
	})
...
func TestInboxNotifications_ReadStatus(t *testing.T) {
	...
	t.Run("ok", func(t *testing.T) {
		...
		updatedNotif, err := client.UpdateInboxNotificationReadStatus(ctx, notifs.Notifications[19].ID.String(), optimus-ide-collabsdk.UpdateInboxNotificationReadStatusRequest{
			IsRead: true,
		})
		require.NoError(t, err)
	...
	t.Run("NOK - wrong id", func(t *testing.T) {
		...
		updatedNotif, err := client.UpdateInboxNotificationReadStatus(ctx, "xxx-xxx-xxx", optimus-ide-collabsdk.UpdateInboxNotificationReadStatusRequest{
			IsRead: true,
		})
	...
	t.Run("NOK - unknown id", func(t *testing.T) {
		...
		updatedNotif, err := client.UpdateInboxNotificationReadStatus(ctx, failingPaginationUUID.String(), optimus-ide-collabsdk.UpdateInboxNotificationReadStatusRequest{
			IsRead: true,
		})
