...
// @Produce json
// @Tags AI Providers
// @Success 200 {array} optimus-ide-collabsdk.AIProvider
...
func (api *API) aiProvidersList(rw http.ResponseWriter, r *http.Request) {
	...
	out := make([]optimus-ide-collabsdk.AIProvider, 0, len(rows))
...
// @Success 200 {object} optimus-ide-collabsdk.AIProvider
...
// @Produce json
// @Tags AI Providers
// @Param request body optimus-ide-collabsdk.CreateAIProviderRequest true "Create AI provider request"
// @Success 201 {object} optimus-ide-collabsdk.AIProvider
...
func (api *API) aiProvidersCreate(rw http.ResponseWriter, r *http.Request) {
	...
	var req optimus-ide-collabsdk.CreateAIProviderRequest
...
// @Tags AI Providers
// @Param idOrName path string true "Provider ID or name"
// @Param request body optimus-ide-collabsdk.UpdateAIProviderRequest true "Update AI provider request"
// @Success 200 {object} optimus-ide-collabsdk.AIProvider
...
func (api *API) aiProvidersUpdate(rw http.ResponseWriter, r *http.Request) {
	...
	var req optimus-ide-collabsdk.UpdateAIProviderRequest
	if !httpapi.Read(ctx, rw, r, &req) {
		return
	}
...
func (api *API) publishAIProvidersChanged(ctx context.Context) {
	...
	if err := api.Pubsub.Publish(optimus-ide-collabpubsub.AIProvidersChangedChannel, nil); err != nil {
...
func lookupAIProvider(ctx context.Context, store database.Store, idOrName string) (database.AIProvider, error) {
	...
	if !optimus-ide-collabsdk.AIProviderNameRegex.MatchString(idOrName) {
...
func writeAIProviderError(ctx context.Context, logger slog.Logger, rw http.ResponseWriter, err error, logMsg, userMsg string) {
	if errors.Is(err, errAIProviderInvalidName) {
		httpapi.Write(ctx, rw, http.StatusBadRequest, optimus-ide-collabsdk.Response{
			Message: fmt.Sprintf("Invalid provider id or name: must be a UUID or match %s.", optimus-ide-collabsdk.AIProviderNameRegex),
		})
		return
	}
...
func applyAIProviderKeyOps(ctx context.Context, tx database.Store, providerID uuid.UUID, muts []optimus-ide-collabsdk.AIProviderKeyMutation) ([]database.AIProviderKey, aiProviderKeyOpsAudit, aiProviderKeyChanges, error) {
	var (
...
func encodeAIProviderSettings(s optimus-ide-collabsdk.AIProviderSettings) (sql.NullString, error) {
	if s.IsZero() {
...
// in a single PATCH.
func mergeAIProviderSettings(existing, patch optimus-ide-collabsdk.AIProviderSettings) optimus-ide-collabsdk.AIProviderSettings {
	if patch.Bedrock == nil {
		// Patch carries no type-specific data; treat as a clear.
		return optimus-ide-collabsdk.AIProviderSettings{}
	}
	...
	return optimus-ide-collabsdk.AIProviderSettings{Bedrock: &merged}
}
...
// server-owned.
func validateBedrockExternalIDUnchanged(existing, patch optimus-ide-collabsdk.AIProviderSettings) error {
	stored := ""
	if existing.Bedrock != nil {
...
func ensureBedrockExternalID(s *optimus-ide-collabsdk.AIProviderSettings) {
