...
// @Success 200 {object} optimus-ide-collabsdk.AuditLogResponse
...
func (api *API) auditLogs(rw http.ResponseWriter, r *http.Request) {
	...
	if count == 0 {
		httpapi.Write(ctx, rw, http.StatusOK, optimus-ide-collabsdk.AuditLogResponse{
			AuditLogs: []optimus-ide-collabsdk.AuditLog{},
			Count:     0,
...
// @Accept json
// @Tags Audit
// @Param request body optimus-ide-collabsdk.CreateTestAuditLogRequest true "Audit log request"
// @Success 204
...
func (api *API) generateFakeAuditLog(rw http.ResponseWriter, r *http.Request) {
	...
	diff, err := json.Marshal(optimus-ide-collabsdk.AuditDiff{
		"foo": optimus-ide-collabsdk.AuditDiffField{Old: "bar", New: "baz"},
	})
	if err != nil {
		httpapi.InternalServerError(rw, err)
		return
	...
	var params optimus-ide-collabsdk.CreateTestAuditLogRequest
	if !httpapi.Read(ctx, rw, r, &params) {
		return
	}
	if params.Action == "" {
		params.Action = optimus-ide-collabsdk.AuditActionWrite
	}
	if params.ResourceType == "" {
		params.ResourceType = optimus-ide-collabsdk.ResourceTypeUser
	}
...
func (api *API) convertAuditLogs(ctx context.Context, dblogs []database.GetAuditLogsOffsetRow) []optimus-ide-collabsdk.AuditLog {
	alogs := make([]optimus-ide-collabsdk.AuditLog, 0, len(dblogs))
...
func (api *API) convertAuditLog(ctx context.Context, dblog database.GetAuditLogsOffsetRow) optimus-ide-collabsdk.AuditLog {
	ip, _ := netip.AddrFromSlice(dblog.AuditLog.Ip.IPNet.IP)

	diff := optimus-ide-collabsdk.AuditDiff{}
	...
	alog := optimus-ide-collabsdk.AuditLog{
		...
		ResourceType:     optimus-ide-collabsdk.ResourceType(dblog.AuditLog.ResourceType),
		ResourceID:       dblog.AuditLog.ResourceID,
		ResourceTarget:   dblog.AuditLog.ResourceTarget,
		ResourceIcon:     dblog.AuditLog.ResourceIcon,
		Action:           optimus-ide-collabsdk.AuditAction(dblog.AuditLog.Action),
		Diff:             diff,
	...
	if dblog.AuditLog.OrganizationID != uuid.Nil {
		alog.Organization = &optimus-ide-collabsdk.MinimalOrganization{
...
func auditLogDescription(alog database.GetAuditLogsOffsetRow) string {
	...
	case alog.AuditLog.StatusCode >= 400:
		...
		_, _ = b.WriteString(string(alog.AuditLog.Action))
	default:
		_, _ = b.WriteString(optimus-ide-collabsdk.AuditAction(alog.AuditLog.Action).Friendly())
	}
	...
	if alog.AuditLog.ResourceType == database.ResourceTypeGitSshKey {
		...
		_, _ = b.WriteString(optimus-ide-collabsdk.ResourceType(alog.AuditLog.ResourceType).FriendlyString())
		return b.String()
	}
...
func chatAuditLogDescription(alog database.GetAuditLogsOffsetRow) (string, bool) {
	...
	var diff optimus-ide-collabsdk.AuditDiff
