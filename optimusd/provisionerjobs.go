...
// @Success 200 {array} optimus-ide-collabsdk.ProvisionerJob
...
func (api *API) provisionerJobResources(rw http.ResponseWriter, r *http.Request, job database.ProvisionerJob) {
	...
	for _, resource := range resources {
		agents := make([]optimus-ide-collabsdk.WorkspaceAgent, 0)
		for _, agent := range resourceAgents {
...
func convertProvisionerJobLogs(provisionerJobLogs []database.ProvisionerJobLog) []optimus-ide-collabsdk.ProvisionerJobLog {
	sdk := make([]optimus-ide-collabsdk.ProvisionerJobLog, 0, len(provisionerJobLogs))
...
func convertProvisionerJobLog(provisionerJobLog database.ProvisionerJobLog) optimus-ide-collabsdk.ProvisionerJobLog {
	return optimus-ide-collabsdk.ProvisionerJobLog{
		ID:        provisionerJobLog.ID,
		CreatedAt: provisionerJobLog.CreatedAt,
		Source:    optimus-ide-collabsdk.LogSource(provisionerJobLog.Source),
		Level:     optimus-ide-collabsdk.LogLevel(provisionerJobLog.Level),
		Stage:     provisionerJobLog.Stage,
		Output:    provisionerJobLog.Output,
	}
}

func convertProvisionerJob(pj database.GetProvisionerJobsByIDsWithQueuePositionRow) optimus-ide-collabsdk.ProvisionerJob {
	...
	job := optimus-ide-collabsdk.ProvisionerJob{
		...
		Type:           optimus-ide-collabsdk.ProvisionerJobType(provisionerJob.Type),
		Error:          provisionerJob.Error.String,
		ErrorCode:      optimus-ide-collabsdk.JobErrorCode(provisionerJob.ErrorCode.String),
	...
	job.Status = optimus-ide-collabsdk.ProvisionerJobStatus(pj.ProvisionerJob.JobStatus)
...
func convertProvisionerJobWithQueuePosition(pj database.GetProvisionerJobsByOrganizationAndStatusWithQueuePositionAndProvisionerRow) optimus-ide-collabsdk.ProvisionerJob {
	...
	job.Metadata = optimus-ide-collabsdk.ProvisionerJobMetadata{
	...
	if pj.WorkspaceBuildTransition.Valid {
		job.Metadata.WorkspaceBuildTransition = optimus-ide-collabsdk.WorkspaceTransition(pj.WorkspaceBuildTransition.WorkspaceTransition)
	}
	return job
}
...
func jobIsComplete(logger slog.Logger, job database.ProvisionerJob) bool {
	status := optimus-ide-collabsdk.ProvisionerJobStatus(job.JobStatus)
	switch status {
	...
	case optimus-ide-collabsdk.ProvisionerJobPending:
		return false
	case optimus-ide-collabsdk.ProvisionerJobCanceling:
		return false
...
type logFollower struct {
	...
	conn      *websocket.Conn
	enc       *wsjson.Enoptimus-ide-collab[optimus-ide-collabsdk.ProvisionerJobLog]
...
func (f *logFollower) follow() {
	...
	if !f.complete {
		...
		if err != nil {
			httpapi.Write(f.ctx, f.rw, http.StatusInternalServerError, optimus-ide-collabsdk.Response{
	...
	if err != nil {
		httpapi.Write(f.ctx, f.rw, http.StatusBadRequest, optimus-ide-collabsdk.Response{
	...
	f.enc = wsjson.NewEnoptimus-ide-collab[optimus-ide-collabsdk.ProvisionerJobLog](f.conn, websocket.MessageText)
