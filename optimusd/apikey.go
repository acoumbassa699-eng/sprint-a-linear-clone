...
// @Param request body optimus-ide-collabsdk.CreateTokenRequest true "Create token request"
// @Success 201 {object} optimus-ide-collabsdk.GenerateAPIKeyResponse
...
func (api *API) postToken(rw http.ResponseWriter, r *http.Request) {
	...
	var createToken optimus-ide-collabsdk.CreateTokenRequest
	...
	// Default to optimus-ide-collab:all scopes for backward compatibility.
	...
	aReq.New = *key
	httpapi.Write(ctx, rw, http.StatusCreated, optimus-ide-collabsdk.GenerateAPIKeyResponse{Key: cookie.Value})
}
...
// @Success 200 {object} optimus-ide-collabsdk.APIKey
...
// @Tags Users
// @Param user path string true "User ID, name, or me"
// @Success 200 {array} optimus-ide-collabsdk.APIKey
...
func (api *API) tokens(rw http.ResponseWriter, r *http.Request) {
	...
	var apiKeys []optimus-ide-collabsdk.APIKeyWithOwner
	for _, key := range keys {
		if user, exists := usersByID[key.UserID]; exists {
			apiKeys = append(apiKeys, optimus-ide-collabsdk.APIKeyWithOwner{
...
// @Failure 500 {object} optimus-ide-collabsdk.Response
...
// @Tags General
// @Param user path string true "User ID, name, or me"
// @Success 200 {object} optimus-ide-collabsdk.TokenConfig
...
func (api *API) getMaxTokenLifetime(ctx context.Context, userID uuid.UUID) (time.Duration, error) {
	...
	for _, role := range roles {
		if role.Identifier.Name == optimus-ide-collabsdk.RoleOwner {
