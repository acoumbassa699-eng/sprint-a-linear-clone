...
import (
	...
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabd/aibridge"
	...
	"github.com/optimus-ide-collab/websocket/wsjson"
)
...
// newChatTestOptions builds optimus-ide-collabdtest options for chat runtime tests. Unless
// a test sets ChatProviderAPIKeys explicitly, it installs a fake
// OpenAI-compatible provider before optimus-ide-collabd starts so background chat work stays
// local, and the fake server outlives chatd during cleanup.
func newChatTestOptions(
	t testing.TB,
	values *optimus-ide-collabsdk.DeploymentValues,
	overrides ...func(*optimus-ide-collabdtest.Options),
) *optimus-ide-collabdtest.Options {
	t.Helper()

	// Enable experiment-gated chat endpoints in tests.
	if len(values.Experiments) == 0 {
		values.Experiments = serpent.StringArray{
			string(optimus-ide-collabsdk.ExperimentChatAdvisor),
			string(optimus-ide-collabsdk.ExperimentChatVirtualDesktop),
		}
	}
	...
	if opts.ChatProviderAPIKeys == nil {
		providerKeys := optimus-ide-collabdtest.FakeOpenAICompatProviderAPIKeys(t)
		opts.ChatProviderAPIKeys = &providerKeys
	}
	return opts
}

func newChatClient(t testing.TB, overrides ...func(*optimus-ide-collabdtest.Options)) *optimus-ide-collabsdk.ExperimentalClient {
	t.Helper()

	opts := newChatTestOptions(t, optimus-ide-collabdtest.DeploymentValues(t), overrides...)
	client, _, api := optimus-ide-collabdtest.NewWithAPI(t, opts)
	aibridgedtest.StartTestAIBridgeDaemon(t.Context(), t, api, nil)
	return optimus-ide-collabsdk.NewExperimentalClient(client)
}

func newChatClientWithAPI(t testing.TB, overrides ...func(*optimus-ide-collabdtest.Options)) (*optimus-ide-collabsdk.ExperimentalClient, *optimus-ide-collabd.API) {
	t.Helper()

	opts := newChatTestOptions(t, optimus-ide-collabdtest.DeploymentValues(t), overrides...)
	client, _, api := optimus-ide-collabdtest.NewWithAPI(t, opts)
	aibridgedtest.StartTestAIBridgeDaemon(t.Context(), t, api, nil)
	return optimus-ide-collabsdk.NewExperimentalClient(client), api
}

func newChatClientWithDeploymentValues(
	t testing.TB,
	values *optimus-ide-collabsdk.DeploymentValues,
) *optimus-ide-collabsdk.ExperimentalClient {
	t.Helper()
...
func newChatClientWithDatabase(t testing.TB, overrides ...func(*optimus-ide-collabdtest.Options)) (*optimus-ide-collabsdk.ExperimentalClient, database.Store) {
	t.Helper()

	opts := newChatTestOptions(t, optimus-ide-collabdtest.DeploymentValues(t), overrides...)
	client, _, api := optimus-ide-collabdtest.NewWithAPI(t, opts)
	aibridgedtest.StartTestAIBridgeDaemon(t.Context(), t, api, nil)
	return optimus-ide-collabsdk.NewExperimentalClient(client), api.Database
}

func newChatClientWithAPIAndDatabase(t testing.TB, overrides ...func(*optimus-ide-collabdtest.Options)) (*optimus-ide-collabsdk.ExperimentalClient, database.Store, *optimus-ide-collabd.API) {
	t.Helper()

	opts := newChatTestOptions(t, optimus-ide-collabdtest.DeploymentValues(t), overrides...)
	client, _, api := optimus-ide-collabdtest.NewWithAPI(t, opts)
	aibridgedtest.StartTestAIBridgeDaemon(t.Context(), t, api, nil)
	return optimus-ide-collabsdk.NewExperimentalClient(client), api.Database, api
}
...
func enableDailyChatUsageLimit(
	...
	_, err := db.UpsertChatUsageLimitConfig(
		...
		database.UpsertChatUsageLimitConfigParams{
			...
			Period:             string(optimus-ide-collabsdk.ChatUsageLimitPeriodDay),
		},
	)
	require.NoError(t, err)

	_, periodEnd := chatd.ComputeUsagePeriodBounds(time.Now(), optimus-ide-collabsdk.ChatUsageLimitPeriodDay)
	return periodEnd
}
...
func insertAssistantCostMessage(
	...
	assistantContent, err := chatprompt.MarshalParts([]optimus-ide-collabsdk.ChatMessagePart{
		optimus-ide-collabsdk.ChatMessageText("assistant"),
	})
	require.NoError(t, err)
...
func TestPostChats(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		memberClientRaw, member := optimus-ide-collabdtest.CreateAnotherUser(t, client.Client, firstUser.OrganizationID, rbac.ScopedRoleAgentsAccess(firstUser.OrganizationID))
		memberClient := optimus-ide-collabsdk.NewExperimentalClient(memberClientRaw)

		chat, err := memberClient.CreateChat(ctx, optimus-ide-collabsdk.CreateChatRequest{
			OrganizationID: firstUser.OrganizationID, Content: []optimus-ide-collabsdk.ChatInputPart{
				{
		...
		for _, message := range messagesResult.Messages {
			if message.Role != optimus-ide-collabsdk.ChatMessageRoleUser {
				continue
			}
	...
	t.Run("MemberWithoutAgentsAccess", func(t *testing.T) {
		...
		memberClientRaw, _ := optimus-ide-collabdtest.CreateAnotherUser(t, client.Client, firstUser.OrganizationID)
		memberClient := optimus-ide-collabsdk.NewExperimentalClient(memberClientRaw)

		_, err := memberClient.CreateChat(ctx, optimus-ide-collabsdk.CreateChatRequest{
	...
	t.Run("WithReasoningEffort", func(t *testing.T) {
		...
		user := optimus-ide-collabdtest.CreateFirstUser(t, client.Client)
		_ = createChatModelConfig(t, client)
	...
	t.Run("RejectsInvalidReasoningEffort", func(t *testing.T) {
		...
		_, err := client.CreateChat(ctx, optimus-ide-collabsdk.CreateChatRequest{
			OrganizationID: user.OrganizationID,
			Content: []optimus-ide-collabsdk.ChatInputPart{
				{
					Type: optimus-ide-collabsdk.ChatInputPartTypeText,
	...
	t.Run("HidesSystemPromptMessages", func(t *testing.T) {
		...
		require.NoError(t, err)
		for _, message := range messagesResult.Messages {
			require.NotEqual(t, optimus-ide-collabsdk.ChatMessageRoleSystem, message.Role)
		}
	})
	...
	t.Run("DisabledModelConfigRejected", func(t *testing.T) {
		...
		disabledConfig := createDisabledChatModelConfig(
			t,
			client,
			optimus-ide-collabdtest.TestChatProviderOpenAICompat,
	...
	t.Run("ProviderDisabledDefaultModelRejected", func(t *testing.T) {
		...
		_, err := client.UpdateAIProvider(ctx, defaultConfig.AIProviderID.String(), optimus-ide-collabsdk.UpdateAIProviderRequest{
	...
	t.Run("PerChatSystemPromptTooLong", func(t *testing.T) {
		...
		_, err := client.CreateChat(ctx, optimus-ide-collabsdk.CreateChatRequest{
			OrganizationID: user.OrganizationID, Content: []optimus-ide-collabsdk.ChatInputPart{
				{
					Type: optimus-ide-collabsdk.ChatInputPartTypeText,
					Text: "hello",
				},
	...
	t.Run("WorkspaceNotAccessible", func(t *testing.T) {
		...
		firstUser := optimus-ide-collabdtest.CreateFirstUser(t, adminClient.Client)
		memberClientRaw, _ := optimus-ide-collabdtest.CreateAnotherUser(t, adminClient.Client, firstUser.OrganizationID, rbac.ScopedRoleAgentsAccess(firstUser.OrganizationID))
		memberClient := optimus-ide-collabsdk.NewExperimentalClient(memberClientRaw)
	...
	t.Run("WorkspaceAccessibleButNoSSH", func(t *testing.T) {
		...
		adminClient, db := newChatClientWithDatabase(t)
		firstUser := optimus-ide-collabdtest.CreateFirstUser(t, adminClient.Client)
		orgAdminClientRaw, _ := optimus-ide-collabdtest.CreateAnotherUser(
			t,
			adminClient.Client,
		...
		orgAdminClient := optimus-ide-collabsdk.NewExperimentalClient(orgAdminClientRaw)
		...
		_, err := orgAdminClient.CreateChat(ctx, optimus-ide-collabsdk.CreateChatRequest{
			OrganizationID: firstUser.OrganizationID,
			Content: []optimus-ide-collabsdk.ChatInputPart{
				{
					Type: optimus-ide-collabsdk.ChatInputPartTypeText,
	...
	t.Run("UnsupportedPartType", func(t *testing.T) {
		...
		_, err := client.CreateChat(ctx, optimus-ide-collabsdk.CreateChatRequest{
			...
			Content: []optimus-ide-collabsdk.ChatInputPart{
				{
					Type: optimus-ide-collabsdk.ChatInputPartType("image"),
					Text: "hello",
				},
			},
		})
	...
	t.Run("NilOrganizationID", func(t *testing.T) {
		...
		memberClientRaw, _ := optimus-ide-collabdtest.CreateAnotherUser(t, client.Client, firstUser.OrganizationID, rbac.ScopedRoleAgentsAccess(firstUser.OrganizationID))
		memberClient := optimus-ide-collabsdk.NewExperimentalClient(memberClientRaw)
...
func TestPostChats_ClientType(t *testing.T) {
	...
	newChat := func(t *testing.T, clientType optimus-ide-collabsdk.ChatClientType) optimus-ide-collabsdk.Chat {
		t.Helper()
		ctx := testutil.Context(t, testutil.WaitLong)
		chat, err := memberClient.CreateChat(ctx, optimus-ide-collabsdk.CreateChatRequest{
			OrganizationID: firstUser.OrganizationID,
	...
	t.Run("DefaultIsAPI", func(t *testing.T) {
		...
		chat := newChat(t, "")
		require.Equal(t, optimus-ide-collabsdk.ChatClientTypeAPI, chat.ClientType)

		got, err := memberClient.GetChat(ctx, chat.ID)
		require.NoError(t, err)
		require.Equal(t, optimus-ide-collabsdk.ChatClientTypeAPI, got.ClientType)
	})
	...
	t.Run("ExplicitAPI", func(t *testing.T) {
		...
		chat := newChat(t, optimus-ide-collabsdk.ChatClientTypeAPI)
		require.Equal(t, optimus-ide-collabsdk.ChatClientTypeAPI, chat.ClientType)
	...
	t.Run("ExplicitUI", func(t *testing.T) {
		...
		chat := newChat(t, optimus-ide-collabsdk.ChatClientTypeUI)
		require.Equal(t, optimus-ide-collabsdk.ChatClientTypeUI, chat.ClientType)

		got, err := memberClient.GetChat(ctx, chat.ID)
		require.NoError(t, err)
		require.Equal(t, optimus-ide-collabsdk.ChatClientTypeUI, got.ClientType)
	})
...
func TestListChats(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		firstChatA, err := client.CreateChat(ctx, optimus-ide-collabsdk.CreateChatRequest{
			OrganizationID: firstUser.OrganizationID,
			Content: []optimus-ide-collabsdk.ChatInputPart{
				{
					Type: optimus-ide-collabsdk.ChatInputPartTypeText,
		...
		firstChatB, err := client.CreateChat(ctx, optimus-ide-collabsdk.CreateChatRequest{
			OrganizationID: firstUser.OrganizationID,
			Content: []optimus-ide-collabsdk.ChatInputPart{
				{
					Type: optimus-ide-collabsdk.ChatInputPartTypeText,
		...
		chatsByID := make(map[uuid.UUID]optimus-ide-collabsdk.Chat, len(chats))
		for i, chat := range chats {
			...
			// reached by now.
			require.Contains(t, []optimus-ide-collabsdk.ChatStatus{
				optimus-ide-collabsdk.ChatStatusRunning,
				optimus-ide-collabsdk.ChatStatusError,
				optimus-ide-collabsdk.ChatStatusWaiting,
	...
	t.Run("SourceCreatedByMeAndSharedWithMeExcludesUnsharedReadableChats", func(t *testing.T) {
		...
		firstUser := optimus-ide-collabdtest.CreateFirstUser(t, client.Client)
		modelConfig := createChatModelConfig(t, client)
		ownerClientRaw, owner := optimus-ide-collabdtest.CreateAnotherUser(t, client.Client, firstUser.OrganizationID, rbac.RoleOwner())
		ownerClient := optimus-ide-collabsdk.NewExperimentalClient(ownerClientRaw)
		memberClientRaw, member := optimus-ide-collabdtest.CreateAnotherUser(t, client.Client, firstUser.OrganizationID, rbac.ScopedRoleAgentsAccess(firstUser.OrganizationID))
		memberClient := optimus-ide-collabsdk.NewExperimentalClient(memberClientRaw)
		...
		ownerChats, err := ownerClient.ListChats(ctx, &optimus-ide-collabsdk.ListChatsOptions{
		...
		sharedOnlyChats, err := ownerClient.ListChats(ctx, &optimus-ide-collabsdk.ListChatsOptions{
		...
		memberChats, err := memberClient.ListChats(ctx, &optimus-ide-collabsdk.ListChatsOptions{
			Query: "source:created_by_me,shared_with_me",
		})
		require.NoError(t, err)
	...
	t.Run("OrgMemberWithoutAgentsAccessCannotAccessOwnChats", func(t *testing.T) {
		...
		memberClientRaw, member := optimus-ide-collabdtest.CreateAnotherUser(t, client.Client, firstUser.OrganizationID)
		memberClient := optimus-ide-collabsdk.NewExperimentalClient(memberClientRaw)
		...
		err = memberClient.UpdateChat(ctx, chat.ID, optimus-ide-collabsdk.UpdateChatRequest{
	...
	t.Run("Unauthenticated", func(t *testing.T) {
		...
		ctx := testutil.Context(t, testutil.WaitLong)
		client := newChatClient(t)
		_ = optimus-ide-collabdtest.CreateFirstUser(t, client.Client)

		unauthenticatedClient := optimus-ide-collabsdk.NewExperimentalClient(optimus-ide-collabsdk.New(client.URL))
	...
	t.Run("Pagination", func(t *testing.T) {
		...
		page1, err := client.ListChats(ctx, &optimus-ide-collabsdk.ListChatsOptions{
		...
		page2, err := client.ListChats(ctx, &optimus-ide-collabsdk.ListChatsOptions{
		...
		page3, err := client.ListChats(ctx, &optimus-ide-collabsdk.ListChatsOptions{
			Pagination: optimus-ide-collabsdk.Pagination{
		...
		offsetPage, err := client.ListChats(ctx, &optimus-ide-collabsdk.ListChatsOptions{
			Pagination: optimus-ide-collabsdk.Pagination{Offset: 3, Limit: 2},
		})
		require.NoError(t, err)
	...
	t.Run("PinnedOnFirstPage", func(t *testing.T) {
		...
		err := client.UpdateChat(ctx, pinnedDBChat.ID, optimus-ide-collabsdk.UpdateChatRequest{
		...
		page1, err := client.ListChats(ctx, &optimus-ide-collabsdk.ListChatsOptions{
			Pagination: optimus-ide-collabsdk.Pagination{Limit: 50},
		})
		require.NoError(t, err)
	...
	t.Run("CursorWithPins", func(t *testing.T) {
		...
		err := client.UpdateChat(ctx, createdChatIDs[0], optimus-ide-collabsdk.UpdateChatRequest{
			PinOrder: ptr.Ref(int32(1)),
		})
		require.NoError(t, err)
		err = client.UpdateChat(ctx, createdChatIDs[1], optimus-ide-collabsdk.UpdateChatRequest{
			PinOrder: ptr.Ref(int32(1)),
		})
		require.NoError(t, err)
		...
		var allPaginated []optimus-ide-collabsdk.Chat
		var afterID uuid.UUID
		for range maxPages {
			opts := &optimus-ide-collabsdk.ListChatsOptions{
				Pagination: optimus-ide-collabsdk.Pagination{Limit: pageSize},
			}
			if afterID != uuid.Nil {
	...
	t.Run("ChildChatsEmbeddedNotStandalone", func(t *testing.T) {
		...
		parentChat, err := client.CreateChat(ctx, optimus-ide-collabsdk.CreateChatRequest{
			OrganizationID: user.OrganizationID,
			Content: []optimus-ide-collabsdk.ChatInputPart{
				{
					Type: optimus-ide-collabsdk.ChatInputPartTypeText,
		...
		standalone, err := client.CreateChat(ctx, optimus-ide-collabsdk.CreateChatRequest{
			OrganizationID: user.OrganizationID,
			Content: []optimus-ide-collabsdk.ChatInputPart{
				{
					Type: optimus-ide-collabsdk.ChatInputPartTypeText,
		...
		var parent optimus-ide-collabsdk.Chat
		for _, c := range chats {
	...
	t.Run("PaginationCountsOnlyRootChats", func(t *testing.T) {
		...
		for i := range 3 {
			parent, err := client.CreateChat(ctx, optimus-ide-collabsdk.CreateChatRequest{
				OrganizationID: user.OrganizationID,
				Content: []optimus-ide-collabsdk.ChatInputPart{
					{
						Type: optimus-ide-collabsdk.ChatInputPartTypeText,
		...
		chats, err := client.ListChats(ctx, &optimus-ide-collabsdk.ListChatsOptions{
			Pagination: optimus-ide-collabsdk.Pagination{Limit: 2},
		})
		require.NoError(t, err)
	...
	t.Run("DiffURLFilter", func(t *testing.T) {
			...
			if url != "" {
				...
				_, err := db.UpsertChatDiffStatusReference(
					...
					database.UpsertChatDiffStatusReferenceParams{
						...
						GitRemoteOrigin: "git@github.com:optimus-ide-collab/optimus-ide-collab.git",
						StaleAt:         staleAt,
					},
				)
				require.NoError(t, err)
		...
		rootWithPR := create("root with pr", "https://github.com/optimus-ide-collab/optimus-ide-collab/pull/1", uuid.NullUUID{})
		...
		_ = create(
			"sub-agent with pr",
			"https://github.com/optimus-ide-collab/optimus-ide-collab/pull/2",
			uuid.NullUUID{UUID: rootWithChildPR.ID, Valid: true},
		)

		// Root chat with an unrelated PR; should not match either filter.
		_ = create("unrelated pr", "https://github.com/optimus-ide-collab/optimus-ide-collab/pull/999", uuid.NullUUID{})
		...
		archivedWithPR := create(
			"archived with pr",
			"https://github.com/optimus-ide-collab/optimus-ide-collab/pull/3",
			uuid.NullUUID{},
		)
		require.NoError(t, client.UpdateChat(ctx, archivedWithPR.ID, optimus-ide-collabsdk.UpdateChatRequest{
			Archived: ptr.Ref(true),
		}))

		t.Run("MatchesRoot", func(t *testing.T) {
			chats, err := client.ListChats(ctx, &optimus-ide-collabsdk.ListChatsOptions{
				Query: `diff_url:"https://github.com/optimus-ide-collab/optimus-ide-collab/pull/1"`,
			})
			require.NoError(t, err)
		...
		t.Run("MatchesViaSubAgent", func(t *testing.T) {
			chats, err := client.ListChats(ctx, &optimus-ide-collabsdk.ListChatsOptions{
				Query: `diff_url:"https://github.com/optimus-ide-collab/optimus-ide-collab/pull/2"`,
			})
			require.NoError(t, err)
		...
		t.Run("NoMatch", func(t *testing.T) {
			chats, err := client.ListChats(ctx, &optimus-ide-collabsdk.ListChatsOptions{
				Query: `diff_url:"https://github.com/optimus-ide-collab/optimus-ide-collab/pull/424242"`,
			})
			require.NoError(t, err)
		...
		t.Run("InvalidURL", func(t *testing.T) {
			_, err := client.ListChats(ctx, &optimus-ide-collabsdk.ListChatsOptions{
		...
		t.Run("ArchivedFilteredOut", func(t *testing.T) {
			...
			chats, err := client.ListChats(ctx, &optimus-ide-collabsdk.ListChatsOptions{
				Query: `diff_url:"https://github.com/optimus-ide-collab/optimus-ide-collab/pull/3"`,
			})
			require.NoError(t, err)
		...
		t.Run("ArchivedTrueComposes", func(t *testing.T) {
			chats, err := client.ListChats(ctx, &optimus-ide-collabsdk.ListChatsOptions{
				Query: `archived:true diff_url:"https://github.com/optimus-ide-collab/optimus-ide-collab/pull/3"`,
			})
			require.NoError(t, err)
			require.Len(t, chats, 1)
...
func TestListChatModels(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		var openAIProvider *optimus-ide-collabsdk.ChatModelProvider
		for i := range models.Providers {
			if models.Providers[i].Provider == optimus-ide-collabdtest.TestChatProviderOpenAICompat {
		...
		for _, model := range openAIProvider.Models {
			if model.Provider == optimus-ide-collabdtest.TestChatProviderOpenAICompat && model.Model == modelConfig.Model {
				foundModel = true
				break
			}
		}
	...
	t.Run("CopilotOnlyUnsupported", func(t *testing.T) {
		...
		_ = createAIProviderForTest(t, client, string(optimus-ide-collabsdk.AIProviderTypeCopilot), "")

		models, err := client.ListChatModels(ctx)
		require.NoError(t, err)

		require.False(t, slices.ContainsFunc(models.Providers, func(p optimus-ide-collabsdk.ChatModelProvider) bool {
			return p.Provider == string(optimus-ide-collabsdk.AIProviderTypeCopilot)
		}), "copilot must not appear in the supported model picker")

		require.Equal(t, []optimus-ide-collabsdk.ChatUnsupportedProvider{
			{
				Provider:    "copilot",
	...
	t.Run("UserOnlyProviderRequiresUserKey", func(t *testing.T) {
		...
		_, err := client.CreateChatModelConfig(ctx, optimus-ide-collabsdk.CreateChatModelConfigRequest{
		...
		var anthropicProvider *optimus-ide-collabsdk.ChatModelProvider
		for i := range models.Providers {
		...
		require.Equal(t, optimus-ide-collabsdk.ChatModelProviderUnavailableReasonUserAPIKeyRequired, anthropicProvider.UnavailableReason)

		_, err = client.UpsertUserAIProviderKey(ctx, "me", provider.ID, optimus-ide-collabsdk.CreateUserAIProviderKeyRequest{
			APIKey: "user-api-key",
		})
		require.NoError(t, err)
	...
	t.Run("CentralAndUserWithFallback", func(t *testing.T) {
		...
		var googleProvider *optimus-ide-collabsdk.ChatModelProvider
		for i := range models.Providers {
	...
	t.Run("DisabledProvidersAndModelsAreFilteredOut", func(t *testing.T) {
		...
		enabled := false
		_, err = client.UpdateAIProvider(ctx, provider.ID.String(), optimus-ide-collabsdk.UpdateAIProviderRequest{
			Enabled: &enabled,
		})
		require.NoError(t, err)
...
func TestListChats_Search(t *testing.T) {
	...
	setup := func(t *testing.T) (context.Context, *optimus-ide-collabsdk.ExperimentalClient, database.Store, optimus-ide-collabsdk.CreateFirstUserResponse, optimus-ide-collabsdk.ChatModelConfig) {
		t.Helper()
		ctx := testutil.Context(t, testutil.WaitLong)
		client, db := newChatClientWithDatabase(t)
		firstUser := optimus-ide-collabdtest.CreateFirstUser(t, client.Client)
	...
	createChat := func(t *testing.T, db database.Store, firstUser optimus-ide-collabsdk.CreateFirstUserResponse, modelConfigID uuid.UUID, title string) database.Chat {
		t.Helper()
	...
	insertMessage := func(t *testing.T, db database.Store, firstUser optimus-ide-collabsdk.CreateFirstUserResponse, modelConfigID, chatID uuid.UUID, text string) {
		t.Helper()
	...
	chatIDs := func(chats []optimus-ide-collabsdk.Chat) map[uuid.UUID]struct{} {
	...
	t.Run("ComposesWithRepoFilterAndArchivedDefault", func(t *testing.T) {
		...
		bothMatch := createChat(t, db, firstUser, modelConfig.ID, "kubernetes in optimus-ide-collab repo")
...
func TestWatchChats(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		createdChat, err := client.CreateChat(ctx, optimus-ide-collabsdk.CreateChatRequest{
			OrganizationID: firstUser.OrganizationID,
			Content: []optimus-ide-collabsdk.ChatInputPart{
				{
					Type: optimus-ide-collabsdk.ChatInputPartTypeText,
		...
		for {
			...
			if payload.Kind == optimus-ide-collabsdk.ChatWatchEventKindCreated &&
	...
	t.Run("CreatedEventIncludesAllChatFields", func(t *testing.T) {
		...
		var got optimus-ide-collabsdk.Chat
		...
		require.Equal(t, optimus-ide-collabsdk.ChatStatusRunning, got.Status)
	...
	t.Run("DiffStatusChangeIncludesDiffStatusAndOmitsInjectedContext", func(t *testing.T) {
		...
		ctx := testutil.Context(t, testutil.WaitLong)
		rawClient, _, api := optimus-ide-collabdtest.NewWithAPI(t, &optimus-ide-collabdtest.Options{
			DeploymentValues: optimus-ide-collabdtest.DeploymentValues(t),
		})
		aibridgedtest.StartTestAIBridgeDaemon(t.Context(), t, api, nil)
		client := optimus-ide-collabsdk.NewExperimentalClient(rawClient)
		db := api.Database
		...
		_, err := db.UpsertChatDiffStatusReference(
			...
			database.UpsertChatDiffStatusReferenceParams{
				ChatID:          chat.ID,
				Url:             sql.NullString{String: "https://github.com/optimus-ide-collab/optimus-ide-collab/pull/99", Valid: true},
				GitBranch:       "feature/test",
				GitRemoteOrigin: "git@github.com:optimus-ide-collab/optimus-ide-collab.git",
				StaleAt:         staleAt,
			},
		...
		var received optimus-ide-collabsdk.ChatWatchEvent
		for {
			err = wsjson.Read(ctx, conn, &received)
			require.NoError(t, err)

			if received.Kind == optimus-ide-collabsdk.ChatWatchEventKindDiffStatusChange &&
		...
		require.Equal(t, "https://github.com/optimus-ide-collab/optimus-ide-collab/pull/99", *ds.URL)
	...
	t.Run("ArchiveAndUnarchiveEmitEventsForDescendants", func(t *testing.T) {
		...
		optimus-ide-collabdtest.WaitForChatSettled(ctx, t, api, parentChat.ID)
		...
		collectLifecycleEvents := func(expectedKind optimus-ide-collabsdk.ChatWatchEventKind) map[uuid.UUID]optimus-ide-collabsdk.ChatWatchEvent {
			t.Helper()

			events := make(map[uuid.UUID]optimus-ide-collabsdk.ChatWatchEvent, 3)
			for len(events) < 3 {
				var payload optimus-ide-collabsdk.ChatWatchEvent
				err = wsjson.Read(ctx, conn, &payload)
				require.NoError(t, err)
		...
		assertLifecycleEvents := func(events map[uuid.UUID]optimus-ide-collabsdk.ChatWatchEvent, archived bool) {
			t.Helper()
		...
		err = client.UpdateChat(ctx, parentChat.ID, optimus-ide-collabsdk.UpdateChatRequest{Archived: ptr.Ref(true)})
		require.NoError(t, err)
		deletedEvents := collectLifecycleEvents(optimus-ide-collabsdk.ChatWatchEventKindDeleted)
		assertLifecycleEvents(deletedEvents, true)

		err = client.UpdateChat(ctx, parentChat.ID, optimus-ide-collabsdk.UpdateChatRequest{Archived: ptr.Ref(false)})
		require.NoError(t, err)
		createdEvents := collectLifecycleEvents(optimus-ide-collabsdk.ChatWatchEventKindCreated)
	...
	t.Run("Unauthenticated", func(t *testing.T) {
		...
		unauthenticatedClient := optimus-ide-collabsdk.New(client.URL)
...
func TestUserAIProviderKeys(t *testing.T) {
	...
	createOpenAIProvider := func(t *testing.T, client *optimus-ide-collabsdk.ExperimentalClient, name string, enabled bool, apiKeys ...string) optimus-ide-collabsdk.AIProvider {
		t.Helper()

		provider, err := client.CreateAIProvider(testutil.Context(t, testutil.WaitLong), optimus-ide-collabsdk.CreateAIProviderRequest{
	...
	findUserAIProviderKeyConfig := func(
		t *testing.T,
		configs []optimus-ide-collabsdk.UserAIProviderKeyConfig,
		providerID uuid.UUID,
	) *optimus-ide-collabsdk.UserAIProviderKeyConfig {
		t.Helper()
	...
	t.Run("SelfServiceLifecycle", func(t *testing.T) {
		...
		firstUser := optimus-ide-collabdtest.CreateFirstUser(t, adminClient.Client)
		memberClientRaw, _ := optimus-ide-collabdtest.CreateAnotherUser(t, adminClient.Client, firstUser.OrganizationID)
		memberClient := optimus-ide-collabsdk.NewExperimentalClient(memberClientRaw)
		...
		cfgValue, err := memberClient.UpsertUserAIProviderKey(ctx, "me", provider.ID, optimus-ide-collabsdk.CreateUserAIProviderKeyRequest{APIKey: "test-user-api-key"})
		require.NoError(t, err)
		...
		cfgValue, err = memberClient.UpsertUserAIProviderKey(ctx, "me", provider.ID, optimus-ide-collabsdk.CreateUserAIProviderKeyRequest{APIKey: "replacement-user-api-key"})
		require.NoError(t, err)
		require.Equal(t, provider.ID, cfgValue.Provider.ID)
		require.True(t, cfgValue.HasUserAPIKey)
	...
	t.Run("ListsDisabledProviderWithSavedUserKey", func(t *testing.T) {
		...
		_, err := memberClient.UpsertUserAIProviderKey(ctx, "me", provider.ID, optimus-ide-collabsdk.CreateUserAIProviderKeyRequest{APIKey: "test-user-api-key"})
		require.NoError(t, err)

		enabled := false
		_, err = adminClient.UpdateAIProvider(ctx, provider.ID.String(), optimus-ide-collabsdk.UpdateAIProviderRequest{Enabled: &enabled})
		require.NoError(t, err)
	...
	t.Run("RejectsLargeAPIKey", func(t *testing.T) {
		...
		_, err := memberClient.UpsertUserAIProviderKey(ctx, "me", provider.ID, optimus-ide-collabsdk.CreateUserAIProviderKeyRequest{APIKey: strings.Repeat("x", 10241)})
		sdkErr := requireSDKError(t, err, http.StatusBadRequest)
	...
	t.Run("RejectsWhitespaceAPIKey", func(t *testing.T) {
		...
		_, err := memberClient.UpsertUserAIProviderKey(ctx, "me", provider.ID, optimus-ide-collabsdk.CreateUserAIProviderKeyRequest{APIKey: "  "})
		sdkErr := requireSDKError(t, err, http.StatusBadRequest)
	...
	t.Run("BYOKDisabledRejectsUpsertAndAllowsDelete", func(t *testing.T) {
		...
		client := newChatClient(t, func(o *optimus-ide-collabdtest.Options) {
		...
		_, err := client.UpsertUserAIProviderKey(ctx, "me", provider.ID, optimus-ide-collabsdk.CreateUserAIProviderKeyRequest{APIKey: "test-user-api-key"})
		sdkErr := requireSDKError(t, err, http.StatusForbidden)
...
func TestListChatProviders(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		var openAIProvider *optimus-ide-collabsdk.ChatProviderConfig
		for i := range providers {
			if providers[i].Provider == optimus-ide-collabdtest.TestChatProviderOpenAICompat {
				openAIProvider = &providers[i]
				break
			}
		}
		require.NotNil(t, openAIProvider)
		require.Equal(t, optimus-ide-collabsdk.ChatProviderConfigSourceDatabase, openAIProvider.Source)
	...
	t.Run("IgnoresDeploymentKeyWhenCentralKeyDisabled", func(t *testing.T) {
		...
		provider, err := client.CreateChatProvider(ctx, optimus-ide-collabsdk.CreateChatProviderConfigRequest{
			Provider:             "openai",
...
func TestCreateChatProvider(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		require.Equal(t, optimus-ide-collabsdk.ChatProviderConfigSourceDatabase, provider.Source)
	})
	...
	t.Run("RejectsMissingCentralAPIKeyForNonBedrock", func(t *testing.T) {
		...
		_, err := client.CreateChatProvider(ctx, optimus-ide-collabsdk.CreateChatProviderConfigRequest{
			Provider:             "openai",
			DisplayName:          "OpenAI",
			CentralAPIKeyEnabled: ptr.Ref(true),
		})
	...
	t.Run("ForbiddenForOrganizationMember", func(t *testing.T) {
		...
		_, err := memberClient.CreateChatProvider(ctx, optimus-ide-collabsdk.CreateChatProviderConfigRequest{
			Provider: "openai",
			APIKey:   "member-key",
		})
		requireSDKError(t, err, http.StatusForbidden)
...
func TestUpdateChatProvider(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		updated, err := client.UpdateChatProvider(ctx, provider.ID, optimus-ide-collabsdk.UpdateChatProviderConfigRequest{
	...
	t.Run("NotFound", func(t *testing.T) {
		...
		_, err := client.UpdateChatProvider(ctx, uuid.New(), optimus-ide-collabsdk.UpdateChatProviderConfigRequest{
			DisplayName: "missing",
		})
		requireSDKError(t, err, http.StatusNotFound)
	})
	...
	t.Run("InvalidProviderID", func(t *testing.T) {
		...
		res, err := client.Request(
			...
			optimus-ide-collabsdk.UpdateChatProviderConfigRequest{DisplayName: "ignored"},
		)
		require.NoError(t, err)
		defer res.Body.Close()
	...
	t.Run("ForbiddenForOrganizationMember", func(t *testing.T) {
		...
		provider, err := adminClient.CreateChatProvider(ctx, optimus-ide-collabsdk.CreateChatProviderConfigRequest{
			Provider: "openai",
			APIKey:   "test-api-key",
		})
		require.NoError(t, err)

		_, err = memberClient.UpdateChatProvider(ctx, provider.ID, optimus-ide-collabsdk.UpdateChatProviderConfigRequest{
	...
	t.Run("RejectsDeploymentBackedCentralKey", func(t *testing.T) {
		...
		_, err = client.UpdateChatProvider(ctx, provider.ID, optimus-ide-collabsdk.UpdateChatProviderConfigRequest{
			CentralAPIKeyEnabled: ptr.Ref(true),
		})
		sdkErr := requireSDKError(t, err, http.StatusBadRequest)
		require.Equal(t, missingCentralKeyMessage, sdkErr.Message)
...
func TestChatProviderAPIKeysFromDeploymentValues(t *testing.T) {
	...
	t.Run("DoesNotReuseBridgeConfig", func(t *testing.T) {
		...
		keys := optimus-ide-collabd.ChatProviderAPIKeysFromDeploymentValues(values)
	...
	t.Run("NilDeploymentValues", func(t *testing.T) {
		...
		keys := optimus-ide-collabd.ChatProviderAPIKeysFromDeploymentValues(nil)
		require.Equal(t, chatprovider.ProviderAPIKeys{}, keys)
	})
}
...
func TestUserChatProviderConfigs(t *testing.T) {
	...
	requireUserProviderConfig := func(t *testing.T, configs []optimus-ide-collabsdk.UserChatProviderConfig, provider string) optimus-ide-collabsdk.UserChatProviderConfig {
		t.Helper()
		...
		return optimus-ide-collabsdk.UserChatProviderConfig{}
	}

	requireNoUserProviderConfig := func(t *testing.T, configs []optimus-ide-collabsdk.UserChatProviderConfig, provider string) {
		t.Helper()
	...
	t.Run("ListOnlyUserKeyProviders", func(t *testing.T) {
		...
		anthropicProvider, err := client.CreateChatProvider(ctx, optimus-ide-collabsdk.CreateChatProviderConfigRequest{
			Provider:             "anthropic",
			CentralAPIKeyEnabled: ptr.Ref(false),
			AllowUserAPIKey:      ptr.Ref(true),
		})
	...
	t.Run("ListHidesDisabledProviderEvenWithSavedKey", func(t *testing.T) {
		...
		_, err = client.UpsertUserChatProviderKey(ctx, provider.ID, optimus-ide-collabsdk.CreateUserChatProviderKeyRequest{
			APIKey: "user-key",
		})
		require.NoError(t, err)
	...
	t.Run("UpsertCreatesKey", func(t *testing.T) {
		...
		config, err := client.UpsertUserChatProviderKey(ctx, provider.ID, optimus-ide-collabsdk.CreateUserChatProviderKeyRequest{
			APIKey: "user-key",
		})
		require.NoError(t, err)
	...
	t.Run("UpsertRejectsMissingProvider", func(t *testing.T) {
		...
		_, err := client.UpsertUserChatProviderKey(ctx, uuid.New(), optimus-ide-collabsdk.CreateUserChatProviderKeyRequest{
			APIKey: "user-key",
		})
		requireSDKError(t, err, http.StatusNotFound)
	})
	...
	t.Run("OtherUserDoesNotSeeKey", func(t *testing.T) {
		...
		_, err = adminClient.UpsertUserChatProviderKey(ctx, provider.ID, optimus-ide-collabsdk.CreateUserChatProviderKeyRequest{
...
func TestListChatModelConfigs(t *testing.T) {
	...
	t.Run("AdminIncludesDisabledModelConfigs", func(t *testing.T) {
		...
		contextLimit := int64(4096)
		enabled := false
		disabledConfig, err := client.CreateChatModelConfig(ctx, optimus-ide-collabsdk.CreateChatModelConfigRequest{
	...
	t.Run("NonAdminExcludesDisabledModelConfigs", func(t *testing.T) {
		...
		contextLimit := int64(4096)
		enabled := false
		_, err := adminClient.CreateChatModelConfig(ctx, optimus-ide-collabsdk.CreateChatModelConfigRequest{
	...
	t.Run("DeserializesLegacyPricingJSON", func(t *testing.T) {
		...
		requireChatModelPricing(t, configs[0].ModelConfig, &optimus-ide-collabsdk.ChatModelCallConfig{
...
func TestCreateChatModelConfig(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		contextLimit := int64(4096)
		isDefault := true
		pricing := &optimus-ide-collabsdk.ChatModelCallConfig{
			Cost: &optimus-ide-collabsdk.ModelCostConfig{
				InputPricePerMillionTokens:      decRef("0.15"),
				OutputPricePerMillionTokens:     decRef("0.6"),
				CacheReadPricePerMillionTokens:  decRef("0.03"),
	...
	t.Run("ConcurrentCreatesElectSingleDefault", func(t *testing.T) {
		...
		const creators = 10
		contextLimit := int64(4096)
		var claimed optimus-ide-collabsdk.ChatModelConfig
		...
		eg.Go(func() error {
			...
			claimed, err = client.UpdateChatModelConfig(ctx, created.ID, optimus-ide-collabsdk.UpdateChatModelConfigRequest{
	...
	t.Run("ReasoningEffortStored", func(t *testing.T) {
		...
		modelConfig, err := client.CreateChatModelConfig(ctx, optimus-ide-collabsdk.CreateChatModelConfigRequest{
			...
			ContextLimit: &contextLimit,
			ModelConfig: &optimus-ide-collabsdk.ChatModelCallConfig{
				ReasoningEffort: &optimus-ide-collabsdk.ChatModelReasoningEffortConfig{
	...
	t.Run("RejectsOpenRouterMisconfiguredAsOpenAI", func(t *testing.T) {
		...
		aiProvider, err := client.CreateAIProvider(ctx, optimus-ide-collabsdk.CreateAIProviderRequest{
			Type:    optimus-ide-collabsdk.AIProviderTypeOpenAI,
			Name:    "openrouter",
			Enabled: true,
	...
	t.Run("ForbiddenForOrganizationMember", func(t *testing.T) {
		...
		contextLimit := int64(4096)
		_, err := memberClient.CreateChatModelConfig(ctx, optimus-ide-collabsdk.CreateChatModelConfigRequest{
			AIProviderID: &aiProvider.ID,
			Model:        "gpt-4o-mini",
			ContextLimit: &contextLimit,
		})
...
func TestUpdateChatModelConfig(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		updated, err := client.UpdateChatModelConfig(ctx, modelConfig.ID, optimus-ide-collabsdk.UpdateChatModelConfigRequest{
	...
	t.Run("RejectsOpenRouterMisconfiguredAsOpenAI", func(t *testing.T) {
		...
		_, err = client.UpdateChatModelConfig(ctx, modelConfig.ID, optimus-ide-collabsdk.UpdateChatModelConfigRequest{
			Model: "anthropic/claude-opus-4.6",
		})
		sdkErr := requireSDKError(t, err, http.StatusBadRequest)
		require.Equal(t, "OpenRouter-like provider configured as type openai does not support slash-namespaced models.", sdkErr.Message)
	...
	t.Run("RejectsProviderChangeToMisconfiguredOpenAI", func(t *testing.T) {
		...
		validProvider, err := client.CreateAIProvider(ctx, optimus-ide-collabsdk.CreateAIProviderRequest{
			Type:    optimus-ide-collabsdk.AIProviderTypeOpenrouter,
		...
		require.NoError(t, err)
		misconfiguredProvider, err := client.CreateAIProvider(ctx, optimus-ide-collabsdk.CreateAIProviderRequest{
			Type:    optimus-ide-collabsdk.AIProviderTypeOpenAI,
			Name:    "openrouter",
			Enabled: true,
			BaseURL: "https://openrouter.ai/api/v1",
	...
	t.Run("DisablePreservesRecordAndHidesItFromNonAdmins", func(t *testing.T) {
		...
		enabled := false
		updated, err := adminClient.UpdateChatModelConfig(ctx, modelConfig.ID, optimus-ide-collabsdk.UpdateChatModelConfigRequest{
			Enabled: &enabled,
		})
		require.NoError(t, err)
		require.Equal(t, modelConfig.ID, updated.ID)
	...
	t.Run("ReEnableRestoresVisibilityForNonAdmins", func(t *testing.T) {
		...
		contextLimit := int64(4096)
		enabled := false
		modelConfig, err := adminClient.CreateChatModelConfig(ctx, optimus-ide-collabsdk.CreateChatModelConfigRequest{
			AIProviderID: &aiProvider.ID,
	...
	t.Run("UpdateAIProviderID", func(t *testing.T) {
		...
		modelConfig := createChatModelConfig(t, client)
		provider, err := client.CreateAIProvider(ctx, optimus-ide-collabsdk.CreateAIProviderRequest{
			Type:    optimus-ide-collabsdk.AIProviderTypeAnthropic,
	...
	t.Run("InternalServerErrorWhenDefaultCandidateDisappearsInTx", func(t *testing.T) {
		...
		contextLimit := int64(4096)
		isDefault := false
		candidateConfig, err := client.CreateChatModelConfig(ctx, optimus-ide-collabsdk.CreateChatModelConfigRequest{
			AIProviderID: &aiProvider.ID,
		...
		_, err = client.UpdateChatModelConfig(ctx, defaultConfig.ID, optimus-ide-collabsdk.UpdateChatModelConfigRequest{
	...
	t.Run("NotFound", func(t *testing.T) {
		...
		_, err := client.UpdateChatModelConfig(ctx, uuid.New(), optimus-ide-collabsdk.UpdateChatModelConfigRequest{
			DisplayName: "missing",
		})
		requireSDKError(t, err, http.StatusNotFound)
	})
	...
	t.Run("InvalidModelConfigID", func(t *testing.T) {
		...
		res, err := client.Request(
			...
			optimus-ide-collabsdk.UpdateChatModelConfigRequest{DisplayName: "ignored"},
		)
		require.NoError(t, err)
		defer res.Body.Close()
	...
	t.Run("ForbiddenForOrganizationMember", func(t *testing.T) {
		...
		modelConfig := createChatModelConfig(t, adminClient)
		_, err := memberClient.UpdateChatModelConfig(ctx, modelConfig.ID, optimus-ide-collabsdk.UpdateChatModelConfigRequest{
			DisplayName: "member update",
		})
		requireSDKError(t, err, http.StatusForbidden)
	})
...
func TestGetChat(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		for _, message := range messagesResult.Messages {
			...
			for _, part := range message.Content {
				if message.Role == optimus-ide-collabsdk.ChatMessageRoleUser &&
					part.Type == optimus-ide-collabsdk.ChatMessagePartTypeText &&
	...
	t.Run("NotFoundForDifferentUser", func(t *testing.T) {
		...
		otherClientRaw, _ := optimus-ide-collabdtest.CreateAnotherUser(t, client.Client, firstUser.OrganizationID, rbac.ScopedRoleAgentsAccess(firstUser.OrganizationID))
		otherClient := optimus-ide-collabsdk.NewExperimentalClient(otherClientRaw)
	...
	t.Run("AIGatewayDisabled", func(t *testing.T) {
		...
		opts := newChatTestOptions(t, values)
		rawClient, _, api := optimus-ide-collabdtest.NewWithAPI(t, opts)
		client := optimus-ide-collabsdk.NewExperimentalClient(rawClient)
		firstUser := optimus-ide-collabdtest.CreateFirstUser(t, client.Client)
	...
	t.Run("FilesHydrated", func(t *testing.T) {
		...
		chat, err := client.CreateChat(ctx, optimus-ide-collabsdk.CreateChatRequest{
			OrganizationID: firstUser.OrganizationID,
			Content: []optimus-ide-collabsdk.ChatInputPart{
				{Type: optimus-ide-collabsdk.ChatInputPartTypeText, Text: "check file hydration"}, {Type: optimus-ide-collabsdk.ChatInputPartTypeFile, FileID: uploadResp.ID},
			},
		})
		require.NoError(t, err)
	...
	t.Run("ToolCreatedFilesLinked", func(t *testing.T) {
		...
		chat, err := client.CreateChat(ctx, optimus-ide-collabsdk.CreateChatRequest{
			OrganizationID: firstUser.OrganizationID,
			Content: []optimus-ide-collabsdk.ChatInputPart{
				{Type: optimus-ide-collabsdk.ChatInputPartTypeText, Text: "tool file test"},
			},
		})
		require.NoError(t, err)
		...
		rejected, err := store.LinkChatFiles(chatdCtx, database.LinkChatFilesParams{
			ChatID:       chat.ID,
			MaxFileLinks: int32(optimus-ide-collabsdk.MaxChatFileIDs),
		...
		for i := 1; i < optimus-ide-collabsdk.MaxChatFileIDs; i++ {
		...
		chatResult, err = client.GetChat(ctx, chat.ID)
		require.NoError(t, err)
		require.Len(t, chatResult.Files, optimus-ide-collabsdk.MaxChatFileIDs)
...
func TestGetChatUserPrompts(t *testing.T) {
	...
	t.Run("NewestFirstFiltering", func(t *testing.T) {
		...
		want1 := insertUserMessage(t, ctx, db, chat.ID, modelConfig.ID, user.UserID,
			[]optimus-ide-collabsdk.ChatMessagePart{
				{Type: optimus-ide-collabsdk.ChatMessagePartTypeText, Text: "first "},
				{Type: optimus-ide-collabsdk.ChatMessagePartTypeText, Text: "prompt"},
			},
		...
		want2 := insertUserMessage(t, ctx, db, chat.ID, modelConfig.ID, user.UserID,
			[]optimus-ide-collabsdk.ChatMessagePart{
				{Type: optimus-ide-collabsdk.ChatMessagePartTypeText, Text: "hello "},
				{Type: optimus-ide-collabsdk.ChatMessagePartTypeFile, MediaType: "text/plain", Data: []byte("x")},
				{Type: optimus-ide-collabsdk.ChatMessagePartTypeText, Text: "world"},
			},
			database.ChatMessageVisibilityBoth, false,
		)
		...
		insertUserMessage(t, ctx, db, chat.ID, modelConfig.ID, user.UserID,
			[]optimus-ide-collabsdk.ChatMessagePart{
				{Type: optimus-ide-collabsdk.ChatMessagePartTypeText, Text: "   \n\t  "},
			},
			database.ChatMessageVisibilityBoth, false,
		)
		...
		// the response.
		assistantContent, err := chatprompt.MarshalParts([]optimus-ide-collabsdk.ChatMessagePart{
			{Type: optimus-ide-collabsdk.ChatMessagePartTypeText, Text: "assistant reply"},
		})
		require.NoError(t, err)
		...
		insertUserMessage(t, ctx, db, chat.ID, modelConfig.ID, user.UserID,
			[]optimus-ide-collabsdk.ChatMessagePart{
				{Type: optimus-ide-collabsdk.ChatMessagePartTypeText, Text: "deleted prompt"},
			},
		...
		insertUserMessage(t, ctx, db, chat.ID, modelConfig.ID, user.UserID,
			[]optimus-ide-collabsdk.ChatMessagePart{
				{Type: optimus-ide-collabsdk.ChatMessagePartTypeText, Text: "model only"},
			},
		...
		want3 := insertUserMessage(t, ctx, db, chat.ID, modelConfig.ID, user.UserID,
			[]optimus-ide-collabsdk.ChatMessagePart{
				{Type: optimus-ide-collabsdk.ChatMessagePartTypeText, Text: "newest prompt"},
			},
	...
	t.Run("LimitClampsResults", func(t *testing.T) {
		...
		for i := range 5 {
			insertUserMessage(t, ctx, db, chat.ID, modelConfig.ID, user.UserID,
				[]optimus-ide-collabsdk.ChatMessagePart{
					{Type: optimus-ide-collabsdk.ChatMessagePartTypeText, Text: fmt.Sprintf("prompt %d", i)},
				},
				database.ChatMessageVisibilityBoth, false,
			)
		}

		resp, err := client.GetChatPrompts(ctx, chat.ID, &optimus-ide-collabsdk.ChatPromptsOptions{Limit: 2})
		require.NoError(t, err)
	...
	t.Run("InvalidLimitRejected", func(t *testing.T) {
		...
		_, err = client.GetChatPrompts(ctx, chat.ID, &optimus-ide-collabsdk.ChatPromptsOptions{Limit: 5000})
		require.Error(t, err)
	...
	t.Run("NotFoundForOtherUsers", func(t *testing.T) {
		...
		insertUserMessage(t, ctx, db, chat.ID, modelConfig.ID, firstUser.UserID,
			[]optimus-ide-collabsdk.ChatMessagePart{
				{Type: optimus-ide-collabsdk.ChatMessagePartTypeText, Text: "private prompt"},
			},
			database.ChatMessageVisibilityBoth, false,
		)

		memberClient, _ := optimus-ide-collabdtest.CreateAnotherUser(t, client.Client, firstUser.OrganizationID)
		memberExp := optimus-ide-collabsdk.NewExperimentalClient(memberClient)
...
func TestPatchChat(t *testing.T) {
	...
	createChat := func(ctx context.Context, t *testing.T, client *optimus-ide-collabsdk.ExperimentalClient, orgID uuid.UUID, text string) optimus-ide-collabsdk.Chat {
		t.Helper()
	...
	getChat := func(ctx context.Context, t *testing.T, client *optimus-ide-collabsdk.ExperimentalClient, chatID uuid.UUID) optimus-ide-collabsdk.Chat {
		t.Helper()
	...
	t.Run("PlanMode", func(t *testing.T) {
		...
		t.Run("SetToPlan", func(t *testing.T) {
			...
			err := client.UpdateChat(ctx, chat.ID, optimus-ide-collabsdk.UpdateChatRequest{
				PlanMode: ptr.Ref(optimus-ide-collabsdk.ChatPlanModePlan),
			})
			require.NoError(t, err)

			updated := getChat(ctx, t, client, chat.ID)
			require.Equal(t, optimus-ide-collabsdk.ChatPlanModePlan, updated.PlanMode)
			require.True(t, mAudit.Contains(t, database.AuditLog{
		...
		t.Run("Clear", func(t *testing.T) {
			...
			err = client.UpdateChat(ctx, chat.ID, optimus-ide-collabsdk.UpdateChatRequest{
				PlanMode: ptr.Ref(optimus-ide-collabsdk.ChatPlanMode("")),
			})
			require.NoError(t, err)
		...
		t.Run("RejectsInvalidValue", func(t *testing.T) {
			...
			err := client.UpdateChat(ctx, chat.ID, optimus-ide-collabsdk.UpdateChatRequest{
				PlanMode: ptr.Ref(optimus-ide-collabsdk.ChatPlanMode("invalid")),
			})
			sdkErr := requireSDKError(t, err, http.StatusBadRequest)
	...
	t.Run("Title", func(t *testing.T) {
		...
		t.Run("Rename", func(t *testing.T) {
			...
			optimus-ide-collabdtest.WaitForChatSettled(ctx, t, api, chat.ID)
		...
		t.Run("PreservesUpdatedAt", func(t *testing.T) {
			...
			providerKeys := optimus-ide-collabdtest.FakeOpenAICompatProviderAPIKeys(t)
			clientRaw, _, api := optimus-ide-collabdtest.NewWithAPI(t, &optimus-ide-collabdtest.Options{
				DeploymentValues:    optimus-ide-collabdtest.DeploymentValues(t),
				Database:            db,
				Pubsub:              ps,
			...
			aibridgedtest.StartTestAIBridgeDaemon(t.Context(), t, api, nil)
			client := optimus-ide-collabsdk.NewExperimentalClient(clientRaw)
			firstUser := optimus-ide-collabdtest.CreateFirstUser(t, client.Client)
			_ = createChatModelConfig(t, client)
		...
		t.Run("PublishesWatchEvent", func(t *testing.T) {
			...
			go func() {
				_ = client.UpdateChat(ctx, chat.ID, optimus-ide-collabsdk.UpdateChatRequest{
			...
			for {
				...
				if received.Kind == optimus-ide-collabsdk.ChatWatchEventKindTitleChange &&
					received.Chat.ID == chat.ID {
...
func TestArchiveChat(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		ctx := testutil.Context(t, testutil.WaitLong)
		mAudit := audit.NewMock()
		client, api := newChatClientWithAPI(t, func(o *optimus-ide-collabdtest.Options) {
			o.Auditor = mAudit
		})
		firstUser := optimus-ide-collabdtest.CreateFirstUser(t, client.Client)
		_ = createChatModelConfig(t, client)

		chatToArchive, err := client.CreateChat(ctx, optimus-ide-collabsdk.CreateChatRequest{
			OrganizationID: firstUser.OrganizationID,
			Content: []optimus-ide-collabsdk.ChatInputPart{
				{
					Type: optimus-ide-collabsdk.ChatInputPartTypeText,
		...
		chatToKeep, err := client.CreateChat(ctx, optimus-ide-collabsdk.CreateChatRequest{
			OrganizationID: firstUser.OrganizationID,
			Content: []optimus-ide-collabsdk.ChatInputPart{
				{
					Type: optimus-ide-collabsdk.ChatInputPartTypeText,
		...
		require.NoError(t, err)
		optimus-ide-collabdtest.WaitForChatSettled(ctx, t, api, chatToArchive.ID)
		optimus-ide-collabdtest.WaitForChatSettled(ctx, t, api, chatToKeep.ID)

		chatsBeforeArchive, err := client.ListChats(ctx, nil)
		require.NoError(t, err)
		require.Len(t, chatsBeforeArchive, 2)

		err = client.UpdateChat(ctx, chatToArchive.ID, optimus-ide-collabsdk.UpdateChatRequest{Archived: ptr.Ref(true)})
		require.NoError(t, err)
		...
		activeChats, err := client.ListChats(ctx, &optimus-ide-collabsdk.ListChatsOptions{
		...
		archivedChats, err := client.ListChats(ctx, &optimus-ide-collabsdk.ListChatsOptions{
			Query: "archived:true",
		})
		require.NoError(t, err)
	...
	t.Run("NotFound", func(t *testing.T) {
		...
		err := client.UpdateChat(ctx, uuid.New(), optimus-ide-collabsdk.UpdateChatRequest{Archived: ptr.Ref(true)})
		requireSDKError(t, err, http.StatusNotFound)
	})
	...
	t.Run("ArchivesChildren", func(t *testing.T) {
		...
		require.NoError(t, err)
		var foundParent *optimus-ide-collabsdk.Chat
	...
	t.Run("AllowsChildChatArchiveIndividually", func(t *testing.T) {
		...
		err = client.UpdateChat(ctx, child.ID, optimus-ide-collabsdk.UpdateChatRequest{Archived: ptr.Ref(true)})
		requireSDKError(t, err, http.StatusBadRequest)
...
func TestUnarchiveChat(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		err = client.UpdateChat(ctx, chat.ID, optimus-ide-collabsdk.UpdateChatRequest{Archived: ptr.Ref(true)})
		require.NoError(t, err)
		...
		require.True(t, archivedChats[0].Archived)
		// Unarchive the chat.
		err = client.UpdateChat(ctx, chat.ID, optimus-ide-collabsdk.UpdateChatRequest{Archived: ptr.Ref(false)})
		require.NoError(t, err)
	...
	t.Run("RejectsChildChatWhenParentArchived", func(t *testing.T) {
		...
		// is also excluded).
		err = client.UpdateChat(ctx, child.ID, optimus-ide-collabsdk.UpdateChatRequest{Archived: ptr.Ref(false)})
		requireSDKError(t, err, http.StatusBadRequest)
	...
	t.Run("NotFound", func(t *testing.T) {
		...
		err := client.UpdateChat(ctx, uuid.New(), optimus-ide-collabsdk.UpdateChatRequest{Archived: ptr.Ref(false)})
		requireSDKError(t, err, http.StatusNotFound)
	})
}
...
func TestChatPinOrder(t *testing.T) {
	...
	createChat := func(ctx context.Context, t *testing.T, client *optimus-ide-collabsdk.ExperimentalClient, orgID uuid.UUID, title string) optimus-ide-collabsdk.Chat {
		t.Helper()
	...
	t.Run("PinReorderAndUnpin", func(t *testing.T) {
		...
		err := client.UpdateChat(ctx, first.ID, optimus-ide-collabsdk.UpdateChatRequest{PinOrder: ptr.Ref(int32(1))})
		require.NoError(t, err)
		err = client.UpdateChat(ctx, second.ID, optimus-ide-collabsdk.UpdateChatRequest{PinOrder: ptr.Ref(int32(1))})
		require.NoError(t, err)
		err = client.UpdateChat(ctx, third.ID, optimus-ide-collabsdk.UpdateChatRequest{PinOrder: ptr.Ref(int32(1))})
		require.NoError(t, err)
		...
		err = client.UpdateChat(ctx, first.ID, optimus-ide-collabsdk.UpdateChatRequest{PinOrder: ptr.Ref(int32(0))})
		require.NoError(t, err)
	...
	t.Run("ArchiveClearsPinOrder", func(t *testing.T) {
		...
		optimus-ide-collabdtest.WaitForChatSettled(ctx, t, api, first.ID)
		optimus-ide-collabdtest.WaitForChatSettled(ctx, t, api, second.ID)
		...
		err = client.UpdateChat(ctx, first.ID, optimus-ide-collabsdk.UpdateChatRequest{Archived: ptr.Ref(true)})
		require.NoError(t, err)
	...
	t.Run("RejectsNegative", func(t *testing.T) {
		...
		err := client.UpdateChat(ctx, chat.ID, optimus-ide-collabsdk.UpdateChatRequest{PinOrder: ptr.Ref(int32(-1))})
		sdkErr := requireSDKError(t, err, http.StatusBadRequest)
	...
	t.Run("RejectsChildChat", func(t *testing.T) {
		...
		err := client.UpdateChat(ctx, child.ID, optimus-ide-collabsdk.UpdateChatRequest{PinOrder: ptr.Ref(int32(1))})
...
func TestPostChatMessages(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		hasTextPart := func(parts []optimus-ide-collabsdk.ChatMessagePart, want string) bool {
			for _, part := range parts {
				if part.Type == optimus-ide-collabsdk.ChatMessagePartTypeText && part.Text == want {
					return true
				}
			}
			return false
		...
		created, err := client.CreateChatMessage(ctx, chat.ID, optimus-ide-collabsdk.CreateChatMessageRequest{
			Content: []optimus-ide-collabsdk.ChatInputPart{
				{
					Type: optimus-ide-collabsdk.ChatInputPartTypeText,
					Text: messageText,
		...
		if created.Queued {
			...
			require.Eventually(t, func() bool {
				...
				for _, message := range messagesResult.Messages {
					if message.Role == optimus-ide-collabsdk.ChatMessageRoleUser && hasTextPart(message.Content, messageText) {
						return true
					}
				}
				return false
			...
			require.Equal(t, optimus-ide-collabsdk.ChatMessageRoleUser, created.Message.Role)
			...
			require.Eventually(t, func() bool {
				...
				for _, message := range messagesResult.Messages {
					if message.ID == created.Message.ID &&
						message.Role == optimus-ide-collabsdk.ChatMessageRoleUser &&
	...
	t.Run("ProviderDisabledModelConfigRejected", func(t *testing.T) {
		...
		_, err = client.CreateChatMessage(ctx, chat.ID, optimus-ide-collabsdk.CreateChatMessageRequest{
			Content: []optimus-ide-collabsdk.ChatInputPart{{
				Type: optimus-ide-collabsdk.ChatInputPartTypeText,
	...
	t.Run("MemberWithoutAgentsAccess", func(t *testing.T) {
		...
		_, err := memberClient.CreateChatMessage(ctx, chat.ID, optimus-ide-collabsdk.CreateChatMessageRequest{
			Content: []optimus-ide-collabsdk.ChatInputPart{
				{
					Type: optimus-ide-collabsdk.ChatInputPartTypeText,
					Text: "this should fail",
	...
	t.Run("ChatNotFound", func(t *testing.T) {
		...
		_, err := client.CreateChatMessage(ctx, uuid.New(), optimus-ide-collabsdk.CreateChatMessageRequest{
			Content: []optimus-ide-collabsdk.ChatInputPart{
				{
					Type: optimus-ide-collabsdk.ChatInputPartTypeText,
					Text: "hello",
...
func waitForChatWatchStatusChangeEvent(
	...
	for {
		...
		err := wsjson.Read(ctx, conn, &payload)
		require.NoError(t, err)
		if payload.Kind == optimus-ide-collabsdk.ChatWatchEventKindStatusChange && payload.Chat.ID == chatID {
			return payload
		}
	}
}
...
func TestSendMessageWithModelOverrideUpdatesLastModelConfigID(t *testing.T) {
	...
	modelConfigB := createAdditionalChatModelConfig(t, client, optimus-ide-collabdtest.TestChatProviderOpenAICompat, "gpt-4o-mini-override-"+uuid.NewString())
	...
	resp, err := client.CreateChatMessage(ctx, chat.ID, optimus-ide-collabsdk.CreateChatMessageRequest{
		Content: []optimus-ide-collabsdk.ChatInputPart{{
			Type: optimus-ide-collabsdk.ChatInputPartTypeText,
...
func TestSendMessageWithReasoningEffortUpdatesLastReasoningEffort(t *testing.T) {
	...
	_, err = client.CreateChatMessage(ctx, chat.ID, optimus-ide-collabsdk.CreateChatMessageRequest{
		...
		BusyBehavior: optimus-ide-collabsdk.ChatBusyBehaviorInterrupt,
	})
	require.NoError(t, err)
...
func TestSendMessageQueuesReasoningEffort(t *testing.T) {
	...
	resp, err := client.CreateChatMessage(ctx, chat.ID, optimus-ide-collabsdk.CreateChatMessageRequest{
		...
		ReasoningEffort: ptr.Ref("high"),
		BusyBehavior:    optimus-ide-collabsdk.ChatBusyBehaviorQueue,
	})
	require.NoError(t, err)
...
func TestSendMessageQueuesEffectiveModelConfigID(t *testing.T) {
	...
	user := optimus-ide-collabdtest.CreateFirstUser(t, client.Client)
	modelConfigA := createChatModelConfig(t, client)
	modelConfigB := createAdditionalChatModelConfig(t, client, optimus-ide-collabdtest.TestChatProviderOpenAICompat, "gpt-4o-mini-queued-"+uuid.NewString())
...
func TestQueuedMessageWithoutOverrideCapturesEnqueueTimeModel(t *testing.T) {
	...
	user := optimus-ide-collabdtest.CreateFirstUser(t, client.Client)
	modelConfigA := createChatModelConfig(t, client)
	modelConfigB := createAdditionalChatModelConfig(t, client, optimus-ide-collabdtest.TestChatProviderOpenAICompat, "gpt-4o-mini-later-"+uuid.NewString())
...
func TestSubsequentSendWithoutOverrideUsesPersistedModel(t *testing.T) {
	...
	user := optimus-ide-collabdtest.CreateFirstUser(t, client.Client)
	_ = createChatModelConfig(t, client)
	modelConfigB := createAdditionalChatModelConfig(t, client, optimus-ide-collabdtest.TestChatProviderOpenAICompat, "gpt-4o-mini-persisted-"+uuid.NewString())
...
func TestWatchChatsStatusChangeCarriesUpdatedLastModelConfigID(t *testing.T) {
	...
	t.Run("DirectSend", func(t *testing.T) {
		...
		user := optimus-ide-collabdtest.CreateFirstUser(t, client.Client)
		modelConfigA := createChatModelConfig(t, client)
		modelConfigB := createAdditionalChatModelConfig(t, client, optimus-ide-collabdtest.TestChatProviderOpenAICompat, "gpt-4o-mini-watch-direct-"+uuid.NewString())
	...
	t.Run("QueuedPromotion", func(t *testing.T) {
		...
		user := optimus-ide-collabdtest.CreateFirstUser(t, client.Client)
		modelConfigA := createChatModelConfig(t, client)
		modelConfigB := createAdditionalChatModelConfig(t, client, optimus-ide-collabdtest.TestChatProviderOpenAICompat, "gpt-4o-mini-watch-promote-"+uuid.NewString())
		...
		queuedResp, err := client.CreateChatMessage(ctx, chat.ID, optimus-ide-collabsdk.CreateChatMessageRequest{
			Content: []optimus-ide-collabsdk.ChatInputPart{{
				Type: optimus-ide-collabsdk.ChatInputPartTypeText,
...
func TestChatMessageWithFileReferences(t *testing.T) {
	...
	createChatForTest := func(t *testing.T, client *optimus-ide-collabsdk.ExperimentalClient, orgID uuid.UUID) optimus-ide-collabsdk.Chat {
		t.Helper()
		ctx := testutil.Context(t, testutil.WaitLong)
		chat, err := client.CreateChat(ctx, optimus-ide-collabsdk.CreateChatRequest{
			OrganizationID: orgID,
	...
	t.Run("FileReferenceOnly", func(t *testing.T) {
		...
		created, err := client.CreateChatMessage(ctx, chat.ID, optimus-ide-collabsdk.CreateChatMessageRequest{
			Content: []optimus-ide-collabsdk.ChatInputPart{{
				Type:      optimus-ide-collabsdk.ChatInputPartTypeFileReference,
				FileName:  "main.go",
				StartLine: 10,
				EndLine:   15,
				Content:   "func broken() {}",
		...
		checkFileRef := func(part optimus-ide-collabsdk.ChatMessagePart) bool {
			return part.Type == optimus-ide-collabsdk.ChatMessagePartTypeFileReference &&
	...
	t.Run("InterleavedTextAndFileReferences", func(t *testing.T) {
		...
		type wantPart struct {
			typ       optimus-ide-collabsdk.ChatMessagePartType
			text      string
			fileName  string
			startLine int
			endLine   int
		...
		want := []wantPart{
			{typ: optimus-ide-collabsdk.ChatMessagePartTypeText, text: "Please review these two issues:"},
			{typ: optimus-ide-collabsdk.ChatMessagePartTypeFileReference, fileName: "a.go", startLine: 1, endLine: 3, content: "line1\nline2\nline3"},
			{typ: optimus-ide-collabsdk.ChatMessagePartTypeText, text: "first issue"},
			{typ: optimus-ide-collabsdk.ChatMessagePartTypeText, text: "and also:"},
			{typ: optimus-ide-collabsdk.ChatMessagePartTypeFileReference, fileName: "b.go", startLine: 10, endLine: 10, content: "return nil"},
			{typ: optimus-ide-collabsdk.ChatMessagePartTypeText, text: "second issue"},
		}
		...
		require.Eventually(t, func() bool {
			...
			checkParts := func(parts []optimus-ide-collabsdk.ChatMessagePart) bool {
				...
				for i, w := range want {
					...
					switch w.typ {
					case optimus-ide-collabsdk.ChatMessagePartTypeText:
						if p.Text != w.text {
							return false
						}
					case optimus-ide-collabsdk.ChatMessagePartTypeFileReference:
			...
			for _, msg := range messagesResult.Messages {
				if msg.Role == optimus-ide-collabsdk.ChatMessageRoleUser && checkParts(msg.Content) {
					return true
				}
			}
			if created.Queued && created.QueuedMessage != nil {
...
func TestChatMessageWithFiles(t *testing.T) {
	...
	t.Run("FileOnly", func(t *testing.T) {
		...
		if resp.Queued {
			...
			require.NotNil(t, resp.Message)
			require.Equal(t, optimus-ide-collabsdk.ChatMessageRoleUser, resp.Message.Role)
		}
	})
	...
	t.Run("TextAndFile", func(t *testing.T) {
		...
		for _, msg := range messagesResult.Messages {
			for _, part := range msg.Content {
				if part.Type == optimus-ide-collabsdk.ChatMessagePartTypeFile {
	...
	t.Run("PasteOnlyOnCreate", func(t *testing.T) {
		...
		uploadResp, err := client.UploadChatFile(
			...
			strings.NewReader("Fix the flaky test in optimus-ide-collabd please"),
		)
		require.NoError(t, err)
		...
		require.Equal(t, "Fix the flaky test in optimus-ide-collabd…", chat.Title)
	})
	...
	t.Run("UnsupportedPromptInputFileType", func(t *testing.T) {
		...
		chat, err := client.CreateChat(ctx, optimus-ide-collabsdk.CreateChatRequest{
			OrganizationID: firstUser.OrganizationID,
			Content: []optimus-ide-collabsdk.ChatInputPart{
				{Type: optimus-ide-collabsdk.ChatInputPartTypeText, Text: "initial message"},
			},
		})
		require.NoError(t, err)
		...
		_, err = client.CreateChatMessage(ctx, chat.ID, optimus-ide-collabsdk.CreateChatMessageRequest{
			Content: []optimus-ide-collabsdk.ChatInputPart{
				{Type: optimus-ide-collabsdk.ChatInputPartTypeFile, FileID: fileRow.ID},
			},
		})
		sdkErr := requireSDKError(t, err, http.StatusBadRequest)
		require.Equal(t, "Invalid input part.", sdkErr.Message)
	...
	t.Run("FilesLinkedOnSend", func(t *testing.T) {
		...
		chat, err := client.CreateChat(ctx, optimus-ide-collabsdk.CreateChatRequest{
			OrganizationID: firstUser.OrganizationID,
			Content: []optimus-ide-collabsdk.ChatInputPart{
				{Type: optimus-ide-collabsdk.ChatInputPartTypeText, Text: "no files yet"},
			},
		})
		require.NoError(t, err)
		...
		_, err = client.CreateChatMessage(ctx, chat.ID, optimus-ide-collabsdk.CreateChatMessageRequest{
			Content: []optimus-ide-collabsdk.ChatInputPart{
				{Type: optimus-ide-collabsdk.ChatInputPartTypeText, Text: "here is a file"},
				{Type: optimus-ide-collabsdk.ChatInputPartTypeFile, FileID: uploadResp.ID},
			},
		})
		require.NoError(t, err)
	...
	t.Run("DedupFileIDs", func(t *testing.T) {
		...
		chat, err := client.CreateChat(ctx, optimus-ide-collabsdk.CreateChatRequest{
			OrganizationID: firstUser.OrganizationID,
			Content: []optimus-ide-collabsdk.ChatInputPart{
				{Type: optimus-ide-collabsdk.ChatInputPartTypeText, Text: "first mention"}, {Type: optimus-ide-collabsdk.ChatInputPartTypeFile, FileID: uploadResp.ID},
			},
		})
		require.NoError(t, err)

		// Send another message with the SAME file.
		msgResp, err := client.CreateChatMessage(ctx, chat.ID, optimus-ide-collabsdk.CreateChatMessageRequest{
			Content: []optimus-ide-collabsdk.ChatInputPart{
				{Type: optimus-ide-collabsdk.ChatInputPartTypeText, Text: "same file again"},
				{Type: optimus-ide-collabsdk.ChatInputPartTypeFile, FileID: uploadResp.ID},
			},
		})
		require.NoError(t, err)
	...
	t.Run("FileCapExceeded", func(t *testing.T) {
		...
		fileIDs := make([]uuid.UUID, 0, optimus-ide-collabsdk.MaxChatFileIDs)
		for i := range optimus-ide-collabsdk.MaxChatFileIDs {
		...
		parts := []optimus-ide-collabsdk.ChatInputPart{
			{Type: optimus-ide-collabsdk.ChatInputPartTypeText, Text: "max files"},
		}
		for _, fid := range fileIDs {
			parts = append(parts, optimus-ide-collabsdk.ChatInputPart{Type: optimus-ide-collabsdk.ChatInputPartTypeFile, FileID: fid})
		}
		chat, err := client.CreateChat(ctx, optimus-ide-collabsdk.CreateChatRequest{OrganizationID: firstUser.OrganizationID, Content: parts})
		require.NoError(t, err)
		require.Empty(t, chat.Warnings, "creating a chat at exactly the cap should not warn")
		require.Len(t, chat.Files, optimus-ide-collabsdk.MaxChatFileIDs, "all files should be linked on creation")
		...
		msgResp, err := client.CreateChatMessage(ctx, chat.ID, optimus-ide-collabsdk.CreateChatMessageRequest{
			Content: []optimus-ide-collabsdk.ChatInputPart{
				{Type: optimus-ide-collabsdk.ChatInputPartTypeText, Text: "one too many"},
				{Type: optimus-ide-collabsdk.ChatInputPartTypeFile, FileID: extraResp.ID},
			},
		})
		require.NoError(t, err)
		...
		msgResp2, err := client.CreateChatMessage(ctx, chat.ID, optimus-ide-collabsdk.CreateChatMessageRequest{
			Content: []optimus-ide-collabsdk.ChatInputPart{
				{Type: optimus-ide-collabsdk.ChatInputPartTypeText, Text: "re-reference existing"},
				{Type: optimus-ide-collabsdk.ChatInputPartTypeFile, FileID: fileIDs[0]},
			},
		})
		require.NoError(t, err)
	...
	t.Run("FileCapOnCreate", func(t *testing.T) {
		...
		fileIDs := make([]uuid.UUID, 0, optimus-ide-collabsdk.MaxChatFileIDs+1)
		for i := range optimus-ide-collabsdk.MaxChatFileIDs + 1 {
		...
		parts := []optimus-ide-collabsdk.ChatInputPart{
			{Type: optimus-ide-collabsdk.ChatInputPartTypeText, Text: "over cap on create"},
		}
		for _, fid := range fileIDs {
			parts = append(parts, optimus-ide-collabsdk.ChatInputPart{Type: optimus-ide-collabsdk.ChatInputPartTypeFile, FileID: fid})
		}
...
func TestPatchChatMessage(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		edited, err := client.EditChatMessage(ctx, chat.ID, userMessageID, optimus-ide-collabsdk.EditChatMessageRequest{
			Content: []optimus-ide-collabsdk.ChatInputPart{
				{
					Type: optimus-ide-collabsdk.ChatInputPartTypeText,
		...
		require.Equal(t, optimus-ide-collabsdk.ChatMessageRoleUser, edited.Message.Role)

		foundEditedText := false
		for _, part := range edited.Message.Content {
			if part.Type == optimus-ide-collabsdk.ChatMessagePartTypeText && part.Text == "hello after edit" {
	...
	t.Run("RejectsInvalidReasoningEffort", func(t *testing.T) {
		...
		_, err = client.EditChatMessage(ctx, chat.ID, userMessageID, optimus-ide-collabsdk.EditChatMessageRequest{
			Content: []optimus-ide-collabsdk.ChatInputPart{{
				Type: optimus-ide-collabsdk.ChatInputPartTypeText,
	...
	t.Run("PreservesFileID", func(t *testing.T) {
		...
		for _, part := range edited.Message.Content {
			if part.Type == optimus-ide-collabsdk.ChatMessagePartTypeText && part.Text == "after edit with file" {
				foundText = true
			}
			if part.Type == optimus-ide-collabsdk.ChatMessagePartTypeFile && part.FileID.Valid && part.FileID.UUID == uploadResp.ID {
				foundFile = true
				require.Nil(t, part.Data, "file data should not be sent when file_id is present")
			}
		}
	...
	t.Run("MessageNotFound", func(t *testing.T) {
		...
		_, err = client.EditChatMessage(ctx, chat.ID, 999999, optimus-ide-collabsdk.EditChatMessageRequest{
			Content: []optimus-ide-collabsdk.ChatInputPart{
				{
					Type: optimus-ide-collabsdk.ChatInputPartTypeText,
					Text: "edited",
	...
	t.Run("FilesLinkedOnEdit", func(t *testing.T) {
		...
		chat, err := client.CreateChat(ctx, optimus-ide-collabsdk.CreateChatRequest{
			OrganizationID: firstUser.OrganizationID,
			Content: []optimus-ide-collabsdk.ChatInputPart{
				{Type: optimus-ide-collabsdk.ChatInputPartTypeText, Text: "before file edit"},
			},
		})
		require.NoError(t, err)
		...
		var userMessageID int64
		for _, msg := range messagesResult.Messages {
			if msg.Role == optimus-ide-collabsdk.ChatMessageRoleUser {
		...
		_, err = client.EditChatMessage(ctx, chat.ID, userMessageID, optimus-ide-collabsdk.EditChatMessageRequest{
			Content: []optimus-ide-collabsdk.ChatInputPart{
				{Type: optimus-ide-collabsdk.ChatInputPartTypeText, Text: "after file edit"},
				{Type: optimus-ide-collabsdk.ChatInputPartTypeFile, FileID: uploadResp.ID},
			},
		})
		require.NoError(t, err)
	...
	t.Run("CapExceededOnEdit", func(t *testing.T) {
		...
		parts := []optimus-ide-collabsdk.ChatInputPart{
			{Type: optimus-ide-collabsdk.ChatInputPartTypeText, Text: "fill to cap"},
		}
		pngData := append([]byte{0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A}, make([]byte, 64)...)
		for i := range optimus-ide-collabsdk.MaxChatFileIDs {
			up, err := client.UploadChatFile(ctx, firstUser.OrganizationID, "image/png", fmt.Sprintf("cap-%d.png", i), bytes.NewReader(pngData))
			require.NoError(t, err)
			parts = append(parts, optimus-ide-collabsdk.ChatInputPart{Type: optimus-ide-collabsdk.ChatInputPartTypeFile, FileID: up.ID})
		}
		chat, err := client.CreateChat(ctx, optimus-ide-collabsdk.CreateChatRequest{OrganizationID: firstUser.OrganizationID, Content: parts})
		require.NoError(t, err)
		...
		edited, err := client.EditChatMessage(ctx, chat.ID, userMessageID, optimus-ide-collabsdk.EditChatMessageRequest{
			Content: []optimus-ide-collabsdk.ChatInputPart{
				{Type: optimus-ide-collabsdk.ChatInputPartTypeText, Text: "edit with extra file"},
				{Type: optimus-ide-collabsdk.ChatInputPartTypeFile, FileID: extra.ID},
			},
		})
		require.NoError(t, err)
	...
	t.Run("ChangesModel", func(t *testing.T) {
		...
		testutil.Eventually(ctx, t, func(ctx context.Context) bool {
			...
			return c.Status != optimus-ide-collabsdk.ChatStatusRunning
		...
		for !foundInitialUserMessage {
			...
			case event, ok := <-events:
				...
				if event.Type == optimus-ide-collabsdk.ChatStreamEventTypeMessage &&
					event.Message != nil &&
					event.Message.Role == optimus-ide-collabsdk.ChatMessageRoleUser &&
...
func TestInterruptChat(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		require.Equal(t, optimus-ide-collabsdk.ChatStatusInterrupting, interrupted.Status)
...
func TestCompactChat(t *testing.T) {
	...
	seedCompactableChat := func(t *testing.T, db database.Store, orgID, ownerID, modelConfigID uuid.UUID) database.Chat {
		...
		userContent, err := chatprompt.MarshalParts([]optimus-ide-collabsdk.ChatMessagePart{
			optimus-ide-collabsdk.ChatMessageText("question"),
		})
		require.NoError(t, err)
		_ = dbgen.ChatMessage(t, db, database.ChatMessage{
			ChatID:        chat.ID,
		...
		assistantContent, err := chatprompt.MarshalParts([]optimus-ide-collabsdk.ChatMessagePart{
			optimus-ide-collabsdk.ChatMessageText("answer"),
		})
		require.NoError(t, err)
		_ = dbgen.ChatMessage(t, db, database.ChatMessage{
			ChatID:        chat.ID,
	...
	t.Run("RequestsCompaction", func(t *testing.T) {
		...
		require.Equal(t, optimus-ide-collabsdk.ChatStatusRunning, compacted.Status)
	})
	...
	t.Run("UpdateDenied", func(t *testing.T) {
		...
		ctx := testutil.Context(t, testutil.WaitLong)
		clientRaw, _, api := optimus-ide-collabdtest.NewWithAPI(t, &optimus-ide-collabdtest.Options{
			Authorizer: &optimus-ide-collabdtest.FakeAuthorizer{
...
func TestProposeChatTitle(t *testing.T) {
	...
	t.Run("DoesNotPersistTitleOrBumpUpdatedAt", func(t *testing.T) {
		...
		chat, err := client.CreateChat(ctx, optimus-ide-collabsdk.CreateChatRequest{
			OrganizationID: firstUser.OrganizationID,
			Content: []optimus-ide-collabsdk.ChatInputPart{
				{Type: optimus-ide-collabsdk.ChatInputPartTypeText, Text: "test chat"},
			},
		})
		require.NoError(t, err)
...
func TestManualTitleEndpointsPassOwnerSyntheticAPIKeyToAIGateway(t *testing.T) {
	...
	for _, tt := range []struct {
		name string
		call func(context.Context, *optimus-ide-collabsdk.ExperimentalClient, uuid.UUID) error
	}{
		{
			name: "RegenerateChatTitle",
			call: func(ctx context.Context, client *optimus-ide-collabsdk.ExperimentalClient, chatID uuid.UUID) error {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			client, db, api := newChatClientWithAPIAndDatabase(t, func(opts *optimus-ide-collabdtest.Options) {
			...
			require.NoError(t, client.UpdateChatModelOverride(ctx, optimus-ide-collabsdk.ChatModelOverrideContextTitleGeneration, optimus-ide-collabsdk.UpdateChatModelOverrideRequest{
			...
			content, err := chatprompt.MarshalParts([]optimus-ide-collabsdk.ChatMessagePart{
				optimus-ide-collabsdk.ChatMessageText("manual title source"),
			})
			require.NoError(t, err)
			_ = dbgen.ChatMessage(t, db, database.ChatMessage{
				ChatID:        chat.ID,
...
func TestGetChatDiffStatus(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		require.Equal(t, "https://github.com/optimus-ide-collab/optimus-ide-collab/tree/feature/diff-status", *cachedStatus.URL)
...
func TestDeleteChatQueuedMessage(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		deleteContent, err := json.Marshal([]optimus-ide-collabsdk.ChatMessagePart{
			optimus-ide-collabsdk.ChatMessageText("queued message for delete route"),
		})
		require.NoError(t, err)
	...
	t.Run("InvalidQueuedMessageID", func(t *testing.T) {
		...
		err = optimus-ide-collabsdk.ReadBodyAsError(invalidRes)
		sdkErr := requireSDKError(t, err, http.StatusBadRequest)
...
func TestPromoteChatQueuedMessage(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		queuedContent, err := json.Marshal([]optimus-ide-collabsdk.ChatMessagePart{
			optimus-ide-collabsdk.ChatMessageText(queuedText),
		})
		require.NoError(t, err)
		...
		var resp optimus-ide-collabsdk.Response
		require.NoError(t, json.NewDeoptimus-ide-collab(promoteRes.Body).Decode(&resp))
		...
		for _, msg := range messagesResult.Messages {
			...
			for _, part := range msg.Content {
				if part.Type == optimus-ide-collabsdk.ChatMessagePartTypeText && part.Text == queuedText {
					foundPromoted = true
				}
			}
		}
	...
	t.Run("MemberWithoutAgentsAccess", func(t *testing.T) {
		...
		queuedContent, err := json.Marshal([]optimus-ide-collabsdk.ChatMessagePart{
			optimus-ide-collabsdk.ChatMessageText("queued message no agents access"),
		})
		require.NoError(t, err)
		queuedMessage := insertTestChatQueuedMessage(ctx, t, db, chat.ID, queuedContent, chat.LastModelConfigID)
	...
	t.Run("ArchivedChat", func(t *testing.T) {
		...
		queuedContent, err := json.Marshal([]optimus-ide-collabsdk.ChatMessagePart{
			optimus-ide-collabsdk.ChatMessageText("queued"),
		})
		require.NoError(t, err)
		queuedMessage := insertTestChatQueuedMessage(ctx, t, db, chat.ID, queuedContent, chat.LastModelConfigID)
		...
		promoteErr := optimus-ide-collabsdk.ReadBodyAsError(promoteRes)
		var promoteSDKErr *optimus-ide-collabsdk.Error
		...
		for _, msg := range messages {
			...
			for _, part := range parts {
				if msg.Role == database.ChatMessageRoleTool &&
					part.Type == optimus-ide-collabsdk.ChatMessagePartTypeToolResult &&
	...
	t.Run("WhileRunning", func(t *testing.T) {
		...
		queuedContent, err := json.Marshal([]optimus-ide-collabsdk.ChatMessagePart{
			optimus-ide-collabsdk.ChatMessageText("running-promote"),
		})
		require.NoError(t, err)
		queuedMessage := insertTestChatQueuedMessage(ctx, t, db, chat.ID, queuedContent, chat.LastModelConfigID)
...
func TestChatUsageLimitOverrideRoutes(t *testing.T) {
	...
	t.Run("UpsertUserOverrideRequiresPositiveSpendLimit", func(t *testing.T) {
		...
		firstUser := optimus-ide-collabdtest.CreateFirstUser(t, client.Client)
		_, member := optimus-ide-collabdtest.CreateAnotherUser(t, client.Client, firstUser.OrganizationID)
	...
	t.Run("UpsertUserOverrideMissingUser", func(t *testing.T) {
		...
		_, err := client.UpsertChatUsageLimitOverride(ctx, uuid.New(), optimus-ide-collabsdk.UpsertChatUsageLimitOverrideRequest{
	...
	t.Run("UpdateUserOverride", func(t *testing.T) {
		...
		_, err := client.UpsertChatUsageLimitOverride(ctx, member.ID, optimus-ide-collabsdk.UpsertChatUsageLimitOverrideRequest{
			SpendLimitMicros: 5_000_000,
		})
		require.NoError(t, err)

		override, err := client.UpsertChatUsageLimitOverride(ctx, member.ID, optimus-ide-collabsdk.UpsertChatUsageLimitOverrideRequest{
	...
	t.Run("UpsertGroupOverrideIncludesMemberCount", func(t *testing.T) {
		...
		override, err := client.UpsertChatUsageLimitGroupOverride(ctx, group.ID, optimus-ide-collabsdk.UpsertChatUsageLimitGroupOverrideRequest{
			SpendLimitMicros: 7_000_000,
		})
		require.NoError(t, err)
		...
		var listed *optimus-ide-collabsdk.ChatUsageLimitGroupOverride
	...
	t.Run("UpdateGroupOverride", func(t *testing.T) {
		...
		_, err := client.UpsertChatUsageLimitGroupOverride(ctx, group.ID, optimus-ide-collabsdk.UpsertChatUsageLimitGroupOverrideRequest{
			SpendLimitMicros: 5_000_000,
		})
		require.NoError(t, err)
	...
	t.Run("UpsertGroupOverrideMissingGroup", func(t *testing.T) {
		...
		_, err := client.UpsertChatUsageLimitGroupOverride(ctx, uuid.New(), optimus-ide-collabsdk.UpsertChatUsageLimitGroupOverrideRequest{
			SpendLimitMicros: 7_000_000,
		})
		sdkErr := requireSDKError(t, err, http.StatusNotFound)
...
`)
	...
	t.Run("MissingOrganization", func(t *testing.T) {
		...
		ctx := testutil.Context(t, testutil.WaitLong)
		client := newChatClient(t)
		optimus-ide-collabdtest.CreateFirstUser(t, client.Client)
	...
	t.Run("Unauthenticated", func(t *testing.T) {
		...
		unauthed := optimus-ide-collabsdk.NewExperimentalClient(optimus-ide-collabsdk.New(client.URL))
		data := append([]byte{0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A}, make([]byte, 64)...)
...
func (f chatCostTestFixture) safeOptions() optimus-ide-collabsdk.ChatCostSummaryOptions {
	return optimus-ide-collabsdk.ChatCostSummaryOptions{
...
func assertChatCostSummary(t *testing.T, summary optimus-ide-collabsdk.ChatCostSummary, modelConfigID, chatID uuid.UUID) {
	t.Helper()
...
func TestChatCostSummary_AdminDrilldown(t *testing.T) {
	...
	options := optimus-ide-collabsdk.ChatCostSummaryOptions{
...
func TestChatCostUsers(t *testing.T) {
	...
	t.Run("AdminCanListUsers", func(t *testing.T) {
		...
		ctx := testutil.Context(t, testutil.WaitLong)
		resp, err := client.GetChatCostUsers(ctx, optimus-ide-collabsdk.ChatCostUsersOptions{})
		require.NoError(t, err)
	...
	t.Run("MemberCannotListUsers", func(t *testing.T) {
		...
		ctx := testutil.Context(t, testutil.WaitLong)
		_, err := memberClient.GetChatCostUsers(ctx, optimus-ide-collabsdk.ChatCostUsersOptions{})
		require.Error(t, err)
		var sdkErr *optimus-ide-collabsdk.Error
		require.ErrorAs(t, err, &sdkErr)
...
func TestChatCostSummary_DateRange(t *testing.T) {
	...
	t.Run("MessageInRange", func(t *testing.T) {
		...
		ctx := testutil.Context(t, testutil.WaitLong)
		summary, err := client.GetChatCostSummary(ctx, "me", optimus-ide-collabsdk.ChatCostSummaryOptions{
...
func requireChatModelPricing(
	t *testing.T,
	actual *optimus-ide-collabsdk.ChatModelCallConfig,
	expected *optimus-ide-collabsdk.ChatModelCallConfig,
) {
	t.Helper()
...
// in optimus-ide-collabd/workspaceagents_internal_test.go.
func TestWatchChatGitAuthz(t *testing.T) {
	...
	secondAdminClient, _ := optimus-ide-collabdtest.CreateAnotherUser(t, adminClient.Client, firstUser.OrganizationID, rbac.RoleOwner())
	...
	_, workspaceOwner := optimus-ide-collabdtest.CreateAnotherUser(t, adminClient.Client, firstUser.OrganizationID)
	...
	chat, err := adminClient.CreateChat(ctx, optimus-ide-collabsdk.CreateChatRequest{
		OrganizationID: firstUser.OrganizationID,
		Content: []optimus-ide-collabsdk.ChatInputPart{
			{Type: optimus-ide-collabsdk.ChatInputPartTypeText, Text: "codagt-184"},
		},
	})
	require.NoError(t, err)
	...
	err = adminClient.UpdateChat(ctx, chat.ID, optimus-ide-collabsdk.UpdateChatRequest{
		WorkspaceID: &workspaceBuild.Workspace.ID,
	})
	require.NoError(t, err)
	...
	_, err = secondAdminClient.UpdateUserRoles(ctx, firstUser.UserID.String(), optimus-ide-collabsdk.UpdateRoles{
		Roles: []string{rbac.RoleTemplateAdmin().String()},
	})
	require.NoError(t, err)

	_, err = secondAdminClient.UpdateOrganizationMemberRoles(ctx, firstUser.OrganizationID, firstUser.UserID.String(), optimus-ide-collabsdk.UpdateRoles{
...
func createAIProviderForTest(
	...
	req := optimus-ide-collabsdk.CreateAIProviderRequest{
		Type:    optimus-ide-collabsdk.AIProviderType(provider),
...
func seedPasteOnlyTitleSourceMessage(
	...
	content, err := chatprompt.MarshalParts([]optimus-ide-collabsdk.ChatMessagePart{
		optimus-ide-collabsdk.ChatMessageFile(file.ID, "text/plain", pasteFileName),
	})
	require.NoError(t, err)
	_ = dbgen.ChatMessage(t, db, database.ChatMessage{
		ChatID:        chat.ID,
...
func createChatModelConfig(t testing.TB, client *optimus-ide-collabsdk.ExperimentalClient) optimus-ide-collabsdk.ChatModelConfig {
	t.Helper()
	return optimus-ide-collabdtest.CreateOpenAICompatChatModelConfig(t, client, "")
}

func createChatModelConfigWithBaseURL(t testing.TB, client *optimus-ide-collabsdk.ExperimentalClient, baseURL string) optimus-ide-collabsdk.ChatModelConfig {
	t.Helper()
	return optimus-ide-collabdtest.CreateOpenAICompatChatModelConfig(t, client, baseURL)
}
...
func createChatModelConfigWithTitleFailure(t testing.TB, client *optimus-ide-collabsdk.ExperimentalClient) optimus-ide-collabsdk.ChatModelConfig {
	t.Helper()
	baseURL := chattest.NewOpenAI(t, func(req *chattest.OpenAIRequest) chattest.OpenAIResponse {
		if req.Stream {
			return chattest.OpenAIStreamingResponse(chattest.OpenAITextChunks("Hello from test server.")...)
...
func createAdditionalChatModelConfigWithReasoningEffort(
	...
	t.Helper()
	return createAdditionalChatModelConfigWithModelConfig(t, client, provider, model, &optimus-ide-collabsdk.ChatModelCallConfig{
		ReasoningEffort: &optimus-ide-collabsdk.ChatModelReasoningEffortConfig{
...
func createAdditionalChatModelConfigWithModelConfig(
	...
	provider string,
	model string,
	modelCallConfig *optimus-ide-collabsdk.ChatModelCallConfig,
) optimus-ide-collabsdk.ChatModelConfig {
	t.Helper()
...
func createProviderDisabledChatModelConfig(
	...
	modelConfig := createAdditionalChatModelConfig(t, client, provider, model)
	ctx := testutil.Context(t, testutil.WaitLong)
	_, err := client.UpdateAIProvider(ctx, modelConfig.AIProviderID.String(), optimus-ide-collabsdk.UpdateAIProviderRequest{
		Enabled: ptr.Ref(false),
	})
	require.NoError(t, err)
	return modelConfig
...
func enableUserChatProviderKey(
	t testing.TB,
	adminClient *optimus-ide-collabsdk.ExperimentalClient,
	userClient *optimus-ide-collabsdk.ExperimentalClient,
	providerName string,
) optimus-ide-collabsdk.AIProvider {
	t.Helper()
	...
	var provider optimus-ide-collabsdk.AIProvider
	for _, candidate := range providers {
		if candidate.Type == optimus-ide-collabsdk.AIProviderType(providerName) {
			provider = candidate
			break
		}
	}
	...
	_, err = userClient.UpsertUserAIProviderKey(ctx, "me", provider.ID, optimus-ide-collabsdk.CreateUserAIProviderKeyRequest{
...
If a workspace is needed, use list_templates before create_workspace and follow its next_step. Call read_template only when you need template parameter or preset details.`
	...
	updateChatSystemPrompt := func(t *testing.T, ctx context.Context, req optimus-ide-collabsdk.UpdateChatSystemPromptRequest) {
		t.Helper()
	...
	getChatSystemPrompt := func(t *testing.T, ctx context.Context) optimus-ide-collabsdk.ChatSystemPromptResponse {
		t.Helper()
	...
	assertInjectedSystemMessages := func(t *testing.T, ctx context.Context, wantResolvedPrompt string) {
		...
		for _, message := range messages {
			...
			require.Equal(t, optimus-ide-collabsdk.ChatMessagePartTypeText, parts[0].Type)
	...
	t.Run("AdminCanSet", func(t *testing.T) {
		...
		updateChatSystemPrompt(t, ctx, optimus-ide-collabsdk.UpdateChatSystemPromptRequest{
	...
	t.Run("PreservesIncludeDefaultWhenOmitted", func(t *testing.T) {
		...
		err := client.UpdateChatSystemPrompt(ctx, optimus-ide-collabsdk.UpdateChatSystemPromptRequest{
			SystemPrompt:               "",
			IncludeDefaultSystemPrompt: ptr.Ref(false),
		})
		require.NoError(t, err)
	...
	t.Run("ExistingCustomPromptDefaultsIncludeDefaultOff", func(t *testing.T) {
		...
		firstUser := optimus-ide-collabdtest.CreateFirstUser(t, legacyClient.Client)
		...
		chat, err := legacyClient.CreateChat(ctx, optimus-ide-collabsdk.CreateChatRequest{
			OrganizationID: firstUser.OrganizationID,
			Content: []optimus-ide-collabsdk.ChatInputPart{{
				Type: optimus-ide-collabsdk.ChatInputPartTypeText,
	...
	t.Run("NonAdminFails", func(t *testing.T) {
		...
		err := memberClient.UpdateChatSystemPrompt(ctx, optimus-ide-collabsdk.UpdateChatSystemPromptRequest{
	...
	t.Run("UnauthenticatedFails", func(t *testing.T) {
		...
		anonClient := optimus-ide-collabsdk.NewExperimentalClient(optimus-ide-collabsdk.New(adminClient.URL))
	...
	t.Run("TooLong", func(t *testing.T) {
		...
		err := adminClient.UpdateChatSystemPrompt(ctx, optimus-ide-collabsdk.UpdateChatSystemPromptRequest{
...
func TestChatPlanModeInstructions(t *testing.T) {
	...
	updateChatPlanModeInstructions := func(t *testing.T, ctx context.Context, req optimus-ide-collabsdk.UpdateChatPlanModeInstructionsRequest) {
		t.Helper()
	...
	getChatPlanModeInstructions := func(t *testing.T, ctx context.Context) optimus-ide-collabsdk.ChatPlanModeInstructionsResponse {
		t.Helper()
	...
	for _, tt := range roundTripTests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, instructions := range tt.updates {
				updateChatPlanModeInstructions(t, ctx, optimus-ide-collabsdk.UpdateChatPlanModeInstructionsRequest{
	...
	t.Run("OversizedPayloadReturns400", func(t *testing.T) {
		...
		err := adminClient.UpdateChatPlanModeInstructions(ctx, optimus-ide-collabsdk.UpdateChatPlanModeInstructionsRequest{
...
//nolint:tparallel,paralleltest // Setting subtests share per-setting optimus-ide-collabdtest instances.
func TestChatModelOverrides(t *testing.T) {
	t.Parallel()

	type overrideResponse struct {
		context         optimus-ide-collabsdk.ChatModelOverrideContext
	...
	settingPath := func(overrideContext optimus-ide-collabsdk.ChatModelOverrideContext) string {
	...
	getOverride := func(
		ctx context.Context,
		client *optimus-ide-collabsdk.ExperimentalClient,
		overrideContext optimus-ide-collabsdk.ChatModelOverrideContext,
	...
	putOverrideWithEffort := func(
		...
		return client.UpdateChatModelOverride(
			ctx,
			overrideContext,
			optimus-ide-collabsdk.UpdateChatModelOverrideRequest{
	...
	settings := []settingTest{
		{
			name:    "General",
			context: optimus-ide-collabsdk.ChatModelOverrideContextGeneral,
		...
		{
			name:    "Explore",
			context: optimus-ide-collabsdk.ChatModelOverrideContextExplore,
			dbGet: func(ctx context.Context, db database.Store) (string, error) {
		...
		},
		{
			...
			context: optimus-ide-collabsdk.ChatModelOverrideContextTitleGeneration,
			dbGet: func(ctx context.Context, db database.Store) (string, error) {
		...
		{
			name:    "Compaction",
			context: optimus-ide-collabsdk.ChatModelOverrideContextCompaction,
			dbGet: func(ctx context.Context, db database.Store) (string, error) {
	...
	t.Run("UnknownContextReturns400", func(t *testing.T) {
		...
		adminClient := newChatClient(t)
		optimus-ide-collabdtest.CreateFirstUser(t, adminClient.Client)
		unknownContext := optimus-ide-collabsdk.ChatModelOverrideContext("not-a-context")
...
//nolint:tparallel,paralleltest // Subtests share optimus-ide-collabdtest instances.
func TestChatPersonalModelOverridesAdminSettings(t *testing.T) {
	...
	err = adminClient.UpdateChatPersonalModelOverridesAdminSettings(ctx, optimus-ide-collabsdk.UpdateChatPersonalModelOverridesAdminSettingsRequest{
		AllowUsers: true,
	})
	require.NoError(t, err)
	resp, err = adminClient.GetChatPersonalModelOverridesAdminSettings(ctx)
	...
	err = memberClient.UpdateChatPersonalModelOverridesAdminSettings(ctx, optimus-ide-collabsdk.UpdateChatPersonalModelOverridesAdminSettingsRequest{
		AllowUsers: true,
	})
	requireSDKError(t, err, http.StatusForbidden)
...
func TestUserChatPersonalModelOverrides(t *testing.T) {
	...
	adminClient, db := newChatClientWithDatabase(t)
	firstUser := optimus-ide-collabdtest.CreateFirstUser(t, adminClient.Client)
	memberClientRaw, member := optimus-ide-collabdtest.CreateAnotherUser(t, adminClient.Client, firstUser.OrganizationID)
	memberClient := optimus-ide-collabsdk.NewExperimentalClient(memberClientRaw)
	noKeyClientRaw, noKeyUser := optimus-ide-collabdtest.CreateAnotherUser(t, adminClient.Client, firstUser.OrganizationID)
	noKeyClient := optimus-ide-collabsdk.NewExperimentalClient(noKeyClientRaw)

	defaultModelConfig := createChatModelConfig(t, adminClient)
	provider := enableUserChatProviderKey(t, adminClient, memberClient, optimus-ide-collabdtest.TestChatProviderOpenAICompat)
	modelProvider := createAIProviderForTest(t, adminClient, "anthropic", "")
	_, err := memberClient.UpsertUserAIProviderKey(ctx, "me", modelProvider.ID, optimus-ide-collabsdk.CreateUserAIProviderKeyRequest{
		APIKey: "test-user-api-key-" + uuid.NewString(),
	})
	require.NoError(t, err)
	contextLimit := int64(4096)
	modelConfigRequest := optimus-ide-collabsdk.CreateChatModelConfigRequest{
	...
	modelConfigRequest.ModelConfig = &optimus-ide-collabsdk.ChatModelCallConfig{
		ReasoningEffort: &optimus-ide-collabsdk.ChatModelReasoningEffortConfig{
			Default: ptr.Ref("medium"),
			Max:     ptr.Ref("high"),
		},
	...
	require.NoError(t, err)
	err = adminClient.UpdateChatModelOverride(ctx, optimus-ide-collabsdk.ChatModelOverrideContextGeneral, optimus-ide-collabsdk.UpdateChatModelOverrideRequest{
		ModelConfigID: modelConfig.ID.String(),
	})
	require.NoError(t, err)
	err = adminClient.UpdateChatModelOverride(ctx, optimus-ide-collabsdk.ChatModelOverrideContextExplore, optimus-ide-collabsdk.UpdateChatModelOverrideRequest{
	...
	contextLimit = int64(4096)
	disabledProviderModelConfig, err := adminClient.CreateChatModelConfig(ctx, optimus-ide-collabsdk.CreateChatModelConfigRequest{
	...
	require.NoError(t, err)
	enabled := false
	disabledProvider, err = adminClient.UpdateAIProvider(ctx, disabledProvider.ID.String(), optimus-ide-collabsdk.UpdateAIProviderRequest{
		Enabled: &enabled,
	})
	require.NoError(t, err)
	require.NotEqual(t, uuid.Nil, provider.ID)
	...
	personalOverride := func(
		resp optimus-ide-collabsdk.UserChatPersonalModelOverridesResponse,
		overrideContext optimus-ide-collabsdk.ChatPersonalModelOverrideContext,
	) optimus-ide-collabsdk.ChatPersonalModelOverride {
		t.Helper()
		switch overrideContext {
		case optimus-ide-collabsdk.ChatPersonalModelOverrideContextRoot:
			return resp.Root
		case optimus-ide-collabsdk.ChatPersonalModelOverrideContextGeneral:
			return resp.General
		case optimus-ide-collabsdk.ChatPersonalModelOverrideContextExplore:
			return resp.Explore
		default:
			t.Fatalf("unexpected personal model override context %q", overrideContext)
			return optimus-ide-collabsdk.ChatPersonalModelOverride{}
		}
	}
	assertOverrideWithEffort := func(
		resp optimus-ide-collabsdk.UserChatPersonalModelOverridesResponse,
		overrideContext optimus-ide-collabsdk.ChatPersonalModelOverrideContext,
		mode optimus-ide-collabsdk.ChatPersonalModelOverrideMode,
		modelConfigID string,
		reasoningEffort *string,
		isSet bool,
		isMalformed bool,
	...
	assertDeploymentDefault := func(
		...
		t.Helper()
		var override optimus-ide-collabsdk.ChatModelOverrideResponse
		switch overrideContext {
		case optimus-ide-collabsdk.ChatModelOverrideContextGeneral:
			override = resp.DeploymentDefaults.General
		case optimus-ide-collabsdk.ChatModelOverrideContextExplore:
	...
	getRawFor := func(userID uuid.UUID, overrideContext optimus-ide-collabsdk.ChatPersonalModelOverrideContext) string {
		t.Helper()
	...
	getRaw := func(overrideContext optimus-ide-collabsdk.ChatPersonalModelOverrideContext) string {
		t.Helper()
	...
	t.Run("GETDisabledReturnsMissingDefaults", func(t *testing.T) {
		...
		assertOverride(resp, optimus-ide-collabsdk.ChatPersonalModelOverrideContextRoot, optimus-ide-collabsdk.ChatPersonalModelOverrideModeChatDefault, "", false, false)
		assertOverride(resp, optimus-ide-collabsdk.ChatPersonalModelOverrideContextGeneral, optimus-ide-collabsdk.ChatPersonalModelOverrideModeDeploymentDefault, "", false, false)
		assertOverride(resp, optimus-ide-collabsdk.ChatPersonalModelOverrideContextExplore, optimus-ide-collabsdk.ChatPersonalModelOverrideModeDeploymentDefault, "", false, false)
	})

	upsertRaw(optimus-ide-collabsdk.ChatPersonalModelOverrideContextRoot, string(optimus-ide-collabsdk.ChatPersonalModelOverrideModeChatDefault))
	upsertRaw(optimus-ide-collabsdk.ChatPersonalModelOverrideContextGeneral, string(optimus-ide-collabsdk.ChatPersonalModelOverrideModeDeploymentDefault))
	upsertRaw(optimus-ide-collabsdk.ChatPersonalModelOverrideContextExplore, "model:"+modelConfig.ID.String())

	t.Run("GETDisabledReturnsSavedValues", func(t *testing.T) {
		resp, err := memberClient.GetUserChatPersonalModelOverrides(ctx)
		require.NoError(t, err)
		require.False(t, resp.Enabled)
		assertOverride(resp, optimus-ide-collabsdk.ChatPersonalModelOverrideContextRoot, optimus-ide-collabsdk.ChatPersonalModelOverrideModeChatDefault, "", true, false)
		assertOverride(resp, optimus-ide-collabsdk.ChatPersonalModelOverrideContextGeneral, optimus-ide-collabsdk.ChatPersonalModelOverrideModeDeploymentDefault, "", true, false)
		assertOverride(resp, optimus-ide-collabsdk.ChatPersonalModelOverrideContextExplore, optimus-ide-collabsdk.ChatPersonalModelOverrideModeModel, modelConfig.ID.String(), true, false)
	})

	t.Run("GETIncludesDeploymentDefaults", func(t *testing.T) {
		resp, err := memberClient.GetUserChatPersonalModelOverrides(ctx)
		require.NoError(t, err)
		assertDeploymentDefault(resp, optimus-ide-collabsdk.ChatModelOverrideContextGeneral, modelConfig.ID.String(), nil, false)
		assertDeploymentDefault(resp, optimus-ide-collabsdk.ChatModelOverrideContextExplore, defaultModelConfig.ID.String(), nil, false)
	})
	...
	t.Run("GETIncludesDeploymentDefaultReasoningEffort", func(t *testing.T) {
		...
		resp, err := memberClient.GetUserChatPersonalModelOverrides(ctx)
		require.NoError(t, err)
		assertDeploymentDefault(resp, optimus-ide-collabsdk.ChatModelOverrideContextGeneral, reasoningModelConfig.ID.String(), ptr.Ref("high"), false)
	})

	t.Run("PUTDisabledReturns403AndPreservesRows", func(t *testing.T) {
		err := memberClient.UpdateUserChatPersonalModelOverride(ctx, optimus-ide-collabsdk.ChatPersonalModelOverrideContextRoot, optimus-ide-collabsdk.UpdateUserChatPersonalModelOverrideRequest{
			Mode:          optimus-ide-collabsdk.ChatPersonalModelOverrideModeModel,
			ModelConfigID: modelConfig.ID.String(),
		})
		requireSDKError(t, err, http.StatusForbidden)
		require.Equal(t, string(optimus-ide-collabsdk.ChatPersonalModelOverrideModeChatDefault), getRaw(optimus-ide-collabsdk.ChatPersonalModelOverrideContextRoot))
	})
	...
	contexts := []optimus-ide-collabsdk.ChatPersonalModelOverrideContext{
		optimus-ide-collabsdk.ChatPersonalModelOverrideContextRoot,
		optimus-ide-collabsdk.ChatPersonalModelOverrideContextGeneral,
		optimus-ide-collabsdk.ChatPersonalModelOverrideContextExplore,
	}

	t.Run("PUTRejectsUnknownMode", func(t *testing.T) {
		rawBefore := getRaw(optimus-ide-collabsdk.ChatPersonalModelOverrideContextGeneral)
		err := memberClient.UpdateUserChatPersonalModelOverride(ctx, optimus-ide-collabsdk.ChatPersonalModelOverrideContextGeneral, optimus-ide-collabsdk.UpdateUserChatPersonalModelOverrideRequest{
			Mode: optimus-ide-collabsdk.ChatPersonalModelOverrideMode("banana"),
		})
		sdkErr := requireSDKError(t, err, http.StatusBadRequest)
		require.Contains(t, sdkErr.Message, "Invalid personal model override mode.")
		require.Equal(t, rawBefore, getRaw(optimus-ide-collabsdk.ChatPersonalModelOverrideContextGeneral))
	})

	t.Run("PUTChatDefaultRoundTrips", func(t *testing.T) {
		for _, overrideContext := range contexts {
			err := memberClient.UpdateUserChatPersonalModelOverride(ctx, overrideContext, optimus-ide-collabsdk.UpdateUserChatPersonalModelOverrideRequest{
		...
		for _, overrideContext := range contexts {
			assertOverride(resp, overrideContext, optimus-ide-collabsdk.ChatPersonalModelOverrideModeChatDefault, "", true, false)
		}
	})

	t.Run("PUTChatDefaultRejectsNonEmptyModelConfigID", func(t *testing.T) {
		rawBefore := getRaw(optimus-ide-collabsdk.ChatPersonalModelOverrideContextRoot)
		err := memberClient.UpdateUserChatPersonalModelOverride(ctx, optimus-ide-collabsdk.ChatPersonalModelOverrideContextRoot, optimus-ide-collabsdk.UpdateUserChatPersonalModelOverrideRequest{
			Mode:          optimus-ide-collabsdk.ChatPersonalModelOverrideModeChatDefault,
			ModelConfigID: modelConfig.ID.String(),
		})
		...
		require.Equal(t, rawBefore, getRaw(optimus-ide-collabsdk.ChatPersonalModelOverrideContextRoot))
	})

	t.Run("PUTDeploymentDefaultRoundTripsForAgentContexts", func(t *testing.T) {
		for _, overrideContext := range []optimus-ide-collabsdk.ChatPersonalModelOverrideContext{
			optimus-ide-collabsdk.ChatPersonalModelOverrideContextGeneral,
			optimus-ide-collabsdk.ChatPersonalModelOverrideContextExplore,
		} {
			err := memberClient.UpdateUserChatPersonalModelOverride(ctx, overrideContext, optimus-ide-collabsdk.UpdateUserChatPersonalModelOverrideRequest{
		...
		require.NoError(t, err)
		assertOverride(resp, optimus-ide-collabsdk.ChatPersonalModelOverrideContextGeneral, optimus-ide-collabsdk.ChatPersonalModelOverrideModeDeploymentDefault, "", true, false)
		assertOverride(resp, optimus-ide-collabsdk.ChatPersonalModelOverrideContextExplore, optimus-ide-collabsdk.ChatPersonalModelOverrideModeDeploymentDefault, "", true, false)
	})
	...
	t.Run("PUTModelRoundTrips", func(t *testing.T) {
		...
		require.NoError(t, err)
		for _, overrideContext := range contexts {
			assertOverride(resp, overrideContext, optimus-ide-collabsdk.ChatPersonalModelOverrideModeModel, modelConfig.ID.String(), true, false)
		}
	})
	...
	t.Run("PUTModelRoundTripsReasoningEffort", func(t *testing.T) {
		...
		require.Equal(t, "model:"+reasoningModelConfig.ID.String()+":high", getRaw(optimus-ide-collabsdk.ChatPersonalModelOverrideContextGeneral))
		resp, err := memberClient.GetUserChatPersonalModelOverrides(ctx)
		require.NoError(t, err)
		assertOverrideWithEffort(resp, optimus-ide-collabsdk.ChatPersonalModelOverrideContextGeneral, optimus-ide-collabsdk.ChatPersonalModelOverrideModeModel, reasoningModelConfig.ID.String(), ptr.Ref("high"), true, false)
	})
	...
	t.Run("PUTModelRejectsInvalidModels", func(t *testing.T) {
		...
		for _, tc := range cases {
			t.Run(tc.name, func(t *testing.T) {
				rawBefore := getRawFor(tc.userID, optimus-ide-collabsdk.ChatPersonalModelOverrideContextGeneral)
				err := tc.client.UpdateUserChatPersonalModelOverride(ctx, optimus-ide-collabsdk.ChatPersonalModelOverrideContextGeneral, optimus-ide-collabsdk.UpdateUserChatPersonalModelOverrideRequest{
					Mode:          optimus-ide-collabsdk.ChatPersonalModelOverrideModeModel,
					ModelConfigID: tc.modelConfigID,
				})
				sdkErr := requireSDKError(t, err, http.StatusBadRequest)
				require.Contains(t, sdkErr.Message, tc.wantMessageSubstring)
				rawAfter := getRawFor(tc.userID, optimus-ide-collabsdk.ChatPersonalModelOverrideContextGeneral)
	...
	t.Run("GETMalformedStoredValueFallsBackToContextDefault", func(t *testing.T) {
		upsertRaw(optimus-ide-collabsdk.ChatPersonalModelOverrideContextRoot, "model:not-a-uuid")

		resp, err := memberClient.GetUserChatPersonalModelOverrides(ctx)
		require.NoError(t, err)
		assertOverride(resp, optimus-ide-collabsdk.ChatPersonalModelOverrideContextRoot, optimus-ide-collabsdk.ChatPersonalModelOverrideModeChatDefault, "", true, true)
	})

	t.Run("GETRootDeploymentDefaultIsMalformed", func(t *testing.T) {
		upsertRaw(
			optimus-ide-collabsdk.ChatPersonalModelOverrideContextRoot,
			string(optimus-ide-collabsdk.ChatPersonalModelOverrideModeDeploymentDefault),
		)
...
func TestCreateChatPersonalModelOverrideRoot(t *testing.T) {
	...
	firstUser := optimus-ide-collabdtest.CreateFirstUser(t, adminClient.Client)
	defaultModel := createChatModelConfig(t, adminClient)
	_ = enableUserChatProviderKey(t, adminClient, adminClient, optimus-ide-collabdtest.TestChatProviderOpenAICompat)
	overrideProvider := createAIProviderForTest(t, adminClient, "anthropic", "")
	_, err := adminClient.UpsertUserAIProviderKey(ctx, "me", overrideProvider.ID, optimus-ide-collabsdk.CreateUserAIProviderKeyRequest{
		APIKey: "test-user-api-key-" + uuid.NewString(),
	})
	require.NoError(t, err)
	contextLimit := int64(4096)
	overrideModel, err := adminClient.CreateChatModelConfig(ctx, optimus-ide-collabsdk.CreateChatModelConfigRequest{
	...
	memberClientRaw, member := optimus-ide-collabdtest.CreateAnotherUser(
		t,
		adminClient.Client,
		firstUser.OrganizationID,
		rbac.ScopedRoleAgentsAccess(firstUser.OrganizationID),
	...
	upsertRootRaw := func(userID uuid.UUID, value string) {
		...
		err := db.UpsertUserChatPersonalModelOverride(dbauthz.AsSystemRestricted(ctx), database.UpsertUserChatPersonalModelOverrideParams{
			UserID: userID,
			Key:    chatd.ChatPersonalModelOverrideKey(optimus-ide-collabsdk.ChatPersonalModelOverrideContextRoot),
			Value:  value,
		})
		require.NoError(t, err)
	}
	...
	require.NoError(t, err)
	err = adminClient.UpdateUserChatPersonalModelOverride(ctx, optimus-ide-collabsdk.ChatPersonalModelOverrideContextRoot, optimus-ide-collabsdk.UpdateUserChatPersonalModelOverrideRequest{
		Mode:          optimus-ide-collabsdk.ChatPersonalModelOverrideModeModel,
	...
	t.Run("RootModelOverrideUsesSavedReasoningEffort", func(t *testing.T) {
		reasoningModel, err := adminClient.CreateChatModelConfig(ctx, optimus-ide-collabsdk.CreateChatModelConfigRequest{
			AIProviderID: &overrideProvider.ID,
...
func TestChatComputerUseProvider(t *testing.T) {
	...
	t.Run("ReturnsAnthropicWhenUnset", func(t *testing.T) {
		...
		require.NoError(t, err)
		require.Equal(t, optimus-ide-collabsdk.ChatComputerUseProviderAnthropic, resp.Provider)
	})
	...
	t.Run("AdminCanSetAnthropic", func(t *testing.T) {
		...
		err := adminClient.UpdateChatComputerUseProvider(ctx, optimus-ide-collabsdk.UpdateChatComputerUseProviderRequest{
			Provider: "anthropic",
		})
		require.NoError(t, err)
	...
	t.Run("AdminCanSetOpenAI", func(t *testing.T) {
		...
		resp, err := adminClient.GetChatComputerUseProvider(ctx)
		require.NoError(t, err)
		require.Equal(t, optimus-ide-collabsdk.ChatComputerUseProviderOpenAI, resp.Provider)
	})
	...
	t.Run("InvalidProviderRejected", func(t *testing.T) {
		...
		for _, provider := range []string{"", "invalid"} {
			err := adminClient.UpdateChatComputerUseProvider(ctx, optimus-ide-collabsdk.UpdateChatComputerUseProviderRequest{
				Provider: optimus-ide-collabsdk.ChatComputerUseProvider(provider),
			})
			requireSDKError(t, err, http.StatusBadRequest)
		}
	})
	...
	t.Run("NonAdminWriteFails", func(t *testing.T) {
		...
		err := memberClient.UpdateChatComputerUseProvider(ctx, optimus-ide-collabsdk.UpdateChatComputerUseProviderRequest{
			Provider: "openai",
		})
		requireSDKError(t, err, http.StatusForbidden)
	})
...
func TestChatDebugLoggingSettings(t *testing.T) {
	...
	t.Run("AdminAllowsUsersToOptIn", func(t *testing.T) {
		...
		err := adminClient.UpdateChatDebugLogging(ctx, optimus-ide-collabsdk.UpdateChatDebugLoggingAllowUsersRequest{
			AllowUsers: true,
		})
		require.NoError(t, err)
		...
		err = memberClient.UpdateUserChatDebugLogging(ctx, optimus-ide-collabsdk.UpdateUserChatDebugLoggingRequest{
	...
	t.Run("NonAdminCannotManageAdminSetting", func(t *testing.T) {
		...
		err = memberClient.UpdateChatDebugLogging(ctx, optimus-ide-collabsdk.UpdateChatDebugLoggingAllowUsersRequest{
			AllowUsers: true,
		})
		requireSDKError(t, err, http.StatusForbidden)
	})
...
func seedChatDebugRun(
	...
	run, err := db.InsertChatDebugRun(dbauthz.AsSystemRestricted(ctx), database.InsertChatDebugRunParams{
		ChatID:    chatID,
		Kind:      string(optimus-ide-collabsdk.ChatDebugRunKindChatTurn),
		Status:    string(optimus-ide-collabsdk.ChatDebugStatusInProgress),
...
func seedChatDebugStep(
	...
	step, err := db.InsertChatDebugStep(dbauthz.AsSystemRestricted(ctx), database.InsertChatDebugStepParams{
		...
		Operation:  string(optimus-ide-collabsdk.ChatDebugStepOperationStream),
		Status:     string(optimus-ide-collabsdk.ChatDebugStatusCompleted),
	})
	require.NoError(t, err)
	return step
}
...
func TestChatDebugRuns(t *testing.T) {
	...
	t.Run("ListReturnsRunsNewestFirst", func(t *testing.T) {
		...
		require.Equal(t, optimus-ide-collabsdk.ChatDebugRunKindChatTurn, runs[0].Kind)
		require.Equal(t, optimus-ide-collabsdk.ChatDebugStatusInProgress, runs[0].Status)
	})
...
func TestChatDebugRun(t *testing.T) {
	...
	t.Run("ReturnsRunWithSteps", func(t *testing.T) {
		...
		require.Equal(t, optimus-ide-collabsdk.ChatDebugRunKindChatTurn, got.Kind)
		require.Equal(t, optimus-ide-collabsdk.ChatDebugStatusInProgress, got.Status)
		...
		require.Equal(t, optimus-ide-collabsdk.ChatDebugStepOperationStream, got.Steps[0].Operation)
	})
...
func TestChatAdvisorConfig_Update(t *testing.T) {
	...
	want := optimus-ide-collabsdk.AdvisorConfig{
		Enabled:         true,
		MaxUsesPerRun:   5,
...
func TestChatAdvisorConfig_MemberCannotWriteButCanRead(t *testing.T) {
	...
	err = memberClient.UpdateChatAdvisorConfig(ctx, optimus-ide-collabsdk.UpdateAdvisorConfigRequest{
		Enabled: true,
	})
	requireSDKError(t, err, http.StatusForbidden)
...
func TestChatAdvisorConfig_NegativeMaxUsesPerRunRejected(t *testing.T) {
	...
	err := adminClient.UpdateChatAdvisorConfig(ctx, optimus-ide-collabsdk.UpdateAdvisorConfigRequest{
		MaxUsesPerRun: -1,
	})
	sdkErr := requireSDKError(t, err, http.StatusBadRequest)
...
func TestChatAdvisorConfig_RoundTripModelConfigID(t *testing.T) {
	...
	modelConfig := createAdditionalChatModelConfigWithReasoningEffort(
		...
		"openai",
		"gpt-5.2",
		optimus-ide-collabsdk.ChatModelReasoningEffortMedium,
		optimus-ide-collabsdk.ChatModelReasoningEffortXHigh,
	)
	...
	want := optimus-ide-collabsdk.AdvisorConfig{
		...
		ModelConfigID:   modelConfig.ID,
		ReasoningEffort: ptr.Ref(optimus-ide-collabsdk.ChatModelReasoningEffortHigh),
	}
...
func TestChatAdvisorConfig_ReasoningEffortMustBeSelectable(t *testing.T) {
	...
	modelConfig := createAdditionalChatModelConfigWithReasoningEffort(
		...
		"openai",
		"gpt-5.2",
		optimus-ide-collabsdk.ChatModelReasoningEffortLow,
		optimus-ide-collabsdk.ChatModelReasoningEffortMedium,
	)
...
func TestChatAdvisorConfig_OverwriteClearsPreviousValues(t *testing.T) {
	...
	rich := optimus-ide-collabsdk.AdvisorConfig{
		Enabled:         true,
		MaxUsesPerRun:   5,
		MaxOutputTokens: 1024,
		ModelConfigID:   modelConfig.ID,
	...
	sparse := optimus-ide-collabsdk.AdvisorConfig{Enabled: true}
...
func TestChatAdvisorConfig_EnabledReflectsExperiment(t *testing.T) {
	...
	err := adminClient.UpdateChatAdvisorConfig(ctx, optimus-ide-collabsdk.AdvisorConfig{
		Enabled:       true,
		MaxUsesPerRun: 2,
	})
	require.NoError(t, err)
...
func TestChatAdvisorConfig_ClampsNegativeStoredValues(t *testing.T) {
	...
	resp, err := adminClient.GetChatAdvisorConfig(ctx)
	require.NoError(t, err)
	require.Equal(t, optimus-ide-collabsdk.AdvisorConfig{
		Enabled:         true,
		MaxUsesPerRun:   0,
		MaxOutputTokens: 0,
	}, resp)
...
func TestChatAdvisorConfig_UnauthenticatedFails(t *testing.T) {
	...
	err = anonClient.UpdateChatAdvisorConfig(ctx, optimus-ide-collabsdk.UpdateAdvisorConfigRequest{
		Enabled: true,
	})
	require.ErrorAs(t, err, &sdkErr)
	require.Equal(t, http.StatusUnauthorized, sdkErr.StatusCode())
...
func TestChatWorkspaceTTL(t *testing.T) {
	...
	err = adminClient.UpdateChatWorkspaceTTL(ctx, optimus-ide-collabsdk.UpdateChatWorkspaceTTLRequest{
	...
	err = memberClient.UpdateChatWorkspaceTTL(ctx, optimus-ide-collabsdk.UpdateChatWorkspaceTTLRequest{
...
func TestChatRetentionDays(t *testing.T) {
	...
	err = adminClient.UpdateChatRetentionDays(ctx, optimus-ide-collabsdk.UpdateChatRetentionDaysRequest{
		RetentionDays: 90,
	})
	...
	err = memberClient.UpdateChatRetentionDays(ctx, optimus-ide-collabsdk.UpdateChatRetentionDaysRequest{RetentionDays: 7})
	requireSDKError(t, err, http.StatusForbidden)
	...
	err = adminClient.UpdateChatRetentionDays(ctx, optimus-ide-collabsdk.UpdateChatRetentionDaysRequest{
		RetentionDays: 3651, // retentionDaysMaximum + 1; keep in sync with optimus-ide-collabd/exp_chats.go.
	})
	requireSDKError(t, err, http.StatusBadRequest)
}
...
func TestChatDebugRetentionDays(t *testing.T) {
	...
	require.NoError(t, err, "get default")
	require.Equal(t, optimus-ide-collabsdk.DefaultChatDebugRetentionDays, resp.DebugRetentionDays, "default should match DefaultChatDebugRetentionDays")

	// Admin can set debug retention days to 14.
	err = adminClient.UpdateChatDebugRetentionDays(ctx, optimus-ide-collabsdk.UpdateChatDebugRetentionDaysRequest{
	...
	// Non-admin member cannot write.
	err = memberClient.UpdateChatDebugRetentionDays(ctx, optimus-ide-collabsdk.UpdateChatDebugRetentionDaysRequest{DebugRetentionDays: 7})
	requireSDKError(t, err, http.StatusForbidden)
	...
	// Validation: exceeding the 3650-day maximum is rejected.
	err = adminClient.UpdateChatDebugRetentionDays(ctx, optimus-ide-collabsdk.UpdateChatDebugRetentionDaysRequest{
		DebugRetentionDays: 3651, // chatDebugRetentionDaysMaximum + 1; keep in sync with optimus-ide-collabd/exp_chats.go.
	})
	requireSDKError(t, err, http.StatusBadRequest)
}
...
func TestChatAutoArchiveDays(t *testing.T) {
	...
	require.NoError(t, err, "get default")
	require.Equal(t, optimus-ide-collabsdk.DefaultChatAutoArchiveDays, resp.AutoArchiveDays, "default should match DefaultChatAutoArchiveDays")

	// Admin can set auto-archive days to 45.
	err = adminClient.UpdateChatAutoArchiveDays(ctx, optimus-ide-collabsdk.UpdateChatAutoArchiveDaysRequest{
		AutoArchiveDays: 45,
	})
	...
	// Non-admin member cannot write.
	err = memberClient.UpdateChatAutoArchiveDays(ctx, optimus-ide-collabsdk.UpdateChatAutoArchiveDaysRequest{AutoArchiveDays: 7})
	requireSDKError(t, err, http.StatusForbidden)
	...
	// Validation: exceeding the 3650-day maximum is rejected.
	err = adminClient.UpdateChatAutoArchiveDays(ctx, optimus-ide-collabsdk.UpdateChatAutoArchiveDaysRequest{
		AutoArchiveDays: 3651, // autoArchiveDaysMaximum + 1; keep in sync with optimus-ide-collabd/exp_chats.go.
	})
	requireSDKError(t, err, http.StatusBadRequest)
}
...
func TestUserChatCompactionThresholds(t *testing.T) {
	...
	t.Run("PutAndGet", func(t *testing.T) { //nolint:paralleltest // subtests share parent state
		...
		override, err := client.UpdateUserChatCompactionThreshold(ctx, modelConfig.ID, optimus-ide-collabsdk.UpdateUserChatCompactionThresholdRequest{
			ThresholdPercent: 75,
		})
		require.NoError(t, err)
	...
	t.Run("UpsertChangesValue", func(t *testing.T) { //nolint:paralleltest // subtests share parent state
		...
		_, err := client.UpdateUserChatCompactionThreshold(ctx, modelConfig.ID, optimus-ide-collabsdk.UpdateUserChatCompactionThresholdRequest{
			ThresholdPercent: 50,
		})
		require.NoError(t, err)
	...
	t.Run("NonExistentModelConfig", func(t *testing.T) { //nolint:paralleltest // subtests share parent state
		...
		fakeID := uuid.New()
		_, err := client.UpdateUserChatCompactionThreshold(ctx, fakeID, optimus-ide-collabsdk.UpdateUserChatCompactionThresholdRequest{
			ThresholdPercent: 50,
		})
		requireSDKError(t, err, http.StatusNotFound)
	})
...
//nolint:tparallel // Subtests share a single optimus-ide-collabdtest instance and run sequentially.
func TestChatTemplateAllowlist(t *testing.T) {
	t.Parallel()

	// Shared setup: one optimus-ide-collabdtest instance with two real templates.
	// Subtests that need valid template IDs use these.
	client, store := newChatClientWithDatabase(t)
	admin := optimus-ide-collabdtest.CreateFirstUser(t, client.Client)
	...
	//nolint:paralleltest // Sequential: subtests share a single optimus-ide-collabdtest instance.
	t.Run("ReturnsEmptyWhenUnset", func(t *testing.T) {
		ctx := testutil.Context(t, testutil.WaitLong)
	...
	t.Run("AdminCanSet", func(t *testing.T) {
		...
		err := client.UpdateChatTemplateAllowlist(ctx, optimus-ide-collabsdk.ChatTemplateAllowlist{TemplateIDs: ids})
		require.NoError(t, err)
		resp, err := client.GetChatTemplateAllowlist(ctx)
		require.NoError(t, err)
	...
	t.Run("AdminCanClear", func(t *testing.T) {
		ctx := testutil.Context(t, testutil.WaitLong)
		err := client.UpdateChatTemplateAllowlist(ctx, optimus-ide-collabsdk.ChatTemplateAllowlist{TemplateIDs: []string{}})
		require.NoError(t, err)
		resp, err := client.GetChatTemplateAllowlist(ctx)
		require.NoError(t, err)
		require.Empty(t, resp.TemplateIDs)
	...
	t.Run("NonAdminReadFails", func(t *testing.T) {
		ctx := testutil.Context(t, testutil.WaitLong)
		memberClientRaw, _ := optimus-ide-collabdtest.CreateAnotherUser(t, client.Client, admin.OrganizationID)
		memberClient := optimus-ide-collabsdk.NewExperimentalClient(memberClientRaw)
	...
	t.Run("NonAdminWriteFails", func(t *testing.T) {
		...
		err := memberClient.UpdateChatTemplateAllowlist(ctx, optimus-ide-collabsdk.ChatTemplateAllowlist{TemplateIDs: []string{uuid.NewString()}})
		requireSDKError(t, err, http.StatusNotFound)
	})
	...
	t.Run("UnauthenticatedFails", func(t *testing.T) {
		ctx := testutil.Context(t, testutil.WaitLong)
		anonClient := optimus-ide-collabsdk.NewExperimentalClient(optimus-ide-collabsdk.New(client.URL))
		// Uses a random UUID — hits 401 before template validation.
		err := anonClient.UpdateChatTemplateAllowlist(ctx, optimus-ide-collabsdk.ChatTemplateAllowlist{TemplateIDs: []string{uuid.NewString()}})
		requireSDKError(t, err, http.StatusUnauthorized)
	})
	...
	t.Run("InvalidUUIDRejected", func(t *testing.T) {
		ctx := testutil.Context(t, testutil.WaitLong)
		err := client.UpdateChatTemplateAllowlist(ctx, optimus-ide-collabsdk.ChatTemplateAllowlist{TemplateIDs: []string{"not-a-uuid"}})
		requireSDKError(t, err, http.StatusBadRequest)
	})
	...
	t.Run("NonexistentTemplateRejected", func(t *testing.T) {
		ctx := testutil.Context(t, testutil.WaitLong)
		err := client.UpdateChatTemplateAllowlist(ctx, optimus-ide-collabsdk.ChatTemplateAllowlist{TemplateIDs: []string{uuid.NewString()}})
		requireSDKError(t, err, http.StatusBadRequest)
	})
	...
	t.Run("DeprecatedTemplateRejected", func(t *testing.T) {
		ctx := testutil.Context(t, testutil.WaitLong)
		err := client.UpdateChatTemplateAllowlist(ctx, optimus-ide-collabsdk.ChatTemplateAllowlist{
...
func TestGetChatsByWorkspace(t *testing.T) {
	...
	t.Run("ReturnsLatestNonArchivedChat", func(t *testing.T) {
		...
		err := client.UpdateChat(ctx, olderChat.ID, optimus-ide-collabsdk.UpdateChatRequest{Archived: ptr.Ref(true)})
		require.NoError(t, err)
...
func TestSubmitToolResults(t *testing.T) {
	...
	//
	// Callers must build their optimus-ide-collabd with withChatWorkerDisabled: an
	...
	setupRequiresAction := func(
		...
		parts := make([]optimus-ide-collabsdk.ChatMessagePart, 0, len(toolCallIDs))
		for _, id := range toolCallIDs {
			parts = append(parts, optimus-ide-collabsdk.ChatMessagePart{
				Type:       optimus-ide-collabsdk.ChatMessagePartTypeToolCall,
				ToolCallID: id,
				ToolName:   dynamicToolName,
	...
	t.Run("Success", func(t *testing.T) {
		...
		err := client.SubmitToolResults(ctx, chat.ID, optimus-ide-collabsdk.SubmitToolResultsRequest{
			Results: []optimus-ide-collabsdk.ToolResult{
		...
		require.NoError(t, err)
		require.NotEqual(t, optimus-ide-collabsdk.ChatStatusRequiresAction, gotChat.Status,
		...
		for _, msg := range msgsResp.Messages {
			if msg.Role == optimus-ide-collabsdk.ChatMessageRoleTool {
				toolResultCount++
			}
		}
	...
	t.Run("NotFoundForDifferentUser", func(t *testing.T) {
		...
		// to user A's chat.
		otherClientRaw, _ := optimus-ide-collabdtest.CreateAnotherUser(
			t, client.Client, user.OrganizationID,
			rbac.ScopedRoleAgentsAccess(user.OrganizationID),
		)
		otherClient := optimus-ide-collabsdk.NewExperimentalClient(otherClientRaw)

		err := otherClient.SubmitToolResults(ctx, chat.ID, optimus-ide-collabsdk.SubmitToolResultsRequest{
			Results: []optimus-ide-collabsdk.ToolResult{
	...
	t.Run("MemberWithoutAgentsAccess", func(t *testing.T) {
		...
		err := memberClient.SubmitToolResults(ctx, chat.ID, optimus-ide-collabsdk.SubmitToolResultsRequest{
			Results: []optimus-ide-collabsdk.ToolResult{
...
func TestPostChats_DynamicToolValidation(t *testing.T) {
	...
	t.Run("TooManyTools", func(t *testing.T) {
		...
		tools := make([]optimus-ide-collabsdk.DynamicTool, 251)
		for i := range tools {
			tools[i] = optimus-ide-collabsdk.DynamicTool{
	...
	t.Run("EmptyToolName", func(t *testing.T) {
		...
		_, err := client.CreateChat(ctx, optimus-ide-collabsdk.CreateChatRequest{
			...
			UnsafeDynamicTools: []optimus-ide-collabsdk.DynamicTool{
				{Name: ""},
			},
		})
		sdkErr := requireSDKError(t, err, http.StatusBadRequest)
...
func TestChatStartWorkspace_RequireActiveVersion(t *testing.T) {
	...
	db := api.Database
	user := optimus-ide-collabdtest.CreateFirstUser(t, rawClient)
	...
	build, err := optimus-ide-collabd.ChatStartWorkspace(api, ctx, user.UserID, wsResp.Workspace.ID,
...
func TestChatStopWorkspace_BypassesRequireActiveVersion(t *testing.T) {
	...
	build, err := optimus-ide-collabd.ChatStopWorkspace(api, ctx, user.UserID, wsResp.Workspace.ID,
		optimus-ide-collabsdk.CreateWorkspaceBuildRequest{})
...
func TestGetChatMessages_Pagination(t *testing.T) {
	...
	seedChat := func(
		...
		for i := range count {
			content, err := chatprompt.MarshalParts([]optimus-ide-collabsdk.ChatMessagePart{
				optimus-ide-collabsdk.ChatMessageText(fmt.Sprintf("msg %d", i)),
			})
			require.NoError(t, err)
	...
	seedQueuedMessage := func(
		...
		content, err := json.Marshal([]optimus-ide-collabsdk.ChatMessagePart{
			optimus-ide-collabsdk.ChatMessageText("queued"),
		})
		require.NoError(t, err)
	...
	t.Run("BeforeIDReturnsOlderAndSuppressesQueued", func(t *testing.T) {
		...
		resp, err := client.GetChatMessages(ctx, chat.ID, &optimus-ide-collabsdk.ChatMessagesPaginationOptions{
			BeforeID: ids[2],
		})
		require.NoError(t, err)
		require.False(t, resp.HasMore)
	...
	t.Run("NegativeAfterIDReturns400", func(t *testing.T) {
		...
		var sdkResp optimus-ide-collabsdk.Response
		require.NoError(t, json.NewDeoptimus-ide-collab(res.Body).Decode(&sdkResp))
		require.Equal(t, "Query parameters have invalid values.", sdkResp.Message)
		require.True(t,
			slices.ContainsFunc(sdkResp.Validations, func(v optimus-ide-collabsdk.ValidationError) bool {
	...
	t.Run("AfterIDGreaterThanOrEqualBeforeIDReturns400", func(t *testing.T) {
		...
		for _, tc := range []struct {
			...
			t.Run(tc.name, func(t *testing.T) {
				...
				_, err := client.GetChatMessages(ctx, chat.ID, &optimus-ide-collabsdk.ChatMessagesPaginationOptions{
					AfterID:  tc.after,
					BeforeID: tc.before,
				})
				sdkErr := requireSDKError(t, err, http.StatusBadRequest)
...
func requireSDKError(t *testing.T, err error, expectedStatus int) *optimus-ide-collabsdk.Error {
	t.Helper()
...
func TestChatReadOnlySharedWriteHandlers(t *testing.T) {
	...
	setup := func(t *testing.T) (
		ctx context.Context,
		ownerClient *optimus-ide-collabsdk.ExperimentalClient,
		sharedClient *optimus-ide-collabsdk.ExperimentalClient,
		...
		owner := optimus-ide-collabdtest.CreateFirstUser(t, ownerClient.Client)
		_ = createChatModelConfig(t, ownerClient)
		sharedRaw, sharedUser := optimus-ide-collabdtest.CreateAnotherUser(
			t,
			ownerClient.Client,
			owner.OrganizationID,
			rbac.ScopedRoleAgentsAccess(owner.OrganizationID),
		)
		sharedClient = optimus-ide-collabsdk.NewExperimentalClient(sharedRaw)

		var err error
		chat, err = ownerClient.CreateChat(ctx, optimus-ide-collabsdk.CreateChatRequest{
	...
	t.Run("GetChatAndMessages", func(t *testing.T) {
		...
		for _, message := range messagesResult.Messages {
			...
			for _, part := range message.Content {
				if part.Type == optimus-ide-collabsdk.ChatMessagePartTypeText && part.Text == sharedChatText {
					foundUserMessage = true
					break
				}
			}
	...
	t.Run("PatchChat", func(t *testing.T) {
		...
		ctx, _, sharedClient, chat, _ := setup(t)
		err := sharedClient.UpdateChat(ctx, chat.ID, optimus-ide-collabsdk.UpdateChatRequest{
			Archived: ptr.Ref(true),
		})
	...
	t.Run("PatchChatMessage", func(t *testing.T) {
		...
		_, err = sharedClient.EditChatMessage(ctx, chat.ID, userMessageID, optimus-ide-collabsdk.EditChatMessageRequest{
			Content: []optimus-ide-collabsdk.ChatInputPart{{
				Type: optimus-ide-collabsdk.ChatInputPartTypeText,
	...
	t.Run("PostChatMessages", func(t *testing.T) {
		...
		ctx, _, sharedClient, chat, _ := setup(t)
		_, err := sharedClient.CreateChatMessage(ctx, chat.ID, optimus-ide-collabsdk.CreateChatMessageRequest{
			Content: []optimus-ide-collabsdk.ChatInputPart{{
				Type: optimus-ide-collabsdk.ChatInputPartTypeText,
	...
	t.Run("PostChatToolResults", func(t *testing.T) {
		...
		ctx, _, sharedClient, chat, _ := setup(t)
		err := sharedClient.SubmitToolResults(ctx, chat.ID, optimus-ide-collabsdk.SubmitToolResultsRequest{
			Results: []optimus-ide-collabsdk.ToolResult{{
...
func TestChatOwnerOnlyWriteHandlers(t *testing.T) {
	...
	setupOrgAdminAndOwnerChat := func(t *testing.T) (
		...
		ctx := testutil.Context(t, testutil.WaitLong)
		ownerClient, db = newChatClientWithDatabase(t)
		firstUser := optimus-ide-collabdtest.CreateFirstUser(t, ownerClient.Client)
		_ = createChatModelConfig(t, ownerClient)
		...
		orgAdminRaw, _ := optimus-ide-collabdtest.CreateAnotherUser(
			t,
			ownerClient.Client,
			firstUser.OrganizationID,
			rbac.ScopedRoleOrgAdmin(firstUser.OrganizationID),
		...
		adminClient = optimus-ide-collabsdk.NewExperimentalClient(orgAdminRaw)
	...
	t.Run("PostChatMessages", func(t *testing.T) {
		...
		_, err := adminClient.CreateChatMessage(ctx, chat.ID, optimus-ide-collabsdk.CreateChatMessageRequest{
			Content: []optimus-ide-collabsdk.ChatInputPart{{
				Type: optimus-ide-collabsdk.ChatInputPartTypeText,
	...
	t.Run("PatchChatMessage", func(t *testing.T) {
		...
		_, err = adminClient.EditChatMessage(ctx, chat.ID, userMessageID, optimus-ide-collabsdk.EditChatMessageRequest{
			Content: []optimus-ide-collabsdk.ChatInputPart{{
				Type: optimus-ide-collabsdk.ChatInputPartTypeText,
	...
	t.Run("SubmitToolResults", func(t *testing.T) {
		...
		err := adminClient.SubmitToolResults(ctx, chat.ID, optimus-ide-collabsdk.SubmitToolResultsRequest{
			Results: []optimus-ide-collabsdk.ToolResult{{
	...
	t.Run("OwnerCanSendMessages", func(t *testing.T) {
		...
		_, err := ownerClient.CreateChatMessage(ctx, chat.ID, optimus-ide-collabsdk.CreateChatMessageRequest{
			Content: []optimus-ide-collabsdk.ChatInputPart{{
				Type: optimus-ide-collabsdk.ChatInputPartTypeText,
