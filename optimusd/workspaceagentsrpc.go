...
func (api *API) workspaceAgentRPC(rw http.ResponseWriter, r *http.Request) {
	...
	if version == "" {
		...
		// Optimus-IDE-Collab v2.7.1 (not to be confused with the Agent API version) calls this endpoint without
	...
	// other clients using the same agent token (e.g. optimus-ide-collab-logstream-kube).
