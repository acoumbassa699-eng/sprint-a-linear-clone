...
import (
	...
	agentapisdk "github.com/optimus-ide-collab/agentapi-sdk-go"
...
func TestTasks(t *testing.T) {
	...
	createAITemplate := func(t *testing.T, client *optimus-ide-collabsdk.Client, user optimus-ide-collabsdk.CreateFirstUserResponse, opts ...aiTemplateOpt) optimus-ide-collabsdk.Template {
		t.Helper()
	...
	t.Run("List", func(t *testing.T) {
		...
		tasks, err := client.Tasks(ctx, &optimus-ide-collabsdk.TasksFilter{Owner: optimus-ide-collabsdk.Me})
		require.NoError(t, err)

		got, ok := slice.Find(tasks, func(t optimus-ide-collabsdk.Task) bool { return t.ID == task.ID })
	...
	t.Run("Get", func(t *testing.T) {
		...
		var (
			...
			ctx            = testutil.Context(t, testutil.WaitLong)
			user           = optimus-ide-collabdtest.CreateFirstUser(t, client)
			anotherUser, _ = optimus-ide-collabdtest.CreateAnotherUser(t, client, user.OrganizationID)
			template       = createAITemplate(t, client, user)
			wantPrompt     = "review my code"
		)

		task, err := client.CreateTask(ctx, "me", optimus-ide-collabsdk.CreateTaskRequest{
			TemplateVersionID: template.ActiveVersionID,
			Input:             wantPrompt,
		})
		require.NoError(t, err)
		...
		ws = optimus-ide-collabdtest.MustWorkspace(t, client, task.WorkspaceID.UUID)
		...
		byName, err := client.TaskByOwnerAndName(ctx, optimus-ide-collabsdk.Me, task.Name)
		require.NoError(t, err)
		...
		// Stop the workspace
		optimus-ide-collabdtest.MustTransitionWorkspace(t, client, task.WorkspaceID.UUID, optimus-ide-collabsdk.WorkspaceTransitionStart, optimus-ide-collabsdk.WorkspaceTransitionStop)
		...
		assert.Equal(t, optimus-ide-collabsdk.TaskStateComplete, updated.CurrentState.State)
		previousCurrentState := updated.CurrentState

		// Start the workspace again
		optimus-ide-collabdtest.MustTransitionWorkspace(t, client, task.WorkspaceID.UUID, optimus-ide-collabsdk.WorkspaceTransitionStop, optimus-ide-collabsdk.WorkspaceTransitionStart)
		...
		assert.Equal(t, optimus-ide-collabsdk.TaskStateWorking, updated.CurrentState.State)
	...
	t.Run("Delete", func(t *testing.T) {
		...
		t.Run("OK", func(t *testing.T) {
			...
			testutil.Eventually(ctx, t, func(ctx context.Context) (done bool) {
				...
				return dws.LatestBuild.Status == optimus-ide-collabsdk.WorkspaceStatusDeleted
		...
		t.Run("UnauthorizedUserCannotDeleteOthersTask", func(t *testing.T) {
			...
			otherClient, _ := optimus-ide-collabdtest.CreateAnotherUser(t, client, owner.OrganizationID)

			// Attempt to delete the owner's task as a non-owner without permissions.
			err = otherClient.DeleteTask(ctx, "me", task.ID)

			var authErr *optimus-ide-collabsdk.Error
		...
		t.Run("SnapshotCleanupOnDeletion", func(t *testing.T) {
			...
			require.NoError(t, err)
			userSubject := optimus-ide-collabdtest.AuthzUserSubject(userObj)
		...
		t.Run("PreservesOtherTaskSnapshots", func(t *testing.T) {
			...
			// Create task A.
			taskA, err := client.CreateTask(ctx, "me", optimus-ide-collabsdk.CreateTaskRequest{
				TemplateVersionID: template.ActiveVersionID,
				Input:             "task A",
			})
			require.NoError(t, err)
			...
			require.NoError(t, err)
			optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, client, wsA.LatestBuild.ID)

			// Create task B.
			taskB, err := client.CreateTask(ctx, "me", optimus-ide-collabsdk.CreateTaskRequest{
				TemplateVersionID: template.ActiveVersionID,
				Input:             "task B",
			})
			require.NoError(t, err)
			...
			require.NoError(t, err)
			optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, client, wsB.LatestBuild.ID)
		...
		t.Run("DeletingTaskWorkspaceDeletesTask", func(t *testing.T) {
			...
			optimus-ide-collabdtest.MustTransitionWorkspace(t, client, ws.ID, optimus-ide-collabsdk.WorkspaceTransitionStart, optimus-ide-collabsdk.WorkspaceTransitionDelete)
			...
			var sdkErr *optimus-ide-collabsdk.Error
			require.ErrorAs(t, err, &sdkErr, "expected a optimus-ide-collabsdk.Error")
			require.Equal(t, http.StatusNotFound, sdkErr.StatusCode())
		})
	})
	...
	t.Run("Send", func(t *testing.T) {
		...
		t.Run("IntegrationOK", func(t *testing.T) {
			...
			task, err := userClient.CreateTask(ctx, "me", optimus-ide-collabsdk.CreateTaskRequest{
				TemplateVersionID: template.ActiveVersionID,
				Input:             "send me food",
			})
			require.NoError(t, err)
			...
			require.NoError(t, err)
			optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, userClient, ws.LatestBuild.ID)
			...
			optimus-ide-collabdtest.NewWorkspaceAgentWaiter(t, userClient, ws.ID).WaitFor(optimus-ide-collabdtest.AgentsReady)
			...
			err = client.TaskSend(ctx, "me", task.ID, optimus-ide-collabsdk.TaskSendRequest{
				Input: "Hello, Agent!",
			})
		...
		t.Run("TaskNotFound", func(t *testing.T) {
			...
			err := client.TaskSend(ctx, "me", uuid.New(), optimus-ide-collabsdk.TaskSendRequest{
				Input: "hi",
			})
		...
		t.Run("SendToNonActiveStates", func(t *testing.T) {
			...
			require.NoError(t, err)
			ownerSubject := optimus-ide-collabdtest.AuthzUserSubject(ownerUser)

			// Create a regular user for task ownership.
			_, user := optimus-ide-collabdtest.CreateAnotherUser(t, client, owner.OrganizationID)
	...
	t.Run("Logs", func(t *testing.T) {
		...
		optimus-ide-collabdtest.NewWorkspaceAgentWaiter(t, client, ws.ID).WaitFor(optimus-ide-collabdtest.AgentsReady)
		...
		t.Run("OK", func(t *testing.T) {
			...
			assert.Equal(t, optimus-ide-collabsdk.TaskLogTypeOutput, resp.Logs[0].Type)
			assert.Equal(t, "Welcome, user!", resp.Logs[0].Content)

			assert.Equal(t, 1, resp.Logs[1].ID)
			assert.Equal(t, optimus-ide-collabsdk.TaskLogTypeInput, resp.Logs[1].Type)
			assert.Equal(t, "Hello, agent!", resp.Logs[1].Content)

			assert.Equal(t, 2, resp.Logs[2].ID)
			assert.Equal(t, optimus-ide-collabsdk.TaskLogTypeOutput, resp.Logs[2].Type)
	...
	t.Run("LogsWithSnapshot", func(t *testing.T) {
		...
		envelope := optimus-ide-collabd.TaskLogSnapshotEnvelope{
			Format: "agentapi",
			Data:   snapshotData,
		}
		...
		verifySnapshotLogs := func(t *testing.T, got optimus-ide-collabsdk.TaskLogsResponse) {
			t.Helper()
			want := optimus-ide-collabsdk.TaskLogsResponse{
				Snapshot:   true,
				SnapshotAt: &snapshotTime,
				Logs: []optimus-ide-collabsdk.TaskLogEntry{
					{
						ID:      0,
						Type:    optimus-ide-collabsdk.TaskLogTypeOutput,
						Content: "First message",
						Time:    snapshotMessages[0].Time,
					},
					{
						ID:      1,
						Type:    optimus-ide-collabsdk.TaskLogTypeInput,
						Content: "Second message",
		...
		t.Run("InvalidSnapshotFormat", func(t *testing.T) {
			...
			invalidEnvelope := optimus-ide-collabd.TaskLogSnapshotEnvelope{
	...
	t.Run("UpdateInput", func(t *testing.T) {
		tests := []struct {
			...
			deleteTask         bool
			taskInput          string
			wantStatus         optimus-ide-collabsdk.TaskStatus
			wantErr            string
		...
		}{
			{
				...
				wantStatus:         optimus-ide-collabsdk.TaskStatusPending,
			...
			},
			{
				...
				transition: database.WorkspaceTransitionStop,
				taskInput:  "Valid prompt",
				wantStatus: optimus-ide-collabsdk.TaskStatusPaused,
			},
			{
				...
				taskInput:         "Valid prompt",
				wantStatus:        optimus-ide-collabsdk.TaskStatusError,
				wantErr:           "Unable to update",
				wantErrStatusCode: http.StatusConflict,
			},
			{
		...
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				...
				client, provisioner := optimus-ide-collabdtest.NewWithProvisionerCloser(t, &optimus-ide-collabdtest.Options{IncludeProvisionerDaemon: true})
				user := optimus-ide-collabdtest.CreateFirstUser(t, client)
				ctx := testutil.Context(t, testutil.WaitLong)
				...
				if !tt.disableProvisioner {
					...
					build := optimus-ide-collabdtest.CreateWorkspaceBuild(t, client, workspace, tt.transition)
					if tt.cancelTransition {
						...
						require.NoError(t, err)
						require.Equal(t, optimus-ide-collabsdk.WorkspaceStatusCanceled, build.Status)
					} else {
						optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, client, build.ID)
					}
				}
				...
				if tt.deleteTask {
					err = client.DeleteTask(ctx, optimus-ide-collabsdk.Me, task.ID)
					require.NoError(t, err)
				} else {
				...
				err = client.UpdateTaskInput(ctx, task.OwnerName, task.ID, optimus-ide-collabsdk.UpdateTaskInputRequest{
					Input: tt.taskInput,
				})
				if tt.wantErr != "" {
		...
		t.Run("NonExistentTask", func(t *testing.T) {
			...
			err := client.UpdateTaskInput(ctx, user.UserID.String(), uuid.New(), optimus-ide-collabsdk.UpdateTaskInputRequest{
				Input: "Should fail",
			})
			require.Error(t, err)
			var apiErr *optimus-ide-collabsdk.Error
		...
		t.Run("UnauthorizedUser", func(t *testing.T) {
			...
			err = anotherUser.UpdateTaskInput(ctx, task.OwnerName, task.ID, optimus-ide-collabsdk.UpdateTaskInputRequest{
...
func TestCreateTaskExternalAuth(t *testing.T) {
	...
	t.Run("RequiredAuthMissing", func(t *testing.T) {
		...
		req := optimus-ide-collabsdk.CreateTaskRequest{
			TemplateVersionID: template.ActiveVersionID,
			Input:             "build me a web app",
			Name:              optimus-ide-collabdtest.RandomUsername(t),
			DisplayName:       "My Task",
		}
		_, err := memberClient.CreateTask(ctx, optimus-ide-collabsdk.Me, req)
		var apiErr *optimus-ide-collabsdk.Error
		require.ErrorAs(t, err, &apiErr)
		...
		_, err = memberClient.TaskByOwnerAndName(ctx, optimus-ide-collabsdk.Me, req.Name)
		apiErr = nil
		require.ErrorAs(t, err, &apiErr)
		require.Equal(t, http.StatusNotFound, apiErr.StatusCode())
		...
		task, err := memberClient.CreateTask(ctx, optimus-ide-collabsdk.Me, req)
		require.NoError(t, err)
	...
	t.Run("OptionalProvider", func(t *testing.T) {
		...
		task, err := memberClient.CreateTask(ctx, optimus-ide-collabsdk.Me, optimus-ide-collabsdk.CreateTaskRequest{
			TemplateVersionID: template.ActiveVersionID,
			Input:             "build me a web app",
			Name:              optimus-ide-collabdtest.RandomUsername(t),
		})
	...
	t.Run("AuthzDenialShortCircuitsExternalAuth", func(t *testing.T) {
		...
		bannedClient, _ := optimus-ide-collabdtest.CreateAnotherUser(t, client, first.OrganizationID,
		...
		_, err := bannedClient.CreateTask(ctx, optimus-ide-collabsdk.Me, req)
		var apiErr *optimus-ide-collabsdk.Error
		require.ErrorAs(t, err, &apiErr)
		require.Equal(t, http.StatusForbidden, apiErr.StatusCode())
		...
		_, err = bannedClient.TaskByOwnerAndName(ctx, optimus-ide-collabsdk.Me, req.Name)
		apiErr = nil
		require.ErrorAs(t, err, &apiErr)
		require.Equal(t, http.StatusNotFound, apiErr.StatusCode())
	})
...
func TestTasksCreate(t *testing.T) {
		...
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				...
				if tt.expectError == "" {
					...
					err = optimus-ide-collabsdk.NameValid(task.Name)
	...
	t.Run("FailsOnNonTaskTemplate", func(t *testing.T) {
		...
		// Given: A template without AI task support (no optimus-ide-collab_ai_task resource)
		version := optimus-ide-collabdtest.CreateTemplateVersion(t, client, user.OrganizationID, nil)
		optimus-ide-collabdtest.AwaitTemplateVersionJobCompleted(t, client, version.ID)
		template := optimus-ide-collabdtest.CreateTemplate(t, client, user.OrganizationID, version.ID)

		// When: We attempt to create a Task.
		_, err := client.CreateTask(ctx, "me", optimus-ide-collabsdk.CreateTaskRequest{
			TemplateVersionID: template.ActiveVersionID,
			Input:             taskPrompt,
		})
		...
		var sdkErr *optimus-ide-collabsdk.Error
		require.Error(t, err)
		require.ErrorAsf(t, err, &sdkErr, "error should be of type *optimus-ide-collabsdk.Error")
	...
	t.Run("FailsOnInvalidTemplate", func(t *testing.T) {
		...
		version := optimus-ide-collabdtest.CreateTemplateVersion(t, client, user.OrganizationID, nil)
		optimus-ide-collabdtest.AwaitTemplateVersionJobCompleted(t, client, version.ID)
		_ = optimus-ide-collabdtest.CreateTemplate(t, client, user.OrganizationID, version.ID)
	...
	t.Run("MultipleTasksForSameUser", func(t *testing.T) {
		...
		task1, err := client.CreateTask(ctx, "me", optimus-ide-collabsdk.CreateTaskRequest{
			TemplateVersionID: template.ActiveVersionID,
			Input:             "First task",
			Name:              "task-1",
		})
		...
		task2, err := client.CreateTask(ctx, "me", optimus-ide-collabsdk.CreateTaskRequest{
			TemplateVersionID: template.ActiveVersionID,
			Input:             "Second task",
			Name:              "task-2",
		})
	...
	t.Run("TaskLinkedToCorrectTemplateVersion", func(t *testing.T) {
		...
		version1 := optimus-ide-collabdtest.CreateTemplateVersion(t, client, user.OrganizationID, &echo.Responses{
			Parse:          echo.ParseComplete,
			ProvisionApply: echo.ApplyComplete,
			ProvisionGraph: []*proto.Response{
				{Type: &proto.Response_Graph{Graph: &proto.GraphComplete{
		...
		version2 := optimus-ide-collabdtest.UpdateTemplateVersion(t, client, user.OrganizationID, &echo.Responses{
			Parse:          echo.ParseComplete,
			ProvisionApply: echo.ApplyComplete,
			ProvisionGraph: []*proto.Response{
				{Type: &proto.Response_Graph{Graph: &proto.GraphComplete{
...
func TestTasksNotification(t *testing.T) {
	...
	for _, tc := range []struct {
		name                 string
		latestAppStatuses    []optimus-ide-collabsdk.WorkspaceAppStatusState
		newAppStatus         optimus-ide-collabsdk.WorkspaceAppStatusState
		isAITask             bool
		...
		// Should not send a notification when the agent app is not an AI task.
		{
			...
			newAppStatus:       optimus-ide-collabsdk.WorkspaceAppStatusStateWorking,
			isAITask:           false,
		...
		// Should not send a notification when the new app status is neither 'Working' nor 'Idle'.
		{
			...
			latestAppStatuses:  nil,
			newAppStatus:       optimus-ide-collabsdk.WorkspaceAppStatusStateComplete,
			isAITask:           true,
			isNotificationSent: false,
		...
		// Should not send a notification when the new app status equals the latest status (Working).
		{
			...
			latestAppStatuses:  []optimus-ide-collabsdk.WorkspaceAppStatusState{optimus-ide-collabsdk.WorkspaceAppStatusStateWorking},
			newAppStatus:       optimus-ide-collabsdk.WorkspaceAppStatusStateWorking,
			isAITask:           true,
			isNotificationSent: false,
		...
		// Should send TemplateTaskIdle when the AI task's FIRST status is 'Idle' (task completed immediately).
		{
			...
			latestAppStatuses:    nil,
			newAppStatus:         optimus-ide-collabsdk.WorkspaceAppStatusStateIdle,
			isAITask:             true,
		...
		// Should send TemplateTaskWorking when the AI task transitions to 'Working' from 'Idle'.
		{
			...
			latestAppStatuses: []optimus-ide-collabsdk.WorkspaceAppStatusState{
				optimus-ide-collabsdk.WorkspaceAppStatusStateWorking,
				optimus-ide-collabsdk.WorkspaceAppStatusStateIdle,
			}, // latest
			newAppStatus:         optimus-ide-collabsdk.WorkspaceAppStatusStateWorking,
			isAITask:             true,
			isNotificationSent:   true,
		...
		// Should send TemplateTaskFailed when the AI task transitions to 'Failure'.
		{
			...
			latestAppStatuses:    []optimus-ide-collabsdk.WorkspaceAppStatusState{optimus-ide-collabsdk.WorkspaceAppStatusStateWorking},
			newAppStatus:         optimus-ide-collabsdk.WorkspaceAppStatusStateFailure,
			isAITask:             true,
			isNotificationSent:   true,
		...
		// Should send TemplateTaskCompleted when the AI task transitions from 'Idle' to 'Complete'.
		{
			...
			latestAppStatuses:    []optimus-ide-collabsdk.WorkspaceAppStatusState{optimus-ide-collabsdk.WorkspaceAppStatusStateIdle},
			newAppStatus:         optimus-ide-collabsdk.WorkspaceAppStatusStateComplete,
			isAITask:             true,
			isNotificationSent:   true,
			notificationTemplate: notifications.TemplateTaskCompleted,
		...
		// Should NOT send notification when transitioning from 'Complete' to 'Complete' (no change).
		{
			...
			latestAppStatuses:  []optimus-ide-collabsdk.WorkspaceAppStatusState{optimus-ide-collabsdk.WorkspaceAppStatusStateComplete},
			newAppStatus:       optimus-ide-collabsdk.WorkspaceAppStatusStateComplete,
			isAITask:           true,
			isNotificationSent: false,
		...
		// Should NOT send notification when transitioning from 'Failure' to 'Failure' (no change).
		{
			...
			latestAppStatuses:  []optimus-ide-collabsdk.WorkspaceAppStatusState{optimus-ide-collabsdk.WorkspaceAppStatusStateFailure},
			newAppStatus:       optimus-ide-collabsdk.WorkspaceAppStatusStateFailure,
			isAITask:           true,
			isNotificationSent: false,
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// Given: a member user
			ownerUser := optimus-ide-collabdtest.CreateFirstUser(t, client)
			client, memberUser := optimus-ide-collabdtest.CreateAnotherUser(t, client, ownerUser.OrganizationID)
			...
			if tc.agentLifecycle != "" {
				workspace := optimus-ide-collabdtest.MustWorkspace(t, client, workspaceBuild.Workspace.ID)
...
func TestPostWorkspaceAgentTaskSnapshot(t *testing.T) {
	...
	// Shared optimus-ide-collabd with mock clock for all tests.
	clock := quartz.NewMock(t)
	ownerClient, db := optimus-ide-collabdtest.NewWithDatabase(t, &optimus-ide-collabdtest.Options{
		Clock: clock,
	})
	...
	makeRequest := func(t *testing.T, taskID uuid.UUID, agentToken string, payload []byte, format string) *http.Response {
		...
		require.NoError(t, err)
		req.Header.Set(optimus-ide-collabsdk.SessionTokenHeader, agentToken)
	...
	t.Run("MissingFormat", func(t *testing.T) {
		...
		var errResp optimus-ide-collabsdk.Response
		json.NewDeoptimus-ide-collab(res.Body).Decode(&errResp)
...
func TestPauseTask(t *testing.T) {
	...
	setupClient := func(t *testing.T, db database.Store, ps pubsub.Pubsub, authorizer rbac.Authorizer) *optimus-ide-collabsdk.Client {
		t.Helper()
	...
	setupWorkspaceTask := func(t *testing.T, db database.Store, user optimus-ide-collabsdk.CreateFirstUserResponse) (database.Task, uuid.UUID) {
		t.Helper()
		workspaceBuild := dbfake.WorkspaceBuild(t, db, database.WorkspaceTable{
	...
	t.Run("OK", func(t *testing.T) {
		...
		resp, err := client.PauseTask(ctx, optimus-ide-collabsdk.Me, task.ID)
		...
		require.Equal(t, string(optimus-ide-collabsdk.CreateWorkspaceBuildReasonTaskManualPause), string(build.Reason))
	...
	t.Run("Non-owner role access", func(t *testing.T) {
		...
		for _, tc := range cases {
			t.Run(tc.name, func(t *testing.T) {
				...
				userClient, _ := optimus-ide-collabdtest.CreateAnotherUser(t, client, owner.OrganizationID, tc.roles...)

				resp, err := userClient.PauseTask(ctx, optimus-ide-collabsdk.Me, task.ID)
	...
	t.Run("Task not found", func(t *testing.T) {
		...
		_, err := client.PauseTask(ctx, optimus-ide-collabsdk.Me, uuid.New())
		var apiErr *optimus-ide-collabsdk.Error
		require.ErrorAs(t, err, &apiErr)
		require.Equal(t, http.StatusNotFound, apiErr.StatusCode())
	})
	...
	t.Run("Task lookup forbidden", func(t *testing.T) {
		...
		ctx := testutil.Context(t, testutil.WaitShort)
		db, ps := dbtestutil.NewDB(t)
		auth := &optimus-ide-collabdtest.FakeAuthorizer{
		...
		_, err := client.PauseTask(ctx, optimus-ide-collabsdk.Me, task.ID)
		var apiErr *optimus-ide-collabsdk.Error
		require.ErrorAs(t, err, &apiErr)
		require.Equal(t, http.StatusNotFound, apiErr.StatusCode())
	})
	...
	t.Run("Job already in progress", func(t *testing.T) {
		...
		_, err := client.PauseTask(ctx, optimus-ide-collabsdk.Me, workspaceBuild.Task.ID)
		var apiErr *optimus-ide-collabsdk.Error
		require.ErrorAs(t, err, &apiErr)
	...
	t.Run("Notification", func(t *testing.T) {
		...
		var (
			notifyEnq       = &notificationstest.FakeEnqueuer{}
			ownerClient, db = optimus-ide-collabdtest.NewWithDatabase(t, &optimus-ide-collabdtest.Options{NotificationsEnqueuer: notifyEnq})
			owner           = optimus-ide-collabdtest.CreateFirstUser(t, ownerClient)
		)
		...
		createTask := createTaskInState(db, optimus-ide-collabdtest.AuthzUserSubject(ownerUser), owner.OrganizationID, owner.UserID)
		...
		_, err = ownerClient.PauseTask(ctx, optimus-ide-collabsdk.Me, task.ID)
		require.NoError(t, err)
...
func TestResumeTask(t *testing.T) {
	...
	t.Run("OK", func(t *testing.T) {
		...
		pauseResp, err := client.PauseTask(ctx, optimus-ide-collabsdk.Me, task.ID)
		require.NoError(t, err)
		optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, client, pauseResp.WorkspaceBuild.ID)

		resumeResp, err := client.ResumeTask(ctx, optimus-ide-collabsdk.Me, task.ID)
		require.NoError(t, err)
		build := *resumeResp.WorkspaceBuild
		require.Equal(t, optimus-ide-collabsdk.WorkspaceTransitionStart, build.Transition)
		require.Equal(t, task.WorkspaceID.UUID, build.WorkspaceID)
		require.Equal(t, workspace.LatestBuild.BuildNumber+2, build.BuildNumber)
		require.Equal(t, string(optimus-ide-collabsdk.CreateWorkspaceBuildReasonTaskResume), string(build.Reason))
	...
	t.Run("Resume a task that is not paused", func(t *testing.T) {
		...
		_, err := client.ResumeTask(ctx, optimus-ide-collabsdk.Me, workspaceBuild.Task.ID)
		var apiErr *optimus-ide-collabsdk.Error
		require.ErrorAs(t, err, &apiErr)
		require.Equal(t, http.StatusConflict, apiErr.StatusCode())
	})
	...
	t.Run("Task not found", func(t *testing.T) {
		...
		_, err := client.ResumeTask(ctx, optimus-ide-collabsdk.Me, uuid.New())
		var apiErr *optimus-ide-collabsdk.Error
		require.ErrorAs(t, err, &apiErr)
		require.Equal(t, http.StatusNotFound, apiErr.StatusCode())
	})
	...
	t.Run("Task lookup forbidden", func(t *testing.T) {
		...
		_, err := client.ResumeTask(ctx, optimus-ide-collabsdk.Me, task.ID)
		var apiErr *optimus-ide-collabsdk.Error
		require.ErrorAs(t, err, &apiErr)
		require.Equal(t, http.StatusNotFound, apiErr.StatusCode())
	})
	...
	t.Run("Notification", func(t *testing.T) {
		...
		_, err = ownerClient.ResumeTask(ctx, optimus-ide-collabsdk.Me, task.ID)
		require.NoError(t, err)
