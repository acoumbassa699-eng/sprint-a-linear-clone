...
// Duplicated in optimus-ide-collabsdk.
...
// @Success 200 {object} optimus-ide-collabsdk.DAUsResponse
...
func (api *API) returnDAUsInternal(rw http.ResponseWriter, r *http.Request, templateIDs []uuid.UUID) {
	...
	resp := optimus-ide-collabsdk.DAUsResponse{
		TZHourOffset: tzOffset,
		Entries:      make([]optimus-ide-collabsdk.DAUEntry, 0, len(rows)),
	}
	for _, row := range rows {
		resp.Entries = append(resp.Entries, optimus-ide-collabsdk.DAUEntry{
...
// @Success 200 {object} optimus-ide-collabsdk.UserActivityInsightsResponse
...
func (api *API) insightsUserActivity(rw http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		if xerrors.Is(err, sql.ErrNoRows) {
			httpapi.Write(ctx, rw, http.StatusOK, optimus-ide-collabsdk.UserActivityInsightsResponse{
				Report: optimus-ide-collabsdk.UserActivityInsightsReport{
					StartTime:   startTime,
					EndTime:     endTime,
					TemplateIDs: []uuid.UUID{},
					Users:       []optimus-ide-collabsdk.UserActivity{},
				},
			})
			return
		}
	...
	userActivities := make([]optimus-ide-collabsdk.UserActivity, 0, len(rows))
	for _, row := range rows {
		for _, templateID := range row.TemplateIDs {
			templateIDSet[templateID] = struct{}{}
		}
		userActivities = append(userActivities, optimus-ide-collabsdk.UserActivity{
	...
	resp := optimus-ide-collabsdk.UserActivityInsightsResponse{
		Report: optimus-ide-collabsdk.UserActivityInsightsReport{
			StartTime:   startTime,
			EndTime:     endTime,
...
// @Param end_time query string true "End time" format(date-time)
// @Param template_ids query []string false "Template IDs" collectionFormat(csv)
// @Success 200 {object} optimus-ide-collabsdk.UserLatencyInsightsResponse
...
func (api *API) insightsUserLatency(rw http.ResponseWriter, r *http.Request) {
	...
	templateIDSet := make(map[uuid.UUID]struct{})
	userLatencies := make([]optimus-ide-collabsdk.UserLatency, 0, len(rows))
	for _, row := range rows {
		for _, templateID := range row.TemplateIDs {
			templateIDSet[templateID] = struct{}{}
		}
		userLatencies = append(userLatencies, optimus-ide-collabsdk.UserLatency{
			TemplateIDs: row.TemplateIDs,
			UserID:      row.UserID,
			Username:    row.Username,
			AvatarURL:   row.AvatarURL,
			LatencyMS: optimus-ide-collabsdk.ConnectionLatency{
	...
	resp := optimus-ide-collabsdk.UserLatencyInsightsResponse{
		Report: optimus-ide-collabsdk.UserLatencyInsightsReport{
			StartTime:   startTime,
			EndTime:     endTime,
			TemplateIDs: seenTemplateIDs,
			Users:       userLatencies,
...
// @Success 200 {object} optimus-ide-collabsdk.GetUserStatusCountsResponse
...
func (api *API) insightsUserStatusCounts(rw http.ResponseWriter, r *http.Request) {
	...
	resp := optimus-ide-collabsdk.GetUserStatusCountsResponse{
		StatusCounts: make(map[optimus-ide-collabsdk.UserStatus][]optimus-ide-collabsdk.UserStatusChangeCount),
	}

	for _, row := range rows {
		status := optimus-ide-collabsdk.UserStatus(row.Status)
		resp.StatusCounts[status] = append(resp.StatusCounts[status], optimus-ide-collabsdk.UserStatusChangeCount{
			Date:  row.Date,
			Count: row.Count,
		})
	}
...
// @Param template_ids query []string false "Template IDs" collectionFormat(csv)
// @Success 200 {object} optimus-ide-collabsdk.TemplateInsightsResponse
...
func (api *API) insightsTemplates(rw http.ResponseWriter, r *http.Request) {
	...
	var (
		...
		templateIDs     = p.UUIDs(vals, []uuid.UUID{}, "template_ids")
		sectionStrings  = p.Strings(vals, templateInsightsSectionAsStrings(optimus-ide-collabsdk.TemplateInsightsSectionIntervalReports, optimus-ide-collabsdk.TemplateInsightsSectionReport), "sections")
	)
	p.ErrorExcessParams(vals)
	if len(p.Errors) > 0 {
		httpapi.Write(ctx, rw, http.StatusBadRequest, optimus-ide-collabsdk.Response{
	...
	eg.Go(func() error {
		var err error
		if interval != "" && slices.Contains(sections, optimus-ide-collabsdk.TemplateInsightsSectionIntervalReports) {
	...
	eg.Go(func() error {
		if !slices.Contains(sections, optimus-ide-collabsdk.TemplateInsightsSectionReport) {
			return nil
		}
	...
	resp := optimus-ide-collabsdk.TemplateInsightsResponse{
		IntervalReports: []optimus-ide-collabsdk.TemplateInsightsIntervalReport{},
	}

	if slices.Contains(sections, optimus-ide-collabsdk.TemplateInsightsSectionReport) {
		resp.Report = &optimus-ide-collabsdk.TemplateInsightsReport{
			StartTime:       startTime,
			EndTime:         endTime,
	...
	for _, row := range dailyUsage {
		resp.IntervalReports = append(resp.IntervalReports, optimus-ide-collabsdk.TemplateInsightsIntervalReport{
...
// templates.
func convertTemplateInsightsApps(usage database.GetTemplateInsightsRow, appUsage []database.GetTemplateAppInsightsRow) []optimus-ide-collabsdk.TemplateAppUsage {
	// Builtin apps.
	apps := []optimus-ide-collabsdk.TemplateAppUsage{
		{
			TemplateIDs: usage.VscodeTemplateIds,
			Type:        optimus-ide-collabsdk.TemplateAppsTypeBuiltin,
			DisplayName: optimus-ide-collabsdk.TemplateBuiltinAppDisplayNameVSCode,
			Slug:        "vscode",
			Icon:        "/icon/code.svg",
		...
		},
		{
			...
			Type:        optimus-ide-collabsdk.TemplateAppsTypeBuiltin,
			DisplayName: optimus-ide-collabsdk.TemplateBuiltinAppDisplayNameJetBrains,
			Slug:        "jetbrains",
		...
		// !app.IsApp && app.AccessMethod == "terminal" && app.SlugOrPort == ""
		{
			...
			Type:        optimus-ide-collabsdk.TemplateAppsTypeBuiltin,
			DisplayName: optimus-ide-collabsdk.TemplateBuiltinAppDisplayNameWebTerminal,
		...
		},
		{
			...
			Type:        optimus-ide-collabsdk.TemplateAppsTypeBuiltin,
			DisplayName: optimus-ide-collabsdk.TemplateBuiltinAppDisplayNameSSH,
			Slug:        "ssh",
			Icon:        "/icon/terminal.svg",
		...
		},
		{
			...
			Type:        optimus-ide-collabsdk.TemplateAppsTypeBuiltin,
			DisplayName: optimus-ide-collabsdk.TemplateBuiltinAppDisplayNameSFTP,
			Slug:        "sftp",
			Icon:        "/icon/terminal.svg",
	...
	for _, app := range appUsage {
		apps = append(apps, optimus-ide-collabsdk.TemplateAppUsage{
			TemplateIDs: app.TemplateIDs,
			Type:        optimus-ide-collabsdk.TemplateAppsTypeApp,
...
func parseInsightsInterval(ctx context.Context, rw http.ResponseWriter, intervalString string, startTime, endTime time.Time) (optimus-ide-collabsdk.InsightsReportInterval, bool) {
	switch v := optimus-ide-collabsdk.InsightsReportInterval(intervalString); v {
	case optimus-ide-collabsdk.InsightsReportIntervalDay, "":
		return v, true
	case optimus-ide-collabsdk.InsightsReportIntervalWeek:
	...
	default:
		httpapi.Write(ctx, rw, http.StatusBadRequest, optimus-ide-collabsdk.Response{
			...
			Validations: []optimus-ide-collabsdk.ValidationError{
				{
					Field:  "interval",
					Detail: fmt.Sprintf("must be one of %v", []optimus-ide-collabsdk.InsightsReportInterval{optimus-ide-collabsdk.InsightsReportIntervalDay, optimus-ide-collabsdk.InsightsReportIntervalWeek}),
				},
			},
		})
		return "", false
...
func templateInsightsSectionAsStrings(sections ...optimus-ide-collabsdk.TemplateInsightsSection) []string {
...
func parseTemplateInsightsSections(ctx context.Context, rw http.ResponseWriter, sections []string) ([]optimus-ide-collabsdk.TemplateInsightsSection, bool) {
	t := make([]optimus-ide-collabsdk.TemplateInsightsSection, len(sections))
	for i, s := range sections {
		switch v := optimus-ide-collabsdk.TemplateInsightsSection(s); v {
		case optimus-ide-collabsdk.TemplateInsightsSectionIntervalReports, optimus-ide-collabsdk.TemplateInsightsSectionReport:
			t[i] = v
		default:
			httpapi.Write(ctx, rw, http.StatusBadRequest, optimus-ide-collabsdk.Response{
				Message: "Query parameter has invalid value.",
				Validations: []optimus-ide-collabsdk.ValidationError{
					{
						Field:  "sections",
						Detail: fmt.Sprintf("must be one of %v", []optimus-ide-collabsdk.TemplateInsightsSection{optimus-ide-collabsdk.TemplateInsightsSectionIntervalReports, optimus-ide-collabsdk.TemplateInsightsSectionReport}),
					},
				},
			})
			return nil, false
