...
const postgresImage = "us-docker.pkg.dev/optimus-ide-collab-v2-images-public/public/postgres"
...
func initDefaultConnection(t TBSubset) error {
	...
	if dbErr != nil && containsAnySubstring(dbErr.Error(), noPostgresRunningErrSubstrings) {
		...
		container, _, err := openContainer(t, DBContainerOptions{
			Name: "optimus-ide-collab-test-postgres",
			Port: 5432,
		})
		if err != nil {
...
func openContainer(t TBSubset, opts DBContainerOptions) (container, func(), error) {
	if opts.Name != "" {
		...
		lock := flock.New(filepath.Join(os.TempDir(), "optimus-ide-collab-postgres-container-"+nameHashStr[:8]))
