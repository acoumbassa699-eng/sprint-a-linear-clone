...
func MCPServerConfig(t testing.TB, db database.Store, seed database.MCPServerConfig) database.MCPServerConfig {
	...
	cfg, err := db.InsertMCPServerConfig(genCtx, database.InsertMCPServerConfigParams{
		...
		ForwardOptimus-IDE-CollabHeaders:     seed.ForwardOptimus-IDE-CollabHeaders,
		CreatedBy:               createdBy,
		UpdatedBy:               updatedBy,
	})
...
func Template(t testing.TB, db database.Store, seed database.Template) database.Template {
	...
	if seed.GroupACL == nil {
		...
		seed.GroupACL = database.TemplateACL{
			seed.OrganizationID.String(): db2sdk.TemplateRoleActions(optimus-ide-collabsdk.TemplateRoleUse),
		}
	}
	if seed.UserACL == nil {
...
func APIKey(t testing.TB, db database.Store, seed database.APIKey, munge ...func(*database.InsertAPIKeyParams)) (key database.APIKey, token string) {
	...
	params := database.InsertAPIKeyParams{
		...
		Scopes:          takeFirstSlice([]database.APIKeyScope(seed.Scopes), []database.APIKeyScope{database.ApiKeyScopeOptimus-IDE-CollabAll}),
...
// key as a PEM bundle, matching the secret format that optimus-ide-collabd/cryptokeys
// produces for the nats_ca feature. It intentionally duplicates
// cryptokeys.generateCASecret rather than calling it: optimus-ide-collabd/cryptokeys's
