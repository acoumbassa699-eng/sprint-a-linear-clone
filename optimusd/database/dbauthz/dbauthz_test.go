...
func TestAsNoActor(t *testing.T) {
	...
	t.Run("AsActor", func(t *testing.T) {
		t.Parallel()
		ctx := dbauthz.As(context.Background(), optimus-ide-collabdtest.RandomRBACSubject())
...
func TestPing(t *testing.T) {
	...
	q := dbauthz.New(db, &optimus-ide-collabdtest.RecordingAuthorizer{}, slog.Make(), optimus-ide-collabdtest.AccessControlStorePointer())
...
func TestInTX(t *testing.T) {
	...
	q := dbauthz.New(db, &optimus-ide-collabdtest.RecordingAuthorizer{
		Wrapped: (&optimus-ide-collabdtest.FakeAuthorizer{}).AlwaysReturn(xerrors.New("custom error")),
	}, slog.Make(), optimus-ide-collabdtest.AccessControlStorePointer())
...
func TestNew(t *testing.T) {
	...
	// should be made.
	az := dbauthz.New(db, rec, slog.Make(), optimus-ide-collabdtest.AccessControlStorePointer())
	az = dbauthz.New(az, rec, slog.Make(), optimus-ide-collabdtest.AccessControlStorePointer())
...
func TestChatFilesAllowLinkedChatReads(t *testing.T) {
	...
	t.Run("GetChatFileByID", func(t *testing.T) {
		...
		q := dbauthz.New(db, authorizer, slogtest.Make(t, nil), optimus-ide-collabdtest.AccessControlStorePointer())
...
func TestUpdateChatACLByIDGuards(t *testing.T) {
	...
	t.Run("Disabled", func(t *testing.T) { //nolint:paralleltest // It toggles the global chat ACL flag.
		...
		q := dbauthz.New(db, &optimus-ide-collabdtest.FakeAuthorizer{}, slogtest.Make(t, nil), optimus-ide-collabdtest.AccessControlStorePointer())
...
func (s *MethodTestSuite) TestAPIKey() {
	...
	s.Run("InsertAPIKey", s.Mocked(func(dbm *dbmock.MockStore, faker *gofakeit.Faker, check *expects) {
		...
		arg := database.InsertAPIKeyParams{UserID: u.ID, LoginType: database.LoginTypePassword, Scopes: database.APIKeyScopes{database.ApiKeyScopeOptimus-IDE-CollabAll}, IPAddress: defaultIPAddress()}
	...
	s.Run("DeleteApplicationConnectAPIKeysByUserID", s.Mocked(func(dbm *dbmock.MockStore, faker *gofakeit.Faker, check *expects) {
		a := testutil.Fake(s.T(), faker, database.APIKey{Scopes: database.APIKeyScopes{database.ApiKeyScopeOptimus-IDE-CollabApplicationConnect}})
...
func (s *MethodTestSuite) TestOrganization() {
	...
	s.Run("InsertOrganizationMember", s.Mocked(func(dbm *dbmock.MockStore, faker *gofakeit.Faker, check *expects) {
		...
		u := testutil.Fake(s.T(), faker, database.User{})
		arg := database.InsertOrganizationMemberParams{OrganizationID: o.ID, UserID: u.ID, Roles: []string{optimus-ide-collabsdk.RoleOrganizationAdmin}}
		dbm.EXPECT().GetOrganizationByID(gomock.Any(), o.ID).Return(o, nil).AnyTimes()
	...
	s.Run("UpdateOrganization", s.Mocked(func(dbm *dbmock.MockStore, faker *gofakeit.Faker, check *expects) {
		...
		arg := database.UpdateOrganizationParams{ID: o.ID, Name: "something-different", DefaultOrgMemberRoles: []string{optimus-ide-collabsdk.RoleOrganizationAdmin}}
	...
	s.Run("UpdateMemberRoles", s.Mocked(func(dbm *dbmock.MockStore, faker *gofakeit.Faker, check *expects) {
		o := testutil.Fake(s.T(), faker, database.Organization{DefaultOrgMemberRoles: []string{}})
		u := testutil.Fake(s.T(), faker, database.User{})
		mem := testutil.Fake(s.T(), faker, database.OrganizationMember{OrganizationID: o.ID, UserID: u.ID, Roles: []string{optimus-ide-collabsdk.RoleOrganizationAdmin}})
		out := mem
		out.Roles = []string{}
...
func (s *MethodTestSuite) TestUser() {
	...
	s.Run("ListUserChatCompactionThresholds", s.Mocked(func(dbm *dbmock.MockStore, faker *gofakeit.Faker, check *expects) {
		u := testutil.Fake(s.T(), faker, database.User{})
		uc := database.UserConfig{UserID: u.ID, Key: optimus-ide-collabsdk.ChatCompactionThresholdKeyPrefix + "00000000-0000-0000-0000-000000000001", Value: "75"}
		dbm.EXPECT().GetUserByID(gomock.Any(), u.ID).Return(u, nil).AnyTimes()
	...
	s.Run("GetUserChatCompactionThreshold", s.Mocked(func(dbm *dbmock.MockStore, faker *gofakeit.Faker, check *expects) {
		u := testutil.Fake(s.T(), faker, database.User{})
		arg := database.GetUserChatCompactionThresholdParams{UserID: u.ID, Key: optimus-ide-collabsdk.ChatCompactionThresholdKeyPrefix + "00000000-0000-0000-0000-000000000001"}
		dbm.EXPECT().GetUserByID(gomock.Any(), u.ID).Return(u, nil).AnyTimes()
	...
	s.Run("DeleteUserChatCompactionThreshold", s.Mocked(func(dbm *dbmock.MockStore, faker *gofakeit.Faker, check *expects) {
		u := testutil.Fake(s.T(), faker, database.User{})
		arg := database.DeleteUserChatCompactionThresholdParams{UserID: u.ID, Key: optimus-ide-collabsdk.ChatCompactionThresholdKeyPrefix + "00000000-0000-0000-0000-000000000001"}
		dbm.EXPECT().GetUserByID(gomock.Any(), u.ID).Return(u, nil).AnyTimes()
	...
	s.Run("UpdateUserRoles", s.Mocked(func(dbm *dbmock.MockStore, faker *gofakeit.Faker, check *expects) {
		u := testutil.Fake(s.T(), faker, database.User{RBACRoles: []string{optimus-ide-collabsdk.RoleTemplateAdmin}})
		o := u
		o.RBACRoles = []string{optimus-ide-collabsdk.RoleUserAdmin}
		arg := database.UpdateUserRolesParams{GrantedRoles: []string{optimus-ide-collabsdk.RoleUserAdmin}, ID: u.ID}
		dbm.EXPECT().GetUserByID(gomock.Any(), u.ID).Return(u, nil).AnyTimes()
	...
	s.Run("SitePermissions/UpdateCustomRole", s.Mocked(func(_ *dbmock.MockStore, _ *gofakeit.Faker, check *expects) {
		arg := database.UpdateCustomRoleParams{
			...
			SitePermissions: slice.List(optimus-ide-collabsdk.CreatePermissions(map[optimus-ide-collabsdk.RBACResource][]optimus-ide-collabsdk.RBACAction{
				optimus-ide-collabsdk.ResourceTemplate: {optimus-ide-collabsdk.ActionCreate, optimus-ide-collabsdk.ActionRead, optimus-ide-collabsdk.ActionUpdate, optimus-ide-collabsdk.ActionDelete, optimus-ide-collabsdk.ActionViewInsights},
			}), convertSDKPerm),
			OrgPermissions: nil,
			UserPermissions: slice.List(optimus-ide-collabsdk.CreatePermissions(map[optimus-ide-collabsdk.RBACResource][]optimus-ide-collabsdk.RBACAction{
	...
	s.Run("OrgPermissions/UpdateCustomRole", s.Mocked(func(dbm *dbmock.MockStore, _ *gofakeit.Faker, check *expects) {
		...
		arg := database.UpdateCustomRoleParams{
			...
			OrgPermissions: slice.List(optimus-ide-collabsdk.CreatePermissions(map[optimus-ide-collabsdk.RBACResource][]optimus-ide-collabsdk.RBACAction{
				optimus-ide-collabsdk.ResourceTemplate: {optimus-ide-collabsdk.ActionCreate, optimus-ide-collabsdk.ActionRead},
			}), convertSDKPerm),
		}
		dbm.EXPECT().UpdateCustomRole(gomock.Any(), arg).Return(database.CustomRole{}, nil).AnyTimes()
		check.Args(arg).Asserts(
...
func TestGetLatestWorkspaceBuildByWorkspaceID_FastPath(t *testing.T) {
	...
	authorizer := &optimus-ide-collabdtest.RecordingAuthorizer{
		Wrapped: (&optimus-ide-collabdtest.FakeAuthorizer{}).AlwaysReturn(nil),
	}
	...
	t.Run("WithWorkspaceRBAC", func(t *testing.T) {
		...
		q := dbauthz.New(dbm, authorizer, slogtest.Make(t, nil), optimus-ide-collabdtest.AccessControlStorePointer())
...
func TestGetWorkspaceAgentByID_FastPath(t *testing.T) {
	...
	t.Run("WithWorkspaceRBAC", func(t *testing.T) {
		...
		q := dbauthz.New(mockDB, authorizer, slogtest.Make(t, nil), optimus-ide-collabdtest.AccessControlStorePointer())
