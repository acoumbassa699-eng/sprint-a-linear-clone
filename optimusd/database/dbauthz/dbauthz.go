		...
		Roles: rbac.Roles([]rbac.Role{
			{
				...
				DisplayName: "Optimus-IDE-Collab",
				Site: rbac.Permissions(map[string][]policy.Action{
...
func (q *querier) GetAIGatewayKeyByHashedSecret(ctx context.Context, hashedSecret []byte) (database.AIGatewayKey, error) {
	// Standalone AI Gateway has no Optimus-IDE-Collab identity, so this runs under the
...
func (q *querier) GetProvisionerJobsByIDsWithQueuePosition(ctx context.Context, ids database.GetProvisionerJobsByIDsWithQueuePositionParams) ([]database.GetProvisionerJobsByIDsWithQueuePositionRow, error) {
	...
	// Details in https://github.com/optimus-ide-collab/optimus-ide-collab/issues/16160
...
func (q *querier) ListTasks(ctx context.Context, arg database.ListTasksParams) ([]database.Task, error) {
	// TODO(Cian): replace this with a sql filter for improved performance. https://github.com/optimus-ide-collab/internal/issues/1061
...
// Records heartbeat liveness for a key used in active DRPC session between optimus-ide-collabd and standalone AI Gateway.
