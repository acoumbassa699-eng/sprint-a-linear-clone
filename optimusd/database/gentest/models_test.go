...
// TestAuditDBEnumsCovered ensures that all enums in the database are covered by the optimus-ide-collabsdk enums
...
func TestAuditDBEnumsCovered(t *testing.T) {
	...
	for _, ty := range dbTypes {
		str := optimus-ide-collabsdk.ResourceType(ty).FriendlyString()
		require.NotEqualf(t, "unknown", str, "ResourceType %q not covered by optimus-ide-collabsdk.ResourceType", ty)
	}
}
