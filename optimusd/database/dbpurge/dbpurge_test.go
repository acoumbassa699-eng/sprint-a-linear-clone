...
func TestPurge(t *testing.T) {
	...
	mDB.EXPECT().GetChatDebugRetentionDays(gomock.Any(), optimus-ide-collabsdk.DefaultChatDebugRetentionDays).Return(int32(0), nil).AnyTimes()
	mDB.EXPECT().InTx(gomock.Any(), database.DefaultTXOptions().WithID("db_purge")).Return(nil).Times(2)
	purger := dbpurge.New(context.Background(), testutil.Logger(t), mDB, &optimus-ide-collabsdk.DeploymentValues{}, prometheus.NewRegistry(), dbpurge.WithClock(clk))
...
func TestMetrics(t *testing.T) {
	t.Run("SuccessfulIteration", func(t *testing.T) {
		...
		closer := dbpurge.New(ctx, logger, db, &optimus-ide-collabsdk.DeploymentValues{
			Retention: optimus-ide-collabsdk.RetentionConfig{
		...
		hist := promhelp.HistogramValue(t, reg, "optimus-ide-collabd_dbpurge_iteration_duration_seconds", prometheus.Labels{
			"success": "true",
		})
		require.NotNil(t, hist)
		require.Greater(t, hist.GetSampleCount(), uint64(0), "should have at least one sample")

		expiredAPIKeys := promhelp.CounterValue(t, reg, "optimus-ide-collabd_dbpurge_records_purged_total", prometheus.Labels{
		...
		workspaceAgentLogs := promhelp.CounterValue(t, reg, "optimus-ide-collabd_dbpurge_records_purged_total", prometheus.Labels{
			"record_type": "workspace_agent_logs",
		})
		require.GreaterOrEqual(t, workspaceAgentLogs, 0)

		aibridgeRecords := promhelp.CounterValue(t, reg, "optimus-ide-collabd_dbpurge_records_purged_total", prometheus.Labels{
			"record_type": "aibridge_records",
		})
		require.GreaterOrEqual(t, aibridgeRecords, 0)

		connectionLogs := promhelp.CounterValue(t, reg, "optimus-ide-collabd_dbpurge_records_purged_total", prometheus.Labels{
			"record_type": "connection_logs",
		})
		require.GreaterOrEqual(t, connectionLogs, 0)

		auditLogs := promhelp.CounterValue(t, reg, "optimus-ide-collabd_dbpurge_records_purged_total", prometheus.Labels{
			"record_type": "audit_logs",
		})
		require.GreaterOrEqual(t, auditLogs, 0)

		workspaceBuildOrchestrations := promhelp.CounterValue(t, reg, "optimus-ide-collabd_dbpurge_records_purged_total", prometheus.Labels{
			"record_type": "workspace_build_orchestrations",
		})
		require.GreaterOrEqual(t, workspaceBuildOrchestrations, 0)

		chats := promhelp.CounterValue(t, reg, "optimus-ide-collabd_dbpurge_records_purged_total", prometheus.Labels{
			"record_type": "chats",
		})
		require.GreaterOrEqual(t, chats, 0)

		chatDebugRuns := promhelp.CounterValue(t, reg, "optimus-ide-collabd_dbpurge_records_purged_total", prometheus.Labels{
			"record_type": "chat_debug_runs",
		})
		require.GreaterOrEqual(t, chatDebugRuns, 0)

		chatFiles := promhelp.CounterValue(t, reg, "optimus-ide-collabd_dbpurge_records_purged_total", prometheus.Labels{
	...
	t.Run("LockNotAcquiredSkipsIterationMetric", func(t *testing.T) {
		...
		mDB.EXPECT().GetChatRetentionDays(gomock.Any()).Return(int32(0), nil).AnyTimes()
		mDB.EXPECT().GetChatDebugRetentionDays(gomock.Any(), optimus-ide-collabsdk.DefaultChatDebugRetentionDays).
		...
		closer := dbpurge.New(ctx, logger, mDB, &optimus-ide-collabsdk.DeploymentValues{}, reg, dbpurge.WithClock(clk))
		defer closer.Close()
		testutil.TryReceive(ctx, t, done)

		successHist := promhelp.MetricValue(t, reg, "optimus-ide-collabd_dbpurge_iteration_duration_seconds", prometheus.Labels{
			"success": "true",
		})
		require.Nil(t, successHist, "lock contention should not record a successful purge iteration")

		failedHist := promhelp.MetricValue(t, reg, "optimus-ide-collabd_dbpurge_iteration_duration_seconds", prometheus.Labels{
			"success": "false",
		})
...
func TestDeleteOldWorkspaceAgentStats(t *testing.T) {
	...
	// when
	closer := dbpurge.New(ctx, logger, db, &optimus-ide-collabsdk.DeploymentValues{}, prometheus.NewRegistry(), dbpurge.WithClock(clk))
	defer closer.Close()
...
func TestDeleteOldWorkspaceAgentLogsRetention(t *testing.T) {
	...
	testCases := []struct {
		name            string
		retentionConfig optimus-ide-collabsdk.RetentionConfig
...
func TestDeleteOldAIBridgeRecords(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			closer := dbpurge.New(ctx, logger, db, &optimus-ide-collabsdk.DeploymentValues{
				AI: optimus-ide-collabsdk.AIConfig{
	...
	createDebugRunWithStep := func(ctx context.Context, t *testing.T, db database.Store, chatID uuid.UUID, updatedAt time.Time, finished bool) database.ChatDebugRun {
		...
		if finished {
			run, err = db.UpdateChatDebugRun(ctx, database.UpdateChatDebugRunParams{
				Status:     sql.NullString{String: string(optimus-ide-collabsdk.ChatDebugStatusCompleted), Valid: true},
				FinishedAt: sql.NullTime{Time: updatedAt, Valid: true},
				Now:        updatedAt,
				ID:         run.ID,
				ChatID:     run.ChatID,
	...
	tests := []struct {
	...
	}{
		{
			...
			run: func(t *testing.T) {
				...
				closer := dbpurge.New(ctx, logger, db, &optimus-ide-collabsdk.DeploymentValues{}, reg, dbpurge.WithClock(clk))
				defer closer.Close()
				testutil.TryReceive(ctx, t, done)
...
func TestBackfillChatMessagesSearchTsv(t *testing.T) {
	...
	t.Run("BackfillsNewestFirst", func(t *testing.T) {
		...
		tick := awaitDoTicks(ctx, t, clk, 1)
		closer := dbpurge.New(ctx, logger, db, &optimus-ide-collabsdk.DeploymentValues{}, prometheus.NewRegistry(),
	...
	t.Run("SteadyStateNoop", func(t *testing.T) {
		...
		require.Zero(t, countPending(ctx, t, rawDB))
		backfilled := promhelp.CounterValue(t, reg, "optimus-ide-collabd_dbpurge_chat_search_rows_backfilled_total", nil)
	...
	t.Run("SkippedWhenLockHeld", func(t *testing.T) {
		...
		closer := dbpurge.New(ctx, logger, mDB, &optimus-ide-collabsdk.DeploymentValues{}, prometheus.NewRegistry(), dbpurge.WithClock(clk))
		defer closer.Close()
		testutil.TryReceive(ctx, t, done)
	})
}
