...
// This is also in optimus-ide-collabsdk/workspaceagents.go and should be kept in sync.
const (
...
func (s APIKeyScope) ToRBAC() rbac.ScopeName {
	switch s {
	case ApiKeyScopeOptimus-IDE-CollabAll:
		return rbac.ScopeAll
	case ApiKeyScopeOptimus-IDE-CollabApplicationConnect:
...
func (s APIKeyScopes) Name() rbac.RoleIdentifier {
	if len(s) == 0 {
		...
		return rbac.RoleIdentifier{Name: string(ApiKeyScopeOptimus-IDE-CollabAll)}
	}
