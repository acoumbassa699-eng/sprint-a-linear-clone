...
func TestProvisionerJobStatus(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			status: optimus-ide-collabsdk.ProvisionerJobCanceling,
		},
		{
			name: "canceled",
			job: database.ProvisionerJob{
			...
			status: optimus-ide-collabsdk.ProvisionerJobCanceled,
		},
		{
			...
			status: optimus-ide-collabsdk.ProvisionerJobFailed,
		},
		{
			name:   "pending",
			job:    database.ProvisionerJob{},
		...
		},
		{
			...
			status: optimus-ide-collabsdk.ProvisionerJobSucceeded,
		},
		{
	...
	for i, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			actual := optimus-ide-collabsdk.ProvisionerJobStatus(inserted.JobStatus)
...
func TestChatDebugRunSummary(t *testing.T) {
	...
	require.Equal(t, optimus-ide-collabsdk.ChatDebugRunKindChatTurn, sdk.Kind)
	require.Equal(t, optimus-ide-collabsdk.ChatDebugStatusCompleted, sdk.Status)
...
func TestChatDebugStep(t *testing.T) {
	...
	require.Equal(t, optimus-ide-collabsdk.ChatDebugStepOperationStream, sdk.Operation)
	require.Equal(t, optimus-ide-collabsdk.ChatDebugStatusCompleted, sdk.Status)
	require.Equal(t, startedAt, sdk.StartedAt)
	require.Equal(t, finishedAt, sdk.UpdatedAt)
	require.Equal(t, &finishedAt, sdk.FinishedAt)
...
func TestChatDebugRunDetail(t *testing.T) {
	...
	require.Equal(t, optimus-ide-collabsdk.ChatDebugStepOperationStream, sdk.Steps[0].Operation)
	require.Equal(t, steps[1].ID, sdk.Steps[1].ID)
	require.Equal(t, optimus-ide-collabsdk.ChatDebugStepOperationGenerate, sdk.Steps[1].Operation)
}
...
func TestChatMessage_PreservesProviderExecutedOnToolResults(t *testing.T) {
	...
	require.Equal(t, optimus-ide-collabsdk.ChatMessagePartTypeToolCall, result.Content[0].Type)
	...
	require.Equal(t, optimus-ide-collabsdk.ChatMessagePartTypeToolResult, result.Content[1].Type)
...
func TestChatQueuedMessage_ParsesUserContentParts(t *testing.T) {
	...
	rawContent, err := json.Marshal([]optimus-ide-collabsdk.ChatMessagePart{
		optimus-ide-collabsdk.ChatMessageText("queued text"),
	})
	require.NoError(t, err)
	...
	require.Equal(t, optimus-ide-collabsdk.ChatMessagePartTypeText, queued.Content[0].Type)
...
func TestChat_AllFieldsPopulated(t *testing.T) {
	...
	// field to optimus-ide-collabsdk.Chat, this test will fail until the
	// converter is updated.
	now := dbtime.Now()
	lastErrorPayload := optimus-ide-collabsdk.ChatError{
		Message:    "boom",
	...
	input := database.Chat{
		...
		// Pinned-context columns drive optimus-ide-collabsdk.Chat.Context. Set all of
	...
	for i := range typ.NumField() {
		...
		require.False(t, v.Field(i).IsZero(),
			"optimus-ide-collabsdk.Chat field %q is zero-valued — db2sdk.Chat may not be populating it",
			field.Name,
		)
	}
}
...
func TestChat_LastErrorFallback(t *testing.T) {
	...
	tests := []struct {
		name          string
		raw           json.RawMessage
		expectPayload *optimus-ide-collabsdk.ChatError
	}{
		{
			name: "MalformedJSON",
			raw:  json.RawMessage(`{`),
