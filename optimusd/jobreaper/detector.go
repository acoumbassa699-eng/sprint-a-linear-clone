...
func JobLogMessages(reapType ReapType, threshold time.Duration) []string {
	return []string{
		"",
		"====================",
		fmt.Sprintf("Optimus-IDE-Collab: Build has been detected as %s for %.0f minutes and will be terminated.", reapType, threshold.Minutes()),
		"====================",
		"",
	}
}
	...
	err := db.InTx(func(db database.Store) error {
		...
		err = db.UpdateProvisionerJobWithCompleteWithStartedAtByID(ctx, database.UpdateProvisionerJobWithCompleteWithStartedAtByIDParams{
			...
			Error: sql.NullString{
				String: fmt.Sprintf("Optimus-IDE-Collab: Build has been detected as %s for %.0f minutes and has been terminated by the reaper.", jobToReap.Type, jobToReap.Threshold.Minutes()),
				Valid:  true,
			},
