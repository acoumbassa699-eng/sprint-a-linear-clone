...
// @Produce json
// @Tags Authorization
// @Param request body optimus-ide-collabsdk.AuthorizationRequest true "Authorization request"
// @Success 200 {object} optimus-ide-collabsdk.AuthorizationResponse
...
func (api *API) checkAuthorization(rw http.ResponseWriter, r *http.Request) {
	...
	var params optimus-ide-collabsdk.AuthorizationRequest
	...
	response := make(optimus-ide-collabsdk.AuthorizationResponse)
	...
	for k, v := range params.Checks {
		...
		if obj.Owner == optimus-ide-collabsdk.Me {
			obj.Owner = auth.ID
		}
		...
		if v.Object.ResourceID != "" {
			...
			if err != nil {
				httpapi.Write(ctx, rw, http.StatusBadRequest, optimus-ide-collabsdk.Response{
					...
					Validations: []optimus-ide-collabsdk.ValidationError{{Field: "resource_id", Detail: err.Error()}},
				})
				return
			}
			...
			default:
				...
				httpapi.Write(ctx, rw, http.StatusBadRequest, optimus-ide-collabsdk.Response{
					Message:     msg,
					Validations: []optimus-ide-collabsdk.ValidationError{{Field: "resource_type", Detail: msg}},
				})
				return
			}
			if dbErr != nil {
