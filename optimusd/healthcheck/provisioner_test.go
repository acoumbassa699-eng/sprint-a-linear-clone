...
func TestProvisionerDaemonReport(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		},
		{
			...
			expectedItems: []healthsdk.ProvisionerDaemonsReportItem{
				{
					ProvisionerDaemon: optimus-ide-collabsdk.ProvisionerDaemon{
						ID:           uuid.Nil,
						Name:         "pd-ok",
						CreatedAt:    now,
						LastSeenAt:   optimus-ide-collabsdk.NewNullTime(now, true),
						Version:      "v1.2.3",
						APIVersion:   "1.0",
						Provisioners: []optimus-ide-collabsdk.ProvisionerType{optimus-ide-collabsdk.ProvisionerTypeEcho, optimus-ide-collabsdk.ProvisionerTypeTerraform},
