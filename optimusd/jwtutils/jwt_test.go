	...
	cases := []testcase{
		{
			name: "OK",
			claims: jwt.Claims{
				Issuer:    "optimus-ide-collab",
				Subject:   "user@optimus-ide-collab.com",
				Audience:  jwt.Audience{"optimus-ide-collab"},
		...
		},
		{
			...
			expectedClaims: jwt.Expected{
				Subject: "user2@optimus-ide-collab.com",
			},
...
func TestJWS(t *testing.T) {
	...
	t.Run("WithKeycache", func(t *testing.T) {
		...
		cache, err := cryptokeys.NewSigningCache(ctx, log, fetcher, optimus-ide-collabsdk.CryptoKeyFeatureOIDCConvert)
		require.NoError(t, err)
...
func TestJWE(t *testing.T) {
	...
	t.Run("WithKeycache", func(t *testing.T) {
		...
		cache, err := cryptokeys.NewEncryptionCache(ctx, log, fetcher, optimus-ide-collabsdk.CryptoKeyFeatureWorkspaceAppsAPIKey)
		require.NoError(t, err)
