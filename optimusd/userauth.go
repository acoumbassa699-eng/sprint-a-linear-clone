...
const (
	...
	OAuthConvertCookieValue = "optimus-ide-collab_oauth_convert_jwt"
...
type OAuthConvertStateClaims struct {
	...
	FromLoginType optimus-ide-collabsdk.LoginType `json:"from_login_type"`
	ToLoginType   optimus-ide-collabsdk.LoginType `json:"to_login_type"`
}
...
// @Produce json
// @Tags Authorization
// @Param request body optimus-ide-collabsdk.ConvertLoginRequest true "Convert request"
// @Param user path string true "User ID, name, or me"
// @Success 201 {object} optimus-ide-collabsdk.OAuthConversionResponse
...
func (api *API) postConvertLoginType(rw http.ResponseWriter, r *http.Request) {
	...
	var req optimus-ide-collabsdk.ConvertLoginRequest
	if !httpapi.Read(ctx, rw, r, &req) {
		return
	}

	switch req.ToType {
	case optimus-ide-collabsdk.LoginTypeGithub, optimus-ide-collabsdk.LoginTypeOIDC:
		// Allowed!
	case optimus-ide-collabsdk.LoginTypeNone, optimus-ide-collabsdk.LoginTypePassword, optimus-ide-collabsdk.LoginTypeToken:
	...
	user, _, ok := api.loginRequest(ctx, rw, optimus-ide-collabsdk.LoginWithPasswordRequest{
		Email:    user.Email,
	...
	// When the user does the oauth flow, this jwt will be sent back to optimus-ide-collabd.
	// The included information in this payload links it to a state string, so
	// this request is tied 1:1 with an oauth state.
	// This JWT also includes information to tie it 1:1 with a optimus-ide-collab deployment
	// and user account. This is mainly to inform the user if they are accidentally
	// switching between optimus-ide-collab deployments if the OIDC is misconfigured.
	...
	claims := &OAuthConvertStateClaims{
		...
		UserID:        user.ID,
		State:         stateString,
		FromLoginType: optimus-ide-collabsdk.LoginType(user.LoginType),
	...
	httpapi.Write(ctx, rw, http.StatusCreated, optimus-ide-collabsdk.OAuthConversionResponse{
...
// @Accept json
// @Tags Authorization
// @Param request body optimus-ide-collabsdk.RequestOneTimePasscodeRequest true "One-time passcode request"
// @Success 204
...
func (api *API) postRequestOneTimePasscode(rw http.ResponseWriter, r *http.Request) {
	...
	var req optimus-ide-collabsdk.RequestOneTimePasscodeRequest
	if !httpapi.Read(ctx, rw, r, &req) {
		return
	}
...
// @Accept json
// @Tags Authorization
// @Param request body optimus-ide-collabsdk.ChangePasswordWithOneTimePasscodeRequest true "Change password request"
// @Success 204
...
func (api *API) postChangePasswordWithOneTimePasscode(rw http.ResponseWriter, r *http.Request) {
	...
	var req optimus-ide-collabsdk.ChangePasswordWithOneTimePasscodeRequest
	if !httpapi.Read(ctx, rw, r, &req) {
		return
	}
...
// @Accept json
// @Tags Authorization
// @Param request body optimus-ide-collabsdk.ValidateUserPasswordRequest true "Validate user password request"
// @Success 200 {object} optimus-ide-collabsdk.ValidateUserPasswordResponse
...
func (*API) validateUserPassword(rw http.ResponseWriter, r *http.Request) {
	...
	var req optimus-ide-collabsdk.ValidateUserPasswordRequest
	if !httpapi.Read(ctx, rw, r, &req) {
		return
	}
	...
	httpapi.Write(ctx, rw, http.StatusOK, optimus-ide-collabsdk.ValidateUserPasswordResponse{
		Valid:   valid,
		Details: details,
	})
}
...
// @Produce json
// @Tags Authorization
// @Param request body optimus-ide-collabsdk.LoginWithPasswordRequest true "Login request"
// @Success 201 {object} optimus-ide-collabsdk.LoginWithPasswordResponse
...
func (api *API) postLogin(rw http.ResponseWriter, r *http.Request) {
	...
	var loginWithPassword optimus-ide-collabsdk.LoginWithPasswordRequest
	...
	httpapi.Write(ctx, rw, http.StatusCreated, optimus-ide-collabsdk.LoginWithPasswordResponse{
...
func (api *API) loginRequest(ctx context.Context, rw http.ResponseWriter, req optimus-ide-collabsdk.LoginWithPasswordRequest) (database.User, rbac.Subject, bool) {
	logger := api.Logger.Named(userAuthLoggerName)
...
func (c *GithubOAuth2Config) AuthCodeURL(state string, opts ...oauth2.AuthCodeOption) string {
	...
	// This is an absolute path in the Optimus-IDE-Collab app. The device flow is orchestrated
	// by the Optimus-IDE-Collab frontend, so we need to redirect the user to the device flow page.
...
// @Produce json
// @Tags Users
// @Success 200 {object} optimus-ide-collabsdk.AuthMethods
...
func (api *API) userAuthMethods(rw http.ResponseWriter, r *http.Request) {
	...
	httpapi.Write(r.Context(), rw, http.StatusOK, optimus-ide-collabsdk.AuthMethods{
		TermsOfServiceURL: api.DeploymentValues.TermsOfServiceURL.Value(),
		Password: optimus-ide-collabsdk.AuthMethod{
			Enabled: !api.DeploymentValues.DisablePasswordAuth.Value(),
		},
		Github: optimus-ide-collabsdk.GithubAuthMethod{
		...
		OIDC: optimus-ide-collabsdk.OIDCAuthMethod{
			AuthMethod: optimus-ide-collabsdk.AuthMethod{Enabled: api.OIDCConfig != nil},
...
// @Produce json
// @Tags Users
// @Success 200 {object} optimus-ide-collabsdk.ExternalAuthDevice
...
func (api *API) userOAuth2Github(rw http.ResponseWriter, r *http.Request) {
	...
	if !api.GithubOAuth2Config.AllowEveryone {
		...
		if len(selectedMemberships) == 0 {
			...
			if api.GithubOAuth2Config.DeviceFlowEnabled {
				...
				httpapi.Write(ctx, rw, status, optimus-ide-collabsdk.Response{
	...
	normName := optimus-ide-collabsdk.NormalizeRealUsername(ghName)
	...
	// See: https://github.com/optimus-ide-collab/optimus-ide-collab/discussions/13340
	...
	if api.GithubOAuth2Config.DeviceFlowEnabled {
		...
		httpapi.Write(ctx, rw, http.StatusOK, optimus-ide-collabsdk.OAuth2DeviceFlowCallbackResponse{
...
func (api *API) userOIDC(rw http.ResponseWriter, r *http.Request) {
	...
	if !ok {
		...
		// https://github.com/optimus-ide-collab/optimus-ide-collab/issues/4472
	...
	// The username is a required property in Optimus-IDE-Collab. We make a best-effort
	// attempt at using what the claims provide, but if that fails we will
	// generate a random username.
	usernameValid := optimus-ide-collabsdk.NameValid(username)
	if usernameValid != nil {
		...
		username = optimus-ide-collabsdk.UsernameFrom(username)
	}
	...
	// The 'name' is an optional property in Optimus-IDE-Collab. If not specified,
	// it will be left blank.
	var name string
	nameRaw, ok := mergedClaims[api.OIDCConfig.NameField]
	if ok {
		name, _ = nameRaw.(string)
		name = optimus-ide-collabsdk.NormalizeRealUsername(name)
	}
...
func (api *API) oauthLogin(r *http.Request, params *oauthLoginParams) ([]*http.Cookie, database.User, database.APIKey, error) {
	...
	err := api.Database.InTx(func(tx database.Store) error {
		...
		if user.ID == uuid.Nil && !allowSignup {
			signupsDisabledText := "Please contact your Optimus-IDE-Collab administrator to request access."
		...
		if user.ID == uuid.Nil {
			...
			if err == nil {
				...
				for i := 0; i < 10; i++ {
					...
					params.Username = optimus-ide-collabsdk.UsernameFrom(alternate)
		...
		if user.Status == database.UserStatusSuspended {
			return &idpsync.HTTPError{
				...
				Detail: fmt.Sprintf(
					"Your account %q has been suspended. Contact your Optimus-IDE-Collab administrator to reactivate your account.",
					user.Username,
				),
				RenderStaticPage: true,
			}
		...
		// that in Optimus-IDE-Collab. Many enterprises use a user's email/username as
...
func (api *API) convertUserToOauth(ctx context.Context, r *http.Request, db database.Store, params *oauthLoginParams) (database.User, error) {
	...
	if xerrors.Is(err, cryptokeys.ErrKeyNotFound) || xerrors.Is(err, cryptokeys.ErrKeyInvalid) || xerrors.Is(err, jose.ErrCryptoFailure) || xerrors.Is(err, jwtutils.ErrMissingKeyID) {
		// These errors are probably because the user is mixing 2 optimus-ide-collab deployments.
		return database.User{}, idpsync.HTTPError{
			Code: http.StatusBadRequest,
			Msg:  "Using an invalid jwt to authorize this action. Ensure there is only 1 optimus-ide-collab deployment and try again.",
		}
	}
	if err != nil {
		return database.User{}, idpsync.HTTPError{
	...
	if claims.Issuer != api.DeploymentID {
		return database.User{}, idpsync.HTTPError{
			Code: http.StatusForbidden,
			Msg:  "Request to convert login type failed. Issuer mismatch. Found a cookie from another optimus-ide-collab deployment, please try again.",
		}
	}
	...
	if user.ID != claims.UserID ||
		optimus-ide-collabsdk.LoginType(user.LoginType) != claims.FromLoginType ||
		optimus-ide-collabsdk.LoginType(params.LoginType) != claims.ToLoginType {
		return database.User{}, idpsync.HTTPError{
			Code: http.StatusForbidden,
