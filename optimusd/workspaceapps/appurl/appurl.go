...
//
// Arguments:
// apphost: Expected to contain a wildcard, example: "*.optimus-ide-collab.com"
...
func CompileHostnamePattern(pattern string) (*regexp.Regexp, error) {
	...
	for i, label := range strings.Split(pattern, ".") {
		if i == 0 {
			...
			// test below. This makes `*.optimus-ide-collab.com` become `a.optimus-ide-collab.com` and
			// `*--prod.optimus-ide-collab.com` become `a--prod.optimus-ide-collab.com`.
			label = "a" + label
		}
...
// For example *--apps.optimus-ide-collab.com must become *.optimus-ide-collab.com.  If there is no
