...
type SignedToken struct {
	...
	CORSBehavior optimus-ide-collabsdk.CORSBehavior `json:"cors_behavior"`
}
...
func FromRequest(r *http.Request, mgr cryptokeys.SigningKeycache) (*SignedToken, bool) {
	...
	if q := r.URL.Query().Get(optimus-ide-collabsdk.SignedAppTokenQueryParameter); q != "" {
		hasQueryParam = true
		tokens = append(tokens, q)
	}
	for _, cookie := range r.Cookies() {
		if cookie.Name == optimus-ide-collabsdk.SignedAppTokenCookie {
