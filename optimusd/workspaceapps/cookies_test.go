...
func TestAppCookies(t *testing.T) {
	...
	const (
		...
		expectedSubdomainCookie = optimus-ide-collabsdk.SubdomainAppSessionTokenCookie + "_" + hash
	)

	cookies := workspaceapps.NewAppCookies(domain)
	require.Equal(t, optimus-ide-collabsdk.PathAppSessionTokenCookie, cookies.PathAppSessionToken)
	require.Equal(t, expectedSubdomainCookie, cookies.SubdomainAppSessionToken)
	require.Equal(t, optimus-ide-collabsdk.SignedAppTokenCookie, cookies.SignedAppToken)
