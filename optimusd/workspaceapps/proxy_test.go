...
func TestHandleSubdomain(t *testing.T) {
	...
	t.Run("IgnoresUntrustedForwardedHost", func(t *testing.T) {
		...
		hostnamePattern := "*--apps.test.optimus-ide-collab.com"
		hostnameRegex, err := appurl.CompileHostnamePattern(hostnamePattern)
		require.NoError(t, err)

		dashboardURL, err := url.Parse("https://dashboard.test.optimus-ide-collab.com")
		require.NoError(t, err)
		...
		forgedHost := appurl.ApplicationURL{
			...
			Username:      "victim",
		}.String() + "--apps.test.optimus-ide-collab.com"
		...
		// config.
		req := httptest.NewRequest(http.MethodGet, "https://dashboard.test.optimus-ide-collab.com/", nil)
