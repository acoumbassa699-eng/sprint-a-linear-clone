...
func WriteWorkspaceOffline(log slog.Logger, accessURL *url.URL, rw http.ResponseWriter, r *http.Request, appReq *Request) {
	...
	site.RenderStaticErrorPage(rw, r, site.ErrorPageData{
		...
		Description: fmt.Sprintf("Last workspace transition was to the %q state. Start the workspace to access its applications.", optimus-ide-collabsdk.WorkspaceTransitionStop),
		Actions:     actions,
	})
}
