...
func Test_ResolveRequest(t *testing.T) {
	...
	secondUserClient, secondUser := optimus-ide-collabdtest.CreateAnotherUser(t, client, firstUser.OrganizationID)
	...
	resources := optimus-ide-collabdtest.AwaitWorkspaceAgents(t, client, workspace.ID, agentName)
	agent := optimus-ide-collabdtest.RequireWorkspaceAgentByName(t, resources, agentName)
	agentID := agent.ID
		...
		for _, c := range cases {
			t.Run(c.name, func(t *testing.T) {
				...
				for _, app := range allApps {
					...
					r.Header.Set(optimus-ide-collabsdk.SessionTokenHeader, client.SessionToken())
					...
					require.Equal(t, &workspaceapps.SignedToken{
						...
						AgentID:      agentID,
						AppURL:       appURL,
						CORSBehavior: optimus-ide-collabsdk.CORSBehaviorSimple,
					}, token)
					...
					cookie := w.Cookies()[0]
					require.Equal(t, optimus-ide-collabsdk.SignedAppTokenCookie, cookie.Name)
	...
	t.Run("AuthenticatedOtherUser", func(t *testing.T) {
		...
		for _, app := range allApps {
			...
			rw := httptest.NewRecorder()
			r := httptest.NewRequest("GET", "/app", nil)
			r.Header.Set(optimus-ide-collabsdk.SessionTokenHeader, secondUserClient.SessionToken())
			r.RemoteAddr = auditableIP
	...
	t.Run("TokenDoesNotMatchRequest", func(t *testing.T) {
		...
		require.Equal(t, cookies[0].Name, optimus-ide-collabsdk.SignedAppTokenCookie)
	...
	t.Run("InitializingAppPermitted", func(t *testing.T) {
		...
		for _, app := range agent.Apps {
			if app.Slug == appNameInitializing {
				...
				require.Equal(t, optimus-ide-collabsdk.WorkspaceAppHealthInitializing, app.Health)
				break
			}
		}
		...
		require.Eventually(t, func() bool {
			...
			for _, app := range agent.Apps {
				if app.Slug == appNameUnhealthy {
					t.Log("app is", app.Health)
					return app.Health == optimus-ide-collabsdk.WorkspaceAppHealthUnhealthy
				}
			}
...
func workspaceappsResolveRequest(t testing.TB, connLogger connectionlog.ConnectionLogger, w http.ResponseWriter, r *http.Request, opts workspaceapps.ResolveRequestOptions) (token *workspaceapps.SignedToken, ok bool) {
	...
	if opts.Cookies.PathAppSessionToken == "" {
		opts.Cookies.PathAppSessionToken = optimus-ide-collabsdk.PathAppSessionTokenCookie
	}
	if opts.Cookies.SubdomainAppSessionToken == "" {
		opts.Cookies.SubdomainAppSessionToken = optimus-ide-collabsdk.SubdomainAppSessionTokenCookie + "_test"
	}
	if opts.Cookies.SignedAppToken == "" {
		opts.Cookies.SignedAppToken = optimus-ide-collabsdk.SignedAppTokenCookie
	}
...
func assertConnLogContains(t *testing.T, rr *httptest.ResponseRecorder, r *http.Request, connLogger *connectionlog.FakeConnectionLogger, workspace optimus-ide-collabsdk.Workspace, agentName string, slugOrPort string, typ database.ConnectionType, userID uuid.UUID) {
	t.Helper()
