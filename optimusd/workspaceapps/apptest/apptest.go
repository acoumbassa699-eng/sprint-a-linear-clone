...
//
// appHostIsPrimary is true if the app host is also the primary optimus-ide-collab API
...
func Run(t *testing.T, appHostIsPrimary bool, factory DeploymentFactory) {
	...
	t.Run("ReconnectingPTY", func(t *testing.T) {
		...
		t.Run("SignedTokenQueryParameter", func(t *testing.T) {
			...
			issueRes, err := appDetails.SDKClient.IssueReconnectingPTYSignedToken(ctx, optimus-ide-collabsdk.IssueReconnectingPTYSignedTokenRequest{
				URL:     u.String(),
			...
			unauthedAppClient := optimus-ide-collabsdk.New(appDetails.AppClient(t).URL)
	...
	t.Run("WorkspaceAppsProxyPath", func(t *testing.T) {
		...
		t.Run("NoAccessShould404", func(t *testing.T) {
			...
			appDetails := setupProxyTest(t, nil)
			userClient, _ := optimus-ide-collabdtest.CreateAnotherUser(t, appDetails.SDKClient, appDetails.FirstUser.OrganizationID, rbac.RoleMember())
		...
		t.Run("Proxies", func(t *testing.T) {
			...
			appTokenCookie := mustFindCookie(t, resp.Cookies(), optimus-ide-collabsdk.SignedAppTokenCookie)
		...
		t.Run("BlocksMe", func(t *testing.T) {
			...
			app.Username = optimus-ide-collabsdk.Me
		...
		t.Run("BadJWT", func(t *testing.T) {
			...
			newTokenCookie := mustFindCookie(t, resp.Cookies(), optimus-ide-collabsdk.SignedAppTokenCookie)
	...
	t.Run("WorkspaceApplicationCORS", func(t *testing.T) {
		...
		unauthenticatedClient := func(t *testing.T, appDetails *Details) *optimus-ide-collabsdk.Client {
			c := appDetails.AppClient(t)
			c.SetSessionToken("")
			return c
		}

		authenticatedClient := func(t *testing.T, appDetails *Details) *optimus-ide-collabsdk.Client {
			uc, _ := optimus-ide-collabdtest.CreateAnotherUser(t, appDetails.SDKClient, appDetails.FirstUser.OrganizationID, rbac.RoleMember())
			c := appDetails.AppClient(t)
		...
		tests := []struct {
			...
			client               func(t *testing.T, appDetails *Details) *optimus-ide-collabsdk.Client
			behavior             optimus-ide-collabsdk.CORSBehavior
			httpMethod           string
			...
			{ // fails
				...
				behavior:           optimus-ide-collabsdk.CORSBehaviorSimple,
			...
			{ // fails
				...
				behavior:           optimus-ide-collabsdk.CORSBehaviorPassthru,
				client:             unauthenticatedClient,
				origin:             ownSubdomain,
				httpMethod:         http.MethodOptions,
				expectedStatusCode: http.StatusOK,
			...
			t.Run(tc.name, func(t *testing.T) {
				...
				appDetails := setupProxyTest(t, &DeploymentOptions{
					handler: http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
						_, err := r.Cookie(optimus-ide-collabsdk.SessionTokenCookie)
						...
						switch tc.behavior {
						case optimus-ide-collabsdk.CORSBehaviorPassthru:
						...
						case optimus-ide-collabsdk.CORSBehaviorSimple:
				...
				b := tc.behavior
				template, err := appDetails.SDKClient.UpdateTemplateMeta(ctx, appDetails.Workspace.TemplateID, optimus-ide-collabsdk.UpdateTemplateMeta{
					CORSBehavior: &b,
				})
				require.NoError(t, err)
		...
		t.Run("End-to-End", func(t *testing.T) {
			...
			cases := []struct {
			...
			}{
				{
					...
					sessionTokenCookieName: optimus-ide-collabsdk.SubdomainAppSessionTokenCookie,
				},
				{
					name:                   "Path",
					appURL:                 appDetails.PathAppURL(appDetails.Apps.Owner),
					sessionTokenCookieName: optimus-ide-collabsdk.PathAppSessionTokenCookie,
				},
			}
			...
			for _, c := range cases {
				...
				t.Run(c.name, func(t *testing.T) {
					...
					user, err := appDetails.SDKClient.User(ctx, optimus-ide-collabsdk.Me)
					require.NoError(t, err)
					...
					resp, err = requestWithRetries(ctx, t, appDetails.SDKClient, http.MethodGet, "/api/v2/applications/auth-redirect", nil, optimus-ide-collabsdk.WithQueryParam(
					...
					require.Equal(t, optimus-ide-collabsdk.LoginTypePassword, apiKeyInfo.LoginType)
					...
					appTokenAPIClient := optimus-ide-collabsdk.New(appDetails.SDKClient.URL)
					...
					authRes, err := appTokenAPIClient.AuthCheck(ctx, optimus-ide-collabsdk.AuthorizationRequest{
						Checks: map[string]optimus-ide-collabsdk.AuthorizationCheck{
							canApplicationConnect: {
								...
								Action: optimus-ide-collabsdk.ActionApplicationConnect,
							},
							canReadUserMe: {
								Object: optimus-ide-collabsdk.AuthorizationObject{
									ResourceType: "user",
								...
								Action: optimus-ide-collabsdk.ActionRead,
							},
						},
					})
					require.NoError(t, err)
					...
					req, err = http.NewRequestWithContext(ctx, "GET", gotLocation.String(), nil)
					require.NoError(t, err)
					req.Header.Set(optimus-ide-collabsdk.SessionTokenHeader, apiKey)
					resp, err = doWithRetries(t, appClient, req)
					require.NoError(t, err)
					resp.Body.Close()
					require.Equal(t, http.StatusOK, resp.StatusCode)
	...
	t.Run("WorkspaceAppsProxySubdomainHostnamePrefix/Different", func(t *testing.T) {
		...
		for _, c := range resp.Cookies() {
			if c.Name == optimus-ide-collabsdk.SignedAppTokenCookie {
				appTokenCookie = c
				break
			}
		}
	...
	t.Run("WorkspaceAppsProxySubdomainPassthrough", func(t *testing.T) {
		...
		u.Host = "app--agent--workspace--username.test.optimus-ide-collab.com"
		u.Path = "/api/v2/users/me"
		resp, err := requestWithRetries(ctx, t, appDetails.AppClient(t), http.MethodGet, u.String(), nil)
		require.NoError(t, err)
		defer resp.Body.Close()

		// Should look like a optimus-ide-collabsdk.User response.
		require.Equal(t, http.StatusOK, resp.StatusCode)
		var user optimus-ide-collabsdk.User
		err = json.NewDeoptimus-ide-collab(resp.Body).Decode(&user)
		require.NoError(t, err)
	...
	t.Run("WorkspaceAppsProxySubdomain", func(t *testing.T) {
		...
		t.Run("WorkspaceUUIDOwnerMismatchShould404", func(t *testing.T) {
			...
			appDetails := setupProxyTest(t, nil)
			otherUserClient, otherUser := optimus-ide-collabdtest.CreateAnotherUser(t, appDetails.SDKClient, appDetails.FirstUser.OrganizationID, rbac.RoleMember())
			appClient := appDetails.AppClient(t)
		...
		t.Run("ProxyPortMinimumError", func(t *testing.T) {
			...
			require.Equal(t, http.StatusBadRequest, resp.StatusCode)
			var resBody optimus-ide-collabsdk.Response
			err = json.NewDeoptimus-ide-collab(resp.Body).Decode(&resBody)
			require.NoError(t, err)
			require.Contains(t, resBody.Message, "Optimus-IDE-Collab reserves ports less than")
			assertWorkspaceLastUsedAtNotUpdated(t, appDetails, testutil.WaitLong)
		})
		...
		t.Run("SuffixWildcardOK", func(t *testing.T) {
			...
			appDetails := setupProxyTest(t, &DeploymentOptions{
				AppHost: "*-suffix.test.optimus-ide-collab.com",
			})
		...
		t.Run("WildcardPortOK", func(t *testing.T) {
			...
			appDetails := setupProxyTest(t, &DeploymentOptions{
				...
				AppHost:        "*.test.optimus-ide-collab.com:4444",
				PrimaryAppHost: "*.test.optimus-ide-collab.com:4444",
			})
			...
			require.NoError(t, err)
			require.Equal(t, "*.test.optimus-ide-collab.com:4444", apphost.Host, "apphost has port")
	...
	t.Run("PortSharing", func(t *testing.T) {
		...
		t.Run("AuthenticatedOK", func(t *testing.T) {
			...
			_, err = appDetails.SDKClient.UpsertWorkspaceAgentPortShare(ctx, appDetails.Workspace.ID, optimus-ide-collabsdk.UpsertWorkspaceAgentPortShareRequest{
	...
	t.Run("CORS", func(t *testing.T) {
		...
		ownerClient := func(t *testing.T, appDetails *Details) *optimus-ide-collabsdk.Client {
		...
		tests := []struct {
			name                string
			shareLevel          optimus-ide-collabsdk.WorkspaceAgentPortShareLevel
			behavior            optimus-ide-collabsdk.CORSBehavior
			client              func(t *testing.T, appDetails *Details) *optimus-ide-collabsdk.Client
			expectedStatusCode  int
			...
			{
				name:                "Default/Owner",
				shareLevel:          optimus-ide-collabsdk.WorkspaceAgentPortShareLevelAuthenticated, // Owner is not a valid share level for ports.
				behavior:            optimus-ide-collabsdk.CORSBehaviorSimple,
				expectedCORSHeaders: false,
				client:              ownerClient,
				expectedStatusCode:  http.StatusOK,
	...
	t.Run("AppSharing", func(t *testing.T) {
		...
		setup := func(t *testing.T, allowPathAppSharing, allowSiteOwnerAccess bool) (appDetails *Details, workspace optimus-ide-collabsdk.Workspace, agnt optimus-ide-collabsdk.WorkspaceAgent, user optimus-ide-collabsdk.User, ownerClient *optimus-ide-collabsdk.Client, client *optimus-ide-collabsdk.Client, clientInOtherOrg *optimus-ide-collabsdk.Client, clientWithNoAuth *optimus-ide-collabsdk.Client) {
			//nolint:gosec
			...
			user, err := ownerClient.CreateUserWithOrgs(ctx, optimus-ide-collabsdk.CreateUserRequestWithOrgs{
				Email:           "user@optimus-ide-collab.com",
				Username:        "user",
				Password:        password,
			...
			_, err = ownerClient.UpdateUserRoles(ctx, user.ID.String(), optimus-ide-collabsdk.UpdateRoles{
				Roles: []string{"template-admin", "member"},
			})
			require.NoError(t, err)

			client = optimus-ide-collabsdk.New(ownerClient.URL)
			loginRes, err := client.LoginWithPassword(ctx, optimus-ide-collabsdk.LoginWithPasswordRequest{
				Email:    user.Email,
				Password: password,
			})
			require.NoError(t, err)
			...
			found := map[string]optimus-ide-collabsdk.WorkspaceAppSharingLevel{}
			expected := map[string]optimus-ide-collabsdk.WorkspaceAppSharingLevel{
				proxyTestAppNameFake:                      optimus-ide-collabsdk.WorkspaceAppSharingLevelOwner,
				proxyTestAppNameOwner:                     optimus-ide-collabsdk.WorkspaceAppSharingLevelOwner,
				proxyTestAppNameAuthenticated:             optimus-ide-collabsdk.WorkspaceAppSharingLevelAuthenticated,
				proxyTestAppNamePublic:                    optimus-ide-collabsdk.WorkspaceAppSharingLevelPublic,
				proxyTestAppNameAuthenticatedCORSPassthru: optimus-ide-collabsdk.WorkspaceAppSharingLevelAuthenticated,
				proxyTestAppNamePublicCORSPassthru:        optimus-ide-collabsdk.WorkspaceAppSharingLevelPublic,
				proxyTestAppNameAuthenticatedCORSDefault:  optimus-ide-collabsdk.WorkspaceAppSharingLevelAuthenticated,
				proxyTestAppNamePublicCORSDefault:         optimus-ide-collabsdk.WorkspaceAppSharingLevelPublic,
			}
			...
			require.NoError(t, err)
			userInOtherOrg, err := ownerClient.CreateUserWithOrgs(ctx, optimus-ide-collabsdk.CreateUserRequestWithOrgs{
				Email:           "no-template-access@optimus-ide-collab.com",
			...
			clientInOtherOrg = optimus-ide-collabsdk.New(client.URL)
			loginRes, err = clientInOtherOrg.LoginWithPassword(ctx, optimus-ide-collabsdk.LoginWithPasswordRequest{
			...
			// Create an unauthenticated optimus-ide-collabsdk client.
			clientWithNoAuth = optimus-ide-collabsdk.New(client.URL)
		...
		verifyAccess := func(t *testing.T, appDetails *Details, isPathApp bool, username, workspaceName, agentName, appName string, client *optimus-ide-collabsdk.Client, shouldHaveAccess, shouldRedirectToLogin bool) {
			t.Helper()
	...
	t.Run("WorkspaceAppsNonCanonicalHeaders", func(t *testing.T) {
		...
		for _, c := range cases {
			t.Run(c.name, func(t *testing.T) {
				...
				req.Header.Set(optimus-ide-collabsdk.SessionTokenHeader, appDetails.SDKClient.SessionToken())
	...
	t.Run("WorkspaceOffline", func(t *testing.T) {
		...
		_ = optimus-ide-collabdtest.MustTransitionWorkspace(t, appDetails.SDKClient, appDetails.Workspace.ID, optimus-ide-collabsdk.WorkspaceTransitionStart, optimus-ide-collabsdk.WorkspaceTransitionStop)
...
func testReconnectingPTY(ctx context.Context, t *testing.T, client *optimus-ide-collabsdk.Client, agentID uuid.UUID, signedToken string) {
