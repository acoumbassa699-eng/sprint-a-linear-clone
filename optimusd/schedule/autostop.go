...
func CalculateAutostop(ctx context.Context, params CalculateAutostopParams) (AutostopTime, error) {
	ctx, span := tracing.StartSpan(ctx,
		trace.WithAttributes(attribute.String("optimus-ide-collab.workspace_id", params.Workspace.ID.String())),
		trace.WithAttributes(attribute.String("optimus-ide-collab.template_id", params.Workspace.TemplateID.String())),
	)
	defer span.End()
	...
	if ttl > 0 {
		...
		if params.WorkspaceAutostart != "" {
			...
			// 2. Optimus-IDE-Collab stops workspace at 9pm
...
func WeeksSinceEpoch(now time.Time) (int64, error) {
	...
	if now.Before(epoch) {
		return 0, xerrors.New("optimus-ide-collab server system clock is incorrect, cannot calculate template autostop requirement")
	}
