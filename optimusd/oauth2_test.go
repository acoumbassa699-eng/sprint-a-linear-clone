...
func TestOAuth2ProviderApps(t *testing.T) {
	...
	t.Run("IntegrationFlow", func(t *testing.T) {
		...
		app, err := client.PostOAuth2ProviderApp(ctx, optimus-ide-collabsdk.PostOAuth2ProviderAppRequest{
...
func TestOAuth2ProviderTokenExchange(t *testing.T) {
	...
	//             Optimus-IDE-Collab can just immediately redirect back to backstage without user intervention.
	tests := []struct {
		name string
		app  optimus-ide-collabsdk.OAuth2ProviderApp
		// The flow is setup(ctx, client, user) -> preAuth(cfg) -> cfg.AuthCodeURL() -> preToken(cfg) -> cfg.Exchange()
		setup      func(context.Context, *optimus-ide-collabsdk.Client, optimus-ide-collabsdk.User) error
		...
		},
		{
			...
			app:  apps.Default,
			preToken: func(valid *oauth2.Config) {
				valid.ClientSecret = "notoptimus-ide-collab_1234_4321"
			},
			tokenError: "The client credentials are invalid",
		},
		{
			...
			app:  apps.Default,
			preToken: func(valid *oauth2.Config) {
				valid.ClientSecret = "optimus-ide-collab_1234"
			},
			tokenError: "The client credentials are invalid",
		},
		{
			...
			app:  apps.Default,
			preToken: func(valid *oauth2.Config) {
				valid.ClientSecret = "optimus-ide-collab__1234"
			},
			tokenError: "The client credentials are invalid",
		},
		{
			...
			app:  apps.Default,
			preToken: func(valid *oauth2.Config) {
				valid.ClientSecret = "optimus-ide-collab_1234_4321"
			},
			tokenError: "The client credentials are invalid",
		},
		{
		...
		},
		{
			...
			app:         apps.Default,
			defaultCode: ptr.Ref("notoptimus-ide-collab_1234_4321"),
			tokenError:  "The authorization code is invalid or expired",
		},
		{
			name:        "MissingCodeSecret",
			app:         apps.Default,
			defaultCode: ptr.Ref("optimus-ide-collab_1234"),
			tokenError:  "The authorization code is invalid or expired",
		},
		{
			name:        "MissingCodePrefix",
			app:         apps.Default,
			defaultCode: ptr.Ref("optimus-ide-collab__1234"),
			tokenError:  "The authorization code is invalid or expired",
		},
		{
			name:        "InvalidCodePrefix",
			app:         apps.Default,
			defaultCode: ptr.Ref("optimus-ide-collab_1234_4321"),
			tokenError:  "The authorization code is invalid or expired",
		},
		{
			name:        "MissingCode",
		...
		},
		{
			name:        "ExpiredCode",
			app:         apps.Default,
			defaultCode: ptr.Ref("optimus-ide-collab_prefix_code"),
			tokenError:  "The authorization code is invalid or expired",
			setup: func(ctx context.Context, client *optimus-ide-collabsdk.Client, user optimus-ide-collabsdk.User) error {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			userClient, user := optimus-ide-collabdtest.CreateAnotherUser(t, ownerClient, owner.OrganizationID)
			if test.setup != nil {
			...
			if test.tokenError != "" {
				...
				newClient := optimus-ide-collabsdk.New(userClient.URL)
				newClient.SetSessionToken(token.AccessToken)

				gotUser, err := newClient.User(ctx, optimus-ide-collabsdk.Me)
				require.NoError(t, err)
...
func TestOAuth2ProviderTokenRefresh(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			app:          apps.Default,
			defaultToken: ptr.Ref("notoptimus-ide-collab_1234_4321"),
			error:        "The refresh token is invalid or expired",
		},
		{
			name:         "MissingTokenSecret",
			app:          apps.Default,
			defaultToken: ptr.Ref("optimus-ide-collab_1234"),
			error:        "The refresh token is invalid or expired",
		},
		{
			name:         "MissingTokenPrefix",
			app:          apps.Default,
			defaultToken: ptr.Ref("optimus-ide-collab__1234"),
			error:        "The refresh token is invalid or expired",
		},
		{
			name:         "InvalidTokenPrefix",
			app:          apps.Default,
			defaultToken: ptr.Ref("optimus-ide-collab_1234_4321"),
			error:        "The refresh token is invalid or expired",
		},
		{
			name:    "Expired",
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.error != "" {
				...
				_, err = newClient.User(ctx, optimus-ide-collabsdk.Me)
				require.Error(t, err)
...
type exchangeSetup struct {
	cfg      *oauth2.Config
	app      optimus-ide-collabsdk.OAuth2ProviderApp
	secret   optimus-ide-collabsdk.OAuth2ProviderAppSecretFull
	code     string
	verifier string
}
...
func TestOAuth2ProviderRevoke(t *testing.T) {
	...
	tests := []struct {
		...
		fn func(context.Context, *optimus-ide-collabsdk.Client, exchangeSetup)
		...
		{
			name: "DeleteApp",
			fn: func(ctx context.Context, _ *optimus-ide-collabsdk.Client, s exchangeSetup) {
				//nolint:gocritic // OAauth2 app management requires owner permission.
		...
		{
			name: "DeleteApp",
			fn: func(ctx context.Context, client *optimus-ide-collabsdk.Client, s exchangeSetup) {
	...
	setup := func(ctx context.Context, testClient *optimus-ide-collabsdk.Client, name string) exchangeSetup {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			t.Parallel()
			ctx := testutil.Context(t, testutil.WaitLong)
			testClient, testUser := optimus-ide-collabdtest.CreateAnotherUser(t, client, owner.OrganizationID)
			...
			newClient := optimus-ide-collabsdk.New(client.URL)
			newClient.SetSessionToken(token.AccessToken)
			...
			filter := optimus-ide-collabsdk.OAuth2ProviderAppFilter{UserID: testUser.ID}
			...
			apps, err = client.OAuth2ProviderApps(ctx, optimus-ide-collabsdk.OAuth2ProviderAppFilter{UserID: owner.UserID})
			require.NoError(t, err)
...
type provisionedApps struct {
	Default   optimus-ide-collabsdk.OAuth2ProviderApp
	NoPort    optimus-ide-collabsdk.OAuth2ProviderApp
	Subdomain optimus-ide-collabsdk.OAuth2ProviderApp
	// For sorting purposes these are included. You will likely never touch them.
	Extra []optimus-ide-collabsdk.OAuth2ProviderApp
}

func generateApps(ctx context.Context, t *testing.T, client *optimus-ide-collabsdk.Client, suffix string) provisionedApps {
	create := func(name, callback string) optimus-ide-collabsdk.OAuth2ProviderApp {
...
func authorizationFlow(ctx context.Context, client *optimus-ide-collabsdk.Client, cfg *oauth2.Config) (code, codeVerifier string, err error) {
...
func TestOAuth2ProviderCrossResourceAudienceValidation(t *testing.T) {
	...
	// Set up first Optimus-IDE-Collab instance (resource server 1)
	server1 := optimus-ide-collabdtest.New(t, &optimus-ide-collabdtest.Options{
		Database: db,
		Pubsub:   pubsub,
	})
	owner := optimus-ide-collabdtest.CreateFirstUser(t, server1)

	// Set up second Optimus-IDE-Collab instance (resource server 2) - simulate different host
	server2 := optimus-ide-collabdtest.New(t, &optimus-ide-collabdtest.Options{
		Database: db,
		Pubsub:   pubsub,
	})
	...
	require.NoError(t, err)
	userClient, user := optimus-ide-collabdtest.CreateAnotherUser(t, server1, owner.OrganizationID)
	...
	client1 := optimus-ide-collabsdk.New(server1.URL)
	client1.SetSessionToken(token.AccessToken)
	gotUser, err := client1.User(ctx, optimus-ide-collabsdk.Me)
	require.NoError(t, err)
	require.Equal(t, user.ID, gotUser.ID)
	...
	client2 := optimus-ide-collabsdk.New(server2.URL)
	...
	if _, err := client2.User(ctx, optimus-ide-collabsdk.Me); err != nil {
...
func TestOAuth2RefreshExpiryOutlivesAccess(t *testing.T) {
	...
	db, pubsub := dbtestutil.NewDB(t)
	dv := optimus-ide-collabdtest.DeploymentValues(t, func(d *optimus-ide-collabsdk.DeploymentValues) {
	...
	// Parse refresh token prefix (optimus-ide-collab_<prefix>_<secret>)
...
func customTokenExchange(ctx context.Context, baseURL, clientID, clientSecret, code, redirectURI, resource, codeVerifier string) (*oauth2.Token, error) {
	...
	if resp.StatusCode != http.StatusOK {
		...
		_ = json.NewDeoptimus-ide-collab(resp.Body).Decode(&errorResp)
		return nil, xerrors.Errorf("oauth2: %q %q", errorResp.Error, errorResp.ErrorDescription)
	}

	var token oauth2.Token
	if err := json.NewDeoptimus-ide-collab(resp.Body).Decode(&token); err != nil {
		return nil, err
	}
...
func TestOAuth2DynamicClientRegistration(t *testing.T) {
	...
	t.Run("BasicRegistration", func(t *testing.T) {
		...
		require.Contains(t, resp.GrantTypes, optimus-ide-collabsdk.OAuth2ProviderGrantTypeAuthorizationCode)
		require.Contains(t, resp.GrantTypes, optimus-ide-collabsdk.OAuth2ProviderGrantTypeRefreshToken)
		require.Contains(t, resp.ResponseTypes, optimus-ide-collabsdk.OAuth2ProviderResponseTypeCode)
		require.Equal(t, optimus-ide-collabsdk.OAuth2TokenEndpointAuthMethodClientSecretBasic, resp.TokenEndpointAuthMethod)
...
func TestOAuth2DynamicClientRegistrationDisabled(t *testing.T) {
	...
	regResp, err := client.PostOAuth2ClientRegistration(ctx, optimus-ide-collabsdk.OAuth2ClientRegistrationRequest{
	...
	t.Run("NewRegistrationRejected", func(t *testing.T) {
		t.Parallel()
		ctx := testutil.Context(t, testutil.WaitLong)
		_, err := client.PostOAuth2ClientRegistration(ctx, optimus-ide-collabsdk.OAuth2ClientRegistrationRequest{
			RedirectURIs: []string{oauth2providertest.TestRedirectURI},
		})
		require.Error(t, err)
	...
	t.Run("DiscoveryOmitsRegistrationEndpoint", func(t *testing.T) {
		...
		require.NoError(t, json.NewDeoptimus-ide-collab(res.Body).Decode(&metadata))
...
func TestOAuth2ClientConfiguration(t *testing.T) {
	...
	t.Run("UpdateConfiguration", func(t *testing.T) {
		...
		updateReq := optimus-ide-collabsdk.OAuth2ClientRegistrationRequest{
...
func TestOAuth2RegistrationAccessToken(t *testing.T) {
	...
	t.Run("ManuallyCreatedClient", func(t *testing.T) {
		...
		appReq := optimus-ide-collabsdk.PostOAuth2ProviderAppRequest{
	...
	t.Run("TokenPasswordComparison", func(t *testing.T) {
		...
		req1 := optimus-ide-collabsdk.OAuth2ClientRegistrationRequest{
			RedirectURIs: []string{"https://client1.com/callback"},
			ClientName:   fmt.Sprintf("client-1-%d", timestamp),
		}
		req2 := optimus-ide-collabsdk.OAuth2ClientRegistrationRequest{
...
// TestOAuth2Optimus-IDE-CollabClient verfies a optimus-ide-collabsdk client can be used with an oauth client.
func TestOAuth2Optimus-IDE-CollabClient(t *testing.T) {
	t.Parallel()
	...
	// Setup an oauth app
	ctx := testutil.Context(t, testutil.WaitLong)
	app, err := owner.PostOAuth2ProviderApp(ctx, optimus-ide-collabsdk.PostOAuth2ProviderAppRequest{
		Name:        "new-app",
		CallbackURL: "http://localhost",
	})
	require.NoError(t, err)
	...
	// Make a new user
	client, user := optimus-ide-collabdtest.CreateAnotherUser(t, owner, first.OrganizationID)
	...
	usingOauth := optimus-ide-collabsdk.New(owner.URL)
	usingOauth.HTTPClient = oauthClient

	me, err := usingOauth.User(ctx, optimus-ide-collabsdk.Me)
	require.NoError(t, err)
	...
	_, err = usingOauth.User(ctx, optimus-ide-collabsdk.Me)
	require.Error(t, err)
}
