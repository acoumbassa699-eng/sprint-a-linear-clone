...
import (
	...
	mcpserver "github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabd/mcp"
...
func TestMCPHTTP_E2E_ClientIntegration(t *testing.T) {
	...
	// Setup Optimus-IDE-Collab server with authentication
	optimus-ide-collabClient, closer, api := optimus-ide-collabdtest.NewWithAPI(t, nil)
	defer closer.Close()

	_ = optimus-ide-collabdtest.CreateFirstUser(t, optimus-ide-collabClient)
	...
	mcpClient := newIsolatedMCPClient(t, mcpURL,
		transport.WithHTTPHeaders(map[string]string{
			"Authorization": "Bearer " + optimus-ide-collabClient.SessionToken(),
		}))
	defer func() {
	...
	// Verify we have some expected Optimus-IDE-Collab tools
...
func TestMCPHTTP_E2E_UnauthenticatedAccess(t *testing.T) {
	...
	// Setup Optimus-IDE-Collab server
	_, closer, api := optimus-ide-collabdtest.NewWithAPI(t, nil)
	defer closer.Close()
...
func TestMCPHTTP_E2E_ToolWithWorkspace(t *testing.T) {
	...
	user := optimus-ide-collabdtest.CreateFirstUser(t, optimus-ide-collabClient)
	...
	_ = agenttest.New(t, optimus-ide-collabClient.URL, r.AgentToken, func(opts *agent.Options) {
		opts.Filesystem = fs
	})
	optimus-ide-collabdtest.NewWorkspaceAgentWaiter(t, optimus-ide-collabClient, r.Workspace.ID).Wait()
...
func TestMCPHTTP_E2E_ErrorHandling(t *testing.T) {
	...
	// Setup Optimus-IDE-Collab server
	optimus-ide-collabClient, closer, api := optimus-ide-collabdtest.NewWithAPI(t, &optimus-ide-collabdtest.Options{
...
func TestMCPHTTP_E2E_RFC6750_UnauthenticatedRequest(t *testing.T) {
	...
	require.Contains(t, wwwAuth, `realm="optimus-ide-collab"`, "WWW-Authenticate header should include realm")
...
func TestMCPHTTP_E2E_OAuth2_EndToEnd(t *testing.T) {
	...
	// Setup Optimus-IDE-Collab server with OAuth2 provider enabled
	optimus-ide-collabClient, closer, api := optimus-ide-collabdtest.NewWithAPI(t, nil)
	t.Cleanup(func() { closer.Close() })

	_ = optimus-ide-collabdtest.CreateFirstUser(t, optimus-ide-collabClient)
	oauth2providertest.EnableDCR(t, optimus-ide-collabClient)

	ctx := t.Context()

	// Create OAuth2 app (for demonstration that OAuth2 provider is working)
	_, err := optimus-ide-collabClient.PostOAuth2ProviderApp(ctx, optimus-ide-collabsdk.PostOAuth2ProviderAppRequest{
		Name:        "test-mcp-app",
	...
	t.Run("MCPWithOAuth2BearerToken", func(t *testing.T) {
		...
		sessionToken := optimus-ide-collabClient.SessionToken()
	...
	t.Run("OAuth2FullFlowWithTokenRefresh", func(t *testing.T) {
		...
		app, err := optimus-ide-collabClient.PostOAuth2ProviderApp(ctx, optimus-ide-collabsdk.PostOAuth2ProviderAppRequest{
		...
		secret, err := optimus-ide-collabClient.PostOAuth2ProviderAppSecret(ctx, app.ID)
		require.NoError(t, err)
		...
		req.Header.Set("Authorization", "Bearer "+optimus-ide-collabClient.SessionToken())
		...
		if resp.StatusCode == http.StatusOK {
			...
			require.NoError(t, err)
			consentReq.Header.Set("Authorization", "Bearer "+optimus-ide-collabClient.SessionToken())
		...
		err = json.NewDeoptimus-ide-collab(tokenResp.Body).Decode(&tokenResponse)
		require.NoError(t, err)
		...
		err = json.NewDeoptimus-ide-collab(refreshResp.Body).Decode(&refreshResponse)
		require.NoError(t, err)
	...
	t.Run("InvalidBearerToken", func(t *testing.T) {
		...
		require.Contains(t, wwwAuth, `realm="optimus-ide-collab"`)
	...
	t.Run("DynamicClientRegistrationWithMCPFlow", func(t *testing.T) {
		...
		err = json.NewDeoptimus-ide-collab(regResp.Body).Decode(&regResponse)
		require.NoError(t, err)
		...
		require.NoError(t, err)
		authReq.Header.Set("Cookie", fmt.Sprintf("optimus-ide-collab_session_token=%s", optimus-ide-collabClient.SessionToken()))
		authReq.Header.Set("Authorization", "Bearer "+optimus-ide-collabClient.SessionToken())
		...
		if authResp.StatusCode == http.StatusOK {
			...
			consentReq, err := http.NewRequestWithContext(ctx, "POST", authURL, nil)
			require.NoError(t, err)
			consentReq.Header.Set("Cookie", fmt.Sprintf("optimus-ide-collab_session_token=%s", optimus-ide-collabClient.SessionToken()))
			consentReq.Header.Set("Authorization", "Bearer "+optimus-ide-collabClient.SessionToken())
			consentReq.Header.Set("Content-Type", "application/x-www-form-urlencoded")
	...
	t.Run("DuplicateClientNamesAllowed", func(t *testing.T) {
		...
		err = json.NewDeoptimus-ide-collab(regResp1.Body).Decode(&regResponse1)
		require.NoError(t, err)
		...
		err = json.NewDeoptimus-ide-collab(regResp2.Body).Decode(&regResponse2)
		require.NoError(t, err)
...
func TestMCPHTTP_E2E_ChatGPTEndpoint(t *testing.T) {
	...
	version := optimus-ide-collabdtest.CreateTemplateVersion(t, optimus-ide-collabClient, user.OrganizationID, nil)
	optimus-ide-collabdtest.AwaitTemplateVersionJobCompleted(t, optimus-ide-collabClient, version.ID)
	template := optimus-ide-collabdtest.CreateTemplate(t, optimus-ide-collabClient, user.OrganizationID, version.ID)
...
func TestMCPHTTP_E2E_WorkspaceSSHAuthz(t *testing.T) {
	...
	admin := optimus-ide-collabdtest.CreateFirstUser(t, optimus-ide-collabClient)
	...
	tmplAdminClient, _ := optimus-ide-collabdtest.CreateAnotherUser(
		t, optimus-ide-collabClient, admin.OrganizationID, rbac.RoleTemplateAdmin(),
	)
	...
	workspaceIdent := optimus-ide-collabdtest.FirstUserParams.Username + "/" + r.Workspace.Name
	toolResult, err := mcpClient.CallTool(ctx, mcp.CallToolRequest{
		Params: mcp.CallToolParams{
...
func newIsolatedMCPClient(t *testing.T, mcpURL string, opts ...transport.StreamableHTTPCOption) *mcpclient.Client {
	t.Helper()
	isolated := optimus-ide-collabdtest.NewIsolatedHTTPClient(nil)
