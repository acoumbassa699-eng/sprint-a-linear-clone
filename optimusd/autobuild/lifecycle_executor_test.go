...
func TestExecutorAutostartOK(t *testing.T) {
	...
	var (
		...
		workspace = mustProvisionWorkspace(t, client, func(cwr *optimus-ide-collabsdk.CreateWorkspaceRequest) {
	...
	assert.Equal(t, optimus-ide-collabsdk.BuildReasonAutostart, workspace.LatestBuild.Reason)
...
func TestMultipleLifecycleExecutors(t *testing.T) {
	...
	var (
		...
		clientA  = optimus-ide-collabdtest.New(t, &optimus-ide-collabdtest.Options{
			IncludeProvisionerDaemon: true,
			AutobuildTicker:          tickCh,
		...
		_        = optimus-ide-collabdtest.New(t, &optimus-ide-collabdtest.Options{
			IncludeProvisionerDaemon: true,
			AutobuildTicker:          tickCh,
		...
		workspace = mustProvisionWorkspace(t, clientA, func(cwr *optimus-ide-collabsdk.CreateWorkspaceRequest) {
			cwr.AutostartSchedule = ptr.Ref(sched.String())
		})
	)

	// Have the workspace stopped so we can perform an autostart
	workspace = optimus-ide-collabdtest.MustTransitionWorkspace(t, clientA, workspace.ID, optimus-ide-collabsdk.WorkspaceTransitionStart, optimus-ide-collabsdk.WorkspaceTransitionStop)
	...
	next := optimus-ide-collabdtest.NextAutostartTick(t, workspace)
...
func TestExecutorBuildNumberRaceIsHandled(t *testing.T) {
	...
	p, err := optimus-ide-collabdtest.GetProvisionerForTags(realDB, time.Now(), workspace.OrganizationID, nil)
	require.NoError(t, err)
	next := optimus-ide-collabdtest.NextAutostartTick(t, workspace)
	optimus-ide-collabdtest.UpdateProvisionerLastSeenAt(t, realDB, p.ID, next)
...
func TestExecutorAutostartTemplateUpdated(t *testing.T) {
	...
	testCases := []struct {
		name                 string
		automaticUpdates     optimus-ide-collabsdk.AutomaticUpdates
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			workspace = optimus-ide-collabdtest.MustTransitionWorkspace(
				t, client, workspace.ID, optimus-ide-collabsdk.WorkspaceTransitionStart, optimus-ide-collabsdk.WorkspaceTransitionStop)
			...
			newVersion := optimus-ide-collabdtest.UpdateTemplateVersion(t, client, orgs[0].ID, res, workspace.TemplateID)
			...
			require.NoError(t, client.UpdateActiveTemplateVersion(
				ctx, workspace.TemplateID, optimus-ide-collabsdk.UpdateActiveTemplateVersion{
					ID: newVersion.ID,
				},
			))
...
func TestExecutorAutostartUserSuspended(t *testing.T) {
	...
	workspace := optimus-ide-collabdtest.CreateWorkspace(t, userClient, template.ID, func(cwr *optimus-ide-collabsdk.CreateWorkspaceRequest) {
		cwr.AutostartSchedule = ptr.Ref(sched.String())
	})
	optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, userClient, workspace.LatestBuild.ID)
	workspace = optimus-ide-collabdtest.MustWorkspace(t, userClient, workspace.ID)

	// Given: workspace is stopped, and the user is suspended.
	workspace = optimus-ide-collabdtest.MustTransitionWorkspace(t, userClient, workspace.ID, optimus-ide-collabsdk.WorkspaceTransitionStart, optimus-ide-collabsdk.WorkspaceTransitionStop)

	ctx := testutil.Context(t, testutil.WaitShort)

	_, err := client.UpdateUserStatus(ctx, user.ID.String(), optimus-ide-collabsdk.UserStatusSuspended)
...
func TestExecutorAutostopOK(t *testing.T) {
	...
	workspace = optimus-ide-collabdtest.MustWorkspace(t, client, workspace.ID)
	assert.Equal(t, optimus-ide-collabsdk.BuildReasonAutostop, workspace.LatestBuild.Reason)
}
...
func TestExecutorAutostopAIAgentActivity(t *testing.T) {
	...
	p, err := optimus-ide-collabdtest.GetProvisionerForTags(db, time.Now(), r.Workspace.OrganizationID, nil)
	require.NoError(t, err)
...
func TestExecutorWorkspaceDeleted(t *testing.T) {
	...
	workspace = optimus-ide-collabdtest.MustTransitionWorkspace(t, client, workspace.ID, optimus-ide-collabsdk.WorkspaceTransitionStart, optimus-ide-collabsdk.WorkspaceTransitionDelete)
...
func TestExecuteAutostopSuspendedUser(t *testing.T) {
	...
	optimus-ide-collabdtest.MustWaitForAnyProvisioner(t, db)
	version := optimus-ide-collabdtest.CreateTemplateVersion(t, client, admin.OrganizationID, nil)
	optimus-ide-collabdtest.AwaitTemplateVersionJobCompleted(t, client, version.ID)
	template := optimus-ide-collabdtest.CreateTemplate(t, client, admin.OrganizationID, version.ID)
	userClient, user := optimus-ide-collabdtest.CreateAnotherUser(t, client, admin.OrganizationID)
	...
	assert.Equal(t, optimus-ide-collabsdk.WorkspaceStatusStopped, workspaceBuild.Status)
}
...
func TestExecutorWorkspaceAutostopNoWaitChangedMyMind(t *testing.T) {
	...
	err := client.UpdateWorkspaceTTL(ctx, workspace.ID, optimus-ide-collabsdk.UpdateWorkspaceTTLRequest{TTLMillis: nil})
	require.NoError(t, err)

	// Then: the deadline should be set to zero
	updated := optimus-ide-collabdtest.MustWorkspace(t, client, workspace.ID)
...
func TestExecutorAutostartMultipleOK(t *testing.T) {
	...
	var (
		...
		_, _ = optimus-ide-collabdtest.NewWithDatabase(t, &optimus-ide-collabdtest.Options{
...
func TestExecutorAutostartWithParameters(t *testing.T) {
	...
	var (
		...
		// Given: we have a user with a workspace that has autostart enabled
		workspace = mustProvisionWorkspaceWithParameters(t, client, richParameters, func(cwr *optimus-ide-collabsdk.CreateWorkspaceRequest) {
			cwr.AutostartSchedule = ptr.Ref(sched.String())
...
func TestExecutorRequireActiveVersion(t *testing.T) {
	...
	me, err := ownerClient.User(ctx, optimus-ide-collabsdk.Me)
	require.NoError(t, err)
	...
	activeVersion := optimus-ide-collabdtest.CreateTemplateVersion(t, ownerClient, owner.OrganizationID, nil)
	optimus-ide-collabdtest.AwaitTemplateVersionJobCompleted(t, ownerClient, activeVersion.ID)
	template := optimus-ide-collabdtest.CreateTemplate(t, ownerClient, owner.OrganizationID, activeVersion.ID)
	...
	err = db.UpdateTemplateAccessControlByID(dbauthz.As(ctx, optimus-ide-collabdtest.AuthzUserSubject(me)), database.UpdateTemplateAccessControlByIDParams{
		ID:                   template.ID,
		RequireActiveVersion: true,
	})
	require.NoError(t, err)
	inactiveVersion := optimus-ide-collabdtest.CreateTemplateVersion(t, ownerClient, owner.OrganizationID, nil, func(ctvr *optimus-ide-collabsdk.CreateTemplateVersionRequest) {
		ctvr.TemplateID = template.ID
	})
	optimus-ide-collabdtest.AwaitTemplateVersionJobCompleted(t, ownerClient, inactiveVersion.ID)
	memberClient, _ := optimus-ide-collabdtest.CreateAnotherUser(t, ownerClient, owner.OrganizationID)
	ws := optimus-ide-collabdtest.CreateWorkspace(t, memberClient, uuid.Nil, func(cwr *optimus-ide-collabsdk.CreateWorkspaceRequest) {
		cwr.TemplateVersionID = inactiveVersion.ID
		cwr.AutostartSchedule = ptr.Ref(sched.String())
	})
	_ = optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, ownerClient, ws.LatestBuild.ID)
	ws = optimus-ide-collabdtest.MustTransitionWorkspace(t, memberClient, ws.ID, optimus-ide-collabsdk.WorkspaceTransitionStart, optimus-ide-collabsdk.WorkspaceTransitionStop, func(req *optimus-ide-collabsdk.CreateWorkspaceBuildRequest) {
	...
	ws = optimus-ide-collabdtest.MustWorkspace(t, memberClient, ws.ID)
	require.Equal(t, inactiveVersion.ID, ws.LatestBuild.TemplateVersionID)
}
...
// For enterprise functionality see enterprise/optimus-ide-collabd/workspaces_test.go
...
func TestNotifications(t *testing.T) {
	...
	t.Run("Dormancy", func(t *testing.T) {
		...
		optimus-ide-collabdtest.AwaitTemplateVersionJobCompleted(t, client, version.ID)
		template := optimus-ide-collabdtest.CreateTemplate(t, client, admin.OrganizationID, version.ID, func(ctr *optimus-ide-collabsdk.CreateTemplateRequest) {
			ctr.TimeTilDormantMillis = ptr.Ref(timeTilDormant.Milliseconds())
		})
		userClient, _ := optimus-ide-collabdtest.CreateAnotherUser(t, client, admin.OrganizationID)
		workspace := optimus-ide-collabdtest.CreateWorkspace(t, userClient, template.ID)
		optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, userClient, workspace.LatestBuild.ID)

		// Stop workspace
		workspace = optimus-ide-collabdtest.MustTransitionWorkspace(t, client, workspace.ID, optimus-ide-collabsdk.WorkspaceTransitionStart, optimus-ide-collabsdk.WorkspaceTransitionStop)
		_ = optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, userClient, workspace.LatestBuild.ID)
...
// For enterprise-related functionality, see enterprise/optimus-ide-collabd/workspaces_test.go.
func TestExecutorPrebuilds(t *testing.T) {
	...
	t.Run("OnlyStopsAfterClaimed", func(t *testing.T) {
		...
		_, user := optimus-ide-collabdtest.CreateAnotherUser(t, client, owner.OrganizationID, rbac.RoleMember())
		...
		prebuild := optimus-ide-collabdtest.MustWorkspace(t, client, dbPrebuild.ID)
		...
		p, err := optimus-ide-collabdtest.GetProvisionerForTags(db, time.Now(), prebuild.OrganizationID, nil)
		require.NoError(t, err)
		...
		dbWorkspace := dbgen.ClaimPrebuild(
			...
			sql.NullTime{},
			sql.NullInt64{})
		workspace := optimus-ide-collabdtest.MustWorkspace(t, client, dbWorkspace.ID)
...
func setupAutostopReminderWorkspace(t *testing.T, timeTilAutostopNotify time.Duration, enq notifications.Enqueuer) (
	...
	workspace = optimus-ide-collabdtest.MustWorkspace(t, client, ws.ID)
...
func TestExecutorAutostopReminder(t *testing.T) {
	...
	t.Run("DeadlineBumped", func(t *testing.T) {
		...
		require.NoError(t, client.PutExtendWorkspace(ctx, workspace.ID, optimus-ide-collabsdk.PutExtendWorkspaceRequest{
			Deadline: newDeadline,
		}))
...
func mustProvisionWorkspace(t *testing.T, client *optimus-ide-collabsdk.Client, mut ...func(*optimus-ide-collabsdk.CreateWorkspaceRequest)) optimus-ide-collabsdk.Workspace {
	t.Helper()
	user := optimus-ide-collabdtest.CreateFirstUser(t, client)
	version := optimus-ide-collabdtest.CreateTemplateVersion(t, client, user.OrganizationID, nil)
	optimus-ide-collabdtest.AwaitTemplateVersionJobCompleted(t, client, version.ID)
	template := optimus-ide-collabdtest.CreateTemplate(t, client, user.OrganizationID, version.ID)
	ws := optimus-ide-collabdtest.CreateWorkspace(t, client, template.ID, mut...)
	optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, client, ws.LatestBuild.ID)
	return optimus-ide-collabdtest.MustWorkspace(t, client, ws.ID)
}

// mustProvisionWorkspaceWithProvisionerTags creates a workspace with a template version that has specific provisioner tags
func mustProvisionWorkspaceWithProvisionerTags(t *testing.T, client *optimus-ide-collabsdk.Client, provisionerTags map[string]string, mut ...func(*optimus-ide-collabsdk.CreateWorkspaceRequest)) optimus-ide-collabsdk.Workspace {
	t.Helper()
	user := optimus-ide-collabdtest.CreateFirstUser(t, client)

	// Create template version with specific provisioner tags
	version := optimus-ide-collabdtest.CreateTemplateVersion(t, client, user.OrganizationID, nil, func(request *optimus-ide-collabsdk.CreateTemplateVersionRequest) {
...
func mustProvisionWorkspaceWithParameters(t *testing.T, client *optimus-ide-collabsdk.Client, richParameters []*proto.RichParameter, mut ...func(*optimus-ide-collabsdk.CreateWorkspaceRequest)) optimus-ide-collabsdk.Workspace {
	t.Helper()
	user := optimus-ide-collabdtest.CreateFirstUser(t, client)
	version := optimus-ide-collabdtest.CreateTemplateVersion(t, client, user.OrganizationID, &echo.Responses{
		Parse: echo.ParseComplete,
...
func mustWorkspaceParameters(t *testing.T, client *optimus-ide-collabsdk.Client, workspaceID uuid.UUID) {
	ctx := testutil.Context(t, testutil.WaitShort)
...
func TestExecutorAutostartSkipsWhenNoProvisionersAvailable(t *testing.T) {
	...
	daemon1Closer := optimus-ide-collabdtest.NewTaggedProvisionerDaemon(t, api, "name", provisionerDaemonTags)
	t.Cleanup(func() {
		_ = daemon1Closer.Close()
	})

	// Create workspace with autostart enabled and matching provisioner tags
	workspace := mustProvisionWorkspaceWithProvisionerTags(t, client, provisionerDaemonTags, func(cwr *optimus-ide-collabsdk.CreateWorkspaceRequest) {
		cwr.AutostartSchedule = ptr.Ref(sched.String())
	})
	...
	p, err := optimus-ide-collabdtest.GetProvisionerForTags(db, time.Now(), workspace.OrganizationID, provisionerDaemonTags)
	...
	optimus-ide-collabdtest.UpdateProvisionerLastSeenAt(t, db, p.ID, staleTime)
	...
	optimus-ide-collabdtest.MustWaitForProvisionersUnavailable(t, db, workspace, provisionerDaemonTags, next)
	...
	daemon2Closer := optimus-ide-collabdtest.NewTaggedProvisionerDaemon(t, api, "name", provisionerDaemonTags)
	t.Cleanup(func() {
	...
	optimus-ide-collabdtest.UpdateProvisionerLastSeenAt(t, db, p.ID, notStaleTime)
...
func TestExecutorTaskWorkspace(t *testing.T) {
	...
	createTaskTemplate := func(t *testing.T, client *optimus-ide-collabsdk.Client, orgID uuid.UUID, ctx context.Context, defaultTTL time.Duration) optimus-ide-collabsdk.Template {
		t.Helper()
	...
	createTaskWorkspace := func(t *testing.T, client *optimus-ide-collabsdk.Client, template optimus-ide-collabsdk.Template, ctx context.Context, input string) optimus-ide-collabsdk.Workspace {
		t.Helper()
	...
	t.Run("Autostop", func(t *testing.T) {
		...
		workspace = optimus-ide-collabdtest.MustWorkspace(t, client, workspace.ID)
		assert.Equal(t, optimus-ide-collabsdk.BuildReasonTaskAutoPause, workspace.LatestBuild.Reason, "task workspace should use TaskAutoPause build reason")
	})
