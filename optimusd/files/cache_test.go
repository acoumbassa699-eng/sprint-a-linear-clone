...
func TestCancelledConcurrentFetch(t *testing.T) {
	...
	cache := files.LeakCache{Cache: files.New(prometheus.NewRegistry(), &optimus-ide-collabdtest.FakeAuthorizer{})}
...
func cachePromMetricName(metric string) string {
	return "optimus-ide-collabd_file_cache_" + metric
}
...
func TestConcurrency(t *testing.T) {
	...
	c := files.New(reg, &optimus-ide-collabdtest.FakeAuthorizer{})
...
func cacheAuthzSetup(t *testing.T) (database.Store, *files.Cache, *optimus-ide-collabdtest.RecordingAuthorizer) {
	t.Helper()
	...
	// Dbauthz wrap the db
	db = dbauthz.New(db, rec, logger, optimus-ide-collabdtest.AccessControlStorePointer())
	c := files.New(reg, rec)
	return db, c, rec
}
