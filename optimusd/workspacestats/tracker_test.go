...
func TestTracker_MultipleInstances(t *testing.T) {
	...
	// Given we have two optimus-ide-collabd instances connected to the same database
	var (
		...
		clientB = optimus-ide-collabdtest.New(t, &optimus-ide-collabdtest.Options{
		...
		owner = optimus-ide-collabdtest.CreateFirstUser(t, clientA)
		now   = dbtime.Now()
	)
	...
	updated := make([]optimus-ide-collabsdk.Workspace, numWorkspaces)
	for i := 0; i < numWorkspaces; i++ {
