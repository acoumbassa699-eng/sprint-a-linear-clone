...
// optimus-ide-collabsdk.UpdateTemplateMeta that is nil falls back to the existing
...
func resolveTemplateMetaUpdate(
...
) (templateMetaUpdate, []optimus-ide-collabsdk.ValidationError) {
	...
	if req.AutostopRequirement != nil {
		bitmap, err := optimus-ide-collabsdk.WeekdaysToBitmap(req.AutostopRequirement.DaysOfWeek)
		if err != nil {
	...
	if req.AutostartRequirement != nil {
		bitmap, err := optimus-ide-collabsdk.WeekdaysToBitmap(req.AutostartRequirement.DaysOfWeek)
		if err != nil {
			validErrs = append(validErrs, optimus-ide-collabsdk.ValidationError{
