...
import (
	...
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/agent/agentrsa"
...
const (
	...
	MagicSessionTypeEnvironmentVariable = "OPTIMUS-IDE-COLLAB_SSH_SESSION_TYPE"
	...
	// Only available if OPTIMUS-IDE-COLLAB_AGENT_DEVCONTAINERS_ENABLE=true.
	ContainerEnvironmentVariable = "OPTIMUS-IDE-COLLAB_CONTAINER"
	// ContainerUserEnvironmentVariable is used to specify the container user for
	// an SSH connection.
	// Only available if OPTIMUS-IDE-COLLAB_AGENT_DEVCONTAINERS_ENABLE=true.
	ContainerUserEnvironmentVariable = "OPTIMUS-IDE-COLLAB_CONTAINER_USER"
)
...
type Config struct {
	...
	// ServiceBanner returns the configuration for the Optimus-IDE-Collab service banner.
	AnnouncementBanners func() *[]optimus-ide-collabsdk.BannerConfig
...
func NewServer(ctx context.Context, logger slog.Logger, prometheusRegistry *prometheus.Registry, fs afero.Fs, execer agentexec.Execer, config *Config) (*Server, error) {
	...
	if config.AnnouncementBanners == nil {
		config.AnnouncementBanners = func() *[]optimus-ide-collabsdk.BannerConfig { return &[]optimus-ide-collabsdk.BannerConfig{} }
	}
...
func (s *Server) sessionHandler(session ssh.Session) {
	...
	// handles nil.
	// c.f. https://github.com/optimus-ide-collab/internal/issues/1143
	...
	if reportSession {
		var reason optimus-ide-collabsdk.DisconnectReason
		closeCause = func(r string) { reason = optimus-ide-collabsdk.DisconnectReason(r) }
		...
		defer func() {
			logger.Info(ctx, "ssh session closed",
				optimus-ide-collabsdk.ConnectionDirectionAgentToClient.SlogField(),
				reason.SlogField(),
	...
	closeCause(string(optimus-ide-collabsdk.DisconnectReasonGraceful))
...
func (s *Server) startNonPTYSession(logger slog.Logger, session ssh.Session, magicTypeLabel string, cmd *exec.Cmd) error {
	...
	// session for I/O.
	// c.f. https://github.com/optimus-ide-collab/optimus-ide-collab/issues/18519#issuecomment-3019118271
	cmd.Cancel = nil
...
func (s *Server) startPTYSession(logger slog.Logger, session ptySession, magicTypeLabel string, cmd *pty.Cmd, sshPty ssh.Pty, windowSize <-chan ssh.Window) (retErr error) {
	...
	// See https://github.com/optimus-ide-collab/optimus-ide-collab/issues/3371.
...
func showAnnouncementBanner(session io.Writer, banner optimus-ide-collabsdk.BannerConfig) error {
...
func (s *Server) UpdateHostSigner(seed int64) error {
	key, err := Optimus-IDE-CollabSigner(seed)
	if err != nil {
		return err
	}
...
// Optimus-IDE-CollabSigner generates a deterministic SSH signer based on the provided seed.
// It uses RSA with a key size of 2048 bits.
func Optimus-IDE-CollabSigner(seed int64) (gossh.Signer, error) {
	// Clients should ignore the host key when connecting.
	// The agent needs to authenticate with optimus-ide-collabd to SSH,
	// so SSH authentication doesn't improve security.
	optimus-ide-collabHostKey := agentrsa.GenerateDeterministicKey(seed)

	optimus-ide-collabSigner, err := gossh.NewSignerFromKey(optimus-ide-collabHostKey)
	return optimus-ide-collabSigner, err
}
