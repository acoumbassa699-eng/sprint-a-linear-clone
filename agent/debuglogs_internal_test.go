...
func TestHandleHTTPDebugLogsWithAfterCapsResponse(t *testing.T) {
	...
	logDir := t.TempDir()
	activePath := filepath.Join(logDir, "optimus-ide-collab-agent.log")
	...
	rotatedPath := filepath.Join(logDir, "optimus-ide-collab-agent-2026-05-17T20-00-00.000.log")
...
func TestHandleHTTPDebugLogsWithAfterOpenFailure(t *testing.T) {
	...
	logDir, err := os.MkdirTemp("/tmp", "optimus-ide-collab-debuglogs-")
	require.NoError(t, err)
	t.Cleanup(func() {
...
func TestRotatedAgentLogFilesReadsLogDirLiterally(t *testing.T) {
	...
	activePath := filepath.Join(logDir, "optimus-ide-collab-agent.log")
	rotatedPath := filepath.Join(logDir, "optimus-ide-collab-agent-2026-05-18T00-00-00.000.log")
	...
	require.Equal(t, "optimus-ide-collab-agent-2026-05-18T00-00-00.000.log", files[0].name)
}
