...
func (a *API) handleWatch(rw http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		case msg, ok := <-updates:
			...
			switch msg.Type {
			case optimus-ide-collabsdk.WorkspaceAgentGitClientMessageTypeRefresh:
				handler.RequestScan()
			default:
				if err := stream.Send(optimus-ide-collabsdk.WorkspaceAgentGitServerMessage{
					Type:    optimus-ide-collabsdk.WorkspaceAgentGitServerMessageTypeError,
