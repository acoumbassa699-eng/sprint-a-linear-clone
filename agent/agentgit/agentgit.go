...
func (h *Handler) Scan(ctx context.Context) *optimus-ide-collabsdk.WorkspaceAgentGitServerMessage {
	if !h.gitAvailable() {
		return nil
	}
	...
	for _, res := range results {
		if res.err != nil {
			if isRepoDeleted(h.gitBin, res.root) {
				...
				removal := optimus-ide-collabsdk.WorkspaceAgentRepoChanges{
					RepoRoot: res.root,
					Removed:  true,
				}
...
func getRepoChanges(ctx context.Context, logger slog.Logger, gitBin string, repoRoot string) (optimus-ide-collabsdk.WorkspaceAgentRepoChanges, error) {
	result := optimus-ide-collabsdk.WorkspaceAgentRepoChanges{
		RepoRoot: repoRoot,
	}
