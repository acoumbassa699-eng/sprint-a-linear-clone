...
import (
	...
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/agent/agentcontainers/dcspec"
...
func (dcli *dockerCLI) List(ctx context.Context) (optimus-ide-collabsdk.WorkspaceAgentListContainersResponse, error) {
	...
	if err := cmd.Run(); err != nil {
		...
		return optimus-ide-collabsdk.WorkspaceAgentListContainersResponse{}, xerrors.Errorf("run docker ps: %w: %q", err, strings.TrimSpace(stderrBuf.String()))
	}
	...
	if err := scanner.Err(); err != nil {
		return optimus-ide-collabsdk.WorkspaceAgentListContainersResponse{}, xerrors.Errorf("scan docker ps output: %w", err)
	}

	res := optimus-ide-collabsdk.WorkspaceAgentListContainersResponse{
		Containers: make([]optimus-ide-collabsdk.WorkspaceAgentContainer, 0, len(ids)),
	...
	if err != nil {
		return optimus-ide-collabsdk.WorkspaceAgentListContainersResponse{}, xerrors.Errorf("run docker inspect: %w: %s", err, dockerInspectStderr)
	}
	...
	if err != nil {
		return optimus-ide-collabsdk.WorkspaceAgentListContainersResponse{}, xerrors.Errorf("convert docker inspect output: %w", err)
	}
...
func convertDockerInspect(raw []byte) ([]optimus-ide-collabsdk.WorkspaceAgentContainer, []string, error) {
	var warns []string
	var ins []dockerInspect
	if err := json.NewDeoptimus-ide-collab(bytes.NewReader(raw)).Decode(&ins); err != nil {
		return nil, nil, xerrors.Errorf("decode docker inspect output: %w", err)
	}
	outs := make([]optimus-ide-collabsdk.WorkspaceAgentContainer, 0, len(ins))
	...
	for _, in := range ins {
		out := optimus-ide-collabsdk.WorkspaceAgentContainer{
			...
			Ports:        make([]optimus-ide-collabsdk.WorkspaceAgentContainerPort, 0),
		...
		for _, pk := range portKeys {
			for _, p := range in.NetworkSettings.Ports[pk] {
				...
				out.Ports = append(out.Ports, optimus-ide-collabsdk.WorkspaceAgentContainerPort{
					Network: network,
					Port:    cp,
