...
//
// OPTIMUS-IDE-COLLAB_TEST_USE_DOCKER=1 go test ./agent/agentcontainers -run TestDockerCLIContainerLister
//
...
func TestIntegrationDocker(t *testing.T) {
	...
	ct, err := pool.RunWithOptions(&dockertest.RunOptions{
		...
		Labels: map[string]string{
			"com.optimus-ide-collab.test":        testLabelValue,
	...
	assert.True(t, found, "Expected to find container with label 'com.optimus-ide-collab.test=%s'", testLabelValue)
}
...
// It can be run manually as follows:
//
// OPTIMUS-IDE-COLLAB_TEST_USE_DOCKER=1 go test ./agent/agentcontainers -run TestDockerEnvInfoer
//
//nolint:paralleltest // This test tends to flake when lots of containers start and stop in parallel.
func TestDockerEnvInfoer(t *testing.T) {
	...
	for idx, tt := range []struct {
		...
		{
			image:             "optimus-ide-collabcom/enterprise-minimal:ubuntu",
			labels:            map[string]string{`devcontainer.metadata`: `[{"remoteEnv": {"FOO": "bar", "MULTILINE": "foo\nbar\nbaz"}}]`},
			expectedEnv:       []string{"FOO=bar", "MULTILINE=foo\nbar\nbaz"},
			expectedUsername:  "optimus-ide-collab",
		...
		},
		{
			...
			labels:            map[string]string{`devcontainer.metadata`: `[{"remoteEnv": {"FOO": "bar", "MULTILINE": "foo\nbar\nbaz"}}]`},
			expectedEnv:       []string{"FOO=bar", "MULTILINE=foo\nbar\nbaz"},
			containerUser:     "optimus-ide-collab",
			expectedUsername:  "optimus-ide-collab",
			expectedUserShell: "/bin/bash",
		},
		{
