...
func TestDevcontainerCLI_ArgsAndParsing(t *testing.T) {
	...
	t.Run("ReadConfig", func(t *testing.T) {
		...
		tests := []struct {
			...
			{
				name:            "WithOptimus-IDE-CollabCustomization",
				logFile:         "read-config-with-optimus-ide-collab-customization.log",
				workspaceFolder: "/test/workspace",
				configPath:      "",
			...
			{
				name:            "WithoutOptimus-IDE-CollabCustomization",
				logFile:         "read-config-without-optimus-ide-collab-customization.log",
				workspaceFolder: "/test/workspace",
				configPath:      "/test/config.json",
...
//
//	OPTIMUS-IDE-COLLAB_TEST_USE_DOCKER=1 go test ./agent/agentcontainers -run TestDockerDevcontainerCLI
//
...
func TestDockerDevcontainerCLI(t *testing.T) {
	...
	if os.Getenv("OPTIMUS-IDE-COLLAB_TEST_USE_DOCKER") != "1" {
		t.Skip("skipping Docker test; set OPTIMUS-IDE-COLLAB_TEST_USE_DOCKER=1 to run")
	}
...
func setupDevcontainerWorkspace(t *testing.T, workspaceFolder string) string {
	...
	"runArgs": ["--label=com.optimus-ide-collab.test=devcontainercli", "--label=` + agentcontainers.DevcontainerIsTestRunLabel + `=true"]
}`
...
func findDevcontainerByID(t *testing.T, pool *dockertest.Pool, id string) (*docker.Container, bool) {
	...
	require.Equal(t, "devcontainercli", container.Config.Labels["com.optimus-ide-collab.test"], "sanity check failed: container should have the test label")
