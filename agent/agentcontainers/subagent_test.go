	...
	t.Run("CreateWithDisplayApps", func(t *testing.T) {
		...
		tests := []struct {
		...
		}{
			{
				...
				displayApps: []optimus-ide-collabsdk.DisplayApp{optimus-ide-collabsdk.DisplayAppVSCodeDesktop},
				expectedApps: []agentproto.CreateSubAgentRequest_DisplayApp{
		...
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				...
				subAgent, err := subAgentClient.Create(ctx, agentcontainers.SubAgent{
					...
					Directory:       "/workspaces/optimus-ide-collab",
					Architecture:    "amd64",
			...
			},
			{
				...
				apps: []agentcontainers.SubAgentApp{
					...
					},
					{
						...
						Share: optimus-ide-collabsdk.WorkspaceAppSharingLevelOrganization,
					},
				},
				expectedApps: []*agentproto.CreateSubAgentRequest_App{
					{
...
func TestSubAgent_EqualConfig(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			modify:    func(s *agentcontainers.SubAgent) { s.DisplayApps = []optimus-ide-collabsdk.DisplayApp{optimus-ide-collabsdk.DisplayAppSSH} },
			wantEqual: false,
		},
		{
			name: "different Apps",
