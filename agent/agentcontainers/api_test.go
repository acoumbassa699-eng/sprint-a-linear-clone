...
import (
	...
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/agent/agentcontainers/acmock"
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/agent/agentcontainers/watcher"
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/agent/usershell"
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabd/util/slice"
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabsdk"
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/pty"
...
type fakeContainerCLI struct {
	mu         sync.Mutex
	containers optimus-ide-collabsdk.WorkspaceAgentListContainersResponse
	listErr    error
	arch       string
	archErr    error
	copyErr    error
...
func (f *fakeContainerCLI) List(_ context.Context) (optimus-ide-collabsdk.WorkspaceAgentListContainersResponse, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
...
func (f *fakeContainerCLI) Stop(ctx context.Context, name string) error {
	...
	f.containers.Devcontainers = slice.Filter(f.containers.Devcontainers, func(dc optimus-ide-collabsdk.WorkspaceAgentDevcontainer) bool {
...
func (f *fakeContainerCLI) Remove(ctx context.Context, name string) error {
	...
	f.containers.Containers = slice.Filter(f.containers.Containers, func(container optimus-ide-collabsdk.WorkspaceAgentContainer) bool {
...
func expectSubAgentInjection(
	...
	containerID string,
	arch string,
	optimus-ide-collabBin string,
) {
	gomock.InOrder(
		mCCLI.EXPECT().DetectArchitecture(gomock.Any(), containerID).Return(arch, nil),
		mCCLI.EXPECT().ExecAs(gomock.Any(), containerID, "root", "mkdir", "-p", "/.optimus-ide-collab-agent").Return(nil, nil),
		mCCLI.EXPECT().Copy(gomock.Any(), containerID, optimus-ide-collabBin, "/.optimus-ide-collab-agent/optimus-ide-collab").Return(nil),
		mCCLI.EXPECT().ExecAs(gomock.Any(), containerID, "root", "chmod", "0755", "/.optimus-ide-collab-agent", "/.optimus-ide-collab-agent/optimus-ide-collab").Return(nil, nil),
		mCCLI.EXPECT().ExecAs(gomock.Any(), containerID, "root", "/bin/sh", "-c", "chown $(id -u):$(id -g) /.optimus-ide-collab-agent/optimus-ide-collab").Return(nil, nil),
	)
}
...
func TestAPI(t *testing.T) {
	...
	t.Run("Watch", func(t *testing.T) {
		...
		fakeContainer1 := fakeContainer(t, func(c *optimus-ide-collabsdk.WorkspaceAgentContainer) {
			c.ID = "container1"
			c.FriendlyName = "devcontainer1"
			c.Image = "busybox:latest"
			c.Labels = map[string]string{
				agentcontainers.DevcontainerLocalFolderLabel: "/home/optimus-ide-collab/project1",
				agentcontainers.DevcontainerConfigFileLabel:  "/home/optimus-ide-collab/project1/.devcontainer/devcontainer.json",
			}
		})

		fakeContainer2 := fakeContainer(t, func(c *optimus-ide-collabsdk.WorkspaceAgentContainer) {
			c.ID = "container2"
			c.FriendlyName = "devcontainer2"
			c.Image = "ubuntu:latest"
			c.Labels = map[string]string{
				agentcontainers.DevcontainerLocalFolderLabel: "/home/optimus-ide-collab/project2",
				agentcontainers.DevcontainerConfigFileLabel:  "/home/optimus-ide-collab/project2/.devcontainer/devcontainer.json",
			}
		})

		stages := []struct {
			containers []optimus-ide-collabsdk.WorkspaceAgentContainer
			expected   optimus-ide-collabsdk.WorkspaceAgentListContainersResponse
		}{
			{
				containers: []optimus-ide-collabsdk.WorkspaceAgentContainer{fakeContainer1},
				expected: optimus-ide-collabsdk.WorkspaceAgentListContainersResponse{
					Containers: []optimus-ide-collabsdk.WorkspaceAgentContainer{fakeContainer1},
			...
			{
				containers: []optimus-ide-collabsdk.WorkspaceAgentContainer{fakeContainer1, fakeContainer2},
				expected: optimus-ide-collabsdk.WorkspaceAgentListContainersResponse{
					Containers: []optimus-ide-collabsdk.WorkspaceAgentContainer{fakeContainer1, fakeContainer2},
					Devcontainers: []optimus-ide-collabsdk.WorkspaceAgentDevcontainer{
						{
			...
			{
				containers: []optimus-ide-collabsdk.WorkspaceAgentContainer{fakeContainer2},
				expected: optimus-ide-collabsdk.WorkspaceAgentListContainersResponse{
					Containers: []optimus-ide-collabsdk.WorkspaceAgentContainer{fakeContainer2},
					Devcontainers: []optimus-ide-collabsdk.WorkspaceAgentDevcontainer{
						{
		...
		mLister.EXPECT().List(gomock.Any()).Return(optimus-ide-collabsdk.WorkspaceAgentListContainersResponse{Containers: stages[0].containers}, nil)
		...
		var got optimus-ide-collabsdk.WorkspaceAgentListContainersResponse
		...
		for i, stage := range stages[1:] {
			mLister.EXPECT().List(gomock.Any()).Return(optimus-ide-collabsdk.WorkspaceAgentListContainersResponse{Containers: stage.containers}, nil)
	...
	t.Run("List", func(t *testing.T) {
		...
		makeResponse := func(cts ...optimus-ide-collabsdk.WorkspaceAgentContainer) optimus-ide-collabsdk.WorkspaceAgentListContainersResponse {
			return optimus-ide-collabsdk.WorkspaceAgentListContainersResponse{Containers: cts}
		}

		type initialDataPayload struct {
			val optimus-ide-collabsdk.WorkspaceAgentListContainersResponse
			err error
		}
		...
		for _, tc := range []struct {
			...
			t.Run(tc.name, func(t *testing.T) {
				...
				if tc.initialData.err != nil {
					got := &optimus-ide-collabsdk.Error{}
					err := json.NewDeoptimus-ide-collab(rec.Body).Decode(got)
	...
	t.Run("Recreate", func(t *testing.T) {
		...
		devContainer1 := optimus-ide-collabsdk.WorkspaceAgentContainer{
			ID:           "container-1",
		...
		devContainer2 := optimus-ide-collabsdk.WorkspaceAgentContainer{
			ID:           "container-2",
		...
		tests := []struct {
			...
			setupDevcontainers []optimus-ide-collabsdk.WorkspaceAgentDevcontainer
			...
			},
			{
				...
				lister: &fakeContainerCLI{
					containers: optimus-ide-collabsdk.WorkspaceAgentListContainersResponse{
						Containers: []optimus-ide-collabsdk.WorkspaceAgentContainer{devContainer1},
					},
					arch: "<none>", // Unsupported architecture, don't inject subagent.
				},
				devcontainerCLI: &fakeDevcontainerCLI{
			...
			},
			{
				...
				lister: &fakeContainerCLI{
					containers: optimus-ide-collabsdk.WorkspaceAgentListContainersResponse{
						Containers: []optimus-ide-collabsdk.WorkspaceAgentContainer{devContainer2},
					},
					arch: "<none>", // Unsupported architecture, don't inject subagent.
				},
				devcontainerCLI: &fakeDevcontainerCLI{},
		...
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				...
				var resp optimus-ide-collabsdk.WorkspaceAgentListContainersResponse
				t.Log(rec.Body.String())
				err := json.NewDeoptimus-ide-collab(rec.Body).Decode(&resp)
				require.NoError(t, err, "unmarshal response failed")
				require.Len(t, resp.Devcontainers, 1, "expected one devcontainer in response")
				assert.Equal(t, optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatusStarting, resp.Devcontainers[0].Status, "devcontainer is not starting")
				...
				if tt.devcontainerCLI.upErr != nil {
					...
					assert.Equal(t, optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatusError, resp.Devcontainers[0].Status, "devcontainer is not in an error state after up failure")
				...
				assert.Equal(t, optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatusRunning, resp.Devcontainers[0].Status, "devcontainer is not running after recreation")
				...
				setupDevcontainers: []optimus-ide-collabsdk.WorkspaceAgentDevcontainer{
					{
						...
						WorkspaceFolder: workspaceFolder1,
						ConfigPath:      configPath1,
						Status:          optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatusDeleting,
						Container:       &devContainer1,
					},
				},
				lister: &fakeContainerCLI{
		...
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				...
				if withSubAgent {
					...
					optimus-ide-collabBin, err := os.Executable()
					require.NoError(t, err)
					optimus-ide-collabBin, err = filepath.EvalSymlinks(optimus-ide-collabBin)
					require.NoError(t, err)

					mCCLI.EXPECT().List(gomock.Any()).Return(optimus-ide-collabsdk.WorkspaceAgentListContainersResponse{
						Containers: tt.lister.containers.Containers,
					}, nil).AnyTimes()
					expectSubAgentInjection(mCCLI, devContainer1.ID, runtime.GOARCH, optimus-ide-collabBin)
	...
	t.Run("List devcontainers", func(t *testing.T) {
		...
		knownDevcontainers := []optimus-ide-collabsdk.WorkspaceAgentDevcontainer{
			{
		...
		tests := []struct {
			...
			verify             func(t *testing.T, devcontainers []optimus-ide-collabsdk.WorkspaceAgentDevcontainer)
		}{
			{
				name: "List error",
				lister: &fakeContainerCLI{
			...
			},
			{
				...
				verify: func(t *testing.T, devcontainers []optimus-ide-collabsdk.WorkspaceAgentDevcontainer) {
					for _, dc := range devcontainers {
						assert.Equal(t, optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatusStopped, dc.Status, "devcontainer should be stopped")
			...
			},
			{
				...
				verify: func(t *testing.T, devcontainers []optimus-ide-collabsdk.WorkspaceAgentDevcontainer) {
					...
					assert.Equal(t, optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatusRunning, dc.Status)
			...
			},
			{
				...
				verify: func(t *testing.T, devcontainers []optimus-ide-collabsdk.WorkspaceAgentDevcontainer) {
					...
					assert.Equal(t, optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatusRunning, known1.Status)
					assert.Equal(t, optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatusStopped, known2.Status)
					assert.Equal(t, optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatusRunning, runtime1.Status)
			...
			},
			{
				...
				verify: func(t *testing.T, devcontainers []optimus-ide-collabsdk.WorkspaceAgentDevcontainer) {
					...
					assert.Equal(t, optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatusRunning, running.Status)
					assert.Equal(t, optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatusStopped, nonRunning.Status)
			...
			},
			{
				...
				wantCount:          2,
				verify: func(t *testing.T, devcontainers []optimus-ide-collabsdk.WorkspaceAgentDevcontainer) {
					var dc2 *optimus-ide-collabsdk.WorkspaceAgentDevcontainer
					...
					assert.Equal(t, optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatusRunning, dc2.Status)
		...
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				...
				tt.lister.containers.Containers = append(tt.lister.containers.Containers, optimus-ide-collabsdk.WorkspaceAgentContainer{
					ID:           "test-container-1",
					FriendlyName: "test-container-1",
					Running:      true,
					Labels: map[string]string{
				...
				var scripts []optimus-ide-collabsdk.WorkspaceAgentScript
				for i := range tt.knownDevcontainers {
					scripts = append(scripts, optimus-ide-collabsdk.WorkspaceAgentScript{
				...
				var response optimus-ide-collabsdk.WorkspaceAgentListContainersResponse
				err := json.NewDeoptimus-ide-collab(rec.Body).Decode(&response)
				require.NoError(t, err, "unmarshal response failed")
	...
	t.Run("List devcontainers running then not running", func(t *testing.T) {
		...
		container := optimus-ide-collabsdk.WorkspaceAgentContainer{
			...
			Labels: map[string]string{
				agentcontainers.DevcontainerLocalFolderLabel: "/home/optimus-ide-collab/project",
				agentcontainers.DevcontainerConfigFileLabel:  "/home/optimus-ide-collab/project/.devcontainer/devcontainer.json",
			},
		}
		dc := optimus-ide-collabsdk.WorkspaceAgentDevcontainer{
			ID:              uuid.New(),
			Name:            "test-devcontainer",
			WorkspaceFolder: "/home/optimus-ide-collab/project",
			ConfigPath:      "/home/optimus-ide-collab/project/.devcontainer/devcontainer.json",
			Status:          optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatusRunning, // Corrected enum
		}
		...
		fLister := &fakeContainerCLI{
			containers: optimus-ide-collabsdk.WorkspaceAgentListContainersResponse{
				Containers: []optimus-ide-collabsdk.WorkspaceAgentContainer{container},
			},
		}
		fWatcher := newFakeWatcher(t)
		mClock := quartz.NewMock(t)
		...
		api := agentcontainers.NewAPI(logger,
			...
			agentcontainers.WithDevcontainers(
				[]optimus-ide-collabsdk.WorkspaceAgentDevcontainer{dc},
				[]optimus-ide-collabsdk.WorkspaceAgentScript{{LogSourceID: uuid.New(), ID: dc.ID}},
			),
		)
		api.Start()
		defer api.Close()
		...
		fWatcher.sendEventWaitNextCalled(ctx, fsnotify.Event{
			Name: "/home/optimus-ide-collab/project/.devcontainer/devcontainer.json",
			Op:   fsnotify.Write,
		})
		...
		var resp1 optimus-ide-collabsdk.WorkspaceAgentListContainersResponse
		err := json.NewDeoptimus-ide-collab(rec.Body).Decode(&resp1)
		require.NoError(t, err)
		require.Len(t, resp1.Devcontainers, 1)
		require.Equal(t, optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatusRunning, resp1.Devcontainers[0].Status, "devcontainer should be running initially")
		...
		// running.
		fLister.containers.Containers = []optimus-ide-collabsdk.WorkspaceAgentContainer{}
		...
		require.Equal(t, http.StatusOK, rec.Code)
		var resp2 optimus-ide-collabsdk.WorkspaceAgentListContainersResponse
		err = json.NewDeoptimus-ide-collab(rec.Body).Decode(&resp2)
		require.NoError(t, err)
		require.Len(t, resp2.Devcontainers, 1)
		require.Equal(t, optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatusStopped, resp2.Devcontainers[0].Status, "devcontainer should not be running after empty list")
	...
	t.Run("FileWatcher", func(t *testing.T) {
		...
		assert.Equal(t, optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatusRunning, response.Devcontainers[0].Status, "devcontainer should be running initially")
		...
		assert.Equal(t, optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatusRunning, response.Devcontainers[0].Status, "devcontainer should be running after config file was modified")
		require.NotNil(t, response.Devcontainers[0].Container, "container should not be nil")
		...
		fLister.containers.Containers = []optimus-ide-collabsdk.WorkspaceAgentContainer{container}
		...
		assert.Equal(t, optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatusRunning, response.Devcontainers[0].Status, "devcontainer should be running after recreation")
		require.NotNil(t, response.Devcontainers[0].Container, "container should not be nil")
	})
	...
	t.Run("SubAgentLifecycle", func(t *testing.T) {
		...
		var (
			...
			fakeDCCLI, cleanupDCCLI = newFakeDevcontainerCLI(t, agentcontainers.DevcontainerConfig{
				Workspace: agentcontainers.DevcontainerWorkspace{
					WorkspaceFolder: "/workspaces/optimus-ide-collab",
				},
			})

			testContainer = optimus-ide-collabsdk.WorkspaceAgentContainer{
				...
				CreatedAt:    time.Now(),
				Labels: map[string]string{
					agentcontainers.DevcontainerLocalFolderLabel: "/home/optimus-ide-collab/optimus-ide-collab",
					agentcontainers.DevcontainerConfigFileLabel:  "/home/optimus-ide-collab/optimus-ide-collab/.devcontainer/devcontainer.json",
				},
			}
		)
		...
		mCCLI.EXPECT().List(gomock.Any()).Return(optimus-ide-collabsdk.WorkspaceAgentListContainersResponse{
			Containers: []optimus-ide-collabsdk.WorkspaceAgentContainer{testContainer},
		}, nil).Times(3) // 1 initial call + 2 updates.
		expectSubAgentInjection(mCCLI, "test-container-id", runtime.GOARCH, optimus-ide-collabBin)
		...
		testutil.RequireSend(ctx, t, fakeDCCLI.readConfigErrC, func(envs []string) error {
			assert.Contains(t, envs, "OPTIMUS-IDE-COLLAB_WORKSPACE_AGENT_NAME=optimus-ide-collab")
			assert.Contains(t, envs, "OPTIMUS-IDE-COLLAB_WORKSPACE_NAME=test-workspace")
			assert.Contains(t, envs, "OPTIMUS-IDE-COLLAB_WORKSPACE_OWNER_NAME=test-user")
			assert.Contains(t, envs, "OPTIMUS-IDE-COLLAB_WORKSPACE_PARENT_AGENT_NAME=test-parent-agent")
			assert.Contains(t, envs, "OPTIMUS-IDE-COLLAB_URL=test-subagent-url")
		...
		assert.Equal(t, "optimus-ide-collab", fakeSAC.created[0].Name)
		assert.Equal(t, "/workspaces/optimus-ide-collab", fakeSAC.created[0].Directory)
	...
	WaitStartLoop:
		...
		gomock.InOrder(
			...
			mCCLI.EXPECT().ExecAs(gomock.Any(), "new-test-container-id", "root", "mkdir", "-p", "/.optimus-ide-collab-agent").Return(nil, nil),
			mCCLI.EXPECT().Copy(gomock.Any(), "new-test-container-id", optimus-ide-collabBin, "/.optimus-ide-collab-agent/optimus-ide-collab").Return(nil),
			mCCLI.EXPECT().ExecAs(gomock.Any(), "new-test-container-id", "root", "chmod", "0755", "/.optimus-ide-collab-agent", "/.optimus-ide-collab-agent/optimus-ide-collab").Return(nil, nil),
			mCCLI.EXPECT().ExecAs(gomock.Any(), "new-test-container-id", "root", "/bin/sh", "-c", "chown $(id -u):$(id -g) /.optimus-ide-collab-agent/optimus-ide-collab").Return(nil, nil),
		)

		fakeDCCLI.readConfig.MergedConfiguration.Customizations.Optimus-IDE-Collab = []agentcontainers.Optimus-IDE-CollabCustomization{
			{
				DisplayApps: map[optimus-ide-collabsdk.DisplayApp]bool{
					optimus-ide-collabsdk.DisplayAppSSH:            true,
					optimus-ide-collabsdk.DisplayAppWebTerminal:    true,
					optimus-ide-collabsdk.DisplayAppVSCodeDesktop:  true,
					optimus-ide-collabsdk.DisplayAppVSCodeInsiders: true,
					optimus-ide-collabsdk.DisplayAppPortForward:    true,
				},
			},
		}
	...
	t.Run("SubAgentCleanupPreservesTerraformDefined", func(t *testing.T) {
		...
		var (
			...
			terraformDevcontainer = optimus-ide-collabsdk.WorkspaceAgentDevcontainer{
				ID:              uuid.New(),
		...
		api := agentcontainers.NewAPI(logger,
			...
			agentcontainers.WithSubAgentClient(fakeSAC),
			agentcontainers.WithDevcontainerCLI(&fakeDevcontainerCLI{}),
			agentcontainers.WithDevcontainers([]optimus-ide-collabsdk.WorkspaceAgentDevcontainer{terraformDevcontainer}, nil),
		)
		api.Start()
		defer api.Close()
	...
	t.Run("TerraformDefinedSubAgentNotRecreatedOnConfigChange", func(t *testing.T) {
		...
		var (
			...
			terraformAgentID   = uuid.New()
			terraformContainer = optimus-ide-collabsdk.WorkspaceAgentContainer{
				ID:           "test-container-id",
				FriendlyName: "test-container",
				Image:        "test-image",
				Running:      true,
			...
			fCCLI = &fakeContainerCLI{
				containers: optimus-ide-collabsdk.WorkspaceAgentListContainersResponse{
					Containers: []optimus-ide-collabsdk.WorkspaceAgentContainer{terraformContainer},
				},
				arch: runtime.GOARCH,
			}
				...
				readConfig: agentcontainers.DevcontainerConfig{
					MergedConfiguration: agentcontainers.DevcontainerMergedConfiguration{
						Customizations: agentcontainers.DevcontainerMergedCustomizations{
							Optimus-IDE-Collab: []agentcontainers.Optimus-IDE-CollabCustomization{{
		...
		api := agentcontainers.NewAPI(logger,
			...
			agentcontainers.WithDevcontainerCLI(fDCCLI),
			agentcontainers.WithDevcontainers(
				[]optimus-ide-collabsdk.WorkspaceAgentDevcontainer{terraformDevcontainer},
				[]optimus-ide-collabsdk.WorkspaceAgentScript{{ID: terraformDevcontainer.ID, LogSourceID: uuid.New()}},
			),
		...
		fCCLI.mu.Lock()
		fCCLI.containers.Containers = []optimus-ide-collabsdk.WorkspaceAgentContainer{terraformContainer}
		fCCLI.mu.Unlock()
		fDCCLI.upID = terraformContainer.ID
		fDCCLI.readConfig.MergedConfiguration.Customizations.Optimus-IDE-Collab = []agentcontainers.Optimus-IDE-CollabCustomization{{
	...
	t.Run("TerraformDefinedSubAgentRebuildViaHTTP", func(t *testing.T) {
		...
		assert.Contains(t, rebuiltAgent.DisplayApps, optimus-ide-collabsdk.DisplayAppVSCodeDesktop,
			"rebuilt agent should include updated display apps")
		assert.Contains(t, rebuiltAgent.DisplayApps, optimus-ide-collabsdk.DisplayAppVSCodeInsiders,
			"rebuilt agent should include updated display apps")
	})
	...
	// https://linear.app/optimus-ide-collabcom/issue/PRODUCT-259:
	...
	t.Run("Error", func(t *testing.T) {
		...
		t.Run("DuringUp", func(t *testing.T) {
			...
			var (
				...
				testDevcontainer = optimus-ide-collabsdk.WorkspaceAgentDevcontainer{
					ID:              uuid.New(),
					Name:            "test-devcontainer",
					WorkspaceFolder: "/workspaces/project",
			...
			api := agentcontainers.NewAPI(logger,
				...
				agentcontainers.WithDevcontainerCLI(fDCCLI),
				agentcontainers.WithDevcontainers(
					[]optimus-ide-collabsdk.WorkspaceAgentDevcontainer{testDevcontainer},
					[]optimus-ide-collabsdk.WorkspaceAgentScript{{ID: testDevcontainer.ID, LogSourceID: uuid.New()}},
				),
			...
			testutil.RequireSend(ctx, t, fDCCLI.upErrC, func() error {
				...
				response = optimus-ide-collabsdk.WorkspaceAgentListContainersResponse{}
				err = json.NewDeoptimus-ide-collab(rec.Body).Decode(&response)
				require.NoError(t, err)
		...
		t.Run("DuringUpWithContainerID", func(t *testing.T) {
			...
			// available for use.
			require.Len(t, response.Devcontainers, 1)
			assert.Equal(t, optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatusRunning, response.Devcontainers[0].Status)
		...
		t.Run("DuringInjection", func(t *testing.T) {
			...
			gomock.InOrder(
				...
				mCCLI.EXPECT().ExecAs(gomock.Any(), testContainer.ID, "root", "mkdir", "-p", "/.optimus-ide-collab-agent").Return(nil, nil),
				mCCLI.EXPECT().Copy(gomock.Any(), testContainer.ID, gomock.Any(), "/.optimus-ide-collab-agent/optimus-ide-collab").Return(nil),
				mCCLI.EXPECT().ExecAs(gomock.Any(), testContainer.ID, "root", "chmod", "0755", "/.optimus-ide-collab-agent", "/.optimus-ide-collab-agent/optimus-ide-collab").Return(nil, nil),
				mCCLI.EXPECT().ExecAs(gomock.Any(), testContainer.ID, "root", "/bin/sh", "-c", "chown $(id -u):$(id -g) /.optimus-ide-collab-agent/optimus-ide-collab").Return(nil, nil),
			)
	...
	t.Run("Create", func(t *testing.T) {
		...
		tests := []struct {
			name                 string
			customization        agentcontainers.Optimus-IDE-CollabCustomization
			mergedCustomizations []agentcontainers.Optimus-IDE-CollabCustomization
			...
			},
			{
				...
				afterCreate: func(t *testing.T, subAgent agentcontainers.SubAgent) {
					...
					assert.Contains(t, subAgent.DisplayApps, optimus-ide-collabsdk.DisplayAppVSCodeDesktop)
					assert.Contains(t, subAgent.DisplayApps, optimus-ide-collabsdk.DisplayAppWebTerminal)
					assert.Contains(t, subAgent.DisplayApps, optimus-ide-collabsdk.DisplayAppSSH)
					assert.Contains(t, subAgent.DisplayApps, optimus-ide-collabsdk.DisplayAppPortForward)
				},
			},
			{
				...
				afterCreate: func(t *testing.T, subAgent agentcontainers.SubAgent) {
					...
					assert.Contains(t, subAgent.DisplayApps, optimus-ide-collabsdk.DisplayAppWebTerminal)
					assert.Contains(t, subAgent.DisplayApps, optimus-ide-collabsdk.DisplayAppVSCodeDesktop)
					assert.Contains(t, subAgent.DisplayApps, optimus-ide-collabsdk.DisplayAppVSCodeInsiders)
					assert.Contains(t, subAgent.DisplayApps, optimus-ide-collabsdk.DisplayAppPortForward)
				},
			},
			{
				...
				mergedCustomizations: []agentcontainers.Optimus-IDE-CollabCustomization{
					{
						DisplayApps: map[optimus-ide-collabsdk.DisplayApp]bool{
							optimus-ide-collabsdk.DisplayAppSSH:            false,
							optimus-ide-collabsdk.DisplayAppWebTerminal:    false,
							optimus-ide-collabsdk.DisplayAppVSCodeInsiders: false,
							...
							// that behavior works.
							optimus-ide-collabsdk.DisplayAppPortForward: false,
						},
					},
					{
						DisplayApps: map[optimus-ide-collabsdk.DisplayApp]bool{
							optimus-ide-collabsdk.DisplayAppVSCodeDesktop: false,
							optimus-ide-collabsdk.DisplayAppPortForward:   true,
						},
					},
				},
			...
			},
			{
						...
						Apps: []agentcontainers.SubAgentApp{
							{
								...
								OpenIn:      optimus-ide-collabsdk.WorkspaceAppOpenInTab,
								Share:       optimus-ide-collabsdk.WorkspaceAppSharingLevelOwner,
								Icon:        "/icons/web.svg",
								Order:       int32(1),
							},
							{
								...
								OpenIn:      optimus-ide-collabsdk.WorkspaceAppOpenInSlimWindow,
								Share:       optimus-ide-collabsdk.WorkspaceAppSharingLevelAuthenticated,
								Icon:        "/icons/api.svg",
								Order:       int32(2),
								Hidden:      true,
							},
							{
								...
								OpenIn:      optimus-ide-collabsdk.WorkspaceAppOpenInTab,
								Share:       optimus-ide-collabsdk.WorkspaceAppSharingLevelPublic,
								Icon:        "/icons/book.svg",
								Order:       int32(3),
							},
						},
				...
				afterCreate: func(t *testing.T, subAgent agentcontainers.SubAgent) {
					...
					assert.Equal(t, optimus-ide-collabsdk.WorkspaceAppOpenInTab, subAgent.Apps[0].OpenIn)
					assert.Equal(t, optimus-ide-collabsdk.WorkspaceAppSharingLevelOwner, subAgent.Apps[0].Share)
					...
					assert.Equal(t, optimus-ide-collabsdk.WorkspaceAppOpenInSlimWindow, subAgent.Apps[1].OpenIn)
					assert.Equal(t, optimus-ide-collabsdk.WorkspaceAppSharingLevelAuthenticated, subAgent.Apps[1].Share)
					...
					assert.Equal(t, optimus-ide-collabsdk.WorkspaceAppOpenInTab, subAgent.Apps[2].OpenIn)
					assert.Equal(t, optimus-ide-collabsdk.WorkspaceAppSharingLevelPublic, subAgent.Apps[2].Share)
		...
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
						...
						readConfig: agentcontainers.DevcontainerConfig{
							Configuration: agentcontainers.DevcontainerConfiguration{
								Customizations: agentcontainers.DevcontainerCustomizations{
									Optimus-IDE-Collab: tt.customization,
								},
							},
							MergedConfiguration: agentcontainers.DevcontainerMergedConfiguration{
								Customizations: agentcontainers.DevcontainerMergedCustomizations{
									Optimus-IDE-Collab: tt.mergedCustomizations,
								},
							},
						},
					}
				...
				// Mock the steps used for injecting the optimus-ide-collab agent.
				gomock.InOrder(
					mCCLI.EXPECT().DetectArchitecture(gomock.Any(), testContainer.ID).Return(runtime.GOARCH, nil),
					mCCLI.EXPECT().ExecAs(gomock.Any(), testContainer.ID, "root", "mkdir", "-p", "/.optimus-ide-collab-agent").Return(nil, nil),
					mCCLI.EXPECT().Copy(gomock.Any(), testContainer.ID, optimus-ide-collabBin, "/.optimus-ide-collab-agent/optimus-ide-collab").Return(nil),
					mCCLI.EXPECT().ExecAs(gomock.Any(), testContainer.ID, "root", "chmod", "0755", "/.optimus-ide-collab-agent", "/.optimus-ide-collab-agent/optimus-ide-collab").Return(nil, nil),
					mCCLI.EXPECT().ExecAs(gomock.Any(), testContainer.ID, "root", "/bin/sh", "-c", "chown $(id -u):$(id -g) /.optimus-ide-collab-agent/optimus-ide-collab").Return(nil, nil),
				)
	...
	t.Run("CreateReadsConfigTwice", func(t *testing.T) {
		...
		var (
			...
			testContainer = optimus-ide-collabsdk.WorkspaceAgentContainer{
				...
				CreatedAt:    time.Now(),
				Labels: map[string]string{
					agentcontainers.DevcontainerLocalFolderLabel: "/workspaces/optimus-ide-collab",
					agentcontainers.DevcontainerConfigFileLabel:  "/workspaces/optimus-ide-collab/.devcontainer/devcontainer.json",
				},
			}
		)
		...
		testutil.RequireSend(ctx, t, fDCCLI.readConfigErrC, func(env []string) error {
			...
			assert.Contains(t, env, "OPTIMUS-IDE-COLLAB_WORKSPACE_AGENT_NAME=optimus-ide-collab")
			return nil
		})
		testutil.RequireSend(ctx, t, fDCCLI.readConfigErrC, func(env []string) error {
			// We then expect the agent name passed here to have been read from the config.
			assert.Contains(t, env, "OPTIMUS-IDE-COLLAB_WORKSPACE_AGENT_NAME=custom-name")
			assert.NotContains(t, env, "OPTIMUS-IDE-COLLAB_WORKSPACE_AGENT_NAME=optimus-ide-collab")
			return nil
		})
	...
	t.Run("IgnoreCustomization", func(t *testing.T) {
		...
		fDCCLI := &fakeDevcontainerCLI{
			...
			readConfig: agentcontainers.DevcontainerConfig{
				Configuration: agentcontainers.DevcontainerConfiguration{
					Customizations: agentcontainers.DevcontainerCustomizations{
						Optimus-IDE-Collab: agentcontainers.Optimus-IDE-CollabCustomization{Ignore: true},
					},
				},
		...
		fDCCLI.readConfig.Configuration.Customizations.Optimus-IDE-Collab.Ignore = false
		var (
		...
		fDCCLI.readConfig.Configuration.Customizations.Optimus-IDE-Collab.Ignore = true
		allowSubAgentDelete(ctx, t, fakeSAC)
...
func mustFindDevcontainerByPath(t *testing.T, devcontainers []optimus-ide-collabsdk.WorkspaceAgentDevcontainer, path string) optimus-ide-collabsdk.WorkspaceAgentDevcontainer {
	t.Helper()
	...
	return optimus-ide-collabsdk.WorkspaceAgentDevcontainer{} // Unreachable, but required for compilation
}
...
func TestSubAgentCreationWithNameRetry(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			workspaceFolders: []string{
				"/home/optimus-ide-collab/foo/project",
				"/home/optimus-ide-collab/bar/project",
			},
			expectedNames: []string{
				"project",
				"bar-project",
		...
		},
		{
			...
			workspaceFolders: []string{
				"/home/optimus-ide-collab/foo/x/project",
				"/home/optimus-ide-collab/bar/x/project",
				"/home/optimus-ide-collab/baz/x/project",
			},
			expectedNames: []string{
				"project",
				"x-project",
...
func newFakeContainer(id, configPath, workspaceFolder string) optimus-ide-collabsdk.WorkspaceAgentContainer {
	return optimus-ide-collabsdk.WorkspaceAgentContainer{
		ID:           id,
...
func fakeContainer(t *testing.T, mut ...func(*optimus-ide-collabsdk.WorkspaceAgentContainer)) optimus-ide-collabsdk.WorkspaceAgentContainer {
	t.Helper()
	ct := optimus-ide-collabsdk.WorkspaceAgentContainer{
		...
		Running: true,
		Ports: []optimus-ide-collabsdk.WorkspaceAgentContainerPort{
			{
				Network:  "tcp",
...
func TestWithDevcontainersNameGeneration(t *testing.T) {
	...
	devcontainers := []optimus-ide-collabsdk.WorkspaceAgentDevcontainer{
		{
			ID:              uuid.New(),
			Name:            "original-name",
			WorkspaceFolder: "/home/optimus-ide-collab/foo/project",
			ConfigPath:      "/home/optimus-ide-collab/foo/project/.devcontainer/devcontainer.json",
		},
		{
			ID:              uuid.New(),
			Name:            "another-name",
			WorkspaceFolder: "/home/optimus-ide-collab/bar/project",
			ConfigPath:      "/home/optimus-ide-collab/bar/project/.devcontainer/devcontainer.json",
		},
	}
	...
	api := agentcontainers.NewAPI(logger,
		...
		agentcontainers.WithContainerCLI(&fakeContainerCLI{
			containers: optimus-ide-collabsdk.WorkspaceAgentListContainersResponse{
				Containers: []optimus-ide-collabsdk.WorkspaceAgentContainer{
					fakeContainer(t, func(c *optimus-ide-collabsdk.WorkspaceAgentContainer) {
						c.ID = "some-container-id-1"
						c.FriendlyName = "container-name-1"
						c.Labels[agentcontainers.DevcontainerLocalFolderLabel] = "/home/optimus-ide-collab/baz/project"
						c.Labels[agentcontainers.DevcontainerConfigFileLabel] = "/home/optimus-ide-collab/baz/project/.devcontainer/devcontainer.json"
					}),
				},
			},
		}),
...
func TestDevcontainerDiscovery(t *testing.T) {
	...
	tests := []struct {
		...
		expected []optimus-ide-collabsdk.WorkspaceAgentDevcontainer
	}{
		{
			name:     "GitProjectInRootDir/SingleProject",
			agentDir: "/home/optimus-ide-collab",
			fs: map[string]string{
				"/home/optimus-ide-collab/.git/HEAD":                       "",
				"/home/optimus-ide-collab/.devcontainer/devcontainer.json": "",
			},
			expected: []optimus-ide-collabsdk.WorkspaceAgentDevcontainer{
				{
					WorkspaceFolder: "/home/optimus-ide-collab",
					ConfigPath:      "/home/optimus-ide-collab/.devcontainer/devcontainer.json",
					Status:          optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatusStopped,
				},
			},
		},
		{
			...
			fs: map[string]string{
				"/home/optimus-ide-collab/.git/HEAD":                            "",
				"/home/optimus-ide-collab/.devcontainer/devcontainer.json":      "",
				"/home/optimus-ide-collab/site/.devcontainer/devcontainer.json": "",
			},
			expected: []optimus-ide-collabsdk.WorkspaceAgentDevcontainer{
				{
					WorkspaceFolder: "/home/optimus-ide-collab",
				...
				{
					WorkspaceFolder: "/home/optimus-ide-collab/site",
					ConfigPath:      "/home/optimus-ide-collab/site/.devcontainer/devcontainer.json",
					Status:          optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatusStopped,
				},
			},
		},
		{
			...
			agentDir: "/home/optimus-ide-collab",
			fs: map[string]string{
				"/home/optimus-ide-collab/optimus-ide-collab/.git/HEAD":                       "",
				"/home/optimus-ide-collab/optimus-ide-collab/.devcontainer/devcontainer.json": "",
			},
			expected: []optimus-ide-collabsdk.WorkspaceAgentDevcontainer{
				{
					WorkspaceFolder: "/home/optimus-ide-collab/optimus-ide-collab",
					ConfigPath:      "/home/optimus-ide-collab/optimus-ide-collab/.devcontainer/devcontainer.json",
					Status:          optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatusStopped,
				},
			},
		},
		{
			...
			fs: map[string]string{
				"/home/optimus-ide-collab/optimus-ide-collab/.git/HEAD":                            "",
				"/home/optimus-ide-collab/optimus-ide-collab/.devcontainer/devcontainer.json":      "",
				"/home/optimus-ide-collab/optimus-ide-collab/site/.devcontainer/devcontainer.json": "",
			},
			expected: []optimus-ide-collabsdk.WorkspaceAgentDevcontainer{
				{
					WorkspaceFolder: "/home/optimus-ide-collab/optimus-ide-collab",
				...
				{
					WorkspaceFolder: "/home/optimus-ide-collab/optimus-ide-collab/site",
					ConfigPath:      "/home/optimus-ide-collab/optimus-ide-collab/site/.devcontainer/devcontainer.json",
					Status:          optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatusStopped,
				},
			},
		},
		{
			...
			fs: map[string]string{
				"/home/optimus-ide-collab/optimus-ide-collab/.git/HEAD":                            "",
				"/home/optimus-ide-collab/optimus-ide-collab/.devcontainer/devcontainer.json":      "",
				"/home/optimus-ide-collab/envbuilder/.git/HEAD":                       "",
				"/home/optimus-ide-collab/envbuilder/.devcontainer/devcontainer.json": "",
			},
			expected: []optimus-ide-collabsdk.WorkspaceAgentDevcontainer{
				{
					WorkspaceFolder: "/home/optimus-ide-collab/optimus-ide-collab",
				...
				{
					WorkspaceFolder: "/home/optimus-ide-collab/envbuilder",
					ConfigPath:      "/home/optimus-ide-collab/envbuilder/.devcontainer/devcontainer.json",
					Status:          optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatusStopped,
				},
			},
		},
		{
			...
			fs: map[string]string{
				...
				"/home/optimus-ide-collab/envbuilder/.git/HEAD":                         "",
				"/home/optimus-ide-collab/envbuilder/.devcontainer/devcontainer.json":   "",
				"/home/optimus-ide-collab/envbuilder/x/.devcontainer/devcontainer.json": "",
			},
			expected: []optimus-ide-collabsdk.WorkspaceAgentDevcontainer{
				{
					WorkspaceFolder: "/home/optimus-ide-collab/optimus-ide-collab",
				...
				{
					WorkspaceFolder: "/home/optimus-ide-collab/envbuilder/x",
					ConfigPath:      "/home/optimus-ide-collab/envbuilder/x/.devcontainer/devcontainer.json",
					Status:          optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatusStopped,
				},
			},
		},
		{
			...
			fs: map[string]string{
				"/home/optimus-ide-collab/optimus-ide-collab/.git/HEAD":              "",
				"/home/optimus-ide-collab/optimus-ide-collab/.gitignore":             "y/",
				"/home/optimus-ide-collab/optimus-ide-collab/.devcontainer.json":     "",
				"/home/optimus-ide-collab/optimus-ide-collab/x/y/.devcontainer.json": "",
			},
			expected: []optimus-ide-collabsdk.WorkspaceAgentDevcontainer{
				{
					WorkspaceFolder: "/home/optimus-ide-collab/optimus-ide-collab",
					ConfigPath:      "/home/optimus-ide-collab/optimus-ide-collab/.devcontainer.json",
					Status:          optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatusStopped,
				},
			},
		},
		{
			...
			fs: map[string]string{
				"/home/optimus-ide-collab/optimus-ide-collab/.git/HEAD":              "",
				"/home/optimus-ide-collab/optimus-ide-collab/.devcontainer.json":     "",
				"/home/optimus-ide-collab/optimus-ide-collab/y/.devcontainer.json":   "",
				"/home/optimus-ide-collab/optimus-ide-collab/x/.gitignore":           "y/",
				"/home/optimus-ide-collab/optimus-ide-collab/x/y/.devcontainer.json": "",
			},
			expected: []optimus-ide-collabsdk.WorkspaceAgentDevcontainer{
				{
				...
				{
					WorkspaceFolder: "/home/optimus-ide-collab/optimus-ide-collab/y",
					ConfigPath:      "/home/optimus-ide-collab/optimus-ide-collab/y/.devcontainer.json",
					Status:          optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatusStopped,
				},
			},
		},
		{
			...
			fs: map[string]string{
				"/home/optimus-ide-collab/optimus-ide-collab/.git/HEAD":              "",
				"/home/optimus-ide-collab/optimus-ide-collab/.git/info/exclude":      "y/",
				"/home/optimus-ide-collab/optimus-ide-collab/.devcontainer.json":     "",
				"/home/optimus-ide-collab/optimus-ide-collab/x/y/.devcontainer.json": "",
			},
			expected: []optimus-ide-collabsdk.WorkspaceAgentDevcontainer{
		...
		},
		{
			...
			fs: map[string]string{
				...
				"/home/optimus-ide-collab/.devcontainer/devcontainer.json.bak": "",
				"/home/optimus-ide-collab/.devcontainer/devcontainer.json.old": "",
				"/home/optimus-ide-collab/.devcontainer/devcontainer.json~":    "",
				"/home/optimus-ide-collab/.devcontainer/notdevcontainer.json":  "",
				"/home/optimus-ide-collab/.devcontainer/devcontainer.json.swp": "",

				"/home/optimus-ide-collab/foo/.devcontainer.json.bak": "",
				"/home/optimus-ide-collab/foo/.devcontainer.json.old": "",
				"/home/optimus-ide-collab/foo/.devcontainer.json~":    "",
				"/home/optimus-ide-collab/foo/.notdevcontainer.json":  "",
				"/home/optimus-ide-collab/foo/.devcontainer.json.swp": "",

				"/home/optimus-ide-collab/bar/.devcontainer.json": "",
			},
			expected: []optimus-ide-collabsdk.WorkspaceAgentDevcontainer{
				{
					WorkspaceFolder: "/home/optimus-ide-collab/bar",
					ConfigPath:      "/home/optimus-ide-collab/bar/.devcontainer.json",
					Status:          optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatusStopped,
				},
			},
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.Eventuallyf(t, func() bool {
				...
				got := optimus-ide-collabsdk.WorkspaceAgentListContainersResponse{}
				err := json.NewDeoptimus-ide-collab(rec.Body).Decode(&got)
				require.NoError(t, err)
			...
			slices.SortFunc(tt.expected, func(a, b optimus-ide-collabsdk.WorkspaceAgentDevcontainer) int {
				return strings.Compare(a.WorkspaceFolder, b.WorkspaceFolder)
			})
			slices.SortFunc(got.Devcontainers, func(a, b optimus-ide-collabsdk.WorkspaceAgentDevcontainer) int {
				return strings.Compare(a.WorkspaceFolder, b.WorkspaceFolder)
			})
		...
		tests := []struct {
			...
			},
			{
				...
				fs: map[string]string{
					"/home/optimus-ide-collab/.git/HEAD":                       "",
					"/home/optimus-ide-collab/.devcontainer/devcontainer.json": "",
					"/home/optimus-ide-collab/project/.devcontainer.json":      "",
				},
				configMap: map[string]agentcontainers.DevcontainerConfig{
					"/home/optimus-ide-collab/.devcontainer/devcontainer.json": {
						Configuration: agentcontainers.DevcontainerConfiguration{
		...
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				...
				api := agentcontainers.NewAPI(logger,
					...
					agentcontainers.WithWatcher(watcher.NewNoop()),
					agentcontainers.WithFileSystem(initFS(t, tt.fs)),
					agentcontainers.WithManifestInfo("owner", "workspace", "parent-agent", "/home/optimus-ide-collab"),
					agentcontainers.WithContainerCLI(fCCLI),
					agentcontainers.WithDevcontainerCLI(fDCCLI),
					agentcontainers.WithProjectDiscovery(true),
				...
				for configPath, config := range tt.configMap {
					autoStart := config.Configuration.Customizations.Optimus-IDE-Collab.AutoStart
...
func TestDevcontainerPrebuildSupport(t *testing.T) {
	...
	var (
		...
		testDC = optimus-ide-collabsdk.WorkspaceAgentDevcontainer{
			ID:              uuid.New(),
			WorkspaceFolder: "/home/optimus-ide-collab/optimus-ide-collab",
			ConfigPath:      "/home/optimus-ide-collab/optimus-ide-collab/.devcontainer/devcontainer.json",
		}
	...
	fCCLI.containers = optimus-ide-collabsdk.WorkspaceAgentListContainersResponse{}
	...
	api := agentcontainers.NewAPI(logger,
		...
		agentcontainers.WithManifestInfo(prebuildOwner, prebuildWorkspace, "dev", "/home/optimus-ide-collab"),
		// Given: We start with a single dev container resource.
		agentcontainers.WithDevcontainers(
			[]optimus-ide-collabsdk.WorkspaceAgentDevcontainer{testDC},
			[]optimus-ide-collabsdk.WorkspaceAgentScript{{ID: testDC.ID, LogSourceID: uuid.New()}},
		),
		agentcontainers.WithSubAgentClient(fSAC),
		agentcontainers.WithContainerCLI(fCCLI),
		agentcontainers.WithDevcontainerCLI(fDCCLI),
	...
	require.Contains(t, readConfigEnvVars, "OPTIMUS-IDE-COLLAB_WORKSPACE_OWNER_NAME="+prebuildOwner)
	require.Contains(t, readConfigEnvVars, "OPTIMUS-IDE-COLLAB_WORKSPACE_NAME="+prebuildWorkspace)
	...
	api = agentcontainers.NewAPI(logger,
		...
		agentcontainers.WithManifestInfo(userOwner, userWorkspace, "dev", "/home/optimus-ide-collab"),
		// Given: We start with a single dev container resource.
		agentcontainers.WithDevcontainers(
			[]optimus-ide-collabsdk.WorkspaceAgentDevcontainer{testDC},
			[]optimus-ide-collabsdk.WorkspaceAgentScript{{ID: testDC.ID, LogSourceID: uuid.New()}},
	...
	require.Contains(t, readConfigEnvVars, "OPTIMUS-IDE-COLLAB_WORKSPACE_OWNER_NAME="+userOwner)
	require.Contains(t, readConfigEnvVars, "OPTIMUS-IDE-COLLAB_WORKSPACE_NAME="+userWorkspace)
