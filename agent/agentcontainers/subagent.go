...
type SubAgent struct {
	...
	Apps            []SubAgentApp
	DisplayApps     []optimus-ide-collabsdk.DisplayApp
}
...
func (s SubAgent) CloneConfig(dc optimus-ide-collabsdk.WorkspaceAgentDevcontainer) SubAgent {
	return SubAgent{
...
type SubAgentApp struct {
	...
	OpenIn      optimus-ide-collabsdk.WorkspaceAppOpenIn       `json:"openIn"`
	Order       int32                             `json:"order"`
	Share       optimus-ide-collabsdk.WorkspaceAppSharingLevel `json:"share"`
...
func (app SubAgentApp) ToProtoApp() (*agentproto.CreateSubAgentRequest_App, error) {
	...
	if app.OpenIn != "" {
		switch app.OpenIn {
		case optimus-ide-collabsdk.WorkspaceAppOpenInSlimWindow:
			proto.OpenIn = agentproto.CreateSubAgentRequest_App_SLIM_WINDOW.Enum()
		case optimus-ide-collabsdk.WorkspaceAppOpenInTab:
			proto.OpenIn = agentproto.CreateSubAgentRequest_App_TAB.Enum()
		default:
			return nil, xerrors.Errorf("unexpected optimus-ide-collabsdk.WorkspaceAppOpenIn: %#v", app.OpenIn)
		}
	}

	if app.Share != "" {
		switch app.Share {
		case optimus-ide-collabsdk.WorkspaceAppSharingLevelAuthenticated:
			proto.Share = agentproto.CreateSubAgentRequest_App_AUTHENTICATED.Enum()
		case optimus-ide-collabsdk.WorkspaceAppSharingLevelOwner:
			proto.Share = agentproto.CreateSubAgentRequest_App_OWNER.Enum()
		case optimus-ide-collabsdk.WorkspaceAppSharingLevelPublic:
			proto.Share = agentproto.CreateSubAgentRequest_App_PUBLIC.Enum()
		case optimus-ide-collabsdk.WorkspaceAppSharingLevelOrganization:
			proto.Share = agentproto.CreateSubAgentRequest_App_ORGANIZATION.Enum()
		default:
			return nil, xerrors.Errorf("unexpected optimus-ide-collabsdk.WorkspaceAppSharingLevel: %#v", app.Share)
		}
	}
...
func (a *subAgentAPIClient) Create(ctx context.Context, agent SubAgent) (_ SubAgent, err error) {
	...
	for _, displayApp := range agent.DisplayApps {
		...
		switch displayApp {
		case optimus-ide-collabsdk.DisplayAppPortForward:
			app = agentproto.CreateSubAgentRequest_PORT_FORWARDING_HELPER
		case optimus-ide-collabsdk.DisplayAppSSH:
			app = agentproto.CreateSubAgentRequest_SSH_HELPER
		case optimus-ide-collabsdk.DisplayAppVSCodeDesktop:
			app = agentproto.CreateSubAgentRequest_VSCODE
		case optimus-ide-collabsdk.DisplayAppVSCodeInsiders:
			app = agentproto.CreateSubAgentRequest_VSCODE_INSIDERS
		case optimus-ide-collabsdk.DisplayAppWebTerminal:
			app = agentproto.CreateSubAgentRequest_WEB_TERMINAL
		default:
			return SubAgent{}, xerrors.Errorf("unexpected optimus-ide-collabsdk.DisplayApp: %#v", displayApp)
		}
