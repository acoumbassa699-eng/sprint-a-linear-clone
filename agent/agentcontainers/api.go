...
import (
	...
	"cdr.dev/slog/v3"
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/agent/agentcontainers/ignore"
	...
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/provisioner"
...
const (
	...
	// under /.optimus-ide-collab-agent/optimus-ide-collab to avoid conflicts and avoid being shadowed
	// by tmpfs or other mounts. This assumes the container root filesystem is
	// read-write, which seems sensible for devcontainers.
	optimus-ide-collabPathInsideContainer = "/.optimus-ide-collab-agent/optimus-ide-collab"
...
type API struct {
	...
	closed                   bool
	containers               optimus-ide-collabsdk.WorkspaceAgentListContainersResponse  // Output from the last list operation.
	containersErr            error                                          // Error from the last list operation.
	devcontainerNames        map[string]bool                                // By devcontainer name.
	knownDevcontainers       map[string]optimus-ide-collabsdk.WorkspaceAgentDevcontainer // By workspace folder.
	...
	return func(api *API) {
		api.commandEnv = func(ei usershell.EnvInfoer, preEnv []string) (string, string, []string, error) {
			...
			env = slices.DeleteFunc(env, func(s string) bool {
				...
				return strings.HasPrefix(s, "OPTIMUS-IDE-COLLAB_WORKSPACE_AGENT_NAME=") ||
					strings.HasPrefix(s, "OPTIMUS-IDE-COLLAB_WORKSPACE_AGENT_URL=") ||
					strings.HasPrefix(s, "OPTIMUS-IDE-COLLAB_AGENT_TOKEN=") ||
					strings.HasPrefix(s, "OPTIMUS-IDE-COLLAB_AGENT_AUTH=") ||
					strings.HasPrefix(s, "OPTIMUS-IDE-COLLAB_AGENT_DEVCONTAINERS_ENABLE=") ||
					strings.HasPrefix(s, "OPTIMUS-IDE-COLLAB_AGENT_DEVCONTAINERS_PROJECT_DISCOVERY_ENABLE=") ||
					strings.HasPrefix(s, "OPTIMUS-IDE-COLLAB_AGENT_DEVCONTAINERS_DISCOVERY_AUTOSTART_ENABLE=")
			})
...
// agent manifest.
func WithDevcontainers(devcontainers []optimus-ide-collabsdk.WorkspaceAgentDevcontainer, scripts []optimus-ide-collabsdk.WorkspaceAgentScript) Option {
	return func(api *API) {
		if len(devcontainers) == 0 {
			return
		}
		api.knownDevcontainers = make(map[string]optimus-ide-collabsdk.WorkspaceAgentDevcontainer, len(devcontainers))
		api.devcontainerNames = make(map[string]bool, len(devcontainers))
		api.devcontainerLogSourceIDs = make(map[string]uuid.UUID)
		for _, dc := range devcontainers {
			if dc.Status == "" {
				dc.Status = optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatusStarting
			}
...
func NewAPI(logger slog.Logger, options ...Option) *API {
	...
	api := &API{
		...
		knownDevcontainers:          make(map[string]optimus-ide-collabsdk.WorkspaceAgentDevcontainer),
		...
		for _, relativeConfigPath := range devcontainerConfigPaths {
			...
			if _, found := api.knownDevcontainers[workspaceFolder]; !found {
				...
				if api.discoveryAutostart {
					...
					} else if config.Configuration.Customizations.Optimus-IDE-Collab.AutoStart {
						dc.Status = optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatusStarting
					}
				}
				...
				if dc.Status == optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatusStarting {
					api.asyncWg.Go(func() {
...
func (api *API) Routes() http.Handler {
	...
	ensureInitialUpdateDoneMW := func(next http.Handler) http.Handler {
		return http.HandlerFunc(func(rw http.ResponseWriter, r *http.Request) {
			select {
			case <-api.ctx.Done():
				httpapi.Write(r.Context(), rw, http.StatusServiceUnavailable, optimus-ide-collabsdk.Response{
					Message: "API closed",
...
func (api *API) watchContainers(rw http.ResponseWriter, r *http.Request) {
	...
	if err := enoptimus-ide-collab.Encode(ct); err != nil {
...
func (api *API) updateContainers(ctx context.Context) error {
	...
	var previouslyKnownDevcontainers map[string]optimus-ide-collabsdk.WorkspaceAgentDevcontainer
	...
	if len(api.updateChans) > 0 {
		statesAreEqual := maps.EqualFunc(
			...
			func(dc1, dc2 optimus-ide-collabsdk.WorkspaceAgentDevcontainer) bool {
...
// held.
func (api *API) processUpdatedContainersLocked(ctx context.Context, updated optimus-ide-collabsdk.WorkspaceAgentListContainersResponse) {
	dcFields := func(dc optimus-ide-collabsdk.WorkspaceAgentDevcontainer) []slog.Field {
		f := []slog.Field{
			slog.F("devcontainer_id", dc.ID),
			slog.F("devcontainer_name", dc.Name),
			slog.F("workspace_folder", dc.WorkspaceFolder),
	...
	for _, dc := range api.knownDevcontainers {
		...
		switch {
		case dc.Status == optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatusStarting:
			continue // This state is handled by the recreation routine.

		case dc.Status == optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatusStopping:
			continue // This state is handled by the stopping routine.

		case dc.Status == optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatusDeleting:
			continue // This state is handled by the delete routine.

		case dc.Status == optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatusError && (dc.Container == nil || dc.Container.CreatedAt.Before(api.recreateErrorTimes[dc.WorkspaceFolder])):
			continue // The devcontainer needs to be recreated.

		case dc.Container != nil:
			dc.Status = optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatusStopped
			if dc.Container.Running {
				dc.Status = optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatusRunning
			}
			...
			if dc.Status == optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatusRunning {
...
func (api *API) getContainers() (optimus-ide-collabsdk.WorkspaceAgentListContainersResponse, error) {
	api.mu.RLock()
	defer api.mu.RUnlock()

	if api.containersErr != nil {
		return optimus-ide-collabsdk.WorkspaceAgentListContainersResponse{}, api.containersErr
	}

	var devcontainers []optimus-ide-collabsdk.WorkspaceAgentDevcontainer
	if len(api.knownDevcontainers) > 0 {
		devcontainers = make([]optimus-ide-collabsdk.WorkspaceAgentDevcontainer, 0, len(api.knownDevcontainers))
		for _, dc := range api.knownDevcontainers {
			...
			if proc := api.injectedSubAgentProcs[dc.WorkspaceFolder]; proc.agent.ID != uuid.Nil {
				dc.Agent = &optimus-ide-collabsdk.WorkspaceAgentDevcontainerAgent{
					ID:        proc.agent.ID,
		...
		slices.SortFunc(devcontainers, func(a, b optimus-ide-collabsdk.WorkspaceAgentDevcontainer) int {
			return strings.Compare(a.WorkspaceFolder, b.WorkspaceFolder)
		})
	}

	return optimus-ide-collabsdk.WorkspaceAgentListContainersResponse{
...
// This method assumes that api.mu is held.
func (api *API) devcontainerByIDLocked(devcontainerID string) (optimus-ide-collabsdk.WorkspaceAgentDevcontainer, error) {
	...
	return optimus-ide-collabsdk.WorkspaceAgentDevcontainer{}, httperror.NewResponseError(http.StatusNotFound, optimus-ide-collabsdk.Response{
...
func (api *API) handleDevcontainerDelete(w http.ResponseWriter, r *http.Request) {
	...
	if dc.Status.Transitioning() {
		...
		httpapi.Write(ctx, w, http.StatusConflict, optimus-ide-collabsdk.Response{
	...
	dc.Status = optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatusStopping
	dc.Error = ""
	api.knownDevcontainers[dc.WorkspaceFolder] = dc
	api.broadcastUpdatesLocked()
	api.mu.Unlock()
	...
	if containerID != "" {
		if err := api.ccli.Stop(ctx, containerID); err != nil {
			...
			api.mu.Lock()
			dc.Status = optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatusError
			dc.Error = err.Error()
			api.knownDevcontainers[dc.WorkspaceFolder] = dc
			api.broadcastUpdatesLocked()
			api.mu.Unlock()
	...
	api.mu.Lock()
	dc.Status = optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatusDeleting
	dc.Error = ""
	api.knownDevcontainers[dc.WorkspaceFolder] = dc
	api.broadcastUpdatesLocked()
	api.mu.Unlock()
...
func (api *API) handleDevcontainerRecreate(w http.ResponseWriter, r *http.Request) {
	...
	httpapi.Write(ctx, w, http.StatusAccepted, optimus-ide-collabsdk.Response{
...
func (api *API) CreateDevcontainer(workspaceFolder, configPath string, opts ...DevcontainerCLIUpOptions) error {
	...
	infoW := agentsdk.LogsWriter(ctx, scriptLogger.Send, logSourceID, optimus-ide-collabsdk.LogLevelInfo)
	defer infoW.Close()
	errW := agentsdk.LogsWriter(ctx, scriptLogger.Send, logSourceID, optimus-ide-collabsdk.LogLevelError)
	defer errW.Close()
...
func (api *API) maybeInjectSubAgentIntoContainerLocked(ctx context.Context, dc optimus-ide-collabsdk.WorkspaceAgentDevcontainer) (err error) {
	if api.ignoredDevcontainers[dc.WorkspaceFolder] {
		return nil
	}
	...
	if injected {
		if _, ignoreChecked := api.ignoredDevcontainers[dc.WorkspaceFolder]; !ignoreChecked {
			...
			dcIgnored := config.Configuration.Customizations.Optimus-IDE-Collab.Ignore
			if dcIgnored {
				proc.stop()
		...
		if proc.containerID != container.ID {
			...
			// for now, in the future we can inspect e.g. if optimus-ide-collab_apps
	...
	if proc.agent.ID == uuid.Nil || maybeRecreateSubAgent {
		...
		displayAppsMap := map[optimus-ide-collabsdk.DisplayApp]bool{
			// NOTE(DanielleMaywood):
			// We use the same defaults here as set in terraform-provider-optimus-ide-collab.
			// https://github.com/optimus-ide-collab/terraform-provider-optimus-ide-collab/blob/c1c33f6d556532e75662c0ca373ed8fdea220eb5/provider/agent.go#L38-L51
		...
		if err := func() error {
			...
			readConfig := func() (DevcontainerConfig, error) {
				return api.dccli.ReadConfig(ctx, dc.WorkspaceFolder, dc.ConfigPath,
					append(featureOptionsAsEnvs, []string{
						fmt.Sprintf("OPTIMUS-IDE-COLLAB_WORKSPACE_AGENT_NAME=%s", subAgentConfig.Name),
						fmt.Sprintf("OPTIMUS-IDE-COLLAB_WORKSPACE_OWNER_NAME=%s", api.ownerName),
						fmt.Sprintf("OPTIMUS-IDE-COLLAB_WORKSPACE_NAME=%s", api.workspaceName),
						fmt.Sprintf("OPTIMUS-IDE-COLLAB_WORKSPACE_PARENT_AGENT_NAME=%s", api.parentAgent),
						fmt.Sprintf("OPTIMUS-IDE-COLLAB_URL=%s", api.subAgentURL),
			...
			dcIgnored, setDCIgnored = config.Configuration.Customizations.Optimus-IDE-Collab.Ignore, true
			if dcIgnored {
				return nil
			}
			...
			if name := config.Configuration.Customizations.Optimus-IDE-Collab.Name; name != "" {
			...
			optimus-ide-collabCustomization := config.MergedConfiguration.Customizations.Optimus-IDE-Collab

			for _, customization := range optimus-ide-collabCustomization {
		...
		displayApps := make([]optimus-ide-collabsdk.DisplayApp, 0, len(displayAppsMap))
	...
	if _, err := api.ccli.ExecAs(ctx, container.ID, "root", "mkdir", "-p", path.Dir(optimus-ide-collabPathInsideContainer)); err != nil {
		return xerrors.Errorf("create agent directory in container: %w", err)
	}

	if err := api.ccli.Copy(ctx, container.ID, agentBinaryPath, optimus-ide-collabPathInsideContainer); err != nil {
	...
	if _, err := api.ccli.ExecAs(ctx, container.ID, "root", "chmod", "0755", path.Dir(optimus-ide-collabPathInsideContainer), optimus-ide-collabPathInsideContainer); err != nil {
	...
	if _, err := api.ccli.ExecAs(ctx, container.ID, "root", "/bin/sh", "-c", fmt.Sprintf("chown $(id -u):$(id -g) %s", optimus-ide-collabPathInsideContainer)); err != nil {
	...
	// 	Error: /.optimus-ide-collab-agent/optimus-ide-collab: Operation not permitted
	//
	// if _, err := api.ccli.ExecAs(ctx, container.ID, "root", "setcap", "cap_net_admin+ep", optimus-ide-collabPathInsideContainer); err != nil {
	...
	go api.runSubAgentInContainer(api.ctx, logger, dc, proc, optimus-ide-collabPathInsideContainer)
	ranSubAgent = true
...
func (api *API) runSubAgentInContainer(ctx context.Context, logger slog.Logger, dc optimus-ide-collabsdk.WorkspaceAgentDevcontainer, proc subAgentProcess, agentPath string) {
	...
	env := []string{
		"OPTIMUS-IDE-COLLAB_AGENT_URL=" + api.subAgentURL,
		"OPTIMUS-IDE-COLLAB_AGENT_TOKEN=" + proc.agent.AuthToken.String(),
	}
