	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.errCode != 0 {
				...
				err := json.NewDeoptimus-ide-collab(w.Body).Decode(got)
				require.NoError(t, err)
...
func TestHandleWriteFile_ChatHeaders_UpdatesPathStore(t *testing.T) {
	...
	req.Header.Set(workspacesdk.Optimus-IDE-CollabAncestorChatIDsHeader, string(ancestorJSON))
...
func TestReadFileLines(t *testing.T) {
	...
	tests := []struct {
		...
		expRead    int
		expSize    int64
		// useOptimus-IDE-Collabsdk is set for cases where the handler returns
		// optimus-ide-collabsdk.Response (query param validation) instead of ReadFileLinesResponse.
		useOptimus-IDE-Collabsdk bool
	}{
		{
			name:        "NoPath",
			path:        "",
			useOptimus-IDE-Collabsdk: true,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.useOptimus-IDE-Collabsdk {
				// Query param validation errors return optimus-ide-collabsdk.Response.
...
func runEditFiles(t *testing.T, api *agentfiles.API, req workspacesdk.FileEditRequest) workspacesdk.FileEditResponse {
	...
	require.NoError(t, json.NewDeoptimus-ide-collab(w.Body).Decode(&resp))
	return resp
}
...
func TestFuzzyReplace_FuzzyCollapse_PreservesNextLine(t *testing.T) {
	...
	tests := []struct {
		...
		// optimus-ide-collabd/httpapi/httpapi.go, inline: the original had
	...
	for _, ct := range cases {
		t.Run(ct.name, func(t *testing.T) {
			...
			if ct.errSub != "" {
				...
				got := &optimus-ide-collabsdk.Error{}
				require.NoError(t, json.NewDeoptimus-ide-collab(w.Body).Decode(got))
