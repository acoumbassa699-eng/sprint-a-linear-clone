...
func filterParts(parts []optimus-ide-collabsdk.ChatMessagePart, t optimus-ide-collabsdk.ChatMessagePartType) []optimus-ide-collabsdk.ChatMessagePart {
	var out []optimus-ide-collabsdk.ChatMessagePart
	for _, p := range parts {
		if p.Type == t {
			out = append(out, p)
		}
...
func TestContextPartsFromDir(t *testing.T) {
	// Prevent ~/.optimus-ide-collab/skills on the host from leaking into results.
	...
	t.Run("ReturnsInstructionFilePart", func(t *testing.T) {
		...
		contextParts := filterParts(parts, optimus-ide-collabsdk.ChatMessagePartTypeContextFile)
		skillParts := filterParts(parts, optimus-ide-collabsdk.ChatMessagePartTypeSkill)
...
func TestResolve(t *testing.T) {
	...
	t.Run("CustomEnvVars", func(t *testing.T) {
		...
		ctxFiles := filterParts(cfg.Parts, optimus-ide-collabsdk.ChatMessagePartTypeContextFile)
		require.Len(t, ctxFiles, 1)
		require.Equal(t, "custom instructions", ctxFiles[0].ContextFileContent)
		skillParts := filterParts(cfg.Parts, optimus-ide-collabsdk.ChatMessagePartTypeSkill)
		require.Len(t, skillParts, 1)
		require.Equal(t, "my-skill", skillParts[0].SkillName)
	...
	t.Run("ReadsInstructionFiles", func(t *testing.T) {
		...
		// Create ~/.optimus-ide-collab/AGENTS.md
		optimus-ide-collabDir := filepath.Join(fakeHome, ".optimus-ide-collab")
		require.NoError(t, os.MkdirAll(optimus-ide-collabDir, 0o755))
		require.NoError(t, os.WriteFile(
			filepath.Join(optimus-ide-collabDir, "AGENTS.md"),
		...
		require.Equal(t, filepath.Join(optimus-ide-collabDir, "AGENTS.md"), ctxFiles[0].ContextFilePath)
	...
	t.Run("DefaultDiscoversHomeAndProjectSkillsHomeWins", func(t *testing.T) {
		...
		homeSkills := filepath.Join(fakeHome, ".optimus-ide-collab", "skills")
		...
		for _, p := range filterParts(cfg.Parts, optimus-ide-collabsdk.ChatMessagePartTypeSkill) {
...
// OPTIMUS-IDE-COLLAB_AGENT_EXP_* env var from the process.
//
...
func TestResolve_ConfigOverridesEnv(t *testing.T) {
	...
	ctxFiles := filterParts(result.Parts, optimus-ide-collabsdk.ChatMessagePartTypeContextFile)
	require.Len(t, ctxFiles, 1)
