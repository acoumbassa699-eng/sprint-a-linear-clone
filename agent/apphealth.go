...
// NewWorkspaceAppHealthReporter creates a WorkspaceAppHealthReporter that reports app health to optimus-ide-collabd.
func NewWorkspaceAppHealthReporter(logger slog.Logger, apps []optimus-ide-collabsdk.WorkspaceApp, postWorkspaceAgentAppHealth PostWorkspaceAgentAppHealth) WorkspaceAppHealthReporter {
...
func NewAppHealthReporterWithClock(
	...
	return func(ctx context.Context) {
		...
		health := make(map[uuid.UUID]optimus-ide-collabsdk.WorkspaceAppHealth, 0)
		for _, app := range apps {
			if app.Health == optimus-ide-collabsdk.WorkspaceAppHealthDisabled {
				continue
			}
				...
				_ = clk.TickerFunc(ctx, time.Duration(app.Healthcheck.Interval)*time.Second, func() error {
					...
					if err != nil {
						...
						if failures[app.ID] < int(app.Healthcheck.Threshold) {
							...
							health[app.ID] = optimus-ide-collabsdk.WorkspaceAppHealthUnhealthy
							nowUnhealthy = true
						}
						mu.Unlock()
						...
						health[app.ID] = optimus-ide-collabsdk.WorkspaceAppHealthHealthy
						failures[app.ID] = 0
						mu.Unlock()
...
func shouldStartTicker(app optimus-ide-collabsdk.WorkspaceApp) bool {
	return app.Healthcheck.URL != "" && app.Healthcheck.Interval > 0 && app.Healthcheck.Threshold > 0
}

func healthChanged(old map[uuid.UUID]optimus-ide-collabsdk.WorkspaceAppHealth, updated map[uuid.UUID]optimus-ide-collabsdk.WorkspaceAppHealth) bool {
...
func copyHealth(h1 map[uuid.UUID]optimus-ide-collabsdk.WorkspaceAppHealth) map[uuid.UUID]optimus-ide-collabsdk.WorkspaceAppHealth {
	h2 := make(map[uuid.UUID]optimus-ide-collabsdk.WorkspaceAppHealth, 0)
	for k, v := range h1 {
		h2[k] = v
	}
