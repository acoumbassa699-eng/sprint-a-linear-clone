...
func TestHandleDesktopVNC_StartError(t *testing.T) {
	...
	err := json.NewDeoptimus-ide-collab(rr.Body).Decode(&resp)
...
func TestHandleAction_Screenshot(t *testing.T) {
	...
	err = json.NewDeoptimus-ide-collab(rr.Body).Decode(&result)
	require.NoError(t, err)
...
func TestRecordingStopCorrupted(t *testing.T) {
	...
	var respStop optimus-ide-collabsdk.Response
	err = json.NewDeoptimus-ide-collab(rr.Body).Decode(&respStop)
	require.NoError(t, err)
