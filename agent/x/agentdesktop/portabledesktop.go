...
type portableDesktop struct {
	...
	scriptBinDir string // optimus-ide-collab script bin directory
	clock        quartz.Clock
...
// the optimus-ide-collab script bin directory checked for the binary. If clk is
...
func (p *portableDesktop) Start(ctx context.Context) (DisplayConfig, error) {
	...
	if err := json.NewDeoptimus-ide-collab(stdout).Decode(&output); err != nil {
		sessionCancel()
		_ = cmd.Process.Kill()
		_ = cmd.Wait()
...
func (p *portableDesktop) StartRecording(ctx context.Context, recordingID string) error {
	...
	filePath := filepath.Join(os.TempDir(), "optimus-ide-collab-recording-"+recordingID+".mp4")
	thumbPath := filepath.Join(os.TempDir(), "optimus-ide-collab-recording-"+recordingID+".thumb.jpg")
...
// optimus-ide-collab script bin directory. It must be called while p.mu is held.
func (p *portableDesktop) ensureBinary(ctx context.Context) error {
	...
	// 2. Check the optimus-ide-collab script bin directory.
