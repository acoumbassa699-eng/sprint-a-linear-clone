...
func TestPortableDesktop_StartRecording(t *testing.T) {
	...
	for _, cmd := range cmds {
		joined := strings.Join(cmd, " ")
		if strings.Contains(joined, "record") && strings.Contains(joined, "optimus-ide-collab-recording-"+recID) {
			found = true
...
func TestPortableDesktop_StopRecording_ReturnsArtifact(t *testing.T) {
	...
	rec := &recordedExecer{
		scripts: map[string]string{
			...
			// and the process exits immediately. (See optimus-ide-collab/internal#1462.)
	...
	filePath := filepath.Join(os.TempDir(), "optimus-ide-collab-recording-"+recID+".mp4")
...
func TestPortableDesktop_StopRecording_WithThumbnail(t *testing.T) {
	...
	thumbPath := filepath.Join(os.TempDir(), "optimus-ide-collab-recording-"+recID+".thumb.jpg")
