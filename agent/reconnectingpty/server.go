	...
	for {
		...
		go func() {
			...
			case err != nil && ctx.Err() != nil:
				reason = optimus-ide-collabsdk.DisconnectReasonServerShutdown
				code = 1
			case err != nil:
			...
			clog.Info(ctx, "reconnecting pty closed",
				...
				optimus-ide-collabsdk.SlogDisconnectDetail(detail),
...
func (s *Server) handleConn(ctx context.Context, logger slog.Logger, conn net.Conn) (retErr error) {
	...
	// This cannot use a JSON deoptimus-ide-collab, since that can
