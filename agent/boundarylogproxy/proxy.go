...
// audit logs and forwards them to optimus-ide-collabd via the agent API.
...
import (
	...
	"cdr.dev/slog/v3"
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/agent/boundarylogproxy/codec"
...
// for forwarding to optimus-ide-collabd. The socket server and the forwarder are decoupled:
// - Start() creates the socket and accepts a connection from boundary
// - RunForwarder() drains the buffer and sends logs to optimus-ide-collabd via AgentAPI
type Server struct {
	logger     slog.Logger
	socketPath string
	metrics    *Metrics
...
// RunForwarder drains the log buffer and forwards logs to optimus-ide-collabd.
