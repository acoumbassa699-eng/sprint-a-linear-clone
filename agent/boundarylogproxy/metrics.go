...
// Metrics tracks observability for the boundary -> agent -> optimus-ide-collabd audit log
// pipeline.
//
// Audit logs from boundary workspaces pass through several async buffers
// before reaching optimus-ide-collabd, and any stage can silently drop data. These
...
//     the agent cannot reach optimus-ide-collabd.
//
// Drops are captured at two stages:
//   - Agent-side: the agent's channel buffer overflows (reason
//     "buffer_full") or the RPC forward to optimus-ide-collabd fails (reason
//     "forward_failed").
...
func newMetrics(registerer prometheus.Registerer) *Metrics {
	batchesDropped := prometheus.NewCounterVec(prometheus.CounterOpts{
		...
		Help: "Total number of boundary log batches dropped before reaching optimus-ide-collabd. " +
			"Reason: buffer_full = the agent's internal buffer is full, meaning boundary is producing logs faster than the agent can forward them to optimus-ide-collabd; " +
			"forward_failed = the agent failed to send the batch to optimus-ide-collabd, potentially because optimus-ide-collabd is unreachable or the connection was interrupted.",
	}, []string{"reason"})
	...
	logsDropped := prometheus.NewCounterVec(prometheus.CounterOpts{
		...
		Help: "Total number of individual boundary log entries dropped before reaching optimus-ide-collabd. " +
			"Reason: buffer_full = the agent's internal buffer is full; " +
			"forward_failed = the agent failed to send the batch to optimus-ide-collabd; " +
	...
	batchesForwarded := prometheus.NewCounter(prometheus.CounterOpts{
		...
		Help: "Total number of boundary log batches successfully forwarded to optimus-ide-collabd. " +
