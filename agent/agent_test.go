...
func TestMain(m *testing.M) {
	if os.Getenv("OPTIMUS-IDE-COLLAB_TEST_RUN_SUB_AGENT_MAIN") == "1" {
...
// TestAgent_CloseWhileStarting is a regression test for https://github.com/optimus-ide-collab/optimus-ide-collab/issues/17328
...
func TestAgent_SessionExec(t *testing.T) {
	...
	for _, port := range sshPorts {
		t.Run(fmt.Sprintf("(:%d)", port), func(t *testing.T) {
			...
			session := setupSSHSessionOnPort(t, agentsdk.Manifest{}, optimus-ide-collabsdk.ServiceBannerConfig{}, nil, port)
...
func TestAgent_Session_EnvironmentVariables(t *testing.T) {
	...
	// Defined by the optimus-ide-collab script runner, hardcoded here since we don't
	// have a reference to it.
	scriptBinDir := filepath.Join(tmpdir, "optimus-ide-collab-script-data", "bin")
	...
	banner := optimus-ide-collabsdk.ServiceBannerConfig{}
	...
	for k, partialV := range map[string]string{
		"OPTIMUS-IDE-COLLAB":               "true",  // From the agent.
...
func TestAgent_StartupScript_SecretInjection(t *testing.T) {
	...
	var got []optimus-ide-collabsdk.WorkspaceAgentLifecycle
	assert.Eventually(t, func() bool {
		got = client.GetLifecycleStates()
		return len(got) > 0 && got[len(got)-1] == optimus-ide-collabsdk.WorkspaceAgentLifecycleReady
	}, testutil.WaitLong, testutil.IntervalMedium)
	require.Contains(t, got, optimus-ide-collabsdk.WorkspaceAgentLifecycleReady, "agent never reached ready")
...
func TestAgent_GitSSH(t *testing.T) {
	t.Parallel()
	session := setupSSHSession(t, agentsdk.Manifest{}, optimus-ide-collabsdk.ServiceBannerConfig{}, nil)
...
func TestAgent_Session_TTY_MOTD(t *testing.T) {
	...
	wantMOTD := "Welcome to your Optimus-IDE-Collab workspace!"
	...
	for _, port := range sshPorts {
		...
		for i, test := range tests {
			t.Run(fmt.Sprintf("(:%d)/%d", port, i), func(t *testing.T) {
				...
				client.SetAnnouncementBannersFunc(func() ([]optimus-ide-collabsdk.BannerConfig, error) {
					select {
					case ready <- struct{}{}:
					default:
					}
					return []optimus-ide-collabsdk.BannerConfig{test.banner}, nil
				})
...
func TestAgent_Session_TTY_QuietLogin(t *testing.T) {
	...
	wantNotMOTD := "Welcome to your Optimus-IDE-Collab workspace!"
	wantServiceBanner := "Service banner text goes here"
	...
	t.Run("NotLogin", func(t *testing.T) {
		session := setupSSHSession(t, agentsdk.Manifest{
			MOTDFile: motdPath,
		}, optimus-ide-collabsdk.ServiceBannerConfig{
			Enabled: true,
			Message: wantServiceBanner,
		}, func(fs afero.Fs) {
...
func TestAgent_Session_TTY_FastCommandHasOutput(t *testing.T) {
	...
	//
	// See: https://github.com/optimus-ide-collab/optimus-ide-collab/issues/6656
	ctx, cancel := context.WithTimeout(context.Background(), testutil.WaitLong)
	defer cancel()
	//nolint:dogsled
	conn, _, _, _, _ := setupAgent(t, agentsdk.Manifest{}, 0)
...
func TestAgent_EnvironmentVariables(t *testing.T) {
	...
	}, optimus-ide-collabsdk.ServiceBannerConfig{}, nil)
...
func TestAgent_Optimus-IDE-CollabEnvVars(t *testing.T) {
	t.Parallel()

	for _, key := range []string{"OPTIMUS-IDE-COLLAB", "OPTIMUS-IDE-COLLAB_WORKSPACE_NAME", "OPTIMUS-IDE-COLLAB_WORKSPACE_OWNER_NAME", "OPTIMUS-IDE-COLLAB_WORKSPACE_AGENT_NAME"} {
...
func TestAgent_SSHEnvInfoShell(t *testing.T) {
	...
	session := setupSSHSession(t, agentsdk.Manifest{}, optimus-ide-collabsdk.ServiceBannerConfig{}, nil, func(_ *agenttest.Client, o *agent.Options) {
...
func TestAgent_Metadata(t *testing.T) {
	...
	t.Run("Once", func(t *testing.T) {
		...
		_, client, _, _, _ := setupAgent(t, agentsdk.Manifest{
			Metadata: []optimus-ide-collabsdk.WorkspaceAgentMetadataDescription{
				{
					Key:      "greeting1",
					Interval: 0,
					Script:   echoHello,
...
func TestAgent_Lifecycle(t *testing.T) {
	...
	t.Run("StartTimeout", func(t *testing.T) {
		...
		want := []optimus-ide-collabsdk.WorkspaceAgentLifecycle{
			optimus-ide-collabsdk.WorkspaceAgentLifecycleStarting,
			optimus-ide-collabsdk.WorkspaceAgentLifecycleStartTimeout,
		}
	...
	t.Run("StartError", func(t *testing.T) {
		...
		want := []optimus-ide-collabsdk.WorkspaceAgentLifecycle{
			optimus-ide-collabsdk.WorkspaceAgentLifecycleStarting,
			optimus-ide-collabsdk.WorkspaceAgentLifecycleStartError,
		}
	...
	t.Run("Ready", func(t *testing.T) {
		...
		want := []optimus-ide-collabsdk.WorkspaceAgentLifecycle{
			optimus-ide-collabsdk.WorkspaceAgentLifecycleStarting,
			optimus-ide-collabsdk.WorkspaceAgentLifecycleReady,
		}
	...
	t.Run("ShuttingDown", func(t *testing.T) {
		...
		assert.Eventually(t, func() bool {
			return slices.Contains(client.GetLifecycleStates(), optimus-ide-collabsdk.WorkspaceAgentLifecycleReady)
		}, testutil.WaitShort, testutil.IntervalMedium)
		...
		want := []optimus-ide-collabsdk.WorkspaceAgentLifecycle{
			optimus-ide-collabsdk.WorkspaceAgentLifecycleStarting,
			optimus-ide-collabsdk.WorkspaceAgentLifecycleReady,
			optimus-ide-collabsdk.WorkspaceAgentLifecycleShuttingDown,
		}
	...
	t.Run("ShutdownTimeout", func(t *testing.T) {
		...
		want := []optimus-ide-collabsdk.WorkspaceAgentLifecycle{
			...
			optimus-ide-collabsdk.WorkspaceAgentLifecycleReady,
			optimus-ide-collabsdk.WorkspaceAgentLifecycleShuttingDown,
			optimus-ide-collabsdk.WorkspaceAgentLifecycleShutdownTimeout,
		}
	...
	t.Run("ShutdownError", func(t *testing.T) {
		...
		want := []optimus-ide-collabsdk.WorkspaceAgentLifecycle{
			...
			optimus-ide-collabsdk.WorkspaceAgentLifecycleReady,
			optimus-ide-collabsdk.WorkspaceAgentLifecycleShuttingDown,
			optimus-ide-collabsdk.WorkspaceAgentLifecycleShutdownError,
		}
	...
	t.Run("ShutdownScriptOnce", func(t *testing.T) {
		...
		client := agenttest.NewClient(t,
			...
			agentsdk.Manifest{
				...
				Scripts: []optimus-ide-collabsdk.WorkspaceAgentScript{{
					ID:         uuid.New(),
					LogPath:    "optimus-ide-collab-startup-script.log",
					Script:     "echo 1",
					RunOnStart: true,
				}, {
					ID:        uuid.New(),
					LogPath:   "optimus-ide-collab-shutdown-script.log",
		...
		require.Eventually(t, func() bool {
			outputPath := filepath.Join(os.TempDir(), "optimus-ide-collab-startup-script.log")
		...
		outputPath := filepath.Join(os.TempDir(), "optimus-ide-collab-shutdown-script.log")
...
func TestAgent_Startup(t *testing.T) {
	...
	t.Run("NotAbsoluteDirectory", func(t *testing.T) {
		...
		_, client, _, _, _ := setupAgent(t, agentsdk.Manifest{
			Directory: "optimus-ide-collab/optimus-ide-collab",
		}, 0)
		startup := testutil.TryReceive(ctx, t, client.GetStartup())
		homeDir, err := os.UserHomeDir()
		require.NoError(t, err)
		require.Equal(t, filepath.Join(homeDir, "optimus-ide-collab/optimus-ide-collab"), startup.GetExpandedDirectory())
	})
	...
	for _, backendType := range backends {
		t.Run(backendType, func(t *testing.T) {
			if backendType == "Screen" {
				...
				require.NoError(t, err)
				dir, err := os.MkdirTemp("/tmp", "optimus-ide-collab-test-reconnecting-pty-PATH")
			...
			if !hasTmux {
				...
				glyphs := "⚠╭╮╰╯•›│─█▓░▄❯✔╌"
				tmuxSocket := "optimus-ide-collab-test-" + strings.ReplaceAll(uuid.NewString(), "-", "")
				t.Cleanup(func() {
...
//
// OPTIMUS-IDE-COLLAB_TEST_USE_DOCKER=1 go test -count=1 ./agent -run TestAgent_ReconnectingPTYContainer
func TestAgent_ReconnectingPTYContainer(t *testing.T) {
	t.Parallel()
	if os.Getenv("OPTIMUS-IDE-COLLAB_TEST_USE_DOCKER") != "1" {
	...
	require.NoError(t, json.NewEnoptimus-ide-collab(ac).Encode(workspacesdk.ReconnectingPTYRequest{
		Data: "hostname\r",
	}), "write hostname")
	require.NoError(t, tr.ReadUntil(ctx, func(line string) bool {
...
// to the control plane. It reads the OPTIMUS-IDE-COLLAB_AGENT_URL and
// OPTIMUS-IDE-COLLAB_AGENT_TOKEN environment variables, sends the token, and exits
// with a status code based on the response.
func runSubAgentMain() int {
	url := os.Getenv("OPTIMUS-IDE-COLLAB_AGENT_URL")
	token := os.Getenv("OPTIMUS-IDE-COLLAB_AGENT_TOKEN")
	if url == "" || token == "" {
		_, _ = fmt.Fprintln(os.Stderr, "OPTIMUS-IDE-COLLAB_AGENT_URL and OPTIMUS-IDE-COLLAB_AGENT_TOKEN must be set")
		return 10
	}
...
// You can run it manually as follows:
//
// OPTIMUS-IDE-COLLAB_TEST_USE_DOCKER=1 go test -count=1 ./agent -run TestAgent_DevcontainerAutostart
//
//nolint:paralleltest // This test sets an environment variable.
func TestAgent_DevcontainerAutostart(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if err := json.NewDeoptimus-ide-collab(r.Body).Decode(&payload); err != nil {
	...
	_, agentClient, _, _, _ := setupAgent(t, manifest, 0, func(_ *agenttest.Client, o *agent.Options) {
		...
		o.DevcontainerAPIOptions = append(
			...
			agentcontainers.WithSubAgentEnv("OPTIMUS-IDE-COLLAB_TEST_RUN_SUB_AGENT_MAIN=1"),
		)
	})
...
// You can run it manually as follows:
//
// OPTIMUS-IDE-COLLAB_TEST_USE_DOCKER=1 go test -count=1 ./agent -run TestAgent_DevcontainerRecreate
func TestAgent_DevcontainerRecreate(t *testing.T) {
	...
	testutil.Eventually(ctx, t, func(context.Context) bool {
		...
		return slices.Contains(states, optimus-ide-collabsdk.WorkspaceAgentLifecycleReady)
	...
	var container optimus-ide-collabsdk.WorkspaceAgentContainer
	testutil.Eventually(ctx, t, func(context.Context) bool {
	...
	defer func(container optimus-ide-collabsdk.WorkspaceAgentContainer) {
		// We can't rely on pool here because the container is not
		// managed by it (it is managed by @devcontainer/cli).
		err := pool.Client.RemoveContainer(docker.RemoveContainerOptions{
			ID:            container.ID,
	...
	// concurrently.
	go func(container optimus-ide-collabsdk.WorkspaceAgentContainer) {
...
// You can run it manually as follows:
//
// OPTIMUS-IDE-COLLAB_TEST_USE_DOCKER=1 go test -count=1 ./agent -run TestAgent_DevcontainerPrebuildClaim
//
//nolint:paralleltest // This test sets an environment variable.
func TestAgent_DevcontainerPrebuildClaim(t *testing.T) {
	...
	var dcPrebuild optimus-ide-collabsdk.WorkspaceAgentDevcontainer
	testutil.Eventually(ctx, t, func(ctx context.Context) bool {
		resp, err := conn.ListContainers(ctx)
		require.NoError(t, err)
	...
	require.Equal(t, "zed://ssh/project.prebuilds-xyz-123.prebuilds.optimus-ide-collab/workspaces/project", subAgentApp.GetUrl())
	...
	var dcClaimed optimus-ide-collabsdk.WorkspaceAgentDevcontainer
	testutil.Eventually(ctx, t, func(ctx context.Context) bool {
		resp, err := conn.ListContainers(ctx)
		require.NoError(t, err)
	...
	subAgentApp = subAgentApps[0]
	require.Equal(t, "zed://ssh/project.user-workspace.user.optimus-ide-collab/workspaces/project", subAgentApp.GetUrl())
}
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			testutil.RunRetry(t, 3, func(t testing.TB) {
				...
				// also connect via the Optimus-IDE-CollabServicePrefix, to test that we can reach the agent on this
				// IP. This will be required for Optimus-IDE-CollabVPN.
				_, rawPort, _ := net.SplitHostPort(l.Addr().String())
				port, _ := strconv.ParseUint(rawPort, 10, 16)
				ipp := netip.AddrPortFrom(tailnet.Optimus-IDE-CollabServicePrefix.AddrFromUUID(agentID), uint16(port))
...
func TestAgent_DebugServer(t *testing.T) {
	...
	logDir := t.TempDir()
	logPath := filepath.Join(logDir, "optimus-ide-collab-agent.log")
	randLogStr, err := cryptorand.String(32)
	require.NoError(t, err)
	require.NoError(t, os.WriteFile(logPath, []byte(randLogStr), 0o600))
	newRotatedLogPath := filepath.Join(logDir, "optimus-ide-collab-agent-2026-05-17T20-00-00.000.log")
	oldRotatedLogPath := filepath.Join(logDir, "optimus-ide-collab-agent-2026-05-17T19-00-00.000.log")
	...
	t.Run("Manifest", func(t *testing.T) {
		...
		require.NoError(t, json.NewDeoptimus-ide-collab(res.Body).Decode(&v))
		require.NotNil(t, v)
	})
	...
	t.Run("LogsIncludeActiveOnlyWithAfter", func(t *testing.T) {
		...
		require.Contains(t, body, "optimus-ide-collab-agent.log")
	...
	t.Run("LogsIncludeRotatedWithAfter", func(t *testing.T) {
		...
		require.Contains(t, body, randLogStr)
		require.Contains(t, body, "optimus-ide-collab-agent.log")
		require.Contains(t, body, "optimus-ide-collab-agent-2026-05-17T20-00-00.000.log")
...
func setupSSHSession(
	t *testing.T,
	manifest agentsdk.Manifest,
	banner optimus-ide-collabsdk.BannerConfig,
...
func setupSSHSessionOnPort(
	...
	opts = append(opts, func(c *agenttest.Client, o *agent.Options) {
		c.SetAnnouncementBannersFunc(func() ([]optimus-ide-collabsdk.BannerConfig, error) {
			return []optimus-ide-collabsdk.BannerConfig{banner}, nil
		})
	})
	//nolint:dogsled
...
// the Manifest doc comment in optimus-ide-collabsdk/agentsdk.
...
func TestAgent_Metrics_SSH(t *testing.T) {
	...
	expected := []struct {
		...
		{
			Name: "optimus-ide-collabd_agentstats_currently_reachable_peers",
		...
		{
			Name: "optimus-ide-collabd_agentstats_startup_script_seconds",
			Type: proto.Stats_Metric_GAUGE,
