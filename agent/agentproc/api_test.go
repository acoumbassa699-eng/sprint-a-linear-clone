...
func waitForExit(t *testing.T, handler http.Handler, id string) workspacesdk.ProcessOutputResponse {
	...
	for {
		...
		case <-ticker.C:
			...
			err := json.NewDeoptimus-ide-collab(w.Body).Decode(&resp)
			require.NoError(t, err)
...
func TestStartProcess(t *testing.T) {
	...
	t.Run("DefaultWorkDirIsHome", func(t *testing.T) {
		...
		require.NoError(t, json.NewDeoptimus-ide-collab(w.Body).Decode(&listResp))
...
func TestListProcesses(t *testing.T) {
	...
	t.Run("FilterByChatID", func(t *testing.T) {
		...
		headersA := http.Header{workspacesdk.Optimus-IDE-CollabChatIDHeader: {chatA}}
		headersB := http.Header{workspacesdk.Optimus-IDE-CollabChatIDHeader: {chatB}}
		...
		err = json.NewDeoptimus-ide-collab(w2.Body).Decode(&resp2)
		require.NoError(t, err)
		...
		err = json.NewDeoptimus-ide-collab(w3.Body).Decode(&resp3)
		require.NoError(t, err)
	...
	t.Run("ChatIDFiltering", func(t *testing.T) {
		...
		handler := newTestAPI(t)
		chatID := uuid.New().String()
		headers := http.Header{workspacesdk.Optimus-IDE-CollabChatIDHeader: {chatID}}
...
// Optimus-IDE-Collab-Chat-Id header set and returns the recorder.
func getListWithChatHeader(t *testing.T, handler http.Handler, chatID string) *httptest.ResponseRecorder {
	...
	r := httptest.NewRequestWithContext(ctx, http.MethodGet, "/list", nil)
	if chatID != "" {
		r.Header.Set(workspacesdk.Optimus-IDE-CollabChatIDHeader, chatID)
	}
	handler.ServeHTTP(w, r)
	return w
}
...
func TestProcessOutput(t *testing.T) {
	...
	t.Run("ChatIDEnforcement", func(t *testing.T) {
		...
		id := startAndGetID(t, handler, workspacesdk.StartProcessRequest{
			...
			workspacesdk.Optimus-IDE-CollabChatIDHeader: {chatA.String()},
		})
		waitForExit(t, handler, id)
		...
		w1 := getOutputWithHeaders(t, handler, id, http.Header{
			workspacesdk.Optimus-IDE-CollabChatIDHeader: {chatB.String()},
		})
	...
	t.Run("ConcurrentWaiters", func(t *testing.T) {
		...
		for i := range 2 {
			wg.Go(func() {
				w := getOutputWithWait(t, handler, id)
				codes[i] = w.Code
				_ = json.NewDeoptimus-ide-collab(w.Body).Decode(&resps[i])
			})
		}
...
func TestProcessLifecycle(t *testing.T) {
	...
	t.Run("StartSignalVerifyExit", func(t *testing.T) {
		...
		err := json.NewDeoptimus-ide-collab(w.Body).Decode(&running)
		require.NoError(t, err)
		require.True(t, running.Running)
