...
// Pusher delivers snapshots to optimus-ide-collabd. Concrete implementations
...
// ErrPushUnimplemented signals that the optimus-ide-collabd peer does not
...
// ErrPushUnimplemented.
//
// The first push is always sent with Initial=true so optimus-ide-collabd can
...
func (m *Manager) RunPush(ctx context.Context, p Pusher, opts PushOptions) error {
	...
	for {
		...
		case errors.Is(err, ErrPushUnimplemented):
			logger.Warn(ctx, "optimus-ide-collabd peer does not implement PushContextState; stopping")
			return nil
