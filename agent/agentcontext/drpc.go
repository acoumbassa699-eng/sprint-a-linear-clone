...
// when an old optimus-ide-collabd is on the other end.
...
// is always set (zero-valued if necessary) so optimus-ide-collabd can tell
...
// when status is not OK so optimus-ide-collabd can attribute the failure to a
