...
type ManagerOptions struct {
	...
	// time. Sources from OPTIMUS-IDE-COLLAB_AGENT_EXP_*_DIRS env vars or
	...
	// to [~, ~/.optimus-ide-collab, ~/.claude] plus the working directory.
...
type Manager struct {
	...
	// version-0 value, which the push loop never sends to optimus-ide-collabd.
	// Guarded by mu.
	ready bool
...
// for manifest-triggered seeding from OPTIMUS-IDE-COLLAB_AGENT_EXP_*_DIRS.
...
// default policy ([~, ~/.optimus-ide-collab, ~/.claude]).
