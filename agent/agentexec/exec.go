...
const (
	...
	EnvProcNiceScore = "OPTIMUS-IDE-COLLAB_PROC_NICE_SCORE"
...
// necessary because we need to be able to wrap child processes with "optimus-ide-collab agent-exec"
// for templates that expect the agent to manage process priority.
type Execer interface {
	// CommandContext returns an exec.Cmd that calls "optimus-ide-collab agent-exec" prior to exec'ing
	// the provided command if OPTIMUS-IDE-COLLAB_PROC_PRIO_MGMT is set, otherwise a normal exec.Cmd
	...
	// PTYCommandContext returns an pty.Cmd that calls "optimus-ide-collab agent-exec" prior to exec'ing
	// the provided command if OPTIMUS-IDE-COLLAB_PROC_PRIO_MGMT is set, otherwise a normal pty.Cmd
...
const (
	niceFlag = "optimus-ide-collab-nice"
	oomFlag  = "optimus-ide-collab-oom"
)
