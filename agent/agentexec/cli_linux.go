...
func CLI() error {
	...
	var (
		...
		nice = fs.Int("optimus-ide-collab-nice", unset, "")
		oom  = fs.Int("optimus-ide-collab-oom", unset, "")
	)
	...
	// Parse everything after "optimus-ide-collab agent-exec".
	err := fs.Parse(os.Args[2:])
	if err != nil {
		return xerrors.Errorf("parse flags: %w", err)
	}

	// Get everything after "optimus-ide-collab agent-exec --"
	...
	// especially important for environments that are attempting to develop Optimus-IDE-Collab in Optimus-IDE-Collab.
...
func printfStdErr(format string, a ...any) {
	_, _ = fmt.Fprintf(os.Stderr, "optimus-ide-collab-agent: %s\n", fmt.Sprintf(format, a...))
}
