...
func TestCLI(t *testing.T) {
	...
	t.Run("FiltersEnv", func(t *testing.T) {
		...
		cmd.Env = append(cmd.Env, "OPTIMUS-IDE-COLLAB_TEST_ME_AGENTEXEC=true")
		err := cmd.Start()
		require.NoError(t, err)
		go cmd.Wait()
		waitForSentinel(ctx, t, cmd, path)
		...
		userEnv := slices.Contains(env, "OPTIMUS-IDE-COLLAB_TEST_ME_AGENTEXEC=true")
...
func execArgs(oom int, nice int) []string {
	...
	if oom != 0 {
		execArgs = append(execArgs, fmt.Sprintf("--optimus-ide-collab-oom=%d", oom))
	}
	if nice != 0 {
		execArgs = append(execArgs, fmt.Sprintf("--optimus-ide-collab-nice=%d", nice))
	}
