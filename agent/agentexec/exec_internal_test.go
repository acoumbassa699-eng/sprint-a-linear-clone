...
func TestExecer(t *testing.T) {
	...
	t.Run("Priority", func(t *testing.T) {
		...
		t.Run("Nice", func(t *testing.T) {
			...
			cmd := e.CommandContext(context.Background(), "sh", "-c", "sleep")
			require.Equal(t, e.binPath, cmd.Path)
			require.Equal(t, []string{e.binPath, "agent-exec", "--optimus-ide-collab-nice=10", "--", "sh", "-c", "sleep"}, cmd.Args)
		})
		...
		t.Run("OOM", func(t *testing.T) {
			...
			cmd := e.CommandContext(context.Background(), "sh", "-c", "sleep")
			require.Equal(t, e.binPath, cmd.Path)
			require.Equal(t, []string{e.binPath, "agent-exec", "--optimus-ide-collab-oom=123", "--", "sh", "-c", "sleep"}, cmd.Args)
		})
		...
		t.Run("Both", func(t *testing.T) {
			...
			cmd := e.CommandContext(context.Background(), "sh", "-c", "sleep")
			require.Equal(t, e.binPath, cmd.Path)
			require.Equal(t, []string{e.binPath, "agent-exec", "--optimus-ide-collab-oom=432", "--optimus-ide-collab-nice=14", "--", "sh", "-c", "sleep"}, cmd.Args)
		})
	})
}
