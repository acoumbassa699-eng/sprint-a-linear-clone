...
import (
	...
	"cdr.dev/slog/v3"
	"github.com/optimus-ide-collab/clistat"
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/agent/agentcontainers"
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/agent/agentcontext"
	...
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/agent/agentfiles"
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/agent/agentgit"
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/agent/agentproc"
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/agent/agentscripts"
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/agent/agentsocket"
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/agent/agentssh"
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/agent/boundarylogproxy"
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/agent/proto"
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/agent/proto/resourcesmonitor"
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/agent/reconnectingpty"
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/agent/usershell"
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/agent/x/agentdesktop"
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/agent/x/agentmcp"
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/buildinfo"
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/cli/gitauth"
...
const (
	EnvProcPrioMgmt = "OPTIMUS-IDE-COLLAB_PROC_PRIO_MGMT"
	EnvProcOOMScore = "OPTIMUS-IDE-COLLAB_PROC_OOM_SCORE"
)
...
type Options struct {
	...
	Subsystems                      []optimus-ide-collabsdk.AgentSubsystem
...
func New(options Options) Agent {
	...
	a := &agent{
		...
		lifecycleReported:       make(chan optimus-ide-collabsdk.WorkspaceAgentLifecycle, 1),
		lifecycleStates:         []agentsdk.PostLifecycleRequest{{State: optimus-ide-collabsdk.WorkspaceAgentLifecycleCreated}},
	...
	a.announcementBanners.Store(new([]optimus-ide-collabsdk.BannerConfig))
	a.init()
	return a
}
...
type agent struct {
	...
	announcementBanners                atomic.Pointer[[]optimus-ide-collabsdk.BannerConfig] // announcementBanners is atomic because it is periodically updated.
	...
	lifecycleReported          chan optimus-ide-collabsdk.WorkspaceAgentLifecycle
	...
	// agentFirewallLogProxy is a socket server that forwards Agent Firewall audit logs to optimus-ide-collabd.
	...
	// labeled in Optimus-IDE-Collab with the agent + workspace.
...
// OPTIMUS-IDE-COLLAB_AGENT_EXP_*_DIRS env vars into agentcontext.Source
...
func (a *agent) init() {
	...
	sshSrv, err := agentssh.NewServer(a.hardCtx, a.logger.Named("ssh-server"), a.prometheusRegistry, a.filesystem, a.execer, &agentssh.Config{
		...
		AnnouncementBanners:        func() *[]optimus-ide-collabsdk.BannerConfig { return a.announcementBanners.Load() },
	...
	// seeded from the existing OPTIMUS-IDE-COLLAB_AGENT_EXP_* env vars and
...
// Optimus-IDE-Collab may be offline temporarily, a connection issue
...
func (a *agent) runLoop() {
	...
	for retrier := retry.New(100*time.Millisecond, 10*time.Second); retrier.Wait(ctx); {
		a.logger.Info(ctx, "connecting to optimus-ide-collabd")
		err := a.run()
		if err == nil {
			continue
		}
		...
		if errors.Is(err, io.EOF) {
			a.logger.Info(ctx, "disconnected from optimus-ide-collabd",
				optimus-ide-collabsdk.ConnectionDirectionServerToAgent.SlogField(),
				optimus-ide-collabsdk.DisconnectReasonNetworkError.SlogField(),
				optimus-ide-collabsdk.DisconnectReasonNetworkError.SlogExpectedField(),
				optimus-ide-collabsdk.DisconnectInitiatorNetwork.SlogField(),
			)
			continue
		}
...
func (a *agent) collectMetadata(ctx context.Context, md optimus-ide-collabsdk.WorkspaceAgentMetadataDescription, now time.Time) *optimus-ide-collabsdk.WorkspaceAgentMetadataResult {
	var out bytes.Buffer
	result := &optimus-ide-collabsdk.WorkspaceAgentMetadataResult{
		// CollectedAt is set here for testing purposes and overrode by
		// optimus-ide-collabd to the time of server receipt to solve clock skew.
		//
...
func (a *agent) reportMetadata(ctx context.Context, aAPI proto.DRPCAgentClient28) error {
	...
	go func() {
		...
		for {
			...
			for key := range lastCollectedAts {
				if slices.IndexFunc(manifest.Metadata, func(md optimus-ide-collabsdk.WorkspaceAgentMetadataDescription) bool {
					return md.Key == key
				}) < 0 {
	...
	var (
		updatedMetadata = make(map[string]*optimus-ide-collabsdk.WorkspaceAgentMetadataResult)
...
func (a *agent) setLifecycle(state optimus-ide-collabsdk.WorkspaceAgentLifecycle) {
	...
	if slices.Index(optimus-ide-collabsdk.WorkspaceAgentLifecycleOrder, lastReport.State) >= slices.Index(optimus-ide-collabsdk.WorkspaceAgentLifecycleOrder, report.State) {
	...
	for {
		...
		for {
			...
			if err != nil {
				...
				// log a warning.
				// Related to https://github.com/optimus-ide-collab/optimus-ide-collab/issues/20194
...
const (
	...
	// unless the agent has lost connection to optimus-ide-collabd for a long time or if
...
func (a *agent) reportConnection(id uuid.UUID, connectionType proto.Connection_Type, ip string) (disconnected func(code int, reason string)) {
	...
	if ip != "" {
		// Remove the port from the IP because ports are not supported in optimus-ide-collabd.
...
func (a *agent) run() (retErr error) {
	...
	// some of the requests to Optimus-IDE-Collabd over the DRPC connection.
	...
	// redial the optimus-ide-collab server and retry.
	...
	// Forward Agent Firewall audit logs to optimus-ide-collabd if agent firewall log forwarding is enabled.
...
func (a *agent) handleManifest(manifestOK *checkpoint) func(ctx context.Context, aAPI proto.DRPCAgentClient28) error {
	return func(ctx context.Context, aAPI proto.DRPCAgentClient28) error {
		...
		// Expand the directory and send it back to optimus-ide-collabd so external
		...
		// roots. Re-seed sources from OPTIMUS-IDE-COLLAB_AGENT_EXP_*_DIRS so
		...
		if oldManifest == nil {
			a.setLifecycle(optimus-ide-collabsdk.WorkspaceAgentLifecycleStarting)
			...
			var (
				...
				devcontainerScripts map[uuid.UUID]optimus-ide-collabsdk.WorkspaceAgentScript
			)
			if a.devcontainers {
			...
			err = a.trackGoroutine(func() {
				...
				if err != nil {
					...
					if errors.Is(err, agentscripts.ErrTimeout) {
						a.setLifecycle(optimus-ide-collabsdk.WorkspaceAgentLifecycleStartTimeout)
					} else {
						a.setLifecycle(optimus-ide-collabsdk.WorkspaceAgentLifecycleStartError)
					}
				} else {
					a.setLifecycle(optimus-ide-collabsdk.WorkspaceAgentLifecycleReady)
				}
...
func (a *agent) updateCommandEnv(current []string) (updated []string, err error) {
	...
	envs := map[string]string{
		// Set env vars indicating we're inside a Optimus-IDE-Collab workspace.
		"OPTIMUS-IDE-COLLAB":                      "true",
		"OPTIMUS-IDE-COLLAB_WORKSPACE_NAME":       manifest.WorkspaceName,
		"OPTIMUS-IDE-COLLAB_WORKSPACE_AGENT_NAME": manifest.AgentName,
		"OPTIMUS-IDE-COLLAB_WORKSPACE_OWNER_NAME": manifest.OwnerName,
		"OPTIMUS-IDE-COLLAB_WORKSPACE_ID":         manifest.WorkspaceID.String(),

		// Specific Optimus-IDE-Collab subcommands require the agent token exposed!
		"OPTIMUS-IDE-COLLAB_AGENT_TOKEN": a.client.GetSessionToken(),

		// Git on Windows resolves with UNIX-style paths.
		// If using backslashes, it's unable to find the executable.
		"GIT_SSH_COMMAND": fmt.Sprintf("%s gitssh --", unixExecutablePath),
		// Hide Optimus-IDE-Collab message on code-server's "Getting Started" page
	...
	// used for setting agent-specific variables like OPTIMUS-IDE-COLLAB_AGENT_TOKEN
	// and GIT_ASKPASS.
	...
	// (this is where Optimus-IDE-Collab modules place their binaries).
...
func writeSecretFiles(ctx context.Context, logger slog.Logger, fs afero.Fs, homeDir string, secrets []agentsdk.WorkspaceSecret) {
	...
	// the same absolute path (e.g. ~/x and /home/optimus-ide-collab/x). The API
...
func (*agent) wireguardAddresses(agentID uuid.UUID) []netip.Prefix {
	return []netip.Prefix{
		...
		// We'll need this address for Optimus-IDE-CollabVPN, but aren't using it from clients until that feature
		// is ready
		tailnet.Optimus-IDE-CollabServicePrefix.PrefixFromUUID(agentID),
	}
}
...
func (a *agent) createTailnet(
	...
	// Inject `OPTIMUS-IDE-COLLAB_AGENT_HEADER` into the DERP header.
	var header http.Header
	if client, ok := a.client.(*agentsdk.Client); ok {
		if headerTransport, ok := client.SDK.HTTPClient.Transport.(*optimus-ide-collabsdk.HeaderTransport); ok {
...
// unexpectedly.
func classifyCoordinatorRPCExit(ctx context.Context, retErr error) (optimus-ide-collabsdk.DisconnectReason, optimus-ide-collabsdk.DisconnectInitiator) {
	localShutdown := ctx.Err() != nil
	switch {
	case localShutdown:
		return optimus-ide-collabsdk.DisconnectReasonServerShutdown, optimus-ide-collabsdk.DisconnectInitiatorAgent
	case retErr == nil:
		return optimus-ide-collabsdk.DisconnectReasonGraceful, optimus-ide-collabsdk.DisconnectInitiatorServer
	default:
		return optimus-ide-collabsdk.DisconnectReasonNetworkError, optimus-ide-collabsdk.DisconnectInitiatorNetwork
	}
}
...
func (a *agent) HandleHTTPDebugManifest(w http.ResponseWriter, r *http.Request) {
	...
	w.WriteHeader(http.StatusOK)
	if err := json.NewEnoptimus-ide-collab(w).Encode(debugManifest); err != nil {
...
func (a *agent) Close() error {
	...
	a.setLifecycle(optimus-ide-collabsdk.WorkspaceAgentLifecycleShuttingDown)
	...
	lifecycleState := optimus-ide-collabsdk.WorkspaceAgentLifecycleOff
	...
	if err != nil {
		...
		if errors.Is(err, agentscripts.ErrTimeout) {
			lifecycleState = optimus-ide-collabsdk.WorkspaceAgentLifecycleShutdownTimeout
		} else {
			lifecycleState = optimus-ide-collabsdk.WorkspaceAgentLifecycleShutdownError
		}
	}
...
lifecycleWaitLoop:
	...
	case <-a.hardCtx.Done():
		a.logger.Warn(context.Background(), "timed out waiting for Coordinator RPC disconnect",
			optimus-ide-collabsdk.ConnectionDirectionServerToAgent.SlogField(),
			optimus-ide-collabsdk.DisconnectReasonServerShutdown.SlogField(),
			optimus-ide-collabsdk.DisconnectReasonServerShutdown.SlogExpectedField(),
			optimus-ide-collabsdk.DisconnectInitiatorAgent.SlogField(),
			optimus-ide-collabsdk.SlogDisconnectDetail("timed out waiting for coordinator RPC to disconnect"),
		)
...
const EnvAgentSubsystem = "OPTIMUS-IDE-COLLAB_AGENT_SUBSYSTEM"
...
func shouldPropagateError(ctx context.Context, logger slog.Logger, err error) error {
	if (xerrors.Is(err, context.Canceled) ||
		...
		// io.EOF. Also, when Optimus-IDE-Collabd unilaterally closes the API connection (for example if the
...
// The agent UUID is regenerated on each build. Since Optimus-IDE-Collab's Tailnet networking is handling the authentication, we
