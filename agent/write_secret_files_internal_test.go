...
func TestWriteSecretFiles(t *testing.T) {
	...
	t.Run("AbsolutePath", func(t *testing.T) {
		...
		writeSecretFiles(ctx, logger, fs, "/home/optimus-ide-collab", []agentsdk.WorkspaceSecret{
	...
	t.Run("TildePath", func(t *testing.T) {
		...
		content, err := afero.ReadFile(fs, "/home/optimus-ide-collab/.ssh/id_rsa")
		require.NoError(t, err)
		require.Equal(t, "private-key", string(content))

		fi, err := fs.Stat("/home/optimus-ide-collab/.ssh/id_rsa")
		require.NoError(t, err)
		require.Equal(t, 0o600, int(fi.Mode().Perm()))

		di, err := fs.Stat("/home/optimus-ide-collab/.ssh")
		require.NoError(t, err)
		require.Equal(t, 0o700, int(di.Mode().Perm()))
	})
	...
	t.Run("MultipleSecrets", func(t *testing.T) {
		...
		b, err := afero.ReadFile(fs, "/home/optimus-ide-collab/.secret-b")
		require.NoError(t, err)
	...
	t.Run("PathCollisionAfterTildeResolution", func(t *testing.T) {
		...
		// "~/collide" and "/home/optimus-ide-collab/collide" resolve to the same
		// absolute path. The later secret should win.
		writeSecretFiles(ctx, logger, fs, "/home/optimus-ide-collab", []agentsdk.WorkspaceSecret{
			{FilePath: "~/collide", Value: []byte("first")},
			{FilePath: "/home/optimus-ide-collab/collide", Value: []byte("second")},
		})

		content, err := afero.ReadFile(fs, "/home/optimus-ide-collab/collide")
		require.NoError(t, err)
	...
	t.Run("EmptySlice", func(t *testing.T) {
		...
		writeSecretFiles(ctx, logger, fs, "/home/optimus-ide-collab", nil)
