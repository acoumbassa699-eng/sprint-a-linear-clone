...
func New(opts Options) *Runner {
	...
	return &Runner{
		...
		dataDir:       filepath.Join(opts.DataDirBase, "optimus-ide-collab-script-data"),
...
func (r *Runner) Init(scripts []optimus-ide-collabsdk.WorkspaceAgentScript, scriptCompleted ScriptCompletedFunc, opts ...InitOption) error {
	r.initMutex.Lock()
...
func (r *Runner) trackRun(ctx context.Context, script optimus-ide-collabsdk.WorkspaceAgentScript, option ExecuteOption) error {
...
func (r *Runner) run(ctx context.Context, script optimus-ide-collabsdk.WorkspaceAgentScript, option ExecuteOption) error {
	logPath := script.LogPath
	if logPath == "" {
		logPath = fmt.Sprintf("optimus-ide-collab-script-%s.log", script.LogSourceID)
	}
	if logPath[0] == '~' {
	...
	// for the script to use, like OPTIMUS-IDE-COLLAB_SCRIPT_DATA_DIR for persistent
	// storage.
	cmd.Env = append(cmd.Env, "OPTIMUS-IDE-COLLAB_SCRIPT_DATA_DIR="+scriptDataDir)
	cmd.Env = append(cmd.Env, "OPTIMUS-IDE-COLLAB_SCRIPT_BIN_DIR="+r.ScriptBinDir())
	...
	infoW := agentsdk.LogsWriter(ctx, scriptLogger.Send, script.LogSourceID, optimus-ide-collabsdk.LogLevelInfo)
	defer infoW.Close()
	errW := agentsdk.LogsWriter(ctx, scriptLogger.Send, script.LogSourceID, optimus-ide-collabsdk.LogLevelError)
	defer errW.Close()
	...
	case errors.Is(err, exec.ErrWaitDelay):
		...
		details := fmt.Sprint(
			"This usually means a child process was started with references to stdout or stderr. As a result, this " +
				...
				"\"optimus-ide-collab_script\" for the process, see " +
				"https://optimus-ide-collab.com/docs/admin/templates/troubleshooting#startup-script-issues for more information.",
		)
