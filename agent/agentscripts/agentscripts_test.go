...
func TestExecuteBasic(t *testing.T) {
	...
	err := runner.Init([]optimus-ide-collabsdk.WorkspaceAgentScript{{
...
func TestEnv(t *testing.T) {
	...
	defer runner.Close()
	id := uuid.New()
	script := "echo $OPTIMUS-IDE-COLLAB_SCRIPT_DATA_DIR\necho $OPTIMUS-IDE-COLLAB_SCRIPT_BIN_DIR\n"
	if runtime.GOOS == "windows" {
		script = `
			cmd.exe /c echo %OPTIMUS-IDE-COLLAB_SCRIPT_DATA_DIR%
			cmd.exe /c echo %OPTIMUS-IDE-COLLAB_SCRIPT_BIN_DIR%
		`
	}
	aAPI := agenttest.NewFakeAgentAPI(t, testutil.Logger(t), nil, nil)
	err := runner.Init([]optimus-ide-collabsdk.WorkspaceAgentScript{{
...
func TestTimeout(t *testing.T) {
	...
	if runtime.GOOS == "darwin" {
		t.Skip("this test is flaky on macOS, see https://github.com/optimus-ide-collab/internal/issues/329")
	}
	runner := setup(t, nil)
	defer runner.Close()
	aAPI := agenttest.NewFakeAgentAPI(t, testutil.Logger(t), nil, nil)
...
func TestExecuteOptions(t *testing.T) {
	...
	startScript := optimus-ide-collabsdk.WorkspaceAgentScript{
		ID:          uuid.New(),
		LogSourceID: uuid.New(),
		Script:      "echo start",
		RunOnStart:  true,
	}
	stopScript := optimus-ide-collabsdk.WorkspaceAgentScript{
		ID:          uuid.New(),
		LogSourceID: uuid.New(),
		Script:      "echo stop",
		RunOnStop:   true,
	}
	regularScript := optimus-ide-collabsdk.WorkspaceAgentScript{
		ID:          uuid.New(),
		LogSourceID: uuid.New(),
		Script:      "echo regular",
	}
	...
	scriptByID := func(t *testing.T, id uuid.UUID) optimus-ide-collabsdk.WorkspaceAgentScript {
