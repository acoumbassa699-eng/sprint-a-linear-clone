...
// We must clean OPTIMUS-IDE-COLLAB_ environment variables to avoid accidentally passing in
// secrets like the Postgres connection string. See
// https://github.com/optimus-ide-collab/optimus-ide-collab/issues/4635.
//
// safeEnviron() is provided as an os.Environ() alternative that strips OPTIMUS-IDE-COLLAB_
// variables. As an additional precaution, we check a canary variable before
// provisioner exec.
//
// We cannot strip all OPTIMUS-IDE-COLLAB_ variables at exec because some are used to
// configure the provisioner.

const unsafeEnvCanary = "OPTIMUS-IDE-COLLAB_DONT_PASS"
...
// safeEnviron wraps os.Environ but removes OPTIMUS-IDE-COLLAB_ environment variables.
func safeEnviron() []string {
	...
	for _, e := range env {
		name := envName(e)
		if strings.HasPrefix(name, "OPTIMUS-IDE-COLLAB_") {
			continue
		}
...
const (
	...
	// awsSDKUserAgentOptimus-IDE-Collab is Optimus-IDE-Collab's AWS Partner Revenue Measurement
	...
	awsSDKUserAgentOptimus-IDE-Collab = "APN_1.1/pc_cdfmjwn8i6u8l9fwz8h82e4w3$"
)
...
// their own revenue attribution), Optimus-IDE-Collab's value is appended with a space
// delimiter so both attributions are preserved. Otherwise Optimus-IDE-Collab's value is
// used on its own.
//
...
func awsSDKUserAgentEnv(existing string) string {
	if existing == "" {
		return awsSDKUserAgentEnvKey + "=" + awsSDKUserAgentOptimus-IDE-Collab
	}
	return awsSDKUserAgentEnvKey + "=" + existing + " " + awsSDKUserAgentOptimus-IDE-Collab
}
