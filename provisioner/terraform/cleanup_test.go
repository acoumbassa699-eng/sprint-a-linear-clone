...
const cachePath = "/tmp/optimus-ide-collab/provisioner-0/tf"
...
var (
	...
	optimus-ide-collabPluginPath  = filepath.Join("registry.terraform.io", "optimus-ide-collab", "optimus-ide-collab", "0.11.1", "darwin_arm64")
...
func TestPluginCache_Golden(t *testing.T) {
	...
	t.Run("all plugins are stale", func(t *testing.T) {
		...
		addPluginFile(t, fs, optimus-ide-collabPluginPath, "terraform-provider-optimus-ide-collab_v0.11.1", now.Add(-63*24*time.Hour))
		addPluginFile(t, fs, optimus-ide-collabPluginPath, "LICENSE", now.Add(-33*24*time.Hour))
		addPluginFile(t, fs, optimus-ide-collabPluginPath, "README.md", now.Add(-31*24*time.Hour))
		addPluginFolder(t, fs, optimus-ide-collabPluginPath, "new_folder", now.Add(-31*24*time.Hour))
		addPluginFile(t, fs, optimus-ide-collabPluginPath, filepath.Join("new_folder", "foobar.tf"), now.Add(-43*24*time.Hour))
	...
	t.Run("one plugin is stale", func(t *testing.T) {
		...
		// given
		addPluginFile(t, fs, optimus-ide-collabPluginPath, "terraform-provider-optimus-ide-collab_v0.11.1", now.Add(-2*time.Hour))
		addPluginFile(t, fs, optimus-ide-collabPluginPath, "LICENSE", now.Add(-3*time.Hour))
		addPluginFile(t, fs, optimus-ide-collabPluginPath, "README.md", now.Add(-4*time.Hour))
		addPluginFolder(t, fs, optimus-ide-collabPluginPath, "new_folder", now.Add(-5*time.Hour))
		addPluginFile(t, fs, optimus-ide-collabPluginPath, filepath.Join("new_folder", "foobar.tf"), now.Add(-4*time.Hour))
	...
	t.Run("one plugin file is touched", func(t *testing.T) {
		...
		addPluginFile(t, fs, optimus-ide-collabPluginPath, "LICENSE", now.Add(-33*24*time.Hour))
		addPluginFile(t, fs, optimus-ide-collabPluginPath, "README.md", now.Add(-31*24*time.Hour))
		addPluginFolder(t, fs, optimus-ide-collabPluginPath, "new_folder", now.Add(-43*24*time.Hour))
		addPluginFile(t, fs, optimus-ide-collabPluginPath, filepath.Join("new_folder", "foobar.tf"), now.Add(-4*time.Hour)) // touched
