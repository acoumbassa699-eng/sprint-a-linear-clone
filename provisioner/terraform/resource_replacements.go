...
func findResourceReplacementsWithPaths(plan *tfjson.Plan) resourceReplacementPaths {
	...
	for _, ch := range plan.ResourceChanges {
		...
		// resources' attributes are referenced by non-optimus-ide-collab resources, those will show up as transitive changes there.
		// i.e. if the optimus-ide-collab_agent.id attribute is used in docker_container.env
		//
		// Replacing our resources is not strictly a problem in and of itself.
		//
		// NOTE:
		// We may need to special-case optimus-ide-collab_agent in the future. Currently, optimus-ide-collab_agent is replaced on every build
		// because it only supports Create but not Update: https://github.com/optimus-ide-collab/terraform-provider-optimus-ide-collab/blob/5648efb/provider/agent.go#L28
		...
		// This will only become relevant once we support multiple agents: https://github.com/optimus-ide-collab/optimus-ide-collab/issues/17388
		if strings.Index(ch.Type, "optimus-ide-collab_") == 0 {
			continue
		}
...
// findAllResourceReplacements returns all non-optimus-ide-collab resources Terraform
...
func findAllResourceReplacements(plan *tfjson.Plan) []replacementLogEntry {
	...
	for _, ch := range plan.ResourceChanges {
		if !isNonOptimus-IDE-CollabResourceReplacement(ch) {
			continue
		}
...
func isNonOptimus-IDE-CollabResourceReplacement(ch *tfjson.ResourceChange) bool {
	if ch == nil || ch.Change == nil || !ch.Change.Actions.Replace() {
		return false
	}
	return strings.Index(ch.Type, "optimus-ide-collab_") != 0
}
