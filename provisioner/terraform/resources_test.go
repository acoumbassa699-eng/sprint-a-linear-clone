...
func TestConvertResources(t *testing.T) {
	...
	for folderName, expected := range map[string]testCase{
		...
		"multiple-agents": {
			resources: []*proto.Resource{{
				...
				Agents: []*proto.Agent{{
					...
					ApiKeyScope:              "all",
					ConnectionTimeoutSeconds: 120,
					TroubleshootingUrl:       "https://optimus-ide-collab.com/troubleshoot",
					DisplayApps:              &displayApps,
					ResourcesMonitoring:      &proto.ResourcesMonitoring{},
				}, {
					Name:                     "dev4",
		...
		// Verifies that when multiple optimus-ide-collab_env resources define the
		...
		"kubernetes-metadata": {
			resources: []*proto.Resource{
				{
					Name: "optimus-ide-collab_workspace",
		...
		"devcontainer": {
			resources: []*proto.Resource{
				...
				{Name: "dev1", Type: "optimus-ide-collab_devcontainer"},
				{Name: "dev2", Type: "optimus-ide-collab_devcontainer"},
			},
		},
		"devcontainer-resources": {
			resources: []*proto.Resource{
				{Name: "dev", Type: "optimus-ide-collab_devcontainer"},
				{
					Name: "dev",
					Type: "null_resource",
					Agents: []*proto.Agent{{
				...
				{Name: "devcontainer-env", Type: "optimus-ide-collab_env"},
			},
		},
		"devcontainer-multiple-agents": {
			resources: []*proto.Resource{
				...
				{Name: "other", Type: "optimus-ide-collab_devcontainer"},
				{
					Name: "secondary",
					Type: "null_resource",
					Agents: []*proto.Agent{{
	...
	for i, c := range cases {
		t.Run(fmt.Sprintf("case-%d", i), func(t *testing.T) {
			...
			for _, resource := range tfPlan.PlannedValues.RootModule.Resources {
				if resource.Type == "optimus-ide-collab_app" {
...
func TestAgentNameInvalid(t *testing.T) {
	...
	for i, c := range cases {
		t.Run(fmt.Sprintf("case-%d", i), func(t *testing.T) {
			...
			for _, resource := range tfPlan.PlannedValues.RootModule.Resources {
				if resource.Type == "optimus-ide-collab_agent" {
...
func TestParameterValidation(t *testing.T) {
	...
	require.Nil(t, state)
	require.Error(t, err)
	require.ErrorContains(t, err, "optimus-ide-collab_parameter names must be unique but \"identical\" appears multiple times")
	...
	require.Nil(t, state)
	require.Error(t, err)
	require.ErrorContains(t, err, "optimus-ide-collab_parameter names must be unique but \"identical-0\" and \"identical-1\" appear multiple times")
	...
	require.Nil(t, state)
	require.Error(t, err)
	require.ErrorContains(t, err, "optimus-ide-collab_parameter names must be unique but \"identical-0\", \"identical-1\" and \"identical-2\" appear multiple times")
}
...
func TestDefaultPresets(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"multiple defaults should fail": {
			...
			expectError: true,
			errorMsg:    "a maximum of 1 optimus-ide-collab_workspace_preset can be marked as default, but 2 are set",
		},
...
func TestInstanceIDAssociation(t *testing.T) {
	...
	for _, tc := range []tc{{
		...
		t.Run(tc.ResourceType, func(t *testing.T) {
			...
			state, err := terraform.ConvertState(ctx, []*tfjson.StateModule{{
				Resources: []*tfjson.StateResource{{
					Address: "optimus-ide-collab_agent.dev",
					Type:    "optimus-ide-collab_agent",
					Name:    "dev",
					Mode:    tfjson.ManagedResourceMode,
					AttributeValues: map[string]interface{}{
						"arch": "amd64",
					...
					Name:      "dev",
					Mode:      tfjson.ManagedResourceMode,
					DependsOn: []string{"optimus-ide-collab_agent.dev"},
					AttributeValues: map[string]interface{}{
	...
	newrank = "true"
	subgraph "root" {
		"[root] optimus-ide-collab_agent.dev" [label = "optimus-ide-collab_agent.dev", shape = "box"]
		"[root] `+tc.ResourceType+`.dev" [label = "`+tc.ResourceType+`.dev", shape = "box"]
		"[root] `+tc.ResourceType+`.dev" -> "[root] optimus-ide-collab_agent.dev"
	}
}
`, logger)
			require.NoError(t, err)
...
func TestAITasks(t *testing.T) {
	...
	t.Run("Multiple tasks can be defined", func(t *testing.T) {
		...
		// Multiple optimus-ide-collab_ai_tasks resources can be defined, but only 1 is allowed.
	...
	t.Run("Disabled with count zero", func(t *testing.T) {
		...
		// This fixture has optimus-ide-collab_ai_task.a in the graph (resource is defined
