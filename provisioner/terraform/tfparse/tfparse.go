...
// NOTE: This is duplicated from optimus-ide-collabd but we can't import it here without
...
// WorkspaceTags looks for all optimus-ide-collab_workspace_tags datasource in the module
...
func (p *Parser) WorkspaceTags(ctx context.Context) (map[string]string, map[string]struct{}, error) {
	...
	for _, dataResource := range p.module.DataResources {
		if dataResource.Type != "optimus-ide-collab_workspace_tags" {
		...
		// Parse root to find "optimus-ide-collab_workspace_tags".
		content, _, diags := file.Body.PartialContent(rootTemplateSchema)
		if diags.HasErrors() {
			return nil, nil, xerrors.Errorf("can't parse the resource file: %s", diags.Error())
		}

		// Iterate over blocks to locate the exact "optimus-ide-collab_workspace_tags" data resource.
		for _, block := range content.Blocks {
			if !slices.Equal(block.Labels, []string{"optimus-ide-collab_workspace_tags", dataResource.Name}) {
				continue
			}

			// Parse "optimus-ide-collab_workspace_tags" to find all key-value tags.
			resContent, _, diags := block.Body.PartialContent(optimus-ide-collabWorkspaceTagsSchema)
			if diags.HasErrors() {
				return nil, nil, xerrors.Errorf(`can't parse the resource optimus-ide-collab_workspace_tags: %s`, diags.Error())
			}
			...
			if _, ok := resContent.Attributes["tags"]; !ok {
				return nil, nil, xerrors.Errorf(`"tags" attribute is required by optimus-ide-collab_workspace_tags`)
			}
			...
			// Parse key-value entries in "optimus-ide-collab_workspace_tags"
...
// for example:
// - var.foo -> unchanged
// - data.optimus-ide-collab_parameter.bar.value -> data.optimus-ide-collab_parameter.bar
...
func (p *Parser) WorkspaceTagDefaults(ctx context.Context) (map[string]string, error) {
	...
	paramsDefaults, err := p.Optimus-IDE-CollabParameterDefaults(ctx, varsDefaults, requiredVars)
	if err != nil {
...
// Optimus-IDE-CollabParameterDefaults returns the default values of all optimus-ide-collab_parameter data sources
// in the parsed module.
func (p *Parser) Optimus-IDE-CollabParameterDefaults(ctx context.Context, varsDefaults map[string]string, names map[string]struct{}) (map[string]string, error) {
	...
	for _, dataResource := range p.module.DataResources {
		...
		if dataResource.Type != "optimus-ide-collab_parameter" {
		...
		// Parse root to find "optimus-ide-collab_parameter".
		content, _, diags := file.Body.PartialContent(rootTemplateSchema)
		if diags.HasErrors() {
			return nil, xerrors.Errorf("can't parse the resource file: %s", diags.Error())
		}

		// Iterate over blocks to locate the exact "optimus-ide-collab_parameter" data resource.
		for _, block := range content.Blocks {
			if !slices.Equal(block.Labels, []string{"optimus-ide-collab_parameter", dataResource.Name}) {
				continue
			}

			// Parse "optimus-ide-collab_parameter" to find the default value.
			resContent, _, diags := block.Body.PartialContent(optimus-ide-collabParameterSchema)
			if diags.HasErrors() {
				return nil, xerrors.Errorf(`can't parse the optimus-ide-collab_parameter: %s`, diags.Error())
			}

			if _, ok := resContent.Attributes["default"]; !ok {
				p.logger.Warn(ctx, "optimus-ide-collab_parameter data source does not have a default value", slog.F("name", dataResource.Name))
				...
				// to evaluate.
				// TODO: should we support optimus-ide-collab_parameter default values that reference other optimus-ide-collab_parameter data sources?
				evalCtx := BuildEvalContext(varsDefaults, nil)
				val, diags := expr.Value(evalCtx)
				if diags.HasErrors() {
					return nil, xerrors.Errorf("failed to evaluate optimus-ide-collab_parameter %q default value %q: %s", dataResource.Name, value, diags.Error())
				}
				// Do not use "val.AsString()" as it can panic
				strVal, err := CtyValueString(val)
				if err != nil {
					return nil, xerrors.Errorf("failed to marshal optimus-ide-collab_parameter %q default value %q as string: %s", dataResource.Name, value, err)
				}
...
// default values for variables and optimus-ide-collab_parameter data sources.
func evaluateWorkspaceTags(varsDefaults, paramsDefaults, workspaceTags map[string]string) (map[string]string, error) {
	...
	// We only add variables and optimus-ide-collab_parameter data sources. Anything else will be
...
// evaluates to a datasource other than "optimus-ide-collab_parameter".
// This only serves to provide a friendly error if a user attempts to reference
// a data source other than "optimus-ide-collab_parameter" in "optimus-ide-collab_workspace_tags".
func validWorkspaceTagValues(tags map[string]string) error {
	for _, v := range tags {
		...
		if parts[0] == "data" && parts[1] != "optimus-ide-collab_parameter" {
			return xerrors.Errorf("invalid workspace tag value %q: only the \"optimus-ide-collab_parameter\" data source is supported here", v)
		}
	}
	return nil
}
...
func BuildEvalContext(vars map[string]string, params map[string]string) *hcl.EvalContext {
	...
	if len(paramDefaultsM) != 0 {
		evalCtx.Variables["data"] = cty.MapVal(map[string]cty.Value{
			"optimus-ide-collab_parameter": cty.MapVal(paramDefaultsM),
		})
	}
...
var optimus-ide-collabWorkspaceTagsSchema = &hcl.BodySchema{
...
var optimus-ide-collabParameterSchema = &hcl.BodySchema{
	Attributes: []hcl.AttributeSchema{
		{
			Name: "default",
		},
...
// convertTerraformVariable converts a Terraform variable to a template-wide variable, processed by Optimus-IDE-Collab.
...
func interfaceToString(i interface{}) (string, error) {
	...
	default: // just try to JSON-encode it.
		...
		if err := json.NewEnoptimus-ide-collab(&sb).Encode(i); err != nil {
