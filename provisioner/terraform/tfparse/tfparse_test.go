...
func Test_WorkspaceTagDefaultsFromFile(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		},
		{
			...
			files: map[string]string{
				"main.tf": `
					...
					data "optimus-ide-collab_parameter" "az" {
						name = "az"
						type = "string"
						default = "a"
					}`,
		...
		},
		{
			...
			expectTags:  map[string]string{},
			expectError: `"tags" attribute is required by optimus-ide-collab_workspace_tags`,
		},
		{
				...
				"main.tf": `
					...
					data "optimus-ide-collab_workspace_tags" "tags" {
						tags = {
							...
							"cluster"  = "${"devel"}${"opers"}"
							"region"   = var.region
							"az"       = data.optimus-ide-collab_parameter.az.value
						}
					}`,
			},
		...
		},
		{
			...
			files: map[string]string{
				"main.tf": `
					...
					data "optimus-ide-collab_parameter" "az2" {
					  name = "az"
						type = "string"
						default = data.optimus-ide-collab_parameter.az.value
					}
					data "optimus-ide-collab_workspace_tags" "tags" {
						tags = {
							"platform" = "kubernetes",
							"cluster"  = "${"devel"}${"opers"}"
							"region"   = var.region
							"az"       = data.optimus-ide-collab_parameter.az2.value
						}
					}`,
			},
		...
		},
		{
			...
			files: map[string]string{
				"main.tf": `
					...
					data "optimus-ide-collab_workspace_tags" "more_tags" {
						tags = {
							"foo" = "bar"
						}
					}`,
		...
		},
		{
			...
			files: map[string]string{
				"main.tf": `
					...
					data "optimus-ide-collab_parameter" "notaz" {
						name = "notaz"
						type = "string"
					}
					data "optimus-ide-collab_workspace_tags" "tags" {
		...
		},
		{
			...
			expectTags:  nil,
			expectError: `invalid workspace tag value "data.local_file.hostname.content": only the "optimus-ide-collab_parameter" data source is supported here`,
		},
		{
				...
				"main.tf": `
					...
					data "optimus-ide-collab_workspace_tags" "tags" {
						tags = {
							...
							"az"        = try(split(".", data.optimus-ide-collab_parameter.az.value)[1], "placeholder")
						}
					}`,
			},
		...
		},
		{
			...
			files: map[string]string{
				"main.tf": `
					...
					data "optimus-ide-collab_parameter" "stringparam" {
						name    = "stringparam"
						type    = "string"
						default = "a"
					}
					data "optimus-ide-collab_parameter" "numparam" {
						name    = "numparam"
						type    = "number"
						default = 1
					}
					data "optimus-ide-collab_parameter" "boolparam" {
						name    = "boolparam"
						type    = "bool"
						default = true
					}
					data "optimus-ide-collab_parameter" "listparam" {
						name    = "listparam"
						type    = "list(string)"
						default = "[\"a\", \"b\"]"
					}
					data "optimus-ide-collab_workspace_tags" "tags" {
						tags = {
							...
							"listvar"     = var.listvar
							"mapvar"      = var.mapvar
							"stringparam" = data.optimus-ide-collab_parameter.stringparam.value
							"numparam"    = data.optimus-ide-collab_parameter.numparam.value
							"boolparam"   = data.optimus-ide-collab_parameter.boolparam.value
							"listparam"   = data.optimus-ide-collab_parameter.listparam.value
						}
					}`,
			},
			expectTags: map[string]string{
...
// goos: linux
// goarch: amd64
// pkg: github.com/optimus-ide-collab/optimus-ide-collab/v2/provisioner/terraform/tfparse
