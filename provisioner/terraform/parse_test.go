	...
	testCases := []struct {
		...
		},
		{
			...
			Files: map[string]string{
				`main.tf`: `
				`,
				"parameters.tf": `data "optimus-ide-collab_parameter" "os_selector" {
					name         = "os_selector"
				  ...
				  data "optimus-ide-collab_parameter" "feature_cache_enabled" {
				  ...
				  data "optimus-ide-collab_parameter" "feature_debug_enabled" {
				...
				"tags.tf": `data "optimus-ide-collab_workspace_tags" "custom_workspace_tags" {
					tags = {
					  "cluster" = "developers"
					  "os"      = data.optimus-ide-collab_parameter.os_selector.value
					  "debug"   = "${data.optimus-ide-collab_parameter.feature_debug_enabled.value}+12345"
					  "cache"   = data.optimus-ide-collab_parameter.feature_cache_enabled.value == "true" ? "nix-with-cache" : "no-cache"
					}
				  }`,
			},
			Response: &proto.ParseComplete{
		...
		},
		{
			...
			Files: map[string]string{
				"main.tf": `
				  data "optimus-ide-collab_parameter" "os_selector" {
					name         = "os_selector"
					display_name = "Operating System"
					mutable      = false
		...
		},
		{
			...
			ParseErrorContains: `"tags" attribute is required by optimus-ide-collab_workspace_tags`,
		},
		{
			Name: "empty-main",
			Files: map[string]string{
