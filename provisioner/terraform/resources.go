...
// A mapping of attributes on the "optimus-ide-collab_agent" resource.
...
// A mapping of attributes on the "optimus-ide-collab_app" resource.
...
// A mapping of attributes on the "optimus-ide-collab_metadata" resource.
...
func hasExternalAgentResources(graph *gographviz.Graph) bool {
	for _, node := range graph.Nodes.Lookup {
		if label, exists := node.Attrs["label"]; exists {
			...
			if strings.HasPrefix(labelValue, "optimus-ide-collab_external_agent.") || strings.Contains(labelValue, ".optimus-ide-collab_external_agent.") {
				return true
			}
		}
	}
...
// produced by `terraform graph` to produce resources consumable by Optimus-IDE-Collab.
...
func ConvertState(ctx context.Context, modules []*tfjson.StateModule, rawGraph string, logger slog.Logger) (*State, error) {
	...
	findTerraformResources = func(mod *tfjson.StateModule) {
		...
		for _, resource := range mod.Resources {
			if resource.Type == "optimus-ide-collab_parameter" {
				tfResourcesRichParameters = append(tfResourcesRichParameters, resource)
			}
			if resource.Type == "optimus-ide-collab_workspace_preset" {
				tfResourcesPresets = append(tfResourcesPresets, resource)
			}
			if resource.Type == "optimus-ide-collab_ai_task" {
	...
	for _, tfResource := range sortedResources["optimus-ide-collab_agent"] {
		...
		// `optimus-ide-collab_app`s and Optimus-IDE-CollabVPN DNS names.
		...
		startupScriptBehavior := string(optimus-ide-collabsdk.WorkspaceAgentStartupScriptBehaviorNonBlocking)
		if attrs.StartupScriptBehavior != "" {
			...
			if _, ok := tfResource.AttributeValues["login_before_ready"]; ok && !attrs.LoginBeforeReady {
				startupScriptBehavior = string(optimus-ide-collabsdk.WorkspaceAgentStartupScriptBehaviorBlocking)
			}
		}
		...
		if attrs.StartupScript != "" {
			agent.Scripts = append(agent.Scripts, &proto.Script{
				...
				StartBlocksLogin: startupScriptBehavior == string(optimus-ide-collabsdk.WorkspaceAgentStartupScriptBehaviorBlocking),
				RunOnStart:       true,
			})
		}
	...
	for _, resource := range sortedResources["optimus-ide-collab_devcontainer"] {
	...
	for _, resource := range sortedResources["optimus-ide-collab_agent_instance"] {
		...
		for _, agents := range resourceAgents {
			for _, agent := range agents {
				...
				// https://github.com/optimus-ide-collab/optimus-ide-collab/issues/4551#issuecomment-1336293468
	...
	for _, resource := range sortedResources["optimus-ide-collab_app"] {
		...
		if appID == "" {
			...
			// https://github.com/optimus-ide-collab/terraform-provider-optimus-ide-collab/blob/cfa101df4635e405e66094fa7779f9a89d92f400/provider/app.go#L37
			logger.Warn(ctx, "optimus-ide-collab_app's id was unexpectedly empty", slog.F("name", attrs.Name))
	...
	// When multiple optimus-ide-collab_env resources define the same key, the last one
	// by sorted address wins, ensuring stable behavior across builds.
	sortedEnvResources := sortedResources["optimus-ide-collab_env"]
	...
	sortedScriptResources := sortedResources["optimus-ide-collab_script"]
	...
	for _, resource := range sortedResources["optimus-ide-collab_metadata"] {
	...
	for _, resource := range managedNonOptimus-IDE-CollabResources(sortedResources) {
		label := convertAddressToLabel(resource.Address)
	...
	for _, resource := range tfResourcesRichParameters {
		...
		if err != nil {
			return nil, xerrors.Errorf("decode map values for optimus-ide-collab_parameter.%s: %w", resource.Name, err)
		}
		...
		if err != nil {
			return nil, xerrors.Errorf("decode form_type for optimus-ide-collab_parameter.%s: %w", resource.Name, err)
		}
		...
		if len(param.Validation) == 1 {
			...
			if ok {
				...
				if ok {
					...
					if ok {
						// Backward compatibility with terraform-optimus-ide-collab-plugin < v0.8.2:
	...
	if len(duplicatedParamNames) > 0 {
		...
		return nil, xerrors.Errorf(
			"optimus-ide-collab_parameter names must be unique but %s appear%s multiple times",
	...
	for _, resource := range tfResourcesPresets {
		...
		if len(duplicatedPresetParameterNames) > 0 {
			...
			return nil, xerrors.Errorf(
				"optimus-ide-collab_workspace_preset parameters must be unique but %s appear%s multiple times", stringutil.JoinWithConjunction(duplicatedPresetParameterNames), s,
			)
		}
		...
		if len(nonExistentParameters) > 0 {
			logger.Warn(
				ctx,
				"optimus-ide-collab_workspace_preset defines preset values for at least one parameter that is not defined by the template",
		...
		if len(preset.Prebuilds) != 1 {
			logger.Warn(
				ctx,
				"optimus-ide-collab_workspace_preset must have exactly one prebuild block",
			)
		}
	...
	if len(duplicatedPresetNames) > 0 {
		...
		return nil, xerrors.Errorf(
			"optimus-ide-collab_workspace_preset names must be unique but %s appear%s multiple times",
	...
	if defaultPresets > 1 {
		return nil, xerrors.Errorf("a maximum of 1 optimus-ide-collab_workspace_preset can be marked as default, but %d are set", defaultPresets)
	}
	...
	for _, resource := range tfResourcesAITasks {
		...
		if err != nil {
			return nil, xerrors.Errorf("decode optimus-ide-collab_ai_task attributes: %w", err)
		}
	...
	// Process the legacy optimus-ide-collab_git_auth type first so that
	// optimus-ide-collab_external_auth takes precedence when both exist
	// with the same provider ID.
	for _, resource := range sortedResources["optimus-ide-collab_git_auth"] {
	...
	for _, resource := range sortedResources["optimus-ide-collab_external_auth"] {
		id, ok := resource.AttributeValues["id"].(string)
		if !ok {
			return nil, xerrors.Errorf("external auth id is not a string")
		}
...
// managedNonOptimus-IDE-CollabResources returns all managed resources that are not
// internal Optimus-IDE-Collab types, sorted by address. It uses the pre-grouped
// map from sortResourcesByType.
func managedNonOptimus-IDE-CollabResources(byType map[string][]*tfjson.StateResource) []*tfjson.StateResource {
	skip := map[string]bool{
		"optimus-ide-collab_script": true, "optimus-ide-collab_agent": true,
		"optimus-ide-collab_agent_instance": true, "optimus-ide-collab_app": true,
		"optimus-ide-collab_metadata": true,
	}
...
func dependsOnAgent(graph *gographviz.Graph, agent *proto.Agent, resourceAgentID string, resource *tfjson.StateResource) bool {
	...
	if agent.Id == "" && resourceAgentID == "" {
		...
		agentNodeSuffix := fmt.Sprintf(`] optimus-ide-collab_agent.%s (expand)"`, agent.Name)

		// Traverse the graph to check if the optimus-ide-collab_<resource_type> depends on optimus-ide-collab_agent.
...
func dependsOnDevcontainer(graph *gographviz.Graph, dc *proto.Devcontainer, resourceAgentID string, resource *tfjson.StateResource) bool {
	...
	if dc.SubagentId == "" && resourceAgentID == "" {
		resourceNodeSuffix := fmt.Sprintf(`] %s.%s (expand)"`, resource.Type, resource.Name)
		agentNodeSuffix := fmt.Sprintf(`] optimus-ide-collab_devcontainer.%s (expand)"`, dc.Name)

		// Traverse the graph to check if the optimus-ide-collab_<resource_type> depends on optimus-ide-collab_devcontainer.
		for _, dst := range graph.Edges.SrcToDsts {
			for _, edges := range dst {
				for _, edge := range edges {
					if strings.HasSuffix(edge.Src, resourceNodeSuffix) &&
...
func findResourcesInGraph(graph *gographviz.Graph, tfResourcesByLabel map[string]map[string]*tfjson.StateResource, nodeName string, currentDepth uint, up bool) []*graphResource {
	...
	for destination := range mapping[nodeName] {
		...
		for _, resource := range resources {
			...
			// Don't associate Optimus-IDE-Collab resources with other Optimus-IDE-Collab resources!
			// Except for optimus-ide-collab_external_agent, which is a special case.
			if strings.HasPrefix(resource.Type, "optimus-ide-collab_") && resource.Type != "optimus-ide-collab_external_agent" {
				continue
			}
