...
// terraform-provider-optimus-ide-collab
// nolint: paralleltest
func TestProvision_TextFileBusy(t *testing.T) {
	...
	err = os.Mkdir(filepath.Join(workDir, ".optimus-ide-collab"), 0o700)
	require.NoError(t, err)
	l, err := net.Listen("unix", filepath.Join(workDir, ".optimus-ide-collab", "pprof"))
	require.NoError(t, err)
	defer l.Close()
	handlerCalled := 0
	// nolint: gosec
	...
	testCases := []struct {
		...
		},
		{
			...
			Files: map[string]string{
				"main.tf": `terraform {
				  ...
				  data "optimus-ide-collab_parameter" "sample" {
					name = "Sample"
					type = "string"
					default = "foobaz"
				  }

				  data "optimus-ide-collab_parameter" "example" {
					name = "Example"
					type = "string"
					default = "foobar"
				  }
				  ...
				  resource "null_resource" "example" {
					triggers = {
						misc = "${data.optimus-ide-collab_parameter.example.value}"
					}
				  }`,
			},
			Request: &proto.PlanRequest{
		...
		},
		{
			...
			Files: map[string]string{
				...
				data "optimus-ide-collab_git_auth" "github" {
					id = "github"
				}

				resource "null_resource" "example" {}

				resource "optimus-ide-collab_metadata" "example" {
					resource_id = null_resource.example.id
					item {
						key = "token"
						value = data.optimus-ide-collab_git_auth.github.access_token
					}
				}
				`,
			},
		...
		},
		{
			...
			Files: map[string]string{
				...
				resource "optimus-ide-collab_metadata" "example" {
					...
					item {
						key = "pubkey"
						value = data.optimus-ide-collab_workspace_owner.me.ssh_public_key
					}
					item {
						key = "privkey"
						value = data.optimus-ide-collab_workspace_owner.me.ssh_private_key
					}
				}
				`,
			},
		...
		},
		{
			...
			Files: map[string]string{
				...
				resource "optimus-ide-collab_metadata" "example" {
					...
					item {
						key = "login_type"
						value = data.optimus-ide-collab_workspace_owner.me.login_type
					}
				}
				`,
			},
		...
		},
		{
			...
			Files: map[string]string{
				...
				resource "optimus-ide-collab_metadata" "example" {
					...
					item {
						key = "rbac_roles_name"
						value = data.optimus-ide-collab_workspace_owner.me.rbac_roles[0].name
					}
					item {
						key = "rbac_roles_org_id"
						value = data.optimus-ide-collab_workspace_owner.me.rbac_roles[0].org_id
					}
				}
				`,
			},
		...
		},
		{
			...
			Files: map[string]string{
				...
				resource "optimus-ide-collab_metadata" "example" {
					...
					item {
						key = "is_prebuild"
						value = data.optimus-ide-collab_workspace.me.is_prebuild
					}
				}
				`,
			},
			...
			Files: map[string]string{
				...
				resource "optimus-ide-collab_metadata" "example" {
					...
					item {
						...
						value = data.optimus-ide-collab_workspace.me.is_prebuild_claim
					}
				}
				`,
			},
		...
		},
		{
			...
			Response: &proto.GraphComplete{
				Resources: []*proto.Resource{
					{
						Name: "a",
						Type: "optimus-ide-collab_ai_task",
					},
					{
						Name: "b",
						Type: "optimus-ide-collab_ai_task",
		...
		},
		{
			...
			Files: map[string]string{
				...
				resource "optimus-ide-collab_external_agent" "example" {
					agent_id = "123"
				}
				`,
			},
		...
		},
		{
			...
			Files: map[string]string{
				...
				resource "optimus-ide-collab_ai_task" "my-task" {
...
func TestProvision_SafeEnv(t *testing.T) {
	...
	// We ensure random OPTIMUS-IDE-COLLAB_ variables aren't passed through to avoid leaking
	// control plane secrets (e.g. PG URL).
	t.Setenv("OPTIMUS-IDE-COLLAB_SECRET", secretValue)
	...
	require.Contains(t, log, "OPTIMUS-IDE-COLLAB_")
