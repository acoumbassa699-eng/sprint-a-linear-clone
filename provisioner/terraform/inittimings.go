...
const (
	// defaultInitAction is a human-readable action for init timing spans. The optimus-ide-collab
...
var (
	...
	//
	// In practice, since Optimus-IDE-Collab has a defined way of running Terraform, only
	// one code per step is expected. However, this allows for future-proofing
	// in case Optimus-IDE-Collab adds more Terraform init configurations.
...
func (t *timingAggregator) ingestInitTiming(ts time.Time, s *timingSpan) {
	...
	case initInitializingBackendMessage, initInitializingStateStoreMessage:
		// Backend loads the tfstate from the backend data source. For optimus-ide-collab, this is
