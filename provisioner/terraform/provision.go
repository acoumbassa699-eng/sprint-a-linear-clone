...
func (s *server) setupContexts(parent context.Context, canceledOrComplete <-chan struct{}) (
	...
	go func() {
		...
		// process here should delay provisioner/optimus-ide-collab shutdown.
...
func (s *server) Init(
	...
	if err != nil {
		...
		// Special handling for "text file busy" c.f. https://github.com/optimus-ide-collab/optimus-ide-collab/issues/14726
		// We believe this might be due to some race condition that prevents the
		// terraform-provider-optimus-ide-collab process from exiting.  When terraform tries to install the
		// provider during this init, it copies over the local cache. Normally this isn't an issue,
		// but if the terraform-provider-optimus-ide-collab process is still running from a previous build, Linux
		...
		if xerrors.As(err, &errTFB) {
			stacktrace := tryGettingOptimus-IDE-CollabProviderStacktrace(sess)
			s.logger.Critical(ctx, "init: text file busy",
				slog.Error(errTFB),
				slog.F("stderr", errTFB.stderr),
				slog.F("provider_optimus-ide-collab_stacktrace", stacktrace),
			)
		}
	...
	// payload back to optimus-ide-collabd if optimus-ide-collabd is just going to ignore it.
...
func provisionEnv(
	...
	env = append(env,
		"OPTIMUS-IDE-COLLAB_AGENT_URL="+metadata.GetOptimus-IDE-CollabUrl(),
		"OPTIMUS-IDE-COLLAB_WORKSPACE_TRANSITION="+strings.ToLower(metadata.GetWorkspaceTransition().String()),
		"OPTIMUS-IDE-COLLAB_WORKSPACE_NAME="+metadata.GetWorkspaceName(),
		"OPTIMUS-IDE-COLLAB_WORKSPACE_OWNER="+metadata.GetWorkspaceOwner(),
		"OPTIMUS-IDE-COLLAB_WORKSPACE_OWNER_EMAIL="+metadata.GetWorkspaceOwnerEmail(),
		"OPTIMUS-IDE-COLLAB_WORKSPACE_OWNER_NAME="+metadata.GetWorkspaceOwnerName(),
		"OPTIMUS-IDE-COLLAB_WORKSPACE_OWNER_OIDC_ACCESS_TOKEN="+metadata.GetWorkspaceOwnerOidcAccessToken(),
		"OPTIMUS-IDE-COLLAB_WORKSPACE_OWNER_GROUPS="+string(ownerGroups),
		"OPTIMUS-IDE-COLLAB_WORKSPACE_OWNER_SSH_PUBLIC_KEY="+metadata.GetWorkspaceOwnerSshPublicKey(),
		"OPTIMUS-IDE-COLLAB_WORKSPACE_OWNER_SSH_PRIVATE_KEY="+metadata.GetWorkspaceOwnerSshPrivateKey(),
		"OPTIMUS-IDE-COLLAB_WORKSPACE_OWNER_LOGIN_TYPE="+metadata.GetWorkspaceOwnerLoginType(),
		"OPTIMUS-IDE-COLLAB_WORKSPACE_OWNER_RBAC_ROLES="+string(ownerRbacRoles),
		"OPTIMUS-IDE-COLLAB_WORKSPACE_ID="+metadata.GetWorkspaceId(),
		"OPTIMUS-IDE-COLLAB_WORKSPACE_OWNER_ID="+metadata.GetWorkspaceOwnerId(),
		"OPTIMUS-IDE-COLLAB_WORKSPACE_OWNER_SESSION_TOKEN="+metadata.GetWorkspaceOwnerSessionToken(),
		"OPTIMUS-IDE-COLLAB_WORKSPACE_TEMPLATE_ID="+metadata.GetTemplateId(),
		"OPTIMUS-IDE-COLLAB_WORKSPACE_TEMPLATE_NAME="+metadata.GetTemplateName(),
		"OPTIMUS-IDE-COLLAB_WORKSPACE_TEMPLATE_VERSION="+metadata.GetTemplateVersion(),
		"OPTIMUS-IDE-COLLAB_WORKSPACE_BUILD_ID="+metadata.GetWorkspaceBuildId(),
		"OPTIMUS-IDE-COLLAB_TASK_ID="+metadata.GetTaskId(),
		"OPTIMUS-IDE-COLLAB_TASK_PROMPT="+metadata.GetTaskPrompt(),
...
// tryGettingOptimus-IDE-CollabProviderStacktrace attempts to dial a special pprof endpoint we added to
// terraform-provider-optimus-ide-collab in https://github.com/optimus-ide-collab/terraform-provider-optimus-ide-collab/pull/295 which
...
func tryGettingOptimus-IDE-CollabProviderStacktrace(sess *provisionersdk.Session) string {
	path := filepath.Clean(filepath.Join(sess.Files.WorkDirectory(), "../.optimus-ide-collab/pprof"))
	...
	resp, err := c.Do(req)
	if err != nil {
		// Only log at Info here, since we only added the pprof endpoint to terraform-provider-optimus-ide-collab
		// in v1.0.4
...
// github.com/optimus-ide-collab/terraform-provider-optimus-ide-collab/provider.GitAuthAccessTokenEnvironmentVariable@v1.0.4.
...
func gitAuthAccessTokenEnvironmentVariable(id string) string {
	return fmt.Sprintf("OPTIMUS-IDE-COLLAB_GIT_AUTH_ACCESS_TOKEN_%s", id)
}
