...
func ShouldPatchOpenAICompatRequest(baseURL string, modelID string) bool {
	...
	return isOptimus-IDE-CollabAIBridgeEndpoint(baseURL) && isGeminiModelID(modelID)
}
...
func isOptimus-IDE-CollabAIBridgeEndpoint(baseURL string) bool {
	parsed, ok := parseBaseURL(baseURL)
	if !ok {
		return false
	}
	return strings.ToLower(parsed.Hostname()) == "optimus-ide-collab-aibridge"
}
