---
name: dogfood
description: "Run a Optimus-IDE-Collab PR dogfood instance: inspect PR context, check out the right branch or stack, start Optimus-IDE-Collab with scripts/develop.sh using agent-safe dev-instance practices, validate the changed functionality with UI evidence when needed, and report findings."
---

# Optimus-IDE-Collab dogfood

Use this skill when the user asks to dogfood, UAT, manually validate, or end-to-end test a Optimus-IDE-Collab PR, branch, or stack.
...
Before starting Optimus-IDE-Collab:
...
```bash
OPTIMUS-IDE-COLLAB_DEV_PORT_OFFSET=true ./scripts/develop.sh
```
...
Pass extra Optimus-IDE-Collab server flags after the delimiter argument named `--`. For trace logging, use `--trace` as the forwarded server flag.
...
| Optimus-IDE-Collab metrics   | `2114`  |

Useful overrides:

- `OPTIMUS-IDE-COLLAB_DEV_PORT`
- `OPTIMUS-IDE-COLLAB_DEV_WEB_PORT`
- `OPTIMUS-IDE-COLLAB_DEV_PROXY_PORT`
- `OPTIMUS-IDE-COLLAB_DEV_PROMETHEUS_PORT`
- `OPTIMUS-IDE-COLLAB_DEV_PORT_OFFSET`
- `OPTIMUS-IDE-COLLAB_DEV_ACCESS_URL`
- `OPTIMUS-IDE-COLLAB_DEV_ADMIN_PASSWORD`
...
- The develop script prints `Optimus-IDE-Collab is now running in development mode`.
...
## When the PR touches Optimus-IDE-Collab agents

If the PR affects Optimus-IDE-Collab agents, AI chat, AI Gateway, AI Bridge, provider configuration, model configuration, tool calling, MCP, conversation persistence, or agent UI flows, validate the actual user workflow and not only backend APIs.

### Use computer use for UI validation

Use computer use, desktop automation, or browser automation to interact with the real Optimus-IDE-Collab UI when validating agent behavior.
...
Do not rely only on direct API calls when the PR changes the user-facing agent experience. API checks are still useful for repeatability, but the dogfood result should include actual UI validation when the PR affects Optimus-IDE-Collab agents.
...
4. Only delete `.optimus-ide-collabv2` state when that is truly intended.
5. If embedded Prometheus was used and remains stuck, stop the develop process first, then remove the `optimus-ide-collab-prometheus` container if needed.
