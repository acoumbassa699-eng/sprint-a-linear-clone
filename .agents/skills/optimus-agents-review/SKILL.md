---
name: optimus-ide-collab-agents-review
description: "Use this skill when a repository already has an open pull request and you need to run the Optimus-IDE-Collab Agents Review loop: request review with `/optimus-ide-collab-agents-review` when needed, wait for feedback from the `optimus-ide-collab-agents-review` GitHub app, fix issues, and repeat until the app comments `approved`."
---

# Optimus-IDE-Collab Agents Review Loop

## Goal

Drive an existing pull request until the GitHub app `optimus-ide-collab-agents-review`
has approved the current work.

The loop is:

1. if the PR has no existing `optimus-ide-collab-agents-review` review, comment, or
   pending trigger, post `/optimus-ide-collab-agents-review`
2. wait for `optimus-ide-collab-agents-review` to respond
3. fix actionable issues with the smallest safe diff
4. validate and push
5. request another review with `/optimus-ide-collab-agents-review`
...
- the latest `optimus-ide-collab-agents-review` response for the current work says
  `approved` (case-insensitive), or is a GitHub `APPROVED` review from
  that app
- there are no unresolved actionable `optimus-ide-collab-agents-review` review threads
...
- If the PR has no review or comment from `optimus-ide-collab-agents-review` and no
  pending trigger comment, post a top-level PR comment with the exact
  body `/optimus-ide-collab-agents-review`.
- If `optimus-ide-collab-agents-review` activity is already present, start from that
  feedback instead of posting a duplicate trigger immediately.
- After every fix push, post `/optimus-ide-collab-agents-review` again.
...
- `REVIEW_TRIGGER`: exact request comment.
  ...
  ```text
  /optimus-ide-collab-agents-review
  ```

- `REVIEW_APP_LOGIN_REGEX`: default match for the app author login.

  ```text
  ^optimus-ide-collab-agents-review(\[bot\])?$
  ```
...
- latest exact trigger comment with body `/optimus-ide-collab-agents-review`
...
If the PR has no review or comment from `optimus-ide-collab-agents-review`, and no
existing `/optimus-ide-collab-agents-review` trigger comment that the app has not yet
responded to, post the exact trigger comment:

```bash
gh pr comment "$PR_NUMBER" --body "/optimus-ide-collab-agents-review"
```
...
If the PR already has `optimus-ide-collab-agents-review` activity, do not post another
...
3. if the latest app response contains actionable feedback, fix that
   feedback first
4. after pushing fixes, post `/optimus-ide-collab-agents-review` again
...
A response counts when a new `optimus-ide-collab-agents-review` comment or review is
...
7. post `/optimus-ide-collab-agents-review` again
...
- PR number and URL
- current head SHA
- when `/optimus-ide-collab-agents-review` was last requested
- when `optimus-ide-collab-agents-review` last responded
