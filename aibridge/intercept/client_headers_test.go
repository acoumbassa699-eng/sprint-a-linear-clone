...
func TestPrepareClientHeaders(t *testing.T) {
	...
	t.Run("auth headers are removed", func(t *testing.T) {
		...
		input := http.Header{
			"Authorization": {"Bearer optimus-ide-collab-session-token"},
	...
	t.Run("agent firewall headers are removed", func(t *testing.T) {
		...
		input := http.Header{
			"X-Optimus-IDE-Collab-Agent-Firewall-Session-Id":      {"e5f6a7b8-1234-5678-9abc-def012345678"},
			"X-Optimus-IDE-Collab-Agent-Firewall-Sequence-Number": {"42"},
			"X-Custom":                               {"preserved"},
		}

		result := intercept.PrepareClientHeaders(input)

		assert.Empty(t, result.Get("X-Optimus-IDE-Collab-Agent-Firewall-Session-Id"))
		assert.Empty(t, result.Get("X-Optimus-IDE-Collab-Agent-Firewall-Sequence-Number"))
		assert.Equal(t, "preserved", result.Get("X-Custom"))
	})
}
