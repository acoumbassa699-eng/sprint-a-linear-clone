...
const mockToolName = "optimus-ide-collab_list_workspaces"
...
// setupMCPForTest creates a ready-to-use MCP server with proxy named "optimus-ide-collab".
func setupMCPForTest(t *testing.T, tracer trace.Tracer) *mockMCP {
	t.Helper()
	return setupMCPForTestWithName(t, "optimus-ide-collab", tracer)
}
...
func createMockMCPSrv(t *testing.T) (http.Handler, *callAccumulator) {
	...
	s := server.NewMCPServer(
		"Mock optimus-ide-collab MCP server",
		"1.0.0",
	...
	for _, name := range []string{mockToolName, "optimus-ide-collab_list_templates", "optimus-ide-collab_template_version_parameters", "optimus-ide-collab_get_authenticated_user", "optimus-ide-collab_create_workspace_build", "optimus-ide-collab_delete_template"} {
