...
func TestResponsesInjectedTool(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			mcpToolName: "optimus-ide-collab_template_version_parameters",
			expectToolArgs: map[string]any{
		...
		},
		{
			...
			mcpToolName: "optimus-ide-collab_delete_template",
			expectToolArgs: map[string]any{
		...
		},
		{
			...
			streaming:      true,
			mcpToolName:    "optimus-ide-collab_list_templates",
			expectToolArgs: map[string]any{},
			expectPrompt:   "List my optimus-ide-collab templates.",
			expectTokenUsages: []recorder.TokenUsageRecord{
				{
		...
		},
		{
			...
			streaming:   true,
			mcpToolName: "optimus-ide-collab_create_workspace_build",
			expectToolArgs: map[string]any{
				"transition":   "start",
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			// See https://github.com/optimus-ide-collab/internal/issues/1544.
