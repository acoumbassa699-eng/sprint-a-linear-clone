				...
				if pool != nil {
					...
					for _, reason := range []string{"rate_limited", "unauthorized"} {
						if want := tc.expectedTransitions[reason]; want > 0 {
							assert.True(t, optimus-ide-collabtestutil.PromCounterHasValue(t, gathered, float64(want), "key_pool_state_transitions_total", "test", reason))
						} else {
							assert.False(t, optimus-ide-collabtestutil.PromCounterGathered(t, gathered, "key_pool_state_transitions_total", "test", reason))
						}
					}
					...
					for _, outcome := range []string{"rate_limited", "auth_failed"} {
						if want := tc.expectedExhaustions[outcome]; want > 0 {
							assert.True(t, optimus-ide-collabtestutil.PromCounterHasValue(t, gathered, float64(want), "key_pool_exhaustions_total", outcome, "test"))
						} else {
							assert.False(t, optimus-ide-collabtestutil.PromCounterGathered(t, gathered, "key_pool_exhaustions_total", outcome, "test"))
						}
					}
