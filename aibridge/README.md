...
The handler is mounted by a host process. Today that host is `optimus-ide-collabd`, which [mounts the handler](../enterprise/optimus-ide-collabd/optimus-ide-collabd.go#L294) at `/api/v2/ai-gateway/<provider>/*`. Running aibridge as a separate process is planned for the future.
