		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, reason := range []string{"rate_limited", "unauthorized"} {
				if reason == tc.expectedReason {
					assert.True(t, optimus-ide-collabtestutil.PromCounterHasValue(t, gathered, 1, "key_pool_state_transitions_total", providerName, reason))
				} else {
					assert.False(t, optimus-ide-collabtestutil.PromCounterGathered(t, gathered, "key_pool_state_transitions_total", providerName, reason))
				}
			}
