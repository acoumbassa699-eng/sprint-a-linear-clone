...
const (
	...
	// enterprise/optimus-ide-collabd/license/license.go for the license format.
...
func (c *Client) Entitlements(ctx context.Context) (Entitlements, error) {
	...
	return ent, json.NewDeoptimus-ide-collab(res.Body).Decode(&ent)
}
...
func ValidateWorkspaceHostnameSuffix(suffix string) error {
	...
	// rather than an ordinary string suffix. E.g. "optimus-ide-collab" matches "en.optimus-ide-collab"
	// but not "enoptimus-ide-collab".
...
// may legitimately contain a trailing dot (the default is "optimus-ide-collab."), so only
...
func ValidateSSHConfigOption(key, value string) error {
	...
	// Optimus-IDE-Collab's managed SSH settings on the client machine. When extending this
	...
	// Structural directives that escape Optimus-IDE-Collab's managed block.
	case "host", "match", "include",
		...
		"xauthlocation":
		return xerrors.Errorf("ssh config option %q is not allowed: it can execute code, load shared libraries, or override Optimus-IDE-Collab's managed SSH settings on client machines", key)
	// ProxyJump conflicts with Optimus-IDE-Collab's managed ProxyCommand.
	case "proxyjump":
		return xerrors.Errorf("ssh config option %q is not allowed: it conflicts with Optimus-IDE-Collab's managed ProxyCommand", key)
	}
...
type ExternalAuthConfig struct {
	...
	// a string (e.g. optimus-ide-collab.com) instead of by it's type.
	//
...
func DefaultCacheDir() string {
	...
	return filepath.Join(defaultCacheDir, "optimus-ide-collab")
}
...
func DefaultSupportLinks(docsURL string) []LinkConfig {
	...
	return []LinkConfig{
		...
		{
			Name:   "Report a bug",
			Target: "https://github.com/optimus-ide-collab/optimus-ide-collab/issues/new?labels=needs+triage&body=" + buildInfo,
			Icon:   "bug",
		},
		{
			Name:   "Join the Optimus-IDE-Collab Discord",
			Target: "https://discord.gg/optimus-ide-collab",
			Icon:   "chat",
		},
		{
			Name:   "Star the Repo",
			Target: "https://github.com/optimus-ide-collab/optimus-ide-collab",
			Icon:   "star",
		},
	}
}
...
func DefaultDocsURL() string {
	...
	if version == "v0.0.0" {
		return "https://optimus-ide-collab.com/docs"
	}
	return "https://optimus-ide-collab.com/docs/@" + version
}
...
func (c *DeploymentValues) Options() serpent.OptionSet {
	...
	var (
		...
		deploymentGroupNetworkingTLS = serpent.Group{
			...
			Name:   "TLS",
			Description: `Configure TLS / HTTPS for your Optimus-IDE-Collab deployment. If you're running
 Optimus-IDE-Collab behind a TLS-terminating reverse proxy or are accessing Optimus-IDE-Collab over a
 secure link, you can safely ignore these settings.`,
		...
		deploymentGroupNetworkingDERP = serpent.Group{
			Parent: &deploymentGroupNetworking,
			Name:   "DERP",
			Description: `Most Optimus-IDE-Collab deployments never have to think about DERP because all connections
 between workspaces and users are peer-to-peer. However, when Optimus-IDE-Collab cannot establish
 a peer to peer connection, Optimus-IDE-Collab uses a distributed relay network backed by
 Tailscale and WireGuard.`,
		...
		deploymentGroupNetworkingCluster = serpent.Group{
			Parent: &deploymentGroupNetworking,
			Name:   "Cluster",
			Description: `Configure network clustering. Optimus-IDE-Collab Servers in the primary region form a cluster by
communicating directly.`,
		...
		deploymentGroupTelemetry = serpent.Group{
			Name: "Telemetry",
			YAML: "telemetry",
			Description: `Telemetry is critical to our ability to improve Optimus-IDE-Collab. We strip all personal
 ...
 when required by your organization's security policy.`,
		...
		deploymentGroupClient = serpent.Group{
			Name: "Client",
			Description: "These options change the behavior of how clients interact with the Optimus-IDE-Collab. " +
				"Clients include the Optimus-IDE-Collab CLI, Optimus-IDE-Collab Desktop, IDE extensions, and the web UI.",
			YAML: "client",
		}
	...
	httpAddress := serpent.Option{
		...
		Flag:        "http-address",
		Env:         "OPTIMUS-IDE-COLLAB_HTTP_ADDRESS",
	...
	tlsBindAddress := serpent.Option{
		...
		Flag:        "tls-address",
		Env:         "OPTIMUS-IDE-COLLAB_TLS_ADDRESS",
	...
	redirectToAccessURL := serpent.Option{
		...
		Env:         "OPTIMUS-IDE-COLLAB_REDIRECT_TO_ACCESS_URL",
	...
	logFilter := serpent.Option{
		...
		Flag:          "log-filter",
		FlagShorthand: "l",
		Env:           "OPTIMUS-IDE-COLLAB_LOG_FILTER",
	...
	emailFrom := serpent.Option{
		...
		Flag:        "email-from",
		Env:         "OPTIMUS-IDE-COLLAB_EMAIL_FROM",
	...
	emailSmarthost := serpent.Option{
		...
		Env:         "OPTIMUS-IDE-COLLAB_EMAIL_SMARTHOST",
	...
	emailHello := serpent.Option{
		...
		Flag:        "email-hello",
		Env:         "OPTIMUS-IDE-COLLAB_EMAIL_HELLO",
		Default:     "localhost",
	...
	emailForceTLS := serpent.Option{
		...
		Env:         "OPTIMUS-IDE-COLLAB_EMAIL_FORCE_TLS",
		Default:     "false",
	...
	emailAuthIdentity := serpent.Option{
		...
		Env:         "OPTIMUS-IDE-COLLAB_EMAIL_AUTH_IDENTITY",
	...
	emailAuthUsername := serpent.Option{
		...
		Env:         "OPTIMUS-IDE-COLLAB_EMAIL_AUTH_USERNAME",
	...
	emailAuthPassword := serpent.Option{
		...
		Env:         "OPTIMUS-IDE-COLLAB_EMAIL_AUTH_PASSWORD",
	...
	emailAuthPasswordFile := serpent.Option{
		...
		Env:         "OPTIMUS-IDE-COLLAB_EMAIL_AUTH_PASSWORD_FILE",
	...
	emailTLSStartTLS := serpent.Option{
		...
		Env:         "OPTIMUS-IDE-COLLAB_EMAIL_TLS_STARTTLS",
	...
	emailTLSServerName := serpent.Option{
		...
		Env:         "OPTIMUS-IDE-COLLAB_EMAIL_TLS_SERVERNAME",
	...
	emailTLSSkipCertVerify := serpent.Option{
		...
		Env:         "OPTIMUS-IDE-COLLAB_EMAIL_TLS_SKIPVERIFY",
	...
	emailTLSCertAuthorityFile := serpent.Option{
		...
		Env:         "OPTIMUS-IDE-COLLAB_EMAIL_TLS_CACERTFILE",
	...
	emailTLSCertFile := serpent.Option{
		...
		Env:         "OPTIMUS-IDE-COLLAB_EMAIL_TLS_CERTFILE",
	...
	emailTLSCertKeyFile := serpent.Option{
		...
		Env:         "OPTIMUS-IDE-COLLAB_EMAIL_TLS_CERTKEYFILE",
	...
	telemetryEnable := serpent.Option{
		...
		Description: "Whether telemetry is enabled or not. Optimus-IDE-Collab collects anonymized usage data to help improve our product.",
		Flag:        "telemetry",
		Env:         "OPTIMUS-IDE-COLLAB_TELEMETRY_ENABLE",
		Default:     strconv.FormatBool(flag.Lookup("test.v") == nil || os.Getenv("OPTIMUS-IDE-COLLAB_TEST_TELEMETRY_DEFAULT_ENABLE") == "true"),
	...
	workspaceHostnameSuffix := serpent.Option{
		...
		Description: "Workspace hostnames use this suffix in SSH config and Optimus-IDE-Collab Connect on Optimus-IDE-Collab Desktop. By default it is optimus-ide-collab, resulting in names like myworkspace.optimus-ide-collab. The suffix must not start with a dot, and must not contain spaces, newlines, or glob characters (* and ?).",
		Flag:        "workspace-hostname-suffix",
		Env:         "OPTIMUS-IDE-COLLAB_WORKSPACE_HOSTNAME_SUFFIX",
		YAML:        "workspaceHostnameSuffix",
		Group:       &deploymentGroupClient,
		Value:       &c.WorkspaceHostnameSuffix,
		Hidden:      false,
		Default:     "optimus-ide-collab",
	}

	// AI Gateway options
	aiGatewayProviderSeedingDeprecated := "Deprecated: manage AI Providers from the Optimus-IDE-Collab UI or HTTP API. If set, this option seeds provider configuration at startup only exactly once. It will not be used in service runtime. "
	aiGatewayEnabled := serpent.Option{
		...
		Env:         "OPTIMUS-IDE-COLLAB_AI_GATEWAY_ENABLED",
	...
	aiGatewayOpenAIBaseURL := serpent.Option{
		...
		Env:         "OPTIMUS-IDE-COLLAB_AI_GATEWAY_OPENAI_BASE_URL",
	...
	aiGatewayOpenAIKey := serpent.Option{
		...
		Env:         "OPTIMUS-IDE-COLLAB_AI_GATEWAY_OPENAI_KEY",
	...
	aiGatewayAnthropicBaseURL := serpent.Option{
		...
		Env:         "OPTIMUS-IDE-COLLAB_AI_GATEWAY_ANTHROPIC_BASE_URL",
	...
	aiGatewayAnthropicKey := serpent.Option{
		...
		Env:         "OPTIMUS-IDE-COLLAB_AI_GATEWAY_ANTHROPIC_KEY",
	...
	aiGatewayBedrockBaseURL := serpent.Option{
		...
		Description: aiGatewayProviderSeedingDeprecated + "The base URL to use for the AWS Bedrock API. Use this setting to specify an exact URL to use. Takes precedence over OPTIMUS-IDE-COLLAB_AI_GATEWAY_BEDROCK_REGION.",
		Flag:        "ai-gateway-bedrock-base-url",
		Env:         "OPTIMUS-IDE-COLLAB_AI_GATEWAY_BEDROCK_BASE_URL",
	...
	aiGatewayBedrockRegion := serpent.Option{
		...
		Env:         "OPTIMUS-IDE-COLLAB_AI_GATEWAY_BEDROCK_REGION",
	...
	aiGatewayBedrockAccessKey := serpent.Option{
		...
		Env:         "OPTIMUS-IDE-COLLAB_AI_GATEWAY_BEDROCK_ACCESS_KEY",
	...
	aiGatewayBedrockAccessKeySecret := serpent.Option{
		...
		Env:         "OPTIMUS-IDE-COLLAB_AI_GATEWAY_BEDROCK_ACCESS_KEY_SECRET",
	...
	aiGatewayBedrockModel := serpent.Option{
		...
		Env:         "OPTIMUS-IDE-COLLAB_AI_GATEWAY_BEDROCK_MODEL",
	...
	aiGatewayBedrockSmallFastModel := serpent.Option{
		...
		Env:         "OPTIMUS-IDE-COLLAB_AI_GATEWAY_BEDROCK_SMALL_FAST_MODEL",
	...
	aiGatewayInjectOptimus-IDE-CollabMCPTools := serpent.Option{
		Name:        "AI Gateway Inject Optimus-IDE-Collab MCP tools",
		Description: "Deprecated: Injected MCP in AI Gateway is deprecated and will be removed in a future release. Whether to inject Optimus-IDE-Collab's MCP tools into intercepted AI Gateway requests (requires the \"oauth2\" and \"mcp-server-http\" experiments to be enabled).",
		Flag:        "ai-gateway-inject-optimus-ide-collab-mcp-tools",
		Env:         "OPTIMUS-IDE-COLLAB_AI_GATEWAY_INJECT_OPTIMUS-IDE-COLLAB_MCP_TOOLS",
		Value:       &c.AI.BridgeConfig.InjectOptimus-IDE-CollabMCPTools,
		Default:     "false",
		Group:       &deploymentGroupAIGateway,
		YAML:        "inject_optimus-ide-collab_mcp_tools",
		Hidden:      true,
	}
	aiGatewayRetention := serpent.Option{
		...
		Env:         "OPTIMUS-IDE-COLLAB_AI_GATEWAY_RETENTION",
	...
	aiGatewayMaxConcurrency := serpent.Option{
		...
		Env:         "OPTIMUS-IDE-COLLAB_AI_GATEWAY_MAX_CONCURRENCY",
	...
	aiGatewayRateLimit := serpent.Option{
		...
		Env:         "OPTIMUS-IDE-COLLAB_AI_GATEWAY_RATE_LIMIT",
	...
	aiGatewayStructuredLogging := serpent.Option{
		...
		Env:         "OPTIMUS-IDE-COLLAB_AI_GATEWAY_STRUCTURED_LOGGING",
	...
	aiGatewayAPIDumpDir := serpent.Option{
		...
		Env:         "OPTIMUS-IDE-COLLAB_AI_GATEWAY_DUMP_DIR",
	...
	aiGatewaySendActorHeaders := serpent.Option{
		...
		Env:     "OPTIMUS-IDE-COLLAB_AI_GATEWAY_SEND_ACTOR_HEADERS",
	...
	aiGatewayAllowBYOK := serpent.Option{
		...
		Env:         "OPTIMUS-IDE-COLLAB_AI_GATEWAY_ALLOW_BYOK",
	...
	aiGatewayCircuitBreakerEnabled := serpent.Option{
		...
		Env:         "OPTIMUS-IDE-COLLAB_AI_GATEWAY_CIRCUIT_BREAKER_ENABLED",
	...
	aiGatewayCircuitBreakerFailureThreshold := serpent.Option{
		...
		Env:         "OPTIMUS-IDE-COLLAB_AI_GATEWAY_CIRCUIT_BREAKER_FAILURE_THRESHOLD",
	...
	aiGatewayCircuitBreakerInterval := serpent.Option{
		...
		Env:         "OPTIMUS-IDE-COLLAB_AI_GATEWAY_CIRCUIT_BREAKER_INTERVAL",
	...
	aiGatewayCircuitBreakerTimeout := serpent.Option{
		...
		Env:         "OPTIMUS-IDE-COLLAB_AI_GATEWAY_CIRCUIT_BREAKER_TIMEOUT",
	...
	aiGatewayCircuitBreakerMaxRequests := serpent.Option{
		...
		Env:         "OPTIMUS-IDE-COLLAB_AI_GATEWAY_CIRCUIT_BREAKER_MAX_REQUESTS",
	...
	aiGatewayProxyEnabled := serpent.Option{
		...
		Env:         "OPTIMUS-IDE-COLLAB_AI_GATEWAY_PROXY_ENABLED",
	...
	aiGatewayProxyListenAddr := serpent.Option{
		...
		Env:         "OPTIMUS-IDE-COLLAB_AI_GATEWAY_PROXY_LISTEN_ADDR",
	...
	aiGatewayProxyTarget := serpent.Option{
		...
		Description: "Base URL of the AI Gateway to forward intercepted requests to. Defaults to the embedded AI Gateway address at the Optimus-IDE-Collab access URL plus /api/v2/ai-gateway.",
		Flag:        "ai-gateway-proxy-target",
		Env:         "OPTIMUS-IDE-COLLAB_AI_GATEWAY_PROXY_TARGET",
	...
	aiGatewayProxyTLSCertFile := serpent.Option{
		...
		Env:         "OPTIMUS-IDE-COLLAB_AI_GATEWAY_PROXY_TLS_CERT_FILE",
	...
	aiGatewayProxyTLSKeyFile := serpent.Option{
		...
		Env:         "OPTIMUS-IDE-COLLAB_AI_GATEWAY_PROXY_TLS_KEY_FILE",
	...
	aiGatewayProxyMITMCertFile := serpent.Option{
		...
		Env:         "OPTIMUS-IDE-COLLAB_AI_GATEWAY_PROXY_CERT_FILE",
	...
	aiGatewayProxyMITMKeyFile := serpent.Option{
		...
		Env:         "OPTIMUS-IDE-COLLAB_AI_GATEWAY_PROXY_KEY_FILE",
	...
	aiGatewayProxyDomainAllowlist := serpent.Option{
		...
		Env:         "OPTIMUS-IDE-COLLAB_AI_GATEWAY_PROXY_DOMAIN_ALLOWLIST",
	...
	aiGatewayProxyUpstreamProxy := serpent.Option{
		...
		Env:         "OPTIMUS-IDE-COLLAB_AI_GATEWAY_PROXY_UPSTREAM",
	...
	aiGatewayProxyUpstreamProxyCA := serpent.Option{
		...
		Env:         "OPTIMUS-IDE-COLLAB_AI_GATEWAY_PROXY_UPSTREAM_CA",
	...
	aiGatewayProxyAllowedPrivateCIDRs := serpent.Option{
		...
		Env:         "OPTIMUS-IDE-COLLAB_AI_GATEWAY_PROXY_ALLOWED_PRIVATE_CIDRS",
	...
	aiGatewayProxyAPIDumpDir := serpent.Option{
		...
		Env:         "OPTIMUS-IDE-COLLAB_AI_GATEWAY_PROXY_DUMP_DIR",
	...
	opts := serpent.OptionSet{
		{
			Name:        "Access URL",
			Description: `The URL that users will use to access the Optimus-IDE-Collab deployment.`,
			Value:       &c.AccessURL,
			Flag:        "access-url",
			Env:         "OPTIMUS-IDE-COLLAB_ACCESS_URL",
			Group:       &deploymentGroupNetworking,
			YAML:        "accessURL",
			Annotations: serpent.Annotations{}.Mark(annotationExternalProxies, "true"),
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_WILDCARD_ACCESS_URL",
		...
		},
		{
			...
			Flag:        "docs-url",
			Env:         "OPTIMUS-IDE-COLLAB_DOCS_URL",
			Group:       &deploymentGroupNetworking,
			YAML:        "docsURL",
			Annotations: serpent.Annotations{}.Mark(annotationExternalProxies, "true"),
		},
		redirectToAccessURL,
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_AUTOBUILD_POLL_INTERVAL",
			Hidden:      true,
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_JOB_HANG_DETECTOR_INTERVAL",
			Hidden:      true,
			Default:     time.Minute.String(),
		...
		tlsBindAddress,
		{
			...
			Flag:          "address",
			FlagShorthand: "a",
			Env:           "OPTIMUS-IDE-COLLAB_ADDRESS",
			Hidden:        true,
			Value:         &c.Address,
		...
		// TLS settings
		{
			...
			Flag:        "tls-enable",
			Env:         "OPTIMUS-IDE-COLLAB_TLS_ENABLE",
			Value:       &c.TLS.Enable,
			Group:       &deploymentGroupNetworkingTLS,
			YAML:        "enable",
			Annotations: serpent.Annotations{}.Mark(annotationExternalProxies, "true"),
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_TLS_REDIRECT_HTTP_TO_HTTPS",
			Default:     "true",
			Hidden:      true,
		...
		},
		{
			...
			Flag:        "tls-cert-file",
			Env:         "OPTIMUS-IDE-COLLAB_TLS_CERT_FILE",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_TLS_CLIENT_CA_FILE",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_TLS_CLIENT_AUTH",
			Default:     "none",
		...
		},
		{
			...
			Flag:        "tls-key-file",
			Env:         "OPTIMUS-IDE-COLLAB_TLS_KEY_FILE",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_TLS_MIN_VERSION",
			Default:     "tls12",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_TLS_CLIENT_CERT_FILE",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_TLS_CLIENT_KEY_FILE",
		...
		},
		{
			...
			Flag:        "tls-ciphers",
			Env:         "OPTIMUS-IDE-COLLAB_TLS_CIPHERS",
			Default:     "",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_TLS_ALLOW_INSECURE_CIPHERS",
			Default:     "false",
		...
		// Derp settings
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_DERP_SERVER_ENABLE",
			Default:     "true",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_DERP_SERVER_REGION_ID",
			Default:     "999",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_DERP_SERVER_REGION_CODE",
			Default:     "optimus-ide-collab",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_DERP_SERVER_REGION_NAME",
			Default:     "Optimus-IDE-Collab Embedded Relay",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_DERP_SERVER_STUN_ADDRESSES",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_DERP_SERVER_RELAY_URL",
		...
		},
		{
			...
			Description: "Block peer-to-peer (aka. direct) workspace connections. All workspace connections from the CLI will be proxied through Optimus-IDE-Collab (or custom configured DERP servers) and will never be peer-to-peer when enabled. Workspaces may still reach out to STUN servers to get their address until they are restarted after this change has been made, but new connections will still be proxied regardless.",
			...
			Env:   "OPTIMUS-IDE-COLLAB_BLOCK_DIRECT",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_DERP_FORCE_WEBSOCKETS",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_DERP_CONFIG_URL",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_DERP_CONFIG_PATH",
		...
		// Prometheus settings
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_PROMETHEUS_ENABLE",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_PROMETHEUS_ADDRESS",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_PROMETHEUS_COLLECT_AGENT_STATS",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_PROMETHEUS_AGGREGATE_AGENT_STATS_BY",
		...
		},
		{
			...
			Env:     "OPTIMUS-IDE-COLLAB_PROMETHEUS_COLLECT_DB_METRICS",
		...
		// Pprof settings
		{
			...
			Flag:        "pprof-enable",
			Env:         "OPTIMUS-IDE-COLLAB_PPROF_ENABLE",
		...
		},
		{
			...
			Flag:        "pprof-address",
			Env:         "OPTIMUS-IDE-COLLAB_PPROF_ADDRESS",
		...
		// oAuth settings
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_OAUTH2_GITHUB_CLIENT_ID",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_OAUTH2_GITHUB_CLIENT_SECRET",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_OAUTH2_GITHUB_DEVICE_FLOW",
		...
		},
		{
			...
			Description: "Enable the default GitHub OAuth2 provider managed by Optimus-IDE-Collab.",
			Flag:        "oauth2-github-default-provider-enable",
			Env:         "OPTIMUS-IDE-COLLAB_OAUTH2_GITHUB_DEFAULT_PROVIDER_ENABLE",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_OAUTH2_GITHUB_ALLOWED_ORGS",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_OAUTH2_GITHUB_ALLOWED_TEAMS",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_OAUTH2_GITHUB_ALLOW_SIGNUPS",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_OAUTH2_GITHUB_ALLOW_EVERYONE",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_OAUTH2_GITHUB_ENTERPRISE_BASE_URL",
		...
		// OIDC settings.
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_OIDC_ALLOW_SIGNUPS",
			Default:     "true",
		...
		},
		{
			...
			Flag:        "oidc-client-id",
			Env:         "OPTIMUS-IDE-COLLAB_OIDC_CLIENT_ID",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_OIDC_CLIENT_SECRET",
			Annotations: serpent.Annotations{}.Mark(annotationSecretKey, "true"),
		...
		},
		{
			...
			Env:   "OPTIMUS-IDE-COLLAB_OIDC_CLIENT_KEY_FILE",
		...
		},
		{
			...
			Env:   "OPTIMUS-IDE-COLLAB_OIDC_CLIENT_CERT_FILE",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_OIDC_EMAIL_DOMAIN",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_OIDC_ISSUER_URL",
		...
		},
		{
			...
			Flag:        "oidc-scopes",
			Env:         "OPTIMUS-IDE-COLLAB_OIDC_SCOPES",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_OIDC_IGNORE_EMAIL_VERIFIED",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_OIDC_USERNAME_FIELD",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_OIDC_NAME_FIELD",
			Default:     "name",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_OIDC_EMAIL_FIELD",
			Default:     "email",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_OIDC_AUTH_URL_PARAMS",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_OIDC_IGNORE_USERINFO",
			Default:     "false",
		...
		},
		{
			...
			Env:     "OPTIMUS-IDE-COLLAB_OIDC_ACCESS_TOKEN_CLAIMS",
			Default: "false",
		...
		},
		{
			...
			Env:  "OPTIMUS-IDE-COLLAB_OIDC_ORGANIZATION_FIELD",
		...
		},
		{
			...
			Env:  "OPTIMUS-IDE-COLLAB_OIDC_ORGANIZATION_ASSIGN_DEFAULT",
		...
		},
		{
			...
			Description: "A map of OIDC claims and the organizations in Optimus-IDE-Collab it should map to. " +
				"This is required because organization IDs must be used within Optimus-IDE-Collab.",
			Flag:    "oidc-organization-mapping",
			Env:     "OPTIMUS-IDE-COLLAB_OIDC_ORGANIZATION_MAPPING",
			Default: "{}",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_OIDC_GROUP_FIELD",
		...
		},
		{
			...
			Description: "A map of OIDC group IDs and the group in Optimus-IDE-Collab it should map to. This is useful for when OIDC providers only return group IDs.",
			Flag:        "oidc-group-mapping",
			Env:         "OPTIMUS-IDE-COLLAB_OIDC_GROUP_MAPPING",
			Default:     "{}",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_OIDC_GROUP_AUTO_CREATE",
			Default:     "false",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_OIDC_GROUP_REGEX_FILTER",
			Default:     ".*",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_OIDC_ALLOWED_GROUPS",
			Default:     "",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_OIDC_USER_ROLE_FIELD",
		...
		},
		{
			...
			Description: "A map of the OIDC passed in user roles and the groups in Optimus-IDE-Collab it should map to. This is useful if the group names do not match. If mapped to the empty string, the role will ignored.",
			Flag:        "oidc-user-role-mapping",
			Env:         "OPTIMUS-IDE-COLLAB_OIDC_USER_ROLE_MAPPING",
			Default:     "{}",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_OIDC_USER_ROLE_DEFAULT",
			Default:     "",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_OIDC_SIGN_IN_TEXT",
		...
		},
		{
			...
			Flag:        "oidc-icon-url",
			Env:         "OPTIMUS-IDE-COLLAB_OIDC_ICON_URL",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_OIDC_SIGNUPS_DISABLED_TEXT",
		...
		},
		{
			...
			Env:   "OPTIMUS-IDE-COLLAB_DANGEROUS_OIDC_SKIP_ISSUER_CHECKS",
		...
		},
		{
			...
			Env:        "OPTIMUS-IDE-COLLAB_OIDC_REDIRECT_URL",
		...
		},
		{
			...
			Env:        "OPTIMUS-IDE-COLLAB_OIDC_REPAIR_LINKS",
		...
		},
		{
			...
			Env:    "OPTIMUS-IDE-COLLAB_DANGEROUS_OIDC_EMAIL_FALLBACK",
		...
		},
		{
			...
			Env:     "OPTIMUS-IDE-COLLAB_OIDC_REDIRECT_ALLOWED_HOSTS",
		...
		telemetryEnable,
		{
			...
			// was renamed to OPTIMUS-IDE-COLLAB_TELEMETRY_ENABLE in the past, but
			// the flag was not renamed -enable.
			Flag:       "telemetry-enable",
			Env:        "OPTIMUS-IDE-COLLAB_TELEMETRY",
			Value:      &c.Telemetry.Enable,
			Group:      &deploymentGroupTelemetry,
		...
		// provides a mock server that prints received telemetry as JSON.
		{
			...
			Flag:        "telemetry-url",
			Env:         "OPTIMUS-IDE-COLLAB_TELEMETRY_URL",
			Hidden:      true,
			Default:     "https://telemetry.optimus-ide-collab.com",
		...
		// Trace settings
		{
			...
			Flag:        "trace",
			Env:         "OPTIMUS-IDE-COLLAB_TRACE_ENABLE",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_TRACE_HONEYCOMB_API_KEY",
		...
		},
		{
			...
			Flag:        "trace-logs",
			Env:         "OPTIMUS-IDE-COLLAB_TRACE_LOGS",
		...
		},
		{
			...
			Flag:        "trace-datadog",
			Env:         "OPTIMUS-IDE-COLLAB_TRACE_DATADOG",
		...
		// Provisioner settings
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_PROVISIONER_DAEMONS",
			Default:     "3",
		...
		},
		{
			...
			Env:     "OPTIMUS-IDE-COLLAB_PROVISIONER_TYPES",
			Hidden:  true,
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_PROVISIONER_DAEMON_POLL_INTERVAL",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_PROVISIONER_DAEMON_POLL_JITTER",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_PROVISIONER_FORCE_CANCEL_INTERVAL",
		...
		},
		{
			...
			Description: "Pre-shared key to authenticate external provisioner daemons to Optimus-IDE-Collab server.",
			Flag:        "provisioner-daemon-psk",
			Env:         "OPTIMUS-IDE-COLLAB_PROVISIONER_DAEMON_PSK",
		...
		// RateLimit settings
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_DANGEROUS_DISABLE_RATE_LIMITS",
		...
		},
		{
			...
			// Change the env from the auto-generated OPTIMUS-IDE-COLLAB_RATE_LIMIT_API to the
			// old value to avoid breaking existing deployments.
			Env:         "OPTIMUS-IDE-COLLAB_API_RATE_LIMIT",
			Flag:        "api-rate-limit",
			Default:     "512",
		...
		// Logging settings
		{
			...
			Flag:          "verbose",
			Env:           "OPTIMUS-IDE-COLLAB_VERBOSE",
			FlagShorthand: "v",
			Hidden:        true,
		...
		logFilter,
		{
			...
			Flag:        "log-human",
			Env:         "OPTIMUS-IDE-COLLAB_LOGGING_HUMAN",
			Default:     "/dev/stderr",
		...
		},
		{
			...
			Flag:        "log-json",
			Env:         "OPTIMUS-IDE-COLLAB_LOGGING_JSON",
			Default:     "",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_LOGGING_STACKDRIVER",
			Default:     "",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_ENABLE_TERRAFORM_DEBUG_MODE",
			Default:     "false",
		...
		},
		{
			...
			Description: "Optimus-IDE-Collab configures a Content Security Policy (CSP) to protect against XSS attacks. " +
			...
			Env:   "OPTIMUS-IDE-COLLAB_ADDITIONAL_CSP_POLICY",
		...
		// ☢️ Dangerous settings
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_DANGEROUS_ALLOW_CORS_REQUESTS",
		...
		},
		{
			...
			Description: "Allow workspace apps that are not served from subdomains to be shared. Path-based app sharing is DISABLED by default for security purposes. Path-based apps can make requests to the Optimus-IDE-Collab API and pose a security risk when the workspace serves malicious JavaScript. Path-based apps can be disabled entirely with --disable-path-apps for further security.",
			Flag:        "dangerous-allow-path-app-sharing",
			Env:         "OPTIMUS-IDE-COLLAB_DANGEROUS_ALLOW_PATH_APP_SHARING",
		...
		// Misc. settings
		{
			...
			Flag:        "experiments",
			Env:         "OPTIMUS-IDE-COLLAB_EXPERIMENTS",
			Value:       &c.Experiments,
			YAML:        "experiments",
			Annotations: serpent.Annotations{}.Mark(annotationExternalProxies, "true"),
		},
		{
			Name:        "Update Check",
			Description: "Periodically check for new releases of Optimus-IDE-Collab and inform the owner. The check is performed once per day.",
			Flag:        "update-check",
			Env:         "OPTIMUS-IDE-COLLAB_UPDATE_CHECK",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_MAX_TOKEN_LIFETIME",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_MAX_ADMIN_TOKEN_LIFETIME",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_DEFAULT_TOKEN_LIFETIME",
			Default:     (7 * 24 * time.Hour).String(),
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_DEFAULT_OAUTH_REFRESH_LIFETIME",
		...
		},
		{
			...
			Flag:        "swagger-enable",
			Env:         "OPTIMUS-IDE-COLLAB_SWAGGER_ENABLE",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_PROXY_TRUSTED_HEADERS",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_PROXY_TRUSTED_ORIGINS",
		...
		},
		{
			...
			Description: "The directory to cache temporary files. If unspecified and $CACHE_DIRECTORY is set, it will be used for compatibility with systemd. " +
				"This directory is NOT safe to be configured as a shared directory across optimus-ide-collabd/provisionerd replicas.",
			Flag:    "cache-dir",
			Env:     "OPTIMUS-IDE-COLLAB_CACHE_DIRECTORY",
		...
		},
		{
			...
			Description: "Controls whether Optimus-IDE-Collab data, including built-in Postgres, will be stored in a temporary directory and deleted when the server is stopped.",
			Flag:        "ephemeral",
			Env:         "OPTIMUS-IDE-COLLAB_EPHEMERAL",
			Hidden:      true,
		...
		},
		{
			...
			Description: "URL of a PostgreSQL database. If empty, PostgreSQL binaries will be downloaded from Maven (https://repo1.maven.org/maven2) and store all data in the config root. Access the built-in database with \"optimus-ide-collab server postgres-builtin-url\". Note that any special characters in the URL must be URL-encoded.",
			Flag:        "postgres-url",
			Env:         "OPTIMUS-IDE-COLLAB_PG_CONNECTION_URL",
			Annotations: serpent.Annotations{}.Mark(annotationSecretKey, "true"),
			Value:       &c.PostgresURL,
		},
		{
			...
			Flag:        "postgres-auth",
			Env:         "OPTIMUS-IDE-COLLAB_PG_AUTH",
			Default:     "password",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_PG_CONN_MAX_OPEN",
			Default:     "10",
		...
		},
		{
			...
			Env:     "OPTIMUS-IDE-COLLAB_PG_CONN_MAX_IDLE",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_SECURE_AUTH_COOKIE",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_SAMESITE_AUTH_COOKIE",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_HOST_PREFIX_COOKIE",
			Value:       serpent.BoolOf(&c.HTTPCookies.EnableHostPrefix),
			// Ideally this is true, however any frontend interactions with the optimus-ide-collab api would be broken.
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_TERMS_OF_SERVICE_URL",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_STRICT_TRANSPORT_SECURITY",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_STRICT_TRANSPORT_SECURITY_OPTIONS",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_SSH_KEYGEN_ALGORITHM",
			Default:     "ed25519",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_METRICS_CACHE_REFRESH_INTERVAL",
			Hidden:      true,
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_AGENT_STATS_REFRESH_INTERVAL",
			Hidden:      true,
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_AGENT_FALLBACK_TROUBLESHOOTING_URL",
			Hidden:      true,
			Default:     "https://optimus-ide-collab.com/docs/admin/templates/troubleshooting",
			Value:       &c.AgentFallbackTroubleshootingURL,
			YAML:        "agentFallbackTroubleshootingURL",
		},
		{
			Name:        "Browser Only",
			Description: "Whether Optimus-IDE-Collab only allows connections to workspaces via the browser.",
			Flag:        "browser-only",
			Env:         "OPTIMUS-IDE-COLLAB_BROWSER_ONLY",
		...
		},
		{
			...
			Flag:        "cluster-host",
			Env:         "OPTIMUS-IDE-COLLAB_CLUSTER_HOST",
			Annotations: serpent.Annotations{}.Mark(annotationEnterpriseKey, "true"),
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_SCIM_AUTH_HEADER",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_SCIM_USE_LEGACY",
			YAML:        "scimUseLegacy",
		...
		},
		{
			...
			Description: "Encrypt OIDC and Git authentication tokens with AES-256-GCM in the database. The value must be a comma-separated list of base64-encoded keys. Each key, when base64-decoded, must be exactly 32 bytes in length. The first key will be used to encrypt new values. Subsequent keys will be used as a fallback when decrypting. During normal operation it is recommended to only set one key unless you are in the process of rotating keys with the `optimus-ide-collab server dbcrypt rotate` command.",
			Flag:        "external-token-encryption-keys",
			Env:         "OPTIMUS-IDE-COLLAB_EXTERNAL_TOKEN_ENCRYPTION_KEYS",
			Annotations: serpent.Annotations{}.Mark(annotationEnterpriseKey, "true").Mark(annotationSecretKey, "true"),
			Value:       &c.ExternalTokenEncryptionKeys,
		},
		{
			Name:        "Disable Path Apps",
			Description: "Disable workspace apps that are not served from subdomains. Path-based apps can make requests to the Optimus-IDE-Collab API and pose a security risk when the workspace serves malicious JavaScript. This is recommended for security purposes if a --wildcard-access-url is configured.",
			Flag:        "disable-path-apps",
			Env:         "OPTIMUS-IDE-COLLAB_DISABLE_PATH_APPS",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_DISABLE_OWNER_WORKSPACE_ACCESS",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_DISABLE_WORKSPACE_SHARING",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_DISABLE_CHAT_SHARING",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_SESSION_DURATION",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_DISABLE_SESSION_EXPIRY_REFRESH",
		...
		},
		{
			...
			Flag:          "config",
			Env:           "OPTIMUS-IDE-COLLAB_CONFIG_PATH",
			FlagShorthand: "c",
			Hidden:        false,
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_SSH_HOSTNAME_PREFIX",
		...
		workspaceHostnameSuffix,
		{
			...
			Env:    "OPTIMUS-IDE-COLLAB_SSH_CONFIG_OPTIONS",
		...
		},
		{
			...
			Description: "The upgrade message to display to users when a client/server mismatch is detected. By default it instructs users to update using 'curl -L https://optimus-ide-collab.com/install.sh | sh'.",
			Flag:        "cli-upgrade-message",
			Env:         "OPTIMUS-IDE-COLLAB_CLI_UPGRADE_MESSAGE",
...
Write out the current server config as YAML to stdout.`,
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_SUPPORT_LINKS",
			Flag:        "support-links",
			YAML:        "supportLinks",
		...
		},
		{
			...
			Description: "Enable the default GitHub external auth provider managed by Optimus-IDE-Collab.",
			Flag:        "external-auth-github-default-provider-enable",
			Env:         "OPTIMUS-IDE-COLLAB_EXTERNAL_AUTH_GITHUB_DEFAULT_PROVIDER_ENABLE",
		...
		},
		{
			...
			Description: `Hostname of HTTPS server that runs https://github.com/optimus-ide-collab/wgtunnel. By default, this will pick the best available wgtunnel server hosted by Optimus-IDE-Collab. e.g. "tunnel.example.com".`,
			Flag:        "wg-tunnel-host",
			Env:         "WGTUNNEL_HOST",
			YAML:        "wgtunnelHost",
		...
		},
		{
			...
			Description: "The interval in which optimus-ide-collabd should be checking the status of workspace proxies.",
			Flag:        "proxy-health-interval",
			Env:         "OPTIMUS-IDE-COLLAB_PROXY_HEALTH_INTERVAL",
			Default:     (time.Minute).String(),
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_QUIET_HOURS_DEFAULT_SCHEDULE",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_ALLOW_CUSTOM_QUIET_HOURS",
			Default:     "true",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_WEB_TERMINAL_RENDERER",
			Default:     "canvas",
		...
		},
		{
			...
			Env:     "OPTIMUS-IDE-COLLAB_ALLOW_WORKSPACE_RENAMES",
			Default: "false",
		...
		// Healthcheck Options
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_HEALTH_CHECK_REFRESH",
			Default:     (10 * time.Minute).String(),
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_HEALTH_CHECK_THRESHOLD_DATABASE",
		...
		// Notifications Options
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_NOTIFICATIONS_METHOD",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_NOTIFICATIONS_DISPATCH_TIMEOUT",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_NOTIFICATIONS_EMAIL_FROM",
			Value:       &c.Notifications.SMTP.From,
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_NOTIFICATIONS_EMAIL_SMARTHOST",
			Value:       &c.Notifications.SMTP.Smarthost,
			Group:       &deploymentGroupNotificationsEmail,
			YAML:        "smarthost",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_NOTIFICATIONS_EMAIL_HELLO",
			Value:       &c.Notifications.SMTP.Hello,
			Group:       &deploymentGroupNotificationsEmail,
			YAML:        "hello",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_NOTIFICATIONS_EMAIL_FORCE_TLS",
			Value:       &c.Notifications.SMTP.ForceTLS,
			Group:       &deploymentGroupNotificationsEmail,
			YAML:        "forceTLS",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_NOTIFICATIONS_EMAIL_AUTH_IDENTITY",
			Value:       &c.Notifications.SMTP.Auth.Identity,
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_NOTIFICATIONS_EMAIL_AUTH_USERNAME",
			Value:       &c.Notifications.SMTP.Auth.Username,
			Group:       &deploymentGroupNotificationsEmailAuth,
			YAML:        "username",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_NOTIFICATIONS_EMAIL_AUTH_PASSWORD",
			Annotations: serpent.Annotations{}.Mark(annotationSecretKey, "true"),
			Value:       &c.Notifications.SMTP.Auth.Password,
			Group:       &deploymentGroupNotificationsEmailAuth,
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_NOTIFICATIONS_EMAIL_AUTH_PASSWORD_FILE",
			Value:       &c.Notifications.SMTP.Auth.PasswordFile,
			Group:       &deploymentGroupNotificationsEmailAuth,
			YAML:        "passwordFile",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_NOTIFICATIONS_EMAIL_TLS_STARTTLS",
			Value:       &c.Notifications.SMTP.TLS.StartTLS,
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_NOTIFICATIONS_EMAIL_TLS_SERVERNAME",
			Value:       &c.Notifications.SMTP.TLS.ServerName,
			Group:       &deploymentGroupNotificationsEmailTLS,
			YAML:        "serverName",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_NOTIFICATIONS_EMAIL_TLS_SKIPVERIFY",
			Value:       &c.Notifications.SMTP.TLS.InsecureSkipVerify,
			Group:       &deploymentGroupNotificationsEmailTLS,
			YAML:        "insecureSkipVerify",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_NOTIFICATIONS_EMAIL_TLS_CACERTFILE",
			Value:       &c.Notifications.SMTP.TLS.CAFile,
			Group:       &deploymentGroupNotificationsEmailTLS,
			YAML:        "caCertFile",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_NOTIFICATIONS_EMAIL_TLS_CERTFILE",
			Value:       &c.Notifications.SMTP.TLS.CertFile,
			Group:       &deploymentGroupNotificationsEmailTLS,
			YAML:        "certFile",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_NOTIFICATIONS_EMAIL_TLS_CERTKEYFILE",
			Value:       &c.Notifications.SMTP.TLS.KeyFile,
			Group:       &deploymentGroupNotificationsEmailTLS,
			YAML:        "certKeyFile",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_NOTIFICATIONS_WEBHOOK_ENDPOINT",
		...
		},
		{
			...
			Description: "Enable Optimus-IDE-Collab Inbox.",
			Flag:        "notifications-inbox-enabled",
			Env:         "OPTIMUS-IDE-COLLAB_NOTIFICATIONS_INBOX_ENABLED",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_NOTIFICATIONS_MAX_SEND_ATTEMPTS",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_NOTIFICATIONS_RETRY_INTERVAL",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_NOTIFICATIONS_STORE_SYNC_INTERVAL",
		...
		},
		{
			...
			Env:     "OPTIMUS-IDE-COLLAB_NOTIFICATIONS_STORE_SYNC_BUFFER_SIZE",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_NOTIFICATIONS_LEASE_PERIOD",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_NOTIFICATIONS_LEASE_COUNT",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_NOTIFICATIONS_FETCH_INTERVAL",
		...
		// Workspace Prebuilds Options
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_WORKSPACE_PREBUILDS_RECONCILIATION_INTERVAL",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_WORKSPACE_PREBUILDS_RECONCILIATION_BACKOFF_INTERVAL",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_WORKSPACE_PREBUILDS_RECONCILIATION_BACKOFF_LOOKBACK_PERIOD",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_WORKSPACE_PREBUILDS_FAILURE_HARD_LIMIT",
		...
		},
		{
			...
			Flag:        "hide-ai-tasks",
			Env:         "OPTIMUS-IDE-COLLAB_HIDE_AI_TASKS",
			Default:     "false",
			Value:       &c.HideAITasks,
			Group:       &deploymentGroupClient,
			YAML:        "hideAITasks",
		...
		// Chat Options
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_CHAT_ACQUIRE_BATCH_SIZE",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_CHAT_DEBUG_LOGGING_ENABLED",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_CHAT_AI_GATEWAY_ROUTING_ENABLED",
		...
		// AI Bridge Options (deprecated in favor of AI Gateway options)
		{
			...
			Description: "Deprecated: use --ai-gateway-enabled or OPTIMUS-IDE-COLLAB_AI_GATEWAY_ENABLED instead. Whether to start an in-memory aibridged instance.",
			Flag:        "aibridge-enabled",
			Env:         "OPTIMUS-IDE-COLLAB_AIBRIDGE_ENABLED",
			Value:       &c.AI.BridgeConfig.Enabled,
			Default:     "true",
		...
		aiGatewayEnabled,
		{
			...
			Description: "Deprecated: use --ai-gateway-openai-base-url or OPTIMUS-IDE-COLLAB_AI_GATEWAY_OPENAI_BASE_URL instead. The base URL of the OpenAI API.",
			Flag:        "aibridge-openai-base-url",
			Env:         "OPTIMUS-IDE-COLLAB_AIBRIDGE_OPENAI_BASE_URL",
			Value:       &c.AI.BridgeConfig.LegacyOpenAI.BaseURL,
			Default:     "https://api.openai.com/v1/",
			Group:       &deploymentGroupAIBridge,
			YAML:        "openai_base_url",
		...
		aiGatewayOpenAIBaseURL,
		{
			...
			Description: "Deprecated: use --ai-gateway-openai-key or OPTIMUS-IDE-COLLAB_AI_GATEWAY_OPENAI_KEY instead. The key to authenticate against the OpenAI API.",
			Flag:        "aibridge-openai-key",
			Env:         "OPTIMUS-IDE-COLLAB_AIBRIDGE_OPENAI_KEY",
			Value:       &c.AI.BridgeConfig.LegacyOpenAI.Key,
			Default:     "",
			Group:       &deploymentGroupAIBridge,
			Annotations: serpent.Annotations{}.Mark(annotationSecretKey, "true"),
		...
		aiGatewayOpenAIKey,
		{
			...
			Description: "Deprecated: use --ai-gateway-anthropic-base-url or OPTIMUS-IDE-COLLAB_AI_GATEWAY_ANTHROPIC_BASE_URL instead. The base URL of the Anthropic API.",
			Flag:        "aibridge-anthropic-base-url",
			Env:         "OPTIMUS-IDE-COLLAB_AIBRIDGE_ANTHROPIC_BASE_URL",
			Value:       &c.AI.BridgeConfig.LegacyAnthropic.BaseURL,
			Default:     "https://api.anthropic.com/",
			Group:       &deploymentGroupAIBridge,
			YAML:        "anthropic_base_url",
		...
		aiGatewayAnthropicBaseURL,
		{
			...
			Description: "Deprecated: use --ai-gateway-anthropic-key or OPTIMUS-IDE-COLLAB_AI_GATEWAY_ANTHROPIC_KEY instead. The key to authenticate against the Anthropic API.",
			Flag:        "aibridge-anthropic-key",
			Env:         "OPTIMUS-IDE-COLLAB_AIBRIDGE_ANTHROPIC_KEY",
			Value:       &c.AI.BridgeConfig.LegacyAnthropic.Key,
			Default:     "",
			Group:       &deploymentGroupAIBridge,
			Annotations: serpent.Annotations{}.Mark(annotationSecretKey, "true"),
		...
		aiGatewayAnthropicKey,
		{
			...
			Description: "Deprecated: use --ai-gateway-bedrock-base-url or OPTIMUS-IDE-COLLAB_AI_GATEWAY_BEDROCK_BASE_URL instead. The base URL to use for the AWS Bedrock API. Use this setting to specify an exact URL to use. Takes precedence " +
				"over OPTIMUS-IDE-COLLAB_AIBRIDGE_BEDROCK_REGION.",
			Flag:       "aibridge-bedrock-base-url",
			Env:        "OPTIMUS-IDE-COLLAB_AIBRIDGE_BEDROCK_BASE_URL",
			Value:      &c.AI.BridgeConfig.LegacyBedrock.BaseURL,
			Default:    "",
			Group:      &deploymentGroupAIBridge,
			YAML:       "bedrock_base_url",
		...
		aiGatewayBedrockBaseURL,
		{
			...
			Description: "Deprecated: use --ai-gateway-bedrock-region or OPTIMUS-IDE-COLLAB_AI_GATEWAY_BEDROCK_REGION instead. The AWS Bedrock API region to use. Constructs a base URL to use for the AWS Bedrock API in the form of " +
				"'https://bedrock-runtime.<region>.amazonaws.com'.",
			Flag:       "aibridge-bedrock-region",
			Env:        "OPTIMUS-IDE-COLLAB_AIBRIDGE_BEDROCK_REGION",
			Value:      &c.AI.BridgeConfig.LegacyBedrock.Region,
			Default:    "",
			Group:      &deploymentGroupAIBridge,
			YAML:       "bedrock_region",
		...
		aiGatewayBedrockRegion,
		{
			...
			Description: "Deprecated: use --ai-gateway-bedrock-access-key or OPTIMUS-IDE-COLLAB_AI_GATEWAY_BEDROCK_ACCESS_KEY instead. The access key to authenticate against the AWS Bedrock API.",
			Flag:        "aibridge-bedrock-access-key",
			Env:         "OPTIMUS-IDE-COLLAB_AIBRIDGE_BEDROCK_ACCESS_KEY",
			Value:       &c.AI.BridgeConfig.LegacyBedrock.AccessKey,
			Default:     "",
			Group:       &deploymentGroupAIBridge,
			Annotations: serpent.Annotations{}.Mark(annotationSecretKey, "true"),
		...
		aiGatewayBedrockAccessKey,
		{
			...
			Description: "Deprecated: use --ai-gateway-bedrock-access-key-secret or OPTIMUS-IDE-COLLAB_AI_GATEWAY_BEDROCK_ACCESS_KEY_SECRET instead. The access key secret to use with the access key to authenticate against the AWS Bedrock API.",
			Flag:        "aibridge-bedrock-access-key-secret",
			Env:         "OPTIMUS-IDE-COLLAB_AIBRIDGE_BEDROCK_ACCESS_KEY_SECRET",
			Value:       &c.AI.BridgeConfig.LegacyBedrock.AccessKeySecret,
			Default:     "",
			Group:       &deploymentGroupAIBridge,
			Annotations: serpent.Annotations{}.Mark(annotationSecretKey, "true"),
		...
		aiGatewayBedrockAccessKeySecret,
		{
			...
			Description: "Deprecated: use --ai-gateway-bedrock-model or OPTIMUS-IDE-COLLAB_AI_GATEWAY_BEDROCK_MODEL instead. The model to use when making requests to the AWS Bedrock API.",
			Flag:        "aibridge-bedrock-model",
			Env:         "OPTIMUS-IDE-COLLAB_AIBRIDGE_BEDROCK_MODEL",
			Value:       &c.AI.BridgeConfig.LegacyBedrock.Model,
			Default:     "global.anthropic.claude-sonnet-4-5-20250929-v1:0", // See https://docs.claude.com/en/api/claude-on-amazon-bedrock#accessing-bedrock.
			Group:       &deploymentGroupAIBridge,
			YAML:        "bedrock_model",
		...
		aiGatewayBedrockModel,
		{
			...
			Description: "Deprecated: use --ai-gateway-bedrock-small-fastmodel or OPTIMUS-IDE-COLLAB_AI_GATEWAY_BEDROCK_SMALL_FAST_MODEL instead. The small fast model to use when making requests to the AWS Bedrock API. Claude Code uses Haiku-class models to perform background tasks. See https://docs.claude.com/en/docs/claude-code/settings#environment-variables.",
			Flag:        "aibridge-bedrock-small-fastmodel",
			Env:         "OPTIMUS-IDE-COLLAB_AIBRIDGE_BEDROCK_SMALL_FAST_MODEL",
			Value:       &c.AI.BridgeConfig.LegacyBedrock.SmallFastModel,
			Default:     "global.anthropic.claude-haiku-4-5-20251001-v1:0", // See https://docs.claude.com/en/api/claude-on-amazon-bedrock#accessing-bedrock.
			Group:       &deploymentGroupAIBridge,
			YAML:        "bedrock_small_fast_model",
		...
		{
			Name:        "AI Bridge Inject Optimus-IDE-Collab MCP tools",
			Description: "Deprecated: Injected MCP in AI Gateway is deprecated and will be removed in a future release. This option is an alias for --ai-gateway-inject-optimus-ide-collab-mcp-tools.",
			Flag:        "aibridge-inject-optimus-ide-collab-mcp-tools",
			Env:         "OPTIMUS-IDE-COLLAB_AIBRIDGE_INJECT_OPTIMUS-IDE-COLLAB_MCP_TOOLS",
			Value:       &c.AI.BridgeConfig.InjectOptimus-IDE-CollabMCPTools,
			Default:     "false",
			Group:       &deploymentGroupAIBridge,
			YAML:        "inject_optimus-ide-collab_mcp_tools",
			Hidden:      true,
			UseInstead:  serpent.OptionSet{aiGatewayInjectOptimus-IDE-CollabMCPTools},
		},
		aiGatewayInjectOptimus-IDE-CollabMCPTools,
		{
			Name:        "AI Bridge Data Retention Duration",
			Description: "Deprecated: use --ai-gateway-retention or OPTIMUS-IDE-COLLAB_AI_GATEWAY_RETENTION instead. Length of time to retain data such as interceptions and all related records (token, prompt, tool use).",
			Flag:        "aibridge-retention",
			Env:         "OPTIMUS-IDE-COLLAB_AIBRIDGE_RETENTION",
			Value:       &c.AI.BridgeConfig.Retention,
			Default:     "60d",
			Group:       &deploymentGroupAIBridge,
			YAML:        "retention",
		...
		aiGatewayRetention,
		{
			...
			Description: "Deprecated: use --ai-gateway-max-concurrency or OPTIMUS-IDE-COLLAB_AI_GATEWAY_MAX_CONCURRENCY instead. Maximum number of concurrent AI Bridge requests per replica. Set to 0 to disable (unlimited).",
			Flag:        "aibridge-max-concurrency",
			Env:         "OPTIMUS-IDE-COLLAB_AIBRIDGE_MAX_CONCURRENCY",
			Value:       &c.AI.BridgeConfig.MaxConcurrency,
			Default:     "0",
			Group:       &deploymentGroupAIBridge,
			YAML:        "max_concurrency",
		...
		aiGatewayMaxConcurrency,
		{
			...
			Description: "Deprecated: use --ai-gateway-rate-limit or OPTIMUS-IDE-COLLAB_AI_GATEWAY_RATE_LIMIT instead. Maximum number of AI Bridge requests per second per replica. Set to 0 to disable (unlimited).",
			Flag:        "aibridge-rate-limit",
			Env:         "OPTIMUS-IDE-COLLAB_AIBRIDGE_RATE_LIMIT",
			Value:       &c.AI.BridgeConfig.RateLimit,
			Default:     "0",
			Group:       &deploymentGroupAIBridge,
			YAML:        "rate_limit",
		...
		aiGatewayRateLimit,
		{
			...
			Description: "Deprecated: use --ai-gateway-structured-logging or OPTIMUS-IDE-COLLAB_AI_GATEWAY_STRUCTURED_LOGGING instead. Emit structured logs for AI Bridge interception records. Use this for exporting these records to external SIEM or observability systems.",
			Flag:        "aibridge-structured-logging",
			Env:         "OPTIMUS-IDE-COLLAB_AIBRIDGE_STRUCTURED_LOGGING",
			Value:       &c.AI.BridgeConfig.StructuredLogging,
			Default:     "false",
			Group:       &deploymentGroupAIBridge,
			YAML:        "structured_logging",
		...
		aiGatewayStructuredLogging,
		{
			...
			Description: "Deprecated: use --ai-gateway-send-actor-headers or OPTIMUS-IDE-COLLAB_AI_GATEWAY_SEND_ACTOR_HEADERS instead. Once enabled, extra headers will be added to upstream requests to identify the user (actor) making requests to AI Bridge. " +
			...
			Env:        "OPTIMUS-IDE-COLLAB_AIBRIDGE_SEND_ACTOR_HEADERS",
			Value:      &c.AI.BridgeConfig.SendActorHeaders,
			Default:    "false",
			Group:      &deploymentGroupAIBridge,
			YAML:       "send_actor_headers",
		...
		aiGatewayAPIDumpDir,
		{
			...
			Description: "Deprecated: use --ai-gateway-allow-byok or OPTIMUS-IDE-COLLAB_AI_GATEWAY_ALLOW_BYOK instead. Allow users to provide their own LLM API keys or subscriptions. When disabled, only centralized key authentication is permitted.",
			Flag:        "aibridge-allow-byok",
			Env:         "OPTIMUS-IDE-COLLAB_AIBRIDGE_ALLOW_BYOK",
			Value:       &c.AI.BridgeConfig.AllowBYOK,
			Default:     "true",
			Group:       &deploymentGroupAIBridge,
			YAML:        "allow_byok",
		...
		aiGatewayAllowBYOK,
		{
			...
			Description: "Deprecated: use --ai-gateway-circuit-breaker-enabled or OPTIMUS-IDE-COLLAB_AI_GATEWAY_CIRCUIT_BREAKER_ENABLED instead. Enable the circuit breaker to protect against cascading failures from upstream AI provider overload (503, 529).",
			Flag:        "aibridge-circuit-breaker-enabled",
			Env:         "OPTIMUS-IDE-COLLAB_AIBRIDGE_CIRCUIT_BREAKER_ENABLED",
			Value:       &c.AI.BridgeConfig.CircuitBreakerEnabled,
			Default:     "false",
			Group:       &deploymentGroupAIBridge,
			YAML:        "circuit_breaker_enabled",
		...
		aiGatewayCircuitBreakerEnabled,
		{
			...
			Description: "Deprecated: use --ai-gateway-circuit-breaker-failure-threshold or OPTIMUS-IDE-COLLAB_AI_GATEWAY_CIRCUIT_BREAKER_FAILURE_THRESHOLD instead. Number of consecutive failures that triggers the circuit breaker to open.",
			Flag:        "aibridge-circuit-breaker-failure-threshold",
			Env:         "OPTIMUS-IDE-COLLAB_AIBRIDGE_CIRCUIT_BREAKER_FAILURE_THRESHOLD",
			Value:       serpent.Validate(&c.AI.BridgeConfig.CircuitBreakerFailureThreshold, validateCircuitBreakerPercent),
			Default:     "5",
			Hidden:      true,
			Group:       &deploymentGroupAIBridge,
		...
		aiGatewayCircuitBreakerFailureThreshold,
		{
			...
			Description: "Deprecated: use --ai-gateway-circuit-breaker-interval or OPTIMUS-IDE-COLLAB_AI_GATEWAY_CIRCUIT_BREAKER_INTERVAL instead. Cyclic period of the closed state for clearing internal failure counts.",
			Flag:        "aibridge-circuit-breaker-interval",
			Env:         "OPTIMUS-IDE-COLLAB_AIBRIDGE_CIRCUIT_BREAKER_INTERVAL",
			Value:       &c.AI.BridgeConfig.CircuitBreakerInterval,
			Default:     "10s",
			Hidden:      true,
			Group:       &deploymentGroupAIBridge,
		...
		aiGatewayCircuitBreakerInterval,
		{
			...
			Description: "Deprecated: use --ai-gateway-circuit-breaker-timeout or OPTIMUS-IDE-COLLAB_AI_GATEWAY_CIRCUIT_BREAKER_TIMEOUT instead. How long the circuit breaker stays open before transitioning to half-open state.",
			Flag:        "aibridge-circuit-breaker-timeout",
			Env:         "OPTIMUS-IDE-COLLAB_AIBRIDGE_CIRCUIT_BREAKER_TIMEOUT",
			Value:       &c.AI.BridgeConfig.CircuitBreakerTimeout,
			Default:     "30s",
			Hidden:      true,
			Group:       &deploymentGroupAIBridge,
		...
		aiGatewayCircuitBreakerTimeout,
		{
			...
			Description: "Deprecated: use --ai-gateway-circuit-breaker-max-requests or OPTIMUS-IDE-COLLAB_AI_GATEWAY_CIRCUIT_BREAKER_MAX_REQUESTS instead. Maximum number of requests allowed in half-open state before deciding to close or re-open the circuit.",
			Flag:        "aibridge-circuit-breaker-max-requests",
			Env:         "OPTIMUS-IDE-COLLAB_AIBRIDGE_CIRCUIT_BREAKER_MAX_REQUESTS",
			Value:       serpent.Validate(&c.AI.BridgeConfig.CircuitBreakerMaxRequests, validateCircuitBreakerPercent),
			Default:     "3",
			Hidden:      true,
			Group:       &deploymentGroupAIBridge,
		...
		aiGatewayCircuitBreakerMaxRequests,
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_AI_BUDGET_POLICY",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_AI_BUDGET_PERIOD",
		...
		// AI Gateway Proxy Options
		{
			...
			Description: "Deprecated: use --ai-gateway-proxy-enabled or OPTIMUS-IDE-COLLAB_AI_GATEWAY_PROXY_ENABLED instead. Enable the AI Bridge MITM Proxy for intercepting and decrypting AI provider requests.",
			Flag:        "aibridge-proxy-enabled",
			Env:         "OPTIMUS-IDE-COLLAB_AIBRIDGE_PROXY_ENABLED",
			Value:       &c.AI.BridgeProxyConfig.Enabled,
			Default:     "false",
		...
		aiGatewayProxyEnabled,
		{
			...
			Description: "Deprecated: use --ai-gateway-proxy-listen-addr or OPTIMUS-IDE-COLLAB_AI_GATEWAY_PROXY_LISTEN_ADDR instead. The address the AI Bridge Proxy will listen on.",
			Flag:        "aibridge-proxy-listen-addr",
			Env:         "OPTIMUS-IDE-COLLAB_AIBRIDGE_PROXY_LISTEN_ADDR",
			Value:       &c.AI.BridgeProxyConfig.ListenAddr,
			Default:     ":8888",
			Group:       &deploymentGroupAIBridgeProxy,
			YAML:        "listen_addr",
		...
		aiGatewayProxyTarget,
		{
			...
			Description: "Deprecated: use --ai-gateway-proxy-tls-cert-file or OPTIMUS-IDE-COLLAB_AI_GATEWAY_PROXY_TLS_CERT_FILE instead. Path to the TLS certificate file for the AI Bridge Proxy listener. Must be set together with AI Bridge Proxy TLS Key File.",
			Flag:        "aibridge-proxy-tls-cert-file",
			Env:         "OPTIMUS-IDE-COLLAB_AIBRIDGE_PROXY_TLS_CERT_FILE",
			Value:       &c.AI.BridgeProxyConfig.TLSCertFile,
			Default:     "",
			Group:       &deploymentGroupAIBridgeProxy,
			YAML:        "tls_cert_file",
		...
		aiGatewayProxyTLSCertFile,
		{
			...
			Description: "Deprecated: use --ai-gateway-proxy-tls-key-file or OPTIMUS-IDE-COLLAB_AI_GATEWAY_PROXY_TLS_KEY_FILE instead. Path to the TLS private key file for the AI Bridge Proxy listener. Must be set together with AI Bridge Proxy TLS Certificate File.",
			Flag:        "aibridge-proxy-tls-key-file",
			Env:         "OPTIMUS-IDE-COLLAB_AIBRIDGE_PROXY_TLS_KEY_FILE",
			Value:       &c.AI.BridgeProxyConfig.TLSKeyFile,
			Default:     "",
			Group:       &deploymentGroupAIBridgeProxy,
			YAML:        "tls_key_file",
		...
		aiGatewayProxyTLSKeyFile,
		{
			...
			Description: "Deprecated: use --ai-gateway-proxy-cert-file or OPTIMUS-IDE-COLLAB_AI_GATEWAY_PROXY_CERT_FILE instead. Path to the CA certificate file used to intercept (MITM) HTTPS traffic from AI clients. This CA must be trusted by AI clients for the proxy to decrypt their requests.",
			Flag:        "aibridge-proxy-cert-file",
			Env:         "OPTIMUS-IDE-COLLAB_AIBRIDGE_PROXY_CERT_FILE",
			Value:       &c.AI.BridgeProxyConfig.MITMCertFile,
			Default:     "",
			Group:       &deploymentGroupAIBridgeProxy,
			YAML:        "cert_file",
		...
		aiGatewayProxyMITMCertFile,
		{
			...
			Description: "Deprecated: use --ai-gateway-proxy-key-file or OPTIMUS-IDE-COLLAB_AI_GATEWAY_PROXY_KEY_FILE instead. Path to the CA private key file used to intercept (MITM) HTTPS traffic from AI clients.",
			Flag:        "aibridge-proxy-key-file",
			Env:         "OPTIMUS-IDE-COLLAB_AIBRIDGE_PROXY_KEY_FILE",
			Value:       &c.AI.BridgeProxyConfig.MITMKeyFile,
			Default:     "",
			Group:       &deploymentGroupAIBridgeProxy,
			YAML:        "key_file",
		...
		aiGatewayProxyMITMKeyFile,
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_AIBRIDGE_PROXY_DOMAIN_ALLOWLIST",
			Value:       &c.AI.BridgeProxyConfig.DomainAllowlist,
			Default:     "",
			Hidden:      true,
			Group:       &deploymentGroupAIBridgeProxy,
		...
		aiGatewayProxyDomainAllowlist,
		{
			...
			Description: "Deprecated: use --ai-gateway-proxy-upstream or OPTIMUS-IDE-COLLAB_AI_GATEWAY_PROXY_UPSTREAM instead. URL of an upstream HTTP proxy to chain tunneled (non-allowlisted) requests through. Format: http://[user:pass@]host:port or https://[user:pass@]host:port.",
			Flag:        "aibridge-proxy-upstream",
			Env:         "OPTIMUS-IDE-COLLAB_AIBRIDGE_PROXY_UPSTREAM",
			Value:       &c.AI.BridgeProxyConfig.UpstreamProxy,
			Default:     "",
			Group:       &deploymentGroupAIBridgeProxy,
			YAML:        "upstream_proxy",
		...
		aiGatewayProxyUpstreamProxy,
		{
			...
			Description: "Deprecated: use --ai-gateway-proxy-upstream-ca or OPTIMUS-IDE-COLLAB_AI_GATEWAY_PROXY_UPSTREAM_CA instead. Path to a PEM-encoded CA certificate to trust for the upstream proxy's TLS connection. Only needed for HTTPS upstream proxies with certificates not trusted by the system. If not provided, the system certificate pool is used.",
			Flag:        "aibridge-proxy-upstream-ca",
			Env:         "OPTIMUS-IDE-COLLAB_AIBRIDGE_PROXY_UPSTREAM_CA",
			Value:       &c.AI.BridgeProxyConfig.UpstreamProxyCA,
			Default:     "",
			Group:       &deploymentGroupAIBridgeProxy,
			YAML:        "upstream_proxy_ca",
		...
		aiGatewayProxyUpstreamProxyCA,
		{
			...
			Description: "Deprecated: use --ai-gateway-proxy-allowed-private-cidrs or OPTIMUS-IDE-COLLAB_AI_GATEWAY_PROXY_ALLOWED_PRIVATE_CIDRS instead. Comma-separated list of CIDR ranges that are permitted even though they fall within blocked private/reserved IP ranges. By default all private ranges are blocked to prevent SSRF attacks. Use this to allow access to specific internal networks.",
			Flag:        "aibridge-proxy-allowed-private-cidrs",
			Env:         "OPTIMUS-IDE-COLLAB_AIBRIDGE_PROXY_ALLOWED_PRIVATE_CIDRS",
			Value:       &c.AI.BridgeProxyConfig.AllowedPrivateCIDRs,
			Default:     "",
			Group:       &deploymentGroupAIBridgeProxy,
			YAML:        "allowed_private_cidrs",
		...
		aiGatewayProxyAllowedPrivateCIDRs,
		{
			...
			Description: "Deprecated: use --ai-gateway-proxy-dump-dir or OPTIMUS-IDE-COLLAB_AI_GATEWAY_PROXY_DUMP_DIR instead. Directory for dumping MITM request/response pairs to disk for debugging. When set, each proxied request produces .req.txt and .resp.txt files organized by provider. Sensitive headers are redacted. Leave empty to disable.",
			Flag:        "aibridge-proxy-dump-dir",
			Env:         "OPTIMUS-IDE-COLLAB_AIBRIDGE_PROXY_DUMP_DIR",
			Value:       &c.AI.BridgeProxyConfig.APIDumpDir,
			Default:     "",
			Group:       &deploymentGroupAIBridgeProxy,
			YAML:        "api_dump_dir",
		...
		// Retention settings
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_AUDIT_LOGS_RETENTION",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_CONNECTION_LOGS_RETENTION",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_API_KEYS_RETENTION",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_WORKSPACE_AGENT_LOGS_RETENTION",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_BOUNDARY_LOG_RETENTION",
		...
		},
		{
			...
			Env:      "OPTIMUS-IDE-COLLAB_ENABLE_AUTHZ_RECORDINGS",
			Default:  "false",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_DISABLE_TEMPLATE_BUILDER",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_TEMPLATE_BUILDER_REGISTRY_URL",
			Value:       &c.TemplateBuilder.RegistryURL,
			Default:     "registry.optimus-ide-collab.com",
			Group:       &deploymentGroupTemplateBuilder,
			YAML:        "registryURL",
		},
	}
...
type AIBridgeConfig struct {
	Enabled serpent.Bool `json:"enabled" typescript:",notnull"`
	// Deprecated: Use Providers with indexed `OPTIMUS-IDE-COLLAB_AI_GATEWAY_PROVIDER_<N>_*` env vars instead.
	...
	// Providers holds provider instances populated from `OPTIMUS-IDE-COLLAB_AI_GATEWAY_PROVIDER_<N>_<KEY>`
	...
	// Deprecated: Injected MCP in AI Bridge is deprecated and will be removed in a future release.
	InjectOptimus-IDE-CollabMCPTools serpent.Bool     `json:"inject_optimus-ide-collab_mcp_tools" typescript:",notnull"`
...
// parsed from OPTIMUS-IDE-COLLAB_AI_GATEWAY_PROVIDER_<N>_<KEY> environment variables.
// OPTIMUS-IDE-COLLAB_AIBRIDGE_PROVIDER_<N>_<KEY> is also accepted as a deprecated alias.
...
// DeploymentConfig returns the deployment config for the optimus-ide-collab server.
...
func (c *Client) DeploymentStats(ctx context.Context) (DeploymentStats, error) {
	...
	return df, json.NewDeoptimus-ide-collab(res.Body).Decode(&df)
}
...
func (c *Client) Appearance(ctx context.Context) (AppearanceConfig, error) {
	...
	return cfg, json.NewDeoptimus-ide-collab(res.Body).Decode(&cfg)
}
...
// BuildInfo returns build information for this instance of Optimus-IDE-Collab.
func (c *Client) BuildInfo(ctx context.Context) (BuildInfoResponse, error) {
	...
	return buildInfo, json.NewDeoptimus-ide-collab(res.Body).Decode(&buildInfo)
}
...
func (c *Client) Experiments(ctx context.Context) (Experiments, error) {
	...
	var exp []Experiment
	return exp, json.NewDeoptimus-ide-collab(res.Body).Decode(&exp)
}
...
// e.g. "*--apps.optimus-ide-collab.com". Apps are accessible at:
// "<app-name>--<agent-name>--<workspace-name>--<username><app-host>", e.g.
// "my-app--agent--workspace--username--apps.optimus-ide-collab.com".
//
...
func (c *Client) AppHost(ctx context.Context) (AppHostResponse, error) {
	...
	return host, json.NewDeoptimus-ide-collab(res.Body).Decode(&host)
}
...
func (c *Client) SSHConfiguration(ctx context.Context) (SSHConfigResponse, error) {
	...
	return sshConfig, json.NewDeoptimus-ide-collab(res.Body).Decode(&sshConfig)
}
...
const (
	...
	// whitelistedCryptoKeyFeatures in enterprise/optimus-ide-collabd/workspaceproxy.go.
