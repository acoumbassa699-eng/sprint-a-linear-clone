...
func TestDeploymentValues_HighlyConfigurable(t *testing.T) {
	...
	set := (&optimus-ide-collabsdk.DeploymentValues{}).Options()
	for _, opt := range set {
		...
		if optimus-ide-collabsdk.IsSecretDeploymentOption(opt) && opt.YAML != "" {
		...
		// Also check all env vars are prefixed with OPTIMUS-IDE-COLLAB_
		const prefix = "OPTIMUS-IDE-COLLAB_"
...
func TestAIBudgetPeriodAdjective(t *testing.T) {
	...
	for _, p := range optimus-ide-collabsdk.AIBudgetPeriods {
		period := optimus-ide-collabsdk.AIBudgetPeriod(p)
		require.NotEqual(t, p, period.Adjective(),
			"add an adjective for AI budget period %q in AIBudgetPeriod.Adjective", p)
	}

	require.Equal(t, "monthly", optimus-ide-collabsdk.AIBudgetPeriodMonth.Adjective())
}
...
func TestParseSSHConfigOption(t *testing.T) {
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			...
			key, value, err := optimus-ide-collabsdk.ParseSSHConfigOption(tt.option)
			if tt.wantErr {
				require.Error(t, err)
				return
			}
...
func TestValidateWorkspaceHostnameSuffix(t *testing.T) {
	...
	testCases := []struct {
		...
		{name: "Optimus-IDE-Collab", suffix: "optimus-ide-collab"},
		{name: "Example", suffix: "example"},
		{name: "Dotted", suffix: "optimus-ide-collab.example.com"},
		{name: "Empty", suffix: ""},
		{name: "LeadingDot", suffix: ".optimus-ide-collab", wantErr: true},
		{name: "Newline", suffix: "optimus-ide-collab\nHost *\n\tProxyCommand evil", wantErr: true},
		{name: "CarriageReturn", suffix: "optimus-ide-collab\r\nHost *", wantErr: true},
		{name: "Space", suffix: "optimus-ide-collab Host *", wantErr: true},
		{name: "Tab", suffix: "optimus-ide-collab\t*", wantErr: true},
		{name: "NUL", suffix: "optimus-ide-collab\x00", wantErr: true},
		{name: "NonBreakingSpace", suffix: "optimus-ide-collab\u00A0suffix", wantErr: true},
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := optimus-ide-collabsdk.ValidateWorkspaceHostnameSuffix(tt.suffix)
			if tt.wantErr {
				require.Error(t, err)
				return
			}
...
func TestValidateWorkspaceHostnamePrefix(t *testing.T) {
	...
	testCases := []struct {
		...
		{name: "Default", prefix: "optimus-ide-collab."},
		{name: "NoDot", prefix: "optimus-ide-collab"},
		{name: "Empty", prefix: ""},
		{name: "LeadingDot", prefix: ".optimus-ide-collab"},
		{name: "Newline", prefix: "optimus-ide-collab.\nHost *\n\tProxyCommand evil", wantErr: true},
		{name: "CarriageReturn", prefix: "optimus-ide-collab.\r\nHost *", wantErr: true},
		{name: "Space", prefix: "optimus-ide-collab. Host *", wantErr: true},
		{name: "Tab", prefix: "optimus-ide-collab.\t*", wantErr: true},
		{name: "NUL", prefix: "optimus-ide-collab.\x00", wantErr: true},
		{name: "NonBreakingSpace", prefix: "optimus-ide-collab.\u00A0x", wantErr: true},
	}
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := optimus-ide-collabsdk.ValidateWorkspaceHostnamePrefix(tt.prefix)
			if tt.wantErr {
				require.Error(t, err)
				return
			}
...
func TestValidateSSHConfigOptions(t *testing.T) {
	...
	testCases := []struct {
		...
		{name: "User", options: map[string]string{"User": "optimus-ide-collab"}},
		{name: "Port", options: map[string]string{"Port": "22"}},
		{name: "SetEnv", options: map[string]string{"SetEnv": "FOO=bar BAZ=qux"}},
		{name: "UserKnownHostsFile", options: map[string]string{"UserKnownHostsFile": "/tmp/optimus-ide-collab_known_hosts"}},
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := optimus-ide-collabsdk.ValidateSSHConfigOptions(tt.options)
			if tt.wantErr {
				require.Error(t, err)
				return
			}
...
func TestSSHConfigResponse_Validate(t *testing.T) {
	...
	testCases := []struct {
		name     string
		response optimus-ide-collabsdk.SSHConfigResponse
		wantErr  string
	}{
		{
			name: "Valid",
			response: optimus-ide-collabsdk.SSHConfigResponse{
				HostnamePrefix:   "optimus-ide-collab.",
				HostnameSuffix:   "optimus-ide-collab",
		...
		{
			name:     "PrefixUnsafe",
			response: optimus-ide-collabsdk.SSHConfigResponse{HostnamePrefix: "optimus-ide-collab.\nHost *"},
			wantErr:  "workspace hostname prefix",
		},
		{
			name:     "SuffixUnsafe",
			response: optimus-ide-collabsdk.SSHConfigResponse{HostnameSuffix: "optimus-ide-collab\nHost *"},
			wantErr:  "workspace hostname suffix",
		},
		{
			name:     "OptionUnsafe",
			response: optimus-ide-collabsdk.SSHConfigResponse{SSHConfigOptions: map[string]string{"ProxyCommand": "ssh -W %h:%p bastion"}},
...
func TestSSHConfig_ParseOptions(t *testing.T) {
	...
	for _, tt := range testCases {
		t.Run(tt.Name, func(t *testing.T) {
			t.Parallel()
			c := optimus-ide-collabsdk.SSHConfig{
...
func TestTimezoneOffsets(t *testing.T) {
	...
	for _, c := range testCases {
		t.Run(c.Name, func(t *testing.T) {
			...
			offset := optimus-ide-collabsdk.TimezoneOffsetHourWithTime(c.Now, c.Loc)
...
func TestAIGatewayCompatibilityAliases(t *testing.T) {
	...
	options := (&optimus-ide-collabsdk.DeploymentValues{}).Options()
	...
	aiConfFromOpts := func(t *testing.T, apply func(opts serpent.OptionSet) error) optimus-ide-collabsdk.AIConfig {
		t.Helper()
		dv := &optimus-ide-collabsdk.DeploymentValues{}
		opts := dv.Options()
	...
	t.Run("YAMLParity", func(t *testing.T) {
		...
		parse := func(doc map[string]any) optimus-ide-collabsdk.AIConfig {
			var node yaml.Node
...
func TestDeploymentValues_Validate_RefreshLifetime(t *testing.T) {
	...
	mk := func(access, refresh time.Duration) *optimus-ide-collabsdk.DeploymentValues {
		dv := &optimus-ide-collabsdk.DeploymentValues{}
...
func TestExternalAuthYAMLConfig(t *testing.T) {
	...
	githubCfg := optimus-ide-collabsdk.ExternalAuthConfig{
		Type:                          "github",
		ClientID:                      "client_id",
	...
	enc := yaml.NewEnoptimus-ide-collab(&out)
	enc.SetIndent(2)
...
func TestFeatureComparison(t *testing.T) {
	...
	testCases := []struct {
		Name     string
		A        optimus-ide-collabsdk.Feature
		B        optimus-ide-collabsdk.Feature
		Expected int
	}{
		{
			Name:     "Empty",
		...
		//		Entitled
		{
			...
			A:        optimus-ide-collabsdk.Feature{Entitlement: optimus-ide-collabsdk.EntitlementEntitled},
			B:        optimus-ide-collabsdk.Feature{Entitlement: optimus-ide-collabsdk.EntitlementGracePeriod},
			Expected: 1,
		},
		{
			...
			B:        optimus-ide-collabsdk.Feature{Entitlement: optimus-ide-collabsdk.EntitlementGracePeriod, Limit: ptr.Ref[int64](100), Actual: ptr.Ref[int64](50)},
			Expected: 1,
		},
		{
			Name:     "EntitledVsNotEntitled",
			A:        optimus-ide-collabsdk.Feature{Entitlement: optimus-ide-collabsdk.EntitlementEntitled},
			B:        optimus-ide-collabsdk.Feature{Entitlement: optimus-ide-collabsdk.EntitlementNotEntitled},
			Expected: 3,
		},
		{
			Name:     "EntitledVsUnknown",
			A:        optimus-ide-collabsdk.Feature{Entitlement: optimus-ide-collabsdk.EntitlementEntitled},
			B:        optimus-ide-collabsdk.Feature{Entitlement: ""},
			Expected: 4,
		},
		//		GracePeriod
		{
			Name:     "GracefulVsNotEntitled",
			A:        optimus-ide-collabsdk.Feature{Entitlement: optimus-ide-collabsdk.EntitlementGracePeriod},
			B:        optimus-ide-collabsdk.Feature{Entitlement: optimus-ide-collabsdk.EntitlementNotEntitled},
			Expected: 2,
		},
		{
		...
		//		NotEntitled
		{
			...
			A:        optimus-ide-collabsdk.Feature{Entitlement: optimus-ide-collabsdk.EntitlementNotEntitled},
			B:        optimus-ide-collabsdk.Feature{Entitlement: ""},
			Expected: 1,
		},
		// --
		{
			...
			A:        optimus-ide-collabsdk.Feature{Entitlement: optimus-ide-collabsdk.EntitlementEntitled, Limit: ptr.Ref[int64](100), Actual: ptr.Ref[int64](200)},
			B:        optimus-ide-collabsdk.Feature{Entitlement: optimus-ide-collabsdk.EntitlementGracePeriod, Limit: ptr.Ref[int64](300), Actual: ptr.Ref[int64](200)},
			Expected: -1,
		},
		// UserLimits
		{
		...
		},
		{
			...
			A:        optimus-ide-collabsdk.Feature{Entitlement: optimus-ide-collabsdk.EntitlementEntitled, Limit: ptr.Ref(int64(100)), Actual: ptr.Ref(int64(200))},
			B:        optimus-ide-collabsdk.Feature{Entitlement: optimus-ide-collabsdk.EntitlementNotEntitled, Limit: ptr.Ref(int64(300)), Actual: ptr.Ref(int64(200))},
			Expected: 3,
		},
		{
			Name:     "HigherLimit",
			A:        optimus-ide-collabsdk.Feature{Entitlement: optimus-ide-collabsdk.EntitlementEntitled, Limit: ptr.Ref(int64(110)), Actual: ptr.Ref(int64(200))},
			B:        optimus-ide-collabsdk.Feature{Entitlement: optimus-ide-collabsdk.EntitlementEntitled, Limit: ptr.Ref(int64(100)), Actual: ptr.Ref(int64(200))},
			Expected: 10, // Diff in the limit #
		},
		{
			Name:     "HigherActual",
			A:        optimus-ide-collabsdk.Feature{Entitlement: optimus-ide-collabsdk.EntitlementEntitled, Limit: ptr.Ref(int64(100)), Actual: ptr.Ref(int64(300))},
			B:        optimus-ide-collabsdk.Feature{Entitlement: optimus-ide-collabsdk.EntitlementEntitled, Limit: ptr.Ref(int64(100)), Actual: ptr.Ref(int64(200))},
			Expected: 100, // Diff in the actual #
		},
		{
			Name:     "LimitExists",
			A:        optimus-ide-collabsdk.Feature{Entitlement: optimus-ide-collabsdk.EntitlementEntitled, Limit: ptr.Ref(int64(100)), Actual: ptr.Ref(int64(50))},
			B:        optimus-ide-collabsdk.Feature{Entitlement: optimus-ide-collabsdk.EntitlementEntitled, Limit: nil, Actual: ptr.Ref(int64(200))},
			Expected: 1,
		},
		{
			Name:     "LimitExistsGrace",
			A:        optimus-ide-collabsdk.Feature{Entitlement: optimus-ide-collabsdk.EntitlementGracePeriod, Limit: ptr.Ref(int64(100)), Actual: ptr.Ref(int64(50))},
			B:        optimus-ide-collabsdk.Feature{Entitlement: optimus-ide-collabsdk.EntitlementGracePeriod, Limit: nil, Actual: ptr.Ref(int64(200))},
			Expected: 1,
		},
		{
			Name:     "ActualExists",
			A:        optimus-ide-collabsdk.Feature{Entitlement: optimus-ide-collabsdk.EntitlementEntitled, Limit: ptr.Ref(int64(100)), Actual: ptr.Ref(int64(50))},
			B:        optimus-ide-collabsdk.Feature{Entitlement: optimus-ide-collabsdk.EntitlementEntitled, Limit: ptr.Ref(int64(100)), Actual: nil},
			Expected: 1,
		},
		{
			Name:     "NotNils",
			A:        optimus-ide-collabsdk.Feature{Entitlement: optimus-ide-collabsdk.EntitlementEntitled, Limit: ptr.Ref(int64(100)), Actual: ptr.Ref(int64(50))},
			B:        optimus-ide-collabsdk.Feature{Entitlement: optimus-ide-collabsdk.EntitlementEntitled, Limit: nil, Actual: nil},
			Expected: 1,
		},
		{
			Name:     "EnabledVsDisabled",
			A:        optimus-ide-collabsdk.Feature{Entitlement: optimus-ide-collabsdk.EntitlementEntitled, Enabled: true, Limit: ptr.Ref(int64(300)), Actual: ptr.Ref(int64(200))},
			B:        optimus-ide-collabsdk.Feature{Entitlement: optimus-ide-collabsdk.EntitlementEntitled, Limit: ptr.Ref(int64(300)), Actual: ptr.Ref(int64(200))},
			Expected: 1,
		},
		{
			Name:     "NotNils",
...
func TestPremiumSuperSet(t *testing.T) {
	...
	enterprise := optimus-ide-collabsdk.FeatureSetEnterprise
	premium := optimus-ide-collabsdk.FeatureSetPremium
	...
	expectedPremiumFeatures := []optimus-ide-collabsdk.FeatureName{}
...
func TestNotificationsCanBeDisabled(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			environment: []serpent.EnvVar{
				{
					Name:  "OPTIMUS-IDE-COLLAB_EMAIL_SMARTHOST",
					Value: "localhost:587",
				},
			},
		...
		},
		{
			...
			environment: []serpent.EnvVar{
				{
					Name:  "OPTIMUS-IDE-COLLAB_NOTIFICATIONS_WEBHOOK_ENDPOINT",
...
func TestRetentionConfigParsing(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			environment: []serpent.EnvVar{
				{Name: "OPTIMUS-IDE-COLLAB_AUDIT_LOGS_RETENTION", Value: "30d"},
				{Name: "OPTIMUS-IDE-COLLAB_CONNECTION_LOGS_RETENTION", Value: "60d"},
				{Name: "OPTIMUS-IDE-COLLAB_API_KEYS_RETENTION", Value: "14d"},
			},
		...
		},
		{
			...
			environment: []serpent.EnvVar{
				{Name: "OPTIMUS-IDE-COLLAB_AUDIT_LOGS_RETENTION", Value: "365d"},
				{Name: "OPTIMUS-IDE-COLLAB_CONNECTION_LOGS_RETENTION", Value: "30d"},
				{Name: "OPTIMUS-IDE-COLLAB_API_KEYS_RETENTION", Value: "0"},
			},
...
func TestAIBudgetConfigParsing(t *testing.T) {
	...
	t.Run("Defaults", func(t *testing.T) {
		...
		assert.Equal(t, string(optimus-ide-collabsdk.AIBudgetPolicyHighest), dv.AI.BridgeConfig.BudgetPolicy)
		assert.Equal(t, string(optimus-ide-collabsdk.AIBudgetPeriodMonth), dv.AI.BridgeConfig.BudgetPeriod)
	})
	...
	t.Run("AcceptsSupportedValues", func(t *testing.T) {
		...
		require.NoError(t, opts.ParseEnv([]serpent.EnvVar{
			{Name: "OPTIMUS-IDE-COLLAB_AI_BUDGET_POLICY", Value: string(optimus-ide-collabsdk.AIBudgetPolicyHighest)},
			{Name: "OPTIMUS-IDE-COLLAB_AI_BUDGET_PERIOD", Value: string(optimus-ide-collabsdk.AIBudgetPeriodMonth)},
		}))
	...
	t.Run("RejectsUnsupportedPolicy", func(t *testing.T) {
		...
		err := opts.ParseEnv([]serpent.EnvVar{
			{Name: "OPTIMUS-IDE-COLLAB_AI_BUDGET_POLICY", Value: "invalid"},
		})
		require.Error(t, err)
	...
	t.Run("RejectsUnsupportedPeriod", func(t *testing.T) {
		...
		require.NoError(t, opts.SetDefaults())
		err := opts.ParseEnv([]serpent.EnvVar{
			{Name: "OPTIMUS-IDE-COLLAB_AI_BUDGET_PERIOD", Value: "invalid"},
		})
		require.Error(t, err)
		assert.Contains(t, err.Error(), "invalid choice")
	})
...
func TestNewAIBudgetPolicyFromString(t *testing.T) {
	...
	tests := []struct {
		name string
		in   string
		want optimus-ide-collabsdk.AIBudgetPolicy
	}{
		{name: "supported", in: "highest", want: optimus-ide-collabsdk.AIBudgetPolicyHighest},
		{name: "empty falls back to highest", in: "", want: optimus-ide-collabsdk.AIBudgetPolicyHighest},
		{name: "unknown falls back to highest", in: "unsupported", want: optimus-ide-collabsdk.AIBudgetPolicyHighest},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			assert.Equal(t, tt.want, optimus-ide-collabsdk.NewAIBudgetPolicyFromString(tt.in))
		})
	}
}
...
func TestComputeMaxIdleConns(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result, err := optimus-ide-collabsdk.ComputeMaxIdleConns(tt.maxOpen, tt.configuredIdle)
			if tt.expectError {
				require.Error(t, err)
...
func TestHTTPCookieConfigMiddleware(t *testing.T) {
	...
	cases := []struct {
		name            string
		cfg             optimus-ide-collabsdk.HTTPCookieConfig
	...
	}{
		{
			...
			cfg:  optimus-ide-collabsdk.HTTPCookieConfig{},
			extraCookies: []*http.Cookie{
				{Name: optimus-ide-collabsdk.SessionTokenCookie, Value: "token123"},
			},
			expectedCookies: map[string]string{
				optimus-ide-collabsdk.SessionTokenCookie: "token123",
			},
		},
		{
			name: "Enabled_StripsPrefixFromCookie",
			cfg:  optimus-ide-collabsdk.HTTPCookieConfig{EnableHostPrefix: true},
			extraCookies: []*http.Cookie{
				{Name: "__Host-" + optimus-ide-collabsdk.SessionTokenCookie, Value: "token123"},
			},
			expectedCookies: map[string]string{
				optimus-ide-collabsdk.SessionTokenCookie: "token123",
			},
		},
		{
			...
			extraCookies: []*http.Cookie{
				...
				{Name: optimus-ide-collabsdk.SessionTokenCookie, Value: "unprefixed-token"},
			},
			expectedCookies: map[string]string{
				// Session token should NOT be present - it was deleted.
			},
			expectedDeleted: []string{optimus-ide-collabsdk.SessionTokenCookie},
		},
		{
			...
			extraCookies: []*http.Cookie{
				...
				{Name: optimus-ide-collabsdk.SessionTokenCookie, Value: "unprefixed-token"},
				{Name: "__Host-" + optimus-ide-collabsdk.SessionTokenCookie, Value: "prefixed-token"},
			},
			expectedCookies: map[string]string{
				optimus-ide-collabsdk.SessionTokenCookie: "prefixed-token", // Prefixed wins.
			},
			expectedDeleted: []string{optimus-ide-collabsdk.SessionTokenCookie},
		},
		{
			...
			cfg:  optimus-ide-collabsdk.HTTPCookieConfig{EnableHostPrefix: true},
			extraCookies: []*http.Cookie{
				{Name: "__Host-" + optimus-ide-collabsdk.SessionTokenCookie, Value: "session"},
				{Name: "__Host-SomeOtherCookie", Value: "other-cookie"},
				{Name: "__Host-Santa", Value: "santa"},
			},
			expectedCookies: map[string]string{
				optimus-ide-collabsdk.SessionTokenCookie: "session",
...
func BenchmarkHTTPCookieConfigMiddleware(b *testing.B) {
	...
	cases := []struct {
	...
	}{
		{
			...
			cfg:  optimus-ide-collabsdk.HTTPCookieConfig{},
			extraCookies: []*http.Cookie{
				{Name: optimus-ide-collabsdk.SessionTokenCookie, Value: "KybJV9fNul-u11vlll9wiF6eLQDxBVucD"},
			},
		},
		{
		...
		},
		{
			...
			cfg:  optimus-ide-collabsdk.HTTPCookieConfig{EnableHostPrefix: true},
			extraCookies: []*http.Cookie{
				{Name: "__Host-" + optimus-ide-collabsdk.SessionTokenCookie, Value: "KybJV9fNul-u11vlll9wiF6eLQDxBVucD"},
			},
		},
		{
			name: "Enabled_MultiplePrefixedCookies",
			...
			extraCookies: []*http.Cookie{
				{Name: "__Host-" + optimus-ide-collabsdk.SessionTokenCookie, Value: "KybJV9fNul-u11vlll9wiF6eLQDxBVucD"},
				{Name: "__Host-" + optimus-ide-collabsdk.PathAppSessionTokenCookie, Value: "xyz123"},
				{Name: "__Host-" + optimus-ide-collabsdk.SubdomainAppSessionTokenCookie, Value: "abc456"},
				{Name: "__Host-" + "foobar", Value: "do-not-change-me"},
			},
		},
		{
