...
// These cookies are Optimus-IDE-Collab-specific. If a new one is added or changed, the name
// shouldn't be likely to conflict with any user-application set cookies.
// Be sure to strip additional cookies in httpapi.StripOptimus-IDE-CollabCookies!
...
var SessionTokenCookie = "optimus-ide-collab_session_token"

const (
	// SessionTokenHeader is the custom header to use for authentication.
	SessionTokenHeader = "Optimus-IDE-Collab-Session-Token"
	...
	// when OPTIMUS-IDE-COLLAB_OIDC_REDIRECT_ALLOWED_HOSTS is enabled. The same value must be
	...
	//nolint:gosec
	PathAppSessionTokenCookie = "optimus-ide-collab_path_app_session_token"
	...
	//nolint:gosec
	SubdomainAppSessionTokenCookie = "optimus-ide-collab_subdomain_app_session_token"
	...
	//nolint:gosec
	SignedAppTokenCookie = "optimus-ide-collab_signed_app_token"
	...
	// apps.
	//nolint:gosec
	SignedAppTokenQueryParameter = "optimus-ide-collab_signed_app_token_23db1dde"
	...
	// nolint: gosec
	BypassRatelimitHeader = "X-Optimus-IDE-Collab-Bypass-Ratelimit"
	...
	// only.
	CLITelemetryHeader = "Optimus-IDE-Collab-CLI-Telemetry"
...
// New creates a Optimus-IDE-Collab client for the provided URL.
...
// Client is an HTTP caller for methods to the Optimus-IDE-Collab API.
...
type Client struct {
	...
	// Trace can be enabled to propagate tracing spans to the Optimus-IDE-Collab API.
...
// ReadBodyAsError reads the response as a optimus-ide-collabsdk.Response, and
// wraps it in a optimus-ide-collabsdk.Error type for easy marshaling.
//
...
func ReadBodyAsError(res *http.Response) error {
	...
	if res.StatusCode == http.StatusUnauthorized {
		...
		helpMessage = "Try logging in using 'optimus-ide-collab login'."
	}
	...
	var m Response
	err = json.NewDeoptimus-ide-collab(bytes.NewBuffer(resp)).Decode(&m)
	if err != nil {
...
// Optimus-IDE-CollabDesktopTelemetry represents the telemetry data sent from Optimus-IDE-Collab Desktop clients.
// @typescript-ignore Optimus-IDE-CollabDesktopTelemetry
type Optimus-IDE-CollabDesktopTelemetry struct {
...
func (t *Optimus-IDE-CollabDesktopTelemetry) FromHeader(headerValue string) error {
	if headerValue == "" {
		return nil
	}
...
func (t *Optimus-IDE-CollabDesktopTelemetry) IsEmpty() bool {
	return t.DeviceID == "" && t.DeviceOS == "" && t.Optimus-IDE-CollabDesktopVersion == ""
}
