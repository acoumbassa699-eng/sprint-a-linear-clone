...
func TestSignClaimsVerify(t *testing.T) {
	...
	claims := validClaims(t, "https://hooks.example.com/optimus-ide-collab", agenthooks.EventPreToolUse, nil)
...
func TestVerifyRejectsAlgorithmConfusion(t *testing.T) {
	...
	claims := validClaims(t, "https://hooks.example.com/optimus-ide-collab", agenthooks.EventStop, nil)
...
func TestHTTPHandlerRoutesEvents(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			require.NoError(t, json.NewDeoptimus-ide-collab(response.Body).Decode(&got))
...
func validClaims(t *testing.T, audience string, eventType agenthooks.EventType, request *agenthooks.Request) agenthooks.Claims {
	...
	claims := agenthooks.Claims{
		...
		Subject:    "optimus-ide-collab:chat:" + uuid.NewString(),
		Audience:   audience,
		IssuedAt:   now.Unix(),
	...
	if request != nil {
		claims.Subject = "optimus-ide-collab:chat:" + request.Meta.ChatID.String()
