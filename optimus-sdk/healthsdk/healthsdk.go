...
func New(c *optimus-ide-collabsdk.Client) *HealthClient {
...
func (c *HealthClient) DebugHealth(ctx context.Context) (HealthcheckReport, error) {
	...
	if res.StatusCode != http.StatusOK {
		return HealthcheckReport{}, optimus-ide-collabsdk.ReadBodyAsError(res)
	}
	var rpt HealthcheckReport
	return rpt, json.NewDeoptimus-ide-collab(res.Body).Decode(&rpt)
}
...
func (c *HealthClient) HealthSettings(ctx context.Context) (HealthSettings, error) {
	...
	defer res.Body.Close()
	if res.StatusCode != http.StatusOK {
		return HealthSettings{}, optimus-ide-collabsdk.ReadBodyAsError(res)
	}
	var settings HealthSettings
	return settings, json.NewDeoptimus-ide-collab(res.Body).Decode(&settings)
}
...
type HealthcheckReport struct {
	...
	Optimus-IDE-CollabVersion string `json:"optimus-ide-collab_version"`
}
...
type ProvisionerDaemonsReportItem struct {
	optimus-ide-collabsdk.ProvisionerDaemon `json:"provisioner_daemon"`
	Warnings                   []health.Message `json:"warnings"`
}
...
type WorkspaceProxyReport struct {
	...
	Healthy bool `json:"healthy"`
	BaseReport
	WorkspaceProxies optimus-ide-collabsdk.RegionsResponse[optimus-ide-collabsdk.WorkspaceProxy] `json:"workspace_proxies"`
}
