...
func (c *Client) UpdateTemplateMeta(ctx context.Context, templateID uuid.UUID, req UpdateTemplateMeta) (Template, error) {
	...
	return updated, json.NewDeoptimus-ide-collab(res.Body).Decode(&updated)
}
...
// parameters applied server-side; pass optimus-ide-collabsdk.UsersRequest{} when no
...
// AgentStatsReportRequest is a WebSocket request by optimus-ide-collabd
...
// TemplateExamples lists example templates available in Optimus-IDE-Collab.
//
...
// StarterTemplates lists example templates available in Optimus-IDE-Collab.
func (c *Client) StarterTemplates(ctx context.Context) ([]TemplateExample, error) {
	...
	return templateExamples, json.NewDeoptimus-ide-collab(res.Body).Decode(&templateExamples)
}
