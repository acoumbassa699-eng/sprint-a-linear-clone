...
type startupLogsWriter struct {
	...
	level  optimus-ide-collabsdk.LogLevel
	source uuid.UUID
}
...
func LogsWriter(ctx context.Context, sender func(ctx context.Context, log ...Log) error, source uuid.UUID, level optimus-ide-collabsdk.LogLevel) io.WriteCloser {
		...
		for {
			...
			if (done || flushed) && len(backlog) > 0 {
				...
				for r := retry.New(time.Second, 5*time.Second); r.Wait(ctx); {
					...
					// This error is expected to be optimus-ide-collabsdk.Error, but it has
...
func (l *LogSender) Enqueue(src uuid.UUID, logs ...Log) {
	...
	for k, log := range logs {
		...
		// examined the Optimus-IDE-Collab agent logs.
		...
		if err != nil {
			...
			pl = &proto.Log{
				...
				Output:    "**Optimus-IDE-Collab Internal Error**: Failed to convert log",
			}
		}
