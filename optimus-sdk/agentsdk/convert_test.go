...
func TestManifest(t *testing.T) {
	...
	manifest := agentsdk.Manifest{
		...
		Apps: []optimus-ide-collabsdk.WorkspaceApp{
			{
				...
				SharingLevel:  optimus-ide-collabsdk.WorkspaceAppSharingLevelAuthenticated,
				Healthcheck: optimus-ide-collabsdk.Healthcheck{
					URL:       "http://localhost:3030/healthz",
					Interval:  55555666,
					Threshold: 55555666,
				},
				Health: optimus-ide-collabsdk.WorkspaceAppHealthHealthy,
				Hidden: false,
			},
			{
				ID:            uuid.New(),
				...
				SharingLevel:  optimus-ide-collabsdk.WorkspaceAppSharingLevelPublic,
				Healthcheck: optimus-ide-collabsdk.Healthcheck{
		...
		Directory:                "/home/optimus-ide-collab",
		MOTDFile:                 "/etc/motd",
...
func TestSubsystems(t *testing.T) {
	t.Parallel()
	ss := []optimus-ide-collabsdk.AgentSubsystem{
		optimus-ide-collabsdk.AgentSubsystemEnvbox,
		optimus-ide-collabsdk.AgentSubsystemEnvbuilder,
...
func TestProtoFromLifecycle(t *testing.T) {
	t.Parallel()
	now := dbtime.Now()
	for _, s := range optimus-ide-collabsdk.WorkspaceAgentLifecycleOrder {
