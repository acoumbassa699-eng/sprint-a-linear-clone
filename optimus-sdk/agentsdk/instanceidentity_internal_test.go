...
func runAWSInstanceIdentityExchange(t *testing.T, opts ...InstanceIdentityOption) []byte {
	...
	client := newOptimus-IDE-CollabsdkClient(t, server, roundTripFunc(func(req *http.Request) (*http.Response, error) {
		switch {
...
func runGoogleInstanceIdentityExchange(t *testing.T, opts ...InstanceIdentityOption) []byte {
	...
	metadataClient := metadata.NewClient(&http.Client{Transport: roundTripFunc(func(req *http.Request) (*http.Response, error) {
		...
		require.Equal(t, "audience=optimus-ide-collab&format=full", req.URL.RawQuery)
		require.Equal(t, "Google", req.Header.Get("Metadata-Flavor"))
		return httpResponse(req, http.StatusOK, "fake-jwt", nil), nil
	})})
	client := newOptimus-IDE-CollabsdkClient(t, server, http.DefaultTransport)
...
func newInstanceIdentityServer(t *testing.T, path string, capturedBody *[]byte) *httptest.Server {
	...
	return httptest.NewServer(http.HandlerFunc(func(rw http.ResponseWriter, req *http.Request) {
		...
		*capturedBody = body
		...
		require.NoError(t, json.NewEnoptimus-ide-collab(rw).Encode(AuthenticateResponse{SessionToken: "test-session-token"}))
	}))
}

func newOptimus-IDE-CollabsdkClient(t *testing.T, server *httptest.Server, transport http.RoundTripper) *optimus-ide-collabsdk.Client {
	t.Helper()

	serverURL, err := url.Parse(server.URL)
	require.NoError(t, err)

	return &optimus-ide-collabsdk.Client{
		URL: serverURL,
