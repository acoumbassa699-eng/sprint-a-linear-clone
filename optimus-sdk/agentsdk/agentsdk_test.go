...
// optimus-ide-collab/internal#1451.
...
func TestExternalAuthRequestQuery(t *testing.T) {
	...
	t.Run("IncludesGitRefFieldsAndOmitsWorkdir", func(t *testing.T) {
		...
		srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			require.Equal(t, "https://github.com/optimus-ide-collab/optimus-ide-collab.git", r.URL.Query().Get("git_remote_origin"))
		...
		_, err = client.ExternalAuth(testutil.Context(t, testutil.WaitShort), agentsdk.ExternalAuthRequest{
			...
			Listen:          true,
			GitBranch:       "main",
			GitRemoteOrigin: "https://github.com/optimus-ide-collab/optimus-ide-collab.git",
			ChatID:          "test-chat-id",
		})
		require.NoError(t, err)
	})
