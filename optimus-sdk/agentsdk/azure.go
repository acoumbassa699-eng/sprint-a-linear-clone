...
// AzureSessionTokenExchanger exchanges Azure attested metadata for a Optimus-IDE-Collab session token.
...
func (a *AzureSessionTokenExchanger) exchange(ctx context.Context) (AuthenticateResponse, error) {
	...
	err = json.NewDeoptimus-ide-collab(res.Body).Decode(&token)
	if err != nil {
		return AuthenticateResponse{}, err
	}
