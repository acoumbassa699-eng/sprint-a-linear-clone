...
// way because cloud instance identity (AWS, Azure, Google, etc.) requires interacting with optimus-ide-collabd to exchange tokens.
// This means that the token providers need a optimus-ide-collabsdk.Client. However, the SessionTokenProvider is itself used by
// the client to authenticate requests. Thus, the dependency is bidirectional. Functions of this type are used in
// New() to ensure that things are set up correctly so there is only one instance of the optimus-ide-collabsdk.Client created.
// @typescript-ignore SessionTokenSetup
type SessionTokenSetup func(client *optimus-ide-collabsdk.Client) RefreshableSessionTokenProvider

// New creates a new *Client which can be used by an agent to connect to Optimus-IDE-Collabd. Use a SessionTokenSetup function
// to define the session token provider for the Client. This overrides the SessionTokenProvider on the underlying
// `*optimus-ide-collabsdk.Client`, so any `optimus-ide-collabsdk.ClientOptions` passed as `opts` should not set this property.
func New(serverURL *url.URL, setup SessionTokenSetup, opts ...optimus-ide-collabsdk.ClientOption) *Client {
	var provider RefreshableSessionTokenProvider
	opts = append(opts, func(c *optimus-ide-collabsdk.Client) {
		provider = setup(c)
		c.SessionTokenProvider = provider
	})
	c := optimus-ide-collabsdk.New(serverURL, opts...)
	return &Client{
		SDK:                             c,
...
// Client wraps `optimus-ide-collabsdk.Client` with specific functions
// scoped to a workspace agent.
type Client struct {
	RefreshableSessionTokenProvider
	SDK *optimus-ide-collabsdk.Client
}
...
func (c *Client) GitSSHKey(ctx context.Context) (GitSSHKey, error) {
	...
	if res.StatusCode != http.StatusOK {
		return GitSSHKey{}, optimus-ide-collabsdk.ReadBodyAsError(res)
	}

	var gitSSHKey GitSSHKey
	return gitSSHKey, json.NewDeoptimus-ide-collab(res.Body).Decode(&gitSSHKey)
}

type Metadata struct {
	Key string `json:"key"`
	optimus-ide-collabsdk.WorkspaceAgentMetadataResult
}
...
// performance.
type PostMetadataRequestDeprecated = optimus-ide-collabsdk.WorkspaceAgentMetadataResult
...
type Manifest struct {
	...
	// the Optimus-IDE-Collab deployment has. If this number is >0, we
	// set up special configuration in the workspace.
	GitAuthConfigs           int                                          `json:"git_auth_configs"`
	VSCodePortProxyURI       string                                       `json:"vscode_port_proxy_uri"`
	Apps                     []optimus-ide-collabsdk.WorkspaceApp                      `json:"apps"`
	...
	Metadata                 []optimus-ide-collabsdk.WorkspaceAgentMetadataDescription `json:"metadata"`
	Scripts                  []optimus-ide-collabsdk.WorkspaceAgentScript              `json:"scripts"`
	Devcontainers            []optimus-ide-collabsdk.WorkspaceAgentDevcontainer        `json:"devcontainers"`
}
...
// GetAnnouncementBanners, but is useful when you want to be maximally compatible with Optimus-IDE-Collabd
...
// maximally compatible with Optimus-IDE-Collabd Release Versions from 2.12+
...
// maximally compatible with Optimus-IDE-Collabd Release Versions from 2.13+
...
// maximally compatible with Optimus-IDE-Collabd Release Versions from 2.18+
...
// maximally compatible with Optimus-IDE-Collabd Release Versions from 2.20+
...
// maximally compatible with Optimus-IDE-Collabd Release Versions from 2.23+
...
// maximally compatible with Optimus-IDE-Collabd Release Versions from 2.24+
...
// maximally compatible with Optimus-IDE-Collabd Release Versions from 2.30+
...
// maximally compatible with Optimus-IDE-Collabd Release Versions from 2.31+
...
// maximally compatible with Optimus-IDE-Collabd Release Versions from 2.32+
...
// you want to be maximally compatible with newer Optimus-IDE-Collabd Release Versions that
...
func (c *Client) connectRPCVersion(ctx context.Context, version *apiversion.APIVersion, role string) (drpc.Conn, error) {
	...
	conn, res, err := websocket.Dial(ctx, rpcURL.String(), &websocket.DialOptions{
		...
		HTTPHeader: http.Header{
			optimus-ide-collabsdk.SessionTokenHeader: []string{c.SDK.SessionToken()},
		},
	})
	if err != nil {
		if res == nil {
		...
		return nil, optimus-ide-collabsdk.ReadBodyAsError(res)
	}
...
type PostAppHealthsRequest struct {
	...
	Healths map[uuid.UUID]optimus-ide-collabsdk.WorkspaceAppHealth
}
...
type RefreshableSessionTokenProvider interface {
	optimus-ide-collabsdk.SessionTokenProvider
...
// TokenExchanger obtains a session token by exchanging a cloud instance identity credential for a Optimus-IDE-Collab session token.
// @typescript-ignore TokenExchanger
type TokenExchanger interface {
	exchange(ctx context.Context) (AuthenticateResponse, error)
}

func (i *InstanceIdentitySessionTokenProvider) AsRequestOption() optimus-ide-collabsdk.RequestOption {
	t := i.GetSessionToken()
	return func(req *http.Request) {
		req.Header.Set(optimus-ide-collabsdk.SessionTokenHeader, t)
	}
}
...
func (i *InstanceIdentitySessionTokenProvider) SetDialOption(opts *websocket.DialOptions) {
	...
	if opts.HTTPHeader.Get(optimus-ide-collabsdk.SessionTokenHeader) == "" {
		opts.HTTPHeader.Set(optimus-ide-collabsdk.SessionTokenHeader, t)
	}
}
...
// FixedSessionTokenProvider wraps the optimus-ide-collabsdk variant to add a no-op RefreshToken method to satisfy the
...
type FixedSessionTokenProvider struct {
	optimus-ide-collabsdk.FixedSessionTokenProvider
}
...
func WithFixedToken(token string) SessionTokenSetup {
	return func(_ *optimus-ide-collabsdk.Client) RefreshableSessionTokenProvider {
		return FixedSessionTokenProvider{FixedSessionTokenProvider: optimus-ide-collabsdk.FixedSessionTokenProvider{SessionToken: token}}
	}
}
...
type PostLifecycleRequest struct {
	State     optimus-ide-collabsdk.WorkspaceAgentLifecycle `json:"state"`
...
type PostStartupRequest struct {
	...
	Subsystems        []optimus-ide-collabsdk.AgentSubsystem `json:"subsystems"`
}

type Log struct {
	CreatedAt time.Time         `json:"created_at"`
	Output    string            `json:"output"`
	Level     optimus-ide-collabsdk.LogLevel `json:"level"`
}
...
func (c *Client) PatchLogs(ctx context.Context, req PatchLogs) error {
	...
	defer res.Body.Close()
	if res.StatusCode != http.StatusOK {
		return optimus-ide-collabsdk.ReadBodyAsError(res)
	}
	return nil
}
...
type PatchAppStatus struct {
	...
	State   optimus-ide-collabsdk.WorkspaceAppStatusState `json:"state"`
...
func (c *Client) PostLogSource(ctx context.Context, req PostLogSourceRequest) (optimus-ide-collabsdk.WorkspaceAgentLogSource, error) {
	res, err := c.SDK.Request(ctx, http.MethodPost, "/api/v2/workspaceagents/me/log-source", req)
	if err != nil {
		return optimus-ide-collabsdk.WorkspaceAgentLogSource{}, err
	}
	defer res.Body.Close()
	if res.StatusCode != http.StatusCreated {
		return optimus-ide-collabsdk.WorkspaceAgentLogSource{}, optimus-ide-collabsdk.ReadBodyAsError(res)
	}
	var logSource optimus-ide-collabsdk.WorkspaceAgentLogSource
	return logSource, json.NewDeoptimus-ide-collab(res.Body).Decode(&logSource)
}
...
func (c *Client) ExternalAuth(ctx context.Context, req ExternalAuthRequest) (ExternalAuthResponse, error) {
	...
	if res.StatusCode != http.StatusOK {
		return ExternalAuthResponse{}, optimus-ide-collabsdk.ReadBodyAsError(res)
	}

	var authResp ExternalAuthResponse
	return authResp, json.NewDeoptimus-ide-collab(res.Body).Decode(&authResp)
}
...
func (c *Client) WaitForReinit(ctx context.Context) (*ReinitializationEvent, error) {
	...
	req.Header[optimus-ide-collabsdk.SessionTokenHeader] = []string{c.SDK.SessionToken()}
...
func (s *SSEAgentReinitTransmitter) Transmit(ctx context.Context, reinitEvents <-chan ReinitializationEvent) error {
	...
	for {
		...
		case reinitEvent, ok := <-reinitEvents:
			...
			err := sseSendEvent(optimus-ide-collabsdk.ServerSentEvent{
...
func (s *SSEAgentReinitReceiver) Receive(ctx context.Context) (*ReinitializationEvent, error) {
	nextEvent := optimus-ide-collabsdk.ServerSentEventReader(ctx, s.r)
	for {
		select {
		case <-ctx.Done():
			return nil, ctx.Err()
		...
		case sse.Type == optimus-ide-collabsdk.ServerSentEventTypeError:
			return nil, xerrors.Errorf("unexpected server sent event type error")
		case sse.Type == optimus-ide-collabsdk.ServerSentEventTypePing:
			continue
		case sse.Type != optimus-ide-collabsdk.ServerSentEventTypeData:
			return nil, xerrors.Errorf("unexpected server sent event type: %s", sse.Type)
		}

		// At this point we know that the sent event is of type optimus-ide-collabsdk.ServerSentEventTypeData
...
// the in-workspace `optimus-ide-collab exp chat context refresh` (no chat argument),
...
func (c *Client) RefreshChatContext(ctx context.Context) (RefreshChatContextResponse, error) {
	...
	if res.StatusCode != http.StatusOK {
		return RefreshChatContextResponse{}, optimus-ide-collabsdk.ReadBodyAsError(res)
	}
