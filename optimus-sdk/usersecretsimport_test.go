...
func TestParseSecretsFileEnv(t *testing.T) {
	...
	reqs, err := optimus-ide-collabsdk.ParseSecretsFile(optimus-ide-collabsdk.SecretsFileFormatEnv, content)
	require.NoError(t, err)

	want := []optimus-ide-collabsdk.CreateUserSecretRequest{
...
func TestParseSecretsFileEnvCRLFAndBOM(t *testing.T) {
	...
	reqs, err := optimus-ide-collabsdk.ParseSecretsFile(optimus-ide-collabsdk.SecretsFileFormatEnv, content)
	require.NoError(t, err)
	require.Equal(t, []optimus-ide-collabsdk.CreateUserSecretRequest{
...
func TestParseSecretsFileEnvErrors(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			_, err := optimus-ide-collabsdk.ParseSecretsFile(optimus-ide-collabsdk.SecretsFileFormatEnv, tt.content)
			require.Error(t, err)
...
func TestParseSecretsFileJSON(t *testing.T) {
	...
	reqs, err := optimus-ide-collabsdk.ParseSecretsFile(optimus-ide-collabsdk.SecretsFileFormatJSON, `{"A":"1","B":"two","C":"a=b#c"}`)
	require.NoError(t, err)
	require.Equal(t, []optimus-ide-collabsdk.CreateUserSecretRequest{
...
func TestParseSecretsFileJSONErrors(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			_, err := optimus-ide-collabsdk.ParseSecretsFile(optimus-ide-collabsdk.SecretsFileFormatJSON, tt.content)
			require.Error(t, err)
...
func TestParseSecretsFileYAML(t *testing.T) {
	...
	reqs, err := optimus-ide-collabsdk.ParseSecretsFile(optimus-ide-collabsdk.SecretsFileFormatYAML, content)
	require.NoError(t, err)
	require.Equal(t, []optimus-ide-collabsdk.CreateUserSecretRequest{
...
func TestParseSecretsFileYAMLErrors(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			_, err := optimus-ide-collabsdk.ParseSecretsFile(optimus-ide-collabsdk.SecretsFileFormatYAML, tt.content)
			require.Error(t, err)
			assert.Contains(t, err.Error(), tt.errMsg)
		})
	}
...
func TestParseSecretsFileYAMLAlias(t *testing.T) {
	...
	_, err := optimus-ide-collabsdk.ParseSecretsFile(optimus-ide-collabsdk.SecretsFileFormatYAML, "a: &a \"x\"\nb: *a\n")
	require.Error(t, err)
...
func TestParseSecretsFileYAMLMultiDocument(t *testing.T) {
	...
	t.Run("SecondMappingRejected", func(t *testing.T) {
		t.Parallel()
		_, err := optimus-ide-collabsdk.ParseSecretsFile(optimus-ide-collabsdk.SecretsFileFormatYAML, "A: \"1\"\n---\nB: \"2\"\n")
		require.Error(t, err)
...
func FuzzParseSecretsFile(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, format string, content string) {
		reqs, err := optimus-ide-collabsdk.ParseSecretsFile(optimus-ide-collabsdk.SecretsFileFormat(format), content)
		if err != nil {
		...
		require.LessOrEqual(t, len(reqs), optimus-ide-collabsdk.MaxUserSecretsPerUserCount)
...
func TestParseSecretsFileGeneralErrors(t *testing.T) {
	...
	t.Run("UnknownFormat", func(t *testing.T) {
		t.Parallel()
		_, err := optimus-ide-collabsdk.ParseSecretsFile("toml", "A=1")
		require.Error(t, err)
	...
	t.Run("EmptyFormat", func(t *testing.T) {
		t.Parallel()
		_, err := optimus-ide-collabsdk.ParseSecretsFile("", "A=1")
		require.Error(t, err)
	...
	t.Run("MaxBytesBoundary", func(t *testing.T) {
		t.Parallel()
		value := strings.Repeat("a", optimus-ide-collabsdk.MaxSecretsFileBytes-len("KEY="))
		reqs, err := optimus-ide-collabsdk.ParseSecretsFile(optimus-ide-collabsdk.SecretsFileFormatEnv, "KEY="+value)
		require.NoError(t, err)
		require.Equal(t, []optimus-ide-collabsdk.CreateUserSecretRequest{
	...
	t.Run("Oversized", func(t *testing.T) {
		t.Parallel()
		content := strings.Repeat("a", optimus-ide-collabsdk.MaxSecretsFileBytes+1)
		_, err := optimus-ide-collabsdk.ParseSecretsFile(optimus-ide-collabsdk.SecretsFileFormatEnv, content)
		require.Error(t, err)
	...
	t.Run("TooManySecrets", func(t *testing.T) {
		t.Parallel()
		lines := make([]string, 0, optimus-ide-collabsdk.MaxUserSecretsPerUserCount+1)
		for i := 0; i < optimus-ide-collabsdk.MaxUserSecretsPerUserCount+1; i++ {
			lines = append(lines, fmt.Sprintf("KEY_%d=value", i))
		}
		_, err := optimus-ide-collabsdk.ParseSecretsFile(optimus-ide-collabsdk.SecretsFileFormatEnv, strings.Join(lines, "\n"))
		require.Error(t, err)
	...
	emptyCases := []struct {
		name    string
		format  optimus-ide-collabsdk.SecretsFileFormat
		content string
	}{
		{name: "EnvEmpty", format: optimus-ide-collabsdk.SecretsFileFormatEnv, content: ""},
		{name: "EnvWhitespace", format: optimus-ide-collabsdk.SecretsFileFormatEnv, content: "   \n\t\n"},
		{name: "EnvAllComments", format: optimus-ide-collabsdk.SecretsFileFormatEnv, content: "# one\n# two\n"},
		{name: "JSONEmptyObject", format: optimus-ide-collabsdk.SecretsFileFormatJSON, content: "{}"},
		{name: "YAMLEmpty", format: optimus-ide-collabsdk.SecretsFileFormatYAML, content: ""},
		{name: "YAMLCommentsOnly", format: optimus-ide-collabsdk.SecretsFileFormatYAML, content: "# nothing here\n"},
	}
	for _, tt := range emptyCases {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			_, err := optimus-ide-collabsdk.ParseSecretsFile(tt.format, tt.content)
			require.Error(t, err)
...
func TestParseSecretsFileBestEffortEnvName(t *testing.T) {
	...
	cases := []struct {
		...
		{format: optimus-ide-collabsdk.SecretsFileFormatEnv, content: "PATH=value"},
		{format: optimus-ide-collabsdk.SecretsFileFormatJSON, content: `{"PATH":"value"}`},
		{format: optimus-ide-collabsdk.SecretsFileFormatYAML, content: "PATH: value"},
	}
	for _, tc := range cases {
		t.Run(string(tc.format), func(t *testing.T) {
			t.Parallel()
			reqs, err := optimus-ide-collabsdk.ParseSecretsFile(tc.format, tc.content)
			require.NoError(t, err)
