...
func TestNullTime_MarshalJSON(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			tr := optimus-ide-collabsdk.NewNullTime(tt.input.Time, tt.input.Valid)
...
func TestNullTime_UnmarshalJSON(t *testing.T) {
	...
	type request struct {
		Time optimus-ide-collabsdk.NullTime `json:"time"`
	}

	tests := []struct {
		name    string
		data    string
		want    optimus-ide-collabsdk.NullTime
		wantErr bool
	}{
		{
			name: "null",
		...
		},
		{
			...
			want: optimus-ide-collabsdk.NewNullTime(t1, true),
		},
		{
			name:    "invalid time",
...
func TestNullTime_IsZero(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			tr := optimus-ide-collabsdk.NullTime{NullTime: tt.input}
