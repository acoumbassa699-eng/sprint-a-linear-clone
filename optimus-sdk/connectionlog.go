...
const (
	...
	// ConnectionTypeTunnel is recorded by optimus-ide-collabd when a client
	...
	// tailnet API (e.g. Optimus-IDE-Collab Desktop) are not currently recorded.
...
func (c *Client) ConnectionLogs(ctx context.Context, req ConnectionLogsRequest) (ConnectionLogResponse, error) {
	...
	err = json.NewDeoptimus-ide-collab(res.Body).Decode(&logRes)
	if err != nil {
		return ConnectionLogResponse{}, err
	}
	return logRes, nil
