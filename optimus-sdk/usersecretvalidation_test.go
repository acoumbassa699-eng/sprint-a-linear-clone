...
func TestValidateCreateUserSecretRequest(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got := optimus-ide-collabsdk.ValidateCreateUserSecretRequest(tt.req)
...
func TestUserSecretNameValid(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			err := optimus-ide-collabsdk.UserSecretNameValid(tt.input)
			if tt.wantErr {
				assert.Error(t, err)
				if tt.errMsg != "" {
...
func TestUserSecretEnvNameValid(t *testing.T) {
	...
	tests := []struct {
		...
		// Length cap.
		{name: "ExactlyAtLengthLimit", input: strings.Repeat("A", optimus-ide-collabsdk.MaxUserSecretEnvNameLength)},
		{name: "OverLengthLimit", input: strings.Repeat("A", optimus-ide-collabsdk.MaxUserSecretEnvNameLength+1), wantErr: true, errMsg: "256 bytes"},
		...
		// OPTIMUS-IDE-COLLAB_ prefix.
		{name: "Optimus-IDE-CollabExact", input: "OPTIMUS-IDE-COLLAB", wantErr: true, errMsg: "OPTIMUS-IDE-COLLAB_"},
		{name: "Optimus-IDE-CollabPrefix", input: "OPTIMUS-IDE-COLLAB_WORKSPACE_NAME", wantErr: true, errMsg: "OPTIMUS-IDE-COLLAB_"},
		{name: "Optimus-IDE-CollabAgentToken", input: "OPTIMUS-IDE-COLLAB_AGENT_TOKEN", wantErr: true, errMsg: "OPTIMUS-IDE-COLLAB_"},
		{name: "Optimus-IDE-CollabLowerCase", input: "optimus-ide-collab_foo", wantErr: true, errMsg: "OPTIMUS-IDE-COLLAB_"},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			err := optimus-ide-collabsdk.UserSecretEnvNameValid(tt.input)
			if tt.wantErr {
				assert.Error(t, err)
				if tt.errMsg != "" {
					assert.Contains(t, err.Error(), tt.errMsg)
...
func TestUserSecretFilePathValid(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "AbsolutePath", input: "/home/optimus-ide-collab/.ssh/id_rsa"},
		{name: "RootPath", input: "/"},
		{name: "Empty", input: ""},
		...
		{name: "NullByte", input: "/home/\x00optimus-ide-collab", wantErr: true},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			err := optimus-ide-collabsdk.UserSecretFilePathValid(tt.input)
			if tt.wantErr {
				assert.Error(t, err)
			} else {
				assert.NoError(t, err)
...
func TestUserSecretValueValid(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "ExactlyAtLimit", input: strings.Repeat("a", optimus-ide-collabsdk.MaxUserSecretValueBytes)},
		{name: "OverLimit", input: strings.Repeat("a", optimus-ide-collabsdk.MaxUserSecretValueBytes+1), wantErr: true},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			err := optimus-ide-collabsdk.UserSecretValueValid(tt.input)
			if tt.wantErr {
				assert.Error(t, err)
			} else {
				assert.NoError(t, err)
