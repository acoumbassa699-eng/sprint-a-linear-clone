...
func TestResolveWorkspace(t *testing.T) {
	...
	writeJSON := func(w http.ResponseWriter, status int, v any) {
		...
		_ = json.NewEnoptimus-ide-collab(w).Encode(v)
	}

	// errResponse builds a optimus-ide-collabsdk.Response suitable for error
	// replies.
	errResponse := func(msg string) optimus-ide-collabsdk.Response {
		return optimus-ide-collabsdk.Response{Message: msg}
	}

	// newWorkspace returns a Workspace with the given ID and name.
	newWorkspace := func(id uuid.UUID, name string) optimus-ide-collabsdk.Workspace {
		return optimus-ide-collabsdk.Workspace{ID: id, Name: name}
	}
	...
	t.Run("TransportError", func(t *testing.T) {
		...
		require.NoError(t, err)
		client := optimus-ide-collabsdk.New(baseURL, optimus-ide-collabsdk.WithHTTPClient(&http.Client{
		...
		// fallback path. The error should NOT be a *optimus-ide-collabsdk.Error.
		var sdkErr *optimus-ide-collabsdk.Error
		require.False(t, errors.As(err, &sdkErr), "transport error should not be a optimus-ide-collabsdk.Error")
	})
