...
func (c *Client) CreateTask(ctx context.Context, user string, request CreateTaskRequest) (Task, error) {
	...
	var task Task
	if err := json.NewDeoptimus-ide-collab(res.Body).Decode(&task); err != nil {
		return Task{}, err
	}
...
func (c *Client) Tasks(ctx context.Context, filter *TasksFilter) ([]Task, error) {
	...
	if err := json.NewDeoptimus-ide-collab(res.Body).Decode(&tres); err != nil {
		return nil, err
	}
...
// Only tasks owned by optimus-ide-collabsdk.Me are supported.
...
func (c *Client) TaskLogs(ctx context.Context, user string, id uuid.UUID) (TaskLogsResponse, error) {
	...
	if err := json.NewDeoptimus-ide-collab(res.Body).Decode(&logs); err != nil {
