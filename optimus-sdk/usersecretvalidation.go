...
//     (OPTIMUS-IDE-COLLAB_*, PATH, HOME, ...), and template-defined env. Not
...
var (
	...
	reservedEnvNames = map[string]struct{}{
		...
		// SSH session variables. The Optimus-IDE-Collab agent sets
		...
		// Editor/pager. The Optimus-IDE-Collab agent sets these so that git
	...
	// exact-name map. The OPTIMUS-IDE-COLLAB / OPTIMUS-IDE-COLLAB_* namespace is handled
	// separately with its own error message (see below).
	reservedEnvPrefixes = []string{
		// The Optimus-IDE-Collab agent sets GIT_SSH_COMMAND, GIT_ASKPASS,
...
func UserSecretEnvNameValid(s string) error {
	...
	if upper == "OPTIMUS-IDE-COLLAB" || strings.HasPrefix(upper, "OPTIMUS-IDE-COLLAB_") {
		return xerrors.New("environment variable names starting with OPTIMUS-IDE-COLLAB_ are reserved for internal use")
	}
