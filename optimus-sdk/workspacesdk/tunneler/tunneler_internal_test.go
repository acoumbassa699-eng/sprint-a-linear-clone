...
func TestHandleBuildUpdate_Coverage(t *testing.T) {
	...
	for s := range maxState {
		for _, trans := range optimus-ide-collabsdk.WorkspaceTransitionEnums() {
			for _, jobStatus := range optimus-ide-collabsdk.ProvisionerJobStatusEnums() {
...
func TestBuildUpdatesStoppedWorkspace(t *testing.T) {
	...
	uut.handleBuildUpdate(&workspacesdk.BuildUpdate{Transition: optimus-ide-collabsdk.WorkspaceTransitionStop, JobStatus: optimus-ide-collabsdk.ProvisionerJobPending})
	require.Equal(t, waitToStart, uut.state)
	waitForGoroutines(testCtx, t, uut)
	require.False(t, fWorkspaceStarter.started)

	uut.handleBuildUpdate(&workspacesdk.BuildUpdate{Transition: optimus-ide-collabsdk.WorkspaceTransitionStop, JobStatus: optimus-ide-collabsdk.ProvisionerJobRunning})
	require.Equal(t, waitToStart, uut.state)
	waitForGoroutines(testCtx, t, uut)
	require.False(t, fWorkspaceStarter.started)

	// when stop job succeeds, we start the workspace
	uut.handleBuildUpdate(&workspacesdk.BuildUpdate{Transition: optimus-ide-collabsdk.WorkspaceTransitionStop, JobStatus: optimus-ide-collabsdk.ProvisionerJobSucceeded})
	require.Equal(t, waitForWorkspaceStarted, uut.state)
	waitForGoroutines(testCtx, t, uut)
	require.True(t, fWorkspaceStarter.started)

	uut.handleBuildUpdate(&workspacesdk.BuildUpdate{Transition: optimus-ide-collabsdk.WorkspaceTransitionStart, JobStatus: optimus-ide-collabsdk.ProvisionerJobPending})
	require.Equal(t, waitForWorkspaceStarted, uut.state)
	waitForGoroutines(testCtx, t, uut)

	uut.handleBuildUpdate(&workspacesdk.BuildUpdate{Transition: optimus-ide-collabsdk.WorkspaceTransitionStart, JobStatus: optimus-ide-collabsdk.ProvisionerJobRunning})
	require.Equal(t, waitForWorkspaceStarted, uut.state)
	waitForGoroutines(testCtx, t, uut)

	uut.handleBuildUpdate(&workspacesdk.BuildUpdate{Transition: optimus-ide-collabsdk.WorkspaceTransitionStart, JobStatus: optimus-ide-collabsdk.ProvisionerJobSucceeded})
...
func TestBuildUpdatesBadJobs(t *testing.T) {
	t.Parallel()
	for _, jobStatus := range []optimus-ide-collabsdk.ProvisionerJobStatus{
		optimus-ide-collabsdk.ProvisionerJobFailed,
		optimus-ide-collabsdk.ProvisionerJobCanceling,
		optimus-ide-collabsdk.ProvisionerJobCanceled,
		optimus-ide-collabsdk.ProvisionerJobUnknown,
	} {
		t.Run(string(jobStatus), func(t *testing.T) {
			...
			uut.handleBuildUpdate(&workspacesdk.BuildUpdate{Transition: optimus-ide-collabsdk.WorkspaceTransitionStop, JobStatus: jobStatus})
...
func TestAgentUpdate_Coverage(t *testing.T) {
	...
	for s := range maxState {
		for _, lifecycle := range optimus-ide-collabsdk.WorkspaceAgentLifecycleOrder {
			for _, noAutostart := range []bool{true, false} {
				for _, noWaitForScripts := range []bool{true, false} {
...
func TestAgentUpdateReady(t *testing.T) {
	...
	uut.handleAgentUpdate(&workspacesdk.AgentUpdate{Lifecycle: optimus-ide-collabsdk.WorkspaceAgentLifecycleReady, ID: agentID})
...
func TestAgentUpdateNoWait(t *testing.T) {
	...
	uut.handleAgentUpdate(&workspacesdk.AgentUpdate{Lifecycle: optimus-ide-collabsdk.WorkspaceAgentLifecycleStarting, ID: agentID})
	require.Equal(t, establishTailnet, uut.state)
	event := testutil.RequireReceive(testCtx, t, uut.events)
	require.NotNil(t, event.tailnetUpdate)
	require.True(t, fClient.dialed)
...
func TestTunneler_EventLoop_Signal(t *testing.T) {
	...
	testutil.RequireSend(testCtx, t, uut.events, tunnelerEvent{
		buildUpdate: &workspacesdk.BuildUpdate{
			...
			JobStatus:  optimus-ide-collabsdk.ProvisionerJobPending,
		},
	})
	testutil.RequireSend(testCtx, t, uut.events, tunnelerEvent{
		buildUpdate: &workspacesdk.BuildUpdate{
			Transition: optimus-ide-collabsdk.WorkspaceTransitionStart,
			JobStatus:  optimus-ide-collabsdk.ProvisionerJobRunning,
		},
	})
	testutil.RequireSend(testCtx, t, uut.events, tunnelerEvent{
		buildUpdate: &workspacesdk.BuildUpdate{
			Transition: optimus-ide-collabsdk.WorkspaceTransitionStart,
			JobStatus:  optimus-ide-collabsdk.ProvisionerJobSucceeded,
		},
	})
	testutil.RequireSend(testCtx, t, uut.events, tunnelerEvent{
		agentUpdate: &workspacesdk.AgentUpdate{
			Lifecycle: optimus-ide-collabsdk.WorkspaceAgentLifecycleReady,
			ID:        agentID,
		},
	})
...
func (*fakeClient) WorkspaceAgentConnectionWatch(context.Context, uuid.UUID, string) (dec *wsjson.Deoptimus-ide-collab[workspacesdk.ConnectionWatchEvent], err error) {
	// TODO implement me
	panic("implement me")
}
