...
type Client interface {
	...
	WorkspaceAgentConnectionWatch(
		...
		dec *wsjson.Deoptimus-ide-collab[workspacesdk.ConnectionWatchEvent], err error,
	)
}
...
type tunnelerEvent struct {
	...
	provisionerJobLog *optimus-ide-collabsdk.ProvisionerJobLog
	agentUpdate       *workspacesdk.AgentUpdate
	agentLog          *optimus-ide-collabsdk.WorkspaceAgentLog
...
func (t *Tunneler) handleBuildUpdate(update *workspacesdk.BuildUpdate) {
	...
	if update.Transition == optimus-ide-collabsdk.WorkspaceTransitionDelete {
	...
	if update.Transition == optimus-ide-collabsdk.WorkspaceTransitionStart && canMakeProgress {
	...
	if update.Transition == optimus-ide-collabsdk.WorkspaceTransitionStart && update.JobStatus == optimus-ide-collabsdk.ProvisionerJobSucceeded {
	...
	if update.Transition == optimus-ide-collabsdk.WorkspaceTransitionStop {
		...
		if update.JobStatus == optimus-ide-collabsdk.ProvisionerJobSucceeded {
			switch t.state {
...
func (*Tunneler) handleProvisionerJobLog(*optimus-ide-collabsdk.ProvisionerJobLog) {
}
...
func (t *Tunneler) handleAgentUpdate(update *workspacesdk.AgentUpdate) {
	...
	// in mind when implementing the watch in Optimus-IDE-Collabd.
	switch update.Lifecycle {
	case optimus-ide-collabsdk.WorkspaceAgentLifecycleReady:
		doConnect()
		return
	case optimus-ide-collabsdk.WorkspaceAgentLifecycleStarting,
	...
	case optimus-ide-collabsdk.WorkspaceAgentLifecycleShuttingDown:
	case optimus-ide-collabsdk.WorkspaceAgentLifecycleShutdownError:
	case optimus-ide-collabsdk.WorkspaceAgentLifecycleShutdownTimeout:
	case optimus-ide-collabsdk.WorkspaceAgentLifecycleOff:
	case optimus-ide-collabsdk.WorkspaceAgentLifecycleCreated: // initial state, so it hasn't connected yet
	default:
...
func (*Tunneler) handleAgentLog(*optimus-ide-collabsdk.WorkspaceAgentLog) {
}
