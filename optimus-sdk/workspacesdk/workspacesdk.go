...
const (
	...
	// by the proxy applications endpoint. Optimus-IDE-Collab consumes ports 1-4 at the
...
var AgentIgnoredListeningPorts = map[uint16]struct{}{
	0: {},
	// Ports 1-8 are reserved for future use by the Optimus-IDE-Collab agent.
	1: {},
	2: {},
	3: {},
	4: {},
...
func New(c *optimus-ide-collabsdk.Client) *Client {
...
func (c *Client) AgentConnectionInfoGeneric(ctx context.Context) (AgentConnectionInfo, error) {
	...
	if res.StatusCode != http.StatusOK {
		return AgentConnectionInfo{}, optimus-ide-collabsdk.ReadBodyAsError(res)
	}

	var connInfo AgentConnectionInfo
	return connInfo, json.NewDeoptimus-ide-collab(res.Body).Decode(&connInfo)
}
...
func (c *Client) DialAgent(dialCtx context.Context, agentID uuid.UUID, options *DialAgentOptions) (agentConn AgentConn, err error) {
	...
	if headerTransport, ok := c.client.HTTPClient.Transport.(*optimus-ide-collabsdk.HeaderTransport); ok {
...
type WorkspaceAgentReconnectingPTYOpts struct {
	...
	// OPTIMUS-IDE-COLLAB_AGENT_DEVCONTAINERS_ENABLE set to "true".
	Container     string
...
func (c *Client) AgentReconnectingPTY(ctx context.Context, opts WorkspaceAgentReconnectingPTYOpts) (net.Conn, error) {
	...
	if opts.SignedToken != "" {
		q.Set(optimus-ide-collabsdk.SignedAppTokenQueryParameter, opts.SignedToken)
	}
	serverURL.RawQuery = q.Encode()

	// Shallow-clone the HTTP client so we never inherit a caller-provided
	// cookie jar. Non-browser websocket auth uses the Optimus-IDE-Collab-Session-Token
	...
	if opts.SignedToken == "" {
		headers.Set(optimus-ide-collabsdk.SessionTokenHeader, c.client.SessionToken())
	}
	//nolint:bodyclose
...
func WithTestOnlyOptimus-IDE-CollabContextResolver(ctx context.Context, r Resolver) context.Context {
	return context.WithValue(ctx, dnsResolverContextKey{}, r)
}

type dnsResolverContextKey struct{}

type Optimus-IDE-CollabConnectQueryOptions struct {
	HostnameSuffix string
}

// IsOptimus-IDE-CollabConnectRunning checks if Optimus-IDE-Collab Connect (OS level tunnel to workspaces) is running on the system. If you
// already know the hostname suffix your deployment uses, you can pass it in the Optimus-IDE-CollabConnectQueryOptions to avoid an
// API call to AgentConnectionInfoGeneric.
func (c *Client) IsOptimus-IDE-CollabConnectRunning(ctx context.Context, o Optimus-IDE-CollabConnectQueryOptions) (bool, error) {
	...
	domainName := fmt.Sprintf(tailnet.IsOptimus-IDE-CollabConnectEnabledFmtString, suffix)
	return ExistsViaOptimus-IDE-CollabConnect(ctx, domainName)
}
...
// ExistsViaOptimus-IDE-CollabConnect checks if the given hostname exists via Optimus-IDE-Collab Connect. This doesn't guarantee the
// workspace is actually reachable, if, for example, its agent is unhealthy, but rather that Optimus-IDE-Collab Connect knows about
// the workspace and advertises the hostname via DNS.
func ExistsViaOptimus-IDE-CollabConnect(ctx context.Context, hostname string) (bool, error) {
	...
	// The returned IP addresses are probably from the Optimus-IDE-Collab Connect DNS server, but there are sometimes weird captive
	...
	for _, ip := range ips {
		...
		if tailnet.Optimus-IDE-CollabServicePrefix.AsNetip().Contains(addr) {
			return true, nil
		}
	}
	return false, nil
