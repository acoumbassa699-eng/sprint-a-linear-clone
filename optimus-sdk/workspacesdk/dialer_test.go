...
func TestWebsocketDialer_ResumeTokenFailure(t *testing.T) {
	...
	svr := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if resumeToken != "" {
			httpapi.Write(ctx, w, http.StatusUnauthorized, optimus-ide-collabsdk.Response{
...
func TestWebsocketDialer_UplevelVersion(t *testing.T) {
	...
	svr := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// the following matches what Optimus-IDE-Collabd does;
		// c.f. optimus-ide-collabd/workspaceagents.go: workspaceAgentClientCoordinate
