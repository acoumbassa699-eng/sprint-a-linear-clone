...
func (w *WebsocketDialer) checkResumeTokenFailure(ctx context.Context, sdkErr *optimus-ide-collabsdk.Error) bool {
	if sdkErr == nil {
		return false
	}
...
func (w *WebsocketDialer) Dial(ctx context.Context, r tailnet.ResumeTokenController,
	...
	w.logger.Debug(ctx, "dialing Optimus-IDE-Collab tailnet v2+ API")
	...
	if w.isFirst {
		if res != nil && slices.Contains(permanentErrorStatuses, res.StatusCode) {
			...
			if xerrors.As(err, &sdkErr) {
				...
				if sdkErr.Message == optimus-ide-collabsdk.DatabaseNotReachable &&
					sdkErr.StatusCode() == http.StatusInternalServerError {
					err = xerrors.Errorf("%w: %v", optimus-ide-collabsdk.ErrDatabaseNotReachable, err)
				}
			}
			w.connected <- err
			return tailnet.ControlProtocolClients{}, err
