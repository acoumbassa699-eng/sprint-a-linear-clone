...
func TestWebsocketNetConn_LargeWrites(t *testing.T) {
	...
	svr := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, nc := optimus-ide-collabsdk.WebsocketNetConn(r.Context(), sws, websocket.MessageBinary)
		defer nc.Close()
	...
	require.NoError(t, err)
	_, cnc := optimus-ide-collabsdk.WebsocketNetConn(ctx, cws, websocket.MessageBinary)
	ck, err := cnc.Write(cb)
	require.NoError(t, err)
