...
func TestAIProviderSettings_Marshal(t *testing.T) {
	...
	t.Run("EmptyEmitsNull", func(t *testing.T) {
		t.Parallel()
		got, err := json.Marshal(optimus-ide-collabsdk.AIProviderSettings{})
...
func TestAIProviderSettings_Unmarshal(t *testing.T) {
	...
	t.Run("EmptyInputZeroes", func(t *testing.T) {
		...
		// (e.g. row deoptimus-ide-collabs) that hand it raw column bytes.
	...
	t.Run("ResetsBetweenCalls", func(t *testing.T) {
		...
		s := optimus-ide-collabsdk.AIProviderSettings{
			Bedrock: &optimus-ide-collabsdk.AIProviderBedrockSettings{Region: "us-east-1"},
		}
		require.NoError(t, json.Unmarshal([]byte(`null`), &s))
		require.True(t, s.IsZero())
...
func TestAIProviderSettings_Roundtrip(t *testing.T) {
	t.Parallel()
	orig := optimus-ide-collabsdk.AIProviderSettings{
		Bedrock: &optimus-ide-collabsdk.AIProviderBedrockSettings{
			Region:          "us-west-2",
	...
	var got optimus-ide-collabsdk.AIProviderSettings
...
func TestAIProviderRequest_ValidateRoleARN(t *testing.T) {
	...
	hasRoleARNError := func(vs []optimus-ide-collabsdk.ValidationError) bool {
		for _, v := range vs {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			create := optimus-ide-collabsdk.CreateAIProviderRequest{
			...
			update := optimus-ide-collabsdk.UpdateAIProviderRequest{Settings: &settings}
...
func TestAIProviderRequest_ValidateBedrockProtocol(t *testing.T) {
	...
	cases := []struct {
		name     string
		protocol optimus-ide-collabsdk.AIProviderBedrockProtocol
		wantErr  bool
	}{
		{name: "empty is allowed", protocol: "", wantErr: false},
		{name: "invoke-model", protocol: optimus-ide-collabsdk.AIProviderBedrockProtocolInvokeModel, wantErr: false},
		{name: "mantle", protocol: optimus-ide-collabsdk.AIProviderBedrockProtocolMantle, wantErr: false},
		{name: "typo", protocol: "mnatle", wantErr: true},
		{name: "unknown", protocol: "http", wantErr: true},
	}

	hasProtocolError := func(vs []optimus-ide-collabsdk.ValidationError) bool {
		for _, v := range vs {
...
func TestAIProviderRequest_ValidateBedrockMantle(t *testing.T) {
	...
	hasFieldError := func(vs []optimus-ide-collabsdk.ValidationError, field string) bool {
		for _, v := range vs {
			if v.Field == field {
				return true
			}
	...
	t.Run("MantleRequiresRegion", func(t *testing.T) {
		...
		create := optimus-ide-collabsdk.CreateAIProviderRequest{
			...
			Settings: optimus-ide-collabsdk.AIProviderSettings{
				Bedrock: &optimus-ide-collabsdk.AIProviderBedrockSettings{
					Protocol: optimus-ide-collabsdk.AIProviderBedrockProtocolMantle,
				},
			},
		}
	...
	t.Run("InvokeModelDoesNotRequireRegionField", func(t *testing.T) {
		...
		for _, protocol := range []optimus-ide-collabsdk.AIProviderBedrockProtocol{"", optimus-ide-collabsdk.AIProviderBedrockProtocolInvokeModel} {
			create := optimus-ide-collabsdk.CreateAIProviderRequest{
				Type:    optimus-ide-collabsdk.AIProviderTypeBedrock,
				Name:    "bedrock",
				BaseURL: "https://bedrock-runtime.us-east-1.amazonaws.com",
				Settings: optimus-ide-collabsdk.AIProviderSettings{
					Bedrock: &optimus-ide-collabsdk.AIProviderBedrockSettings{Protocol: protocol},
				},
			}
			require.False(t, hasFieldError(create.Validate(), "settings.region"))
		}
