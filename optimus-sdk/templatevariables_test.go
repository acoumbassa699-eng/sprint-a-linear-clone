...
func TestDiscoverVarsFiles(t *testing.T) {
	...
	// When
	found, err := optimus-ide-collabsdk.DiscoverVarsFiles(tempDir)
	require.NoError(t, err)
...
go_image = ["1.19","1.20","1.21"]`
	...
	// When
	actual, err := optimus-ide-collabsdk.ParseUserVariableValues([]string{
	...
	// Then
	expected := []optimus-ide-collabsdk.VariableValue{
