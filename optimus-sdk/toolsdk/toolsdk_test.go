...
func setupWorkspaceForAgent(t *testing.T, opts *optimus-ide-collabdtest.Options) (*optimus-ide-collabsdk.Client, database.WorkspaceTable, string) {
	t.Helper()
...
// nolint:gocritic // This is in a test package and does not end up in the build
func setupWorkspaceForAgentWithName(t *testing.T, opts *optimus-ide-collabdtest.Options, workspaceName string) (*optimus-ide-collabsdk.Client, database.WorkspaceTable, string) {
	t.Helper()

	client, store := optimus-ide-collabdtest.NewWithDatabase(t, opts)
...
// These tests are dependent on the state of the optimus-ide-collab server.
...
func TestTools(t *testing.T) {
	// Given: a running optimus-ide-collabd instance using SSH test setup pattern
	...
	t.Run("ListTemplates", func(t *testing.T) {
		...
		expected, err := memberClient.Templates(context.Background(), optimus-ide-collabsdk.TemplateFilter{})
		require.NoError(t, err)
	...
	t.Run("CreateWorkspaceBuild", func(t *testing.T) {
		t.Run("Stop", func(t *testing.T) {
			...
			require.NoError(t, err)
			require.Equal(t, optimus-ide-collabsdk.WorkspaceTransitionStop, result.Transition)
			...
			require.NoError(t, client.CancelWorkspaceBuild(ctx, result.ID, optimus-ide-collabsdk.CancelWorkspaceBuildParams{}))
		})
		...
		t.Run("TemplateVersionChange", func(t *testing.T) {
			...
			require.NoError(t, err)
			require.Equal(t, optimus-ide-collabsdk.WorkspaceTransitionStart, updateBuild.Transition)
			...
			require.NoError(t, client.CancelWorkspaceBuild(ctx, updateBuild.ID, optimus-ide-collabsdk.CancelWorkspaceBuildParams{}))
			...
			require.NoError(t, err)
			require.Equal(t, optimus-ide-collabsdk.WorkspaceTransitionStart, rollbackBuild.Transition)
			...
			// Cancel the build so it doesn't remain in the 'pending' state indefinitely.
			require.NoError(t, client.CancelWorkspaceBuild(ctx, rollbackBuild.ID, optimus-ide-collabsdk.CancelWorkspaceBuildParams{}))
		})
		...
		t.Run("Start_WithPreset", func(t *testing.T) {
			...
			require.NoError(t, err)
			require.Equal(t, optimus-ide-collabsdk.WorkspaceTransitionStart, result.Transition)
			require.Equal(t, r.Workspace.ID, result.WorkspaceID)
	...
	t.Run("WorkspaceSSHExec", func(t *testing.T) {
		...
		optimus-ide-collabdtest.NewWorkspaceAgentWaiter(t, uuidClient, uuidWorkspace.ID).Wait()
	...
	t.Run("WorkspacePortForward", func(t *testing.T) {
		...
		tests := []struct {
		...
		}{
			{
				...
				port:      1234,
				host:      "*.test.optimus-ide-collab.com",
				expect:    "%s://1234--dev--myworkspace--myuser.test.optimus-ide-collab.com:%s",
			},
			{
		...
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				t.Parallel()
				client, workspace, agentToken := setupWorkspaceForAgent(t, &optimus-ide-collabdtest.Options{
					AppHostname: tt.host,
				})
				_ = agenttest.New(t, client.URL, agentToken)
				optimus-ide-collabdtest.NewWorkspaceAgentWaiter(t, client, workspace.ID).Wait()
	...
	t.Run("CreateTask", func(t *testing.T) {
		...
		tests := []struct {
			...
			},
			{
				...
				error: "Template does not have a valid \"optimus-ide-collab_ai_task\" resource.",
			},
			{
				name: "WithPreset",
				args: toolsdk.CreateTaskArgs{
	...
	t.Run("ListTasks", func(t *testing.T) {
		...
		owner := optimus-ide-collabdtest.CreateFirstUser(t, client)
		_, member := optimus-ide-collabdtest.CreateAnotherUser(t, client, owner.OrganizationID)
		taskClient, _ := optimus-ide-collabdtest.CreateAnotherUser(t, client, owner.OrganizationID)
		...
		_, err := client.CreateTask(ctx, member.Username, optimus-ide-collabsdk.CreateTaskRequest{
		...
		for i := range 5 {
			...
			task, err := taskClient.CreateTask(ctx, optimus-ide-collabsdk.Me, optimus-ide-collabsdk.CreateTaskRequest{
				TemplateVersionID: template.ActiveVersionID,
			...
			if i == 0 {
				...
				require.NoError(t, err)
				optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, taskClient, ws.LatestBuild.ID)

				// Stop the workspace to set task status to paused.
				build, err := taskClient.CreateWorkspaceBuild(ctx, task.WorkspaceID.UUID, optimus-ide-collabsdk.CreateWorkspaceBuildRequest{
					Transition: optimus-ide-collabsdk.WorkspaceTransitionStop,
				})
				require.NoError(t, err)
				optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, taskClient, build.ID)
			}
		}
		...
		tests := []struct {
			...
			},
			{
				name: "ListFiltered",
				args: toolsdk.ListTasksArgs{
					Status: optimus-ide-collabsdk.TaskStatusPaused,
				},
				expected: []string{
					"list-task-workspace-0",
				},
	...
	t.Run("GetTask", func(t *testing.T) {
		...
		tests := []struct {
			...
			expected optimus-ide-collabsdk.TaskStatus
			error    string
		}{
			{
				name: "ByUUID",
				...
				expected: optimus-ide-collabsdk.TaskStatusInitializing,
			},
			{
				name: "ByIdentifier",
				args: toolsdk.GetTaskStatusArgs{
	...
	t.Run("WorkspaceListApps", func(t *testing.T) {
		...
		_ = dbfake.WorkspaceBuild(t, store, database.WorkspaceTable{
			...
			agents[0].Apps = []*proto.App{
				{
					Slug: "zero",
					Url:  "http://zero.dev.optimus-ide-collab.com",
				},
			}
			return agents
		}).Do()
		...
		_ = dbfake.WorkspaceBuild(t, store, database.WorkspaceTable{
			...
			agents[0].Apps = []*proto.App{
				{
					Slug: "one",
					Url:  "http://one.dev.optimus-ide-collab.com",
				},
				{
					Slug: "two",
					Url:  "http://two.dev.optimus-ide-collab.com",
				},
				{
					Slug: "three",
					Url:  "http://three.dev.optimus-ide-collab.com",
				},
			}
			agents = append(agents, &proto.Agent{
				...
				Apps: []*proto.App{
					{
						Slug: "four",
						Url:  "http://four.dev.optimus-ide-collab.com",
					},
				},
			})
			return agents
	...
	t.Run("SendTaskInput", func(t *testing.T) {
		...
		optimus-ide-collabdtest.NewWorkspaceAgentWaiter(t, client, ws.Workspace.ID).
				...
				if tt.error != "" {
					...
					for i, msg := range tt.expected {
						...
						if msg.Role == agentapi.RoleUser {
							require.Equal(t, optimus-ide-collabsdk.TaskLogTypeInput, res.Logs[i].Type)
						} else {
							require.Equal(t, optimus-ide-collabsdk.TaskLogTypeOutput, res.Logs[i].Type)
						}
...
func TestReportTaskNilPointerDeref(t *testing.T) {
	...
	client, _ := optimus-ide-collabdtest.NewWithDatabase(t, nil)
	...
	args := toolsdk.ReportTaskArgs{
		Summary: "Test task",
		Link:    "https://example.com",
		State:   string(optimus-ide-collabsdk.WorkspaceAppStatusStateWorking),
	}
...
func TestReportTaskWithReporter(t *testing.T) {
	...
	reporter := func(args toolsdk.ReportTaskArgs) error {
		...
		require.Equal(t, string(optimus-ide-collabsdk.WorkspaceAppStatusStateWorking), args.State)
		return nil
	}
...
func TestNormalizeWorkspaceInput(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:     "Optimus-IDE-CollabConnectFormat",
			input:    "agent.workspace.owner", // Special Optimus-IDE-Collab Connect reverse format
			expected: "owner/workspace.agent",
		},
	}
