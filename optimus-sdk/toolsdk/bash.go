...
var WorkspaceBash = Tool[WorkspaceBashArgs, WorkspaceBashResult]{
	Tool: aisdk.Tool{
		...
		Description: `Execute a bash command in a Optimus-IDE-Collab workspace.

This tool provides the same functionality as the 'optimus-ide-collab ssh <workspace> <command>' CLI command.
...
func findWorkspaceAndAgent(ctx context.Context, client *optimus-ide-collabsdk.Client, workspaceName string) (optimus-ide-collabsdk.Workspace, optimus-ide-collabsdk.WorkspaceAgent, error) {
	...
	if err != nil {
		return optimus-ide-collabsdk.Workspace{}, optimus-ide-collabsdk.WorkspaceAgent{}, err
	}
	...
	if workspace.LatestBuild.Transition != optimus-ide-collabsdk.WorkspaceTransitionStart {
		if workspace.LatestBuild.Transition == optimus-ide-collabsdk.WorkspaceTransitionDelete {
			return optimus-ide-collabsdk.Workspace{}, optimus-ide-collabsdk.WorkspaceAgent{}, xerrors.Errorf("workspace %q is deleted", workspace.Name)
		}
		if workspace.LatestBuild.Job.Status == optimus-ide-collabsdk.ProvisionerJobFailed {
			return optimus-ide-collabsdk.Workspace{}, optimus-ide-collabsdk.WorkspaceAgent{}, xerrors.Errorf("workspace %q is in failed state", workspace.Name)
		}
		if workspace.LatestBuild.Status != optimus-ide-collabsdk.WorkspaceStatusStopped {
			return optimus-ide-collabsdk.Workspace{}, optimus-ide-collabsdk.WorkspaceAgent{}, xerrors.Errorf("workspace must be started; was unable to autostart as the last build job is %q, expected %q",
				workspace.LatestBuild.Status, optimus-ide-collabsdk.WorkspaceStatusStopped)
		}
		...
		if err != nil {
			return optimus-ide-collabsdk.Workspace{}, optimus-ide-collabsdk.WorkspaceAgent{}, xerrors.Errorf("failed to start workspace: %w", err)
		}
		...
		if build.Job.CompletedAt == nil {
			...
			if err != nil {
				return optimus-ide-collabsdk.Workspace{}, optimus-ide-collabsdk.WorkspaceAgent{}, xerrors.Errorf("failed to wait for build completion: %w", err)
			}
		}
...
func getWorkspaceAgent(workspace optimus-ide-collabsdk.Workspace, agentName string) (optimus-ide-collabsdk.WorkspaceAgent, error) {
	...
	if len(agents) == 0 {
		return optimus-ide-collabsdk.WorkspaceAgent{}, xerrors.Errorf("workspace %q has no agents", workspace.Name)
	}
	...
	if agentName != "" {
		...
		return optimus-ide-collabsdk.WorkspaceAgent{}, xerrors.Errorf("agent not found by name %q, available agents: %v", agentName, availableNames)
	}
	...
	return optimus-ide-collabsdk.WorkspaceAgent{}, xerrors.Errorf("multiple agents found, please specify the agent name, available agents: %v", availableNames)
}
