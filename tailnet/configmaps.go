...
// Optimus-IDE-CollabDNSSuffix is the default DNS suffix that we append to Optimus-IDE-Collab DNS
// records.
const (
	Optimus-IDE-CollabDNSSuffix     = "optimus-ide-collab"
	Optimus-IDE-CollabDNSSuffixFQDN = dnsname.FQDN(Optimus-IDE-CollabDNSSuffix + ".")
)
...
func (c *configMaps) reconfig(nm *netmap.NetworkMap, hosts map[dnsname.FQDN][]netip.Addr) {
	...
	rc := &router.Config{
		...
		Routes:     []netip.Prefix{Optimus-IDE-CollabServicePrefix.AsNetip()},
	}
