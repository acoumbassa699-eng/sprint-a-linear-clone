...
// perspective of a protocol client (i.e. the Optimus-IDE-Collab Agent is also a client of this interface).
...
type BasicCoordinationController struct {
	...
	Direction optimus-ide-collabsdk.ConnectionDirection
}
...
func (c *BasicCoordination) Close(ctx context.Context) (retErr error) {
	...
	if err != nil && !xerrors.Is(err, io.EOF) {
		reason := optimus-ide-collabsdk.DisconnectReasonNetworkError
		...
		reason := optimus-ide-collabsdk.DisconnectReasonGraceful
	...
	case <-ctx.Done():
		...
		c.logger.Warn(ctx, "context expired while waiting for coordinate responses to close",
			...
			reason.SlogField(),
			c.initiator.SlogField(),
			optimus-ide-collabsdk.SlogDisconnectDetail("context expired before coordinator hung up"),
		)
	}
...
// tunnel sources (that is, they create tunnel --- Optimus-IDE-Collab clients not workspaces).
func NewTunnelSrcCoordController(
	...
	return &TunnelSrcCoordController{
		BasicCoordinationController: &BasicCoordinationController{
			...
			Coordinatee: coordinatee,
			SendAcks:    false,
			Initiator:   optimus-ide-collabsdk.DisconnectInitiatorClient,
			Direction:   optimus-ide-collabsdk.ConnectionDirectionClientToServer,
		},
...
// NewAgentCoordinationController creates a CoordinationController for Optimus-IDE-Collab Agents, which never
...
func NewAgentCoordinationController(
	...
	return &BasicCoordinationController{
		...
		Coordinatee: coordinatee,
		SendAcks:    true,
		Initiator:   optimus-ide-collabsdk.DisconnectInitiatorAgent,
		Direction:   optimus-ide-collabsdk.ConnectionDirectionServerToAgent,
	}
}
...
func (w *Workspace) updateDNSNames(options DNSNameOptions) error {
	...
	for id, a := range w.agents {
		...
		names[fqdn] = []netip.Addr{Optimus-IDE-CollabServicePrefix.AddrFromUUID(a.ID)}
...
func (t *TunnelAllWorkspaceUpdatesController) New(client WorkspaceUpdatesClient) CloserWaiter {
	...
	// workspace update arrives, breaking .optimus-ide-collab resolution.
...
func (t *tunnelUpdater) updateDNSNamesLocked() map[dnsname.FQDN][]netip.Addr {
	...
	isOptimus-IDE-CollabConnectEnabledFQDN, err := dnsname.ToFQDN(fmt.Sprintf(IsOptimus-IDE-CollabConnectEnabledFmtString, t.dnsNameOptions.Suffix))
	if err != nil {
		t.logger.Critical(context.Background(),
			"failed to include Optimus-IDE-Collab Connect enabled DNS name", slog.F("suffix", t.dnsNameOptions.Suffix))
	} else {
		names[isOptimus-IDE-CollabConnectEnabledFQDN] = []netip.Addr{tsaddr.Optimus-IDE-CollabServiceIPv6()}
	}
	return names
}
...
func NewTunnelAllWorkspaceUpdatesController(
	...
	t := &TunnelAllWorkspaceUpdatesController{
		logger:         logger,
		coordCtrl:      c,
		dnsNameOptions: DNSNameOptions{Optimus-IDE-CollabDNSSuffix},
	}
	...
	go func() {
		...
		for retrier := retry.New(50*time.Millisecond, 10*time.Second); retrier.Wait(c.ctx); {
			...
			if err != nil {
				...
				if errors.Is(err, optimus-ide-collabsdk.ErrDatabaseNotReachable) {
