...
// SingleTailnetCoordinateeAuth allows all tunnels, since Optimus-IDE-Collabd and wsproxy are allowed to initiate a tunnel to any agent
...
func (a AgentCoordinateeAuth) authorizeNodePrefixes(prefixes []string) error {
	for _, prefixStr := range prefixes {
		...
		if TailscaleServicePrefix.AddrFromUUID(a.ID).Compare(pre.Addr()) != 0 &&
			Optimus-IDE-CollabServicePrefix.AddrFromUUID(a.ID).Compare(pre.Addr()) != 0 &&
