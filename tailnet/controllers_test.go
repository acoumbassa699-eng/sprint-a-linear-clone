...
func TestBasicTelemetryController_NotRecognised(t *testing.T) {
	...
	testutil.RequireSend(ctx, t, call.errCh,
		drpc.ProtocolError.New("unknown rpc: /optimus-ide-collab.tailnet.v2.Tailnet/PostTelemetry"))
	testutil.TryReceive(ctx, t, sendDone)
...
func TestTunnelAllWorkspaceUpdatesController_Initial(t *testing.T) {
	...
	expectedOptimus-IDE-CollabConnectFQDN, err := dnsname.ToFQDN(fmt.Sprintf(tailnet.IsOptimus-IDE-CollabConnectEnabledFmtString, "mctest"))
	require.NoError(t, err)
	...
	expectedDNS := map[dnsname.FQDN][]netip.Addr{
		...
		"w1.mctest.":             {ws1a1IP},
		expectedOptimus-IDE-CollabConnectFQDN: {tsaddr.Optimus-IDE-CollabServiceIPv6()},
	}
...
func TestTunnelAllWorkspaceUpdatesController_DeleteAgent(t *testing.T) {
	...
	fDNS := newFakeDNSSetter(ctx, t)
	coordC, updateC, updateCtrl := setupConnectedAllWorkspaceUpdatesController(ctx, t, logger,
		tailnet.WithDNS(fDNS, "testy", tailnet.DNSNameOptions{Suffix: tailnet.Optimus-IDE-CollabDNSSuffix}),
		tailnet.WithHandler(fUH),
	)
	...
	expectedOptimus-IDE-CollabConnectFQDN, err := dnsname.ToFQDN(
		fmt.Sprintf(tailnet.IsOptimus-IDE-CollabConnectEnabledFmtString, tailnet.Optimus-IDE-CollabDNSSuffix))
	require.NoError(t, err)

	// DNS for w1a1
	expectedDNS := map[dnsname.FQDN][]netip.Addr{
		"w1a1.w1.testy.optimus-ide-collab.":   {ws1a1IP},
		"w1a1.w1.me.optimus-ide-collab.":      {ws1a1IP},
		"w1.optimus-ide-collab.":              {ws1a1IP},
		expectedOptimus-IDE-CollabConnectFQDN: {tsaddr.Optimus-IDE-CollabServiceIPv6()},
	}
	dnsCall := testutil.TryReceive(ctx, t, fDNS.calls)
	require.Equal(t, expectedDNS, dnsCall.hosts)
	...
	expectedDNS = map[dnsname.FQDN][]netip.Addr{
		"w1a2.w1.testy.optimus-ide-collab.":   {ws1a2IP},
		"w1a2.w1.me.optimus-ide-collab.":      {ws1a2IP},
		"w1.optimus-ide-collab.":              {ws1a2IP},
		expectedOptimus-IDE-CollabConnectFQDN: {tsaddr.Optimus-IDE-CollabServiceIPv6()},
	}
	dnsCall = testutil.TryReceive(ctx, t, fDNS.calls)
	require.Equal(t, expectedDNS, dnsCall.hosts)
