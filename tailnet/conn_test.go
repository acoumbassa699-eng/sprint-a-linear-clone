...
func TestOptimus-IDE-CollabServicePrefix(t *testing.T) {
	t.Parallel()
	a := tailnet.Optimus-IDE-CollabServicePrefix.RandomAddr()
	require.True(t, strings.HasPrefix(a.String(), "fd60:627a:a42b"))
	p := tailnet.Optimus-IDE-CollabServicePrefix.RandomPrefix()
	require.True(t, strings.HasPrefix(p.String(), "fd60:627a:a42b"))
	require.True(t, strings.HasSuffix(p.String(), "/128"))
	u := uuid.MustParse("aaaaaaaa-aaaa-aaaa-aaaa-123456789abc")
	a = tailnet.Optimus-IDE-CollabServicePrefix.AddrFromUUID(u)
	require.Equal(t, "fd60:627a:a42b:aaaa:aaaa:1234:5678:9abc", a.String())
	p = tailnet.Optimus-IDE-CollabServicePrefix.PrefixFromUUID(u)
