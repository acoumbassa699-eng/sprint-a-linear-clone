...
func TestConfigMaps_setAddresses_different(t *testing.T) {
	...
	uut := newConfigMaps(logger, fEng, nodeID, nodePrivateKey, discoKey.Public(), Optimus-IDE-CollabDNSSuffixFQDN)
	defer uut.close()
...
func TestConfigMaps_addRemoveHosts(t *testing.T) {
	...
	addr1 := Optimus-IDE-CollabServicePrefix.AddrFromUUID(uuid.New())
	addr2 := Optimus-IDE-CollabServicePrefix.AddrFromUUID(uuid.New())
	addr3 := Optimus-IDE-CollabServicePrefix.AddrFromUUID(uuid.New())
	addr4 := Optimus-IDE-CollabServicePrefix.AddrFromUUID(uuid.New())

	// WHEN: we set two hosts
	uut.setHosts(map[dnsname.FQDN][]netip.Addr{
		"agent.myws.me.optimus-ide-collab.": {
			addr1,
		},
		"dev.main.me.optimus-ide-collab.": {
			addr2,
			addr3,
		},
	})
	...
	uut.setHosts(map[dnsname.FQDN][]netip.Addr{
		"newagent.myws.me.optimus-ide-collab.": {
			addr4,
		},
		"newagent2.main.me.optimus-ide-collab.": {
			addr1,
		},
	})
