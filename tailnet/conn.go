...
const EnvMagicsockDebugLogging = "OPTIMUS-IDE-COLLAB_MAGICSOCK_DEBUG_LOGGING"
...
func init() {
	...
	// agents, it's connections to a small number of clients (CLI or Optimus-IDE-Collabd)
...
type Options struct {
	...
	ForceNetworkUp bool
	// Network Telemetry Client Type: CLI | Agent | optimus-ide-collabd
...
// TelemetrySink allows tailnet.Conn to send network telemetry to the Optimus-IDE-Collab
// server.
...
func NewConn(options *Options) (conn *Conn, err error) {
	...
	// The Optimus-IDE-Collab soft isolation mode is a workaround to allow Optimus-IDE-Collab Connect to
	// connect to Optimus-IDE-Collab servers behind corporate VPNs, and relaxes some of the
	// loop protections that come with Tailscale.
	// See the comment above the netns function for more details.
	netns.SetOptimus-IDE-CollabSoftIsolation(useNetNS)
	...
	if options.DNSMatchDomain == "" {
		options.DNSMatchDomain = Optimus-IDE-CollabDNSSuffix
	}
...
// A FQDN to be mapped to `tsaddr.Optimus-IDE-CollabServiceIPv6`. This address can be used
// when you want to know if Optimus-IDE-Collab Connect is running, but are not trying to
// connect to a specific known workspace.
const IsOptimus-IDE-CollabConnectEnabledFmtString = "is.optimus-ide-collab--connect--enabled--right--now.%s."

type ServicePrefix [6]byte

var (
	// TailscaleServicePrefix is the IPv6 prefix for all tailnet nodes since it was first added to
	// Optimus-IDE-Collab.  It is identical to the service prefix Tailscale.com uses. With the introduction of
	// Optimus-IDE-CollabVPN, we would like to stop using the Tailscale prefix so that we don't conflict with
	...
	// Optimus-IDE-CollabServicePrefix is the Optimus-IDE-Collab-specific IPv6 prefix for tailnet nodes, which we are in the
	// process of migrating to. It allows Optimus-IDE-Collab to run alongside Tailscale without conflicts even
	// if both are set up as TUN interfaces into the OS (e.g. Optimus-IDE-CollabVPN).
	// fd60:627a:a42b::/48
	Optimus-IDE-CollabServicePrefix ServicePrefix = [6]byte{0xfd, 0x60, 0x62, 0x7a, 0xa4, 0x2b}
)
