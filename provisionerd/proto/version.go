...
//     back to Optimus-IDE-Collabd. Used for uploading module files in support of dynamic
//     parameters.
...
//   - Added new field named `id` to `App`, which transports the ID generated by the optimus-ide-collab_app provider to be persisted.
...
//
// API v1.13:
//   - Removed experimental fields `exp_reuse_terraform_workspace`. Caching moved into Optimus-IDE-Collabd
//
// API v1.14:
//   - Added new field `template_version_modules_file` to Metadata
//   - Added `FailedFile` type for file upload failures.
//   - Add `DownloadFile` capability for provisioner daemons to fetch files from optimus-ide-collabd.
...
//     secret values from optimus-ide-collabd to provisioner daemons.
...
//     `optimus-ide-collab_secret` Terraform integration is being removed; user secrets are
