...
import (
	...
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/provisionerd/runner"
...
type Server struct {
	...
	// initConnectionCh will receive when the daemon connects to optimus-ide-collabd for the
	// first time.
...
// Connect establishes a connection to optimus-ide-collabd.
func (p *Server) connect() {
	...
	// This is to prevent server spam in case of a optimus-ide-collabd outage.
connectLoop:
	for retrier := retry.New(50*time.Millisecond, 10*time.Second); retrier.Wait(p.closeContext); {
		...
		p.opts.Logger.Debug(p.closeContext, "dialing optimus-ide-collabd")
		...
		if err != nil {
			...
			if errors.As(err, &sdkErr) && sdkErr.StatusCode() == http.StatusForbidden {
				p.opts.Logger.Error(p.closeContext, "not authorized to dial optimus-ide-collabd", slog.Error(err))
				return
			}
			if p.isClosed() {
				return
			}
			p.opts.Logger.Warn(p.closeContext, "optimus-ide-collabd client failed to dial", slog.Error(err))
			continue
		}
		// This log is useful to verify that an external provisioner daemon is
		// successfully connecting to optimus-ide-collabd. It doesn't add much value if the
		...
		// https://github.com/optimus-ide-collab/optimus-ide-collab/blob/5bd86cb1c06561d1d3e90ce689da220467e525c0/docs/admin/provisioners.md#L346
		logConnect(p.closeContext, "successfully connected to optimus-ide-collabd")
		retrier.Reset()
		...
		for {
			...
			case <-client.DRPCConn().Closed():
				logConnect(p.closeContext, "connection to optimus-ide-collabd closed")
...
// UploadModuleFiles will insert a file into the database of optimus-ide-collabd.
...
// DownloadFile will download a module file from optimus-ide-collabd.
