...
import (
	...
	strings2 "github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabd/util/strings"
...
// Fail immediately halts updates and, if the job is not complete sends FailJob to the optimus-ide-collab server. Running goroutines
// are canceled but complete asynchronously (although they are prevented from further updating the job to the optimus-ide-collab
...
// ForceStop signals all goroutines to stop and prevents any further API calls back to optimus-ide-collab server for this job
...
// heartbeatRoutine periodically sends updates on the job, which keeps optimus-ide-collab server
...
func (r *Runner) runTemplateImport(ctx context.Context) (*proto.CompletedJob, *proto.FailedJob) {
	...
	startProvision, err := r.runTemplateImportProvision(ctx, updateResponse.VariableValues, &sdkproto.Metadata{
		Optimus-IDE-CollabUrl:             r.job.GetTemplateImport().Metadata.Optimus-IDE-CollabUrl,
	...
	// For backwards compatibility with older versions of optimus-ide-collabd
...
func (r *Runner) runTemplateDryRun(ctx context.Context) (*proto.CompletedJob, *proto.FailedJob) {
	...
	if metadata.Optimus-IDE-CollabUrl == "" {
		metadata.Optimus-IDE-CollabUrl = "http://localhost:3000"
	}
...
func (r *Runner) runWorkspaceBuild(ctx context.Context) (*proto.CompletedJob, *proto.FailedJob) {
	...
	// Download modules if cached in optimus-ide-collabd
	...
	if planComplete.AiTaskCount > 1 {
		return nil, r.failedWorkspaceBuildf("only one 'optimus-ide-collab_ai_task' resource can be provisioned per template, found %d", planComplete.AiTaskCount)
	}
