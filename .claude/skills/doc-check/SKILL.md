...
> [!IMPORTANT]
> The **canonical** rules for what belongs in the Optimus-IDE-Collab docs (and what
> doesn't) live in
...
1. **Get the code changes.** Use the method provided in the prompt, or if
   none specified:
   - For a PR: `gh pr diff <PR_NUMBER> --repo optimus-ide-collab/optimus-ide-collab`
...
Redirects for [optimus-ide-collab.com/docs](https://optimus-ide-collab.com/docs) are configured in
a separate repo, not in this one. When a doc page is renamed or moved:

1. Update every link that relies on the old location.
2. Add an entry to
   [`optimus-ide-collab/optimus-ide-collab.com:redirects.json`](https://github.com/optimus-ide-collab/optimus-ide-collab.com/blob/master/redirects.json)
   that maps the old path to the new one. Open that PR alongside the
   `optimus-ide-collab/optimus-ide-collab` rename PR.

Do not create a `docs/_redirects` file in this repo; that format isn't
processed by optimus-ide-collab.com.

## Optimus-IDE-Collab-specific patterns
...
```sh
optimus-ide-collab server --flag-name value
```
