...
This guide documents the PR description style used in the Optimus-IDE-Collab repository, based on analysis of recent merged PRs.
...
- `perf(optimus-ide-collabd/database): add index on workspace_app_statuses.app_id`
...
Optimus-IDE-Collab only supports a single database connection URL.
```
...
1. **Link Related Work**
   - `Closes https://github.com/optimus-ide-collab/internal/issues/XXX`
   - `Depends on #XXX`
   - `Fixes: https://github.com/optimus-ide-collab/aibridge/issues/XX`
