...
Optimus-IDE-Collab development. Do not add new endpoints for agent debugging. Prefer the
...
- Optimus-IDE-Collab Prometheus metrics: `http://localhost:2114/`.
...
HTTP request logging is implemented in `optimus-ide-collabd/httpmw/loggermw`. Request log
...
HTTP tracing lives in `optimus-ide-collabd/tracing`. The middleware covers `/api`,
...
- `--trace` or `OPTIMUS-IDE-COLLAB_TRACE_ENABLE` enables application tracing.
- `--trace-logs` or `OPTIMUS-IDE-COLLAB_TRACE_LOGS` adds log events to traces.
- `--trace-honeycomb-api-key` or `OPTIMUS-IDE-COLLAB_TRACE_HONEYCOMB_API_KEY` enables the
  Honeycomb exporter.
- `--trace-datadog` or `OPTIMUS-IDE-COLLAB_TRACE_DATADOG` enables sending Go runtime
...
`./scripts/develop.sh` enables Optimus-IDE-Collab Prometheus metrics by default on
`0.0.0.0:2114`, served at `http://localhost:2114/`. The port is controlled by
`--prometheus-port` or `OPTIMUS-IDE-COLLAB_DEV_PROMETHEUS_PORT`. Set it to `0` to disable
...
If `--prometheus-server` or `OPTIMUS-IDE-COLLAB_DEV_PROMETHEUS_SERVER` is set, the develop
script attempts to start a Docker container named `optimus-ide-collab-prometheus` on Linux.
...
- `optimus-ide-collabd/httpmw/prometheus.go` for HTTP request counters, concurrency gauges,
  websocket gauges, and latency histograms.
- `optimus-ide-collabd/prometheusmetrics/` for active users, workspaces, agents, build
  info, experiments, insights, and agent stats collectors.
- `optimus-ide-collabd/database/dbmetrics/` for database query and transaction metrics.
...
4. Check `http://localhost:2114/` for metrics that match the route or subsystem.
   Start with `optimus-ide-collabd_api_requests_processed_total`,
   `optimus-ide-collabd_api_request_latencies_seconds`, and database metrics under the
   `optimus-ide-collabd_db_` prefix.
...
- Check `optimus-ide-collabd_api_requests_processed_total` and
  `optimus-ide-collabd_api_request_latencies_seconds` for the matching route.
- If database work is involved, check `optimus-ide-collabd_db_query_counts_total`,
  `optimus-ide-collabd_db_query_latencies_seconds`, and transaction metrics.
