...
- How to reproduce: Modify `optimus-ide-collabd/database/queries/*.sql` and run tests or
  builds without regenerating `optimus-ide-collabd/database/queries.sql.go` and related
  generated files.
...
- Proposed prevention: Assign each worktree a unique `OPTIMUS-IDE-COLLAB_DEV_PORT`,
  `OPTIMUS-IDE-COLLAB_DEV_WEB_PORT`, `OPTIMUS-IDE-COLLAB_DEV_PROXY_PORT`, and
  `OPTIMUS-IDE-COLLAB_DEV_PROMETHEUS_PORT` before starting the app.
