# Optimus-IDE-Collab Architecture

This document provides an overview of Optimus-IDE-Collab's architecture and core systems.

## What is Optimus-IDE-Collab?

Optimus-IDE-Collab is a platform for creating, managing, and using remote development environments (also known as Cloud Development Environments or CDEs). It leverages Terraform to define and provision these environments, which are referred to as "workspaces" within the project. The system is designed to be extensible, secure, and provide developers with a seamless remote development experience.

## Core Architecture

The heart of Optimus-IDE-Collab is a control plane that orchestrates the creation and management of workspaces. This control plane interacts with separate Provisioner processes over gRPC to handle workspace builds. The Provisioners consume workspace definitions and use Terraform to create the actual infrastructure.
...
Optimus-IDE-Collab's API architecture combines REST and gRPC approaches. The REST API is defined in `optimus-ide-collabd/optimus-ide-collabd.go` and uses Chi for HTTP routing. This provides the primary interface for the frontend and external integrations.
...
Optimus-IDE-Collab implements a secure networking layer based on Tailscale's Wireguard implementation. The `tailnet` package provides connectivity between workspace agents and clients through DERP (Designated Encrypted Relay for Packets) servers when direct connections aren't possible. This creates a secure overlay network allowing access to workspaces regardless of network topology, firewalls, or NAT configurations.

### Tailnet and DERP System

The networking system has three key components:

1. **Tailnet**: An overlay network implemented in the `tailnet` package that provides secure, end-to-end encrypted connections between clients, the Optimus-IDE-Collab server, and workspace agents.

2. **DERP Servers**: These relay traffic when direct connections aren't possible. Optimus-IDE-Collab provides several options:
   - A built-in DERP server that runs on the Optimus-IDE-Collab control plane
...
- Deployed as independent servers that authenticate with the Optimus-IDE-Collab control plane
- Relay connections for SSH, workspace apps, port forwarding, and web terminals
- Do not make direct database connections
- Managed through the `optimus-ide-collab wsproxy` commands
...
The implementation is primarily in the `optimus-ide-collabd/workspaceapps/` directory with components for URL generation, proxying connections, and managing application state.
...
Database interactions are carefully managed through migrations in `optimus-ide-collabd/database/migrations/` and queries in `optimus-ide-collabd/database/queries/`. All new queries require proper database authorization (dbauthz) implementation to ensure that only users with appropriate permissions can access specific resources.
...
2. **optimus-ide-collabdtest Package**: This package in `optimus-ide-collabd/optimus-ide-collabdtest/` provides utilities for creating test instances of the Optimus-IDE-Collab server, setting up test users and workspaces, and mocking external components.
...
4. **Enterprise Testing**: Enterprise features have dedicated test utilities in the `optimus-ide-collabdenttest` package.
...
Optimus-IDE-Collab emphasizes clear error handling, with specific patterns required:
...
If the development database gets into a bad state, it can be completely reset by removing the PostgreSQL data directory with `rm -rf .optimus-ide-collabv2/postgres`. This will destroy all data in the development database, requiring you to recreate any test users, templates, or workspaces after restarting the application.

Code generation for the database layer uses `optimus-ide-collabd/database/generate.sh`, and developers should refer to `sqlc.yaml` for the appropriate style and patterns to follow when creating new queries or tables.
