...
|--------------------------|--------------|--------------------------------------------------|
| API server               | `3000`       | `--port`, `OPTIMUS-IDE-COLLAB_DEV_PORT`                       |
| Frontend dev server      | `8080`       | `--web-port`, `OPTIMUS-IDE-COLLAB_DEV_WEB_PORT`               |
| Workspace proxy          | `3010`       | `--proxy-port`, `OPTIMUS-IDE-COLLAB_DEV_PROXY_PORT`           |
| Optimus-IDE-Collab Prometheus metrics | `2114`       | `--prometheus-port`, `OPTIMUS-IDE-COLLAB_DEV_PROMETHEUS_PORT` |
...
`3000`, `8080`, `3010`, and `2114` for Optimus-IDE-Collab Prometheus metrics. Set
`--port-offset` or `OPTIMUS-IDE-COLLAB_DEV_PORT_OFFSET=true` to opt in to a deterministic
per-worktree offset for API, frontend, workspace proxy, and Optimus-IDE-Collab Prometheus
metrics ports.
...
`OPTIMUS-IDE-COLLAB_DEV_PROMETHEUS_SERVER` is set, Docker is available, and the host is
...
|-----------------------------------|----------------------|------------------------------|
| Per-worktree port offset          | `--port-offset`      | `OPTIMUS-IDE-COLLAB_DEV_PORT_OFFSET`      |
| Access URL                        | `--access-url`       | `OPTIMUS-IDE-COLLAB_DEV_ACCESS_URL`       |
| Admin password                    | `--password`         | `OPTIMUS-IDE-COLLAB_DEV_ADMIN_PASSWORD`   |
| Starter template                  | `--starter-template` | `OPTIMUS-IDE-COLLAB_DEV_STARTER_TEMPLATE` |
| Roll back missing migrations      | `--db-rollback`      | `OPTIMUS-IDE-COLLAB_DEV_DB_ROLLBACK`      |
| Reset the development database    | `--db-reset`         | `OPTIMUS-IDE-COLLAB_DEV_DB_RESET`         |
| Accept changed migration tracking | `--db-continue`      | `OPTIMUS-IDE-COLLAB_DEV_DB_CONTINUE`      |

Extra `optimus-ide-collab server` flags can be passed after `--`. For example,
`./scripts/develop.sh -- --trace` passes `--trace` to the API server.

## Multi-worktree guidance

Each worktree gets its own `.optimus-ide-collabv2` directory because `scripts/develop.sh`
sets the global config directory to `<project-root>/.optimus-ide-collabv2`. This isolates
...
`--port-offset` or `OPTIMUS-IDE-COLLAB_DEV_PORT_OFFSET=true`. Enable the offset when running
...
effective API, web, proxy, and Optimus-IDE-Collab metrics ports with their offset status.

Use overrides when you need fixed ports or when two worktree paths hash to the
same offset. For example:

```sh
OPTIMUS-IDE-COLLAB_DEV_PORT=3100 \
OPTIMUS-IDE-COLLAB_DEV_WEB_PORT=8180 \
OPTIMUS-IDE-COLLAB_DEV_PROXY_PORT=3110 \
OPTIMUS-IDE-COLLAB_DEV_PROMETHEUS_PORT=2214 \
...
fixed to `optimus-ide-collab-prometheus`. Delve is fixed at `127.0.0.1:12345` when `--debug`
is used.

## Known collision risks

- Two worktree paths can hash to the same opt-in offset. If preflight reports a
  busy effective port, set the relevant `OPTIMUS-IDE-COLLAB_DEV_*` environment variables or
  flags for one worktree.
- The embedded Prometheus UI always uses port `9090`.
- The embedded Prometheus Docker container name is always `optimus-ide-collab-prometheus`.
...
- The develop script only checks the proxy port when `--use-proxy` is set, so
  ...
  is enabled.
- External databases configured through `OPTIMUS-IDE-COLLAB_PG_CONNECTION_URL` are shared if
...
`Optimus-IDE-Collab is now running in development mode`, the effective port list, and the API
and Web UI URLs.
...
3. To reset the built-in development database for the current worktree, rerun
   with `./scripts/develop.sh --db-reset` or remove `.optimus-ide-collabv2/postgres` after
   stopping the app.
4. To clear local Optimus-IDE-Collab session and generated state for the current worktree,
   remove the specific files under `.optimus-ide-collabv2` that are relevant to the failure.
5. To clean the embedded Prometheus container, stop the develop script first,
   then remove the `optimus-ide-collab-prometheus` container if it remains.
