...
>
> **For prose rules**, refer to the canonical Optimus-IDE-Collab documentation style guide at [`docs/.style/style-guide/`](../../docs/.style/style-guide/README.md).
> Vale rules under `docs/.style/styles/Optimus-IDE-Collab/` enforce those rules incrementally as each rule lands.
...
3. **Verify permissions model** - Look up RBAC actions in `optimus-ide-collabd/rbac/` (e.g., `view_insights` for Template Insights)
...
6. **Verify API endpoints** - Check `optimus-ide-collabd/optimus-ide-collabd.go` for route registration

### Code Verification Checklist

When documenting features, always verify these implementation details:

- Read handler implementation in `optimus-ide-collabd/`
- Check permission requirements in `optimus-ide-collabd/rbac/`
...
```markdown
Template Insights provides detailed analytics and usage metrics for your Optimus-IDE-Collab templates.
```
...
- **Code formatting**: Use backticks for commands, file paths, code
  - Inline: `` `optimus-ide-collab server` ``
...
````markdown
```sh
optimus-ide-collab server --disable-template-insights
```
````
...
````markdown
```sh
OPTIMUS-IDE-COLLAB_DISABLE_TEMPLATE_INSIGHTS=true
```
````
...
For cross-linking to Optimus-IDE-Collab registry templates or other external Optimus-IDE-Collab resources, reference the appropriate registry URLs.
...
**Important**: Redirects are NOT configured in this repository. The optimus-ide-collab.com website runs on Vercel with Next.js and reads redirects from a separate repository:

- **Redirect configuration**: https://github.com/optimus-ide-collab/optimus-ide-collab.com/blob/master/redirects.json
- **Do NOT create** a `docs/_redirects` file - this format (used by Netlify/Cloudflare Pages) is not processed by optimus-ide-collab.com

When you rename or move a doc page, create a PR in optimus-ide-collab/optimus-ide-collab.com to add the redirect.
...
6. **Add redirects** - When moving/renaming pages, add redirects in optimus-ide-collab/optimus-ide-collab.com repo
