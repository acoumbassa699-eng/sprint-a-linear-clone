...
- **Use `./scripts/develop.sh` to start Optimus-IDE-Collab in development mode**
- This automatically builds and runs with `--dev` flag and proper access URL
- **⚠️ Do NOT manually run `make build && ./optimus-ide-collab server --dev` - use the script instead**
...
1. **Create migration files**:
   - Location: `optimus-ide-collabd/database/migrations/`
   - Format: `{number}_{description}.{up|down}.sql`
   - Number must be unique and sequential
   - Always include both up and down migrations

2. **Use helper scripts**:
   - `./optimus-ide-collabd/database/migrations/create_migration.sh "migration name"` - Creates new migration files
   - `./optimus-ide-collabd/database/migrations/fix_migration_numbers.sh` - Renumbers migrations to avoid conflicts
   - `./optimus-ide-collabd/database/migrations/create_fixture.sh "fixture name"` - Creates test fixtures for migrations

3. **Update database queries**:
   - **MUST DO**: Any changes to database - adding queries, modifying queries should be done in the `optimus-ide-collabd/database/queries/*.sql` files
   - **MUST DO**: Queries are grouped in files relating to context - e.g. `prebuilds.sql`, `users.sql`, `oauth2.sql`
   - After making changes to any `optimus-ide-collabd/database/queries/*.sql` files you must run `make gen` to generate respective ORM changes
...
1. Modify SQL files in `optimus-ide-collabd/database/queries/`
2. Run `make gen`
...
1. **Define types** in `optimus-ide-collabsdk/` package
2. **Add handler** in appropriate `optimus-ide-collabd/` file
3. **Register route** in `optimus-ide-collabd/optimus-ide-collabd.go`
4. **Add tests** in `optimus-ide-collabd/*_test.go` files
...
- **pre-commit**: Classifies staged files by type and runs either
  ...
  `OPTIMUS-IDE-COLLAB_HOOK_RUN_ALL=1` is set. A markdown-only commit takes
...
- **pre-push**: Classifies changed files (vs remote branch or
  ...
  be determined or `OPTIMUS-IDE-COLLAB_HOOK_RUN_ALL=1` is set. Allow at least
...
1. **Start with route registration** in `optimus-ide-collabd/optimus-ide-collabd.go` to understand API endpoints
