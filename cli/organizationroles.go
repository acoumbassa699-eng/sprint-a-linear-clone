...
func (r *RootCmd) showOrganizationRoles(orgContext *OrganizationContext) *serpent.Command {
	formatter := cliui.NewOutputFormatter(
		cliui.ChangeFormatterData(
			...
			func(data any) (any, error) {
				inputs, ok := data.([]optimus-ide-collabsdk.AssignableRoles)
				if !ok {
					return nil, xerrors.Errorf("expected []optimus-ide-collabsdk.AssignableRoles got %T", data)
				}
	...
	cmd := &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			if len(inv.Args) > 0 {
				// filter roles
				filtered := make([]optimus-ide-collabsdk.AssignableRoles, 0)
...
func (r *RootCmd) createOrganizationRole(orgContext *OrganizationContext) *serpent.Command {
	formatter := cliui.NewOutputFormatter(
		cliui.ChangeFormatterData(
			cliui.TableFormat([]roleTableRow{}, []string{"name", "display name", "site permissions", "organization permissions", "user permissions"}),
			func(data any) (any, error) {
				typed, _ := data.(optimus-ide-collabsdk.Role)
	...
	cmd := &serpent.Command{
		...
		Long: FormatExamples(
			Example{
				...
				Command:     "optimus-ide-collab organization -O <organization_name> roles create --stdin < role.json",
			},
		),
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			var customRole optimus-ide-collabsdk.Role
			if jsonInput {
				...
				if len(inv.Args) == 0 {
					return xerrors.Errorf("missing role name argument, usage: \"optimus-ide-collab organizations roles create <role_name>\"")
				}
			...
			var updated optimus-ide-collabsdk.Role
			if dryRun {
...
func (r *RootCmd) updateOrganizationRole(orgContext *OrganizationContext) *serpent.Command {
	...
	cmd := &serpent.Command{
		...
		Long: FormatExamples(
			Example{
				Description: "Run with an input.json file",
				Command:     "optimus-ide-collab roles update --stdin < role.json",
			},
		),
		Options: []serpent.Option{
			cliui.SkipPromptOption(),
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			if jsonInput {
				...
				if len(inv.Args) == 0 {
					return xerrors.Errorf("missing role name argument, usage: \"optimus-ide-collab organizations roles edit <role_name>\"")
				}
...
func interactiveOrgRoleEdit(inv *serpent.Invocation, orgID uuid.UUID, updateRole *optimus-ide-collabsdk.Role) (*optimus-ide-collabsdk.Role, error) {
	var originalRole optimus-ide-collabsdk.Role
	if updateRole == nil {
		originalRole = optimus-ide-collabsdk.Role{
			Name:           inv.Args[0],
	...
	role := &originalRole
	allowedResources := []optimus-ide-collabsdk.RBACResource{
		optimus-ide-collabsdk.ResourceTemplate,
		optimus-ide-collabsdk.ResourceWorkspace,
		optimus-ide-collabsdk.ResourceUser,
		optimus-ide-collabsdk.ResourceGroup,
	}
	...
	for {
		...
		default:
			...
			actions, err := cliui.MultiSelect(inv, cliui.MultiSelectOptions{
				...
				Options:  slice.ToStrings(optimus-ide-collabsdk.RBACResourceActions[optimus-ide-collabsdk.RBACResource(resource)]),
...
func applyOrgResourceActions(role *optimus-ide-collabsdk.Role, resource string, actions []string) {
	if role.OrganizationPermissions == nil {
		role.OrganizationPermissions = make([]optimus-ide-collabsdk.Permission, 0)
	}

	// Construct new site perms with only new perms for the resource
	keep := make([]optimus-ide-collabsdk.Permission, 0)
	...
	for _, action := range actions {
		keep = append(keep, optimus-ide-collabsdk.Permission{
			Negate:       false,
			ResourceType: optimus-ide-collabsdk.RBACResource(resource),
			Action:       optimus-ide-collabsdk.RBACAction(action),
		})
	}
...
func defaultActions(role *optimus-ide-collabsdk.Role, resource string) []string {
	if role.OrganizationPermissions == nil {
		role.OrganizationPermissions = []optimus-ide-collabsdk.Permission{}
	}
...
func permissionPreviews(role *optimus-ide-collabsdk.Role, resources []optimus-ide-collabsdk.RBACResource) []string {
...
func permissionPreview(role *optimus-ide-collabsdk.Role, resource optimus-ide-collabsdk.RBACResource) string {
	if role.OrganizationPermissions == nil {
		role.OrganizationPermissions = []optimus-ide-collabsdk.Permission{}
	}
...
func roleToTableView(role optimus-ide-collabsdk.Role) roleTableRow {
	return roleTableRow{
		Name:                    role.Name,
...
func existingRole(newRoleName string, existingRoles []optimus-ide-collabsdk.AssignableRoles) *optimus-ide-collabsdk.AssignableRoles {
