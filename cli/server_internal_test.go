...
func TestRedirectHTTPToHTTPSDeprecation(t *testing.T) {
	...
	testcases := []struct {
		...
		{
			name:     "OPTIMUS-IDE-COLLAB_TLS_REDIRECT_HTTP=true",
			environ:  serpent.Environ{{Name: "OPTIMUS-IDE-COLLAB_TLS_REDIRECT_HTTP", Value: "true"}},
			flags:    []string{},
			expected: true,
		},
		{
			name:     "OPTIMUS-IDE-COLLAB_TLS_REDIRECT_HTTP_TO_HTTPS=true",
			environ:  serpent.Environ{{Name: "OPTIMUS-IDE-COLLAB_TLS_REDIRECT_HTTP_TO_HTTPS", Value: "true"}},
			flags:    []string{},
			expected: true,
		},
		{
			name:     "OPTIMUS-IDE-COLLAB_TLS_REDIRECT_HTTP=false",
			environ:  serpent.Environ{{Name: "OPTIMUS-IDE-COLLAB_TLS_REDIRECT_HTTP", Value: "false"}},
			flags:    []string{},
			expected: false,
		},
		{
			name:     "OPTIMUS-IDE-COLLAB_TLS_REDIRECT_HTTP_TO_HTTPS=false",
			environ:  serpent.Environ{{Name: "OPTIMUS-IDE-COLLAB_TLS_REDIRECT_HTTP_TO_HTTPS", Value: "false"}},
			flags:    []string{},
			expected: false,
		},
		{
...
func TestIsReplicaRelayRequest(t *testing.T) {
	...
	t.Run("WithHeader", func(t *testing.T) {
		...
		r.Header.Set("X-Optimus-IDE-Collab-Relay-Source-Replica", "some-uuid")
	...
	t.Run("EmptyHeader", func(t *testing.T) {
		t.Parallel()
		r, _ := http.NewRequestWithContext(context.Background(), "GET", "/api/experimental/chats/abc/stream", nil)
		r.Header.Set("X-Optimus-IDE-Collab-Relay-Source-Replica", "")
		require.False(t, isReplicaRelayRequest(r))
	})
}
...
func TestEscapePostgresURLUserInfo(t *testing.T) {
	...
	testcases := []struct {
		...
		{
			input:  "postgres://optimus-ide-collab:optimus-ide-collab@localhost:5432/optimus-ide-collab",
			output: "postgres://optimus-ide-collab:optimus-ide-collab@localhost:5432/optimus-ide-collab",
			err:    nil,
		},
		{
			input:  "postgres://optimus-ide-collab:co{der@localhost:5432/optimus-ide-collab",
			output: "postgres://optimus-ide-collab:co%7Bder@localhost:5432/optimus-ide-collab",
			err:    nil,
		},
		{
			input:  "postgres://optimus-ide-collab:co:der@localhost:5432/optimus-ide-collab",
			output: "postgres://optimus-ide-collab:co:der@localhost:5432/optimus-ide-collab",
			err:    nil,
		},
		{
			input:  "postgres://optimus-ide-collab:co der@localhost:5432/optimus-ide-collab",
			output: "postgres://optimus-ide-collab:co%20der@localhost:5432/optimus-ide-collab",
			err:    nil,
		},
		{
			input:  "postgres://local host:5432/optimus-ide-collab",
			output: "",
			err:    xerrors.New("parse postgres url: parse \"postgres://local host:5432/optimus-ide-collab\": invalid character \" \" in host name"),
		},
		{
			input:  "postgres://optimus-ide-collab:co?der@localhost:5432/optimus-ide-collab",
			output: "postgres://optimus-ide-collab:co%3Fder@localhost:5432/optimus-ide-collab",
			err:    nil,
		},
		{
			input:  "postgres://optimus-ide-collab:co#der@localhost:5432/optimus-ide-collab",
			output: "postgres://optimus-ide-collab:co%23der@localhost:5432/optimus-ide-collab",
			err:    nil,
		},
	}
	for _, tc := range testcases {
