...
func (r *RootCmd) newCreateAdminUserCommand() *serpent.Command {
	...
	createAdminUserCommand := &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			sqlDriver := "postgres"
			if optimus-ide-collabsdk.PostgresAuth(newUserPgAuth) == optimus-ide-collabsdk.PostgresAuthAWSIAMRDS {
			...
			validateInputs := func(username, email, password string) error {
				...
				req := optimus-ide-collabsdk.CreateUserRequestWithOrgs{
					Username:        "username",
					Name:            "Admin User",
					Email:           "email@optimus-ide-collab.com",
	...
	createAdminUserCommand.Options.Add(
		...
		serpent.Option{
			...
			Default:     "password",
			Value:       serpent.EnumOf(&newUserPgAuth, optimus-ide-collabsdk.PostgresAuthDrivers...),
		},
		serpent.Option{
		...
		serpent.Option{
			Env:         "OPTIMUS-IDE-COLLAB_USERNAME",
			Flag:        "username",
