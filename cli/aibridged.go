...
import (
	...
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/aibridge/config"
...
//
// Reloads fetch the provider set from optimus-ide-collabd over the in-memory DRPC
...
// negligible.
func newAIBridgeDaemon(optimus-ide-collabAPI *optimus-ide-collabd.API, cfg optimus-ide-collabsdk.AIBridgeConfig, reg prometheus.Registerer, metrics *aibridge.Metrics) (*aibridged.Server, func(), error) {
	ctx := context.Background()
	optimus-ide-collabAPI.Logger.Debug(ctx, "starting in-memory aibridge daemon")

	logger := optimus-ide-collabAPI.Logger.Named("ai-gateway")
	...
	srv, err := aibridged.New(ctx, pool, func(dialCtx context.Context) (aibridged.DRPCClient, error) {
		return optimus-ide-collabAPI.CreateInMemoryAIBridgeServer(dialCtx)
	}, logger, tracer)
	if err != nil {
	...
	unsubscribe, err := aibridged.SubscribeProviderReload(ctx, optimus-ide-collabAPI.Pubsub, reloader, logger.Named("provider-reload"))
	if err != nil {
...
// live provider set from optimus-ide-collabd over a DRPC client and forwarding it to the
...
func (r *poolRPCReloader) Reload(ctx context.Context) error {
	...
	// r.client blocks until the daemon connects to optimus-ide-collabd or ctx is canceled.
...
func BuildProvidersFromProto(ctx context.Context, protoProviders []*proto.AIProvider, cfg optimus-ide-collabsdk.AIBridgeConfig, logger slog.Logger, metrics *aibridge.Metrics) ([]aibridge.Provider, []aibridged.ProviderOutcome) {
...
func protoToProviderSpec(pp *proto.AIProvider) aiProviderSpec {
	...
	if b := pp.GetBedrock(); b != nil {
		bedrock := optimus-ide-collabsdk.NewAIProviderBedrockSettings(
			b.GetRegion(),
			b.GetAccessKey(),
		...
		bedrock.Protocol = optimus-ide-collabsdk.AIProviderBedrockProtocol(b.GetProtocol())
...
func buildProvider(ctx context.Context, spec aiProviderSpec, cfg optimus-ide-collabsdk.AIBridgeConfig, metrics *aibridge.Metrics) (aibridge.Provider, error) {
	if !spec.Enabled {
...
// [optimus-ide-collabsdk.AIProviderBedrockSettings.IsConfigured].
func bedrockConfig(baseURL string, bedrock *optimus-ide-collabsdk.AIProviderBedrockSettings) *aibridge.AWSBedrockConfig {
	if bedrock == nil {
		return nil
	}
...
func circuitBreakerConfig(cfg optimus-ide-collabsdk.AIBridgeConfig) *config.CircuitBreaker {
