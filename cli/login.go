...
func (r *RootCmd) loginWithPassword(
	...
	resp, err := client.LoginWithPassword(inv.Context(), optimus-ide-collabsdk.LoginWithPasswordRequest{
		Email:    email,
		Password: password,
	})
	if err != nil {
	...
	_, _ = fmt.Fprintf(
		inv.Stdout,
		"Welcome to Optimus-IDE-Collab, %s! You're authenticated.",
...
func (r *RootCmd) login() *serpent.Command {
	const firstUserTrialEnv = "OPTIMUS-IDE-COLLAB_FIRST_USER_TRIAL"
	...
	cmd := &serpent.Command{
		Use:   "login [<url>]",
		Short: "Authenticate with Optimus-IDE-Collab deployment",
		Long: "By default, the session token is stored in the operating system keyring on " +
			...
			"or OPTIMUS-IDE-COLLAB_USE_KEYRING environment variable to change the storage mechanism.",
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			if err != nil {
				return xerrors.Errorf("Failed to check server %q for first user, is the URL correct and is optimus-ide-collab accessible from your browser? Error - has initial user: %w", serverURL.String(), err)
			}
			...
			// nolint: nestif
			if !hasFirstUser {
				_, _ = fmt.Fprint(inv.Stdout, Caret+"Your Optimus-IDE-Collab deployment hasn't been set up!\n")
				...
				var trialInfo optimus-ide-collabsdk.CreateFirstUserTrialInfo
				if trial {
				...
				_, _ = fmt.Fprintf(
					...
					pretty.Sprint(cliui.DefaultStyles.Code, "optimus-ide-collab templates init"),
				)
				return nil
			}
			...
			// If OPTIMUS-IDE-COLLAB_SESSION_TOKEN is set in the environment, abort
			...
			client.SetSessionToken(sessionToken)
			resp, err := client.User(ctx, optimus-ide-collabsdk.Me)
			if err != nil {
				return xerrors.Errorf("get user: %w", err)
			}
			...
			_, _ = fmt.Fprintf(inv.Stdout, Caret+"Welcome to Optimus-IDE-Collab, %s! You're authenticated.\n", pretty.Sprint(cliui.DefaultStyles.Keyword, resp.Username))
			return nil
		},
	}
	cmd.Options = serpent.OptionSet{
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_FIRST_USER_EMAIL",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_FIRST_USER_USERNAME",
		...
		},
		{
			...
			Description: "Specifies whether a trial license should be provisioned for the Optimus-IDE-Collab deployment or not.",
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			if err != nil {
				if xerrors.Is(err, os.ErrNotExist) {
					return xerrors.New("no session token found - run 'optimus-ide-collab login' first")
				}
				if xerrors.Is(err, sessionstore.ErrNotImplemented) {
					return errKeyringNotSupported
				}
...
// isWSL determines if optimus-ide-collab-cli is running within Windows Subsystem for Linux
...
func promptCountry(inv *serpent.Invocation) (string, error) {
	options := make([]string, len(optimus-ide-collabsdk.Countries))
	for i, country := range optimus-ide-collabsdk.Countries {
