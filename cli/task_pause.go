	...
	cmd := &serpent.Command{
		...
		Long: FormatExamples(
			Example{
				...
				Command:     "optimus-ide-collab task pause my-task",
			},
			Example{
				Description: "Pause another user's task",
				Command:     "optimus-ide-collab task pause alice/my-task",
			},
			Example{
				Description: "Pause a task without confirmation",
				Command:     "optimus-ide-collab task pause my-task --yes",
			},
		),
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			if task.Status == optimus-ide-collabsdk.TaskStatusPaused {
