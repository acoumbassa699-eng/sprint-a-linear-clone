...
func (r *RootCmd) provisionerJobsList() *serpent.Command {
	type provisionerJobRow struct {
		optimus-ide-collabsdk.ProvisionerJob `table:"provisioner_job,recursive_inline,nosort"`
	...
	cmd := &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			jobs, err := client.OrganizationProvisionerJobs(ctx, org.ID, &optimus-ide-collabsdk.OrganizationProvisionerJobsOptions{
				Status:    slice.StringEnums[optimus-ide-collabsdk.ProvisionerJobStatus](status),
				Limit:     int(limit),
				Initiator: initiator,
			})
			if err != nil {
	...
	cmd.Options = append(cmd.Options, []serpent.Option{
		{
			Flag:          "status",
			FlagShorthand: "s",
			Env:           "OPTIMUS-IDE-COLLAB_PROVISIONER_JOB_LIST_STATUS",
			Description:   "Filter by job status.",
			Value:         serpent.EnumArrayOf(&status, slice.ToStrings(optimus-ide-collabsdk.ProvisionerJobStatusEnums())...),
		},
		{
			Flag:          "limit",
			FlagShorthand: "l",
			Env:           "OPTIMUS-IDE-COLLAB_PROVISIONER_JOB_LIST_LIMIT",
		...
		},
		{
			Flag:          "initiator",
			FlagShorthand: "i",
			Env:           "OPTIMUS-IDE-COLLAB_PROVISIONER_JOB_LIST_INITIATOR",
...
func (r *RootCmd) provisionerJobsCancel() *serpent.Command {
	...
	cmd := &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			switch job.Type {
			case optimus-ide-collabsdk.ProvisionerJobTypeTemplateVersionDryRun:
			...
			case optimus-ide-collabsdk.ProvisionerJobTypeTemplateVersionImport:
			...
			case optimus-ide-collabsdk.ProvisionerJobTypeWorkspaceBuild:
				_, _ = fmt.Fprintf(inv.Stdout, "Canceling workspace build job %s...\n", job.ID)
				err = client.CancelWorkspaceBuild(ctx, ptr.NilToEmpty(job.Input.WorkspaceBuildID), optimus-ide-collabsdk.CancelWorkspaceBuildParams{})
			}
			if err != nil {
