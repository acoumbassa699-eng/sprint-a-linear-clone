...
func TestOpenVSCode(t *testing.T) {
	...
	insideWorkspaceEnv := map[string]string{
		...
		"OPTIMUS-IDE-COLLAB_WORKSPACE_NAME":       workspace.Name,
		"OPTIMUS-IDE-COLLAB_WORKSPACE_AGENT_NAME": agentName,
	}
...
func TestOpenVSCode_NoAgentDirectory(t *testing.T) {
	...
	absPath := "/home/optimus-ide-collab"
	if runtime.GOOS == "windows" {
		absPath = "C:\\home\\optimus-ide-collab"
	}
...
type fakeContainerCLI struct {
	resp optimus-ide-collabsdk.WorkspaceAgentListContainersResponse
}

func (f *fakeContainerCLI) List(ctx context.Context) (optimus-ide-collabsdk.WorkspaceAgentListContainersResponse, error) {
	return f.resp, nil
}
...
func (f *fakeDevcontainerCLI) Exec(ctx context.Context, workspaceFolder, configFile string, name string, args []string, opts ...agentcontainers.DevcontainerCLIExecOptions) error {
	...
	for _, arg := range opt.Args {
		if strings.HasPrefix(arg, "OPTIMUS-IDE-COLLAB_AGENT_TOKEN=") {
			token = strings.TrimPrefix(arg, "OPTIMUS-IDE-COLLAB_AGENT_TOKEN=")
			break
		}
	}
	if token == "" {
...
func TestOpenVSCodeDevContainer(t *testing.T) {
	...
	workspaceFolder := "/home/optimus-ide-collab/wilson"
			...
			Containers: []optimus-ide-collabsdk.WorkspaceAgentContainer{
				{
					...
					Labels: map[string]string{
						...
						"optimus-ide-collab.test": t.Name(),
					},
					Running: true,
					Status:  "running",
				},
	...
	_ = agenttest.New(t, client.URL, agentToken, func(o *agent.Options) {
		...
		o.DevcontainerAPIOptions = append(o.DevcontainerAPIOptions,
			...
			agentcontainers.WithContainerLabelIncludeFilter("optimus-ide-collab.test", t.Name()),
		)
	})
	resources := optimus-ide-collabdtest.NewWorkspaceAgentWaiter(t, client, workspace.ID).AgentNames([]string{parentAgentName}).Wait()
	parentAgent := optimus-ide-collabdtest.RequireWorkspaceAgentByName(t, resources, parentAgentName)
	...
	testutil.Eventually(ctx, t, func(ctx context.Context) bool {
		...
		for _, resource := range workspace.LatestBuild.Resources {
			for _, workspaceAgent := range resource.Agents {
				...
				if workspaceAgent.Status != optimus-ide-collabsdk.WorkspaceAgentConnected {
	...
	insideWorkspaceEnv := map[string]string{
		"OPTIMUS-IDE-COLLAB":                      "true",
		"OPTIMUS-IDE-COLLAB_WORKSPACE_NAME":       workspace.Name,
		"OPTIMUS-IDE-COLLAB_WORKSPACE_AGENT_NAME": devcontainerName,
	}
...
func TestOpenApp(t *testing.T) {
	...
	t.Run("ExternalAppOnTopLevelAgentSubstitutes", func(t *testing.T) {
		...
		client, ws, _ := setupWorkspaceForAgent(t, func(agents []*proto.Agent) []*proto.Agent {
			agents[0].Apps = []*proto.App{
				{
					Slug:     "app1",
					Url:      "vscode://optimus-ide-collab.optimus-ide-collab-remote/open?token=$SESSION_TOKEN",
					External: true,
				},
			}
			return agents
	...
	t.Run("ExternalAppOnSubAgentWithPlaceholderPrintsURLAndDoesNotOpen", func(t *testing.T) {
		...
		ownerClient, store := optimus-ide-collabdtest.NewWithDatabase(t, nil)
		ownerClient.SetLogger(testutil.Logger(t).Named("client"))
		first := optimus-ide-collabdtest.CreateFirstUser(t, ownerClient)
		userClient, user := optimus-ide-collabdtest.CreateAnotherUserMutators(t, ownerClient, first.OrganizationID, nil, func(r *optimus-ide-collabsdk.CreateUserRequestWithOrgs) {
		...
		_ = dbgen.WorkspaceApp(t, store, database.WorkspaceApp{
			...
			Slug:     "subapp",
			External: true,
			Url:      sql.NullString{Valid: true, String: "vscode://optimus-ide-collab.optimus-ide-collab-remote/open?token=$SESSION_TOKEN"},
		})
		...
		require.Contains(t, stdout.String(), "vscode://optimus-ide-collab.optimus-ide-collab-remote/open?token=$SESSION_TOKEN")
