...
func DefaultCases() []CommandHelpCase {
	return []CommandHelpCase{
		{
			Name: "optimus-ide-collab --help",
			Cmd:  []string{"--help"},
		},
		{
			Name: "optimus-ide-collab server --help",
...
ExtractCommandPathsLoop:
	for _, cp := range extractVisibleCommandPaths(nil, root.Children) {
		name := fmt.Sprintf("optimus-ide-collab %s --help", strings.Join(cp, " "))
		//nolint:gocritic
		cmd := append(cp, "--help")
	...
	for _, tt := range cases {
		t.Run(tt.Name, func(t *testing.T) {
			...
			inv.Stderr = &outBuf
			inv.Stdout = &outBuf
			inv.Environ.Set("OPTIMUS-IDE-COLLAB_URL", rootClient.URL.String())
			inv.Environ.Set("OPTIMUS-IDE-COLLAB_SESSION_TOKEN", rootClient.SessionToken())
			inv.Environ.Set("OPTIMUS-IDE-COLLAB_CACHE_DIRECTORY", "~/.cache")
...
func normalizeGoldenFile(t *testing.T, byt []byte) []byte {
	...
	byt = bytes.ReplaceAll(byt, []byte(configDir), []byte("~/.config/optimus-ide-collabv2"))

	byt = bytes.ReplaceAll(byt, []byte(optimus-ide-collabsdk.DefaultCacheDir()), []byte("[cache dir]"))
	...
	for _, r := range []struct {
		...
		{"\r\n", "\n"},
		{`~\.cache\optimus-ide-collab`, "~/.cache/optimus-ide-collab"},
...
func prepareTestData(t *testing.T) (*optimus-ide-collabsdk.Client, map[string]string) {
	t.Helper()
	...
	rootClient := optimus-ide-collabdtest.New(t, &optimus-ide-collabdtest.Options{
		Database:                 db,
		Pubsub:                   pubsub,
		IncludeProvisionerDaemon: true,
	})
	firstUser := optimus-ide-collabdtest.CreateFirstUser(t, rootClient)
	secondUser, err := rootClient.CreateUserWithOrgs(ctx, optimus-ide-collabsdk.CreateUserRequestWithOrgs{
		Email:           "testuser2@optimus-ide-collab.com",
		Username:        "testuser2",
		Password:        optimus-ide-collabdtest.FirstUserParams.Password,
		OrganizationIDs: []uuid.UUID{firstUser.OrganizationID},
	})
	require.NoError(t, err)
	version := optimus-ide-collabdtest.CreateTemplateVersion(t, rootClient, firstUser.OrganizationID, nil)
	version = optimus-ide-collabdtest.AwaitTemplateVersionJobCompleted(t, rootClient, version.ID)
	template := optimus-ide-collabdtest.CreateTemplate(t, rootClient, firstUser.OrganizationID, version.ID, func(req *optimus-ide-collabsdk.CreateTemplateRequest) {
		req.Name = "test-template"
	})
	workspace := optimus-ide-collabdtest.CreateWorkspace(t, rootClient, template.ID, func(req *optimus-ide-collabsdk.CreateWorkspaceRequest) {
		req.Name = "test-workspace"
	})
	workspaceBuild := optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, rootClient, workspace.LatestBuild.ID)
