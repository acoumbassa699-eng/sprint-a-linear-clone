...
func Test_sshConfigSplitOnOptimus-IDE-CollabSection(t *testing.T) {
	t.Parallel()
	...
	for _, tc := range testCases {
		t.Run(tc.Name, func(t *testing.T) {
			...
			before, section, after, err := sshConfigSplitOnOptimus-IDE-CollabSection([]byte(tc.Input))
			if tc.Err {
				require.Error(t, err)
				return
			}
...
func Test_sshConfigProxyCommandEscape(t *testing.T) {
	...
	tests := []struct {
		...
		{"windows path", `C:\Program Files\Optimus-IDE-Collab\bin\optimus-ide-collab.exe`, false},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.NoError(t, err)
			bin := filepath.Join(dir, "optimus-ide-collab")
...
func Test_sshConfigMatchExecEscape(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			dir := filepath.Join(t.TempDir(), tt.path)
			bin := filepath.Join(dir, "optimus-ide-collab.bat") // Windows will treat it as batch, Linux doesn't care
...
func Test_sshConfigExecEscapeSeparatorForce(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			path:           `C:\Program Files\Optimus-IDE-Collab\bin\optimus-ide-collab.exe`,
			expWindowsPath: `"C:\Program Files\Optimus-IDE-Collab\bin\optimus-ide-collab.exe"`,
			expOtherPath:   `"C:\Program Files\Optimus-IDE-Collab\bin\optimus-ide-collab.exe"`,
			forceUnix:      false,
			wantErr:        false,
		},
		{
			name:           "windows_keep_forward_slashes",
			path:           `C:\ProgramFiles\Optimus-IDE-Collab\bin\optimus-ide-collab.exe`,
			expWindowsPath: `C:\ProgramFiles\Optimus-IDE-Collab\bin\optimus-ide-collab.exe`,
			expOtherPath:   `C:\ProgramFiles\Optimus-IDE-Collab\bin\optimus-ide-collab.exe`,
			forceUnix:      false,
			wantErr:        false,
		},
		{
...
func Test_mergeSSHOptions_RejectsUnsafeServerConfig(t *testing.T) {
	...
	testCases := []struct {
		name    string
		optimus-ide-collabd  optimus-ide-collabsdk.SSHConfigResponse
		wantErr string
	}{
		{
			name: "HostnameSuffix",
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := mergeSSHOptions(sshConfigOptions{}, tt.optimus-ide-collabd, t.TempDir(), "/tmp/optimus-ide-collab")
...
func Test_mergeSSHOptions_UserOptionsOverrideServerConfig(t *testing.T) {
	...
	got, err := mergeSSHOptions(user, optimus-ide-collabsdk.SSHConfigResponse{
		HostnamePrefix: "optimus-ide-collab.",
		HostnameSuffix: "optimus-ide-collab",
	}, t.TempDir(), "/tmp/optimus-ide-collab")
	require.NoError(t, err)
...
func Test_mergeSSHOptions_ExplicitEmptyNotOverridden(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			wantUserHostPrefix: "",
			wantHostnameSuffix: "optimus-ide-collab",
		},
		{
			name:               "SuffixExplicitlyEmpty",
			userHostPrefixSet:  false,
			hostnameSuffixSet:  true,
			wantUserHostPrefix: "optimus-ide-collab.",
			wantHostnameSuffix: "",
		},
		{
...
func Test_mergeSSHOptions_AllowsSafeServerConfig(t *testing.T) {
	...
	got, err := mergeSSHOptions(sshConfigOptions{}, optimus-ide-collabsdk.SSHConfigResponse{
		HostnamePrefix: "optimus-ide-collab.",
		HostnameSuffix: "optimus-ide-collab",
		SSHConfigOptions: map[string]string{
			...
			"UserKnownHostsFile": "/tmp/optimus-ide-collab_known_hosts",
		},
	}, t.TempDir(), "/tmp/optimus-ide-collab")
	require.NoError(t, err)
	require.Equal(t, "optimus-ide-collab.", got.userHostPrefix)
	require.Equal(t, "optimus-ide-collab", got.hostnameSuffix)
...
func TestSSHConfigOptions_writeToBuffer(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			opts: sshConfigOptions{
				optimus-ide-collabBinaryPath:  "/usr/bin/optimus-ide-collab",
				globalConfigPath: "/tmp/optimus-ide-collab",
				hostnameSuffix:   "optimus-ide-collab",
				waitEnum:         "auto",
			},
			want:    []string{"Host *.optimus-ide-collab\n", "ProxyCommand", "--hostname-suffix optimus-ide-collab %h"},
		...
		},
		{
			...
			opts: sshConfigOptions{
				optimus-ide-collabBinaryPath:  "/usr/bin/optimus-ide-collab",
				globalConfigPath: "/tmp/optimus-ide-collab",
				userHostPrefix:   "optimus-ide-collab.",
				waitEnum:         "auto",
			},
			want:    []string{"Host optimus-ide-collab.*\n", "ProxyCommand", "--ssh-host-prefix optimus-ide-collab. %h"},
			notWant: []string{"Host optimus-ide-collab.workspace"},
		},
		{
			...
			want: []string{
				"Host workspace1.optimus-ide-collab\n",
				"Host workspace2.optimus-ide-collab\n",
				"Match host workspace1.optimus-ide-collab !exec",
				"Match host workspace2.optimus-ide-collab !exec",
				"--hostname-suffix optimus-ide-collab %h",
			},
			notWant: []string{"Host *.optimus-ide-collab", "Match host *.optimus-ide-collab"},
		},
		{
		...
		},
		{
			...
			want: []string{
				"Host optimus-ide-collab.workspace1\n",
				"Host optimus-ide-collab.workspace2\n",
				"--ssh-host-prefix optimus-ide-collab. %h",
			},
			notWant: []string{"Host optimus-ide-collab.*"},
		},
		{
			...
			want:    []string{"Host workspace1.optimus-ide-collab\n"},
			notWant: []string{"ProxyCommand", "Match host", "Host *.optimus-ide-collab"},
		},
		{
			...
			want:    []string{"Host optimus-ide-collab.workspace1\n"},
			notWant: []string{"ProxyCommand", "Host optimus-ide-collab.*"},
		},
		{
		...
		},
		{
			...
			notWant: []string{"Host optimus-ide-collab.*", "Host *.testy"},
		},
	}
