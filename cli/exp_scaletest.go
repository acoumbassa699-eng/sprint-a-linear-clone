...
import (
	...
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/scaletest/agentconn"
	...
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/scaletest/createworkspaces"
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/scaletest/dashboard"
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/scaletest/harness"
	...
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/scaletest/reconnectingpty"
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/scaletest/workspacebuild"
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/scaletest/workspacetraffic"
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/scaletest/workspaceupdates"
	"github.com/optimus-ide-collab/serpent"
)

const scaletestTracerName = "optimus-ide-collab_scaletest"

var BypassHeader = map[string][]string{optimus-ide-collabsdk.BypassRatelimitHeader: {"true"}}
...
func (s *scaletestTracingFlags) attach(opts *serpent.OptionSet) {
	*opts = append(
		*opts,
		serpent.Option{
			Flag:        "trace",
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_TRACE",
		...
		serpent.Option{
			Flag:        "trace-optimus-ide-collab",
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_TRACE_OPTIMUS-IDE-COLLAB",
			Description: "Whether opentelemetry traces are sent to Optimus-IDE-Collab. We recommend keeping this disabled unless we advise you to enable it.",
			Value:       serpent.BoolOf(&s.traceOptimus-IDE-Collab),
		},
		serpent.Option{
			Flag:        "trace-honeycomb-api-key",
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_TRACE_HONEYCOMB_API_KEY",
		...
		serpent.Option{
			...
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_TRACE_PROPAGATE",
			Description: "Enables trace propagation to the Optimus-IDE-Collab backend, which will be used to correlate server-side spans with client-side spans. Only enable this if the server is configured with the exact same tracing configuration as the client.",
...
func (s *scaletestTracingFlags) provider(ctx context.Context) (trace.TracerProvider, func(context.Context) error, bool, error) {
	shouldTrace := s.traceEnable || s.traceOptimus-IDE-Collab || s.traceHoneycombAPIKey != ""
	if !shouldTrace {
...
func (c *concurrencyFlags) attach(opts *serpent.OptionSet) {
	concurrencyLong, concurrencyEnv, concurrencyDescription := "concurrency", "OPTIMUS-IDE-COLLAB_SCALETEST_CONCURRENCY", "Number of concurrent jobs to run. 0 means unlimited."
	if c.cleanup {
		concurrencyLong, concurrencyEnv, concurrencyDescription = "cleanup-"+concurrencyLong, "OPTIMUS-IDE-COLLAB_SCALETEST_CLEANUP_CONCURRENCY", strings.ReplaceAll(concurrencyDescription, "jobs", "cleanup jobs")
	}
...
func (t *timeoutFlags) attach(opts *serpent.OptionSet) {
	timeoutLong, timeoutEnv, timeoutDescription := "timeout", "OPTIMUS-IDE-COLLAB_SCALETEST_TIMEOUT", "Timeout for the entire test run. 0 means unlimited."
	jobTimeoutLong, jobTimeoutEnv, jobTimeoutDescription := "job-timeout", "OPTIMUS-IDE-COLLAB_SCALETEST_JOB_TIMEOUT", "Timeout per job. Jobs may take longer to complete under higher concurrency limits."
	if t.cleanup {
		timeoutLong, timeoutEnv, timeoutDescription = "cleanup-"+timeoutLong, "OPTIMUS-IDE-COLLAB_SCALETEST_CLEANUP_TIMEOUT", strings.ReplaceAll(timeoutDescription, "test", "cleanup")
		jobTimeoutLong, jobTimeoutEnv, jobTimeoutDescription = "cleanup-"+jobTimeoutLong, "OPTIMUS-IDE-COLLAB_SCALETEST_CLEANUP_JOB_TIMEOUT", strings.ReplaceAll(jobTimeoutDescription, "jobs", "cleanup jobs")
	}
...
func (o *scaleTestOutput) write(res harness.Results, stdout io.Writer) error {
	...
	case scaleTestOutputFormatJSON:
		err := json.NewEnoptimus-ide-collab(w).Encode(res)
		if err != nil {
...
func (s *scaletestOutputFlags) attach(opts *serpent.OptionSet) {
	*opts = append(*opts, serpent.Option{
		Flag:        "output",
		Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_OUTPUTS",
...
func (s *scaletestPrometheusFlags) attach(opts *serpent.OptionSet) {
	*opts = append(*opts,
		serpent.Option{
			...
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_PROMETHEUS_ADDRESS",
		...
		serpent.Option{
			...
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_PROMETHEUS_WAIT",
			Default:     "15s",
...
func (f *workspaceTargetFlags) attach(opts *serpent.OptionSet) {
	*opts = append(*opts,
		serpent.Option{
			Flag:          "template",
			FlagShorthand: "t",
			Env:           "OPTIMUS-IDE-COLLAB_SCALETEST_TEMPLATE",
		...
		serpent.Option{
			...
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_TARGET_WORKSPACES",
		...
		serpent.Option{
			Flag:        "use-host-login",
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_USE_HOST_LOGIN",
			Default:     "false",
...
func (f *workspaceTargetFlags) getTargetedWorkspaces(ctx context.Context, client *optimus-ide-collabsdk.Client, organizationIDs []uuid.UUID, warnWriter io.Writer) ([]optimus-ide-collabsdk.Workspace, error) {
...
func RequireAdmin(ctx context.Context, client *optimus-ide-collabsdk.Client) (optimus-ide-collabsdk.User, error) {
	me, err := client.User(ctx, optimus-ide-collabsdk.Me)
	if err != nil {
		return optimus-ide-collabsdk.User{}, xerrors.Errorf("fetch current user: %w", err)
	}
...
func getScaletestPrebuildWorkspaces(ctx context.Context, client *optimus-ide-collabsdk.Client, template string) ([]optimus-ide-collabsdk.Workspace, error) {
	const pageSize = 100
	...
	var result []optimus-ide-collabsdk.Workspace

	for _, tmpl := range templates {
		for page := 0; ; page++ {
			resp, err := client.Workspaces(ctx, optimus-ide-collabsdk.WorkspaceFilter{
				Template: tmpl.Name,
...
func getScaletestPrebuildsTemplates(ctx context.Context, client *optimus-ide-collabsdk.Client, template string) ([]optimus-ide-collabsdk.Template, error) {
	var filter optimus-ide-collabsdk.TemplateFilter
	if template != "" {
		...
		filter = optimus-ide-collabsdk.TemplateFilter{ExactName: template}
	} else {
		filter = optimus-ide-collabsdk.TemplateFilter{FuzzyName: prebuilds.TemplatePrefix}
	}
...
func (r *RootCmd) scaletestCleanup() *serpent.Command {
	...
	cmd := &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			client.HTTPClient = &http.Client{
				Transport: &optimus-ide-collabsdk.HeaderTransport{
					...
					Header: map[string][]string{
						optimus-ide-collabsdk.BypassRatelimitHeader: {"true"},
					},
				},
			}
			...
			setPrebuild := func(val bool) error {
				return client.PutPrebuildsSettings(ctx, optimus-ide-collabsdk.PrebuildsSettings{ReconciliationPaused: val})
			}
	...
	cmd.Options = serpent.OptionSet{
		{
			Flag:        "template",
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_CLEANUP_TEMPLATE",
	...
	cmd := &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			for i := 0; i < int(count); i++ {
				...
				// requests being unbalanced among Optimus-IDE-Collab instances.
	...
	cmd.Options = serpent.OptionSet{
		{
			Flag:          "count",
			FlagShorthand: "c",
			Env:           "OPTIMUS-IDE-COLLAB_SCALETEST_COUNT",
			Default:       "1",
		...
		{
			Flag:        "retry",
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_RETRY",
			Default:     "0",
		...
		{
			Flag:        "no-cleanup",
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_NO_CLEANUP",
			Description: "Do not clean up resources after the test completes. You can cleanup manually using optimus-ide-collab scaletest cleanup.",
			Value:       serpent.BoolOf(&noCleanup),
		},
		{
			Flag:        "no-wait-for-agents",
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_NO_WAIT_FOR_AGENTS",
			Description: `Do not wait for agents to start before marking the test as succeeded. This can be useful if you are running the test against a template that does not start the agent quickly. This is REQUIRED for templates whose workspaces use optimus-ide-collab_external_agent resources, since external agents never connect on their own; pair with "optimus-ide-collab exp scaletest agentfake" to drive those agents.`,
			Value:       serpent.BoolOf(&noWaitForAgents),
		},
		{
			Flag:        "run-command",
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_RUN_COMMAND",
		...
		{
			Flag:        "run-timeout",
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_RUN_TIMEOUT",
			Default:     "5s",
		...
		},
		{
			...
			Env:  "OPTIMUS-IDE-COLLAB_SCALETEST_RUN_EXPECT_TIMEOUT",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_RUN_EXPECT_OUTPUT",
		...
		{
			Flag:        "run-log-output",
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_RUN_LOG_OUTPUT",
		...
		{
			Flag:        "connect-url",
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_CONNECT_URL",
		...
		{
			Flag:        "connect-mode",
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_CONNECT_MODE",
			Default:     "derp",
		...
		{
			Flag:        "connect-hold",
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_CONNECT_HOLD",
			Default:     "30s",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_CONNECT_INTERVAL",
			Default:     "1s",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_CONNECT_TIMEOUT",
			Default:     "5s",
		...
		{
			Flag:        "max-failures",
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_MAX_FAILURES",
			Default:     "0",
...
func (r *RootCmd) scaletestWorkspaceUpdates() *serpent.Command {
	...
	cmd := &serpent.Command{
		...
		Short: "Simulate the load of Optimus-IDE-Collab Desktop clients receiving workspace updates",
		Handler: func(inv *serpent.Invocation) error {
			ctx := inv.Context()
	...
	cmd.Options = serpent.OptionSet{
		{
			...
			FlagShorthand: "c",
			Env:           "OPTIMUS-IDE-COLLAB_SCALETEST_WORKSPACE_COUNT",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_POWER_USER_WORKSPACES",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_POWER_USER_PERCENTAGE",
			Default:     "50.0",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_WORKSPACE_UPDATES_TIMEOUT",
			Default:     "5m",
		...
		{
			Flag:        "dial-timeout",
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_DIAL_TIMEOUT",
			Default:     "2m",
...
func (r *RootCmd) scaletestWorkspaceTraffic() *serpent.Command {
	...
	cmd := &serpent.Command{
		...
		Short: "Generate traffic to scaletest workspaces through optimus-ide-collabd",
		Handler: func(inv *serpent.Invocation) (err error) {
			...
			for idx, ws := range workspaces {
				...
				var webClient *optimus-ide-collabsdk.Client
				if workspaceProxyURL != "" {
					...
					webClient = optimus-ide-collabsdk.New(u,
						optimus-ide-collabsdk.WithHTTPClient(client.HTTPClient),
	...
	cmd.Options = []serpent.Option{
		{
			Flag:        "bytes-per-tick",
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_WORKSPACE_TRAFFIC_BYTES_PER_TICK",
			Default:     "1024",
		...
		{
			Flag:        "tick-interval",
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_WORKSPACE_TRAFFIC_TICK_INTERVAL",
			Default:     "100ms",
		...
		{
			Flag:        "ssh",
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_WORKSPACE_TRAFFIC_SSH",
			Default:     "",
		...
		{
			Flag:        "disable-direct",
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_WORKSPACE_TRAFFIC_DISABLE_DIRECT_CONNECTIONS",
			Default:     "false",
		...
		{
			Flag:        "app",
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_WORKSPACE_TRAFFIC_APP",
			Default:     "",
			Description: "Send WebSocket traffic to a workspace app (proxied via optimus-ide-collabd), cannot be used with --ssh.",
			Value:       serpent.StringOf(&app),
		},
		{
			Flag:        "workspace-proxy-url",
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_WORKSPACE_PROXY_URL",
			Default:     "",
...
func (r *RootCmd) scaletestDashboard() *serpent.Command {
	...
	cmd := &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			for idx, usr := range users {
				...
				userTokResp, err := client.CreateToken(ctx, usr.ID.String(), optimus-ide-collabsdk.CreateTokenRequest{
				...
				optimus-ide-collabsdk.WithSessionToken(userTokResp.Key)(userClient)
	...
	cmd.Options = []serpent.Option{
		{
			Flag:        "target-users",
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_DASHBOARD_TARGET_USERS",
		...
		{
			Flag:        "interval",
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_DASHBOARD_INTERVAL",
			Default:     "10s",
		...
		{
			Flag:        "jitter",
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_DASHBOARD_JITTER",
			Default:     "5s",
		...
		{
			Flag:        "headless",
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_DASHBOARD_HEADLESS",
			Default:     "true",
		...
		{
			Flag:        "rand-seed",
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_DASHBOARD_RAND_SEED",
			Default:     "0",
...
func (r *RootCmd) scaletestAutostart() *serpent.Command {
	...
	cmd := &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			if !experiments.Enabled(optimus-ide-collabsdk.ExperimentWorkspaceBuildUpdates) {
			...
			dispatcher.Start(ctx, deoptimus-ide-collab.Chan())
	...
	cmd.Options = serpent.OptionSet{
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_WORKSPACE_JOB_TIMEOUT",
			Default:     "5m",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_AUTOSTART_BUILD_TIMEOUT",
			Default:     "15m",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_AUTOSTART_DELAY",
			Default:     "2m",
...
func getScaletestWorkspaces(ctx context.Context, client *optimus-ide-collabsdk.Client, owner, template string) ([]optimus-ide-collabsdk.Workspace, int, error) {
	var (
		pageNumber = 0
		limit      = 100
	...
	for {
		page, err := client.Workspaces(ctx, optimus-ide-collabsdk.WorkspaceFilter{
			Name:     "scaletest-",
			Template: template,
			Owner:    owner,
		...
		pageWorkspaces := make([]optimus-ide-collabsdk.Workspace, 0, len(page.Workspaces))
...
func getScaletestUsers(ctx context.Context, client *optimus-ide-collabsdk.Client) ([]optimus-ide-collabsdk.User, error) {
	var (
		pageNumber = 0
		limit      = 100
		users      []optimus-ide-collabsdk.User
	)
	...
	for {
		...
		pageUsers := make([]optimus-ide-collabsdk.User, 0, len(page.Users))
...
func parseTemplate(ctx context.Context, client *optimus-ide-collabsdk.Client, organizationIDs []uuid.UUID, template string) (tpl optimus-ide-collabsdk.Template, err error) {
...
func createWorkspaceAppConfig(client *optimus-ide-collabsdk.Client, appHost, app string, workspace optimus-ide-collabsdk.Workspace, agent optimus-ide-collabsdk.WorkspaceAgent) (workspacetraffic.AppConfig, error) {
	if app == "" {
		return workspacetraffic.AppConfig{}, nil
	}

	i := slices.IndexFunc(agent.Apps, func(a optimus-ide-collabsdk.WorkspaceApp) bool { return a.Slug == app })
	if i == -1 {
