		...
		Handler: func(inv *serpent.Invocation) error {
					...
					if errors.Is(err, io.EOF) {
						return xerrors.Errorf(
							"Couldn't find a matching template!\n" +
								...
								"optimus-ide-collab templates init --id <template_id> instead!",
						)
					}
					return err
				}
			...
			_, _ = fmt.Fprintln(
				...
				pretty.Sprint(
					...
					"cd "+relPath+" && optimus-ide-collab templates push"),
			)
...
func templateByID(templateID string, tes []optimus-ide-collabsdk.TemplateExample) (optimus-ide-collabsdk.TemplateExample, bool) {
	for _, te := range tes {
		if te.ID == templateID {
			return te, true
		}
	}
	return optimus-ide-collabsdk.TemplateExample{}, false
}
