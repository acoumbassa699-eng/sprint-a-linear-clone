...
import (
	...
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/cli/sessionstore"
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/cli/telemetry"
...
const (
	...
	envNoVersionCheck    = "OPTIMUS-IDE-COLLAB_NO_VERSION_WARNING"
	envNoFeatureWarning  = "OPTIMUS-IDE-COLLAB_NO_FEATURE_WARNING"
	envSessionToken      = "OPTIMUS-IDE-COLLAB_SESSION_TOKEN"
	envUseKeyring        = "OPTIMUS-IDE-COLLAB_USE_KEYRING"
	envClientTLSCAFile   = "OPTIMUS-IDE-COLLAB_CLIENT_TLS_CA_FILE"
	envClientTLSCertFile = "OPTIMUS-IDE-COLLAB_CLIENT_TLS_CERT_FILE"
	envClientTLSKeyFile  = "OPTIMUS-IDE-COLLAB_CLIENT_TLS_KEY_FILE"
	//nolint:gosec
	envAgentToken = "OPTIMUS-IDE-COLLAB_AGENT_TOKEN"
	//nolint:gosec
	envAgentTokenFile = "OPTIMUS-IDE-COLLAB_AGENT_TOKEN_FILE"
	envAgentURL       = "OPTIMUS-IDE-COLLAB_AGENT_URL"
	envAgentAuth      = "OPTIMUS-IDE-COLLAB_AGENT_AUTH"
	envAgentName      = "OPTIMUS-IDE-COLLAB_AGENT_NAME"
	envURL            = "OPTIMUS-IDE-COLLAB_URL"
)
...
func (r *RootCmd) AGPL() []*serpent.Command {
	all := append(
		r.CoreSubcommands(),
		r.Server( /* Do not import optimus-ide-collabd here. */ nil),
		r.Provisioners(),
...
func (r *RootCmd) RunWithSubcommands(subcommands []*serpent.Command) {
	...
	goTraceFilePath, ok := os.LookupEnv("OPTIMUS-IDE-COLLAB_GO_TRACE")
	if ok {
...
func (r *RootCmd) Command(subcommands []*serpent.Command) (*serpent.Command, error) {
	...
	fmtLong := `Optimus-IDE-Collab %s — A tool for provisioning self-hosted development environments with Terraform.
`
	hiddenAgentAuth := &AgentAuth{}
	cmd := &serpent.Command{
		Use: "optimus-ide-collab [global-flags] <subcommand>",
		Long: fmt.Sprintf(fmtLong, buildinfo.Version()) + FormatExamples(
			Example{
				Description: "Start a Optimus-IDE-Collab server",
				Command:     "optimus-ide-collab server",
			},
			Example{
				Description: "Get started by creating a template from an example",
				Command:     "optimus-ide-collab templates init",
			},
		),
		Handler: func(i *serpent.Invocation) error {
			if r.versionFlag {
			...
			// cross-platform scripts to invoke the Optimus-IDE-Collab binary
	...
	cmd.Options = serpent.OptionSet{
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_HEADER_COMMAND",
		...
		{
			Flag:        varNoOpen,
			Env:         "OPTIMUS-IDE-COLLAB_NO_OPEN",
		...
		{
			Flag:        varForceTty,
			Env:         "OPTIMUS-IDE-COLLAB_FORCE_TTY",
			Hidden:      false,
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_DISABLE_DIRECT_CONNECTIONS",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_DISABLE_NETWORK_TELEMETRY",
			Description: "Disable network telemetry. Network telemetry is collected when connecting to workspaces using the CLI, and is forwarded to the server. If telemetry is also enabled on the server, it may be sent to Optimus-IDE-Collab. Network telemetry is used to measure network quality and detect regressions.",
		...
		{
			Flag:        "debug-http",
			Description: "Debug optimus-ide-collabsdk HTTP requests.",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_CONFIG_DIR",
			Description: "Path to the global `optimus-ide-collab` config directory.",
		...
		},
		{
			...
			// They have two Optimus-IDE-Collab CLIs, and want to tell the difference by running
...
// wasn't provided via --url or OPTIMUS-IDE-COLLAB_URL.
...
// and version checks.
func (r *RootCmd) InitClient(inv *serpent.Invocation) (*optimus-ide-collabsdk.Client, error) {
	...
	clientOpts := []optimus-ide-collabsdk.ClientOption{
		optimus-ide-collabsdk.WithSessionToken(r.token),
		optimus-ide-collabsdk.WithHTTPClient(httpClient),
	}

	if r.disableDirect {
		clientOpts = append(clientOpts, optimus-ide-collabsdk.WithDisableDirectConnections())
	}

	if r.tlsConfig != nil {
		clientOpts = append(clientOpts, optimus-ide-collabsdk.WithDERPTLSConfig(r.tlsConfig))
	}

	if r.debugHTTP {
		clientOpts = append(clientOpts,
			optimus-ide-collabsdk.WithPlainLogger(os.Stderr),
			optimus-ide-collabsdk.WithLogBodies(),
		)
	}

	return optimus-ide-collabsdk.New(r.clientURL, clientOpts...), nil
}
...
func (r *RootCmd) TryInitClient(inv *serpent.Invocation) (*optimus-ide-collabsdk.Client, error) {
	conf := r.createConfig()
	...
	return &optimus-ide-collabsdk.Client{}, nil
}

// HeaderTransport creates a new transport that executes `--header-command`
// if it is set to add headers for all outbound requests.
func (r *RootCmd) HeaderTransport(ctx context.Context, serverURL *url.URL) (*optimus-ide-collabsdk.HeaderTransport, error) {
...
func (r *RootCmd) createHTTPClient(ctx context.Context, serverURL *url.URL, inv *serpent.Invocation) (*http.Client, error) {
	...
	if !r.noVersionCheck {
		...
		transport = wrapTransportWithVersionCheck(transport, inv, buildinfo.Version(), func(ctx context.Context) (optimus-ide-collabsdk.BuildInfoResponse, error) {
			// Create a new client without any wrapped transport
			// otherwise it creates an infinite loop!
			basicClient := optimus-ide-collabsdk.New(serverURL, optimus-ide-collabsdk.WithHTTPClient(&http.Client{Transport: buildInfoTransport}))
	...
	// optimus-ide-collabsdk checks for the header transport to get headers
...
func (r *RootCmd) createUnauthenticatedClient(ctx context.Context, serverURL *url.URL, inv *serpent.Invocation) (*optimus-ide-collabsdk.Client, error) {
	...
	client := optimus-ide-collabsdk.New(serverURL, optimus-ide-collabsdk.WithHTTPClient(httpClient))
	return client, nil
}
...
func (r *RootCmd) ensureTokenBackend() sessionstore.Backend {
	if r.tokenBackend == nil {
		...
		// either access the credential in the keyring (like Optimus-IDE-Collab Desktop) or some other
...
func (a *AgentAuth) CreateClient() (*agentsdk.Client, error) {
	...
	case "token":
		...
		if token == "" {
			return nil, xerrors.Errorf("OPTIMUS-IDE-COLLAB_AGENT_TOKEN or OPTIMUS-IDE-COLLAB_AGENT_TOKEN_FILE must be set for token auth")
		}
...
func (o *OrganizationContext) AttachOptions(cmd *serpent.Command) {
	cmd.Options = append(cmd.Options, serpent.Option{
		...
		Flag:          "org",
		FlagShorthand: "O",
		Env:           "OPTIMUS-IDE-COLLAB_ORGANIZATION",
...
func (o *OrganizationContext) Selected(inv *serpent.Invocation, client *optimus-ide-collabsdk.Client) (optimus-ide-collabsdk.Organization, error) {
	// Fetch the set of organizations the user is a member of.
	orgs, err := client.OrganizationsByUser(inv.Context(), optimus-ide-collabsdk.Me)
	if err != nil {
		return optimus-ide-collabsdk.Organization{}, xerrors.Errorf("get organizations: %w", err)
	}

	// User manually selected an organization
	if o.FlagSelect != "" {
		index := slices.IndexFunc(orgs, func(org optimus-ide-collabsdk.Organization) bool {
		...
		if err != nil {
			...
			if errors.As(err, &sdkErr) && sdkErr.StatusCode() == http.StatusNotFound {
				return optimus-ide-collabsdk.Organization{}, xerrors.Errorf("organization %q not found, are you sure you are a member of this organization? "+
					"Valid options for '--org=' are [%s].", o.FlagSelect, strings.Join(names, ", "))
			}
			return optimus-ide-collabsdk.Organization{}, xerrors.Errorf("get organization %q: %w", o.FlagSelect, err)
		}
		return org, nil
	}
	...
	return optimus-ide-collabsdk.Organization{}, xerrors.Errorf("Must select an organization with --org=<org_name>. Choose from: %s", strings.Join(validOrgs, ", "))
}

func initAppearance(ctx context.Context, client *optimus-ide-collabsdk.Client) optimus-ide-collabsdk.AppearanceConfig {
	// best effort
	cfg, _ := client.Appearance(ctx)
	if cfg.DocsURL == "" {
		cfg.DocsURL = optimus-ide-collabsdk.DefaultDocsURL()
	}
	return cfg
}
...
func DumpHandler(ctx context.Context, name string) {
	...
	for {
		...
		fpath := filepath.Join(dir, fmt.Sprintf("optimus-ide-collab-%s-%s.dump", name, filesystemSafeTime))
...
func cliHumanFormatError(from string, err error, opts *formatOpts) (string, bool) {
	...
	if sdkError, ok := err.(*optimus-ide-collabsdk.Error); ok {
		return formatOptimus-IDE-CollabSDKError(from, sdkError, opts), true
	}
...
// If you know the error is something else, like a optimus-ide-collabsdk.Error, make a new
...
// formatOptimus-IDE-CollabSDKError come from API requests. In verbose mode, add the
// request debug information.
func formatOptimus-IDE-CollabSDKError(from string, err *optimus-ide-collabsdk.Error, opts *formatOpts) string {
	var str strings.Builder
	if opts.Verbose {
...
func SlimUnsupported(w io.Writer, cmd string) {
	_, _ = fmt.Fprintf(w, "You are using a 'slim' build of Optimus-IDE-Collab, which does not support the %s subcommand.\n", pretty.Sprint(cliui.DefaultStyles.Code, cmd))
	_, _ = fmt.Fprintln(w, "")
	_, _ = fmt.Fprintln(w, "Please use a build of Optimus-IDE-Collab from GitHub releases:")
	_, _ = fmt.Fprintln(w, "  https://github.com/optimus-ide-collab/optimus-ide-collab/releases")
...
func defaultUpgradeMessage(version string) string {
	...
	if runtime.GOOS == "windows" {
		return fmt.Sprintf("download the server version from: https://github.com/optimus-ide-collab/optimus-ide-collab/releases/v%s", version)
	}
	return fmt.Sprintf("download the server version with: 'curl -L https://optimus-ide-collab.com/install.sh | sh -s -- --version %s'", version)
}
...
func serverVersionMessage(serverVersion string) string {
	...
	case buildinfo.IsRCVersion(serverVersion):
		return fmt.Sprintf("the server is running a release candidate of Optimus-IDE-Collab (%s)", serverVersion)
	case buildinfo.IsDevVersion(serverVersion):
		return fmt.Sprintf("the server is running a development version of Optimus-IDE-Collab (%s)", serverVersion)
	default:
		return ""
	}
}
...
func wrapTransportWithEntitlementsCheck(rt http.RoundTripper, w io.Writer) http.RoundTripper {
	...
	return roundTripper(func(req *http.Request) (*http.Response, error) {
		...
		once.Do(func() {
			for _, warning := range res.Header.Values(optimus-ide-collabsdk.EntitlementsWarningHeader) {
...
func wrapTransportWithVersionCheck(rt http.RoundTripper, inv *serpent.Invocation, clientVersion string, getBuildInfo func(ctx context.Context) (optimus-ide-collabsdk.BuildInfoResponse, error)) http.RoundTripper {
	var once sync.Once
	return roundTripper(func(req *http.Request) (*http.Response, error) {
		res, err := rt.RoundTrip(req)
		if err != nil {
		...
		once.Do(func() {
			serverVersion := res.Header.Get(optimus-ide-collabsdk.BuildVersionHeader)
			if serverVersion == "" {
				return
			}
...
func wrapTransportWithTelemetryHeader(transport http.RoundTripper, inv *serpent.Invocation) http.RoundTripper {
	...
	return roundTripper(func(req *http.Request) (*http.Response, error) {
		...
		if value != "" {
			req.Header.Add(optimus-ide-collabsdk.CLITelemetryHeader, value)
		}
...
func wrapTransportWithUserAgentHeader(transport http.RoundTripper, inv *serpent.Invocation) http.RoundTripper {
	...
	return roundTripper(func(req *http.Request) (*http.Response, error) {
		once.Do(func() {
			userAgent = fmt.Sprintf("optimus-ide-collab-cli/%s (%s/%s; %s)", buildinfo.Version(), runtime.GOOS, runtime.GOARCH, inv.Command.FullName())
		})
...
// HeaderTransport creates a new transport that executes `--header-command`
// if it is set to add headers for all outbound requests.
func headerTransport(ctx context.Context, serverURL *url.URL, header []string, headerCommand string) (*optimus-ide-collabsdk.HeaderTransport, error) {
	...
	if headerCommand != "" {
		...
		cmd.Env = append(os.Environ(), "OPTIMUS-IDE-COLLAB_URL="+serverURL.String())
		cmd.Stdout = &outBuf
