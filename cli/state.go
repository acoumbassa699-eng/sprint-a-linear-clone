	...
	cmd := &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			if buildNumber == 0 {
				...
				owner, workspace, err := optimus-ide-collabsdk.SplitWorkspaceIdentifier(inv.Args[0])
				if err != nil {
					return err
				}
...
func (r *RootCmd) statePush() *serpent.Command {
	...
	cmd := &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			build, err = client.CreateWorkspaceBuild(inv.Context(), workspace.ID, optimus-ide-collabsdk.CreateWorkspaceBuildRequest{
