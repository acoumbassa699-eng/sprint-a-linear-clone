...
func (r *RootCmd) templatePush() *serpent.Command {
	...
	cmd := &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			err = optimus-ide-collabsdk.NameValid(name)
			if err != nil {
				return xerrors.Errorf("template name %q is invalid: %w", name, err)
			}

			if versionName != "" {
				err = optimus-ide-collabsdk.TemplateVersionNameValid(versionName)
				if err != nil {
			...
			if !uploadFlags.stdin(inv) {
				varsFiles, err = optimus-ide-collabsdk.DiscoverVarsFiles(uploadFlags.directory)
				if err != nil {
					return err
				}
			...
			userVariableValues, err := optimus-ide-collabsdk.ParseUserVariableValues(
				varsFiles,
				variablesFile,
			...
			args := createValidTemplateVersionArgs{
				...
				Provisioner:        optimus-ide-collabsdk.ProvisionerType(provisioner),
				FileID:             resp.ID,
			...
			if job.Job.Status != optimus-ide-collabsdk.ProvisionerJobSucceeded {
				return xerrors.Errorf("job failed: %s", job.Job.Status)
			}

			if createTemplate {
				_, err = client.CreateTemplate(inv.Context(), organization.ID, optimus-ide-collabsdk.CreateTemplateRequest{
					Name:      name,
					VersionID: job.ID,
				})
				if err != nil {
				...
				err = client.UpdateActiveTemplateVersion(inv.Context(), template.ID, optimus-ide-collabsdk.UpdateActiveTemplateVersion{
					ID: job.ID,
				})
				if err != nil {
					return err
...
func (pf *templateUploadFlags) upload(inv *serpent.Invocation, client *optimus-ide-collabsdk.Client) (*optimus-ide-collabsdk.UploadResponse, error) {
	...
	resp, err := client.Upload(inv.Context(), optimus-ide-collabsdk.ContentTypeTar, bufio.NewReader(content))
	if err != nil {
...
func (pf *templateUploadFlags) checkForLockfile(inv *serpent.Invocation) error {
	...
	if !hasLockfile {
		cliui.Warn(inv.Stdout, "No .terraform.lock.hcl file found",
			"When provisioning, Optimus-IDE-Collab will be unable to cache providers without a lockfile and must download them from the internet each time.",
...
type createValidTemplateVersionArgs struct {
	...
	Organization optimus-ide-collabsdk.Organization
	Provisioner  optimus-ide-collabsdk.ProvisionerType
	FileID       uuid.UUID
...
func createValidTemplateVersion(inv *serpent.Invocation, args createValidTemplateVersionArgs) (*optimus-ide-collabsdk.TemplateVersion, error) {
	client := args.Client
	...
	if err != nil {
		...
		if errors.As(err, &jobErr) {
			if optimus-ide-collabsdk.JobIsMissingRequiredTemplateVariableErrorCode(jobErr.Code) {
				return handleMissingTemplateVariables(inv, args, version.ID)
			}
			if !optimus-ide-collabsdk.JobIsMissingParameterErrorCode(jobErr.Code) {
				return nil, err
			}
		}
		return nil, err
	...
	if version.Job.Status != optimus-ide-collabsdk.ProvisionerJobSucceeded {
	...
	var startResources []optimus-ide-collabsdk.WorkspaceResource
	for _, r := range resources {
		if r.Transition == optimus-ide-collabsdk.WorkspaceTransitionStart {
...
func handleMissingTemplateVariables(inv *serpent.Invocation, args createValidTemplateVersionArgs, failedVersionID uuid.UUID) (*optimus-ide-collabsdk.TemplateVersion, error) {
	client := args.Client
	...
	var missingVariables []optimus-ide-collabsdk.TemplateVersionVariable
	for _, variable := range templateVariables {
		...
		// See: https://github.com/optimus-ide-collab/optimus-ide-collab/blob/a78790c632974e04babfef6de0e2ddf044787a7a/optimus-ide-collabd/provisionerdserver/provisionerdserver.go#L3206
	...
	var promptedValues []optimus-ide-collabsdk.VariableValue
	for _, variable := range missingVariables {
		...
		promptedValues = append(promptedValues, optimus-ide-collabsdk.VariableValue{
			Name:  variable.Name,
			Value: value,
		})
	}

	combinedValues := optimus-ide-collabsdk.CombineVariableValues(args.UserVariableValues, promptedValues)
...
func promptForTemplateVariable(inv *serpent.Invocation, variable optimus-ide-collabsdk.TemplateVersionVariable) (string, error) {
...
func displayVariableInfo(inv *serpent.Invocation, variable optimus-ide-collabsdk.TemplateVersionVariable) {
...
func promptForBoolVariable(inv *serpent.Invocation, variable optimus-ide-collabsdk.TemplateVersionVariable) (string, error) {
...
func promptForNumberVariable(inv *serpent.Invocation, variable optimus-ide-collabsdk.TemplateVersionVariable) (string, error) {
	prompt := "Enter value:"
...
func promptForStringVariable(inv *serpent.Invocation, variable optimus-ide-collabsdk.TemplateVersionVariable) (string, error) {
	prompt := "Enter value:"
	if !variable.Sensitive {
		if !variable.Required && variable.DefaultValue != "" {
			prompt = fmt.Sprintf("Enter value (default: %q):", variable.DefaultValue)
...
func createVariableValidator(variable optimus-ide-collabsdk.TemplateVersionVariable) func(string) error {
