...
func prepareTestGitSSH(ctx context.Context, t *testing.T) (*agentsdk.Client, string, gossh.PublicKey) {
	...
	// get user public key
	keypair, err := client.GitSSHKey(ctx, optimus-ide-collabsdk.Me)
	...
	_ = optimus-ide-collabdtest.NewWorkspaceAgentWaiter(t, client, r.Workspace.ID).WithContext(ctx).Wait()
...
func TestGitSSH(t *testing.T) {
	...
	t.Run("Local SSH Keys", func(t *testing.T) {
		...
		client, token, optimus-ide-collabPubkey := prepareTestGitSSH(setupCtx, t)
		...
		}, localPubkey, optimus-ide-collabPubkey)
		...
		// With the local file deleted, the optimus-ide-collab key should be used.
		inv, _ = clitest.New(t, cmdArgs...)
		// This occasionally times out at 15s on Windows CI runners. Use a
		// longer timeout to reduce flakes.
		...
		select {
		case key := <-authkey:
			require.Equal(t, optimus-ide-collabPubkey, key)
		case <-ctx.Done():
			t.Fatal("timeout waiting for auth")
		}
	})
