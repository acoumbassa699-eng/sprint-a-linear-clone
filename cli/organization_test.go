...
func TestCurrentOrganization(t *testing.T) {
	...
	// 2. The user is connecting to an older Optimus-IDE-Collab instance.
	t.Run("no-default", func(t *testing.T) {
		...
		srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			json.NewEnoptimus-ide-collab(w).Encode([]optimus-ide-collabsdk.Organization{
				{
		...
		client := optimus-ide-collabsdk.New(must(url.Parse(srv.URL)))
	...
	t.Run("OK", func(t *testing.T) {
		...
		server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			case r.Method == http.MethodGet && r.URL.Path == "/api/v2/organizations":
				_ = json.NewEnoptimus-ide-collab(w).Encode([]optimus-ide-collabsdk.Organization{
					{
						MinimalOrganization: optimus-ide-collabsdk.MinimalOrganization{
							ID:          orgID,
							Name:        "my-org",
