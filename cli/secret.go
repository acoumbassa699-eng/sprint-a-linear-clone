	...
	cmd := &serpent.Command{
		...
		Long: FormatExamples(
			Example{
				...
				Command:     "printf %s \"$MYCLI_API_KEY\" | optimus-ide-collab secret create api-key --description \"API key for workspace tools\" --env API_KEY --file \"~/.api-key\"",
			},
			Example{
				Description: "Update a secret",
				Command:     "echo -n \"$NEW_SECRET_VALUE\" | optimus-ide-collab secret update api-key --description \"Rotated API key\" --env API_KEY --file \"~/.api-key\"",
			},
			Example{
				Description: "Import secrets from a file",
				Command:     "optimus-ide-collab secret import ./secrets.env",
			},
			Example{
				Description: "List your secrets",
				Command:     "optimus-ide-collab secret list",
			},
			Example{
				Description: "Show a specific secret",
				Command:     "optimus-ide-collab secret list api-key",
			},
			Example{
				Description: "Delete a secret",
				Command:     "optimus-ide-collab secret delete api-key",
			},
		),
...
func (r *RootCmd) secretCreate() *serpent.Command {
	...
	cmd := &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			req := optimus-ide-collabsdk.CreateUserSecretRequest{
				Name:        inv.Args[0],
				Value:       resolvedValue,
			...
			secret, err := client.CreateUserSecret(inv.Context(), optimus-ide-collabsdk.Me, req)
			if err != nil {
...
func (r *RootCmd) secretUpdate() *serpent.Command {
	...
	cmd := &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			req := optimus-ide-collabsdk.UpdateUserSecretRequest{}
			resolvedValue, ok, err := secretValue(inv, value)
			if err != nil {
				return err
			}
			...
			secret, err := client.UpdateUserSecret(inv.Context(), optimus-ide-collabsdk.Me, inv.Args[0], req)
			if err != nil {
...
var secretsFileFormats = []string{
	string(optimus-ide-collabsdk.SecretsFileFormatEnv),
	string(optimus-ide-collabsdk.SecretsFileFormatJSON),
	string(optimus-ide-collabsdk.SecretsFileFormatYAML),
}
...
func (r *RootCmd) secretImport() *serpent.Command {
	...
	cmd := &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			format := optimus-ide-collabsdk.SecretsFileFormat(strings.ToLower(inputFormat))
			if format == "" {
			...
			requests, err := optimus-ide-collabsdk.ParseSecretsFile(format, string(content))
			if err != nil {
			...
			secrets, err := client.ImportUserSecrets(inv.Context(), optimus-ide-collabsdk.Me, optimus-ide-collabsdk.ImportUserSecretsRequest{
				Format:  format,
...
// --input-format.
func secretsFileFormatFromPath(path string) (optimus-ide-collabsdk.SecretsFileFormat, error) {
	switch strings.ToLower(filepath.Ext(path)) {
	case ".env":
		return optimus-ide-collabsdk.SecretsFileFormatEnv, nil
	case ".json":
		return optimus-ide-collabsdk.SecretsFileFormatJSON, nil
	case ".yaml", ".yml":
		return optimus-ide-collabsdk.SecretsFileFormatYAML, nil
	default:
...
func readSecretsFile(inv *serpent.Invocation, path string) ([]byte, error) {
	...
	content, err := io.ReadAll(io.LimitReader(reader, optimus-ide-collabsdk.MaxSecretsFileBytes+1))
	if err != nil {
		return nil, xerrors.Errorf("read secrets file: %w", err)
	}
	if len(content) > optimus-ide-collabsdk.MaxSecretsFileBytes {
		return nil, xerrors.Errorf("secrets file exceeds the maximum allowed size of %d bytes", optimus-ide-collabsdk.MaxSecretsFileBytes)
	}
...
func validateImportedSecrets(requests []optimus-ide-collabsdk.CreateUserSecretRequest) error {
	var validationErrors []string
	for i, request := range requests {
		for _, validation := range optimus-ide-collabsdk.ValidateCreateUserSecretRequest(request) {
...
func warnSecretsWithoutEnvName(w io.Writer, secrets []optimus-ide-collabsdk.UserSecret) {
	...
	cliui.Warn(w,
		...
		"Set each with `optimus-ide-collab secret update <name> --env <ENV_NAME>` to inject it into workspaces.",
	)
}
...
type secretListRow struct {
	optimus-ide-collabsdk.UserSecret `table:"-"`
...
func secretListRowFromSecret(secret optimus-ide-collabsdk.UserSecret) secretListRow {
...
// secretEnabledState distinguishes the two `optimus-ide-collab secret enable` and
// `optimus-ide-collab secret disable` subcommands without using a bare bool, which
...
// secretEnabledSetter builds the `optimus-ide-collab secret enable` and `optimus-ide-collab secret
...
func (r *RootCmd) secretEnabledSetter(state secretEnabledState) *serpent.Command {
	...
	cmd := &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			name := inv.Args[0]
			secret, err := client.UpdateUserSecret(inv.Context(), optimus-ide-collabsdk.Me, name, optimus-ide-collabsdk.UpdateUserSecretRequest{
...
func (r *RootCmd) secretList() *serpent.Command {
	formatter := cliui.NewOutputFormatter(
		...
		cliui.ChangeFormatterData(
			...
			func(data any) (any, error) {
				switch rows := data.(type) {
				case []secretListRow:
					secrets := make([]optimus-ide-collabsdk.UserSecret, len(rows))
					for i := range rows {
						secrets[i] = rows[i].UserSecret
					}
					return secrets, nil
				case secretListRow:
					return []optimus-ide-collabsdk.UserSecret{rows.UserSecret}, nil
				default:
					return nil, xerrors.Errorf("expected []secretListRow or secretListRow, got %T", data)
				}
			},
	...
	cmd := &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			var data any
			if len(inv.Args) == 1 {
				secret, err := client.UserSecretByName(inv.Context(), optimus-ide-collabsdk.Me, inv.Args[0])
				if err != nil {
					return xerrors.Errorf("get secret %q: %w", inv.Args[0], err)
				}
				data = secretListRowFromSecret(secret)
			} else {
				secrets, err := client.UserSecrets(inv.Context(), optimus-ide-collabsdk.Me)
				if err != nil {
...
func (r *RootCmd) secretDelete() *serpent.Command {
	cmd := &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			if err = client.DeleteUserSecret(inv.Context(), optimus-ide-collabsdk.Me, name); err != nil {
