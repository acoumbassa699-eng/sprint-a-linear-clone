	...
	cmd := &serpent.Command{
		...
		Long: FormatExamples(
			Example{
				...
				Command:     "optimus-ide-collab task status task1",
			},
			Example{
				Description: "Watch the status of a given task until it completes (idle or stopped).",
				Command:     "optimus-ide-collab task status task1 --watch",
			},
		),
		Use:     "status",
		Aliases: []string{"stat"},
...
func taskWatchIsEnded(task optimus-ide-collabsdk.Task) bool {
	if task.WorkspaceStatus == optimus-ide-collabsdk.WorkspaceStatusStopped {
		return true
	}
	...
	if task.CurrentState == nil || task.CurrentState.State == optimus-ide-collabsdk.TaskStateWorking {
		return false
	}
	return true
}
...
type taskStatusRow struct {
	optimus-ide-collabsdk.Task `table:"r,recursive_inline"`
...
func toStatusRow(task optimus-ide-collabsdk.Task, now time.Time) taskStatusRow {
	tsr := taskStatusRow{
		Task:       task,
...
func taskStateEqual(se1, se2 *optimus-ide-collabsdk.TaskStateEntry) bool {
