...
import (
	...
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/scaletest/bridge"
...
func (r *RootCmd) scaletestBridge() *serpent.Command {
	...
	cmd := &serpent.Command{
		...
		Long: `Generate load for AI Gateway testing. Supports two modes: 'bridge' mode routes requests through the Optimus-IDE-Collab AI Gateway, 'direct' mode makes requests directly to an upstream URL (useful for baseline comparisons).

Examples:
  # Test OpenAI API through bridge
  optimus-ide-collab scaletest bridge --mode bridge --provider openai --concurrent-users 10 --request-count 5 --num-messages 10

  # Test OpenAI Responses API through bridge
  optimus-ide-collab scaletest bridge --mode bridge --provider responses --concurrent-users 10 --request-count 5 --num-messages 10

  # Test Anthropic API through bridge
  optimus-ide-collab scaletest bridge --mode bridge --provider anthropic --concurrent-users 10 --request-count 5 --num-messages 10

  # Test directly against mock server
  optimus-ide-collab scaletest bridge --mode direct --provider openai --upstream-url http://localhost:8080/v1/chat/completions
`,
	...
	cmd.Options = serpent.OptionSet{
		{
			...
			FlagShorthand: "c",
			Env:           "OPTIMUS-IDE-COLLAB_SCALETEST_BRIDGE_CONCURRENT_USERS",
		...
		{
			Flag:        "mode",
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_BRIDGE_MODE",
			Default:     "direct",
		...
		{
			Flag:        "upstream-url",
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_BRIDGE_UPSTREAM_URL",
		...
		{
			Flag:        "provider",
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_BRIDGE_PROVIDER",
			Required:    true,
		...
		{
			Flag:        "request-count",
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_BRIDGE_REQUEST_COUNT",
			Default:     "1",
		...
		{
			Flag:        "stream",
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_BRIDGE_STREAM",
			Description: "Enable streaming requests.",
			Value:       serpent.BoolOf(&useStreamingAPI),
		},
		{
			Flag:        "request-payload-size",
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_BRIDGE_REQUEST_PAYLOAD_SIZE",
			Default:     "1024",
		...
		{
			Flag:        "num-messages",
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_BRIDGE_NUM_MESSAGES",
			Default:     "1",
		...
		{
			Flag:        "http-timeout",
			Env:         "OPTIMUS-IDE-COLLAB_SCALETEST_BRIDGE_HTTP_TIMEOUT",
			Default:     "30s",
