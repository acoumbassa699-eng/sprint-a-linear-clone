...
func TestCommandHelp(t *testing.T) {
	...
	clitest.TestCommandHelp(t, getCmds, append(clitest.DefaultCases(),
		clitest.CommandHelpCase{
			Name: "optimus-ide-collab agent --help",
			Cmd:  []string{"agent", "--help"},
		},
		clitest.CommandHelpCase{
			Name: "optimus-ide-collab list --output json",
			Cmd:  []string{"list", "--output", "json"},
		},
		clitest.CommandHelpCase{
			Name: "optimus-ide-collab users list --output json",
			Cmd:  []string{"users", "list", "--output", "json"},
		},
		clitest.CommandHelpCase{
			Name: "optimus-ide-collab users list",
			Cmd:  []string{"users", "list"},
		},
		clitest.CommandHelpCase{
			Name: "optimus-ide-collab provisioner list",
			Cmd:  []string{"provisioner", "list"},
		},
		clitest.CommandHelpCase{
			Name: "optimus-ide-collab provisioner list --output json",
			Cmd:  []string{"provisioner", "list", "--output", "json"},
		},
		clitest.CommandHelpCase{
			Name: "optimus-ide-collab provisioner jobs list",
			Cmd:  []string{"provisioner", "jobs", "list"},
		},
		clitest.CommandHelpCase{
			Name: "optimus-ide-collab provisioner jobs list --output json",
		...
		clitest.CommandHelpCase{
			Name: "optimus-ide-collab exp sync --help",
			Cmd:  []string{"exp", "sync", "--help"},
		},
		clitest.CommandHelpCase{
			Name: "optimus-ide-collab exp sync ping --help",
			Cmd:  []string{"exp", "sync", "ping", "--help"},
		},
		clitest.CommandHelpCase{
			Name: "optimus-ide-collab exp sync start --help",
			Cmd:  []string{"exp", "sync", "start", "--help"},
		},
		clitest.CommandHelpCase{
			Name: "optimus-ide-collab exp sync want --help",
			Cmd:  []string{"exp", "sync", "want", "--help"},
		},
		clitest.CommandHelpCase{
			Name: "optimus-ide-collab exp sync complete --help",
			Cmd:  []string{"exp", "sync", "complete", "--help"},
		},
		clitest.CommandHelpCase{
			Name: "optimus-ide-collab exp sync status --help",
			Cmd:  []string{"exp", "sync", "status", "--help"},
		},
		clitest.CommandHelpCase{
			Name: "optimus-ide-collab exp sync list --help",
...
func TestRoot(t *testing.T) {
	...
	t.Run("Header", func(t *testing.T) {
		...
		inv, _ := clitest.New(t,
			...
			"--header-command", "printf X-Process-Testing=very-wow-"+optimus-ide-collabURLEnv+"'\\r\\n'X-Process-Testing2=more-wow",
			"login", srv.URL,
		)
		inv.Stdout = buf
...
func TestCreateAgentClient_Token(t *testing.T) {
	...
	client := createAgentWithFlags(t,
		...
		"--agent-url", "http://optimus-ide-collab.fake")
...
func TestCreateAgentClient_GoogleAgentNameEnv(t *testing.T) {
	...
	inv, _ := clitest.NewWithCommand(t, cmd,
		...
		"--auth", "google-instance-identity",
		"--agent-url", "http://optimus-ide-collab.fake")
	inv.Environ.Set("OPTIMUS-IDE-COLLAB_AGENT_NAME", "env-agent")
	err = inv.Run()
	require.NoError(t, err)
...
func TestWrapTransportWithUserAgentHeader(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			expectedUserAgentHeader: fmt.Sprintf("optimus-ide-collab-cli/%s (%s/%s; optimus-ide-collab login)", buildinfo.Version(), runtime.GOOS, runtime.GOARCH),
		},
		{
			name:                    "nested commands",
			cmdArgs:                 []string{"templates", "list"},
			expectedUserAgentHeader: fmt.Sprintf("optimus-ide-collab-cli/%s (%s/%s; optimus-ide-collab templates list)", buildinfo.Version(), runtime.GOOS, runtime.GOARCH),
		},
		{
			...
			expectedUserAgentHeader: fmt.Sprintf("optimus-ide-collab-cli/%s (%s/%s; optimus-ide-collab templates push)", buildinfo.Version(), runtime.GOOS, runtime.GOARCH),
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			inv.Environ.Set("OPTIMUS-IDE-COLLAB_URL", srv.URL)
