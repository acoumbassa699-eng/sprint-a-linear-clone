...
func buildFromEnv(t *testing.T, cfg optimus-ide-collabsdk.AIBridgeConfig) ([]aibridge.Provider, error) {
	t.Helper()
	db, _ := dbtestutil.NewDB(t)
	ctx := testutil.Context(t, testutil.WaitShort)
	logger := slogtest.Make(t, nil)
	if err := optimus-ide-collabd.SeedAIProvidersFromEnv(ctx, db, cfg, logger); err != nil {
		return nil, err
	}
...
func buildFromDB(ctx context.Context, t *testing.T, db database.Store, cfg optimus-ide-collabsdk.AIBridgeConfig, logger slog.Logger) ([]aibridge.Provider, []aibridged.ProviderOutcome, error) {
	t.Helper()
...
func TestBuildProviders(t *testing.T) {
	...
	t.Run("EmptyConfig", func(t *testing.T) {
		t.Parallel()
		providers, err := buildFromEnv(t, optimus-ide-collabsdk.AIBridgeConfig{})
	...
	t.Run("IndexedOnly", func(t *testing.T) {
		t.Parallel()
		cfg := optimus-ide-collabsdk.AIBridgeConfig{
			Providers: []optimus-ide-collabsdk.AIProviderConfig{
				{
	...
	t.Run("LegacyBedrockWithoutAnthropicKey", func(t *testing.T) {
		...
		// Anthropic provider. No OPTIMUS-IDE-COLLAB_AIBRIDGE_ANTHROPIC_KEY needed.
		cfg := optimus-ide-collabsdk.AIBridgeConfig{}
		cfg.LegacyBedrock.Region = serpent.String("us-west-2")
		cfg.LegacyBedrock.AccessKey = serpent.String("AKID")
		cfg.LegacyBedrock.AccessKeySecret = serpent.String("secret")
	...
	t.Run("NativeAnthropicDefaultBaseURL", func(t *testing.T) {
		...
		assert.Nil(t, bedrockConfig(row.BaseUrl, optimus-ide-collabsdk.AIProviderSettings{}.Bedrock))
	})
...
func TestBuildProvidersSkipsBadRows(t *testing.T) {
	...
	t.Run("CorruptSettings", func(t *testing.T) {
		...
		providers, outcomes, err := buildFromDB(ctx, t, db, optimus-ide-collabsdk.AIBridgeConfig{}, logger)
		require.NoError(t, err)
		assert.Empty(t, providers)
