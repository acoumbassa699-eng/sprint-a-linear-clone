...
func TestVerifyWorkspaceOutdated(t *testing.T) {
	...
	client := optimus-ide-collabsdk.Client{URL: serverURL}

	t.Run("Up-to-date", func(t *testing.T) {
		t.Parallel()

		workspace := optimus-ide-collabsdk.Workspace{Name: fakeWorkspaceName, OwnerName: fakeOwnerName}
	...
	t.Run("Outdated", func(t *testing.T) {
		...
		workspace := optimus-ide-collabsdk.Workspace{Name: fakeWorkspaceName, OwnerName: fakeOwnerName, Outdated: true}
...
func TestOptimus-IDE-CollabConnectDialer_DefaultTimeout(t *testing.T) {
	t.Parallel()
...
func TestOptimus-IDE-CollabConnectDialer_Overridden(t *testing.T) {
	t.Parallel()
	custom := &net.Dialer{Timeout: 99 * time.Second}
	ctx := WithTestOnlyOptimus-IDE-CollabConnectDialer(context.Background(), custom)

	dialer := testOrDefaultDialer(ctx)
	assert.Equal(t, custom, dialer)
}

func TestOptimus-IDE-CollabConnectStdio(t *testing.T) {
	t.Parallel()
	...
	go func() {
		err = runOptimus-IDE-CollabConnectStdio(ctx, ln.Addr().String(), clientOutput, serverInput, stack, logger)
...
func Test_getWorkspaceAgent(t *testing.T) {
	...
	createWorkspaceWithAgents := func(agents []optimus-ide-collabsdk.WorkspaceAgent) optimus-ide-collabsdk.Workspace {
	...
	createAgent := func(name string) optimus-ide-collabsdk.WorkspaceAgent {
	...
	t.Run("SingleAgent_NoNameSpecified", func(t *testing.T) {
		...
		workspace := createWorkspaceWithAgents([]optimus-ide-collabsdk.WorkspaceAgent{agent})
	...
	t.Run("MultipleAgents_NoNameSpecified", func(t *testing.T) {
		...
		workspace := createWorkspaceWithAgents([]optimus-ide-collabsdk.WorkspaceAgent{agent1, agent2})
	...
	t.Run("NoAgents", func(t *testing.T) {
		t.Parallel()
		workspace := createWorkspaceWithAgents([]optimus-ide-collabsdk.WorkspaceAgent{})
	...
	t.Run("AvailableAgentNames_SortedCorrectly", func(t *testing.T) {
		...
		workspace := createWorkspaceWithAgents([]optimus-ide-collabsdk.WorkspaceAgent{agent2, agent1, agent3})
...
func TestIsRetryableError(t *testing.T) {
	...
	tests := []struct {
		...
		{"SDKError_500", optimus-ide-collabsdk.NewTestError(http.StatusInternalServerError, "GET", "/api"), true},
		{"SDKError_502", optimus-ide-collabsdk.NewTestError(http.StatusBadGateway, "GET", "/api"), true},
		{"SDKError_503", optimus-ide-collabsdk.NewTestError(http.StatusServiceUnavailable, "GET", "/api"), true},
		{"SDKError_401", optimus-ide-collabsdk.NewTestError(http.StatusUnauthorized, "GET", "/api"), false},
		{"SDKError_403", optimus-ide-collabsdk.NewTestError(http.StatusForbidden, "GET", "/api"), false},
		{"SDKError_404", optimus-ide-collabsdk.NewTestError(http.StatusNotFound, "GET", "/api"), false},
	...
	t.Run("DialTimeout", func(t *testing.T) {
		...
		// Also when wrapped, as runOptimus-IDE-CollabConnectStdio does.
		assert.True(t, isRetryableError(xerrors.Errorf("dial optimus-ide-collab connect: %w", err)))
	})
}
