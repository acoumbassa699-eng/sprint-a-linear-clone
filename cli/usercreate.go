	...
	cmd := &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) error {
			if serviceAccount {
				...
				case loginType == string(optimus-ide-collabsdk.LoginTypeNone):
			...
			if username == "" {
				username, err = cliui.Prompt(inv, cliui.PromptOptions{
					...
					Validate: func(username string) error {
						err = optimus-ide-collabsdk.NameValid(username)
						if err != nil {
			...
			if name == "" && shouldPromptName {
				...
				name = optimus-ide-collabsdk.NormalizeRealUsername(rawName)
			...
			if disableLogin || serviceAccount {
				...
				userLoginType = optimus-ide-collabsdk.LoginType(loginType)
			}

			if password == "" && userLoginType == optimus-ide-collabsdk.LoginTypePassword {
			...
			_, err = client.CreateUserWithOrgs(inv.Context(), optimus-ide-collabsdk.CreateUserRequestWithOrgs{
				Email:           email,
				Username:        username,
				Name:            name,
				Password:        password,
			...
			switch optimus-ide-collabsdk.LoginType(strings.ToLower(string(userLoginType))) {
...
https://github.com/optimus-ide-collab/optimus-ide-collab/releases

Run `+pretty.Sprint(cliui.DefaultStyles.Code, "optimus-ide-collab login "+client.URL.String())+` to authenticate.

Your email is: `+pretty.Sprint(cliui.DefaultStyles.Field, email)+`
`+authenticationMethod+`

Create a workspace  `+pretty.Sprint(cliui.DefaultStyles.Code, "optimus-ide-collab create")+`!`)
			return nil
		},
	}
	cmd.Options = serpent.OptionSet{
		...
		},
		{
			...
			Description: fmt.Sprintf("Optionally specify the login type for the user. Valid values are: %s. "+
				...
				strings.Join([]string{
					string(optimus-ide-collabsdk.LoginTypePassword), string(optimus-ide-collabsdk.LoginTypeNone), string(optimus-ide-collabsdk.LoginTypeGithub), string(optimus-ide-collabsdk.LoginTypeOIDC),
				}, ", ",
				)),
