...
import (
	...
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabd/util/tz"
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			if len(inv.Args) == 1 {
				...
				if strings.Contains(inv.Args[0], "/") {
					_, workspaceName, err := optimus-ide-collabsdk.SplitWorkspaceIdentifier(inv.Args[0])
					if err != nil {
						return err
					}
	...
	cmd := &serpent.Command{
		...
		Long: scheduleStartDescriptionLong + "\n" + FormatExamples(
			Example{
				...
				Command:     "optimus-ide-collab schedule start my-workspace 9:30AM Mon-Fri Europe/Dublin",
			},
		),
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			err = client.UpdateWorkspaceAutostart(inv.Context(), workspace.ID, optimus-ide-collabsdk.UpdateWorkspaceAutostartRequest{
				Schedule: schedStr,
			})
			if err != nil {
				return err
...
func (r *RootCmd) scheduleStop() *serpent.Command {
	return &serpent.Command{
		...
		Long: scheduleStopDescriptionLong + "\n" + FormatExamples(
			Example{
				Command: "optimus-ide-collab schedule stop my-workspace 2h30m",
			},
		),
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			if err := client.UpdateWorkspaceTTL(inv.Context(), workspace.ID, optimus-ide-collabsdk.UpdateWorkspaceTTLRequest{
				TTLMillis: durMillis,
			}); err != nil {
				return err
			}
...
func (r *RootCmd) scheduleExtend() *serpent.Command {
	extendCmd := &serpent.Command{
		...
		Long: scheduleExtendDescriptionLong + "\n" + FormatExamples(
			Example{
				Command: "optimus-ide-collab schedule extend my-workspace 90m",
			},
		),
		Middleware: serpent.Chain(
			serpent.RequireNArgs(2),
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			if err := client.PutExtendWorkspace(inv.Context(), workspace.ID, optimus-ide-collabsdk.PutExtendWorkspaceRequest{
...
func displaySchedule(ws optimus-ide-collabsdk.Workspace, out io.Writer) error {
...
func scheduleListRowFromWorkspace(now time.Time, workspace optimus-ide-collabsdk.Workspace) scheduleListRow {
	autostartDisplay := ""
	nextStartDisplay := ""
	...
	if !ptr.NilOrZero(workspace.TTLMillis) {
		...
		if !workspace.LatestBuild.Deadline.IsZero() && workspace.LatestBuild.Transition == optimus-ide-collabsdk.WorkspaceTransitionStart {
