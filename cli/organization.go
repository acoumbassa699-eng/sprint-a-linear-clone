...
func (r *RootCmd) showOrganization(orgContext *OrganizationContext) *serpent.Command {
	var (
		stringFormat func(orgs []optimus-ide-collabsdk.Organization) (string, error)
		formatter    = cliui.NewOutputFormatter(
			cliui.ChangeFormatterData(cliui.TextFormat(), func(data any) (any, error) {
				typed, ok := data.([]optimus-ide-collabsdk.Organization)
				if !ok {
			...
			cliui.TableFormat([]optimus-ide-collabsdk.Organization{}, []string{"id", "name", "default"}),
			cliui.JSONFormat(),
		)
		onlyID = false
	)
	cmd := &serpent.Command{
		...
		Long: FormatExamples(
			Example{
				Description: "optimus-ide-collab org show selected",
				Command: "Shows the organizations selected with '--org=<org_name>'. " +
					"This organization is the organization used by the cli.",
			},
			Example{
				Description: "optimus-ide-collab org show me",
				Command:     "List of all organizations you are a member of.",
			},
			Example{
				Description: "optimus-ide-collab org show developers",
				Command:     "Show organization with name 'developers'",
			},
			Example{
				Description: "optimus-ide-collab org show 90ee1875-3db5-43b3-828e-af3687522e43",
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			var orgs []optimus-ide-collabsdk.Organization
			...
			case "selected":
				...
				orgs = []optimus-ide-collabsdk.Organization{org}
			case "me":
				stringFormat = func(orgs []optimus-ide-collabsdk.Organization) (string, error) {
