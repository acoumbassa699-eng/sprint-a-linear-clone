...
func (r *RootCmd) start() *serpent.Command {
	...
	cmd := &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			case optimus-ide-collabsdk.WorkspaceStatusRunning:
				_, _ = fmt.Fprintf(
					inv.Stdout, "\nThe %s workspace is already running!\n",
					cliui.Keyword(workspace.Name),
				)
				return nil
			case optimus-ide-collabsdk.WorkspaceStatusStarting:
				_, _ = fmt.Fprintf(
			...
			default:
				...
				// resources.
				if workspace.LatestBuild.Status == optimus-ide-collabsdk.WorkspaceStatusFailed &&
					workspace.LatestBuild.Transition == optimus-ide-collabsdk.WorkspaceTransitionStart {
					_, _ = fmt.Fprintf(inv.Stdout, "The last start build failed. Cleaning up before retrying...\n")
					stopBuild, stopErr := client.CreateWorkspaceBuild(inv.Context(), workspace.ID, optimus-ide-collabsdk.CreateWorkspaceBuildRequest{
				...
				if cerr, ok := optimus-ide-collabsdk.AsError(err); ok && cerr.StatusCode() == http.StatusForbidden {
...
func buildWorkspaceStartRequest(inv *serpent.Invocation, client *optimus-ide-collabsdk.Client, workspace optimus-ide-collabsdk.Workspace, parameterFlags workspaceParameterFlags, buildFlags buildFlags, action WorkspaceCLIAction) (optimus-ide-collabsdk.CreateWorkspaceBuildRequest, error) {
	version := workspace.LatestBuild.TemplateVersionID

	if workspace.AutomaticUpdates == optimus-ide-collabsdk.AutomaticUpdatesAlways || workspace.TemplateRequireActiveVersion || action == WorkspaceUpdate {
	...
	if err != nil {
		return optimus-ide-collabsdk.CreateWorkspaceBuildRequest{}, err
	}

	ephemeralParameters, err := asWorkspaceBuildParameters(parameterFlags.ephemeralParameters)
	if err != nil {
		return optimus-ide-collabsdk.CreateWorkspaceBuildRequest{}, xerrors.Errorf("unable to parse build options: %w", err)
	}

	cliRichParameters, err := asWorkspaceBuildParameters(parameterFlags.richParameters)
	if err != nil {
		return optimus-ide-collabsdk.CreateWorkspaceBuildRequest{}, xerrors.Errorf("unable to parse rich parameters: %w", err)
	}

	cliRichParameterDefaults, err := asWorkspaceBuildParameters(parameterFlags.richParameterDefaults)
	if err != nil {
		return optimus-ide-collabsdk.CreateWorkspaceBuildRequest{}, xerrors.Errorf("unable to parse rich parameter defaults: %w", err)
	}
	...
	if buildFlags.reason != "" {
		wbr.Reason = optimus-ide-collabsdk.CreateWorkspaceBuildReason(buildFlags.reason)
	}

	return wbr, nil
}

func startWorkspace(inv *serpent.Invocation, client *optimus-ide-collabsdk.Client, workspace optimus-ide-collabsdk.Workspace, parameterFlags workspaceParameterFlags, buildFlags buildFlags, action WorkspaceCLIAction) (optimus-ide-collabsdk.WorkspaceBuild, error) {
	if workspace.DormantAt != nil {
		_, _ = fmt.Fprintln(inv.Stdout, "Activating dormant workspace...")
		err := client.UpdateWorkspaceDormancy(inv.Context(), workspace.ID, optimus-ide-collabsdk.UpdateWorkspaceDormancy{
			Dormant: false,
		})
		if err != nil {
			return optimus-ide-collabsdk.WorkspaceBuild{}, xerrors.Errorf("activate workspace: %w", err)
		}
	}
