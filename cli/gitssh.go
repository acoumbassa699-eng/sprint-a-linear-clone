...
func gitssh() *serpent.Command {
	...
	cmd := &serpent.Command{
		Use:    "gitssh",
		Hidden: true,
		Short:  `Wraps the "ssh" command and uses the optimus-ide-collab gitssh key for authentication`,
		Handler: func(inv *serpent.Invocation) error {
			...
			privateKeyFile, err := os.CreateTemp("", "optimus-ide-collab-gitsshkey-*")
			if err != nil {
				...
				if xerrors.As(err, &exitErr) && exitErr.ExitCode() == 255 {
					_, _ = fmt.Fprintln(inv.Stderr,
						"\n"+pretty.Sprintf(
							...
							"Optimus-IDE-Collab authenticates with "+pretty.Sprint(cliui.DefaultStyles.Field, "git")+
...
// Example invocation:
//
//	ssh -G -o SendEnv=GIT_PROTOCOL git@github.com git-upload-pack 'optimus-ide-collab/optimus-ide-collab'
//
// The extra arguments work without issue and lets us run the command
// as-is without stripping out the excess (git-upload-pack 'optimus-ide-collab/optimus-ide-collab').
