...
func Test_Tasks(t *testing.T) {
	...
	for _, tc := range []struct {
		name     string
		cmdArgs  []string
		assertFn func(stdout string, userClient *optimus-ide-collabsdk.Client)
	}{
		{
			name:    "create task",
		...
		},
		{
			...
			assertFn: func(stdout string, userClient *optimus-ide-collabsdk.Client) {
				...
				err := json.NewDeoptimus-ide-collab(strings.NewReader(stdout)).Decode(&tasks)
				...
				ws := optimus-ide-collabdtest.MustWorkspace(t, userClient, tasks[0].WorkspaceID.UUID)
				optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, client, ws.LatestBuild.ID)
				agentClient := agentsdk.New(client.URL, agentsdk.WithFixedToken(authToken))
				_ = agenttest.New(t, client.URL, authToken, func(o *agent.Options) {
					o.Client = agentClient
				})
				optimus-ide-collabdtest.NewWorkspaceAgentWaiter(t, userClient, tasks[0].WorkspaceID.UUID).WithContext(ctx).WaitFor(optimus-ide-collabdtest.AgentsReady)
		...
		},
		{
			...
			assertFn: func(stdout string, userClient *optimus-ide-collabsdk.Client) {
				var task optimus-ide-collabsdk.Task
				require.NoError(t, json.NewDeoptimus-ide-collab(strings.NewReader(stdout)).Decode(&task), "should unmarshal task status")
				require.Equal(t, task.Name, taskName, "task name should match")
				require.Equal(t, optimus-ide-collabsdk.TaskStatusActive, task.Status, "task should be active")
			},
		},
		{
		...
		},
		{
			...
			assertFn: func(stdout string, userClient *optimus-ide-collabsdk.Client) {
				var logs []optimus-ide-collabsdk.TaskLogEntry
				require.NoError(t, json.NewDeoptimus-ide-collab(strings.NewReader(stdout)).Decode(&logs), "should unmarshal task logs")
				...
				require.Equal(t, logs[0].Type, optimus-ide-collabsdk.TaskLogTypeInput, "first message should be an input")
				require.Equal(t, logs[1].Content, "hello", "second message should be the sent message")
				require.Equal(t, logs[1].Type, optimus-ide-collabsdk.TaskLogTypeInput, "second message should be an input")
				require.Equal(t, logs[2].Content, "hello", "third message should be the echoed message")
				require.Equal(t, logs[2].Type, optimus-ide-collabsdk.TaskLogTypeOutput, "third message should be an output")
			},
		},
		{
			name:    "pause task",
		...
		},
		{
			...
			assertFn: func(stdout string, userClient *optimus-ide-collabsdk.Client) {
				...
				require.Equal(t, optimus-ide-collabsdk.TaskStatusPaused, task.Status, "task should be paused")
			},
		},
		{
			name:    "resume task",
		...
		},
		{
			...
			assertFn: func(stdout string, userClient *optimus-ide-collabsdk.Client) {
				...
				require.NoError(t, json.NewDeoptimus-ide-collab(strings.NewReader(stdout)).Decode(&task), "should unmarshal task status")
				require.Equal(t, taskName, task.Name, "task name should match")
				require.Equal(t, optimus-ide-collabsdk.TaskStatusInitializing, task.Status, "task should be initializing after resume")
			},
		},
		{
			name:    "delete task",
			...
			assertFn: func(stdout string, userClient *optimus-ide-collabsdk.Client) {
				...
				testutil.Eventually(ctx, t, func(ctx context.Context) bool {
					tasks, err := userClient.Tasks(ctx, &optimus-ide-collabsdk.TasksFilter{})
					if !assert.NoError(t, err) {
						return false
					}
					return slices.IndexFunc(tasks, func(task optimus-ide-collabsdk.Task) bool {
...
func fakeAgentAPIEcho(ctx context.Context, t testing.TB, initMsg agentapisdk.Message, want ...string) map[string]http.HandlerFunc {
	...
	writeAgentAPIError := func(w http.ResponseWriter, err error, status int) {
		...
		_ = json.NewEnoptimus-ide-collab(w).Encode(agentapisdk.ErrorModel{
	...
	return map[string]http.HandlerFunc{
		"/status": func(w http.ResponseWriter, r *http.Request) {
			...
			_ = json.NewEnoptimus-ide-collab(w).Encode(agentapisdk.GetStatusResponse{
				Status: "stable",
			})
		},
		...
		"/message": func(w http.ResponseWriter, r *http.Request) {
			...
			w.Header().Set("Content-Type", "application/json")
			err := json.NewDeoptimus-ide-collab(r.Body).Decode(&params)
			...
			assert.NoError(t, json.NewEnoptimus-ide-collab(w).Encode(agentapisdk.PostMessageResponse{
				Ok: true,
			}))
		},
	}
...
type setupCLITaskTestResult struct {
	ownerClient *optimus-ide-collabsdk.Client
	userClient  *optimus-ide-collabsdk.Client
	task        optimus-ide-collabsdk.Task
	agentToken  string
	agent       agent.Agent
}
...
func setupCLITaskTest(ctx context.Context, t *testing.T, agentAPIHandlers map[string]http.HandlerFunc, opts ...setupCLITaskTestOpt) setupCLITaskTestResult {
	...
	userClient, _ := optimus-ide-collabdtest.CreateAnotherUser(t, ownerClient, owner.OrganizationID)
	...
	task, err := userClient.CreateTask(ctx, optimus-ide-collabsdk.Me, optimus-ide-collabsdk.CreateTaskRequest{
	...
	optimus-ide-collabdtest.NewWorkspaceAgentWaiter(t, userClient, workspace.ID).
		WaitFor(optimus-ide-collabdtest.AgentsReady)
...
func pauseTask(ctx context.Context, t *testing.T, client *optimus-ide-collabsdk.Client, task optimus-ide-collabsdk.Task) {
	t.Helper()
...
func resumeTask(ctx context.Context, t *testing.T, client *optimus-ide-collabsdk.Client, task optimus-ide-collabsdk.Task) {
	t.Helper()

	resumeResp, err := client.ResumeTask(ctx, task.OwnerName, task.ID)
	require.NoError(t, err)
	require.NotNil(t, resumeResp.WorkspaceBuild)
	optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, client, resumeResp.WorkspaceBuild.ID)
}
...
func setupCLITaskTestWithSnapshot(ctx context.Context, t *testing.T, status optimus-ide-collabsdk.TaskStatus, messages []agentapisdk.Message) (*optimus-ide-collabsdk.Client, optimus-ide-collabsdk.Task) {
	t.Helper()
...
// set up database state and don't need actual provisioning. This also avoids potential
// interference from the provisioner daemon polling for jobs.
func setupCLITaskTestWithoutSnapshot(t *testing.T, status optimus-ide-collabsdk.TaskStatus) (*optimus-ide-collabsdk.Client, optimus-ide-collabsdk.Task) {
	t.Helper()
...
func createTaskInStatus(t *testing.T, db database.Store, orgID, ownerID uuid.UUID, status optimus-ide-collabsdk.TaskStatus) optimus-ide-collabsdk.Task {
	t.Helper()
	...
	switch status {
	case optimus-ide-collabsdk.TaskStatusPending:
		builder = builder.Pending()
	case optimus-ide-collabsdk.TaskStatusInitializing:
		builder = builder.Starting()
	case optimus-ide-collabsdk.TaskStatusPaused:
...
func createAITaskTemplate(t *testing.T, client *optimus-ide-collabsdk.Client, orgID uuid.UUID, opts ...aiTemplateOpt) optimus-ide-collabsdk.Template {
	t.Helper()
...
func startFakeAgentAPI(t *testing.T, handlers map[string]http.HandlerFunc) *fakeAgentAPI {
	...
	// Known agentapi endpoints without a handler fail the test: a optimus-ide-collabd
