...
func (r *RootCmd) userList() *serpent.Command {
	formatter := cliui.NewOutputFormatter(
		cliui.TableFormat([]optimus-ide-collabsdk.User{}, []string{"username", "email", "created at", "status"}),
		cliui.JSONFormat(),
	)
	...
	cmd := &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			req := optimus-ide-collabsdk.UsersRequest{}
			if githubUserID != 0 {
...
func (r *RootCmd) userSingle() *serpent.Command {
	...
	cmd := &serpent.Command{
		...
		Long: FormatExamples(
			Example{
				Command: "optimus-ide-collab users show me",
			},
		),
		Middleware: serpent.Chain(
