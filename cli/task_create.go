	...
	cmd := &serpent.Command{
		...
		Long: FormatExamples(
			Example{
				...
				Command:     "optimus-ide-collab task create \"Add authentication to the user service\"",
			},
			Example{
				Description: "Create a task with stdin input",
				Command:     "echo \"Add authentication to the user service\" | optimus-ide-collab task create",
			},
			Example{
				Description: "Create a task with a specific name",
				Command:     "optimus-ide-collab task create --name task1 \"Add authentication to the user service\"",
			},
			Example{
				Description: "Create a task from a specific template / preset",
				Command:     "optimus-ide-collab task create --template backend-dev --preset \"My Preset\" \"Add authentication to the user service\"",
			},
			Example{
				Description: "Create a task for another user (requires appropriate permissions)",
				Command:     "optimus-ide-collab task create --owner user@example.com \"Add authentication to the user service\"",
			},
		),
		...
		Options: serpent.OptionSet{
			...
			},
			{
				...
				Required:    false,
				Default:     optimus-ide-collabsdk.Me,
			},
			{
				Name:  "template",
				Flag:  "template",
				Env:   "OPTIMUS-IDE-COLLAB_TASK_TEMPLATE_NAME",
				Value: serpent.StringOf(&templateName),
			},
			{
				Name:  "template-version",
				Flag:  "template-version",
				Env:   "OPTIMUS-IDE-COLLAB_TASK_TEMPLATE_VERSION",
				Value: serpent.StringOf(&templateVersionName),
			},
			{
				Name:    "preset",
				Flag:    "preset",
				Env:     "OPTIMUS-IDE-COLLAB_TASK_PRESET_NAME",
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			switch {
			case templateName == "":
				templates, err := client.Templates(ctx, optimus-ide-collabsdk.TemplateFilter{SearchQuery: "has-ai-task:true", OrganizationID: organization.ID})
				if err != nil {
			...
			task, err := client.CreateTask(ctx, ownerArg, optimus-ide-collabsdk.CreateTaskRequest{
				Name:                    taskName,
