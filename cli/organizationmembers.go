...
func (r *RootCmd) assignOrganizationRoles(orgContext *OrganizationContext) *serpent.Command {
	cmd := &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			member, err := client.UpdateOrganizationMemberRoles(ctx, organization.ID, userIdentifier, optimus-ide-collabsdk.UpdateRoles{
				Roles: roles,
			})
			if err != nil {
...
func (r *RootCmd) listOrganizationMembers(orgContext *OrganizationContext) *serpent.Command {
	formatter := cliui.NewOutputFormatter(
		cliui.TableFormat([]optimus-ide-collabsdk.OrganizationMemberWithUserData{}, []string{"username", "organization roles"}),
		cliui.JSONFormat(),
	)
