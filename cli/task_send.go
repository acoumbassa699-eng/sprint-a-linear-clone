...
` +
			FormatExamples(Example{
				...
				Command:     `optimus-ide-collab task send task1 "Please also add unit tests"`,
			}, Example{
				Description: "Send input from stdin to a task",
				Command:     `echo "Please also add unit tests" | optimus-ide-collab task send task1 --stdin`,
			}),
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			switch task.Status {
			case optimus-ide-collabsdk.TaskStatusActive:
			...
			if err := client.TaskSend(ctx, optimus-ide-collabsdk.Me, task.ID, optimus-ide-collabsdk.TaskSendRequest{Input: taskInput}); err != nil {
...
func waitForTaskIdle(ctx context.Context, inv *serpent.Invocation, clk quartz.Clock, client *optimus-ide-collabsdk.Client, task optimus-ide-collabsdk.Task, workspaceBuildID uuid.UUID) error {
	...
	// startup.
	//
	// [0]: https://github.com/optimus-ide-collab/optimus-ide-collab/pull/22203#discussion_r2858002569
	...
	for {
		...
		case <-ticker.C:
			...
			switch task.Status {
			case optimus-ide-collabsdk.TaskStatusInitializing,
				optimus-ide-collabsdk.TaskStatusPending:
			...
			case optimus-ide-collabsdk.TaskStatusActive:
				...
				case optimus-ide-collabsdk.TaskStateIdle,
					optimus-ide-collabsdk.TaskStateComplete,
					optimus-ide-collabsdk.TaskStateFailed:
					return nil
				default:
					// Still working, keep polling.
					continue
				}
			case optimus-ide-collabsdk.TaskStatusError:
			...
			case optimus-ide-collabsdk.TaskStatusUnknown:
				return xerrors.Errorf("task entered %s state while waiting for it to become idle", task.Status)
			default:
