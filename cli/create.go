...
type CreateOptions struct {
	BeforeCreate func(ctx context.Context, client *optimus-ide-collabsdk.Client, template optimus-ide-collabsdk.Template, templateVersionID uuid.UUID) error
	AfterCreate  func(ctx context.Context, inv *serpent.Invocation, client *optimus-ide-collabsdk.Client, workspace optimus-ide-collabsdk.Workspace) error
}
...
func (r *RootCmd) Create(opts CreateOptions) *serpent.Command {
	...
	cmd := &serpent.Command{
		...
		Long: FormatExamples(
			Example{
				...
				Command:     "optimus-ide-collab create <username>/<workspace_name>",
			},
		),
		Handler: func(inv *serpent.Invocation) error {
			...
			workspaceOwner := optimus-ide-collabsdk.Me
			if len(inv.Args) >= 1 {
				workspaceOwner, workspaceName, err = optimus-ide-collabsdk.SplitWorkspaceIdentifier(inv.Args[0])
				if err != nil {
					return err
				}
			}
			...
			if workspaceName == "" {
				workspaceName, err = cliui.Prompt(inv, cliui.PromptOptions{
					...
					Validate: func(workspaceName string) error {
						err = optimus-ide-collabsdk.NameValid(workspaceName)
						if err != nil {
							return xerrors.Errorf("workspace name %q is invalid: %w", workspaceName, err)
						}
						_, err = client.WorkspaceByOwnerAndName(inv.Context(), workspaceOwner, workspaceName, optimus-ide-collabsdk.WorkspaceOptions{})
						if err == nil {
			...
			var sourceWorkspace optimus-ide-collabsdk.Workspace
			if copyParametersFrom != "" {
				sourceWorkspaceOwner, sourceWorkspaceName, err := optimus-ide-collabsdk.SplitWorkspaceIdentifier(copyParametersFrom)
				if err != nil {
					return err
				}

				sourceWorkspace, err = client.WorkspaceByOwnerAndName(inv.Context(), sourceWorkspaceOwner, sourceWorkspaceName, optimus-ide-collabsdk.WorkspaceOptions{})
				if err != nil {
					return xerrors.Errorf("get source workspace: %w", err)
				}

				_, _ = fmt.Fprintf(inv.Stdout, "Optimus-IDE-Collab will use the same template %q as the source workspace.\n", sourceWorkspace.TemplateName)
			...
			case templateName == "":
				...
				slices.SortFunc(templates, func(a, b optimus-ide-collabsdk.Template) int {
					return slice.Descending(a.ActiveUserCount, b.ActiveUserCount)
				})

				templateNames := make([]string, 0, len(templates))
				templateByName := make(map[string]optimus-ide-collabsdk.Template, len(templates))
			...
			default:
				...
				if len(templates) > 1 {
					...
					index := slices.IndexFunc(templates, func(i optimus-ide-collabsdk.Template) bool {
					...
					templates = []optimus-ide-collabsdk.Template{templates[index]}
				}
			...
			if orgValue != "" && !(orgValueSource == serpent.ValueSourceDefault || orgValueSource == serpent.ValueSourceNone) {
				...
				if template.OrganizationID != selectedOrg.ID {
					...
					if orgValueSource == serpent.ValueSourceEnv {
						orgNameFormat = "OPTIMUS-IDE-COLLAB_ORGANIZATION=%q"
					}
			...
			var sourceWorkspaceParameters []optimus-ide-collabsdk.WorkspaceBuildParameter
			if copyParametersFrom != "" {
			...
			var preset *optimus-ide-collabsdk.Preset
			var presetParameters []optimus-ide-collabsdk.WorkspaceBuildParameter
			...
			req := optimus-ide-collabsdk.CreateWorkspaceRequest{
				...
				AutomaticUpdates:    optimus-ide-collabsdk.AutomaticUpdates(autoUpdates),
			}
	...
	cmd.Options = append(cmd.Options,
		...
		serpent.Option{
			Flag:        "start-at",
			Env:         "OPTIMUS-IDE-COLLAB_WORKSPACE_START_AT",
			Description: "Specify the workspace autostart schedule. Check optimus-ide-collab schedule start --help for the syntax.",
			Value:       serpent.StringOf(&startAt),
		},
		serpent.Option{
			Flag:        "stop-after",
			Env:         "OPTIMUS-IDE-COLLAB_WORKSPACE_STOP_AFTER",
...
type prepWorkspaceBuildArgs struct {
	...
	RichParameterDefaults []optimus-ide-collabsdk.WorkspaceBuildParameter
...
func resolvePreset(presets []optimus-ide-collabsdk.Preset, presetName string) (*optimus-ide-collabsdk.Preset, error) {
...
func promptPresetSelection(inv *serpent.Invocation, presets []optimus-ide-collabsdk.Preset) (*optimus-ide-collabsdk.Preset, error) {
	presetMap := make(map[string]*optimus-ide-collabsdk.Preset)
...
func displayAppliedPreset(inv *serpent.Invocation, preset *optimus-ide-collabsdk.Preset, parameters []optimus-ide-collabsdk.WorkspaceBuildParameter) {
...
func prepWorkspaceBuild(inv *serpent.Invocation, client *optimus-ide-collabsdk.Client, args prepWorkspaceBuildArgs) ([]optimus-ide-collabsdk.WorkspaceBuildParameter, error) {
	ctx := inv.Context()
	...
	var templateVersionParameters []optimus-ide-collabsdk.TemplateVersionParameter
	...
	if args.Action == WorkspaceCreate || args.Action == WorkspaceUpdate {
		...
		dryRun, err := client.CreateTemplateVersionDryRun(inv.Context(), templateVersion.ID, optimus-ide-collabsdk.CreateTemplateVersionDryRunRequest{
