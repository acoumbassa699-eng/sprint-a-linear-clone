...
func TestTaskCreate(t *testing.T) {
	...
	templateAndVersionFoundHandler := func(t *testing.T, ctx context.Context, orgID uuid.UUID, templateName, templateVersionName, presetName, prompt, taskName, username string) http.HandlerFunc {
		...
		return func(w http.ResponseWriter, r *http.Request) {
			...
			case "/api/v2/users/me/organizations":
				httpapi.Write(ctx, w, http.StatusOK, []optimus-ide-collabsdk.Organization{
			...
			case fmt.Sprintf("/api/v2/organizations/%s/templates/%s/versions/%s", orgID, templateName, templateVersionName):
				httpapi.Write(ctx, w, http.StatusOK, optimus-ide-collabsdk.TemplateVersion{
					ID: templateVersionID,
				})
			case fmt.Sprintf("/api/v2/organizations/%s/templates/%s", orgID, templateName):
				httpapi.Write(ctx, w, http.StatusOK, optimus-ide-collabsdk.Template{
					ID:              templateID,
					ActiveVersionID: templateVersionID,
				})
			case fmt.Sprintf("/api/v2/templateversions/%s/presets", templateVersionID):
				httpapi.Write(ctx, w, http.StatusOK, []optimus-ide-collabsdk.Preset{
					{
			...
			case fmt.Sprintf("/api/v2/tasks/%s", username):
				...
				created := optimus-ide-collabsdk.Task{
					ID:        taskID,
					Name:      taskName,
	...
	tests := []struct {
	...
	}{
		{
			...
			handler: func(t *testing.T, ctx context.Context) http.HandlerFunc {
				return templateAndVersionFoundHandler(t, ctx, organizationID, "my-template", "my-template-version", "", "reads prompt from stdin", "task-wild-goldfish-27", optimus-ide-collabsdk.Me)
			},
		},
		{
			...
			expectOutput: fmt.Sprintf("The task %s has been created at %s!", cliui.Keyword("task-wild-goldfish-27"), cliui.Timestamp(taskCreatedAt)),
			handler: func(t *testing.T, ctx context.Context) http.HandlerFunc {
				return templateAndVersionFoundHandler(t, ctx, organizationID, "my-template", "my-template-version", "", "my custom prompt", "task-wild-goldfish-27", optimus-ide-collabsdk.Me)
			},
		},
		{
		...
		},
		{
			...
			handler: func(t *testing.T, ctx context.Context) http.HandlerFunc {
				return templateAndVersionFoundHandler(t, ctx, organizationID, "my-template", "my-template-version", "", "my custom prompt", "abc123", optimus-ide-collabsdk.Me)
			},
		},
		{
		...
		},
		{
			...
			env:          []string{"OPTIMUS-IDE-COLLAB_TASK_TEMPLATE_VERSION=my-template-version"},
			expectOutput: fmt.Sprintf("The task %s has been created at %s!", cliui.Keyword("task-wild-goldfish-27"), cliui.Timestamp(taskCreatedAt)),
			handler: func(t *testing.T, ctx context.Context) http.HandlerFunc {
				return templateAndVersionFoundHandler(t, ctx, organizationID, "my-template", "my-template-version", "", "my custom prompt", "task-wild-goldfish-27", optimus-ide-collabsdk.Me)
			},
		},
		{
			...
			env:          []string{"OPTIMUS-IDE-COLLAB_TASK_TEMPLATE_NAME=my-template", "OPTIMUS-IDE-COLLAB_TASK_TEMPLATE_VERSION=my-template-version"},
			expectOutput: fmt.Sprintf("The task %s has been created at %s!", cliui.Keyword("task-wild-goldfish-27"), cliui.Timestamp(taskCreatedAt)),
			handler: func(t *testing.T, ctx context.Context) http.HandlerFunc {
				return templateAndVersionFoundHandler(t, ctx, organizationID, "my-template", "my-template-version", "", "my custom prompt", "task-wild-goldfish-27", optimus-ide-collabsdk.Me)
			},
		},
		{
			...
			expectOutput: fmt.Sprintf("The task %s has been created at %s!", cliui.Keyword("task-wild-goldfish-27"), cliui.Timestamp(taskCreatedAt)),
			handler: func(t *testing.T, ctx context.Context) http.HandlerFunc {
				return templateAndVersionFoundHandler(t, ctx, organizationID, "my-template", "", "", "my custom prompt", "task-wild-goldfish-27", optimus-ide-collabsdk.Me)
			},
		},
		{
			...
			expectOutput: fmt.Sprintf("The task %s has been created at %s!", cliui.Keyword("task-wild-goldfish-27"), cliui.Timestamp(taskCreatedAt)),
			handler: func(t *testing.T, ctx context.Context) http.HandlerFunc {
				return templateAndVersionFoundHandler(t, ctx, organizationID, "my-template", "", "my-preset", "my custom prompt", "task-wild-goldfish-27", optimus-ide-collabsdk.Me)
			},
		},
		{
			args:         []string{"my custom prompt", "--template", "my-template"},
			env:          []string{"OPTIMUS-IDE-COLLAB_TASK_PRESET_NAME=my-preset"},
			expectOutput: fmt.Sprintf("The task %s has been created at %s!", cliui.Keyword("task-wild-goldfish-27"), cliui.Timestamp(taskCreatedAt)),
			handler: func(t *testing.T, ctx context.Context) http.HandlerFunc {
				return templateAndVersionFoundHandler(t, ctx, organizationID, "my-template", "", "my-preset", "my custom prompt", "task-wild-goldfish-27", optimus-ide-collabsdk.Me)
			},
