...
func TestProvisioners_Golden(t *testing.T) {
	...
	updateReplaceUUIDs := func(optimus-ide-collabdAPI *optimus-ide-collabd.API) {
		systemCtx := dbauthz.AsSystemRestricted(context.Background())
		provisioners, err := optimus-ide-collabdAPI.Database.GetProvisionerDaemons(systemCtx)
		...
		jobs, err := optimus-ide-collabdAPI.Database.GetProvisionerJobsCreatedAfter(systemCtx, time.Time{})
		require.NoError(t, err)
	...
	wb := optimus-ide-collabdtest.AwaitWorkspaceBuildJobCompleted(t, client, workspace.LatestBuild.ID)
	require.Equal(t, optimus-ide-collabsdk.ProvisionerJobSucceeded, wb.Job.Status,
	...
	systemCtx := dbauthz.AsSystemRestricted(context.Background())
	existingJobs, err := optimus-ide-collabdAPI.Database.GetProvisionerJobsCreatedAfter(systemCtx, time.Time{})
	require.NoError(t, err)
	...
	_ = dbgen.ProvisionerDaemon(t, optimus-ide-collabdAPI.Database, database.ProvisionerDaemon{
		Name:       "provisioner-3",
	...
	updateReplaceUUIDs(optimus-ide-collabdAPI)
