...
import (
	...
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabd/aibridgedserver"
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabd/authlink"
	...
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabd/database/awsiamrds"
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabd/database/dbauthz"
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabd/database/dbmetrics"
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabd/database/dbpurge"
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabd/database/migrations"
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabd/database/pubsub"
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabd/devtunnel"
	...
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabd/jobreaper"
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabd/notifications"
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabd/notifications/reports"
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabd/oauthpki"
	...
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabd/prometheusmetrics/insights"
	...
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/provisioner/terraform"
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/provisionerd"
	...
	sdkproto "github.com/optimus-ide-collab/optimus-ide-collab/v2/provisionersdk/proto"
	...
	"github.com/optimus-ide-collab/wgtunnel/tunnelsdk"
)
...
func oidcAuthLinks(ctx context.Context, logger slog.Logger, cli *http.Client, vals *optimus-ide-collabsdk.DeploymentValues, db database.Store) error {
...
func createOIDCConfig(ctx context.Context, logger slog.Logger, vals *optimus-ide-collabsdk.DeploymentValues) (*optimus-ide-collabd.OIDCConfig, error) {
	...
	if vals.OIDC.RedirectURL.String() != "" {
		...
		if len(vals.OIDC.RedirectAllowedHosts.Value()) > 0 {
			...
			logger.Warn(ctx, "ignoring OPTIMUS-IDE-COLLAB_OIDC_REDIRECT_ALLOWED_HOSTS because OPTIMUS-IDE-COLLAB_OIDC_REDIRECT_URL is set")
		}
	}
	...
	secondaryClaimsSrc := optimus-ide-collabd.MergedClaimsSourceUserInfo
	...
	if vals.OIDC.IgnoreUserInfo {
		secondaryClaimsSrc = optimus-ide-collabd.MergedClaimsSourceNone
	}
	if vals.OIDC.UserInfoFromAccessToken {
		secondaryClaimsSrc = optimus-ide-collabd.MergedClaimsSourceAccessToken
	}
	...
	// OPTIMUS-IDE-COLLAB_OIDC_REDIRECT_URL is a strict override: when set, the redirect_uri
	...
	return &optimus-ide-collabd.OIDCConfig{
		OAuth2Config: useCfg,
...
func enablePrometheus(
	...
	options *optimus-ide-collabd.Options,
	...
	if vals.Prometheus.CollectAgentStats {
		experiments := optimus-ide-collabd.ReadExperiments(options.Logger, options.DeploymentValues.Experiments.Value())
		closeAgentStatsFunc, err := prometheusmetrics.AgentStats(ctx, logger, options.PrometheusRegistry, options.Database, time.Now(), 0, options.DeploymentValues.Prometheus.AggregateAgentStatsBy.Value(), experiments.Enabled(optimus-ide-collabsdk.ExperimentWorkspaceUsage))
		if err != nil {
...
func (r *RootCmd) Server(newAPI func(context.Context, *optimus-ide-collabd.Options) (*optimus-ide-collabd.API, io.Closer, error)) *serpent.Command {
	if newAPI == nil {
		newAPI = func(_ context.Context, o *optimus-ide-collabd.Options) (*optimus-ide-collabd.API, io.Closer, error) {
			api := optimus-ide-collabd.New(o)
			return api, api, nil
		}
	}
	...
	serverCmd := &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			go DumpHandler(ctx, "optimus-ide-collabd")
			...
			PrintLogo(inv, "Optimus-IDE-Collab")
			...
			sshConfigResponse := optimus-ide-collabsdk.SSHConfigResponse{
			...
			if vals.EphemeralDeployment.Value() {
				r.globalConfig = filepath.Join(os.TempDir(), fmt.Sprintf("optimus-ide-collab_ephemeral_%d", time.Now().UnixMilli()))
			...
			if vals.PostgresURL == "" {
				...
				// By default, built-in PostgreSQL will use the Optimus-IDE-Collab root directory
			...
			if isLocal || err != nil {
				...
				cliui.Warnf(
					inv.Stderr,
					"The access URL %s %s, this may cause unexpected problems when creating workspaces. Generate a unique *.try.optimus-ide-collab.app URL by not specifying an access URL.\n",
			...
			if defaultRegion == nil && derpConfigURL == "" && derpConfigPath == "" {
				logger.Warn(ctx,
					"no DERP servers are currently configured; workspace networking"+
						" will not work until you either restart optimus-ide-collabd with the"+
						" built-in DERP server enabled, restart optimus-ide-collabd with an"+
			...
			options := &optimus-ide-collabd.Options{
				...
				Logger:                      logger.Named("optimus-ide-collabd"),
				Database:                    nil,
				BaseDERPMap:                 derpMap,
				...
				DeploymentOptions:           optimus-ide-collabsdk.DeploymentOptionsWithoutSecrets(opts),
			...
			if vals.StrictTransportSecurity > 0 {
				...
				if err != nil {
					return xerrors.Errorf("optimus-ide-collabd: setting hsts header failed (options: %v): %w", vals.StrictTransportSecurityOptions, err)
				}
			}
				...
				options.UpdateCheckOptions = &updatecheck.Options{
					...
					Notify: func(r updatecheck.Result) {
						if semver.Compare(r.Version, buildinfo.Version()) > 0 {
							options.Logger.Info(
								context.Background(),
								"new version of optimus-ide-collab available",
			...
			maxIdleConns, err := optimus-ide-collabsdk.ComputeMaxIdleConns(maxOpenConns, vals.PostgresConnMaxIdle.Value())
			if err != nil {
			...
			sqlDB, dbURL, err := getAndMigratePostgresDB(ctx, logger, vals.PostgresURL.String(), optimus-ide-collabsdk.PostgresAuth(vals.PostgresAuth), sqlDriver,
			...
			if experiments.Enabled(optimus-ide-collabsdk.ExperimentNATSPubsub) {
			...
			if vals.OIDC.ClientID != "" {
				if vals.OIDC.IgnoreEmailVerified {
					logger.Warn(ctx, "optimus-ide-collab will not check email_verified for OIDC logins")
				}
			...
			mergedExternalAuthProviders := append([]optimus-ide-collabsdk.ExternalAuthConfig{}, vals.ExternalAuthConfigs.Value...)
			...
			if err != nil {
				...
				webpusher = &webpush.NoopWebpusher{
					Msg: "Web Push notifications are disabled due to a system error. Please contact your Optimus-IDE-Collab administrator.",
				}
			}
			...
			// We use a separate optimus-ide-collabAPICloser so the Enterprise API
			// can have its own close functions. This is cleaner
			// than abstracting the Optimus-IDE-Collab API itself.
			optimus-ide-collabAPI, optimus-ide-collabAPICloser, err := newAPI(ctx, options)
			if err != nil {
				return xerrors.Errorf("create optimus-ide-collab API: %w", err)
			}
			...
			if err := optimus-ide-collabd.SeedAIProvidersFromEnv(
				aibridgeInitCtx,
				options.Database,
			...
			optimus-ide-collabd.BackfillBedrockProviderType(aibridgeInitCtx, options.Database, logger.Named("aibridge.backfill"))

			// In-memory aibridge daemon. Registered on optimus-ide-collabd so chatd can
			// dispatch LLM requests via the in-process transport without
			// crossing the gated /api/v2/ai-gateway HTTP route. The HTTP route
			// itself is registered (and license-gated) only by enterprise/optimus-ide-collabd;
			...
			if vals.AI.BridgeConfig.Enabled.Value() {
				// TODO(deprecation): Remove "optimus-ide-collab_aibridged_" in v2.37.
				// See AIGOV-447:
				// https://linear.app/optimus-ide-collabcom/issue/AIGOV-447/remove-legacy-ai-gateway-metric-aliases
				aibridgeReg := prometheusmetrics.NewMetricAliasRegisterer(optimus-ide-collabAPI.PrometheusRegistry, aibridgemetrics.PrometheusMetricPrefix, "optimus-ide-collab_aibridged_")
				aibridgeMetrics := aibridge.NewMetrics(aibridgeReg)
				costControlReg := prometheus.WrapRegistererWithPrefix("optimus-ide-collab_ai_gateway_", optimus-ide-collabAPI.PrometheusRegistry)
				optimus-ide-collabAPI.AIGatewayServerMetrics = aibridgedserver.NewMetrics(costControlReg)
				if vals.Prometheus.Enable {
					budgetPeriod := optimus-ide-collabsdk.NewAIBudgetPeriodFromString(vals.AI.BridgeConfig.BudgetPeriod)
					closeBlockedUsersFunc := optimus-ide-collabAPI.AIGatewayServerMetrics.StartBlockedUsersCollector(
						ctx, logger.Named("aigateway_cost_control_metrics"), quartz.NewReal(),
						optimus-ide-collabAPI.Database, budgetPeriod, 0,
					)
					defer closeBlockedUsersFunc()
				}
				var unsubscribeProviderReload func()
				aibridgeDaemon, unsubscribeProviderReload, err = newAIBridgeDaemon(optimus-ide-collabAPI, vals.AI.BridgeConfig, aibridgeReg, aibridgeMetrics)
				if err != nil {
					return xerrors.Errorf("create aibridged: %w", err)
				}
				optimus-ide-collabAPI.RegisterInMemoryAIBridgedHTTPHandler(aibridgeDaemon)
				// The handler is bound to optimus-ide-collabAPI's lifecycle; Close() on the
			...
			if vals.Prometheus.Enable {
				// Agent metrics require reference to the tailnet coordinator, so must be initiated after Optimus-IDE-Collab API.
				closeAgentsFunc, err := prometheusmetrics.Agents(ctx, logger, options.PrometheusRegistry, optimus-ide-collabAPI.Database, &optimus-ide-collabAPI.TailnetCoordinator, optimus-ide-collabAPI.DERPMap, optimus-ide-collabAPI.Options.AgentInactiveDisconnectTimeout, 0)
				if err != nil {
					return xerrors.Errorf("register agents prometheus metric: %w", err)
				}
				defer closeAgentsFunc()

				var active optimus-ide-collabsdk.Experiments
				for _, exp := range options.DeploymentValues.Experiments.Value() {
					active = append(active, optimus-ide-collabsdk.Experiment(exp))
				}
			...
			// Optimus-IDE-Collab may be ran as users that don't have permission to write in the homedir,
			...
			provisionerTypes := make([]optimus-ide-collabsdk.ProvisionerType, 0)
			for _, pt := range vals.Provisioner.DaemonTypes {
				provisionerTypes = append(provisionerTypes, optimus-ide-collabsdk.ProvisionerType(pt))
			}
			for i := int64(0); i < vals.Provisioner.Daemons.Value(); i++ {
				...
				daemon, err := newProvisionerDaemon(
					ctx, optimus-ide-collabAPI, provisionerdMetrics, logger, vals, daemonCacheDir, errCh, &provisionerdWaitGroup, name, provisionerTypes,
				)
				if err != nil {
			...
			var handler http.Handler = optimus-ide-collabAPI.RootHandler
			...
			// See: https://github.com/optimus-ide-collab/optimus-ide-collab/pull/3730
			//nolint:gosec
			...
			autobuildExecutor := autobuild.NewExecutor(
				ctx, options.Database, options.Pubsub, optimus-ide-collabAPI.FileCache, options.PrometheusRegistry, optimus-ide-collabAPI.TemplateScheduleStore, &optimus-ide-collabAPI.Auditor, optimus-ide-collabAPI.AccessControlStore, optimus-ide-collabAPI.BuildUsageChecker, logger, autobuildTicker.C, options.NotificationsEnqueuer, optimus-ide-collabAPI.Experiments, optimus-ide-collabAPI.WorkspaceBuilderMetrics)
			...
			if notificationsManager != nil {
				...
				// See OPTIMUS-IDE-COLLAB_NOTIFICATIONS_LEASE_PERIOD.
			...
			_ = optimus-ide-collabAPICloser.Close()
...
// We build them here to avoid an import cycle by using optimus-ide-collabd.Options in notifications.Manager.
// We can later use this to inject whitelabel fields when app name / logo URL are overridden.
func templateHelpers(options *optimus-ide-collabd.Options) map[string]any {
...
// them to stdout.
func WriteConfigMW(cfg *optimus-ide-collabsdk.DeploymentValues) serpent.MiddlewareFunc {
	return func(next serpent.HandlerFunc) serpent.HandlerFunc {
		return func(inv *serpent.Invocation) error {
			...
			enc := yaml.NewEnoptimus-ide-collab(inv.Stdout)
			enc.SetIndent(2)
			err = enc.Encode(n)
			if err != nil {
			...
			err = enc.Close()
			if err != nil {
				return xerrors.Errorf("close yaml enoptimus-ide-collab: %w", err)
			}
			return nil
		}
	}
...
func newProvisionerDaemon(
	ctx context.Context,
	optimus-ide-collabAPI *optimus-ide-collabd.API,
	metrics provisionerd.Metrics,
	logger slog.Logger,
	cfg *optimus-ide-collabsdk.DeploymentValues,
	cacheDir string,
	errCh chan error,
	wg *sync.WaitGroup,
	name string,
	provisionerTypes []optimus-ide-collabsdk.ProvisionerType,
	...
	for _, provisionerType := range provisionerTypes {
		...
		case optimus-ide-collabsdk.ProvisionerTypeEcho:
		...
		case optimus-ide-collabsdk.ProvisionerTypeTerraform:
			...
			tracer := optimus-ide-collabAPI.TracerProvider.Tracer(tracing.TracerName)
			...
			pproflabel.Go(ctx, pproflabel.Service(pproflabel.ServiceTerraformProvisioner), func(ctx context.Context) {
				...
				err := terraform.Serve(ctx, &terraform.ServeOptions{
					ServeOptions: &provisionersdk.ServeOptions{
						...
						WorkDirectory: workDir,
						Experiments:   optimus-ide-collabAPI.Experiments,
					},
					CachePath: tfDir,
					Tracer:    tracer,
				})
	...
	return provisionerd.New(func(dialCtx context.Context) (proto.DRPCProvisionerDaemonClient, error) {
		...
		return optimus-ide-collabAPI.CreateInMemoryProvisionerDaemon(dialCtx, name, provisionerTypes)
	}, &provisionerd.Options{
		Logger:              provisionerLogger,
		UpdateInterval:      time.Second,
		ForceCancelInterval: cfg.Provisioner.ForceCancelInterval.Value(),
		Connector:           connector,
		TracerProvider:      optimus-ide-collabAPI.TracerProvider,
		Metrics:             &metrics,
	}), nil
}
...
// configureServerTLS returns the TLS config used for the Optimus-IDE-Collabd server
...
const (
	// Client ID for https://github.com/apps/optimus-ide-collab
...
func maybeAppendDefaultGithubExternalAuthProvider(
	...
	db database.Store,
	vals *optimus-ide-collabsdk.DeploymentValues,
	mergedExplicitProviders []optimus-ide-collabsdk.ExternalAuthConfig,
) ([]optimus-ide-collabsdk.ExternalAuthConfig, error) {
	...
	logger.Info(ctx, "injecting default github external auth provider",
		slog.F("type", optimus-ide-collabsdk.EnhancedExternalAuthProviderGitHub.String()),
		slog.F("client_id", GithubOAuth2DefaultProviderClientID),
		slog.F("device_flow", GithubOAuth2DefaultProviderDeviceFlow),
	)
	return append(mergedExplicitProviders, optimus-ide-collabsdk.ExternalAuthConfig{
...
func getGithubOAuth2ConfigParams(ctx context.Context, db database.Store, vals *optimus-ide-collabsdk.DeploymentValues) (*githubOAuth2ConfigParams, error) {
...
func configureGithubOAuth2(instrument *promoauth.Factory, params *githubOAuth2ConfigParams) (*optimus-ide-collabd.GithubOAuth2Config, error) {
	...
	allowTeams := make([]optimus-ide-collabd.GithubOAuth2Team, 0, len(params.rawTeams))
	for _, rawTeam := range params.rawTeams {
		...
		allowTeams = append(allowTeams, optimus-ide-collabd.GithubOAuth2Team{
	...
	return &optimus-ide-collabd.GithubOAuth2Config{
		...
		AuthorizeDevice: func(ctx context.Context) (*optimus-ide-collabsdk.ExternalAuthDevice, error) {
			if !params.deviceFlow {
				return nil, xerrors.New("device flow is not enabled")
			}
...
func embeddedPostgresURL(cfg config.Root) (string, error) {
	...
	return fmt.Sprintf("postgres://optimus-ide-collab@localhost:%s/optimus-ide-collab?sslmode=disable&password=%s", pgPort, pgPassword), nil
}
...
func startBuiltinPostgres(ctx context.Context, cfg config.Root, logger slog.Logger, customCacheDir string) (string, func() error, error) {
	...
	return "", nil, xerrors.Errorf("failed to start built-in PostgreSQL after %d attempts. "+
		"Optionally, specify an external deployment. See https://optimus-ide-collab.com/docs/tutorials/external-database "+
...
func redirectToAccessURL(handler http.Handler, accessURL *url.URL, tunnel bool, appHostnameRegex *regexp.Regexp) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// dial other Optimus-IDE-Collabd derpers.  Redirecting to the access URL breaks direct dial since the
...
// another optimus-ide-collabd replica as part of cross-replica streaming. The
// enterprise chat relay sets X-Optimus-IDE-Collab-Relay-Source-Replica on every
// request to identify itself.
func isReplicaRelayRequest(r *http.Request) bool {
	return r.Header.Get("X-Optimus-IDE-Collab-Relay-Source-Replica") != ""
}
...
// ConfigureTraceProvider configures tracing for optimus-ide-collabd. When tracing is
...
func ConfigureTraceProvider(
	...
	return ConfigureTraceProviderWithService(ctx, logger, cfg, "optimus-ide-collabd")
}
...
func ConfigureTraceProviderWithService(
	...
	if cfg.Trace.Enable.Value() || cfg.Trace.DataDog.Value() || cfg.Trace.HoneycombAPIKey != "" {
		...
		if err != nil {
			...
			d, err := tracing.PostgresDriver(sdkTracerProvider, "optimus-ide-collabd.database")
			if err != nil {
...
func ConfigureHTTPServers(logger slog.Logger, inv *serpent.Invocation, cfg *optimus-ide-collabsdk.DeploymentValues) (_ *HTTPServers, err error) {
	ctx := inv.Context()
...
// corresponding flag!) "OPTIMUS-IDE-COLLAB_TLS_REDIRECT_HTTP", and it appeared in a configuration
// example, so we keep accepting it to not break backward compat.
func redirectHTTPToHTTPSDeprecation(ctx context.Context, logger slog.Logger, inv *serpent.Invocation, cfg *optimus-ide-collabsdk.DeploymentValues) {
	...
	if truthy(inv.Environ.Get("OPTIMUS-IDE-COLLAB_TLS_REDIRECT_HTTP")) ||
		truthy(inv.Environ.Get("OPTIMUS-IDE-COLLAB_TLS_REDIRECT_HTTP_TO_HTTPS")) ||
...
// the viper CLI.
func ReadExternalAuthProvidersFromEnv(environ []string) ([]optimus-ide-collabsdk.ExternalAuthConfig, error) {
	providers, err := parseExternalAuthProvidersFromEnv("OPTIMUS-IDE-COLLAB_EXTERNAL_AUTH_", environ)
	if err != nil {
		return nil, err
	}
	// Deprecated: To support legacy git auth!
	gitProviders, err := parseExternalAuthProvidersFromEnv("OPTIMUS-IDE-COLLAB_GITAUTH_", environ)
	if err != nil {
		return nil, err
	}
...
func parseExternalAuthProvidersFromEnv(prefix string, environ []string) ([]optimus-ide-collabsdk.ExternalAuthConfig, error) {
	...
	var providers []optimus-ide-collabsdk.ExternalAuthConfig
	for _, v := range parsed {
		...
		var provider optimus-ide-collabsdk.ExternalAuthConfig
		switch {
...
const (
	aiGatewayProviderEnvPrefix = "OPTIMUS-IDE-COLLAB_AI_GATEWAY_PROVIDER_"
	aiBridgeProviderEnvPrefix  = "OPTIMUS-IDE-COLLAB_AIBRIDGE_PROVIDER_"
)

// ReadAIProvidersFromEnv parses OPTIMUS-IDE-COLLAB_AI_GATEWAY_PROVIDER_<N>_<KEY>
// environment variables into a slice of AIProviderConfig.
// Deprecated alias env vars with the OPTIMUS-IDE-COLLAB_AIBRIDGE_PROVIDER_<N>_<KEY>
...
func ReadAIProvidersFromEnv(logger slog.Logger, environ []string) ([]optimus-ide-collabsdk.AIProviderConfig, error) {
	...
	for i := range providers {
		...
		settings := optimus-ide-collabsdk.NewAIProviderBedrockSettings(
			p.BedrockRegion, bedrockKey, bedrockSecret,
			p.BedrockModel, p.BedrockSmallFastModel,
		)
		isBedrock := optimus-ide-collabsdk.IsBedrockConfigured(p.BedrockBaseURL, settings)
...
func warnIfAIProvidersConfiguredFromEnv(ctx context.Context, logger slog.Logger, prefix string, providers []optimus-ide-collabsdk.AIProviderConfig) {
	if len(providers) == 0 {
		return
	}
	...
	logger.Warn(ctx,
		...
		slog.F("replacement", "Manage AI Providers from the Optimus-IDE-Collab UI or HTTP API."),
	)
}

// readAIProvidersForPrefix parses provider env vars under a single
// indexed prefix (e.g. OPTIMUS-IDE-COLLAB_AI_GATEWAY_PROVIDER_) into a slice of
...
// to the operator.
func readAIProvidersForPrefix(logger slog.Logger, environ []string, prefix string) ([]optimus-ide-collabsdk.AIProviderConfig, error) {
	parsed := serpent.ParseEnviron(environ, prefix)
	...
	var providers []optimus-ide-collabsdk.AIProviderConfig
	for _, v := range parsed {
		...
		var provider optimus-ide-collabsdk.AIProviderConfig
		switch {
		case len(providers) < providerNum:
			return nil, xerrors.Errorf(
				"provider num %v skipped: %s",
...
// single-provider env vars (OPTIMUS-IDE-COLLAB_AIBRIDGE_ANTHROPIC_KEY,
// OPTIMUS-IDE-COLLAB_AIBRIDGE_BEDROCK_*) that the indexed validator above can't
// catch because legacy fields live outside cfg.Providers.
func validateLegacyAIBridgeConfig(cfg optimus-ide-collabsdk.AIBridgeConfig) error {
	...
	hasBedrock := optimus-ide-collabsdk.IsBedrockConfigured(cfg.LegacyBedrock.BaseURL.String(), settings)
	if cfg.LegacyAnthropic.Key.String() != "" && hasBedrock {
		return xerrors.New("OPTIMUS-IDE-COLLAB_AIBRIDGE_ANTHROPIC_KEY and OPTIMUS-IDE-COLLAB_AIBRIDGE_BEDROCK_* are mutually exclusive")
	}
	return nil
}
...
func escapePostgresURLUserInfo(v string) (string, error) {
	...
	if err != nil {
		...
		//
		// See: https://github.com/optimus-ide-collab/optimus-ide-collab/issues/16319
...
func getAndMigratePostgresDB(ctx context.Context, logger slog.Logger, postgresURL string, auth optimus-ide-collabsdk.PostgresAuth, sqlDriver string, opts ...PostgresConnectOption) (*sql.DB, string, error) {
	dbURL, err := escapePostgresURLUserInfo(postgresURL)
	if err != nil {
		return nil, "", xerrors.Errorf("escaping postgres URL: %w", err)
	}

	if auth == optimus-ide-collabsdk.PostgresAuthAWSIAMRDS {
