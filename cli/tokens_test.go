...
func TestTokens(t *testing.T) {
	...
	secondUserClient, secondUser := optimus-ide-collabdtest.CreateAnotherUser(t, client, adminUser.OrganizationID)
	thirdUserClient, thirdUser := optimus-ide-collabdtest.CreateAnotherUser(t, client, adminUser.OrganizationID)
	...
	inv, root = clitest.New(t, "tokens", "create", "--name", "scoped-token", "--scope", string(optimus-ide-collabsdk.APIKeyScopeWorkspaceRead), "--allow", allowSpec)
	clitest.SetupConfig(t, client, root)
	buf = new(bytes.Buffer)
	inv.Stdout = buf
	err = inv.WithContext(ctx).Run()
	...
	require.NoError(t, err)
	res = buf.String()
	require.Contains(t, res, string(optimus-ide-collabsdk.APIKeyScopeWorkspaceRead))
	...
	var tokens []optimus-ide-collabsdk.APIKey
	require.NoError(t, json.Unmarshal(buf.Bytes(), &tokens))
	require.Len(t, tokens, 2)
	tokenByName := make(map[string]optimus-ide-collabsdk.APIKey, len(tokens))
	...
	require.Contains(t, scopedToken.Scopes, optimus-ide-collabsdk.APIKeyScopeWorkspaceRead)
