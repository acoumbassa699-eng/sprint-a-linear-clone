...
import (
	...
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabd/autobuild/notify"
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabd/util/maps"
...
const (
	...
	// Optimus-IDE-Collab Connect DNS should answer locally, so a slow probe should fall
	// back to the normal SSH tunnel.
	optimus-ide-collabConnectProbeTimeout = 100 * time.Millisecond
)
...
func isRetryableError(err error) bool {
	...
	if optimus-ide-collabsdk.IsConnectionError(err) {
		return true
	}
...
func (r *RootCmd) ssh() *serpent.Command {
	...
	cmd := &serpent.Command{
		...
		Long: "This command does not have full parity with the standard SSH command. For users who need the full functionality of SSH, create an ssh configuration with `optimus-ide-collab config-ssh`.\n\n" +
			FormatExamples(
				Example{
					Description: "Use `--` to separate and pass flags directly to the command executed via SSH.",
					Command:     "optimus-ide-collab ssh <workspace> -- ls -la",
				},
			),
		...
		CompletionHandler: func(inv *serpent.Invocation) []string {
			...
			res, err := client.Workspaces(inv.Context(), optimus-ide-collabsdk.WorkspaceFilter{
			...
			for _, ws := range res.Workspaces {
				...
				go func() {
					...
					var agents []optimus-ide-collabsdk.WorkspaceAgent
		...
		Handler: func(inv *serpent.Invocation) (retErr error) {
			...
			// See: https://github.com/optimus-ide-collab/optimus-ide-collab/issues/13144
			...
			cliConfig := optimus-ide-collabsdk.SSHConfigResponse{
			...
			var workspaceAgent optimus-ide-collabsdk.WorkspaceAgent
			...
			for _, warning := range templateVersion.Warnings {
				if warning == optimus-ide-collabsdk.TemplateVersionWarningUnsupportedWorkspaces {
			...
			// If we're in stdio mode, check to see if we can use Optimus-IDE-Collab Connect.
			// We don't support Optimus-IDE-Collab Connect over non-stdio optimus-ide-collab ssh yet.
			if stdio && !forceNewTunnel {
				...
				optimus-ide-collabConnectHost := fmt.Sprintf("%s.%s.%s.%s",
				...
				// configured.
				// Some DNS paths blackhole absolute .optimus-ide-collab. lookups instead of
				// returning NXDOMAIN, so keep fallback fast.
				optimus-ide-collabConnectCtx, optimus-ide-collabConnectCancel := context.WithTimeout(ctx, optimus-ide-collabConnectProbeTimeout)
				exists, ccErr := workspacesdk.ExistsViaOptimus-IDE-CollabConnect(optimus-ide-collabConnectCtx, optimus-ide-collabConnectHost+".")
				optimus-ide-collabConnectCancel()
				if ccErr != nil {
					logger.Debug(ctx, "failed to check optimus-ide-collab connect",
						slog.F("hostname", optimus-ide-collabConnectHost),
						slog.Error(ccErr),
					)
				}
				if exists {
					...
					if networkInfoDir != "" {
						if err := writeOptimus-IDE-CollabConnectNetInfo(ctx, networkInfoDir); err != nil {
							logger.Error(ctx, "failed to write optimus-ide-collab connect net info file", slog.Error(err))
						}
					}
					...
					if usageAppName != "" {
						closeUsage := client.UpdateWorkspaceUsageWithBodyContext(ctx, workspace.ID, optimus-ide-collabsdk.PostWorkspaceUsageRequest{
							AgentID: workspaceAgent.ID,
							AppName: usageAppName,
						})
						defer closeUsage()
					}
					return runOptimus-IDE-CollabConnectStdio(ctx, fmt.Sprintf("%s:22", optimus-ide-collabConnectHost), stdioReader, stdioWriter, stack, logger)
				}
			}
	...
	waitOption := serpent.Option{
		Flag:        "wait",
		Env:         "OPTIMUS-IDE-COLLAB_SSH_WAIT",
	...
	cmd.Options = serpent.OptionSet{
		{
			Flag:        "stdio",
			Env:         "OPTIMUS-IDE-COLLAB_SSH_STDIO",
		...
		},
		{
			Flag:          "tty",
			FlagShorthand: "t",
			Env:           "OPTIMUS-IDE-COLLAB_SSH_TTY",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_SSH_SSH_HOST_PREFIX",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_SSH_HOSTNAME_SUFFIX",
		...
		},
		{
			Flag:          "forward-agent",
			FlagShorthand: "A",
			Env:           "OPTIMUS-IDE-COLLAB_SSH_FORWARD_AGENT",
		...
		},
		{
			Flag:          "forward-gpg",
			FlagShorthand: "G",
			Env:           "OPTIMUS-IDE-COLLAB_SSH_FORWARD_GPG",
		...
		{
			Flag:        "identity-agent",
			Env:         "OPTIMUS-IDE-COLLAB_SSH_IDENTITY_AGENT",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_WORKSPACE_POLL_INTERVAL",
		...
		{
			Flag:        "no-wait",
			Env:         "OPTIMUS-IDE-COLLAB_SSH_NO_WAIT",
		...
		},
		{
			...
			Env:           "OPTIMUS-IDE-COLLAB_SSH_LOG_DIR",
			FlagShorthand: "l",
		...
		},
		{
			...
			Env:           "OPTIMUS-IDE-COLLAB_SSH_REMOTE_FORWARD",
			FlagShorthand: "R",
		...
		},
		{
			...
			Env:           "OPTIMUS-IDE-COLLAB_SSH_ENV",
			FlagShorthand: "e",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_SSH_USAGE_APP",
		...
		},
		{
			...
			Description: "Force the creation of a new tunnel to the workspace, even if the Optimus-IDE-Collab Connect tunnel is available.",
...
// corresponds to, taking into account any name prefixes or suffixes configured (e.g. myworkspace.optimus-ide-collab, or
// vscode-optimus-ide-collab--myusername--myworkspace).
func findWorkspaceAndAgentByHostname(
	ctx context.Context, inv *serpent.Invocation, client *optimus-ide-collabsdk.Client,
	hostname string, config optimus-ide-collabsdk.SSHConfigResponse, disableAutostart bool,
) (
	optimus-ide-collabsdk.Workspace, optimus-ide-collabsdk.WorkspaceAgent, error,
) {
	// for suffixes, we don't explicitly get the . and must add it. This is to ensure that the suffix is always
	// interpreted as a dotted label in DNS names, not just any string suffix. That is, a suffix of 'optimus-ide-collab' will
	// match a hostname like 'en.optimus-ide-collab', but not 'enoptimus-ide-collab'.
...
//
// See: https://github.com/optimus-ide-collab/optimus-ide-collab/issues/6180
func watchAndClose(ctx context.Context, closer func() error, logger slog.Logger, client *optimus-ide-collabsdk.Client, workspace optimus-ide-collabsdk.Workspace, errCh <-chan error) {
...
startWatchLoop:
	for {
		logger.Debug(ctx, "connecting to the optimus-ide-collab server to watch workspace events")
		var wsWatch <-chan optimus-ide-collabsdk.Workspace
		var err error
		...
		for {
			...
			case w, ok := <-wsWatch:
				...
				if workspace.LatestBuild.ID != w.LatestBuild.ID && w.LatestBuild.Transition == optimus-ide-collabsdk.WorkspaceTransitionStart {
				...
				// during "stopping".
				if w.LatestBuild.Status == optimus-ide-collabsdk.WorkspaceStatusStopped {
...
func GetWorkspaceAndAgent(ctx context.Context, inv *serpent.Invocation, client *optimus-ide-collabsdk.Client, autostart bool, input string) (optimus-ide-collabsdk.Workspace, optimus-ide-collabsdk.WorkspaceAgent, []optimus-ide-collabsdk.WorkspaceAgent, error) { //nolint:revive
	var (
	...
	if err != nil {
		return optimus-ide-collabsdk.Workspace{}, optimus-ide-collabsdk.WorkspaceAgent{}, nil, err
	}

	if workspace.LatestBuild.Transition != optimus-ide-collabsdk.WorkspaceTransitionStart {
		if !autostart {
			return optimus-ide-collabsdk.Workspace{}, optimus-ide-collabsdk.WorkspaceAgent{}, nil, xerrors.New("workspace must be started")
		}
		// Autostart the workspace for the user.
		// For some failure modes, return a better message.
		if workspace.LatestBuild.Transition == optimus-ide-collabsdk.WorkspaceTransitionDelete {
			// Any sort of deleting status, we should reject with a nicer error.
			return optimus-ide-collabsdk.Workspace{}, optimus-ide-collabsdk.WorkspaceAgent{}, nil, xerrors.Errorf("workspace %q is deleted", workspace.Name)
		}
		if workspace.LatestBuild.Job.Status == optimus-ide-collabsdk.ProvisionerJobFailed {
			return optimus-ide-collabsdk.Workspace{}, optimus-ide-collabsdk.WorkspaceAgent{}, nil,
		...
		if workspace.LatestBuild.Status != optimus-ide-collabsdk.WorkspaceStatusStopped {
			return optimus-ide-collabsdk.Workspace{}, optimus-ide-collabsdk.WorkspaceAgent{}, nil,
				xerrors.Errorf("workspace must be started; was unable to autostart as the last build job is %q, expected %q",
					...
					optimus-ide-collabsdk.WorkspaceStatusStopped,
				)
		}
		...
		_, err = startWorkspace(inv, client, workspace, workspaceParameterFlags{
			...
			reason: string(optimus-ide-collabsdk.BuildReasonSSHConnection),
		}, WorkspaceStart)
		if cerr, ok := optimus-ide-collabsdk.AsError(err); ok {
			...
			case http.StatusForbidden:
				...
				if err != nil {
					return optimus-ide-collabsdk.Workspace{}, optimus-ide-collabsdk.WorkspaceAgent{}, nil, xerrors.Errorf("start workspace with active template version: %w", err)
				}
			...
			default:
				return optimus-ide-collabsdk.Workspace{}, optimus-ide-collabsdk.WorkspaceAgent{}, nil, xerrors.Errorf("start workspace with current template version: %w", err)
			}
		} else if err != nil {
			return optimus-ide-collabsdk.Workspace{}, optimus-ide-collabsdk.WorkspaceAgent{}, nil, xerrors.Errorf("start workspace with current template version: %w", err)
		}
	...
	if workspace.LatestBuild.Transition == optimus-ide-collabsdk.WorkspaceTransitionDelete {
		return optimus-ide-collabsdk.Workspace{}, optimus-ide-collabsdk.WorkspaceAgent{}, nil, xerrors.Errorf("workspace %q is being deleted", workspace.Name)
	}
	...
	if err != nil {
		return workspace, optimus-ide-collabsdk.WorkspaceAgent{}, otherWorkspaceAgents, err
	}

	return workspace, workspaceAgent, otherWorkspaceAgents, nil
}

func getWorkspaceAgent(workspace optimus-ide-collabsdk.Workspace, agentName string) (workspaceAgent optimus-ide-collabsdk.WorkspaceAgent, otherAgents []optimus-ide-collabsdk.WorkspaceAgent, err error) {
	...
	if len(agents) == 0 {
		return optimus-ide-collabsdk.WorkspaceAgent{}, nil, xerrors.Errorf("workspace %q has no agents", workspace.Name)
	}
	...
	if agentName != "" {
		...
		return optimus-ide-collabsdk.WorkspaceAgent{}, nil, xerrors.Errorf("agent not found by name %q, available agents: %v", agentName, availableNames)
	}
	if len(agents) == 1 {
		return agents[0], nil, nil
	}
	return optimus-ide-collabsdk.WorkspaceAgent{}, agents, xerrors.Errorf("multiple agents found, please specify the agent name, available agents: %v", availableNames)
}

// Attempt to poll workspace autostop. We write a per-workspace lockfile to
// avoid spamming the user with notifications in case of multiple instances
// of the CLI running simultaneously.
func tryPollWorkspaceAutostop(ctx context.Context, client *optimus-ide-collabsdk.Client, workspace optimus-ide-collabsdk.Workspace) (stop func()) {
	lock := flock.New(filepath.Join(os.TempDir(), "optimus-ide-collab-autostop-notify-"+workspace.ID.String()))
...
func notifyCondition(ctx context.Context, client *optimus-ide-collabsdk.Client, workspaceID uuid.UUID, lock *flock.Flock) notify.Condition {
	return func(now time.Time) (deadline time.Time, callback func()) {
		...
		callback = func() {
			...
			if ttl > time.Minute {
				...
				body = fmt.Sprintf(
					`Your Optimus-IDE-Collab workspace %s is scheduled to stop in %.0f mins`, ws.Name, ttl.Minutes())
			} else {
				title = fmt.Sprintf("Workspace %s stopping!", ws.Name)
				body = fmt.Sprintf("Your Optimus-IDE-Collab workspace %s is stopping any time now!", ws.Name)
			}
...
func verifyWorkspaceOutdated(client *optimus-ide-collabsdk.Client, workspace optimus-ide-collabsdk.Workspace) (string, bool) {
...
// Build the user workspace link which navigates to the Optimus-IDE-Collab web UI.
func buildWorkspaceLink(serverURL *url.URL, workspace optimus-ide-collabsdk.Workspace) *url.URL {
...
func sshDisableAutostartOption(src *serpent.Bool) serpent.Option {
	return serpent.Option{
		...
		Env:         "OPTIMUS-IDE-COLLAB_SSH_DISABLE_AUTOSTART",
		Value:       src,
		Default:     "false",
	}
}
...
func getUsageAppName(usageApp string) optimus-ide-collabsdk.UsageAppName {
	if usageApp == disableUsageApp {
		return ""
	}

	allowedUsageApps := []string{
		string(optimus-ide-collabsdk.UsageAppNameSSH),
		string(optimus-ide-collabsdk.UsageAppNameVscode),
		string(optimus-ide-collabsdk.UsageAppNameJetbrains),
	}
	if slices.Contains(allowedUsageApps, usageApp) {
		return optimus-ide-collabsdk.UsageAppName(usageApp)
	}

	return optimus-ide-collabsdk.UsageAppNameSSH
}
...
type sshNetworkStats struct {
	...
	UsingOptimus-IDE-CollabConnect bool               `json:"using_optimus-ide-collab_connect"`
}
...
type optimus-ide-collabConnectDialerContextKey struct{}

type optimus-ide-collabConnectDialer interface {
	DialContext(ctx context.Context, network, addr string) (net.Conn, error)
}

func WithTestOnlyOptimus-IDE-CollabConnectDialer(ctx context.Context, dialer optimus-ide-collabConnectDialer) context.Context {
	return context.WithValue(ctx, optimus-ide-collabConnectDialerContextKey{}, dialer)
}

func testOrDefaultDialer(ctx context.Context) optimus-ide-collabConnectDialer {
	dialer, ok := ctx.Value(optimus-ide-collabConnectDialerContextKey{}).(optimus-ide-collabConnectDialer)
	if !ok || dialer == nil {
...
func runOptimus-IDE-CollabConnectStdio(ctx context.Context, addr string, stdin io.Reader, stdout io.Writer, stack *closerStack, logger slog.Logger) error {
	...
	if err := retryWithInterval(ctx, logger, sshRetryInterval, sshMaxAttempts, func() error {
		...
		if err != nil {
			return xerrors.Errorf("dial optimus-ide-collab connect host %q over tcp: %w", addr, err)
		}
		return nil
	}); err != nil {
		return err
...
func writeOptimus-IDE-CollabConnectNetInfo(ctx context.Context, networkInfoDir string) error {
	fs, ok := ctx.Value("fs").(afero.Fs)
	if !ok {
		fs = afero.NewOsFs()
	}
	...
	stats := &sshNetworkStats{
		UsingOptimus-IDE-CollabConnect: true,
	}
	rawStats, err := json.Marshal(stats)
	if err != nil {
...
// owner--workspace--agent
// owner--workspace.agent
// agent.workspace.owner - for parity with Optimus-IDE-Collab Connect
func normalizeWorkspaceInput(input string) string {
	...
	case 3:
		// If the only separator is a dot, it's the Optimus-IDE-Collab Connect format
