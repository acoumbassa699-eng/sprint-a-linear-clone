...
func (r *RootCmd) tokens() *serpent.Command {
	cmd := &serpent.Command{
		...
		Long: "Tokens are used to authenticate automated clients to Optimus-IDE-Collab.\n" + FormatExamples(
			Example{
				Description: "Create a token for automation",
				Command:     "optimus-ide-collab tokens create",
			},
			Example{
				Description: "List your tokens",
				Command:     "optimus-ide-collab tokens ls",
			},
			Example{
				Description: "Create a scoped token",
				Command:     "optimus-ide-collab tokens create --scope workspace:read --allow workspace:<uuid>",
			},
			Example{
				Description: "Remove a token by ID",
				Command:     "optimus-ide-collab tokens rm WuoWs4ZsMX",
			},
		),
...
func (r *RootCmd) createToken() *serpent.Command {
	var (
		...
		user          string
		scopes        []string
		allowList     []optimus-ide-collabsdk.APIAllowListTarget
	)
	cmd := &serpent.Command{
		Use:   "create",
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			req := optimus-ide-collabsdk.CreateTokenRequest{
				Lifetime:  parsedLifetime,
				TokenName: name,
			}
			if len(req.Scopes) == 0 {
				req.Scopes = slice.StringEnums[optimus-ide-collabsdk.APIKeyScope](scopes)
			}
			if len(allowList) > 0 {
				req.AllowList = append([]optimus-ide-collabsdk.APIAllowListTarget(nil), allowList...)
			}
	...
	cmd.Options = serpent.OptionSet{
		{
			Flag:        "lifetime",
			Env:         "OPTIMUS-IDE-COLLAB_TOKEN_LIFETIME",
		...
		},
		{
			Flag:          "name",
			FlagShorthand: "n",
			Env:           "OPTIMUS-IDE-COLLAB_TOKEN_NAME",
		...
		},
		{
			Flag:          "user",
			FlagShorthand: "u",
			Env:           "OPTIMUS-IDE-COLLAB_TOKEN_USER",
...
type tokenListRow struct {
	// For JSON format:
	optimus-ide-collabsdk.APIKey `table:"-"`
...
func tokenListRowFromToken(token optimus-ide-collabsdk.APIKeyWithOwner) tokenListRow {
	return tokenListRowFromKey(token.APIKey, token.Username)
}

func tokenListRowFromKey(token optimus-ide-collabsdk.APIKey, owner string) tokenListRow {
	return tokenListRow{
		APIKey:    token,
		ID:        token.ID,
...
func joinScopes(scopes []optimus-ide-collabsdk.APIKeyScope) string {
	if len(scopes) == 0 {
		return ""
	}
...
func joinAllowList(entries []optimus-ide-collabsdk.APIAllowListTarget) string {
	if len(entries) == 0 {
		return ""
	}
...
func (r *RootCmd) listTokens() *serpent.Command {
	...
	cmd := &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			tokens, err := client.Tokens(inv.Context(), optimus-ide-collabsdk.Me, optimus-ide-collabsdk.TokensFilter{
				IncludeAll:     all,
...
func (r *RootCmd) viewToken() *serpent.Command {
	...
	cmd := &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			token, err := client.APIKeyByName(inv.Context(), optimus-ide-collabsdk.Me, tokenName)
			if err != nil {
				maybeID := strings.Split(tokenName, "-")[0]
				token, err = client.APIKeyByID(inv.Context(), optimus-ide-collabsdk.Me, maybeID)
				if err != nil {
...
func (r *RootCmd) removeToken() *serpent.Command {
	...
	cmd := &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			token, err := client.APIKeyByName(inv.Context(), optimus-ide-collabsdk.Me, inv.Args[0])
			if err != nil {
			...
			if deleteToken {
				err = client.DeleteAPIKey(inv.Context(), optimus-ide-collabsdk.Me, token.ID)
				if err != nil {
			...
			err = client.ExpireAPIKey(inv.Context(), optimus-ide-collabsdk.Me, token.ID)
			if err != nil {
