...
import (
	...
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/cli/sessionstore/testhelpers"
...
func TestUseKeyring(t *testing.T) {
	...
	t.Run("EnvironmentVariable", func(t *testing.T) {
		...
		// Login using OPTIMUS-IDE-COLLAB_USE_KEYRING environment variable set to disable keyring usage,
		...
		stdout := expecter.NewAttachedToInvocation(t, inv)
		stdin := testutil.NewWriterAttachedToInvocation(t, logger.Named("stdin"), inv)
		inv.Environ.Set("OPTIMUS-IDE-COLLAB_USE_KEYRING", "false")
		...
		sessionFile := path.Join(string(env.cfg), "session")
		_, err := os.Stat(sessionFile)
		require.NoError(t, err, "session file should exist when OPTIMUS-IDE-COLLAB_USE_KEYRING set to false")
