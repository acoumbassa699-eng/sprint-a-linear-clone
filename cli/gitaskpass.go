...
// gitAskpass is used by the Optimus-IDE-Collab agent to automatically authenticate
	...
	cmd := &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			token, err := client.ExternalAuth(ctx, agentsdk.ExternalAuthRequest{
				...
				ChatID:          inv.Environ.Get("OPTIMUS-IDE-COLLAB_CHAT_ID"),
			})
			if err != nil {
				var apiError *optimus-ide-collabsdk.Error
				if errors.As(err, &apiError) && apiError.StatusCode() == http.StatusNotFound {
					// This prevents the "Run 'optimus-ide-collab --help' for usage"
					// message from occurring.
					lines := []string{apiError.Message}
					if apiError.Detail != "" {
						lines = append(lines, apiError.Detail)
					}
					cliui.Warn(inv.Stderr, "Optimus-IDE-Collab was unable to handle this git request. The default git behavior will be used instead.",
						lines...,
					)
			...
			if token.URL != "" {
				...
				if inv.Environ.Get("OPTIMUS-IDE-COLLAB_CHAT_AGENT") == "true" {
