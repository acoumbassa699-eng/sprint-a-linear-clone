...
func TestExpTaskList(t *testing.T) {
	...
	t.Run("NoTasks_Table", func(t *testing.T) {
		...
		client, _ := optimus-ide-collabdtest.NewWithDatabase(t, &optimus-ide-collabdtest.Options{Logger: &quiet})
	...
	t.Run("Single_Table", func(t *testing.T) {
		...
		// Quiet logger to reduce noise.
		quiet := slog.Make(sloghuman.Sink(io.Discard))
		client, db := optimus-ide-collabdtest.NewWithDatabase(t, &optimus-ide-collabdtest.Options{Logger: &quiet})
		owner := optimus-ide-collabdtest.CreateFirstUser(t, client)
	...
	t.Run("UserFlag_Me_Table", func(t *testing.T) {
		...
		client, db := optimus-ide-collabdtest.NewWithDatabase(t, &optimus-ide-collabdtest.Options{Logger: &quiet})
		owner := optimus-ide-collabdtest.CreateFirstUser(t, client)
		_, memberUser := optimus-ide-collabdtest.CreateAnotherUser(t, client, owner.OrganizationID)
...
func TestExpTaskList_OwnerCanListOthers(t *testing.T) {
	...
	// Quiet logger to reduce noise.
	quiet := slog.Make(sloghuman.Sink(io.Discard))
	ownerClient, db := optimus-ide-collabdtest.NewWithDatabase(t, &optimus-ide-collabdtest.Options{Logger: &quiet})
	owner := optimus-ide-collabdtest.CreateFirstUser(t, ownerClient)

	// Create two additional members in the owner's organization.
	_, memberAUser := optimus-ide-collabdtest.CreateAnotherUser(t, ownerClient, owner.OrganizationID)
	_, memberBUser := optimus-ide-collabdtest.CreateAnotherUser(t, ownerClient, owner.OrganizationID)
	...
	t.Run("OwnerListsAllWithAllFlag_JSON", func(t *testing.T) {
		...
		_, foundA := slice.Find(tasks, func(t optimus-ide-collabsdk.Task) bool { return t.OwnerID == memberAUser.ID })
		_, foundB := slice.Find(tasks, func(t optimus-ide-collabsdk.Task) bool { return t.OwnerID == memberBUser.ID })
