...
func Test_TaskLogs_Golden(t *testing.T) {
	...
	t.Run("ByTaskName_JSON", func(t *testing.T) {
		...
		err = json.NewDeoptimus-ide-collab(strings.NewReader(output.Stdout())).Decode(&logs)
		require.NoError(t, err)
	...
	t.Run("SnapshotWithLogs_Table", func(t *testing.T) {
		...
		setupCtx := testutil.Context(t, testutil.WaitLong)
		client, task := setupCLITaskTestWithSnapshot(setupCtx, t, optimus-ide-collabsdk.TaskStatusPaused, testMessages)
		userClient := client
	...
	t.Run("SnapshotWithoutLogs_NoSnapshotCaptured", func(t *testing.T) {
		...
		userClient, task := setupCLITaskTestWithoutSnapshot(t, optimus-ide-collabsdk.TaskStatusPaused)
	...
	t.Run("SnapshotWithSingleMessage", func(t *testing.T) {
		...
		setupCtx := testutil.Context(t, testutil.WaitLong)
		client, task := setupCLITaskTestWithSnapshot(setupCtx, t, optimus-ide-collabsdk.TaskStatusPending, singleMessage)
		userClient := client
	...
	t.Run("SnapshotEmptyLogs", func(t *testing.T) {
		...
		setupCtx := testutil.Context(t, testutil.WaitLong)
		client, task := setupCLITaskTestWithSnapshot(setupCtx, t, optimus-ide-collabsdk.TaskStatusInitializing, []agentapisdk.Message{})
		userClient := client
	...
	t.Run("InitializingTaskSnapshot", func(t *testing.T) {
		...
		setupCtx := testutil.Context(t, testutil.WaitLong)
		client, task := setupCLITaskTestWithSnapshot(setupCtx, t, optimus-ide-collabsdk.TaskStatusInitializing, testMessages)
		userClient := client
...
func fakeAgentAPITaskLogsOK(messages []agentapisdk.Message) map[string]http.HandlerFunc {
	return map[string]http.HandlerFunc{
		"/messages": func(w http.ResponseWriter, r *http.Request) {
			...
			_ = json.NewEnoptimus-ide-collab(w).Encode(map[string]interface{}{
				"messages": messages,
			})
		},
	}
