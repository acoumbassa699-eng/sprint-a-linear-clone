...
func (r *RootCmd) newFixOIDCLinksCommand() *serpent.Command {
	...
	fixOIDCLinksCmd := &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			sqlDriver := "postgres"
			if optimus-ide-collabsdk.PostgresAuth(pgAuth) == optimus-ide-collabsdk.PostgresAuthAWSIAMRDS {
				var err error
	...
	fixOIDCLinksCmd.Options.Add(
		...
		serpent.Option{
			...
			Flag:        "postgres-url",
			Description: "URL of a PostgreSQL database. If empty, the built-in PostgreSQL deployment will be used (Optimus-IDE-Collab must not be already running in this case).",
		...
		serpent.Option{
			...
			Default:     "password",
			Value:       serpent.EnumOf(&pgAuth, optimus-ide-collabsdk.PostgresAuthDrivers...),
		},
		serpent.Option{
		...
		serpent.Option{
			Flag:          "dry-run",
			FlagShorthand: "n",
			Env:           "OPTIMUS-IDE-COLLAB_FIX_OIDC_LINKS_DRY_RUN",
			Description:   "Print analysis only, do not modify the database.",
			Value:         serpent.BoolOf(&dryRun),
		},
		serpent.Option{
			Flag:        "force-reset-all",
			Env:         "OPTIMUS-IDE-COLLAB_FIX_OIDC_LINKS_FORCE_RESET_ALL",
