...
import (
	...
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/support"
...
func (r *RootCmd) support() *serpent.Command {
	supportCmd := &serpent.Command{
		Use:   "support",
		Short: "Commands for troubleshooting issues with a Optimus-IDE-Collab deployment.",
...
func supportBundleBlurb(workspaceFilePatterns []string) string {
	blurb := cliui.Bold("This will collect the following information:\n") +
		`  - Optimus-IDE-Collab deployment version
  - Optimus-IDE-Collab deployment Configuration (sanitized), including enabled experiments
  - Optimus-IDE-Collab deployment health snapshot
  - Optimus-IDE-Collab deployment stats (aggregated workspace/session metrics)
  - Entitlements (if available)
  - Health settings (dismissed healthchecks)
  - Optimus-IDE-Collab deployment Network troubleshooting information
...
func (r *RootCmd) supportBundle() *serpent.Command {
	...
	var optimus-ide-collabURLOverride string
	...
	cmd := &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			if ok, err := support.CanGenerateFull(inv.Context(), client); err == nil && ok {
				...
				client.HTTPClient.Transport = &optimus-ide-collabsdk.HeaderTransport{
					Transport: client.HTTPClient.Transport,
					Header:    http.Header{optimus-ide-collabsdk.BypassRatelimitHeader: {"true"}},
				}
			} else if !ok {
			...
			if val, found := os.LookupEnv("OPTIMUS-IDE-COLLAB"); found && val == "true" {
				cliui.Warn(inv.Stderr, "Running inside Optimus-IDE-Collab workspace; this can affect results!")
				cliLog.Debug(inv.Context(), "running inside optimus-ide-collab workspace")
			}

			if optimus-ide-collabURLOverride != "" && optimus-ide-collabURLOverride != client.URL.String() {
				u, err := url.Parse(optimus-ide-collabURLOverride)
				if err != nil {
					return xerrors.Errorf("invalid value for Optimus-IDE-Collab URL override: %w", err)
				}
				_, _ = fmt.Fprintf(inv.Stderr, "Overrode Optimus-IDE-Collab URL to %q; this can affect results!\n", optimus-ide-collabURLOverride)
				cliLog.Debug(inv.Context(), "optimus-ide-collab url overridden", slog.F("url", optimus-ide-collabURLOverride))
				client.URL = u
			}
			...
			if len(inv.Args) == 0 {
				...
				// Prefer OPTIMUS-IDE-COLLAB_WORKSPACE_ID for a direct UUID lookup;
				// fall back to owner/name for older agents that do not
				// set the ID variable.
				if inv.Environ.Get("OPTIMUS-IDE-COLLAB") == "true" {
					var wsArg string
					if v := inv.Environ.Get("OPTIMUS-IDE-COLLAB_WORKSPACE_ID"); v != "" {
						wsArg = v
					} else {
						wsOwner := inv.Environ.Get("OPTIMUS-IDE-COLLAB_WORKSPACE_OWNER_NAME")
						wsName := inv.Environ.Get("OPTIMUS-IDE-COLLAB_WORKSPACE_NAME")
						if wsOwner != "" && wsName != "" {
							wsArg = wsOwner + "/" + wsName
						}
					}
					agtName := inv.Environ.Get("OPTIMUS-IDE-COLLAB_WORKSPACE_AGENT_NAME")
					if wsArg != "" {
			...
			if outputPath == "" {
				...
				fname := fmt.Sprintf("optimus-ide-collab-support-%d.zip", time.Now().Unix())
	...
	cmd.Options = serpent.OptionSet{
		cliui.SkipPromptOption(),
		{
			Flag:          "output-file",
			FlagShorthand: "O",
			Env:           "OPTIMUS-IDE-COLLAB_SUPPORT_BUNDLE_OUTPUT_FILE",
			Description:   "File path for writing the generated support bundle. Defaults to optimus-ide-collab-support-$(date +%s).zip.",
			Value:         serpent.StringOf(&outputPath),
		},
		{
			Flag:        "url-override",
			Env:         "OPTIMUS-IDE-COLLAB_SUPPORT_BUNDLE_URL_OVERRIDE",
			Description: "Override the URL to your Optimus-IDE-Collab deployment. This may be useful, for example, if you need to troubleshoot a specific Optimus-IDE-Collab replica.",
			Value:       serpent.StringOf(&optimus-ide-collabURLOverride),
		},
		{
			Flag:        "workspaces-total-cap",
			Env:         "OPTIMUS-IDE-COLLAB_SUPPORT_BUNDLE_WORKSPACES_TOTAL_CAP",
		...
		{
			Flag:        "template",
			Env:         "OPTIMUS-IDE-COLLAB_SUPPORT_BUNDLE_TEMPLATE",
		...
		{
			Flag:        "workspace-file",
			Env:         "OPTIMUS-IDE-COLLAB_SUPPORT_BUNDLE_WORKSPACE_FILE",
		...
		{
			Flag:        "pprof",
			Env:         "OPTIMUS-IDE-COLLAB_SUPPORT_BUNDLE_PPROF",
			Description: "Collect pprof profiling data from the Optimus-IDE-Collab server and agent. Requires Optimus-IDE-Collab server version 2.28.0 or newer.",
...
func resolveTemplateID(ctx context.Context, client *optimus-ide-collabsdk.Client, templateArg string) (uuid.UUID, error) {
	orgPart := ""
	...
	resolveInOrg := func(orgID uuid.UUID) (optimus-ide-collabsdk.Template, bool, error) {
		if t, err := client.TemplateByName(ctx, orgID, namePart); err == nil {
			return t, true, nil
		}
		tpls, err := client.TemplatesByOrganization(ctx, orgID)
		if err != nil {
			return optimus-ide-collabsdk.Template{}, false, nil
		}
		for _, t := range tpls {
	...
	var (
		foundTpl  optimus-ide-collabsdk.Template
		foundOrgs []string
	)
...
func findAgent(agentName string, haystack []optimus-ide-collabsdk.WorkspaceResource) (*optimus-ide-collabsdk.WorkspaceAgent, bool) {
...
func humanizeAgentLogs(ls []optimus-ide-collabsdk.WorkspaceAgentLog) string {
...
func humanizeBuildLogs(ls []optimus-ide-collabsdk.ProvisionerJobLog) string {
	var buf bytes.Buffer
	tw := tabwriter.NewWriter(&buf, 0, 2, 1, ' ', 0)
	for _, l := range ls {
...
func humanizeLicenses(licenses []optimus-ide-collabsdk.License) (string, error) {
