...
func Test_TaskSend(t *testing.T) {
	...
	t.Run("WaitsForInitializingTask", func(t *testing.T) {
		...
		optimus-ide-collabdtest.NewWorkspaceAgentWaiter(t, setup.userClient, setup.task.WorkspaceID.UUID).
		...
		require.NoError(t, err)
		require.Equal(t, optimus-ide-collabsdk.TaskStatusActive, updated.Status)
	})
	...
	t.Run("SendToNonIdleAppState", func(t *testing.T) {
		...
		for _, appState := range []optimus-ide-collabsdk.WorkspaceAppStatusState{
			optimus-ide-collabsdk.WorkspaceAppStatusStateComplete,
			optimus-ide-collabsdk.WorkspaceAppStatusStateFailure,
		} {
...
func fakeAgentAPITaskSendOK(t *testing.T, expectMessage, returnMessage string) map[string]http.HandlerFunc {
	return map[string]http.HandlerFunc{
		...
		"/message": func(w http.ResponseWriter, r *http.Request) {
			...
			if err := json.NewDeoptimus-ide-collab(r.Body).Decode(&msg); err != nil {
			...
			_ = json.NewEnoptimus-ide-collab(w).Encode(message)
		},
	}
}
