...
func (r *RootCmd) openVSCode() *serpent.Command {
	...
	cmd := &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			insideAWorkspace := inv.Environ.Get("OPTIMUS-IDE-COLLAB") == "true"
			inWorkspaceName := inv.Environ.Get("OPTIMUS-IDE-COLLAB_WORKSPACE_NAME") + "." + inv.Environ.Get("OPTIMUS-IDE-COLLAB_WORKSPACE_AGENT_NAME")

			// We need a started workspace to figure out e.g. expanded directory.
			// Pehraps the vscode-optimus-ide-collab extension could handle this by accepting
			...
			var parentWorkspaceAgent optimus-ide-collabsdk.WorkspaceAgent
			var devcontainer optimus-ide-collabsdk.WorkspaceAgentDevcontainer
			if workspaceAgent.ParentID.Valid {
				...
				for {
					...
					if devcontainer.ID == uuid.Nil {
						...
						parentWorkspaceAgent = optimus-ide-collabsdk.WorkspaceAgent{} // Reset to empty, so we don't use it later.
						break
					}

					// Precondition, the devcontainer must be running to enter
					// it. Once running, devcontainer.Container will be set.
					if devcontainer.Status == optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatusRunning {
						break
					}
					if devcontainer.Status != optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatusStarting {
						return xerrors.Errorf("devcontainer %q is in unexpected status %q, expected %q or %q",
							devcontainer.Name, devcontainer.Status,
							optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatusRunning,
							optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatusStarting,
						)
					}

					if !printedWaiting {
						_, _ = fmt.Fprintf(inv.Stderr, "Waiting for devcontainer %q status to change from %q to %q...\n", devcontainer.Name, devcontainer.Status, optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatusRunning)
						printedWaiting = true
					}
			...
			if !insideThisWorkspace {
				...
				if workspaceAgent.Directory != "" {
					workspace, workspaceAgent, err = waitForAgentCond(ctx, client, workspace, workspaceAgent, func(wa optimus-ide-collabsdk.WorkspaceAgent) bool {
						return wa.LifecycleState != optimus-ide-collabsdk.WorkspaceAgentLifecycleCreated
					})
					if err != nil {
			...
			if !insideAWorkspace || generateToken {
				...
				apiKey, err := client.CreateAPIKey(ctx, optimus-ide-collabsdk.Me)
				if err != nil {
			...
			if err != nil {
				if !generateToken {
					...
					wait := doAsync(func() {
						...
						_ = client.DeleteAPIKey(ctx, optimus-ide-collabsdk.Me, apiKeyID)
					})
					defer wait()
	...
	cmd.Options = serpent.OptionSet{
		{
			Flag: "generate-token",
			Env:  "OPTIMUS-IDE-COLLAB_OPEN_VSCODE_GENERATE_TOKEN",
...
func (r *RootCmd) openApp() *serpent.Command {
	...
	cmd := &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			appSlug := inv.Args[1]
			var foundApp optimus-ide-collabsdk.WorkspaceApp
			appIdx := slices.IndexFunc(agt.Apps, func(a optimus-ide-collabsdk.WorkspaceApp) bool {
			...
			var region optimus-ide-collabsdk.Region
			preferredIdx := slices.IndexFunc(regions, func(r optimus-ide-collabsdk.Region) bool {
	...
	cmd.Options = serpent.OptionSet{
		{
			Flag: "region",
			Env:  "OPTIMUS-IDE-COLLAB_OPEN_APP_REGION",
			Description: fmt.Sprintf("Region to use when opening the app." +
				" By default, the app will be opened using the main Optimus-IDE-Collab deployment (a.k.a. \"primary\")."),
...
func buildVSCodeWorkspaceLink(
	...
	return &url.URL{
		Scheme:   "vscode",
		Host:     "optimus-ide-collab.optimus-ide-collab-remote",
		Path:     "/open",
		RawQuery: qp.Encode(),
	}, qp
}
...
func waitForAgentCond(ctx context.Context, client *optimus-ide-collabsdk.Client, workspace optimus-ide-collabsdk.Workspace, workspaceAgent optimus-ide-collabsdk.WorkspaceAgent, cond func(optimus-ide-collabsdk.WorkspaceAgent) bool) (optimus-ide-collabsdk.Workspace, optimus-ide-collabsdk.WorkspaceAgent, error) {
...
func buildAppLinkURL(baseURL *url.URL, workspace optimus-ide-collabsdk.Workspace, agent optimus-ide-collabsdk.WorkspaceAgent, app optimus-ide-collabsdk.WorkspaceApp, appsHost, preferredPathBase string) string {
