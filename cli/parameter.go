...
func (wpf *workspaceParameterFlags) cliEphemeralParameters() []serpent.Option {
	return serpent.OptionSet{
		...
		{
			Flag:        "build-option",
			Env:         "OPTIMUS-IDE-COLLAB_BUILD_OPTION",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_EPHEMERAL_PARAMETER",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_PROMPT_EPHEMERAL_PARAMETERS",
...
func (wpf *workspaceParameterFlags) cliParameterDefaults() []serpent.Option {
	return serpent.OptionSet{
		serpent.Option{
			...
			Env:         "OPTIMUS-IDE-COLLAB_RICH_PARAMETER_DEFAULT",
...
func (wpf *workspaceParameterFlags) useParameterDefaultsOption() serpent.Option {
	return serpent.Option{
		...
		Env:         "OPTIMUS-IDE-COLLAB_WORKSPACE_USE_PARAMETER_DEFAULTS",
...
func presetParameterAsWorkspaceBuildParameters(presetParameters []optimus-ide-collabsdk.PresetParameter) []optimus-ide-collabsdk.WorkspaceBuildParameter {
	var params []optimus-ide-collabsdk.WorkspaceBuildParameter
	for _, parameter := range presetParameters {
		params = append(params, optimus-ide-collabsdk.WorkspaceBuildParameter(parameter))
	}
	return params
}

func asWorkspaceBuildParameters(nameValuePairs []string) ([]optimus-ide-collabsdk.WorkspaceBuildParameter, error) {
	var params []optimus-ide-collabsdk.WorkspaceBuildParameter
	for _, nameValue := range nameValuePairs {
		...
		params = append(params, optimus-ide-collabsdk.WorkspaceBuildParameter{
			Name:  split[0],
			Value: split[1],
		})
	}
...
This is useful for troubleshooting build issues.`,
		...
		},
		{
			...
			Value: serpent.EnumOf(
				&bf.reason,
				string(optimus-ide-collabsdk.BuildReasonCLI),
				string(optimus-ide-collabsdk.BuildReasonVSCodeConnection),
				string(optimus-ide-collabsdk.BuildReasonJetbrainsConnection),
			),
			Default: string(optimus-ide-collabsdk.BuildReasonCLI),
			Hidden:  true,
		},
	}
}
