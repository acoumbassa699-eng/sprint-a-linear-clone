...
func TestSecretsFileFormatFromPath(t *testing.T) {
	...
	tests := []struct {
		path string
		want optimus-ide-collabsdk.SecretsFileFormat
	}{
		{path: ".env", want: optimus-ide-collabsdk.SecretsFileFormatEnv},
		{path: "/tmp/prod.env", want: optimus-ide-collabsdk.SecretsFileFormatEnv},
		{path: "secrets.ENV", want: optimus-ide-collabsdk.SecretsFileFormatEnv},
		{path: "config.json", want: optimus-ide-collabsdk.SecretsFileFormatJSON},
		{path: "values.yaml", want: optimus-ide-collabsdk.SecretsFileFormatYAML},
		{path: "values.yml", want: optimus-ide-collabsdk.SecretsFileFormatYAML},
	}
	for _, tt := range tests {
...
func TestReadSecretsFile(t *testing.T) {
	...
	t.Run("AtMaxSize", func(t *testing.T) {
		...
		content := strings.Repeat("a", optimus-ide-collabsdk.MaxSecretsFileBytes)
		inv := newSecretTestInvocation(t, strings.NewReader(content), nil)

		got, err := readSecretsFile(inv, "-")
		require.NoError(t, err)
		require.Len(t, got, optimus-ide-collabsdk.MaxSecretsFileBytes)
	})
...
func TestWarnSecretsWithoutEnvNameEscapesNames(t *testing.T) {
	...
	var stderr bytes.Buffer
	warnSecretsWithoutEnvName(&stderr, []optimus-ide-collabsdk.UserSecret{{Name: "\x1b[31mBAD"}})
