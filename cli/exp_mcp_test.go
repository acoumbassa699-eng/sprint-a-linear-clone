...
// Used to mock github.com/optimus-ide-collab/agentapi events
const (
	ServerSentEventTypeMessageUpdate optimus-ide-collabsdk.ServerSentEventType = "message_update"
	ServerSentEventTypeStatusChange  optimus-ide-collabsdk.ServerSentEventType = "status_change"
)
...
func TestExpMcpServer(t *testing.T) {
	...
	t.Run("AllowedTools", func(t *testing.T) {
		...
		// Given: a running optimus-ide-collab deployment
		...
		inv, root := clitest.New(t, "exp", "mcp", "server", "--allowed-tools=optimus-ide-collab_get_authenticated_user")
		...
		require.Equal(t, []string{"optimus-ide-collab_get_authenticated_user"}, foundTools)
		...
		toolPayload := `{"jsonrpc":"2.0","id":3,"method":"tools/call", "params": {"name": "optimus-ide-collab_get_authenticated_user", "arguments": {}}}`
...
func TestExpMcpConfigureClaudeCode(t *testing.T) {
	...
	// optimus-ide-collabd server. Sub-tests that don't need one just ignore it.
	client := optimus-ide-collabdtest.New(t, nil)
	_ = optimus-ide-collabdtest.CreateFirstUser(t, client)

	t.Run("CustomOptimus-IDE-CollabPrompt", func(t *testing.T) {
		t.Parallel()
		...
		customOptimus-IDE-CollabPrompt := "This is a custom optimus-ide-collab prompt from flag."

		// This should include the custom optimus-ide-collabPrompt and reportTaskPrompt
		expectedClaudeMD := `<optimus-ide-collab-prompt>
Respect the requirements of the "optimus-ide-collab_report_task" tool. It is pertinent to provide a fantastic user-experience.

This is a custom optimus-ide-collab prompt from flag.
</optimus-ide-collab-prompt>
<system-prompt>
test-system-prompt
</system-prompt>
`
		inv, root := clitest.New(t, "exp", "mcp", "configure", "claude-code", "/path/to/project",
			...
			"--claude-test-binary-name=pathtotheoptimus-ide-collabbinary",
			"--claude-optimus-ide-collab-prompt="+customOptimus-IDE-CollabPrompt,
		)
		clitest.SetupConfig(t, client, root)
	...
	t.Run("NewConfig", func(t *testing.T) {
		...
		expectedConfig := `{
			...
			"projects": {
				"/path/to/project": {
					"allowedTools": [
						"mcp__optimus-ide-collab__optimus-ide-collab_report_task"
					],
					...
					"mcpServers": {
						"optimus-ide-collab": {
							"command": "pathtotheoptimus-ide-collabbinary",
							"args": ["exp", "mcp", "server"],
							"env": {
								"OPTIMUS-IDE-COLLAB_MCP_APP_STATUS_SLUG": "some-app-name",
								"OPTIMUS-IDE-COLLAB_MCP_AI_AGENTAPI_URL": "http://localhost:3284"
							}
						}
					}
				}
...
// optimus-ide-collab_report_task).
func TestExpMcpServerOptionalUserToken(t *testing.T) {
	...
	// Start a real socket server, but with a fake (Optimus-IDE-Collabd) AgentAPI.
	...
	fOptimus-IDE-CollabdAgentAPI := &fakeOptimus-IDE-CollabdAgentAPI{
		t:       t,
		testCtx: ctx,
	}
	socketServer.SetAgentAPI(fOptimus-IDE-CollabdAgentAPI)
	...
	// With agent socket but no user token, we should have the optimus-ide-collab_report_task tool available
	if toolsResponse.Error == nil {
		...
		require.Greater(t, len(toolsResponse.Result.Tools), 0,
			"There should be at least one tool available (optimus-ide-collab_report_task)")

		// Check specifically for the optimus-ide-collab_report_task tool
		var hasReportTaskTool bool
		for _, tool := range toolsResponse.Result.Tools {
			if tool.Name == "optimus-ide-collab_report_task" {
				hasReportTaskTool = true
				break
			}
		}
		require.True(t, hasReportTaskTool,
			"The optimus-ide-collab_report_task tool should be available with agent token")
	} else {
		// We got an error response which doesn't match expectations
		// (When OPTIMUS-IDE-COLLAB_AGENT_TOKEN and app status are set, tools/list should work)
...
func TestExpMcpReporter(t *testing.T) {
	...
	makeStatusEvent := func(status agentapi.AgentStatus) *optimus-ide-collabsdk.ServerSentEvent {
		return &optimus-ide-collabsdk.ServerSentEvent{
	...
	makeMessageEvent := func(id int64, role agentapi.ConversationRole) *optimus-ide-collabsdk.ServerSentEvent {
		return &optimus-ide-collabsdk.ServerSentEvent{
	...
	type testCase struct {
		...
		event *optimus-ide-collabsdk.ServerSentEvent
	...
	runs := []struct {
		...
		// that it finished.
		{
			...
			testCases: []testCase{
				// First the AI agent updates with a state change.
				{
					...
					summary: "doing work",
					uri:     "https://dev.optimus-ide-collab.com",
					expected: &agentproto.UpdateAppStatusRequest{
		...
		// We override idle from the agent to working, but trust final states.
		{
			...
			testCases: []testCase{
				...
				{
					state:   optimus-ide-collabsdk.WorkspaceAppStatusStateFailure,
	...
	for _, run := range runs {
		t.Run(run.name, func(t *testing.T) {
			...
			args := []string{
				...
				"--socket-path", socketPath,
				"--app-status-slug", "vscode",
				"--allowed-tools=optimus-ide-collab_report_task",
			}

			// Mock the AI AgentAPI server.
			listening := make(chan func(sse optimus-ide-collabsdk.ServerSentEvent) error)
			...
			var sender func(sse optimus-ide-collabsdk.ServerSentEvent) error
			if !run.disableAgentAPI {
				sender = <-listening
			}
			...
			for _, tc := range run.testCases {
				if tc.event != nil {
					...
					// Call the tool and ensure it works.
					payload := fmt.Sprintf(`{"jsonrpc":"2.0","id":3,"method":"tools/call", "params": {"name": "optimus-ide-collab_report_task", "arguments": {"state": %q, "summary": %q, "link": %q}}}`, tc.state, tc.summary, tc.uri)
					stdin.WriteLine(payload)
					output := stdout.ReadLine(ctx)
					require.NotEmpty(t, output, "did not receive a response from optimus-ide-collab_report_task")
					// Ensure it is valid JSON.
					_, err := json.Marshal(output)
					require.NoError(t, err, "did not receive valid JSON from optimus-ide-collab_report_task")
				}
				if tc.expected != nil {
	...
	t.Run("Reconnect", func(t *testing.T) {
		...
		srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			send, closed, err := httpapi.ServerSentEventSender(w, r)
			if err != nil {
				httpapi.Write(sseCtx, w, http.StatusInternalServerError, optimus-ide-collabsdk.Response{
					Message: "Internal error setting up server-sent events.",
					Detail:  err.Error(),
				})
				return
		...
		toolPayload := `{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"optimus-ide-collab_report_task","arguments":{"state":"working","summary":"doing work","link":""}}}`
		stdin.WriteLine(toolPayload)
		...
		toolPayload = `{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"optimus-ide-collab_report_task","arguments":{"state":"working","summary":"reconnected","link":""}}}`
		stdin.WriteLine(toolPayload)
		_ = stdout.ReadLine(ctx) // ignore response
		got = testutil.RequireReceive(ctx, t, requests)
		require.Equal(t, agentproto.UpdateAppStatusRequest_WORKING, got.State)
...
type fakeOptimus-IDE-CollabdAgentAPI struct {
...
func (f *fakeOptimus-IDE-CollabdAgentAPI) UpdateAppStatus(ctx context.Context, req *agentproto.UpdateAppStatusRequest) (*agentproto.UpdateAppStatusResponse, error) {
	select {
	case f.requests <- req:
	case <-f.testCtx.Done():
