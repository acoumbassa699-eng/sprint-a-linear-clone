...
func TestSupportBundle(t *testing.T) {
	...
	// Support bundle tests can share a single optimus-ide-collabdtest instance.
	var dc optimus-ide-collabsdk.DeploymentConfig
	dc.Values = optimus-ide-collabdtest.DeploymentValues(t)
	...
	t.Run("WorkspaceWithAgent", func(t *testing.T) {
		...
		tempDir := t.TempDir()
		logPath := filepath.Join(tempDir, "optimus-ide-collab-agent.log")
		...
		defer agt.Close()
		optimus-ide-collabdtest.NewWorkspaceAgentWaiter(t, client, workspaceWithAgent.Workspace.ID).Wait()
	...
	t.Run("WorkspaceWithRotatedAgentLogs", func(t *testing.T) {
		...
		rotatedPath := filepath.Join(tempDir, "optimus-ide-collab-agent-2026-05-18T00-00-00.000.log")
		require.NoError(t, os.WriteFile(rotatedPath, []byte("rotated log"), 0o600))
		oldRotatedPath := filepath.Join(tempDir, "optimus-ide-collab-agent-2026-05-17T00-00-00.000.log")
		...
		defer agt.Close()
		optimus-ide-collabdtest.NewWorkspaceAgentWaiter(t, client, workspaceWithRotatedAgentLogs.Workspace.ID).Wait()
	...
	t.Run("InferWorkspaceFromEnvByID", func(t *testing.T) {
		...
		inv, root := clitest.New(t, "support", "bundle", "--output-file", path, "--yes")
		inv.Environ.Set("OPTIMUS-IDE-COLLAB", "true")
		inv.Environ.Set("OPTIMUS-IDE-COLLAB_WORKSPACE_ID", workspaceWithoutAgent.Workspace.ID.String())
		inv.Environ.Set("OPTIMUS-IDE-COLLAB_WORKSPACE_AGENT_NAME", "dev")
		//nolint: gocritic // requires owner privilege
		clitest.SetupConfig(t, client, root)
		err := inv.Run()
		require.NoError(t, err)
	...
	t.Run("InferWorkspaceFromEnvByName", func(t *testing.T) {
		...
		d := t.TempDir()
		path := filepath.Join(d, "bundle.zip")
		// No workspace arg and no OPTIMUS-IDE-COLLAB_WORKSPACE_ID; fall back to
		// owner/name resolution for older agents.
		inv, root := clitest.New(t, "support", "bundle", "--output-file", path, "--yes")
		inv.Environ.Set("OPTIMUS-IDE-COLLAB", "true")
		inv.Environ.Set("OPTIMUS-IDE-COLLAB_WORKSPACE_NAME", workspaceWithoutAgent.Workspace.Name)
		inv.Environ.Set("OPTIMUS-IDE-COLLAB_WORKSPACE_OWNER_NAME", optimus-ide-collabdtest.FirstUserParams.Username)
		inv.Environ.Set("OPTIMUS-IDE-COLLAB_WORKSPACE_AGENT_NAME", "dev")
		//nolint: gocritic // requires owner privilege
		clitest.SetupConfig(t, client, root)
		err := inv.Run()
			...
			t.Run(http.StatusText(code), func(t *testing.T) {
				...
				srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
					...
					case "/api/v2/users/me":
						resp := optimus-ide-collabsdk.User{}
						w.WriteHeader(http.StatusOK)
						assert.NoError(t, json.NewEnoptimus-ide-collab(w).Encode(resp))
					case "/api/v2/authcheck":
						// Fake auth check
						resp := optimus-ide-collabsdk.AuthorizationResponse{
				...
				require.NoError(t, err)
				client := optimus-ide-collabsdk.New(u)
...
func TestSupportBundleCollectsWorkspaceFiles(t *testing.T) {
	...
	logDir := t.TempDir()
	require.NoError(t, os.WriteFile(filepath.Join(logDir, "optimus-ide-collab-agent.log"), []byte("hello from the agent"), 0o600))
	agt := agenttest.New(t, client.URL, workspaceWithAgent.AgentToken, func(o *agent.Options) {
		o.LogDir = logDir
	})
	defer agt.Close()
...
func assertBundleContents(t *testing.T, path string, wantWorkspace bool, wantAgent bool, badValues []string) {
	...
	for _, f := range r.File {
		...
		case "workspace/template.json":
			var v optimus-ide-collabsdk.Template
			decodeJSONFromZip(t, f, &v)
			if !wantWorkspace {
		...
		case "workspace/template_version.json":
			var v optimus-ide-collabsdk.TemplateVersion
			decodeJSONFromZip(t, f, &v)
			if !wantWorkspace {
		...
		case "workspace/parameters.json":
			var v []optimus-ide-collabsdk.WorkspaceBuildParameter
			decodeJSONFromZip(t, f, &v)
			if !wantWorkspace {
		...
		case "agent/agent.json":
			var v optimus-ide-collabsdk.WorkspaceAgent
			decodeJSONFromZip(t, f, &v)
			if !wantAgent {
		...
		case "agent/listening_ports.json":
			var v optimus-ide-collabsdk.WorkspaceAgentListeningPortsResponse
			decodeJSONFromZip(t, f, &v)
			if !wantAgent {
...
func decodeJSONFromZip(t *testing.T, f *zip.File, dest any) {
	...
	defer rc.Close()
	require.NoError(t, json.NewDeoptimus-ide-collab(rc).Decode(&dest))
}
...
func seedSecretDeploymentOptions(t *testing.T, dc *optimus-ide-collabsdk.DeploymentConfig, secretValue string) {
	t.Helper()
	if dc == nil {
		dc = &optimus-ide-collabsdk.DeploymentConfig{}
	}
	for _, opt := range dc.Options {
		if optimus-ide-collabsdk.IsSecretDeploymentOption(opt) {
