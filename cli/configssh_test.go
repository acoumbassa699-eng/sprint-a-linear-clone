...
func TestConfigSSH(t *testing.T) {
	...
	if runtime.GOOS == "windows" {
		t.Skip("See optimus-ide-collab/internal#117")
	}

	logger := testutil.Logger(t)
	ctx := testutil.Context(t, testutil.WaitMedium)
	const hostname = "test-optimus-ide-collab."
	...
	// Set HOME because optimus-ide-collab config is included from ~/.ssh/optimus-ide-collab.
...
func TestConfigSSH_RejectsUnsafeServerConfig(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:      "HostnameSuffix",
			configSSH: optimus-ide-collabsdk.SSHConfigResponse{HostnameSuffix: "optimus-ide-collab\nHost *"},
			wantErr:   "workspace hostname suffix",
		},
		{
			name:      "HostnamePrefix",
			configSSH: optimus-ide-collabsdk.SSHConfigResponse{HostnamePrefix: "optimus-ide-collab.\nHost *"},
			wantErr:   "workspace hostname prefix",
		},
		{
			name:      "HostnameSuffixGlob",
			configSSH: optimus-ide-collabsdk.SSHConfigResponse{HostnameSuffix: "*"},
			wantErr:   "glob",
		},
	}
...
func TestConfigSSH_FileWriteAndOptionsFlow(t *testing.T) {
	...
	headerStart := strings.Join([]string{
		"# ------------START-OPTIMUS-IDE-COLLAB-----------",
		"# This section is managed by optimus-ide-collab. DO NOT EDIT.",
		"#",
		"# You should not hand-edit this section unless you are removing it, all",
		"# changes will be lost when running \"optimus-ide-collab config-ssh\".",
		"#",
	}, "\n")
	headerEnd := "# ------------END-OPTIMUS-IDE-COLLAB------------"
	...
	tests := []struct {
			...
			wantConfig: wantConfig{
				ssh: []string{
					strings.Join([]string{
						...
						"# Last config-ssh options:",
						"# :wait=yes",
						"# :ssh-host-prefix=optimus-ide-collab-test.",
						"# :hostname-suffix=optimus-ide-collab-suffix",
		...
		},
		{
			...
			args: []string{
				"--yes",
				"--hostname-suffix", "optimus-ide-collab",
				"--no-wildcard",
			},
			hasAgent: true,
			wantConfig: wantConfig{
				...
				regexMatch: `Host [a-z0-9_-]+\.optimus-ide-collab`,
			},
		},
		{
		...
		},
		{
			...
			writeConfig: writeConfig{
				ssh: strings.Join([]string{
					"# Content before optimus-ide-collab block",
					headerEnd,
					headerStart,
					"# Content after optimus-ide-collab block",
				}, "\n"),
			},
			wantErr: true,
		},
		...
		},
		{
			...
			args: []string{
				"-y", "--optimus-ide-collab-binary-path", "/foo/bar/optimus-ide-collab",
			},
			wantErr:  false,
			hasAgent: true,
			wantConfig: wantConfig{
				regexMatch: "ProxyCommand /foo/bar/optimus-ide-collab",
			},
		},
		{
			name: "Header",
			...
			hasAgent: true,
			wantConfig: wantConfig{
				regexMatch: `ProxyCommand .* --header "X-Test-Header=foo" --header "X-Test-Header2=bar" ssh .* --ssh-host-prefix optimus-ide-collab. %h`,
			},
		},
		{
			name: "Header command",
			...
			hasAgent: true,
			wantConfig: wantConfig{
				regexMatch: `ProxyCommand .* --header-command "echo h1=v1" ssh .* --ssh-host-prefix optimus-ide-collab. %h`,
			},
		},
		{
			...
			hasAgent: true,
			wantConfig: wantConfig{
				regexMatch: `ProxyCommand .* --header-command "echo h1=v1 h2=\\\"v2\\\"" ssh .* --ssh-host-prefix optimus-ide-collab. %h`,
			},
		},
		{
		...
		{
			// Regression test for https://github.com/optimus-ide-collab/internal/issues/1208:
			...
			wantConfig: wantConfig{
				...
				notWant: []string{"Host optimus-ide-collab.*", "--ssh-host-prefix optimus-ide-collab."},
			},
		},
		{
			...
			args: []string{"--yes"},
			env: map[string]string{
				"OPTIMUS-IDE-COLLAB_CONFIGSSH_SSH_HOST_PREFIX": "",
			},
			wantErr: false,
			wantConfig: wantConfig{
				ssh: []string{
...
func TestConfigSSH_NoWildcard(t *testing.T) {
	...
	// The server always injects a "optimus-ide-collab." hostname prefix in addition to the
	// user-supplied "--hostname-suffix optimus-ide-collab" entries. With stable workspace
	...
	wantHosts := []string{
		"Host optimus-ide-collab." + ws2.Workspace.Name,      // optimus-ide-collab.ws-alpha
		"Host optimus-ide-collab." + ws1.Workspace.Name,      // optimus-ide-collab.ws-beta
		"Host " + ws2.Workspace.Name + ".optimus-ide-collab", // ws-alpha.optimus-ide-collab
		"Host " + ws1.Workspace.Name + ".optimus-ide-collab", // ws-beta.optimus-ide-collab
	}
	require.Empty(t, cmp.Diff(wantHosts, hostLines(config)))

	// No wildcard entries must appear in the Optimus-IDE-Collab section.
	require.NotContains(t, config, "Host *.optimus-ide-collab")
