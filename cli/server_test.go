...
func TestReadExternalAuthProvidersFromEnv(t *testing.T) {
	...
	t.Run("Valid", func(t *testing.T) {
		...
		providers, err := cli.ReadExternalAuthProvidersFromEnv([]string{
			"OPTIMUS-IDE-COLLAB_EXTERNAL_AUTH_0_ID=1",
			"OPTIMUS-IDE-COLLAB_EXTERNAL_AUTH_0_TYPE=gitlab",
			"OPTIMUS-IDE-COLLAB_EXTERNAL_AUTH_1_ID=2",
			"OPTIMUS-IDE-COLLAB_EXTERNAL_AUTH_1_CLIENT_ID=sid",
			"OPTIMUS-IDE-COLLAB_EXTERNAL_AUTH_1_CLIENT_SECRET=hunter12",
			"OPTIMUS-IDE-COLLAB_EXTERNAL_AUTH_1_TOKEN_URL=google.com",
			"OPTIMUS-IDE-COLLAB_EXTERNAL_AUTH_1_VALIDATE_URL=bing.com",
			"OPTIMUS-IDE-COLLAB_EXTERNAL_AUTH_1_REVOKE_URL=revoke.url",
			"OPTIMUS-IDE-COLLAB_EXTERNAL_AUTH_1_SCOPES=repo:read repo:write",
			"OPTIMUS-IDE-COLLAB_EXTERNAL_AUTH_1_NO_REFRESH=true",
			"OPTIMUS-IDE-COLLAB_EXTERNAL_AUTH_1_DISPLAY_NAME=Google",
			"OPTIMUS-IDE-COLLAB_EXTERNAL_AUTH_1_DISPLAY_ICON=/icon/google.svg",
		})
		require.NoError(t, err)
	...
	t.Run("MoreThan10Providers", func(t *testing.T) {
		...
		for i := 0; i < count; i++ {
			environ = append(environ,
				fmt.Sprintf("OPTIMUS-IDE-COLLAB_EXTERNAL_AUTH_%d_ID=id-%d", i, i),
				fmt.Sprintf("OPTIMUS-IDE-COLLAB_EXTERNAL_AUTH_%d_TYPE=type-%d", i, i),
			)
		}
...
func TestReadExternalAuthProvidersFromEnv_APIBaseURL(t *testing.T) {
	...
	providers, err := cli.ReadExternalAuthProvidersFromEnv([]string{
		...
		"OPTIMUS-IDE-COLLAB_EXTERNAL_AUTH_0_API_BASE_URL=https://ghes.corp.com/api/v3",
	})
	require.NoError(t, err)
...
// TestReadGitAuthProvidersFromEnv ensures that the deprecated `OPTIMUS-IDE-COLLAB_GITAUTH_`
...
func TestReadGitAuthProvidersFromEnv(t *testing.T) {
	...
	t.Run("InvalidKey", func(t *testing.T) {
		t.Parallel()
		providers, err := cli.ReadExternalAuthProvidersFromEnv([]string{
			"OPTIMUS-IDE-COLLAB_GITAUTH_XXX=invalid",
		})
	...
	t.Run("SkipKey", func(t *testing.T) {
		t.Parallel()
		providers, err := cli.ReadExternalAuthProvidersFromEnv([]string{
			"OPTIMUS-IDE-COLLAB_GITAUTH_0_ID=invalid",
			"OPTIMUS-IDE-COLLAB_GITAUTH_2_ID=invalid",
		})
	...
	t.Run("Valid", func(t *testing.T) {
		t.Parallel()
		providers, err := cli.ReadExternalAuthProvidersFromEnv([]string{
			"OPTIMUS-IDE-COLLAB_GITAUTH_0_ID=1",
			"OPTIMUS-IDE-COLLAB_GITAUTH_0_TYPE=gitlab",
			"OPTIMUS-IDE-COLLAB_GITAUTH_1_ID=2",
			"OPTIMUS-IDE-COLLAB_GITAUTH_1_CLIENT_ID=sid",
			"OPTIMUS-IDE-COLLAB_GITAUTH_1_CLIENT_SECRET=hunter12",
			"OPTIMUS-IDE-COLLAB_GITAUTH_1_TOKEN_URL=google.com",
			"OPTIMUS-IDE-COLLAB_GITAUTH_1_VALIDATE_URL=bing.com",
			"OPTIMUS-IDE-COLLAB_GITAUTH_1_SCOPES=repo:read repo:write",
			"OPTIMUS-IDE-COLLAB_GITAUTH_1_NO_REFRESH=true",
		})
		require.NoError(t, err)
		require.Len(t, providers, 2)
...
func TestServer(t *testing.T) {
	...
	t.Run("TLSAndHTTP", func(t *testing.T) {
		...
		require.NoError(t, err)
		client := optimus-ide-collabsdk.New(httpURL)
		...
		require.NoError(t, err)
		client = optimus-ide-collabsdk.New(tlsURL)
		client.HTTPClient.CheckRedirect = func(req *http.Request, via []*http.Request) error {
			return http.ErrUseLastResponse
		}
		client.HTTPClient = &http.Client{
	...
	t.Run("Prometheus", func(t *testing.T) {
		...
		t.Run("DBMetricsDisabled", func(t *testing.T) {
			...
			testutil.Eventually(ctx, t, func(ctx context.Context) bool {
				...
				for scanner.Scan() {
					...
					if strings.HasPrefix(line, "optimus-ide-collabd_db_query_latencies_seconds") {
						t.Errorf("db metrics should not be tracked when --prometheus-collect-db-metrics is not enabled")
					}
					// This metric is manually registered to be tracked in the server. That's
					// why we test it's tracked here.
					if strings.HasPrefix(line, "optimus-ide-collabd_api_active_users_duration_hour") {
						activeUsersFound = true
					}
				}
				return activeUsersFound
			}, testutil.IntervalSlow, "didn't find optimus-ide-collabd_api_active_users_duration_hour in time")
		})
		...
		t.Run("DBMetricsEnabled", func(t *testing.T) {
			...
			}, testutil.IntervalSlow, "didn't find optimus-ide-collabd_db_query_latencies_seconds in time")
		})
	})
	...
	t.Run("OIDC", func(t *testing.T) {
		...
		t.Run("Defaults", func(t *testing.T) {
			...
			// This is just needed to get Optimus-IDE-Collab to start up.
			...
			// These are the default values returned from the API. See optimus-ide-collabsdk/deployment.go for the default values.
		...
		t.Run("Overrides", func(t *testing.T) {
			...
			inv, cfg := clitest.New(t,
				...
				"--oidc-sign-in-text", "Sign In With Optimus-IDE-Collab",
			...
			require.Equal(t, "Sign In With Optimus-IDE-Collab", deploymentConfig.Values.OIDC.SignInText.Value())
			...
			for _, opt := range deploymentConfig.Options {
				...
				case "oidc-sign-in-text":
					require.Equal(t, "Sign In With Optimus-IDE-Collab", opt.Value.String())
		...
		t.Run("RedirectAllowedHosts", func(t *testing.T) {
			...
			inv, cfg := clitest.New(t,
				...
				"--oidc-client-secret", "fake",
				"--oidc-issuer-url", oidcServer.URL,
				"--oidc-redirect-allowed-hosts", "optimus-ide-collab.example.com,optimus-ide-collab-walle.example.com",
			)
			...
			require.Equal(t,
				[]string{"optimus-ide-collab.example.com", "optimus-ide-collab-walle.example.com"},
	...
	t.Run("Logging", func(t *testing.T) {
		...
		t.Run("CreatesFile", func(t *testing.T) {
			t.Parallel()
			fiName := testutil.TempFile(t, "", "optimus-ide-collab-logging-test-*")
		...
		t.Run("Human", func(t *testing.T) {
			t.Parallel()
			fi := testutil.TempFile(t, "", "optimus-ide-collab-logging-test-*")
	...
	t.Run("YAML", func(t *testing.T) {
		...
		t.Run("WriteThenReadConfig", func(t *testing.T) {
			...
			args := []string{
				...
				"--http-address", ":0",
				"--access-url", "http://example.com",
				"--log-human", filepath.Join(t.TempDir(), "optimus-ide-collab-logging-test-human"),
			...
			client := optimus-ide-collabsdk.New(gotURL)
			...
			fi, err := os.OpenFile(testutil.TempFile(t, "", "optimus-ide-collab-config-test-*"), os.O_WRONLY|os.O_CREATE, 0o600)
			require.NoError(t, err)
			defer fi.Close()
			...
			w = clitest.StartWithWaiter(t, inv)
			client = optimus-ide-collabsdk.New(waitAccessURL(t, cfg))
			_ = optimus-ide-collabdtest.CreateFirstUser(t, client)
...
func TestServer_InvalidSSHDeploymentConfig(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			flag:    "--workspace-hostname-suffix=.optimus-ide-collab",
		...
		},
		{
			...
			flag:    "--ssh-hostname-prefix=optimus-ide-collab.\nHost *",
...
func TestServer_ExternalAuthGitHubDefaultProvider(t *testing.T) {
	...
	run := func(t *testing.T, tc testCase) {
		...
		unsetPrefixedEnv("OPTIMUS-IDE-COLLAB_EXTERNAL_AUTH_")
		unsetPrefixedEnv("OPTIMUS-IDE-COLLAB_GITAUTH_")
		...
		const (
			existingUserEmail    = "existing-user@optimus-ide-collab.com"
		...
		gotProviders := map[string]optimus-ide-collabsdk.ExternalAuthLinkProvider{}
		...
		for _, providerID := range tc.expectedProviders {
			...
			if providerID == optimus-ide-collabsdk.EnhancedExternalAuthProviderGitHub.String() {
				require.Equal(t, optimus-ide-collabsdk.EnhancedExternalAuthProviderGitHub.String(), provider.Type)
	...
	for _, tc := range []testCase{
		{
			...
			expectedProviders: []string{optimus-ide-collabsdk.EnhancedExternalAuthProviderGitHub.String()},
		},
		{
		...
		},
		{
			...
			expectedProviders: []string{optimus-ide-collabsdk.EnhancedExternalAuthProviderGitLab.String()},
		},
		{
			name: "ExplicitProviderViaEnv_DoesNotInjectDefaultGithub",
			env: map[string]string{
				"OPTIMUS-IDE-COLLAB_EXTERNAL_AUTH_0_TYPE":      optimus-ide-collabsdk.EnhancedExternalAuthProviderGitLab.String(),
				"OPTIMUS-IDE-COLLAB_EXTERNAL_AUTH_0_CLIENT_ID": "env-client-id",
			},
			expectedProviders: []string{optimus-ide-collabsdk.EnhancedExternalAuthProviderGitLab.String()},
		},
		{
			...
			env: map[string]string{
				"OPTIMUS-IDE-COLLAB_GITAUTH_0_TYPE":      optimus-ide-collabsdk.EnhancedExternalAuthProviderGitLab.String(),
				"OPTIMUS-IDE-COLLAB_GITAUTH_0_CLIENT_ID": "legacy-env-client-id",
			},
			expectedProviders: []string{optimus-ide-collabsdk.EnhancedExternalAuthProviderGitLab.String()},
		},
	} {
...
func TestServer_Logging_NoParallel(t *testing.T) {
	...
	//
	// From cloud.google.com/go/compute/metadata/metadata.go (used by optimus-ide-collab/slog):
	//
	...
	t.Run("Multiple", func(t *testing.T) {
		...
		fi1 := testutil.TempFile(t, "", "optimus-ide-collab-logging-test-*")
		fi2 := testutil.TempFile(t, "", "optimus-ide-collab-logging-test-*")
		fi3 := testutil.TempFile(t, "", "optimus-ide-collab-logging-test-*")
...
func TestServer_Production(t *testing.T) {
	...
	_, err := client.CreateFirstUser(ctx, optimus-ide-collabdtest.FirstUserParams)
	require.NoError(t, err)
}
...
func TestServer_TelemetryDisable(t *testing.T) {
	...
	t.Setenv("OPTIMUS-IDE-COLLAB_TEST_TELEMETRY_DEFAULT_ENABLE", "true")
	...
	for _, tt := range []struct {
		...
		{"", "", true},
		{"OPTIMUS-IDE-COLLAB_TELEMETRY_ENABLE", "true", true},
		{"OPTIMUS-IDE-COLLAB_TELEMETRY_ENABLE", "false", false},
		{"OPTIMUS-IDE-COLLAB_TELEMETRY", "true", true},
		{"OPTIMUS-IDE-COLLAB_TELEMETRY", "false", false},
	} {
		t.Run(fmt.Sprintf("%s=%s", tt.key, tt.val), func(t *testing.T) {
			...
			var dv optimus-ide-collabsdk.DeploymentValues
...
func TestServer_InterruptShutdown(t *testing.T) {
	...
	require.NoError(t, err)
	// We cannot send more signals here, because it's possible Optimus-IDE-Collab
...
// API WebsocketWaitGroup, so it must be closed before optimus-ide-collabAPICloser.Close()
...
func TestServer_AIGatewayShutdownOrdering(t *testing.T) {
	...
	// The WebsocketWaitGroup timeout in optimus-ide-collabd.API.Close() is hard coded
	...
	require.Less(t, time.Since(shutdownStart), 8*time.Second,
		"graceful shutdown took too long; the in-memory AI Gateway daemon is "+
			"likely not being closed before optimus-ide-collabAPICloser.Close()")
}
...
func TestServerYAMLConfig(t *testing.T) {
	...
	var deployValues optimus-ide-collabsdk.DeploymentValues
	...
	enc := yaml.NewEnoptimus-ide-collab(&wantBuf)
	enc.SetIndent(2)
	err = enc.Encode(n)
	require.NoError(t, err)
...
func mockTelemetryServer(t *testing.T) (*url.URL, chan *telemetry.Deployment, chan *telemetry.Snapshot) {
	...
	r.Post("/deployment", func(w http.ResponseWriter, r *http.Request) {
		...
		err := json.NewDeoptimus-ide-collab(r.Body).Decode(dd)
		require.NoError(t, err)
		deployment <- dd
	...
	r.Post("/snapshot", func(w http.ResponseWriter, r *http.Request) {
		...
		err := json.NewDeoptimus-ide-collab(r.Body).Decode(ss)
		require.NoError(t, err)
		snapshot <- ss
