...
func setupTestSchedule(t *testing.T, sched *cron.Schedule) (ownerClient, memberClient *optimus-ide-collabsdk.Client, db database.Store, ws []optimus-ide-collabsdk.Workspace) {
	t.Helper()

	ownerClient, db = optimus-ide-collabdtest.NewWithDatabase(t, nil)
	owner := optimus-ide-collabdtest.CreateFirstUser(t, ownerClient)
	memberClient, memberUser := optimus-ide-collabdtest.CreateAnotherUserMutators(t, ownerClient, owner.OrganizationID, nil, func(r *optimus-ide-collabsdk.CreateUserRequestWithOrgs) {
	...
	resp, err := ownerClient.Workspaces(context.Background(), optimus-ide-collabsdk.WorkspaceFilter{})
...
func TestScheduleStart_TemplateAutostartRequirement(t *testing.T) {
	...
	template, err = client.UpdateTemplateMeta(context.Background(), template.ID, optimus-ide-collabsdk.UpdateTemplateMeta{
