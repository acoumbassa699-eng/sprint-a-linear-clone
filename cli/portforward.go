...
func (r *RootCmd) portForward() *serpent.Command {
	...
	cmd := &serpent.Command{
		...
		Short:   `Forward ports from a workspace to the local machine. For reverse port forwarding, use "optimus-ide-collab ssh -R".`,
		Aliases: []string{"tunnel"},
		Long: FormatExamples(
			Example{
				Description: "Port forward a single TCP port from 1234 in the workspace to port 5678 on your local machine",
				Command:     "optimus-ide-collab port-forward <workspace> --tcp 5678:1234",
			},
			Example{
				Description: "Port forward a single UDP port from port 9000 to port 9000 on your local machine",
				Command:     "optimus-ide-collab port-forward <workspace> --udp 9000",
			},
			Example{
				Description: "Port forward multiple TCP ports and a UDP port",
				Command:     "optimus-ide-collab port-forward <workspace> --tcp 8080:8080 --tcp 9000:3000 --udp 5353:53",
			},
			Example{
				Description: "Port forward multiple ports (TCP or UDP) in condensed syntax",
				Command:     "optimus-ide-collab port-forward <workspace> --tcp 8080,9000:3000,9090-9092,10000-10002:10010-10012",
			},
			Example{
				Description: "Port forward specifying the local address to bind to",
				Command:     "optimus-ide-collab port-forward <workspace> --tcp 1.2.3.4:8080:8080",
			},
		),
