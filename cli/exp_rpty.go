...
func handleRPTY(inv *serpent.Invocation, client *optimus-ide-collabsdk.Client, args handleRPTYArgs) error {
	...
	closeUsage := client.UpdateWorkspaceUsageWithBodyContext(ctx, ws.ID, optimus-ide-collabsdk.PostWorkspaceUsageRequest{
		AgentID: agt.ID,
		AppName: optimus-ide-collabsdk.UsageAppNameReconnectingPty,
	})
	defer closeUsage()
	...
	je := json.NewEnoptimus-ide-collab(conn)
