...
import (
	...
	agentapi "github.com/optimus-ide-collab/agentapi-sdk-go"
	...
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/optimus-ide-collabsdk/toolsdk"
	"github.com/optimus-ide-collab/retry"
	"github.com/optimus-ide-collab/serpent"
)

const (
	envAppStatusSlug = "OPTIMUS-IDE-COLLAB_MCP_APP_STATUS_SLUG"
	envAIAgentAPIURL = "OPTIMUS-IDE-COLLAB_MCP_AI_AGENTAPI_URL"
)

func (r *RootCmd) mcpCommand() *serpent.Command {
	cmd := &serpent.Command{
		Use:   "mcp",
		Short: "Run the Optimus-IDE-Collab MCP server and configure it to work with AI tools.",
		Long:  "The Optimus-IDE-Collab MCP server allows you to automatically create workspaces with parameters.",
	...
	cmd := &serpent.Command{
		...
		Handler: func(_ *serpent.Invocation) error {
			...
			contents["mcpServers"] = map[string]any{
				"optimus-ide-collab": map[string]any{"command": binPath, "args": []string{"exp", "mcp", "server"}},
			}
...
func mcpConfigureClaudeCode() *serpent.Command {
	var (
		...
		claudeMDPath     string
		systemPrompt     string
		optimus-ide-collabPrompt      string
		appStatusSlug    string
		testBinaryName   string
		aiAgentAPIURL    url.URL
		claudeUseBedrock string

		deprecatedOptimus-IDE-CollabMCPClaudeAPIKey string
	)
	cmd := &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			if deprecatedOptimus-IDE-CollabMCPClaudeAPIKey != "" {
				cliui.Warnf(inv.Stderr, "OPTIMUS-IDE-COLLAB_MCP_CLAUDE_API_KEY is deprecated, use CLAUDE_API_KEY instead")
				claudeAPIKey = deprecatedOptimus-IDE-CollabMCPClaudeAPIKey
			}
			...
			if deprecatedSystemPromptEnv, ok := os.LookupEnv("SYSTEM_PROMPT"); ok {
				cliui.Warnf(inv.Stderr, "SYSTEM_PROMPT is deprecated, use OPTIMUS-IDE-COLLAB_MCP_CLAUDE_SYSTEM_PROMPT instead")
				systemPrompt = deprecatedSystemPromptEnv
			}

			if err := configureClaude(fs, ClaudeConfig{
				// TODO: will this always be stable?
				AllowedTools:     []string{`mcp__optimus-ide-collab__optimus-ide-collab_report_task`},
				APIKey:           claudeAPIKey,
			...
			// The Optimus-IDE-Collab Prompt just allows users to extend our
			if optimus-ide-collabPrompt != "" {
				reportTaskPrompt += "\n\n" + optimus-ide-collabPrompt
			}
		...
		Options: []serpent.Option{
			{
				...
				Env:         "OPTIMUS-IDE-COLLAB_MCP_CLAUDE_CONFIG_PATH",
			...
			},
			{
				...
				Env:         "OPTIMUS-IDE-COLLAB_MCP_CLAUDE_MD_PATH",
				Flag:        "claude-md-path",
			...
			},
			{
				...
				Env:         "OPTIMUS-IDE-COLLAB_MCP_CLAUDE_API_KEY",
				Value:       serpent.StringOf(&deprecatedOptimus-IDE-CollabMCPClaudeAPIKey),
				Hidden:      true,
			},
			{
				Name:        "system-prompt",
				Description: "The system prompt to use for the Claude Code server.",
				Env:         "OPTIMUS-IDE-COLLAB_MCP_CLAUDE_SYSTEM_PROMPT",
				Flag:        "claude-system-prompt",
				Value:       serpent.StringOf(&systemPrompt),
				Default:     "Send a task status update to notify the user that you are ready for input, and then wait for user input.",
			},
			{
				Name:        "optimus-ide-collab-prompt",
				Description: "The optimus-ide-collab prompt to use for the Claude Code server.",
				Env:         "OPTIMUS-IDE-COLLAB_MCP_CLAUDE_OPTIMUS-IDE-COLLAB_PROMPT",
				Flag:        "claude-optimus-ide-collab-prompt",
				Value:       serpent.StringOf(&optimus-ide-collabPrompt),
				Default:     "", // Empty default means we'll use defaultOptimus-IDE-CollabPrompt from the variable
			},
			{
				Name:        "app-status-slug",
				Description: "The app status slug to use when running the Optimus-IDE-Collab MCP server.",
			...
			},
			{
				...
				Env:         "OPTIMUS-IDE-COLLAB_MCP_CLAUDE_TEST_BINARY_NAME",
...
func (*RootCmd) mcpConfigureCursor() *serpent.Command {
	...
	cmd := &serpent.Command{
		Use:   "cursor",
		Short: "Configure Cursor to use Optimus-IDE-Collab MCP.",
		Options: serpent.OptionSet{
			serpent.Option{
				Flag:        "project",
				Env:         "OPTIMUS-IDE-COLLAB_MCP_CURSOR_PROJECT",
		...
		Handler: func(_ *serpent.Invocation) error {
			...
			mcpServers["optimus-ide-collab"] = map[string]any{
				"command": binPath,
				"args":    []string{"exp", "mcp", "server"},
			}
...
type taskReport struct {
	...
	state optimus-ide-collabsdk.WorkspaceAppStatusState
	// summary is optional.
	summary string
}
...
func (r *RootCmd) mcpServer() *serpent.Command {
	...
	cmd := &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			queue := cliutil.NewQueue[taskReport](512).WithPredicate(func(report taskReport) (taskReport, bool) {
				...
				if report.messageID == nil &&
					report.state == optimus-ide-collabsdk.WorkspaceAppStatusStateWorking &&
			...
			if client != nil && client.URL != nil && client.SessionToken() != "" {
				me, err := client.User(inv.Context(), optimus-ide-collabsdk.Me)
				if err == nil {
		...
		Short: "Start the Optimus-IDE-Collab MCP server.",
		Options: []serpent.Option{
			{
				Name:        "instructions",
				Description: "The instructions to pass to the MCP server.",
				Flag:        "instructions",
				Env:         "OPTIMUS-IDE-COLLAB_MCP_INSTRUCTIONS",
			...
			},
			{
				...
				Flag:        "allowed-tools",
				Env:         "OPTIMUS-IDE-COLLAB_MCP_ALLOWED_TOOLS",
				Value:       serpent.StringArrayOf(&allowedTools),
			},
			{
				Name:        "app-status-slug",
				Description: "When reporting a task, the optimus-ide-collab_app slug under which to report the task.",
			...
			},
			{
				...
				Env:         "OPTIMUS-IDE-COLLAB_AGENT_SOCKET_PATH",
					...
					case event := <-eventsCh:
						...
						case agentapi.EventStatusChange:
							...
							if ev.Status == agentapi.StatusStable {
								state = optimus-ide-collabsdk.WorkspaceAppStatusStateIdle
							}
...
func (s *mcpServer) startServer(ctx context.Context, inv *serpent.Invocation, instructions string, allowedTools []string) error {
	...
	toolOpts := []func(*toolsdk.Deps){
		toolsdk.WithTaskReporter(func(args toolsdk.ReportTaskArgs) error {
			state := optimus-ide-collabsdk.WorkspaceAppStatusState(args.State)
			...
			// cannot produce them.
			if s.aiAgentAPIClient != nil && state == optimus-ide-collabsdk.WorkspaceAppStatusStateIdle {
				state = optimus-ide-collabsdk.WorkspaceAppStatusStateWorking
			}
	...
	for _, tool := range toolsdk.All {
		...
		// Skip the optimus-ide-collab_report_task tool if there is no socket client or slug.
		if tool.Tool.Name == "optimus-ide-collab_report_task" && (s.socketClient == nil || s.appStatusSlug == "") {
...
var (
	defaultReportTaskPrompt = `Respect the requirements of the "optimus-ide-collab_report_task" tool. It is pertinent to provide a fantastic user-experience.`

	// Define the guard strings
	optimus-ide-collabPromptStartGuard  = "<optimus-ide-collab-prompt>"
	optimus-ide-collabPromptEndGuard    = "</optimus-ide-collab-prompt>"
	systemPromptStartGuard = "<system-prompt>"
	systemPromptEndGuard   = "</system-prompt>"
)

func injectClaudeMD(fs afero.Fs, optimus-ide-collabPrompt, systemPrompt, claudeMDPath string) error {
	...
	if err != nil {
		...
		return afero.WriteFile(fs, claudeMDPath, []byte(promptsBlock(optimus-ide-collabPrompt, systemPrompt, "")), 0o600)
	}
	...
	// Remove existing optimus-ide-collab prompt section if it exists
	optimus-ide-collabStartIdx := indexOf(cleanContent, optimus-ide-collabPromptStartGuard)
	optimus-ide-collabEndIdx := indexOf(cleanContent, optimus-ide-collabPromptEndGuard)
	if optimus-ide-collabStartIdx != -1 && optimus-ide-collabEndIdx != -1 && optimus-ide-collabStartIdx < optimus-ide-collabEndIdx {
		beforeOptimus-IDE-CollabPrompt := cleanContent[:optimus-ide-collabStartIdx]
		afterOptimus-IDE-CollabPrompt := cleanContent[optimus-ide-collabEndIdx+len(optimus-ide-collabPromptEndGuard):]
		cleanContent = beforeOptimus-IDE-CollabPrompt + afterOptimus-IDE-CollabPrompt
	}
	...
	// Create the new content with optimus-ide-collab and system prompt prepended
	newContent := promptsBlock(optimus-ide-collabPrompt, systemPrompt, cleanContent)
...
func promptsBlock(optimus-ide-collabPrompt, systemPrompt, existingContent string) string {
	var newContent strings.Builder
	_, _ = newContent.WriteString(optimus-ide-collabPromptStartGuard)
	_, _ = newContent.WriteRune('\n')
	_, _ = newContent.WriteString(optimus-ide-collabPrompt)
	_, _ = newContent.WriteRune('\n')
	_, _ = newContent.WriteString(optimus-ide-collabPromptEndGuard)
	_, _ = newContent.WriteRune('\n')
...
func mcpFromSDK(sdkTool toolsdk.GenericTool, tb toolsdk.Deps) server.ServerTool {
	...
	return server.ServerTool{
		...
		Handler: func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			...
			if err := json.NewEnoptimus-ide-collab(&buf).Encode(request.Params.Arguments); err != nil {
