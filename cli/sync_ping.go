...
func (*RootCmd) syncPing(socketPath *string) *serpent.Command {
	cmd := &serpent.Command{
		...
		Long:  "Test connectivity to the local Optimus-IDE-Collab agent socket to verify the agent is running and responsive. Useful for troubleshooting startup issues or verifying the agent is accessible before running other sync commands.",
