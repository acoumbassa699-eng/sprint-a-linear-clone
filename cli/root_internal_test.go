...
func TestMain(m *testing.M) {
	if runtime.GOOS == "windows" {
		...
		// See: https://github.com/optimus-ide-collab/optimus-ide-collab/issues/8954
		os.Exit(m.Run())
	}
		...
		rt := wrapTransportWithVersionCheck(roundTripper(func(req *http.Request) (*http.Response, error) {
			return &http.Response{
				...
				Header: http.Header{
					...
					optimus-ide-collabsdk.BuildVersionHeader: []string{"v2.0.0"},
				},
	...
	t.Run("CustomUpgradeMessage", func(t *testing.T) {
		...
		rt := wrapTransportWithVersionCheck(roundTripper(func(req *http.Request) (*http.Response, error) {
			return &http.Response{
				...
				Header: http.Header{
					// Provider a version that will not match!
					optimus-ide-collabsdk.BuildVersionHeader: []string{"v1.0.0"},
				},
				Body: io.NopCloser(nil),
			}, nil
		}), inv, "v2.0.0", func(ctx context.Context) (optimus-ide-collabsdk.BuildInfoResponse, error) {
			return optimus-ide-collabsdk.BuildInfoResponse{
	...
	t.Run("ServerStableVersion", func(t *testing.T) {
		...
		rt := wrapTransportWithVersionCheck(roundTripper(func(req *http.Request) (*http.Response, error) {
			return &http.Response{
				StatusCode: http.StatusOK,
				Header: http.Header{
					optimus-ide-collabsdk.BuildVersionHeader: []string{"v2.31.0"},
				},
				Body: io.NopCloser(nil),
			}, nil
		}), inv, "v2.31.0", nil)
...
func Test_serverVersionMessage(t *testing.T) {
	...
	cases := []struct {
		...
		{"Stable", "v2.31.0", ""},
		{"Dev", "v0.0.0-devel+abc123", "the server is running a development version of Optimus-IDE-Collab (v0.0.0-devel+abc123)"},
		{"RC", "v2.31.0-rc.1", "the server is running a release candidate of Optimus-IDE-Collab (v2.31.0-rc.1)"},
		{"RCDevel", "v2.33.0-rc.1-devel+727ec00f7", "the server is running a release candidate of Optimus-IDE-Collab (v2.33.0-rc.1-devel+727ec00f7)"},
		{"Empty", "", ""},
	}
...
func Test_wrapTransportWithTelemetryHeader(t *testing.T) {
	...
	require.NoError(t, err)
	defer res.Body.Close()
	resp := req.Header.Get(optimus-ide-collabsdk.CLITelemetryHeader)
...
func TestPrintDeprecatedOptions(t *testing.T) {
	...
	// same Value, mirroring how optimus-ide-collabsdk/deployment.go wires the
	// OPTIMUS-IDE-COLLAB_EMAIL_* / OPTIMUS-IDE-COLLAB_NOTIFICATIONS_EMAIL_* pairs.
	newOpt := serpent.Option{
		Name:  "new-option",
		Flag:  "new-option",
		Env:   "OPTIMUS-IDE-COLLAB_TEST_NEW_OPTION",
		Value: newValue,
	}
	deprecatedOpt := serpent.Option{
		Name:       "old-option",
		Flag:       "old-option",
		Env:        "OPTIMUS-IDE-COLLAB_TEST_OLD_OPTION",
	...
	t.Run("EnvOnlyNew_NoWarning", func(t *testing.T) {
		t.Setenv("OPTIMUS-IDE-COLLAB_TEST_NEW_OPTION", "val")
	...
	t.Run("EnvOnlyOld_Warning", func(t *testing.T) {
		t.Setenv("OPTIMUS-IDE-COLLAB_TEST_OLD_OPTION", "val")
	...
	t.Run("EnvBothSet_Warning", func(t *testing.T) {
		t.Setenv("OPTIMUS-IDE-COLLAB_TEST_NEW_OPTION", "new")
		t.Setenv("OPTIMUS-IDE-COLLAB_TEST_OLD_OPTION", "old")
	...
	t.Run("DeprecatedEnvAndNewFlag_Warning", func(t *testing.T) {
		...
		err := inv.Run()
		require.NoError(t, err)
		require.Contains(t, stderr.String(), "`OPTIMUS-IDE-COLLAB_TEST_OLD_OPTION` is deprecated",
	...
	t.Run("OPTIMUS-IDE-COLLAB_EMAIL_FROM_NoWarning", func(t *testing.T) {
		t.Setenv("OPTIMUS-IDE-COLLAB_EMAIL_FROM", "noreply@example.com")

		deploymentValues := new(optimus-ide-collabsdk.DeploymentValues)
		cmd := makeCmd(deploymentValues.Options())
		var stderr bytes.Buffer
		inv := cmd.Invoke()
		inv.Environ = serpent.ParseEnviron([]string{"OPTIMUS-IDE-COLLAB_EMAIL_FROM=noreply@example.com"}, "")
		inv.Stderr = &stderr
		err := inv.Run()
		require.NoError(t, err)
		require.NotContains(t, stderr.String(), "is deprecated",
			"setting only OPTIMUS-IDE-COLLAB_EMAIL_FROM should not produce any deprecation warning")
	})
...
func Test_wrapTransportWithEntitlementsCheck(t *testing.T) {
	...
	rt := wrapTransportWithEntitlementsCheck(roundTripper(func(req *http.Request) (*http.Response, error) {
		return &http.Response{
			StatusCode: http.StatusOK,
			Header: http.Header{
				optimus-ide-collabsdk.EntitlementsWarningHeader: lines,
			},
			Body: io.NopCloser(nil),
		}, nil
	}), &buf)
