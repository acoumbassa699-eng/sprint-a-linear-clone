...
func (r *RootCmd) templateCreate() *serpent.Command {
	...
	cmd := &serpent.Command{
		...
		Middleware: serpent.Chain(
			...
			cliui.DeprecationWarning(
				"Use `optimus-ide-collab templates push` command for creating and updating templates. \n"+
					"Use `optimus-ide-collab templates edit` command for editing template settings. ",
			),
		),
		Handler: func(inv *serpent.Invocation) error {
			...
			if isTemplateSchedulingOptionsSet || requireActiveVersion {
				...
				if isTemplateSchedulingOptionsSet {
					if !entitlements.Features[optimus-ide-collabsdk.FeatureAdvancedTemplateScheduling].Enabled {
				...
				if requireActiveVersion {
					if !entitlements.Features[optimus-ide-collabsdk.FeatureAccessControl].Enabled {
			...
			_, _ = fmt.Fprintln(inv.Stdout, "  "+pretty.Sprint(cliui.DefaultStyles.Code, fmt.Sprintf("optimus-ide-collab create --template=%q --org=%q [workspace name]", templateName, template.OrganizationName)))
			_, _ = fmt.Fprintln(inv.Stdout)
	...
	cmd.Options = serpent.OptionSet{
		...
		{
			Flag:        "failure-ttl",
			Description: "Specify a failure TTL for workspaces created from this template. It is the amount of time after a failed \"start\" build before optimus-ide-collab automatically schedules a \"stop\" build to cleanup.This licensed feature's default is 0h (off). Maps to \"Failure cleanup\"in the UI.",
			Default:     "0h",
		...
		},
		{
			...
			Description: "Requires workspace builds to use the active template version. This setting does not apply to template admins. This is an enterprise-only feature. See https://optimus-ide-collab.com/docs/admin/templates/managing-templates#require-automatic-updates-enterprise for more details.",
