...
// optimus-ide-collab exp example-error using golden files.
func TestErrorExamples(t *testing.T) {
	t.Parallel()

	optimus-ide-collabRootCmd := getRoot(t)

	var exampleErrorRootCmd *serpent.Command
	optimus-ide-collabRootCmd.Walk(func(command *serpent.Command) {
...
ExtractCommandPathsLoop:
	for _, cp := range extractCommandPaths(nil, exampleErrorRootCmd.Children) {
		...
		name := fmt.Sprintf("optimus-ide-collab %s", strings.Join(cmd, " "))
	...
	for _, tt := range cases {
		t.Run(tt.Name, func(t *testing.T) {
			...
			inv, _ := clitest.NewWithCommand(t, optimus-ide-collabRootCmd, tt.Cmd...)
			inv.Stderr = &outBuf
			inv.Stdout = &outBuf
