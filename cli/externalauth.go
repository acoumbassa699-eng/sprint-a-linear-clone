...
`,
			...
			Example{
				...
				Command:     "optimus-ide-collab external-auth access-token slack --extra \"authed_user.id\"",
			},
			Example{
				Description: "Print the full token response as JSON.",
				Command:     "optimus-ide-collab external-auth access-token github --output json",
			},
		),
