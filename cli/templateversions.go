	...
	cmd := &serpent.Command{
		...
		Long: FormatExamples(
			Example{
				...
				Command:     "optimus-ide-collab templates versions list my-template",
			},
		),
...
func (r *RootCmd) templateVersionsList() *serpent.Command {
	...
	cmd := &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			req := optimus-ide-collabsdk.TemplateVersionsByTemplateRequest{
...
type templateVersionRow struct {
	// For json format:
	TemplateVersion optimus-ide-collabsdk.TemplateVersion `table:"-"`
...
// for outputting.
func templateVersionsToRows(activeVersionID uuid.UUID, templateVersions ...optimus-ide-collabsdk.TemplateVersion) []templateVersionRow {
...
func (r *RootCmd) templateVersionsPromote() *serpent.Command {
	...
	cmd.Options = serpent.OptionSet{
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_TEMPLATE_VERSION_NAME",
			Required:    true,
