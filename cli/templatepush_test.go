...
func TestTemplatePush(t *testing.T) {
	...
	t.Run("OK", func(t *testing.T) {
		...
		templateVersions, err := client.TemplateVersionsByTemplate(context.Background(), optimus-ide-collabsdk.TemplateVersionsByTemplateRequest{
	...
	t.Run("Message less than or equal to 72 chars", func(t *testing.T) {
		...
		// Assert that the template version changed.
		ctx = testutil.Context(t, testutil.WaitMedium)
		templateVersions, err := client.TemplateVersionsByTemplate(ctx, optimus-ide-collabsdk.TemplateVersionsByTemplateRequest{
			TemplateID: template.ID,
		})
		require.NoError(t, err)
		assert.Len(t, templateVersions, 2)
	...
	t.Run("UseWorkingDir", func(t *testing.T) {
		...
		template := optimus-ide-collabdtest.CreateTemplate(t, client, owner.OrganizationID, version.ID,
			func(r *optimus-ide-collabsdk.CreateTemplateRequest) {
	...
	t.Run("ProvisionerTags", func(t *testing.T) {
		...
		t.Run("WorkspaceTagsTerraform", func(t *testing.T) {
			...
			tests := []struct {
				name         string
				setupDaemon  func(ctx context.Context, store database.Store, owner optimus-ide-collabsdk.CreateFirstUserResponse, tags database.StringMap, now time.Time) error
				expectOutput string
			}{
				{
					name: "no provisioners available",
					setupDaemon: func(_ context.Context, _ database.Store, _ optimus-ide-collabsdk.CreateFirstUserResponse, _ database.StringMap, _ time.Time) error {
						return nil
					},
		...
		t.Run("ChangeTags", func(t *testing.T) {
			...
			client, provisionerDocker, api := optimus-ide-collabdtest.NewWithAPI(t, &optimus-ide-collabdtest.Options{
			...
			provisionerFoobar := optimus-ide-collabdtest.NewTaggedProvisionerDaemon(t, api, "provisioner-foobar", map[string]string{
				"foobar": "foobaz",
			})
			...
			templateVersion := optimus-ide-collabdtest.CreateTemplateVersion(t, client, owner.OrganizationID, nil, func(ctvr *optimus-ide-collabsdk.CreateTemplateVersionRequest) {
				ctvr.ProvisionerTags = map[string]string{
					"docker": "true",
				}
			})
			templateVersion = optimus-ide-collabdtest.AwaitTemplateVersionJobCompleted(t, client, templateVersion.ID)
			template := optimus-ide-collabdtest.CreateTemplate(t, client, owner.OrganizationID, templateVersion.ID)
	...
	t.Run("Variables", func(t *testing.T) {
		...
		t.Run("VariableIsRequired", func(t *testing.T) {
			...
			templateVersion := optimus-ide-collabdtest.CreateTemplateVersion(t, client, owner.OrganizationID, createEchoResponsesWithTemplateVariables(initialTemplateVariables))
			_ = optimus-ide-collabdtest.AwaitTemplateVersionJobCompleted(t, client, templateVersion.ID)
			template := optimus-ide-collabdtest.CreateTemplate(t, client, owner.OrganizationID, templateVersion.ID)
