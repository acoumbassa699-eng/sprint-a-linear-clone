...
// │ └─ dev (linux, amd64)        ⦾ connecting [10s]    optimus-ide-collab ssh dev.dev       │
// ├────────────────────────────────────────────────────────────────────────────┤
// │ kubernetes_pod.dev                                                         │
// │ ├─ go (linux, amd64)         ⦿ connected           optimus-ide-collab ssh dev.go        │
// │ └─ postgres (linux, amd64)   ⦾ disconnected [4s]   optimus-ide-collab ssh dev.postgres  │
// └────────────────────────────────────────────────────────────────────────────┘
func WorkspaceResources(writer io.Writer, resources []optimus-ide-collabsdk.WorkspaceResource, options WorkspaceResourcesOptions) error {
	...
	for _, resource := range resources {
		...
		agents := slice.Filter(resource.Agents, func(agent optimus-ide-collabsdk.WorkspaceAgent) bool {
...
func renderAgentRow(agent optimus-ide-collabsdk.WorkspaceAgent, index, totalAgents int, options WorkspaceResourcesOptions) table.Row {
	row := table.Row{
	...
	if !options.HideAccess {
		sshCommand := "optimus-ide-collab ssh " + options.WorkspaceName
...
func renderPortRow(port optimus-ide-collabsdk.WorkspaceAgentListeningPort, idx, total int) table.Row {
...
func renderDevcontainers(resources []optimus-ide-collabsdk.WorkspaceResource, wro WorkspaceResourcesOptions, agentID uuid.UUID, index, totalAgents int) []table.Row {
	var rows []table.Row
...
func renderDevcontainerRow(resources []optimus-ide-collabsdk.WorkspaceResource, devcontainer optimus-ide-collabsdk.WorkspaceAgentDevcontainer, index, total int, wro WorkspaceResourcesOptions) []table.Row {
	var rows []table.Row
	...
	// name and status.
	var subAgent *optimus-ide-collabsdk.WorkspaceAgent
	displayName := devcontainer.Name
	if devcontainer.Agent != nil && devcontainer.Status == optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatusRunning {
		for _, resource := range resources {
			if agent, found := slice.Find(resource.Agents, func(agent optimus-ide-collabsdk.WorkspaceAgent) bool {
	...
	if !wro.HideAccess {
		if subAgent != nil {
			accessString := fmt.Sprintf("optimus-ide-collab ssh %s.%s", wro.WorkspaceName, subAgent.Name)
...
func renderDevcontainerStatus(status optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatus) string {
	switch status {
	case optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatusRunning:
		return pretty.Sprint(DefaultStyles.Keyword, "▶ running")
	case optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatusStopped:
		return pretty.Sprint(DefaultStyles.Placeholder, "⏹ stopped")
	case optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatusStarting:
		return pretty.Sprint(DefaultStyles.Warn, "⧗ starting")
	case optimus-ide-collabsdk.WorkspaceAgentDevcontainerStatusError:
...
func renderAgentStatus(agent optimus-ide-collabsdk.WorkspaceAgent) string {
	switch agent.Status {
	case optimus-ide-collabsdk.WorkspaceAgentConnecting:
	...
	case optimus-ide-collabsdk.WorkspaceAgentDisconnected:
		...
		return pretty.Sprint(DefaultStyles.Error, "⦾ disconnected") + " " +
			pretty.Sprint(DefaultStyles.Placeholder, "["+strconv.Itoa(int(since.Seconds()))+"s]")
	case optimus-ide-collabsdk.WorkspaceAgentTimeout:
		since := dbtime.Now().Sub(agent.CreatedAt)
		return fmt.Sprintf(
			"%s %s",
...
func renderAgentHealth(agent optimus-ide-collabsdk.WorkspaceAgent) string {
