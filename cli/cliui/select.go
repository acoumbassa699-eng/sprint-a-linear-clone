...
func RichSelect(inv *serpent.Invocation, richOptions RichSelectOptions) (*optimus-ide-collabsdk.TemplateVersionParameterOption, error) {
...
func RichMultiSelect(inv *serpent.Invocation, richOptions RichMultiSelectOptions) ([]string, error) {
	...
	asLine := func(option optimus-ide-collabsdk.TemplateVersionParameterOption) string {
		line := option.Name
		if len(option.Description) > 0 {
			line += ": " + option.Description
		}
