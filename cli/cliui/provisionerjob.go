...
func WorkspaceBuild(ctx context.Context, writer io.Writer, client *optimus-ide-collabsdk.Client, build uuid.UUID) error {
...
func ProvisionerJob(ctx context.Context, wr io.Writer, opts ProvisionerJobOptions) error {
	...
	updateStage := func(stage string, startedAt time.Time) {
		if currentStage != "" {
			...
			if job.CompletedAt != nil && job.Status != optimus-ide-collabsdk.ProvisionerJobSucceeded {
...
func (s *stageWriter) Log(createdAt time.Time, level optimus-ide-collabsdk.LogLevel, line string) {
	w := s.w
	if s.silentLogs {
		w = &s.logBuf
	}
	...
	switch level {
	case optimus-ide-collabsdk.LogLevelTrace, optimus-ide-collabsdk.LogLevelDebug:
		if !s.verbose {
			return
		}
		style = DefaultStyles.Placeholder
	case optimus-ide-collabsdk.LogLevelError:
		style = DefaultStyles.Error
	case optimus-ide-collabsdk.LogLevelWarn:
		style = DefaultStyles.Warn
	case optimus-ide-collabsdk.LogLevelInfo:
	}
