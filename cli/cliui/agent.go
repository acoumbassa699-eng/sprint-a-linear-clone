...
type AgentOptions struct {
	...
	Fetch         func(ctx context.Context, agentID uuid.UUID) (optimus-ide-collabsdk.WorkspaceAgent, error)
	FetchLogs     func(ctx context.Context, agentID uuid.UUID, after int64, follow bool) (<-chan []optimus-ide-collabsdk.WorkspaceAgentLog, io.Closer, error)
...
type agentWaiter struct {
	opts       AgentOptions
	sw         *stageWriter
	logSources map[uuid.UUID]optimus-ide-collabsdk.WorkspaceAgentLogSource
	fetchAgent func(context.Context) (optimus-ide-collabsdk.WorkspaceAgent, error)
}
...
func Agent(ctx context.Context, writer io.Writer, agentID uuid.UUID, opts AgentOptions) error {
	...
	if opts.FetchLogs == nil {
		opts.FetchLogs = func(_ context.Context, _ uuid.UUID, _ int64, _ bool) (<-chan []optimus-ide-collabsdk.WorkspaceAgentLog, io.Closer, error) {
			c := make(chan []optimus-ide-collabsdk.WorkspaceAgentLog)
			close(c)
	...
	fetch := func(ctx context.Context) (optimus-ide-collabsdk.WorkspaceAgent, error) {
		select {
		case <-ctx.Done():
			return optimus-ide-collabsdk.WorkspaceAgent{}, ctx.Err()
		case f := <-fetchedAgent:
			if f.err != nil {
				return optimus-ide-collabsdk.WorkspaceAgent{}, f.err
			}
			return f.agent, nil
		}
	}
...
func (aw *agentWaiter) wait(ctx context.Context, agent optimus-ide-collabsdk.WorkspaceAgent, fetchedAgent chan fetchAgentResult) error {
	var err error
	...
	for {
		...
		switch agent.Status {
		case optimus-ide-collabsdk.WorkspaceAgentConnecting, optimus-ide-collabsdk.WorkspaceAgentTimeout:
		...
		case optimus-ide-collabsdk.WorkspaceAgentConnected:
			return aw.handleConnected(ctx, agent, waitedForConnection, fetchedAgent)

		case optimus-ide-collabsdk.WorkspaceAgentDisconnected:
...
func (aw *agentWaiter) waitForConnection(ctx context.Context, agent optimus-ide-collabsdk.WorkspaceAgent) (optimus-ide-collabsdk.WorkspaceAgent, error) {
	stage := "Waiting for the workspace agent to connect"
	aw.sw.Start(stage)

	agent, err := aw.pollWhile(ctx, agent, func(agent optimus-ide-collabsdk.WorkspaceAgent) bool {
		return agent.Status == optimus-ide-collabsdk.WorkspaceAgentConnecting
	})
	if err != nil {
		return agent, err
	}

	if agent.Status == optimus-ide-collabsdk.WorkspaceAgentTimeout {
		now := time.Now()
		aw.sw.Log(now, optimus-ide-collabsdk.LogLevelInfo, "The workspace agent is having trouble connecting, wait for it to connect or restart your workspace.")
		aw.sw.Log(now, optimus-ide-collabsdk.LogLevelInfo, troubleshootingMessage(agent, fmt.Sprintf("%s/admin/templates/troubleshooting#agent-connection-issues", aw.opts.DocsURL)))
		agent, err = aw.pollWhile(ctx, agent, func(agent optimus-ide-collabsdk.WorkspaceAgent) bool {
			return agent.Status == optimus-ide-collabsdk.WorkspaceAgentTimeout
		})
		if err != nil {
			return agent, err
		}
...
func (aw *agentWaiter) handleConnected(ctx context.Context, agent optimus-ide-collabsdk.WorkspaceAgent, showStartupLogs bool, fetchedAgent chan fetchAgentResult) error {
	if !showStartupLogs && agent.LifecycleState == optimus-ide-collabsdk.WorkspaceAgentLifecycleReady {
	...
	if follow {
		aw.sw.Log(time.Time{}, optimus-ide-collabsdk.LogLevelInfo, "==> ℹ︎ To connect immediately, reconnect with --wait=no or OPTIMUS-IDE-COLLAB_SSH_WAIT=no, see --help for more information.")
	}
	...
	case optimus-ide-collabsdk.WorkspaceAgentLifecycleReady:
		aw.sw.Complete(stage, safeDuration(aw.sw, agent.ReadyAt, agent.StartedAt))
	case optimus-ide-collabsdk.WorkspaceAgentLifecycleStartTimeout:
		// Backwards compatibility: Avoid printing warning if
		// optimus-ide-collabd is old and doesn't set ReadyAt for timeouts.
		if agent.ReadyAt == nil {
			aw.sw.Fail(stage, 0)
		} else {
			aw.sw.Fail(stage, safeDuration(aw.sw, agent.ReadyAt, agent.StartedAt))
		}
		aw.sw.Log(time.Time{}, optimus-ide-collabsdk.LogLevelWarn, "Warning: A startup script timed out and your workspace may be incomplete.")
	case optimus-ide-collabsdk.WorkspaceAgentLifecycleStartError:
		aw.sw.Fail(stage, safeDuration(aw.sw, agent.ReadyAt, agent.StartedAt))
		aw.sw.Log(time.Time{}, optimus-ide-collabsdk.LogLevelWarn, "Warning: A startup script exited with an error and your workspace may be incomplete.")
		aw.sw.Log(time.Time{}, optimus-ide-collabsdk.LogLevelWarn, troubleshootingMessage(agent, fmt.Sprintf("%s/admin/templates/troubleshooting#startup-script-exited-with-an-error", aw.opts.DocsURL)))
	default:
		switch {
		case agent.LifecycleState.Starting():
			aw.sw.Log(time.Time{}, optimus-ide-collabsdk.LogLevelWarn, "Notice: The startup scripts are still running and your workspace may be incomplete.")
			aw.sw.Log(time.Time{}, optimus-ide-collabsdk.LogLevelWarn, troubleshootingMessage(agent, fmt.Sprintf("%s/admin/templates/troubleshooting#your-workspace-may-be-incomplete", aw.opts.DocsURL)))
...
//
//nolint:revive // Control flag is acceptable for internal method.
func (aw *agentWaiter) streamLogs(ctx context.Context, agent optimus-ide-collabsdk.WorkspaceAgent, follow bool, fetchedAgent chan fetchAgentResult) (optimus-ide-collabsdk.WorkspaceAgent, error) {
	...
	var lastLog optimus-ide-collabsdk.WorkspaceAgentLog
...
func (aw *agentWaiter) waitForReconnection(ctx context.Context, agent optimus-ide-collabsdk.WorkspaceAgent) (optimus-ide-collabsdk.WorkspaceAgent, bool, error) {
	...
	aw.sw.Start(stage)
	aw.sw.Log(time.Now(), optimus-ide-collabsdk.LogLevelWarn, "Wait for it to reconnect or restart your workspace.")
	aw.sw.Log(time.Now(), optimus-ide-collabsdk.LogLevelWarn, troubleshootingMessage(agent, fmt.Sprintf("%s/admin/templates/troubleshooting#agent-connection-issues", aw.opts.DocsURL)))

	disconnectedAt := agent.DisconnectedAt
	agent, err := aw.pollWhile(ctx, agent, func(agent optimus-ide-collabsdk.WorkspaceAgent) bool {
		return agent.Status == optimus-ide-collabsdk.WorkspaceAgentDisconnected
	})
	if err != nil {
...
func (aw *agentWaiter) pollWhile(ctx context.Context, agent optimus-ide-collabsdk.WorkspaceAgent, cond func(agent optimus-ide-collabsdk.WorkspaceAgent) bool) (optimus-ide-collabsdk.WorkspaceAgent, error) {
	var err error
	for cond(agent) {
...
func troubleshootingMessage(agent optimus-ide-collabsdk.WorkspaceAgent, url string) string {
...
func safeDuration(sw *stageWriter, a, b *time.Time) time.Duration {
	if a == nil || b == nil {
		if sw != nil {
			...
			sw.Log(time.Now(), optimus-ide-collabsdk.LogLevelWarn, "Warning: Failed to calculate duration from a time being <nil>.")
		}
		return 0
	}
	return a.Sub(*b)
...
func PeerDiagnostics(w io.Writer, d tailnet.PeerDiagnostics) {
	...
	if d.SentNode {
		_, _ = fmt.Fprint(w, "✔ sent local data to Optimus-IDE-Collab networking coordinator\n")
	} else {
		_, _ = fmt.Fprint(w, "✘ have not sent local data to Optimus-IDE-Collab networking coordinator\n")
	}
	if d.ReceivedNode != nil {
		...
		_, _ = fmt.Fprintf(w,
			"✔ received remote agent data from Optimus-IDE-Collab networking coordinator\n    preferred DERP region: %s %s\n    endpoints: %s\n",
			dp, dn, strings.Join(d.ReceivedNode.Endpoints, ", "))
	} else {
		_, _ = fmt.Fprint(w, "✘ have not received remote agent data from Optimus-IDE-Collab networking coordinator\n")
	}
...
func (d ConnDiags) splitDiagnostics() (general, client, agent []string) {
	...
	if d.DisableDirect {
		general = append(general, "❗ Direct connections are disabled locally, by `--disable-direct-connections` or `OPTIMUS-IDE-COLLAB_DISABLE_DIRECT_CONNECTIONS`.\n"+
	...
	if d.ConnInfo.DisableDirectConnections {
		general = append(general,
			fmt.Sprintf("❗ Your Optimus-IDE-Collab administrator has blocked direct connections\n   %s#disabled-deployment-wide", d.TroubleshootingURL))
		if !d.Verbose {
			return general, client, agent
		}
	}
