...
func TestAgent(t *testing.T) {
	...
	for _, tc := range []struct {
		name    string
		iter    []func(context.Context, *testing.T, *optimus-ide-collabsdk.WorkspaceAgent, <-chan string, chan []optimus-ide-collabsdk.WorkspaceAgentLog) error
		logs    chan []optimus-ide-collabsdk.WorkspaceAgentLog
	...
	}{
		{
			...
			iter: []func(context.Context, *testing.T, *optimus-ide-collabsdk.WorkspaceAgent, <-chan string, chan []optimus-ide-collabsdk.WorkspaceAgentLog) error{
				func(_ context.Context, _ *testing.T, agent *optimus-ide-collabsdk.WorkspaceAgent, _ <-chan string, _ chan []optimus-ide-collabsdk.WorkspaceAgentLog) error {
					agent.Status = optimus-ide-collabsdk.WorkspaceAgentConnecting
					return nil
				},
				func(_ context.Context, t *testing.T, agent *optimus-ide-collabsdk.WorkspaceAgent, output <-chan string, _ chan []optimus-ide-collabsdk.WorkspaceAgentLog) error {
		...
		},
		{
			...
			iter: []func(context.Context, *testing.T, *optimus-ide-collabsdk.WorkspaceAgent, <-chan string, chan []optimus-ide-collabsdk.WorkspaceAgentLog) error{
				...
				func(_ context.Context, _ *testing.T, agent *optimus-ide-collabsdk.WorkspaceAgent, _ <-chan string, _ chan []optimus-ide-collabsdk.WorkspaceAgentLog) error {
					agent.Status = optimus-ide-collabsdk.WorkspaceAgentConnected
					agent.LifecycleState = optimus-ide-collabsdk.WorkspaceAgentLifecycleStartTimeout
					agent.FirstConnectedAt = ptr.Ref(time.Now())
		...
		},
		{
			...
			iter: []func(context.Context, *testing.T, *optimus-ide-collabsdk.WorkspaceAgent, <-chan string, chan []optimus-ide-collabsdk.WorkspaceAgentLog) error{
				func(_ context.Context, _ *testing.T, agent *optimus-ide-collabsdk.WorkspaceAgent, _ <-chan string, _ chan []optimus-ide-collabsdk.WorkspaceAgentLog) error {
					agent.Status = optimus-ide-collabsdk.WorkspaceAgentDisconnected
		...
		},
		{
			...
			iter: []func(context.Context, *testing.T, *optimus-ide-collabsdk.WorkspaceAgent, <-chan string, chan []optimus-ide-collabsdk.WorkspaceAgentLog) error{
				func(_ context.Context, _ *testing.T, agent *optimus-ide-collabsdk.WorkspaceAgent, _ <-chan string, logs chan []optimus-ide-collabsdk.WorkspaceAgentLog) error {
					agent.Status = optimus-ide-collabsdk.WorkspaceAgentConnected
					agent.FirstConnectedAt = ptr.Ref(time.Now())
					agent.LifecycleState = optimus-ide-collabsdk.WorkspaceAgentLifecycleStarting
					agent.StartedAt = ptr.Ref(time.Now())
					agent.LogSources = []optimus-ide-collabsdk.WorkspaceAgentLogSource{{
						ID:          uuid.Nil,
						DisplayName: "testing",
					}}
					logs <- []optimus-ide-collabsdk.WorkspaceAgentLog{
						{
							CreatedAt: time.Now(),
							Output:    "Hello world",
							SourceID:  uuid.Nil,
			...
			want: []string{
				...
				"ℹ︎ To connect immediately, reconnect with --wait=no or OPTIMUS-IDE-COLLAB_SSH_WAIT=no, see --help for more information.",
				"testing: Hello world",
				"Bye now",
				"✔ Running workspace agent startup scripts",
			},
		},
		{
			...
			iter: []func(context.Context, *testing.T, *optimus-ide-collabsdk.WorkspaceAgent, <-chan string, chan []optimus-ide-collabsdk.WorkspaceAgentLog) error{
				func(_ context.Context, _ *testing.T, agent *optimus-ide-collabsdk.WorkspaceAgent, output <-chan string, logs chan []optimus-ide-collabsdk.WorkspaceAgentLog) error {
					agent.Status = optimus-ide-collabsdk.WorkspaceAgentConnected
					agent.FirstConnectedAt = ptr.Ref(time.Now())
					agent.StartedAt = ptr.Ref(time.Now())
					agent.LifecycleState = optimus-ide-collabsdk.WorkspaceAgentLifecycleStartError
					agent.ReadyAt = ptr.Ref(time.Now())
					logs <- []optimus-ide-collabsdk.WorkspaceAgentLog{
						{
							CreatedAt: time.Now(),
		...
		},
		{
			...
			iter: []func(context.Context, *testing.T, *optimus-ide-collabsdk.WorkspaceAgent, <-chan string, chan []optimus-ide-collabsdk.WorkspaceAgentLog) error{
				func(_ context.Context, _ *testing.T, agent *optimus-ide-collabsdk.WorkspaceAgent, output <-chan string, logs chan []optimus-ide-collabsdk.WorkspaceAgentLog) error {
					agent.Status = optimus-ide-collabsdk.WorkspaceAgentDisconnected
					agent.LifecycleState = optimus-ide-collabsdk.WorkspaceAgentLifecycleOff
					return nil
				},
			},
			wantErr: true,
		},
		{
			...
			iter: []func(context.Context, *testing.T, *optimus-ide-collabsdk.WorkspaceAgent, <-chan string, chan []optimus-ide-collabsdk.WorkspaceAgentLog) error{
				...
				func(_ context.Context, _ *testing.T, agent *optimus-ide-collabsdk.WorkspaceAgent, _ <-chan string, _ chan []optimus-ide-collabsdk.WorkspaceAgentLog) error {
					agent.ReadyAt = ptr.Ref(time.Now())
					agent.LifecycleState = optimus-ide-collabsdk.WorkspaceAgentLifecycleShuttingDown
					return nil
				},
			},
			want: []string{
		...
		},
		{
			...
			iter: []func(context.Context, *testing.T, *optimus-ide-collabsdk.WorkspaceAgent, <-chan string, chan []optimus-ide-collabsdk.WorkspaceAgentLog) error{
				...
				func(_ context.Context, _ *testing.T, agent *optimus-ide-collabsdk.WorkspaceAgent, output <-chan string, _ chan []optimus-ide-collabsdk.WorkspaceAgentLog) error {
					return xerrors.New("bad")
				},
			},
			want: []string{
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			logs := make(chan []optimus-ide-collabsdk.WorkspaceAgentLog, 1)

			cmd := &serpent.Command{
				Handler: func(inv *serpent.Invocation) error {
					tc.opts.Fetch = func(_ context.Context, _ uuid.UUID) (optimus-ide-collabsdk.WorkspaceAgent, error) {
					...
					tc.opts.FetchLogs = func(ctx context.Context, _ uuid.UUID, _ int64, follow bool) (<-chan []optimus-ide-collabsdk.WorkspaceAgentLog, io.Closer, error) {
						if follow {
							return logs, closeFunc(func() error { return nil }), nil
						}

						fetchLogs := make(chan []optimus-ide-collabsdk.WorkspaceAgentLog, 1)
						select {
						case <-ctx.Done():
	...
	t.Run("ContextCancelDuringLogStreaming", func(t *testing.T) {
		...
		agent := optimus-ide-collabsdk.WorkspaceAgent{
			...
			CreatedAt:        time.Now(),
			LifecycleState:   optimus-ide-collabsdk.WorkspaceAgentLifecycleStarting,
...
func TestPeerDiagnostics(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			want: []*regexp.Regexp{
				regexp.MustCompile(`^✔ sent local data to Optimus-IDE-Collab networking coordinator$`),
			},
		},
		{
			name: "didntSendNode",
			...
			want: []*regexp.Regexp{
				regexp.MustCompile(`^✘ have not sent local data to Optimus-IDE-Collab networking coordinator$`),
			},
		},
		{
			...
			want: []*regexp.Regexp{
				regexp.MustCompile(`^✔ received remote agent data from Optimus-IDE-Collab networking coordinator$`),
		...
		},
		{
			...
			want: []*regexp.Regexp{
				regexp.MustCompile(`^✘ have not received remote agent data from Optimus-IDE-Collab networking coordinator$`),
			},
		},
		{
...
func TestConnDiagnostics(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			want: []string{
				`❗ Your Optimus-IDE-Collab administrator has blocked direct connections`,
			},
		},
		{
			name: "NoStun",
