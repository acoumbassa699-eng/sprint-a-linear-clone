...
const (
	...
	sshStartToken            = "# ------------START-OPTIMUS-IDE-COLLAB-----------"
	sshEndToken              = "# ------------END-OPTIMUS-IDE-COLLAB------------"
	sshConfigSectionHeader   = "# This section is managed by optimus-ide-collab. DO NOT EDIT."
	sshConfigDocsHeader      = `
#
# You should not hand-edit this section unless you are removing it, all
# changes will be lost when running "optimus-ide-collab config-ssh".
`
...
// from the optimus-ide-collab config in ~/.ssh/optimus-ide-collab.
type sshConfigOptions struct {
	...
	optimus-ide-collabBinaryPath        string
...
func (o *sshConfigOptions) addOption(option string) error {
	key, value, err := optimus-ide-collabsdk.ParseSSHConfigOption(option)
	if err != nil {
		return err
	}
...
func (o sshConfigOptions) writeToBuffer(buf *bytes.Buffer) error {
	escapedOptimus-IDE-CollabBinaryProxy, err := sshConfigProxyCommandEscape(o.optimus-ide-collabBinaryPath, o.forceUnixSeparators)
	if err != nil {
		return xerrors.Errorf("escape optimus-ide-collab binary for ProxyCommand failed: %w", err)
	}

	escapedOptimus-IDE-CollabBinaryMatchExec, err := sshConfigMatchExecEscape(o.optimus-ide-collabBinaryPath)
	if err != nil {
		return xerrors.Errorf("escape optimus-ide-collab binary for Match exec failed: %w", err)
	}
			...
			for i, wsName := range o.workspaceNames {
				...
				if !o.skipProxyCommand {
					...
					_, _ = fmt.Fprintf(buf,
						...
						escapedOptimus-IDE-CollabBinaryProxy, rootFlags, flags, o.userHostPrefix,
					)
					_, _ = buf.WriteString("\n")
				}
			}
	...
	if o.noWildcard {
		for _, wsName := range o.workspaceNames {
			...
			// Options always apply; only use the proxy command when Optimus-IDE-Collab Connect is not running.
			if !o.skipProxyCommand {
				_, _ = fmt.Fprintf(buf, "\nMatch host %s !exec \"%s connect exists %%h\"\n",
					hostname, escapedOptimus-IDE-CollabBinaryMatchExec)
				_, _ = buf.WriteString("\t")
				_, _ = fmt.Fprintf(buf,
					"ProxyCommand %s %s ssh --stdio%s --hostname-suffix %s %%h",
					escapedOptimus-IDE-CollabBinaryProxy, rootFlags, flags, o.hostnameSuffix,
				)
				_, _ = buf.WriteString("\n")
			}
		}
		...
		// Options above always apply; only use the proxy command when Optimus-IDE-Collab Connect is not running.
		if !o.skipProxyCommand {
			_, _ = fmt.Fprintf(buf, "\nMatch host *.%s !exec \"%s connect exists %%h\"\n",
				o.hostnameSuffix, escapedOptimus-IDE-CollabBinaryMatchExec)
			_, _ = buf.WriteString("\t")
			_, _ = fmt.Fprintf(buf,
				"ProxyCommand %s %s ssh --stdio%s --hostname-suffix %s %%h",
				escapedOptimus-IDE-CollabBinaryProxy, rootFlags, flags, o.hostnameSuffix,
...
func (r *RootCmd) configSSH() *serpent.Command {
	var (
		...
		dryRun          bool
		optimus-ide-collabCliPath    string
	)
	cmd := &serpent.Command{
		Annotations: workspaceCommand,
		Use:         "config-ssh",
		Short:       "Add an SSH Host entry for your workspaces \"ssh workspace.optimus-ide-collab\"",
		Long: FormatExamples(
			Example{
				Description: "You can use -o (or --ssh-option) so set SSH options to be used for all your workspaces",
				Command:     "optimus-ide-collab config-ssh -o ForwardAgent=yes",
			},
			Example{
				Description: "You can use --dry-run (or -n) to see the changes that would be made",
				Command:     "optimus-ide-collab config-ssh --dry-run",
			},
		),
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			optimus-ide-collabBinary := optimus-ide-collabCliPath
			if optimus-ide-collabBinary == "" {
				optimus-ide-collabBinary, err = currentBinPath(out)
				if err != nil {
					return err
				}
			}
			...
			section, ok, err := sshConfigGetOptimus-IDE-CollabSection(configRaw)
			if err != nil {
				return err
			}
			if ok {
			...
			if usePreviousOpts && lastConfig != nil {
				...
				for _, v := range sshConfigOpts.sshOptions {
					...
					// this early.
					if _, _, err := optimus-ide-collabsdk.ParseSSHConfigOption(v); err != nil {
			...
			buf := &bytes.Buffer{}
			before, _, after, err := sshConfigSplitOnOptimus-IDE-CollabSection(configModified)
			if err != nil {
				return err
			}
			...
			optimus-ide-collabdConfig, err := client.SSHConfiguration(ctx)
			if err != nil {
				...
				if !(xerrors.As(err, &sdkErr) && sdkErr.StatusCode() == http.StatusNotFound) {
					return xerrors.Errorf("fetch optimus-ide-collabd config failed: %w", err)
				}
				optimus-ide-collabdConfig.HostnamePrefix = "optimus-ide-collab."
			}

			configOptions, err := mergeSSHOptions(sshConfigOpts, optimus-ide-collabdConfig, string(root), optimus-ide-collabBinary)
			if err != nil {
				return err
			}
			...
			if !bytes.Equal(configModified, buf.Bytes()) {
				changes = append(changes, fmt.Sprintf("Update the optimus-ide-collab section in %s", sshConfigFile))
			...
			if len(res.Workspaces) > 0 {
				...
				_, _ = fmt.Fprint(out, "You don't have any workspaces yet, try creating one with:\n\n\t$ optimus-ide-collab create <workspace>\n")
			}
			return nil
		},
	}
	...
	cmd.Options = serpent.OptionSet{
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_SSH_CONFIG_FILE",
			Default:     sshDefaultConfigFileName,
			Description: "Specifies the path to an SSH config.",
			Value:       serpent.StringOf(&sshConfigFile),
		},
		{
			Flag:    "optimus-ide-collab-binary-path",
			Env:     "OPTIMUS-IDE-COLLAB_SSH_CONFIG_BINARY_PATH",
			Default: "",
			Description: "Optionally specify the absolute path to the optimus-ide-collab binary used in ProxyCommand. " +
				"By default, the binary invoking this command ('config ssh') is used.",
			Value: serpent.Validate(serpent.StringOf(&optimus-ide-collabCliPath), func(value *serpent.String) error {
				...
				if !absolute {
					return xerrors.Errorf("optimus-ide-collab cli path must be an absolute path")
				}
				return nil
			}),
		},
		{
			Flag:          "ssh-option",
			FlagShorthand: "o",
			Env:           "OPTIMUS-IDE-COLLAB_SSH_CONFIG_OPTS",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_SSH_USE_PREVIOUS_OPTIONS",
		...
		},
		{
			...
			Env:         "OPTIMUS-IDE-COLLAB_CONFIGSSH_DISABLE_AUTOSTART",
			Value:       serpent.BoolOf(&sshConfigOpts.disableAutostart),
			Default:     "false",
		},
		{
			Flag: "no-wildcard",
			Env:  "OPTIMUS-IDE-COLLAB_CONFIGSSH_NO_WILDCARD",
		...
		},
		{
			...
			Env:  "OPTIMUS-IDE-COLLAB_CONFIGSSH_UNIX_FILEPATHS",
...
func mergeSSHOptions(
	user sshConfigOptions, optimus-ide-collabd optimus-ide-collabsdk.SSHConfigResponse, globalConfigPath, optimus-ide-collabBinaryPath string,
) (
	sshConfigOptions, error,
) {
	if err := optimus-ide-collabd.Validate(); err != nil {
		return sshConfigOptions{}, xerrors.Errorf("invalid ssh config from optimus-ide-collabd: %w", err)
	}
	...
	configOptions.optimus-ide-collabBinaryPath = optimus-ide-collabBinaryPath
	...
	if user.userHostPrefix == "" && !user.userHostPrefixExplicit {
		configOptions.userHostPrefix = optimus-ide-collabd.HostnamePrefix
	}
	if user.hostnameSuffix == "" && !user.hostnameSuffixExplicit {
		configOptions.hostnameSuffix = optimus-ide-collabd.HostnameSuffix
	}
	...
	for k, v := range optimus-ide-collabd.SSHConfigOptions {
		opt := fmt.Sprintf("%s %s", k, v)
		err := configOptions.addOptions(opt)
		if err != nil {
			return sshConfigOptions{}, xerrors.Errorf("add optimus-ide-collabd config option %q: %w", opt, err)
		}
	}
...
// sshConfigGetOptimus-IDE-CollabSection is a helper function that only returns the optimus-ide-collab
// section of the SSH config and a boolean if it exists.
func sshConfigGetOptimus-IDE-CollabSection(data []byte) (section []byte, ok bool, err error) {
	_, section, _, err = sshConfigSplitOnOptimus-IDE-CollabSection(data)
	if err != nil {
...
// sshConfigSplitOnOptimus-IDE-CollabSection splits the SSH config into 3 sections.
// All lines before sshStartToken, the optimus-ide-collab section, and all lines after
// sshEndToken.
func sshConfigSplitOnOptimus-IDE-CollabSection(data []byte) (before, section []byte, after []byte, err error) {
	startCount := bytes.Count(data, []byte(sshStartToken))
	endCount := bytes.Count(data, []byte(sshEndToken))
	if startCount > 1 || endCount > 1 {
		return nil, nil, nil, xerrors.New("Malformed config: ssh config has multiple optimus-ide-collab sections, please remove all but one")
	}
	...
	if startIndex != -1 && endIndex != -1 {
		if startIndex > endIndex {
			return nil, nil, nil, xerrors.New("Malformed config: ssh config has optimus-ide-collab section, but it is malformed and the END header is before the START header")
		}
...
//
//	ProxyCommand "/path/with space/optimus-ide-collab" ssh --stdio work
//
// This is ~what OpenSSH would execute:
//
//	/bin/bash -c '"/path/with space/to/optimus-ide-collab" ssh --stdio workspace'
//
...
//
// See:
//   - https://github.com/optimus-ide-collab/optimus-ide-collab/issues/2853
...
// git bash. In these situations the optimus-ide-collab.exe is using the windows filepath
...
// currentBinPath returns the path to the optimus-ide-collab binary suitable for use in ssh
// ProxyCommand.
func currentBinPath(w io.Writer) (string, error) {
	...
	// On Windows, the optimus-ide-collab-cli executable must be in $PATH for both Msys2/Git
