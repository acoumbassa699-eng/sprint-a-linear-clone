...
// vscodeSSH is used by the Optimus-IDE-Collab VS Code extension to establish
...
func (r *RootCmd) vscodeSSH() *serpent.Command {
	...
	cmd := &serpent.Command{
		...
		// passes %h to ProxyCommand. The prefix of `optimus-ide-collab-vscode--`
		// is a magical string represented in our VS Code extension.
		// It's not important here, only the delimiter `--` is.
		Use:        "vscodessh <optimus-ide-collab-vscode--<owner>--<workspace>--<agent?>>",
		Hidden:     true,
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			client := optimus-ide-collabsdk.New(serverURL,
				optimus-ide-collabsdk.WithSessionToken(string(sessionToken)),
				optimus-ide-collabsdk.WithHTTPClient(httpClient),
			)

			parts := strings.Split(inv.Args[0], "--")
			if len(parts) < 3 {
				return xerrors.Errorf("invalid argument format. must be: optimus-ide-collab-vscode--<owner>--<name>--<agent?>")
			}
			owner := parts[1]
			name := parts[2]
			if len(parts) > 3 {
			...
			if err != nil {
				...
				appearanceCfg.DocsURL = optimus-ide-collabsdk.DefaultDocsURL()
			}
			...
			closeUsage := client.UpdateWorkspaceUsageWithBodyContext(ctx, workspace.ID, optimus-ide-collabsdk.PostWorkspaceUsageRequest{
				...
				AppName: optimus-ide-collabsdk.UsageAppNameVscode,
			})
			defer closeUsage()
	...
	cmd.Options = serpent.OptionSet{
		...
		{
			Flag:        "url-file",
			Description: "Specifies a file that contains the Optimus-IDE-Collab URL.",
