...
import (
	...
	"github.com/optimus-ide-collab/optimus-ide-collab/v2/scaletest/chat"
...
func (r *RootCmd) scaletestChat() *serpent.Command {
	...
	cmd := &serpent.Command{
		Use:   "chat",
		Short: "Generate Optimus-IDE-Collab Agents load.",
		Handler: func(inv *serpent.Invocation) error {
			...
			if len(workspaces) == 0 {
				workspaces = append(workspaces, optimus-ide-collabsdk.Workspace{OrganizationID: me.OrganizationIDs[0]})
	...
	cmd.Options = serpent.OptionSet{
		...
		},
		{
			...
			Description: "Time to wait after creating or updating the mock LLM provider so every optimus-ide-collabd replica's cached provider config expires. The default exceeds the server-side cache TTL.",
